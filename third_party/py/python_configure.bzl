...
def _symlink_genrule_for_dir(repository_ctx, src_dir, dest_dir, genrule_name):
  ...
  if _is_windows(repository_ctx):
    ...
    # Create a single junction to the src_dir. (Junctions are symlinks to
    # directories.) We can't create symlinks to files on Windows, but by
    # declaring the same outputs as on Linux/Darwin and creating a directory
    # symlink, we can fool Bazel into believing that the genrule successfully
    # generated the expected outputs.
    # This trick only works in a single-machine environment.
