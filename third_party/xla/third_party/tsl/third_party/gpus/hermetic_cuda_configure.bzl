...
`hermetic_cuda_configure` depends on the following environment variables:
  ...
  * `HERMETIC_CUDA_COMPUTE_CAPABILITIES`: The CUDA compute capabilities. Default is
    `3.5,5.2`. If not specified, the value will be determined by the
...
def _compute_capabilities(repository_ctx):
    ...
    """
    capabilities = (get_host_environ(repository_ctx, _HERMETIC_CUDA_COMPUTE_CAPABILITIES) or
                    get_host_environ(repository_ctx, _TF_CUDA_COMPUTE_CAPABILITIES))
