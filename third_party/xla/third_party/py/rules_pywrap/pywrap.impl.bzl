...
def python_extension(
        ...
        **kwargs):
    """
    A macro responsible for creating each individual Python C++ extension. It consists of two parts:
    a cc_library compiling the extension and a custom rule which preserves enough information for
    pywrap_library to be able to do its job of linking multiple extensions together.
...
def pybind_extension(name, default_deps = None, **kwargs):
    """
    Wrapper around pybind_extension that specifies default dependency on pybind11. Note
    python_extension works with nanobind as well.
    """
