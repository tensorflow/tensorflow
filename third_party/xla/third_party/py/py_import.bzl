...
def _unpacked_wheel_impl(ctx):
    ...
    ctx.actions.run(
        ...
        inputs = srcs,
        outputs = [output_dir],
        executable = ctx.executable.create_py_import_output_tool,
        mnemonic = "CreatePyImportOutput",
    )
...
_unpacked_wheel = rule(
    ...
    attrs = {
        ...
        "create_py_import_output_tool": attr.label(
            default = Label("//third_party/py:create_py_import_output_tool"),
            executable = True,
            cfg = "exec",
        ),
