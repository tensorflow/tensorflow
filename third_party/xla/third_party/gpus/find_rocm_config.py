...
def _find_roctracer_config(rocm_install_path):
    def roctracer_version_numbers(path):
        ...
        if not version_file:
            raise ConfigError(
                "roctracer version file not found in {}".format(possible_version_files)
            )
...
def _find_hipsparse_config(rocm_install_path):
    def hipsparse_version_numbers(path):
        ...
        if not version_file:
            raise ConfigError(
                "hipsparse version file not found in {}".format(possible_version_files)
            )
...
def _find_hipsolver_config(rocm_install_path):
    def hipsolver_version_numbers(path):
        ...
        if not version_file:
            raise ConfigError(
                "hipsolver version file not found in {}".format(possible_version_files)
            )
    ...
    hipsolver_config = {
        "hipsolver_version_number": _get_composite_version_number(major, minor, patch)
    }
...
def _find_rocsolver_config(rocm_install_path):
    def rocsolver_version_numbers(path):
        ...
        if not version_file:
            raise ConfigError(
                "rocsolver version file not found in {}".format(possible_version_files)
            )
