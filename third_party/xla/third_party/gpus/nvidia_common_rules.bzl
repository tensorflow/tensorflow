...
def _redist_json_impl(repository_ctx):
    ...
    supported_redist_versions = repository_ctx.attr.redist_json_dict.keys()
    if (redist_version and not local_redist_path and
        (redist_version not in supported_reidist_versions)):
        fail(
    ...
    if redist_version and not local_redist_path:
        if redist_version in repository_ctx.attr.mirrored_tars_redist_json_dict.keys():
            mirrored_tars_url_to_sha256 = repository_ctx.attr.mirrored_tars_redist_json_dict[redist_version]
        else:
            mirrored_tars_url_to_sha256 = {}
        redistributions = _get_json_file_content(
            repository_ctx,
            url_to_sha256 = repository_ctx.attr.redist_json_dict[redist_version],
...
def _redist_repo_impl(repository_ctx):
    ...
    if local_redist_path:
        _use_local_redist_path(repository_ctx, local_redist_path, repository_ctx.attr.local_source_dirs)
    else:
