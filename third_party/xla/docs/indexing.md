...
- *index* of the element of the tensor `A` we are mapping from; values are known
  statically. Index elements are also called *dimension variables*.

- *range* variables. They are defined by the nature of operation; values are
  known statically. For example the contracting dimension of a matrix
  multiplication is a range variable.
...
*Dimensions* correspond to the dimension (index) variables *d*; *symbols*
correspond to the range variables *r* and runtime variables *rt*. `AffineMap`
does not contain any metadata about constraints of the parameters, so we have to
...
`range_vars_` encode possible values that range variables **s** can take.
...
out = (f32[10], s32[4]) reduce(p0, p1, p0_init, p1_init),
