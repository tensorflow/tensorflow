...
GLOBAL_BAZEL_CONFIG_PATTERNS: Tuple[str, ...] = (
    ...
    r"\.bzl$",
    # D5: @tsl is vendored as a symlink to third_party/tsl
    r"^third_party/tsl/",
)
...
def touches_build_keywords(
  ...
  build_or_bzl_files = [
      f
      for f in changed_files
      if f.endswith((".bzl", "BUILD", "BUILD.bazel"))
  ]
  if not build_or_bzl_files:
    return False

  try:
    cmd = ["git", "diff", base_sha, head_sha, "--", *build_or_bzl_files]
    res = subprocess.run(
...
def download_and_verify_jar(
    dest_path: str = "/tmp/bazel-diff.jar",
...
def report_decision(
    decision: BazelDiffDecision, mode: str, build_name: str
) -> None:
  """Emits decision summary to stdout and appends to $GITHUB_STEP_SUMMARY."""
  summary_line = (
      f"bazel-diff [{mode.upper()}] Build='{build_name}': "
...
def compute_impacted_targets(
    ...
    head_sha: str = "HEAD",
    workspace_dir: str = ".",
    dest_jar_path: str = "/tmp/bazel-diff.jar",
) -> BazelDiffDecision:
  ...
  try:
    changed_files = get_changed_files(base_sha, head_sha, cwd=workspace_dir)
  except Exception as e:  # pylint: disable=broad-exception-caught
