...
def touches_build_keywords(
  ...
  try:
    diff_base = get_diff_base(base_sha, head_sha, cwd=cwd)
    cmd = ["git", "diff", "-U0", diff_base, head_sha, "--", *build_or_bzl_files]
    res = subprocess.run(
...
def target_allowed_by_patterns(target: str, patterns: Sequence[str]) -> bool:
  ...
  if not patterns:
    return True
  pos_patterns = [p for p in patterns if not p.startswith("-")]
  neg_patterns = [p[1:] for p in patterns if p.startswith("-")]
  if pos_patterns and not any(
      target_matches_pattern(target, p) for p in pos_patterns
  ):
    return False
  if any(target_matches_pattern(target, p) for p in neg_patterns):
    return False
  return True
...
def get_cquery_command_options(build: Any) -> List[str]:
  """Extracts --config, --repo_env, and Starlark build flags from a Build."""
  opts = [f"--config={c}" for c in getattr(build, "configs", ())]
  for k, v in getattr(build, "repo_env", {}).items():
    opts.append(f"--repo_env={k}={v}")
  for k, v in getattr(build, "options", {}).items():
    if k.startswith(("//", "@")):
      # pylint: disable=g-bool-id-comparison
      opts.append(f"--{k}" if v is True else f"--{k}={v}")
  return opts
...
def compute_impacted_targets(
  ...
  cquery_opts = get_cquery_command_options(build)
  use_cquery = getattr(
  ...
  try:
    ...
    target_patterns = getattr(build, "target_patterns", ())
