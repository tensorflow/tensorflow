...
def check_file(path):
  ...
  Args:
    path: The path to the header file to check.

  Returns:
    A tuple (is_valid, error_message).
    is_valid: True if the file has a valid header guard, False otherwise.
    error_message: A string describing the error if is_valid is False.
  """
  with open(path, "r", encoding="utf-8") as f:
    content = f.read()
  # Check for #ifndef and #define at the top of the file.
  ifndef_match = re.search(r"#ifndef\s+([A-Z0-9_]+_H_)\b", content)
  define_match = re.search(r"#define\s+([A-Z0-9_]+_H_)\b", content)

  if not ifndef_match or not define_match:
    return False, "Missing #ifndef or #define guard"
  ...
  if not re.search(r"#endif\s+//\s*" + re.escape(guard_name) + r"\b", content):
