...
def load_results_data(results_json_file):
    ...
    try:
        ...
        print(f"::error file={results_json_file}::Failed to parse results JSON: {e}")
        sys.exit(1)
...
def load_baseline_data(baseline_yaml_file):
    ...
    try:
        ...
        print(f"::error file={baseline_yaml_file}::Failed to parse baseline YAML: {e}")
        sys.exit(1)
    except OSError as e:  # Catch other potential file reading errors
        print(f"::error file={baseline_yaml_file}::Error reading baseline YAML: {e}")
        sys.exit(1)
...
def compare_metrics(results_data, config_baselines, results_json_file, config_id):
    ...
    if not actual_metrics_container or not isinstance(actual_metrics_container, dict):
    ...
    for metric_name, baseline_info in config_baselines.items():
        ...
        if not actual_unit:
            summary_messages.append(
                f"Metric '{metric_name}': Actual value unit is not specified. Skipping."
            )
            continue

        summary_messages.append(f"\nComparing metric: {metric_name}")
        summary_messages.append(f"  Actual Value: {actual_value:.3f} {actual_unit}")
        summary_messages.append(f"  Baseline Value: {baseline_value:.3f} {actual_unit}")
        summary_messages.append(
...
def main():
    parser = argparse.ArgumentParser(
        description=("Compare benchmark results to baselines and fail on regression."),
        formatter_class=argparse.RawTextHelpFormatter,
    )  # For better help text formatting
    parser.add_argument(
        "--results-json-file",
        required=True,
        help=("Path to the benchmark results JSON file (e.g., output/results.json)."),
    )
    parser.add_argument(
        "--baseline-yaml-file",
        required=True,
    ...
    if not os.path.exists(args.results_json_file):
        print(f"::error file={args.results_json_file}::Results JSON file not found.")
        sys.exit(1)
