...
def load_results_data(results_json_file):
  ...
  try:
    ...
    print(
        f"::error file={results_json_file}::Failed to parse results"
        f" JSON: {e}"
    )
    sys.exit(1)
  except OSError as e:  # Catch other potential file reading errors
    print(
        f"::error file={results_json_file}::Error reading results"
        f" JSON: {e}"
    )
    sys.exit(1)
  return results_data
...
def load_baseline_data(baseline_yaml_file):
  ...
  try:
    ...
    print(
        f"::error file={baseline_yaml_file}::Failed to parse baseline"
        f" YAML: {e}"
    )
    sys.exit(1)
  except OSError as e:  # Catch other potential file reading errors
    print(
        f"::error file={baseline_yaml_file}::Error reading baseline"
        f" YAML: {e}"
    )
    sys.exit(1)
...
def compare_metrics(
  ...
  for metric_name, baseline_info in config_baselines.items():
    ...
    if (
        not actual_metric_entry
        or "value_ms" not in actual_metric_entry
    ):
      summary_messages.append(
    ...
    if actual_value_ms > allowed_upper_bound:
      ...
      if (
        ...
        percentage_diff = (
          ...
          actual_value_ms > 0
      ):  # If baseline is effectively zero, any positive value is inf worse.
