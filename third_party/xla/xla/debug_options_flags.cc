...
DebugOptions DefaultDebugOptionsIgnoringFlags() {
  ...
  opts.set_xla_gpu_annotate_with_emitter_loc(false);
...
void MakeDebugOptionsFlags(std::vector<tsl::Flag>* flag_list,
  ...
  flag_list->push_back(tsl::Flag(
      ...
      bool_setter_for(&DebugOptions::set_xla_gpu_annotate_with_emitter_loc),
      debug_options->xla_gpu_annotate_with_emitter_loc(),
      "Forces MLIR emitters to annotate all the created MLIR instructions with "
      "the emitter's C++ source file and line number. The annotations should "
      "appear in the MLIR dumps. The emitters should use EmitterLocOpBuilder "
      "for that."));
  flag_list->push_back(tsl::Flag(
