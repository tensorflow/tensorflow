...
void Store16LE(char* p, uint16_t v) {
...
void Store32LE(char* p, uint32_t v) {
  if constexpr (absl::endian::native != absl::endian::little) {
    v = absl::byteswap(v);
  }
  std::memcpy(p, &v, sizeof(v));
...
void Store64LE(char* p, uint64_t v) {
  if constexpr (absl::endian::native != absl::endian::little) {
    v = absl::byteswap(v);
  }
  std::memcpy(p, &v, sizeof(v));
...
std::string SerializeGraphDataCompressed(const GraphData& data) {
  ...
  Store32LE(node_count_buf, node_count);
  ...
  for (const auto& node : data.nodes) {
    ...
    Store32LE(buffer, absl::bit_cast<uint32_t>(x));
    binary_data.append(buffer, sizeof(x));
    Store32LE(buffer, absl::bit_cast<uint32_t>(y));
    binary_data.append(buffer, sizeof(y));
    Store32LE(buffer, absl::bit_cast<uint32_t>(diff_score));
    binary_data.append(buffer, sizeof(diff_score));
    Store64LE(buffer, anchor_id);
    binary_data.append(buffer, sizeof(anchor_id));
    Store16LE(buffer, key_len);
  ...
  Store32LE(edge_count_buf, edge_count);
  ...
  for (const auto& edge : data.edges) {
    ...
    Store64LE(buffer, supplier_id);
    binary_data.append(buffer, sizeof(supplier_id));
    Store64LE(buffer, consumer_id);
...
std::string GenerateBackgroundStyles(
  ...
  absl::btree_set<std::string> unique_colors;
  // NOLINTNEXTLINE
...
struct TokenAnnotationMapping {
  ...
  absl::btree_map<size_t, std::vector<const TensorAnnotation*>> span_starts;
  absl::btree_map<size_t, std::vector<const TensorAnnotation*>> span_ends;
...
TokenAnnotationMapping GetTokenAnnotationMapping(
  ...
  absl::btree_map<ShapeId, std::vector<size_t>> shape_tokens_indices;

  static const auto& kComputationAttributes =
...
std::string GenerateHloHtmlContent(
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    if (auto it = mapping.span_starts.find(i);
        it != mapping.span_starts.end()) {
    ...
    if (auto it = mapping.token_anchors.find(i);
        it != mapping.token_anchors.end()) {
      anchor_attr = absl::StrCat(" id=\"", it->second, "\"");
    }
...
absl::flat_hash_map<TensorKey, TensorAnnotation> PopulateMismatchAnnotations(
  ...
  for (const MismatchDetails& mismatch : mismatches) {
    ...
    ann.tooltip_data =
        nlohmann::json(absl::StrJoin(tooltip_parts, "<br/>")).dump();
