...
int RunDirectoryComparison(tsl::Env* env, absl::string_view golden_dir,
  ...
  absl::Status s =
      WriteModelComparisonOutputs(result, tsv_path, device_tsv_path);
  if (!s.ok()) {
  ...
  if (!device_tsv_path.empty()) {
    std::cout << "Wrote per-device TSV output to: " << device_tsv_path << "\n";
  }

  bool all_passed =
      (result.missing_in_golden.empty() && result.missing_in_test.empty() &&
       result.summary.differing_literals == 0 &&
       result.summary.shape_mismatch_literals == 0 &&
