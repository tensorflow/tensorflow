...
struct ModelSummaryStats {
  ...
  int64_t shape_mismatch_literals = 0;
...
// Returns std::nullopt if the filename does not match expected literal filename
// pattern.
...
// Writes output files (TSV, device TSV) based on non-empty paths.
absl::Status WriteModelComparisonOutputs(
    const ModelComparisonResult& result, absl::string_view tsv_path,
