...
struct StaticSpec {
  ...
  static absl::StatusOr<StaticSpec> FromDotProfile(
    ...
    spec.n = n;
    spec.k = k;
    spec.dtype_lhs = PrimitiveType_Name(lhs.element_type());
    spec.dtype_rhs = PrimitiveType_Name(rhs.element_type());
    spec.dtype_out =
        PrimitiveType_Name(profile.instruction().shape().element_type());
    return spec;
  }
};
...
/*static*/ absl::StatusOr<GemmPerfTable> MatmulPerfTableGen::Compact(
  ...
  for (const auto& [device_info, profile_list] : profiles.entries()) {
    ...
    for (const HloInstructionProfile& profile : profile_list.entries()) {
      ...
      std::string dtype_key = absl::StrCat(spec.dtype_lhs, "x", spec.dtype_rhs,
                                           "->", spec.dtype_out);
