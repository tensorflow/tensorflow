...
absl::Status DumpHloModule(HloModule* module, const std::string& file_name,
  ...
  if (output_format == "hlo") {
    tsl::Env* env = tsl::Env::Default();
    ABSL_RETURN_IF_ERROR(CreateDirIfNeeded(std::string(dir_path), env));
...
absl::StatusOr<bool> ScriptChecker::Run(const HloModule& module) {
  ...
  ABSL_RETURN_IF_ERROR(tsl::WriteStringToFile(env, hlo_path, hlo_contents));
...
absl::StatusOr<std::unique_ptr<HloModule>> BisectRunner::RunEntry() {
  ...
  ABSL_ASSIGN_OR_RETURN(bool has_bug, hlo_bisect.ShouldProcess());
  if (!has_bug) {
  ...
  ABSL_RETURN_IF_ERROR(hlo_bisect.TrimEntryComputation().status());
