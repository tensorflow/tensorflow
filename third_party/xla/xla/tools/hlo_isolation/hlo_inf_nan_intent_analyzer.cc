...
bool IsInfNanPropagatingOperand(const HloInstruction* user,
  ...
  switch (user->opcode()) {
    ...
    case HloOpcode::kScatter:
      ...
      if (user->operand(1) == operand) {
        return false;
      }
      break;
    case HloOpcode::kReduce: {
      // Operands [0, input_count - 1] are data inputs to reduce, and operands
      // [input_count, 2 * input_count - 1] are init values.
      int64_t input_count = user->operand_count() / 2;
...
using internal::IsInfNanPropagatingOperand;
using internal::IsStructuralOperand;
      ...
      if (user->opcode() == HloOpcode::kReduce) {
        ...
        for (int64_t i = 0; i < input_count; ++i) {
          if (user->operand(i) == instr &&
              IsConstantInfOrNan(user->operand(i + input_count))) {
            return true;
          }
        }
      }
