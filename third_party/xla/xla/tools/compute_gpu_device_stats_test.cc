...
#include "third_party/tensorflow/core/profiler/protobuf/op_metrics.pb.h"
...
TEST_F(ComputeGpuDeviceStatsTest, CalculateDeviceTimeAndMemcpyTest) {
  ...
  tensorflow::profiler::OpStats op_stats = stats.op_stats;
  const tensorflow::profiler::PerfEnv& perf_env = op_stats.perf_env();

  EXPECT_NEAR(125.34, perf_env.peak_tera_flops_per_second(), kMaxError);
  EXPECT_NEAR(900,
              perf_env.peak_bws_giga_bytes_per_second(
                  tensorflow::profiler::MemBwType::MEM_BW_TYPE_HBM_RW),
              kMaxError);
