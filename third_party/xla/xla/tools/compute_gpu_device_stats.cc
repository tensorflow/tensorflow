...
#include "third_party/tensorflow/core/profiler/convert/xplane_to_op_stats.h"
...
absl::StatusOr<tensorflow::profiler::OpStats> ConvertToOpStats(
    const tensorflow::profiler::XSpace& xspace) {
  tensorflow::profiler::OpStatsOptions options;
  options.generate_op_metrics_db = true;
  options.generate_step_db = true;
  options.generate_kernel_stats_db = true;
  return ConvertXSpaceToOpStats(xspace, options);
}
...
absl::StatusOr<GpuDeviceStats> ComputeGPUDeviceStats(
  ...
  TF_ASSIGN_OR_RETURN(result.op_stats, ConvertToOpStats(xspace));
...
absl::Status Run(absl::string_view input_file) {
  ...
  // device memory usage. OpStats proto is also calculated and printed.
  ...
  std::cout << "OpStats proto: " << stats->op_stats.DebugString() << "\n";
