...
class ChecksumMismatchReport:
    ...
    mismatches: dict[ThunkId, dict[BufferChecksums, dict[BufferIdx, set[Checksum]]]]

    @classmethod
    def from_protos(
        cls,
        log_protos: dict[ModuleExecutionId, buffer_debug_log_pb2.BufferDebugLogProto],
...
def group_by(values: Iterable[T], key_getter: Callable[[T], K]) -> dict[K, list[T]]:
...
def _find_inconsistent_output_checksums(
    ...
    """
    checksums_by_buffer_idx: dict[BufferIdx, set[Checksum]] = collections.defaultdict(
        set
    )
...
def _find_inconsistent_thunks(
    ...
    for thunk_id, executions in executions_by_thunk_id.items():
        executions_by_inputs: dict[BufferChecksums, list[ThunkExecution]] = group_by(
            executions, lambda e: e.input_checksums
        )

        mismatches_by_inputs: dict[BufferChecksums, dict[BufferIdx, set[Checksum]]] = {}
