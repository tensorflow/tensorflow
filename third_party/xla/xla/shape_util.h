...
class ShapeUtil {
 public:
  ...
  // Sets element_size_in_bits on each subshape's layout. If
  // 'pack_subbyte_types' is true, sets the element size to the dtype bitwidth
  // for subbyte types (S4, U4) and 0 for non-subbyte types, which indicates
  // that for arrays of subbyte types, multiple elements are packed in a single
  // byte. If 'pack_subbyte_types' is false, sets the element size to 0 for all
  // types.
  static Shape SetElementSizeInBits(Shape s, bool pack_subbyte_types);
