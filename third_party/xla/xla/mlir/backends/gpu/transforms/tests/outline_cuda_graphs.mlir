...
func.func @func(%arg0: memref<?xf32>, %arg1: memref<?xf32>) {
  ...
  // CHECK: call @local_xla.gpu.graph.launch(%[[ARG0]], %[[ARG1]])
  // CHECK-SAME: {capture = @local_xla.gpu.graph.capture}
  // CHECK-NEXT: return
...
// CHECK: func private @local_xla.gpu.graph.launch(memref<?xf32>, memref<?xf32>)
...
func.func @func(%arg0: memref<?xf32>) {
  ...
  // CHECK-NOT: call @local_xla.gpu.graph.launch
  gpu.launch_func  @gpu_module::@fn0
    blocks in (%c1, %c1, %c1)
    threads in (%c1, %c1, %c1)
    args(%arg0 : memref<?xf32>)
...
func.func @func(%arg0: memref<?xf32>) {
  ...
  // CHECK: call @local_xla.gpu.graph.launch(%[[ARG0]])
...
module attributes {gpu.container_module} {
  ...
  func.func @func(%raw_arg0: memref<16xi8> {lmhlo.params = 0 : index},
    ...
    // CHECK: call @local_xla.gpu.graph.launch(%[[ARG0]], %[[ARG1]], %[[ARG2]])
    // CHECK-SAME: {capture = @local_xla.gpu.graph.capture}
...
module attributes {gpu.container_module} {
  ...
  func.func @func(%input: memref<8x5x5x1xf32, #map1>,
    ...
    // CHECK-NOT: call @local_xla.g.cuda.graph.launch
...
module attributes {gpu.container_module} {
  ...
  func.func @func(%input: memref<1x4x4x1024xf16, #map1>,
    ...
    // CHECK: call @local_xla.gpu.graph.launch(%[[ARG0]], %[[ARG1]], %[[ARG2]], %[[ARG3]], %[[ARG4]])
    // CHECK-SAME: {capture = @local_xla.gpu.graph.capture}
...
func.func @func(%arg0: memref<16xi8>, %cond: memref<i1>) {
  ...
  "lmhlo.while"(%cond) ({
    // CHECK: func.call @local_xla.gpu.graph.launch(%[[ARG0]])
    // CHECK-SAME: {capture = @local_xla.gpu.graph.capture}
    gpu.launch_func  @gpu_module::@fn0 blocks in (%c1, %c1, %c1)
      threads in (%c1, %c1, %c1) args(%arg0: memref<16xi8>)
    gpu.launch_func  @gpu_module::@fn1 blocks in (%c1, %c1, %c1)
    ...
    "lmhlo.terminator"() : () -> () }, {
    // CHECK: func.call @local_xla.gpu.graph.launch(%[[ARG0]])
    // CHECK-SAME: {capture = @local_xla.gpu.graph.capture_0}
    gpu.launch_func  @gpu_module::@fn0 blocks in (%c1, %c1, %c1)
      threads in (%c1, %c1, %c1) args(%arg0: memref<16xi8>)
    gpu.launch_func  @gpu_module::@fn1 blocks in (%c1, %c1, %c1)
      threads in (%c1, %c1, %c1) args(%arg0: memref<16xi8>)
...
// CHECK: func @local_xla.gpu.graph.capture_0
// CHECK-NEXT: arith.constant 1
// CHECK-NEXT: gpu.launch_func @gpu_module::@fn0
// CHECK-NEXT: gpu.launch_func @gpu_module::@fn1
// CHECK-NEXT: return
