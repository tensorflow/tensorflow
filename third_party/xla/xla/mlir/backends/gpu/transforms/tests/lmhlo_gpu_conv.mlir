...
func.func @conv_forward(%input: memref<1x4x4x1024xf16, #map1>,
  ...
  // CHECK: call @local_xla.gpu.conv.forward(
...
// CHECK: func private @local_xla.gpu.conv.forward(
...
func.func @conv_backwardfilter(%input: memref<1x3x3x5xf16, #map0>,
                               ...
                               %scratch: memref<0xui8>) {
  // CHECK: call @local_xla.gpu.conv.backward.filter(
...
// CHECK: func private @local_xla.gpu.conv.backward.filter(
...
func.func @conv_backwardinput(%d_output: memref<4x5x16x16xf64>,
                              ...
                              %scratch: memref<0xui8>) {
  // CHECK: call @local_xla.gpu.conv.backward.input(
...
// CHECK: func private @local_xla.gpu.conv.backward.input(
...
func.func @conv_forward_fused(%input: memref<8x5x5x1xf32, #map1>,
                              ...
                              %scratch: memref<0xui8>) {
  // CHECK: call @local_xla.gpu.conv.forward.fused(
...
// CHECK: func private @local_xla.gpu.conv.forward.fused(
...
func.func @conv_forward_fused_with_side_input(
  ...
  // CHECK: call @local_xla.gpu.conv.forward.fused.side_input(
...
// CHECK: func private @local_xla.gpu.conv.forward.fused.side_input(
...
func.func @conv_reorder_filter(
  ...
  // CHECK: call @local_xla.gpu.conv.reorder.filter(
...
// CHECK: func private @local_xla.gpu.conv.reorder.filter(
...
func.func @conv_reorder_filter_and_bias(
  ...
  // CHECK: call @local_xla.gpu.conv.reorder.filter_and_bias(
...
// CHECK: func private @local_xla.gpu.conv.reorder.filter_and_bias(
// CHECK-SAME:   memref<1x1x3x3x32xi8, #map{{[0-9]*}}>,
