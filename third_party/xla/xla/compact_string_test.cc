...
TEST(CompactString, Simple) {
  ...
  s2 = s2;  // Test self-assignment
  EXPECT_EQ(s2.view(), s.view());

  s2 = s2.view();  // Test self-assignment
  EXPECT_EQ(s2.view(), s.view());
}
...
TEST(CompactString, StringConversions) {
  ...
  // Just const char* so relies on \0 termination
  std::string str2(s.view().data());
  // Check that it is null terminated as we expect
  EXPECT_EQ(s.view().data()[s.view().size()], '\0');
  EXPECT_EQ(str2, "stringmore");
}
