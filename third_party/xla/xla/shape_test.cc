...
TEST_F(ShapeTest, IsBuffer) {
  ...
  Shape buffer_shape_array = ShapeUtil::MakeShapeForBuffer(F32, {4, 6}, 120);
  EXPECT_TRUE(buffer_shape_array.is_buffer());
  EXPECT_EQ(buffer_shape_array.buffer_id(), 120);
  EXPECT_EQ(buffer_shape_array.ToString(true), "f32[4,6]{buffer_id=120}{1,0}");

  Shape buffer_shape_scalar = ShapeUtil::MakeShapeForBuffer(S32, {}, 10);
  EXPECT_TRUE(buffer_shape_scalar.is_buffer());
  EXPECT_EQ(buffer_shape_scalar.buffer_id(), 10);
  EXPECT_EQ(buffer_shape_scalar.ToString(true), "s32[]{buffer_id=10}");
}
