...
XLA_TEST_P(CollectivePipelineParallelismTest,
  ...
  while_body {
    ...
    prev_iteration_compute_res = f32[16] get-tuple-element(tuple), index=4
    i = u32[] get-tuple-element(tuple), index=5
    prev_iter_fwd_recv_done = (f32[16], token[]) get-tuple-element(tuple), index=6
    prev_iter_bwd_recv_done = (f32[16], token[]) get-tuple-element(tuple), index=7
    prev_stage_slice_fwd = f32[16] get-tuple-element(prev_iter_fwd_recv_done), index=0
    prev_stage_slice_bwd = f32[16] get-tuple-element(prev_iter_bwd_recv_done), index=0
    ...
    fwd_send = (f32[16], u32[], token[]) send(next_stage_slice, after_all_fwd), frontend_attributes={
    ...
    fwd_recv = (f32[16], u32[], token[]) recv(after_all_fwd), frontend_attributes={
        _xla_send_recv_source_target_pairs={{0,1},{1,2},{2,3}}}
    fwd_recv_done = (f32[16], token[]) recv-done(fwd_recv), frontend_attributes={
        _xla_send_recv_source_target_pairs={{0,1},{1,2},{2,3}}}, control-predecessors={fwd_send}

    after_all_bwd = token[] after-all()
    bwd_send = (f32[16], u32[], token[]) send(next_stage_slice, after_all_bwd), frontend_attributes={
        _xla_send_recv_source_target_pairs={{3,0}}}
    bwd_recv = (f32[16], u32[], token[]) recv(after_all_bwd), frontend_attributes={
        _xla_send_recv_source_target_pairs={{3,0}}}
    bwd_recv_done = (f32[16], token[]) recv-done(bwd_recv), frontend_attributes={
        _xla_send_recv_source_target_pairs={{3,0}}}, control-predecessors={bwd_send}
  ...
  ENTRY main {
    ...
    bwd_recv_done = (f32[16], token[]) recv-done(bwd_recv), frontend_attributes={
        _xla_send_recv_source_target_pairs={{3,0}}}
    bwd_send = (f32[16], u32[], token[]) send(input_slice, after_all_bwd), frontend_attributes={
        _xla_send_recv_source_target_pairs={{3,0}}}
    bwd_send_done = token[] send-done(bwd_send)


    // Iterate through pipeline stages.
    tuple = (f32[16,16], f32[5,16], f32[5,16], f32[5,16], f32[16], u32[], (f32[16], token[]), (f32[16], token[]))
        tuple(weights, input, output, buffer, prev_iteration_compute_res, c0, fwd_recv_done, bwd_recv_done)
    tuple_ = (f32[16,16], f32[5,16], f32[5,16], f32[5,16], f32[16], u32[], (f32[16], token[]), (f32[16], token[]))
        while(tuple), condition=while_condition, body=while_body
