...
TEST_F(CollectiveOpsE2ETestBase, ConstructCollectiveMetadata) {
  ...
  const HloInstruction* root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root,
              GmockMatch(m::Op().WithCustomCallTarget({"CollectiveMetadata"})));
...
TEST_F(CollectiveOpsE2ETestBase, ConstructCollectiveMetadataWithReplicaGroup) {
  ...
  ENTRY test_computation {
    ...
    result_tuple = (f32[4], f32[4]{0:S(1)}, u64[7]) custom-call(param_0, copy_1), custom_call_target="CollectiveMetadata", output_to_operand_aliasing={{0}: (0, {}), {1}: (1, {})}, backend_config="{\"collective_metadata_backend_config\":{\"replica_groups\": [{\"replica_ids\": [0,1]}, {\"replica_ids\": [2,3]}]}}"
