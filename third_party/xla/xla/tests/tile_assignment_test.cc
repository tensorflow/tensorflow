...
TEST(IotaTileAssignmentTest, TransposeCase1) {
  IotaTileAssignment tile =
      IotaTileAssignment::Create({32, 82, 1, 1, 128}, {1312, 32, 8}, {1, 0, 2});
  auto transposed = tile.Transpose({0, 2, 3, 4, 1});
  EXPECT_EQ(transposed.has_value(), true);
  EXPECT_EQ(transposed->ToString(), "[32,1,1,128,82]<=[82,16,32,8]T(2,1,3,0)");
}
