...
class HloTestBase : public NewHloTestBase {
 ...
 protected:
  ...
  // Shadowed on purpose in order to target runner_ instead of test_runner_.
  ...
  [[nodiscard]] absl::StatusOr<std::vector<Literal>>
  ...
  // Some test fixures rely on functionality that is not supported by other
  // HloRunnerInterface implementations, thus we expose it here.
  Backend& backend() { return test_runner_as_hlo_runner().backend(); }
  ...
  // DO NOT USE: This is a temporary method to help migrate to the
  // NewHloTestBase class.
  HloRunnerInterface& test_runner_;
  HloRunnerInterface& reference_runner_;
