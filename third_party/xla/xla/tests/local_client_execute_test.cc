...
TEST_F(LocalClientExecuteTest, DISABLED_ON_CPU(RunOnStreamForWrongPlatform)) {
...
TEST_F(LocalClientExecuteTest, DISABLED_ON_CPU(AllocatorDoesNotMatchPlatform)) {
  se::Platform* wrong_platform =
      se::PlatformManager::PlatformWithId(se::host::kHostPlatformId).value();
...
TEST_F(LocalClientExecuteTest,
       DISABLED_ON_CPU(DISABLED_ON_INTERPRETER(SizeOfGeneratedCodeInBytes))) {
  XlaBuilder builder(TestName());
  auto x = Parameter(&builder, 0, ShapeUtil::MakeShape(F32, {}), "x");
...
TEST_F(LocalClientExecuteTest, DISABLED_ON_INTERPRETER(InfeedTest)) {
  XlaBuilder builder(TestName());
...
TEST_F(LocalClientExecuteTest, DISABLED_ON_INTERPRETER(InfeedOutfeedTest)) {
  XlaBuilder builder(TestName());
  const Shape shape = ShapeUtil::MakeShape(F32, {3});
  auto in = Infeed(&builder, shape);
  auto constant = ConstantR1<float>(&builder, {1.0f, 2.0f, 3.0f});
...
TEST_F(LocalClientExecuteTest, ValidateMemoryFittingEffort) {
  XlaBuilder builder(TestName());
  auto x = Parameter(&builder, 0, ShapeUtil::MakeShape(F32, {3}), "x");
  auto y = ConstantR1<float>(&builder, {2.0f, 3.0f, 4.0f});
  Add(x, y);
