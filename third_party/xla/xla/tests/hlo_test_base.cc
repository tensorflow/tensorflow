...
// whether there exists a registered PjRtClientFactory and
// `use_pjrt_if_available` is true.
absl::StatusOr<std::unique_ptr<HloRunnerInterface>> GetHloRunnerForTest(
    se::Platform* test_platform, const bool use_pjrt_if_available = false) {
  if (use_pjrt_if_available && ShouldUsePjRt()) {
...
HloTestBase::GetHloRunner() {
  ...
  absl::StatusOr<std::unique_ptr<HloRunnerInterface>> status_or_runner =
      GetHloRunnerForTest(test_platform_, /*use_pjrt_if_available=*/true);
