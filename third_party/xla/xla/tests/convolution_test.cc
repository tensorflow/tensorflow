...
// custom convolution algorithm for this case, so we need to tests all cases
...
class Transposed2DConvHloTest
 ...
 public:
  int GetPaddingValueLow(int kernel_size) {
    switch (padding_mode_) {
      case PaddingMode::kFull:
        return 0;
      case PaddingMode::kHalf:
        return std::ceil((kernel_size - 1) / 2.0);
      case PaddingMode::kNo:
        return kernel_size - 1;
    }
  }

  int GetPaddingValueHigh(int kernel_size) {
    switch (padding_mode_) {
      case PaddingMode::kFull:
        return 0;
      case PaddingMode::kHalf:
        return std::floor((kernel_size - 1) / 2.0);
      case PaddingMode::kNo:
        return kernel_size - 1;
    }
  }
  ...
  std::string GetPaddingString(int kernel_x, int kernel_y) {
    return absl::StrCat(
        GetPaddingValueLow(kernel_x), "_", GetPaddingValueHigh(kernel_x), "x",
        GetPaddingValueLow(kernel_y), "_", GetPaddingValueHigh(kernel_y));
  }
  ...
  int GetOutputShape(int input_size, int kernel_size, int lhs_dilation) {
    return lhs_dilation * (input_size - 1) + kernel_size -
           (kernel_size - GetPaddingValueLow(kernel_size) - 1) -
           (kernel_size - GetPaddingValueHigh(kernel_size) - 1);
  }
