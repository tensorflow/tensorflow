...
absl::Status Consume(absl::string_view json, size_t& index, char c,
  ...
  TF_XLA_RETURN_IF_ERROR(CheckNotEndOfString(json, index, std::string(1, c)));
  if (json[index] == c) {
    ++index;
    SkipWhitespace(json, index);
  } else if (!optional) {
...
absl::StatusOr<std::unique_ptr<T>> ParseSequence(absl::string_view outer_json,
                                                 size_t& index,
                                                 ElemFn elem_fn) {
  TF_XLA_RETURN_IF_ERROR(Consume(outer_json, index, begin));
  TF_XLA_RETURN_IF_ERROR(CheckNotEndOfString(outer_json, index, name));

  auto seq = std::make_unique<T>();
  while (outer_json[index] != end) {
    TF_XLA_ASSIGN_OR_RETURN(auto elem, elem_fn(outer_json, index));
    seq->elements.emplace_back(std::move(elem));
    TF_XLA_RETURN_IF_ERROR(Consume(outer_json, index, ',', /*optional=*/true));
    TF_XLA_RETURN_IF_ERROR(CheckNotEndOfString(outer_json, index, name));
  }
  TF_XLA_RETURN_IF_ERROR(Consume(outer_json, index, end));
  return seq;
}
...
absl::StatusOr<absl::string_view> ParseLiteral(absl::string_view outer_json,
  ...
  TF_XLA_RETURN_IF_ERROR(CheckNotEndOfString(outer_json, index, "literal"));

  auto c = outer_json[index];
  TF_XLA_RETURN_IF_ERROR(EnsureValidLiteralStart(c));
...
absl::StatusOr<JsonValue> ParseValue(absl::string_view outer_json,
  ...
  JsonValue value;
  SkipWhitespace(outer_json, index);
  TF_XLA_RETURN_IF_ERROR(CheckNotEndOfString(outer_json, index, "value"));
  auto c = outer_json[index];
  if (c == '{') {
    constexpr static char kObject[] = "object";
    auto seq = ParseSequence<JsonObject, '{', '}', kObject>(outer_json, index,
                                                            ParseField);
    TF_XLA_ASSIGN_OR_RETURN(value, std::move(seq));
  } else if (c == '[') {
    ...
    TF_XLA_ASSIGN_OR_RETURN(value, ParseLiteral(outer_json, index));
  }
  return value;
}
...
absl::StatusOr<JsonField> ParseField(absl::string_view outer_json,
                                     size_t& index) {
  JsonField field;
  TF_XLA_ASSIGN_OR_RETURN(field.name, ParseLiteral(outer_json, index));
  TF_XLA_RETURN_IF_ERROR(Consume(outer_json, index, ':'));
  TF_XLA_ASSIGN_OR_RETURN(field.value, ParseValue(outer_json, index));
  return field;
}
...
absl::StatusOr<std::string> SortJson(absl::string_view json) {
  size_t index = 0;
  TF_XLA_ASSIGN_OR_RETURN(auto value, ParseValue(json, index));
  SkipWhitespace(json, index);
  if (index < json.size()) {
