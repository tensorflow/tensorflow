...
static std::string GetNameFromLocImpl(Location loc) {
  ...
  while (!locs.empty()) {
    ...
    if (auto name_loc = dyn_cast<NameLoc>(curr_loc)) {
    ...
    } else if (auto call_loc = dyn_cast<CallSiteLoc>(curr_loc)) {
      // Use location of the Callee to generate the name.
      locs.push_back(call_loc.getCallee());
    } else if (auto fused_loc = dyn_cast<FusedLoc>(curr_loc)) {
