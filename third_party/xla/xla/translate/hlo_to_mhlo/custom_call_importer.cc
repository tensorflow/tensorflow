...
absl::StatusOr<mlir::Operation*> ImportDynamicBroadcastInDimOp(
  ...
  auto attr = dyn_cast<mlir::DictionaryAttr>(
  ...
  auto broadcast_dimensions_attr =
      dyn_cast_or_null<mlir::ArrayAttr>(attr.get("broadcast_dimensions"));
  ...
  for (auto [i, broadcast_dimension] :
    ...
    broadcast_dimensions[i] =
        cast<mlir::IntegerAttr>(broadcast_dimension).getInt();
  }
...
mlir::Type getQuantizedType(mlir::DictionaryAttr& backend_config) {
  ...
  if (const mlir::Attribute scales_attr = backend_config.get("scale");
      scales_attr) {
    for (auto scale_attr : cast<mlir::ArrayAttr>(scales_attr)) {
      scales.push_back(cast<mlir::FloatAttr>(scale_attr).getValueAsDouble());
    }
  }

  auto zero_points_attr = backend_config.get("zero_point");
  if (zero_points_attr) {
    for (auto zero_point_attr : cast<mlir::ArrayAttr>(zero_points_attr)) {
      zero_points.push_back(cast<mlir::IntegerAttr>(zero_point_attr).getInt());
    }
  }
  ...
  if (quantization_dimension_attr) {
    quantization_dimension =
        cast<mlir::IntegerAttr>(quantization_dimension_attr).getInt();
  }

  auto storage_max_attr = backend_config.get("storage_max");
  if (storage_max_attr) {
    storage_max = cast<mlir::IntegerAttr>(storage_max_attr).getInt();
  }

  auto storage_min_attr = backend_config.get("storage_min");
  if (storage_min_attr) {
    storage_min = cast<mlir::IntegerAttr>(storage_min_attr).getInt();
  }

  auto storage_type_attr = backend_config.get("storage_type");
  if (storage_type_attr) {
    storage_type = cast<mlir::TypeAttr>(storage_type_attr).getValue();
  }

  auto expressed_type_attr = backend_config.get("expressed_type");
  if (expressed_type_attr) {
    expressed_type = cast<mlir::TypeAttr>(expressed_type_attr).getValue();
  }

  auto is_signed = cast<mlir::IntegerType>(storage_type).isSignless();
...
absl::StatusOr<mlir::Operation*> ImportCustomCallAsOp(
  ...
  auto backend_config = dyn_cast<mlir::DictionaryAttr>(
    ...
    return builder
        ->create<mlir::mhlo::UniformQuantizeOp>(
            ...
            mlir::RankedTensorType::get(
                cast<mlir::RankedTensorType>(result_type).getShape(),
