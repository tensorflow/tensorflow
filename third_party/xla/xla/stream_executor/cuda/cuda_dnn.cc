...
absl::Status CudnnSupport::Init() {
  ...
  if (status == CUDNN_STATUS_SUCCESS) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(SemanticVersion loaded_version,
    ...
    TF_XLA_RETURN_IF_ERROR(cudnn_->InitializeCompilationHandle());
...
absl::StatusOr<stream_executor::dnn::VersionInfo> CudnnSupport::GetVersion() {
  TF_XLA_ASSIGN_OR_RETURN(SemanticVersion version,
 ...
 public:
  ...
  static absl::StatusOr<CudnnDropoutDescriptor> Create(
    ...
    if (state_allocator) {
      ...
      TF_XLA_ASSIGN_OR_RETURN(state_memory,
...
class CudnnRnnDescriptor : public dnn::RnnDescriptor {
 ...
 public:
  ...
  static absl::StatusOr<CudnnRnnDescriptor> Create(
    ...
    TF_XLA_ASSIGN_OR_RETURN(auto params_desc,
...
absl::StatusOr<CudnnRnnParamsDescriptor> CudnnRnnParamsDescriptor::Create(
  ...
  for (int layer = 0; layer < layer_count; layer++) {
    for (int region = 0; region < region_count_per_layer; region++) {
      ...
      TF_XLA_ASSIGN_OR_RETURN(int64_t m_size, get_size(m_region_desc_handle));
      ...
      TF_XLA_ASSIGN_OR_RETURN(int64_t b_size, get_size(b_region_desc_handle));
    ...
    TF_XLA_RETURN_IF_ERROR(CheckAndFetchProjectionWeights(
...
absl::Status CreateRnnTempSpace(
  ...
  if (workspace_size_in_bytes > 0) {
    TF_XLA_ASSIGN_OR_RETURN(*workspace, workspace_allocator->AllocateBytes(
                                        workspace_size_in_bytes));
  }
  if (reserve_space_allocator != nullptr && is_fwd_training &&
      reserve_space_size_in_bytes > 0) {
    TF_XLA_ASSIGN_OR_RETURN(*reserve_space, reserve_space_allocator->AllocateBytes(
...
absl::Status CudnnSupport::DoRnnForwardImpl(
  ...
  TF_XLA_RETURN_IF_ERROR(CheckRNNParameterSize(cudnn, rnn_desc, input_desc));

  DeviceMemory<uint8_t> reserve_space;
  DeviceMemory<uint8_t> workspace;
  TF_XLA_RETURN_IF_ERROR(CreateRnnTempSpace(
...
absl::Status CudnnSupport::DoRnnBackwardImpl(
  ...
  DeviceMemory<uint8_t> workspace;
  TF_XLA_RETURN_IF_ERROR(CreateRnnTempSpace(stream, cudnn, rnn_desc, model_dims,
...
CudnnSupport::CreateRnnSequenceTensorDescriptor(int max_seq_length,
                                                int batch_size, int data_size,
                                                dnn::DataType data_type) {
  TF_XLA_ASSIGN_OR_RETURN(CudnnRnnSequenceTensorDescriptor descriptor,
...
GetCudnnOperationGraph(dnn::ConvolutionKind kind, dnn::DataType input_type,
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto tensor_x,
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto tensor_y,
...
GetGenericCudnnOperationGraph(
  ...
  auto deserialize_cudnn_graph = [&]() -> absl::StatusOr<OpGraph> {
    ...
    while (pos < serialized_graph.size()) {
      ...
      TF_XLA_ASSIGN_OR_RETURN(output_type,
      ...
      if (op_string == "conv") {
        ...
        TF_XLA_ASSIGN_OR_RETURN(std::tie(binary_operand_kind, output_kind, mode),
                            OpNameStringToOperandKindAndMode(op_string));
      }
      TF_XLA_RETURN_IF_ERROR(op_graph.AddOp(
          uid, operands, mode, binary_operand_kind, output_kind, output_type));
    }
    return op_graph;
  };

  TF_XLA_ASSIGN_OR_RETURN(OpGraph op_graph, deserialize_cudnn_graph());
  if (op_graph.Empty()) {
  ...
  // Result tensor.
  TF_XLA_ASSIGN_OR_RETURN(OpDescriptor op_descriptor, op_graph.OpDescriptorAt(0));
  std::tie(vector_size, vector_dim) =
  ...
  for (int op_index = 1; op_index < op_graph.Size(); ++op_index) {
    TF_XLA_ASSIGN_OR_RETURN(op_descriptor, op_graph.OpDescriptorAt(op_index));
    ...
    if (op_descriptor.result_kind == TensorKind::kScalar) {
      std::vector<int64_t> scale_dim(4, 1);
      TF_XLA_ASSIGN_OR_RETURN(cudnn_frontend::Tensor result,
                          CreateCudnnTensor(scale_dim, scale_dim,
                                            next_uid(/*is_operand=*/false,
                                                     /*is_virtual=*/false),
...
GetCudnnFusedOperationGraph(
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto tensor_z,
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto tensor_b, std::move(maybe_tensor_b));
...
GetCudnnFusedMatmulGraph(dnn::DataType input_type, dnn::DataType bias_type,
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto tensor_a,
...
absl::StatusOr<CudnnGraph> GetCudnnFlashAttentionOperationGraph(
  ...
  if (score_mod) {
    TF_XLA_RETURN_IF_ERROR(score_mod->UpdateCudnnMap(graph, next_uid));
  ...
  TF_XLA_RETURN_IF_ERROR(cudnnGraph.Prepare(
      dnn_support, EngineOptions{/*require_determinism=*/false,
                                 /*allow_tf32=*/true,
                                 /*require_command_buffer=*/false}));
  TF_XLA_RETURN_IF_ERROR(cudnnGraph.Build(dnn_support, /*plan_id=*/std::nullopt));
...
absl::StatusOr<CudnnGraph> GetCudnnBlockScaledDotOperationGraph(
  ...
  auto get_tensor_attr = [&](const dnn::TensorDescriptor& desc,
    ...
    TF_XLA_ASSIGN_OR_RETURN(std::vector<int64_t> dimensions,
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto a_data_attr, get_tensor_attr(lhs_data, false));
  TF_XLA_ASSIGN_OR_RETURN(auto b_data_attr, get_tensor_attr(rhs_data, true));
  TF_XLA_ASSIGN_OR_RETURN(auto a_scale_attr, get_tensor_attr(lhs_scale, false));
  TF_XLA_ASSIGN_OR_RETURN(auto b_scale_attr, get_tensor_attr(rhs_scale, true));
...
absl::StatusOr<std::vector<BackendDescriptor>> GetDescriptorAttribute(
  ...
  for (int i = 0; i < n; ++i) {
    TF_XLA_ASSIGN_OR_RETURN(result[i], CreateBackendDesc(type));
  }
...
class CudnnExecutionPlanRunner<void(Args...)>
 ...
 public:
  ...
  absl::Status operator()(Stream* stream, dnn::ProfileResult* profile_result,
    ...
    if (timer != nullptr) {
      TF_XLA_ASSIGN_OR_RETURN(auto desc, ToAlgorithmDesc());
      TF_XLA_RETURN_IF_ERROR(PopulateProfileFromTimer(
...
CudnnSupport::ConvolveRunnerFromDesc(
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto execution_plan,
...
CudnnSupport::FusedConvolveRunnerFromDesc(
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto op_graph,
                      GetCudnnFusedOperationGraph(
...
CudnnSupport::NormRunnerFromDesc(
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto x_tensor, create_cudnn_tensor(x_descriptor));
  TF_XLA_ASSIGN_OR_RETURN(auto scale_tensor, create_cudnn_tensor(scale_descriptor));
  TF_XLA_ASSIGN_OR_RETURN(auto y_or_dx_tensor,
  ...
  if (kind == dnn::NormKind::LAYER_FWD_INFER ||
    ...
    TF_XLA_ASSIGN_OR_RETURN(bias_tensor,
                        create_cudnn_tensor(bias_descriptor.value()));
  }

  if (kind == dnn::LAYER_FWD_TRAIN) {
    TF_XLA_ASSIGN_OR_RETURN(expectation_tensor,
                        create_cudnn_tensor(expectation_descriptor.value()));
    TF_XLA_ASSIGN_OR_RETURN(norm_factor_tensor,
                        create_cudnn_tensor(norm_factor_descriptor.value()));
  }

  if (kind == dnn::LAYER_BWD) {
    TF_XLA_ASSIGN_OR_RETURN(dy_tensor, create_cudnn_tensor(dy_descriptor.value()));
    TF_XLA_ASSIGN_OR_RETURN(expectation_tensor,
                        create_cudnn_tensor(expectation_descriptor.value()));
    TF_XLA_ASSIGN_OR_RETURN(norm_factor_tensor,
                        create_cudnn_tensor(norm_factor_descriptor.value()));
    TF_XLA_ASSIGN_OR_RETURN(dscale_tensor,
                        create_cudnn_tensor(dscale_descriptor.value()));
    TF_XLA_ASSIGN_OR_RETURN(dbias_tensor,
  ...
  if (is_training) {
    ...
    if (!batch_mean->is_null() && !batch_var->is_null()) {
      if (exponential_average_factor == 1.0) {
        TF_XLA_RETURN_IF_ERROR(stream->MemZero(batch_mean, batch_mean->size()));
        TF_XLA_RETURN_IF_ERROR(stream->MemZero(batch_var, batch_var->size()));
      }
    ...
    if (!called) {
      TF_XLA_RETURN_IF_ERROR(check_no_side_input_or_activation());
...
absl::Status CudnnGraph::Prepare(dnn::DnnSupport& dnn_support,
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto cudnn_handle,
