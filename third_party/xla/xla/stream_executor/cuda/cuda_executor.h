...
class CudaExecutor : public GpuExecutor {
 ...
 private:
  ...
  struct MemoryTracker {
    absl::Mutex mutex;
    absl::flat_hash_set<CUdeviceptr> allocated_memory ABSL_GUARDED_BY(mutex);

    // Adds a pointer to the set of allocated memory. Returns true if the memory
    // was not already tracked.
    bool Add(CUdeviceptr ptr);
    // Returns true if the memory is tracked.
    bool Contains(CUdeviceptr ptr);
