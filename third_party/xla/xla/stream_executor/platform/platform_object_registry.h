...
class PlatformObjectRegistry {
 public:
  ...
  absl::StatusOr<typename Trait::Type> FindObject(Platform::Id platform_id) {
    TF_ASSIGN_OR_RETURN(std::any obj, FindObject(typeid(Trait), platform_id));
    return std::any_cast<typename Trait::Type>(std::move(obj));
  }
 ...
 private:
  ...
  absl::StatusOr<std::any> FindObject(const std::type_info& type,
