...
class ExecutionUnitDescription {
 public:
  ...
  // Sets or overwrites the `RateInfo` for multiple `dtypes`.
  void SetRateInfo(absl::Span<const xla::PrimitiveType> dtypes,
                   RateInfo rate_info) {
    for (xla::PrimitiveType dtype : dtypes) {
      SetRateInfo(dtype, rate_info);
    }
  }
