...
absl::StatusOr<GemmConfig> GemmConfig::FromProto(
  ...
  return GemmConfig(
      std::move(lhs_layout), std::move(rhs_layout), std::move(c_layout),
      std::move(output_layout), {proto.alpha_real(), proto.alpha_imag()},
      proto.beta(), proto.compute_precision(), proto.precision_algorithm(),
      proto.has_algorithm() ? std::optional(proto.algorithm()) : std::nullopt,
      proto.grad_x(), proto.grad_y(), compute_type);
}
