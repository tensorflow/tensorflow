...
// searches for a section that starts with the name `section_name`.
...
static absl::StatusOr<std::string> GetSectionContentsFromStaticLibraryDump(
    const llvm::MemoryBuffer& buffer, absl::string_view section_name) {
  ...
  return absl::InternalError(absl::StrFormat(
      "The section with the name %s was not found in the given archive.",
      section_name));
}
...
absl::StatusOr<std::string> GetSectionContentsFromLibraryDump(
  ...
  if (binary.get()->isArchive()) {
    return GetSectionContentsFromStaticLibraryDump(*buffer, section_name);
  }
  if (binary.get()->isELF()) {
    return GetSectionContentsFromELFDump(**binary, section_name);
  }
  return absl::UnimplementedError(
      "Only ELF and static library archives are supported.");
}
}  // namespace stream_executor::gpu
