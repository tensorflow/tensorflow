...
bool operator==(const GroupedGemmConfig& rhs, const GroupedGemmConfig& lhs) {
  return detail::AsTuple(rhs) == detail::AsTuple(lhs);
}
    ...
    for (uint32_t j = 0; j < algorithms.size(); j++) {
      ...
      for (i = 0, total_ms = 0; i < n_total; i++) {
        ...
        if (!res.ok() || !profile.is_valid()) {
          VLOG(1) << j << ": gemm algorithm is not valid: " << res.message();
          break;
        }
...
absl::Status BlasLtGemmRunner::RunBatchedImpl(
  ...
  absl::FailedPreconditionError("Not implemented");
}

absl::Status BlasLtGemmRunner::RunStridedBatchedImpl(
    Stream& stream, blas::Transpose trans_a, blas::Transpose trans_b, int64_t m,
  ...
  TF_ASSIGN_OR_RETURN(auto xla_type_a, AsXlaPrimitiveType(type_a));
  TF_ASSIGN_OR_RETURN(auto xla_type_c, AsXlaPrimitiveType(type_c));
  TF_ASSIGN_OR_RETURN(auto xla_type_b, AsXlaPrimitiveType(type_b));

  auto res = strided_gemm_map_.find(scfg);
  if (res == strided_gemm_map_.end()) {
    TF_ASSIGN_OR_RETURN(
        auto compute_type,  // TODO:PrecisionConfig::ALG_UNSET
        gpu::GetBlasComputationType(xla::PrecisionConfig::ALG_UNSET, xla_type_a,
                                    xla_type_c, 0));
    ...
    GemmConfig cfg = {
        .lhs_layout = MatrixLayout(xla_type_a, row_a, col_a, order, batch_count,
                                   lda, stride_a, trans_a),

        .rhs_layout = MatrixLayout(xla_type_b, row_b, col_b, order, batch_count,
                                   ldb, stride_b, trans_b),

        .c_layout =
            MatrixLayout(xla_type_c, m, n, order, batch_count, ldc, stride_c),
        .output_layout =
            MatrixLayout(xla_type_c, m, n, order, batch_count, ldc, stride_c),
        .alpha = alpha,
        .beta = beta,
    ...
    TF_ASSIGN_OR_RETURN(auto plan_res,
                        gpu::BlasLt::GetMatmulPlan(
                            &stream, cfg, gpu::BlasLt::Epilogue::kDefault));
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        Autotune(algorithms, [&](const gpu::BlasLt::MatmulAlgorithm& algo,
          ...
          return res->second->ExecuteOnStream(
              &stream, a, b, *c, *c, DeviceMemoryBase{},  // bias
              ...
              gpu::BlasLt::MatmulAlgorithm{},             // algorithm
              *allocator,                                 // allocator
              profile);
        }));
