...
thread_local GpuCommandBufferListener* tls_listener = nullptr;
...
GpuCommandBufferListener* GetGpuCommandBufferListener() {
  if (tls_listener != nullptr) {
    return tls_listener;
  }
  return global_listener.load(std::memory_order_acquire);
}

GpuCommandBufferListener* GpuCommandBufferListener::ExchangeForTesting(
    GpuCommandBufferListener* listener) {
  GpuCommandBufferListener* old = tls_listener;
