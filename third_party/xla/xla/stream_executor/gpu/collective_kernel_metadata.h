...
//   ...
// ]
// Each parameter might also have a corresponding multicast pointer in the
// `param_to_multicast_ptrs` array. This pointer can be used with multimem
// oprations.
struct CollectiveKernelMetadata {
  uint64_t rank;
  uint64_t* param_to_peers;
  uint64_t* param_to_multicast_ptrs;
};
