...
absl::StatusOr<std::vector<uint8_t>> GetGpuTestKernelsFatbin() {
  ...
  for (const auto& section : executable_elf_object_file->sections()) {
    if (absl::StartsWith(section.getName().get().str(), ".nv_fatbin")) {
      const std::string nv_fatbin_contents = section.getContents().get().str();
      return std::vector<uint8_t>(nv_fatbin_contents.begin(),
                                  nv_fatbin_contents.end());
    } else if (absl::StartsWith(section.getName().get().str(), ".hip_fatbin")) {
      const std::string hip_fatbin_contents = section.getContents().get().str();
      return std::vector<uint8_t>(hip_fatbin_contents.begin(),
                                  hip_fatbin_contents.end());
    }
  }
