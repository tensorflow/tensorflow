...
// A temporary helper function to convert a list of GpuGraphNodeInfo pointers
// into a list of backend specific node handles. It will go away once all
// Node factory functions have been migrated into the subclasses.
static std::vector<GpuGraphNodeHandle> AsNodeHandles(
...
GpuCommandBuffer::Dependencies GpuCommandBuffer::GetBarrier(
  ...
  return execution_scope.barriers.empty()
             ? Dependencies{}
             : Dependencies{execution_scope.barriers.back().node};
}
...
absl::Status GpuCommandBuffer::DisableBarriersExecution(
  ...
  for (GpuGraphBarrierInfo& barrier : execution_scope.barriers) {
    if (barrier.is_barrier_node) {
      TF_RETURN_IF_ERROR(
          barrier.node->SetExecutionEnabled(root_command_buffer, false));
    }
  }
...
GpuCommandBuffer::Dependencies GpuCommandBuffer::GetBarrierDependencies(
  ...
  for (size_t i = barriers.empty() ? 0 : barriers.back().nodes_offset;
    ...
    dependencies.push_back(execution_scope.nodes[i]);
  }
  return dependencies;
}
  ...
  if (state_ == State::kCreate) {
    ...
    if (dependencies.size() == 1) {
      execution_scope.barriers.push_back(
          {execution_scope.nodes.back(), false, nodes_offset});
      return absl::OkStatus();
    }
...
absl::Status GpuCommandBuffer::Barrier(
  ...
  if (state_ == State::kCreate) {
    ...
    for (ExecutionScopeId execution_scope_id : execution_scope_ids) {
      ExecutionScope& execution_scope = execution_scopes_[execution_scope_id];
      dependencies.push_back(execution_scope.barriers.back().node);
    }
...
absl::Status GpuCommandBuffer::Barrier(ExecutionScopeId from_execution_scope_id,
  ...
  if (state_ == State::kCreate) {
    ...
    Dependencies dependencies = {
        execution_scopes_[from_execution_scope_id].barriers.back().node,
        execution_scopes_[to_execution_scope_id].barriers.back().node};
...
absl::Status GpuCommandBuffer::LaunchWithPackedArgs(
  ...
  if (state_ == State::kCreate) {
    ...
    execution_scope.nodes.push_back(node_info);
    return absl::OkStatus();
  }

  // Updates kernel node in the executable graph.
  if (state_ == State::kUpdate) {
    return execution_scope.nodes[execution_scope.update_state.node_idx++]
        ->UpdateKernelNode(threads, blocks, kernel, packed_args);
  }
...
absl::Status GpuCommandBuffer::AddNestedCommandBuffer(
  ...
  if (state_ == State::kCreate) {
    Dependencies barrier = GetBarrier(execution_scope_id);
    TF_ASSIGN_OR_RETURN(GpuGraphNodeInfo * node_info,
...
absl::Status GpuCommandBuffer::MemcpyDeviceToDevice(
  ...
  if (state_ == State::kUpdate) {
    return execution_scope.nodes[execution_scope.update_state.node_idx++]
        ->UpdateMemcpyD2DNode(*dst, src, size);
  }
...
absl::Status GpuCommandBuffer::Memset(ExecutionScopeId execution_scope_id,
  ...
  if (state_ == State::kUpdate) {
    GpuGraphNodeInfo* node_info =
        execution_scope.nodes[execution_scope.update_state.node_idx++];
    return node_info->UpdateMemsetNode(*dst, bit_pattern, num_elements);
  }
...
GpuCommandBuffer::ToConditionBuilder(Builder builder) {
  return [builder = std::move(builder)](CommandBuffer* cmd_buffer,
                                        GraphConditional* condition) {
    return builder(cmd_buffer);
  };
}

absl::StatusOr<std::vector<GraphConditional*>>
GpuCommandBuffer::CreateConditionalHandles(size_t num_handles) {
  std::vector<GraphConditional*> handles;
...
GpuCommandBuffer::CreateConditionalCommandBuffers(
    absl::Span<GraphConditional* const> handles,
...
GpuCommandBuffer::CreateConditionalNodes(
    ...
    absl::Span<GraphConditional* const> conditionals) {
  ExecutionScope& execution_scope = execution_scopes_[execution_scope_id];
  ...
  for (GraphConditional* conditional : conditionals) {
    std::vector<GpuGraphNodeHandle> barrier =
        AsNodeHandles(GetBarrier(execution_scope_id));
    node_storage_.push_back(std::make_unique<GpuGraphNodeInfo>());
    execution_scope.nodes.emplace_back(node_storage_.back().get());
    GpuGraphNodeInfo& node_info = *node_storage_.back();

    ConditionalParams params;
    params.type = type;
    params.handle = conditional->handle();
...
absl::Status GpuCommandBuffer::If(ExecutionScopeId execution_scope_id,
  ...
  auto set_cond_fn = [&](ExecutionScopeId id, GraphConditionals handles) {
    return CommandBuffer::Launch(*set_if_condition, id, ThreadDim(), BlockDim(),
                                 handles[0]->handle(), predicate);
  };
...
absl::Status GpuCommandBuffer::IfElse(ExecutionScopeId execution_scope_id,
  ...
  auto set_cond_fn = [&](ExecutionScopeId id, GraphConditionals handles) {
    return CommandBuffer::Launch(*set_if_else_condition, id, ThreadDim(),
                                 BlockDim(), handles[0]->handle(),
                                 handles[1]->handle(), predicate);
  };
...
absl::Status GpuCommandBuffer::Case(ExecutionScopeId execution_scope_id,
  ...
  while (batch_offset < branches.size()) {
    ...
    auto set_cond_fn = [&, batch_offset, enable_conditional_default](
                           ExecutionScopeId id,
                           GraphConditionals conditionals) {
      ...
      std::transform(
          conditionals.begin(), conditionals.end(), padded_handles.begin(),
          [](GraphConditional* conditional) { return conditional->handle(); });
...
absl::Status GpuCommandBuffer::For(ExecutionScopeId execution_scope_id,
  ...
  auto set_cond_fn = [&](ExecutionScopeId id, GraphConditionals handles) {
    return CommandBuffer::Launch(*set_for_condition, id, ThreadDim(),
                                 BlockDim(), handles[0]->handle(), loop_counter,
                                 num_iteration);
  };

  auto body = [&](CommandBuffer* body, GraphConditional* conditional) {
    ...
    return body->Launch(*set_for_condition, ThreadDim(), BlockDim(),
                        conditional->handle(), loop_counter, num_iteration);
  };
...
absl::Status GpuCommandBuffer::While(ExecutionScopeId execution_scope_id,
  ...
  auto set_cond_fn = [&](ExecutionScopeId id, GraphConditionals handles) {
    return CommandBuffer::Launch(*set_while_condition, id, ThreadDim(),
                                 BlockDim(), handles[0]->handle(), pred);
  };
...
absl::Span<const GpuCommandBuffer::GpuGraphNodeInfo* const>
GpuCommandBuffer::nodes(ExecutionScopeId id) const {
