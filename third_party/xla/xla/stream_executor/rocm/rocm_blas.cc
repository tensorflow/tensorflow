...
absl::Status ROCMBlas::DoBlasGemmWithAlgorithm(
  ...
  if (algorithm == blas::kDefaultAlgorithm && type_a == type_c) {
    TF_XLA_RETURN_IF_ERROR(DoBlasGemm(stream, transa, transb, m, n, k, type_a,
    ...
    TF_XLA_ASSIGN_OR_RETURN(auto roc_type_a, AsRocBlasType(type_a));
    TF_XLA_ASSIGN_OR_RETURN(auto roc_type_c, AsRocBlasType(type_c));
    TF_XLA_ASSIGN_OR_RETURN(auto roc_comp_type,
    ...
    TF_XLA_RETURN_IF_ERROR(DoBlasInternalImpl(
...
absl::Status ROCMBlas::DoBlasGemmStridedBatchedWithAlgorithm(
  ...
  // fall back to the default implementation
  if (algorithm == blas::kDefaultAlgorithm && type_a == type_c) {
    TF_XLA_RETURN_IF_ERROR(DoBlasGemmStridedBatched(
...
absl::Status ReorganizeMemory(Stream *stream,
  ...
  for (auto &x : mem_copy_ops) {
    if (x.src_count > 1 || x.count > 1) {
      ...
      TF_XLA_RETURN_IF_ERROR(stream->Memcpy(&target_mem, src_mem, x.size));
    }
    i++;
  }
  return absl::OkStatus();
...
absl::StatusOr<AllocateStridedResult<T>> AllocateStridedBuffer(
  ...
  TF_XLA_ASSIGN_OR_RETURN(DeviceMemory<uint8_t> batch_matrix_bytes,
  ...
  if (copy_data) {
    TF_XLA_RETURN_IF_ERROR(ReorganizeMemory(stream, &res.device_mem, raw_ptrs,
