...
absl::Status RocmCommandBuffer::LaunchSetIfElseConditionKernel(
    ExecutionScopeId execution_scope_id, GraphConditional* if_conditional,
    GraphConditional* else_conditional, DeviceMemory<bool> predicate) {
  return absl::UnimplementedError("Conditionals are not supported on ROCM.");
}
...
RocmCommandBuffer::CreateBarrierNode(const Dependencies& dependencies) {
  ...
  node_storage_.push_back(std::make_unique<HipGraphNode>(barrier_handle, this));
  return node_storage_.back().get();
}
...
absl::StatusOr<GraphConditional*> RocmCommandBuffer::CreateConditionalHandle() {
