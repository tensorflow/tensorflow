 ...
 public:
  ...
  DefaultSchedulerCore::CandidateResult operator()(
    ...
    if (sched_state_.config.enable_selective_resources) {
      int64_t num_hops_to_closest_selective_overlap_for_a =
          GetNumHopsToClosestSelectiveOverlap(sched_state_.ready_set, a.node);
      int64_t num_hops_to_closest_selective_overlap_for_b =
      ...
      if (sched_state_.selective_resource_releasers.empty() &&
          (num_hops_to_closest_selective_overlap_for_a <=
               sched_state_.config.max_hops_to_closest_selective_overlap ||
           num_hops_to_closest_selective_overlap_for_b <=
               sched_state_.config.max_hops_to_closest_selective_overlap)) {
        ...
        // scheduling a and schedule b instead. To break ties (in scenarios
        // where both a and b are valuable for selective overlap), hold of
        // scheduling a if the nearest overlap for a is closer than that of b's.
        // Same logic applies in reverse.
        if (auto value = DefaultSchedulerCore::ChooseBestCandidate(
                (a.node->GetValuableForSelectiveOverlap() &&
                 num_hops_to_closest_selective_overlap_for_a <=
                     sched_state_.config
                         .max_hops_to_closest_selective_overlap &&
