...
ENTRY entry {
  ...
  dot = f32[8,256]{1,0} dot(a, b), lhs_contracting_dims={1}, rhs_contracting_dims={0}, frontend_attributes={sdy.has_unreduced_axis="true"}
  ROOT copy = f32[8,256]{1,0} copy(dot), sharding={replicated}
})";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          PartitionComputation(hlo_string, /*num_devices=*/2));
  VLOG(1) << module->ToString();
  auto root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root, op::Copy(op::Dot()));
  auto* all_reduce = FindInstruction(module.get(), HloOpcode::kAllReduce);
  EXPECT_EQ(all_reduce, nullptr);
}
...
ENTRY entry {
  constant = s32[2,4]{1,0} constant({{1,1,1,1},{1,1,1,1}}),
  sharding={maximal device=0}
  a = s32[2,4]{1,0} parameter(0), sharding={devices=[1,2]0,1}
  add = s32[2,4]{1,0} add(constant, a), frontend_attributes={sdy.has_unreduced_axis="true"}
  ROOT %copy = s32[2,4]{1,0} copy(%add), sharding={replicated}
})";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          PartitionComputation(hlo_string, /*num_devices=*/2));
  VLOG(1) << module->ToString();
  auto root = module->entry_computation()->root_instruction();
  // Check that both operands of the root are not skipped due to frontend
  // attributes population.
  EXPECT_THAT(root, op::Copy(op::Add(op::AllReduce(), op::AllReduce())));
}
