...
ENTRY entry {
  ...
  ROOT copy = f32[8,256]{1,0} copy(dot), sharding={replicated}, frontend_attributes={sdy.has_unreduced_axis="true"}
})";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          PartitionComputation(hlo_string, /*num_devices=*/2));
  VLOG(1) << module->ToString();
...
ENTRY entry {
  ...
  ROOT %copy = s32[2,4]{1,0} copy(%add), sharding={replicated}, frontend_attributes={sdy.has_unreduced_axis="true"}
})";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          PartitionComputation(hlo_string, /*num_devices=*/2));
  VLOG(1) << module->ToString();
