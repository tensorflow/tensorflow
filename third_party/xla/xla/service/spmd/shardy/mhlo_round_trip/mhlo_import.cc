...
xla::HloSharding parseShardingFromString(const StringAttr& sharding) {
  ...
  CHECK_OK(hloSharding) << *shardingProto;
  return *hloSharding;
}
...
// these shardings.
SmallVector<MeshAxisAttr> findMeshAxes(ModuleOp moduleOp) {
...
// `globalMesh`.
TensorShardingAttr convertToNewSharding(const xla::HloSharding& hloSharding,
                                        MeshAttr globalMesh, StringRef meshName,
  ...
  if (hloSharding.IsReplicated() || hloSharding.IsManual() ||
    ...
    return hloSharding.IsUnknown() || openDims
               ? TensorShardingAttr::getFullyOpen(ctx, rank, meshName)
               : TensorShardingAttr::getFullyClosed(ctx, rank, meshName);
  }
  ...
  return TensorShardingAttr::get(ctx, StringAttr::get(ctx, meshName),
...
LogicalResult importShardings(FuncOp funcOp, MeshAttr mesh, StringRef meshName,
  ...
  for (auto [argNum, argType] : llvm::enumerate(funcOp.getArgumentTypes())) {
    if (auto oldSharding =
      ...
      funcOp.setArgAttr(
          ...
          convertToNewSharding(parseShardingFromString(oldSharding), mesh,
                               meshName,
    ...
    if (auto oldSharding =
      ...
      funcOp.setResultAttr(
          ...
          convertToNewSharding(
              parseShardingFromString(oldSharding), mesh, meshName,
    ...
    if (auto oldSharding = op->getAttrOfType<StringAttr>(kXlaShardingAttr)) {
      ...
      for (const auto& [resHloSharding, resType] :
        ...
        newShardings.push_back(convertToNewSharding(
            resHloSharding, mesh, meshName,
            mlir::cast<RankedTensorType>(resType).getRank()));
      }
      op->setAttr(kShardingAttr, TensorShardingPerValueAttr::get(
                                     mesh.getContext(), newShardings));
...
class ImportShardingsPass
 ...
 public:
  ...
  void runOnOperation() final {
    ...
    SmallVector<MeshAxisAttr> namedAxes = findMeshAxes(moduleOp);
    if (namedAxes.empty()) {
      // There are no shardings in the `moduleOp`.
      return;
    }
    MeshAttr mesh = MeshAttr::get(moduleOp.getContext(), namedAxes);
    const std::string meshName = "mesh";

    mlir::SymbolTableCollection symbolTableCollection;
    mlir::SymbolTable& symbolTable =
        symbolTableCollection.getSymbolTable(moduleOp);
    symbolTable.insert(mlir::OpBuilder::atBlockBegin(moduleOp.getBody())
                           .create<MeshOp>(moduleOp.getLoc(), meshName, mesh));
