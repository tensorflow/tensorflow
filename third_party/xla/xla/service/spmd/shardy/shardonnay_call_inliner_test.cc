...
TEST_F(ShardonnayCallInlinerTest, MhloToHloShmapBodyNotInlined) {
  const char* const hloString = R"(
    ...
    %shmap_body.4 (Arg_0.5: f32[1,8]) -> f32[1,8] {
    ...
    ENTRY %main.10 (Arg_0.1: f32[8,8]) -> f32[8,8] {
      ...
      %call.7 = f32[1,8]{1,0} call(f32[1,8]{1,0} %custom-call.3), to_apply=%shmap_body.4
  ...
  ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(hloString));
  ASSERT_OK_AND_ASSIGN(bool changed, ShardonnayCallInliner().Run(module.get()));
  ...
  EXPECT_EQ(call->to_apply()->name(), "shmap_body.4");
}
