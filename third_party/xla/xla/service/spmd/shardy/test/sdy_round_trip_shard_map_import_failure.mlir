...
func.func @manual_computation_missing_global_to_local_shape(%arg0: tensor<0x16xf32>) -> (tensor<0x16xf32>) {
  ...
  // expected-error @+2 {{'func.call' op expected at least one operand of @xla.sdy.manual_computation_body_0 to be produced by a xla.sdy.GlobalToLocalShape CustomCallOp}}
...
func.func @manual_computation_missing_local_to_global_shape(%arg0: tensor<0x16xf32>) -> (tensor<0x8xf32>) {
  ...
  // expected-error @+2 {{'func.call' op expected the first use of @xla.sdy.manual_computation_body_0 to be a xla.sdy.LocalToGlobalShape CustomCallOp}}
  // expected-error @+1 {{failed to rewrite func.call to manual computation}}
