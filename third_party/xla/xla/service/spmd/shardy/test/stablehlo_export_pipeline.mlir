...
// CHECK-V3-SAME:      %arg1: tensor<8x8xf32> {mhlo.sharding = "{mesh['axis_0'=16], [{}, {}]}"},
...
// CHECK-V3-SAME:      %arg1: tensor<8x8xf32> {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}, {}]}"},
...
// CHECK-V3-LABEL: func @mesh_empty_should_be_converted_to_replicated_sharding(%arg0: tensor<8x8xf32> {mhlo.sharding = "{mesh[], [{}, {}]}"}, %arg1: tensor<8x8xf32>)
func.func @mesh_empty_should_be_converted_to_replicated_sharding(%arg0: tensor<8x8xf32> {sdy.sharding = #sdy.sharding<@empty_mesh_0, [{}, {}]>}, %arg1: tensor<8x8xf32>) -> tensor<8x8xf32> {
    ...
    // CHECK-V3: %[[ADD_WITH_SHARDING:.*]] = stablehlo.add %[[ADD]], %[[ADD]] {mhlo.sharding = "{mesh[], [{}, {}]}"}
...
// CHECK-LABEL: func @call_in_manual_computation_partially_manual(
...
func.func @call_in_manual_computation_partially_manual(
...
// CHECK-LABEL: func @call_in_manual_computation_fully_manual(
...
func.func @call_in_manual_computation_fully_manual(
  ...
  // CHECK-V3-NEXT: %[[FULL_TO_SHARD:.*]] = stablehlo.custom_call @SPMDFullToShardShape(%[[COPY_0]]) {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"} : (tensor<32xi32>) -> tensor<1xi32>
  // CHECK-V3-NEXT: %[[CALL:.*]] = call @xla.sdy.inlinable_manual_computation_body_1(%[[FULL_TO_SHARD]]) {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"} : (tensor<1xi32>) -> tensor<1xi32>
  // CHECK-V3-NEXT: %[[COPY_1:.*]] = mhlo.copy %[[CALL]] {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"} : tensor<1xi32>
  ...
  %0 = sdy.manual_computation(%arg0) in_shardings=[<@mesh_2, [{"x", "y"}]>] out_shardings=[<@mesh_2, [{"x", "y"}]>] manual_axes={"x", "y"} (%arg1: tensor<1xi32>) {
    %1 = func.call @foo_0(%arg1) {sdy.sharding = #sdy.sharding_per_value<[<@mesh_2, [{}]>]>} : (tensor<1xi32>) -> tensor<1xi32>
...
func.func @callback_transform_to_tuple(%arg0: tensor<2xf64> {sdy.sharding = #sdy.sharding<@mesh_5, [{"i"}]>}) -> (tensor<2xf64> {sdy.sharding = #sdy.sharding<@mesh_5, [{"i"}]>}) {
  ...
  // CHECK-V3-NEXT: %[[GET_TUPLE:.*]] = stablehlo.get_tuple_element %[[CALLBACK]][0] {mhlo.sharding = "{mesh[], [{}]}"} : (tuple<tensor<2xf64>>) -> tensor<2xf64>
...
func.func @while_with_no_sharding_inside_manual_comp(
  ...
  // CHECK-V3:      %[[CALL:.*]] = call @xla.sdy.inlinable_manual_computation_body_3(%[[FULL_TO_SHARD]]) {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}, {}], manual={'x', 'y'}}"} : (tensor<1x2xi32>) -> tensor<1x2xi32>
...
func.func @all_reduce_input_with_unreduced_axes(%arg0: tensor<8x8xf32> {sdy.sharding = #sdy.sharding<@mesh_5, [{"j"}, {}], unreduced={"i"}>}) -> tensor<8x8xf32> {
  ...
  // CHECK-V3-NEXT: %[[FULL_TO_SHARD:.*]] = stablehlo.custom_call @SPMDFullToShardShape(%[[COPY_0]]) {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"}
  // CHECK-V3-NEXT: %[[CALL:.*]] = call @xla.sdy.inlinable_manual_computation_body_4(%[[FULL_TO_SHARD]]) {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"} : (tensor<4x8xf32>) -> tensor<4x8xf32>
  // CHECK-V3-NEXT: %[[COPY_1:.*]] = mhlo.copy %[[CALL]] {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"}
...
func.func @unreduced_op(%arg0: tensor<4x64x16xf32> {sdy.sharding = #sdy.sharding<@mesh_5, [{}, {"i", "j"}, {}]>}) -> tensor<4x16xf32> {
  ...
  // CHECK-V3-NEXT: %[[FULL_TO_SHARD:.*]] = stablehlo.custom_call @SPMDFullToShardShape(%[[COPY]]) {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"} : (tensor<4x16xf32>) -> tensor<2x16xf32>
  // CHECK-V3-NEXT: %[[CALL:.*]] = call @xla.sdy.inlinable_manual_computation_body_5(%[[FULL_TO_SHARD]]) {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"} : (tensor<2x16xf32>) -> tensor<2x16xf32>
  // CHECK-V3-NEXT: %[[COPY_1:.*]] = mhlo.copy %[[CALL]] {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"} : tensor<2x16xf32>
...
func.func private @foo(%arg0 : tensor<4x2xi32> {sdy.sharding = #sdy.sharding<@mesh_2, [{"y"}, {}]>}) -> (tensor<4x2xi32> {sdy.sharding = #sdy.sharding<@mesh_2, [{"y"}, {}]>}) {
...
// CHECK-V3-SAME:    %arg0: tensor<1xi32> {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"}
// CHECK-V3-SAME:    -> (tensor<1xi32> {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"}) {
// CHECK-V3-NEXT:    %[[NEGATE:.*]] = stablehlo.negate %arg0 {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"} : tensor<1xi32>
// CHECK-NEXT:    return %[[NEGATE]] : tensor<1xi32>
func.func private @foo_0(%arg0 : tensor<1xi32> {sdy.sharding = #sdy.sharding<@mesh_2, [{}]>}) -> (tensor<1xi32> {sdy.sharding = #sdy.sharding<@mesh_2, [{}]>}) {
  %0 = stablehlo.negate %arg0 : tensor<1xi32>
...
// CHECK-V3-SAME{LITERAL}:     %arg0: tensor<1xi32> {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"})
// CHECK-V3-SAME{LITERAL}:     -> (tensor<1xi32> {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"}) {
// CHECK-V3-NEXT:            %[[CALL:.*]] = call @foo_0(%arg0) {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}], manual={'x', 'y'}}"} : (tensor<1xi32>) -> tensor<1xi32>
...
// CHECK-V3-SAME{LITERAL}:     %arg0: tensor<1x2xi32> {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}, {}], manual={'x', 'y'}}"})
// CHECK-V3-SAME{LITERAL}:      -> (tensor<1x2xi32> {mhlo.sharding = "{mesh['x'=8,'y'=4], [{}, {}], manual={'x', 'y'}}"}) {
...
// CHECK-V3-SAME{LITERAL}:        attributes {mhlo.sharding = "{{mesh['x'=8,'y'=4], [{}, {}], manual={'x', 'y'}}, {mesh['x'=8,'y'=4], manual}}"}
// CHECK-NEXT:            cond {
...
// CHECK-V3-SAME{LITERAL}:     %arg0: tensor<4x8xf32> {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"})
// CHECK-V3-SAME{LITERAL}:     -> (tensor<4x8xf32> {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"}) {
...
// CHECK-V3-NEXT:            }) {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"} : (tensor<4x8xf32>) -> tensor<4x8xf32>
...
// CHECK-V3-SAME{LITERAL}:     %arg0: tensor<2x16xf32> {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"}
// CHECK-V3-SAME{LITERAL}:     -> (tensor<2x16xf32> {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"}) {
// CHECK-NEXT:            %[[ALL_REDUCE:.*]] = "stablehlo.all_reduce"(%arg0)
...
// CHECK-V3-NEXT:            }) {mhlo.sharding = "{mesh['i'=2,'j'=2], [{}, {}], manual={'i', 'j'}}"} : (tensor<2x16xf32>) -> tensor<2x16xf32>
...
func.func private @foo(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"c"}]>}) -> tensor<8xf32> {
...
func.func private @foo_0(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"c"}]>}) -> tensor<8xf32>
...
// CHECK-LABEL: func private @xla.sdy.inlinable_manual_computation_body_0(
// CHECK:         return
func.func @calls_with_same_original_with_different_shardings_manual_computations(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"c"}]>}) -> (tensor<8xf32>, tensor<8xf32>) {
  %0 = call @foo(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  %1 = call @foo_0(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  return %0, %1 : tensor<8xf32>, tensor<8xf32>
}
...
func.func private @foo_0(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{}]>}) -> tensor<8xf32>
attributes {sdy.original_func_name = "foo"} {
  %0 = sdy.manual_computation(%arg0) in_shardings=[<@mesh, [{"a", "b"}]>] out_shardings=[<@mesh, [{"a", "b"}]>] manual_axes={"a"} (%arg1: tensor<4xf32>) {
    %1 = stablehlo.multiply %arg1, %arg1 {sdy.sharding = #sdy.sharding_per_value<[<@mesh, [{"b"}]>]>} : tensor<4xf32>
    sdy.return %1 : tensor<4xf32>
...
// CHECK-LABEL: func private @xla.sdy.inlinable_manual_computation_body_0(
// CHECK:         return
func.func @calls_with_same_original_with_different_shardings_manual_computations_twice_calls_to_unsharded(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"c"}]>}) -> (tensor<8xf32>, tensor<8xf32>) {
  %0 = call @foo(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  %1 = call @foo_0(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  %2 = call @foo_0(%arg0) : (tensor<8xf32>) -> tensor<8xf32>
  return %0, %1 : tensor<8xf32>, tensor<8xf32>
}
func.func private @foo(%arg0: tensor<8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"c"}]>}) -> tensor<8xf32> {
  %0 = sdy.manual_computation(%arg0) in_shardings=[<@mesh, [{"a", "b"}]>] out_shardings=[<@mesh, [{"a", "b"}]>] manual_axes={"a"} (%arg1: tensor<4xf32>) {
