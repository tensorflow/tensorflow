...
func.func @unreduced_sub_axis(%arg0: tensor<4x64x16xf32> {sdy.sharding = #sdy.sharding<@mesh_2, [{}, {"x", "y"}, {}]>}) -> tensor<4x16xf32> {
  %0 = stablehlo.constant dense<0.000000e+00> : tensor<f32>
  // CHECK:      %[[REDUCE:.*]] = stablehlo.reduce(%arg0 init: %cst) applies stablehlo.add across dimensions = [1]
  // CHECK-SAME{LITERAL}: {mhlo.sharding = "{devices=[4,1,2,4]<=[4,2,4]T(2,1,0) last_tile_dims={unreduced, replicated}}"}
  ...
  %2 = stablehlo.add %1, %1 {sdy.sharding = #sdy.sharding_per_value<[<@mesh_2, [{"y"}, {}], unreduced={"x":(1)2}>]>} : tensor<4x16xf32>
  %3 = sdy.all_reduce {"x":(1)2} %2 out_sharding=<@mesh_2, [{"y"}, {}]> : tensor<4x16xf32>
  return %3 : tensor<4x16xf32>
}
...
func.func @unreduced_canonicalization(%arg0: tensor<4x64x16xf32> {sdy.sharding = #sdy.sharding<@mesh_2, [{}, {"x"}, {}]>}) -> tensor<4x16xf32> {
  %0 = stablehlo.constant dense<0.000000e+00> : tensor<f32>
  // CHECK:      %[[REDUCE:.*]] = stablehlo.reduce(%arg0 init: %cst) applies stablehlo.add across dimensions = [1]
  // CHECK-SAME{LITERAL}: {mhlo.sharding = "{devices=[1,1,8,4]<=[4,8]T(1,0) last_tile_dims={unreduced, replicated}}"}
  ...
  %2 = stablehlo.add %1, %1 {sdy.sharding = #sdy.sharding_per_value<[<@mesh_2, [{}, {}], unreduced={"x":(1)2, "y"}>]>} : tensor<4x16xf32>
  %3 = sdy.all_reduce {"x":(1)2, "y"} %2 out_sharding=<@mesh_2, [{}, {}]> : tensor<4x16xf32>
  return %3 : tensor<4x16xf32>
}
