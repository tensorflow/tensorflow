...
// CHECK-SAME{LITERAL}:     %arg0: tensor<4x16xf32> {mhlo.sharding = "{manual}"})
// CHECK-SAME{LITERAL}:     -> (tensor<4x16xf32> {mhlo.sharding = "{manual}"}) {
// CHECK-NEXT:            %[[ALL_REDUCE:.*]] = "stablehlo.all_reduce"(%arg0)
...
// CHECK-NEXT:              %[[ADD:.*]] = stablehlo.add %arg1, %arg2 {mhlo.sharding = "{manual}"} : tensor<f32>
// CHECK-NEXT:              stablehlo.return %[[ADD]] : tensor<f32>
// CHECK-NEXT:            }) {mhlo.sharding = "{manual}"} : (tensor<4x16xf32>) -> tensor<4x16xf32>
