...
// CHECK-LABEL: func @all_reduce_max_multiple_axes
func.func @all_reduce_max_multiple_axes(%arg0: tensor<8x8xf32> {sdy.sharding = #sdy.sharding<@mesh_x_2_y_2, [{}, {}], unreduced=max{"x", "y"}>}) -> tensor<8x8xf32> {
  // CHECK:            %[[ALL_REDUCE:.*]] = "stablehlo.all_reduce"(%arg1) <{
  ...
  // CHECK-NEXT:              %[[MAX:.*]] = stablehlo.maximum %arg2, %arg3 : tensor<f32>
  // CHECK-NEXT:              stablehlo.return %[[MAX]] : tensor<f32>
  %0 = sdy.all_reduce max {"x", "y"} %arg0 out_sharding=<@mesh_x_2_y_2, [{}, {}]> : tensor<8x8xf32>
  return %0 : tensor<8x8xf32>
}
...
// CHECK-LABEL: func @all_reduce_min_non_iota_device_order
func.func @all_reduce_min_non_iota_device_order(%arg0: tensor<8x8xf32> {sdy.sharding = #sdy.sharding<@mesh_x_2_y_2_non_iota, [{"y"}, {}], unreduced=min{"x"}>}) -> tensor<8x8xf32> {
  // CHECK:            %[[ALL_REDUCE:.*]] = "stablehlo.all_reduce"(%arg1) <{
  // CHECK-SAME:              channel_handle = #stablehlo.channel_handle<handle = {{.*}}, type = 1>,
  // CHECK-SAME{LITERAL}:     replica_groups = dense<[[3, 1], [2, 0]]> : tensor<2x2xi64>,
  // CHECK-SAME:              use_global_device_ids}> ({
  // CHECK-NEXT:            ^bb0(%arg2: tensor<f32>, %arg3: tensor<f32>):
  // CHECK-NEXT:              %[[MIN:.*]] = stablehlo.minimum %arg2, %arg3 : tensor<f32>
  // CHECK-NEXT:              stablehlo.return %[[MIN]] : tensor<f32>
  %0 = sdy.all_reduce min {"x"} %arg0 out_sharding=<@mesh_x_2_y_2_non_iota, [{"y"}, {}]> : tensor<8x8xf32>
  return %0 : tensor<8x8xf32>
}
...
// CHECK-LABEL: func @all_reduce_max_three_axes
func.func @all_reduce_max_three_axes(%arg0: tensor<8x8xf32> {sdy.sharding = #sdy.sharding<@mesh_x_2_y_4_z_2, [{"x"}, {"z"}], unreduced=max{"y"}>}) -> tensor<8x8xf32> {
  // CHECK:            %[[ALL_REDUCE:.*]] = "stablehlo.all_reduce"(%arg1) <{
  // CHECK-SAME:              channel_handle = #stablehlo.channel_handle<handle = {{.*}}, type = 1>,
  ...
  // CHECK-NEXT:              %[[MAX:.*]] = stablehlo.maximum %arg2, %arg3 : tensor<f32>
  // CHECK-NEXT:              stablehlo.return %[[MAX]] : tensor<f32>
  %0 = sdy.all_reduce max {"y"} %arg0 out_sharding=<@mesh_x_2_y_4_z_2, [{"x"}, {"z"}]> : tensor<8x8xf32>
  return %0 : tensor<8x8xf32>
}
...
// CHECK-LABEL: func @reduce_scatter_max_multiple_dims
func.func @reduce_scatter_max_multiple_dims(%arg0: tensor<8x8x8xf32> {sdy.sharding = #sdy.sharding<@mesh_x_4_y_2_z_2, [{}, {}, {}], unreduced=max{"x", "y", "z"}>}) -> tensor<8x8x8xf32> {
  // CHECK:            %[[REDUCE_SCATTER_1:.*]] = "stablehlo.reduce_scatter"(%arg1) <{
  // CHECK-SAME:              channel_handle = #stablehlo.channel_handle<handle = {{.*}}, type = 1>,
  // CHECK-SAME{LITERAL}:     replica_groups = dense<[[0, 4, 8, 12, 1, 5, 9, 13], [2, 6, 10, 14, 3, 7, 11, 15]]> : tensor<2x8xi64>,
  // CHECK-SAME:              scatter_dimension = 0 : i64,
  // CHECK-SAME:              use_global_device_ids}> ({
  // CHECK-NEXT:            ^bb0(%arg2: tensor<f32>, %arg3: tensor<f32>):
  // CHECK-NEXT:              %[[MAX_1:.*]] = stablehlo.maximum %arg2, %arg3 : tensor<f32>
  // CHECK-NEXT:              stablehlo.return %[[MAX_1]] : tensor<f32>
  // CHECK:            %[[REDUCE_SCATTER_2:.*]] = "stablehlo.reduce_scatter"(%[[REDUCE_SCATTER_1]]) <{
  // CHECK-SAME:              channel_handle = #stablehlo.channel_handle<handle = {{.*}}, type = 1>,
  // CHECK-SAME{LITERAL}:     replica_groups = dense<[[0, 2], [1, 3], [4, 6], [5, 7], [8, 10], [9, 11], [12, 14], [13, 15]]> : tensor<8x2xi64>,
  // CHECK-SAME:              scatter_dimension = 2 : i64,
  // CHECK-SAME:              use_global_device_ids}> ({
  // CHECK-NEXT:            ^bb0(%arg2: tensor<f32>, %arg3: tensor<f32>):
  // CHECK-NEXT:              %[[MAX_2:.*]] = stablehlo.maximum %arg2, %arg3 : tensor<f32>
  // CHECK-NEXT:              stablehlo.return %[[MAX_2]] : tensor<f32>
  %0 = sdy.reduce_scatter max [{"z", "x"}, {}, {"y"}] %arg0 out_sharding=<@mesh_x_4_y_2_z_2, [{"z", "x"}, {}, {"y"}]> : tensor<8x8x8xf32>
  return %0 : tensor<8x8x8xf32>
}
