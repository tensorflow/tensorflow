...
func.func @single_manual_comp(%arg0: tensor<8x16xf32> {sdy.sharding = #sdy.sharding<@mesh_0, [{"a", ?}, {"b", ?}]>}, %arg1: tensor<16x32xf32> {sdy.sharding = #sdy.sharding<@mesh_0, [{"b", ?}, {?}]>}) -> (tensor<8x32xf32> {sdy.sharding = #sdy.sharding<@mesh_0, [{"a"}, {}]>}) {
  ...
  // CHECK-NEXT: %[[SHMAP:.*]] = call @local_xla.sdy.manual_computation_body(%[[GLOBAL_TO_LOCAL]]#0, %[[GLOBAL_TO_LOCAL]]#1) {mhlo.frontend_attributes = {inlineable = "false"}} : (tensor<2x8xf32>, tensor<8x32xf32>) -> tensor<2x32xf32>
...
func.func @manual_comp_using_another(%arg0: tensor<8x8xf32> {sdy.sharding = #sdy.sharding<@mesh_0, [{"a"}, {}]>})
  ...
  // CHECK-NEXT: %[[SHMAP_0:.*]] = call @local_xla.sdy.manual_computation_body_0(%[[GLOBAL_TO_LOCAL_0]]) {mhlo.frontend_attributes = {inlineable = "false"}} : (tensor<2x8xf32>) -> tensor<2x8xf32>
  ...
  // CHECK-NEXT: %[[SHMAP_1:.*]] = call @local_xla.sdy.manual_computation_body_1(%[[GLOBAL_TO_LOCAL_1]]) {mhlo.frontend_attributes = {inlineable = "false"}} : (tensor<8x4xf32>) -> tensor<8x4xf32>
...
func.func @nested_shmaps(%arg0: tensor<4x8xf32> {sdy.sharding = #sdy.sharding<@mesh_1, [{"a"}, {"b"}]>}) -> (tensor<4x8xf32> {sdy.sharding = #sdy.sharding<@mesh_1, [{"a", ?}, {?}]>}) {
  ...
  // CHECK-NEXT: %[[SHMAP:.*]] = call @local_xla.sdy.manual_computation_body_3(%[[GLOBAL_TO_LOCAL]]) {mhlo.frontend_attributes = {inlineable = "false"}} : (tensor<2x8xf32>) -> tensor<2x8xf32>
  // CHECK-NEXT: %[[LOCAL_TO_GLOBAL:.*]] = stablehlo.custom_call @local_xla.sdy.LocalToGlobalShape(%[[SHMAP]])
  // CHECK-SAME: {has_side_effect = true,
  // CHECK-SAME:  mhlo.frontend_attributes = {
  // CHECK-SAME:    xla.sdy.manual_axes = "#sdy<manual_axes{\22a\22}>",
...
func.func @nested_shmaps_extra_op(%arg0: tensor<4x8xf32> {sdy.sharding = #sdy.sharding<@mesh_1, [{"a"}, {"b"}]>}) -> (tensor<4x8xf32> {sdy.sharding = #sdy.sharding<@mesh_1, [{"a", ?}, {?}]>}) {
  ...
  // CHECK-SAME:    xla.sdy.manual_axes = "#sdy<manual_axes{\22a\22}>"}}
  // CHECK-SAME: (tensor<4x8xf32>) -> tensor<2x8xf32>
  // CHECK-NEXT: %[[SHMAP:.*]] = call @local_xla.sdy.manual_computation_body_5(%[[GLOBAL_TO_LOCAL]]) {mhlo.frontend_attributes = {inlineable = "false"}} : (tensor<2x8xf32>) -> tensor<2x8xf32>
  // CHECK-NEXT: %[[LOCAL_TO_GLOBAL:.*]] = stablehlo.custom_call @local_xla.sdy.LocalToGlobalShape(%[[SHMAP]])
  // CHECK-SAME: {has_side_effect = true,
  // CHECK-SAME:  mhlo.frontend_attributes = {
  // CHECK-SAME:    xla.sdy.manual_axes = "#sdy<manual_axes{\22a\22}>",
...
func.func @manual_computation_no_inputs() -> tensor<4xi64> {
  // CHECK-NEXT: %[[SHMAP:.*]] = call @local_xla.sdy.manual_computation_body_6() {mhlo.frontend_attributes = {inlineable = "false"}} : () -> tensor<2xi64>
  // CHECK-NEXT: %[[LOCAL_TO_GLOBAL:.*]] = stablehlo.custom_call @local_xla.sdy.LocalToGlobalShape(%[[SHMAP]])
  // CHECK-SAME: {has_side_effect = true,
  // CHECK-SAME:  mhlo.frontend_attributes = {
  // CHECK-SAME:    xla.sdy.manual_axes = "#sdy<manual_axes{\22b\22}>",
...
func.func @manual_computation_no_outputs(%arg0: tensor<4xi64>) {
  ...
  // CHECK-NEXT: call @local_xla.sdy.manual_computation_body_7(%[[GLOBAL_TO_LOCAL]]) {mhlo.frontend_attributes = {inlineable = "false"}} : (tensor<2xi64>) -> ()
  // CHECK-NEXT: return
...
func.func @manual_computation_no_inputs_no_outputs() {
  // CHECK-NEXT: call @local_xla.sdy.manual_computation_body_8() : () -> ()
...
// CHECK-NEXT:   %[[SHMAP:.*]] = call @local_xla.sdy.manual_computation_body_2(%[[GLOBAL_TO_LOCAL]]) {mhlo.frontend_attributes = {inlineable = "false"}} : (tensor<2x4xf32>) -> tensor<2x4xf32>
...
// CHECK-SAME:        xla.sdy.manual_axes = "#sdy<manual_axes{\22b\22}>"}}
// CHECK-SAME:     (tensor<2x8xf32>) -> tensor<2x4xf32
// CHECK-NEXT:   %[[SHMAP:.*]] = call @local_xla.sdy.manual_computation_body_4(%[[GLOBAL_TO_LOCAL]]) {mhlo.frontend_attributes = {inlineable = "false"}} : (tensor<2x4xf32>) -> tensor<2x4xf32>
// CHECK-NEXT:   %[[LOCAL_TO_GLOBAL:.*]] = stablehlo.custom_call @local_xla.sdy.LocalToGlobalShape(%[[SHMAP]])
// CHECK-SAME:     {has_side_effect = true,
// CHECK-SAME:      mhlo.frontend_attributes = {
// CHECK-SAME:        xla.sdy.manual_axes = "#sdy<manual_axes{\22b\22}>",
