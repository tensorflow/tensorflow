...
// Verifies that:
// 1. Every op carries a native `sdy.sharding` that describes its own result:
//    - a `@Sharding` custom call per operand holds the global in-shardings,
//      making the reshard `sdy.manual_computation` performs implicitly on entry
//      explicit. It is side-effecting so that the HLO round trip cannot drop it
//      (see `ZeroSizedHloElimination` for a zero-element operand) or CSE two
//      wrappers carrying different shardings;
//    - `GlobalToLocalShape` and the body call op hold the local shardings plus
//      typed `xla.sdy.manual_axes`;
//    - `LocalToGlobalShape` holds the global out-shardings.
...
// Tests basic export with multiple inputs, outputs, and all-reduce.
...
func.func @single_manual_comp(%arg0: tensor<8x16xf32> {sdy.sharding = #sdy.sharding<@mesh_0, [{"a", ?}, {"b", ?}]>}, %arg1: tensor<16x32xf32> {sdy.sharding = #sdy.sharding<@mesh_0, [{"b", ?}, {?}]>}) -> (tensor<8x32xf32> {sdy.sharding = #sdy.sharding<@mesh_0, [{"a"}, {}]>}) {
  // CHECK-NEXT: %[[SHARDING_0:.*]] = stablehlo.custom_call @Sharding(%arg0)
  // CHECK-SAME:   has_side_effect = true
  // CHECK-SAME{LITERAL}: sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=4, "b"=2]>, [{"a"}, {"b"}]>]>
  // CHECK-SAME:   (tensor<8x16xf32>) -> tensor<8x16xf32>
  // CHECK-NEXT: %[[SHARDING_1:.*]] = stablehlo.custom_call @Sharding(%arg1)
  // CHECK-SAME:   has_side_effect = true
  // CHECK-SAME{LITERAL}: sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=4, "b"=2]>, [{"b"}, {}], replicated={"a"}>]>
  // CHECK-SAME:   (tensor<16x32xf32>) -> tensor<16x32xf32>
  // CHECK-NEXT: %[[GLOBAL_TO_LOCAL:.*]]:2 = stablehlo.custom_call @xla.sdy.GlobalToLocalShape(%[[SHARDING_0]], %[[SHARDING_1]])
...
// Tests chained manual computations where the output of one feeds the input of
// another with a *different* sharding. The `@Sharding` custom call on the
// second boundary is what preserves the reshard between them; without it the
// `in_shardings` of the second manual computation would be lost.
...
func.func @manual_comp_using_another(%arg0: tensor<8x8xf32> {sdy.sharding = #sdy.sharding<@mesh_0, [{"a"}, {}]>})
  ...
  // CHECK-NEXT: %[[GLOBAL_TO_LOCAL_0:.*]] = stablehlo.custom_call @xla.sdy.GlobalToLocalShape(%[[SHARDING_0]])
  // CHECK-SAME{LITERAL}: sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=4, "b"=2]>, [{}, {}]>]>
  ...
  // CHECK-NEXT: %[[SHARDING_1:.*]] = stablehlo.custom_call @Sharding(%[[LOCAL_TO_GLOBAL_0]])
  // CHECK-SAME{LITERAL}: sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=4, "b"=2]>, [{}, {"b"}]>]>
  // CHECK-NEXT: %[[GLOBAL_TO_LOCAL_1:.*]] = stablehlo.custom_call @xla.sdy.GlobalToLocalShape(%[[SHARDING_1]])
  // CHECK-SAME{LITERAL}: sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=4, "b"=2]>, [{}, {}]>]>
...
func.func @nested_shmaps(%arg0: tensor<4x8xf32> {sdy.sharding = #sdy.sharding<@mesh_1, [{"a"}, {"b"}]>}) -> (tensor<4x8xf32> {sdy.sharding = #sdy.sharding<@mesh_1, [{"a", ?}, {?}]>}) {
  ...
  // CHECK-NEXT: %[[GLOBAL_TO_LOCAL:.*]] = stablehlo.custom_call @xla.sdy.GlobalToLocalShape(%[[SHARDING]])
...
// Edge case: Manual computation producing an output but taking no inputs. There
// is no `GlobalToLocalShape`, so the body call op is the only carrier of the
// manual axes.
...
// The body call op has no results, so `GlobalToLocalShape` is the only carrier
// of the manual axes.
...
func.func @manual_computation_tokens(%arg0: !stablehlo.token, %arg1: tensor<2xi64>) -> (!stablehlo.token, tensor<2xi64>) {
  // CHECK-NEXT: %[[SHARDING_0:.*]] = stablehlo.custom_call @Sharding(%arg0)
  // CHECK-SAME{LITERAL}: sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=4, "b"=2]>, []>]>
  // CHECK-SAME:   (!stablehlo.token) -> !stablehlo.token
  // CHECK-NEXT: %[[SHARDING_1:.*]] = stablehlo.custom_call @Sharding(%arg1)
  // CHECK-SAME{LITERAL}: sdy.sharding = #sdy.sharding_per_value<[<mesh<["a"=4, "b"=2]>, [{"b"}]>]>
