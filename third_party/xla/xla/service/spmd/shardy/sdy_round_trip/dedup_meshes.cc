...
TensorShardingAttr replaceAxesInSharding(
  ...
  MeshAttr newMesh = sdy::getMeshOp(symbolTable, mainMeshName).getMesh();
  ...
  auto buildNewAxisRefList = [buildNewAxisRef,
                              newMesh](ArrayRef<AxisRefAttr> oldAxisRefs) {
    AxisRefVector newAxisRefs;
    newAxisRefs.reserve(oldAxisRefs.size());
    llvm::transform(oldAxisRefs, AddAxisOrMergeInserter(&newAxisRefs, &newMesh),
                    buildNewAxisRef);
    return newAxisRefs;
  };
...
// Replaces all shardings and manual axes, that referred to some mesh that isn't
// the main mesh saved in the pair of `MeshToAxisMap`, with the main mesh.
//
// All shardings which before referred to different meshes with the same axis
// sizes will now refer to one meshes. So there can still be multiple meshes,
// but they will all all have different axis sizes.
