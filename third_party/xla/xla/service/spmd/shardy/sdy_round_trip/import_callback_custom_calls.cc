  ...
  void runOnOperation() final {
    getOperation().walk([&](CustomCallOp op) {
      ...
      if (op->getNumResults() != 0 ||
          (targetName != kPythonCpuCallbackCustomCallTargetName &&
      ...
      // have a maximal sharding, we add a dummy result and set it and the
      // result layout to the 0th operand layout.
      CustomCallOp newCustomCall =
          cloneOpWithNewResultTypes(op, op->getOperand(0).getType(), rewriter);
