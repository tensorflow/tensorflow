 ...
 public:
  ...
  void runOnOperation() final {
    ...
    moduleOp->walk([&](sdy::ManualComputationOp manualComputation) {
      ...
      if (!manualComputation.getManualAxesAttr().empty()) {
  ...
  StringRef getDescription() const override {
    return "Converts a `ManualComputationOp` to the following."
           ...
           "2. A `CallOp` calling the function in 1. It is marked as not "
           "inlineable if manual axes are not empty."
           "3. A pair of `CustomCallOp`s that change the shape of the "
           "arguments/results. They save the in/out shardings and manual axes "
           "as frontend attrs.";
  }
