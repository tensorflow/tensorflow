...
absl::StatusOr<ShardMapArgumentsResults> getJaxShardMapPatternOps(CallOp op) {
  ...
  for (Value operand : op.getOperands()) {
    ...
    argumentOps.emplace_back(shardingCustomCall, fullToShardCustomCall);
  }
  ...
  for (Value result : op.getResults()) {
    if (result.use_empty()) {
      ...
      // ManualComputationOp.
      resultOps.emplace_back(nullptr, nullptr);
      continue;
    }
    if (!result.hasOneUse()) {
      return absl::NotFoundError(
    ...
    resultOps.emplace_back(shardingCustomCall, shardToFullCustomCall);
  }
 ...
 private:
  void runOnOperation() final {
    ...
    module->walk([&](CallOp op) {
      ...
      jaxShardMapOpsToBeErased.emplace_back(
