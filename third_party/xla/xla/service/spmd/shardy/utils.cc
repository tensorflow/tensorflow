...
mlir::sdy::TensorShardingAttr convertToSdyShardingAttr(
  ...
  if (!hloSharding.UseNamedShardingLeaf()) {
    CHECK(hloSharding.IsReplicated() || hloSharding.IsManual() ||
          hloSharding.IsUnreduced())
        << "Expected HloShardingV3 during Shardy import when "
           "'xla_enable_hlo_sharding_v3' flag is enabled, but got "
           "non-replicated/manual/unreduced HloShardingV2: "
        << hloSharding
        << ". Please contact OpenXLA/Shardy team if you encounter this error.";
    if (globalMesh) {
      mlir::Attribute meshOrRef = globalMeshRef ? mlir::Attribute(globalMeshRef)
                                                : mlir::Attribute(globalMesh);
      if (hloSharding.IsManual() || hloSharding.IsReplicated()) {
  ...
  if (globalMesh && globalMeshRef && meshAttr == globalMesh) {
    meshOrRef = globalMeshRef;
  }
...
mlir::sdy::TensorShardingPerValueAttr convertToSdySharding(
  ...
  if (hloSharding.IsTuple()) {
    ...
    for (int i = 0; i < types.size(); ++i) {
      sdyShardings.push_back(convertToSdyShardingAttr(
          ...
          globalMesh, globalMeshRef, context));
    }
  ...
  if (types.empty()) {
    return TensorShardingPerValueAttr::get(
        context, convertToSdyShardingAttr(hloSharding, /*rank=*/0, globalMesh,
                                          globalMeshRef, context));
  }
