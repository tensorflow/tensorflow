...
SpmdPartitioningVisitor::HandleDUSAllPartitionedSliceDimsHaveConstantIndices(
  ...
  if (update_tensor->opcode() == HloOpcode::kBroadcast) {
    ...
    for (int i = 0; i < hlo->shape().dimensions().size(); ++i) {
      ...
      std::cout << " dim i: " << i << " slice_start: " << slice_start
                << " dus_start: " << dus_start << "\n";
      if (slice_start > dus_start) {
        needs_slice = true;
        new_slice_starts[i] = slice_start - dus_start;
        std::cout << " ++ dim i: " << i
                  << " new_slice_starts: " << new_slice_starts[i] << "\n";
      ...
      std::cout << " ** dim i: " << i
                ...
                << padding_config.dimensions(i).edge_padding_high()
                << " length_of_operand_to_pad:" << length_of_operand_to_pad
                << " slice->operand(0)->shape().dimensions(i)"
                << slice->operand(0)->shape().dimensions(i)
                << " slice->shape().dimensions(i): "
                << slice->shape().dimensions(i) << "\n";
      if (padding_config.dimensions(i).edge_padding_high() <
        ...
        new_slice_limits[i] -=
            ...
            (padding_config.dimensions(i).edge_padding_high());
        std::cout << " ++2 dim i: " << i
                  << " new_slice_limits: " << new_slice_limits[i] << "\n";
        std::cout
            << " ++3 dim i: " << i
            << "  (length_of_operand_to_pad - slice->shape().dimensions(i)) : "
            << (length_of_operand_to_pad - slice->shape().dimensions(i))
    ...
    if (legal) {
      ...
      std::cout << " original replacement: " << replacement->ToString() << "\n";
      if (needs_slice) {
        ...
        std::cout << " created new slice: " << replacement->ToString() << "\n";
      }
      ...
      if (needs_pad) {
        ...
        replacement->set_sharding(hlo->sharding());
        std::cout << " created new pad: " << replacement->ToString() << "\n";
      }
