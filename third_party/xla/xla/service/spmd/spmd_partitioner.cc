...
absl::Status SpmdPartitioningVisitor::HandleScan(HloInstruction* hlo) {
  ...
  // 2. Partition inits. Shape of init is carry shape. We just partition it
  // using its own sharding.
  ...
  for (HloInstruction* init : scan->inits()) {
    partitioned_inits.push_back(
        GetPartitionedHlo(init).Reshard(init->sharding()).hlo());
  }
  ...
  if (sharding.IsTuple()) {
    ...
    for (const HloSharding& sub : sharding.tuple_elements()) {
      if (!sub.IsReplicated() && !sub.IsTuple() && !sub.IsManual() &&
          sub.dimension(scan_dimension) > 1) {
    ...
    if (!sharding.IsReplicated() && !sharding.IsManual() &&
        sharding.dimension(scan_dimension) > 1) {
          ...
          for (int64_t i = 0; i < scan->operand_count(); ++i) {
            ...
            if (i < num_inputs) {
              if (!operand_sharding.IsReplicated() &&
                  !operand_sharding.IsTuple() && !operand_sharding.IsManual()) {
                HloSharding replicated_sharding = operand_sharding;
                if (!operand_sharding.IsTuple() &&
                    !operand_sharding.IsManual() &&
                    operand_sharding.dimension(scan_dimension) > 1) {
                  replicated_sharding =
                      hlo_sharding_util::PartiallyReplicateTiledShardingOnDims(
