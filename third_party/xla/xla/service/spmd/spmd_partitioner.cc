...
absl::Status ClearShardingAttributes(
  ...
  // Recover the unreduced sharding for inputs and output.
  ...
  for (HloComputation* computation : module->computations(execution_threads)) {
    for (HloInstruction* hlo : computation->instructions()) {
      ...
      if (sharding.IsUnreduced()) {
        ...
        hlo->add_frontend_attribute(sdy::kHasUnreducedAxes, "true");
        if (!sharding.IsTuple()) {
          hlo->set_sharding(HloSharding::PartialTile(sharding.tile_assignment(),
                                                     sharding.metadata()));
        } else {
          std::vector<HloSharding> elements;
          elements.reserve(sharding.tuple_elements().size());
          for (int64_t i = 0; i < sharding.tuple_elements().size(); ++i) {
            elements.push_back(HloSharding::PartialTile(
                sharding.tile_assignment(), sharding.metadata()));
          }
          hlo->set_sharding(HloSharding::Tuple(hlo->shape(), elements));
        }
      }
    }
  }
