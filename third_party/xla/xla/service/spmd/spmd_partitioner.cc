...
// Returns true if any of the input or outputs changed between the old and new
// entry shapes. NOTE that we explicitly ignore the layout, since it is either
// defined beforehand or during layout assignment.
...
absl::StatusOr<bool> SpmdPartitioner::RunImpl(
  ...
  bool shapes_changed =
      ShapeChangesBetween(old_entry_layout, new_program_shape);
  if (shapes_changed) {
