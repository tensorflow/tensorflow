...
std::optional<HloInstruction*> ExchangeHalo(
  ...
  for (int64_t i = CeilOfRatio(max_left_halo_size, input_shard_size) - 1;
    ...
    // Check if the left halo has zero-bcasted data to insert before the
    // collective permute.
    if (left_coalesced_zero_halo_size > 0) {
      ...
      VLOG(1) << "ExchangeHalo:left halo zero-bcasted coalesced "
              << padding->ToString();
      concat_pieces.push_back(padding);
      // reset the coalesced zero halo size to 0.
      left_coalesced_zero_halo_size = 0;
    }
    auto left_halo =
  ...
  if (right_coalesced_zero_halo_size > 0) {
    ...
    HloInstruction* padding = CreateZero(zero_bcast_shape, b);
    VLOG(1) << "ExchangeHalo:right halo zero-bcasted coalesced "
            << padding->ToString();
    concat_pieces.push_back(padding);
  }
