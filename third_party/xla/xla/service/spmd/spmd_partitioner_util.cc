...
HloInstruction* GetInGroupPartitionId(
  ...
  std::vector<uint32_t> in_group_ids(device_groups.num_devices_total());
      ...
      [device_groups_ptr](
        ...
        if (partition_subgroups.empty()) {
          std::vector<std::vector<int64_t>> result(
              device_groups_ptr->num_groups());
          for (int64_t g = 0; g < device_groups_ptr->num_groups(); ++g) {
            result[g].resize(device_groups_ptr->num_devices_per_group());
            for (int64_t j = 0; j < device_groups_ptr->num_devices_per_group();
                 ++j) {
              result[g][j] = device_groups_ptr->array()(g, j);
            }
          }
          return result;
        }
...
std::optional<GroupedSharding> AlignGroupsWithInternal(
  ...
  auto get_permutation = [&](int64_t base_group_idx,
    ...
    CHECK(grouped_sharding.device_groups.num_devices_per_group() ==
    ...
    std::vector<int> permutation(num_devices_per_group);
    for (int64_t i = 0; i < num_devices_per_group; ++i) {
  ...
  CHECK_EQ(grouped_sharding.device_groups.dim(0),
           reference.device_groups.dim(0));
  std::vector<int64_t> device_to_ref_group(
      reference.device_groups.num_devices_total());
  ...
  std::vector<int> original_src_to_ref_permutation;
...
HloInstruction* PerGroupSliceFromReplicated(
  ...
  std::vector<uint32_t> group_ids(device_groups.num_devices_total());
