...
absl::StatusOr<HloInstruction*> EmitWindowedDotGeneral(
  ...
  if (options.bidirectional_windowed_einsum && num_partitions % 4 == 0 &&
    ...
    TF_XLA_ASSIGN_OR_RETURN(outputs,
    ...
    if (operands_sharded_at_contracting_dims) {
      ...
      TF_XLA_ASSIGN_OR_RETURN(extra_inout,
      ...
      TF_XLA_ASSIGN_OR_RETURN(o, get_partial_unid_result(l, r, o, real_i));
      ...
      TF_XLA_ASSIGN_OR_RETURN(o, get_partial_unid_result(l, r, o, i));
      ...
      TF_XLA_ASSIGN_OR_RETURN(o, get_partial_unid_result(next_l, next_r, o, i));
...
absl::StatusOr<HloInstruction*> PartitionBaseCase(
  ...
  if (lhs_batch_partitions == rhs_batch_partitions &&
    ...
    TF_XLA_ASSIGN_OR_RETURN(auto dot, create_sharded_dot(lhs, rhs, b, conv_window));
...
absl::Status SinkInputNodesIntoWindowedDotGeneralLoopOnContractingDimensions(
  ...
  TF_XLA_RETURN_IF_ERROR(input_tuple->ReplaceOperandWithDifferentShape(
  ...
  TF_XLA_RETURN_IF_ERROR(body->root_instruction()->ReplaceOperandWithDifferentShape(
  ...
  for (auto ou : old_body_param_users) {
    if (ou->opcode() == HloOpcode::kGetTupleElement &&
      ...
      TF_XLA_RETURN_IF_ERROR(body->RemoveInstruction(ou));
    }
  }
  return absl::OkStatus();
}
...
absl::Status MoveUsersIntoWindowedDotGeneralLoopOnNonContractingDimensions(
  ...
  for (int64_t i = 0; i < reduce_outputs.size(); ++i) {
    ...
    TF_XLA_RETURN_IF_ERROR(reduce_outputs[i]->ReplaceAllUsesWith(new_output));
    TF_XLA_RETURN_IF_ERROR(
