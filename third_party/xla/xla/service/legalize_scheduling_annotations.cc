...
absl::StatusOr<bool> LegalizeSchedulingAnnotations::PropagateAnnotations(
  ...
  for (auto& [annotation_id, sources] : annotation_id_to_instructions) {
    ...
    if (!status.ok()) {
      // Annotation propagation failed; this means there is a conflict in the
      // annotation id assignment. We resolve this by discarding the current
      // group of annotation.
      LOG(WARNING) << "Annotation propagation failed: " << status;
      for (HloInstruction* instr : to_annotate) {
        LOG(WARNING) << "Dropping annotation id " << annotation_id << " from "
                     << instr->name();
