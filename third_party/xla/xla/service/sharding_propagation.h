...
absl::StatusOr<bool> ProcessShardingInstruction(
    ...
    const std::string& copy_metadata_op_name = "_sharding_custom_call_to_copy");

// Remove a copy instruction if it satisfies:
// 1. its metadata's op_name is the same as `copy_metadata_op_name`;
// 2. the copy instruction and its operand have the same sharding.
// The copy instruction is usually generated by ProcessShardingInstruction.
absl::StatusOr<bool> RemoveRedundantCopyInstructions(
    HloModule* module,
    const absl::flat_hash_set<absl::string_view>& execution_threads,
    absl::string_view copy_metadata_op_name = "_sharding_custom_call_to_copy");
...
class ShardingPropagation : public HloModulePass {
 ...
 private:
  ...
  const std::string sharding_custom_call_to_copy_metadata_op_name_ =
      "_sharding_custom_call_to_copy";
};
