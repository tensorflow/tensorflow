...
class CopyRemover {
 public:
  ...
  //
  // The walk is flat, and correct only under a contract the producer of views
  // (the TPU view passes) enforces: a view colored value never escapes into a
  // tuple, a called computation (while, call, conditional) or an async pair,
  // where the reads would happen later than the position recorded here; it
  // is never written through on the read side; and its operand 0 is never
  // itself view colored (views are one level deep, so the readers found here
  // read `value`'s own buffer). A view colored user that is not a bitcast or
  // copy is recorded as a reader at its own position on purpose (the TPU
  // write side wrapper, whose write is also a dataflow use of the viewed
  // buffer).
