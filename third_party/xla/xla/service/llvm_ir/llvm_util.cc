...
llvm::Value* HandleHalfwayPointsFxToF8(
  ...
  llvm::Type* fx_float_type = fx_scalar_float_type;
  if (vector_width.has_value()) {
    fx_float_type = llvm::VectorType::get(
        fx_scalar_float_type, llvm::ElementCount::getFixed(*vector_width));
  }

  // Lambda to create scalar or vector ConstantFP values in the source format.
  auto fp_const = [&](APFloat val) {
    bool losesInfo;
    val.convert(*fx_semantics, llvm::RoundingMode::NearestTiesToEven,
                &losesInfo);
    llvm::Constant* scalar_const = llvm::ConstantFP::get(context, val);
    if (fx_float_type->isVectorTy()) {
      auto vec_ty = llvm::cast<llvm::FixedVectorType>(fx_float_type);
  ...
  // This is the halfway point for the *largest* subnormal step (e.g., 7.5 * q).
  ...
  Value* scale_constant = fp_const(scale_apfloat);
  ...
  Value* scaled = b->CreateFMul(input_float, scale_constant);

  // Use llvm.nearbyint, which rounds to the nearest integer using ties-to-even.
  llvm::Module* module = b->GetInsertBlock()->getModule();
  ...
  Value* int_bucket = b->CreateFPToSI(rounded, ix_type);
