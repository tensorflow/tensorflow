...
llvm::Value* HandleHalfwayPointsFxToF8(PrimitiveType fx_type,
  ...
  CHECK(3 <= f8_exponent_bits && f8_exponent_bits <= 4);
  ...
  llvm::Type* ix_type = b->getIntNTy(primitive_util::BitWidth(fx_type));
  ...
  int arr_sz;
  if (f8_exponent_bits == 4) {
    halfway_points = halfway_points_e4;
    arr_sz = 8;
  } else if (f8_exponent_bits == 3) {
    halfway_points = halfway_points_e3;
    arr_sz = 16;
  } else {
    LOG(FATAL) << "Unsupported F8 exponent bits: " << f8_exponent_bits;
  }
