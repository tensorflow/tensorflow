...
class BufferAssigner {
 ...
 private:
  ...
  // sequential instruction ordering. Instead of allocating them immediately,
  // they are collected in `buffers_to_assign_sequentially` (mapping computation
  // to the set of delayed HloValues) to be packed tightly using the Heap
  // Simulator in a subsequent pass. Returns true if assignment was delayed.
  ...
  // over a unified schedule for all global computations (Whole-Module
  // Simulation), which allows temporal memory reuse across computation
  // boundaries and reduces peak memory footprint. Otherwise, it runs on a
  // per-computation basis.
  ...
  // buffers. This compiles a unified global schedule, enabling the heap
  // simulator to reuse memory offsets between parent and sub-computations
  // (e.g. kWhile or kCall bodies) that do not overlap in lifetime, resulting
  // in a smaller overall memory footprint.
