...
Service::DeconstructTuple(const GlobalData& data) {
  TF_XLA_ASSIGN_OR_RETURN(std::vector<GlobalDataHandle> elements,
...
absl::Status Service::ValidateResultShape(const Shape& client_shape,
  ...
  TF_XLA_RETURN_IF_ERROR(ShapeUtil::ValidateShapeWithOptionalLayout(client_shape));
...
Service::ResolveAndValidateArguments(
  ...
  for (size_t i = 0; i < arguments.size(); ++i) {
    TF_XLA_ASSIGN_OR_RETURN(std::vector<const ShapedBuffer*> replicated_buffers,
...
Service::BuildExecutables(const HloModuleProto* module_proto,
  ...
  if (!run_backend_only) {
    TF_XLA_ASSIGN_OR_RETURN(executables,
...
absl::StatusOr<GlobalDataHandle> Service::ExecuteAndRegisterResult(
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto replicas, Replicas(*backend, device_handle));
  TF_RET_CHECK(!replicas.empty());
  for (se::StreamExecutor* executor : replicas) {
    TF_XLA_ASSIGN_OR_RETURN(StreamPool::Ptr stream,
  ...
  if (options_.number_of_replicas() == 1) {
    TF_XLA_ASSIGN_OR_RETURN(auto result, executable->ExecuteOnStreamWrapper(
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto results, executable->ExecuteOnStreams(
...
absl::StatusOr<std::vector<se::StreamExecutor*>> Service::GetExecutors(
  ...
  for (const auto& device_handle : execution_options.device_handles()) {
    TF_XLA_ASSIGN_OR_RETURN(auto replicas,
...
absl::StatusOr<std::vector<std::unique_ptr<GlobalData>>> Service::ExecuteGraph(
  ...
  // Get the executors.
  TF_XLA_ASSIGN_OR_RETURN(std::vector<se::StreamExecutor*> executors,
  ...
  if (executable_ptr->dumping_snapshot()) {
    *snapshot.mutable_hlo() = *executable_ptr->hlo_proto();
    TF_XLA_ASSIGN_OR_RETURN(auto stream, execute_backend_->BorrowStream(
  ...
  TF_XLA_RETURN_IF_ERROR(execution_status);
  ...
  if (executable_ptr->dumping_snapshot()) {
    TF_XLA_ASSIGN_OR_RETURN(const ShapedBuffer* result_buffer,
    ...
    TF_XLA_RETURN_IF_ERROR(RecordResult(*result_buffer, stream.get(),
...
absl::StatusOr<std::unique_ptr<Executable>> Service::BuildExecutable(
  ...
  if (!run_backend_only) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(module, backend->compiler()->RunHloPasses(
...
absl::StatusOr<ExecutionHandle> Service::Compile(
  ...
  TF_XLA_ASSIGN_OR_RETURN(std::unique_ptr<HloModuleConfig> module_config,
...
absl::StatusOr<std::unique_ptr<GlobalData>> Service::Execute(
  ...
  TF_XLA_ASSIGN_OR_RETURN(std::shared_ptr<Executable> executable,
...
absl::StatusOr<Literal> Service::TransferToClient(
  ...
  TF_XLA_ASSIGN_OR_RETURN(const ShapedBuffer* shaped_buffer,
...
absl::StatusOr<std::unique_ptr<GlobalData>> Service::TransferToServer(
  ...
  std::vector<se::StreamExecutor*> replicas;
  if (device_handle) {
    TF_XLA_ASSIGN_OR_RETURN(replicas, Replicas(*execute_backend_, *device_handle));
  } else {
    TF_XLA_ASSIGN_OR_RETURN(
  ...
  for (se::StreamExecutor* executor : replicas) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(auto stream, execute_backend_->BorrowStream(executor));
    TF_XLA_RETURN_IF_ERROR(
  ...
  TF_XLA_ASSIGN_OR_RETURN(GlobalDataHandle out,
...
absl::StatusOr<Literal> Service::ComputeConstantGraph(
  ...
  if (output_layout) {
    TF_XLA_RETURN_IF_ERROR(LayoutUtil::ValidateLayoutForShape(
  ...
  TF_XLA_RETURN_IF_ERROR(dynamic_padder.Run(module.get()).status());

  TF_XLA_ASSIGN_OR_RETURN(DynamicDimensionInference dynamic_dimension_inference,
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto result_literal, evaluator.Evaluate(*module, {}));
...
absl::StatusOr<Shape> Service::GetShape(const GlobalData& data) {
  TF_XLA_ASSIGN_OR_RETURN(const ShapedBuffer* buffer,
                      allocation_tracker_.ResolveForReplica(data.handle(), 0));
...
absl::StatusOr<std::vector<se::StreamExecutor*>> Service::Replicas(
  ...
  for (int replica = 0; replica < options_.number_of_replicas(); ++replica) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(auto executor, backend.stream_executor(device_ordinal));
