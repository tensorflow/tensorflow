...
class CopyRemover {
 public:
  ...
  void DfsValueSort(const HloComputation* comp,
                    const absl::flat_hash_map<const HloInstruction*,
                                              std::vector<const HloValue*>>&
                        instruction_to_values,
                    std::vector<const HloValue*>& sorted_values,
                    absl::flat_hash_set<const HloComputation*>& visited_comps) {
    if (visited_comps.contains(comp)) {
      VLOG(2) << "already visited: " << comp->name();
      return;
    }
    VLOG(2) << "visiting: " << comp->name();
    visited_comps.insert(comp);

    auto insert_instruction = [&](HloInstruction* instruction) {
      if (instruction_to_values.find(instruction) !=
          instruction_to_values.end()) {
        VLOG(2) << "found instruction: " << instruction->name() << " with "
                << instruction_to_values.at(instruction).size() << " values";
        sorted_values.insert(sorted_values.end(),
                             instruction_to_values.at(instruction).begin(),
                             instruction_to_values.at(instruction).end());
      }
    };
    for (HloInstruction* instruction : comp->parameter_instructions()) {
    ...
    for (HloInstruction* instruction : comp->MakeInstructionPostOrder()) {
      switch (instruction->opcode()) {
        case HloOpcode::kParameter:
          continue;
        case HloOpcode::kWhile:
          VLOG(2) << "visiting while: " << instruction->name();
          DfsValueSort(instruction->while_condition(), instruction_to_values,
                       sorted_values, visited_comps);
          DfsValueSort(instruction->while_body(), instruction_to_values,
                       sorted_values, visited_comps);
          break;
        case HloOpcode::kConditional:
          VLOG(2) << "visiting conditional: " << instruction->name();
          for (int i = 0; i < instruction->branch_count(); ++i) {
            DfsValueSort(instruction->branch_computation(i),
                         instruction_to_values, sorted_values, visited_comps);
          }
          break;
        case HloOpcode::kCall:
          VLOG(2) << "visiting call: " << instruction->name();
          for (HloComputation* called_computation :
               instruction->called_computations()) {
            DfsValueSort(called_computation, instruction_to_values,
                         sorted_values, visited_comps);
          }
          break;
        case HloOpcode::kFusion:
          VLOG(2) << "visiting fusion: " << instruction->name();
          DfsValueSort(instruction->fused_instructions_computation(),
                       instruction_to_values, sorted_values, visited_comps);
          break;
        case HloOpcode::kAsyncStart:
          VLOG(2) << "visiting async start: " << instruction->name();
          DfsValueSort(instruction->async_wrapped_computation(),
                       instruction_to_values, sorted_values, visited_comps);
          break;
        default:
          break;
  ...
  CopyRemover(const HloModule& module, const HloAliasAnalysis& alias_analysis,
    ...
    for (const HloBuffer& buffer : alias_analysis.buffers()) {
      ...
      for (const HloValue* value : values) {
        instruction_to_values[value->defining_instruction()].push_back(value);
      }
      absl::flat_hash_set<const HloComputation*> visited_comps;
      std::vector<const HloValue*> sorted_values;
      DfsValueSort(values.back()->defining_instruction()->parent(),
                   instruction_to_values, sorted_values, visited_comps);

      CHECK_EQ(sorted_values.size(), values.size());

      // Create a list containing all of the values in the buffer.
      AddValueList(sorted_values, &value_to_node);
    }
