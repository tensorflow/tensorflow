...
class CopyRemover {
 public:
  ...
  CopyRemover(const HloModule& module, const HloAliasAnalysis& alias_analysis,
    ...
    std::function<void(HloComputation*)> assign_ids_dfs =
        [&](HloComputation* computation) {
          ...
          for (HloInstruction* instruction :
            ...
            instruction_ids_[instruction->unique_id()] = id++;
          }
    ...
    for (const HloBuffer& buffer : alias_analysis.buffers()) {
      ...
      // Sort values based on instruction, which are post-ordered or ordered by
  ...
  bool IsOrderedBefore(const HloValue* a, const HloValue* b) {
    if (a == b) {
      return false;
    }
    return instruction_ids_.at(a->defining_instruction()->unique_id()) <
           instruction_ids_.at(b->defining_instruction()->unique_id());
  }
  ...
  bool IsCopyToFromHost(const HloInstruction* copy) {
  ...
  bool LiveRangeBefore(const ValueNode& a, const ValueNode& b) {
    if (a.uses.empty()) {
      ...
      return this->IsOrderedBefore(a.value, b.value);
    }
 ...
 private:
  ...
  // instructions. Used for ordering values in buffers.
  absl::flat_hash_map<int, int64_t> instruction_ids_;
