...
TEST_F(CollectivePipelinerTest, ForwardSinkAllReduceFeedingReduceScatter) {
  constexpr absl::string_view hlo_string = R"(
HloModule module
...
while_cond {
  param = (s32[], bf16[3,2,128], bf16[3,8,128]) parameter(0)
  gte = s32[] get-tuple-element(param), index=0
  constant.1 = s32[] constant(3)
  ROOT cmp = pred[] compare(gte, constant.1), direction=LT
}
...
while_body {
  param = (s32[], bf16[3,2,128], bf16[3,8,128]) parameter(0)
  get-tuple-element.394 = s32[] get-tuple-element(param), index=0
  get-tuple-element.395 = bf16[3,2,128] get-tuple-element(param), index=1
  get-tuple-element.35 = bf16[3,8,128] get-tuple-element(param), index=2
  constant.2557 = s32[] constant(1)
  add.230 = s32[] add(get-tuple-element.394, constant.2557)
  constant.2559 = s32[] constant(3)
  ...
  dynamic-slice.99 = bf16[1,8,128] dynamic-slice(get-tuple-element.35, select.1348, constant.2561, constant.2561), dynamic_slice_sizes={1,8,128}
  mul = bf16[1,8,128] multiply(dynamic-slice.99, dynamic-slice.99)
  ar.1 = bf16[1,8,128] all-reduce(mul), replica_groups={{0,1,2,3}}, to_apply=add
  rs.1 = bf16[1,2,128] reduce-scatter(ar.1), replica_groups={{0,1,2,3}}, dimensions={1}, to_apply=add
  %c = bf16[] custom-call(), custom_call_target="Boh"
  %b = bf16[1,2,128] broadcast(c), dimensions={}
  %a = bf16[1,2,128] add(rs.1, b)
  dynamic-update-slice.35 = bf16[3,2,128] dynamic-update-slice(get-tuple-element.395, a, select.1348, constant.2561, constant.2561)
  ROOT tuple = (s32[], bf16[3,2,128], bf16[3,8,128]) tuple(add.230, dynamic-update-slice.35, get-tuple-element.35), control-predecessors={select.1348}
}

ENTRY entry {
  c0 = s32[] constant(0)
  p0 = bf16[3,2,128] parameter(0)
  p1 = bf16[3,8,128] parameter(1)
  tuple = (s32[], bf16[3,2,128], bf16[3,8,128]) tuple(c0, p0, p1)
  while = (s32[], bf16[3,2,128], bf16[3,8,128]) while(tuple), condition=while_cond, body=while_body
  ROOT gte1 = bf16[3,2,128] get-tuple-element(while), index=1
}
)";
  auto module = ParseAndReturnUnverifiedModule(hlo_string, config_).value();
  EXPECT_TRUE(RunOptimizer(
                  ...
                  .value());
  XLA_VLOG_LINES(1, module->ToString());
  // Reduce-scatter is sunk outside the loop.
  const HloInstruction* reduce_scatter =
      FindInstruction(module.get(), HloOpcode::kReduceScatter);
  EXPECT_NE(reduce_scatter, nullptr);
  // Check that the reduce-scatter dimension has been incremented by 1.
  EXPECT_EQ(
      Cast<HloReduceScatterInstruction>(reduce_scatter)->scatter_dimension(),
      2);
  // The reduce-scatter should have the expanded shape with the loop dimension.
  EXPECT_EQ(reduce_scatter->shape().dimensions(0), 3);
}
...
while_cond {
  param = (s32[], s32[], bf16[14,8,128], bf16[14,8,128]) parameter(0)
  gte = s32[] get-tuple-element(param), index=0
  constant.1 = s32[] constant(14)
  ROOT cmp = pred[] compare(gte, constant.1), direction=LT
}
...
while_body {
  ...
  get-tuple-element.256161 = s32[] get-tuple-element(param), index=0
  constant.43667 = s32[] constant(1)
  get-tuple-element.250973 = s32[] get-tuple-element(param), index=1
  constant.43668 = s32[] constant(13)
  constant.31596.clone.5 = s32[] constant(1)
  multiply.6379 = s32[] multiply(get-tuple-element.256161, constant.31596.clone.5)
  add.36255 = s32[] add(get-tuple-element.250973, multiply.6379)
  sub.7624 = s32[] subtract(constant.43668, add.36255)
  add = s32[] add(get-tuple-element.256161, constant.43667)
  constant.2561 = s32[] constant(0)
  get-tuple-element.395 = bf16[14,8,128] get-tuple-element(param), index=2
  get-tuple-element.5 = bf16[14,8,128] get-tuple-element(param), index=3
  dynamic-slice.99 = bf16[1,8,128] dynamic-slice(get-tuple-element.5, sub.7624, constant.2561, constant.2561), dynamic_slice_sizes={1,8,128}
  mul = bf16[1,8,128] multiply(dynamic-slice.99, dynamic-slice.99)
  ar.1 = bf16[1,8,128] all-reduce(mul), replica_groups={}, to_apply=add, channel_id=1
  dynamic-update-slice.35 = bf16[14,8,128] dynamic-update-slice(get-tuple-element.395, ar.1, sub.7624, constant.2561, constant.2561)
  ROOT tuple = (s32[], s32[], bf16[14,8,128], bf16[14,8,128]) tuple(add, get-tuple-element.250973, dynamic-update-slice.35, get-tuple-element.5)
}

ENTRY entry {
  c0 = s32[] constant(0)
  p0 = bf16[14,8,128] parameter(0)
  tuple = (s32[], s32[], bf16[14,8,128], bf16[14,8,128]) tuple(c0, c0, p0, p0)
  while = (s32[], s32[], bf16[14,8,128], bf16[14,8,128]) while(tuple), condition=while_cond, body=while_body
  ROOT gte1 = bf16[14,8,128] get-tuple-element(while), index=2
}
)";
  auto module = ParseAndReturnUnverifiedModule(hlo_string, config_).value();
  EXPECT_TRUE(RunOptimizer(module.get(), /*last_run=*/true).value());
  ...
  const HloInstruction* index = sliced->operand(1);
  EXPECT_EQ(index->opcode(), HloOpcode::kGetTupleElement);
  EXPECT_EQ(index->tuple_index(), 4);
  const HloInstruction* while_inst = index->operand(0);
  EXPECT_EQ(while_inst->opcode(), HloOpcode::kWhile);
  const HloInstruction* while_root =
      while_inst->while_body()->root_instruction();
  EXPECT_EQ(while_root->opcode(), HloOpcode::kTuple);
  const HloInstruction* dyn_upd = while_root->operand(2);
  EXPECT_EQ(dyn_upd->opcode(), HloOpcode::kDynamicUpdateSlice);
  const HloInstruction* dyn_upd2 = dyn_upd->operand(0);
  EXPECT_EQ(dyn_upd2->opcode(), HloOpcode::kDynamicUpdateSlice);
  const HloInstruction* prev_ar = dyn_upd2->operand(1);
  ...
  EXPECT_EQ(get_tuple_value->opcode(), HloOpcode::kGetTupleElement);
  EXPECT_EQ(get_tuple_index->opcode(), HloOpcode::kGetTupleElement);
  EXPECT_EQ(get_tuple_value->tuple_index(), 2);
  EXPECT_EQ(get_tuple_index->tuple_index(), 4);
}

TEST_F(CollectivePipelinerTest, ForwardSinkSubtractIndexWithGTELeaf) {
  constexpr absl::string_view hlo_string = R"(
HloModule module
...
ENTRY entry {
  ...
  EXPECT_THAT(root, op::GetTupleElement(
                        op::Tuple(_, _, op::Reshape(op::AllReduce()), _)));
  const HloInstruction* sunk_all_reduce =
      root->operand(0)->operand(2)->operand(0);
  EXPECT_EQ(sunk_all_reduce->opcode(), HloOpcode::kAllReduce);
  EXPECT_THAT(sunk_all_reduce, op::AllReduce(op::GetTupleElement(op::While())));
  const HloInstruction* while_inst = sunk_all_reduce->operand(0)->operand(0);
  EXPECT_EQ(while_inst->opcode(), HloOpcode::kWhile);
  const HloInstruction* while_root =
      while_inst->while_body()->root_instruction();
  EXPECT_THAT(while_root, op::Tuple(_, _, _, _, op::DynamicUpdateSlice()));
...
TEST_F(CollectivePipelinerTest,
       NonFlatSupport_HoistedCallCreatesSharedCalledComputation) {
  constexpr absl::string_view hlo_string = R"(
HloModule module
...
foo {
  param = bf16[1,8,128] parameter(0)
  ROOT mul = bf16[1,8,128] multiply(param, param)
}
...
// CHECK: %[[CALL_INLOOP:.+]] = {{.*}} call({{.*}}), to_apply=%foo
while_body {
  param = (s32[], bf16[3,8,128], bf16[3,8,128]) parameter(0)
  get-tuple-element.394 = s32[] get-tuple-element(param), index=0
  get-tuple-element.395 = bf16[3,8,128] get-tuple-element(param), index=1
  ...
  select.1348 = s32[] select(compare.747, add.232, add.231)
  dynamic-slice.99 = bf16[1,8,128] dynamic-slice(get-tuple-element.5, select.1348, constant.2561, constant.2561), dynamic_slice_sizes={1,8,128}
  call.1 = bf16[1,8,128] call(dynamic-slice.99), to_apply=foo
  ar.1 = bf16[1,8,128] all-reduce(call.1), replica_groups={}, to_apply=add, channel_id=1
  dynamic-update-slice.35 = bf16[3,8,128] dynamic-update-slice(get-tuple-element.395, ar.1, select.1348, constant.2561, constant.2561)
  ROOT tuple = (s32[], bf16[3,8,128], bf16[3,8,128]) tuple(add.230, dynamic-update-slice.35, get-tuple-element.5)
}

// CHECK-LABEL: ENTRY %entry
// CHECK: %[[CALL_PROLOGUE:.+]] = {{.*}} call({{.*}}), to_apply=%foo
// CHECK: %[[WHILE:.+]] = {{.*}} while({{.*}}), condition=%while_cond{{.*}}, body=%while_body{{.*}}
// CHECK: %[[EPILOGUE_AR:.+]] = {{.*}} all-reduce({{.*}})
// CHECK: ROOT {{.*}} = {{.*}} dynamic-update-slice({{.*}}, %[[EPILOGUE_AR]], {{.*}})
...
ENTRY entry {
  ...
  ASSERT_OK_AND_ASSIGN(bool changed,
                       RunOptimizer(module.get(), /*last_run=*/true));
  EXPECT_TRUE(changed);
...
TEST_F(CollectivePipelinerTest,
       NonFlatSupport_AlreadyNonFlatCallGraphGetsMoreSharedCallers) {
  constexpr absl::string_view hlo_string = R"(
HloModule module
...
// CHECK: %[[WHILE:.+]] = {{.*}} while({{.*}}), condition=%while_cond{{.*}}, body=%while_body{{.*}}
// CHECK: %[[EPILOGUE_AR:.+]] = {{.*}} all-reduce({{.*}})
// CHECK: %[[DUS:.+]] = {{.*}} dynamic-update-slice({{.*}}, %[[EPILOGUE_AR]], {{.*}})
// CHECK: %[[CALL_ENTRY:.+]] = {{.*}} call({{.*}}), to_apply=%foo
// CHECK: ROOT {{.*}} = {{.*}} tuple(%[[DUS]], %[[CALL_ENTRY]])
// CHECK-NOT: call({{.*}}), to_apply=%foo
ENTRY entry {
  c0 = s32[] constant(0)
  p0 = bf16[3,8,128] parameter(0)
  slice_entry = bf16[1,8,128] slice(p0), slice={[0:1], [0:8], [0:128]}
  call_entry = bf16[1,8,128] call(slice_entry), to_apply=foo
  tuple = (s32[], bf16[3,8,128], bf16[3,8,128]) tuple(c0, p0, p0)
  while = (s32[], bf16[3,8,128], bf16[3,8,128]) while(tuple), condition=while_cond, body=while_body
  gte1 = bf16[3,8,128] get-tuple-element(while), index=1
  ROOT root_tuple = (bf16[3,8,128], bf16[1,8,128]) tuple(gte1, call_entry)
}
)";
  ASSERT_OK_AND_ASSIGN(auto module,
                       ParseAndReturnVerifiedModule(hlo_string, config_));
...
TEST_F(CollectivePipelinerTest,
       NonFlatSupport_TranslationIsolatedWithinRegularCallComputation) {
  constexpr absl::string_view hlo_string = R"(
HloModule module
...
// CHECK-LABEL: %while_wrapper_comp
// CHECK: %[[WHILE:.+]] = {{.*}} while({{.*}})
// CHECK: %[[EPILOGUE_AR:.+]] = {{.*}} all-reduce({{.*}})
// CHECK: ROOT {{.*}} = {{.*}} dynamic-update-slice({{.*}}, %[[EPILOGUE_AR]], {{.*}})
while_wrapper_comp {
...
// CHECK-LABEL: ENTRY %entry
// CHECK: ROOT {{.*}} = {{.*}} call({{.*}}), to_apply=%while_wrapper_comp
// CHECK-NOT: while
// CHECK-NOT: all-reduce
ENTRY entry {
  p0 = bf16[3,8,128] parameter(0)
  ROOT call = bf16[3,8,128] call(p0), to_apply=while_wrapper_comp
}
)";
  ASSERT_OK_AND_ASSIGN(auto module,
                       ParseAndReturnVerifiedModule(hlo_string, config_));
...
TEST_F(CollectivePipelinerTest,
       NonFlatSupport_TracingForCollectiveSubcomputationDoesNotAllowCalls) {
  constexpr absl::string_view hlo_string = R"(
HloModule module
...
id_comp {
  param = bf16[1,8,128] parameter(0)
  ROOT id = bf16[1,8,128] copy(param)
}
...
// CHECK-LABEL: %while_body
// CHECK: %[[AR:.+]] = {{.*}} all-reduce({{.*}})
// CHECK: %[[CALL:.+]] = {{.*}} call(%[[AR]]), to_apply=%id_comp
// CHECK: %[[DUS:.+]] = {{.*}} dynamic-update-slice({{.*}}, %[[CALL]], {{.*}})
while_body {
  param = (s32[], bf16[3,8,128], bf16[3,8,128]) parameter(0)
  get-tuple-element.394 = s32[] get-tuple-element(param), index=0
  get-tuple-element.395 = bf16[3,8,128] get-tuple-element(param), index=1
  ...
  mul = bf16[1,8,128] multiply(dynamic-slice.99, dynamic-slice.99)
  ar.1 = bf16[1,8,128] all-reduce(mul), replica_groups={}, to_apply=add, channel_id=1
  call_after_ar = bf16[1,8,128] call(ar.1), to_apply=id_comp
  dynamic-update-slice.35 = bf16[3,8,128] dynamic-update-slice(get-tuple-element.395, call_after_ar, select.1348, constant.2561, constant.2561)
  ROOT tuple = (s32[], bf16[3,8,128], bf16[3,8,128]) tuple(add.230, dynamic-update-slice.35, get-tuple-element.5)
}

// CHECK-LABEL: ENTRY %entry
// CHECK: ROOT {{.*}} = {{.*}} get-tuple-element({{.*}})
// CHECK-NOT: all-reduce
// CHECK-NOT: call({{.*}}), to_apply=%id_comp
ENTRY entry {
  c0 = s32[] constant(0)
  p0 = bf16[3,8,128] parameter(0)
  tuple = (s32[], bf16[3,8,128], bf16[3,8,128]) tuple(c0, p0, p0)
...
TEST_F(CollectivePipelinerTest,
       NonFlatSupport_SharedCallAcrossWhileBodiesOneHoistedOneNot) {
  constexpr absl::string_view hlo_string = R"(
HloModule module
...
// CHECK-LABEL: %while_body_2
// CHECK: %[[INLOOP_CALL2:.+]] = {{.*}} call({{.*}}), to_apply=%foo
// CHECK-LABEL: %while_body
// CHECK: %[[INLOOP_CALL1:.+]] = {{.*}} call({{.*}}), to_apply=%foo
while_body {
  param = (s32[], bf16[3,8,128], bf16[3,8,128]) parameter(0)
  get-tuple-element.394 = s32[] get-tuple-element(param), index=0
  get-tuple-element.395 = bf16[3,8,128] get-tuple-element(param), index=1
...
while_cond_2 {
  param = (s32[], bf16[1,8,128]) parameter(0)
  gte = s32[] get-tuple-element(param), index=0
  constant.1 = s32[] constant(3)
  ROOT cmp = pred[] compare(gte, constant.1), direction=LT
}
...
while_body_2 {
  ...
  gte.1 = bf16[1,8,128] get-tuple-element(param), index=1
  c.1 = s32[] constant(1)
  add.0 = s32[] add(gte.0, c.1)
  call.2 = bf16[1,8,128] call(gte.1), to_apply=foo
  ROOT tuple = (s32[], bf16[1,8,128]) tuple(add.0, call.2)
}

// CHECK-LABEL: ENTRY %entry
// CHECK: %[[PROLOGUE_CALL:.+]] = {{.*}} call({{.*}}), to_apply=%foo
// CHECK: %[[WHILE_1:.+]] = {{.*}} while({{.*}}), condition=%while_cond{{.*}}, body=%while_body{{.*}}
// CHECK: %[[EPILOGUE_AR:.+]] = {{.*}} all-reduce({{.*}})
// CHECK-NOT: call({{.*}}), to_apply=%foo
// CHECK: %[[WHILE_2:.+]] = {{.*}} while({{.*}}), condition=%while_cond_2, body=%while_body_2
// CHECK-NOT: call({{.*}}), to_apply=%foo
ENTRY entry {
  c0 = s32[] constant(0)
  p0 = bf16[3,8,128] parameter(0)
  p1 = bf16[1,8,128] parameter(1)
  tuple_1 = (s32[], bf16[3,8,128], bf16[3,8,128]) tuple(c0, p0, p0)
  while_1 = (s32[], bf16[3,8,128], bf16[3,8,128]) while(tuple_1), condition=while_cond, body=while_body
  tuple_2 = (s32[], bf16[1,8,128]) tuple(c0, p1)
  while_2 = (s32[], bf16[1,8,128]) while(tuple_2), condition=while_cond_2, body=while_body_2
  gte_1 = bf16[3,8,128] get-tuple-element(while_1), index=1
  gte_2 = bf16[1,8,128] get-tuple-element(while_2), index=1
  ROOT root_tuple = (bf16[3,8,128], bf16[1,8,128]) tuple(gte_1, gte_2)
}
)";
  ASSERT_OK_AND_ASSIGN(auto module,
                       ParseAndReturnVerifiedModule(hlo_string, config_));
