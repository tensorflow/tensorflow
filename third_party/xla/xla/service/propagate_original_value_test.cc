...
// CHECK: %[[ADD2:.*]] = f32[] add(%[[LHS]], %[[RHS2]]), origin={{[{]}}{"call.2/add"}
  ...
  ENTRY main () -> f32[] {
    lhs = f32[] constant(42)
    call.3 = f32[] call(f32[] lhs), to_apply=incr, origin={{"call.1"}}
    call.4 = f32[] call(f32[] lhs), to_apply=incr, origin={{"call.2"}}
    ROOT add = f32[] add(f32[] call.3, f32[] call.4)
  })";
...
// CHECK-LABEL:test
// CHECK: %[[LHS:.*]] =
// CHECK:  %[[RHS:.*]] = f32[] constant(2)
// CHECK: %[[ADD:.*]] = f32[] add(%[[LHS]], %[[RHS]])
