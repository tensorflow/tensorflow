...
class LayoutCanonicalizer : public HloModulePass {
 ...
 private:
  ...
  // layout will be descending. It adds the instruction and its major-to-minor
  // map to the map of all the canonicalized instructions. We lookup this map
  // later when canonicalizing operands of an instruction.
 ...
 private:
  ...
  absl::flat_hash_map<HloInstruction*, std::vector<int64_t>> original_layout_;
  absl::flat_hash_set<HloInstruction*> canonicalized_instrs_;
};
}  // namespace xla
