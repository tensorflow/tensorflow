...
bool IsBatchGather(const HloGatherInstruction* gather) {
  ...
  return dims.operand_batching_dims_size();
}
...
bool IsBatchScatter(const HloScatterInstruction* scatter) {
  ...
  return dims.input_batching_dims_size();
}
...
absl::StatusOr<HloInstruction*> NormalizeBatchGather(
  ...
  std::vector<int64_t> start_index_map(dims.start_index_map().begin(),
                                       dims.start_index_map().end());
  for (int64_t operand_batching_dim : dims.operand_batching_dims()) {
    start_index_map.push_back(operand_batching_dim);
  }
  // Update gather indices by adding fake batching iota dimensions.
  Shape iota_shape = gather_indices->shape();
  if (dims.index_vector_dim() == iota_shape.rank()) {
    iota_shape.add_dimensions(1);
  }
  iota_shape.set_dimensions(dims.index_vector_dim(), 1);
  std::vector<HloInstruction*> gather_indices_to_concat = {gather_indices};
  for (int64_t start_indices_batching_dim :
       dims.start_indices_batching_dims()) {
    gather_indices_to_concat.push_back(gather->parent()->AddInstruction(
        HloInstruction::CreateIota(iota_shape, start_indices_batching_dim)));
  }
  Shape concat_shape = iota_shape;
  concat_shape.set_dimensions(
      dims.index_vector_dim(),
      dims.start_indices_batching_dims_size() +
          (dims.index_vector_dim() == iota_shape.rank()
               ? 1
               : gather_indices->shape().dimensions(dims.index_vector_dim())));
  gather_indices = gather->AddInstruction(HloInstruction::CreateConcatenate(
      concat_shape, gather_indices_to_concat, dims.index_vector_dim()));
...
absl::StatusOr<HloInstruction*> NormalizeBatchScatter(
  ...
  std::vector<int64_t> scatter_dims_to_operand_dims(
      dims.scatter_dims_to_operand_dims().begin(),
      dims.scatter_dims_to_operand_dims().end());
  for (int64_t input_batching_dim : dims.input_batching_dims()) {
    scatter_dims_to_operand_dims.push_back(input_batching_dim);
  }
  // Update scatter indices by adding fake batching iota dimensions.
  Shape iota_shape = scatter_indices->shape();
  if (dims.index_vector_dim() == iota_shape.rank()) {
    iota_shape.add_dimensions(1);
  }
  iota_shape.set_dimensions(dims.index_vector_dim(), 1);
  std::vector<HloInstruction*> scatter_indices_to_concat = {scatter_indices};
  for (int64_t scatter_indices_batching_dim :
       dims.scatter_indices_batching_dims()) {
    scatter_indices_to_concat.push_back(scatter->parent()->AddInstruction(
        HloInstruction::CreateIota(iota_shape, scatter_indices_batching_dim)));
  }
  Shape concat_shape = iota_shape;
  concat_shape.set_dimensions(
      dims.index_vector_dim(),
      dims.scatter_indices_batching_dims_size() +
          (dims.index_vector_dim() == iota_shape.rank()
               ? 1
               : scatter_indices->shape().dimensions(dims.index_vector_dim())));
  scatter_indices = scatter->AddInstruction(HloInstruction::CreateConcatenate(
      concat_shape, scatter_indices_to_concat, dims.index_vector_dim()));
