...
absl::Status ScalesShapeVerifier(
  ...
  if (scale_operand->opcode() == HloOpcode::kConstant &&
      scale_operand->shape().dimensions().empty()) {
    ...
    if (scale_operand->shape().element_type() !=
      ...
      return absl::FailedPreconditionError(absl::StrFormat(
          "Scale operand '%s' has a different type than operand '%s'. %s vs "
          "%s in %s",
    ...
    if (primitive_util::IsFloatingPointType(operand->shape().element_type())) {
      if (!constant->literal().IsAllFloat(1.0)) {
        return absl::FailedPreconditionError(absl::StrFormat(
            "Scale operand %s of %s is not a scalar with value 1.0",
    ...
    if (!constant->literal().IsAll(1)) {
      return absl::FailedPreconditionError(absl::StrFormat(
          "Scale operand %s of %s is not a constant with value 1",
          scale_operand->name(), dot->ToString()));
    }
  }
