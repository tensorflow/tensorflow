...
int64_t CountWritersInUser(const HloInstruction* inst,
  ...
  if (dynamic_cast<const HloCallableInstruction*>(user) ||
    ...
    // Without interprocedural analysis, we assume if a buffer is passed into
    // a callable or a while loop, it is written there.
    return 1;
  }
...
// Before we reach here, we already verify that an alias pair of operand and
// result should be both buffers or both non-buffers. We further verify the
// following:
  ...
  for (auto* comp : module.computations()) {
    for (auto* inst : comp->instructions()) {
      if (inst->IsCustomCall(kPinCustomCallTarget)) {
        TF_RETURN_IF_ERROR(VerifyPin(Cast<HloCustomCallInstruction>(inst)));
      } else if (inst->IsCustomCall(kAllocateBufferCustomCallTarget)) {
        TF_RETURN_IF_ERROR(VerifyAllocateBuffer(inst));
      } else if (inst->IsCustomCall(kUnpinCustomCallTarget)) {
        TF_RETURN_IF_ERROR(VerifyUnpin(Cast<HloCustomCallInstruction>(inst)));
