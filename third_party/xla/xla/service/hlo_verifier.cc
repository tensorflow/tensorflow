...
absl::Status ShapeVerifier::VerifyAsyncStartAliasConfig(
    HloInstruction* async_op) {
  CHECK(async_op->opcode() == HloOpcode::kAsyncDone);
  HloComputation* computation = async_op->async_wrapped_computation();
  std::vector<const HloInstruction*> bound_operands =
      hlo_instruction_utils::async::GetAsyncBoundOperands(async_op);
  HloInstruction* async_start = async_op->async_chain_start();

  CHECK_EQ(computation->num_parameters(), bound_operands.size());
  ProgramShape computation_shape = computation->ComputeProgramShape();

  // Create a shape object that represents the expected output shape of the
  // operands of the async start instruction, based on the parameter and result
  // shapes of the wrapped computation.
  Shape async_operands_output_shape = async_start->shape();
  *async_operands_output_shape.mutable_tuple_shapes(0) =
      ShapeUtil::MakeTupleShape(computation_shape.parameters());
  *async_operands_output_shape.mutable_tuple_shapes(1) =
  ...
  for (const auto& pair : Cast<HloAsyncStartInstruction>(async_start)
    ...
    RET_CHECK(operand_nr < computation->num_parameters())
        << "Invalid aliasing operand index.";
    RET_CHECK(ShapeUtil::IndexIsValid(
        bound_operands[pair.second.first]->shape(), operand_index))
        << "Invalid aliasing operand shape index.";
    RET_CHECK(
        ShapeUtil::IndexIsValid(async_operands_output_shape, output_index))
        << "Invalid aliasing output shape index.";
    const Shape& output_subshape =
        ShapeUtil::GetSubshape(async_operands_output_shape, output_index);
    const Shape& operand_subshape = ShapeUtil::GetSubshape(
        bound_operands[operand_nr]->shape(), operand_index);
    if (opts_.layout_sensitive) {
      RET_CHECK(
...
absl::Status ShapeVerifier::CheckAsyncOpWithComputationShapes(
  ...
  if (!CheckShapePrefix(async_param_shape, compute_param_shape)) {
    return Internal(
        ...
        "shape: `%s` vs `%s`.",
        HloOpcodeString(async_op->opcode()),
        async_param_shape.ToString(/*print_layout=*/true),
...
absl::Status ShapeVerifier::CheckAsyncUpdateWithPrevAsyncOp(
  ...
  bool is_prefix = CheckShapePrefix(prev_async_op_shape.tuple_shapes(0),
...
absl::Status ShapeVerifier::HandleAsyncStart(HloInstruction* async_start) {
  RETURN_IF_ERROR(CheckAsyncOpWithComputationShapes(async_start));
  RETURN_IF_ERROR(CheckAsyncOpComputationThreadName(async_start));
  const Shape& param_shape = async_start->shape().tuple_shapes(0);
  // Check if the operands match the subshapes of the async (output) op.
...
absl::Status ShapeVerifier::HandleAsyncUpdate(HloInstruction* async_update) {
  ...
  RETURN_IF_ERROR(CheckAsyncOpWithComputationShapes(async_update));
  // there is some duplicated check here,
  // TODO(phui): refactor the code to remove duplicated checks here
  RETURN_IF_ERROR(CheckAsyncUpdateWithPrevAsyncOp(async_update));
  ...
  if (async_update->operand_count() > 1) {
    ...
    RET_CHECK(prev_async_op_shape.IsTuple() &&
              prev_async_op_shape.tuple_shapes().size() >= 2);
    RET_CHECK(curr_async_update_shape.IsTuple() &&
    ...
    RET_CHECK(prev_async_op_params_shape.IsTuple());
    RET_CHECK(curr_async_update_params_shape.IsTuple());
...
absl::Status ShapeVerifier::HandleAsyncDone(HloInstruction* async_done) {
  ...
  RETURN_IF_ERROR(VerifyAsyncStartAliasConfig(async_done));
  ...
  for (const HloComputation* computation : module.computations()) {
    for (const HloInstruction* instruction : computation->instructions()) {
      ...
      switch (instruction->opcode()) {
        ...
        case HloOpcode::kAsyncUpdate: {
          RET_CHECK(!instruction->operands().empty());
          const HloInstruction* operand = instruction->operand(0);
          RET_CHECK(operand->opcode() == HloOpcode::kAsyncStart ||
                    operand->opcode() == HloOpcode::kAsyncUpdate)
