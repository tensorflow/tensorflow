...
absl::StatusOr<HloInstruction*> TransformStartIndices(
  ...
  if (index_vector_dim == rank) {
    ...
    // implicit.
    TF_XLA_ASSIGN_OR_RETURN(indices,
  ...
  if (rank > 2) {
    TF_XLA_ASSIGN_OR_RETURN(indices, CollapseFirstNDims(indices, rank - 1));
  } else if (rank == 1) {
    TF_XLA_ASSIGN_OR_RETURN(indices, InsertDegenerateDims(indices, {0}));
  }
  return indices;
}
...
absl::StatusOr<HloInstruction*> MaybeTranspose(
  ...
  TF_XLA_ASSIGN_OR_RETURN(auto* result, MakeTransposeHlo(operand, permutation));
