...
void TransformFormattingOp(
  ...
  LOG(FATAL) << "Unsupported instruction " << formatting_op->ToString();
}
...
absl::Status TransformLoopForwardSink(const WhileLoopAnalysis& loop_analysis,
  ...
  for (auto& to_move : loop_analysis.GetMoveInfos()) {
    ...
    for (HloInstruction* formatting_op : to_move.formatting_ops) {
      TransformFormattingOp(formatting_op, to_move, loop_computation,
                            pipelined_map, to_add_batch_set, next_channel_id);
    }
