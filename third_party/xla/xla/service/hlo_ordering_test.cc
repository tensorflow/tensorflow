...
TEST_F(HloOrderingTest, IgnoreTokenUsersInOrdering) {
  const char* module_str = R"(
  HloModule Module

  condition  {
    param1 = (f32[100], f32[100], u32[], token[]) parameter(0)
    ROOT cond = pred[] constant(1)
  }

  body {
    param2 = (f32[100], f32[100], u32[], token[]) parameter(0)

    data = f32[100] get-tuple-element(param2), index=0

    param.gte0 = f32[100] get-tuple-element(param2), index=1
    param.copy0 = f32[100] copy(param.gte0)
    param.gte1 = u32[] get-tuple-element(param2), index=2
    param.copy1 = u32[] copy(param.gte1)
    param.gte2 = token[] get-tuple-element(param2), index=3
    send3.tuple = tuple(param.copy0, param.copy1, param.gte0)

    done3 = token[] send-done(send3.tuple), channel_id=1,
      frontend_attributes={
       _xla_send_recv_pipeline="0"
      }
    token4 = token[] after-all()
    send4 = (f32[100], u32[], token[]) send(data, token4), channel_id=1,
      frontend_attributes={
        _xla_send_recv_pipeline="0"
      }
    data2 = add(data, data)
    // Mimic the code generated by copy-insertion. In parcitular,
    // elements with non-token types are copied and control dependences are
    // added to represent the ordering; the element with a token type is not
    // copied, causing (send.gte2, param2) not ordered.
    send.gte0 = f32[100] get-tuple-element(send4), index=0
    send.copy0 = f32[100] copy(send.gte0), control-predecessors={param.copy0}
    send.gte1 = u32[] get-tuple-element(send4), index=1
    send.copy1 = u32[] copy(send.gte1), control-predecessors={param.copy1}
    send.gte2 = token[] get-tuple-element(send4), index=2
    send4.tuple = tuple(send.copy0, send.copy1, send.gte2)
    gte = f32[100] get-tuple-element(send4.tuple), index=0
    ROOT body = tuple(data2, gte, send.copy1, send.gte2)
  }

  ENTRY main {
    param0 = f32[100] parameter(0)
    token0 = token[] after-all()
    send0 = (f32[100], u32[], token[]) send(param0, token0), channel_id=1,
      frontend_attributes={
        _xla_send_recv_pipeline="0"
      }
    send0.gte0 = f32[100] get-tuple-element(send0), index=0
    send0.gte1 = u32[] get-tuple-element(send0), index=1
    send0.gte2 = token[] get-tuple-element(send0), index=2
    init = tuple(param0, send0.gte0, send0.gte1, send0.gte2)
    loop = while(init), condition=condition, body=body

    loop.gte0 = f32[100] get-tuple-element(loop), index=1
    loop.gte1 = u32[] get-tuple-element(loop), index=2
    loop.gte2 = token[] get-tuple-element(loop), index=3
    send1.tuple = tuple(loop.gte0, loop.gte1, loop.gte2)

    done0 = token[] send-done(send1.tuple), channel_id=1,
      frontend_attributes={
       _xla_send_recv_pipeline="0"
      }
    ROOT result = f32[100] get-tuple-element(loop), index=0
})";
  ...
  HloInstruction* add_2 = FindInstruction(module.get(), "send4.tuple");
  HloInstruction* add_1 = FindInstruction(module.get(), "param2");

  EXPECT_TRUE(ordering.LiveRangeStrictlyBefore(
      dataflow->GetValueDefinedAt(add_1), dataflow->GetValueDefinedAt(add_2),
      *dataflow, true));
}
}  // namespace
}  // namespace xla
