...
absl::StatusOr<bool> CombineConstants(HloComputation* computation,
  ...
  while (inst_it != computation->instructions().end()) {
    ...
    if (only_scalar && !ShapeUtil::IsScalar(instruction->shape())) {
      continue;
    }
...
StatusOr<bool> HloCSE::Run(
  ...
  for (auto* computation : module->computations(execution_threads)) {
    ...
    TF_ASSIGN_OR_RETURN(
        bool combined,
        is_layout_sensitive_
            ? CombineConstants<true>(computation, only_scalar_)
            : CombineConstants<false>(computation, only_scalar_));
    changed |= combined;
    ...
    for (auto instruction : computation->MakeInstructionPostOrder()) {
      ...
      if (only_scalar_ && !ShapeUtil::IsScalar(instruction->shape())) {
        continue;
      }
