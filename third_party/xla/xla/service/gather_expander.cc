...
std::vector<HloInstruction*> GenerateExplicitBatchDimIndices(
  ...
  auto add_explicit_batch_dim_index = [&](int64_t start_indices_dim,
                                          HloInstruction* index) {
    absl::Span<const int64_t> start_indices_batching_dims =
        dim_numbers.start_indices_batching_dims();
    auto it = absl::c_find(start_indices_batching_dims, start_indices_dim);
    CHECK_NE(it, start_indices_batching_dims.end());
  ...
  for (int64_t i = start_indices_shape.dimensions_size() - 1; i >= 0; i--) {
    ...
    if (num_batch_dims == 0) {
      if (absl::c_linear_search(dim_numbers.start_indices_batching_dims(), i)) {
        // Avoid generating a remainder that just returns the divident itself.
        add_explicit_batch_dim_index(i, divident);
      }
      break;
    }
    ...
    if (absl::c_linear_search(dim_numbers.start_indices_batching_dims(), i)) {
      add_explicit_batch_dim_index(
          i, computation->AddInstruction(HloInstruction::CreateBinary(
