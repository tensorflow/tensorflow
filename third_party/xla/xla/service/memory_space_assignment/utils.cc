...
double GetInstructionUniformRandom(
    const HloInstruction& instruction,
    const InstructionIdentifierOptions& instruction_identifier_options,
    int64_t seed) {
  std::string instruction_identifier;
  switch (instruction_identifier_options.options_case()) {
    case InstructionIdentifierOptions::OPTIONS_NOT_SET:
      // We need a default value, and the fingerprint seems the most logical.
      // The alternative is just dieing, and that seems like a worse experience
      // for users that just want to try things out.
    case InstructionIdentifierOptions::kUseFingerprint:
      instruction_identifier =
          instruction.ToString(HloPrintOptions::Fingerprint());
      break;
    case InstructionIdentifierOptions::kUseToString:
      instruction_identifier = instruction.ToString();
      break;
    case InstructionIdentifierOptions::kUseSignatureString:
      instruction_identifier = instruction.SignatureString();
      break;
    case InstructionIdentifierOptions::kUseName:
      instruction_identifier = instruction.name();
      break;
  }
  ...
  //
  // We add this anyway to make it clear we're looking for a random number.
...
bool MemorySpaceAssignmentUtils::DoesInstructionMatchRandomFilter(
  ...
  double rnd = GetInstructionUniformRandom(
      instruction, hlo_random_filter.instruction_identifier_options(), seed);
