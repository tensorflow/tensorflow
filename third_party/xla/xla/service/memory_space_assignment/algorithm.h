...
class MsaAlgorithm : public GlobalDecreasingSizeBestFitHeap<HloValue> {
 ...
 private:
  ...
  // Whether an HloBuffer is pinned to alternate memory for its entire live
  // range.
  bool IsBufferPinnedToAlternateMemory(const HloBuffer& buffer) const;
  ...
  // List of reserved allocations for pinned buffers. We reserve the entire
  // live range of a pinned buffer in alternate memory to make repacking
  // simpler.
  std::vector<std::unique_ptr<ReservedAllocation>>
      reserved_allocations_for_pinned_buffers_;

  // Maps a pinned buffer allocation to the value allocations that are assigned
  // to it. It is used to update the value allocations when the pinned buffer
  // allocation is updated, for instance, when the pinned buffer allocation is
  // imported after repacking.
