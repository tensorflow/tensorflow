...
class AllocationValue {
 public:
  ...
  struct Use {
    ...
    // A synchronous memory call operand of this use.
    // TODO(mehrdadk): extend this to support multiple sync data movement
    // operands.
    HloInstruction* sync_mem_call_operand = nullptr;
...
struct AllocationSegmentContext {
  ...
  // The index of the use that we are processing in the AllocationValue's uses
  // vector.
  int use_idx;
  // The index of the AllocationValue that we are going to enqueue the resulting
  // allocation to its allocation sequence.
...
class MsaAlgorithm : public GlobalDecreasingSizeBestFitHeap<HloValue> {
 ...
 private:
  ...
  struct AllocationRequest {
    ...
    // The resulting Allocation will be enqueued in queuing_allocation_value's
    // AllocationSequence.
  ...
  AsyncConversionResult IsReplaceableSyncSliceCandidate(
  ...
  // Allocation to queuing_allocation_value's AllocationSequece. When
  // is_extension_only is true, no new Allocations will be created while
  // processing the resulting AllocationRequest, and we only need to extend an
  // existing Allocation's end_time.
  ...
  // Adds an asynchronous copy to allocations. When sync_instruction is set, we
  // will insert the async version of sync_instruction instead and
  // sync_instruction will be removed during the final Allocation::Process()
  // call.
  void AddAsyncCopy(
  ...
  // Returns true if we are trying to replace instruction with its async version
  // while joint-processing the latest set of allocation values
  ...
  // Updates the given JointAllocationProposal with the values, allocation
  // values, and colocated intervals that are proposed to be processed jointly
  // for the given interval. Also, if the consumes or produces any synchronous
  // memory call instructions (e.g., kCopy, kSlice) and the option to replace
  // them with their asynchronous versions is enabled, this method will add
  // those instructions to the sorted_async_conversion_candidates_ vector.
