...
class MsaAlgorithm : public GlobalDecreasingSizeBestFitHeap<HloValue> {
 ...
 private:
  ...
  void CreatePinnedAllocationsInAltMemoryForPositions(
      ...
      // allocation (can be used to extend an allocation's end time). When
      // nullopt (the default), the position's own exact true live range end
      // time computed by LiveRangeCalculator is used. For scalar buffer pinning
      // in alternate memory, we rely on exact true live ranges rather than
      // extending allocations to the end of the program.
  ...
  // buffer's positions in the alternate memory using its exact live range.
