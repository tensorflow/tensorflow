...
class MsaAlgorithm : public GlobalDecreasingSizeBestFitHeap<HloValue> {
 ...
 private:
  ...
  struct SegmentConstraints {
    ...
    std::optional<MemorySpace> start_space() const {
      return start_assignment.has_value()
                 ? std::make_optional(start_assignment->memory_space)
                 : std::nullopt;
    }
    std::optional<MemorySpace> end_space() const {
      return end_assignment.has_value()
                 ? std::make_optional(end_assignment->memory_space)
                 : std::nullopt;
    }
  };
  ...
  void ApplyStartConstraint(const SegmentConstraints& constraints,
                            AllocationRequest& request);
