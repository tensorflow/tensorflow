...
class MsaAlgorithm : public GlobalDecreasingSizeBestFitHeap<HloValue> {
 ...
 private:
  ...
  // Whether an hlo position is allowed in alternate memory.
  bool IsPositionAllowedInAlternateMemory(const HloPosition& position) const;

  // Whether an hlo value is allowed in alternate memory.
  bool IsValueAllowedInAlternateMemory(const HloValue& hlo_value) const;
