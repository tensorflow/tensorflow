...
ENTRY entry {
  ...
  BitcastSplitFn split_fn = nullptr;
  ASSERT_OK(pinned.UpdateUses(module->entry_computation(), vmem_p0, split_fn,
                              *hlo_live_range, *alias_analysis));
...
ENTRY entry {
  ...
  BitcastSplitFn split_fn = nullptr;
  ASSERT_OK(pinned.UpdateUses(module->entry_computation(), flat_p0, split_fn,
                              *hlo_live_range, *alias_analysis));
...
ENTRY entry {
  ...
  CustomFusionChunkSizingFn chunk_sizing_fn = [](const HloInstruction*, int64_t)
    ...
    sizing.num_slices = 4;
    sizing.slice_bytes = 256;
    sizing.max_slices_per_chunk = 2;
    sizing.chunk_size_bytes = 512;
    sizing.double_buffered_staging_bytes = 1024;
    return sizing;
  };
