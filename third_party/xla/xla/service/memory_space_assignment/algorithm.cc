...
MsaAlgorithm::SegmentConstraints MsaAlgorithm::GetSegmentConstraints(
  ...
  if (constraints.start_assignment.has_value()) {
    constraints.start_space = constraints.start_assignment->memory_space;
  }
  ...
  if (constraints.end_assignment.has_value()) {
    constraints.end_space = constraints.end_assignment->memory_space;
  }
  return constraints;
}
...
void MsaAlgorithm::ApplyStartConstraint(AllocationRequest& request,
...
std::optional<AllocationResult> MsaAlgorithm::TryPrefetch(
    AllocationRequest& request, const SegmentConstraints& constraints,
  ...
  if ((request.allow_prefetch ||
    ...
    AllocationResult prefetch_result =
        Prefetch(request, prev_default_allocation,
                 /*force_prefetch=*/
                 request.require_end_colored_in_alternate_memory);
...
std::optional<AllocationResult> MsaAlgorithm::HandlePrefetchFailure(
...
void MsaAlgorithm::FallbackToDefaultMemory(
    AllocationRequest& request, Allocation& prev_default_allocation) {
  // If a copy wasn't inserted, then add this use to the latest allocation in
  // default memory.
  prev_default_allocation.Extend(request.end_time);
  prev_default_allocation.AddUse(request.use->hlo_use);
...
AllocationResult MsaAlgorithm::AllocateSegment(AllocationRequest& request) {
  ...
  if (auto prefetch_result =
          TryPrefetch(request, constraints, *prev_allocation_in_default_mem)) {
    if (*prefetch_result == AllocationResult::kSuccess) {
      return AllocationResult::kSuccess;
    }
  ...
  if (auto failure =
          HandlePrefetchFailure(request, constraints, allocation_result)) {
    return *failure;
  }

  FallbackToDefaultMemory(request, *prev_allocation_in_default_mem);
  return allocation_result;
}
