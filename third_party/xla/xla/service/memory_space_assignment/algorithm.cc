...
MsaAlgorithm::MsaAlgorithm(HloModule* module, AllocationSequence* allocations,
  ...
  if (options.cost_analysis) {
    ...
    int64_t last_bandwidth_limiting_async_done_found_sofar = -1;
    float current_bw_reduction_factor = 1.0f;
    for (int i = 0; i < flattened_instructions.size(); ++i) {
      const HloInstruction* inst = flattened_instructions[i];
      std::optional<float> bw_reduction_factor =
          options.async_instruction_bw_reduction_factor_fn(inst);
      if (bw_reduction_factor.has_value()) {
        CHECK_EQ(inst->users().size(), 1);
        current_bw_reduction_factor = bw_reduction_factor.value();
        last_bandwidth_limiting_async_done_found_sofar = std::max(
            last_bandwidth_limiting_async_done_found_sofar,
      ...
      if (i <= last_bandwidth_limiting_async_done_found_sofar) {
        ...
        initial_resources[i] *= current_bw_reduction_factor;
      }
