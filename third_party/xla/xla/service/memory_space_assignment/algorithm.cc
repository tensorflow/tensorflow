...
absl::Status MsaAlgorithm::PinScalarBuffersInAlternateMemory(
  ...
  const auto& instruction_schedule = hlo_live_range_.instruction_schedule();
  // We collect and sort candidate buffers deterministically by buffer ID before
  // processing chunk allocations to prevent memory layout non-determinism
...
absl::Status MsaAlgorithm::PinScalarBufferInAlternateMemory(
  ...
  //    using their exact true live ranges (passing last_position_end_time =
  //    nullopt rather than extending to the end of the program).
  ...
  // so all real positions (starting at index 0) get pinned allocations using
  // their exact live ranges (last_position_end_time = nullopt). Note: a
  // scalar buffer is guaranteed to have at least one real non-trivial position
  // because its definition position is always non-trivial.
  CreatePinnedAllocationsInAltMemoryForPositions(
...
absl::StatusOr<HeapSimulator::Result<HloValue>> MsaAlgorithm::Finish() {
  ...
  for (MsaBufferInterval& interval : sorted_buffer_intervals) {
    ...
    // we only process unfinalized scalar buffers here. As confirmed during
    // review, the buffer in buffer interval is never a tuple shape.
