...
MsaAlgorithm::IsAsyncConversionSliceCandidate(
  ...
  if (instruction->IsCustomFusion() &&
      options_.custom_fusion_block_prefetch_operand_index_fn(instruction)
          .has_value()) {
    CHECK(IsBlockPrefetchingEnabled())
        << "Please enable block prefetching for custom fusion block "
           "prefetching.";
    CHECK(options_.enable_sync_slice_replacement)
        << "Please enable sync slice replacement for custom fusion block "
           "prefetching.";
...
void MsaAlgorithm::AnalyzeBlockPrefetchPosition(
  ...
  // TODO(subhankarshah): Allow block prefetching if the buffer is not the
  // output of the conditional.
  ...
  for (auto& [dest_buffer, src_buffer] :
    ...
    if (block_prefetched_buffer_it == buffers_to_block_prefetch.end()) {
      ...
      async_conv_candidate_instructions.insert(
          buffer_to_first_defining_value[dest_buffer]->instruction());
    ...
    if (async_conversion_candidate_non_trivial_source_position.at(
            dest_buffer) !=
        buffer_to_first_defining_value[src_buffer]->defining_position()) {
      VLOG(3) << "Not block prefetching an async conversion candidate buffer "
...
MsaAlgorithm::BlockPrefetchContext MsaAlgorithm::BuildBlockPrefetchingContext(
  ...
  absl::c_sort(block_prefetched_buffers_list, [&](const HloBuffer* a,
                                                  const HloBuffer* b) {
    return std::forward_as_tuple(buffer_to_first_use_time[a], a->id()) <
           std::forward_as_tuple(buffer_to_first_use_time[b], b->id());
  });
...
absl::StatusOr<Decision> MsaAlgorithm::BlockPrefetchBuffer(
  ...
  if (is_async_conversion_candidate) {
    ...
    // Calculate the latest scheduling time of its non-constant source operands,
    // ensuring we don't start prefetching before the inputs are available.
    int64_t latest_source_operand_time = std::numeric_limits<int64_t>::min();
    for (HloInstruction* operand :
         first_defining_value->instruction()->mutable_operands()) {
    ...
    if (latest_source_operand_time != std::numeric_limits<int64_t>::min()) {
      earliest_start_time =
          std::min(earliest_start_time, latest_source_operand_time);
    }
  }
