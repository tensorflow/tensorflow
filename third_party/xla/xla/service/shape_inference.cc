...
absl::Status ValidatePermuteMode(const Shape& operand_shape,
  ...
  // Every element of a shuffled dimension is moved on its own, so it needs its
  // own coordinates; elements along any other dimension are moved alike and may
  // share coordinates, which a size of 1 that is broadcast expresses.
