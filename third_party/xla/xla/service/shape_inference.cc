  ...
  switch (fft_type) {
    ...
    case RFFT: {
      ...
      if (fft_length[fft_rank - 1] != 0) {
        result.set_dimensions(result.dimensions().size() - 1,
                              fft_length[fft_rank - 1] / 2 + 1);
        if (in.is_unbounded_dynamic_dimension(result.dimensions().size() - 1)) {
          result.set_dynamic_dimension(result.dimensions().size() - 1, false);
        }
      }
      return result;
    }
