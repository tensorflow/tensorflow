...
void CopyRemover::AddViewUses(const HloValue* value, ValueNode* node) {
  ...
  // value, so a producer of views must never let one be written through on
  // the read side. The one such user the TPU backend emits is the write side
  // wrapper, whose in place write of the viewed buffer is also a dataflow use
  // of that buffer (the dynamic-update-slice it feeds), which orders it.
