...
class HloRunnerLegacy : public HloRunnerInterface {
 public:
  ...
  // `HloRunnerLegacy` in a program without any backends linked in.
  Backend& backend();
 ...
 private:
  ...
  // Calls `UpdateEntryComputationLayout` if HloRunnerLegacy has not called it
  // on the module before. This method is called before the module is executed.
  // The reason `UpdateEntryComputationLayout` is only called once instead of
  // every time is to avoid one thread updating the layout while another thread
  // is reading it during execution.
