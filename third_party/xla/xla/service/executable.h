...
class Executable {
 public:
  ...
  virtual ComputationLayout entry_computation_layout() const {
    CHECK(hlo_module_ != nullptr);
    return hlo_module_->config().entry_computation_layout();
  }

  virtual std::string name() const {
    return has_module() ? module().name() : "<unknown executable>";
  }
