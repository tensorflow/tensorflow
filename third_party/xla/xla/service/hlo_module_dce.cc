...
absl::StatusOr<bool> RunWhileDCE(
  ...
  for (auto* computation : module->computations(execution_threads)) {
    for (auto* instruction : computation->instructions()) {
      ...
      for (int64_t i = 0; i < tuple_element_count; ++i) {
        ...
        RETURN_IF_ERROR(while_body_root->ReplaceOperandWith(i, pass_thru_gte));
        changed = true;
  ...
  for (auto* while_body_comp : while_body_comps_to_dce) {
    ASSIGN_OR_RETURN(bool changed_for_computation,
...
absl::StatusOr<bool> HloModuleDCE::RunImpl(
  ...
  ASSIGN_OR_RETURN(liveness, HloLivenessAnalysis::Run(*module));
  ...
  ASSIGN_OR_RETURN(bool hlo_module_dce_changed,
