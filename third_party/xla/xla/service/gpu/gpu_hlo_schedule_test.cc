...
TEST_F(IsUnifiedAnalyticalModelEnabledTest, EnabledBySolEstimatorFlag) {
  HloModuleConfig config;
...
TEST_F(IsUnifiedAnalyticalModelEnabledTest,
       EnabledByMultihostHopperAndSupportedCollectives) {
  HloModuleConfig config;
...
TEST_F(IsUnifiedAnalyticalModelEnabledTest,
       DisabledForMultihostHopperWithUnsupportedCollective) {
  HloModuleConfig config;
  config.set_num_partitions(16);
  config.set_replica_count(1);
...
TEST_F(IsUnifiedAnalyticalModelEnabledTest,
       DisabledForMultihostHopperWithMixedCollectives) {
  HloModuleConfig config;
  config.set_num_partitions(16);
  config.set_replica_count(1);
  gpu_device_info_.set_cuda_compute_capability(9, 0);  // Hopper
