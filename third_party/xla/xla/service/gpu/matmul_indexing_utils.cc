...
absl::StatusOr<Shape> DotOperandDims::IntoOutputShape(
  ...
  Shape output_shape = ShapeUtil::MakeShape(element_type, output_dimensions);
...
absl::StatusOr<int64_t> DotOperandDims::LocalIndex(Category category,
                                                   int64_t dim) const {
  const auto& dims = dim_numbers_[category];
  auto iter = absl::c_find_if(dims, [&](int64_t d) { return d == dim; });
