...
// Floating-point sort: ascending.
TEST_F(GpuSortRewriterTest, SortFloatingPointAscending) {
  constexpr char kHlo[] = R"(
HloModule TestModule
...
%compare {
  %lhs = f32[] parameter(0)
  %rhs = f32[] parameter(1)
  %lcmp = pred[] compare(%lhs, %rhs), direction=LT
  %rcmp = pred[] compare(%rhs, %lhs), direction=LT
  %nan = pred[] compare(%lcmp, %rcmp), direction=EQ
  %cst = pred[] constant(false)
  ROOT %cmp = pred[] select(%nan, %cst, %lcmp)
}
...
// Floating-point sort: descending.
TEST_F(GpuSortRewriterTest, SortFloatingPointDescending) {
  constexpr char kHlo[] = R"(
HloModule TestModule
...
%compare {
  %lhs = f32[] parameter(0)
  %rhs = f32[] parameter(1)
  %lcmp = pred[] compare(%lhs, %rhs), direction=GT
  %rcmp = pred[] compare(%rhs, %lhs), direction=GT
  %nan = pred[] compare(%lcmp, %rcmp), direction=EQ
  %cst = pred[] constant(true)
  ROOT %cmp = pred[] select(%nan, %cst, %lcmp)
}
