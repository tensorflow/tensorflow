...
TEST_F(CommandBufferTest, DISABLED_ForLoop) {
  constexpr absl::string_view hlo_text = R"(
  HloModule m, is_scheduled=true
  ...
  command_buffer {
    ...
    ROOT while = while(p0), condition=cond, body=body,
                 backend_config={"known_trip_count":{"n":"20"}}
  }
  ...
  // Because we set the known trip count to 20, the loop will execute 20 times,
  // and it will ignore the `cond` result, which would terminate the loop after
  // 10 iterations (see WhileLoop test below that runs the real while loop).
  Literal expected_cnt = LiteralUtil::CreateR0<int32_t>(20);
  Literal expected_value = LiteralUtil::CreateR0<float>(40.0);
