...
absl::StatusOr<std::unique_ptr<GpuExecutable>> GpuExecutable::Create(
  ...
  TF_XLA_RETURN_IF_ERROR(RunThunkPasses(
...
absl::Status ExecuteThunksImpl(
  ...
  if (run_options->HasStreamBorrower()) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(borrowed_command_buffer_trace_stream,
  ...
  if (!execution_stream_ids.empty()) {
    if (run_options->HasStreamBorrower()) {
      TF_XLA_ASSIGN_OR_RETURN(additional_streams, run_options->BorrowStreams(
  ...
  if (ExecutionProfile* profile =
      ...
      profile) {
    TF_XLA_ASSIGN_OR_RETURN(execution_timer, main_stream->CreateEventBasedTimer(
                                             profile->warmup_run_executed()));
  }

  // Parameters for executing collective operations.
  TF_XLA_ASSIGN_OR_RETURN(Thunk::CollectiveExecuteParams collective_params,
  ...
  {  // Initialize thunks using prepared resources before execution.
    ...
    TF_XLA_RETURN_IF_ERROR(thunk_sequence.Initialize(initialize_params));
  }
  ...
  TF_XLA_RETURN_IF_ERROR(thunk_sequence.ExecuteOnStream(execute_params));
  VLOG(1) << "[" << run_options->device_ordinal() << "] "
          << "End GpuExecutable::ExecuteOnStream module: " << module_name;

  if (collective_params.need_barrier) {
    TF_XLA_RETURN_IF_ERROR(BarrierAfterExecutable(debug_options, main_stream));
  }
...
GpuExecutable::ResolveConstantGlobals(se::Stream* stream) {
  ...
  if (!(executor->GetPlatform()->id() ==
    ...
    TF_XLA_ASSIGN_OR_RETURN(module_handle, executor->LoadModule(module_spec));
  }
  ...
  for (const ConstantInfo& info : constants_) {
    ...
    if (static_cast<bool>(module_handle) && global_status.ok()) {
      ...
      if (!info.content.span().empty()) {
        ...
        TF_XLA_RETURN_IF_ERROR(stream->Memcpy(&global, info.content.span().data(),
      ...
      TF_XLA_ASSIGN_OR_RETURN(auto shared, executor->CreateOrShareConstant(
  ...
  for (int64_t i = 0; i < num_buffers; ++i) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(
        ...
        BufferForAllocation(arguments, globals, allocation, memory_allocator,
                            device_ordinal, i));
    TF_XLA_RETURN_IF_ERROR(CheckAlignment(allocation, buffers.back(), i));
  }
...
absl::StatusOr<ExecutionOutput> GpuExecutable::ExecuteAsyncOnStreamImpl(
  ...
  const GpuExecutable::BufferAllocToDeviceMemoryMap* globals;
  {
    ...
    TF_XLA_ASSIGN_OR_RETURN(globals, ResolveConstantGlobals(run_options->stream()));
  }
    ...
    if (output_info.alias_config) {
      ...
      if (maybe_owning_memory && maybe_owning_memory->HasOwnership()) {
        ...
        *maybe_owning_memory = argument_buffer;
        ...
        TF_XLA_RETURN_IF_ERROR(run_options->stream()->MemcpyD2D(
  ...
  TF_XLA_RETURN_IF_ERROR(ExecuteThunks(buffer_allocations, run_options));
...
absl::Status GpuExecutable::ExecuteThunks(
  ...
  TF_XLA_RETURN_IF_ERROR(ExecuteThunksImpl(
...
absl::StatusOr<GpuExecutableProto> GpuExecutable::ToProto() const {
  ...
  *proto.mutable_gpu_compute_capability() = gpu_version_.ToProto();
  ...
  TF_XLA_ASSIGN_OR_RETURN(*proto.mutable_thunk(), thunks_->ToProto());
...
absl::StatusOr<std::unique_ptr<GpuExecutable>> GpuExecutable::FromProto(
  ...
  for (const auto& output_info_proto : proto.output_info_map()) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(OutputInfo output_info,
  ...
  TF_XLA_ASSIGN_OR_RETURN(params.program_shape,
