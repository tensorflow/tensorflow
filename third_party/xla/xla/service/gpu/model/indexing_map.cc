...
static RTVarOptimizationResult OptimizeRTVar(
  ...
  while (true) {
    ...
    if (rt_var.hlo->opcode() == HloOpcode::kAdd ||
      ...
      if (!lhs.expr.isFunctionOfSymbol(symbol_index)) {
        // This means that lhs is something constant
        result_expr = result_expr.replace(symbol, apply_op(lhs.expr, symbol));
      ...
      if (!rhs.expr.isFunctionOfSymbol(symbol_index)) {
        // This means that rhs is something constant
        result_expr = result_expr.replace(symbol, apply_op(symbol, rhs.expr));
        // We can also take advantage of the optimization already done for lhs:
        result_expr = result_expr.replace(symbol, lhs.expr);
        rt_var.hlo = lhs.rt_var.hlo;
        continue;
      }
    }
...
bool IndexingMap::ReplaceConstantRTVars(
  ...
  for (auto index = 0; index < rt_vars_.size(); ++index) {
    ...
    if (result.expr != rt_var_symbol) {
      affine_map_ = affine_map_.replace({{rt_var_symbol, result.expr}});

      llvm::DenseMap<AffineExpr, AffineExpr> replacements;

      for (const auto& [constraint, interval] : constraints_) {
        auto modified_constraint =
            constraint.replace(rt_var_symbol, result.expr);
    ...
    if (result.expr.isFunctionOfSymbol(symbol_index)) {
