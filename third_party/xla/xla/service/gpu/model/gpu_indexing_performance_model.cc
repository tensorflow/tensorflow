...
//
// It computes the FLOPs for the reduction combiner (the called computation)
// and scales it by:
// 1. The number of combiner applications needed per output element (handling
//    both tiled loop reductions and untiled reductions).
// 2. The number of output elements processed per block in parallel.
// 3. The total number of thread blocks executing this HLO.
template <typename TiledHloInstructionType>
int64_t GetReduceFlops(
  ...
  int64_t reduce_dim = hlo->dimensions()[0];
  ...
  if (reduce_tile_size < reduce_dim_size) {
    int64_t loop_iterations = CeilOfRatio(reduce_dim_size, reduce_tile_size);
    ops_per_output_element =
        loop_iterations * reduce_tile_size + reduce_tile_size - 1;
  } else {
    ops_per_output_element = reduce_dim_size - 1;
  }
