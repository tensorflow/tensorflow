...
tsl::Future<TopKTiledRunTimeDataOrError> TryFindTopKBestTilingsWithTilingSpace(
    ...
    const se::DeviceDescription& device_info,
    HloCostAnalysis::ShapeSizeFunction shape_size) {
  // Candidates evaluated on a real executor must not share `mlir_context`:
  // SymbolicExpr uniques through mlir::MLIRContext::getAffineUniquer(), which
  // is unsynchronized when MLIR multithreading is disabled. Contexts created by
  // CreateMlirContext(), including every context in GpuCompiler's pool, have it
  // disabled.
