...
TEST_P(GpuIndexingPerformanceModelTest,
       TryFindTopKBestTilingsForFusionAsyncRequiresPoolOrMultithreading) {
  if (!use_experimental_tiling()) {
    GTEST_SKIP() << "Only the experimental tiling path evaluates candidates "
                    "concurrently.";
  }
  ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(R"(
HloModule m
...
ENTRY main {
  ...
  mlir::MLIRContext single_threaded_context(
      mlir::MLIRContext::Threading::DISABLED);
  RegisterSymbolicExprStorage(&single_threaded_context);
  GpuPerformanceModelWithIndexingAnalysis model(
      &device_info_, &fusion_analysis_cache_, HloCostAnalysis::DefaultShapeSize,
      &single_threaded_context, /*use_experimental_tiling=*/true,
      /*enable_same_shape_multi_output_fusion=*/false,
      /*mlir_context_pool=*/nullptr);

  // The thread pool is created inside the statement so that the process is
  // still single-threaded when the death test forks.
  EXPECT_DEATH(
      {
        tsl::thread::ThreadPool thread_pool(tsl::Env::Default(), "test", 2);
        model
            .TryFindTopKBestTilingsForFusionAsync(*fusion_adaptor, /*top_k=*/1,
                                                  thread_pool.AsExecutor())
            .Await()
            .status()
            .IgnoreError();
      },
      "Concurrent tiling evaluation requires");
}
