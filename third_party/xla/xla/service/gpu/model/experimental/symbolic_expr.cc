...
std::string SymbolicExprStorage::ToString() const {
  switch (type_) {
    case SymbolicExprType::kConstant:
      return std::to_string(value_);
    case SymbolicExprType::kVariable:
      return absl::StrCat("v", value_);
    case SymbolicExprType::kAdd:
    case SymbolicExprType::kMul:
    case SymbolicExprType::kFloorDiv:
    case SymbolicExprType::kCeilDiv:
    case SymbolicExprType::kMod: {
      ...
      return absl::StrCat("(", lhs_->ToString(), " ", bin_op_str, " ",
                          rhs_->ToString(), ")");
    }
    case SymbolicExprType::kMax:
    case SymbolicExprType::kMin: {
      auto bin_op_str = GetBinaryOpString(type_);
      return absl::StrCat(bin_op_str, "( ", lhs_->ToString(), ", ",
                          rhs_->ToString(), ")");
    }
    default:
...
int64_t SymbolicExprStorage::Evaluate(
...
SymbolicExpr* SymbolicExprStorage::ReplaceVariables(
  ...
  switch (type_) {
    ...
    case SymbolicExprType::kMax:
    case SymbolicExprType::kMin: {
      SymbolicExpr* new_lhs = lhs_->ReplaceVariables(substitutions, ctx);
      SymbolicExpr* new_rhs = rhs_->ReplaceVariables(substitutions, ctx);
      return ctx->CreateBinaryOp(type_, new_lhs, new_rhs);
    }
    default:
...
SymbolicExpr* SymbolicExprContext::GetOrCreate(SymbolicExprStorage::KeyTy key) {
  ...
  SymbolicExprStorage& storage = storage_pool_.emplace_back(
      std::get<0>(key), std::get<1>(key), std::get<2>(key), std::get<3>(key));

  SymbolicExpr& handle = handle_pool_.emplace_back(&storage);
...
SymbolicExpr* SymbolicExprContext::CreateConstant(int64_t value) {
  return GetOrCreate({SymbolicExprType::kConstant, value, nullptr, nullptr});
}

SymbolicExpr* SymbolicExprContext::CreateVariable(int64_t var_id) {
  return GetOrCreate({SymbolicExprType::kVariable, var_id, nullptr, nullptr});
}
...
SymbolicExpr* SymbolicExprContext::CreateBinaryOp(SymbolicExprType type,
  ...
  CHECK(lhs != nullptr && rhs != nullptr &&
        type != SymbolicExprType::kConstant &&
