...
MatmulInterpolator::MatmulDTypeKey Parse(absl::string_view dtype_key) {
  std::vector<absl::string_view> parts = absl::StrSplit(dtype_key, "->");
  CHECK_EQ(parts.size(), 2);
  std::vector<absl::string_view> in_parts = absl::StrSplit(parts[0], 'x');
  CHECK_EQ(in_parts.size(), 2);
  return MatmulInterpolator::MatmulDTypeKey{
      std::string(in_parts[0]),
      std::string(in_parts[1]),
      std::string(parts[1]),
  };
}
...
bool IsBF16Matmul(const MatmulInterpolator::MatmulDTypeKey& dtype_key) {
  return absl::AsciiStrToLower(dtype_key.lhs_dtype) == "bf16" &&
         absl::AsciiStrToLower(dtype_key.rhs_dtype) == "bf16" &&
         absl::AsciiStrToLower(dtype_key.out_dtype) == "bf16";
}
...
MatmulInterpolator::Create(const se::DeviceDescription& device_info) {
  ...
  absl::flat_hash_map<MatmulInterpolator::MatmulDTypeKey,
  ...
  for (const GemmPerfTableEntry& entry : table.entries()) {
    for (const auto& [dtype_key, flops] : entry.flops()) {
      MatmulInterpolator::MatmulDTypeKey key = Parse(dtype_key);
...
std::optional<absl::Duration> MatmulInterpolator::EstimatedRuntime(
  ...
  MatmulInterpolator::MatmulDTypeKey key;
  key.lhs_dtype = PrimitiveType_Name(spec->lhs_type);
  key.rhs_dtype = PrimitiveType_Name(spec->rhs_type);
  key.out_dtype = PrimitiveType_Name(spec->out_type);
