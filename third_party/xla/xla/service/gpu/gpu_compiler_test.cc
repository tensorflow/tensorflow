...
TEST_F(GpuCompilerTest, HostNativeCallIsHostOffloadSet) {
  const char* hlo_text = R"(
HloModule test
...
%async_computation (p0: f32[4]) -> f32[4] {
  %p0 = f32[4] parameter(0)
  ROOT %call = f32[4] call(%p0), to_apply=%host_fn, frontend_attributes={_xla_compute_type="host"}
}

ENTRY main {
  %p0 = f32[4]{0} parameter(0)
  %start = ((f32[4]{0}), f32[4]{0}, u32[]) async-start(%p0),
    calls=%async_computation,
    async_execution_thread="host"
  ROOT %done = f32[4]{0} async-done(%start)
}
)";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                       ParseAndReturnVerifiedModule(hlo_text));
