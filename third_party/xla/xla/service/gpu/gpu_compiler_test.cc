...
TEST_F(GpuCompilerTest, MosaicMultimemRequiresSymmetricMemoryCopies) {
  ...
  constexpr absl::string_view kHlo = R"(
    ...
    ENTRY main {
      ...
      cc_multimem = (s32[1]{0}) custom-call(p_multimem), custom_call_target="mosaic_gpu_v2", backend_config={xla_symmetrical_memory_parameters = "0"}, api_version=API_VERSION_TYPED_FFI
  ...
  constexpr absl::string_view kExpected = R"(
    ...
    // CHECK-DAG: [[CC_MULTI:%[^ ]+]] = (s32[1]{0:S(1)}) custom-call([[COPY_MULTI_IN]]){{.*}}backend_config={xla_symmetrical_memory_parameters = "0"}
