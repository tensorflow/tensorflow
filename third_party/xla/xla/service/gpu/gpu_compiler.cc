...
absl::Status RunCollectiveOptimizationPasses(
  ...
  if (debug_options.xla_gpu_experimental_enable_nvshmem()) {
    collectives_pipeline.AddPass<CollectiveBackendAssigner>(
        device_info.gpu_compute_capability(), num_devices_per_host);
  }
  ...
  collectives_pipeline.AddPass<CollectivePipeliningAnalyzer>(device_info,
                                                             pointer_size);
  ...
  collectives_pipeline.AddPass<GpuAlgebraicSimplifier>(
      layout_insensitive_algsimp_opts, device_info.gpu_compute_capability());
...
absl::Status GpuCompiler::OptimizeHloModule(
  ...
  TF_RETURN_IF_ERROR(RunCollectiveOptimizationPasses(
      hlo_module, layout_insensitive_algsimp_opts, device_description,
