...
// Returns the number of participating devices for the given all-reduce
// operation.
int64_t GetNumParticipatingDevices(HloAllReduceInstruction* all_reduce) {
  absl::Span<const ReplicaGroup> replica_groups =
      all_reduce->device_list().replica_groups();

  if (!replica_groups.empty()) {
    // Assume that all replica groups have the same number of replicas.
    return replica_groups.begin()->replica_ids_size();
  }

  // If replica groups are not specified, the collective operation will run on
  // all participating devices.
  return all_reduce->GetModule()->config().replica_count() *
...
absl::StatusOr<bool> DecomposeAllReduce(HloInstruction* hlo,
  ...
  HloInstruction* reshape = AddSize1MajorDimension(input, computation);

  int64_t shard_count = GetNumParticipatingDevices(all_reduce);

  TF_ASSIGN_OR_RETURN(
      Shape all_gather_shape,
      ShapeInference::InferAllGatherShape(
          {&reshape->shape()}, /*all_gather_dimension=*/0, shard_count));
