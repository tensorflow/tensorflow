...
class HoistBitcastVisitor : public DfsHloRewriteVisitor {
 public:
  explicit HoistBitcastVisitor(
 ...
 private:
  absl::Status HandleFusion(HloInstruction* instruction) override {
    ...
    TF_ASSIGN_OR_RETURN(auto changed, TryHoistBitcastsInComputationToCallers(
                                          instr, call_graph_));
    if (changed) {
      MarkAsChanged();
    }
...
absl::StatusOr<bool> HoistFusedBitcasts::RunOnModule(
  ...
  for (HloComputation* computation :
    ...
    HoistBitcastVisitor visitor(&ctx, call_graph.get(), compute_capability_);
...
absl::StatusOr<bool> HoistFusedBitcasts::Run(
    HloModule* module,
    const absl::flat_hash_set<absl::string_view>& execution_threads) {
