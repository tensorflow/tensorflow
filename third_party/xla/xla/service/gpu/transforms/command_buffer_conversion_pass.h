...
class CommandBufferConversionPass : public ThunkPassInterface {
 public:
  ...
  absl::StatusOr<bool> Run(std::unique_ptr<SequentialThunk>& root_thunk,
 ...
 private:
  ...
  void GetThunksInPostOrder(const Thunk* thunk,
                            absl::flat_hash_set<const Thunk*>& visited,
                            std::vector<const Thunk*>& post_order);

  std::vector<const Thunk*> GetThunksInPostOrder(
