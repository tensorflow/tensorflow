...
absl::StatusOr<HloInstruction*> InvertAndConvertScalar(HloInstruction* scalar,
  ...
  if (invert) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(scalar, MakeBinaryHlo(HloOpcode::kDivide, one, scalar,
                                              &scalar->metadata()));
  }
...
class GemmRewriterVisitor : public DfsHloRewriteVisitor {
 public:
    ...
    switch (options_.dtype) {
      case GemmRewriterOptions::DType::kFp8Only: {
        ...
        if (supported_by_cublaslt && HloPredicateIsOp<HloOpcode::kDot>(instr) &&
          ...
          TF_XLA_ASSIGN_OR_RETURN(bool created_call,
        ...
        if (IsF8Type(instr->operand(0))) {
          ...
          TF_XLA_ASSIGN_OR_RETURN(instr, TurnF8DotIntoF16Dot(instr));
        }
        break;
      }
      case GemmRewriterOptions::DType::kNonFp8Only: {
        if (gemm_backend_config.precision_config().algorithm() ==
          ...
          TF_XLA_RETURN_IF_ERROR(TurnDotIntoConvertAndDotForBF16BF16F32(
          ...
          TF_XLA_RETURN_IF_ERROR(gemm_call->set_backend_config(gpu_backend_config));
          TF_XLA_RETURN_IF_ERROR(ReplaceInstruction(instr, gemm_call));
        }
      } break;
    };
    return absl::OkStatus();
  ...
  absl::Status HandleAdd(HloInstruction* instr) override {
    ...
    if (Match(instr,
      ...
      TF_XLA_ASSIGN_OR_RETURN(HloInstruction * new_add,
                          MakeBinaryHlo(HloOpcode::kAdd, existing_gemm,
  ...
  absl::Status HandleMaximum(HloInstruction* instr) override {
    ...
    if (Match(instr,
      ...
      TF_XLA_RETURN_IF_ERROR(FuseReluActivation(instr, zeros, existing_gemm,
                                            optional_slice_or_bitcast));
    }
    return absl::OkStatus();
  }
  ...
  absl::StatusOr<bool> CreateF8CustomCall(HloInstruction* instr,
    ...
    if (gpu_version_.IsCuda()) {
      TF_XLA_ASSIGN_OR_RETURN(cuda_compute_capability,
    ...
    if (gpu_version_.IsRocm()) {
      TF_XLA_ASSIGN_OR_RETURN(auto rocm_compute_capability,
    ...
    TF_XLA_RETURN_IF_ERROR(new_custom_call->set_backend_config(gpu_backend_config));
    TF_XLA_RETURN_IF_ERROR(SetName(instr->GetModule(), new_custom_call));
  ...
  absl::Status F8ScaleD(HloInstruction* instr, HloInstruction* existing_gemm,
    ...
    // has been fused.
    TF_XLA_ASSIGN_OR_RETURN(auto gpu_backend_config,
                        existing_gemm->backend_config<GpuBackendConfig>());
    ...
    TF_XLA_RETURN_IF_ERROR(existing_gemm->ReplaceOperandWith(2, d_scale));
    TF_XLA_RETURN_IF_ERROR(ReplaceInstruction(instr, existing_gemm));
  ...
  absl::Status F8ConvertD(HloInstruction* instr, HloInstruction* existing_gemm,
    ...
    TF_XLA_RETURN_IF_ERROR(ReplaceWithNewInstruction(instr, std::move(new_gemm)));
  ...
  absl::Status F8AddDAmax(HloInstruction* instr, HloInstruction* existing_gemm,
    ...
    TF_XLA_RETURN_IF_ERROR(gemm_and_damax->set_backend_config(gpu_config));
    ...
    TF_XLA_RETURN_IF_ERROR(ReplaceInstruction(reduce_damax, damax_converted));
    TF_XLA_RETURN_IF_ERROR(ReplaceInstruction(instr, d));
  ...
  absl::Status FuseMatrixBiasAdd(HloInstruction* instr, HloInstruction* bias,
    ...
    TF_XLA_RETURN_IF_ERROR(fused_op->set_backend_config(gpu_config));
    ...
    TF_XLA_RETURN_IF_ERROR(SetName(instr->GetModule(), fused_op.get()));
    if (slice) {
  ...
  absl::StatusOr<bool> FuseVectorBiasAdd(HloInstruction* instr,
    ...
    TF_XLA_RETURN_IF_ERROR(result->set_backend_config(gpu_config));
    TF_XLA_RETURN_IF_ERROR(SetName(gemm->GetModule(), result));
    if (slice) {
    ...
    TF_XLA_RETURN_IF_ERROR(ReplaceInstruction(instr, result));
    return true;
  }
  ...
  absl::Status FuseGeluActivation(HloInstruction* multiply,
    ...
    std::unique_ptr<HloInstruction> output = gemm->CloneWithNewShape(
        has_aux ? ShapeUtil::MakeTupleShape({gemm->shape(), gemm->shape()})
                : gemm->shape());
    TF_XLA_RETURN_IF_ERROR(output->set_backend_config(gpu_config));
    TF_XLA_RETURN_IF_ERROR(SetName(multiply->GetModule(), output.get()));
 ...
 private:
  ...
  absl::StatusOr<absl::string_view> GetNonFp8GemmCustomCallTarget(
    ...
    // cublasLt.
    TF_XLA_ASSIGN_OR_RETURN(bool gemm_is_supported_by_cublas_lt,
                        GemmIsSupportedByCublasLt(instr, gemm_backend_config));
  ...
  absl::StatusOr<bool> TypesAreSupportedByLegacyCublas(
    ...
    TF_XLA_ASSIGN_OR_RETURN(const se::blas::DataType output_dtype,
  ...
  absl::StatusOr<HloInstruction*> TurnF8DotWithUnsupportedOutputTypeIntoF32(
    ...
    TF_XLA_RETURN_IF_ERROR(ReplaceInstruction(instr, convert));
    return f32_dot;
  }
  ...
  absl::StatusOr<HloInstruction*> TurnF8DotIntoF16Dot(HloInstruction* instr) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      TF_XLA_RETURN_IF_ERROR(instr->ReplaceOperandWith(i, convert));
    }
    ...
    if (IsF8Type(instr)) {
      ...
      TF_XLA_RETURN_IF_ERROR(ReplaceInstruction(instr, convert_to_f8));
      return f16_dot;
    }
    return instr;
  }
  ...
  absl::Status HandleCustomCall(HloInstruction* instr) override {
    ...
    if (instr->shape().IsTuple()) {
      for (auto user : instr->users()) {
        ...
        TF_XLA_RETURN_IF_ERROR(ReplaceInstruction(user_get_tuple, get_output));
      }
      return absl::OkStatus();
    } else {
...
absl::StatusOr<bool> RunOnComputation(HloComputation* computation,
  ...
  TF_XLA_RETURN_IF_ERROR(computation->Accept(&workspace_visitor));
