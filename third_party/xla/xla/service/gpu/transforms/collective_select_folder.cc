...
std::optional<FoldableSelect> MatchFoldableSelect(HloInstruction* select) {
  ...
  const HloInstruction* predicate_candidate =
      find_inner_scalar_op(select->operand(0));
  ...
  const HloInstruction* id_op = find_inner_scalar_op(compare->operand(0));
  const HloInstruction* constant_op = find_inner_scalar_op(compare->operand(1));
