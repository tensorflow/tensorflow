...
TEST_F(CollectivePermuteCycleDecomposerTest, NoCycleNotTransformed) {
  ...
  TF_ASSERT_OK(
      RunAndCheckHloRewrite(hlo, CollectivePermuteCycleDecomposer(0), false));
}
...
TEST_F(CollectivePermuteCycleDecomposerTest, BelowThresholdNotTransformed) {
  ...
  TF_ASSERT_OK(
      RunAndCheckHloRewrite(hlo, CollectivePermuteCycleDecomposer(33), false));

  TF_ASSERT_OK(
      RunAndCheckHloRewrite(hlo, CollectivePermuteCycleDecomposer(16), true));
}

HloPrintOptions GetHloPrintOptions() {
...
TEST_F(CollectivePermuteCycleDecomposerTest, ForwardCycle) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      auto module,
      RunAndCheckHloRewrite(hlo, CollectivePermuteCycleDecomposer(0), true));

  EXPECT_TRUE(*RunFileCheck(module->ToString(GetHloPrintOptions()), R"(
