...
class CollectivePipeliningAnalyzerTest : public HloHardwareIndependentTestBase {
 protected:
  absl::Status Run(HloModule* module) {
    se::DeviceDescription device_info =
        TestGpuDeviceInfo::RTXH100SXMDeviceInfo();
    int64_t pointer_size = 4;
    return CollectivePipeliningAnalyzer(device_info, pointer_size)
        .Run(module)
        .status();
  }
};
...
ENTRY main {
  ...
  EXPECT_TRUE(ar->frontend_attributes().map().contains(kTriviallyPipelineable));
  EXPECT_TRUE(module->frontend_attributes().map().contains(
...
ENTRY main {
  ...
  ASSERT_NE(ar, nullptr);
  EXPECT_FALSE(
      ar->frontend_attributes().map().contains(kTriviallyPipelineable));
...
ENTRY main {
  ...
  EXPECT_TRUE(rs->frontend_attributes().map().contains(kTriviallyPipelineable));
  EXPECT_TRUE(module->frontend_attributes().map().contains(
      kAvailableMemoryPostScheduling));
}
...
ENTRY main {
  ...
  ASSERT_NE(rs, nullptr);
  EXPECT_FALSE(
      rs->frontend_attributes().map().contains(kTriviallyPipelineable));
  EXPECT_FALSE(module->frontend_attributes().map().contains(
      kAvailableMemoryPostScheduling));
}
...
ENTRY main {
  ...
  EXPECT_TRUE(ag->frontend_attributes().map().contains(kTriviallyPipelineable));
  EXPECT_TRUE(module->frontend_attributes().map().contains(
      kAvailableMemoryPostScheduling));
}
...
ENTRY main {
  ...
  ASSERT_NE(ag, nullptr);
  EXPECT_FALSE(
      ag->frontend_attributes().map().contains(kTriviallyPipelineable));
  EXPECT_FALSE(module->frontend_attributes().map().contains(
      kAvailableMemoryPostScheduling));
}
