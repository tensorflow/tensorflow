  ...
  while (!computations.empty()) {
    ...
    for (HloInstruction* instruction :
      ...
      if (predicate_(instruction)) {
        // If the predicate matches, then wrap the instructions in an async
        // block. Because  we can only call async-start on fusion blocks, we may
        // need to wrap it first.
        XLA_VLOG_LINES(
            1, absl::StrCat(
                   "AsyncWrapper will make the following instruction async:\n",
                   instruction->ToString()));
        HloInstruction* fusion = instruction;
        if (fusion->opcode() != HloOpcode::kFusion) {
          fusion = computation->CreateFusionInstruction(
              {instruction}, HloInstruction::FusionKind::kLoop);
        }
        TF_RETURN_IF_ERROR(computation
                               ->CreateAsyncInstructions(
                                   fusion, {ShapeUtil::MakeScalarShape(U32)})
                               .status());
        changed = true;
        continue;
      }
