...
absl::StatusOr<std::unique_ptr<CubSortRunnerInterface>> CreateCubSortRunner(
  ...
  switch (key_type) {
    ...
    case U8:
      if (valueWidth == 32) {
        return std::make_unique<CubSortPairsImpl>(CubSortPairs_u8_b32, U32);
      }
      break;
    case F32:
      if (valueWidth == 32) {
        return std::make_unique<CubSortPairsImpl>(CubSortPairs_f32_b32, U32);
      }
      break;
    default:
      return InvalidArgument(
  ...
  return InvalidArgument(
      "Unsupported key/value type combination: %s/%s",
      primitive_util::LowercasePrimitiveTypeName(key_type),
      primitive_util::LowercasePrimitiveTypeName(value_type));
}
