...
TEST_F(RaggedAllToAllKernelTest, SimpleKernelTest) {
  ...
  CHECK_OK(stream->Memcpy(input_buffer.memory_ptr(), input_data.data(),
                          n * sizeof(T)));
  ...
  CHECK_OK(stream->Memcpy(input_offsets_buffer.memory_ptr(),
                          input_offsets.data(),
                          input_offsets.size() * sizeof(int64_t)));
  CHECK_OK(stream->Memcpy(send_sizes_buffer.memory_ptr(), send_sizes.data(),
                          send_sizes.size() * sizeof(int64_t)));
  CHECK_OK(stream->Memcpy(output_offsets_buffer.memory_ptr(),
                          output_offsets.data(),
  ...
  for (int64_t i = 0; i < num_outputs; ++i) {
    ...
    CHECK_OK(stream->Memcpy(output_results[i].data(),
