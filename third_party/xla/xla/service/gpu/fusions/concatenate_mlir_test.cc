...
TEST_F(MlirConcatenateFusionTest, StandAloneConcatenate) {
  ...
  TF_ASSERT_OK(EmitAndCheckIR(kHloString, R"(
    ...
    // CHECK: xla_gpu.loop ({{.*}})[{{.*}}] in #[[LOOPMAP_1]]
    // CHECK:   %[[OUTPUT_INDEX_1:.*]] = xla_gpu.apply_indexing #[[MAP_1]]
    // CHECK:   %[[VAL_1:.*]] = xla_gpu.pure_call @fused_computation_param0
    // CHECK:   %[[INSERTED_1:.*]] = tensor.insert %[[VAL_1:.*]] into {{.*}}[%[[OUTPUT_INDEX_1]]]

    // CHECK: xla_gpu.loop ({{.*}})[{{.*}}] in #[[LOOPMAP_2]]
    ...
    // CHECK: xla_gpu.loop ({{.*}})[{{.*}}] in #[[LOOPMAP_3]]
...
TEST_F(MlirConcatenateFusionTest, PrologueEpilogue) {
  ...
  TF_ASSERT_OK(EmitAndCheckIR(kHloString, R"(
    ...
    // CHECK:   tensor.insert %[[VAL_1_2:.*]] into {{.*}}[%[[THREAD_ID]]]
    ...
    // CHECK:   tensor.insert %[[VAL_2_2:.*]] into {{.*}}[%[[INDEX_2]]]
