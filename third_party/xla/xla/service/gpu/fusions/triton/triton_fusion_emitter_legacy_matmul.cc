...
bool TritonInt4RewritesEnabled(const HloInstruction& hlo) {
...
absl::StatusOr<Value> EmitScope(
  ...
  for (const HloInstruction* hlo : instructions) {
    ...
    if (hlo->opcode() == HloOpcode::kConvert &&
      ...
      Value unpacked;
      if (TritonInt4RewritesEnabled(*hlo)) {
...
class MatMulEmitterHelper {
 public:
  ...
  absl::StatusOr<Value> EmitTensorPointer(
    ...
    if (hlo->shape().element_type() == PrimitiveType::S4 &&
        TritonInt4RewritesEnabled(*hlo)) {
      SetPackedDimAttr(side, tensor_ptr);
    }
  ...
  void SetPackedDimAttr(const Side& side, Value tensor_ptr) {
    std::string packed_dim;
    ...
    auto packed_dim_attr = b_.getStringAttr(packed_dim);
    tensor_ptr.getDefiningOp()->setAttr("packed_dim", packed_dim_attr);
  }
...
class Scopes {
 public:
  Scopes(EmitterLocOpBuilder& b, const HloInstruction* dot_instr,
    ...
    if (!TritonInt4RewritesEnabled(*dot_instr) &&
