...
TEST_F(AddressComputationFusionTest, CublasGemmSimple) {
  ...
  ENTRY %main.9 (Arg_0.1: bf16[2,8,8], Arg_1.2: bf16[2,8,8]) -> bf16[8,8] {
    ...
    ROOT %custom-call.1 = bf16[8,8]{1,0} custom-call(bf16[8,8]{1,0} %bitcast.41, bf16[8,8]{1,0} %bitcast.42),
...
TEST_F(AddressComputationFusionTest, CublasGemmWithWorkspace) {
  ...
  ENTRY %main.9 (Arg_0.1: f16[2,8,8], Arg_1.2: f16[2,8,8]) -> (f16[8,8]{1,0}, s8[256]{0}) {
    ...
    ROOT %custom-call.1 = (f16[8,8]{1,0}, s8[256]{0}) custom-call(f16[8,8]{1,0} %bitcast.41, f16[8,8]{1,0} %bitcast.42),
      custom_call_target="__cublas$gemm",
      backend_config={"gemm_backend_config":{
        "alpha_real":1,
        "beta":0,
  ...
  %fused_computation (param_0_0: f16[2,8,8], param_1_0: f16[2,8,8]) -> (f16[8,8]{1,0}, s8[256]{0}) {
    ...
    %custom-call.1 = (f16[8,8]{1,0}, s8[256]{0}) custom-call(f16[8,8]{1,0} %bitcast.41, f16[8,8]{1,0} %bitcast.42),
      custom_call_target="__cublas$gemm",
      backend_config={"gemm_backend_config":{
        "alpha_real":1,
        "beta":0,
...
TEST_F(AddressComputationFusionTest, ContiguousSlice) {
  ...
  ENTRY %main.9 (Arg_0.1: bf16[2,8,8], Arg_1.2: bf16[8,8,10,8]) -> bf16[4,8] {
    %Arg_0.1 = bf16[2,8,8]{2,1,0} parameter(0), sharding={replicated}
    %Arg_1.2 = bf16[8,8,10,8]{3,2,1,0} parameter(1), sharding={replicated}
    %slice.13 = bf16[1,4,8]{2,1,0} slice(bf16[2,8,8]{2,1,0} %Arg_0.1), slice={[1:2], [0:4], [0:8]}
    %bitcast.41 = bf16[4,8]{1,0} bitcast(bf16[1,4,8]{2,1,0} %slice.13)
    %slice.14 = bf16[1,1,8,8]{3,2,1,0} slice(bf16[8,8,10,8]{3,2,1,0} %Arg_1.2), slice={[0:1], [5:6], [2:10], [0:8]}
    %bitcast.42 = bf16[8,8]{1,0} bitcast(bf16[1,1,8,8]{3,2,1,0} %slice.14)

    ROOT %custom-call.1 = bf16[4,8]{1,0} custom-call(bf16[4,8]{1,0} %bitcast.41, bf16[8,8]{1,0} %bitcast.42),
      custom_call_target="__cublas$gemm",
      backend_config={"gemm_backend_config":{
        "alpha_real":1,
        "beta":0,
  ...
  %fused_computation (param_0_0: bf16[2,8,8], param_1_0: bf16[8,8,10,8]) -> bf16[4,8]{1,0} {
    %param_0_0 = bf16[2,8,8]{2,1,0} parameter(0)
    %slice.13 = bf16[1,4,8]{2,1,0} slice(bf16[2,8,8]{2,1,0} %param_0_0), slice={[1:2], [0:4], [0:8]}
    %bitcast.41 = bf16[4,8]{1,0} bitcast(bf16[1,4,8]{2,1,0} %slice.13)
    %param_1_0 = bf16[8,8,10,8]{3,2,1,0} parameter(1)
    %slice.14 = bf16[1,1,8,8]{3,2,1,0} slice(bf16[8,8,10,8]{3,2,1,0} %param_1_0), slice={[0:1], [5:6], [2:10], [0:8]}
    %bitcast.42 = bf16[8,8]{1,0} bitcast(bf16[1,1,8,8]{3,2,1,0} %slice.14)
  ...
  ENTRY %main.9 (Arg_0.1: bf16[2,8,8], Arg_1.2: bf16[8,8,10,8]) -> bf16[4,8] {
    %Arg_0.1 = bf16[2,8,8]{2,1,0} parameter(0), sharding={replicated}
    %Arg_1.2 = bf16[8,8,10,8]{3,2,1,0} parameter(1), sharding={replicated}
    ROOT %fusion.2 = bf16[4,8]{1,0} fusion(bf16[2,8,8]{2,1,0} %Arg_0.1, bf16[8,8,10,8]{3,2,1,0} %Arg_1.2), kind=kCustom, calls=%fused_computation,
        backend_config={"fusion_backend_config":{"kind":"__custom_fusion","custom_fusion_config":{"name":"address_computation"}}}
  })";
