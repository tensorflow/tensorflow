...
TEST_F(DotSearchSpaceTest, ReturnsAllConfigsIfNoHints) {
  ...
  EXPECT_THAT(
      search_space.OptimizeConfigsForCompilationPerformance(configs, {}),
      ElementsAreArray(configs));
}

TEST_F(DotSearchSpaceTest, OptimizesCompilationPerformanceForEmptyConfigs) {
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                          GetDefaultDotModule());
  TritonDotFusionSearchSpace search_space = MakeSearchSpace(module.get());
  ...
  EXPECT_THAT(search_space.OptimizeConfigsForCompilationPerformance({}, {hint}),
              IsEmpty());
}

TEST_F(DotSearchSpaceTest, RestrictsConfigsForCompilationPerformance) {
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                          GetDefaultDotModule());
  TritonDotFusionSearchSpace search_space = MakeSearchSpace(module.get());
  ...
  EXPECT_THAT(
      search_space.OptimizeConfigsForCompilationPerformance(
          {other_config, matching_hint}, {matching_hint, non_matching_hint}),
      ElementsAre(matching_hint));
}

TEST_F(DotSearchSpaceTest, RestrictsConfigsForPartialMatch) {
  TF_ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> module,
      GetDefaultDotModule(/*lhs_parallel_dim=*/4096, /*rhs_parallel_dim=*/16,
                          /*contracting_dim=*/1024));
...
TEST_F(DotSearchSpaceTest, ReturnsNonEmptySetForUnusualHints) {
  ...
  EXPECT_THAT(search_space.OptimizeConfigsForCompilationPerformance(
                  search_space.GenerateConfigs(), {hint}),
              Not(IsEmpty()));
}
