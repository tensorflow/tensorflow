...
void TritonDotFusionSearchSpace::EliminateLowOccupancyConfigs(
  ...
  auto has_too_few_tiles = [](const ConfigWithNotes& config) {
    // Small dots frequently lead to split_k values that are not compatible
    // with codegen. We skip occupancy optimization for them to be able to
    // consider smaller splits in non-exhaustive mode.
    if (config.config.block_n == 8) {
      return false;
    }
