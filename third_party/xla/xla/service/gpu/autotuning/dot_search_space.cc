...
std::vector<TritonGemmConfig> TritonDotFusionSearchSpace::GenerateConfigs(
  ...
  if (force_contracting_split.has_value()) {
    ...
    int split = force_contracting_split.value();
...
TritonDotFusionSearchSpace::GetMaxOutputTile() const {
  ...
  const int64_t max_elements_per_block =
      device_description_.registers_per_block_limit() * kRegisterSizeInBits /
      compute_bitwidth_;
  auto limit_other_size_to_fit = [&](int64_t this_size) {
    return PreviousPowerOfTwo(max_elements_per_block / this_size);
  };
  ...
  OutputTile max_tile{};
  max_tile.lhs_dim = PreviousPowerOfTwo(std::sqrt(max_elements_per_block));
...
int TritonDotFusionSearchSpace::GetMinContractingTileSize() const {
  ...
  const int min_contracting_tile_size =
      kMmaContractingBitwidth / operand_bitwidth_;
...
int TritonDotFusionSearchSpace::GetMaxContractingSplit(
    OutputTile output_tile) const {
  const int64_t desired_num_blocks = desired_total_warps_ / min_warps_per_cta_;
...
void TritonDotFusionSearchSpace::AddOutputTilings(
  ...
  int split = config.config.split_k;
...
void TritonDotFusionSearchSpace::AddCTASizeParameter(
  ...
  auto& warps = new_config.config.num_warps;
...
void TritonDotFusionSearchSpace::AddContractingTiling(
  ...
  auto new_config = config;
  auto& k = new_config.config.block_k;
  for (k = min_contracting_tile_size_; k <= max_tile_size; k *= 2) {
...
void TritonDotFusionSearchSpace::AddPipeliningParameter(
  ...
  auto new_config = config;
  auto& num_stages = new_config.config.num_stages;
  for (num_stages = 1; num_stages <= max_stages; ++num_stages) {
...
void TritonDotFusionSearchSpace::EliminateLowOccupancyConfigs(
  ...
  if (configs.empty()) {
    // We can get no configs if the problem is small enough so that we can't
    // even occupy all cores. In that case, we just use the largest split and
    // smallest tiling.
