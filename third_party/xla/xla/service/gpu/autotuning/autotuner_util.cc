...
/*static*/ absl::StatusOr<std::optional<AutotuneResult>>
AutotunerUtil::TryFindInCache(const AutotuneCacheKey& key,
                              absl::string_view cache_dir)
    ABSL_LOCKS_EXCLUDED(autotune_cache_mu) {
...
/*static*/ absl::StatusOr<ResultAndInserted> AutotunerUtil::AddResultToCaches(
    const AutotuneCacheKey& key, AutotuneResult result,
    absl::string_view cache_dir,
    DebugOptions::AutotuneCacheMode autotune_cache_mode)
    ABSL_LOCKS_EXCLUDED(autotune_cache_mu) {
