...
bool IsAllowedFormattingOp(const HloInstruction* instr,
                           absl::Span<const HloOpcode> opcodes) {
  return !opcodes.empty() && absl::c_any_of(opcodes, [instr](HloOpcode opcode) {
...
UseDefDataflowPaths GetSlicedOperandPaths(
    const HloInstruction& instr, const CallGraph& call_graph,
    absl::Span<const HloOpcode> allowed_formatting_ops, bool check_alignment) {
  ...
  for (const auto* operand : instr.operands()) {
    ...
    auto maybe_slice_instr =
        HloBfsFindIf({operand}, [&](const HloInstruction* cur) {
          ...
          return !IsNoOp(cur) &&
                 !IsAllowedFormattingOp(cur, allowed_formatting_ops);
        });
...
DefUseDataflowPaths GetSlicedUserPaths(
    const HloInstruction& instr, const CallGraph& call_graph,
    absl::Span<const HloOpcode> allowed_formatting_ops, bool check_alignment) {
