...
absl::StatusOr<bool> ReduceScatterReassociate::RunImpl(
  ...
  for (auto computation : module->computations(execution_threads)) {
    for (HloInstruction *inst : computation->MakeInstructionPostOrder()) {
      ...
      TF_XLA_ASSIGN_OR_RETURN(auto rs0_annotation, GetSchedulingAnnotation(rs0));
      TF_XLA_ASSIGN_OR_RETURN(auto rs1_annotation, GetSchedulingAnnotation(rs1));
      ...
      TF_XLA_RETURN_IF_ERROR(inst->ReplaceAllUsesWith(new_rs));
      ...
      TF_XLA_RETURN_IF_ERROR(computation->RemoveInstruction(rs0));
      if (rs0 != rs1) {
        TF_XLA_RETURN_IF_ERROR(computation->RemoveInstruction(rs1));
      }
      changed = true;
    }
  }
