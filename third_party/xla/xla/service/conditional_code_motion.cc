...
void UpdateInstructionOriginalValue(HloInstruction* instruction,
  ...
  int64_t old_size = old_to_new_map.size();
  CopyOriginalValue(instruction, instruction, old_to_new_map);
  if (operand_source != nullptr &&
      operand_source->opcode() == HloOpcode::kTuple &&
      instruction->original_value() != nullptr) {
    for (int64_t i = old_size; i < operand_source->operand_count(); ++i) {
      if (instruction->original_value()->mutable_tree()->find({i}) !=
              instruction->original_value()->mutable_tree()->end() &&
          operand_source->operand(i)->original_value() != nullptr) {
        instruction->original_value()->mutable_tree()->CopySubtreeFrom(
            operand_source->operand(i)->original_value()->tree(), {}, {i});
      }
    }
  }
}
