...
absl::Status CpuCompiler::RunHloPasses(HloModule* module, bool is_aot_compile,
  ...
  bool has_uploader =
      cpu::GetGlobalSymbolUploaderRegistry().uploader() != nullptr;
  ...
  if (has_uploader) {
    ...
    unoptimized_fingerprint = cpu::MaybeUploadUnoptimizedCpuSymbols(
        module, target_machine_options_proto);
  }
  ...
  if (has_uploader) {
    const std::optional<std::string> optimized_fingerprint =
        cpu::MaybeUploadOptimizedCpuSymbols(module,
                                            target_machine_options_proto);

    if (unoptimized_fingerprint.has_value() &&
        optimized_fingerprint.has_value()) {
      cpu::MaybeUploadCpuSymbolMapping(*unoptimized_fingerprint,
