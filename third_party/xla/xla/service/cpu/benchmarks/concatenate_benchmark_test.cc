...
static void BM_ConcatenateTwoR3F32(benchmark::State& state) {
  int64_t dims[3] = {state.range(0), state.range(1), state.range(2)};
  Shape shape = ShapeUtil::MakeShape(F32, dims);
  int64_t axis = state.range(3);
  ...
  CHECK_OK(RunHloBenchmark(
      state, hlo, args,
      {{"$shape_repr", absl::StrCat(dims[0], "x", dims[1], "x", dims[2])},
       {"$shape", absl::StrCat(dims[0], ",", dims[1], ",", dims[2])},
       {"$out_shape",
        absl::StrCat(out_dims[0], ",", out_dims[1], ",", out_dims[2])},
       {"$axis", absl::StrCat(axis)}}));
}

BENCHMARK(BM_ConcatenateTwoR3F32)
    ->MeasureProcessCPUTime()
    ->ArgNames({"batch", "width", "height", "axis"})
    // axis=0
    ->Args({256, 128, 64, 0})
    ->Args({64, 256, 128, 0})
    ->Args({128, 64, 256, 0})
