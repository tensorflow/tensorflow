...
// Simple DFS check to see if a computation contains a custom call or collective
// instruction.
static bool ContainsCustomCallOrCollective(const HloInstruction* instr) {
  if (instr->opcode() == HloOpcode::kCustomCall || IsCollective(instr)) {
    return true;
  }
  ...
  for (const HloComputation* called_comp : instr->called_computations()) {
    for (const HloInstruction* sub_instr : called_comp->instructions()) {
      if (ContainsCustomCallOrCollective(sub_instr)) {
        return true;
      }
    }
  }
...
absl::StatusOr<bool> SmallWhileLoopHoistingPass::Run(
  ...
  for (HloInstruction* while_instr : while_instrs) {
    if (ContainsCustomCallOrCollective(while_instr)) {
      // Custom calls & collectives are not currently supported in the call
      // kernel emitter.
      continue;
    }
