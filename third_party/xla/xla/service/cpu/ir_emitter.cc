...
absl::StatusOr<llvm::Function*> IrEmitter::EmitComputation(
  ...
  for (const HloInstruction* param : computation->parameter_instructions()) {
    TF_XLA_ASSIGN_OR_RETURN(BufferAllocation::Slice param_slice,
  ...
  TF_XLA_RETURN_IF_ERROR(computation->AcceptOrdered(this, instruction_order));
...
absl::Status IrEmitter::HandleCopy(HloInstruction* copy) {
  if (copy->shape().IsTuple() ||
    ...
    TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(copy));
...
absl::Status IrEmitter::HandleInfeed(HloInstruction* instruction) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(infeed));

  // Write the tuple index table.
  TF_XLA_ASSIGN_OR_RETURN(BufferAllocation::Slice data_slice,
  ...
  TF_XLA_ASSIGN_OR_RETURN(BufferAllocation::Slice token_slice,
  ...
  if (data_shape.IsTuple()) {
    ...
    for (int i = 0; i < data_shape.tuple_shapes().size(); ++i) {
      TF_XLA_ASSIGN_OR_RETURN(BufferAllocation::Slice buffer,
      ...
      TF_XLA_RETURN_IF_ERROR(EmitXfeedTransfer(
...
absl::Status IrEmitter::HandleOutfeed(HloInstruction* outfeed) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(outfeed));
  ...
  for (int i = 0; i < operand_shape.tuple_shapes().size(); ++i) {
    ...
    TF_XLA_RETURN_IF_ERROR(EmitXfeedTransfer(XfeedKind::kOutfeed,
...
absl::Status IrEmitter::HandleSort(HloInstruction* hlo) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(sort));
...
absl::Status IrEmitter::HandleTuple(HloInstruction* tuple) {
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(tuple));
...
absl::Status IrEmitter::HandleDot(HloInstruction* dot) {
  ...
  TF_XLA_RETURN_IF_ERROR(ElementTypesSameAndSupported(
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(dot));
...
absl::Status IrEmitter::HandleConvolution(HloInstruction* convolution) {
  ...
  if (allow_runtime_calls_ && PotentiallyImplementedAsEigenConvolution(
    ...
    if (LayoutUtil::IsMonotonicWithDim0Major(lhs_shape.layout()) &&
      ...
      TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(convolution));
...
absl::Status IrEmitter::HandleAllReduceSingleReplica(HloInstruction* crs) {
  ...
  // and when it's run.
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(crs));
  ...
  for (int64_t i = 0; i < crs->operand_count(); ++i) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(const BufferAllocation::Slice out_slice,
...
absl::Status IrEmitter::HandleAllReduceMultipleReplica(HloInstruction* crs) {
  ...
  if (is_tuple) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(BufferAllocation::Slice input_slice,
                        assignment_.GetUniqueSlice(crs->operand(0), {}));
    TF_XLA_ASSIGN_OR_RETURN(BufferAllocation::Slice output_slice,
  ...
  int32_t shape_length;
  TF_XLA_ASSIGN_OR_RETURN(llvm::Value * shape_ptr,
...
absl::Status IrEmitter::HandleReduceScatter(HloInstruction* rs) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(rs));
...
absl::Status IrEmitter::HandleAllToAll(HloInstruction* instruction) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(instruction));
...
absl::Status IrEmitter::HandleAllGather(HloInstruction* instruction) {
  ...
  TF_XLA_ASSIGN_OR_RETURN(const BufferAllocation::Slice in_slice,
...
absl::Status IrEmitter::HandleCollectivePermute(HloInstruction* crs) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(instr));
...
absl::Status IrEmitter::HandlePartitionId(HloInstruction* hlo) {
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(hlo));
  TF_XLA_ASSIGN_OR_RETURN(BufferAllocation::Slice output_slice,
...
absl::StatusOr<bool> IrEmitter::EmitVectorizedReduce(
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(reduce));
  ...
  if (innermost_dimension_size >= vectorization_factor) {
    ...
    llvm_ir::IrArray::Index array_index(array_multi_index, reduce->shape(),
                                        b()->getInt64Ty());
    TF_XLA_ASSIGN_OR_RETURN(std::vector<llvm::Value*> accumulator,
...
absl::Status IrEmitter::HandleSlice(HloInstruction* slice) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(slice));
...
absl::Status IrEmitter::HandleDynamicSlice(HloInstruction* dynamic_slice) {
  if (ShapeUtil::IsScalar(dynamic_slice->shape())) {
    TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(dynamic_slice));
...
absl::Status IrEmitter::HandleDynamicUpdateSlice(
  ...
  if (ShapeUtil::IsScalar(dynamic_update_slice->shape())) {
    TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(dynamic_update_slice));
...
absl::Status IrEmitter::HandlePad(HloInstruction* pad) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(pad));
...
absl::Status IrEmitter::HandlePad(HloInstruction* pad,
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetElementLoop(
...
absl::Status IrEmitter::HandleFusion(HloInstruction* fusion) {
  ...
  if (llvm_ir::CanEmitFusedDynamicUpdateSliceInPlace(fusion, assignment_)) {
    ...
    TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(fusion));
    ...
    FusedIrEmitter fused_emitter(elemental_emitter);
    BindFusionArguments(fusion, &fused_emitter);
    TF_XLA_ASSIGN_OR_RETURN(auto generator, fused_emitter.GetGenerator(
...
absl::Status IrEmitter::HandleCall(HloInstruction* call) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(call));
  ...
  if (backend_config_or.ok() &&
    ...
    TF_XLA_RETURN_IF_ERROR(EmitCallToParallelForkJoin(
...
absl::Status IrEmitter::HandlePadToStatic(HloInstruction* hlo) {
  ...
  for (int i = 1; i < hlo->shape().tuple_shapes().size(); ++i) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(BufferAllocation::Slice dim_size_slice,
...
absl::Status IrEmitter::HandleTopK(HloInstruction* hlo) {
  ...
  TF_XLA_ASSIGN_OR_RETURN(const BufferAllocation::Slice values_slice,
                      assignment_.GetUniqueSlice(hlo->operand(0), {}));
  TF_XLA_ASSIGN_OR_RETURN(const BufferAllocation::Slice out_values_slice,
                      assignment_.GetUniqueSlice(hlo, {0}));
  TF_XLA_ASSIGN_OR_RETURN(const BufferAllocation::Slice out_indices_slice,
...
absl::Status IrEmitter::HandleOneDnnMatMulCalls(
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(custom_call));
  ...
  if (use_scratchpad) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(const BufferAllocation::Slice scratch_slice,
...
absl::Status IrEmitter::HandleWhile(HloInstruction* xla_while) {
  ...
  TF_XLA_RETURN_IF_ERROR(ShapeUtil::ForEachSubshapeWithStatus(
      ...
      [this, &xla_while](const Shape& /*subshape*/,
        ...
        TF_XLA_RETURN_IF_ERROR(check(xla_while, xla_while->operand(0), index));
        TF_XLA_RETURN_IF_ERROR(check(
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(xla_while));
...
absl::Status IrEmitter::HandleConcatenate(HloInstruction* concatenate) {
  ...
  if (fast_impl_reason.ok()) {
    TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(concatenate));
...
absl::Status IrEmitter::HandleConditional(HloInstruction* conditional) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(conditional));
...
absl::Status IrEmitter::HandleAfterAll(HloInstruction* after_all) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(after_all));
  return absl::OkStatus();
}
...
absl::Status IrEmitter::HandleRngGetAndUpdateState(HloInstruction* rng_state) {
  ...
  TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(rng_state));
...
absl::Status IrEmitter::EmitTargetElementLoop(
  ...
  if (result_array_opt.has_value()) {
    ...
    TF_XLA_RETURN_IF_ERROR(EmitTargetAddressForOp(target_op));
  ...
  if (target_shape.IsTuple() &&
    ...
    if (ShouldEmitParallelLoopFor(*target_op)) {
      ...
      TF_XLA_RETURN_IF_ERROR(ParallelLoopEmitter(element_generator, target_array,
...
absl::StatusOr<llvm::Function*> IrEmitter::EmitNestedComputation(
  ...
  for (HloInstruction* instr : callee.instructions()) {
    ...
    for (HloComputation* called_computation : instr->called_computations()) {
      ...
      TF_XLA_RETURN_IF_ERROR(EmitNestedComputation(*called_computation,
