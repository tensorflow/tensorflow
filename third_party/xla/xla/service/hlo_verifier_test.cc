...
TEST_F(HloVerifierTest, AsyncInstructionInvalidOperandUsage) {
  const char* const hlo_string = R"(
HloModule Module

async_computation {
  p0 = f32[3] parameter(0)
  ROOT tuple = (f32[3]) tuple(p0)
}

ENTRY Entry {
  p0 = f32[3] parameter(0)
  async-start = ((f32[3]), (f32[3]), s32[]) async-start(p0), calls=async_computation
  // Use async-start as operand 1 (invalid, must be operand 0)
  ROOT async-update = ((f32[3]), (f32[3]), s32[]) async-update(async-start, async-start)
}
  )";
  ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnUnverifiedModule(hlo_string));
  auto status = verifier().Run(module.get()).status();
  ...
  EXPECT_THAT(
      status.message(),
      HasSubstr("Async instructions can only be used as the first operand"));
}
