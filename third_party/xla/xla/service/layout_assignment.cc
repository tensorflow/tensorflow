...
Status LayoutAssignment::SetBufferLayout(const Layout& layout1,
                                         const LogicalBuffer& buffer,
                                         bool mandatory, bool dfs,
                                         int64_t priority,
                                         const HloInstruction* user) {
  ...
  LOG(INFO) << "ValidateLayoutForShape: " << layout.ToString() << " "
            << buffer.ToString() << " " << buffer.shape().ToString();
  /*
  if (buffer.instruction()->name().starts_with("unique_sparse_ids")) {
    layout = LayoutUtil::MakeDescendingLayout(buffer.shape().rank());
  }
  */
  TF_RETURN_IF_ERROR(
...
Status LayoutAssignment::SetInstructionLayout(const Shape& shape_with_layout,
                                              ...
                                              bool allow_alias,
                                              int64_t priority) {
  LOG(INFO) << "SetInstructionLayout : " << instruction->name() << ", "
            << ShapeUtil::HumanStringWithLayout(shape_with_layout)
            << ": priority = " << priority << " : mandatory = " << mandatory
            << "; dfs = " << dfs << "\n";
  ...
  if (shape_with_layout.IsArray() &&
    ...
    LOG(INFO) << "Setting operand layout: " << instruction->ToString();
...
Status LayoutAssignment::AddMandatoryConstraints(
  ...
  for (auto* instruction : constraints->computation()->instructions()) {
    if (instruction->opcode() == HloOpcode::kCall &&
      ...
      LOG(INFO) << "kCall instruction: " << instruction->ToString();
      ...
      auto result_shape = UnShardedShape(
          instruction, called_computation_layout.result_layout().shape());
      LOG(INFO) << "result_shape: " << result_shape.ToString();
      ...
      for (int64_t i = 0; i < instruction->operand_count(); ++i) {
        auto operand_shape = UnShardedShape(
            ...
            i);
        LOG(INFO) << "operand_shape: " << operand_shape.ToString();
...
Status LayoutAssignment::CheckCallLayout(
  ...
  TF_RET_CHECK(computation_layout.result_layout().MatchesLayoutInShape(
      ShardedShape(call, call->shape()), /*minor_to_major_only=*/true));
  return OkStatus();
}
...
absl::StatusOr<HloInstruction*> LayoutAssignment::CreateCopyWithNewLayout(
  ...
  LOG(INFO) << "CreateCopyWithNewLayout "
            << ShapeUtil::HumanString(shape_with_layout) << " "
            << instruction->ToString();

  TF_RET_CHECK(LayoutUtil::HasLayout(shape_with_layout));
  bool compatible =
      ShapeUtil::Compatible(shape_with_layout, instruction->shape());
  DCHECK(compatible) << ShapeUtil::HumanString(shape_with_layout) << " "
  ...
  if (instruction->shape().IsTuple()) {
    ...
    LOG(INFO) << "Tuple copy " << " " << tuple_copy->ToString();
    return tuple_copy;
...
Status LayoutAssignment::CopyOperandIfLayoutsDiffer(
  ...
  LOG(INFO) << "Operand " << operand->ToString() << " layout does not match "
    ...
    for (auto* instruction : computation->instructions()) {
      ...
      switch (instruction->opcode()) {
        case HloOpcode::kCall:
          LOG(INFO) << "Checking layout for call instruction: "
                    << instruction->ToString();
...
Status LayoutAssignment::CalculateComputationLayout(
  ...
  auto UpdateLayout = [this](const HloInstruction* operand, ShapeLayout* update,
                             const HloInstruction* call) -> bool {
    bool change = false;
    ShapeUtil::ForEachSubshape(
        operand->shape(), [this, &change, operand, update, call](
  ...
  auto SetCalleeLayout =
    ...
    if (callee_constraint->priority() < priority ||
      ...
      if (conditional_mismatch_.count(callee->computation()) == 0 &&
          UpdateLayout(result, callee_layout.mutable_result_layout(), result)) {
  ...
  for (HloInstruction* instruction :
    ...
    switch (instruction->opcode()) {
      case HloOpcode::kFusion:
        ...
        break;
      case HloOpcode::kCall:
        LOG(INFO) << "Call instruction for callee: " << instruction->ToString()
                  << "\n";
        if (reverse_computation_order_ &&
            SetCalleeLayout(
