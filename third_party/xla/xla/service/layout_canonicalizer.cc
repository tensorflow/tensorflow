...
bool HandleBroadcast(
    HloInstruction* broadcast, std::vector<int64_t> output_map,
    absl::flat_hash_map<HloInstruction*, std::vector<int64_t>>& operands_map,
    bool is_entry_root) {
  ...
  if (operands_map.contains(broadcast->mutable_operand(0))) {
    std::vector<int64_t> new_broadcast_dimensions = ComposePermutations(
        broadcast->dimensions(), operands_map[broadcast->mutable_operand(0)]);
...
std::vector<int64_t> LayoutCanonicalizer::CanonicalizeOutputShape(
...
bool LayoutCanonicalizer::CanonicalizeInstructionLayout(HloInstruction* instr,
  ...
  VLOG(3) << instr->name()
          << " output_map = " << absl::StrJoin(output_map, ",");

  // For now, we only handle broadcast. I will add other ops gradually.
  bool changed = false;
  switch (instr->opcode()) {
    case HloOpcode::kBroadcast:
      changed = HandleBroadcast(instr, output_map, canonicalized_instrs_,
                                is_entry_root);
      break;
    default:
      break;
  }
  return output_canonicalized || changed || changed_operands;
}
