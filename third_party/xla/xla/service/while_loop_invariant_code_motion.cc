...
static void CreateLoopInvariantCopy(
  ...
  do {
    ...
    if (frame->operand_index == frame->instruction->operand_count()) {
      ...
      std::optional<std::string> original_call_instruction_name;
      if (while_instr->original_value() != nullptr) {
        original_call_instruction_name =
            while_instr->original_value()->GetOriginalCallInstructionName();
      }
      if (original_call_instruction_name.has_value() &&
        ...
        for (auto& [shape_index, original_array] :
          ...
          if (original_array) {
            original_array->instruction_name =
                absl::StrCat(*original_call_instruction_name, "#*/",
                             original_array->instruction_name);
          }
        }
