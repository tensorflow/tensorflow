...
class HostOffloaderTest : public HloTestBase {
 protected:
  ...
  static constexpr int64_t kHbmMemorySpaceColor{0};
...
ENTRY %main {
  ...
  HloInstruction* gte_0 = FindInstruction(module.get(), "gte_0");
  ASSERT_NE(gte_0, nullptr);
  TestShapeHasMemorySpace(gte_0->shape(), kHbmMemorySpaceColor);
  HloInstruction* gte_1 = FindInstruction(module.get(), "gte_1");
  ASSERT_NE(gte_1, nullptr);
  TestShapeHasMemorySpace(gte_1->shape(), kHbmMemorySpaceColor);
}
...
ENTRY %main {
  ...
  TestShapeHasMemorySpace(param_0_non_async->shape(), kHostMemorySpaceColor);
  HloInstruction* param_1_non_async =
      FindInstruction(module.get(), "param_1_non_async");
  ASSERT_NE(param_1_non_async, nullptr);
  TestShapeHasMemorySpace(param_1_non_async->shape(), kHostMemorySpaceColor);
