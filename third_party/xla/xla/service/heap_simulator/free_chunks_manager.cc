...
bool FreeChunksManager::MemoryChunk::OffsetLessThan(
    const FreeChunksManager::MemoryChunk& a,
    const FreeChunksManager::MemoryChunk& b) {
...
bool FreeChunksManager::MemoryChunk::AlignedSizeLessThan(
    const FreeChunksManager::MemoryChunk& a,
    const FreeChunksManager::MemoryChunk& b) {
...
FreeChunksManager::FreeChunksManager(
    ...
    : chunk_alignment_(std::move(chunk_alignment)),
      free_chunks_by_offset_(&FreeChunksManager::MemoryChunk::OffsetLessThan),
      free_chunks_by_size_(
          &FreeChunksManager::MemoryChunk::AlignedSizeLessThan) {
...
void FreeChunksManager::RemoveFreeChunk(
    absl::btree_set<FreeChunksManager::MemoryChunk,
                    decltype(&FreeChunksManager::MemoryChunk::OffsetLessThan)>::
        iterator it) {
...
void FreeChunksManager::Deallocate(int64_t offset, int64_t end) {
  ...
  if (free_chunk_above != free_chunks_by_offset_.end() &&
    ...
    VLOG(1) << "Deallocate removing chunk: offset="
            ...
            << ", size=" << free_chunk_above->aligned_size()
  ...
  if (free_chunk_below->end() >= offset) {
    ...
    VLOG(1) << "Deallocate removing chunk: offset="
            ...
            << ", size=" << free_chunk_below->aligned_size()
