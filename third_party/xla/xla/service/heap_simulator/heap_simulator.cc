...
GlobalDecreasingSizeBestFitHeap<BufferType>::MakeFreeChunks(
  ...
  VLOG(1) << "MakeFreeChunks for buffer: " << buffer_interval.buffer;
  for (const BufferType* col : colocations) {
    VLOG(1) << "  Colocation: " << col;
  }

  auto conflicts = [&](const void* node_buffer_ptr) {
    if (node_buffer_ptr == nullptr) {
      VLOG(1) << "  Node buffer is null -> conflicts";
      return true;
    }
    const BufferType* node_buffer =
        static_cast<const BufferType*>(node_buffer_ptr);
    bool res = !colocations.contains(node_buffer);
    VLOG(1) << "  Checking conflict with node buffer: " << node_buffer << " -> "
            << (res ? "conflicts" : "ignored");
    return res;
  };
