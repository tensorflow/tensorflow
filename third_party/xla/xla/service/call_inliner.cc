 ...
 public:
  ...
  absl::Status DefaultAction(HloInstruction* hlo) override {
    ...
    for (HloInstruction* operand : hlo->operands()) {
      TF_XLA_ASSIGN_OR_RETURN(HloInstruction * new_operand, Resolve(operand));
    ...
    TF_XLA_RETURN_IF_ERROR(NoteMapping(hlo, new_hlo_pointer));
    ...
    for (HloInstruction* control_predecessor : hlo->control_predecessors()) {
      TF_XLA_ASSIGN_OR_RETURN(HloInstruction * new_control_predecessor,
    ...
    for (HloInstruction* control_predecessor : call_->control_predecessors()) {
      TF_XLA_RETURN_IF_ERROR(control_predecessor->AddControlDependencyTo(
  ...
  absl::Status HandleParameter(HloInstruction* parameter) override {
    TF_XLA_RETURN_IF_ERROR(NoteMapping(
  ...
  absl::Status FinishVisit(HloInstruction* root) override {
    TF_XLA_ASSIGN_OR_RETURN(HloInstruction * new_root, Resolve(root));
...
CallInliner::Inline(HloInstruction* call) {
  ...
  TF_XLA_RETURN_IF_ERROR(callee->Accept(&visitor));
    ...
    if (ShouldInline(call_graph, instruction)) {
      ...
      if (update_domain_) {
        ...
        for (const auto& [call_inst, inlined_inst] : inline_map_cur_call) {
          TF_XLA_RETURN_IF_ERROR(isolator.UpdateDomains(inlined_inst).status());
        }
      }
...
absl::StatusOr<bool> CallInliner::RunWithInlineMap(
  ...
  if (did_mutate) {
    ...
    // instructions.
    TF_XLA_RETURN_IF_ERROR(HloDCE().Run(module, execution_threads).status());
    if (module->has_schedule()) {
      TF_XLA_RETURN_IF_ERROR(module->schedule().Update(execution_threads));
    }
  }
  return did_mutate;
}
