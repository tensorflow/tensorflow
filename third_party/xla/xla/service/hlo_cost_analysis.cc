...
absl::Status HloCostAnalysis::RevisitInstruction(
    const HloInstruction* instruction) {
  TF_XLA_RETURN_IF_ERROR(RemoveInstruction(instruction));
  // Now do Preprocess() -> Visit() -> Postprocess() for the instruction same
  // way it is done during the complete analysis.
  TF_XLA_RETURN_IF_ERROR(Preprocess(instruction));
  TF_XLA_RETURN_IF_ERROR(instruction->Visit(this));
  TF_XLA_RETURN_IF_ERROR(Postprocess(instruction));
  return absl::OkStatus();
}
...
absl::Status HloCostAnalysis::HandleMap(const HloInstruction* map) {
  ...
  TF_XLA_ASSIGN_OR_RETURN(const Properties sub_properties,
...
absl::Status HloCostAnalysis::HandleReduceWindow(
  ...
  TF_XLA_ASSIGN_OR_RETURN(Properties sub_properties,
                      ProcessSubcomputation(function));
...
absl::Status HloCostAnalysis::HandleSelectAndScatter(
  ...
  // Compute the properties of the reduction function.
  TF_XLA_ASSIGN_OR_RETURN(Properties select_properties,
                      ProcessSubcomputation(instruction->select()));
  TF_XLA_ASSIGN_OR_RETURN(Properties scatter_properties,
...
absl::Status HloCostAnalysis::HandleFusion(const HloInstruction* fusion) {
  ...
  current_properties_[kBytesAccessedKey] = 0;
  TF_XLA_RETURN_IF_ERROR(FusionProcessOutputBytesAccessed(fusion));
  TF_XLA_RETURN_IF_ERROR(FusionCalculateUtilizations(fusion));
  TF_XLA_RETURN_IF_ERROR(FusionCountConstantsMemoryAccess(fusion));
  TF_XLA_RETURN_IF_ERROR(FusionProcessOperandBytesRead(fusion));

  return absl::OkStatus();
}

absl::Status HloCostAnalysis::HandleCall(const HloInstruction* call) {
  TF_XLA_ASSIGN_OR_RETURN(current_properties_,
...
absl::Status HloCostAnalysis::HandleWhile(const HloInstruction* xla_while) {
  ...
  TF_XLA_ASSIGN_OR_RETURN(const Properties body_properties,
                      ProcessSubcomputation(xla_while->while_body()));

  TF_XLA_ASSIGN_OR_RETURN(const Properties condition_properties,
...
HloCostAnalysis::ProcessSubcomputation(HloComputation* computation) {
  ...
  TF_XLA_RETURN_IF_ERROR(computation->Accept(visitor.get()));
