...
MpmdLoadedExecutable::MpmdLoadedExecutable(
  ...
  loaded_executable_ = std::make_unique<ifrt::proxy::LoadedExecutable>(
      client, rpc_helper, handle, name, num_devices, devices,
      ...
      [promise = std::move(promise)](
        ...
        if (response.value()->has_mpmd_compiled_memory_stats()) {
          ...
          for (const auto& [name, stats_proto] :
            ...
            mpmd_compiled_memory_stats[name] =
                xla::CompiledMemoryStats::FromProto(stats_proto);
          }
          ...
          info->mpmd_compiled_memory_stats = tsl::StatusFromProto(
              response.value()->mpmd_compiled_memory_stats_error());
        } else {
          info->mpmd_compiled_memory_stats = absl::UnimplementedError(
...
MpmdLoadedExecutable::GetMpmdAddressableDevices() const {
  ...
  devices_map_with_spans.reserve(mpmd_addressable_devices_.value().size());
  for (const auto& [mesh_name, device_vector] :
       mpmd_addressable_devices_.value()) {
