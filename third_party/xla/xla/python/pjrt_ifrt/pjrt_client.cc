...
absl::StatusOr<std::vector<ArrayRef>> PjRtClient::CopyArrays(
  ...
  const std::string error_prefix = "Cross-host transfer error: ";
...
absl::Status PjRtClient::CrossHostSendBuffers(PjRtBuffers buffers,
  ...
  for (int i = 0; i < keys.size(); ++i) {
    ...
    if (i == 0 && descriptor.starts_with(error_prefix)) {
      return absl::InternalError(descriptor);
    }
...
absl::StatusOr<PjRtArray::PjRtBuffers> PjRtClient::CrossHostReceiveBuffers(
    ...
    const std::string& error_prefix) {
  auto notifier = [this, keys, key_prefix, error_prefix](
                      absl::StatusOr<xla::PjRtCrossHostRecvState> recv_state) {
    CHECK_GT(keys.size(), 0);
    if (!recv_state.ok()) {
      std::string key_str = absl::StrCat(key_prefix, keys[0]);
      CHECK_OK(kv_store_->Set(
          key_str, absl::StrCat(error_prefix, recv_state.status().message())));
      return;
    }
    if (recv_state->descriptors.size() != keys.size()) {
      std::string error_message = absl::StrCat(
          error_prefix,
          "descriptors must be the same size as keys. Descriptors: ",
          recv_state->descriptors.size(), " keys: ", keys.size());
      std::string key_str = absl::StrCat(key_prefix, keys[0]);
      CHECK_OK(kv_store_->Set(key_str, error_message));
      return;
    }
    for (int i = 0; i < keys.size(); ++i) {
      std::string key = absl::StrCat(key_prefix, keys[i]);
