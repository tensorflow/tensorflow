...
absl::StatusOr<std::unique_ptr<PjRtClient>> PjRtClient::Create(
  ...
  client->cross_host_dcn_max_num_parallel_copies_ =
      options.cross_host_dcn_max_num_parallel_copies;
...
absl::StatusOr<std::vector<ArrayRef>> PjRtClient::CopyArrays(
  ...
  if (pjrt_client_->SupportsCrossHostTransfers()) {
    return CopyArraysForCrossHost(arrays, src_devices, dst_devices,
                                  memory_kind);
  }
  LOG(WARNING) << "Cross-host device transfers are not supported by this "
                  "backend. Falling back to experimental DCN transfer library.";
...
absl::Status PjRtClient::StartTransferServer() {
...
PjRtClient::CopyArraysForCrossHostFallback(
  ...
  if (!socket_server_.has_value()) {
    TF_RETURN_IF_ERROR(StartTransferServer());
  }
  // Maps dst pid to src pids.
  std::map<int, std::set<int>> process_index_map;
  // Maps src pid to addressable destination device inds.
  std::map<int, std::vector<int>> pull_to_device_idxs;
  // Maps dst pid to source buffer inds.
  std::map<int, std::vector<int>> await_pull_buffer_idxs;
  int j = 0;
  ...
  std::map<int, PjRtBuffers> buffers_by_device;
