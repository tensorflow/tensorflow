...
TEST_P(LoadedExecutableImplTest, HloOutputCallbackWrapper) {
  TF_ASSERT_OK_AND_ASSIGN(auto client, test_util::GetClient());
  ...
  hlo_cb->callback = [&invoked](int64_t replica_id, int64_t partition_id,
  ...
  EXPECT_EQ(loaded_host_callback->hlo_output_callback().hlo_id, 42);
  ...
  lit.Set<int32_t>({}, 123);
  const xla::Literal* lit_ptr = &lit;
  loaded_host_callback->hlo_output_callback().callback(
      0, 0, absl::MakeSpan(&lit_ptr, 1));
  EXPECT_TRUE(invoked);
}
