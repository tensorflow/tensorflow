...
TEST(BasicStringArrayTest, InvalidBuffersAreHandledCorrectly) {
  ...
  ASSERT_OK_AND_ASSIGN(auto ret,
                       CreateNonReadyTestArray(client.get(), devices[0],
                                               std::move(on_done_with_buffer)));
...
TEST(AssembleArrayFromSingleDeviceArraysTest,
  ...
  ASSERT_OK_AND_ASSIGN(auto buffers, basic_string_array->buffers().Await());
...
TEST(AssembleArrayFromSingleDeviceArraysTest,
  ...
  ASSERT_OK_AND_ASSIGN(auto buffers, buffers_future.Await());
...
TEST(DisassembleArrayIntoSingleDeviceArrays,
  ...
  ASSERT_OK_AND_ASSIGN(auto disassembled_arrays,
  ...
  ASSERT_OK_AND_ASSIGN(auto new_buffers, basic_string_array->buffers().Await());
...
TEST(DisassembleArrayIntoSingleDeviceArrays, ShardedArrayDisassembleSuccess) {
  ...
  for (int i = 0; i < disassembled_arrays.size(); ++i) {
    ...
    ASSERT_OK_AND_ASSIGN(auto buffer, basic_string_array->buffers().Await());
...
TEST(CopyTest, SuccessSingleDeviceShardedArray) {
  ...
  ASSERT_OK_AND_ASSIGN(auto new_buffers,
...
TEST(FullyReplicatedShardTest, SuccessSingleDeviceShardedArray) {
  ...
  ASSERT_OK_AND_ASSIGN(auto replicated_buffers,
...
TEST(CopyToHostBufferTest, Success) {
  ...
  ASSERT_OK_AND_ASSIGN(auto array, MakeSingleDeviceStringTestArray(
                                       input_data, client.get(), devices[0]));
