...
bool XlaRuntimeAbiVersion::IsCompatibleWith(
  ...
  return runtime_abi_version->IsCompatible(other.runtime_abi_version.get());
}
...
XlaRuntimeAbiVersion::Deserialize(const absl::string_view serialized_proto) {
  Serialized serialized_runtime_abi_version;
  if (!serialized_runtime_abi_version.ParseFromString(serialized_proto)) {
