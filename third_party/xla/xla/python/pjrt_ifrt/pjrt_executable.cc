...
absl::StatusOr<ExecutableRef> PjRtExecutable::Create(
  ...
  ASSIGN_OR_RETURN(const std::vector<xla::Shape> mlir_module_input_xla_shapes,
                   InputShapesOfModule(module.mlir_module()));
  ASSIGN_OR_RETURN(const std::vector<xla::LayoutMode> input_layout_modes,
...
absl::StatusOr<std::string> PjRtExecutable::CommonMetadata::Serialize(
  ...
  *metadata.mutable_executable_version() = serialized_executable_version;
  ...
  for (int i = 0; i < parameter_dtypes.size(); ++i) {
    ...
    if (parameter_layouts[i] != nullptr) { - only populate if it's not the default layout
...
absl::StatusOr<LoadedExecutableRef> PjRtLoadedExecutable::Create(
  ...
  ASSIGN_OR_RETURN(
      std::vector<int> donatable_input_indices,
      GetDonatableInputIndicesFromMlirModule(module.mlir_module()));
  ASSIGN_OR_RETURN(std::vector<xla::Shape> mlir_module_input_xla_shapes,
                   InputShapesOfModule(module.mlir_module()));
  ASSIGN_OR_RETURN(std::vector<xla::LayoutMode> input_layout_modes,
                   GetArgLayoutModes(module.mlir_module()));
  ASSIGN_OR_RETURN(const std::vector<xla::Shape> mlir_module_output_xla_shapes,
                   ResultShapesOfModule(module.mlir_module()));
  ASSIGN_OR_RETURN(const std::vector<xla::LayoutMode> output_layout_modes,
  ...
  ASSIGN_OR_RETURN(auto parameter_dtypes_and_shapes,
