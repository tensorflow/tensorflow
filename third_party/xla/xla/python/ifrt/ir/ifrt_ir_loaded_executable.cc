...
absl::StatusOr<std::optional<std::string>> IfrtIrLoadedExecutable::Fingerprint()
  ...
  // JAX-style (de)serialization on the local host.
...
IfrtIrLoadedExecutable::executable_version() const {
  ...
  for (const auto& [name, executable] : *program_->atom_program_executables) {
    ...
    TF_ASSIGN_OR_RETURN(std::unique_ptr<xla::ifrt::ExecutableVersion> version,
...
IfrtIrLoadedExecutable::GetHloModules() const {
  // This is not exactly what LoadedExecutable::GetHloModules should do,
  // but it is used to print useful debug information without opening up
  // visibility.
  //
  // Keys to `atom_program_executables` are lost, but usually the size of value
  // vector is one and the keys are the same as HloModule::name()
