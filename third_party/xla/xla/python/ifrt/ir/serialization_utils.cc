...
absl::StatusOr<std::string> SerializeIfrtIrExecutable(
  ...
  std::string serialized_loaded_executable;
  {
    ...
    coded_output_stream.WriteVarint32(metadata.ByteSizeLong());
    metadata.SerializeWithCachedSizes(&coded_output_stream);
  }
...
absl::StatusOr<DeserializedIfrtIRProgram> DeserializeIfrtIrExecutable(
  ...
  if (!coded_input_stream.ReadVarint32(&size)) {
    return absl::InvalidArgumentError(
        "Failed to read size for SerializedIfrtIrExecutableMetadata");
  }
  auto limit = coded_input_stream.PushLimit(size);
  if (!metadata.ParseFromCodedStream(&coded_input_stream)) {
    return absl::InvalidArgumentError(
        "Failed to parse SerializedIfrtIrExecutableMetadata");
  }
  if (!coded_input_stream.ConsumedEntireMessage()) {
    return absl::InvalidArgumentError(
        "SerializedIfrtIrExecutableMetadata size mismatch");
  }
