...
class RCReferenceWrapper {
 public:
  ...
  RCReferenceWrapper& operator=(const RCReferenceWrapper<U>& other) {
    ref_ = other.const_tsl_rcref();
    return *this;
  }
  template <typename U>
  RCReferenceWrapper& operator=(RCReferenceWrapper<U>&& other) {
    ref_ = std::forward(other).tsl_rcref();
    return *this;
  }
  ...
  const tsl::RCReference<T>& const_tsl_rcref() const { return ref_; }
  tsl::RCReference<T> tsl_rcref() && { return std::move(ref_); }
