...
class Client : public llvm::RTTIExtends<Client, llvm::RTTIRoot> {
 public:
  ...
  //
  // `name` specifies the name of `DeviceAllocation` unique in the instance of
  // `Client`. A `DeviceAllocation` may be identified by its name in  some
  // `Constraint` (see `DeviceAllocation::Constraints`).
  //
  ...
  //
  // Devices associated with `DeviceAllocation` will not be returned by
  // client-wide device enumeration APIs such as `devices()`,
  // `addressable_devices()`, and `*_count()` methods. However, device lookup
  // APIs such as `LookupDevice()` and `LookupAddressableDevice()` will work.
  //
  //
