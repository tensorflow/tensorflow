...
class Client : public llvm::RTTIExtends<Client, llvm::RTTIRoot> {
 public:
  ...
  // host buffer is no longer used by the runtime. It will not be called if the
  // API processing the host buffer has returned an error. For simple and robust
  // cleanup, it is strongly recommended to capture RAII objects in the closure
  // of the callback and leave the callback's function body empty.
  //
  // If a host buffer is used with a zero-copy semantics, the host buffer data
  // should not be accessed by the user until `on_done` is called. The data may
  // be read throughout the life of the array created with the host buffer, it
  // may be even mutated.
  struct HostBuffer {
    const void* data;
