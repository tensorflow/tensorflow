...
// Basic implementation of `DeviceAllocation` that simply wraps around a
// `DeviceList`. The list must contain at least one device.
class BasicDeviceAllocation final
 ...
 public:
  ...
  // device ids. There must be at least one device.
  static absl::StatusOr<tsl::RCReference<BasicDeviceAllocation>> Create(
