...
absl::StatusOr<py::object> MakeNdarray(const xla::LiteralProto& proto) {
  ...
  if (!m_lit.ok()) {
    // FIXME(wrengr): For some reason this overload of `absl::StrCat`
    // isn't being found on OSS.
    static_assert(absl::HasAbslStringify<absl::Status>::value);
    static_assert(std::is_convertible_v<absl::Status, absl::AlphaNum>);
