...
/*static*/ nb_dtype nb_dtype::from_args(const nb::object& args) {
  ...
  return nb::steal<nb_dtype>(tsl::safe_reinterpret_cast<PyObject*>(descr));
}
...
nb_numpy_ndarray::nb_numpy_ndarray(
  ...
  // The tsl::safe_reinterpret_cast below assumes that npy_intp and int64_t are
  // the same width. If that changes, then the code should be updated to convert
  // instead.
  static_assert(sizeof(int64_t) == sizeof(npy_intp));
  nb::object array = nb::steal<nb::object>(PyArray_NewFromDescr(
      &PyArray_Type,
      tsl::safe_reinterpret_cast<PyArray_Descr*>(dtype.release().ptr()),
      shape.size(), tsl::safe_reinterpret_cast<const npy_intp*>(shape.data()),
      tsl::safe_reinterpret_cast<const npy_intp*>(strides_ptr),
      const_cast<void*>(ptr), flags,
      /*obj=*/nullptr));
  if (!array) {
    throw nb::python_error();
  }
  if (ptr) {
    if (base) {
      PyArray_SetBaseObject(
          tsl::safe_reinterpret_cast<PyArrayObject*>(array.ptr()),
          base.inc_ref().ptr());
    } else {
...
nb_dtype nb_numpy_ndarray::dtype() const {
  PyArrayObject* self = tsl::safe_reinterpret_cast<PyArrayObject*>(ptr());
  return nb::borrow<nb_dtype>(
      tsl::safe_reinterpret_cast<PyObject*>(PyArray_DESCR(self)));
}
