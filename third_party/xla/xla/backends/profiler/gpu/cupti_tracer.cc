...
CuptiTracerEvent &PopulateMemcpyCallbackEvent(
    CuptiTracerEvent &event, CuptiTracerEventType type,
    const CUpti_CallbackData *cbdata, size_t num_bytes, uint32_t src_device,
    uint32_t dst_device, bool async, uint64_t start_time, uint64_t end_time) {
  event.type = type;
  event.source = CuptiTracerEventSource::DriverCallback;
  event.start_time_ns = start_time;
  event.end_time_ns = end_time;
...
struct CallbackAnnotationsAndEvents {
  struct EventWithAnnotation {
    ...
    EventWithAnnotation()
        : correlation_id(0), annotation(), nvtx_range(), event{} {};
  ...
  bool Add(uint32_t device_id, uint32_t correlation_id,
    ...
    if (max_callback_api_events == 0 ||
      ...
      bool too_many_annotations =
          ...
          (annotations.size() >= max_annotation_strings);
      event_annotation_buffer.Emplace(
          correlation_id,
          (too_many_annotations || annotation.empty())
              ? absl::string_view{}
              : absl::string_view(
                    *annotations.emplace(std::string(annotation)).first),
          (too_many_annotations || nvtx_range.empty())
              ? absl::string_view{}
              : absl::string_view(
                    *nvtx_ranges.emplace(std::string(nvtx_range)).first));
      return true;
    }
    num_dropped_events++;
    return false;
...
void CuptiTracer::Disable() {
  ...
  collector_->OnTracerCollectedData(merged_annotation_map, activity_buffers,
                                    collected_annotation_and_events);
...
static size_t CountCuptiActivityEvent(uint8_t *buffer, size_t size) {
  ...
  if (size == 0) return total_event_count;
  CuptiInterface *cupti_interface = GetCuptiInterface();
  CUpti_Activity *record = nullptr;
  while (true) {
    CUptiResult status =
