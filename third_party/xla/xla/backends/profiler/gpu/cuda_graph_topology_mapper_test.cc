...
TEST(CudaGraphTopologyMapperTest, EmptyChildGraphResolutionWithTrailingNodes) {
  // 1. Parent size 3 (nodes P0, P1, P2) with empty inline child at insertion
  // point 1:
  {
    absl::flat_hash_map<uint32_t, size_t> base_sizes = {{1, 3}, {2, 0}};
    absl::flat_hash_map<uint32_t,
                        std::vector<CudaGraphTopologyMapper::ChildGraphEntry>>
        child_graphs;
    child_graphs[1].push_back({2, 1, /*is_conditional=*/false});
  ...
  // 2. Parent size 3 with empty inline child at insertion point 1 AND
  // conditional child (ID 3, size 2):
  {
    absl::flat_hash_map<uint32_t, size_t> base_sizes = {
        {{1, 3}, {2, 0}, {3, 2}}};
    absl::flat_hash_map<uint32_t,
                        std::vector<CudaGraphTopologyMapper::ChildGraphEntry>>
        child_graphs;
    ...
    auto res3 = CudaGraphTopologyMapper::ResolveMergedNode(
        1, 3, base_sizes, child_graphs, &merged_sizes);
    EXPECT_EQ(res3.first, 3);
    EXPECT_EQ(res3.second, 1);
  }
}
