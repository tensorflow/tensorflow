  ...
  if (node_index < total_inline_size) {
    ...
    for (const auto& child : children_it->second) {
      ...
      size_t child_size = CalculateMergedSize(child.child_graph_id, base_sizes,
                                              child_graphs, merged_sizes);
      int64_t child_start =
          static_cast<int64_t>(child.insertion_point) + inline_offset;

      if (child_size > 0) {
        int64_t child_end = child_start + static_cast<int64_t>(child_size) - 1;
        if (static_cast<int64_t>(node_index) >= child_start &&
            static_cast<int64_t>(node_index) <= child_end) {
          uint32_t local_index = static_cast<uint32_t>(
              static_cast<int64_t>(node_index) - child_start);
          return ResolveMergedNode(child.child_graph_id, local_index,
                                   base_sizes, child_graphs, merged_sizes);
        }
      }

      if (static_cast<int64_t>(node_index) < child_start) {
        return {graph_id,
                static_cast<uint32_t>(static_cast<int64_t>(node_index) -
                                      inline_offset)};
      }

      inline_offset += (static_cast<int64_t>(child_size) - 1);
    }
    return {graph_id, static_cast<uint32_t>(static_cast<int64_t>(node_index) -
                                            inline_offset)};
  }
