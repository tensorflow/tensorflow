 ...
 public:
  ...
  void AddGraphIdMapsToPlane(XPlaneBuilder* device_plane) {
    ...
    for (const auto& [graph_id, orig_graph_id] :
      ...
      std::string graph_map_id =
          absl::StrCat("CudaGraphMapId: ", std::to_string(graph_id), ",",
                       std::to_string(orig_graph_id));
    ...
    for (const auto& [graph_id, node_map] :
      ...
      for (const auto& [node_id, orig_node_id] : node_map) {
        std::string graph_node_map_id = absl::StrCat(
            "CudaGraphNodeMapId: ", std::to_string(graph_id), ",",
            std::to_string(node_id), ",", std::to_string(orig_node_id));
