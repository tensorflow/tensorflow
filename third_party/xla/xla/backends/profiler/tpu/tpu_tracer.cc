...
void MergeXSpace(XSpace* target, XSpace* source) {
  for (XPlane& source_plane : *source->mutable_planes()) {
    XPlane* plane = target->add_planes();
    plane->Swap(&source_plane);
  }
  for (auto& error : source->errors()) {
    target->add_errors(error);
  }
  for (auto& hostname : source->hostnames()) {
    target->add_hostnames(hostname);
  }
}
