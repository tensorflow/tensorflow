...
TEST_F(SubprocessProfilingSessionTest, CreateAndStartSession) {
  std::unique_ptr<tensorflow::grpc::ProfilerService::Service> service =
      tsl::profiler::CreateProfilerService();
  ...
  ASSERT_GT(port, 0);
  absl::Cleanup server_cleanup = [&] { server->Shutdown(); };
  ...
  options.set_repository_path(tsl::testing::TmpDir());
  options.set_host_tracer_level(3);
  options.set_python_tracer_level(3);
  ...
  // Wait for profiler to start as the request is async.
