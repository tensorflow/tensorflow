...
KernelThunk<num_arguments, num_results>::ExecuteInternal(
  ...
  if constexpr (ShouldCheckBufferSlices()) {
    ...
    ABSL_RETURN_IF_ERROR(CheckInvariantBuffersMemory(kernel_args));
  }
  ...
  ABSL_RETURN_IF_ERROR(kernel->Launch(num_workgroups_, kernel_args));
  return OkExecuteEvent();
}
