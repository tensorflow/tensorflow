...
tsl::AsyncValueRef<DotThunk::ExecuteEvent> DotThunk::Execute(
  ...
  if (!dot_canonical_dims_.lhs_column_major) {
    std::swap(dot_canonical_dims_.m, dot_canonical_dims_.n);
    std::swap(lhs, rhs);
  ...
  int64_t lhs_stride =
      dot_canonical_dims_.m * dot_canonical_dims_.k * byte_width;
  int64_t rhs_stride =
      dot_canonical_dims_.k * dot_canonical_dims_.n * byte_width;
  int64_t out_stride =
      dot_canonical_dims_.m * dot_canonical_dims_.n * byte_width;
  ...
  auto dispatch = [&](auto type_tag) {
    for (int64_t i = 0; i < dot_shape_.batch_size; ++i) {
      TypedMatMul<decltype(type_tag)>(
          ...
          dot_canonical_dims_.m, dot_canonical_dims_.n, dot_canonical_dims_.k,
