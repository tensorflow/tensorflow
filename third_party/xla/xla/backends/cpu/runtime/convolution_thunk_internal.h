    ...
    for (int h = 0; h < h_patches_number; ++h) {
      ...
      for (int iw = w_patch_begin; iw < w_patch_begin + filter_w; ++iw) {
        for (int ih = h_patch_begin; ih < h_patch_begin + filter_h; ++ih) {
          ...
          // contains data for one feature group, while 'out_im_data' contains
...
bool EigenTransposedConv2D(
  ...
  typedef Eigen::TensorMap<Eigen::Tensor<ScalarType, 3, Eigen::RowMajor>,
                           Eigen::Unaligned>
      TensorMap3D;
  typedef Eigen::TensorMap<Eigen::Tensor<const ScalarType, 4, Eigen::RowMajor>,
                           Eigen::Aligned>
      ConstTensorMap4D;
