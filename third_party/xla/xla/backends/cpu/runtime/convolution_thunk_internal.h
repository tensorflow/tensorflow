...
void Pack2DPatches(const T* col_data, const int depth, const int height,
  ...
  int width_col = (width + filter_w - pad_left - pad_right - 2) / stride_w + 1;
  int height_col =
      (height + filter_h - pad_top - pad_bottom - 2) / stride_h + 1;

  const int filter_spatial_size = filter_h * filter_w;

  int w_patch_data = pad_left - filter_w + 1;
  ...
  for (int w = 0; w < width_col; ++w) {
    ...
    for (int h = 0; h < height_col; ++h) {
      ...
      T* out_im_patch_data =
          out_im_data + (w_patch_data * height + h_patch_begin) * depth;

      for (int iw = w_patch_data; iw < w_patch_data + filter_w; ++iw) {
...
void EigenTransposedConv2D(
  ...
  TensorMap2D C(col_buffer_data, input_batch * input_image_size,
                kernel_total_size);
