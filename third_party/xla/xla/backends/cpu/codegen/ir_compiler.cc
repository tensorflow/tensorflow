  ...
  pb.registerOptimizerLastEPCallback([&](llvm::ModulePassManager& mpm,
    ...
    if (options_.msan_enabled) {
      llvm::MemorySanitizerOptions msan_options(
          options_.msan_track_origins, /*Recover=*/false, /*Kernel=*/false,
          // Set eager checks to true. This is important to avoid msan flakes on
          // KernelThunk's call frame pointer argument with AOT kernels: eager
          // checks + nonnull/noundef annotations on the pointer make the AOT
          // kernel never read the pointer's msan shadow memory. Without either,
          // the AOT kernel would read shadow memory, but the host would not
          // have written to it when compiled with eager checks enabled, which
          // is the default in Clang.
          // Note that if the host is built without eager checks, things still
          // work: the host will write the call frame pointer's shadow memory,
          // and the AOT kernel won't read it. For pointers flowing in the
          // opposite direction (i.e. AOT -> host, such as the return pointer
          // from a kernel) we do not mark them as noundef/nonnull, so we
          // always write their corresponding shadow memory.
          /*EagerChecks=*/true);
