...
absl::StatusOr<std::vector<FlatTiling>> GetTiling(
    const HloFusionInstruction& fusion, SymbolicExprContext& context) {
...
absl::StatusOr<std::vector<FlatTiling>> GetTilingIfSupported(
    const HloFusionInstruction& fusion, SymbolicExprContext& expr_context) {
  ...
  return GetTiling(fusion, expr_context);
}
