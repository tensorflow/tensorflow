...
inline absl::Status InsertConvertIfNecessary(
  ...
  TF_XLA_RETURN_IF_ERROR(instr->ReplaceAllUsesWith(convert));
...
absl::StatusOr<LibraryMatcher*> LibraryRewriter::ChooseLibrary(
  ...
  for (std::unique_ptr<LibraryMatcher>& lib : libs_) {
    TF_XLA_ASSIGN_OR_RETURN(bool op_supported, lib->IsOpSupported(instr));
...
absl::StatusOr<HloFusionInstruction*> LibraryRewriter::MergeFusionInstructions(
  ...
  if (dir == FusionDirection::kUp) {
    ...
    TF_XLA_RETURN_IF_ERROR(main->parent()->RemoveInstruction(neighbor));
    return main;
  }
  if (dir == FusionDirection::kDown) {
    neighbor->MergeFusionInstruction(main);
    TF_XLA_RETURN_IF_ERROR(neighbor->parent()->RemoveInstruction(main));
    return neighbor;
  }
...
absl::StatusOr<HloInstruction*> LibraryRewriter::GrowFusion(
  ...
  if (dir == FusionDirection::kUp) {
    ...
    if (to_fuse->user_count() == 0) {
      TF_XLA_RETURN_IF_ERROR(to_fuse->parent()->RemoveInstruction(to_fuse));
    }
  } else if (dir == FusionDirection::kDown) {
    TF_XLA_ASSIGN_OR_RETURN(new_instr, FuseConsumerInstruction(fusion, to_fuse));
  }
  return new_instr;
}
...
absl::Status LibraryRewriter::FuseNeighbors(HloFusionInstruction* fusion,
  ...
  while (!frontier.empty()) {
    ...
    if (IsCustomFusionWithKind(instr, lib->fusion_kind())) {
      TF_XLA_ASSIGN_OR_RETURN(fusion,
    ...
    TF_XLA_ASSIGN_OR_RETURN(HloInstruction * new_instr,
...
absl::StatusOr<bool> LibraryRewriter::ProcessComputation(
  ...
  for (HloInstruction* centroid : fusion_starters) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(LibraryMatcher * lib, ChooseLibrary(centroid));
    if (lib == nullptr) {
      continue;
    }
    ...
    TF_XLA_ASSIGN_OR_RETURN(HloFusionInstruction * fusion,
                        CreateLibraryFusion(centroid, lib->fusion_prefix(),
                                            lib->fusion_kind()));
    TF_XLA_RETURN_IF_ERROR(InsertConvertIfNecessary(
        fusion->fused_expression_root(), lib->LibraryOpOutputType(centroid)));

    // Fuse as many neighbors as as we can.
    TF_XLA_RETURN_IF_ERROR(FuseNeighbors(fusion, lib));
  }
  return !fused_.empty();
}
...
absl::StatusOr<bool> LibraryRewriter::RunImpl(
  ...
  for (HloComputation* computation : module->MakeNonfusionComputations()) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(bool comp_changed, ProcessComputation(computation));
