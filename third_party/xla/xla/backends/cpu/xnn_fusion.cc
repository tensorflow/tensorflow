...
struct Gemm {
  ...
  auto operator<=>(const Gemm& other) const = default;
};

enum class Opinion { Accept, Reject, NoIdea };

// TODO (ashaposhnikov): This really should use cpu arch/microarch.
struct GemmConfig {
  std::vector<Gemm> fast_gemms;
  std::vector<Gemm> slow_gemms;

  Opinion consult(const Gemm& gemm) const {
    if (std::find(fast_gemms.begin(), fast_gemms.end(), gemm) !=
        fast_gemms.end()) {
      return Opinion::Accept;
    }
    if (std::find(slow_gemms.begin(), slow_gemms.end(), gemm) !=
        slow_gemms.end()) {
      return Opinion::Reject;
    }
    return Opinion::NoIdea;
  }
};
}  // anonymous namespace
...
absl::StatusOr<bool> IsDotSupportedByXnn(
    ...
    const TargetMachineFeatures* cpu_features) {
  std::cerr << "shal1t7: inside IsDotSupportedByXnn \n";
  ...
  const Gemm gemm{.dot_canonical_dims = dot_canonical_dims,
                  .lhs_dtype = lhs_shape.element_type(),
                  .rhs_dtype = rhs_shape.element_type(),
                  .out_dtype = out_shape.element_type()};

  // TODO(ashaposhnikov): this really should be autogenerated ahead of time from
  // benchmarks.
  static const GemmConfig* gemm_config =
      new GemmConfig{.fast_gemms = {Gemm{
                         .dot_canonical_dims =
                             DotCanonicalDims{
                                 .m = 2,
                                 .k = 32,
                                 .n = 16,
                                 .lhs_column_major = false,
                                 .lhs_canonical = true,
  ...
  switch (gemm_config->consult(gemm)) {
    case Opinion::Accept:
      return true;
    case Opinion::Reject:
      return true;
