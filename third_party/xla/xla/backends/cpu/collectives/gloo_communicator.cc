...
Future<> GlooCommunicator::AllReduce(se::DeviceMemoryBase send_buffer,
  ...
  switch (dtype) {
    case S8:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<int8_t>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case PRED:
    case U8:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<uint8_t>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case S16:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<int16_t>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case U16:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<uint16_t>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case S32:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<int32_t>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case U32:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<uint32_t>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case S64:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<int64_t>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case U64:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<uint64_t>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case F16:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<gloo::float16>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case BF16:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<bfloat16>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case F32:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<float>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case F64:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<double>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case C64:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<std::complex<float>>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    case C128:
      TF_XLA_RETURN_IF_ERROR(SetAllReduceOptions<std::complex<double>>(
          reduction_kind, send_buffer, recv_buffer, count, options));
      break;
    default:
...
Future<> GlooCommunicator::ReduceScatter(se::DeviceMemoryBase send_buffer,
  ...
  switch (dtype) {
    case S8:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<int8_t>(context_, reduction_kind,
                                                     temp.get(), count));
      break;
    case PRED:
    case U8:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<uint8_t>(context_, reduction_kind,
                                                      temp.get(), count));
      break;
    case S16:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<int16_t>(context_, reduction_kind,
                                                      temp.get(), count));
      break;
    case U16:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<uint16_t>(context_, reduction_kind,
                                                       temp.get(), count));
      break;
    case S32:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<int32_t>(context_, reduction_kind,
                                                      temp.get(), count));
      break;
    case U32:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<uint32_t>(context_, reduction_kind,
                                                       temp.get(), count));
      break;
    case S64:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<int64_t>(context_, reduction_kind,
                                                      temp.get(), count));
      break;
    case U64:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<uint64_t>(context_, reduction_kind,
                                                       temp.get(), count));
      break;
    case BF16:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<bfloat16>(context_, reduction_kind,
                                                       temp.get(), count));
      break;
    case F16:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<gloo::float16>(
          context_, reduction_kind, temp.get(), count));
      break;
    case F32:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<float>(context_, reduction_kind,
                                                    temp.get(), count));
      break;
    case F64:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<double>(context_, reduction_kind,
                                                     temp.get(), count));
      break;
    case C64:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<std::complex<float>>(
          context_, reduction_kind, temp.get(), count));
      break;
    case C128:
      TF_XLA_RETURN_IF_ERROR(ReduceScatterHelper<std::complex<double>>(
          context_, reduction_kind, temp.get(), count));
      break;
    default:
