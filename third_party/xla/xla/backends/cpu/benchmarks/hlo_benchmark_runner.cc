...
absl::Status RunHloBenchmark(benchmark::State& state,
  ...
  if (benchmark_options.aot_options) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(executable, cpu_client->CompileAheadOfTimeAndLoad(
  ...
  if (args.empty()) {
    TF_XLA_ASSIGN_OR_RETURN(std::vector<Literal> fake_args,
                        MakeFakeArguments(module.get()));
    for (auto& args_buffers : execution_args_buffers) {
      args_buffers.reserve(fake_args.size());
      for (const Literal& arg : fake_args) {
        TF_XLA_ASSIGN_OR_RETURN(args_buffers.emplace_back(),
                            client->BufferFromHostLiteral(arg, memory_space));
        TF_XLA_RETURN_IF_ERROR(args_buffers.back()->GetReadyFuture().Await());
      }
    }
  } else {
  ...
  auto run_benchmark_once = [&]() -> absl::Status {
    ...
    for (size_t i = 0; i < benchmark_options.num_executions; ++i) {
      ...
      std::vector<std::unique_ptr<PjRtBuffer>>& args_buffers =
          execution_args_buffers[i];
      std::vector<PjRtBuffer*>& args_ptrs = execution_args_ptrs[i];
      TF_XLA_RETURN_IF_ERROR(alias_helper.SwapOutputAliasedBuffersToArgumentBuffers(
  ...
  // Warm up executable.
  TF_XLA_RETURN_IF_ERROR(run_benchmark_once());
...
LoadFromHloSnapshotOrHloModuleProto(absl::string_view hlo_data,
  ...
  if (extension == "pb" || extension == "pbtxt") {
    TF_XLA_ASSIGN_OR_RETURN(iteration_literals_proto,
                        LoadInputFromData(hlo_data, extension));
  }

  TF_XLA_ASSIGN_OR_RETURN(auto hlo_module, LoadModuleFromData(hlo_data, extension));
