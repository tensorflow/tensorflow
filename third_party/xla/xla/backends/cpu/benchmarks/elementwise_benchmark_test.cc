...
void GenerateElementwiseArgs(benchmark::internal::Benchmark* benchmark) {
  benchmark->MeasureProcessCPUTime();
  benchmark->ArgNames({"d0", "is_aot"});
  const std::vector<bool> is_aot_values = {false, true};
  std::vector<int64_t> args_values = {128, 256, 512, 1024, 8192, 16384, 32768};
...
BENCHMARK(BM_AddF32)->Apply(GenerateElementwiseArgs);
BENCHMARK(BM_AddBF16)->Apply(GenerateElementwiseArgs);
BENCHMARK(BM_ConvertF32ToBF16)->Apply(GenerateElementwiseArgs);
