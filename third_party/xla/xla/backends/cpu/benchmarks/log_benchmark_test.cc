...
static void BM_LogF32(benchmark::State& state) {
  ...
  absl::string_view hlo = R"(
    HloModule log_f32_$d0

    ENTRY e {
      input = f32[$d0] parameter(0)
      ROOT output = log(input)
    }
  )";
...
void GenerateLogArgs(benchmark::internal::Benchmark* benchmark) {
...
BENCHMARK(BM_LogF32)->Apply(GenerateLogArgs);
// TODO(basioli): something seems of with AOT and f16 log
BENCHMARK(BM_LogF16)
