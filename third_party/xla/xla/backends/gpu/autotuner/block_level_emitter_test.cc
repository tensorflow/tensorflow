...
// Checks if all configs have is_tma_allowed set to the expected value.
bool CheckTmaAllowed(const std::vector<std::unique_ptr<BackendConfig>>& configs,
                     bool tma_enabled) {
  return std::all_of(
      configs.begin(), configs.end(),
      [tma_enabled](const std::unique_ptr<BackendConfig>& config) {
        BlockLevelFusionConfig actual_config;
        if (!config->UnpackTo(&actual_config)) {
          return false;
        }
        return actual_config.is_tma_allowed() == tma_enabled;
      });
}
...
ENTRY %main {
  ...
  bool is_tma_supported = backend_.target_config()
                              ...
                              .IsAtLeastHopper();
  EXPECT_TRUE(CheckTmaAllowed(configs, /*tma_enabled=*/is_tma_supported));
