...
class TritonBlockLevelFusionEmitterBackendTest
 ...
 protected:
  ...
  TritonBlockLevelFusionEmitterBackend backend_;
};
...
ENTRY %main {
  ...
  EXPECT_EQ(config.value()->GetDescriptor(),
  ...
  const BlockLevelFusionConfig& block_level_fusion_config =
      static_cast<const BlockLevelFusionConfig&>(*config.value());
...
ENTRY %main {
  ...
  absl::StatusOr<std::unique_ptr<BackendConfig>> config =
      backend_.GetDefaultConfig(
          *(module->entry_computation()->root_instruction()));
  EXPECT_THAT(config, IsOk());
  // Verify the returned config is a BlockLevelFusionConfig.
  EXPECT_EQ(config.value()->GetDescriptor(),
            BlockLevelFusionConfig::GetDescriptor())
      << "Config is not a BlockLevelFusionConfig";
  const BlockLevelFusionConfig& block_level_fusion_config =
...
ENTRY %main {
  ...
  absl::StatusOr<std::unique_ptr<BackendConfig>> config =
      backend_.GetDefaultConfig(
          *(module->entry_computation()->root_instruction()));
  EXPECT_THAT(config, IsOk());
  // Verify that the returned config is a BlockLevelFusionConfig.
  EXPECT_EQ(config.value()->GetDescriptor(),
            BlockLevelFusionConfig::GetDescriptor())
      << "Config is not a BlockLevelFusionConfig";
  const BlockLevelFusionConfig& block_level_fusion_config =
  ...
  // - The tile size for the dimension with size 10 is 8
  // - The zero-sized dimension remains zero
  // - The tile size for the dimension with size 5 is 4.
  // Also verify default tuning parameters: 1 warp, 1 CTA, 1 stage.
  EXPECT_THAT(block_level_fusion_config, EqualsProto(R"pb(
                output_tiles { sizes: 8 sizes: 0 sizes: 4 }
                num_warps: 1
                num_ctas: 1
                num_stages: 1
              )pb"));
...
ENTRY %main {
  ...
  EXPECT_EQ(configs.value().size(), 35);
  int i = 0;
  ...
  for (int d0 : {1, 2, 4, 8, 16, 32, 64}) {
    for (int d2 : {1, 2, 4, 8, 16}) {
      EXPECT_EQ(configs.value()[i]->GetDescriptor(),
                BlockLevelFusionConfig::GetDescriptor())
          << "Config is not a BlockLevelFusionConfig";
      const BlockLevelFusionConfig& block_level_fusion_config =
          static_cast<const BlockLevelFusionConfig&>(*configs.value()[i]);
...
ENTRY %main {
  ...
  absl::StatusOr<std::vector<std::unique_ptr<BackendConfig>>> configs =
      backend_.GetSupportedConfigs(
          *(module->entry_computation()->root_instruction()));
  EXPECT_THAT(configs, IsOk());
  // Expect 12 total configurations:
  // - 4 choices for d0 (output dim 0 = 10): 1, 2, 4, 8
  // - 3 choices for d2 (output dim 2 = 5): 1, 2, 4
  // The middle dimension (d1 = 0) must always have tile size 0.
  EXPECT_EQ(configs.value().size(), 12);
  int i = 0;
  ...
  for (int d0 : {1, 2, 4, 8}) {
    for (int d2 : {1, 2, 4}) {
      ...
      // - d0: 10 → tile sizes {1,2,4,8}
      // - d1: 0  → must be tile size 0
      // - d2: 5  → tile sizes {1,2,4}
      EXPECT_THAT(block_level_fusion_config,
                  EqualsProto(absl::Substitute(
                      R"pb(
...
ENTRY %main {
  ...
  EXPECT_THAT(backend_.ApplyConfig(*instr, *config.value()), IsOk());
