...
absl::Status LegacyCache::Insert(const HloInstruction* instr,
  ...
  std::optional<AutotuneResult> opt_result =
      GetAutotuneResult(best_config, instr);
...
std::optional<LegacyCache::Config> LegacyCache::GetConfig(
  ...
  std::string codegen_backend;
  google::protobuf::Any backend_config;
  if (result.has_triton()) {
    codegen_backend = "Triton";
    backend_config.PackFrom(result.triton());
  } else if (result.has_gemm()) {
    codegen_backend = "Cublas";
    backend_config.PackFrom(result.gemm());
  } else if (result.has_conv()) {
    codegen_backend = "Cudnn";
    backend_config.PackFrom(result.conv());
  } else {
    return std::nullopt;
  }
  return Config(codegen_backend, backend_config);
}

std::optional<AutotuneResult> LegacyCache::GetAutotuneResult(
    const LegacyCache::Config& config, const HloInstruction* instr) {
  AutotuneResult result;
  if (config.codegen_backend == "Triton") {
    config.backend_config.UnpackTo(result.mutable_triton());
  } else if (config.codegen_backend == "Cublas") {
    config.backend_config.UnpackTo(result.mutable_gemm());
  } else if (config.codegen_backend == "Cudnn") {
