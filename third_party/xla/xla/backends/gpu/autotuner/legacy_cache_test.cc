...
class LegacyCacheTest : public ::testing::Test {
 protected:
  ...
  Config CreateDummyTritonConfig() {
    Config config;
    config.codegen_backend = "Triton";
    google::protobuf::Any any;
    any.PackFrom(AutotuneResult::TritonGemmKey());
    config.backend_config = any;
    return config;
  }

  Config CreateDummyCublasConfig() {
    Config config;
    config.codegen_backend = "Cublas";
    google::protobuf::Any any;
    any.PackFrom(AutotuneResult::GemmKey());
    config.backend_config = any;
    return config;
  }
  ...
  Config CreateDummyCudnnConfig() {
    Config config;
    config.codegen_backend = "Cudnn";
    google::protobuf::Any any;
    any.PackFrom(AutotuneResult::ConvKey());
    config.backend_config = any;
    return config;
  }
};
...
TEST_F(LegacyCacheTest, InsertAndLookupUnsupportedBackendSilentlyFails) {
  auto cache = LegacyCache(test_dir_, mode_, device_desc_);
  auto instr = CreateDummyInstr("hlo4");
  Config config;
  config.codegen_backend = "UnsupportedBackend";
