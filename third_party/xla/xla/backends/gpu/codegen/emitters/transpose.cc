...
PackedTranspose::PackedTranspose(const HloFusionAnalysis& analysis,
  ...
  if (VLOG_IS_ON(5)) {
    llvm::errs() << "Transpose spec: " << spec.ToString()
                 << "Output block tile: "
                 << absl::StrJoin(output_block_tile, ", ")
                 << "\nNumber of warps: " << num_warps << "\n";
  }
  auto elem_type = GetBitwidth(spec_.elem_type());
  vector_size_ = kBankBitwidth / elem_type;
  ...
  auto body_builder = [&](ImplicitLocOpBuilder& nested_b,
    ...
    auto input_indices = [&](const HloInstruction* instr) {
      auto indexing = ComposeIndexingMaps(
          input_indexing,
          GetBitcastMap(spec_.input_shape(), instr->shape(), ctx));
      indexing.Simplify();
      return ApplyIndexing(indexing, thread_and_block_ids, symbol_values,
                           nested_b);
    };
...
void PackedTranspose::EmitReadFromShMemMlir(
  ...
  auto outer_loop_indexing =
      ConvertRangeVariablesToDimensions(shmem_read_indexing, {1, 2});
  auto output_indexing = GetOutputIndexing(mlir_context);
  auto output_indexing_over_vectors =
      ConvertRangeVariablesToDimensions(output_indexing, {0});
...
IndexingMap PackedTranspose::GetInputIndexing(mlir::MLIRContext* ctx) const {
  ...
  IndexingMap canonical_input_indexing{
      mlir::AffineMap::get(6, 2, canonical_offsets, ctx), std::move(dim_vars),
...
std::unique_ptr<EmitterBase> CreateTransposeFusion(
  ...
  auto transpose = std::find_if(analysis.fusion_heroes().begin(),
                                analysis.fusion_heroes().end(),
                                [](const HloInstructionAdaptor& hero) {
                                  return hero.opcode() == HloOpcode::kTranspose;
                                });
  if (transpose != analysis.fusion_heroes().end()) {
    auto spec = GetTransposeSpec(
        Cast<HloTransposeInstruction>(&transpose->instruction()));
