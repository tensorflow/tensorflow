...
TransposeFusion::WriteResult TransposeFusion::EmitWriteToShMemMlir(
  ...
  // TODO(karupayun): This is a temporary solution. Fix it.
  ...
  IndexingMap write_indexing =
      GetSharedMemoryIndexing(/*read=*/false, &symbolic_expr_context);
  auto body_builder = [&](ImplicitLocOpBuilder& nested_b,
    ...
    auto input_indices = [&](const HloInstruction* instr) {
      return ApplyIndexing(
          GetIndexing(/*input=*/true, instr->shape(), &symbolic_expr_context),
...
void TransposeFusion::EmitReadFromShMemMlir(
  ...
  auto output_indexing = *ComputeThreadIdToOutputIndexing(
      shmem_transpose_root_indices_[0], &symbolic_expr_context);
  auto shmem_read_indexing =
      GetSharedMemoryIndexing(/*read=*/true, &symbolic_expr_context);
...
PackedTranspose::WriteResult PackedTranspose::EmitWriteToShMemMlir(
    ...
    ValueRange output_args, mlir::ValueRange thread_and_block_ids) const {
  MLIRContext* mlir_context = builder.getContext();
  // TODO(karupayun): Fix it.
  SymbolicExprContext symbolic_expr_context(mlir_context);
  IndexingMap input_indexing = GetInputIndexing(&symbolic_expr_context);
  IndexingMap shmem_write_indexing =
      GetShmemWriteIndexing(&symbolic_expr_context);
...
void PackedTranspose::EmitReadFromShMemMlir(
  ...
  // TODO(karupayun): Fix it.
  SymbolicExprContext symbolic_expr_context(mlir_context);
  auto shmem_read_indexing = GetShmemReadIndexing(&symbolic_expr_context);
  auto outer_loop_indexing = ConvertRangeVariablesToDimensions(
      shmem_read_indexing, /*range_var_indices=*/{1, 2});
  auto output_indexing = GetOutputIndexing(&symbolic_expr_context);
