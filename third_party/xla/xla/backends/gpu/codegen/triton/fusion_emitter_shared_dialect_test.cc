...
TEST(SharedDialectTest, TestEmittingStableHloIota) {
  constexpr absl::string_view kHloText = R"(
HloModule t, is_scheduled=true
...
ENTRY e {
  ...
  mlir::MLIRContext context;
  EXPECT_OK(CreateSharedDialectIrAndFileCheck(
      context, *module->GetComputationWithName("iota_fusion"),
      block_level_parameters,
      R"(
...
TEST(SharedDialectTest, TestEmittingStableHloBroadcastInDim) {
  constexpr absl::string_view kHloText = R"(
HloModule t, is_scheduled=true

broadcast_in_dim_fusion {
  p0 = f32[150,160] parameter(0)
  ROOT broadcast = s32[150,160,31] broadcast(p0), dimensions={0,1}
}

ENTRY e {
  p0 = f32[150,160] parameter(0)
  ROOT custom-call = s32[150,160,31] fusion(p0), kind=kCustom,
  ...
  mlir::MLIRContext context;
  EXPECT_OK(CreateSharedDialectIrAndFileCheck(
      context, *module->GetComputationWithName("broadcast_in_dim_fusion"),
      block_level_parameters,
      R"(
CHECK: %[[RES:.*]] = "stablehlo.broadcast_in_dim"(%[[ARG:.*]]) <{broadcast_dimensions = array<i64: 0, 1>}> : (tensor<16x32xf32>) -> tensor<16x32x8xf32>
)"));
}
