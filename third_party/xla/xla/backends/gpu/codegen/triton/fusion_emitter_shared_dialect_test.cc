...
TEST_F(XTileDialectTest, HloAllReduceIsLoweredToStableHloAllReduce) {
  constexpr absl::string_view kHloText =
      R"(
      HloModule wrapped_module_all-reduce-start, entry_computation_layout={(f32[65536]{0})->f32[65536]{0}}

      %apply_op (x: f32[], y: f32[]) -> f32[] {
        %x = f32[] parameter(0)
        %y = f32[] parameter(1)
        ROOT %apply_op = f32[] add(%x, %y)
      }

      %wrapped_all-reduce-start (param_0: f32[65536]) -> f32[65536] {
        %param_0 = f32[65536]{0} parameter(0)
        ROOT %all-reduce-start = f32[65536]{0} all-reduce-start(%param_0), replica_groups={{0,1}}, to_apply=%apply_op
      }

      ENTRY %entry (param_0.1: f32[65536]) -> f32[65536] {
        %param_0.1 = f32[65536]{0} parameter(0)
        ROOT %fusion = f32[65536]{0} fusion(%param_0.1), kind=kLoop, calls=%wrapped_all-reduce-start, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"fusion_backend_config":{"kind":"__triton_collective","block_level_fusion_config":{"num_warps":"16","output_tiles":[{"sizes":["4096"]}],"num_ctas":1,"num_stages":1,"is_tma_allowed":false,"is_warp_specialization_allowed":false}},"force_earliest_schedule":false,"reification_cost":[],"device_type":"DEVICE_TYPE_INVALID"}
      }
    )";
