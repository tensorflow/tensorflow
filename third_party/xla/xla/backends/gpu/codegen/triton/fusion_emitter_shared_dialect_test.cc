...
ENTRY e {
  ...
  TF_EXPECT_OK(CreateXTileIrAndFileCheck(
      ...
      block_level_parameters,
      R"(
CHECK: %[[RES:.*]] = "func.call"(%[[ARG:.*]], %[[ARG:.*]], %[[ARG:.*]], %[[ARG:.*]], %[[ARG:.*]]) <{callee = @scaled_dot}> {fast_math = true} : (tensor<128x128xf8E5M2>, tensor<128x256xf8E5M2>, tensor<128x256xf32>, tensor<128x4xf8E8M0FNU>, tensor<4x256xf8E8M0FNU>) -> tensor<128x256xf32>
      )"));
}
