...
func.func @fold_squeeze_dims_of_extract(%arg0: !tt.ptr<f32>, %arg1: i32) -> tensor<4x8xf32> {
  ...
  // CHECK: %[[PTR:.*]] = tt.addptr %arg0, %[[C24]]
  // CHECK: %[[EXTRACT:.*]] = triton_xla.extract from %[[PTR]] as memref<4x8xf32,
  // CHECK-SAME: #triton_xla.layout<[1, 0]>> [0, 0] [4, 8] [1, 1] : tensor<4x8xf32>
  %0 = triton_xla.extract from %arg0
    as memref<4x16x8xf32, #triton_xla.layout<[2, 1, 0]>>
    [0, 3, 0] [4, 16, 8] [1, 1, 1] : tensor<4x1x8xf32>
  %1 = triton_xla.squeeze_dims %0 {axis = 1 : i32} : tensor<4x1x8xf32> -> tensor<4x8xf32>
...
func.func @squeeze_insert(%arg0: !tt.ptr<f32>, %arg1: tensor<4x1x8xf32>) {
  // CHECK: %[[C24:.*]] = arith.constant 24
  // CHECK-DAG: %[[PTR:.*]] = tt.addptr %arg0, %[[C24]]
  // CHECK-DAG: %[[SRC:.*]] = triton_xla.squeeze_dims %arg1 {axis = 1 : i32}
  // CHECK-DAG: triton_xla.insert %[[SRC]] into %[[PTR]]
