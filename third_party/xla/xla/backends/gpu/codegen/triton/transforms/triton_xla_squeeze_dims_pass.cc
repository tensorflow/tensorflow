...
auto RemoveAxis(ContainerT&& values, absl::flat_hash_set<int32_t> squeeze_dims)
    -> llvm::SmallVector<typename ContainerT::value_type> {
  if (values.empty()) {
    return {};
  }

  llvm::SmallVector<typename ContainerT::value_type> result;
  result.reserve(values.size() - 1);
  for (auto [dim, value] : llvm::enumerate(values)) {
    if (!squeeze_dims.contains(dim)) {
