...
template <typename ContainerT>
auto SqueezeLayout(ContainerT&& layout, ArrayRef<uint32_t> squeeze_dims) {
  auto new_layout = SqueezeElements(layout, squeeze_dims);
  for (auto& dim : new_layout) {
    dim -= absl::c_upper_bound(squeeze_dims, dim) - squeeze_dims.begin();
  }
  return new_layout;
}

Value createOffsetPointer(PatternRewriter& rewriter, Location loc, Value ptr,
  ...
  for (uint32_t dim : squeeze_dims) {
    Value stride = rewriter.create<arith::ConstantOp>(
        loc, rewriter.getI64IntegerAttr(strides[dim]));
    Value offset = dyn_cast<Value>(offsets[dim]);
    if (!offset) {
      offset = rewriter.create<arith::ConstantOp>(
          loc, cast<TypedAttr>(cast<Attribute>(offsets[dim])));
    }
    ptr = rewriter.create<AddPtrOp>(
        loc, ptr.getType(), ptr,
        rewriter.create<arith::MulIOp>(loc, offset, stride));
  }
  return ptr;
}
...
LogicalResult FoldSqueezeDimsOfExtract(ExtractOp op,
  ...
  Value src = createOffsetPointer(rewriter, op.getLoc(), op.getSrc(),
                                  op.getMixedOffsets(), op.getSrcShape(),
                                  op.getSrcLayout(), *axis);
  Value new_op = rewriter.create<ExtractOp>(
      op.getLoc(), SqueezeTensorType(op.getType(), *axis), src,
      SqueezeElements(op.getMixedOffsets(), *axis),
      SqueezeElements(op.getMixedStrides(), *axis),
      SqueezeElements(op.getSrcShape(), *axis),
      SqueezeLayout(op.getSrcLayout(), *axis));
...
LogicalResult SqueezeInsert(InsertOp op, PatternRewriter& rewriter) {
  ...
  Value dst =
      createOffsetPointer(rewriter, loc, op.getDst(), op.getMixedOffsets(),
                          op.getDstShape(), op.getDstLayout(), squeeze_dims);
  rewriter.replaceOpWithNewOp<InsertOp>(
      op, src, dst, SqueezeElements(op.getMixedOffsets(), squeeze_dims),
      SqueezeElements(op.getMixedStrides(), squeeze_dims),
      SqueezeElements(op.getDstShape(), squeeze_dims),
      SqueezeLayout(op.getDstLayout(), squeeze_dims));
  return success();
}
...
LogicalResult PushSqueezeDimsUpThroughTrans(SqueezeDimsOp op,
  ...
  auto new_order = SqueezeLayout(order, dst_axis);
