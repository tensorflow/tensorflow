...
LogicalResult PushTransposeThroughExtractTile(TransOp op,
  ...
  auto pushed_transpose = rewriter.create<mlir::memref::TransposeOp>(
      extract.getLoc(), extract.getSource(), permutation_map);

  auto new_extract = rewriter.create<::xla::xtile::ExtractTileOp>(
      op.getLoc(), op.getType(), pushed_transpose,
      permute(extract.getOffsets()), permute(extract.getFullTileShape()),
      permute(extract.getStrides()));
  rewriter.replaceOp(op, new_extract);
