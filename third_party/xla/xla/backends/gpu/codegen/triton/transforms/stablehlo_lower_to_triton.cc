...
std::optional<arith::CmpIPredicate> GetCmpIPredicate(
    stablehlo::ComparisonDirection direction, bool is_unsigned) {
  switch (direction) {
    ...
    case stablehlo::ComparisonDirection::LT:
      return is_unsigned ? arith::CmpIPredicate::ult
                         : arith::CmpIPredicate::slt;
    case stablehlo::ComparisonDirection::GT:
      return is_unsigned ? arith::CmpIPredicate::ugt
                         : arith::CmpIPredicate::sgt;
    case stablehlo::ComparisonDirection::LE:
      return is_unsigned ? arith::CmpIPredicate::ule
                         : arith::CmpIPredicate::sle;
    case stablehlo::ComparisonDirection::GE:
      return is_unsigned ? arith::CmpIPredicate::uge
...
class LowerCompareOp : public mlir::OpRewritePattern<stablehlo::CompareOp> {
 public:
  using OpRewritePattern::OpRewritePattern;

 private:
  mlir::LogicalResult matchAndRewrite(
      stablehlo::CompareOp op, mlir::PatternRewriter& rewriter) const override {
    ...
    if (mlir::isa<mlir::IntegerType>(element_type)) {
      if (element_type.isUnsignedInteger()) {
        lhs = UnsignedIntegerToSignlessInteger(rewriter, lhs);
        rhs = UnsignedIntegerToSignlessInteger(rewriter, rhs);
      }

      new_op = arith::CmpIOp::create(
          rewriter, op.getLoc(),
      ...
      if (src_element_ty.getIntOrFloatBitWidth() <
        ...
        if (src_element_ty.isUnsignedInteger()) {
          Value signless_integer_value =
              UnsignedIntegerToSignlessInteger(rewriter, value);
          Type signless_dst_integer_type = IntegerType::get(
              rewriter.getContext(), dst_element_ty.getIntOrFloatBitWidth(),
              IntegerType::SignednessSemantics::Signless);
          ...
          auto ext_op = arith::ExtUIOp::create(rewriter, op.getLoc(),
                                               signless_dst_integer_type,
                                               signless_integer_value);

          rewriter.replaceOpWithNewOp<UnrealizedConversionCastOp>(
              op, dst_ty, ext_op.getResult());
          return mlir::success();
        } else if (src_element_ty.isInteger(1)) {
 ...
 public:
  void runOnOperation() override {
    ...
    patterns.add<
        ...
        LowerReshape, LowerDotGeneral, LowerCompareOp, LowerConvertOp,
