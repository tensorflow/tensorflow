...
class LowerCompareOp : public mlir::OpRewritePattern<stablehlo::CompareOp> {
 ...
 private:
  mlir::LogicalResult matchAndRewrite(
    ...
    const Type type = mlir::getElementTypeOrSelf(op.getLhs());
    ...
    if (mlir::isa<mlir::IntegerType>(type)) {
      rewriter.replaceOpWithNewOp<arith::CmpIOp>(
          ...
          GetCmpIPredicate(
              direction,
              /*is_signed=*/type.isSignedInteger() || !type.isInteger(1))
              .value(),
          op.getLhs(), op.getRhs());
    } else {
      rewriter.replaceOpWithNewOp<arith::CmpFOp>(
          op, GetCmpFPredicate(direction).value(), op.getLhs(), op.getRhs());
    }
    return mlir::success();
  }
};
