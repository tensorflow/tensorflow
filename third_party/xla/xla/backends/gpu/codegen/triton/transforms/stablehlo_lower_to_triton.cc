...
class LowerTranspose : public mlir::OpRewritePattern<stablehlo::TransposeOp> {
 ...
 private:
  mlir::LogicalResult matchAndRewrite(
    ...
    auto triton_transpose_op = ttir::TransOp::create(
        builder, op.getResult().getType(), op.getOperand(), permutation);
    rewriter.replaceOp(op, triton_transpose_op.getResult());
...
class LowerBitcastConvert
 ...
 private:
  mlir::LogicalResult matchAndRewrite(
      ...
      mlir::PatternRewriter& rewriter) const override {
    ::xla::EmitterLocOpBuilder builder(op.getLoc(), rewriter);
    auto triton_bitcast_op = ttir::BitcastOp::create(
        builder, op.getResult().getType(), op.getOperand());
    rewriter.replaceOp(op, triton_bitcast_op.getResult());
    return mlir::success();
  }
};
...
class StableHLOLowerToTritonPass
 ...
 public:
  void runOnOperation() override {
    ...
    patterns.add<LowerTranspose, LowerBitcastConvert>(mlir_context);
