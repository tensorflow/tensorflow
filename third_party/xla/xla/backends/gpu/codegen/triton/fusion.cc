...
void PopulateNvvmAnnotations(llvm::Module* llvm_module, llvm::Function* kernel,
  ...
  for (auto md : triton_wrapper_result.nvvm_annotations) {
    if (llvm::MDNode* mdNode = llvm::dyn_cast<llvm::MDNode>(md)) {
      if (mdNode->getNumOperands() >= 1) {
        std::vector<llvm::Metadata*> new_operands;
        new_operands.push_back(llvm::ValueAsMetadata::get(kernel));
        for (unsigned i = 1; i < mdNode->getNumOperands(); ++i) {
          new_operands.push_back(mdNode->getOperand(i));
        }
        llvm::MDNode* newMDNode =
            llvm::MDNode::get(llvm_module->getContext(), new_operands);
        dest_nvvm_annotations->addOperand(newMDNode);
      }
    }
  }
}
