 ...
 public:
  static std::string ParamToString(
      const ::testing::TestParamInfo<DotUnsetAlgorithmEmitterTest::ParamType>&
          data) {
    auto [dst_type, src_type] = data.param;
...
TEST_P(DotUnsetAlgorithmEmitterTest, UnsetAlgorithmIsEmittedCorrectly) {
  ...
  if (!internal::AreTypesSupportedByAlgoUnsetDot(input_type, result_type,
...
std::vector<PrimitiveType> AllXlaDataTypesSupportedByAlgUnsetDotLowering() {
  ...
  // in the test body.
  // This assumes that same input and result types are less strictly filtered
  // than different input and result types.
  std::vector<PrimitiveType> supported_types;
  absl::c_copy_if(AllXlaDataTypes(), std::back_inserter(supported_types),
                  [](PrimitiveType type) {
                    return !!internal::AreTypesSupportedByAlgoUnsetDot(
                        type, type, se::CudaComputeCapability::Blackwell());
                  });
