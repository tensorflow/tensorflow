...
CHECK: %[[REDUCE:.*]] = stablehlo.reduce(%{{.*}} init: %[[INIT]]) across dimensions = [1] : (tensor<4x4xf32>, tensor<f32>) -> tensor<4xf32>
)"));
...
CHECK:  %[[ABS:.*]] = math.absf
CHECK: %[[INIT:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
CHECK: %[[REDUCE:.*]] = stablehlo.reduce(%[[ABS]] init: %[[INIT]]) across dimensions = [1] : (tensor<64x512xf32>, tensor<f32>) -> tensor<64xf32>
...
CHECK:  %[[ABS:.*]] = math.absf
CHECK:  %[[INIT:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
CHECK:  %[[REDUCE:.*]] = stablehlo.reduce(%[[ABS]] init: %[[INIT]]) across dimensions = [0]
...
ENTRY main {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      ...
      CreateXTileIrAndFileCheck(this, kHloText, "triton_reduction_computation",
        ...
        CHECK:  %[[INIT:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
        CHECK:  stablehlo.reduce(%[[SELECT:.*]] init: %[[INIT:.*]]) across dimensions = [2] : (tensor<4x2x8x8x1xf32>, tensor<f32>) -> tensor<4x2x8x1xf32>
          )"));
...
CHECK:  %[[INIT:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
CHECK: %[[REDUCE:.*]] = stablehlo.reduce(%[[SELECT]] init: %[[INIT]]) across dimensions = [0] : (tensor<8x4xf32>, tensor<f32>) -> tensor<4xf32>
CHECK:   reducer(%[[ARG0:.*]]: tensor<f32>, %[[ARG1:.*]]: tensor<f32>)  {
CHECK:   %[[EXTRACTED_0:.*]] = tensor.extract %[[ARG0]][] : tensor<f32>
CHECK:   %[[EXTRACTED_1:.*]] = tensor.extract %[[ARG1]][] : tensor<f32>
...
CHECK: %[[INIT:.*]] = stablehlo.constant dense<0.000000e+00> : tensor<f32>
CHECK: %[[REDUCE:.*]] = stablehlo.reduce(%[[REDUCE_ARG:.*]] init: %[[INIT]]) across dimensions = [1] : (tensor<1x128xf32>,    tensor<f32>) -> tensor<1xf32>
...
ENTRY entry_computation {
  ...
  // TODO(basioli): We should emit an xtile specific ReshapeTensorToScalar op.
  // Once we do that we should separate out into emitting shared dialect and
  // lowering to triton.
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
CHECK:     tt.reshape
