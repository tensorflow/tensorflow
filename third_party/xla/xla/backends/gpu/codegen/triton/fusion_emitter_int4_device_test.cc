...
TEST_F(TritonTest, FusedBroadcastAddBroadcastMultiplyDot) {
  constexpr absl::string_view kHloText = R"(
    ...
    %fusion  {
      ...
      %add_p2_broadcast = bf16[1,128,8,8,64]{4,3,2,1,0} broadcast(%add_p2), dimensions={3,4}
    ...
    ENTRY %entry_computation {
      ...
      %p2 = bf16[8,64]{1,0} parameter(2)
      %p3 = bf16[64,512]{1,0} parameter(3)
      ROOT %gemm_fusion_dot.1642 = bf16[1024,64]{1,0} fusion(%p0, %p1, %p2, %p3), kind=kCustom, calls=%fusion
    }
  )";
  EXPECT_TRUE(Run(kHloText));
}
