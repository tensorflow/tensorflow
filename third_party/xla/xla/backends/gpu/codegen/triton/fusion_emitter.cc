...
SmallVector<Value> CreateValuesFrom(EmitterLocOpBuilder& builder,
                                    const SmallVector<int64_t>& values) {
  SmallVector<Value> result;
  for (int64_t value : values) {
    result.push_back(
        CreateConst(builder, builder.getI64Type(), value).UnwrapScalar());
  }
  return result;
}
...
absl::StatusOr<TileOpAndSizes> CreateTileOp(
  ...
  auto layout = LayoutUtil::MinorToMajor(shape);
  // mlir::DenseElementsAttr dense_attr = mlir::DenseElementsAttr::get();

  auto tile_op = b.create<mtx::TileOp>(
      result_type, parent_base_ptr, ptr_offsets,
      CreateValuesFrom(b, padded_tile_sizes),
      CreateValuesFrom(b, tiled_hlo.tile_strides()),
      llvm::ArrayRef<int64_t>(layout.data(), layout.size()));
