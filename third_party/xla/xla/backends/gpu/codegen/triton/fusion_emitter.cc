...
absl::StatusOr<stream_executor::gpu::TmaMetadata> ExtractTmaMetadata(
  ...
  triton_module->walk([&](mlir::LLVM::LLVMFuncOp func) { llvmFuncOp = func; });
  CHECK(llvmFuncOp) << "Expected a LLVMFuncOp in the module.";

  for (auto [idx, arg] : llvm::enumerate(llvmFuncOp.getArguments())) {
    if (auto attr = llvmFuncOp.getArgAttrOfType<mtx::TmaDescriptorAttr>(
...
absl::StatusOr<TritonWrapperResult> CompileTritonToLLVM(
  ...
  auto extract_tma_metadata_result = ExtractTmaMetadata(triton_module);
  if (extract_tma_metadata_result.ok()) {
    tma_metadata = extract_tma_metadata_result.value();
  }
