...
class MatMulEmitterHelper {
 public:
  ...
  absl::StatusOr<Value> EmitTensorPointer(
    ...
    auto add_dim = [this](const HloInstruction* hlo, const Side& side,
                          const DimProperties& properties,
                          const ConcatParams& concat_params, ValueRange& bases,
                          std::vector<int32_t>& boundary_checks,
                          TensorParams& tensor_params) -> absl::Status {
      if (NonTrivialTiledDimensionHasNoIterationAtParameter(side.scope, *hlo,
                                                            properties.index)) {
    ...
    for (const DimProperties& dim : side.tiled_dims) {
      TF_RETURN_IF_ERROR(add_dim(hlo, side, dim, concat_params, bases,
                                 boundary_checks, tensor_params));
    }
