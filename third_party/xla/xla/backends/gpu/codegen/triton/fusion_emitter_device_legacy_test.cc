...
triton_dot {
  ...
  ROOT %dot.1643 = f16[33,33]{1,0} dot(%c0, %p1),
...
triton_dot {
  %p0 = s8[512,4097]{0,1} parameter(0)
  %c0 = f32[512,4097]{1,0} convert(%p0)
  %p1 = f32[4097,16384]{1,0} parameter(1)
  ROOT %dot.1643 = f32[512,16384]{1,0} dot(%c0, %p1),
    lhs_contracting_dims={1}, rhs_contracting_dims={0}
}

ENTRY e {
  p0 = s8[512,4097]{1,0} parameter(0)
  p1 = f32[4097,16384]{1,0} parameter(1)
  ROOT _ = f32[512,16384] fusion(p0, p1), kind=kCustom, calls=triton_dot,
    backend_config={"fusion_backend_config": {kind: "__triton_gemm",
...
triton_dot {
  ...
  ROOT %dot.1643 = bf16[33,33]{1,0} dot(%p0, %p1),
    lhs_contracting_dims={1}, rhs_contracting_dims={0}
}
