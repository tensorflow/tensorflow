...
absl::Status CheckImplementableInst(const HloInstruction* inst,
  ...
  for (HloInstruction* operand : inst->operands()) {
    TF_XLA_RETURN_IF_ERROR(IsValidOperand(operand->shape(), reduction_op));
  }
  ...
  Future<> future =
      gpu_comm->GroupExecute([reduction_kind, &buffers,
        ...
        for (DeviceBufferPair& buffer : buffers) {
          TF_XLA_RETURN_IF_ERROR(comm->LaunchAllReduce(
...
absl::Status AllReduceStartThunk::Prepare(
  ...
  TF_XLA_RETURN_IF_ERROR(CollectiveThunk::Prepare(params, resource_requests));
...
absl::Status AllReduceStartThunk::Initialize(const InitializeParams& params) {
  ...
  TF_XLA_ASSIGN_OR_RETURN(bool use_collective_kernel,
                      collective_kernel_thunk_->IsSupported(
                          clique_key, params.collective_cliques));
  if (use_collective_kernel) {
    TF_XLA_RETURN_IF_ERROR(collective_kernel_thunk_->Initialize(params));
  }
  return absl::OkStatus();
}
...
absl::StatusOr<bool> AllReduceStartThunk::RunCollective(
  ...
  if (use_collective_kernel) {
    TF_XLA_RETURN_IF_ERROR(collective_kernel_thunk_->ExecuteOnStream(params));
    return false;  // No need for "first" invocation to rendezvous when not
                   // using nccl.
  }

  TF_XLA_RETURN_IF_ERROR(RunAllReduce(config_.reduction_kind, device_buffers,
...
absl::StatusOr<bool> ReduceScatterStartThunk::RunCollective(
  ...
  TF_XLA_ASSIGN_OR_RETURN(
      ...
      ConvertToDeviceBuffers(params, buffers_,
                             config_.config.operand_element_type));
  TF_XLA_RETURN_IF_ERROR(RunReduceScatter(config_.reduction_kind, device_buffers,
                                      stream, comm_handle.comm,
                                      config_.config.use_symmetric_buffer));
  return true;
}
  ...
  Future<> future =
      gpu_comm->GroupExecute([num_ranks, reduction_kind, &buffers,
        ...
        for (DeviceBufferPair& buffer : buffers) {
          ...
          TF_XLA_RETURN_IF_ERROR(comm->LaunchReduceScatter(
              buffer.source_buffer, buffer.destination_buffer,
