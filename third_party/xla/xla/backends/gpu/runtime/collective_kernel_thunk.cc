...
absl::Status CollectiveKernelThunk::RendezvousAfterInit(
  ...
  for (auto* rendezvous_state : *rendezvous_values) {
    ...
    const int64_t buffer_size =
        rendezvous_state->local_buffer.memory().size() / 2;
    const int64_t signal_buffer_size =
        rendezvous_state->signal_buffer.memory().size() / 2;
    for (int i = 0; i < state.remote_buffer_ptrs.size(); ++i) {
  ...
  StreamState* state = nullptr;
  {
    ...
    if (!per_stream_state_.contains(params.executor)) {
      ...
      TF_ASSIGN_OR_RETURN(
          ...
          AllocateMemory(params.executor, buffer_.source_buffer.size() * 2,
                         "LocalBuffer"));
      ...
      TF_ASSIGN_OR_RETURN(
          ...
          AllocateMemory(params.executor, kNumSignalFlags * sizeof(int32_t) * 2,
                         "SignalBuffer"));
...
absl::Status CollectiveKernelThunk::ExecuteOnStream(
  ...
  const uint32_t kBufferIndex = state->invocation_count % 2;
  ...
  return RunAllReduceKernel(
      ...
      /*remote_input_buffers=*/state->remote_buffer_ptrs[kBufferIndex],
      /*local_input_buffer=*/source_buffer,
      /*output_buffer=*/destination_buffer,
      /*rank=*/rank.value(),
      /*num_ranks=*/kNumRanks,
      /*num_elements=*/buffer_.element_count,
      /*signal_flags_buffers=*/state->signal_buffer_ptrs[kBufferIndex]);
}
