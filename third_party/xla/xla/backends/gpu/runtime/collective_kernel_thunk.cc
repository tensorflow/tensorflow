...
// - Rank offset.
// - Signal value.
static constexpr int kNumArgs = 7;
  ...
  StreamState* state = nullptr;
  {
    ...
    if (!per_stream_state_.contains(params.executor)) {
      ...
      if (!kernel_name_.empty()) {
        ...
        TF_ASSIGN_OR_RETURN(
            kernel, CreateKernel(kernel_name_, kNumArgs, params.src.text,
                                 params.executor, 0));
      }
...
absl::Status CollectiveKernelThunk::ExecuteOnStream(
  ...
  if (state->kernel != nullptr) {
    ...
    kernel_args.push_back(state->metadata);
    kernel_args.push_back(source_buffer);
    kernel_args.push_back(destination_buffer);
    kernel_args.push_back(buffer.element_count);  // num_elements.
    kernel_args.push_back(buffer.element_count /
                          num_devices);              // num_elements_per_rank.
    kernel_args.push_back(0);                        // rank_offset
    kernel_args.push_back(state->invocation_count);  // Signal value.
    TF_RET_CHECK(kernel_args.size() == kNumArgs)
