...
CollectiveConfig CollectiveMetadataThunk::GetCollectiveConfig(
  ...
  if (hlo->has_backend_config()) {
    ...
    if (backend_config.has_collective_metadata_backend_config()) {
      auto replica_groups =
          backend_config.collective_metadata_backend_config().replica_groups();
...
absl::Status CollectiveMetadataThunk::ConstructCollectiveMetadata(
    ...
    const GpuCliqueKey& clique_key, const void* multimem_address_space,
  ...
  metadata.rank = -1;
  for (int rank = 0; rank < clique_key.num_devices(); ++rank) {
    if (clique_key.devices()[rank].value() == device_ordinal) {
      metadata.rank = rank;
      break;
    }
  }
  ...
  metadata.multicast_buffer_ptr =
      reinterpret_cast<uint64_t>(multimem_address_space);
  const size_t num_parameters = rendezvous_values->at(0).parameters.size();
  ...
  for (int param = 0; param < num_parameters; ++param) {
    for (int peer = 0; peer < clique_key.num_devices(); ++peer) {
      param_to_peers_ptrs.push_back(reinterpret_cast<uint64_t>(
          rendezvous_values->at(peer).parameters.at(param).opaque()));
    }
  }
...
absl::Status CollectiveMetadataThunk::Initialize(
  ...
  TF_RET_CHECK(result_.slice.size() ==
               sizeof(CollectiveKernelMetadata) +
  ...
  se::DeviceMemoryBase result_prt =
      params.buffer_allocations->GetDeviceAddress(result_.slice);
  ...
  return ConstructCollectiveMetadata(
      ...
      params.executor->device_ordinal(), result_prt);
}
...
absl::StatusOr<void*> CollectiveMetadataThunk::SetupMultimem(
  ...
  constexpr int kAlternateMemorySpace = 1;
  for (const auto& parameter : parameters_) {
    if (parameter.memory_space == kAlternateMemorySpace) {
      TF_ASSIGN_OR_RETURN(
          memory_range,
...
absl::StatusOr<void*> CollectiveMetadataThunk::MultimemAddressSpaceProvider::
  ...
  int first_device = clique_key.devices()[0].value();

  if (device_number == first_device) {
    TF_ASSIGN_OR_RETURN(
        auto multicast_memory,
