...
TEST(KernelSpecTableTest, InternDeduplicatesEqualSpecs) {
  ...
  const int32_t first = table.Intern(GenerateCubinSpec("kernel", "cubin"));
  const int32_t second = table.Intern(GenerateCubinSpec("kernel", "cubin"));
  const int32_t third = table.Intern(GenerateCubinSpec("other", "cubin"));
...
TEST(KernelSpecTableTest, InternKeepsDistinctSpecsApart) {
  ...
  for (int i = 0; i < kNumSpecs; ++i) {
    EXPECT_EQ(table.Intern(GenerateCubinSpec(absl::StrCat("kernel_", i),
                                             absl::StrCat("cubin_", i))),
              i);
  }
  EXPECT_EQ(table.size(), kNumSpecs);
  for (int i = 0; i < kNumSpecs; ++i) {
    ASSERT_OK_AND_ASSIGN(const se::KernelLoaderSpec* spec, table.Get(i));
...
TEST(KernelSpecTableTest, InternInternsInProcessSymbolSpecs) {
  KernelSpecTable table;
  EXPECT_EQ(table.Intern(GenerateSymbolSpec("symbol")), 0);
  EXPECT_EQ(table.Intern(GenerateSymbolSpec("symbol")), 0);
...
TEST(KernelSpecTableTest, ToProtoAndFromProtoRoundTrip) {
  KernelSpecTable table;
  EXPECT_EQ(table.Intern(GenerateCubinSpec("kernel_a", "cubin_a")), 0);
  EXPECT_EQ(table.Intern(GenerateCubinSpec("kernel_b", "cubin_b")), 1);
  ...
  ASSERT_OK_AND_ASSIGN(const se::KernelLoaderSpec* spec0, reconstructed.Get(0));
  ASSERT_OK_AND_ASSIGN(const se::KernelLoaderSpec* spec1, reconstructed.Get(1));
...
TEST(KernelSpecTableTest, FromProtoPreservesDuplicates) {
  ...
  EXPECT_EQ(table.Intern(GenerateCubinSpec("kernel", "cubin")), 0);
}

TEST(KernelSpecTableTest, InternSharesPayloadBufferAcrossCopies) {
  KernelSpecTable table;
  table.Intern(GenerateCubinSpec("kernel", "shared_cubin_payload"));
  ASSERT_OK_AND_ASSIGN(const se::KernelLoaderSpec* stored, table.Get(0));

  se::KernelLoaderSpec copy1 = *stored;
  se::KernelLoaderSpec copy2 = *stored;
