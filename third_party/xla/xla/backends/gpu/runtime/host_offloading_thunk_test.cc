...
TEST(HostOffloadingStartThunkTest, SingleArgSingleResult) {
  ...
  HostOffloadingStartThunk thunk(
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      auto execute_event,
      thunk.GetAsyncEvents()->GetEvent(stream_executor, params.execution_id));
...
TEST(HostOffloadingStartThunkTest, MultiArgMultipleResult) {
  se::StreamExecutor* stream_executor = GpuExecutor();
  TF_ASSERT_OK_AND_ASSIGN(auto stream, stream_executor->CreateStream());
...
TEST(HostOffloadingStartThunkTest, ArgAndResultPinnedOnHost) {
  se::StreamExecutor* stream_executor = GpuExecutor();
  TF_ASSERT_OK_AND_ASSIGN(auto stream, stream_executor->CreateStream());
...
TEST(HostOffloadingStartThunkTest, ArgAndResultNonRegisteredHostMemory) {
  se::StreamExecutor* stream_executor = GpuExecutor();
  TF_ASSERT_OK_AND_ASSIGN(auto stream, stream_executor->CreateStream());
...
TEST(HostOffloadingStartThunkTest, TestErrorPropagationFromExecuteEvent) {
  se::StreamExecutor* stream_executor = GpuExecutor();
  TF_ASSERT_OK_AND_ASSIGN(auto stream, stream_executor->CreateStream());
...
TEST(HostOffloadingDoneThunkTest, WaitingOnAvailableEvent) {
  se::StreamExecutor* stream_executor = GpuExecutor();
  TF_ASSERT_OK_AND_ASSIGN(auto stream, stream_executor->CreateStream());

  auto async_events = std::make_shared<HostOffloadingAsyncEvents>();

  HostOffloadingDoneThunk thunk(Thunk::ThunkInfo(), async_events);
  ServiceExecutableRunOptions run_options;
      ...
      run_options, allocations, stream.get(), stream.get(), nullptr, nullptr);
  {
    ...
    TF_ASSERT_OK(async_events->AddEvent(stream_executor, params.execution_id,
...
TEST(HostOffloadingDoneThunkTest, WaitingOnErrorEvent) {
  se::StreamExecutor* stream_executor = GpuExecutor();
  TF_ASSERT_OK_AND_ASSIGN(auto stream, stream_executor->CreateStream());
