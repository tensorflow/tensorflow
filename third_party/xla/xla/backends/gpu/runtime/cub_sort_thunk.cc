...
absl::StatusOr<std::unique_ptr<CubSortRunnerInterface>> CreateCubSortRunner(
    PrimitiveType type, const stream_executor::Platform* platform) {
  TF_ASSIGN_OR_RETURN(
...
CubSortRunnerInterface::Create(PrimitiveType type,
  ...
  return value_type.has_value()
             ? CreateCubSortRunner(type, *value_type, platform)
             : CreateCubSortRunner(type, platform);
}
...
CubSortThunk::CubSortThunk(
    ...
    : Thunk(Thunk::kCubSort, thunk_info),
      runner_(
          CubSortRunnerInterface::Create(type, value_type, platform).value()),
