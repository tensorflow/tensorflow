...
absl::Status SerializeOffsetsToProto(
  ...
  for (const auto& offsets_item : offsets) {
    auto& offsets_proto = *proto->add_offsets();
...
DeserializeOffsetsFromProto(
  ...
  for (const auto& offsets_proto : proto.offsets()) {
    offsets.push_back(std::nullopt);
    if (offsets_proto.has_offsets()) {
      std::vector<DynamicSliceThunk::Offset> offsets_inner;
      for (const auto& offset_proto : offsets_proto.offsets().offsets()) {
        if (offset_proto.has_const_offset()) {
          offsets_inner.push_back(offset_proto.const_offset());
        } else if (offset_proto.has_slice_offset()) {
          TF_ASSIGN_OR_RETURN(
              auto slice, BufferAllocation::Slice::FromProto(
                              offset_proto.slice_offset(), buffer_allocations));
          offsets_inner.push_back(slice);
        } else if (offset_proto.has_hlo_module_offset_idx()) {
          TF_RET_CHECK(offset_as_function_of_indvar_metadata.has_value());
          auto& modules =
              offset_as_function_of_indvar_metadata->extracted_offset_modules;
          TF_RET_CHECK(modules.size() > offset_proto.hlo_module_offset_idx());
          offsets_inner.push_back(
              modules[offset_proto.hlo_module_offset_idx()].get());
        }
      }
      offsets.back() = std::move(offsets_inner);
    }
  }
  return offsets;
}
...
absl::StatusOr<ThunkProto> DynamicSliceThunk::ToProto() const {
  ...
  *dynamic_slice_proto->mutable_embedded_thunk() =
  ...
  if (offset_as_function_of_indvar_metadata_.has_value()) {
    ...
    *proto_metadata.mutable_indvar_init() =
        offset_as_function_of_indvar_metadata_->indvar_init
            ->ToProtoWithConfig();
    *proto_metadata.mutable_indvar_update() =
        offset_as_function_of_indvar_metadata_->indvar_update
            ->ToProtoWithConfig();
    for (const auto& module :
         offset_as_function_of_indvar_metadata_->extracted_offset_modules) {
      *proto_metadata.add_extracted_offset_modules() =
...
absl::StatusOr<std::unique_ptr<DynamicSliceThunk>> DynamicSliceThunk::FromProto(
  ...
  if (proto.has_offset_as_function_of_indvar_modules_metadata()) {
    const auto& metadata_proto =
        proto.offset_as_function_of_indvar_modules_metadata();

    TF_ASSIGN_OR_RETURN(
        std::unique_ptr<HloModule> indvar_init,
        HloModule::CreateFromProtoWithConfig(metadata_proto.indvar_init()));
    TF_ASSIGN_OR_RETURN(
        std::unique_ptr<HloModule> indvar_update,
        HloModule::CreateFromProtoWithConfig(metadata_proto.indvar_update()));
    std::vector<std::unique_ptr<HloModule>> extracted_offset_modules;
    for (const auto& module_proto : metadata_proto.extracted_offset_modules()) {
  ...
  for (auto& size_proto : proto.offset_byte_sizes()) {
    offset_byte_sizes.push_back(size_proto.has_value()
                                    ? std::make_optional(size_proto.value())
                                    : std::nullopt);
  }
