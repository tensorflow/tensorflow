...
// Returns the smallest divisor of q such that divisor divides q perfectly
// and the result is less than max.
int64_t SmallestDivisor(int64_t q, int64_t max) {
  int64_t divisor = 1;
  while (q % divisor != 0 || q / divisor > max) {
    ++divisor;
  }
  return divisor;
};
...
absl::Status LaunchTypedKernel(
  ...
  VLOG(3) << "Launching all-reduce kernel with params: " << "strategy: "
          << TagType::kAllReduceStrategy << ", rank: " << params.rank
...
LaunchDimensions AllReduceLaunchDimensions(int64_t elements, int64_t num_ranks,
  ...
  switch (strategy) {
    ...
    case AllReduceStrategy::kTwoShot: {
      // TwoShot kernel uses a block stride loop (work is divided into blocks).
      // As a result, the number of blocks must be a divisor of the number of
      // threads to distribute work equally across blocks.
      //
      // Step1: Find a minimal blocks_per_grid that puts threads_per_block
      // just within kMaxThreadsPerBlock.
      blocks_per_grid = SmallestDivisor(total_threads, kMaxThreadsPerBlock);
      threads_per_block = total_threads / blocks_per_grid;
      // If blocks_per_grid is larger than kMaxBlocksPerGrid, then we want to
      // reduce it to a smaller value. However we want to make sure that the
      // number of blocks is a divisor of the number of threads hence the need
      // for SmallestDivisor.
      blocks_per_grid =
          blocks_per_grid / SmallestDivisor(blocks_per_grid, kMaxBlocksPerGrid);
      break;
    }
  }
