...
absl::Status RunMemCpyRaggedAllToAll(
  ...
  rendezvous_value.device_ordinal = device_ordinal;
  rendezvous_value.output_buffer = buffers[1].destination_buffer;
  ...
  auto rendezvous_fn = [](absl::Span<const RendezvousValue* const> values) {
    ...
    // Sort by device ordinal to make sure that value are sorted
    // in the same order as the devices are ordered in the
    // communicator.
    absl::c_sort(values_copy,
                 [](const RendezvousValue& a, const RendezvousValue& b) {
                   return a.device_ordinal < b.device_ordinal;
                 });
    return values_copy;
  };
  ...
  std::shared_ptr<std::vector<RendezvousValue>> rendezvous_values =
      Rendezvous<std::vector<RendezvousValue>>(
          /*name=*/
          start_rendezvous_key, /*key=*/std::make_pair(clique_key, 1),
  ...
  for (int64_t i = 0; i < num_updates_per_replica; ++i) {
    for (int peer = 0; peer < num_ranks; ++peer) {
      ...
      se::DeviceMemoryBase dst_slice = collectives->Slice(
          (*rendezvous_values)[peer].output_buffer, data_buffer.element_type,
          output_offsets[idx] * ragged_row_element_size,
          send_sizes[idx] * ragged_row_element_size);
      TF_RETURN_IF_ERROR(
  ...
  Rendezvous(/*name=*/finish_rendezvous_key,
             /*key=*/std::make_pair(clique_key, 2),
