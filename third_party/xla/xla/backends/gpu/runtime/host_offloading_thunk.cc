...
#include "xla/backends/gpu/runtime/host_offloading_thunk.h"
...
tsl::thread::ThreadPool* GetHostOffloadingThreadPool() {
  constexpr int kMaxNumHostOffloadingThreads = 32;
  static tsl::thread::ThreadPool* host_offloading_thread_pool =
      new tsl::thread::ThreadPool(
          tsl::Env::Default(), "HostOffloading",
          std::min(tsl::port::MaxParallelism(), kMaxNumHostOffloadingThreads));
  return host_offloading_thread_pool;
}

class HostOffloadingCallFrame {
 public:
  static absl::StatusOr<HostOffloadingCallFrame> Create(
      se::Stream* stream, const BufferAllocations* buffer_allocations,
      HostOffloadingAllocator& allocator, absl::Span<SliceAndShape> args,
      absl::Span<SliceAndShape> results, const ProgramShape& program_shape);
 ...
 private:
  ...
  absl::Span<SliceAndShape> result_slices_;
...
absl::Status HostOffloadingCallFrame::ValidateArgsAndResults(
    absl::Span<const SliceAndShape> args,
    absl::Span<const SliceAndShape> results,
    const ProgramShape& program_shape) {
...
absl::StatusOr<HostOffloadingCallFrame> HostOffloadingCallFrame::Create(
    se::Stream* stream, const BufferAllocations* buffer_allocations,
    HostOffloadingAllocator& allocator, absl::Span<SliceAndShape> args,
    absl::Span<SliceAndShape> results, const ProgramShape& program_shape) {
  ...
  for (const auto& [slice, shape] : args) {
    ...
    auto memory_type =
        stream->parent()->GetPointerMemorySpace(buffer_allocation.opaque());

    if (memory_type.ok() &&
        *memory_type == stream_executor::MemoryType::kDevice) {
      // Copy device memory to host memory.
      TF_ASSIGN_OR_RETURN(auto buffer, allocator.AllocateTransferBuffer(
  ...
  return HostOffloadingCallFrame(stream, buffer_allocations,
                                 std::move(parameters), std::move(result),
                                 std::move(results), std::move(buffers));
}

HostOffloadingCallFrame::HostOffloadingCallFrame(
    se::Stream* stream, const BufferAllocations* buffer_allocations,
    std::vector<ShapeTree<HostOffloadingBuffer>> parameters,
    ShapeTree<HostOffloadingBuffer> result,
    absl::Span<SliceAndShape> result_slices,
...
absl::Status HostOffloadingCallFrame::PublishResult() && {
  size_t result_leaf_index = 0;
  for (const auto& [index, buffer] : result_.leaves()) {
    ...
    auto memory_type =
        stream_->parent()->GetPointerMemorySpace(result_buffer.opaque());
    if ((!memory_type.ok() ||
         *memory_type == stream_executor::MemoryType::kHost)) {
...
// HostOffloadingAsyncEvents

absl::Status HostOffloadingAsyncEvents::AddEvent(
    stream_executor::StreamExecutor* executor, size_t run_id,
    tsl::AsyncValueRef<tsl::Chain> event) {
...
absl::StatusOr<tsl::AsyncValueRef<tsl::Chain>>
HostOffloadingAsyncEvents::GetEvent(stream_executor::StreamExecutor* executor,
                                    size_t run_id) {
...
absl::Status HostOffloadingAsyncEvents::RemoveEvent(
    stream_executor::StreamExecutor* executor, size_t run_id) {
  VLOG(6) << "Removing event for executor at address " << executor
          << " and event id " << run_id;
...
// HostOffloadingStartThunk

HostOffloadingStartThunk::HostOffloadingStartThunk(
    ...
    : Thunk(Thunk::Kind::kHostOffloadingStart, std::move(thunk_info)),
      args_(std::move(args)),
      results_(std::move(results)),
      async_events_(std::make_shared<HostOffloadingAsyncEvents>()) {
...
std::string HostOffloadingStartThunk::ToString(int indent) const { return ""; }

absl::StatusOr<ThunkProto> HostOffloadingStartThunk::ToProto() const {
  return Unimplemented("Not implemented yet.");
}

absl::StatusOr<std::unique_ptr<HostOffloadingStartThunk>>
HostOffloadingStartThunk::FromProto(
...
absl::Status HostOffloadingStartThunk::Initialize(
...
absl::Status HostOffloadingStartThunk::ExecuteOnStream(
  ...
  TF_RETURN_IF_ERROR(async_events_->AddEvent(
      stream->parent(), params.execution_id, std::move(event)));

  GetHostOffloadingThreadPool()->Schedule([this, stream, params]() {
    auto call_frame_or_status = HostOffloadingCallFrame::Create(
    ...
    execute_event.AndThen([this, stream, execution_id = params.execution_id](
                              absl::Status status) {
      auto event_or_status =
          async_events_->GetEvent(stream->parent(), execution_id);
      CHECK(event_or_status.ok())
          << "Failed to get event: " << event_or_status.status();

      if (!status.ok()) {
        event_or_status.value().SetError(status);
      } else {
        event_or_status.value().SetStateConcrete();
      }
    });
  });
...
// HostOffloadingDoneThunk

HostOffloadingDoneThunk::HostOffloadingDoneThunk(
    Thunk::ThunkInfo thunk_info,
    std::shared_ptr<HostOffloadingAsyncEvents> async_events)
    : Thunk(Thunk::Kind::kHostOffloadingDone, std::move(thunk_info)),
      async_events_(std::move(async_events)) {
  CHECK(async_events_) << "async_events must not be null";
}

std::string HostOffloadingDoneThunk::ToString(int indent) const { return ""; }

absl::StatusOr<ThunkProto> HostOffloadingDoneThunk::ToProto() const {
  return Unimplemented("Not implemented yet.");
}

absl::StatusOr<std::unique_ptr<HostOffloadingDoneThunk>>
HostOffloadingDoneThunk::FromProto(
    ThunkInfo thunk_info, const HostOffloadingDoneThunkProto& proto,
    absl::Span<const BufferAllocation> buffer_allocations) {
  return Unimplemented("Not implemented yet.");
}

absl::Status HostOffloadingDoneThunk::Initialize(
    const InitializeParams& params) {
  return absl::OkStatus();
}

absl::Status HostOffloadingDoneThunk::ExecuteOnStream(
    const ExecuteParams& params) {
  TF_ASSIGN_OR_RETURN(
      se::Stream * stream,
      GetStreamForExecution(Thunk::execution_stream_id(), params));
  TF_ASSIGN_OR_RETURN(
      auto execute_event,
      async_events_->GetEvent(stream->parent(), params.execution_id));
  tsl::BlockUntilReady(execute_event);
  TF_RETURN_IF_ERROR(
      async_events_->RemoveEvent(stream->parent(), params.execution_id));
