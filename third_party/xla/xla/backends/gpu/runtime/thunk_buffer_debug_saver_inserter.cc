...
std::unique_ptr<Thunk> InsertBufferSaverCustomCall(
  ...
  for (int i = uses.size() - 1; i >= 0; i--) {
    ...
    ShapedSlice output{slice, FindShapeFor(slice, sequence[0].get())};
    ffi::AttributesMap attributes{
        {"dir", ffi::Attribute{path}},
        {"metadata", {sequence[0]->thunk_info().profile_annotation}}};

    // FIXME(b/446095941): update after cr/831938371 submitted.
    auto log_thunk = *CustomCallThunk::Create(
        Thunk::ThunkInfo(), std::string{kXlaGpuAppendToFileCustomCallTag},
        {output}, {std::nullopt}, attributes, hlo_module->entry_computation(),
        "GPU");
...
absl::Status RunDebugSaverInserter(SequentialThunk* root_thunk,
  ...
  root_thunk->TransformAllNestedThunks([&](std::unique_ptr<Thunk> thunk) {
    ...
    return InsertBufferSaverCustomCall(
        hlo_module, std::move(thunk),
