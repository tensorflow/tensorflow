...
TEST(GpuHostOffloadingAllocatorTest, AllocateStagingBuffer) {
  ...
  // NOTE(basioli) Invalid argument is returned from CUDA, and internal error
  // from ROCm.
  EXPECT_TRUE(absl::IsInvalidArgument(memory_type_or_status.status()) ||
              absl::IsInternal(memory_type_or_status.status()));
}
