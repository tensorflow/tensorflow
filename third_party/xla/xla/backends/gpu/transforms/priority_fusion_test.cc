...
TEST_F(PriorityFusionTest, LogSkippedCandidates) {
  const std::string kHloText = R"(
HloModule m
%region_2.260.clone.19 (Arg_0.55: f32[], Arg_1.55: f32[]) -> f32[] {
  %Arg_0.55 = f32[] parameter(0)
...
ENTRY main {
  ...
  module->mutable_config()
      .mutable_debug_options()
      .set_xla_priority_fusion_log_skipped_candidates(true);
  ...
  EXPECT_THAT(stderr_output, ::testing::HasSubstr("Producer: producer_fusion"));
}
