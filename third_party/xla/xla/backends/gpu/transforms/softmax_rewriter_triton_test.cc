...
ENTRY main {
  ...
  {
    // SoftmaxRewriterTriton with Cost Model evaluates this as profitable when
    // reduction dimension tiling is enabled, prior to the reduction cost model
    // adjustments arriving in the subsequent CL.
    ...
    auto module = ParseAndReturnVerifiedModule(hlo_string).value();
    bool experimental_tiling = GetParam();
    ASSERT_OK_AND_ASSIGN(bool fused_with_cost_model,
                         fusion_rewriter_with_cost_model.Run(module.get()));
    EXPECT_EQ(fused_with_cost_model, experimental_tiling);
  }
}
