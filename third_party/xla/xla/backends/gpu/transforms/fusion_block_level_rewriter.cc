...
bool IsTupleWithSameSubshapes(const Shape& shape, Shape::Equal eq) {
  if (!shape.IsTuple()) {
    return false;
  }
...
absl::StatusOr<bool> ShouldTryRewriteFusion(
  ...
  const bool is_same_shape_fusion = IsTupleWithSameSubshapes(
      fusion->shape(), Shape::Equal().IgnoreElementType());
