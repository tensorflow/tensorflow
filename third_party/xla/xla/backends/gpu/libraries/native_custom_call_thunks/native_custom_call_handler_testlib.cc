...
absl::StatusOr<const HloCustomCallInstruction*> FindCustomCall(
  ...
  if (instruction_name.empty()) {
    instruction = entry.root_instruction();
  } else {
    for (const HloInstruction* candidate : entry.instructions()) {
      if (candidate->name() == instruction_name) {
...
NativeCustomCallHandlerTester::NativeCustomCallHandlerTester() = default;
NativeCustomCallHandlerTester::~NativeCustomCallHandlerTester() = default;
...
NativeCustomCallHandlerTester::Create(absl::string_view hlo_text,
  ...
  tester->topology_ = std::make_unique<GpuTopology>(
      target_config.platform_name, /*num_partitions=*/1,
