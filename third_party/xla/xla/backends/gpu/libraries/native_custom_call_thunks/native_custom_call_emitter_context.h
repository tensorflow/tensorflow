...
class NativeCustomCallEmitterContext {
 public:
  ...
  // Returns the slice for the result or an operand together with the shape
  // backing it. This is the form consumed by most thunks, so prefer it over the
  // plain allocation slice accessors below.
  ...
  //
  // Use `emitters::KernelArguments::PositionOfOperand` and `PositionOfResult`
  // to find where a given buffer ended up; those positions are exactly the
  // index space of `stream_executor::KernelArgsPackingSpec` relocations.
