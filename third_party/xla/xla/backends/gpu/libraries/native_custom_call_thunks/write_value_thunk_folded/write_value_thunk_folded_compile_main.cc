...
// Ahead-of-time compiles an HLO module containing the thunk-folded
// `xla.gpu.test_write_value_thunk_folded` custom call, and writes the
// serialized executable to the path given as argv[1]. Mirrors the FFI AOT
// example in backends/gpu/ffi/xla_ffi_aot_custom_call_compile_main.cc.
...
HloModule TestWriteValueThunkFoldedModule

ENTRY main {
  ROOT result = s32[4] custom-call(),
    custom_call_target="xla.gpu.test_write_value_thunk_folded",
    backend_config="{val = 42 : i32}"
}
)hlo";
