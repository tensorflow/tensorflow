...
// Records what the context served it, so the test can assert on it.
struct HandlerObservation {
  int64_t num_kernel_arguments = 0;
  int64_t result_argument_index = -1;
  Shape operand_shape;
  int32_t val = 0;
};

HandlerObservation* observation = new HandlerObservation();

absl::StatusOr<ThunkSequence> ObservingHandler(
  ...
  observation->num_kernel_arguments = kernel_args.args().size();
  ABSL_ASSIGN_OR_RETURN(observation->result_argument_index,
                   kernel_args.ResultIndex({}));

  ABSL_ASSIGN_OR_RETURN(ShapedSlice operand, ctx.GetOperandShapedSlice(0, {}));
  observation->operand_shape = operand.shape;

  ABSL_ASSIGN_OR_RETURN(xla::ffi::Attributes attrs, ctx.GetFfiAttributes());
  ABSL_ASSIGN_OR_RETURN(observation->val, attrs.Get<int32_t>("val"));

  return ThunkSequence::Empty();
}

XLA_GPU_REGISTER_NATIVE_CUSTOM_CALL_HANDLER("xla.gpu.test_testlib_observer",
                                            ObservingHandler);
...
constexpr absl::string_view kHlo = R"(
  ENTRY e {
    ...
    ROOT c = f32[4] custom-call(p0),
      custom_call_target="xla.gpu.test_testlib_observer",
...
TEST(NativeCustomCallHandlerTesterTest, ServesTheHandlerARealBufferAssignment) {
  *observation = HandlerObservation();
  ...
  ASSERT_OK(tester->EmitThunks());

  // One operand plus one result.
  EXPECT_EQ(observation->num_kernel_arguments, 2);
  EXPECT_EQ(observation->result_argument_index, 1);
