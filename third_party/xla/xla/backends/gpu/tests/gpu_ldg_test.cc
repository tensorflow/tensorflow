...
class PdlLdgTest : public GpuLdgTest {
 protected:
  ...
  std::unique_ptr<VerifiedHloModule> CreateTestModule(bool enable_pdl_launch) {
...
TEST_F(PdlLdgTest, DoNonCoherentLoadWhenPrecedentKernelHasNoPdlLaunch) {
  CompileAndOptionallyVerifyPtx(CreateTestModule(/*enable_pdl_launch=*/false),
                                R"(
...
TEST_F(PdlLdgTest, DoCoherentLoadWhenPrecedentKernelHasPdlLaunch) {
  CompileAndOptionallyVerifyPtx(CreateTestModule(/*enable_pdl_launch=*/true),
                                R"(
    CHECK-LABEL: .entry triton_gemm
