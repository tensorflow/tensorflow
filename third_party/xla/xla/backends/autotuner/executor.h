...
#ifndef XLA_BACKENDS_AUTOTUNER_EXECUTOR_H_
#define XLA_BACKENDS_AUTOTUNER_EXECUTOR_H_
...
struct ProfileResult {
  explicit ProfileResult(absl::Duration duration) : duration(duration) {}
...
class Executor {
 public:
  ...
  virtual absl::StatusOr<ProfileResult> Profile(
      std::unique_ptr<Executable> executable) = 0;
  ...
  virtual absl::StatusOr<std::vector<ProfileResult>> ProfileWithSharedBuffers(
      absl::Span<std::unique_ptr<Executable>> executables) = 0;
};
}  // namespace xla

#endif  // XLA_BACKENDS_AUTOTUNER_EXECUTOR_H_
