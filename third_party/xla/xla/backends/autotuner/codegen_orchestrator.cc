...
CodegenOrchestrator::GetSupportedConfigs(HloInstruction* instr) {
...
CodegenOrchestrator::GetDefaultConfig(const HloInstruction& instr) {
...
tsl::Future<std::vector<std::pair<CodegenOrchestrator::Config,
...
CodegenOrchestrator::CompileAll(const HloInstruction* instr,
  ...
  if (thread_pool_ == nullptr) {
    ...
    for (Config& config : configs) {
      ...
      executables.emplace_back(std::move(config), std::move(executable));
    }
