...
absl::StatusOr<std::vector<Autotuner::ConfigResult>> Autotuner::ProfileAll(
  ...
  for (int i = 0; i < candidates.size(); ++i) {
    ...
    if (failure.has_value()) {
      results_vec.push_back({std::move(candidates[i].config), failure});
    } else {
      results_vec.push_back({std::move(candidates[i].config), std::nullopt,
                             profile_result->duration,
                             profile_result->scratch_bytes});
    }
  }
  return results_vec;
}
...
absl::Status Autotuner::DumpLogsToFile() {
  if (!autotune_config_.dump_logs_to.empty()) {
    ...
    if (!tsl::io::ResolveTestPrefixes(autotune_config_.dump_logs_to,
                                      resolved_path)) {
      return absl::FailedPreconditionError(absl::StrFormat(
          "File path can not be resolved: %s", autotune_config_.dump_logs_to));
    }
...
std::string Autotuner::ConfigResult::ToString(bool verbose) const {
  const std::string config_str = absl::StrFormat(
      "%s : %s", config.codegen_backend->name(),
      verbose ? UnpackedAnyShortDebugString(*config.backend_config) : "");
  if (failure.has_value()) {
    static const absl::NoDestructor<
        absl::flat_hash_map<FailureKind, absl::string_view>>
        kFailureKindToStr({
            {FailureKind::kCompilationFailed, "COMPILATION FAILED"},
            {FailureKind::kExecutionFailed, "EXECUTION FAILED"},
            {FailureKind::kRedzoneCheckFailed, "REDZONE CHECK FAILED"},
            {FailureKind::kWrongResults, "WRONG RESULTS"},
        });
    return absl::StrFormat("{%s failed with %s: %s}", config_str,
                           kFailureKindToStr->at(failure->kind),
                           failure->message);
  }
  return absl::StrFormat("{%s duration: %s, scratch_bytes: %d}", config_str,
                         absl::FormatDuration(duration), scratch_bytes);
}
// TODO b/407494653 - Temporarily added for backward compatibility.
AutotuneResult Autotuner::ConfigResult::ToProto() const {
  AutotuneResult result;
  static const absl::NoDestructor<
      absl::flat_hash_map<FailureKind, AutotuneResult::FailureKind>>
      kFailureKindToProto({
          {FailureKind::kCompilationFailed, AutotuneResult::UNKNOWN},
          {FailureKind::kExecutionFailed, AutotuneResult::DISQUALIFIED},
          {FailureKind::kRedzoneCheckFailed, AutotuneResult::REDZONE_MODIFIED},
          {FailureKind::kWrongResults, AutotuneResult::WRONG_RESULT},
      });
  ...
  if (config.backend_config->Is<AutotuneResult::GemmKey>()) {
    ...
    *result.mutable_backend_config() = *config.backend_config;
  }
  if (failure.has_value()) {
    auto* failure_result = result.mutable_failure();
    failure_result->set_kind(kFailureKindToProto->at(failure->kind));
    failure_result->set_msg(failure->message);
  } else {
