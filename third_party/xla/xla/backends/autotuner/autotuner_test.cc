...
TEST_F(AutotunerTest, DumpLogsToFile) {
  ...
  std::string content;
  EXPECT_OK(tsl::ReadFileToString(tsl::Env::Default(), config_.dump_logs_to,
                                  &content));
  ...
  *result_1->mutable_backend_config() = *GetTestConfig("test_config_1");
  ...
  *result_2->mutable_backend_config() = *GetTestConfig("test_config_2");
  *result_2->mutable_run_time() = ToDurationProto(absl::Seconds(1));

  EXPECT_THAT(actual_logs, testing::EqualsProto(expected_logs));
}
