...
class PhaseCompileExtensionTest : public ::testing::Test {
 protected:
  std::unique_ptr<pjrt::PhaseCompileExtensionWrapper>
  ...
  PhaseCompileExtensionTest() {
    // For the CPU plugin, the callbacks for the phase compile extension are
    // set to null. So we need to replace them with the sample plugin extension.
    const PJRT_Api* api = cpu_plugin::GetCpuPjrtApi();
    ...
    absl::StatusOr<std::unique_ptr<pjrt::PhaseCompileExtensionWrapper>>
        phase_compile_extension_wrapper_or_status =
            pjrt::PhaseCompileExtensionWrapper::Create(api);
...
// to null.
TEST_F(PhaseCompileExtensionTest, TestExtensionRegistration) {
