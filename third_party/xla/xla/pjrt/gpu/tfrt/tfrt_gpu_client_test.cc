...
TEST(TFRTGpuClientTest, MemorySpace) {
  ...
  for (auto* device : client->devices()) {
    ...
    EXPECT_EQ(memory_space->kind(), TfrtGpuHbmMemorySpace::kKind);
    EXPECT_EQ(memory_space->kind_id(), TfrtGpuHbmMemorySpace::kKindId);
    EXPECT_THAT(device->memory_space_by_kind(TfrtGpuHbmMemorySpace::kKind),
