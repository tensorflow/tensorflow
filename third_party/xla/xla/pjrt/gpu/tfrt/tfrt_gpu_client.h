...
class TfrtGpuDevice final : public PjRtDevice {
 public:
  ...
  se::Stream* h2d_stream() const { return h2d_stream_.get(); }

  se::Stream* d2d_stream() const { return d2d_stream_.get(); }
  ...
  tsl::AsyncValueRef<GpuEvent> GetLastCompleteEvent();

  void SetLastCompleteEvent(tsl::AsyncValueRef<GpuEvent> event);
 ...
 private:
  ...
  std::unique_ptr<se::Stream> h2d_stream_;
  std::unique_ptr<se::Stream> d2d_stream_;
  ...
  tsl::AsyncValueRef<GpuEvent> last_complete_event_ ABSL_GUARDED_BY(mu_);
