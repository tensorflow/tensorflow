...
#ifndef XLA_PJRT_GPU_TFRT_GPU_CLIENT_H_
#define XLA_PJRT_GPU_TFRT_GPU_CLIENT_H_
...
class TfrtGpuClient final : public PjRtClient {
 ...
 private:
  ...
  // 1. Threadpool size is at least as large as device_count so one
  // collective
  //    launch over all devices can succeed.
  // 2. Gang-schedule each collective by conservatively ensuring a total
  // order
...
class TfrtGpuHbmMemorySpace : public TfrtGpuMemorySpace {
 public:
  static constexpr absl::string_view kKind = "device";
  static const int kKindId;

  TfrtGpuHbmMemorySpace(int id, PjRtDevice* device);
};
...
class TfrtGpuDevice final : public PjRtDevice {
 ...
 private:
  ...
  tsl::AsyncValueRef<GpuEvent> GetAllocationSequencingEvent();
  int id_;
  ...
  TfrtEventSet deallocation_events_ ABSL_GUARDED_BY(mu_);
...
class TfrtGpuBuffer final : public PjRtBuffer {
 public:
  ...
  PjRtFuture<> CopyRawToHost(void* dst, int64_t offset,
    ...
    // TODO(b/382117736): Implement this
 ...
 private:
  absl::StatusOr<tsl::AsyncValueRef<Literal>> CopyToHostAsyncInternal(
  ...
  // calling this method.
  void AbortDonation(std::unique_ptr<TrackedTfrtGpuDeviceBuffer> device_buffer);
...
class TfrtGpuExecutable final : public PjRtLoadedExecutable {
 public:
  ...
  ~TfrtGpuExecutable() override = default;

  TfrtGpuClient* client() const override { return client_; }

  std::shared_ptr<LocalExecutable> gpu_executable() const {
    return gpu_executable_;
  }

  absl::string_view name() const override {
    return gpu_executable_->executable()->shared_module()->name();
  }

  int num_replicas() const override {
    return gpu_executable_->build_options().num_replicas();
  }

  int num_partitions() const override {
    return gpu_executable_->build_options().num_partitions();
  }

  int64_t SizeOfGeneratedCodeInBytes() const override {
    return gpu_executable_->executable()->SizeOfGeneratedCodeInBytes();
  }
  ...
  absl::StatusOr<std::vector<std::shared_ptr<HloModule>>> GetHloModules()
    ...
    return std::vector<std::shared_ptr<HloModule>>{
        gpu_executable_->executable()->shared_module()};
  }
  ...
  absl::StatusOr<CompiledMemoryStats> GetCompiledMemoryStats() const override {
    ...
    const HloProto* proto = gpu_executable_->executable()->hlo_proto();
    if (proto) {
 ...
 private:
  ...
  std::shared_ptr<LocalExecutable> gpu_executable_;
  std::shared_ptr<DeviceAssignment> device_assignment_;
  bool parameter_is_tupled_arguments_;
  std::shared_ptr<std::vector<Shape>> parameter_shapes_;

  // Size on device of each leaf buffer of the compiled program, cached here
  // for performance reasons.
  std::shared_ptr<std::vector<int64_t>> input_buffer_sizes_in_bytes_;
...
#endif  // XLA_PJRT_GPU_TFRT_GPU_CLIENT_H_
