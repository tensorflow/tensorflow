...
void StreamExecutorGpuClient::CopyToRemoteDevice(
  ...
  // The arguments passed to f, which is executed on a thread pool.
  struct Args {
  ...
  Args args{
      .gpu_collectives = gpu_collectives,
      .clique_id = std::move(clique_id),
      .mem = hold->device_memory(),
  ...
  auto f = [](Args& args) -> absl::Status {
    ...
    CliqueIds clique_ids(args.clique_id);
    gpu::GpuCollectives::Device collectives_device(
        args.local_device->executor());
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        args.gpu_collectives->CreateCommunicators(clique_key, clique_ids, ranks,
                                                  config));
    ...
    tsl::AsyncValueRef<Communicator::Event> send_event =
        communicator->Send(args.mem->mem(), args.shape.element_type(),
                           ShapeUtil::ElementsIn(args.shape), RankId(0),
                           gpu::GpuCollectives::On(*args.stream));
  ...
  thread_pool()->Schedule([f, args, on_done]() mutable {
    if (absl::Status s = f(args); !s.ok()) {
...
StreamExecutorGpuClient::MakeCrossHostReceiveBuffers(
  ...
  auto f = [](Args& args) -> absl::Status {
    ...
    TF_ASSIGN_OR_RETURN(CliqueId clique_id,
                        args.gpu_collectives->CreateUniqueCliqueId());

    // Notify the caller with the CliqueId. They will send the id to the sender.
    args.notifier(PjRtCrossHostRecvState{
        .descriptors = {PjRtCrossHostRecvDescriptors{{clique_id.ToString()}}},
        .cancel_notifier = nullptr,  // TODO(mwhittaker): Implement cancellation
    });
    ...
    tsl::AsyncValueRef<Communicator::Event> recv_event =
        communicator->Recv(args.mem->mem(), args.shape.element_type(),
                           ShapeUtil::ElementsIn(args.shape), RankId(1),
                           gpu::GpuCollectives::On(*args.stream));
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        args.local_device->event_pool().ThenAllocateAndRecordEvent(
            args.stream));
    args.definition_event->SetSequencingEvent(std::move(event), args.stream);

    return absl::OkStatus();
  };

  thread_pool()->Schedule([f, args = std::move(args)]() mutable {
    if (absl::Status s = f(args); !s.ok()) {
      args.definition_event->SetDefinedStatus(s);
    }
  });
