...
absl::StatusOr<DeviceTopologyPair> BuildDistributedDevices(
    ...
    bool generate_gpu_topology, absl::Duration get_local_topology_timeout,
  ...
  if (generate_gpu_topology) {
    TF_ASSIGN_OR_RETURN(const GpuTopologyProto gpu_topology_proto,
                     BuildGpuTopology(global_topology));
    gpu_topology = std::shared_ptr<const GpuTopology>(
...
absl::StatusOr<std::unique_ptr<PjRtClient>> GetStreamExecutorGpuClient(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      BuildDistributedDevices(
          ...
          options.enable_mock_nccl, options.generate_gpu_topology));
