...
absl::StatusOr<absl::flat_hash_map<GlobalDeviceId, tsl::IncarnationId>>
StreamExecutorGpuClient::GetLatestIncarnations() {
  ...
  TF_ASSIGN_OR_RETURN(std::vector<tsl::IncarnationId> task_incarnations,
                      agent->Incarnations(tasks));

  // Map every device to its incarnation.
  absl::flat_hash_map<GlobalDeviceId, tsl::IncarnationId> device_incarnations;
  for (const PjRtDevice* device : devices()) {
