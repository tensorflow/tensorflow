...
class TransposePlan {
 ...
 private:
  ...
  // If chunk contiguity is requested, this is the stride of the "boundary loop"
  // -- the loop that causes the chunk size to exceed the target. Loops with
  // a smaller contiguous stride must be inner to this loop; loops with a
  // larger contiguous stride must be outer.
  int64_t boundary_stride_ = std::numeric_limits<int64_t>::max();
};
