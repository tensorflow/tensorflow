...
class AbstractLocalPjRtBuffer : public PjRtBuffer {
 public:
  // Helper class to retain a "hold" on a AbstractLocalPjRtBuffer. A ScopedHold
  // may not outlive its parent AbstractLocalPjRtBuffer.
  //
  ...
  // AbstractLocalPjRtBuffer::GetBufferWithHold(kUsage) or the convenience
  ...
  // AbstractLocalPjRtBuffer::GetBufferWithHold(kExternal) or the convenience
  ...
  // not race with reads from the AbstractLocalPjRtBuffer.
  //
  ...
  // AbstractLocalPjRtBuffer::GetBufferWithHold(kDonation). If the enqueue
  ...
  //
  // Calls to AbstractLocalPjRtBuffer::Release (and transitively to
  // AbstractLocalPjRtBuffer::Delete() and ~AbstractLocalPjRtBuffer()) will
  ...
  class ScopedHold {
   public:
    ...
    AbstractLocalPjRtBuffer* parent() const { return parent_; }
   ...
   protected:
    ScopedHold(AbstractLocalPjRtBuffer* parent, Type type)
   ...
   private:
    friend class AbstractLocalPjRtBuffer;
    ...
    // AbstractLocalPjRtBuffer.
    void DropHold();

    AbstractLocalPjRtBuffer* const parent_;
    const Type type_;
 ...
 protected:
  explicit AbstractLocalPjRtBuffer(
      std::unique_ptr<AbstractTrackedDeviceBuffer> device_buffer);
  ~AbstractLocalPjRtBuffer() override;
