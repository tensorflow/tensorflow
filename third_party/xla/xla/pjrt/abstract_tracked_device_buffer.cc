...
AbstractLocalPjRtBuffer::AbstractLocalPjRtBuffer(
...
void AbstractLocalPjRtBuffer::WaitForOutstandingUsageHolds() {
...
void AbstractLocalPjRtBuffer::WaitForOutstandingDonationHold() {
...
AbstractLocalPjRtBuffer::GetBufferForUsageOrExternalHoldLocked(
...
AbstractLocalPjRtBuffer::GetBufferForDonationHoldLocked() {
  // All callers should have called WaitForOutstandingDonationHold().
  CHECK_EQ(holds_[ScopedHold::kDonation], 0);
  if (device_buffer_ == nullptr) {
...
void AbstractLocalPjRtBuffer::AcquireHoldLocked(ScopedHold* hold) {
...
void AbstractLocalPjRtBuffer::DropUsageOrExternalHold(
...
void AbstractLocalPjRtBuffer::DropDonationHold(
...
absl::Status AbstractLocalPjRtBuffer::ScopedHold::status() const {
...
void AbstractLocalPjRtBuffer::ScopedHold::DropHold() {
  if (ok()) {
    if (type_ == kDonation) {
...
AbstractLocalPjRtBuffer::ScopedHold::~ScopedHold() { DropHold(); }

AbstractLocalPjRtBuffer::ScopedHold::ScopedHold(ScopedHold&& other)
...
void AbstractLocalPjRtBuffer::ScopedHold::AcquireDonation(
...
void AbstractLocalPjRtBuffer::ScopedHold::AcquireUsageOrExternalReference(
...
void AbstractLocalPjRtBuffer::ScopedHold::ConfirmDonation() {
  CHECK(ok());
...
void AbstractLocalPjRtBuffer::ConfirmDonation(
...
AbstractLocalPjRtBuffer::ReleaseBuffer() {
  absl::MutexLock lock(&mu_);
  {
...
bool AbstractLocalPjRtBuffer::IsDeleted() {
  absl::MutexLock lock(&mu_);
