...
// An ExternalReference that keeps the refcounted raw buffer alive for as long
// as the reference is held -- exactly the "hold a ref while you use it" rule
// this buffer asks of every consumer. No usage event, no lock.
class UndonatableExternalReference : public PjRtBuffer::ExternalReference {
 public:
  explicit UndonatableExternalReference(PjRtRawBufferRef raw_buffer)
      : raw_buffer_(std::move(raw_buffer)) {
    data_ptr_ = raw_buffer_->OpaqueDeviceMemoryDataPointer();
  }
  ~UndonatableExternalReference() override = default;
...
Future<> UndonatableCommonPjRtBuffer::CopyRawToHostFuture(
  ...
  // Bounds check against the device buffer size (immutable, no lock needed).
  const size_t on_device_size = raw_buffer_->GetOnDeviceSizeInBytes();
  if (offset < 0 || offset > static_cast<int64_t>(on_device_size) ||
      static_cast<int64_t>(on_device_size) - offset < transfer_size) {
    return Future<>(absl::InvalidArgumentError(absl::StrCat(
        "CopyRawToHost called on buffer of size ", on_device_size,
        " with invalid offset ", offset, ", transfer size ", transfer_size)));
  }

  // Snapshot definition events as ordering dependencies for the copy.
  PjRtDeviceEventRefVector dependencies;
  dependencies.reserve(definition_events_.size());
  for (const auto& ev : definition_events_) {
    dependencies.push_back(ev);
  }
  ...
  dst.OnReady(
      [raw_buffer = raw_buffer_, offset, transfer_size,
       dependencies = std::move(dependencies),
       promise = std::move(promise)](absl::StatusOr<void*> dst) mutable {
        if (!dst.ok()) {
          promise.Set(dst.status());
          return;
        }
        auto d2h_event = raw_buffer->CopyRawDeviceToHostAndReturnEvent(
            *dst, offset, transfer_size, std::move(dependencies));
        if (!d2h_event.ok()) {
          promise.Set(d2h_event.status());
          return;
        }
        PjRtDeviceEventRefVector copy_event;
        copy_event.push_back(std::move(*d2h_event));
        PjRtDeviceEventSpan span(copy_event);
        xla::RunWhenReady(span, [raw_buffer = std::move(raw_buffer),
                                 copy_event = std::move(copy_event),
                                 promise = std::move(promise)]() mutable {
          promise.Set(xla::GetErrors(copy_event));
        });
      });
  return future;
}
...
UndonatableCommonPjRtBuffer::AcquireExternalReference() {
  // Does not block on definition events (caller must synchronize via
  // GetReadyFuture()). Holds the memory alive until released.
  return std::make_unique<UndonatableExternalReference>(raw_buffer_);
}
