...
class CpuTopology {
 public:
  ...
  bool operator==(const CpuTopology& other) const {
    if (cpu_devices_ != other.cpu_devices_) {
      return false;
    }
    // TargetMachineOptions does not have operator==, so compare fields
    // manually.
    return target_machine_options_.triple() ==
               other.target_machine_options_.triple() &&
           target_machine_options_.cpu() ==
               other.target_machine_options_.cpu() &&
           target_machine_options_.enabled_features() ==
               other.target_machine_options_.enabled_features() &&
           target_machine_options_.disabled_features() ==
               other.target_machine_options_.disabled_features();
  }
