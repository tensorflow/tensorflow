...
PjRtCpuRawClient::~PjRtCpuRawClient() { Stop(); }

void PjRtCpuRawClient::Stop() {
  async_work_runner_.reset();
  eigen_intraop_device_.reset();
  eigen_intraop_pool_.reset();
}
...
PjRtCpuClient::~PjRtCpuClient() {
  if (raw_client_ != nullptr) {
    raw_client_->Stop();
  }
  VLOG(1) << "PjRtCpuClient destroyed.";
}
...
PjRtCpuClient::LoadInternal(
  ...
  auto load_state = tsl::MakeRef<CpuExecutableLoadState>(raw_client());
...
absl::StatusOr<PjRtRawBufferRef> PjRtCpuRawClient::ImportForeignMemory(
  ...
  return CpuRawBuffer::ImportForeignMemory(
      device_ptr, std::move(on_delete_callback), size, memory_space,
      /*is_mutable=*/true);
}
...
CpuExecutableLoadState::LoadRawExecutable(
  ...
  int process_index = device->process_index();
  ...
  result->device_ = absl::down_cast<PjRtCpuDevice*>(device);
  return result;
}
