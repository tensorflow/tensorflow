...
class PjRtExecutableForwarder : public PjRtExecutable {
 public:
  ...
  // Only valid if the PjRtExecutableForwarder owns the PjRtExecutable. In
  // this case, we check to make sure that the contained object is both a
  ...
  std::unique_ptr<PjRtLoadedExecutable> GetLoadedExecutable() {
    ...
    if (auto* loaded_executable =
      ...
      return std::move(owned_executable_);
    }
    return nullptr;
  }
