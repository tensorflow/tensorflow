...
struct PJRT_Event_Set_Args {
  size_t struct_size;
  PJRT_Extension_Base* extension_start;
  PJRT_Event* event;           // A promise returned by `PJRT_Event_Create`.
  PJRT_Error_Code error_code;  // The error code with which to set the promise.
  // Local to the caller. PJRT_Promise_Set must copy `error_message` so it lives
  // beyond the call.
