...
PJRT_Error* RuntimeAbiVersionFromProto(
  ...
  PJRT_ASSIGN_OR_RETURN(
      std::unique_ptr<xla::PjRtRuntimeAbiVersion> version,
      xla::StreamExecutorGpuPjRtRuntimeAbiVersion::FromProto(
          proto, stream_executor::RuntimeAbiVersionManager::GetInstance()));
  args->abi_version = new PJRT_RuntimeAbiVersion{std::move(version)};
  return nullptr;
}

PJRT_Error* ExecutableAbiVersionFromProto(
  ...
  if (proto.platform() != xla::CudaId()) {
    return new PJRT_Error{absl::InvalidArgumentError(
        "ExecutableAbiVersionFromProto is not implemented for platform: " +
        std::to_string(proto.platform()))};
  }

  PJRT_ASSIGN_OR_RETURN(
      std::unique_ptr<xla::StreamExecutorPjRtExecutableAbiVersion> version,
      xla::StreamExecutorPjRtExecutableAbiVersion::FromProto(proto));
  args->abi_version = new PJRT_ExecutableAbiVersion{std::move(version)};
  return nullptr;
}
...
PJRT_AbiVersion_Extension CreateGpuAbiVersionExtension(
  ...
  return PJRT_AbiVersion_Extension{
      ...
      /*runtime_abi_version_from_proto=*/RuntimeAbiVersionFromProto,
      /*executable_abi_version_from_proto=*/ExecutableAbiVersionFromProto,
  };
}
