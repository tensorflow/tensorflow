...
absl::Status CommonPjRtClient::PrepareArguments(
  ...
  input_buffers.reserve(tpu_argument_handles.size());
  device_buffers.reserve(tpu_argument_handles.size());
  auto donate_it = donated_params.begin();
  {
    ...
    donation_clashes.reserve(tpu_argument_handles.size());
    ...
    for (int i = 0; i < tpu_argument_handles.size(); ++i) {
      PjRtBuffer* handle = tpu_argument_handles[i];
      auto* tfrt_buffer = down_cast<CommonPjRtBufferImpl*>(handle);
    ...
    if (VLOG_IS_ON(1)) {
      ...
      if (!tpu_argument_handles.empty()) {
        LOG(INFO) << donated_buffer_stats.size() << " arguments out of total "
                  << tpu_argument_handles.size()
                  << " arguments will be donated.";
      ...
      for (int i = 0; i < input_buffers.size(); ++i) {
        ...
        TF_ASSIGN_OR_RETURN(Shape actual_input_shape,
                            tpu_argument_handles[i]->logical_on_device_shape());
