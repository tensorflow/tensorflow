...
TEST(PjRtCApiTopologyDescriptionTpuTest, IdForLogicalDeviceOfDefaultType) {
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<PjRtTopologyDescription> topology,
                          GetTpuTopology());
  EXPECT_THAT(topology->IdForLogicalDeviceOfDefaultType({1, 1, 0}, 0),
              IsOkAndHolds(PjRtGlobalDeviceId(10)));
}

TEST(PjRtCApiTopologyDescriptionTpuTest, LogicalDeviceOfDefaultTypeForId) {
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<PjRtTopologyDescription> topology,
                          GetTpuTopology());
  TF_ASSERT_OK_AND_ASSIGN(
      const PjRtDeviceDimensionsAndInt& result,
      topology->LogicalDeviceOfDefaultTypeForId(PjRtGlobalDeviceId(10)));
