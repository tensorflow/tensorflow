...
class UndonatableCommonPjRtBufferTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    std::vector<float> data = {41.0f, 42.0f, 43.0f, 44.0f};
...
// Verify basic construction, acquisition, and identification.
...
// Verify RawBuffer factory registry.
...
// Verify Constructor protections (safety against being born deleted).
...
// Verify calling Delete() on UndonatableCommonPjRtBuffer is a no-op:
...
// Verify rejection of donations (Core Guarantee Contract).
...
// Verify GetReadyFuture behaviors.
...
// Verify CopyRawToHost roundtrips
TEST_F(UndonatableCommonPjRtBufferTest, CopyRawToHostRoundtrips) {
  auto buffer = std::make_unique<UndonatableCommonPjRtBuffer>(
      shape_, raw_buffer_, absl::InlinedVector<PjRtDeviceEventRef, 2>(),
      memory_space_);

  std::vector<float> dst(4, -1.0f);
  ASSERT_OK(buffer->CopyRawToHost(dst.data(), 0, 4 * sizeof(float)).Await());

  EXPECT_EQ(dst[0], 41.0f);
  EXPECT_EQ(dst[1], 42.0f);
  EXPECT_EQ(dst[2], 43.0f);
  EXPECT_EQ(dst[3], 44.0f);
}

// Verify CopyRawToHost rejects out of range
TEST_F(UndonatableCommonPjRtBufferTest, CopyRawToHostRejectsOutOfRange) {
  auto buffer = std::make_unique<UndonatableCommonPjRtBuffer>(
      shape_, raw_buffer_, absl::InlinedVector<PjRtDeviceEventRef, 2>(),
      memory_space_);

  std::vector<float> dst(4, -1.0f);
  ASSERT_OK_AND_ASSIGN(size_t on_device_size, buffer->GetOnDeviceSizeInBytes());

  // Offset out of bounds
  EXPECT_THAT(buffer->CopyRawToHost(dst.data(), on_device_size + 1, 4).Await(),
              StatusIs(absl::StatusCode::kInvalidArgument));

  // Size goes out of bounds
  EXPECT_THAT(buffer->CopyRawToHost(dst.data(), on_device_size - 1, 4).Await(),
              StatusIs(absl::StatusCode::kInvalidArgument));

  // Negative offset
  EXPECT_THAT(buffer->CopyRawToHost(dst.data(), -1, 4).Await(),
              StatusIs(absl::StatusCode::kInvalidArgument));
}

// Verify CopyRawToHost surfaces definition errors
TEST_F(UndonatableCommonPjRtBufferTest, CopyRawToHostSurfacesDefinitionError) {
  auto future = tsl::MakeUnconstructedAsyncValueRef<int>();
  PjRtDeviceEventRef event =
      PjRtDeviceEventPtr::FromAsyncValue(future.GetAsyncValue()).CopyRef();
  absl::InlinedVector<PjRtDeviceEventRef, 2> events;
  ...
  std::vector<float> dst(4, -1.0f);
  Future<> copy_future =
      buffer->CopyRawToHost(dst.data(), 0, 4 * sizeof(float));

  // Resolve with an error!
  future.SetError(absl::InternalError("fake definition error"));

  EXPECT_THAT(copy_future.Await(),
              StatusIs(absl::StatusCode::kInternal, "fake definition error"));
}

// Verify AcquireExternalReference keeps memory valid
TEST_F(UndonatableCommonPjRtBufferTest,
       AcquireExternalReferenceKeepsMemoryValid) {
  auto buffer = std::make_unique<UndonatableCommonPjRtBuffer>(
      shape_, raw_buffer_, absl::InlinedVector<PjRtDeviceEventRef, 2>(),
      memory_space_);

  ASSERT_OK_AND_ASSIGN(auto ext_ref, buffer->AcquireExternalReference());
  EXPECT_NE(ext_ref->OpaqueDeviceMemoryDataPointer(), nullptr);
  EXPECT_EQ(ext_ref->OpaqueDeviceMemoryDataPointer(),
            raw_buffer_->OpaqueDeviceMemoryDataPointer());

  // Destroy the buffer object.
  buffer.reset();

  // The external reference should still be valid.
  EXPECT_EQ(ext_ref->OpaqueDeviceMemoryDataPointer(),
            raw_buffer_->OpaqueDeviceMemoryDataPointer());
}
