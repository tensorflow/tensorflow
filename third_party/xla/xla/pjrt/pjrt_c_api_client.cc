...
void PjRtCApiClient::InitAttributes() {
  ...
  // TODO(emilyaf): Support cross-host transfers on TPU as well.
...
PJRT_CrossHostRecvNotifierInfo CppCrossHostRecvNotifierToC(
  ...
  *notifier_function = [&cpp_notifier, c_api](
  ...
  return PJRT_CrossHostRecvNotifierInfo{
...
PjRtCApiClient::MakeCrossHostReceiveBuffers(
  ...
  args.struct_size = PJRT_Client_MakeCrossHostReceiveBuffers_Args_STRUCT_SIZE;
  args.extension_start = nullptr;
  args.client = c_client_.get();
  ...
  for (int i = 0; i < shapes.size(); ++i) {
    ...
    // TODO(emilyaf): Enable this once ASAN failure is fixed.
  ...
  RETURN_STATUS_IF_PJRT_ERROR(
      c_api->PJRT_Client_MakeCrossHostReceiveBuffers(&args), c_api);
...
void PjRtCApiBuffer::CopyToRemoteDevice(
  ...
  c_api->PJRT_Buffer_CopyToRemoteDevice(&args);
}
