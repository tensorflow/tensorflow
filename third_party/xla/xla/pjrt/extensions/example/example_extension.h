...
typedef struct PJRT_Example_Extension {
  ...
  xla::ExampleExtensionCpp* extension_cpp;  // nullptr if not using CPP impl

  // Example method.
  PJRT_Error* (*example_method)(PJRT_ExampleExtension_ExampleMethod_Args* args);
} PJRT_Example_Extension;
PJRT_DEFINE_STRUCT_TRAITS(PJRT_Example_Extension, example_method);

typedef struct PJRT_ExampleExtension_ExampleMethod_Args {
  PJRT_Example_Extension* extension;  // Input: nullptr if not using CPP impl
