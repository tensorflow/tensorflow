...
#else
#define ENUM(name) enum name : int32_t
#endif
...
struct PJRT_Callback_Type_SliceBuilder_Args {
  size_t struct_size;
  SliceFailureType failure_type;
};
PJRT_DEFINE_STRUCT_TRAITS(PJRT_Callback_Type_SliceBuilder_Args, failure_type);
...
typedef struct PJRT_Callback_InvokeCallback_Args {
  ...
  // The callback type specific arguments.
  void* args;
