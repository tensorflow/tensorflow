...
Shape::Shape(std::vector<Shape> tuple_shapes) {
  ...
  if_tuple_state()->tuple_shapes = std::move(tuple_shapes);
}
...
void Shape::CheckStateIsEmpty() const {
  if (const auto* const state = if_array_state()) {
    CHECK(state->dimensions.empty()) << ShortDebugString();
    CHECK(state->dynamic_dimensions.empty()) << ShortDebugString();
    CHECK(!state->layout.has_value()) << ShortDebugString();
  } else if (const auto* const state = if_tuple_state()) {
    CHECK(state->tuple_shapes.empty()) << ShortDebugString();
  }
}
...
// std::string Shape::GetCategoryName() const {
//   if (IsArray()) {
//     return "array";
//   }
//   if (IsTuple()) {
