...
namespace {
void FromProtoInternal(Shape& shape, const ShapeProto& proto) {
  ...
  for (const ShapeProto& tuple_shape_proto : proto.tuple_shapes()) {
    Shape tuple_shape;
    FromProtoInternal(tuple_shape, tuple_shape_proto);
    shape.add_tuple_shape(std::move(tuple_shape));
  }

  if (proto.has_layout()) {
    CHECK(primitive_util::IsArrayType(proto.element_type()))
        << "Malformed shape proto: element_type "
        << PrimitiveType_Name(proto.element_type()) << " cannot have a layout.";
...
Shape::Shape(const ShapeProto& shape_proto) {
  FromProtoInternal(*this, shape_proto);
}

/* static */
Shape Shape::FromProto(const ShapeProto& proto) {
  Shape shape;
  FromProtoInternal(shape, proto);
  return shape;
}
