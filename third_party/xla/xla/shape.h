...
class Shape {
 ...
 private:
  ...
  const InvalidState& invalid_state() const {
    ...
    CHECK(state) << "Expected an invalid shape. Got " << ShortDebugString();
    return *state;
  }
  const TokenState& token_state() const {
    const auto* const state = if_token_state();
    CHECK(state) << "Expected a token shape. Got " << ShortDebugString();
    return *state;
  }
  const OpaqueState& opaque_state() const {
    const auto* const state = if_opaque_state();
    CHECK(state) << "Expected an opaque shape. Got " << ShortDebugString();
    return *state;
  }
  const ArrayState& array_state() const {
    const auto* const state = if_array_state();
    CHECK(state) << "Expected an array shape. Got " << ShortDebugString();
    return *state;
  }
  ...
  const TupleState& tuple_state() const {
    ...
    CHECK(state) << "Expected a tuple shape. Got " << ShortDebugString();
    return *state;
  }
  ...
  // Returns the name of the shape's category. Used for error messages only.
  // Do not rely on the exact string values.
