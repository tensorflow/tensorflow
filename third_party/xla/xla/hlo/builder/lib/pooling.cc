...
XlaOp ComputeSums(XlaOp operand, XlaOp init_value,
  ...
  return b->ReportErrorOrReturn([&]() -> absl::StatusOr<XlaOp> {
    ASSIGN_OR_RETURN(Shape operand_shape, b->GetShape(operand));
    ASSIGN_OR_RETURN(Shape init_shape, b->GetShape(init_value));
...
XlaOp AvgPoolGrad(XlaOp out_backprop, absl::Span<const int64_t> gradients_size,
  ...
  return b->ReportErrorOrReturn([&]() -> absl::StatusOr<XlaOp> {
    ...
    ASSIGN_OR_RETURN(Shape out_backprop_xla_shape, b->GetShape(out_backprop));
