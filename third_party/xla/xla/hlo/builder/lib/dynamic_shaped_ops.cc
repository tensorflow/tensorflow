...
absl::StatusOr<XlaOp> ReconsileBranchDifference(const Shape& left_branch_shape,
  ...
  if (left_branch_shape.IsTuple()) {
    ...
    for (int i = 0; i < left_branch_shape.tuple_shapes().size(); ++i) {
      ...
      TF_XLA_ASSIGN_OR_RETURN(XlaOp elem,
...
XlaOp DynamicConditional(
  ...
  return builder->ReportErrorOrReturn([&]() -> absl::StatusOr<XlaOp> {
    ...
    for (int64_t i = 0; i < branch_computations.size(); ++i) {
      TF_XLA_ASSIGN_OR_RETURN(auto program_shape,
    ...
    for (int64_t i = 0; i < branch_computations.size(); ++i) {
      TF_XLA_ASSIGN_OR_RETURN(Shape branch_operand_shape,
                          builder->GetShape(branch_operands[i]));

      TF_XLA_ASSIGN_OR_RETURN(auto rewritten,
...
absl::StatusOr<XlaOp> SetDimensionSizeWithRebound(
  ...
  TF_XLA_RETURN_IF_ERROR(inferred_bound_status_or.status());
  TF_XLA_RETURN_IF_ERROR(dynamism_status_or.status());
  if (inferred_bound_status_or->AllValid()) {
    int64_t inferred_bound = inferred_bound_status_or->Get<int32_t>({}).value();
    TF_XLA_ASSIGN_OR_RETURN(auto* shape_ptr,
...
absl::StatusOr<XlaOp> SetAllDimensionSizes(ValueInference* value_inference,
  ...
  TF_XLA_RETURN_IF_ERROR(builder->GetCurrentStatus());
  TF_XLA_ASSIGN_OR_RETURN(auto shape_ptr, builder->GetShapePtr(operand));
  ...
  for (int64_t i = 0; i < shape_ptr->dimensions().size(); ++i) {
    ...
    TF_XLA_ASSIGN_OR_RETURN(auto dynamism,
