...
struct HloProtoEvaluator {
  ...
  absl::StatusOr<Literal> Evaluate() {
    ...
    if (primitive_type.has_value()) {
      ABSL_ASSIGN_OR_RETURN(Shape shape, Shape::FromProto(inst.shape()));
      *inst.mutable_shape() =
...
PostorderDFSVisitor::AnalyzeConstantValueFallback(int64_t handle,
  ...
  ABSL_ASSIGN_OR_RETURN(HloOpcode opcode, StringToHloOpcode(root->opcode()));
      ...
      return result.AddVisit(
          [root, computation_proto, context,
           this](absl::Span<Literal> operands) -> absl::StatusOr<Literal> {
            ABSL_ASSIGN_OR_RETURN(auto computation, HloComputation::CreateFromProto(
                                                   *computation_proto, {}));
    ...
    case HloOpcode::kAbs: {
      ...
      return PostorderDFSNode()
          ...
          .AddVisit([this](Literal lower_bound,
            ...
            ABSL_ASSIGN_OR_RETURN(auto lower_bound_abs,
                             evaluator.EvaluateElementwiseUnaryOp(
                                 HloOpcode::kAbs, lower_bound));
            ABSL_ASSIGN_OR_RETURN(auto upper_bound_abs,
                             evaluator.EvaluateElementwiseUnaryOp(
...
absl::StatusOr<PostorderDFSNode> PostorderDFSVisitor::AnalyzeIsDynamic(
    int64_t handle, PostorderDFSNodeType type, InferenceContext context) {
  ABSL_RETURN_IF_ERROR(handle_to_instruction(handle).status());
  // Invariant check.
  ...
  switch (opcode) {
    ...
    case HloOpcode::kCall: {
      ...
      if (call_proto->operand_ids_size() != 1) {
        // Only support single operand forwarding.
        ABSL_ASSIGN_OR_RETURN(auto shape, Shape::FromProto(call_proto->shape()));
...
absl::StatusOr<Literal> PostorderDFSVisitor::PostOrderDFSVisit(
  ...
  while (!stack.empty()) {
    ...
    if (VLOG_IS_ON(1)) {
      ABSL_RETURN_IF_ERROR(handle_to_instruction(item.handle).status());
      VLOG(1) << "stack top "
    ...
    if (item.state == kVisiting) {
      ...
      ABSL_ASSIGN_OR_RETURN(auto literal, item.visit(absl::MakeSpan(literals)));
    ...
    switch (item.type) {
      case PostorderDFSNodeType::kConstantValue: {
        ...
        ABSL_ASSIGN_OR_RETURN(node, AnalyzeConstant(item.handle, item.context));
        break;
      }
      case PostorderDFSNodeType::kConstantLowerBound: {
        VLOG(1) << "constant lower bound";
        ABSL_ASSIGN_OR_RETURN(node, AnalyzeLowerBound(item.handle, item.context));
        break;
      }
    ...
    for (const PostorderDFSDep& dep : node.dependencies) {
      ABSL_ASSIGN_OR_RETURN(auto dependency_inst, handle_to_instruction(dep.handle));
...
absl::StatusOr<std::optional<int64_t>> ValueInference::CseOpHandle(
    int64_t handle) {
  ABSL_ASSIGN_OR_RETURN(auto inst, builder_->LookUpInstructionByHandle(handle));
  ABSL_ASSIGN_OR_RETURN(HloOpcode opcode, StringToHloOpcode(inst->opcode()));
  ...
  ABSL_ASSIGN_OR_RETURN(auto equivalent_op,
...
absl::StatusOr<Literal> ValueInference::SimplifyOp(int64_t handle) {
  ABSL_ASSIGN_OR_RETURN(auto cse_handle, CseOpHandle(handle));
  if (cse_handle) {
  ...
  {
    ABSL_ASSIGN_OR_RETURN(auto output_shape_stack, Shape::FromProto(inst->shape()));
  ...
  switch (opcode) {
    ...
    case HloOpcode::kBroadcast: {
      for (auto operand_id : inst->operand_ids()) {
        ABSL_ASSIGN_OR_RETURN(auto literal, SimplifyOp(operand_id));
    ...
    case HloOpcode::kConvert: {
      ...
      ABSL_ASSIGN_OR_RETURN(Shape operand_shape, Shape::FromProto(operand->shape()));
    ...
    case HloOpcode::kAdd: {
      ...
      if (output_shape->dimensions().size() == 0) {
        ABSL_ASSIGN_OR_RETURN(auto lhs, SimplifyOp(inst->operand_ids(0)));
        ABSL_ASSIGN_OR_RETURN(auto rhs, SimplifyOp(inst->operand_ids(1)));
...
absl::StatusOr<OptionalLiteral> ValueInference::AnalyzeConstant(
  ...
  ABSL_RETURN_IF_ERROR(builder_->LookUpInstructionByHandle(op.handle()).status());
  PostorderDFSVisitor visitor(
      evaluator_,
      [&](int64_t handle) {
        return builder_->LookUpInstructionByHandle(handle);
  ...
  ABSL_ASSIGN_OR_RETURN(Shape op_shape, builder_->GetShape(op));
  int64_t handle = op.handle();
  if (ShapeUtil::IsScalar(builder_->GetShape(op).value())) {
    ABSL_ASSIGN_OR_RETURN(auto result, SimplifyOp(handle));
  ...
  switch (mode) {
    case ValueInferenceMode::kLowerBound: {
      ...
      ABSL_ASSIGN_OR_RETURN(Literal value,
                       visitor.PostOrderDFSVisit(
