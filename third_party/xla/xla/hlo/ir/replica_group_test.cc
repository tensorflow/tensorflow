...
TEST(MeshAxesReplicaGroupListTest, ReplicaGroupsCountAndSize) {
  Mesh mesh_all_axes(TileAssignment(IotaTileAssignment::Create(
                         /*dims=*/{4, 4})),
                     /*axes_names=*/{"x", "y"});
  ...
  Mesh mesh_one_axes(TileAssignment(IotaTileAssignment::Create(
                         /*dims=*/{3, 5})),
                     /*axes_names=*/{"a", "b"});
  MeshAxesReplicaGroupList replica_group_across_a(mesh_one_axes,
                                                  /*axes=*/{AxisRef(0)});
  MeshAxesReplicaGroupList replica_group_across_b(mesh_one_axes,
                                                  /*axes=*/{AxisRef(1)});
  ...
  Mesh mesh_no_axes(TileAssignment(IotaTileAssignment::Create(
                        /*dims=*/{2, 3, 5})),
                    /*axes_names=*/{"p1", "p2", "p3"});
  testing::ScopedMockLog log(testing::kDoNotCaptureLogsYet);
  EXPECT_CALL(log,
              Log(base_logging::WARNING, testing::_,
                  testing::HasSubstr("has only one device per replica group")))
      .Times(1);
  log.StartCapturingLogs();
  MeshAxesReplicaGroupList replica_group_across_no_axes(mesh_no_axes,
                                                        /*axes=*/{});
