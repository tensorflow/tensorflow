...
TEST(OriginalValueTest, ToStringSynthetic) {
  OriginalValue value(SyntheticCall{});
  EXPECT_EQ(value.ToString(), "[synthetic]");
}
...
TEST(OriginalValueTest, ProtoSerde) {
  ...
  OriginalValue value_synthetic(SyntheticCall{});
...
TEST(OriginalValueTest, EqualityAndHashing) {
  ...
  OriginalValue value_with_root_value(Node::Tuple(
      OriginalArray{"root", {}}, {Node::Leaf(OriginalArray{"inst1", {1}}),
                                  Node::Leaf(OriginalArray{"inst2", {2}})}));
  OriginalValue synthetic1(SyntheticCall{});
  OriginalValue synthetic2(SyntheticCall{});
...
ENTRY main {
  ...
  tuple->set_original_value(std::make_shared<OriginalValue>(SyntheticCall{}));
  gte->set_original_value(OriginalValue::CreateFromInstruction(gte));
...
ENTRY main {
  ...
  p0->set_original_value(OriginalValue::CreateFromInstruction(p0));
  p1->set_original_value(std::make_shared<OriginalValue>(SyntheticCall{}));
  tuple->set_original_value(OriginalValue::CreateFromInstruction(tuple));
...
ENTRY main {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnVerifiedModule(hlo_string));
  HloInstruction* p0 = module->entry_computation()->parameter_instruction(0);
  p0->set_original_value(std::make_shared<OriginalValue>(SyntheticCall{}));
...
ENTRY main {
  ...
  auto value1 = std::make_shared<OriginalValue>(SyntheticCall{});
  auto value2 = std::make_shared<OriginalValue>(SyntheticCall{});
