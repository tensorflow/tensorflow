...
class RecordingVisitor : public DfsHloVisitorWithDefault {
 public:
  absl::Status DefaultAction(HloInstruction* hlo_instruction) override {
    visited_instructions_.push_back(hlo_instruction);
    return absl::OkStatus();
  }

  const std::vector<HloInstruction*>& visited_instructions() const {
    return visited_instructions_;
  }

 private:
  std::vector<HloInstruction*> visited_instructions_;
};
...
TEST(FilteredDfsHloVisitorTest, FiltersInstructions) {
  ...
  // Create a visitor that records visited instructions.
  RecordingVisitor recording_visitor;

  // Create a filtered visitor that only visits Add instructions.
  FilteredDfsHloVisitor filtered_visitor(
      &recording_visitor, [](const HloInstruction* instruction) {
  ...
  EXPECT_THAT(recording_visitor.visited_instructions(), ElementsAre(add));
}
