...
std::string OpMetadataToString(const OpMetadata& metadata, bool only_op_name) {
  ...
  if (metadata.has_interned_metadata_payload()) {
    if (metadata.interned_metadata_payload().has_id() &&
        metadata.interned_metadata_payload().id() != 0) {
      result.push_back(absl::StrCat("interned_metadata_id=",
                                    metadata.interned_metadata_payload().id()));
    }
    if (metadata.interned_metadata_payload().has_value()) {
      result.push_back(absl::StrCat(
          "interned_metadata=\"",
          absl::CEscape(metadata.interned_metadata_payload().value()), "\""));
    }
  }
