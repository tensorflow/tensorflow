...
TEST(BackendConfigPoolTest, LazyGCDoesNotCleanOtherBuckets) {
  auto* pool = BackendConfigPool::Get();
  pool->ResetForTesting();
  pool->GarbageCollect();  // Clean up previous garbage.

  std::string json1 = "{\"foo\": 100}";
  std::string json2 = "{\"foo\": 200}";

  ASSERT_NE(absl::HashOf(json1), absl::HashOf(json2));
...
TEST(BackendConfigPoolTest, InternNormalizesStrings) {
  auto* pool = BackendConfigPool::Get();
  pool->ResetForTesting();

  std::string raw_json = "{\"wait_on_operation_queues\": [], \"foo\": 1}";
  std::string expected_json = "{ \"foo\": 1}";

  auto shared = pool->Intern(raw_json);

  EXPECT_EQ(*shared, expected_json);
}
