...
TEST(MeshAndAxisTest, MeshToAndFromProtoIotaTiling) {
  MeshProto expected;
  expected.add_axes()->set_name("a");
  expected.add_axes()->set_name("b");
  expected.add_axes()->set_name("c");
  expected.mutable_axes(0)->set_size(2);
  expected.mutable_axes(1)->set_size(3);
  expected.mutable_axes(2)->set_size(6);

  IotaTileAssignment iota = IotaTileAssignment::Create({2, 3, 6});
  std::vector<std::string> axes_abc = {"a", "b", "c"};
  EXPECT_THAT(Mesh(TileAssignment(iota), axes_abc).ToProto(),
              EqualsProto(expected));
  EXPECT_EQ(Mesh(TileAssignment(iota), axes_abc), Mesh::FromProto(expected));
}
