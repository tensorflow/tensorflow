...
absl::StatusOr<std::unique_ptr<HloInstruction>> HloInstruction::CreateFromProto(
  ...
  switch (opcode) {
    ...
    case HloOpcode::kCompare: {
      ...
      if (!comparison_type_str.empty()) {
        ...
        TF_XLA_ASSIGN_OR_RETURN(auto comparison_type,
    ...
    case HloOpcode::kFusion: {
      ...
      TF_XLA_ASSIGN_OR_RETURN(FusionKind fusion_kind,
    ...
    case HloOpcode::kOutfeed: {
      TF_XLA_ASSIGN_OR_RETURN(Shape outfeed_shape,
    ...
    case HloOpcode::kCustomCall: {
      if (proto.constrain_layout()) {
        ...
        for (const ShapeProto& shape_proto : operand_shapes_with_layout) {
          TF_XLA_ASSIGN_OR_RETURN(Shape shape, Shape::FromProto(shape_proto));
  ...
  for (const int64_t predecessor_id : proto.control_predecessor_ids()) {
    ...
    TF_RET_CHECK(ContainsKey(instruction_map, local_predecessor_id))
        ...
        << proto.name();
    TF_XLA_RETURN_IF_ERROR(instruction_map.at(local_predecessor_id)
  ...
  if (has_rare()) {
    for (HloInstruction* predecessor : rare()->control_predecessors) {
      for (HloInstruction* successor : rare()->control_successors) {
        TF_XLA_RETURN_IF_ERROR(predecessor->AddControlDependencyTo(successor));
      }
    }
  }
  TF_XLA_RETURN_IF_ERROR(DropAllControlDeps());
  return absl::OkStatus();
}
...
absl::Status HloInstruction::CopyAllControlDepsTo(HloInstruction* start,
  ...
  for (auto* ctrl_pred : control_predecessors()) {
    TF_XLA_RETURN_IF_ERROR(ctrl_pred->AddControlDependencyTo(start));
  }
  for (auto* ctrl_succ : control_successors()) {
    TF_XLA_RETURN_IF_ERROR(end->AddControlDependencyTo(ctrl_succ));
  }
  return absl::OkStatus();
}
...
absl::Status HloInstruction::Defuse() {
  ...
  TF_XLA_RETURN_IF_ERROR(parent()->RemoveInstruction(this));
...
absl::StatusOr<HloInstruction*> HloInstruction::UnfuseInstruction(
  ...
  TF_XLA_RETURN_IF_ERROR(instruction->ReplaceAllUsesWith(new_parameter));
...
absl::Status HloInstruction::ReplaceAllUsesWithDifferentShape(
  ...
  for (HloInstruction* user : users_vector) {
    TF_XLA_RETURN_IF_ERROR(ReplaceUseWithDifferentShape(user, new_producer));
  }
  ...
  do {
    ...
    if (visit_state == Visitor::kVisiting) {
      ...
      if (visitor->ShouldProcessNode(current_node)) {
        TF_XLA_RETURN_IF_ERROR(visitor->Preprocess(current_node));
        VLOG(2) << "Visiting HLO %" << current_node->name();
        TF_XLA_RETURN_IF_ERROR(current_node->Visit(visitor));
        visitor->SetVisitState(current_id, Visitor::kVisited);
        TF_XLA_RETURN_IF_ERROR(visitor->Postprocess(current_node));
      } else {
        visitor->SetVisitState(current_id, Visitor::kVisited);
      }
      continue;
...
absl::Status HloInstruction::Accept(
  ...
  TF_XLA_RETURN_IF_ERROR(PostOrderDFS(this, visitor, std::nullopt,
                                  ignore_control_predecessors,
                                  cross_computation));
  if (call_finish_visit) {
    TF_XLA_RETURN_IF_ERROR(visitor->FinishVisit(this));
  }
  return absl::OkStatus();
}
...
absl::Status HloInstruction::AcceptWithOperandOrder(
  ...
  TF_XLA_RETURN_IF_ERROR(PostOrderDFS(this, visitor, func,
