...
absl::StatusOr<bool> AreOperandsAndOutputFullyBound(
  ...
  if (!async_op->IsAsynchronous()) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Instruction is not asynchronous: ", async_op->ToString()));
  }
  if (index.empty()) {
  ...
  const Shape& async_tuple_shape = (async_op->opcode() == HloOpcode::kAsyncDone)
                                       ...
                                       : async_op->shape();
  // The async tuple shape is a tuple where index 0 contains the operands tuple
  // and index 1 contains the output shape.
