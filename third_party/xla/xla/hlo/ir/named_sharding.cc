...
std::string NamedSharding::ToString(bool include_metadata) const {
  ...
  // Special cases: fully replicated, manual, or unreduced shardings do not
  // require meshes in their string representation.
  ...
  if (IsManual() && num_dimensions() == 0) {
    return absl::StrCat("{manual", metadata_str, "}");
  }
