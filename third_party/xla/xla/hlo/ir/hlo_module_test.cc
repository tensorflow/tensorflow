...
// Common patter across XLA which is not recommended because it creates 2
// copies. The better ways it to use HloModule::shared_config()
...
TEST(HloModuleTest, CloneGeneral) {
  ...
  std::unique_ptr<HloModule> m2 = m1.Clone("clone");

  EXPECT_EQ(&m1.config(), &m2->config());
  EXPECT_EQ(absl::StrCat(m1.entry_computation()->name(), ".clone"),
  ...
  int i = 0;
  std::vector<absl::string_view> m1_names;
  for (HloComputation* c : m1.computations()) {
    m1_names.push_back(c->name());
  }
  for (HloComputation* c : m2->computations()) {
    EXPECT_EQ(absl::StrCat(m1_names[i], ".clone"), c->name())
        << "Computation sequence mismatch at " << i;
    i++;
  }
}

TEST(HloModuleTest, CloneAndSharedConfig) {
  HloModule m1("-", HloModuleConfig());
  std::unique_ptr<HloModule> pm2 = m1.Clone("clone");
  EXPECT_EQ(&m1.config(), &pm2->config());
  EXPECT_EQ(m1.shared_config().use_count(), 3);

  std::unique_ptr<HloModule> pm3 = m1.Clone(m1.shared_config(), "clone");
