...
absl::StatusOr<std::unique_ptr<HloModule>> LinkComputation(
  ...
  TF_ASSIGN_OR_RETURN([[maybe_unused]] bool changed,
                      dce_pass.Run(linked_module.get()));

  if (VLOG_IS_ON(6)) {
    for (const HloComputation* comp : linked_module->computations()) {
      VLOG(6) << comp->name() << " [" << comp->unique_id() << "]";
      for (const HloInstruction* instr : comp->instructions()) {
        VLOG(6) << "  " << instr->ToString() << " [" << instr->unique_id()
                << "]";
      }
    }
    VLOG(6) << linked_module->ToString(HloPrintOptions().set_print_ids(true));
  }
