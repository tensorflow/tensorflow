...
std::string HloDataflowAnalysis::ToString() const {
  ...
  for (const HloValue* value : values()) {
    StrAppend(&out, value->ToString(/*indent=*/1));
  }
  ...
  for (const HloComputation* computation : module_.computations()) {
    ...
    for (const HloInstruction* instruction : computation->instructions()) {
      if (instruction->shape().IsTuple()) {
        StrAppend(&out, "  Instruction: ", instruction->name(), "\n");
...
bool HloDataflowAnalysis::UpdateAsyncUpdateValueSet(
  ...
  ShapeUtil::ForEachSubshape(
      ...
      [&](const Shape& subshape, const ShapeIndex& index) {
        ...
        if (index.front() == 2 &&
            (!ShapeUtil::IndexIsValid(async_update->shape(), index) ||
             !ShapeUtil::Compatible(
                 ShapeUtil::GetSubshape(async_update->shape(), index),
                 subshape))) {
          // the context subshape {2} might not be present or might not be the
          // same in this async_op.
          VLOG(1) << "Skip propagating values from "
                  << "Index " << index.ToString()
                  << ". Reason: it (valid in prev_async_op) "
                  << "is not valid in async_update or the subshape is not the "
                  << "same as in prev_async_op:\n"
                  << "prev_async_op: " << prev_async_op->ToString() << "\nvs\n"
                  << "async_update: " << async_update->ToString();
          return;
        }
    ...
    ShapeUtil::ForEachSubshape(curr_output_shape, [&](const Shape& subshape,
      ...
      if (root != nullptr) {
        ...
        if (prev_value_set != nullptr &&
            !ShapeUtil::GetSubshape(prev_async_op->shape(), instruction_index)
                 .IsTuple()) {
          changed |=
