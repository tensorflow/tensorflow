...
static optional<std::vector<int64_t>> GetGTEOperandIndices(
  ...
  for (const HloInstruction* operand : instr->operands()) {
    ...
    int64_t operand_tuple_idx = possibly_gte->tuple_index();
    tuple_indices.push_back(operand_tuple_idx);
  }
...
optional<int64_t> GetLoopInductionVarTupleIdxWithKnownValues(
  ...
  optional<std::vector<int64_t>> cond_tuple_indices =
      GetGTEOperandIndices(while_cond_root, while_cond_param);
...
optional<Range> MatchTrivialLoopRange(const HloInstruction* while_op) {
  ...
  return MatchTrivialLoopRangeWithKnownValues(while_op, *indvar_tuple_idx, {});
}

optional<Range> MatchTrivialLoopRangeWithKnownValues(
