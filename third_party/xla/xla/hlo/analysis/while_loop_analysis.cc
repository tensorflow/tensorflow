...
static optional<int64_t> GetGTEOperandIndex(const HloInstruction* instr,
  ...
  optional<absl::flat_hash_set<int64_t>> tuple_indices =
      GetGTEOperandIndices(instr, gte_operand);
  if (!tuple_indices.has_value()) {
    return nullopt;
  }
  if (tuple_indices->size() != 1) {
    return nullopt;
  }
  return *tuple_indices->begin();
}
