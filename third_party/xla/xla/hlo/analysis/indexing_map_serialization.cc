...
struct Token {
  enum class Kind {
    ...
    // Arithmetic operation, e.g. "+", "-", "*", "floorDiv", "ceilDiv", "mod".
    kPlus,
    kMinus,
    kTimes,
    kFloorDiv,
...
bool IsPartOfSymbolicExpr(Token token) {
  return token.kind == Token::Kind::kVarName ||
         ...
         token.kind == Token::Kind::kCeilDiv || token.kind == Token::Kind::kMod;
}
...
class Parser {
 public:
  ...
  bool ParseSymbolicExprString(std::string* symbolic_expr_str);
...
bool Parser::ParseSymbolicExprString(std::string* symbolic_expr_str) {
  ...
  while (true) {
    ...
    // If we see a comma and there are unmatched parens, then it is a part of
    // the symbolic expression (e.g. `max(d0, 1)`). Otherwise it is a delimiter.
    if (current_token_.kind == Token::Kind::kComma &&
        num_unmatched_parens > 0) {
      symbolic_expr_str->push_back(',');
      Advance();
      continue;
...
Token Parser::GetNextTokenImpl() {
  ...
  if (absl::ascii_isalpha(*it_)) {
    ...
    if (spelling == "ceilDiv") {
      return Token{spelling, Token::Kind::kCeilDiv};
    }
...
// Parses a comma separated list of symbolic expressions. It is used to parse
// the list of symbolic map results.
bool ParseSymbolicExprStrings(
    Parser& parser, SmallVectorImpl<std::string>& symbolic_expr_strs) {
  auto parse_var_name_fn = [&](Parser& parser) {
    std::string symbolic_expr_str;
    if (!parser.ParseSymbolicExprString(&symbolic_expr_str)) {
      return false;
    }
    symbolic_expr_strs.push_back(symbolic_expr_str);
    return true;
  };
...
std::optional<IndexingMap> ParseIndexingMap(llvm::StringRef input,
  ...
  // Parse symbolic map results.
  SmallVector<std::string, 3> symbolic_expr_strs;
  if (!parser.ConsumeToken(Token::Kind::kArrow) ||
      !ParseSymbolicExprStrings(parser, symbolic_expr_strs)) {
    llvm::errs() << "Failed to parse symbolic map results\n";
    return std::nullopt;
  }
  int num_symbolic_map_results = symbolic_expr_strs.size();

  // Special case: no domain is printed for the empty map.
  if (dim_var_names.empty() && range_var_names.empty() &&
      rt_var_names.empty()) {
    if (num_symbolic_map_results != 0 ||
  ...
  if (!ParseSymbolicExprs(dim_var_names, symbol_var_names, symbolic_expr_strs,
                          mlir_context, symbolic_exprs)) {
    llvm::errs() << "Failed to parse symbolic expressions\n";
    return std::nullopt;
  }
  auto symbolic_map_results =
      ArrayRef(symbolic_exprs).take_front(num_symbolic_map_results);
  auto constraint_exprs =
      ArrayRef(symbolic_exprs).drop_front(num_symbolic_map_results);
  ...
  auto symbolic_map = SymbolicMap::Get(mlir_context, dim_vars.size(),
                                       range_vars.size() + rt_vars.size(),
                                       llvm::to_vector(symbolic_map_results));
