...
HloReachabilityMap::HloReachabilityMap(
  ...
  if (instructions.size() > 10000) {
    LOG(ERROR) << "Allocating HlOReachabilityMap: " << bits_per_bitset_ << " "
               << (total_rows * words_per_bitset_) << " in "
               << bit_storage_.size() << " blocks";
  }
  if (instructions.size() > 10000) {
    LOG(ERROR) << "Done allocating HlOReachabilityMap: " << bits_per_bitset_
               << " " << (total_rows * words_per_bitset_) << " in "
               << bit_storage_.size() << " blocks";
  }
