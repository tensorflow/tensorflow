...
using xla::gpu::AffineExprToSymbolicExpr;
using xla::gpu::AffineMapToSymbolicMap;
using xla::gpu::ConvertAffineConstraintsToSymbolicConstraints;
using xla::gpu::SymbolicExpr;
using xla::gpu::SymbolicExprContext;
using xla::gpu::SymbolicExprType;
using xla::gpu::SymbolicMap;
using xla::gpu::SymbolicMapToAffineMap;
 ...
 public:
  explicit SymbolicExprSimplifier(
      ...
      : range_evaluator_(range_evaluator),
        context_(range_evaluator_->GetSymbolicExprContext()),
        zero_(context_->CreateConstant(0)),
...
std::optional<int64_t> SymbolicExprSimplifier::GetConstantRhs(
    SymbolicExpr expr, SymbolicExprType kind) {
...
SymbolicExpr SymbolicExprSimplifier::SimplifyOnce(SymbolicExpr expr) {
  ...
  if (simplify_point_dimensions_ ==
    ...
    if (bounds.IsPoint()) {
      return context_->CreateConstant(bounds.lower);
    }
  }
...
SmallVector<SymbolicExpr, 4> GetComposedSymbolsPermutationToCorrectOrder(
  ...
  for (int id = 0; id < second.GetRangeVarsCount(); ++id) {
    // TODO(karupayun): Check if this is correct or we should use the first.
    symbol_replacements.push_back(
        second.GetSymbolicMap().GetSymbolExpression(id));
  }
  ...
  for (int64_t id = 0; id < second_rt_vars_count; ++id) {
    symbol_replacements.push_back(
        second.GetSymbolicMap().GetSymbolExpression(rt_vars_second_start++));
  }
  int64_t range_vars_first_start = second_range_vars_count;
  for (int64_t id = 0; id < first_range_vars_count; ++id) {
    symbol_replacements.push_back(
        first.GetSymbolicMap().GetSymbolExpression(range_vars_first_start++));
  }
  ...
  for (int64_t id = 0; id < first_rt_vars_count; ++id) {
    symbol_replacements.push_back(
        first.GetSymbolicMap().GetSymbolExpression(rt_vars_first_start++));
  }
...
SmallVector<SymbolicExpr, 4> MapSymbolsToComposedSymbolsList(
  ...
  for (int64_t id = 0; id < map.GetRangeVarsCount(); ++id) {
    symbol_replacements.push_back(
        map.GetSymbolicMap().GetSymbolExpression(range_vars_start++));
  }
  int64_t rt_vars_start = composed.GetSymbolCount() - map.GetRTVarsCount();
  for (int64_t id = 0; id < map.GetRTVarsCount(); ++id) {
    symbol_replacements.push_back(
        map.GetSymbolicMap().GetSymbolExpression(rt_vars_start++));
  }
  return symbol_replacements;
}
...
IndexingMap::IndexingMap(
    ...
    : symbolic_expr_context_(affine_map.getContext()),
      dim_vars_(std::move(dimensions)),
      range_vars_(std::move(range_vars)),
      rt_vars_(std::move(rt_vars)) {
  mlir_context_ = affine_map.getContext();
  symbolic_map_ = AffineMapToSymbolicMap(affine_map, &symbolic_expr_context_);
  ...
  for (const auto& [expr, range] :
       ConvertAffineConstraintsToSymbolicConstraints(
           constraints_map, &symbolic_expr_context_, affine_map.getNumDims())) {
...
IndexingMap::IndexingMap(
  ...
  constraints_ = ConvertAffineConstraintsToSymbolicConstraints(
      constraints, &symbolic_expr_context_, affine_map.getNumDims());
...
IndexingMap::IndexingMap(
    ...
    : symbolic_map_(symbolic_map),
      symbolic_expr_context_(symbolic_map.GetContext()->GetMLIRContext()),
      dim_vars_(std::move(dimensions)),
      range_vars_(std::move(range_vars)),
      rt_vars_(std::move(rt_vars)) {
  if (!VerifyVariableIntervals()) {
...
RangeEvaluator IndexingMap::GetRangeEvaluator() const {
  return RangeEvaluator(*this, GetSymbolicExprContext());
}
...
llvm::MapVector<gpu::SymbolicExpr, Interval>
IndexingMap::GetSymbolicConstraints() const {
  return constraints_;
}

void IndexingMap::AddConstraint(mlir::AffineExpr expr, Interval range) {
  AddConstraint(AffineExprToSymbolicExpr(expr, GetSymbolicExprContext(),
...
void IndexingMap::AddConstraint(SymbolicExpr expr, Interval range) {
  ...
  if (expr.GetType() == SymbolicExprType::kVariable) {
    ...
    int64_t dim_vars_count = GetDimVarsCount();
    if (var_id < dim_vars_count) {
      Interval& current_range = GetMutableDimensionBound(var_id);
      current_range = current_range.Intersect(range);
      if (!current_range.IsFeasible()) {
        ResetToKnownEmpty();
      }
    } else {
      Interval& current_range = GetMutableSymbolBound(var_id - dim_vars_count);
      current_range = current_range.Intersect(range);
      if (!current_range.IsFeasible()) {
        ResetToKnownEmpty();
      }
...
bool IndexingMap::ConstraintsSatisfied(
  ...
  return ConstraintsSatisfied(
      AffineExprsToSymbolicExprs(dim_const_exprs, GetSymbolicExprContext(),
                                 symbolic_map_.GetNumDims()),
      AffineExprsToSymbolicExprs(symbol_const_exprs, GetSymbolicExprContext(),
...
bool IndexingMap::IsSymbolConstrained(int64_t symbol_id) const {
  for (const auto& [expr, _] : constraints_) {
    bool result = false;
    // Symbols come after dimensions in the symbolic map.
    int64_t dim_vars_count = GetDimVarsCount();
    expr.Walk([&](SymbolicExpr leaf) {
      if (leaf.GetType() == SymbolicExprType::kVariable) {
        if (leaf.GetValue() - dim_vars_count == symbol_id) {
          result = true;
        }
      }
    });
...
Interval RangeEvaluator::ComputeExpressionRange(SymbolicExpr expr) {
  switch (expr.GetType()) {
    case SymbolicExprType::kConstant: {
      return Interval{expr.GetValue(), expr.GetValue()};
    }
    case SymbolicExprType::kVariable: {
      int64_t var_id = expr.GetValue();
      int64_t dim_vars_count = indexing_map_.GetDimVarsCount();
      if (var_id < dim_vars_count) {
        return indexing_map_.GetDimensionBound(var_id);
      }
      return indexing_map_.GetSymbolBound(var_id);
    }
    default:
      break;
  }
...
MLIRContext* IndexingMap::GetMLIRContext() const {
  return IsUndefined() ? nullptr : mlir_context_;
}

SymbolicExprContext* IndexingMap::GetSymbolicExprContext() const {
...
bool IndexingMap::Simplify(SimplifyPointDimensions simplify_point_dimensions) {
  ...
  RangeEvaluator constraint_range_evaluator(*this, GetSymbolicExprContext(),
  ...
  RangeEvaluator range_evaluator(*this, GetSymbolicExprContext(),
...
void GetUsedParametersImpl(const SymbolicExpr& expr,
  ...
  if (expr.GetType() == SymbolicExprType::kVariable) {
    int64_t var_id = expr.GetValue();
    if (var_id < num_dims) {
      used_parameters.dimension_ids.insert(var_id);
    } else {
      used_parameters.symbol_ids.insert(var_id - num_dims);
    }
    return;
  }
  if (expr.GetType() != SymbolicExprType::kConstant) {
...
bool IndexingMap::CompressVars(const llvm::SmallBitVector& unused_dims,
  ...
  SymbolicExprContext* context = GetSymbolicExprContext();
  ...
  if (num_dims_changed) {
    ...
    dim_replacements = SmallVector<SymbolicExpr, 2>(num_dims_before,
                                                    context->CreateConstant(0));
    int64_t used_dims_count = 0;
    for (int i = 0; i < unused_dims.size(); ++i) {
      if (!unused_dims[i]) {
        compressed_dim_vars.push_back(dim_vars_[i]);
        dim_replacements[i] = context->CreateVariable(used_dims_count++);
      }
    }
  ...
  if (num_symbols_changed) {
    ...
    symbol_replacements = SmallVector<SymbolicExpr, 2>(
        num_symbols_before, context->CreateConstant(0));
    ...
    for (int i = 0; i < unused_symbols.size(); ++i) {
      if (!unused_symbols[i]) {
        ...
        symbol_replacements[i] =
            context->CreateVariable(num_dims_before + used_symbols_count++);
      }
    }
  ...
  for (const auto& [expr, range] : constraints_) {
    auto updated_expr = expr.ReplaceDimsAndSymbols(
        dim_replacements, symbol_replacements, symbolic_map_.GetNumDims());
...
void IndexingMap::ResetToKnownEmpty() {
  auto zero = GetSymbolicExprContext()->CreateConstant(0);
  symbolic_map_ = SymbolicMap::Get(
      GetSymbolicExprContext(), symbolic_map_.GetNumDims(),
      symbolic_map_.GetNumSymbols(),
...
bool IndexingMap::MergeModConstraints() {
  ...
  for (const auto& [lhs, binops] : grouped_constraints) {
    ...
    if (lhs.GetType() == SymbolicExprType::kVariable) {
      int64_t var_id = lhs.GetValue();
      if (var_id < GetDimensionCount()) {
        interval_to_update = &GetMutableDimensionBound(var_id);
      } else {
        // TODO: karupayun - Maybe this is wrong. We should think about how to
        // deal better with Dimensions and Symbols.
        interval_to_update =
            &GetMutableSymbolBound(var_id - GetDimensionCount());
      }
    }
...
IndexingMap ComposeIndexingMaps(const IndexingMap& first,
  ...
  SymbolicExprContext* context = first.GetSymbolicExprContext();
  SymbolicMap producer_symbolic_map = second.GetSymbolicMap();
  SymbolicMap consumer_symbolic_map = first.GetSymbolicMap();
  SymbolicMap composed_symbolic_map =
      producer_symbolic_map.Compose(consumer_symbolic_map);
  ...
  if (!symbol_replacements.empty()) {
    composed_symbolic_map = composed_symbolic_map.ReplaceDimsAndSymbols(
        /*dim_replacements=*/{}, symbol_replacements,
        composed_symbolic_map.GetNumDims(),
        composed_symbolic_map.GetNumSymbols());
  }
  IndexingMap composed_indexing_map(composed_symbolic_map, first.GetDimVars(),
  ...
  auto constraints_map =
      SymbolicMap::Get(context, producer_symbolic_map.GetNumDims(),
  ...
  for (const auto& [expr, range] : first.GetSymbolicConstraints()) {
    composed_indexing_map.AddConstraint(
        expr.ReplaceSymbols(first_map_symbols_to_composed_symbols,
                            first.GetDimensionCount()),
        range);
  }
  // Add constraints for consumer's codomain w.r.t. producer's domain.
  for (auto [index, expr] :
       llvm::enumerate(first.GetSymbolicMap().GetResults())) {
...
bool IndexingMap::RescaleSymbols() {
  ...
  for (const auto& [expr, range] : constraints_) {
    ...
    SymbolicExpr mod_lhs = expr.GetLHS();
    SymbolicExpr mod_rhs = expr.GetRHS();

    if (mod_rhs.GetType() != SymbolicExprType::kConstant) {
      continue;
    }
    // We don't rescale mod expressions with non-positive divisors.
    if (mod_rhs.GetValue() <= 0) {
      continue;
    }
    auto scaling_factor = mod_rhs.GetValue();

    if (mod_lhs.GetType() != SymbolicExprType::kVariable) {
      continue;
    }
    SymbolicExpr symbol_expr = mod_lhs;
    ...
    to_replace[symbol_expr] = mod_rhs * symbol_expr + shift_value;
    to_delete.insert(expr);

    symbolic_map_ =
        symbolic_map_.Replace(symbol_expr, mod_rhs * symbol_expr + shift_value);

    // TODO (karupayun): Update the way to refer to symbols.
    auto& symbol_range =
        range_vars_[symbol_expr.GetValue() - GetDimensionCount()].bounds;
...
bool IndexingMap::IsRangeVarSymbol(SymbolicExpr symbol) const {
  unsigned int position = symbol.GetValue() - GetDimensionCount();
...
IndexingMap IndexingMap::ConvertSymbolsToDimensions() const {
  ...
  for (const IndexingMap::Variable& var :
    ...
    syms_replacements.push_back(context->CreateVariable(symbol_id++));
...
IndexingMap ConvertRangeVariablesToDimensions(
  ...
  for (int i = 0; i < map.GetRangeVarsCount(); ++i) {
    ...
    if (updated_count < range_var_indices_count &&
      ...
      symbol_replacements.push_back(
          symbolic_map.GetDimExpression(updated_count));
      dims.push_back(range_var);
      updated_count++;
    } else {
      symbol_replacements.push_back(
          symbolic_map.GetSymbolExpression(i - updated_count));
  ...
  for (int i = 0; i < map.GetRTVarsCount(); ++i) {
    symbol_replacements.push_back(symbolic_map.GetSymbolExpression(
        map.GetRangeVarsCount() - range_var_indices_count + i));
  }
  ...
  SymbolicMap converted_symbolic_map = symbolic_map.ReplaceDimsAndSymbols(
      /*dim_replacements=*/{}, symbol_replacements,
      /*num_result_dims=*/symbolic_map.GetNumDims() + range_var_indices_count,
      /*num_result_symbols=*/symbolic_map.GetNumSymbols() -
          range_var_indices_count);
