...
HloSharding PropagateShardingAlongDimsAndReplicateOthers(
  ...
  if (source_sharding.UseNamedShardingLeaf()) {
    ...
    // TODO: Should I push all replicated axes to 'replicated_axes' or just rely
    // on implicit replicate axes ?
...
HloSharding ReplicateAllDataDims(const HloSharding& sharding,
  ...
  if (sharding.UseNamedShardingLeaf()) {
    ...
    // TODO: Should we just instead create a NamedSharding::Replicate() to avoid
    // empty dim_sharding vectors, when required we can get num of dimensions
    // via shape size ?
    // TODO: Should I push all axes to replicated_axes or just rely on implicit
    // replicate axes ?
    return HloSharding(NamedSharding(
        sharding.named_sharding().mesh(), dim_shardings,
        sharding.named_sharding().replicated_axes(),
...
HloSharding RemoveShapeDimensions(const HloSharding& sharding,
  ...
  if (sharding.UseNamedShardingLeaf()) {
    ...
    std::vector<NamedSharding::DimensionSharding> new_dim_shardings(
        sharding.num_dimensions() - dims_to_remove.size());
...
GroupedSharding GroupShardingOnDims(const HloSharding& sharding,
  ...
  if (sharding.IsManualSubgroup()) {
    int64_t tile_dimensions = sharding.num_dimensions();
