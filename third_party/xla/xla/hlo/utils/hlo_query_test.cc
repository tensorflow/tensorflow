...
TEST_F(HloQueryTest, FindInstructionUsingNameTest) {
  ...
  EXPECT_NE(hlo_query::FindInstruction(main, "zero").first, nullptr);
  EXPECT_NE(hlo_query::FindInstruction(main, "five").first, nullptr);
  EXPECT_NE(hlo_query::FindInstruction(main, "out").first, nullptr);
  EXPECT_EQ(hlo_query::FindInstruction(main, "foo").first, nullptr);
}
...
TEST_F(HloQueryTest, FindInstructionUsingOpcodeTest) {
  ...
  const HloComputation* main = hlo_query::FindComputation(module.get(), "main");
  EXPECT_NE(
      hlo_query::FindInstruction(main, StringToHloOpcode("add").value()).first,
      nullptr);
  EXPECT_NE(
      hlo_query::FindInstruction(main, StringToHloOpcode("constant").value())
          .first,
      nullptr);
  EXPECT_EQ(
      hlo_query::FindInstruction(main, StringToHloOpcode("select").value())
          .first,
      nullptr);
}
