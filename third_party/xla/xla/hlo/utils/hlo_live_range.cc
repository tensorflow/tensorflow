  ...
  for (const auto& entry : instruction_schedule_) {
    ...
    for (const auto& entry : value_set_tree) {
      for (const HloValue* value : entry.second.values()) {
        ...
        LogicalTime value_def_end_time = definition_end_time;
        for (const HloPosition& position : value->positions()) {
          const HloComputation* pos_comp = position.instruction->parent();
          auto async_context_it = computations_in_async_context_.find(pos_comp);
          if (async_context_it != computations_in_async_context_.end()) {
            const HloComputation* async_context = async_context_it->second;
            auto async_start =
                async_context->GetUniqueCaller(HloOpcode::kAsyncStart);
            CHECK(async_start)
                << "Async computations should have a unique caller.";
            auto async_done = (*async_start)->async_chain_done();
            auto async_done_it = instruction_schedule_.find(async_done);
            CHECK(async_done_it != instruction_schedule_.end());
            value_def_end_time =
                std::max(value_def_end_time, async_done_it->second);
          }
        }

        TimeBound live_range = GetLastPosition(*value, value_def_end_time);
