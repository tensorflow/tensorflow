...
TEST(HloShardingUtilTest, IsSubTilingOrEqualNamedShardingBasics) {
  Mesh mesh({2, 2}, {"x", "y"});
  NamedSharding s_x = test_utils::FromAxisNames(mesh, {{"x"}});
  NamedSharding s_y = test_utils::FromAxisNames(mesh, {{"y"}});
  NamedSharding s_xy = test_utils::FromAxisNames(mesh, {{"x", "y"}});
  NamedSharding s_empty = test_utils::FromAxisNames(mesh, {{}});
  Shape shape = ShapeUtil::MakeShape(F32, {8});

  EXPECT_TRUE(IsSubTilingOrEqualNamedSharding(shape, s_x, s_x));
  EXPECT_TRUE(IsSubTilingOrEqualNamedSharding(shape, s_xy, s_x));
  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, s_y, s_x));
  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, s_empty, s_x));
  EXPECT_TRUE(IsSubTilingOrEqualNamedSharding(shape, s_x, s_empty));
  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, s_x, s_xy));
}

TEST(HloShardingUtilTest, IsSubTilingOrEqualNamedShardingDifferentMeshes) {
  Mesh mesh1({2, 2}, {"x", "y"});
  Mesh mesh2({2, 2}, {"a", "b"});
  NamedSharding s1 = test_utils::FromAxisNames(mesh1, {{"x"}});
  NamedSharding s2 = test_utils::FromAxisNames(mesh2, {{"a"}});
  Shape shape = ShapeUtil::MakeShape(F32, {8});

  EXPECT_TRUE(IsSubTilingOrEqualNamedSharding(shape, s1, s2));
}
...
TEST(HloShardingUtilTest, IsSubTilingOrEqualNamedShardingOrderMismatch) {
  ...
  NamedSharding s_x = test_utils::FromAxisNames(mesh, {{"x"}});
  NamedSharding s_xy = test_utils::FromAxisNames(mesh, {{"x", "y"}});
  NamedSharding s_yx = test_utils::FromAxisNames(mesh, {{"y", "x"}});
  Shape shape = ShapeUtil::MakeShape(F32, {8});

  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, s_yx, s_x));
  EXPECT_TRUE(IsSubTilingOrEqualNamedSharding(shape, s_xy, s_x));
}

TEST(HloShardingUtilTest, IsSubTilingOrEqualNamedShardingSubAxes) {
  Mesh mesh({4, 2}, {"x", "y"});
  NamedSharding s_x_full(mesh,
                         {DimensionSharding({AxisRef(0)}, /*is_closed=*/true)});
  NamedSharding s_x_part1(
      mesh, {DimensionSharding({AxisRef(0, {1, 2})}, /*is_closed=*/true)});
  NamedSharding s_x_part2(
      mesh, {DimensionSharding({AxisRef(0, {2, 2})}, /*is_closed=*/true)});
  NamedSharding s_x_part1_y(mesh,
                            {DimensionSharding({AxisRef(0, {1, 2}), AxisRef(1)},
                                               /*is_closed=*/true)});
  Shape shape = ShapeUtil::MakeShape(F32, {8});

  EXPECT_TRUE(IsSubTilingOrEqualNamedSharding(shape, s_x_part1, s_x_part1));
  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, s_x_part1, s_x_part2));
  EXPECT_TRUE(IsSubTilingOrEqualNamedSharding(shape, s_x_full, s_x_part1));
  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, s_x_part1, s_x_full));
  EXPECT_TRUE(IsSubTilingOrEqualNamedSharding(shape, s_x_part1_y, s_x_part1));
  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, s_x_part1_y, s_x_part2));
}

TEST(HloShardingUtilTest,
     IsSubTilingOrEqualNamedShardingAxisProductSameButStructureDifferent) {
  Mesh mesh({2, 3}, {"x", "y"});
  NamedSharding sharding(mesh, {DimensionSharding({AxisRef(0), AxisRef(1)},
                                                  /*is_closed=*/true)});
  NamedSharding sub_sharding(mesh, {DimensionSharding({AxisRef(1), AxisRef(0)},
                                                      /*is_closed=*/true)});
  Shape shape = ShapeUtil::MakeShape(F32, {8});

  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, sub_sharding, sharding));
}
...
TEST(HloShardingUtilTest, IsSubTilingOrEqualNamedShardingDataContainmentFail) {
  ...
  EXPECT_FALSE(IsSubTilingOrEqualNamedSharding(shape, s_xy, s_x));
}
