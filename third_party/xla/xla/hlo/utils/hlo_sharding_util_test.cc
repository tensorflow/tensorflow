...
TEST(HloShardingUtilTest, GetManualSubgroupSharding_ManualOnly) {
  ...
  EXPECT_EQ(group_sharding.device_groups.num_groups(), 2);
  EXPECT_EQ(group_sharding.device_groups.num_devices_per_group(), 2);
  EXPECT_THAT(group_sharding.device_groups.array(),
              ::testing::ElementsAreArray({0, 2, 1, 3}));
}
...
TEST(HloShardingUtilTest, GetManualSubgroupSharding_ManualAndReplicted) {
  ...
  EXPECT_EQ(group_sharding.device_groups.num_groups(), 2);
  EXPECT_EQ(group_sharding.device_groups.num_devices_per_group(), 4);
  EXPECT_THAT(group_sharding.device_groups.array(),
              ::testing::ElementsAreArray({0, 2, 4, 6, 1, 3, 5, 7}));
}
...
TEST(HloShardingUtilTest, GetManualSubgroupSharding_ReplicatedAndManual) {
  ...
  EXPECT_EQ(group_sharding.device_groups.num_devices_per_group(), 4);
  EXPECT_THAT(group_sharding.device_groups.array(),
              ::testing::ElementsAreArray({0, 1, 4, 5, 2, 3, 6, 7}));
}
