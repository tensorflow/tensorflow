...
void BuildStackTracesRecursive(
  ...
  if (parent_comp->caller_instructions().empty()) {
    ...
    std::vector<const HloInstruction*> sorted_callers(
        parent_comp->caller_instructions().size());
    for (int i = 0; i < parent_comp->caller_instructions().size(); ++i) {
      sorted_callers[i] = parent_comp->caller_instructions()[i];
    }
