...
std::vector<std::string> SplitParamsString(const std::string& input) {
  ...
  // In a loop, consume subsequent parts. Using separate regexes for keys and
  // attributes is more robust than using a single regex with an alternation.
  ...
  // After the loop, the entire string must have been consumed, and at least
  // one key must have been present.
