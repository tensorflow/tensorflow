...
TEST_F(HloStackTraceTest, StackTraceFormatMatchesExpected) {
  ...
  std::string expected = R"(  Stack trace breakdown for peak usage: 80 bytes
    ToString (100.0%, 80 bytes)
      ...
      │   │   └── multiply (20.0%, 16 bytes, remaining: 64 bytes)
      │   └── call (20.0%, 16 bytes, remaining: 48 bytes)
      ├── arg1 (20.0%, 16 bytes, remaining: 32 bytes)
      ├── arg2 (20.0%, 16 bytes, remaining: 16 bytes)
      └── call.1 (20.0%, 16 bytes, remaining: 0 bytes)
)";
...
TEST_F(HloStackTraceTest, QuotedParameterNamesAppearInStackTrace) {
  ...
  std::string expected = R"(  Stack trace breakdown for peak usage: 48 bytes
    main (100.0%, 48 bytes)
      ├── params['transformer/layer_1/mlp/linear/AqtEinsum_0'] (33.3%, 16 bytes, remaining: 32 bytes)
      ├── params['transformer/layer_1/mlp/linear/AqtEinsum_0/AqtDotGeneral_0'] (33.3%, 16 bytes, remaining: 16 bytes)
      └── params['transformer/layer_1/mlp/linear/AqtEinsum_0/AqtDotGeneral_0/qrhs']['frozen'].qvalue (33.3%, 16 bytes, remaining: 0 bytes)
)";
  std::string actual = FormatStackTraceBreakdown(buffers, module.get());
  EXPECT_EQ(actual, expected);
}
