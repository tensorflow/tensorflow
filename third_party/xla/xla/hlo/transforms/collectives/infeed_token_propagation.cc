  ...
  for (HloComputation* branch : conditional->branch_computations()) {
    ...
    if (!parameter->shape().IsTuple()) {
      *parameter->mutable_shape() =
      ...
      TF_XLA_RETURN_IF_ERROR(parameter->ReplaceAllUsesWithDifferentShape(original));
    }
    ...
    if (!branch_tuple->shape().IsTuple()) {
      ...
      TF_XLA_RETURN_IF_ERROR(conditional->ReplaceOperandWithDifferentShape(
...
absl::Status CanonicalizeWhileInstruction(HloInstruction* loop) {
  ...
  if (!loop->shape().IsTuple()) {
    *loop->mutable_shape() = ShapeUtil::MakeTupleShape({loop->shape()});
    ...
    TF_XLA_RETURN_IF_ERROR(loop->ReplaceAllUsesWithDifferentShape(original));
  }
  ...
  if (!loop_tuple->shape().IsTuple()) {
    ...
    TF_XLA_RETURN_IF_ERROR(loop->ReplaceOperandWithDifferentShape(0, loop_tuple));
  }
...
absl::Status InfeedTokenPropagation::PropagateTokenThroughConditionalBranch() {
  ...
  TF_XLA_RETURN_IF_ERROR(input_token_->ReplaceAllUsesWith(input_token_gte));
  ...
  TF_XLA_RETURN_IF_ERROR(dangling_instruction_->ReplaceOperandWithDifferentShape(
      branch_operand_idx, branch_tuple));
  input_token_ =
...
absl::Status InfeedTokenPropagation::PropagateToken(
  ...
  if (caller->opcode() == HloOpcode::kConditional) {
    TF_XLA_RETURN_IF_ERROR(CanonicalizeConditionalInstruction(caller));
    TF_XLA_RETURN_IF_ERROR(PropagateTokenThroughConditionalBranch());
  } else if (caller->opcode() == HloOpcode::kWhile &&
             comp == caller->while_body()) {
    TF_XLA_RETURN_IF_ERROR(CanonicalizeWhileInstruction(caller));
    TF_XLA_RETURN_IF_ERROR(PropagateTokenThroughWhileBody());
  } else {
...
absl::StatusOr<bool> InfeedTokenPropagation::RunImpl(
  ...
  if (changed) {
    ...
    for (HloInstruction* dangling_infeed : dangling_infeeds) {
      ...
      TF_XLA_RETURN_IF_ERROR(PropagateToken(ordering));
    }
