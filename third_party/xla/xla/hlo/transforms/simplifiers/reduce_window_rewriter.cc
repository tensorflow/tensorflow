  ...
  ShapeUtil::ForEachSubshape(
      ...
      [&](const Shape& subshape, const ShapeIndex& shape_index) {
        ...
        SliceOutLastColumn(
            parent, subshape,
            /*outer_shape=*/
            ReduceWindowUtil::GetAtIndex(outer_reduce_window, shape_index),
  ...
  ShapeUtil::ForEachSubshape(
      ...
      [&](const Shape& subshape, const ShapeIndex& shape_index) {
        ...
        auto* inner_slice = parent->AddInstruction(HloInstruction::CreateSlice(
            last_col->shape(),
            ReduceWindowUtil::GetAtIndex(inner_reduce_window, shape_index),
  ...
  auto status = ShapeUtil::ForEachSubshapeWithStatus(
      ...
      [&](const Shape& subshape,
        ...
        auto scan = parent->AddInstruction(HloInstruction::CreateMap(
            ReduceWindowUtil::ShapeAtIndex(outer_reduce_window->shape(),
                                           shape_index),
        ...
        if (is_exclusive) {
          ...
          scan = parent->AddInstruction(HloInstruction::CreatePad(
              ReduceWindowUtil::ShapeAtIndex(reduce_window->shape(),
                                             shape_index),
...
absl::StatusOr<bool> ReduceWindowRewriter::Run(
  ...
  for (const auto& computation : module->computations(execution_threads)) {
    for (HloInstruction* instruction :
      ...
      TF_RETURN_IF_ERROR(
          ReduceWindowUtil::Replace1DReduceWindowWithReshape(reduce_window));
