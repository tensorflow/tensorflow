...
int64_t ReduceWindowRewriter::PreparePaddingForRewrite(
    HloReduceWindowInstruction* reduce_window,
    std::vector<HloInstruction*>& inputs, int64_t scan_dim, int64_t last_dim) {
  ...
  const int64_t scan_length = shape.dimensions(scan_dim);

  // getting round up to the base length to ensure that the padding is a
  ...
  if (scan_length != padded_length) {
    for (size_t input_index = 0; input_index < inputs.size(); ++input_index) {
      ...
      // moved scan dimensions to last dimension always -> rank - 1
...
absl::StatusOr<bool> ReduceWindowRewriter::TryOptimizeCumSumOrProd(
  ...
  const int64_t padded_length =
      PreparePaddingForRewrite(reduce_window, sources, scan_dim, last_dim);
