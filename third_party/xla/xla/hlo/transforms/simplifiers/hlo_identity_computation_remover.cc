...
// value is a parameter or a tuple/copy/bitcast of other identity values.
bool IsIdentityValue(const HloInstruction* instruction,
                     absl::flat_hash_map<const HloInstruction*, bool>* memo) {
  if (auto it = memo->find(instruction); it != memo->end()) {
    return it->second;
  }

  (*memo)[instruction] = false;
    ...
    switch (instruction->opcode()) {
      ...
      case HloOpcode::kTuple: {
        ...
        for (const HloInstruction* operand : instruction->operands()) {
          if (!IsIdentityValue(operand, memo)) {
...
HloInstruction* GetReplacement(HloInstruction* caller,
  ...
  // Memoization table for replacements for instructions in the identity
  // computation.
...
bool BypassCallers(
  ...
  for (HloInstruction* caller :
    ...
    if (caller->opcode() == HloOpcode::kCall) {
      TF_CHECK_OK(caller->parent()->ReplaceInstruction(caller, replacement));
      changed = true;
    } else if (caller->opcode() == HloOpcode::kAsyncStart) {
      for (HloInstruction* user : caller->users()) {
        if (user->opcode() == HloOpcode::kAsyncDone) {
          TF_CHECK_OK(user->parent()->ReplaceInstruction(user, replacement));
          changed = true;
        }
      }
    }
...
bool HloIdentityComputationRemover::IsIdentityComputation(
  ...
  absl::flat_hash_map<const HloInstruction*, bool> memo;
  return IsIdentityValue(computation->root_instruction(), &memo);
}
