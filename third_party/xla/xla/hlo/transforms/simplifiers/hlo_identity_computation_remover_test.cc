...
TEST_F(HloIdentityComputationRemoverTest,
       IsIdentityComputationWithNestedTuple) {
  const absl::string_view kHlo = R"(
HloModule test
...
ENTRY main {
  ...
  HloComputation* computation =
      module->GetComputationWithName("IdentityWithNestedTuple");
  EXPECT_TRUE(
      HloIdentityComputationRemover::IsIdentityComputation(computation));
}
...
TEST_F(HloIdentityComputationRemoverTest, RootIsNotParameter) {
  const absl::string_view kHlo = R"(
HloModule test
NotIdentity_RootIsNotParameter {
...
ENTRY main {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(kHlo));
  HloComputation* computation =
      module->GetComputationWithName("NotIdentity_RootIsNotParameter");
  EXPECT_FALSE(
      HloIdentityComputationRemover::IsIdentityComputation(computation));
}
