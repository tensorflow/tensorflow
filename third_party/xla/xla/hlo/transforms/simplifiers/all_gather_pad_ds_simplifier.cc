...
std::optional<OffsetSpec> ExtractValidPadSpec(
  ...
  for (int dim = 0; dim < pad->padding_config().dimensions().size(); ++dim) {
    ...
    // 3. Check for no interior padding.
    ...
    if (dim == split_dim) {
      // 5. Check for one-sided edge padding on the split dimension.

      if (!has_low_padding && !has_high_padding) {
        VLOG(2) << "No padding on the split dim: " << dim
                << " pad:" << pad->ToString();
        return std::nullopt;
      }
      if (has_low_padding && has_high_padding) {
        VLOG(2) << "Pad has both low and high edge padding on the split dim: "
                << pad->ToString();
        return std::nullopt;
      }
      ...
      // 4. Check for no padding on non-split dimensions.
      if (has_low_padding || has_high_padding) {
...
std::optional<OffsetToIdMap::const_iterator> GetPartitionIdForOffset(
  ...
  if (it == offset_to_partition_map.begin()) {
    return std::nullopt;  // Offset is smaller than the first key
  }
  // 5. check if it points to the end, it means the offset is larger than the
  // last key
  if (it == offset_to_partition_map.end()) {
    return std::prev(it);
  }

  // 3. check if the current key is the same as offset
  if (it->first == offset) {
    return std::prev(it);
  }
...
HloInstruction* GeneratesZeroFilledInstruction(HloComputation* computation,
  ...
  // Create a zero constant of the appropriate element type.
  HloInstruction* zero =
...
// Creates a predicate instruction based on the current partition ID and a list.
//
// This function generates HLO instructions to perform the following:
// 1. Create a constant 1D tensor from the provided `select_list`.
// 2. Get the current partition ID.
// 3. Dynamically slice the constant tensor using the partition ID as the index.
// 4. Compare the sliced scalar value with 1.
//
...
//   slice_sizes={1} %reshape = s64[] reshape(%slice) %const_one = s64[]
//   constant(1) %pred = pred[] compare(%reshape, %const_one), direction=EQ
HloInstruction* AddPredInstrBasedOnPartitionIdAndList(
    HloComputation* computation, std::vector<int64_t> select_list) {
  // Create a constant from the select_list.
...
std::optional<std::vector<HloInstruction*>> ProcessReplicaGroup(
  ...
  if (iter->first > start_offset) {
    new_instrs_per_rg.push_back(GeneratesZeroFilledInstruction(
...
std::optional<HloInstruction*> CreateFinalConcatAndSelect(
  ...
  HloInstruction* zero_filled = GeneratesZeroFilledInstruction(
      computation, ds, split_dim, ds_result_size);
...
std::optional<PatternMatchResult> MatchAndValidatePattern(
...
absl::Status AllGatherPadDsSimplifierVisitor::HandleDynamicSlice(
  ...
  std::optional<PatternMatchResult> pattern_match =
      MatchAndValidatePattern(dynamic_slice_hlo);
