...
absl::Status AlgebraicSimplifierVisitor::HandleReduceWindow(
  ...
  if (operand->opcode() == HloOpcode::kPad) {
    ...
    std::vector<int64_t> non_trivial_dimensions =
        reduce_window->non_trivial_dimensions();

    // If there is only one non-trivial dimension, we can check if the padding
    // ratio is big enough to justify the optimization.
    if (non_trivial_dimensions.size() == 1) {
      int64_t non_trivial_dimension = non_trivial_dimensions[0];

      // Getting the scan length.
      const Shape& operand_shape = reduce_window->inputs().front()->shape();
      int64_t scan_length = operand_shape.dimensions(non_trivial_dimension);

      // Padding size of the dimension in interest
      PaddingConfig padding_config = pad->padding_config();
      auto& padding_dim = padding_config.dimensions(non_trivial_dimension);
