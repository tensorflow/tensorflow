...
absl::Status AlgebraicSimplifierVisitor::BroadcastReduce(
    HloReduceInstruction* reduce, const Shape& reduce_result_shape,
...
absl::Status AlgebraicSimplifierVisitor::HandleReduce(HloInstruction* hlo) {
  ...
  if (ShouldBroadcastReduce(arg, reduce_result_shape)) {
    return BroadcastReduce(reduce, reduce_result_shape, multi_output_reduce);
  }
  ...
  if (ShapeUtil::ElementsIn(reduce_result_shape) ==
    ...
    return ReplaceReduceWithReshape(reduce, reduce_result_shape,
                                    multi_output_reduce);
  }
  ...
  if (ReorderReduceDotToDotReduce(reduce, arg, init_value)) {
    return absl::OkStatus();
  }
  ...
  if (arg->opcode() == HloOpcode::kReduce &&
    ...
    return MergeReduces(reduce, arg, reduce_result_shape, init_value, function);
  }
