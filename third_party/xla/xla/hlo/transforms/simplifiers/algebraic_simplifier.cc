...
AlgebraicSimplifierVisitor::TryRemovingReshapeTransposeChain(
  ...
  // Returns if reshape only adds or removes degenerate dimensions.
  auto is_degenerate_only = [](const HloInstruction* reshape) {
    return Shape::Equal().IgnoreLayout()(
        ShapeUtil::DropDegenerateDimensions(reshape->shape()),
        ShapeUtil::DropDegenerateDimensions(reshape->operand(0)->shape()));
  };
  ...
  auto get_effective_permutation =
      [&](absl::Span<const int64_t> permutation, const Shape& from_shape,
          const Shape& to_shape) -> std::vector<int64_t> {
    ...
    return std::vector<int64_t>(permutation.begin(), permutation.end());
  };

  if (!options_.is_layout_sensitive() && is_valid_reshape(reshape)) {
    int64_t effective_size = reshape->shape().dimensions().size();
    if (get_num_of_degenerate_dimensions(reshape->shape()) == 1) {
      effective_size--;
    }
