...
struct OffsetSpec {
  ...
  std::string ToString() const {
    return absl::StrCat("SplitDimOffsetSpec{ start_offset: ", start_offset,
...
//   otherwise.
std::optional<OffsetSpec> ExtractValidPadSpec(
    const HloPadInstruction* absl_nonnull pad, const Shape& ds_shape,
    const Shape& ag_shape, int64_t split_dim);
...
//   all-gather = f64[1,8,40]{2,1,0} all-gather(param)
//   pad = f64[1,96,40]{2,1,0} pad(all-gather, f64[] constant(0)),
//     padding=0_0x0_88x0_0
//   ds_indices = s32[] reshape(s32[1]{0} dynamic-slice(
//     constant({0, 24, 48, 72, 0, 24, 48, 72}), partition-id()))
//   ROOT ds = f64[1,24,40]{2,1,0} dynamic-slice(pad, s32[] constant(0),
...
//                            partition-id()),
//                    s64[] constant(1))
//   cp1 = f64[1,2,40]{2,1,0} collective-permute(param),
//       source_target_pairs={{5,4},{1,0}}
//   cp2 = f64[1,2,40]{2,1,0} collective-permute(param),
//       source_target_pairs={{6,4},{2,0}}
//   cp3 = f64[1,2,40]{2,1,0} collective-permute(param),
//       source_target_pairs={{7,4},{3,0}}
//   concatenate = f64[1,24,40]{2,1,0} concatenate(
//       param, cp1, cp2, cp3, broadcast(f64[] constant(0)))
//   ROOT select = f64[1,24,40]{2,1,0} select(compare,
//       concatenate,
//       f64[1,24,40]{2,1,0} broadcast(f64[] constant(0)))
