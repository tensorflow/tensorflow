...
namespace {
Shape ToFlat(const Shape& shape) {
  return ShapeUtil::MakeTupleShapeWithPtrs(ShapeUtil::FlattenTupleShape(shape));
}
...
absl::Status FlattenAndUpdateUses(HloInstruction* instr) {
  ...
  *instr->mutable_shape() = ToFlat(instr->shape());
  return absl::OkStatus();
}
...
absl::Status TupleSimplifier::FlattenCaller(HloInstruction* caller) {
  switch (caller->opcode()) {
    case HloOpcode::kAsyncStart: {
      ...
      constexpr int64_t kAsyncStartResultTupleSize = 3;
      CHECK_EQ(caller->shape().tuple_shapes().size(),
               kAsyncStartResultTupleSize);
      Shape flattened_shape =
          ToFlat(caller->shape().tuple_shapes(kResultIndex));
...
absl::StatusOr<bool> TupleSimplifier::Run(
  ...
  // First pass: flatten the nested result tuple in computations.
  ...
  for (auto* computation :
    ...
    TF_ASSIGN_OR_RETURN(bool c, FlattenRootTuple(computation));
    if (c) {
  ...
  // Second pass: remove whole tuples and collapse GTE chains.
