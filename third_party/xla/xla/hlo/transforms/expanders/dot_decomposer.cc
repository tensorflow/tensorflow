...
struct DotOperand {
  ...
  HloInstruction* sparcity_meta = nullptr;
...
absl::StatusOr<DotOperand> CanonicalizeDotOperand(
  ...
  HloInstruction* transposed = computation->AddInstruction(
      HloInstruction::CreateTranspose(operand_dims.shape(), operand, transpose),
      &operand->metadata());

  // * Batch dimensions are not collapsed.
  // * Non-contracting dimensionssare collapsed and removed if empty.
  ...
  DotOperand result;
  result.operand = computation->AddInstruction(
  ...
  if (auto iter = absl::c_find_if(sparsity,
    ...
    result.sparcity_meta = computation->AddInstruction(
...
absl::Status CanonicalizeDot(HloDotInstruction* original_dot) {
  ...
  for (auto& operand : {lhs, rhs}) {
    ...
    if (operand.sparcity_meta != nullptr) {
      sparse_meta.push_back(operand.sparcity_meta);
    }
  }
