...
HloEvaluator::DeferredChainResult HloEvaluator::TraceDeferredChain(
  ...
  while (IsDeferrableUnaryOp(result.source)) {
    result.deferred_chain.push_back(result.source);
    CHECK_EQ(result.source->operands().size(), 1);
    result.source = result.source->operand(0);
  }
  return result;
}
...
void HloEvaluator::MaterializeDeferredOp(const HloInstruction* instruction) {
  VLOG(1) << "Materializing deferred op: " << instruction->name()
