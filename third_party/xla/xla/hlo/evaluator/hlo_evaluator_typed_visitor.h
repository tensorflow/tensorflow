...
class HloEvaluatorTypedVisitor : public ConstDfsHloVisitorWithDefault {
 ...
 public:
  ...
  absl::Status HandleConvolutionWithLiterals(const HloInstruction* conv,
    ...
    TF_XLA_RETURN_IF_ERROR(result.PopulateParallel<ReturnT>(func));
  ...
  absl::Status HandleRaggedDotContractingWithLiterals(
    ...
    Shape dot_shape = GetShapeWithLayout(dot->shape());
    Literal result(dot_shape);
    TF_XLA_RETURN_IF_ERROR(result.PopulateParallel<
  ...
  absl::Status HandleScaledDot(const HloInstruction* dot) override {
    ...
    TF_XLA_ASSIGN_OR_RETURN(const Literal lhs_literal,
                        parent_->GetEvaluatedLiteralFor(lhs).Convert(
                            dot->shape().element_type()));
    TF_XLA_ASSIGN_OR_RETURN(const Literal rhs_literal,
    ...
    auto evaluate_scale =
      ...
      if (scale->shape().IsArray() && scale->shape().dimensions().size() > 0) {
        TF_XLA_ASSIGN_OR_RETURN(Literal scale_literal,
    ...
    TF_XLA_ASSIGN_OR_RETURN(Literal lhs_scale_literal,
                        evaluate_scale(lhs, lhs_scale));
    TF_XLA_ASSIGN_OR_RETURN(Literal rhs_scale_literal,
 ...
 private:
  ...
  absl::Status HandleScaledDotSlowPathWithLiterals(
    ...
    TF_XLA_ASSIGN_OR_RETURN(Literal result, Literal::Make(dot_shape));
    TF_XLA_RETURN_IF_ERROR(result.PopulateParallel<ReturnT>(
        [&](absl::Span<const int64_t> result_index, int /*thread_id*/) {
          // Locations in LHS and RHS that we read from.
 ...
 public:
  absl::Status HandlePad(const HloInstruction* pad) override {
    ...
    TF_XLA_ASSIGN_OR_RETURN(auto inferred_return_shape,
    ...
    if constexpr (std::is_floating_point_v<ElementwiseT>) {
      switch (distribution) {
        case RNG_UNIFORM: {
          ...
          TF_XLA_RETURN_IF_ERROR(result.Populate<ReturnT>(
