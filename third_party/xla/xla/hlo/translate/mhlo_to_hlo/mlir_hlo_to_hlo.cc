...
static void ExtractShardingsFromFunction(
  ...
  for (int i = 0, end = function.getNumArguments(); i < end; ++i) {
    if (auto sharding =
      ...
      // the entry function for now.
    } else if (auto sharding = xla::ExtractShardyArgShardingFromFrontendAttrs(
                   function, i, sdy_meshes);
               sharding && is_entry_function) {
  ...
  for (int i = 0, end = function.getNumResults(); i < end; ++i) {
    ...
    } else if (auto sharding =
                   xla::ExtractShardyResultShardingFromFrontendAttrs(
                       function, i, sdy_meshes);
               sharding && is_entry_function) {
