    ...
    case HloOpcode::kShuffle: {
      ...
      switch (shuffle_instruction->mode()) {
        ...
        case ShuffleMode::kPermute: {
          ...
          // element of the operand per element of the result expresses. This is
          // the general lowering; a backend that has an instruction for the
          // permutation is expected to match the shuffle before this.
          ABSL_ASSIGN_OR_RETURN(mlir::Value start_indices,
                           ImportShufflePermuteStartIndices(shuffle_instruction,
                                                            func_builder, loc));
