...
static bool IsNestedTupleInData(Type type) {
  ...
  auto data_tuple_type = llvm::dyn_cast<mlir::TupleType>(data_type);
  if (!data_tuple_type) return false;

  for (auto child_type : data_tuple_type.getTypes()) {
    if (llvm::isa<mlir::TupleType>(child_type)) return true;
  }
  ...
  if (flatten_region_arg_tuple) {
    ...
    for (Type computation_arg_type : computation_arg_types) {
      auto orig_tuple_arg_type =
          llvm::dyn_cast<mlir::TupleType>(computation_arg_type);
  ...
  switch (instruction->opcode()) {
    ...
    case HloOpcode::kSort: {
      ...
      if (mlir::TupleType tuple_ty =
              llvm::dyn_cast<mlir::TupleType>(result_type)) {
    ...
    case HloOpcode::kTopK: {
      ...
      auto topk_op = func_builder->create<mlir::mhlo::TopKOp>(
          loc, llvm::dyn_cast<mlir::TupleType>(result_type).getTypes(),
    ...
    case HloOpcode::kConditional: {
      ...
      mlir::Type pred_or_index_type =
          llvm::cast<mlir::TensorType>(operands[0].getType()).getElementType();
    ...
    case HloOpcode::kRng: {
      auto shape = func_builder->create<mlir::stablehlo::ConstantOp>(
          loc, Convert(llvm::cast<RankedTensorType>(result_type).getShape()));
    ...
    case HloOpcode::kConvert: {
      ...
      auto ranked_type = llvm::dyn_cast<mlir::RankedTensorType>(result_type);
      mlir::IntegerType integer_type =
          (ranked_type)
              ? llvm::dyn_cast<mlir::IntegerType>(ranked_type.getElementType())
              : nullptr;
