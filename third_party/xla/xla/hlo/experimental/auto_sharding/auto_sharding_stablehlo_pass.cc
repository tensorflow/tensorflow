...
#include "learning/deepmind/partir/compiler/aux/dialect.h"
#include "learning/deepmind/partir/compiler/control_flow/dialect.h"
#include "learning/deepmind/partir/compiler/core/dialect.h"
#include "learning/deepmind/partir/compiler/mhlo/dialect.h"
#include "learning/deepmind/partir/compiler/mhlo/rewrite_actions.pb.h"
#include "learning/deepmind/partir/compiler/spmd/dialect.h"
#include "learning/deepmind/partir/compiler/temporal/dialect.h"
...
void RegisterDialectDependencies(mlir::DialectRegistry& registry) {
  ...
  registry.insert<
      deepmind::partir::aux::PartirAuxDialect, deepmind::partir::PartirDialect,
      deepmind::partir::PartirMhloDialect, mlir::func::FuncDialect,
      mlir::mhlo::MhloDialect,
      deepmind::partir::control_flow::PartirControlFlowDialect,
      mlir::chlo::ChloDialect, deepmind::partir::spmd::PartirSpmdDialect,
      deepmind::partir::temporal::PartirTemporalDialect, mlir::sdy::SdyDialect,
...
class AutoShardingWrapperPass
 ...
 public:
  ...
  void runOnOperation() final {
    ...
    auto meshIter = module_op.getOps<mlir::sdy::MeshOp>();
    if (meshIter.empty()) {
      signalPassFailure();
    }
    mlir::sdy::MeshOp mesh_op_handle = *meshIter.begin();
    mlir::sdy::MeshAttr sdy_mesh = mesh_op_handle.getMesh();
    ...
    mlir::ArrayRef<mlir::sdy::MeshAxisAttr> axes = sdy_mesh.getAxes();
    std::vector<int64_t> device_mesh_shape;
    for (mlir::sdy::MeshAxisAttr axis : axes) {
    ...
    auto original_mesh_op =
        builder.create<mlir::sdy::MeshOp>(module_op.getLoc(), "mesh", sdy_mesh);
    symbol_table.insert(original_mesh_op);
    original_mesh_op->moveBefore(&module_op.getBody()->front());
