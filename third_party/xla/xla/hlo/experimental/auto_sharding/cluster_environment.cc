...
double ClusterEnvironment::ReshardingCost(const Shape& shape,
  ...
  // TiledDataRank().
  size_t src_rank =
      src_spec.IsTiled() ? src_spec.TiledDataRank() : shape.dimensions_size();
  size_t dst_rank =
      dst_spec.IsTiled() ? dst_spec.TiledDataRank() : shape.dimensions_size();
