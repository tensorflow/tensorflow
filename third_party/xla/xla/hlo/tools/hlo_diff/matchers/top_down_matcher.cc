...
void GreedyTopDownMatcher::Match(HloGumgraphMappings& mappings) const {
  ...
  HloGumgraphDfs(
      ...
      [&](const HloInstructionNode& left_node) {
        ...
        if (ShouldSkipMatching(left_node, *it->second,
        ...
        std::vector<HloInstructionNode*> right_children =
            GetUnmatchedChildren(it->second->children, mappings);

        MatchLeafInstructions(left_, right_, left_children, right_children,
