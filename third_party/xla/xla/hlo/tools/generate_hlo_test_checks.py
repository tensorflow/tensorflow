...
def _format_function_declaration_file_check(line_text: str) -> str:
    ...
    if match is None:
        raise ValueError(
            f"The line {repr(line_text)} did not match the expected "
    ...
    treat_function_name_as_explicit_symbol: bool = optional_line_number_suffix is None
...
class IterateByCategory(Generic[_T]):
    ...
    def iterate_over_buffer(self, target_buffer: collections.deque[_T]) -> Iterator[_T]:
        while True:
            try:
...
class HloStreamSplitter:
    ...
    def __init__(
        ...
        self._directive_histories: dict[DirectiveComment, collections.deque[str]] = {
...
class HloFileCheckLines:
    ...
    def _replace_symbol_names_with_regex_captures(
        ...
        for line in input_stream:
            ...
            yield re.sub(
                self._SYMBOL_NAME_REGEX,
                functools.partial(
                    self._replacer, is_verbatim=is_verbatim, is_global=is_global
                ),
                line,
            )
    ...
    def _generate_unique_name(self, symbol_name: str) -> str:
        ...
        optional_disambiguation_suffix = (
            ""
            if normalized_name_conflict_count == 0
...
class TestCheckWriter:
    ...
    def __init__(
        ...
        if worker_count is not None and worker_count < 1:
            raise ValueError(
                f"The `worker_count` argument must be either `None` or "
    ...
    def __enter__(self) -> TestCheckWriter:
        ...
        if self._context_manager_active:
            raise RuntimeError(
                "Tried to enter two simultaneous `with` blocks "
    ...
    def split_test_cases(self, test_file: Iterator[str]) -> Iterator[Iterator[str]]:
    ...
    def join_test_cases(
        self,
        test_cases: Union[Iterator[Iterator[str]], Iterator[tuple[Iterator[str], ...]]],
        num_outputs: int = 1,
        ...
        if num_outputs < 1:
            raise ValueError(
                "The `num_outputs` argument must be a positive integer; "
                f"got {num_outputs}."
            )
        elif num_outputs == 1:
    ...
    def annotate_test_case(
        ...
        Returns:
          ...
          original test case.
        """
        split_input = HloStreamSplitter(
            test_case, record_directives={DirectiveComment.RUN}
        )
...
def parse_args(
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            f'The HLO test file to update, or "{_STANDARD_IO_STREAMS}" for '
            f"stdin/stdout."
        ),
    )
    parser.add_argument(
        ...
        default=False,
        help=(
            f"Modify TEST_FILE in place instead of printing the results to "
    ...
    parser.add_argument(
        ...
        help=(
            f"All instances of this substring in OPT_ARGS will be expand to "
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            f"Arguments to pass into OPT_CMD. The input file should be "
    ...
    if parsed_args.in_place and parsed_args.test_file == _STANDARD_IO_STREAMS:
        sys.stderr.write(
            f'Warning: Suppressing the "-i"/"--in_place" flag because '
