...
ENTRY main {
  call-start.0 = ((), (), (s32[]{:S(2)}, s32[1024]{0:T(1024)}, s32[])) call-start(), to_apply={
  ...
  p1 = s32[] parameter(0)
  call-update.0 = ((s32[1024]{0:T(1024)}, s32[]), (s32[1024]{0:T(1024)}, s32[]), ()) call-update(call-start.0, buffer.0, p1)
...
ENTRY main {
  ...
  cc-update = ((s32[1024]{0:T(1024)}, s32[]), s32[1024]{0:T(1024)}, s32[])
...
TEST_F(HloParserTest, DesugarParsingTest_Dot_TooManyOperands) {
  const char* const hlo = R"(
HloModule async_dot_example
...
ENTRY main {
  %lhs = f32[128,128] parameter(0)
  %rhs = f32[128,128] parameter(1)
  %extra = f32[128,128] parameter(2)

  %dot-start = ((), (), u32[]) dot-start()
  %dot-update.0 = ((f32[128,128], f32[128,128]), (), u32[]) dot-update(%dot-start, %lhs, %rhs)
  %dot-update.1 = ((f32[128,128], f32[128,128], f32[128,128]), (), u32[]) dot-update(%dot-update.0, %extra)
  ROOT %result = f32[128,128] dot-done(%dot-update.1)
}
  )";
  auto status = ParseAndReturnUnverifiedModule(hlo).status();
  EXPECT_FALSE(status.ok());
...
ENTRY main {
  ...
  %call-start = ((f32[4]), (), s32[]) call-start(p0), to_apply={
    param0 = f32[4] parameter(0)
    param1 = f32[8] parameter(1)
    ROOT negate = f32[8] negate(param1)
  }
  %call-update = ((f32[4], f32[8]), (f32[8]), s32[]) call-update(%call-start, p1)
  ROOT %call-done = f32[8] call-done(%call-update)
}
  )";
  ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnUnverifiedModule(hlo));
  auto async_done = module->entry_computation()->root_instruction();
