...
// For kCall, the parser can correctly infer the complete shape
// from the to_apply computation, even
// if the async chain is incomplete.
...
TEST_F(HloParserTest,
       DesugarParsingTest_CustomCall_Incomplete_Chain_ZeroOperands) {
  const char* const hlo = R"(
HloModule main
...
// For kCustomCall, the parser infers  shapes of parameters and root
// as operands are progressively bound to the async chain.
TEST_F(HloParserTest,
       DesugarParsingTest_CustomCall_Incomplete_Chain_OneOperand) {
  const char* const hlo = R"(
HloModule main
...
ENTRY main {
  p0 = f32[] parameter(0)
  p1 = f32[] parameter(1)
  %cc-start = ((), (), s32[]) custom-call-start(), custom_call_target="foo"
  ROOT %cc-update = ((f32[]), s32[]) custom-call-update(%cc-start, p0)
}
)";
  ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnUnverifiedModule(hlo));
  HloInstruction* cc_update = module->entry_computation()->root_instruction();
  EXPECT_EQ(cc_update->opcode(), HloOpcode::kAsyncUpdate);
  EXPECT_EQ(cc_update->operand_count(), 2);
  HloComputation* called_computation = cc_update->async_wrapped_computation();
  EXPECT_EQ(called_computation->num_parameters(), 1);
  EXPECT_EQ(called_computation->parameter_instruction(0)->shape().ToString(),
            "f32[]");
  EXPECT_EQ(called_computation->root_instruction()->shape().ToString(),
...
// For kDot, the parse can infer the number of parameters from its arity
// even if the async chain is incomplete.
