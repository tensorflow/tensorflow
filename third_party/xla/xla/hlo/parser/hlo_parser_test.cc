...
ENTRY main {
  p0 = f32[] parameter(0)
  p1 = f32[] parameter(1)
  ROOT %fusion-start = ((), (), s32[]) fusion-start(), calls=fused_computation, kind=kLoop
}
)";
  ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnUnverifiedModule(hlo));
  HloInstruction* fusion_start =
      module->entry_computation()->root_instruction();
  EXPECT_EQ(fusion_start->opcode(), HloOpcode::kAsyncStart);
  EXPECT_EQ(fusion_start->operand_count(), 0);
  HloComputation* called_computation =
      fusion_start->async_wrapped_computation();
  EXPECT_EQ(called_computation->num_parameters(), 2);
  EXPECT_EQ(called_computation->parameter_instruction(0)->shape().ToString(),
            "f32[]");
  EXPECT_EQ(called_computation->parameter_instruction(1)->shape().ToString(),
