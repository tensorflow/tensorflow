...
// The `shifts` and `indices` attributes are each specific to a single mode, so
// each of them must be present if and only if its mode was selected. The tests
// below cover, for both modes, the attribute being missing, the attribute of
// the other mode being present as well, and the attribute of the other mode
...
// it and report the shape inference errors that the mode and its attributes
// violate.
