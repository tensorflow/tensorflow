...
// explicitly call `StartExporters ` from the appropriate place in your main
// function (e.g. after flags have been parsed).
//
// Example usage:
//
...
class ExporterRegistration {
 public:
  ...
  static void PeriodicallyExportMetrics();
  ...
  // metrics after ExporterRegistration::PeriodicallyExportMetrics() is called.
 ...
 private:
  ...
  void PeriodicallyExportMetricsImpl() ABSL_LOCKS_EXCLUDED(exporters_mu_);

  absl::Mutex exporters_mu_;
  std::vector<Exporter*> exporters_ ABSL_GUARDED_BY(exporters_mu_);
  bool periodically_export_metrics_called_ ABSL_GUARDED_BY(exporters_mu_) =
      false;
};
