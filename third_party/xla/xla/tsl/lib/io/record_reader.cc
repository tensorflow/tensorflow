...
absl::Status RecordReader::ReadChecksummed(uint64_t offset, size_t n,
  ...
  ABSL_RETURN_IF_ERROR(input_stream_->ReadNBytes(expected, result));
...
absl::Status RecordReader::GetMetadata(Metadata* md) {
  ...
  if (!cached_metadata_) {
    ...
    while (true) {
      ...
      ABSL_RETURN_IF_ERROR(input_stream_->SkipNBytes(length + kFooterSize));
...
absl::Status RecordReader::PositionInputStream(uint64_t offset) {
  ...
  if (curr_pos > desired_pos || curr_pos < 0 /* EOF */ ||
    ...
    last_read_failed_ = false;
    ABSL_RETURN_IF_ERROR(input_stream_->Reset());
    ABSL_RETURN_IF_ERROR(input_stream_->SkipNBytes(desired_pos));
  } else if (curr_pos < desired_pos) {
    ABSL_RETURN_IF_ERROR(input_stream_->SkipNBytes(desired_pos - curr_pos));
  }
...
absl::Status RecordReader::ReadRecord(uint64_t* offset, tstring* record) {
  ABSL_RETURN_IF_ERROR(PositionInputStream(*offset));
