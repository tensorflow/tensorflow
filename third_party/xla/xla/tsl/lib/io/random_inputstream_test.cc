...
TEST(RandomInputStream, ReadNBytes) {
  ...
  ASSERT_OK(env->NewRandomAccessFile(fname, &file));
  tstring read;
  RandomAccessInputStream in(file.get());
  ASSERT_OK(in.ReadNBytes(3, &read));
  EXPECT_EQ(read, "012");
  EXPECT_EQ(3, in.Tell());
  ASSERT_OK(in.ReadNBytes(0, &read));
  EXPECT_EQ(read, "");
  EXPECT_EQ(3, in.Tell());
  ASSERT_OK(in.ReadNBytes(5, &read));
...
TEST(RandomInputStream, SkipNBytes) {
  ...
  tstring read;
  RandomAccessInputStream in(file.get());
  ASSERT_OK(in.SkipNBytes(3));
  EXPECT_EQ(3, in.Tell());
  ASSERT_OK(in.ReadNBytes(0, &read));
  EXPECT_EQ(read, "");
  EXPECT_EQ(3, in.Tell());
  ASSERT_OK(in.ReadNBytes(4, &read));
  EXPECT_EQ(read, "3456");
  EXPECT_EQ(7, in.Tell());
  ASSERT_OK(in.SkipNBytes(0));
  EXPECT_EQ(7, in.Tell());
  ASSERT_OK(in.ReadNBytes(2, &read));
  EXPECT_EQ(read, "78");
  EXPECT_EQ(9, in.Tell());
