...
absl::Status ReadJsonString(const Json::Value& json, const std::string& name,
  ...
  ABSL_RETURN_IF_ERROR(ReadJsonValue(json, name, &json_value));
...
absl::Status OAuthClient::GetTokenFromRefreshTokenJson(
  ...
  ABSL_RETURN_IF_ERROR(ReadJsonString(json, "client_id", &client_id));
  ABSL_RETURN_IF_ERROR(ReadJsonString(json, "client_secret", &client_secret));
  ABSL_RETURN_IF_ERROR(ReadJsonString(json, "refresh_token", &refresh_token));
...
absl::Status OAuthClient::ParseOAuthResponse(
  ...
  ABSL_RETURN_IF_ERROR(ReadJsonString(root, "token_type", &token_type));
  if (token_type != "Bearer") {
    return absl::FailedPreconditionError("Unexpected Oauth token type: " +
                                         token_type);
  }
  int64_t expires_in = 0;
  ABSL_RETURN_IF_ERROR(ReadJsonInt(root, "expires_in", &expires_in));
  *expiration_timestamp_sec = request_timestamp_sec + expires_in;
  ABSL_RETURN_IF_ERROR(ReadJsonString(root, "access_token", token));
