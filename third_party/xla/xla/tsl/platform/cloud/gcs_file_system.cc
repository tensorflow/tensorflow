...
class GcsRandomAccessFile : public RandomAccessFile {
 public:
  using ReadFn = std::function<absl::Status(
      const std::string& filename, uint64 offset, size_t n,
...
class BufferedGcsRandomAccessFile : public RandomAccessFile {
 public:
  ...
  BufferedGcsRandomAccessFile(const std::string& filename, uint64 buffer_size,
                              ReadFn read_fn)
      : filename_(filename),
...
typedef std::function<absl::Status(
    const std::string& session_uri, uint64 file_size,
    const std::string& gcs_path, bool* completed, uint64* uploaded)>
    StatusPoller;
...
absl::Status GcsFileSystem::NewRandomAccessFile(
  ...
  if (cache_enabled_) {
    ...
    result->reset(new BufferedGcsRandomAccessFile(
        fname, block_size_,
        [this, bucket, object](const std::string& fname, uint64 offset,
...
absl::Status GcsFileSystem::NewWritableFile(
  ...
  auto status_poller = [this](const std::string& session_uri, uint64 file_size,
...
absl::Status GcsFileSystem::GetChildrenBounded(
    const std::string& dirname, uint64 max_results,
