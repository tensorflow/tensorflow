...
class ProfilerServiceImpl : public tensorflow::grpc::ProfilerService::Service {
 public:
  ...
  ::grpc::Status ContinuousProfiling(
    ...
    absl::MutexLock lock(&mutex_);
    if (continuous_profiling_sessions_.contains(req->session_id())) {
      return ::grpc::Status(
          ::grpc::StatusCode::ALREADY_EXISTS,
          "A profiling session with the same ID is already running.");
    }
    std::unique_ptr<ProfilerSession> profiler =
        ProfilerSession::Create(req->opts());
    absl::Status status = profiler->Status();
  ...
  ::grpc::Status GetSnapShot(::grpc::ServerContext* ctx,
    ...
    {
      absl::MutexLock lock(&mutex_);
      auto it = continuous_profiling_sessions_.find(req->session_id());
      if (it == continuous_profiling_sessions_.end()) {
        return ::grpc::Status(
            ::grpc::StatusCode::NOT_FOUND,
            "No continuous profiling session found with the given ID.");
      }
    ...
    // Restart profiling.
    {
      ...
      continuous_profiling_sessions_[req->session_id()] = {
