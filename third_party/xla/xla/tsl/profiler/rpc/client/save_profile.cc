...
absl::Status SaveXSpaceChunks(
  ...
  if (xspaces == nullptr || xspaces->empty()) {
    return absl::OkStatus();
  }
  ...
  if (!writer.status().ok()) {
    absl::Status status = writer.status();
    writer.Close();
    Env::Default()->DeleteFile(temp_path).IgnoreError();
    return status;
  }
  for (size_t i = 0; i < xspaces->size(); ++i) {
    std::string plane_names = GetPlaneNames((*xspaces)[i]);
    VLOG(1) << "SaveXSpaceChunks index: " << i
            << ", size: " << (*xspaces)[i].ByteSizeLong() << " bytes"
            << (plane_names.empty()
                    ? ""
                    : absl::StrCat(" [Planes: ", plane_names, "]"));
    if (!writer.WriteRecord((*xspaces)[i])) {
      absl::Status status = writer.status();
      writer.Close();
      Env::Default()->DeleteFile(temp_path).IgnoreError();
      if (status.ok()) {
        status = absl::InternalError(absl::StrCat(
            "RecordWriter::WriteRecord failed when writing XSpace chunk ", i,
            "."));
      }
      return status;
    }
  ...
  if (!writer.Close()) {
    ...
    if (status.ok()) {
      status = absl::InternalError(
          "RecordWriter::Close failed when writing XSpace chunks.");
    }
    return status;
  }
