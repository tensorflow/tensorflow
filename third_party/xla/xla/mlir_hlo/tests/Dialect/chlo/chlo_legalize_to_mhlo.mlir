...
// RUN: mlir-hlo-opt --chlo-legalize-to-high-level-mhlo="enable-acosh enable-acos enable-asin" --split-input-file -verify-diagnostics %s | FileCheck %s --check-prefix=CHECK-HIGH-LEVEL
...
// CHECK-LABEL: @asin
func.func @asin(%arg : tensor<f64>) -> tensor<f64> {
  // CHECK-HIGH-LEVEL: mhlo.asin
  %1 = "chlo.asin"(%arg) : (tensor<f64>) -> tensor<f64>
  func.return %1 : tensor<f64>
}
