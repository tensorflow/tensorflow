...
Attribute convertAttr(Attribute stablehloAttr) {
  ...
  if (auto attr = dyn_cast<stablehlo::ChannelHandleAttr>(stablehloAttr)) {
  ...
  if (auto attr =
          dyn_cast<stablehlo::ConvDimensionNumbersAttr>(stablehloAttr)) {
  ...
  if (auto attr =
          dyn_cast<stablehlo::CustomCallApiVersionAttr>(stablehloAttr)) {
    RETURN_CONVERTED_ENUM_ATTR(CustomCallApiVersion);
  }
  if (auto attr = dyn_cast<stablehlo::DotDimensionNumbersAttr>(stablehloAttr)) {
  ...
  if (auto attr =
          dyn_cast<stablehlo::GatherDimensionNumbersAttr>(stablehloAttr)) {
  ...
  if (auto attr =
          dyn_cast<stablehlo::ScatterDimensionNumbersAttr>(stablehloAttr)) {
...
Attribute decodePrecisionConfig(Attribute stablehloAttr) {
  auto arrayAttr = dyn_cast<ArrayAttr>(stablehloAttr);
  if (!arrayAttr) return {};
  SmallVector<Attribute> hloAttrs;
  for (auto attr : arrayAttr) {
    auto precisionStr = dyn_cast<StringAttr>(attr);
...
LogicalResult rewriteCustomCallAsMhloOp(stablehlo::CustomCallOp stablehloOp,
  ...
  auto stablehloConvertedAttrs =
      dyn_cast_or_null<DictionaryAttr>(stablehloOp->getAttr("mhlo.attributes"));
  ...
  if (stablehloHasRegion) {
    auto stablehloRegionOp =
        stablehloOp->getParentOfType<ModuleOp>().lookupSymbol<func::FuncOp>(
            cast<FlatSymbolRefAttr>(stablehloCalledComputations[0]));
...
LogicalResult fixupMhloBackendConfig(stablehlo::CustomCallOp stablehloOp,
  ...
  if (stablehloBackendConfig) {
    if (auto oldHloBackendConfig =
            dyn_cast_or_null<StringAttr>(hloOp.getBackendConfigAttr())) {
