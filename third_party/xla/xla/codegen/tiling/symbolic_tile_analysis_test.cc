...
TEST_F(SymbolicTileAnalysisTest,
       SameShapeMultiOutputFusionIsNotSupportedByDefault) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       ParseAndReturnVerifiedModule(R"(
fusion {
...
class SymbolicTileAnalysisWithSameShapeMultiOutputFusionTest
    : public SymbolicTileAnalysisTest {
 protected:
  DebugOptions GetDebugOptionsForTest() const override {
    DebugOptions debug_options =
...
TEST_F(SymbolicTileAnalysisWithSameShapeMultiOutputFusionTest,
       SameShapeMultiOutputFusionIsSupportedWhenEnabled) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       ParseAndReturnVerifiedModule(R"(
fusion {
  p0 = f32[32] parameter(0)
...
TEST_F(SymbolicTileAnalysisTest,
       DifferentShapeMultiOutputFusionIsNotSupported) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       ParseAndReturnVerifiedModule(R"(
fusion {
  p0 = f32[32] parameter(0)
  ...
  add = f32[32] add(p0, p1)
  subtract = f32[32] subtract(p0, p1)
  broadcast = f32[32,2]{1,0} broadcast(subtract), dimensions={0}
  ROOT tuple = (f32[32], f32[32,2]{1,0}) tuple(add, broadcast)
}

ENTRY main {
  p0 = f32[32] parameter(0)
  p1 = f32[32] parameter(1)
  ROOT fusion = (f32[32], f32[32,2]{1,0}) fusion(p0, p1), kind=kLoop, calls=fusion
})"));
  EXPECT_FALSE(TryAnalyzeModule(module.get()).has_value());
}
