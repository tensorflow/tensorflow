...
// Returns true if the reduction instruction has partially or sequentially
// tiled dimensions, indicating it requires a tiled loop.
...
OperandsSpec GetSpec(const TiledHloInstruction& tiled_hlo,
  ...
  OperandsSpec spec;
  if (opcode == HloOpcode::kReduce && IsTiledLoop(tiled_hlo, tiling_space)) {
