...
// Emits loop-tiled reduction across one or more sequential dimensions.
// Builds the nested loop structure over tiles and applies a final multi-axis
// stablehlo.reduce on the accumulated tile.
absl::StatusOr<TensorValue> EmitLoopReduce(
    EmitterContext& emitter_ctx, const ge::TiledHloInstruction& tiled_hlo) {
  TF_RET_CHECK(!tiled_hlo.hlo_regions().empty())
...
absl::StatusOr<TensorValue> EmitReduce(
  ...
  return EmitLoopReduce(emitter_ctx, tiled_hlo);
}
