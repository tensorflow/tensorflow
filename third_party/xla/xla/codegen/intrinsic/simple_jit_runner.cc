...
/**
 * @brief Creates a new LLVM function that wraps an existing function by
 ...
 *
 * @param module The LLVM module to which the new function will be added.
 * @param original_func The function to be wrapped.
 * @param unroll_factor The number of times (K) to call the original_func.
 * @param vector_size The size of the vectors being processed.
 * @return A pointer to the newly created unrolled wrapper function.
 */
  ...
  for (int k = 0; k < unroll_factor; ++k) {
    ...
    if (!call_args.empty()) {
      llvm::Type* scalar_type =
          llvm::cast<llvm::VectorType>(call_args[0]->getType())
              ->getElementType();

      if (scalar_type->isFloatingPointTy()) {
        // Create constants for the calculation
        llvm::Value* k_fp =
    ...
    // Emit the call instruction. `last_result` will hold the llvm::Value*
