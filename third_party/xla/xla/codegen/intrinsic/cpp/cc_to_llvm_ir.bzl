...
def _cc_ir_header_impl(ctx):
    ...
    if not objs:
        ...
        if objs:
            print("WARNING: Falling back to .objects instead of .pic_objects for {}".format(ctx.label.name))
            print("Found objects: {}".format([o.path for o in objs]))
    if not objs:
        fail("No objects found in compilation_outputs. Compilation outputs: {}".format(compilation_outputs))
