...
absl::StatusOr<llvm::Function*> EmitFxxToF8E(llvm::Module* module,
  ...
  const int f8_bias = primitive_util::ExponentBias(to.element_type());
  ...
  Type int_type_helper(ix_primitive_type, from.vector_width());
  llvm::Type* ix_type = int_type_helper.to_ir_type(context);
  ...
  Value* f8_sign = i8_const(0);
  if (primitive_util::HasNegativeZero(to.element_type())) {
    Value* fx_sign = b.CreateAnd(fx_as_int, sign_mask);
    fx_sign = b.CreateLShr(fx_sign, ix_const(fx_width - 8));
    f8_sign = b.CreateTrunc(fx_sign, i8_type);
  }
  ...
  const uint64_t max_finite_f8_exp = (1ULL << f8_exp_bits) - 2;
  ...
  if (primitive_util::HasInfinity(to.element_type())) {
    ...
    // For fn formats, overflow becomes NaN (e.g., 0x7F for E4M3FN).
    const uint64_t f8_nan_pattern =
        (1ULL << (f8_exp_bits + f8_mantissa_bits)) - 1;
    f8_bits = b.CreateSelect(is_overflow, i8_const(f8_nan_pattern), f8_bits);

    if (!primitive_util::HasNaN(to.element_type())) {
      // For fnuz, NaNs are flushed to zero.
      Value* is_nan = b.CreateICmpEQ(f8_bits, i8_const(f8_nan_pattern));
      f8_bits = b.CreateSelect(is_nan, i8_const(0), f8_bits);
    }
  }

  // Handle halfway points for denormals.
  f8_bits = llvm_ir::HandleHalfwayPointsFxToF8(
      fx_type, f8_exp_bits, fx_abs_bits, f8_bits, from.vector_width(), &b);
