...
// XLA autotuner is responsible for choosing the best backends for compiling an
// XLA fusion to thunks. It relies on an executable provider to provide a set of
// candidates to choose from and on the runtime profiles to measure execution
...
//      (i.e. Triton on NVIDIA GPUs), and we can chose from various tile sizes
//      and other compiler options.
//
// Executable provider implementations are responsible for compiling (or
// simply lowering to library calls) fusion to a sequence of thunks, and
// returning them to the autotuner.
//
// Autotuner chooses the best candidate, and uses executable config to
...
// select tha best strategy and emit the most efficient thunks.
class Autotuner {
 public:
  ...
  class ExecutableProvider {
   public:
    ...
    // An enum to limit the search space for the autotuner, by limiting the
    // number of candidates to choose from.
    enum Mode { kAll, kMostPromising };
    ...
    // given fusion to a thunk sequence.
    virtual absl::StatusOr<std::vector<ExecutableConfig>>
    SupportedExecutableConfigs(const HloFusionInstruction* fusion,
                               Mode mode) = 0;

    // Compiles given fusion with a given executable config.
    virtual absl::StatusOr<Executable> Compile(
        const HloFusionInstruction* fusion, const ExecutableConfig& config) = 0;
  };

  // Runs autotuner result and measures execution time + whatever metric is
  // important
  virtual absl::StatusOr<ExecutionProfile> Run(Executable executable) = 0;
};
