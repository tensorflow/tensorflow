...
def _create_dummy_repository(repository_ctx):
    ...
    _tpl(
        ...
        "cuda:cuda_config.h",
        {
            ...
            "%{cuda_lib_version}": _DEFAULT_CUDA_VERSION,
...
def _create_local_cuda_repository(repository_ctx):
    ...
    # Copy cublas headers if not installed to CUDA_TOOLKIT_PATH.
    if not any([file.endswith("cublas_v2.h") for file in included_files]):
        cublas_header_dir = "/usr/include"
        copy_rules.append(make_copy_files_rule(
            repository_ctx,
            name = "cublas-include",
            srcs = [cublas_header_dir + "/cublas.h", cublas_header_dir + "/cublas_v2.h", cublas_header_dir + "/cublas_api.h"],
            outs = ["cuda/include/cublas.h", "cuda/include/cublas_v2.h", "cuda/include/cublas_api.h"],
        ))
    else:
