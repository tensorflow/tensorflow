...
def _find_miopen_config(rocm_install_path):
    ...
    miopen_config = {
        "miopen_version_number": _get_composite_version_number(major, minor, patch)
    }
...
def _find_rocblas_config(rocm_install_path):
    def rocblas_version_numbers(path):
        ...
        if not version_file:
            raise ConfigError(
                "rocblas version file not found in {}".format(possible_version_files)
            )
    ...
    rocblas_config = {
        "rocblas_version_number": _get_composite_version_number(major, minor, patch)
    }
...
def _find_rocfft_config(rocm_install_path):
    def rocfft_version_numbers(path):
        ...
        if not os.path.exists(version_file):
            raise ConfigError('rocfft version file "{}" not found'.format(version_file))
    ...
    rocfft_config = {
        "rocfft_version_number": _get_composite_version_number(major, minor, patch)
    }
...
def _find_hipfft_config(rocm_install_path):
    def hipfft_version_numbers(path):
        ...
        if not os.path.exists(version_file):
            raise ConfigError('hipfft version file "{}" not found'.format(version_file))
    ...
    hipfft_config = {
        "hipfft_version_number": _get_composite_version_number(major, minor, patch)
    }
...
def _find_roctracer_config(rocm_install_path):
    ...
    roctracer_config = {
        "roctracer_version_number": _get_composite_version_number(major, minor, patch)
    }
...
def _find_hipsparse_config(rocm_install_path):
    ...
    hipsparse_config = {
        "hipsparse_version_number": _get_composite_version_number(major, minor, patch)
    }
...
def _find_rocsolver_config(rocm_install_path):
    ...
    rocsolver_config = {
        "rocsolver_version_number": _get_composite_version_number(major, minor, patch)
    }
