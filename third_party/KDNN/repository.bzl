"""Bazel repository rule for the KDNN v3.2.0 binary distribution."""

_KDNN_URL = "https://boostkit-sra-public.obs.cn-north-4.myhuaweicloud.com/artifact/KDNN/master/26.2.0.B163/BoostKit-boostcore-kdnn_3.2.0.zip"
_KDNN_SHA256 = "a594be9c2db84243f67cac5a6a6dda426d5c450fdbf500c4d9bd94dceec1943f"
...
def _kdnn_repository_impl(ctx):
    ...
    if kdnn_root:
        ...
        if len(rpms) != 1:
            fail("expected one KDNN RPM in v3.2.0 archive, found: %s" % rpms)
        rpm = rpms[0]
...
if command -v rpm2cpio >/dev/null 2>&1 && command -v cpio >/dev/null 2>&1; then
    ...
    # Patch the staged headers, rather than the RPM payload, so the patch only
    # covers source files that are exposed through the generated repository.
    ctx.patch(ctx.path(ctx.attr.adapter_patch), strip = 0)
...
kdnn_repository = repository_rule(
    ...
    attrs = {"adapter_patch": attr.label(default = Label("//third_party/KDNN:tensorflow_kdnn_include_adapter.patch"))},  # copybara:comment_replace attrs = {"adapter_patch": attr.label(default = Label("//third_party/tensorflow/third_party/KDNN:tensorflow_kdnn_include_adapter.patch"))},
)
