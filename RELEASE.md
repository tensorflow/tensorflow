...
* Experimental GPU support for Apple silicon Macs through a built-in Metal
  ...
  `Conv2D` and its gradients, `MaxPool`, `AvgPool`, `Relu`, `BiasAdd`,
  `Softmax`, the dense and sparse softmax cross entropies, `FusedBatchNorm`
  v1 through v3 with their gradients, `MatMul`, a broadcasting arithmetic and
  maths library with the matching gradient ops, `Transpose`, `AddN`, `Sum`,
  `Mean`, `Fill`, the random initialisers, and the SGD and Adam updates. It
  remains far short of the CUDA backend's several hundred ops; anything
  unsupported falls back to the host. Set `TF_DISABLE_METAL=1` to
