...
def choose_compiler(environ_cp):
    ...
    var = int(
        get_var(environ_cp, "TF_NEED_CLANG", None, True, question, yes_reply, no_reply)
    )
    return var
...
def retrieve_clang_version(clang_executable):
    ...
    stderr = open(os.devnull, "wb")
    curr_version = run_shell(
        [clang_executable, "--version"], allow_non_zero=True, stderr=stderr
    )
...
def set_cuda_local_path(environ_cp, dist_name, env_var):
    ...
    local_path = get_from_env_or_user_or_default(environ_cp, env_var, ask_path, None)
    if local_path:
