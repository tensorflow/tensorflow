...
struct Resampler2DFunctor<GPUDevice, T> {
  void operator()(::tensorflow::OpKernelContext* ctx, const GPUDevice& d,
    ...
    CudaLaunchKernel(Resampler2DKernel<T>, config.block_count,
                     config.thread_per_block, 0, d.stream(), data, warp, output,
...
struct ResamplerGrad2DFunctor<GPUDevice, T> {
  void operator()(::tensorflow::OpKernelContext* ctx, const GPUDevice& d,
    ...
    ::tensorflow::CudaLaunchKernel(SetZero<T>, config.block_count,
    ...
    CudaLaunchKernel(ResamplerGrad2DKernel<T>, config.block_count,
                     config.thread_per_block, 0, d.stream(), data, warp,
                     grad_output, grad_data, grad_warp, batch_size, data_height,
                     data_width, data_channels, num_sampling_points);
  }
};
