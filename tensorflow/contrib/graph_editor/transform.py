...
def keep_same_tensor_if_possible_handler(info, t):  # pylint: disable=unused-argument
...
def assign_renamed_collections_handler(info, elem, elem_):
  ...
  for name, collection in elem.graph._collections.iteritems():  # pylint: disable=protected-access
...
def transform_op_in_place(info, op, detach_outputs=False):
  ...
  if inputs != list(op.inputs):
    util.reroute_a2b_ts(inputs, op.inputs)
...
class Transformer(object):
  ...
  class _Info(object):
    ...
    def __init__(self, transformer, sgv, dst_graph, dst_scope, src_scope):
      ...
      self.control_outputs = util.create_control_outputs(self.ops)
  ...
  def __call__(self, sgv, dst_graph, dst_scope, src_scope=""):
    ...
    if sgv.graph == dst_graph and not dst_scope:
  ...
  def _transform_sgv(self, sgv):
    ...
    """
    ops_ = [op_ for _, op_ in self._info.transformed_ops.iteritems()]
