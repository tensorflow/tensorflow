...
class AccumulateNV2Test(test_util.TensorFlowTestCase):
  ...
  def testInt(self):
    ...
    tf_x = ops.convert_n_to_tensor(x)
    with self.test_session(use_gpu=True):
      self.assertAllEqual(sum(x), math_ops.accumulate_n_v2(tf_x).eval())
      self.assertAllEqual(x[0] * 6, math_ops.accumulate_n_v2([tf_x[0]] * 6).eval())
  ...
  def testGrad(self):
    ...
    for num_inputs in range(1, 10):
      with self.test_session(use_gpu=True) as sess:
        ...
        accum_n = math_ops.accumulate_n_v2(input_vars)
  ...
  def testSimple(self):
    with self.test_session():
      ...
      tf_val = math_ops.accumulate_n_v2(random_tensors)
  ...
  def testZeroArgs(self):
    with self.test_session():
      with self.assertRaises(ValueError):
        tf_val = math_ops.accumulate_n_v2([])
        tf_val.eval()
  ...
  def testWrongShape(self):
    with self.test_session():
      with self.assertRaises(ValueError):
        ...
        tf_val = math_ops.accumulate_n_v2([a,b], shape=[2,2]) # Should be shape=[]
  ...
  def testIncompatibleShapes(self):
    with self.test_session():
      with self.assertRaises(ValueError):
        ...
        tf_val = math_ops.accumulate_n_v2([a,b]) 
  ...
  def testWrongType(self):
    with self.test_session():
      with self.assertRaises(TypeError):
        ...
        tf_val = math_ops.accumulate_n_v2([a,b], tensor_dtype=np.int32) 
  ...
  def testWrongTypeOneInput(self):
    ...
    with self.test_session():
      with self.assertRaises(TypeError):
        a = variables.Variable(0.2, dtype=np.float32)
        tf_val = math_ops.accumulate_n_v2([a], tensor_dtype=np.int32) 
