...
class LayerNormBackpropOp : public OpKernel {
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES_OK(context, context->allocate_output(0, input.shape(), &in_back));
    ...
    LaunchLayerNormBackpropOp<Device, T>::launch(context, args,
                                           input_ptr,out_back_ptr,
                                            in_back_ptr
                                             );
  }
...
class LayerNormBiasAddBackpropOp : public OpKernel {
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES_OK(context, context->allocate_output(0, input.shape(), &in_back));
    OP_REQUIRES_OK(context, context->allocate_output(1, beta.shape(), &beta_back));
    ...
    LaunchLayerNormBiasAddBackpropOp<Device, T>::launch(context, args,
                                           input_ptr,out_back_ptr,
                                            in_back_ptr,beta_back_ptr
                                             );
  }
...
class LayerNormFusedBackpropOp : public OpKernel {
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES_OK(context, context->allocate_output(1, gamma.shape(), &gamma_back));
    OP_REQUIRES_OK(context, context->allocate_output(2, gamma.shape(), &beta_back));
    ...
    LaunchLayerNormFusedBackpropOp<Device, T>::launch(context, args,
                                           input_ptr,out_back_ptr,gamma_ptr,
                                            in_back_ptr,gamma_back_ptr,beta_back_ptr
                                             );
  }
...
REGISTER_KERNEL_BUILDER(
    Name("LayerNormBackpropCustom").Device(DEVICE_GPU).TypeConstraint<float>("T"),
...
REGISTER_KERNEL_BUILDER(
    Name("LayerNormBiasAddBackpropCustom").Device(DEVICE_GPU).TypeConstraint<float>("T"),
...
REGISTER_KERNEL_BUILDER(
    Name("LayerNormFusedBackpropCustom").Device(DEVICE_GPU).TypeConstraint<float>("T"),
