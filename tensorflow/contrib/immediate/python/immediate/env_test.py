...
## TODO(yaroslavvb): move tests to env_cache_test
  ...
  def atestAddCaching(self):
    ...
    val = np.ones(())
    tensor0 = env.numpy_to_itensor(val)
    tensor1 = env.numpy_to_itensor(np.zeros(()))
  ...
  def atestSplitCaching(self):
    ...
    env = immediate.Env(tf)
    value = env.tf.ones((1, 3))
    to_split_value1 = env.numpy_to_itensor([1, 2])
    to_split_value2 = env.numpy_to_itensor([1, 2, 3])
    split0, split1, split2 = env.tf.split(1, 3, value)
    self.assertAllEqual(env.tf.shape(split0).as_numpy(), [1, 1])
    split0, split1 = env.tf.split(0, 2, env.numpy_to_itensor([1, 2, 3, 4]))
