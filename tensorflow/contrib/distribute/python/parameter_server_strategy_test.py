...
class ParameterServerStrategyTestBase(
  ...
  def _test_reduce_to_cpu(self,
    ...
    with ops.Graph().as_default(), \
      ...
      def minus_one_squared(x):
        y = x - constant_op.constant(1.)
        return y * y

      # two = d.extended.broadcast_to(constant_op.constant(2.), None)
      two = constant_op.constant(2.)
      result_grouped = d.extended.call_for_each_replica(minus_one_squared,
                                                        args=(two,))
      result_mean = d.unwrap(
          d.extended.reduce_to(reduce_util.ReduceOp.MEAN,
                               result_grouped,
                               destinations="/device:CPU:0"))[0]
      result_sum = d.unwrap(
          d.extended.reduce_to(reduce_util.ReduceOp.SUM,
                               result_grouped,
                               destinations="/device:CPU:0"))[0]

      mean_val, sum_val = sess.run([result_mean, result_sum])
      self.assertEqual(mean_val, 1.0)
      self.assertEqual(sum_val, 1.0 * d.num_replicas_in_sync)
      return mean_val == 1.0 and sum_val == 1.0 * d.num_replicas_in_sync
...
class ParameterServerStrategyWithChiefTest(ParameterServerStrategyTestBase,
  ...
  @combinations.generate(
      combinations.combine(mode=['graph'],
                           num_gpus=[0, 1, 2],
                           use_core_strategy=[True, False]))
  def testReduceToCPULocal(self, num_gpus, use_core_strategy):
    self._test_reduce_to_cpu(None, 0, num_gpus, use_core_strategy)

  @combinations.generate(
      combinations.combine(mode=['graph'], use_core_strategy=[True, False]))
  def testReduceToCPUDistributed(self, use_core_strategy):
    self._run_between_graph_clients(
      self._test_reduce_to_cpu,
      self._cluster_spec,
      context.num_gpus(),
      use_core_strategy=use_core_strategy)
