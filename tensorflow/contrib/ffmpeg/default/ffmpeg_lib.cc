...
Status ReadInfoFile(const string& filename, uint32* width, uint32* height,
  ...
  // The output of the info file by ffmpeg could be in different orders, based
  // on the version of the ffmpeg used.
  // Normally, ffmpeg info file consists of the following sections:
  // Input #0, ....
  //   ....
  // Output #0, ....
  //   ....
  ...
  // video:...
  //
  // Sometimes, the `Stream mapping:` section could be also placed in between
  // `Input` and `Output` sections. The following is best effort only.
  enum State {
    STATE_OTHER = 0,
    STATE_INPUT = 1,
    STATE_OUTPUT = 2,
