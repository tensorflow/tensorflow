...
class InfeedQueue(object):
  ...
  def split_inputs_and_generate_enqueue_ops(self,
    ...
    # have 8 devices per host, and figure out what to do with model-parallelism.
    def _default_placement_function(index):
      return "/task:%d/device:CPU:0" % (index / 8)

    def _default_ordinal_function(index):
      return index % 8
    if placement_function is None:
      placement_function = _default_placement_function
    if tpu_ordinal_function is None:
      tpu_ordinal_function = _default_ordinal_function
