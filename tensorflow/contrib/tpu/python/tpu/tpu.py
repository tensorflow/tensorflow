...
def initialize_system(job=None):
...
class TPUReplicateContext(control_flow_ops.ControlFlowContext):
  ...
  * _tpu_num_replicas=k, where k is the number of replicas.
  ...
  def _AddOpInternal(self, op):
    ...
    # TODO(phawkins): nasty hack to force all ops onto TPU_REPLICATED_CORE
    # devices. This comes up, for example, when reading variables defined on
    # other devices. Consider moving this to C++.
    # TODO(phawkins): sanitize colocation constraints too.
    if "TPU_REPLICATED_CORE" not in op.node_def.device:
      op.node_def.device = "/" + core(0)
...
def replicate(computation, inputs=None,
              infeed_queue=None, name=None):
  ...
  with ops.name_scope(name, "replicate"):
    ...
    outputs = [
        tpu_ops.tpu_replicated_output(
            output_tensors[i], num_replicas, name="output{}".format(i))
...
def shard(computation, inputs=None, num_shards=1,
          input_shard_axes=None, outputs_from_all_shards=True,
          output_shard_axes=None, infeed_queue=None, name=None):
  ...
  outputs = replicate(computation, transposed_inputs,
                      infeed_queue=infeed_queue, name=name)
...
def batch_parallel(computation, inputs=None, num_shards=1, infeed_queue=None,
                   name=None):
  ...
  Raises:
    ValueError: if num_shards <= 0
  """
  return shard(computation, inputs, num_shards=num_shards,
               infeed_queue=infeed_queue, name=name)


def rewrite(computation, inputs=None, infeed_queue=None, name=None):
  ...
  # TODO(b/36647078) remove disable when pylint bug is fixed.
  # pylint: disable=indexing-exception
  return replicate(computation, None if inputs is None else [inputs],
                   infeed_queue=infeed_queue, name=name)[0]
  # pylint: enable=indexing-exception
