...
# Operations that are deprecated and won't be implemented.
_DEPRECATED_OPS = set([
    "AdjustContrast",
    "AudioSummary",
    "BatchCholesky",
...
# Operations that don't make it to the TPU but are added to the graph.
_TRANSFORMED_OPS = set([
    "Enter",
    "Exit",
    "LoopCond",
    "Merge",
...
class TPUReplicateContext(control_flow_ops.XLAControlFlowContext):
  ...
  def _AddOpInternal(self, op):
    ...
    def check_supported_type(op_name, op_type):
      # pylint: disable=protected-access
      if op_type in _TRANSFORMED_OPS:
        pass
      elif op_type in _BLACKLISTED_OPS:
        logging.error(
            "Operation of type `%s` (%s) is not supported on the TPU. "
            "Compilation will fail if this op is used in the graph. ", op_type,
            op_name)
      elif op_type in _DEPRECATED_OPS:
        raise NotImplementedError(
            "Operation of type %s (%s) is deprecated and won't be supported on"
            " TPU." % (op_type, op_name))
      elif op_type in _NOT_IMPLEMENTED_OPS:
        raise NotImplementedError(
            "Operation of type %s (%s) not supported on TPU." % (op_type,
                                                                 op_name))
      elif op.graph._is_function(op_type):
        if op_type not in self._verified_functions:
          fun_def = op.graph._get_function(op_type).definition
          for node in fun_def.node_def:
            check_supported_type(node.name, node.op)
          self._verified_functions.add(op_type)
      # pylint: enable=protected-access

    # pylint: disable=protected-access
    check_supported_type(op.name, op.type)
    if any(x.dtype._is_ref_dtype for x in op.inputs) or any(
        x.dtype._is_ref_dtype for x in op.outputs):
      raise NotImplementedError(
