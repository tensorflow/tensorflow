...
class IgniteDataset(Dataset):
  ...
  def __init__(self, cache_name, host="localhost", port=10800, local=False,\
    part=-1, distributed=False, page_size=100, username=None, password=None,\
    ...
    Args:
      ...
      distributed: Distributed flag that defines to query only partition
        current process assigned on.
    ...
    self.distributed = ops.convert_to_tensor(distributed, dtype=dtypes.bool,\
        name="distributed")
  ...
  def _as_variant_tensor(self):
    return gen_dataset_ops.ignite_dataset(self.cache_name, self.host,\
        self.port, self.local, self.part, self.distributed, self.page_size,\
        self.username, self.password, self.certfile, self.keyfile,\
        self.cert_password, self.schema, self.permutation)
