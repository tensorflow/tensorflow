...
void InitSeastarServerTag(protobuf::Message* request,
  ...
  StatusCallback done = [request, response, tag] (const Status& s) {
    ...
    if (s.ok()) {
      ...
      uint16_t err_len = std::min(UINT16_MAX,
                                  (int)(s.error_message().length()));
      memcpy(tag->resp_header_buf_.data_ + 22, &err_len, 2);
      memcpy(tag->resp_header_buf_.data_ + 32, s.error_message().c_str(), err_len);
    }
...
seastar::user_packet* SeastarServerTag::ToUserPacketWithTensor() {
  ...
  frags.emplace_back(seastar::net::fragment {resp_message_buf_.data_,
                     resp_message_buf_.len_});

  if (resp_tensor_buf_.len_ > 0) {
    frags.emplace_back(seastar::net::fragment {resp_tensor_buf_.data_,
                                               resp_tensor_buf_.len_});
  }
  up->_fragments = frags;
  up->_done = [this]() { this->SendRespDone(); };
  return up;
