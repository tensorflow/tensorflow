...
class SparseTileLikeOp : public OpKernel {
 public:
   ...
   void Compute(OpKernelContext *ctx) override {
    // define tensors  
    const Tensor *a_indices_t, *a_values_t, *a_shape_t, *b_indices_t, *b_values_t, *b_shape_t, *axes_t;
    ...
    // TODO(chaihua483) validate all the inputs shape
    // LOG(INFO) << "start";
    ...
    SparseTensor sp_b(tensor::DeepCopy(*b_indices_t), tensor::DeepCopy(*b_values_t),
                    TensorShape(b_shape_t->vec<int64>()));
    ReduceDetails reduction = SparseTensorReduceHelper(
        sp_b, axes_t->flat<int32>());
    sp_b.Reorder<T>(reduction.reorder_dims);

    SparseTensor sp_a(tensor::DeepCopy(*a_indices_t), tensor::DeepCopy(*a_values_t),
                    TensorShape(a_shape_t->vec<int64>()));
    sp_a.Reorder<T>(reduction_a);
    
        // LOG(INFO) <<  group_axes.size();
    ...
    // LOG(INFO) << str_util::Join(group_axes, ",");
    for (const auto &g : sp_b.group(group_axes)) {
        ...
        while (row < group) {
            ...
            // LOG(INFO) << str_util::Join(group, ",") << " | " << str_util::Join(row, ",");
        }
        if (row == group) {
            auto s = a_values(h);
        ...
        // LOG(INFO) << str_util::Join(group, ",") << " | " << str_util::Join(row, ",") << " | " << str_util::Join(out_values_vec, ",") << "|" << output_shape0 << "|" << g_indice_len;
    }
    ...
    OP_REQUIRES_OK(ctx, ctx->allocate_output(0, 
                                            TensorShape({output_shape0, 
                                                        b_indices_shape[1]}), &output_indices_t));
    // LOG(INFO) << str_util::Join(b_indices_shape, ",") << "|" << output_shape0;
    ...
    OP_REQUIRES_OK(ctx, ctx->allocate_output(1, output_values_shape, &out_values_t));
