...
def log_tensor_details(tensor_info):
  ...
  for key, val in tensor_info.iteritems():
...
def get_inputs_outputs(signature_def):
  ...
  inputs = [v.name for v in inputs_tensor_info.itervalues()]
  outputs = [v.name for v in outputs_tensor_info.itervalues()]
...
def convert(saved_model_dir,
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in xrange(len(in_tensors)):
