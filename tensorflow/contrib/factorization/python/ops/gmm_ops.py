...
class GmmAlgorithm(object):
  ...
  def __init__(self,
               ...
               initial_means=None,
               params='wmc',
               covariance_type=constants.FULL_COVARIANCE,
               random_seed=0):
    """Constructor.
    ...
    if self._covariance_type not in [constants.DIAG_COVARIANCE, constants.FULL_COVARIANCE]:
  ...
  def _create_variables(self):
    ...
    self._means = variables.Variable(init_value,
                                     name=constants.CLUSTERS_VARIABLE,
                                     validate_shape=False)
    self._covs = variables.Variable(
        init_value, name=constants.CLUSTERS_COVS_VARIABLE, validate_shape=False)
    ...
    self._alpha = variable_scope.variable(
        ...
        name=constants.CLUSTERS_WEIGHT,
        validate_shape=False)
  ...
  def _initialize_variables(self, data, initial_means=None):
    ...
    if self._covariance_type == constants.FULL_COVARIANCE:
    ...
    elif self._covariance_type == constants.DIAG_COVARIANCE:
  ...
  def _define_maximization_operation(self, num_batches):
    ...
    with ops.control_dependencies(self._w):
      ...
      with ops.control_dependencies([self._means_op]):
        b = math_ops.add_n(self._w_mul_x2) / (points_in_k_expanded + constants.MEPS)
        new_covs = []
        for k in range(self._num_classes):
...
def gmm(inp,
  ...
  """
  initial_means = None
  if initial_clusters != constants.RANDOM_INIT and not isinstance(initial_clusters,
                                                     ops.Tensor):
