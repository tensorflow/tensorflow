 ...
 public:
  ...
  virtual ~SequentialAvroRecordReader() {
    ...
    if (initialized_) {
      ...
      avro_value_decref(&reader_value_);  // we created another reference, so
                                          // decrease regardless of
                                          // do_resolution_
  ...
  Status ReadRecord(string* record) {
    // For any error in this method I'm not cleaning up memory because the user
    // may try again with success
    avro_writer_t mem_writer;
    size_t len;
    char* serialized;
    bool at_end =
        avro_file_reader_read_value(file_reader_, &writer_value_) != 0;
    ...
    if (serialized == nullptr) {
      return Status(errors::ResourceExhausted(
          "Unable to allocate ", len / 1024 / 1024, " MB of memory."));
    }
    ...
    if (avro_value_write(mem_writer, &reader_value_)) {
      free(serialized);
      return Status(
          errors::InvalidArgument("Unable to write value to memory."));
    }
  ...
  Status OnWorkStartup() {
    ...
    // Clear the error message, so we won't get a wrong message because not all
    // errors set a message in avro
    avro_set_error("");
    Status status;
    ...
    // We read the data from one file into memory and get a file handle to that
    // memory
    FILE* fp = fmemopen((void*)data_buffer_, file_size_,
                        "r");  // -1 to remove the trailing space from string
    if (fp == nullptr) {
      // Cleanup
      free(data_buffer_);
    ...
    if (reader_schema_str_.length() > 0) {
      ...
      // We don't need the reader schema anymore if we don't need to do
      // resolution
      if (!do_resolution_) {
        avro_schema_decref(reader_schema_);
      }
    ...
    if (do_resolution_) {
      ...
      p_writer_iface_ =
          avro_resolved_writer_new(writer_schema_, reader_schema_);
      ...
      if (avro_resolved_writer_new_value(p_writer_iface_, &writer_value_) !=
          0) {
        // Cleanup
        avro_value_iface_decref(p_writer_iface_);
        avro_value_decref(&reader_value_);
      ...
      if (p_writer_iface_ == nullptr) {
        ...
        free(data_buffer_);
        return Status(
            errors::InvalidArgument("Unable to create generic class."));
      }
 ...
 private:
  ...
  Status CreateAndLoadFileIntoBuffer(int64 read_buffer_size) {
    ...
    if (data_buffer_ == nullptr) {
      return Status(errors::ResourceExhausted(
          "Unable to allocate ", file_size_ / 1024 / 1024,
    ...
    while (data_buffer_size < file_size_) {
      ...
      status = input_buffer_->ReadNBytes(read_buffer_size, i_data_buffer,
                                         &bytes_read);
    ...
    if (data_buffer_size != file_size_) {
      free(data_buffer_);
      return Status(errors::InvalidArgument(
          "File ", filename_, " size is ", file_size_,
          " B but finished reading ", data_buffer_size, " B."));
    }
    return Status::OK();
  }
  ...
  // TODO(fraudies): Use unique ptr with custom deleter to simplify cleanup in
  // error cases
  avro_file_reader_t file_reader_;  // Avro file reader
  avro_schema_t reader_schema_;  // Schema to read, set only when doing schema
                                 // resolution
