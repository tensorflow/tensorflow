 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    functor::LSTMCellBlockFprop<Device, T, USE_CUBLAS>(
        ...
        w_tensor->matrix<T>(), b_tensor->vec<T>(), xh_tensor.matrix<T>(),
...
class LSTMCellBlockGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES_OK(ctx, ctx->allocate_output("cs_prev_grad",
        TensorShape({batch_size, cell_size_}), &cs_prev_grad_tensor));

    Tensor* h_prev_grad_tensor = nullptr;
    OP_REQUIRES_OK(ctx, ctx->allocate_output("h_prev_grad",
        TensorShape({batch_size, cell_size_}), &h_prev_grad_tensor));
    ...
    functor::LSTMCellBlockBprop<Device, T, USE_CUBLAS>(
        ...
        cs_prev_tensor->matrix<T>(), h_prev_tensor->matrix<T>(),
        w_tensor->matrix<T>(), b_tensor->vec<T>(), i_tensor->matrix<T>(),
        ...
        xh_tensor->matrix<T>(), xh_grad_tensor.matrix<T>(),
        x_grad_tensor->matrix<T>(), do_tensor.matrix<T>(),
        dcs_tensor.matrix<T>(), dci_tensor.matrix<T>(), df_tensor.matrix<T>(),
        di_tensor.matrix<T>(), dicfo_tensor->matrix<T>(),
        cs_prev_grad_tensor->matrix<T>(), h_prev_grad_tensor->matrix<T>(),
...
#define DECLARE_GPU_SPEC(T)                                                       \
  ...
  void LSTMCellBlockBprop<GPUDevice, T, true>::operator()(                        \
      OpKernelContext* ctx, perftools::gputools::Stream* stream,                  \
      const GPUDevice& d, bool compute_dw, typename TTypes<T>::ConstMatrix x,     \
      typename TTypes<T>::ConstMatrix cs_prev,                                    \
      typename TTypes<T>::ConstMatrix h_prev,                                     \
      ...
      typename TTypes<T>::Matrix x_grad, typename TTypes<T>::Matrix do_,          \
