    ...
    for (int i = 0; i < shape[0]; i++) {      // height
      for (int j = 0; j < shape[1]; j++) {    // width
        for (int k = 0; k < shape[2]; k++) {  // input feature map
          // Only k-th input feature map affects the k-th output feature map.
          // This is because interpolation is done per-feature-map and there 
          // no exchange of information between feature maps. 
