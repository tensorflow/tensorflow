...
def train_step(sess, train_op, global_step, train_step_kwargs, metric_op=None, reset_op=None):
    ...
    if metric_op is not None:
        total_loss, accuracy, np_global_step = sess.run([train_op, metric_op, global_step],
                                                        options=trace_run_options,
                                                        run_metadata=run_metadata)
        # Get the number of epoch
        step_per_epoch = train_step_kwargs['num_batches_per_epoch']
        epoch_num = int(math.floor(np_global_step / float(step_per_epoch))) + 1

        # Check if the training on the new epoch is starting.
        if 'num_batches_per_epoch' in train_step_kwargs:
            if np_global_step % train_step_kwargs['num_batches_per_epoch'] == 0:
                print('goint to next epoch!')
                sess.run(reset_op)
    ...
    if metric_op is not None:
        if 'should_log' in train_step_kwargs:
            if sess.run(train_step_kwargs['should_log']):
                logging.info('epoch %d, global step %d: loss = %.4f,accuracy = %.4f (%.3f sec/step)', epoch_num,
                             np_global_step, total_loss, accuracy, time_elapsed)
    else:
        if 'should_log' in train_step_kwargs:
            if sess.run(train_step_kwargs['should_log']):
