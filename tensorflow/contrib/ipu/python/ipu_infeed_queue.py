...
tf.Dataset.batch, set `drop_remainder=True`.""".format(output_shape))
    ...
    with ops.device('/device:CPU:0'):
      ...
      class _IPUInfeedQueueCPUOp:
        def __init__(self, cpu_op):
          self._cpu_op = cpu_op
        @property
        def initializer(self):
          return self._cpu_op

      # Dataset iterator creator - add to the global variables.
      cpu_initializer_op = gen_pop_datastream_ops.ipu_consume_dataset(
        input_dataset=ds_variant, id=self._id, device_ordinal=device_ordinal,
        **dataset_ops.flat_structure(self._dataset))

      ops.add_to_collection(ops.GraphKeys.GLOBAL_VARIABLES,
        _IPUInfeedQueueCPUOp(cpu_initializer_op))
...
automatically dequeued by the loop.""")
  ...
  def initializer(self):
    """Deprecated function."""
    raise ValueError("""Deprecated behaviour - the IPUInfeedQueue is now \
initialized as part of `global_variables_initializer()`.""")
