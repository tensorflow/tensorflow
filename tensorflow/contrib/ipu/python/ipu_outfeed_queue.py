...
class IPUOutfeedQueue:
    ...
    def enqueue(self, tensors):
      ...
      self._structure = _OutfeedStructure(tensors)
    ...
    def dequeue(self):
      ...
      The return value of this operation depends on the enqueued tensors and on
      the execution mode:
      1. Outfeed returning a single tensor:
        ```
        outfeed_queue = ipu_outfeed_queue.IPUOutfeedQueue()
      ...
      into the outfeed. If the Outfeed mode is outfeed_all == True, then the
      shape of the resulting `outfed` tensor will be [20, 4, 4], where the
      first dimension represents the number of times we have enqueued a tensor
      to the outfeed - in this example the loop is repeated 20 times, and
      therefore we get 20 values back from the outfeed. If the Outfeed mode is
      outfeed_all == False, then the shape of the resulting `outfed` tensor will
      be [4, 4], which represents the value of the output tensor the last time
      it was enqueued during execution.
      ...
      with shapes: {"x": [40, 4, 4], "y": [40, 1]}, where the first
      ...
      for each of the tensors in the tuple. If the Outfeed mode is
      outfeed_all == False, then the `outfed` is a dictionary of tensors
      with shapes: {"x": [4, 4], "y": [1]}, which represents the values of
      ...
      outfeed_shapes = deepcopy(self._structure._flat_structure["output_shapes"])
      if self._outfeed_all and self._structure._flat_structure["output_shapes"][0] is not None:
          outfeed_shapes.insert(0, None)
      with ops.device('cpu'):
        outfeed_dequeue = \
          gen_pop_datastream_ops.pop_datastream_outfeed_dequeue(
            output_types=self._structure._flat_structure["output_types"],
...
class _OutfeedStructure:
  ...
  def __init__(self, tensors):
    ...
    if isinstance(tensors, ops.Tensor):
      self._singular = True
      self._flat_types = [tensors.dtype]
      self._flat_shapes = [tensors.get_shape()]
...
Expected all values in the outfeed tuple to be TensorFlow tensors.""")
      for tensor in tensors:
        self._flat_types.append(tensor.dtype)
        self._flat_shapes.append(tensor.get_shape())
...
* Dictionary of strings to TensorFlow tensors""")
  ...
  def from_tensor_list(self, flat_tensors):
    ...
    if (not isinstance(flat_tensors, list) or
        not self._check_list_of_all_type(ops.Tensor, flat_tensors)):
        raise ValueError("""\
