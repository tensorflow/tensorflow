...
from tensorflow.contrib.opt.python.training.elastic_average_optimizer \
import ElasticAverageOptimizer, ElasticAverageCustomGetter
...
def _get_workers(num_workers, period, workers, ps_task, moving_rate):
  sessions = []
  graphs = []
  train_ops = []
  for worker_id in range(num_workers):
    ...
    with graph.as_default():
      ...
      with variable_scope.variable_scope('', custom_getter=ea_coustom):
...
class ElasticAveragingOptimizerTest(test.TestCase):
  ...
  def testPS2TasksWithClusterSpecClass(self):
    ...
    with ops.device(
      ...
      self.assertDeviceEqual("/job:worker/task:0/device:CPU:0", v.device)
      self.assertDeviceEqual("job:ps/task:0/device:CPU:0", v_g.device)
      self.assertDeviceEqual("/job:worker/task:0/device:CPU:0", w.device)
      self.assertDeviceEqual("job:ps/task:1/device:CPU:0", w_g.device)
