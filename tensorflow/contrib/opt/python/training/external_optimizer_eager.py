...
__all__ = ['ExternalOptimizerInterfaceEager', 'ScipyOptimizerInterfaceEager']


class ExternalOptimizerInterfaceEager(object):
  ...
  def __init__(self,
    ...
    """
    assert callable(loss), 'During eager mode only callable loss supported'
    for equality in self._equalities:
      assert callable(
          equality), 'During eager mode only callable equality supported'
    for inequality in self._inequalities:
      assert callable(
          inequality), 'During eager mode only callable inequality supported'
    ...
    with backprop.GradientTape() as tape:
      if var_list is not None:
        tape.watch(var_list)
      loss()
      self._vars = tape.watched_variables()
  ...
  def minimize(self,
    ...
    def loss_grad_func(x):
      with gradient.GradientTape() as tape:
        for var, packing_slice in zip(self._vars, self.packing_slices):
          var.assgin(x[packing_slice].reshape(_get_shape_tuple(var)))
        loss_value = self._loss()
        loss_callback(*fetches())
      grads = tape.gradient(loss_value, self._vars)
      return loss_value, map(self._pack, _zero_default(grads, self._vars))

    initial_packed_var_val = self._packed_var.numpy()

    def equality_func_value(equality_func, x):
        for var, packing_slice in zip(self._vars, self.packing_slices):
          var.assgin(x[packing_slice].reshape(_get_shape_tuple(var)))
        return equality_func()

    def equality_grad_func_value(equality_func, x):
      with gradient.GradientTape() as tape:
        equality_value = equality_funcs_value(equality_funcs, x)
      grads = tape.gradient(equality_value, self._vars)
      return map(self._pack, _zero_default(grads, self._vars))

    equality_funcs = [functools.partial(equality_func_value, equality_func)
                      for equality_func in self._equalities]

    equality_grad_funcs = [functools.partial(equality_grad_func_value, equality_func)
                           for equality_func in self._equalities]

    inequality_funcs = [functools.partial(equality_func_value, inequality_func)
                        for inequality_func in self._inequalities]

    inequality_grad_funcs = [functools.partial(equality_grad_func_value, inequality_func)
                             for inequality_func in self._inequalities]
    ...
    for var, packing_slice in zip(self._vars, self.packing_slices):
        var.assgin(packed_var_val[packing_slice].reshape(
...
class ScipyOptimizerInterfaceEager(ExternalOptimizerInterfaceEager):
...
def _zero_default(grads, var_list):
  return [
