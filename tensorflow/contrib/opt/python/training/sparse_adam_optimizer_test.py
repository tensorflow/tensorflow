...
from tensorflow.contrib.opt.python.training import sparse_adam_optimizer
...
# This test file can only test basic case (skipped step = 1) of SparseAdam.
# When skipped step is 1, the formula of SparseAdam is the same as Adam.
def adam_update_numpy(param,
                      ...
                      beta2=0.999,
                      epsilon=1e-8):
  alpha_t = (alpha * math_ops.sqrt(1 - math_ops.exp(t * math_ops.log(beta2))) /
             (1 - math_ops.exp(math_ops.log(beta1) * t)))
  param_t = param - alpha_t * m / (np.sqrt(v) + epsilon)
  ...
  def testSparse(self, use_resource):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      ...
      with self.cached_session():
        ...
        self.assertAllClose([1.0, 2.0], var0.eval(), rtol=1e-3, atol=1e-3)
        self.assertAllClose([3.0, 4.0], var1.eval(), rtol=1e-3, atol=1e-3)
        ...
        for t in range(1, 4):
          self.assertAllCloseAccordingToType(
              0.9**t, beta1_power.eval(), rtol=1e-3, atol=1e-3)
          self.assertAllCloseAccordingToType(
              0.999**t, beta2_power.eval(), rtol=1e-3, atol=1e-3)
          update.run()
          ...
          self.assertAllCloseAccordingToType(
              var0_np, var0.eval(), rtol=1e-3, atol=1e-3)
          self.assertAllCloseAccordingToType(
              var1_np, var1.eval(), rtol=1e-3, atol=1e-3)
    ...
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with self.session(graph=ops.Graph()):
        ...
        for t in range(1, 4):
          ...
          self.assertAllCloseAccordingToType(
              0.9**(t + 1), self.evaluate(beta1_power), rtol=1e-3, atol=1e-3)
          self.assertAllCloseAccordingToType(
              0.999**(t + 1), self.evaluate(beta2_power), rtol=1e-3, atol=1e-3)
          ...
          # Validate updated params
          self.assertAllCloseAccordingToType(
              var0_np, self.evaluate(var0), rtol=1e-3, atol=1e-3)
          self.assertAllCloseAccordingToType(
              var1_np, self.evaluate(var1), rtol=1e-3, atol=1e-3)
          if use_resource:
