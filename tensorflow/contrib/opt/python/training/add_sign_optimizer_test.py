...
from tensorflow.contrib.opt.python.training import add_sign_optimizer
...
def np_power_sign_update(var, grad, m, alpha=1.0, decay=0.9, lr=1e-4):
  m_t = (grad - m) * decay + m
...
class AddSignOptimizerTest(test.TestCase):
  def doTestBasic(self, use_resource=False):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      with self.test_session():
        ...
        opt = add_sign_optimizer.AddSignOptimizer()
