  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    def _Update_global_variables():
      ...
      for lvar, var in zip(local_center_vars, global_center_vars):
        local_center_vars_update.append(state_ops.assign(lvar, var))

      update_ops = []
      with ops.control_dependencies(local_center_vars_update):
        ed = [math_ops.subtract(v, lv) for v, lv in
              zip(local_vars, local_center_vars)]
        for lvar, var, diff in zip(local_vars, global_center_vars, ed):
