...
def crf_log_likelihood(inputs, targets, transition_params, sequence_lengths=None):
    ...
    sequence_scores = crf_sequence_score(inputs, targets, transition_params, sequence_lengths)
...
def crf_sequence_score(inputs, targets, transition_params, sequence_lengths):
    ...
    Args:
        inputs: a [batch_size, seq_len, num_tags] tensor of unary potentials
            to use as input to CRF layer
        targets: a [batch_size, seq_len] matrix of tag indices for which we compute the
            unnormalized score.
        transition_params: a [num_tags + 1, num_tags + 1] transition matrix.
        sequence_lengths: a [batch_size] vector of sequence lengths.
    """
...
def crf_log_norm(inputs, transition_params, sequence_lengths):
    ...
    Args:
        ...
        transition_params: a [num_tags + 1, num_tags + 1] matrix of transition matrix.
        sequence_lengths: a [batch_size] vector of sequence lengths.
    """
...
def crf_unary_scores(targets, sequence_lengths, inputs):
    ...
    Args:
        ...
        masks: a [batch_size, seq_len] matrix of masks representing the first length
    ...
    offset = math_ops.range(batch_size * seq_len) * num_tags + gen_array_ops.reshape(targets, [-1])

    # simply gather the unary scores of the flattened data and the offset.
    unary_scores = gen_array_ops.reshape(gen_array_ops.gather(flat_input, offset), [batch_size, seq_len])

    if sequence_lengths is None:
        unary_scores = math_ops.reduce_sum(unary_scores, 1)
    else:
        masks = array_ops.sequence_mask(sequence_lengths, seq_len, dtype=dtypes.float32)
...
def crf_binary_scores(targets, sequence_lengths, transition_params):
    ...
    binary_scores = gen_array_ops.gather(gen_array_ops.reshape(transition_params, [-1]), flat_transition_indices)
    ...
    # last_trans_inds = tf.pack([last_state_ids + 1, tf.fill([batch_size], num_tags-1)], 1)
...
def viterbi_decode(inputs, transition_params, sequence_lengths, name=None):
    ...
    Args:
        ...
        sequence_lengths: a [batch_size] vector of sequence lengths.
    """
    batch_size = gen_array_ops.shape(inputs)[0]
    seq_len, num_tags = inputs.get_shape()[1].value, inputs.get_shape()[2].value
    ...
    # reverse sequence, sequence_lengths should minus 1, since the backtracks always one-step behind
    backtracks = gen_array_ops.reverse_sequence(backtracks, sequence_lengths-1, seq_dim=1)
...
class ViterbiBacktrackRnnCell(rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        """
        state = array_ops.squeeze(dtypes.to_int32(state), [1])
