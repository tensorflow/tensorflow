...
class GridRNNCell(rnn_cell.RNNCell):
  ...
  def __init__(self,
    ...
    if not output_is_tuple:
      logging.warn("%s: Using a concatenated output is slower and will soon be "
                   "deprecated.  Use output_is_tuple=True.", self)
    ...
    if cell_fn is None:
      self._cell = rnn_cell.LSTMCell(num_units=num_units)
    else:
      try:
...
def _propagate(dim_indices, conf, cell, c_prev, m_prev, new_output, new_state,
  ...
  for i in dim_indices:
    ...
    if d.non_recurrent_fn:
      linear_args = array_ops.concat(1, [cell_inputs, last_dim_output]) \
