...
usage() {
  ...
  echo "-a [build_arch] build only for specified arch (x86_64, arm64) [default=all]"
...
if [[ "${ONLY_MAKE_TENSORFLOW}" != "true" ]]; then
  ...
  tensorflow/contrib/makefile/compile_ios_protobuf.sh -a ${IOS_ARCH}

  # Compile nsync for the target iOS device architectures.
  # Don't use  export var=`something` syntax; it swallows the exit status.
  rm -rf tensorflow/contrib/makefile/downloads/nsync/builds/lipo*
  rm -rf tensorflow/contrib/makefile/downloads/nsync/builds/default.macos*
  HOST_NSYNC_LIB=`tensorflow/contrib/makefile/compile_nsync.sh`
  if [[ -z "${BUILD_ARCH}" ]]; then
    # No arch specified so build all architectures (x86_64, arm)
...
if [[ -z "${BUILD_ARCH}" ]]; then
  # No arch specified so build iOS TF libs for all architectures (x86_64, arm)
