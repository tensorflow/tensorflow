...
void OutputSparseTensor(OpKernelContext* ctx, const TensorShape& output_shape,
                        ...
                        const std::map<std::vector<int64>, set<T>>& sets) {
...
class SetOperationOp : public OpKernel {
 ...
 private:
  void ApplySetOperation(const set<T>& set1, const set<T>& set2,
                         set<T>* result) const;
...
template <typename T>
void SetOperationOp<T>::ApplySetOperation(const set<T>& set1,
                                          const set<T>& set2,
                                          set<T>* result) const {
...
void SetOperationOp<T>::ComputeDenseToDense(OpKernelContext* ctx) const {
  ...
  std::map<std::vector<int64>, set<T>> group_sets;
  int64 num_result_values = 0;
  int64 max_set_size = 0;

  set<T> set1_group_set;
  set<T> set2_group_set;
  std::vector<int64> group_indices;
