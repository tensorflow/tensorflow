...
def _validate_input(
    flattened_layouts: Sequence[layout_lib.Layout],
    ...
    for layout, elem_spec in zip(flattened_layouts, flattened_elem_spec):
        if elem_spec.shape.rank is None:
            raise ValueError(
                "Dataset element shape must have a valid rank, got spec %s." % elem_spec
            )
        ...
        if dataset_already_batched:
            ...
            if batch_dim_size is None:
                raise ValueError(
                    (
                        ...
                        "drop_remainder=True when batching the dataset."
                    )
                    % elem_spec
                )
            ...
            if elem_spec.shape.as_list()[0] != batch_dim_size:
                raise ValueError(
                    ...
                    % (elem_spec, batch_dim_size)
                )


def _shard_counts(
    layout: layout_lib.Layout, batch_dim: Optional[str] = None
) -> List[int]:
...
def _index_matrix(
    layout: layout_lib.Layout, elem_spec: tensor_spec.TensorSpec
) -> tensor.Tensor:
...
def _pack_iterator_resource_dtensor(
    ...
    d_iterator_resource = api.pack(
        iterator_resources, layout_lib.Layout.replicated(mesh=mesh.host_mesh(), rank=0)
    )
        ...
        *,
        ...
        if batch_dim:
            ...
            self._local_replica_ids = list(
                dict.fromkeys([loc[batch_dim] for loc in mesh.local_device_locations()])
            )
        ...
        num_global_devices_per_replica = (
            config.num_global_devices(mesh.device_type()) // self.num_global_replicas
        )
        self._num_local_replicas = len(self._local_replica_ids)
        self._num_local_devices_per_replica = (
            mesh.num_local_devices() // self._num_local_replicas
        )
        ...
        self._partition_offset = (
            config.client_id() % self._num_clients_per_replica
        ) * self._num_local_devices_per_replica
    ...
    def __iter__(self):
        ...
        if self._batch_dim is not None:
            if self._num_clients_per_replica > 1:
                ...
                local_dataset = self._repeat_batch(
                    local_dataset, self._num_clients_per_replica
                )
    ...
    def _partition(self, dataset):
        ...
        def slice_batch(index, batch):
            ...
            for element, shard_counts, idx_matrix in zip(
                flattened_batch, self._all_shard_counts, self._index_matrices
            ):
                indexes = math_ops.matmul(coords, idx_matrix)
                start = array_ops.reshape(indexes, (-1,))
                size = (
                    array_ops.shape_v2(element, out_type=dtypes.int32) // shard_counts
                )
