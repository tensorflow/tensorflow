...
def unpacked_to_numpy(
    unpacked: List[TensorLike], layout: layout_lib.Layout
) -> np.ndarray:
    ...
    unravelled = np.ndarray(
        [layout.num_shards(i) for i in range(layout.rank)], dtype=object
    )
...
def unpack(
    t: TensorLike, layout: layout_lib.Layout, split_fn=np.split, stack_fn=np.stack
) -> List[TensorLike]:
    ...
    sharded_tensor = _split(
        t,
        [layout.num_shards(i) for i in range(layout.rank)],
        split_fn=split_fn,
        stack_fn=stack_fn,
    )
...
def pack_numpy(
    value: np.ndarray, layout: layout_lib.Layout, make_sparse: bool = False
) -> Tensor:
