...
class DTensorBaseTest(tf_test.TestCase, parameterized.TestCase):
    ...
    def assertDTensorEqual(
        ...
        def hash_key(loc):
            ...
            # Python dict is unhashable. Creates a sorted dict and dumps as
            # json str.
            d = collections.OrderedDict(
                sorted(loc.items(), key=lambda x: x[0]))
            return json.dumps(d)
        ...
        for offset, tensor in enumerate(unpacked):
            ...
            for dim_sharding in replicated_dims:
                if mesh_loc[dim_sharding] != 0:
                    ...
                    # ignore the `tol` passed by caller and choose the default
                    # one.
