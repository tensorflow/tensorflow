...
def _call_op(op, seed, shape, dtype, key, counter, alg, minval, maxval, op_version):
    if op_version == "V1":
...
def _call_dtensor_op(
    op, seed, shape, dtype, key, counter, alg, minval, maxval, op_version, mesh
):
    if op_version == "V1":
        return op(shape=shape, seed=seed, dtype=dtype)
...
class DTensorRNGTest(test_util.DTensorBaseTest):
    ...
    def testStatelessRNGOpsWithSingleDimensionSharded(
    ...
    def testStatelessRandomGetKeyCounter(self):
        ...
        got = gen_stateless_random_ops_v2.stateless_random_get_key_counter(seed=seed)
