...
class DTensorSaveRestoreV2Test(test_util.DTensorBaseTest):
    ...
    def test_checkpoint_simple(self, shard_spec):
        ...
        dvariable_a.assign(
            numpy_util.pack_numpy(array_ops.zeros([4, 8], dtype=dtypes.float32), layout)
        )
        dvariable_b.assign(
            numpy_util.pack_numpy(array_ops.zeros([2, 4], dtype=dtypes.float32), layout)
        )
    ...
    def test_checkpoint_manager(self, shard_spec):
        ...
        dvariable_b.assign(
            numpy_util.pack_numpy(array_ops.zeros([4, 4], dtype=dtypes.float32), layout)
        )
