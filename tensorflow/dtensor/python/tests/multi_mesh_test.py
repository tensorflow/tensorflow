...
class MultiMeshTest(test_util.DTensorBaseTest):
        ...
        with api.default_mesh(self.first_mesh):
            with api.default_mesh(self.second_mesh):
                with api.default_mesh(self.first_mesh):
                    ...
                    self.assertEqual(
                        api.fetch_layout(result).mesh, self.first_mesh)
                result = func(array_ops.ones(shape=()))
                self.assertEqual(
                    api.fetch_layout(result).mesh,
                    self.second_mesh)
            result = func(array_ops.ones(shape=()))
    ...
    def testImplicitCopyToCPUMeshForStrings(self):
        ...
        # not the TPU mesh.
        self.assertEqual(
            api.fetch_layout(string_dtensor),
            replicated_layout_on_cpu)
    ...
    def testMultiMeshInSideFunctionLayoutV2(self):
        ...
        with ops.device_v2(api.device_name()):
            output = tpu_func(a)
            self.assertDTensorEqual(
                golden_result, replicated_layout_on_cpu, output)
    ...
    def testMultiMeshCPUToTPUTransfer(self):
        ...
        replicated_layout_on_cpu = Layout.replicated(
            host_multi_cpu_mesh, rank=1)
        sharded_layout_on_tpu_r1 = Layout([_MESH_DIM_X], self.second_mesh)
        replicated_layout_on_tpu_r1 = Layout.replicated(
            self.second_mesh, rank=1)
    ...
    def testMultiMeshCPUToCPUTransfer(self):
        ...
        replicated_layout_on_cpu_send = Layout.replicated(
            send_cpu_mesh, rank=1)
        replicated_layout_on_cpu_recv = Layout.replicated(
            recv_cpu_mesh, rank=1)
        ...
        def cpu_send_func(t):
            t = math_ops.cast(t, dtypes.float32)
            cpu_recv_tensor = api.copy_to_mesh(
                t, replicated_layout_on_cpu_recv)
    ...
    def testFunctionWithMultiMeshInputOutputs(self):
        ...
        replicated_layout_on_tpu_r0 = Layout.replicated(
            self.second_mesh, rank=0)
        replicated_layout_on_tpu = Layout.replicated(self.second_mesh, rank=1)
        ...
        def func(tpu_input, cpu_input):
            ...
            cpu_tensor = api.copy_to_mesh(
                tpu_input, replicated_layout_on_cpu_r0)
    ...
    def testMultiMeshHostDeviceTransfer(self, host_to_dev, sharded, dtype):
            ...
            if src_layout.is_fully_replicated():
                ...
                src_data = api.copy_to_mesh(
                    data, Layout.replicated(
                        src_layout.mesh, rank=len(data.shape))
                )
        ...
        if sharded:
            src_layout = Layout.batch_sharded(
                src_mesh, src_mesh.dim_names[0], rank=2)
            dst_layout = Layout.batch_sharded(
                dst_mesh, dst_mesh.dim_names[0], rank=2)
        else:
    ...
    def testMultiMeshWithHostMesh(self, host_to_tpu):
        ...
        # TODO(b/193443769): switch to a single copy_to_mesh when this is
        # supported.
    ...
    def testMultiMeshBackward(self):
        ...
        self.assertDTensorEqual(0.5 * 0.5 * (1 / numpy_a)
                                ** 0.5, host_layout, a_grad)
        with ops.device_v2(api.device_name()):
