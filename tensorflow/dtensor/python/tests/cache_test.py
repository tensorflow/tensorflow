...
class DTensorDeviceCacheTest(test_util.DTensorBaseTest):
    ...
    def testFunctionInputConstantFoldingCacheMiss(self):
        ...
        # Second call should also log a cache miss since second constant
        # changed.
        self.assertAllEqual(add(c0, c2), 17.0)

        # Third call should not log a cache miss since the same input as the
        # prev.
        self.assertAllEqual(add(c0, c2), 17.0)
    ...
    def testManyFunctions(self, size, same_value):
        ...
        values = [np.reshape(r[i: i + size], (4, size // 4))
                  for i in range(10)]
        c_layout = Layout.replicated(self.mesh, rank=2)
        values = [
            constant_op.constant(
                v, dtype=dtypes.float32) for v in values]
    ...
    def testManyEagerOps(self, size, same_value):
        if self.mesh.device_type() != "TPU":
            ...
            # MLIR, but run the eager op locally and broadcast to all the
            # devices.
            ...
            # TODO(b/287529295): Remove this branch after the TPU issue is
            # fixed.
            expected_device_cache = 0
        ...
        if same_value:
            ...
            if not test_util.is_tfrt_enabled():
                self.assertEqual(
                    diff["kernel_cache.size"],
                    expected_kernel_cache)
                self.assertEqual(
                    diff["device_cache.size"],
                    expected_device_cache)
        else:
