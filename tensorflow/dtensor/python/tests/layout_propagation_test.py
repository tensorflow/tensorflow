...
class LayoutPropagationV2Test(test_util.DTensorBaseTest):
    ...
    def test_layout_prop_v2_with_const_tf_function(self):
        ...
        self.assertDTensorEqual(
            golden_result, self.unsharded_unsharded_layout, dtensor_result
        )
    ...
    def test_while_microbatch(self, sharded_layout, use_split):
        ...
        random_initial_value = numpy_util.pack_numpy(
            random_initial_value, sharded_layout
        )
    ...
    def test_while_microbatch_with_reused_gradient_accumulator(self, sharded_layout):
        layouts = [
            self.unsharded_unsharded_layout,
            self.x_unsharded_layout,
        ...
        np.random.seed(0)
        random_initial_value_1 = (
            np.random.uniform(size=4 * 4).reshape([4, 4]).astype(np.float32)
        )
        random_initial_value_2 = (
            np.random.uniform(size=4 * 4).reshape([4, 4]).astype(np.float32)
        )
        random_batch = (
            np.random.uniform(size=12 * 4).reshape([12, 4]).astype(np.float32)
        )
        ...
        random_initial_value_1 = numpy_util.pack_numpy(
            random_initial_value_1, sharded_layout_0
        )
        dtensor_variable_1 = d_variable.DVariable(random_initial_value_1)

        random_initial_value_2 = numpy_util.pack_numpy(
            random_initial_value_2, sharded_layout_1
        )
        dtensor_variable_2 = d_variable.DVariable(random_initial_value_2)

        dtensor_accum, dtensor_accum_2 = update_weights(
            random_batch, dtensor_variable_1, dtensor_variable_2
        )
    ...
    def test_strided_slice_grad(self):
        ...
        random_initial_value = numpy_util.pack_numpy(
            random_initial_value, self.unsharded_layout
        )
    ...
    def test_layout_prop_v2_infinite_loop(self):
        ...
        result = func(
            api.call_with_layout(array_ops.ones, shape=(16, 16), layout=x_unsharded),
            api.call_with_layout(array_ops.ones, shape=(16, 16), layout=unsharded_x),
        )
