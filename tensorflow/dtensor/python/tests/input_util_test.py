...
class DTensorDatasetTest(test_util.DTensorBaseTest):
    def setUp(self):
        ...
        self.mesh = mesh_util.create_mesh(
            ...
            mesh_dims=[
                (MESH_DIM_BATCH, MESH_SIZE_BATCH),
    ...
    def testIterPrefetch(self, prefetch):
        ...
        for i in range(num_batches):
            ...
            with condition:
                count = min(
                    (i + prefetch_buffer_size) * multiple, num_batches * multiple
                )
...
class DTensorIteratorSpecTest(test_util.DTensorBaseTest):
    def setUp(self):
        super().setUp()
        mesh = mesh_util.create_mesh(
            devices=["CPU:%d" % i for i in range(8)], mesh_dims=[(MESH_DIM_BATCH, 8)]
        )
    ...
    def testToTensorList(self):
        ...
        d_dataset = input_util.DTensorDataset(
            dataset=dataset, global_batch_size=4, mesh=self.mesh, layouts=images_layout
        )
        d_iterator = iter(d_dataset)
