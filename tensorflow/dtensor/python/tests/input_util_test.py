...
class DTensorDatasetTest(test_util.DTensorBaseTest):
    ...
    def testRangeIteration(self, is_graph):
        ...
        with self.assertRaises(exception):
            if is_graph:
                ...
                # Recreating polymorphic function to avoid running into the
                # error.
    ...
    def testIterPrefetch(self, prefetch):
        ...
        # The number of elements that are expected to be fetched in each
        # iteration.
        ...
        for i in range(num_batches):
            ...
            with condition:
                ...
                result = condition.wait_for(
                    lambda: counter.numpy() >= count, timeout=5)
            self.assertTrue(result)

            start_idx, end_idx = i * batch_size, (i + 1) * batch_size
            self.assertDTensorEqual(
                inputs[start_idx:end_idx], inputs_layout, elem)
    ...
    def testIterWithLayouts(
        ...
        with scope:
            ...
            train_fn = polymorphic_function.function(
                train) if is_graph else train
...
class DTensorIteratorSpecTest(test_util.DTensorBaseTest):
    ...
    def testToTensorList(self):
        ...
        self.assertListEqual(tensor_list,
                             [d_iterator._iterator_resource_dtensor])
    ...
    def testFromTensorList(self):
        ...
        self.assertIs(
            value._global_element_spec,
            d_iterator._global_element_spec)
