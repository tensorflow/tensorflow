...
class DTensorMNISTTest(tf_test.TestCase):
    ...
    def test_mnist(self):
        def train():
            ...
            gw = d_api.relayout(
                gw, d_layout.Layout.replicated(
                    self.mesh, rank=2))
            w = d_api.relayout(
                w, d_layout.Layout.replicated(
                    self.mesh, rank=2))
            gb = d_api.relayout(
                gb, d_layout.Layout.replicated(
                    self.mesh, rank=1))
            b = d_api.relayout(
                b, d_layout.Layout.replicated(
                    self.mesh, rank=1))
    ...
    def test_copy_to_mesh(self):
        ...
        host_layout = d_layout.Layout(
            layout.sharding_specs, self.mesh.host_mesh())
        x = d_api.pack(
