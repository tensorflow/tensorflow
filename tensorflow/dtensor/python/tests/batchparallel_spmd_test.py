...
class DTensorBatchParallelSPMDTest(test_util.DTensorBaseTest):
    def setUp(self):
        ...
        self.batch_layout_4d = Layout.batch_sharded(self.mesh, self._mesh_dim_b, rank=4)

        # 5-d
        self.replicated_layout_5d = Layout.replicated(self.mesh, rank=5)
        self.batch_layout_5d = Layout.batch_sharded(self.mesh, self._mesh_dim_b, rank=5)
    ...
    def test3DWindowOp(self, layout_spec, padding, op):
        ...
        num_rows = (row_window_size - 1) * 5 + 1
        num_cols = (col_window_size - 1) * 7 + 1
        x_in = np.random.normal(
            0.0, 1.0, 8 * num_deps * num_rows * num_cols * 3
        ).reshape([8, num_deps, num_rows, num_cols, 3])
    ...
    @parameterized.parameters(itertools.permutations(["sharded", "replicated"], 2))
    ...
    def testMaxPool3DGradGrad(self, shard_spec, padding):
        ...
        self.assertDTensorEqual(expected_first_grad[0], layout, dtensor_first_grad[0])
        self.assertDTensorEqual(expected_second_grad[0], layout, dtensor_second_grad[0])
