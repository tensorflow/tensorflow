...
class TFDataServiceCluster:
    ...
    def __init__(
        self, test_name, num_workers, worker_ports=None, worker_addresses=None
    ):
    ...
    def _start_dispatcher(self, worker_addresses, port=0):
        ...
        logging.info(
            "Starting remote dispatcher on port %d with worker addresses: %s",
            port,
            worker_addresses,
        )
    ...
    def _start_worker(self, port=None):
        ...
        logging.info(
            "Starting remote worker on port %d with dispatcher address: %s",
            port,
...
def setup_client(
    client_id: int, test_name: str, env: Mapping[str, str], num_local_devices: int
):
...
class MultiClientDTensorDatasetTest(test_util.DTensorBaseTest):
    ...
    def write_dataset(self, dataset, num_files, num_elems):
        ...
        for file_idx in range(num_files):
            dataset_path = os.path.join(
                self.get_temp_dir(), f"dataset-{file_idx}.tfrecords"
            )
    ...
    def testMultiClientIter(self, mesh_dims, idx_sharding, images_sharding):
        ...
        dataset_paths = self.write_dataset(dataset, num_files=8, num_elems=num_elems)
        ...
        dtensor_jobs = [f"localhost:{self.server_ports[i]}" for i in range(NUM_CLIENTS)]
        ...
        for client_id in range(NUM_CLIENTS):
            ...
            args.append(
                (
                    ...
                    self.worker_ports[client_id],
                    batch_size,
                    dataset_paths,
                    mesh,
                    batch_dim,
                    (idx_layout, images_layout),
                )
            )
        ...
        test_mesh = mesh_util.create_mesh(
            mesh_dims=mesh_dims,
            devices=["CPU:%d" % i for i in range(NUM_CLIENTS * NUM_DEVICES_PER_CLIENT)],
        )
        ...
        self.assertEmpty(all_elems, "Not all batches were returned by DTensorDataset.")
