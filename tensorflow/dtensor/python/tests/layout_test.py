    ...
    def test_mesh_use_xla_spmd_for_non_tpu_mesh_raises_error(self, mesh_type):
        ...
        with self.assertRaisesRegex(ValueError,
            ...
            layout.Mesh(
                ...
                global_device_ids=device_ids,
                local_device_ids=np.ravel(device_ids).tolist(),
                local_devices=test_util.create_device_list(
                    mesh_shape, mesh_type),
                use_xla_spmd=True)
...
class LayoutTest(test_util.DTensorBaseTest, parameterized.TestCase):
    ...
    def test_empty_sharding_spec_different_from_single_unsharded(self):
        ...
        self.assertNotEqual(
            ...
            layout.Layout.from_string(
                layout_str_empty_sharding_spec).to_string(),
        )
    ...
    def test_single_device_layout(self):
        tensor_layout = layout.Layout.from_single_device_mesh(
            _SINGLE_DEVICE_MESH)
...
class RelayoutTest(test_util.DTensorBaseTest):
    ...
    def setUp(self):
        ...
        # 1D Layouts
        self.x_layout = layout.Layout.batch_sharded(
            self.mesh, _MESH_DIM_X, rank=1)
        self.y_layout = layout.Layout.batch_sharded(
            self.mesh, _MESH_DIM_Y, rank=1)
        # 2D Layouts
    ...
    @combinations.generate(
        combinations.combine(
            is_graph=[
                False, True], is_replicated=[
                False, True])
    )
    def test_relayout(self, is_graph, is_replicated):
        inp = stateless_random_ops.stateless_random_uniform(
            [4, 4], seed=[0, 1])
        if is_replicated:
    ...
    def test_wus_using_relayout(self):
        sharded_layout = layout.Layout.batch_sharded(
            self.mesh, _MESH_DIM_X, rank=2)
    ...
    def test_call_with_layout(self, size, is_graph):
        ...
        def func():
            tensor_x = api.call_with_layout(
                array_ops.zeros, layout_x, shape=[size])
            tensor_y = api.call_with_layout(
                array_ops.zeros, layout_y, shape=[size])
