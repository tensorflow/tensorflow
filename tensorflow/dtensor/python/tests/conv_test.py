...
class ConvOpTest(test_util.DTensorBaseTest):
    ...
    def testConvFollowedByEinsum(self, conv_op, input_size, kernel_size,
        ...
        # Initialize layouts to avoid "possibly used before assignment" errors
        input_layout = None
        output_layout = None
        ...
        if input_sharding == 'replicated':
            ...
            input_layout = Layout.batch_sharded(
                self.mesh, BATCH_DIM, len(input_size))
            output_layout = self.batch_sharded_2d
        else:
            # Handle unexpected input_sharding values
            raise ValueError(f"Unsupported input_sharding: {input_sharding}")
    ...
    def testConvFollowedByEinsumWithGradient(self, conv_op, input_size,
        ...
        def conv_fn(inputs, img_kernel, layer_weights):
            with backprop.GradientTape() as tape:
                ...
                output = conv_op(
                    ...
                    strides=strides,
                    padding=padding)
                output = special_math_ops.einsum(
                    einsum_eq, output, layer_weights)
        ...
        # TODO(b/208700444): layout of input grads should match layout of
        # input.
...
class SpatiallyPartitionedConvOpTest(test_util.DTensorBaseTest):
    ...
    def testConvWithGradient(self, input_shape, kernel_shape, padding,
        ...
        def conv_fn(inputs, img_kernel, padding):
            ...
            inputs_grad, kernel_grad = tape.gradient(
                output, [inputs, img_kernel])
