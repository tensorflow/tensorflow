...
class _DSaver:  # pylint: disable=protected-access
    ...
    def __init__(
        self, mesh: layout.Mesh, saveable_objects: List[saveable_object.SaveableObject]
    ):
                ...
                # normal SaveSpec.
                    ...
                    restore_specs.append(
                        (
                            ...
                            spec.dtype,
                            layout.Layout.replicated(
                                self._mesh.host_mesh(), spec.tensor.shape.rank
                            ).to_string(),
...
class DTrackableSaver(util.TrackableSaver):
    ...
    def _gather_saveables(self, object_graph_tensor=None):
        ...
        if saveables_dict is None:
            ...
            # to handle it.
            object_graph_tensor = serialized_tensors.pop(None)[
                base.OBJECT_GRAPH_PROTO_KEY
            ]
            saveables_dict = saveable_object_util.serialized_tensors_to_saveable_cache(
                serialized_tensors
            )
    ...
    def _save_cached_when_graph_building(
        self, file_prefix, object_graph_tensor, options, update_ckpt_state=False
    ):
    ...
    def __init__(self, mesh: layout.Mesh, root=None, **kwargs):
        ...
        self._saver = DTrackableSaver(
            ...
            graph_view_lib.ObjectGraphView(
                weakref.ref(saver_root), attached_dependencies=attached_dependencies
            ),
        )
