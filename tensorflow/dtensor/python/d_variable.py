...
class DSaveSpec(saveable_object.SaveSpec):
    ...
    def __init__(
        self, tensor, slice_spec, name, global_shape, layout, dtype=None, device=None
    ):
        super().__init__(
            tensor=tensor, slice_spec=slice_spec, name=name, dtype=dtype, device=device
        )
...
class _DVariableSaveable(saveable_object.SaveableObject):
    ...
    def __init__(self, dvariable, name):
        ...
        host_layout = layout_lib.Layout(
            original_layout.sharding_specs, original_layout.mesh.host_mesh()
        )
        ...
        def get_host_dtensor():
            ...
            return (
                math_ops.cast(host_dtensor, dtypes.bfloat16)
                if self.should_cast(host_dtensor)
                else host_dtensor
            )
        ...
        super(_DVariableSaveable, self).__init__(
            ...
            [
                DSaveSpec(
                    ...
                    if self.should_cast(dvariable)
                    else dvariable.dtype,
    ...
    def restore(self, restored_tensors, restored_shapes):
        ...
        return self._dvariable.assign(
            math_ops.cast(tensor, dtype=self._dvariable.dtype)
            if self._dvariable.save_as_bf16
            else tensor
        )
