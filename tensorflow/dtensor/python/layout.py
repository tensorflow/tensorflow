...
# UNSHARDED indicates a tensor dimension is not sharded over any mesh
# dimension.
UNSHARDED = 'unsharded'
MATCH = 'match'
USE_XLA_SPMD = False
...
class Mesh(_pywrap_dtensor_device.Mesh):
    ...
    def __init__(
        ...
        if not isinstance(local_device_ids, list):
            raise ValueError(
                'Variable local_device_ids must be a list of integers.')

        if not isinstance(local_devices, list):
            raise ValueError(
                'Variable local_devices must be a list of DeviceSpecs.')

        if global_devices and not isinstance(global_devices, list):
            raise ValueError(
                'Variable global_devices must be a list of DeviceSpecs.')
        ...
        device_types = set(
            [device.device_type for device in local_devices_spec])
        if not device_types:
            device_types = set(
                [device.device_type for device in global_devices_spec])
    ...
    def unravel_index(self):
        ...
        ```
        """
        idx_ranges = [range(self.dim_size(dim_name))
...
class Layout(_pywrap_dtensor_device.Layout):
    ...
    @property
    def mesh(self):
        return Mesh._from_mesh(
            mesh=super().mesh)  # pylint: disable=protected-access
    ...
    # TODO(b/242201545): Move this to C++ / find the corresponding function
    # there.
