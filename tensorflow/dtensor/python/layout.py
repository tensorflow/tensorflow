...
tf_export("experimental.dtensor.MATCH", v1=[]).export_constant(__name__, "MATCH")
...
class Mesh(_pywrap_dtensor_device.Mesh):
    ...
    def __init__(
        ...
        if any((gid - i != distance) for i, gid in enumerate(flat_global_device_ids)):
            raise ValueError(
                "global_device_ids must sequentially increase: %s" % global_device_ids
            )
        ...
        local_device_only_contains_host_cpu = (
            len(local_devices_set) == 1
            and list(local_devices_set)[0].device_type == "CPU"
        )
        if not local_device_only_contains_host_cpu and len(local_devices) != len(
            local_devices_set
        ):
            raise ValueError(
                "Duplicate devices found in mesh specification %s."
        ...
        if len(device_types) > 1:
            raise ValueError(
                "Devices containing multiple device_types : %s" % device_types
            )
        device_type = device_types.pop()
        if use_xla_spmd and device_type != "TPU":
            raise ValueError(
                "XLA SPMD is not currently not supported for %s mesh." % device_type
            )
            ...
            if sharding_specs.count(dim_sharding) > 1:
                raise ValueError(
                    (
                        ...
                        + "in a layout."
                    ).format(mesh_dim=dim_sharding, sharding_specs=sharding_specs)
                )
            ...
            if dim_sharding not in mesh:
                raise ValueError(
                    (
                        ...
                        + "valid mesh dimension or UNSHARDED."
                    ).format(dim_sharding=dim_sharding)
                )
