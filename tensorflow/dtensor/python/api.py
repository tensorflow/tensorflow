...
def call_with_layout(
    fn: Callable[..., Any], layout: Optional[layout_lib.Layout], *args, **kwargs
) -> Any:
...
def copy_to_mesh(
    ...
    source_layout: Optional[layout_lib.Layout] = None,
) -> tensor_lib.Tensor:
...
def check_layout(tensor: tensor_lib.Tensor, layout: layout_lib.Layout) -> None:
    ...
    if fetch_layout(tensor) != layout:
        raise ValueError(
            "Layout of tensor: "
            + str(fetch_layout(tensor))
            + ", did not match expected layout: "
            + str(layout)
        )
