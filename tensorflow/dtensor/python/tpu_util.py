...
def _create_device_array(shape, device_type, host_id, local_device_ids=None):
    ...
    local_device_ids = (
        [
            x + host_id * num_local_devices
            for x in range(num_local_devices)  # pytype: disable=unsupported-operands
        ]
        if not local_device_ids
        else local_device_ids
    )
...
def _create_tpu_topology(
    core_locations: List[_CoreLocation], num_tasks: int, num_devices_per_task: int
) -> topology.Topology:
...
def tpu_system_init_helper(
    task_id, num_tasks, num_devices, use_tfrt_host_runtime=True, use_megacore=False
):
    ...
    mesh = layout_lib.Mesh(
        [_MESH_DIM_X],
        *_create_device_array((num_devices,), _TPU_DEVICE_TYPE, config.client_id()),
    )
...
def _enumerate_core_locations(
    ...
    if ring_size < host_size:
        ...
        raise ValueError(
            "Rings too small for can_split_host_across_rings = False: %d" % ring_size
        )
    ...
    if ring_bounds < host_bounds:
        raise ValueError(
            "ring_bounds %s should be >= host_bounds %s" % (ring_bounds, host_bounds)
        )
    ring_sizes = [1]
...
def _build_orthogonal_rings(
    core_locations: List[_CoreLocation], ring_size: int, rotate_ring_across_rings: bool
) -> List[_CoreLocation]:
...
def create_tpu_mesh(
    ...
    global_core_locations = _enumerate_core_locations(
        topology_shape, ring_bounds, ring_axes, can_split_host_across_rings, ring_size
    )
    ...
    if _dtensor_device is None:
        raise ValueError(
            "Invalid system device, run dtensor.initialize_accelerator_system() first"
        )
