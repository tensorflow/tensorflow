...
def _print_context(
    ...
    logging.info(
        "Number of global %s devices: %d", device_type.upper(), num_global_devices
    )
...
def create_mesh(
    ...
    if np.prod(shape) != len(device_specs):
        raise ValueError(
            f"length of devices ({len(device_specs)}) must be "
...
def create_distributed_mesh(
    ...
    if not accelerator_util.is_initialized():
        raise ValueError(
            "Accelerators are uninitialized, please run "
    ...
    if device_type.upper() == "TPU":
        ...
        _print_context(
            config.num_global_devices(device_type),
            config.num_clients(),
            config.client_id(),
            device_type,
            mesh,
...
def barrier(
    ...
    ones = api.pack(
        [component] * mesh.num_local_devices(), layout.Layout(mesh.dim_names, mesh)
    )
    ...
    logging.info(
        "finished running barrier across all clients after op: %s", barrier_name
    )
