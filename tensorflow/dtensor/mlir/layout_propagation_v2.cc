...
void UpdateLayoutForSkippedOps(
  ...
  for (const mlir::Value& skipped_value : skipped_values)
    if ((!isa<mlir::OpResult>(skipped_value) ||
...
mlir::LogicalResult InsertDTensorLayoutOps(
  ...
  for (auto& merged_layout : merged_layouts) {
    ...
    if (auto type = dyn_cast<mlir::TensorType>(value_type)) {
...
class LayoutPrinter : public mlir::OpAsmPrinter {
 public:
  ...
  void printOperand(mlir::Value value, llvm::raw_ostream& os) override {
    if (auto result = dyn_cast<mlir::OpResult>(value)) {
    ...
    } else if (auto argument = dyn_cast<mlir::BlockArgument>(value)) {
...
mlir::LogicalResult InsertRelayoutForWhileLoops(
  ...
  for (mlir::TF::WhileRegionOp op : while_ops) {
    ...
    for (int i = 0; i < op.getBody().getNumArguments(); ++i) {
      ...
      if (!isa<mlir::OpResult>(yield_op->getOperand(i)) ||
      ...
      if (!isa<mlir::TensorType>(yield_op->getOperand(i).getType()))
        return yield_op->emitOpError()
               << "operand " << i << " does not have TensorType";
      mlir::TF::ShapeAttr global_shape = mlir::TF::ShapeAttr::get(
          builder.getContext(),
          cast<mlir::TensorType>(yield_op->getOperand(i).getType()));
