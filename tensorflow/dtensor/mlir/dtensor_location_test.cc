...
void CheckFileLineColLocation(mlir::Location loc, unsigned line,
                              unsigned column) {
  ASSERT_TRUE(isa<mlir::FileLineColLoc>(loc));
  auto file_line_col_loc = cast<mlir::FileLineColLoc>(loc);
...
TEST(DTensorLocationTest, HandlesEmptyLocation) {
  ...
  ASSERT_TRUE(isa<mlir::CallSiteLoc>(loc));
  auto callsite_loc = cast<mlir::CallSiteLoc>(loc);
...
TEST(DTensorLocationTest, HandlesMultipleCalls) {
  ...
  for (int i = 0; i < 4; ++i) {
    ASSERT_TRUE(isa<mlir::CallSiteLoc>(verify_loc));
    auto callsite_loc = cast<mlir::CallSiteLoc>(verify_loc);
...
TEST(DTensorLocationTest, HandlesNameLoc) {
  ...
  ASSERT_TRUE(isa<mlir::CallSiteLoc>(test_loc));
  auto callsite_loc = cast<mlir::CallSiteLoc>(test_loc);
  mlir::Location caller_loc = cast<mlir::CallSiteLoc>(test_loc).getCaller();
  ASSERT_TRUE(isa<mlir::NameLoc>(caller_loc));
  CheckFileLineColLocation(cast<mlir::NameLoc>(caller_loc).getChildLoc(), 10,
                           20);

  mlir::Location callee_loc = callsite_loc.getCallee();
  ASSERT_TRUE(isa<mlir::NameLoc>(callee_loc));
  CheckFileLineColLocation(cast<mlir::NameLoc>(callee_loc).getChildLoc(), 21,
                           0);
