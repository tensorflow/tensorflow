...
TfLiteStatus EvalBlockwise(TfLiteContext* context, TfLiteNode* node,
  ...
  for (int i = 0; i < dimension_size; i++) {
    ...
    if (scale_offset.Overflow() || value_offset.Overflow()) {
      TF_LITE_KERNEL_LOG(context, "Embedding Lookup: offset overflow.");
      return kTfLiteError;
    }
    ...
    for (size_t j = 0; j < num_blocks; ++j) {
      ...
      CheckedInt<size_t> out_off =
          CheckedInt<size_t>(i) * col_size + CheckedInt<size_t>(j) * blocksize;
...
TfLiteStatus EvalHybrid(TfLiteContext* context, TfLiteNode* node,
  ...
  auto copy_row = [&](float scaling_factor, auto output_ptr, auto value_ptr,
                      int idx, size_t out_off) {
    CheckedInt<size_t> offset = CheckedInt<size_t>(idx) * col_size.Value();
    if (offset.Overflow()) {
      TF_LITE_KERNEL_LOG(context, "Embedding Lookup: offset overflow.");
      status = kTfLiteError;
      return;
    ...
    if (value->type == kTfLiteInt4) {
      ...
      for (size_t j = 0; j < col_size.Value(); j++) {
        output_ptr[j + out_off] =
            value_ptr[offset.Value() + j] * scaling_factor;
      }
    }
  };
  ...
  for (int i = 0; i < SizeOfDimension(lookup, 0); i++) {
    ...
    if (idx >= row_size || idx < 0) {
      ...
      if (output_fp32_ptr) {
        copy_row(scaling_factor, output_fp32_ptr, value_ptr, idx,
                 out_off.Value());
      } else {
        copy_row(scaling_factor, output_fp16_ptr, value_ptr, idx,
                 out_off.Value());
      }
