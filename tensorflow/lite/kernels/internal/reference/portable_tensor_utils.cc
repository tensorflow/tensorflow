...
void PortableApplyLayerNormFloat(const int16_t* input,
  ...
  const float bias_scale = std::powf(two, -10) * layer_norm_scale;
  ...
  for (int batch = 0; batch < n_batch; ++batch) {
    ...
    for (int i = 0; i < n_input; ++i) {
      ...
      const int32_t quant_output = static_cast<int32_t>(
          std::round(weighted_normalized_value * std::powf(2, 12)));
  ...
  for (int batch = 0; batch < n_batch; ++batch) {
    for (int i = 0; i < n_input; ++i) {
      ...
      const int32_t quant_output =
          static_cast<int32_t>(float_output * std::powf(2, 15));
