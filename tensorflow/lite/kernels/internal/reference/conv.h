...
inline void SpecialConv(const ConvParams& params, const RuntimeShape& input_shape,
  ...
  TFLITE_DCHECK_LE(output_depth, std::max(input_depth, filter_depth));
  TFLITE_DCHECK_LE(filter_depth, output_depth);
  ...
  for (int batch = 0; batch < batches; ++batch) {
    for (int out_y = 0; out_y < output_height; ++out_y) {
      for (int out_x = 0; out_x < output_width; ++out_x) {
        for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
          if (out_channel >= filter_depth ) {
            if (input_depth < out_channel) break;
            float total = input_data[Offset(input_shape, batch, out_y, out_x, out_channel)];
            output_data[Offset(output_shape, batch, out_y, out_x, out_channel)] =
              ActivationFunctionWithMinMax(total,
                                           output_activation_min,
                                           output_activation_max);
          }
          else {
