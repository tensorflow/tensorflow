  ...
  for (int i = 0; i < outer_size; ++i) {
    ...
    for (size_t c = 0; c < depth; ++c) {
      ...
      int16_t sat_sym_scaled_diff =
          std::min(std::max(sym_scaled_diff, -32768), 32767);
    ...
    int16_t sat_sym_shifted_sum =
        std::min(std::max(sym_shifted_sum, -32768), 32767);
    ...
    for (size_t c = 0; c < depth; ++c) {
      ...
      int32_t result = (static_cast<int64_t>(exp_result_Q015[c]) *
                        ...
                        round) >>
                       right_shift;
      output_data[i * depth + c] = std::min(std::max(result, 0), 32767);
    }
  }
}
