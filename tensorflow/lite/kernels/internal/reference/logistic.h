...
inline void Logistic(const RuntimeShape& input_shape, const T* input_data,
  ...
  for (int i = 0; i < flat_size; i++) {
    ...
    const float clamped_val = std::max(-100.0f, std::min(100.0f, val));
    float result;
    if (clamped_val > cutoff_upper) {
      result = 1.0f;
    } else if (clamped_val < cutoff_lower) {
      result = std::exp(clamped_val);  // -100.0f underflows cleanly to 0.0f
    } else {
      result = 1.f / (1.f + std::exp(-clamped_val));
    }
