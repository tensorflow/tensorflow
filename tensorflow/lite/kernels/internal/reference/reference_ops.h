...
void BroadcastSelect5DSlow(const RuntimeShape& input_condition_shape,
  ...
  int out_dims = output_shape.DimensionsCount();

  for (int n = 0; n < extended_output_shape.Dims(0); ++n) {
    int out_idx_n = dims_data[out_dims - 4] * n;
    int cond_idx_n = desc_condition.strides[0] * n;
    int in_idx1_n = desc_x.strides[0] * n;
    int in_idx2_n = desc_y.strides[0] * n;
    for (int b = 0; b < extended_output_shape.Dims(1); ++b) {
      int out_idx_b = (out_idx_n + b) * dims_data[out_dims - 3];
      ...
      for (int y = 0; y < extended_output_shape.Dims(2); ++y) {
        int out_idx_y = (out_idx_b + y) * dims_data[out_dims - 2];
        ...
        for (int x = 0; x < extended_output_shape.Dims(3); ++x) {
          int out_idx = (out_idx_y + x) * dims_data[out_dims - 1];
          ...
          for (int c = 0; c < extended_output_shape.Dims(4); ++c) {
