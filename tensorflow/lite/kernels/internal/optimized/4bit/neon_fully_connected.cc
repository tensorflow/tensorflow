...
void NeonPackInner(const int8_t* src, uint8_t* box, int src_rows, int src_cols,
  ...
  for (int m = 0; m < src_width; ++m) {
    ...
    for (; i < (real_src_depth & (~15)); i += 16) {
      ...
      int8x8_t uvl = vget_low_s8(uv1);
      int8x8_t uvh = vget_high_s8(uv1);
      int8x8_t lvl = vget_low_s8(lv1);
      int8x8_t lvh = vget_high_s8(lv1);
      uvl = vshl_n_u8(uvl, 4);
      lvl = vshl_n_u8(lvl, 4);
      uint8x8_t uv = vorr_u8(uvl, uvh);
      uint8x8_t lv = vorr_u8(lvl, lvh);
    ...
    for (; i < (real_src_depth & (~7)); i += 8) {
      ...
      uint8x8_t uvl = vshl_n_u8(uv1, 4);
      uint8x8_t lvl = vshl_n_u8(lv1, 4);
