...
inline void PadImpl(const tflite::PadParams& op_params,
                    ...
                    T* output_data) {
  gemmlowp::ScopedProfilingLabel label("Pad5DSlowImpl");
  ...
  // Pad kernels are limited to max 5 dimensions. Copy inputs so we can pad them
  // to 5 dims (yes, we are "padding the padding").
  std::vector<int> left_padding_copy(5, 0);
  const int left_padding_extend = 5 - op_params.left_padding_count;
  for (int i = 0; i < op_params.left_padding_count; ++i) {
    left_padding_copy[left_padding_extend + i] = op_params.left_padding[i];
  }
  std::vector<int> right_padding_copy(5, 0);
  const int right_padding_extend = 5 - op_params.right_padding_count;
  ...
  const int input_channel = ext_input_shape.Dims(4);
  const T pad_value = *pad_value_ptr;

  if (left_b_padding != 0) {
    TypedMemset<T>(
        output_data, pad_value,
        left_b_padding * output_depth * output_height * output_width * output_channel);
  }
  for (int out_b = left_b_padding; out_b < output_batch - right_b_padding;
    ...
    if (left_d_padding != 0) {
      TypedMemset<T>(output_data + Offset(ext_output_shape, out_b, 0, 0, 0, 0),
                     pad_value, left_d_padding * output_height * output_width * output_channel);
    }
    for (int out_d = left_d_padding; out_d < output_depth - right_d_padding;
         ++out_d) {
      if (left_h_padding != 0) {
        TypedMemset<T>(output_data + Offset(ext_output_shape, out_b, out_d, 0, 0, 0),
                      pad_value, left_h_padding * output_width * output_channel);
      }
      for (int out_h = left_h_padding; out_h < output_height - right_h_padding;
          ++out_h) {
        if (left_w_padding != 0) {
          TypedMemset<T>(
              output_data + Offset(ext_output_shape, out_b, out_d, out_h, 0, 0),
              pad_value, left_w_padding * output_channel);
        }
        for (int out_w = left_w_padding; out_w < output_width - right_w_padding;
            ++out_w) {
          if (left_c_padding != 0) {
            TypedMemset<T>(
                output_data + Offset(ext_output_shape, out_b, out_d, out_h, out_w, 0),
                pad_value, left_c_padding);
          }

          T* out = output_data +
                  Offset(ext_output_shape, out_b, out_d, out_h, out_w, left_c_padding);
          const T* in = input_data +
                        Offset(ext_input_shape,
                               out_b - left_b_padding, out_d - left_d_padding,
                               out_h - left_h_padding, out_w - left_w_padding, 0);
          memcpy(out, in, input_channel * sizeof(T));

          if (right_c_padding != 0) {
            TypedMemset<T>(
                output_data + Offset(ext_output_shape, out_b, out_d, out_h, out_w,
                                    output_channel - right_c_padding),
                pad_value, right_c_padding);
          }
        }
        if (right_w_padding != 0) {
          TypedMemset<T>(output_data + Offset(ext_output_shape, out_b, out_d, out_h,
                                              output_width - right_w_padding, 0),
                        pad_value, right_w_padding * output_channel);
        }
      }
      if (right_h_padding != 0) {
        TypedMemset<T>(
            output_data + Offset(ext_output_shape, out_b, out_d,
                                output_height - right_h_padding, 0, 0),
            pad_value, right_h_padding * output_width * output_channel);
      }
    }
    if (right_d_padding != 0) {
      TypedMemset<T>(
          output_data + Offset(ext_output_shape, out_b,
                               output_depth - right_d_padding, 0, 0, 0),
          pad_value, right_d_padding * output_height * output_width * output_channel);
    }
  }
  if (right_b_padding != 0) {
    TypedMemset<T>(
        output_data +
            Offset(ext_output_shape, output_batch - right_b_padding, 0, 0, 0, 0),
        pad_value,
        right_b_padding * output_depth * output_height * output_width * output_channel);
  }
}
