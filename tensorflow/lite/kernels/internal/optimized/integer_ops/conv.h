...
inline void ConvPerChannel(
  ...
  TFLITE_DCHECK_LE(gemm_input_cols_size,
  ...
  size_t output_cols_size = 0;
  TFLITE_DCHECK(output_shape.CheckedFlatSizeSkipDim(3, output_cols_size));
  TFLITE_DCHECK_LE(output_cols_size,
                   static_cast<size_t>(std::numeric_limits<int>::max()));
  const int output_cols = static_cast<int>(output_cols_size);
