...
inline void ConvPerChannel(
  ...
  size_t filter_cols_size = 0;
  TFLITE_DCHECK(filter_shape.CheckedFlatSizeSkipDim(0, filter_cols_size));
  TFLITE_DCHECK_LE(filter_cols_size,
                   static_cast<size_t>(std::numeric_limits<int>::max()));
  const int filter_cols = static_cast<int>(filter_cols_size);
