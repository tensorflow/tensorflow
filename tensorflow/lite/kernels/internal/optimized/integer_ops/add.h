...
inline void AddElementwiseInt16(int size, const ArithmeticParams& params,
...
#ifdef USE_NEON
  const int16x8_t output_activation_min_vector =
      vdupq_n_s16(params.quantized_activation_min);
  const int16x8_t output_activation_max_vector =
      vdupq_n_s16(params.quantized_activation_max);
  ...
  for (; i <= size - 16; i += 16) {
    ...
    const int16x8_t s1 = vcombine_s16(vmovn_s32(s11), vmovn_s32(s12));
    const int16x8_t s2 = vcombine_s16(vmovn_s32(s21), vmovn_s32(s22));

    const int16x8_t clamped_1 =
        vmaxq_s16(output_activation_min_vector,
                  vminq_s16(output_activation_max_vector, s1));
    const int16x8_t clamped_2 =
        vmaxq_s16(output_activation_min_vector,
                  vminq_s16(output_activation_max_vector, s2));

    vst1q_s16(output_data + i, clamped_1);
    vst1q_s16(output_data + 8 + i, clamped_2);
  }
#endif  // NEON
