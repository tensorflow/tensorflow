...
namespace reshape_utils_internal {

template <typename T>
inline TfLiteStatus EnsureNoOverflow(TfLiteContext* context,
                                     const CheckedInt<T>& value,
                                     const char* value_name) {
  TF_LITE_ENSURE_MSG(context, !value.Overflow(), "%s overflowed", value_name);
  return kTfLiteOk;
}

inline TfLiteStatus CheckedMultiply(TfLiteContext* context,
                                    CheckedInt<size_t>& product, int value,
                                    const char* product_name) {
  product *= value;
  TF_LITE_ENSURE_OK(context, EnsureNoOverflow(context, product, product_name));
  return kTfLiteOk;
}
...
inline TfLiteStatus ResolveOutputShape(TfLiteContext* context,
  ...
  for (int i = 0; i < input_shape.DimensionsCount(); ++i) {
    ...
    if (value != 0) {
      TF_LITE_ENSURE_OK(context, reshape_utils_internal::CheckedMultiply(
                                     context, non_zero_num_input_elements,
                                     value, "non_zero_num_input_elements"));
    }
  }
  ...
  for (int i = 0; i < output_shape.size; ++i) {
    ...
    if (value != 0) {
      TF_LITE_ENSURE_OK(context, reshape_utils_internal::CheckedMultiply(
                                     context, non_zero_num_output_elements,
                                     value, "non_zero_num_output_elements"));
    }
    TF_LITE_ENSURE_OK(context, reshape_utils_internal::CheckedMultiply(
                                   context, num_output_elements, value,
                                   "num_output_elements"));
  }
  ...
  if (stretch_dim != -1) {
    ...
    TF_LITE_ENSURE_OK(
        context, reshape_utils_internal::CheckedMultiply(
                     context, num_output_elements,
                     output_shape.data[stretch_dim], "num_output_elements"));
  }
