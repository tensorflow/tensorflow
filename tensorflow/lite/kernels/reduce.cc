...
TfLiteStatus EvalQuantizedMean(TfLiteContext* context,
  ...
  TF_LITE_ENSURE(
      ...
      reference_ops::QuantizedMeanOrSum(
          ...
          GetTensorData<int>(normalized_dims), GetTensorData<int32_t>(temp_sum),
          /*compute_sum=*/false));
...
TfLiteStatus EvalMean(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (op_context.input->type) {
    ...
    case kTfLiteInt8: {
      TF_LITE_ENSURE_OK(
          context, EvalQuantizedMean<int8_t>(context, op_context, num_axis,
                                             data, temp_index, resolved_axis,
                                             normalized_dims, temp_sum));
    } break;
    case kTfLiteInt16: {
      TF_LITE_ENSURE_OK(
          context, EvalQuantizedMean<int16_t>(context, op_context, num_axis,
                                              data, temp_index, resolved_axis,
                                              normalized_dims, temp_sum));
    } break;
    case kTfLiteUInt8: {
      TF_LITE_ENSURE_OK(
          context, EvalQuantizedMean<uint8_t>(context, op_context, num_axis,
                                              data, temp_index, resolved_axis,
                                              normalized_dims, temp_sum));
    } break;
    default:
