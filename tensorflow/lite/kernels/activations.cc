...
#include "tensorflow/lite/kernels/internal/optimized/integer_ops/elu.h"
...
TfLiteStatus EluEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {
    ...
    case kTfLiteInt8: {
      ...
      reference_integer_ops::QElu(params, GetTensorShape(input),
    ...
    case kTfLiteUInt8: {
      ...
      params.input_left_shift = data->input_left_shift;
      params.output_zero_point = output->params.zero_point;
      optimized_integer_ops::QElu(params, GetTensorShape(input),
                          GetTensorData<uint8_t>(input), GetTensorShape(output),
                          GetTensorData<uint8_t>(output));
    } break;
    default:
