 ...
 public:
  ...
  void Setup(const int* dims, const int rank, const int64_t element_size) {
    ...
    for (int i = 0; i < rank; ++i) {
      ...
      if (AddAndCheckOverflow(interior_pad_[i], int64_t{1}, &pad_plus_1) !=
          ...
          MultiplyAndCheckOverflow(dim_minus_1, pad_plus_1, &prod) !=
              kTfLiteOk ||
          AddAndCheckOverflow(prod, int64_t{1}, &tmp1) != kTfLiteOk ||
          AddAndCheckOverflow(tmp1, edge_pad_low_[i], &tmp2) != kTfLiteOk ||
          AddAndCheckOverflow(tmp2, edge_pad_high_[i], &output_shape_[i]) !=
              kTfLiteOk) {
        overflow_ = true;
        return;
      }
    ...
    for (int i = rank - 2; i >= 0; --i) {
      if (MultiplyAndCheckOverflow(output_shape_[i + 1],
                                   output_dimension_sizes_[i + 1],
                                   &output_dimension_sizes_[i]) != kTfLiteOk) {
        overflow_ = true;
        return;
      }
    }
    ...
    if (MultiplyAndCheckOverflow(element_size, interior_pad_[rank - 1] + 1,
                                 &output_strides_[rank - 1]) != kTfLiteOk) {
      overflow_ = true;
      return;
    }
    for (int i = rank - 2; i >= 0; --i) {
      if (MultiplyAndCheckOverflow(output_dimension_sizes_[i],
                                   interior_pad_[i] + 1,
                                   &output_strides_[i]) != kTfLiteOk) {
        overflow_ = true;
        return;
      }
    }
    ...
    for (int i = 0; i < rank; ++i) {
      int64_t pad_offset;
      if (MultiplyAndCheckOverflow(std::max<int64_t>(edge_pad_low_[i], 0),
                                   output_dimension_sizes_[i],
                                   &pad_offset) != kTfLiteOk ||
          AddAndCheckOverflow(output_offset_, pad_offset, &output_offset_) !=
              kTfLiteOk) {
        overflow_ = true;
        return;
      }
    ...
    for (int i = 0; i < rank; ++i) {
      if (MultiplyAndCheckOverflow(output_size_, output_shape_[i],
                                   &output_size_) != kTfLiteOk) {
        overflow_ = true;
        return;
      }
    }
