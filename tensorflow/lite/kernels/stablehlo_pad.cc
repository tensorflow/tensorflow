  ...
  void Setup(const int* dims, const int rank, const int64_t element_size) {
    ...
    for (int i = 0; i < rank; ++i) {
      const CheckedInt<int64_t> dim =
          (CheckedInt<int64_t>(dims[i]) - 1) *
              (CheckedInt(interior_pad_[i]) + 1) +
          1 + edge_pad_low_[i] + edge_pad_high_[i];
      overflow_ |= dim.Overflow();
      output_shape_[i] = dim.Value();
    }
    if (overflow_) {
      return;
    }
    ...
    for (int i = rank - 2; i >= 0; --i) {
      const CheckedInt<int64_t> dim_size =
          CheckedInt(output_shape_[i + 1]) * output_dimension_sizes_[i + 1];
      overflow_ |= dim_size.Overflow();
      output_dimension_sizes_[i] = dim_size.Value();
    }
    ...
    // tensor (i.e. not generated by interior padding).
    {
      const CheckedInt<int64_t> stride =
          CheckedInt(element_size) * (CheckedInt(interior_pad_[rank - 1]) + 1);
      overflow_ |= stride.Overflow();
      output_strides_[rank - 1] = stride.Value();
    }
    for (int i = rank - 2; i >= 0; --i) {
      const CheckedInt<int64_t> stride =
          CheckedInt(output_dimension_sizes_[i]) *
          (CheckedInt(interior_pad_[i]) + 1);
      overflow_ |= stride.Overflow();
      output_strides_[i] = stride.Value();
    }
    if (overflow_) {
      return;
    }
    // Compute the output offset from the eventual pads.
    {
      ...
      for (int i = 0; i < rank; ++i) {
        offset += CheckedInt(std::max<int64_t>(edge_pad_low_[i], 0)) *
                  output_dimension_sizes_[i];
      }
      overflow_ |= offset.Overflow();
      if (overflow_) {
        return;
      }
      output_offset_ = offset.Value();
    }
    // Compute the final output size.
    {
      CheckedInt<int64_t> size = std::accumulate(
          output_shape_, output_shape_ + rank, CheckedInt(element_size),
          std::multiplies<>());
      overflow_ |= size.Overflow();
      if (overflow_) {
        return;
      }
      output_size_ = size.Value();
    }
    // Compute input strides.
    input_strides_[rank - 1] = element_size;
    for (int i = rank - 1; i >= 1; --i) {
      const CheckedInt<int64_t> stride =
          CheckedInt(dims[i]) * input_strides_[i];
      overflow_ |= stride.Overflow();
      input_strides_[i - 1] = stride.Value();
    }
    if (overflow_) {
      return;
    }
