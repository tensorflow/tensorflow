...
class PadData {
 public:
  ...
  // Returns true if the multiplication a * b would overflow int64_t.
  static bool MulOverflows(int64_t a, int64_t b) {
    if (a == 0 || b == 0) return false;
    if (a > 0 && b > 0) return a > std::numeric_limits<int64_t>::max() / b;
    if (a < 0 && b < 0) return a < std::numeric_limits<int64_t>::max() / b;
    if (a > 0) return b < std::numeric_limits<int64_t>::min() / a;
    return a < std::numeric_limits<int64_t>::min() / b;
  }

  // Returns true if the addition a + b would overflow int64_t.
  ...
  void Setup(const int* dims, const int rank, const int64_t element_size) {
    ...
    for (int i = 0; i < rank; ++i) {
      // output_shape = (dims[i] - 1) * (interior_pad[i] + 1) + 1 +
      //                edge_pad_low[i] + edge_pad_high[i]
      // Check each step for overflow.
      int64_t dim_minus_1 = static_cast<int64_t>(dims[i]) - 1;
      int64_t pad_plus_1 = interior_pad_[i] + 1;
      if (AddOverflows(interior_pad_[i], int64_t{1}) ||
          MulOverflows(dim_minus_1, pad_plus_1)) {
        overflow_ = true;
        return;
      }
      int64_t prod = dim_minus_1 * pad_plus_1;
      // prod + 1 + edge_pad_low + edge_pad_high
      if (AddOverflows(prod, int64_t{1}) ||
          AddOverflows(prod + 1, edge_pad_low_[i]) ||
          AddOverflows(prod + 1 + edge_pad_low_[i], edge_pad_high_[i])) {
        overflow_ = true;
        return;
      }
      output_shape_[i] = prod + 1 + edge_pad_low_[i] + edge_pad_high_[i];
      // Ensure output shape fits in int32 for TFLite tensor allocation.
      if (output_shape_[i] > std::numeric_limits<int32_t>::max()) {
        overflow_ = true;
        return;
      }
    }
    ...
    for (int i = rank - 2; i >= 0; --i) {
      if (MulOverflows(output_shape_[i + 1], output_dimension_sizes_[i + 1])) {
        overflow_ = true;
        return;
      }
    ...
    if (MulOverflows(element_size, interior_pad_[rank - 1] + 1)) {
      overflow_ = true;
      return;
    }
    output_strides_[rank - 1] = element_size * (interior_pad_[rank - 1] + 1);
    for (int i = rank - 2; i >= 0; --i) {
      if (MulOverflows(output_dimension_sizes_[i], interior_pad_[i] + 1)) {
        overflow_ = true;
        return;
      }
    ...
    for (int i = 0; i < rank; ++i) {
      ...
      if (MulOverflows(std::max<int64_t>(edge_pad_low_[i], 0),
                       output_dimension_sizes_[i]) ||
          AddOverflows(output_offset_, pad_offset)) {
        overflow_ = true;
        return;
      }
      output_offset_ += pad_offset;
    }
    // Compute the final output size with overflow check.
    output_size_ = element_size;
    for (int i = 0; i < rank; ++i) {
      if (MulOverflows(output_size_, output_shape_[i])) {
        overflow_ = true;
        return;
      }
  ...
  bool has_overflow() const { return overflow_; }
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  pad_data.Setup(input_tensor->dims->data, input_tensor->dims->size,
                 element_size);
  TF_LITE_ENSURE_MSG(
      context, !pad_data.has_overflow(),
      "StableHLO Pad: output dimensions overflow. The combination of input "
      "dimensions, interior padding, and edge padding produces output "
