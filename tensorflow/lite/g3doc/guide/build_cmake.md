...
In order to build **kernel tests**, you need to provide
`-DTFLITE_KERNEL_TEST=on` flag for the CMake configuration phase: 
```sh
cmake ../tensorflow_src/tensorflow/lite -DTFLITE_KERNEL_TEST=on
```

**Tensorflow Lite Evaluation Tools** can be built as a separate CMake target in the CMake build phase:

```sh
cmake --build . -j5 -t evaltools
```

Or, if preferred, each of the Tools can be built separately:
```sh
cmake --build . -j5 -t coco_object_detection_run_eval
cmake --build . -j5 -t imagenet_image_classification_run_eval
cmake --build . -j5 -t inference_diff_run_eval
```

For cross-compilation specifics of both unit tests and Evaluation Tools please refer to the next section.
...
Unit tests and/or evaluation tools cannot be cross-compiled without source files generated by `flatc` (Kernel tests), `protoc` (Evaluation Tools) compilers which are pre-built for the **host** architecture.
...
For a TensorFlow Lite cross-compilation leveraging the native tools, the additional parameter `-DTFLITE_HOST_TOOLS_DIR=<native_tools_dir>`, pointing to their location, needs to be provided:

```sh
cmake -DCMAKE_TOOLCHAIN_FILE=${OE_CMAKE_TOOLCHAIN_FILE} -DTFLITE_HOST_TOOLS_DIR=<native_tools_dir> ../tensorflow/lite/
```
