...
_SITE_PATH = flags.DEFINE_string(
    "site_path", "lite/api_docs/java", "Path prefix in the _toc.yaml"
)
...
SOURCE_PATH_GPU = pathlib.Path("tensorflow/lite/delegates/gpu/java/src/main/java")
SOURCE_PATH_SUPPORT = pathlib.Path("tensorflow_lite_support/java/src/java")
SOURCE_PATH_METADATA = pathlib.Path("tensorflow_lite_support/metadata/java/src/java")
...
def overlay(
    from_root: Union[str, os.PathLike[str]], to_root: Union[str, os.PathLike[str]]
) -> None:
...
def exists_maybe_nested(paths: Iterable[pathlib.Path], root: pathlib.Path) -> bool:
