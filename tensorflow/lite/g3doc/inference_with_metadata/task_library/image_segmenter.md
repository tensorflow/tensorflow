...
// Run inference
let segmentationResult = segmenter.segment(gmlImage: mlImage)
...
TFLSegmentationResult *segmentationResult =
    [segmenter segmentWithGMLImage:gmlImage error:&segmentError];

if (!segmentationResult) {
  // Handle failure. Check `segmentError` to understand the reason for failure.
}
```
