 ...
 * from the NnApiSLDriverImplFL5 instance.  If 'nullptr_no_error' is assigned to
 * 'true' then functions that are not present in the
 * support library are assigned null pointers.  If 'nullptr_no_error' is
 * assigned to 'false' then missing support library will be treated as an error
 * (enforced via a TFLITE_DCHECK call).
 */
...
#define ASSIGN_SL_FUNCTION_TO_NNAPI_IMPL(name)                                \
  if (!nullptr_no_error && (nnapi_support_library_driver->name == nullptr)) { \
    LOG(FATAL) << "nnapi_support_library_driver->" << #name << " == nullptr"; \
  }                                                                           \
