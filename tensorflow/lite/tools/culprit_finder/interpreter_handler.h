...
class InterpreterHandler {
 public:
  ...
  // Creates an InterpreterHandler from a model path.
  static std::optional<std::unique_ptr<InterpreterHandler>> Create(
      absl::string_view model_path);

  // Prepares an interpreter with the given delegate and intermediate outputs.
  std::optional<std::unique_ptr<tflite::Interpreter>> PrepareInterpreter(
      TfLiteDelegatePtr delegate, std::vector<int> intermediate_outputs = {},
