          ...
          if (input.second.number_of_bits == 8 &&
            ...
            if (utils::HasMinMax(tensor)) {
              ...
              error_reporter->Report(
                  ...
                  "subgraph %d, node: %d",
                  index_local, EnumNameBuiltinOperator(op_code), subgraph_idx,
                  op_idx);
              return kTfLiteError;
            }
      ...
      for (const std::pair<int, operator_property::TensorProperty>& input :
        ...
        if (!utils::HasMinMax(tensor) &&
          ...
          error_reporter->Report(
              ...
              tensor->name.c_str());
          if (tensor->quantization == nullptr) {
            error_reporter->Report("No quantization params for tensor %s",
                                   tensor->name.c_str());
          } else if (tensor->quantization->min.empty() ||
                     tensor->quantization->max.empty()) {
            error_reporter->Report("Empty min/max for tensor %s",
                                   tensor->name.c_str());
          }
          return kTfLiteError;
        }
