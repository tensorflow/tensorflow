...
class QuantizeExpModelTest : public QuantizeModelTest {
 protected:
  QuantizeExpModelTest() {
    input_model_ = ReadModel(internal::kExpModel);
    readonly_model_ = input_model_->GetModel();
    readonly_model_->UnPackTo(&model_);
  }
};

TEST_F(QuantizeExpModelTest, QuantizationSucceeds) {
  auto status = QuantizeModel(&builder_, &model_, TensorType_INT8,
                              TensorType_INT8, &error_reporter_);
  EXPECT_EQ(status, kTfLiteOk);
  const uint8_t* buffer = builder_.GetBufferPointer();
...
TEST_F(QuantizeExpModelTest, VerifyMixedQuantization) {
  auto status =
  ...
  const std::vector<BuiltinOperator> op_codes = {BuiltinOperator_EXP};
