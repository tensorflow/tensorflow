...
class QuantizeReluModelTest : public QuantizeModelTest {
 protected:
  QuantizeReluModelTest() {
    input_model_ = ReadModel(internal::kReluModel);
    readonly_model_ = input_model_->GetModel();
    readonly_model_->UnPackTo(&model_);
  }
};

TEST_F(QuantizeReluModelTest, QuantizationSucceeds) {
...
TEST_F(QuantizeReluModelTest, VerifyMixedQuantization) {
  auto status =
  ...
  const std::vector<BuiltinOperator> op_codes = {BuiltinOperator_RESHAPE,
