...
class QuantizeLeakyReluModelTest : public QuantizeModelTest {
 protected:
  // The kLeakyReluModel is a simple Model with one LeakyRelu Operator.
  QuantizeLeakyReluModelTest() {
    input_model_ = ReadModel(internal::kLeakyReluModel);
    readonly_model_ = input_model_->GetModel();
    readonly_model_->UnPackTo(&model_);
  }
};

TEST_F(QuantizeLeakyReluModelTest, QuantizationSucceeds) {
  auto status = QuantizeModel(&builder_, &model_, TensorType_INT8,
                              TensorType_INT8, &error_reporter_);
  EXPECT_EQ(status, kTfLiteOk);
  const uint8_t* buffer = builder_.GetBufferPointer();
...
TEST_F(QuantizeLeakyReluModelTest, VerifyMixedQuantization) {
  auto status =
      QuantizeModel(&builder_, &model_, TensorType_INT8, TensorType_INT8,
                    /*allow_float=*/true, &error_reporter_);
  ASSERT_EQ(kTfLiteOk, status);
  ...
  const std::vector<BuiltinOperator> op_codes = {BuiltinOperator_LEAKY_RELU};
