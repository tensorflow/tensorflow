...
class QuantizationDebugOptionsTest(
    ...
    def test_init_duplicate_keys_raises_ValueError(self):
        with self.assertRaises(ValueError):
            debugger.QuantizationDebugOptions(
                layer_debug_metrics={"a": _dummy_fn, "b": _dummy_fn},
                model_debug_metrics={"c": _dummy_fn, "d": _dummy_fn},
                layer_direct_compare_metrics={"a": _dummy_fn, "e": _dummy_fn},
            )
...
class QuantizationDebuggerTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def test_layer_metrics(self, quantized_io, from_converter):
        ...
        if not from_converter:
            ...
            quant_debugger = debugger.QuantizationDebugger(
                converter=_quantize_converter(
                    self.tf_model_root, self.tf_model, _calibration_gen
                ),
                debug_dataset=_calibration_gen,
                debug_options=options,
            )
        ...
        expected_metrics = (
            expected_quant_io_metrics if quantized_io else expected_float_io_metrics
        )
    ...
    @parameterized.named_parameters(
        ("empty quantization parameter", {"quantization_parameters": {}}, None),
        (
        ...
        (
            ...
            {"quantization_parameters": {"scales": [1.0], "zero_points": []}},
            None,
        ),
        (
            "correct case",
            {"quantization_parameters": {"scales": [0.5, 1.0], "zero_points": [42, 7]}},
            (0.5, 42),
        ),
    )
