...
class QuantizationDebugOptions:
    ...
    def __init__(
        self,
        layer_debug_metrics: Optional[
            Mapping[str, Callable[[np.ndarray], float]]
        ] = None,
        model_debug_metrics: Optional[
            Mapping[str, Callable[[Sequence[np.ndarray], Sequence[np.ndarray]], float]]
        ] = None,
        layer_direct_compare_metrics: Optional[
            Mapping[
                str,
                Callable[
                    [Sequence[np.ndarray], Sequence[np.ndarray], float, int], float
                ],
            ]
        ] = None,
...
class QuantizationDebugger:
    ...
    def _initialize_stats(self):
        ...
        for (
            ...
            self._quant_interpreter._get_ops_details()
    ...
    def _get_quantized_model(self, is_debug: bool) -> bytes:
        if not self.converter:
            raise ValueError(
                "No converter found, use this function with the "
    ...
    def _set_converter_options_for_calibration(
        ...
        converter._experimental_calibrate_only = (
        ...
        for tensor_data in self._data_gen():
            ...
            if self._debug_options.layer_direct_compare_metrics is not None:
                for tensor_detail in self._get_numeric_verify_tensor_details():
                    ...
                    op_detail = self._quant_interpreter._get_op_details(
                        op_idx
                    )  # pylint: disable=protected-access
                    ...
                    for (
                        ...
                        layer_statistics[tensor_name][metric_name].append(
                            metric_fn(
                                ...
                                quant_input_detail["quantization_parameters"]["scales"][
                                    0
                                ],
                                quant_input_detail["quantization_parameters"][
                                    "zero_points"
                                ][0],
                            )
                        )
    ...
    def _collect_model_statistics(self) -> Dict[str, float]:
        ...
        for tensor_data in self._data_gen():
            ...
            for (
                metric_name,
                metric_fn,
            ) in self._debug_options.model_debug_metrics.items():
    ...
    def _get_output_tensors(
        self, interpreter: _interpreter.Interpreter
    ) -> List[np.ndarray]:
    ...
    def layer_statistics_dump(self, file: IO[str]) -> None:
        ...
        for name, metrics in self.layer_statistics.items():
            ...
            data[
                "op_name"
            ] = self._quant_interpreter._get_op_details(  # pylint: disable=protected-access
                self._defining_op[data["tensor_idx"]]
            )[
                "op_name"
            ]
            details = self._quant_interpreter._get_tensor_details(
                data["tensor_idx"]
            )  # pylint: disable=protected-access
