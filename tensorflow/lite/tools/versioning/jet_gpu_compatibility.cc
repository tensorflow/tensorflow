...
// migrated into here.
absl::Status CheckJetGpuDelegateCompatibility(const OpSignature& op_sig) {
...
absl::Status CheckJetGpuDelegateCompatibility(const OperatorCode* op_code,
                                              const Operator* op,
                                              const SubGraph* subgraph,
                                              const Model* model) {
  OpSignature op_sig = GetOpSignature(op_code, op, subgraph, model);
  auto status = CheckJetGpuDelegateCompatibility(op_sig);
...
absl::Status CheckJetGpuDelegateCompatibility(
    const TfLiteContext* context, const TfLiteNode* node,
    const TfLiteRegistration* registration) {
  return CheckJetGpuDelegateCompatibility(
