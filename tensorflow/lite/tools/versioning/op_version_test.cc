...
TEST(OpVersionTest, VersioningArgMaxTest) {
  ...
  fake_op_sig = {
      .op = BuiltinOperator_ARG_MAX,
      .inputs = CreateOpSignatureTensorSpecs(kTfLiteBool),
  };
  EXPECT_EQ(GetBuiltinOperatorVersion(fake_op_sig), 3);

  SimpleVersioningTest(BuiltinOperator_ARG_MAX);
}
TEST(OpVersionTest, VersioningArgMinTest) {
  OpSignature fake_op_sig = {
      .op = BuiltinOperator_ARG_MIN,
      .inputs = CreateOpSignatureTensorSpecs(kTfLiteInt16),
  };
  EXPECT_EQ(GetBuiltinOperatorVersion(fake_op_sig), 4);
  ...
  SimpleVersioningTest(BuiltinOperator_ARG_MIN);
}
