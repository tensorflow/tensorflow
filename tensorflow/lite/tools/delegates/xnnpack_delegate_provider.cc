 ...
 public:
  XnnpackDelegateProvider() {
    ...
    default_params_.AddParam("xnnpack_slinky_disable_schedule",
                             ToolParam::Create<bool>(false));
    default_params_.AddParam("xnnpack_slinky_concrete_bounds",
                             ToolParam::Create<bool>(false));
    default_params_.AddParam("xnnpack_slinky_no_checks",
                             ToolParam::Create<bool>(false));
  }
...
std::vector<Flag> XnnpackDelegateProvider::CreateFlags(
  ...
  std::vector<Flag> flags = {
      ...
      CreateFlag<bool>("xnnpack_slinky_disable_schedule", params,
                       ...
                       "(Ignored if --use_xnnpack is false, or if XNNPACK is "
                       "built without Slinky.)"),
      CreateFlag<bool>("xnnpack_slinky_concrete_bounds", params,
                       "Use concrete bounds in Slinky. "
                       "(Ignored if --use_xnnpack is false, or if XNNPACK is "
                       "built without Slinky.)"),
      CreateFlag<bool>("xnnpack_slinky_no_checks", params,
                       "Disable debug checks in Slinky. "
                       "(Ignored if --use_xnnpack is false, or if XNNPACK is "
                       "built without Slinky.)"),
  };
  return flags;
...
void XnnpackDelegateProvider::LogParams(const ToolParams& params,
  ...
  LOG_TOOL_PARAM(params, bool, "xnnpack_slinky_concrete_bounds",
                 "xnnpack_slinky_concrete_bounds", verbose);
  LOG_TOOL_PARAM(params, bool, "xnnpack_slinky_no_checks",
                 "xnnpack_slinky_no_checks", verbose);
}
...
TfLiteDelegatePtr XnnpackDelegateProvider::CreateTfLiteDelegate(
  ...
  if (params.Get<bool>("use_xnnpack")) {
    ...
    if (params.Get<bool>("xnnpack_slinky_concrete_bounds")) {
      opts.flags |= TFLITE_XNNPACK_DELEGATE_FLAG_SLINKY_CONCRETE_BOUNDS;
    }
    if (params.Get<bool>("xnnpack_slinky_no_checks")) {
      opts.flags |= TFLITE_XNNPACK_DELEGATE_FLAG_SLINKY_NO_CHECKS;
    }
