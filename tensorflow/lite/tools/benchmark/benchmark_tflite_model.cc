...
// Sets feature values in the tensorflow::Example proto.
void SetFeatureValues(const std::vector<float>& float_list,
                      std::string_view feature_name,
                      tensorflow::Example& example) {
  tensorflow::Feature& feature =
      (*example.mutable_features()->mutable_feature())[feature_name];
  for (const float value : float_list) {
  ...
  void OnBenchmarkEnd(const BenchmarkResults& results) override {
    ...
    if (!output_proto_path.empty()) {
      ...
      for (int i = 0; i < interpreter_runner_->outputs().size(); i++) {
        ...
        if (status == kTfLiteOk) {
          SetFeatureValues(float_list, tensor.name, example);
        }
      }
