...
TfLiteStatus InferenceProfilerStage::Init(
  ...
  auto test_delegate =
      config_.specification().tflite_inference_params().delegate();
  auto test_delegate_name =
      config_.specification().tflite_inference_params().Delegate_Name(
          test_delegate);
  auto test_model_path =
      config_.specification().tflite_inference_params().model_file_path();
  auto test_num_threads =
      config_.specification().tflite_inference_params().num_threads();
  auto test_invocations =
      config_.specification().tflite_inference_params().invocations_per_run();
  LOG(INFO) << "Test interpreter has been initialized. Using model "
            << test_model_path << ", delegate " << test_delegate_name << ", "
            << test_num_threads << " threads and " << test_invocations
            << " invocations pre run.";
  ...
  auto ref_delegate =
      reference_config_.specification().tflite_inference_params().delegate();
  auto ref_delegate_name =
      reference_config_.specification().tflite_inference_params().Delegate_Name(
          ref_delegate);
  auto ref_model_path = reference_config_.specification()
                            .tflite_inference_params()
                            .model_file_path();
  auto ref_num_threads =
      reference_config_.specification().tflite_inference_params().num_threads();
  auto ref_invocations = reference_config_.specification()
                             .tflite_inference_params()
                             .invocations_per_run();
  LOG(INFO) << "Reference interpreter has been initialized. Using model "
            << ref_model_path << ", delegate " << ref_delegate_name << ", "
            << ref_num_threads << " threads and " << ref_invocations
            << " invocations pre run.";
  ...
  for (int i = 0; i < reference_model_info_->inputs.size(); i++) {
    if (reference_model_info_->inputs[i]->dims->size !=
        test_model_info_->inputs[i]->dims->size) {
      LOG(ERROR) << "Reference and test model have different input shapes.";
      return kTfLiteError;
    }
  }

  if (reference_model_info_->outputs.size() !=
      test_model_info_->outputs.size()) {
    LOG(ERROR) << "Reference and test model have different output sizes.";
    return kTfLiteError;
  }
  for (int i = 0; i < reference_model_info_->outputs.size(); i++) {
    if (reference_model_info_->outputs[i]->dims->size !=
        test_model_info_->outputs[i]->dims->size) {
  ...
  for (int i = 0; i < reference_model_info_->outputs.size(); ++i) {
    const TfLiteType model_output_type =
        reference_model_info_->outputs[i]->type;
    if (model_output_type == kTfLiteUInt8 || model_output_type == kTfLiteInt8 ||
        model_output_type == kTfLiteFloat32 ||
        model_output_type == kTfLiteInt32) {
    } else {
      LOG(ERROR)
          << "InferenceProfilerStage only supports float32/int8/uint8/int32 "
             "output types";
      return kTfLiteError;
    }
