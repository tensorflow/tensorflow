...
void ACCNAME::store(
  ...
  for(int i=0;i<n ;i++) {
    for(int j=0;j<m ;j++) {
      ...
      // int cr_dex = flipped?j:j;
      int value = Quantised_Multiplier(a_data[j+ (i*m)] , crf_mem[cr_dex],crx_mem[cr_dex]);
      ...
      // if(!flipped) out_store[j+ (i*m)] = svalue.range(7, 0);
      // else out_store[i+ (j*n)] = svalue.range(7, 0);
