...
void BlockIntoVTAv2(conv2d_driver& drv) {
  ...
  uint32_t* weight_buf = new uint32_t[drv.pK * drv.pM];
  int32_t* wt_sum = &drv.wt_sum[0];

  packBufferBlock<uint32_t, 32, int8_t, 8>(weight_buf, drv.padded_weights, 0,
                                           drv.pM, VTA_BLOCK_OUT, 0, drv.pK,
                                           VTA_BLOCK_IN);

  uint32_t* input_buf = new uint32_t[drv.pN * drv.pK];
  packBufferBlock<uint32_t, 32, int8_t, 8>(input_buf, drv.padded_input, 0,
                                           drv.pN, VTA_BATCH, 0, drv.pK,
                                           VTA_BLOCK_IN);

  uint32_t* bias_buf = new uint32_t[drv.pN * drv.pM];
  uint32_t* out_buf = new uint32_t[drv.pN * drv.pM];

  create_2d_biases(0, drv.pN, 0, drv.pM, bias_buf, drv.bias, wt_sum, drv.in_sum,
                   drv.rhs_offset, drv.lhs_offset, drv.K);

  blocked_gemm_test_tflitev2(drv, drv.scs, drv.pN, drv.pK, drv.pM, 0, true, 1,
                             input_buf, weight_buf, bias_buf, out_buf, crf_buf,
                             crx_buf, drv.save, drv.ra, drv.layer);

                             
  if(drv.layer==2){
    for(int i=0;i<16;i++) cout << bias_buf[i] << endl;
  }

  unpackBufferBlock<int8_t, 8, uint32_t, 32>(drv.padded_output, out_buf, 0,
                                             drv.pN, VTA_BATCH, 0, drv.pM,
                                             VTA_BLOCK_OUT);
}
...
void Entry(conv2d_driver& drv) {
  // cout << "VTA ACC - Layer: " << drv.layer << (drv.flipped?" Flipped":"") << endl;
  // cout << "===========================" << endl;
