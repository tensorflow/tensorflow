...
void ACCNAME::gemm(
  ...
  // Outer Loop
  EXE_OUT_LOOP: for (int it_out = 0; it_out < insn.gemm.iter_out; it_out++) {
    acc_idx_T dst_offset_in = dst_offset_out;
    inp_idx_T src_offset_in = src_offset_out;
    wgt_idx_T wgt_offset_in = wgt_offset_out;

    // Inner Loop
    EXE_IN_LOOP: for (int it_in = 0; it_in < insn.gemm.iter_in; it_in++) {

      // Iterate over micro op
      READ_GEMM_UOP: for (int upc = insn.gemm.uop_bgn; upc < insn.gemm.uop_end; upc++) { // 16
        // Read micro-op fields
      	sc_uint<32> uop = uop_mem[upc];
