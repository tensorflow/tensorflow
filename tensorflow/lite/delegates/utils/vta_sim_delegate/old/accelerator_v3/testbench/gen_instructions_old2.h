...
void blocked_gemm_test_tflite(systemC_sigs* scs, int batch, int in_channels,
                             int out_channels, int block, bool uop_compression,
                             int virtual_threads, uint32_t* inputs,
                             uint32_t* weights, uint32_t* biases,
                             uint32_t* outputs, uint32_t* crf, uint32_t* crx,
                             bool save, int ra) {
  ...
  // Derive number of elements that need to be loaded/stored
  int ins_size =
      batch / block * out_feat / block * (2 + in_feat / block * 3) + 2;
  int uop_size = uop_compression ? block / VTA_BATCH * virtual_threads
                                 : block / VTA_BATCH * block / VTA_BLOCK_IN *
                                       block / VTA_BLOCK_OUT * virtual_threads;
      ...
      for (int k = 0; k < in_feat; k += block * virtual_threads) {
        for (int l = 0; l < block * virtual_threads; l += block) {
          ...
          bool pop = (virtual_threads == 1)
                         ? 1
                         : (i > 0 || j > 0 || k > 0 || l > 0) &&
                               (k + l != block * virtual_threads - block);
          bool push_prev =
              (virtual_threads == 1)
                  ? ((k + l) != in_feat - block)
                  : ((k + l) != in_feat - virtual_threads * block) &&
                        ((k + l != in_feat - block) ||
                         (j != out_feat - block) || (i != batch - block));
          ...
          insn_buf[insn_idx++] = get2DLoadStoreInsn(
              ...
              (j / VTA_BLOCK_OUT * in_feat + k + l) /
                  VTA_BLOCK_IN,        // dram offset
          ...
          // Perform GEMM (pop prev, push prev if not last, push next if last)
          insn_buf[insn_idx++] =
              getGEMMInsn(l / block * uop_size / virtual_threads,  // uop offset
  ...
  // Prepare the uop buffer
  VTAUop* uop_buf =
      getGEMMUops(block / VTA_BATCH, block / VTA_BLOCK_IN,
                  block / VTA_BLOCK_OUT, uop_compression, virtual_threads > 1);
  ...
  // printf("=====================================================================================\n");
  printf(
      "INFO - Blocked GEMM test:isns=%d, batch=%d, in_channels=%d, "
      "out_channels=%d, block=%d, uop_comp=%d, vt=%d\n",
      insn_idx, batch, in_channels, out_channels, block, uop_compression,
      virtual_threads);
