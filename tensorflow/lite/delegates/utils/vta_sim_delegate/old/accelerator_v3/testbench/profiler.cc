...
// Metric::Metric(string _name, int value, MetricTypes _type) {
//   name = _name;
//   value = value;
//   type = _type;
// }
...
void Profile::saveProfile(vector<Metric*> captured_metrics) {
  ...
  for (int i = 0; i < captured_metrics.size(); i++) {
    if (captured_metrics[i]->type == TClockCycles) {
      ...
      DataCount temp(captured_metrics[i]->name);
      temp.value = captured_metrics[i]->value;
      captured_metrics[i]->value = 0;
      newRecord.push_back(temp);
