...
void ACCNAME::ppu(insn_T insn_raw, bool flipped,
	  bus_T *acc_mem,
	  crf_type *crf_mem,
	  crx_type *crx_mem,
	  bus_T *out_mem){
...
#pragma HLS INLINE OFF
  ...
  unsigned long long temp_insn[2] ;
	temp_insn[0] = insn_raw.range(63,0).to_uint64();
	temp_insn[1] = insn_raw.range(127,64).to_uint64();
  ...
  for(int i=0;i<n ;i++) {
    for(int j=0;j<m ;j++) {
      ...
      // if(layer==0){
      //   int8_t finval = svalue.range(7, 0);
      //   cout <<   crf_mem[j] << " " <<  (int) crx_mem[j]  
      //   << " " << a_data[j+ (i*m)] << "-->" << (int) finval << endl;
      // }
      DWAIT(2);
    }
  }

  // int rm = insn.mem.x_size;
  // int n = insn.mem.y_size;
  // for(int i=0;i<n ;i++) {
  //   for(int j=0;j<rm ;j++) {
  ...
  //     // cout << dst_idx << endl;
  //     for(int k=0;k<16 ;k++) {
  //       int acc_data = dst_tensor[0][k];
  //       int cr_dex = j*16+k;
  //       int value = Quantised_Multiplier(acc_data, crf_mem[cr_dex],crx_mem[cr_dex]);
  //       sc_int<32> svalue = value + ra;
  //       if (svalue > MAX8) svalue = MAX8;
  //       else if (svalue < MIN8)svalue = MIN8;
  //       o_tensor[0][k] = svalue.range(7, 0);
  //       // if(layer==0){
  //       //   int8_t finval = svalue.range(7, 0);
  //       //   cout <<   crf_mem[cr_dex] << " " <<  (int) crx_mem[cr_dex]  
  //       //   << " " << acc_data << "-->" << (int) finval << endl;
  //       // }
  //     }
...
void ACCNAME::compute() {
  ...
  crx_type* crx_mem = new crx_type[VTA_ACC_BUFF_DEPTH];
    ...
    }else if (insn.generic.opcode == VTA_OPCODE_PPU) {
      ppu(raw_copy, false, acc_mem, crf_mem, crx_mem, out_mem);
    }
