...
#include "axi_mm_api.h"
#include "conv2d_driver.h"
#include "mm_helper.h"
...
void printInstruction(int num_insn, VTAGenericInsn* insns) {
  ...
  for (int i = 0; i < num_insn; i++) {
    ...
    } else if (c.mem.opcode == VTA_OPCODE_PPU) {
      // Print instruction field information
      printf("PPU \n");
      printf(
          "\tdep - pop prev: %d, pop next: %d, push prev: %d, push next: %d\n",
...
void blocked_gemm_test_tflitev2(conv2d_driver& drv, int N, int K_ch, int M_ch,
                                bool uop_compression, uint32_t* crf,
                                uint32_t* crx) {
  prf_start(0);
  ...
  for (int i = 0; i < N; i += iblock) {
    ...
    for (int j = 0; j < M; j += wblock) {
      ...
      insn_buf[insn_idx++] = get2DLoadStoreInsn(
          ...
          0,                                        // pop l2g_dep_queue
          0,                                        // pop s2g_dep_queue
          0,                                        // push g2l_dep_queue
          1);                                       // push g2s_dep_queue
  ...
  prf_end(0, drv.t.vta_ins);

  prf_start(1);
  int insns_len =
      sizeof(VTAGenericInsn) * insn_idx / sizeof(unsigned long long);
  unsigned long long* insn_set = (unsigned long long*)insn_buf;
  copytoMM<unsigned long long>(drv.opc_mm, insn_set, insns_len);

  int uops_len = sizeof(VTAUop) * uop_size / sizeof(unsigned int);
  unsigned int* uops_set = reinterpret_cast<unsigned int*>(uop_buf);
  copytoMM<unsigned int>(drv.uop_mm, uops_set, uops_len);

  int crf_size = M;
  int crf_len = 4 * crf_size / sizeof(unsigned long long);
  unsigned long long* crf_set = (unsigned long long*)crf;
  copytoMM<unsigned long long>(drv.crf_mm, crf_set, crf_len);

  int crx_size = M;
  int crx_len = 1 * crx_size / sizeof(unsigned long long);
  unsigned long long* crx_set = (unsigned long long*)crx;
  copytoMM<unsigned long long>(drv.crx_mm, crx_set, crx_len);
  prf_end(1, drv.t.vta_mm);

  prf_start(2);
  writeMappedReg(drv.acc, 0x2c, insn_idx);
  writeMappedReg(drv.acc, 0x74, drv.ra);
  // writeMappedReg(drv.acc, 0x7c, false);
  writeMappedReg(drv.acc, 0x14, ++drv.vta_count);
  VLOG("========================" << endl);
  VLOG("Starting" << endl);
  VLOG("========================" << endl);

  bool done = readMappedReg(drv.acc, 0x1c) == drv.vta_count;
  while (!done) done = readMappedReg(drv.acc, 0x1c) == drv.vta_count;

  VLOG("Stopping" << endl);
  VLOG("========================" << endl);
  prf_end(2, drv.t.vta_acc);

  // printInstruction(insn_idx, insn_buf);
