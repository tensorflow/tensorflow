...
TfLiteDelegate* CreateRVtaDelegateFromOptions(char** options_keys,
                                               char** options_values,
                                               size_t num_options) {
  RVtaDelegateOptions options = TfLiteRVtaDelegateOptionsDefault();

  // Parse key-values options to RVtaDelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char*> argv;
  argv.reserve(num_options + 1);
  constexpr char kRVtaDelegateParsing[] = "rvta_delegate_parsing";
  argv.push_back(kRVtaDelegateParsing);
  ...
  TFLITE_LOG(INFO) << "RVta delegate: allowed_builtin_code set to "
                   << options.allowed_builtin_code << ".";
  TFLITE_LOG(INFO) << "RVta delegate: error_during_init set to "
                   << options.error_during_init << ".";
  TFLITE_LOG(INFO) << "RVta delegate: error_during_prepare set to "
                   << options.error_during_prepare << ".";
  TFLITE_LOG(INFO) << "RVta delegate: error_during_invoke set to "
                   << options.error_during_invoke << ".";

  return TfLiteRVtaDelegateCreate(&options);
}
...
TFL_CAPI_EXPORT TfLiteDelegate* tflite_plugin_create_delegate(
  ...
  return tflite::tools::CreateRVtaDelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate* delegate) {
  TfLiteRVtaDelegateDelete(delegate);
}
