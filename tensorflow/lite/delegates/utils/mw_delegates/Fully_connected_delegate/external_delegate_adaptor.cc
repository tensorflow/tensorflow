...
namespace tflite {
namespace tools {
    TfLiteDelegate* CreateFullyConnectedDelegateFromOptions(
        const char* const* options_keys, const char* const* options_values,
        size_t num_options) {
      FullyConnectedDelegateOptions options = TfLiteFullyConnectedDelegateOptionsDefault();

      // Parse key-values options to SASimDelegateOptions by mimicking them as
    // command-line flags.
    std::vector<const char*> argv;
    argv.reserve(num_options + 1);
    constexpr char kFullyConnectedDelegateParsing[] = "fully_connected_delegate_parsing";
    argv.push_back(kFullyConnectedDelegateParsing);
    ...
    TFLITE_LOG(INFO) << "Fully Connected Delegates: allowed_builtin_code set to "
                     << options.allowed_builtin_code << ".";
    TFLITE_LOG(INFO) << "Fully Connected Delegates: error_during_init set to "
                     << options.error_during_init << ".";
    TFLITE_LOG(INFO) << "Fully Connected Delegates: error_during_prepare set to "
                     << options.error_during_prepare << ".";
    TFLITE_LOG(INFO) << "Fully Connected Delegates: error_during_invoke set to "
                     << options.error_during_invoke << ".";

    return TfLiteFullyConnectedDelegateCreate(&options);
    }
}  // namespace tools
}  // namespace tflite
...
tflite_plugin_create_delegate(const char* const* options_keys,
  ...
  return tflite::tools::CreateFullyConnectedDelegateFromOptions(
