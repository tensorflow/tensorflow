...
TfLiteDelegate* CreateTCONVFPGADelegateFromOptions(char** options_keys,
                                               char** options_values,
                                               size_t num_options) {
  TCONVFPGADelegateOptions options = TfLiteTCONVFPGADelegateOptionsDefault();

  // Parse key-values options to TCONVFPGADelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char*> argv;
  argv.reserve(num_options + 1);
  constexpr char kTCONVFPGADelegateParsing[] = "tconv_fpga_delegate_parsing";
  argv.push_back(kTCONVFPGADelegateParsing);
  ...
  TFLITE_LOG(INFO) << "TCONVFPGA delegate: allowed_builtin_code set to "
                   << options.allowed_builtin_code << ".";
  TFLITE_LOG(INFO) << "TCONVFPGA delegate: error_during_init set to "
                   << options.error_during_init << ".";
  TFLITE_LOG(INFO) << "TCONVFPGA delegate: error_during_prepare set to "
                   << options.error_during_prepare << ".";
  TFLITE_LOG(INFO) << "TCONVFPGA delegate: error_during_invoke set to "
                   << options.error_during_invoke << ".";

  return TfLiteTCONVFPGADelegateCreate(&options);
}
...
TFL_CAPI_EXPORT TfLiteDelegate* tflite_plugin_create_delegate(
  ...
  return tflite::tools::CreateTCONVFPGADelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate* delegate) {
  TfLiteTCONVFPGADelegateDelete(delegate);
}
