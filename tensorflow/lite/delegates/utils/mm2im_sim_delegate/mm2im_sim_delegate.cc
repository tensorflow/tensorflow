...
#include "tensorflow/lite/delegates/utils/mm2im_sim_delegate/accelerator/driver/gemm_driver.h"
 ...
 public:
  ...
  TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      struct mm2im_params mm2im_params(
      ...
      mm2im_params.input_data = input_data;
      mm2im_params.weight_data = hwoi_ordered_filter_data;
      mm2im_params.output_data = col2im_data;
      mm2im_params.dex_map = dex_map;
      mm2im_params.wt_sum = &(wt_sum[i][0]);
      mm2im_params.MM2IM();
      ...
      saveMatrixCSV(
          "aData/mm2im/" + std::to_string(associated_nodes[i]) + "_del_wgt.csv",
          hwoi_ordered_filter_data, lhs_params.rows, lhs_params.cols);

      saveMatrixCSV(
          "aData/mm2im/" + std::to_string(associated_nodes[i]) + "_del_inp.csv",
      ...
      tflite_vm_mm2im_sim::Entry(drv, col2im_data, output_data);
...
TfLiteDelegate* TfLiteMM2IMSimDelegateCreate(
    const MM2IMSimDelegateOptions* options) {
