...
SC_MODULE(ACCNAME) {
  ...
  sc_signal<bool, SC_MANY_WRITERS> bias_quantize;
  sc_signal<bool, SC_MANY_WRITERS> out_check;
  sc_signal<bool, SC_MANY_WRITERS> send_output;
  ...
  sc_signal<bool> bias_quantize;
  sc_signal<bool> send_output;
  ...
  // First Set (A)
  ACC_DTYPE<32> dex_map1[8192];
  ACC_DTYPE<32> dex_map2[8192];
  ACC_DTYPE<32> dex_map3[8192];
  ACC_DTYPE<32> dex_map4[8192];

  // Outputs
  ACC_DTYPE<32> dst[32768];
  ...
  void PPU(int*, ACC_DTYPE<32>*, ACC_DTYPE<32>*);
  ...
  ACCNAME(sc_module_name name_)
    ...
    SC_CTHREAD(BiasAddQauntize, clock);
    reset_signal_is(reset, true);
