...
struct BuildMMapWeightCacheProviderTest : testing::TestWithParam<bool> {
  enum { kAlgoSeed1, kAlgoSeed2, kAlgoSeed3 };
  enum { kBufferId1, kBufferId2, kBufferId3, kBufferId4 };

  void SetUp() override {
    use_explicit_fd = GetParam();
    AddTensors();
    EndSetup();
  }
  ...
  void EndSetup() {
    ...
    if (use_explicit_fd) {
      ASSERT_TRUE(cache_provider.StartBuild(
          ...
          FileDescriptor::Duplicate(tmp_file.GetFd())));
    } else {
      tmp_file.Close();
  ...
  TempFileDesc tmp_file;
  bool use_explicit_fd = false;
};

std::string TestVariantName(const testing::TestParamInfo<bool>& info) {
  return info.param ? "WithExplicitFileDescriptor" : "ImplicitFd";
}

INSTANTIATE_TEST_SUITE_P(Test, BuildMMapWeightCacheProviderTest,
                         testing::Bool(), TestVariantName);
...
INSTANTIATE_TEST_SUITE_P(Test, LoadMMapWeightCacheProviderTest, testing::Bool(),
                         TestVariantName);
...
struct MMapWeightCacheProviderTest : testing::TestWithParam<bool> {
  bool use_explicit_fd = GetParam();
};

INSTANTIATE_TEST_SUITE_P(Test, MMapWeightCacheProviderTest, testing::Bool(),
                         TestVariantName);

TEST_P(MMapWeightCacheProviderTest, XnnpackCApiJourney) {
  using std::size;
  TempFileDesc temp_fd;
  const char* temp_fd_cpath = "explicit file descriptor";
