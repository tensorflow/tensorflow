...
TEST(MMapHandleTest, Resize) {
  ...
  // We don't have a way to force mremap to succeed/fail depending on the
  // system. This still acts as a compilation check.
  (void)was_resized;
...
TEST(WeightCacheBuilderTest, MultipleStepBuild) {
  ...
  const std::string payload3 =
      GenerateRandomString(/*100 MiB*/ 100 * 1024 * 1024);
