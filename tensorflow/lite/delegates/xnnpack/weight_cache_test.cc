...
TEST_F(IsCompatibleCacheFileTest, ReturnsTrueWhenFingerprintMatches) {
  EXPECT_TRUE(IsCompatibleCacheFile(fd_.GetCPath()));
}

TEST_F(IsCompatibleCacheFileTest, ReturnsFalseWhenFingerprintMismatches) {
  ChangeRuntimeFingerprintValue();
  EXPECT_FALSE(IsCompatibleCacheFile(fd_.GetCPath()));
}

TEST_F(IsCompatibleCacheFileTest, ReturnsFalseWhenFingerprintIsNotFound) {
  xnn_clear_fingerprints();
  EXPECT_FALSE(IsCompatibleCacheFile(fd_.GetCPath()));
}
