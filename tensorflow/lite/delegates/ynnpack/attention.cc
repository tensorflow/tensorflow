...
// Returns the op name of a STABLEHLO_COMPOSITE or CUSTOM node, or nullptr.
// Every SDPA name check goes through this.
const char* GetSdpaOpName(const TfLiteRegistration* registration,
  ...
  if (registration->builtin_code == kTfLiteBuiltinStablehloComposite &&
    ...
    return static_cast<const TfLiteStablehloCompositeParams*>(
               node->builtin_data)
        ->name;
  }
  return nullptr;
...
bool IsSdpaTransposed(const TfLiteRegistration* registration,
                      const TfLiteNode* node) {
  const char* name = GetSdpaOpName(registration, node);
  return name != nullptr && strcmp(name, "odml.sdpa_transposed") == 0;
}
...
bool IsSdpa(const TfLiteRegistration* registration, const TfLiteNode* node) {
  const char* name = GetSdpaOpName(registration, node);
  return name != nullptr &&
         (strcmp(name, "odml.scaled_dot_product_attention") == 0 ||
          strcmp(name, "odml.sdpa_transposed") == 0);
}
...
TfLiteStatus IsSdpaSupported(const TfLiteRegistration* registration,
  ...
  // The output is normally rank 4. `odml.sdpa_transposed` emits a rank-3
  // output for the single-token (decode) signature, with the heads folded
  // into the feature dim: q = [B, N, 1, H], k/v with K kv-heads (N % K == 0,
  // GQA is resolved inside the composite) -> out = [B, 1, N * H]. Only that
  // case is accepted at rank 3.
  if (output.dims->size == 3) {
    TF_LITE_ENSURE(context, IsSdpaTransposed(registration, node));
    TF_LITE_ENSURE_EQ(context, q.dims->data[2], 1);
    TF_LITE_ENSURE_EQ(context, k.dims->data[1], v.dims->data[1]);
    TF_LITE_ENSURE(context, k.dims->data[1] > 0);
    TF_LITE_ENSURE_EQ(context, q.dims->data[1] % k.dims->data[1], 0);
    TF_LITE_ENSURE_EQ(context, output.dims->data[0], q.dims->data[0]);
    TF_LITE_ENSURE_EQ(context, output.dims->data[1], 1);
    TF_LITE_ENSURE_EQ(context, output.dims->data[2],
                      q.dims->data[1] * q.dims->data[3]);
  } else {
  ...
  if (node->inputs->size >= 4 && node->inputs->data[3] != -1) {
    ...
    if (is_float_type(input3.type) || input3.type == kTfLiteBool) {
      ...
      if (output.dims->size == 3) {
        // The rank-3 lowering regroups the query heads, which is only valid for
        // a mask that broadcasts over heads and over the single query position:
        // [B, 1, 1, S].
        TF_LITE_ENSURE_EQ(context, input3.dims->size, 4);
        TF_LITE_ENSURE_EQ(context, input3.dims->data[1], 1);
        TF_LITE_ENSURE_EQ(context, input3.dims->data[2], 1);
      }
...
TfLiteStatus DefineSdpaNode(TfLiteContext* context, ynn_subgraph_t subgraph,
  ...
  // odml.scaled_dot_product_attention is [B, S, H, D]; odml.sdpa_transposed
  // is [B, H, S, D] with V as [B, H, D, S].
  const bool is_seq_major = !IsSdpaTransposed(reg, tflite_node);
  ...
  // Single-token composite with a rank-3 output:
  // q is [B, N, 1, H] against K kv-heads. Rather than broadcasting each kv
  // head across its N / K query heads inside the dots, fold the group into
  // the query sequence dim: [B, N, 1, H] -> [B, K, N / K, H] is the same
  // bytes (query head n = kv head n / (N / K), row n % (N / K)), and it makes
  // the two attention dots plain matmuls with N / K rows. The result
  // [B, K, N / K, H] is then the same bytes as the [B, 1, N * H] output.
  const bool fuse_heads = output_tensor.dims->size == 3;
  if (fuse_heads) {
    const size_t num_kv_heads = k_tensor.dims->data[1];
    const size_t q_folded_dims[4] = {
        static_cast<size_t>(q_tensor.dims->data[0]), num_kv_heads,
        static_cast<size_t>(q_tensor.dims->data[1]) / num_kv_heads,
        static_cast<size_t>(q_tensor.dims->data[3])};
    uint32_t q_folded_id = YNN_INVALID_VALUE_ID;
    TF_LITE_ENSURE_YNN_STATUS(ynn_define_static_reshape(
        subgraph, 4, q_folded_dims, q_trans_id, &q_folded_id, /*flags=*/0));
    q_trans_id = q_folded_id;
  }
  ...
  if (!need_slice_out && !is_seq_major && !fuse_heads) {
  ...
  if (fuse_heads) {
    // [B, K, N / K, H] -> [B, 1, N * H], a pure reinterpretation (see the
    // query fold above). The output dims are fully known from the TFLite
    // tensor.
    size_t out_dims[YNN_MAX_TENSOR_RANK];
    for (int i = 0; i < output_tensor.dims->size; ++i) {
      out_dims[i] = output_tensor.dims->data[i];
    }
    TF_LITE_ENSURE_YNN_STATUS(ynn_define_static_reshape(
        subgraph, output_tensor.dims->size, out_dims, *post_trans_ptr,
