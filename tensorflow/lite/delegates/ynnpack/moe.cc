...
// 16-bit floats are not computed on directly: values of these types are
// converted to fp32 before they enter the MoE subgraph.
...
// Replaces `value_id` with an fp32 copy of itself if `type` is a 16-bit float
// type, and leaves it untouched otherwise.
TfLiteStatus ConvertToFloat32IfNeeded(TfLiteContext* context,
                                      ynn_subgraph_t subgraph, TfLiteType type,
                                      uint32_t& value_id) {
  if (!Is16BitFloatType(type)) {
    return kTfLiteOk;
  }
  uint32_t converted = YNN_INVALID_VALUE_ID;
  TF_LITE_ENSURE_YNN_STATUS(
      ynn_define_convert(subgraph, value_id, ynn_type_fp32, &converted, 0));
  value_id = converted;
  return kTfLiteOk;
}
...
TfLiteStatus DefineMoeNode(TfLiteContext* context, ynn_subgraph_t subgraph,
  ...
  TF_LITE_ENSURE_STATUS(
      ConvertToFloat32IfNeeded(context, subgraph, tokens.type, tokens_5d));
  TF_LITE_ENSURE_STATUS(ConvertToFloat32IfNeeded(
      context, subgraph, routing_weights.type, routing_weights_val));
  TF_LITE_ENSURE_STATUS(
      ConvertToFloat32IfNeeded(context, subgraph, scale.type, scale_val));
  ...
  for (WeightValues& w : weights) {
    TF_LITE_ENSURE_STATUS(ConvertToFloat32IfNeeded(
        context, subgraph, context->tensors[w.tensor_index].type, w.value_id));
  }
  ...
  if (is_quantized && has_weight_scales) {
    ...
    for (WeightValues& w : weights) {
      TF_LITE_ENSURE_STATUS(ConvertToFloat32IfNeeded(
          context, subgraph, context->tensors[w.scale_tensor_index].type,
          w.scale_id));
    }
  }
  ...
  // is just a dot.
  //
  // No YNN_NODE_FLAG_SYMMETRIC_B here. It would let YNNPACK pick the x86
  // `maddubs`-based int8 kernels, but only by promising that no weight is
  // -128, which nothing in this delegate checks and no test covers. Measured
  // on the shapes in moe_bench (Qwen, Gemma 4 700M and 26B-A4B, int8, decode
  // and prefill), setting it changes nothing: these dots are all GEMVs, so
  // they are bandwidth-bound and the extra ALU throughput has nothing to do.
  ...
  // The subgraph computes in fp32, convert back to a 16-bit float output
  // unless the output value was already defined with its own type.
  const TfLiteTensor& output = context->tensors[node.outputs[0]];
  if (Is16BitFloatType(output.type) && out_it == tensor_to_value_id.end()) {
    uint32_t final_out = YNN_INVALID_VALUE_ID;
    TF_LITE_ENSURE_YNN_STATUS(ynn_define_convert(
        subgraph, output_val, GetYnnType(output.type), &final_out, 0));
    output_val = final_out;
  }
