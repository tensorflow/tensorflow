        ...
        with writer.sub_message("invoke") as invoke:
            ...
            for name, value in example["outputs"].items():
                ...
                with invoke.sub_message("output_shape") as output_shape:
                    ...
                    output_shape.write_field(
                        "value", ",".join([str(dim) for dim in value.shape])
                    )
...
def make_zip_of_tests(
        ...
        for curr in itertools.product(*parameters.values()):
            ...
            def build_example(label, param_dict_real, zip_path_label):
                ...
                with tf.Graph().as_default():
                    ...
                    inference_signature = tf.compat.v1.saved_model.signature_def_utils.build_signature_def(
                    ...
                    graph_def = (
                        freeze_graph(
                            sess, tf.compat.v1.global_variables() + inputs + outputs
                        )
                        if use_frozen_graph
                        else sess.graph_def
                    )
                ...
                if tflite_model_binary:
                    if options.make_edgetpu_tests:
                        ...
                        baseline_input_map, baseline_output_map = (
    ...
    converter_success = sum(
        1 for x in convert_report if x[1]["tflite_converter"] == report_lib.SUCCESS
    )
    percent = 0
    if tf_success > 0:
