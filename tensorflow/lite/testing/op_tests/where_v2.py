...
def make_where_v2_tests(options):
    ...
    test_parameters = [
        {
            "condition_dtype": [tf.float32, tf.bool, tf.int32, tf.uint32, tf.uint8],
    ...
    def build_graph(parameters):
        ...
        if (
            ...
            and parameters["input_dtype"] is not None
        ):
            parameters["condition_dtype"] = tf.bool
    ...
    def build_inputs(parameters, sess, inputs, outputs):
        input_condition = create_tensor_data(
            parameters["condition_dtype"], parameters["input_condition_shape"]
        )
        input_value1 = None
        input_value2 = None
        if parameters["input_dtype"] is not None:
            input_value1 = create_tensor_data(
                parameters["input_dtype"], parameters["input_shape_set"][0]
            )
            input_value2 = create_tensor_data(
                parameters["input_dtype"], parameters["input_shape_set"][1]
            )
    ...
    make_zip_of_tests(
        options, test_parameters, build_graph, build_inputs, expected_tf_failures=2
    )
