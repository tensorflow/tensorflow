...
def make_binary_op_tests(
    ...
    def build_graph(parameters):
        ...
        if parameters["activation"] and (
            parameters["dtype"] != tf.int32 and parameters["dtype"] != tf.int64
        ):
            out = tf.nn.relu(out)
    ...
    def build_inputs(parameters, sess, inputs, outputs):
        ...
        return [input1, input2], sess.run(
            outputs, feed_dict={inputs[0]: input1, inputs[1]: input2}
        )
...
def make_add_tests(options):
    ...
    make_binary_op_tests(
        options, tf.add, allow_fully_quantize=True, test_parameters=test_parameters
    )
...
def make_mul_tests(options):
    ...
    make_binary_op_tests(
        options, tf.multiply, allow_fully_quantize=True, test_parameters=test_parameters
    )
