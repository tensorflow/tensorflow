...
def make_matrix_band_part_tests(options):
    ...
    def get_static_shape(shape):
        ...
        if not shape:
            return np.random.randint(
                low=1, high=10, size=np.random.randint(low=2, high=10, size=())
            ).tolist()
    ...
    def build_inputs(parameters, sess, inputs, outputs):
        ...
        return [input_value, num_lower, num_upper], sess.run(
            outputs, dict(zip(inputs, [input_value, num_lower, num_upper]))
        )
