...
class ModelTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def _getMatMulModelWithSmallWeights(self):
        class MatMulModelWithSmallWeights(autotrackable.AutoTrackable):
            ...
            def __init__(self):
                self.weight = constant_op.constant(
                    [[1e-3, -1e-3], [-2e-4, 2e-4]], shape=(2, 2), dtype=dtypes.float32
                )
                self.bias = constant_op.constant(
                    [1.28, 2.55], shape=(2,), dtype=dtypes.float32
                )
    ...
    def _createV2QATLowBitKerasModel(
        self, shape, weight_only, num_bits, bit_min, bit_max
    ):
        ...
        initial_weights = np.linspace(bit_min, bit_max, np.prod(kernel_shape)).reshape(
            kernel_shape
        )
        test_initializer = tf.constant_initializer(initial_weights)
        x = ConvWrapper(
            tf.keras.layers.Conv2D(
                1, (3, 3), kernel_initializer=test_initializer, activation="relu6"
            )
        )(input_tensor)
