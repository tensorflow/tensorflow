...
class FromConcreteFunctionTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testDisableFlexTensorMemoryReusing(self):
        @tf.function(
            input_signature=[tf.TensorSpec(shape=[2, 3], dtype=tf.float32, name="x")]
        )
        def model(x):
...
class TFQuantizationTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        ("DefaultMode", "DEFAULT"), ("LegacyIntegerMode", "LEGACY_INTEGER")
    )
    ...
    def testConvOpWithBias(self, tf_quantization_mode):
        ...
        self.assertCountEqual(
            ["CONV_2D", "RESHAPE"], tflite_test_util.get_ops_list(tflite_model)
        )
