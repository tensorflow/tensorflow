...
class SubComponent(SubComponentItem, enum.Enum):
    ...
    FREEZE_SAVED_MODEL = SubComponentItem(
        "FREEZE_SAVED_MODEL", Component.PREPARE_TF_MODEL
    )
    ...
    FREEZE_KERAS_MODEL = SubComponentItem(
        "FREEZE_KERAS_MODEL", Component.PREPARE_TF_MODEL
    )

    # Replace all the variables with constants in a ConcreteFunction.
    FREEZE_CONCRETE_FUNCTION = SubComponentItem(
        "FREEZE_CONCRETE_FUNCTION", Component.PREPARE_TF_MODEL
    )

    # Run grappler optimization.
    OPTIMIZE_TF_MODEL = SubComponentItem(
        "OPTIMIZE_TF_MODEL", Component.PREPARE_TF_MODEL
    )
    ...
    CONVERT_GRAPHDEF = SubComponentItem(
        "CONVERT_GRAPHDEF", Component.CONVERT_TF_TO_TFLITE_MODEL
    )

    # Convert a SavedModel to TFLite model.
    CONVERT_SAVED_MODEL = SubComponentItem(
        "CONVERT_SAVED_MODEL", Component.CONVERT_TF_TO_TFLITE_MODEL
    )

    # Convert a Jax HLO to TFLite model.
    CONVERT_JAX_HLO = SubComponentItem(
        "CONVERT_JAX_HLO", Component.CONVERT_TF_TO_TFLITE_MODEL
    )
...
class ConverterError(Exception):
    ...
    def _parse_error_message(self, message):
        ...
        error_code_mapping = {
            ...
            "tf/compat/v1/enable_control_flow_v2.": converter_error_data_pb2.ConverterErrorData.ERROR_UNSUPPORTED_CONTROL_FLOW_V1,
        }
