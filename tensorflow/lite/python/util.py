...
def _remove_tensors_from_model(model, remove_tensors_idxs):
    ...
    if len(model.subgraphs) > 1:
        logging.info(
            "Skipping the removal of dangled tensors since the model has "
...
def _modify_model_input_type(model, inference_input_type=dtypes.float32):
    ...
    for signature_index, signature_def in enumerate(model.signatureDefs):
        _modify_model_input_type_per_subgraph(
            model, signature_def.subgraphIndex, signature_index, inference_input_type
        )


def _modify_model_input_type_per_subgraph(
...
def _modify_model_output_type(model, inference_output_type=dtypes.float32):
    ...
    for signature_index, signature_def in enumerate(model.signatureDefs):
        _modify_model_output_type_per_subgraph(
            model, signature_def.subgraphIndex, signature_index, inference_output_type
        )


def _modify_model_output_type_per_subgraph(
    model, subgraph_index, signature_index, inference_output_type
):
...
def _remove_redundant_quantize_ops(model):
    ...
    for signature_index, signature_def in enumerate(model.signatureDefs):
        _remove_redundant_quantize_ops_per_subgraph(
            model, signature_def.subgraphIndex, signature_index
        )
