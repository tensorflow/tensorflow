...
class TestModels(test_util.TensorFlowTestCase):
    ...
    def _run(
        ...
        # Parse command line arguments safely without shell=True
        # Split flags_str into individual arguments using shlex for proper handling
        cmd_args = [tflite_bin, "--output_file", output_file]
        if flags_str:
            # Use shlex.split to safely parse the flags string
            cmd_args.extend(shlex.split(flags_str))
