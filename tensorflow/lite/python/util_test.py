...
class UtilTest(test_util.TensorFlowTestCase):
    def testConvertEnumToDtype(self):
        self.assertEqual(util._convert_tflite_enum_type_to_tf_type(0), dtypes.float32)
        self.assertEqual(util._convert_tflite_enum_type_to_tf_type(1), dtypes.float16)
        ...
        self.assertEqual(util._convert_tflite_enum_type_to_tf_type(5), dtypes.string)
        ...
        self.assertEqual(util._convert_tflite_enum_type_to_tf_type(8), dtypes.complex64)
        self.assertEqual(util._convert_tflite_enum_type_to_tf_type(9), dtypes.int8)
        self.assertEqual(util._convert_tflite_enum_type_to_tf_type(10), dtypes.float64)
        self.assertEqual(
            util._convert_tflite_enum_type_to_tf_type(11), dtypes.complex128
        )
        self.assertEqual(util._convert_tflite_enum_type_to_tf_type(16), dtypes.uint32)
...
class TensorFunctionsTest(test_util.TensorFlowTestCase):
    ...
    def testGetTensorsInvalid(self):
        ...
        self.assertEqual(
            "Invalid tensors 'invalid-input' were found.", str(error.exception)
        )
    ...
    def testSetTensorShapeDimensionInvalid(self):
        ...
        self.assertIn(
            "The shape of tensor 'Placeholder' cannot be changed", str(error.exception)
        )
        self.assertAllEqual([None, 3, 5], tensor.shape)
        ...
        for qtype, v2 in map_quantize_type_to_io_types.items():
            ...
            for itype in v2:
                ...
                for otype in v2:
                    ...
                    params.append(
                        (
                            str_template.format(k1, qstr, istr, ostr),
                            v1,
                            qtype,
                            itype,
                            otype,
...
class UtilModifyIntegerQuantizedModelIOTypeSignatureDefTest(
    ...
    def test(self):
        ...
        post_train_int8_model = self._generate_integer_tflite_model_from_saved_model()
        modified_model = util.modify_model_io_type(
            post_train_int8_model, tf.int8, tf.float32
        )
