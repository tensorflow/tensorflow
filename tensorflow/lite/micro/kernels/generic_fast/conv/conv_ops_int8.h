...
TfLiteStatus EvalConvInt8WithoutPadding(TfLiteConvParams* params, OpData* data,
  ...
  return EvalConvWithoutPadding<int8_t>(params, data, input, filter, bias,
                                        output, context);
}

TfLiteStatus EvalConvInt8WithPadding(TfLiteConvParams* params, OpData* data,
                                     const TfLiteEvalTensor* input,
                                     const TfLiteEvalTensor* filter,
                                     const TfLiteEvalTensor* bias,
                                     TfLiteEvalTensor* output,
                                     TfLiteContext* context) {
  return EvalConvWithPadding<int8_t>(params, data, input, filter, bias, output,
                                     context);
}
...
TfLiteStatus EvalConvInt8(TfLiteConvParams* params, OpData* data,
  ...
  if (use_padding) {
    return EvalConvInt8WithPadding(params, data, input, filter, bias, output,
                                   context);
  } else {
    return EvalConvInt8WithoutPadding(params, data, input, filter, bias, output,
                                      context);
  }
}
