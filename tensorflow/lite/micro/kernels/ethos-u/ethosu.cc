...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  // Ethos-U guarantees that the tensors that require a base pointer are among the 8 first tensors
  num_tensors = std::min(num_tensors, 8);

  result = ethosu_invoke(cms_data, data->cms_data_size, base_addrs, num_tensors);
  if (-1 == result) {
    return kTfLiteError;
  } else {
    return kTfLiteOk;
