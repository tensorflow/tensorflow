...
TfLiteStatus Prepare_16(TfLiteContext* context, TfLiteNode* node) {
  ...
  //   job_params[i_cg] = {(uint32_t)changrp.start,
  //   (channel_count_t)changrp.size};
  // }
...
TfLiteStatus Eval_16(TfLiteContext* context, TfLiteNode* node) {
  ...
  fully_connected_16(output->data.i16, weights->data.int8, input->data.int8,
                     (const nn_bso_block_t*)bso->data.i16, C_in, 0, C_out);
  return kTfLiteOk;
  ...
  //       &tBSO, bso->data.i16, op->execution_plan.GetBiasScratchSize(),
  //       changrp);
...
TfLiteRegistration* Register_FullyConnected_16() {
  static TfLiteRegistration r = {fully_connected::Init_16, nullptr,
