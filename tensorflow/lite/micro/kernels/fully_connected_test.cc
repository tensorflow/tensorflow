...
#if !defined(XTENSA) && !defined(CEVA_BX1) && !defined(CEVA_SP500)  // TODO(b/170503075): xtensa kernels are less general than
                      // reference kernels and we ifdef out test cases that are
                      // currently known to fail.
                      // CEVA's fully connected implementation assumes weights_zero_point=0 as described in TFLite's quantization specification.
                      // tests which use a different zero point will so ifdefed out.
