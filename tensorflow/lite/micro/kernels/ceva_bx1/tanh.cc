...
TfLiteStatus TanhEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {
    case kTfLiteFloat32: {
		...
		MCPS_START_ONE;
#endif
		CEVA_TFLM_Tanh_float32(input_data, output_data, flat_size);
#ifdef MCPS_MEASUREMENT
		MCPS_STOP_ONE("Test params:CEVA_TFLM_Tanh_Float32 loop = %d", flat_size);
#endif
    ...
    case kTfLiteInt8: {
		...
		MCPS_START_ONE;
#endif
		CEVA_TFLM_tanh_Int8(data.input_zero_point, data.input_range_radius,
			data.input_multiplier, data.input_left_shift,
			input_size, input_data,
			output_data);
#ifdef MCPS_MEASUREMENT
		MCPS_STOP_ONE("Test params:CEVA_TFLM_tanh_Int8 loop = %d", input_size);
#endif
#ifdef MCPS_MEASUREMENT
		//MCPS_STOP_ONE("Test params:CEVA_TFLM_tanh_Int8 loop = %d", input_size);
#endif
