...
/* model requires 20ms new data from g_audio_capture_buffer and 10ms old data each time , storing old data in the histrory buffer , { history_samples_to_keep = 10 * 16 } */
...
TfLiteStatus InitAudioRecording(tflite::ErrorReporter* error_reporter) {
    ...
    /* create CaptureSamples Task which will get the i2s_data from mic and fill it in the ring buffer */
...
TfLiteStatus GetAudioSamples(tflite::ErrorReporter* error_reporter,
    ...
    /* copy 320 samples (640 bytes) from rb at ( int16_t*(g_audio_output_buffer) + 160 ), first 160 samples (320 bytes) will be from history */
    int32_t bytes_read = rb_read(g_audio_capture_buffer, ((uint8_t* )(g_audio_output_buffer + history_samples_to_keep)), new_samples_to_get*sizeof(int16_t), 10);
    if (bytes_read < 0) {
