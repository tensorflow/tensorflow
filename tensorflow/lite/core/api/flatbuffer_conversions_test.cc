...
class StablehloScatterFlatbufferConversionsTest
 ...
 protected:
  ...
  void ExpectTooManyDimensions(int update_window_dims_count,
    ...
    EXPECT_EQ(ParseOpData(BuildScatterOperator(update_window_dims_count,
                                               inserted_window_dims_count,
                                               scatter_dims_count),
                          BuiltinOperator_STABLEHLO_SCATTER, &mock_reporter_,
                          &mock_allocator_, (void**)&output_data),
              kTfLiteError);
...
TEST_F(StablehloScatterFlatbufferConversionsTest, AcceptsMaximumDimensions) {
  TfLiteStablehloScatterParams* output_data = nullptr;
  EXPECT_EQ(ParseOpData(BuildScatterOperator(kMaxDims, kMaxDims, kMaxDims),
                        BuiltinOperator_STABLEHLO_SCATTER, &mock_reporter_,
                        &mock_allocator_, (void**)&output_data),
            kTfLiteOk);
  ASSERT_NE(output_data, nullptr);
...
TEST_F(FlatbufferConversionsTest,
  ...
  const Operator* op = BuildTestOperator(
      ...
      CreateStablehloGatherOptions(
          ...
          .Union());
  void* output_data = nullptr;
  EXPECT_EQ(ParseOpData(op, BuiltinOperator_STABLEHLO_GATHER, &mock_reporter_,
                        &mock_allocator_, &output_data),
            kTfLiteError);
  EXPECT_EQ(output_data, nullptr);
  EXPECT_THAT(mock_reporter_.GetString(),
