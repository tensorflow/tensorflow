...
class TestSchemaUpgrade(test_util.TensorFlowTestCase):
    ...
    def CheckConversion(self, data_old, data_expected):
        ...
        converter = upgrade_schema_lib.Converter()
        with (
            tempfile.NamedTemporaryFile(suffix=".json", mode="w+") as in_json,
            tempfile.NamedTemporaryFile(suffix=".json", mode="w+") as out_json,
            tempfile.NamedTemporaryFile(suffix=".bin", mode="w+b") as out_bin,
            tempfile.NamedTemporaryFile(suffix=".tflite", mode="w+b") as out_tflite,
        ):
