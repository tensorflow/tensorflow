...
// Regression test for an integer overflow in InterpreterBuilder's buffer
// offset bounds check (see interpreter_builder.cc ParseTensors). A crafted
// model can specify a Buffer with an offset and size whose sum wraps around
// the uint64_t range, e.g. offset = 2^64 - 1 and size = 2 wraps to 1. The old
// check computed `offset + size` directly, so the wrapped value (1) compared
// as "in bounds" and bypassed the allocation bounds check, producing a wild
// pointer. The fix uses overflow-checked arithmetic (CheckedInt) and must
// reject the model with kTfLiteError.
TEST(BasicInterpreter, BufferOffsetWrappingIsRejected) {
  flatbuffers::FlatBufferBuilder builder;

  // Buffer 0 is the conventional empty sentinel buffer.
  std::vector<flatbuffers::Offset<Buffer>> buffers;
  buffers.push_back(CreateBuffer(builder, /*data=*/0));
  // Buffer 1 has no inline data, so InterpreterBuilder takes the
  // offset-into-allocation path. offset + size wraps uint64_t:
  //   0xFFFFFFFFFFFFFFFF + 2 == 1 (mod 2^64).
  buffers.push_back(CreateBuffer(builder, /*data=*/0,
                                 /*offset=*/0xFFFFFFFFFFFFFFFFull,
                                 /*size=*/2));

  // A single constant tensor that references the overflowing buffer.
  std::vector<int> shape = {1};
  std::vector<flatbuffers::Offset<Tensor>> tensors;
  tensors.push_back(CreateTensorDirect(builder, &shape, TensorType_UINT8,
                                       /*buffer=*/1, "weights"));

  std::vector<int32_t> inputs = {};
  std::vector<int32_t> outputs = {};
  auto subgraph = CreateSubGraph(
      builder, builder.CreateVector(tensors), builder.CreateVector(inputs),
      builder.CreateVector(outputs), /*operators=*/0,
  ...
  auto model = CreateModel(builder, TFLITE_SCHEMA_VERSION, /*operator_codes=*/0,
                           builder.CreateVector(subgraphs),
                           builder.CreateString("overflow_model"),
  ...
  // InterpreterBuilder must detect the wrapping offset and abort instead of
  // storing an out-of-bounds pointer.
  EXPECT_EQ(InterpreterBuilder(*flatbuffer_model, resolver)(&interpreter),
            kTfLiteError);
}
