...
Used to generate Arduino, Particle and ESP-IDF examples.
"""
  ...
  if include_match:
    ...
    for supplied_header in supplied_headers_list:
      if six.ensure_str(supplied_header).endswith(path):
        ...
        break
    line = include_match.group(1) + six.ensure_str(path) \
...
def replace_esp_example_includes(line, source_path):
  ...
  include_match = re.match(
      r'.*#include.*"(' + EXAMPLE_DIR_PATH + r'.*)"', line)
...
def main(unused_args, flags):
  ...
  # The Particle source generation process is the same as what's needed for Particle
  elif flags.platform == 'particle':
    output_text = transform_arduino_sources(input_file_lines, flags)
