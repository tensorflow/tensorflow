...
void custom_am_bsp_low_power_init(void) {
    ...
    for (ux = 0; ux < AM_BSP_NUM_LEDS; ux++) {
        ...
        //
        am_hal_gpio_pinconfig(ui32GPIONumber,
...
void pdm_start_dma(tflite::ErrorReporter* error_reporter) {
  ...
  if (AM_HAL_STATUS_SUCCESS !=
      am_hal_pdm_dma_start(g_pdm_handle, &sTransfer)) {
...
extern "C" void power_button_handler(void) {
      g_PowerOff = 1;
  }
...
TfLiteStatus InitAudioRecording(tflite::ErrorReporter* error_reporter) {
  ...
  if ( am_hal_cachectrl_control(
       AM_HAL_CACHECTRL_CONTROL_LPMMODE_SET, &ui32LPMMode) ) {
  ...
  am_hal_gpio_interrupt_clear(
     AM_HAL_GPIO_BIT(AM_BSP_GPIO_BUTTON_POWER));
  am_hal_gpio_interrupt_register(
     AM_BSP_GPIO_BUTTON_POWER, power_button_handler);
...
TfLiteStatus GetAudioSamples(tflite::ErrorReporter* error_reporter,
  ...
  // before that happens.
  const int start_offset = (start_ms < 0) ?
    0 : start_ms * (kAudioSampleFrequency / 1000);
