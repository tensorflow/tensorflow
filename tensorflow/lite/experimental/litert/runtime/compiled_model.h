...
class LiteRtCompiledModelT {
 ...
 private:
  ...
  // locked and use it with CustomAllocation. The locked buffers will be added
  // to the given `locked_buffers`. The caller is responsible for unlocking
  // these buffers.
  litert::Expected<void> BufferRegister(
      ...
      std::vector<litert::TensorBuffer>& locked_buffers);
