...
Expected<void> LiteRtCompiledModelT::InitializeModel(
  ...
  const auto* tfl_model = tfl_wrapper.PackedModel();

  if (!need_reserialization && tfl_model != nullptr) {
    fb_model_ = tflite::FlatBufferModel::BuildFromModel(tfl_model);
    return {};
  }
...
Expected<LiteRtCompiledModelT::Ptr> LiteRtCompiledModelT::Create(
  ...
  if (auto res = compiled_model->InitializeRuntime(); !res.HasValue()) {
    return Unexpected(kLiteRtStatusErrorRuntimeFailure,
  ...
  model_compilation_data->allocation_base = compiled_model->GetAllocBase();
