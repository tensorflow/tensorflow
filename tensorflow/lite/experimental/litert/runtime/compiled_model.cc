...
Expected<void> LiteRtCompiledModelT::BufferRegister(
    ...
    std::vector<TensorBuffer>& locked_buffers) {
  ...
  if (backend_requires_cpu_buffer) {
    ...
    if (bufer_is_cpu_compatible) {
      ...
      TensorBuffer cpu_buffer(buffer, /*owned=*/false);
      auto addr = cpu_buffer.Lock();
      if (!addr) {
        return Unexpected(kLiteRtStatusErrorRuntimeFailure,
                          "Failed to lock input tensor buffer");
      }
      locked_buffers.push_back(std::move(cpu_buffer));
      TfLiteCustomAllocation custom_allocation{*addr, tensor->bytes};
      if (is_input) {
