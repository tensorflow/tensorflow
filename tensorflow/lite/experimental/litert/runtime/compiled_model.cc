...
Expected<LiteRtCompiledModelT::Ptr> LiteRtCompiledModelT::Create(
  ...
  if (compilation_options != kLiteRtHwAccelatorNone) {
    ...
    if (auto result =
      ...
      new_flatbuffer = std::move(std::get<0>(*result));
      auto& success_message = std::get<1>(*result);
      auto& error_message = std::get<2>(*result);
      LITERT_LOG(LITERT_INFO,
                 "Successfully applied the following compiler plugins: %s",
                 success_message.c_str());
      if (!error_message.empty()) {
        LITERT_LOG(LITERT_WARNING, "The following plugins failed to apply: %s",
                   error_message.c_str());
      }
    }
  }
