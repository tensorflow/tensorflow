...
std::vector<OpWrapper> BuildDynamicUpdateSliceOp(
  ...
  if (input_tensor.GetRank() != update_tensor.GetRank()) {
    std::cout << "QNN LiteRT Delegate only supports Dynamic Update Slice when "
                 "operand "
  ...
  if (indices_tensor.GetDataType() != QNN_DATATYPE_INT_32) {
    std::cout << "Dynamic Update Slice only supports QNN_DATATYPE_INT_32 "
                 "start_indices."
              << std::endl;
    return {};
  }
  ...
  if (input_tensor.GetRank() < 2) {
    std::cout << "Dynamic Update Slice only supports operand tensor rank >= 2"
              << std::endl;
    return {};
  }
