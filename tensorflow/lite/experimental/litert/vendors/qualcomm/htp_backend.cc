...
LiteRtStatus HandleDownvoteConfig(
  ...
  switch (perf_mode) {
    ...
    case TfLiteQnnDelegateHtpPerformanceMode::kHtpBalanced:
      // Unlike SNPE, leave the sleep latency = 65535, which enables deep sleep
      // when HTP is not inferencing.
...
LiteRtStatus HandleUpvoteConfig(
  ...
  switch (perf_mode) {
    ...
    case TfLiteQnnDelegateHtpPerformanceMode::kHtpHighPerformance:
      ...
      break;
    case TfLiteQnnDelegateHtpPerformanceMode::kHtpPowerSaver:
      // Dcvs is enabled here according to Qnn HTP API doc.
      // But actually, it seems that dcvs will have no effect if all voltage
      // corners are set to the same value.
      dcvs_v3.dcvsEnable = kDcvsEnable;
      status = SetQnnHtpPerfInfrastructure(
...
HtpBackend::HtpBackend(const QnnApi* api,
                       const TfLiteQnnDelegateHtpBackendOptions* htp_options)
    : api_(api),
      performance_mode_(htp_options->performance_mode),
      perf_ctrl_strategy_(htp_options->perf_ctrl_strategy) {
  backend_config_ = std::make_unique<BackendConfig>();
}

HtpBackend::~HtpBackend() = default;

LiteRtStatus HtpBackend::CreatePerfPowerConfigPtr(
    const std::uint32_t power_config_id,
    const TfLiteQnnDelegateHtpPerformanceMode perf_mode,
    const PerformanceModeVoteType vote_type) {
...
void HtpBackend::PerformanceVote() {
  std::lock_guard<std::mutex> lock(perf_power_configs_mutex_);
  if (IsPerfModeEnabled() && IsPerfCtrlManual() &&
...
LiteRtStatus HtpBackend::Init(Qnn_DeviceHandle_t* device_handle) {
  ...
  if (GetPerfInfra(api_, backend_config_->htp_perf_infra_) != kLiteRtStatusOk) {
    return kLiteRtStatusErrorRuntimeFailure;
  }
...
LiteRtStatus HtpBackend::Terminate() {
