...
template <typename T>
T Quantize(const float& val, const float& scale, const T& zero_point) {
  return std::round(val / scale) + zero_point;
}

template <typename T>
float Dequantize(const T& val, const float& scale, const T& zero_point) {
...
TEST(Qualcomm, DispatchApiWithDmaBufInt16Model) {
  ...
  const std::int16_t input_tensor_0_data[] = {
      Quantize<int16_t>(0.5, kInputScale, kInputZeroPoint),
      Quantize<int16_t>(0.5, kInputScale, kInputZeroPoint)};
  const std::int16_t input_tensor_1_data[] = {
      Quantize<int16_t>(0.5, kInputScale, kInputZeroPoint),
      Quantize<int16_t>(0.5, kInputScale, kInputZeroPoint)};
  const std::int16_t output_tensor_0_data[] = {
      Quantize<int16_t>(1, kOutputScale, kOutputZeroPoint),
      Quantize<int16_t>(1, kOutputScale, kOutputZeroPoint)};

  auto raw_input_tensor_0 =
      absl::MakeSpan(input_tensor_0_data, kTestInput0Size_3);
  auto raw_input_tensor_1 =
      absl::MakeSpan(input_tensor_1_data, kTestInput1Size_3);
  auto raw_output_tensor_0 =
      absl::MakeSpan(output_tensor_0_data, kTestOutputSize_3);
...

  {
    ...
    for (const auto& data : int16_output) {
      dequant_output.emplace_back(
          Dequantize<std::int16_t>(data, kOutputScale, kOutputZeroPoint));
    }

    for (auto i = 0; i < kTestOutputSize_3; ++i) {
