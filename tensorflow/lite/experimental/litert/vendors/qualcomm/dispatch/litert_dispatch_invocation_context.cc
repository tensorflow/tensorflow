...
LiteRtDispatchInvocationContextT::Create(
  ...
  // TODO: Add profiling_level as an option after header interface merged
...
Expected<void> LiteRtDispatchInvocationContextT::Profile() {
  ...
  for (std::uint32_t i = 0; i < num_events; ++i) {
    ...
    data_ss_ << "    " << event_data.identifier << ": " << event_data.value
             << " " << GetEventUnit(event_data.unit).c_str() << std::endl;
    ...
    if (event_data.type ==
      ...
      for (std::uint32_t j = 0; j < num_sub_events; ++j) {
        ...
        if (sub_event_data.type == QNN_PROFILE_EVENTTYPE_NODE &&
          ...
          data_ss_ << "        " << sub_event_data.identifier << ": "
                   << sub_event_data.value << " "
                   << GetEventUnit(sub_event_data.unit).c_str() << std::endl;
        }
      }
    }
  }

  LITERT_LOG(LITERT_INFO, "%s", data_ss_.str().c_str());
