...
#include "neuron_schema_generated.h"
...
class CompiledGraph {
 public:
  ...
  litert::Expected<std::pair<const void*, size_t>> GetCompiledNetwork() {
    ...
    assert(GetCompiledType() != CompiledType_DLB);
    // TODO: Support the external buffer.
    assert(subgraph_.compiled_index_type() == BufferIndicate_Index);
    auto index = subgraph_.compiled_index_as_Index();
    return GetBuffer(index->value());
  }

  const NeuronSchema::CompiledType GetCompiledType() {
    return subgraph_.type();
  }
  ...
  litert::Expected<std::pair<const void*, size_t>> GetBuffer(int32_t i) {
    ...
    return std::make_pair<const void*, size_t>(buffer->data()->data(),
                                               buffer->data()->size());
  }
