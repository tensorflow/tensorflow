...
Expected<void> LegalizeConcatOp(const NeuronAdapterApi& neuron_adapter_api,
                                ...
                                const litert::Op& op) {
  LITERT_LOG(LITERT_INFO, "Legalize Concate");
  ...
  // A NEURON_CONCAT operation takes an additional scalar operand, which is used
  // to pass as a axis.
  int32_t axis;
  if (auto status = LiteRtGetConcatenationAxisOption(op.Get(), &axis);
      status != kLiteRtStatusOk) {
    return Error(status, "Failed to get new shape option");
  }

  auto axis_operand_index = operand_map.AddScalarInt32(axis);
  if (!axis_operand_index) {
    return axis_operand_index.Error();
  }

  input_indices.push_back(*axis_operand_index);
  ...
  if (ModelAddOperation(neuron_adapter_api, model, /*type=*/NEURON_CONCATENATION,
                        input_indices, output_indices) != NEURON_NO_ERROR) {
    return Error(kLiteRtStatusErrorRuntimeFailure,
                 "Failed to add NEURON_CONCAT operation");
  }
