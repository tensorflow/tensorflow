...
bool EqualsFbQuantizationDetail(
  ...
  for (int i = 0; i < tfl_quantization->zero_point.size(); ++i) {
    if (litert_quantization.zero_point[i] != tfl_quantization->zero_point[i]) {
      return false;
    }
    if (litert_quantization.scale[i] != tfl_quantization->scale[i]) {
      return false;
    }
  }
  return true;
