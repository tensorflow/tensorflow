...
class AudioFeatureGenerationTest(tf.test.TestCase):
    ...
    def testSimple(self):
        with self.test_session():
            audio = tf.constant(
                [0, 32767, 0, -32768] * ((WINDOW_SIZE + 4 * WINDOW_STEP) // 4), tf.int16
            )
    ...
    def testStacking(self):
        with self.test_session():
            ...
            self.assertAllEqual(
                self.evaluate(filterbanks), [[479, 425, 436, 378], [410, 350, 391, 325]]
            )
    ...
    def testStackingWithOverlap(self):
        with self.test_session():
            ...
            self.assertAllEqual(
                self.evaluate(filterbanks),
                [
                    [479, 425, 479, 425, 436, 378],
                    [479, 425, 436, 378, 410, 350],
                    [436, 378, 410, 350, 391, 325],
                    [410, 350, 391, 325, 391, 325],
                ],
            )
    ...
    def testStackingDropFrame(self):
        with self.test_session():
            ...
            self.assertAllEqual(
                self.evaluate(filterbanks), [[479, 425, 479, 425], [436, 378, 410, 350]]
            )

    def testZeroPadding(self):
        with self.test_session():
            audio = tf.constant(
                [0, 32767, 0, -32768] * ((WINDOW_SIZE + 7 * WINDOW_STEP) // 4), tf.int16
            )
            filterbanks = frontend_op.audio_microfrontend(
                audio,
                sample_rate=SAMPLE_RATE,
