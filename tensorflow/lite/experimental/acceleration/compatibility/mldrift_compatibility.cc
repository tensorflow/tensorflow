...
bool IsMlDriftGpuSupportedOnThisDevice() {
  ...
  static const bool is_supported = []() {
    ...
    // Read GpuInfo on a separate thread so EglEnvironment does not reuse or
    // modify an active EGL context on the calling thread.
    ...
    if (!compatibility_list) {
      TFLITE_LOG_PROD(
          TFLITE_LOG_WARNING,
          "IsMlDriftGpuSupportedOnThisDevice: Failed to parse embedded "
          "mldrift_compatibility binary.");
      return false;
    }
