...
namespace {
absl::Status CheckCompareTypeConstraint(const Tensor& lhs,
                                        const CompareOp& op) {
  if (IsIntTensor(lhs) &&
      op.attributes.compare_type != CompareOp::CompareType::kSigned) {
    return absl::FailedPreconditionError(
        "stablehlo.compare: compare_type must be SIGNED for signed integer "
        "tensors.");
  } else if (IsBoolTensor(lhs) &&
             op.attributes.compare_type != CompareOp::CompareType::kUnsigned) {
    return absl::FailedPreconditionError(
        "stablehlo.compare: compare_type must be UNSIGNED for boolean "
        "tensors.");
  } else if (IsUnsignedIntTensor(lhs) &&
             op.attributes.compare_type != CompareOp::CompareType::kUnsigned) {
    return absl::FailedPreconditionError(
        "stablehlo.compare: compare_type must be UNSIGNED for unsigned integer "
        "tensors.");
  } else if ((IsFloatTensor(lhs) || IsQuantizedPerTensorTensor(lhs)) &&
             (op.attributes.compare_type != CompareOp::CompareType::kFloat &&
              op.attributes.compare_type !=
                  CompareOp::CompareType::kTotalOrder)) {
    return absl::FailedPreconditionError(
        "stablehlo.compare: compare_type must be FLOAT or TOTAL_ORDER for "
...
absl::Status Prepare(CompareOp& op, const Tensor& lhs, const Tensor& rhs,
  ...
  SHLO_REF_RETURN_ON_ERROR(CheckCompareTypeConstraint(lhs, op));
  return absl::OkStatus();
}
