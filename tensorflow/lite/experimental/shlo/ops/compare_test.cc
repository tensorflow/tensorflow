...
struct SupportedOpAttributes<CompareOp> {
  static CompareOp::Attributes Get() {
    return {.comparison_direction = CompareOp::ComparisonDirection::kEq,
            .compare_type = CompareOp::CompareType::kFloat};
  }
};
...
TYPED_TEST(CompareTest, SupportedTestTypesTensorsWork) {
  ...
  auto op = Create(CompareOp::Attributes{
      .comparison_direction = comparison_direction, .compare_type = [&] {
        switch (TypeParam::kStorage) {
          case DataType::kI1:
            return CompareOp::CompareType::kUnsigned;
          case DataType::kSI4:
          case DataType::kSI8:
          case DataType::kSI16:
          case DataType::kSI32:
            return CompareOp::CompareType::kSigned;
          case DataType::kBF16:
          case DataType::kF16:
          case DataType::kF32:
            return CompareOp::CompareType::kFloat;
        }
      }()});
...
TYPED_TEST(QuantizedCompareTest, PerTensorWorks) {
  ...
  Vector<StorageT> lhs_data = {46, 24, -29, 7, 14, 15, 4, -3};
  // RandomBuffer<TypeParam::kStorage>(shape, /*min=*/-50, /*max=*/50);
  Vector<StorageT> rhs_data = {6, 6, 5, 3, 5, 5, 4, 6};
