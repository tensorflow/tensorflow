...
LiteRtStatus RegisterMulOpLegalizations(
    std::vector<std::unique_ptr<Legalization>>& legalizations) {
  legalizations.push_back(std::make_unique<MulOpLegalization>());
  return kLiteRtStatusOk;
}

LiteRtStatus MapGraph(QnnManager& qnn, LiteRtSubgraph subgraph,
                      absl::string_view qnn_graph_name) {
  // Register legalizations.
  std::vector<std::unique_ptr<Legalization>> legalizations;
  LITERT_RETURN_STATUS_IF_NOT_OK(RegisterMulOpLegalizations(legalizations));
...
LiteRtStatus ComposeGraph(QnnManager& qnn, LiteRtSubgraph subgraph,
                          absl::string_view qnn_graph_name) {
  LITERT_RETURN_STATUS_IF_NOT_OK(MapGraph(qnn, subgraph, qnn_graph_name));
  return kLiteRtStatusOk;
}
