...
const int kSliceOpInputSize = 1;
const int kSliceOpOutputSize = 1;
const int kSliceOpParamSize = 1;
// QNN StridedSlice op packs "start", "end", and "stride" into a single tensor
// param "ranges".
const int kRangesParamArgSize = 3;
const int kRangesParamRank = 2;
...
LiteRtStatus SliceOpLegalization::LegalizeOp(LiteRtOpManager& src,
  ...
  if (src_begin_indices.empty() || src_size_indices.empty()) {
    return kLrtStatusErrorInvalidLegalization;
  }
