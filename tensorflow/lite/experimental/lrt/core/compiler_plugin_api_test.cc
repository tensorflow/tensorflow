...
TEST(TestCompilerPluginApi, TestResolveApi) {
  ...
  EXPECT_NE(api.get_supported_soc_model_id, nullptr);
  ...
  ASSERT_STREQ(api.soc_manufacturer(), "Test");
  ...
  ASSERT_STATUS_OK(api.get_supported_soc_model_id(plugin, 0, &soc_model));
  EXPECT_STREQ(soc_model, "MulOp");
