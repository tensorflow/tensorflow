...
LiteRtStatus FinishByteCodeAppend(MutableBufferRef<uint8_t> serialized_model,
  ...
  auto placeholder_tag_start =
      serialized_model.StrView().rfind(kLiteRtAppendedByteCodePrefix);

  if (placeholder_tag_start == absl::string_view::npos) {
    return kLiteRtStatusErrorNotFound;
  }
  placeholder_tag_start += sizeof(kLiteRtAppendedByteCodePrefix);
...
LiteRtResult<std::pair<size_t, size_t>> ParseByteCodeOffsetFromMetadata(
  ...
  absl::string_view kPref = kLiteRtAppendedByteCodePrefix;
  absl::string_view kPlaceholder = kLiteRtAppendedByteCodePlaceholder;
