...
constexpr absl::string_view kTestPluginSearchPath =
    "third_party/tensorflow/lite/experimental/lrt/examples";
...
TEST(PluginManagerTest, LoadTestPlugin) {
  ...
  EXPECT_STREQ(plugins[0].Api()->soc_manufacturer(), "Test");
}
...
TEST(PluginManagerTest, MultipleValidPlugins) {
  ...
  EXPECT_STREQ(plugins[0].Api()->soc_manufacturer(), "Test");
  EXPECT_STREQ(plugins[1].Api()->soc_manufacturer(), "Test");
}

TEST(PluginManagerTest, UnloadLib) {
  std::vector<LrtPluginManager> plugins;
  ...
  ASSERT_EQ(plugins.size(), 1);
  ASSERT_STATUS_OK(plugins.front().FreeLib());
  EXPECT_EQ(plugins.front().Api(), nullptr);
}
