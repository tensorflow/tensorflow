...
CancellationManager::CancellationManager(CancellationManager* parent,
  ...
  is_cancelled_.store(registered, std::memory_order_relaxed);
  is_cancel_requested_.store(registered, std::memory_order_relaxed);
}
...
bool CancellationManager::RegisterCallbackConfig(CancellationToken token,
  ...
  if (is_cancel_requested_.load(std::memory_order_relaxed)) {
    // Fast path to avoid state_ creation.
    return false;
  }

  CallbackBucket& bucket = GetCallbackBucket(token);
  mutex_lock l(bucket.mu);
  // Cancel may be started after the fast path, check `is_cancel_requested_`
  // again while holding the lock, to make sure either:
  // a) the callback is invoked during StartCancel(), or
  // b) this method returns false.
  bool should_register = !is_cancel_requested_.load(std::memory_order_relaxed);
  if (should_register) {
...
bool CancellationManager::DeregisterCallback(CancellationToken token) {
  if (is_cancelled_.load(std::memory_order_relaxed)) {
    // Fast path to avoid state_ creation and notification wait.
    return false;
  }
