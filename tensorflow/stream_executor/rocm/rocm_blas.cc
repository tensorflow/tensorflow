...
port::Status ROCMBlas::DoBlasGemmBatchedInternal(
  ...
  if (bsa_is_constant && bsb_is_constant && bsc_is_constant) {
    bool ok = DoBlasInternal(
        ...
        ROCMComplex(&alpha), a_raw_ptrs[0], lda, bsa, b_raw_ptrs[0], ldb, bsb,
        ROCMComplex(&beta), c_raw_ptrs[0], ldc, bsc, batch_count);
...
bool ROCMBlas::DoBlasGemmBatched(
  ...
  LOG(ERROR) << "rocBLAS does not currently support batched GEMM operation "
             << "for \"Eigen::half\" dataype";
  return false;
}
