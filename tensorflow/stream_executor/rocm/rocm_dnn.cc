...
bool MIOpenSupport::DoBatchNormalizationForward(
  ...
  return DoBatchNormalizationForwardImpl<Eigen::half>(
...
bool MIOpenSupport::DoBatchNormalizationForwardImpl(
    ...
    const dnn::BatchDescriptor& x_desc,
    const dnn::BatchDescriptor& scale_offset_desc, const double epsilon,
    DeviceMemory<T>* y, DeviceMemory<float>* batch_mean, DeviceMemory<float>* batch_var,
    DeviceMemory<float>* saved_mean, DeviceMemory<float>* saved_inv_var,
    bool is_training, std::function<const DeviceMemory<float>&()> var_to_inv_var,
    std::function<void()> inv_var_to_var) {
  mutex_lock lock{dnn_handle_mutex_};
  auto status = wrap::miopenSetStream(parent_, ToHandle(dnn_handle_),
                                     AsROCMStreamValue(stream));
...
bool MIOpenSupport::DoBatchNormalizationBackwardImpl(
    ...
    const DeviceMemory<T>& x, const DeviceMemory<float>& scale,
    const DeviceMemory<float>& mean, const DeviceMemory<float>& variance,
    const dnn::BatchDescriptor& x_desc,
    const dnn::BatchDescriptor& scale_offset_desc, const double epsilon,
    DeviceMemory<T>* x_backprop, DeviceMemory<float>* scale_backprop,
    DeviceMemory<float>* offset_backprop) {
  mutex_lock lock{dnn_handle_mutex_};
  auto status = wrap::miopenSetStream(parent_, ToHandle(dnn_handle_),
                                     AsROCMStreamValue(stream));
