...
#define PERFTOOLS_GPUTOOLS_ROCM_DEFINE_FFT(__type, __fft_type1, __fft_type2, \
  ...
  bool ROCMFft::DoFft(Stream *stream, fft::Plan *plan,                       \
    ...
    /* ROCM TODO disable for now until rocFFT properly honors HIP complex    \
     * types */                                                              \
  ...
  bool ROCMFft::DoFft(Stream *stream, fft::Plan *plan,                       \
    ...
    /* ROCM TODO disable for now until rocFFT properly honors HIP complex    \
     * types */                                                              \
    /* return DoFftInternal(stream, plan, wrap::hipfftExec##__fft_type2,     \
     * input, */                                                             \
    /*                     output);                                       */ \
    return false;                                                            \
  }                                                                          \
  bool ROCMFft::DoFft(Stream *stream, fft::Plan *plan,                       \
    ...
    /* ROCM TODO disable for now until rocFFT properly honors HIP complex    \
     * types */                                                              \
    /* return DoFftInternal(stream, plan, wrap::hipfftExec##__fft_type3,     \
     * input, */                                                             \
    /*                     output);                                       */ \
    return false;                                                            \
  }
