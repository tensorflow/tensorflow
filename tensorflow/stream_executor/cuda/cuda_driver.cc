...
string ToString(CUresult result) {
  ...
  if (dynload::cuGetErrorName(result, &error_name)) {
    return absl::StrCat("UNKNOWN ERROR (", static_cast<int>(result), ")");
  }
  const char *error_string;
  if (dynload::cuGetErrorString(result, &error_string)) {
    return error_name;
  }
...
/* static */ bool CUDADriver::GetDeviceName(CUdevice device,
  ...
  CUresult res = dynload::cuDeviceGetName(chars.begin(), kCharLimit - 1, device);
  if (res != CUDA_SUCCESS) {
...
/* static */ port::Status CUDADriver::CreateContext(
  ...
  *context = nullptr;
  ...
  CHECK_EQ(CUDA_SUCCESS, dynload::cuDevicePrimaryCtxGetState(
  ...
  if (former_primary_context_flags != flags) {
    if (former_primary_context_is_active) {
      ...
      CHECK_EQ(CUDA_SUCCESS,
               dynload::cuDevicePrimaryCtxSetFlags(device, flags));
    }
  }
...
/* static */ bool CUDADriver::LoadPtx(CudaContext* context,
  ...
  GetDriverExecutor()->Schedule([context, ptx_contents, module, &ret,
    ...
    CUresult res;
    {
      ...
      res = dynload::cuModuleLoadDataEx(module, ptx_data, TF_ARRAYSIZE(options),
...
/* static */ bool CUDADriver::SynchronousMemsetUint8(CudaContext* context,
  ...
  ScopedActivateContext activation(context);
  CUresult res = dynload::cuMemsetD8(location, value, size);
  if (res != CUDA_SUCCESS) {
...
/* static */ bool CUDADriver::SynchronousMemsetUint32(CudaContext* context,
                                                      ...
                                                      size_t uint32_count) {
  ScopedActivateContext activation(context);
  CUresult res = dynload::cuMemsetD32(location, value, uint32_count);
  if (res != CUDA_SUCCESS) {
    LOG(ERROR) << "failed to memset memory: " << ToString(res);
    return false;
  }
...
/* static */ bool CUDADriver::GetModuleSymbol(CudaContext* context,
  ...
  CUresult res = dynload::cuModuleGetGlobal(dptr, bytes, module, symbol_name);
  if (res != CUDA_SUCCESS) {
...
/* static */ void CUDADriver::DestroyStream(CudaContext* context,
  ...
  ScopedActivateContext activated{context};
  CUresult res = dynload::cuStreamDestroy(*stream);
  if (res != CUDA_SUCCESS) {
...
/* static */ void *CUDADriver::DeviceAllocate(CudaContext *context,
  ...
  CUresult res = dynload::cuMemAlloc(&result, bytes);
  if (res != CUDA_SUCCESS) {
...
/* static */ void CUDADriver::DeviceDeallocate(CudaContext* context,
  ...
  CUresult res = dynload::cuMemFree(pointer);
  if (res != CUDA_SUCCESS) {
...
/* static */ void *CUDADriver::UnifiedMemoryAllocate(CudaContext *context,
  ...
  CUresult res =
      dynload::cuMemAllocManaged(&result, bytes, CU_MEM_ATTACH_GLOBAL);
  if (res != CUDA_SUCCESS) {
...
/* static */ port::Status CUDADriver::DestroyEvent(CudaContext* context,
  ...
  ScopedActivateContext activated{context};
  CUresult res = dynload::cuEventDestroy(*event);
  *event = nullptr;
...
/* static */ port::Status CUDADriver::SynchronousMemcpyD2H(CudaContext *context,
                                                           ...
                                                           uint64 size) {
  ScopedActivateContext activation(context);
  CUresult res = dynload::cuMemcpyDtoH(host_dst, gpu_src, size);
  if (res != CUDA_SUCCESS) {
...
/* static */ port::Status CUDADriver::SynchronousMemcpyH2D(CudaContext *context,
                                                           ...
                                                           uint64 size) {
  ScopedActivateContext activation(context);
  CUresult res = dynload::cuMemcpyHtoD(gpu_dst, host_src, size);
  if (res != CUDA_SUCCESS) {
...
/* static */ port::Status CUDADriver::SynchronousMemcpyD2D(CudaContext *context,
                                                           ...
                                                           uint64 size) {
  ScopedActivateContext activation(context);
  CUresult res = dynload::cuMemcpyDtoD(gpu_dst, gpu_src, size);
  if (res != CUDA_SUCCESS) {
    return port::InternalError(port::Printf(
        "failed to synchronous memcpy from host to device: %s; GPU dst: %p; "
...
/* static */ bool CUDADriver::AsynchronousMemcpyD2H(CudaContext* context,
                                                    ...
                                                    CUstream stream) {
  ScopedActivateContext activation(context);
  CUresult res = dynload::cuMemcpyDtoHAsync(host_dst, gpu_src, size, stream);
  if (res != CUDA_SUCCESS) {
...
/* static */ bool CUDADriver::AsynchronousMemcpyH2D(CudaContext* context,
                                                    ...
                                                    CUstream stream) {
  ScopedActivateContext activation(context);
  CUresult res = dynload::cuMemcpyHtoDAsync(gpu_dst, host_src, size, stream);
  if (res != CUDA_SUCCESS) {
    LOG(ERROR) << port::Printf(
...
/* static */ bool CUDADriver::AsynchronousMemcpyD2D(CudaContext* context,
                                                    ...
                                                    CUstream stream) {
  ScopedActivateContext activation(context);
  CUresult result = dynload::cuMemcpyDtoDAsync(gpu_dst, gpu_src, size, stream);
  if (result != CUDA_SUCCESS) {
    LOG(ERROR) << port::Printf(
...
/* static */ port::StatusOr<int> CUDADriver::GetDeviceAttribute(
  ...
  int val;
  CUresult res = dynload::cuDeviceGetAttribute(&val, attribute, device);
  if (res != CUDA_SUCCESS) {
    return port::Status(
        port::error::INTERNAL,
...
/* static */ bool CUDADriver::GetDeviceMemoryInfo(CudaContext* context,
  ...
  size_t free = 0;
  size_t total = 0;
  CUresult res = dynload::cuMemGetInfo(&free, &total);
  if (res != CUDA_SUCCESS) {
...
/* static */ bool CUDADriver::GetDeviceTotalMemory(CUdevice device,
                                                   uint64 *result) {
  size_t value = -1;
  CUresult res = dynload::cuDeviceTotalMem(&value, device);
  if (res != CUDA_SUCCESS) {
