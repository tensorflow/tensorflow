    ...
    for dev_stat in run_meta.step_stats.dev_stats:
        for node_stat in dev_stat.node_stats:
            ...
            for i, node_stat_out in enumerate(node_stat.output):
                ...
                try:
                    op.outputs[i].set_shape(
                        op.outputs[i].get_shape().merge_with(node_stat_shape)
                    )
                except ValueError as e:
                    sys.stderr.write(
                        "Node %s incompatible shapes: %s.\n" % (node_stat.node_name, e)
                    )
    return graph
        ...
        if add_trace:
            if op.traceback:
                for filename, lineno, funcname, line in op.traceback:
                    ...
                    trace.function_id = (
                        _str_id(funcname, string_to_id) if funcname else 0
                    )
...
def merge_default_with_oplog(
    graph, op_log=None, run_meta=None, add_trace=True, add_trainable_var=True
):
