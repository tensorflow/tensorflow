...
class TPUEmbeddingLayerV2Test(parameterized.TestCase, test.TestCase):
    ...
    def testTwoTablesStackedHaveCorrectInitialValues(self):
        table1_initial_value = np.arange(start=100, stop=120, dtype=np.float32).reshape(
            [10, 2]
        )
        ...
        table2_initial_value = np.arange(start=200, stop=240, dtype=np.float32).reshape(
            [20, 2]
        )
    ...
    def testCpuRestoreForNoStackedTables(self):
        ...
        table2_initial_value = np.arange(start=200, stop=380, dtype=np.float32).reshape(
            [20, 9]
        )
        table2 = tpu_embedding_v2_utils.TableConfig(
            vocabulary_size=20,
    ...
    def testCpuRestoreForStackedTables(self):
        ...
        saved_model_path = os.path.join(self.create_tempdir().full_path, "saved_model")
        tf_save.save(serving_embedding, saved_model_path)
    ...
    def testUnshardedToTpuRestore(self):
        ...
        total_sc_shards = replicas * cores_per_replica * mid_level_api._num_sc_per_chip
        ...
        padded_vocab = get_padded_vocab_size(total_sc_shards, table1.vocabulary_size)
