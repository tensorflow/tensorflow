...
class DeviceAssignment(object):
    ...
    def __init__(self, topology: Topology, core_assignment: np.ndarray):
        ...
        if not isinstance(topology, Topology):
            raise ValueError(
                "topology must be a Topology object, got {}".format(type(topology))
            )
        ...
        if core_assignment.ndim != 3:
            raise ValueError(
                "core_assignment must be a rank 3 numpy array, "
    ...
    def lookup_replicas(self, task_id: int, logical_core: int) -> List[int]:
        ...
        try:
            ...
            raise ValueError(
                "Can not find any replica in task: {} contains logical_core: {} ".format(
    ...
    def host_device(
        self, replica: int = 0, logical_core: int = 0, job: Optional[str] = None
    ) -> str:
...
def _ring_3d(x_size: int, y_size: int, z_size: int) -> List[Tuple[int, int, int]]:
    ...
    if (
        (x_size > 1 and x_size % 2 != 0)
        or (y_size > 1 and y_size % 2 != 0)
        or (z_size > 1 and z_size % 2 != 0)
    ):
...
def device_assignment(
    ...
    if computation_shape is None:
        ...
        computation_shape = numpy_compat.np_asarray(computation_shape, dtype=np.int32)

    if computation_stride is None:
        computation_stride = np.array([1] * topology_rank, dtype=np.int32)
    else:
        computation_stride = numpy_compat.np_asarray(computation_stride, dtype=np.int32)
    ...
    if topology.missing_devices.size == 0:
        ...
        if num_replicas > 0:
            ...
            for x, ni in sorted(
                ((x, ((i + 1) % topology_rank)) for (i, x) in enumerate(replica_counts))
            ):
                i = (ni + topology_rank - 1) % topology_rank
                target_size = int(
                    math.ceil(remaining_replicas ** (1.0 / remaining_dims))
                )
        ...
        enable_3d_tiling = (
            topology_rank == 4
            and computation_shape[-1] == mesh_shape[-1]  # Only handle 3D case.
        ...
        if enable_3d_tiling:
            assignment = []
            inner_ring = _ring_3d(
                computation_shape[0], computation_shape[1], computation_shape[2]
            )
            ...
            indices = [
                np.arange(
                    0,
                    computation_shape[i] * computation_stride[i],
                    computation_stride[i],
                )
                for i in range(topology_rank)
            ]
        ...
        assert (
            ...
            <= topology.num_tasks * topology.num_tpus_per_task
        )
