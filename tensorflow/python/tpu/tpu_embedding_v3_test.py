...
def create_input_data_based_on_hw_requirement(
    ...
    per_sc_per_minibatch_id_nums_for_each_replica = np.random.randint(
        max_unique_ids_per_partition * (num_minibatches_per_physical_sparse_core - 1)
        + 1,
        max_unique_ids_per_partition * num_minibatches_per_physical_sparse_core + 1,
...
class TPUEmbeddingV3Test(parameterized.TestCase, test.TestCase):
    ...
    def test_single_feature_single_table_backwards_pass_with_csr_input(self):
        ...
        golden = np.ones([self.vocabulary_size, self.embedding_dim], dtype=np.float32)
        for i in range(1024):
    ...
    def test_two_feature_single_table_backwards_pass_with_csr_input(self):
        ...
        with strategy.scope():
            ...
            mid_level_api.build()
        random1 = np.random.uniform(size=(16, self.embedding_dim)).astype(np.float32)
        random2 = np.random.uniform(size=(16, self.embedding_dim)).astype(np.float32)
    ...
    def test_two_feature_two_tables_stacked_backwards_pass_with_csr_input(self):
        ...
        goldens = [
            np.full([self.vocabulary_size, self.embedding_dim], 2, dtype=np.float32),
