...
def _enclosing_tpu_device_assignment() -> Optional[
...
class TPUReplicateContext(control_flow_ops.XLAControlFlowContext):
    ...
    def get_replicated_var_handle(
        ...
        name: Text,
        handle_id: Text,
        vars_: Union[List[core_types.Tensor], List[variables.Variable]],
    ...
    def report_unsupported_operations(self) -> None:
        if self._unsupported_ops:
            ...
            logging.warning(
                "%d unsupported operations found: \n%s",
                len(self._unsupported_ops),
                op_str,
            )
            if len(self._unsupported_ops) > _MAX_WARNING_LINES:
                logging.warning(
                    "... and %d more", (len(self._unsupported_ops) - _MAX_WARNING_LINES)
                )
    ...
    def EnterGradientColocation(self, op: ops.Operation, gradient_uid: Text):
        if op is not None:
            if ops.get_default_graph()._control_flow_context is None:  # pylint: disable=protected-access
                ...
                try:
                    outside_attr = op.get_attr(_OUTSIDE_COMPILATION_ATTR).decode(
                        "ascii"
                    )
                except ValueError:
    ...
    def _EnterOutsideCompilationScope(
        ...
        if (
            device.device_type == "TPU_REPLICATED_CORE"
            and device.device_index is not None
        ):
            self._host_compute_core.append(
                self._outside_compilation_cluster + ":" + str(device.device_index)
            )
    ...
    def AddOp(self, op: ops.Operation) -> None:
        ...
        if (
            _TPU_REPLICATE_ATTR in op.node_def.attr
            and "_cloned" not in op.node_def.attr
        ):
...
def _enclosing_tpu_context_and_graph() -> Tuple[Any, Any]:
    ...
    raise ValueError(
        "get_replicated_var_handle() called without "
