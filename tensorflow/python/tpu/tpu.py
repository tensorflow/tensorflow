...
from tensorflow.core.protobuf.tpu import (
    tpu_embedding_configuration_pb2 as embedding_pb2,
)
...
def initialize_system(
    ...
    """
    config_string = (
        "" if embedding_config is None else embedding_config.SerializeToString()
    )
...
def tpu_replicated_input_resolver(
    ...
    resource_writes: object_identity.ObjectIdentitySet,
) -> bool:
    ...
    return bool(
        replace_with_unreplicated_resources(resource_reads)
        or replace_with_unreplicated_resources(resource_writes)
    )
...
class TPUReplicateContext(control_flow_ops.XLAControlFlowContext):
    ...
    def EnterGradientColocation(self, op: ops.Operation, gradient_uid: Text):
        if op is not None:
            if (
                ops.get_default_graph()._control_flow_context is None
            ):  # pylint: disable=protected-access
    ...
    def _EnterOutsideCompilationScope(self, cluster: Optional[Text] = None):
        ...
        self._oc_dev_fn_stack = (
            graph._device_function_stack
        )  # pylint: disable=protected-access
        graph._device_function_stack = (
    ...
    def AddOp(self, op: ops.Operation) -> None:
        ...
        op._set_attr_with_buf(_TPU_REPLICATE_ATTR, self._tpu_relicate_attr_buf.buffer)
        if self._outside_compilation_cluster:
            op._set_attr(
...
def outside_compilation(computation: Callable[..., Any], *args, **kwargs) -> Any:
    ...
    # outside_compilation
    initial_context = (
        graph._get_control_flow_context()
    )  # pylint: disable=protected-access
...
class XLAOptions(
    ...
    def __new__(
        cls, use_spmd_for_xla_partitioning=True, enable_xla_dynamic_padder=True
    ):
        return super(XLAOptions, cls).__new__(
            cls, use_spmd_for_xla_partitioning, enable_xla_dynamic_padder
        )
    ...
    for core_idx, inputs_per_core in enumerate(inputs):
        for idx, input_tensor in enumerate(inputs_per_core):
            ...
            if core_idx == 0:
                ...
                maximum_static_shapes[idx] = max(
                    input_shape, maximum_static_shapes[idx]
                )
            ...
            real_input_shape.op._set_attr(  # pylint: disable=protected-access
                _POST_DEVICE_REWRITE_ATTR, attr_value_pb2.AttrValue(b=True)
            )
                ...
                for i, s in enumerate(padded_shape.dims):
                    if need_padding[idx][i]:
                        ...
                        if s.value is not None:
                            ...
                            # among all the cores.
                            max_dim_size = math_ops.maximum(
                                maximum_shapes[idx][i], minimum_dynamic_dim_size
                            )
...
def _flatten_and_filter_composite(
    maybe_composite, non_composite_output, composite_output=None
):
...
def split_compile_and_replicate(
    ...
    if device_assignment is not None:
        ...
        # operator attribute.
        metadata_kwargs = {
            "topology": device_assignment.topology.serialized(),
            "device_assignment": device_assignment.core_assignment.flatten().tolist(),
        }
        metadata_kwargs[
    ...
    if config.get_soft_device_placement():
        logging.info(
            "Automatic outside compilation is enabled. "
    ...
    for i in range(num_replicas):
        if len(inputs[i]) != input_arity:
            raise ValueError(
                "Replicas must have the same number of inputs. "
    ...
    if maximum_shapes:
        ...
        flat_maximum_shapes = nest.flatten(
            [
                ...
                for x, y in zip(nest.flatten(inputs[0]), nest.flatten(maximum_shapes))
            ]
        )
        flat_maximum_shapes = [
        ...
        nest.assert_same_structure(
            flat_inputs[0], flat_maximum_shapes, check_types=False
        )

        unpadded_inputs = flat_inputs
        flat_inputs, padding_maps = _pad_all_input(
            unpadded_inputs, flat_maximum_shapes, padding_spec
        )
        if padding_maps:
    ...
    for i in range(0, len(flat_inputs[0])):
        ...
        flat_replicated_inputs.append(
            tpu_ops.tpu_replicated_input(replicas, name="input{}".format(i), index=i)
        )
    if isinstance(graph, func_graph.FuncGraph):
    ...
    pivot._set_attr(
        _PIVOT_FOR_CLUSTER,  # pylint: disable=protected-access
    ...
    try:
        ...
        with tpu_function.tpu_shard_context(num_replicas), ops.control_dependencies(
            [metadata]
        ):
            ...
            if dynamic_shape_inputs and xla_options.enable_xla_dynamic_padder:
                for padding_map in padding_maps:
                    ...
                    flat_replicated_inputs[
                        padding_map.arg_index
                    ] = tf2xla.set_dynamic_dimension_size(
            ...
            for i, composite in zip(flat_replicated_inputs, is_composite):
                ...
                if not dynamic_shape_inputs or composite:
                    i.op._set_attr(
                        "_tpu_input_identity", attr_value_pb2.AttrValue(b=True)
                    )
                # pylint: enable=protected-access

            # Clobber replicated placeholders with Nones.
            computation_inputs = [
                None if inp is None else replicated
                for replicated, inp in zip(
        ...
        need_spmd_partitioning = (
            ...
            and device_assignment.num_cores_per_replica > 1
        )
        ...
        if outputs_is_flat:
            ...
            output_tensors, control_deps, pack_template = _postprocess_non_flat_outputs(
                outputs, need_spmd_partitioning
            )
        ...
        if tensor_tracer.TensorTracer.is_enabled():
            if tf2.enabled():
                logging.warn(
                    "TF API ver >= 2.0 detected. " "Tensor Tracer v1 is not enabled."
                )
            else:
                tt = tensor_tracer.TensorTracer()
                output_tensors = tt.trace_tpu(
                    ops.get_default_graph(), output_tensors, control_deps, num_replicas
                )
    ...
    with ops.control_dependencies([metadata]):
        if use_tpu:
            ...
            op._set_attr(
                _TPU_COMPILATION_STATUS_ATTR, attr_value
            )  # pylint: disable=protected-access
        else:
...
def _postprocess_flat_outputs(
    outputs: Any, need_spmd_partitioning: bool
    ...
    try:
        if need_spmd_partitioning:
            outputs = [
                o if isinstance(o, ops.Operation) else maybe_convert(o) for o in outputs
            ]
        else:
...
def split_compile_and_shard(
    ...
    if not isinstance(inputs, list):
        raise TypeError(
            "tpu.shard()'s inputs must be a list of Tensors or None. "
            f"Received {type(inputs)}"
        )
    ...
    results = []
    for (axis, all_shards, x) in zip(
        output_shard_axes, outputs_from_all_shards, zip(*outputs)
    ):
        if all_shards:
