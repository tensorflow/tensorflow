...
class AsyncCheckpointSaverHook(basic_session_run_hooks.CheckpointSaverHook):
    ...
    def __init__(
        ...
        listeners: Optional[
            List[basic_session_run_hooks.CheckpointSaverListener]
        ] = None,
    ):
    ...
    def after_run(
        self, run_context: session_run_hook.SessionRunContext, run_values: Any
    ):
    ...
    def _save(self, session, step, asynchronous=True):
        ...
        def _save_fn():
            ...
            logging.info(
                "Checkpoint actual writing time: (%.3f sec)", end_time - start_time
            )
