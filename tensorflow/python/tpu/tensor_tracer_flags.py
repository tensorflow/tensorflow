...
TT_CHECK_FILTER = flags.DEFINE_bool(
    "tt_check_filter", default=False, help="Terminate early to check op name filtering."
)
...
class TTParameters(object):
    ...
    def __init__(self, env=None):
        ...
        self.use_compact_trace = self.trace_mode in (
            TRACE_MODE_NAN_INF,
            TRACE_MODE_NORM,
            TRACE_MODE_HISTORY,
            TRACE_MODE_MAX_ABS,
        ...
        if self.collect_summary_per_core:
            logging.warning(
                "Aggregate signatures are approximate for mean, variance and sparsity."
            )
    ...
    def _check_flag_errors(self):
        if self.trace_mode in (TRACE_MODE_SUMMARY, TRACE_MODE_FULL_TENSOR_SUMMARY):
            if not self.trace_dir:
                raise ValueError(
                    "trace_dir must be explicitly provided in "
    ...
    def get_signature_to_agg_fn_map(self):
        ...
        # variance.
        return {
            TRACE_MODE_NORM: linalg_ops.norm,
            ...
            TT_SUMMARY_MAX_ABS: lambda t, axis=0: math_ops.reduce_max(
                math_ops.abs(t),  # pylint: disable=g-long-lambda
                axis=axis,
            ),
    ...
    def get_flag_value(self, wanted_flag_name):
        ...
        while True:
            ...
            pos = match.end()
        raise RuntimeError(
            "Invalid tensor tracer flag. Could not recognize %s." % flag_name
        )
