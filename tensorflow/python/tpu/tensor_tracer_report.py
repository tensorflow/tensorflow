    ...
    def _create_tensor_maps(self):
        ...
        for out_tensor in self.traced_tensors:
            ...
            if tensor_name in self.tensorname_to_cache_idx:
                raise ValueError(
                    "Tensor name {} should not be already in "
                    "tensorname_to_cache_idx".format(tensor_name)
                )
            if tensor_name not in self.graph_order.tensor_to_idx:
                raise ValueError(
                    "Tensor name {} is not in the tensor_to_idx, tensor_to_idx={} ".format(
                        tensor_name, self.graph_order.tensor_to_idx
                    )
                )
            ...
            if len(self.tensorname_to_cache_idx) != len(self.cache_idx_to_tensor_idx):
                raise RuntimeError(
                    ...
                    "len(self.cache_idx_to_tensor_idx)={}".format(
                        len(self.tensorname_to_cache_idx),
...
def sort_tensors_and_ops(graph):
    ...
    graph_wrapper = collections.namedtuple(
        "GraphWrapper",
        [
            "graph",
            "operations",
...
class TTReportHandle(object):
    ...
    def create_report_proto(
        ...
        for signature_name, _ in sorted(
            collected_signature_types.items(), key=lambda x: x[1]
        ):
        ...
        for tensor in tensor_trace_order.graph_order.tensors:
            ...
            if tensor.name in tensor_trace_order.tensorname_to_cache_idx:
                ...
                tensor_def.cache_index = tensor_trace_order.tensorname_to_cache_idx[
                    tensor.name
                ]
            else:
    ...
    def create_report(
        self, tt_config, tt_parameters, tensor_trace_order, tensor_trace_points
    ):
            ...
            for out_tensor in op.outputs:
                if out_tensor.name not in graph_order.tensor_to_idx:
                    raise ValueError(
                        ...
                        "tensor_to_idx={}".format(
                            out_tensor.name, graph_order.tensor_to_idx
                        )
                    )
            ...
            for consumer_op in consumers:
                if consumer_op.name not in graph_order.op_to_idx:
                    raise ValueError(
                        ...
                        "got consumer_op={}, op_to_idx={}".format(
                            consumer_op.name, graph_order.op_to_idx
                        )
                    )
