...
class TPUEmbeddingCheckpointTest(tpu_embedding_base_test.TPUEmbeddingBaseTest):
    ...
    def test_checkpoint_save_and_restore(self):
        ...
        with strategy.scope():
            ...
            feature_config = (
                tpu_embedding_v2_utils.FeatureConfig(table=table, name="feature"),
            )
    ...
    def test_model_export_cpu(self):
        ...
        def serve_tensors(features):
            features = tpu_embedding_for_serving.cpu_embedding_lookup(
                features,
                None,
                cpu_mid_level.embedding_tables,
                cpu_mid_level._feature_config,
            )
            return features[0]

        signatures = {
            "serving_default": serve_tensors.get_concrete_function(
                (
        ...
        self.assertAllClose(
            prediction.numpy(), first_mid_level_contents[input_feature_value]
        )
