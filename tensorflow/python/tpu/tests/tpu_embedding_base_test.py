...
class TPUEmbeddingBaseTest(parameterized.TestCase, test.TestCase):
    ...
    def setUp(self):
        ...
        def create_hight_dimensional_indices(indices):
            ...
            batch_size_index = np.repeat(
                np.arange(self.data_batch_size), len(indices)
            ).reshape(-1, 1)
        ...
        self.feature_watched_indices_high_dimensional = (
            create_hight_dimensional_indices(self.feature_watched_indices)
        )
        self.feature_watched_values_high_dimensional = (
            self.feature_watched_values * self.data_batch_size
        )
        self.feature_watched_row_lengths_high_dimensional = (
            self.feature_watched_row_lengths * self.data_batch_size
        )

        # Create high dimensional features with shape(4, 4, 2)
        self.feature_favorited_indices_high_dimensional = (
            create_hight_dimensional_indices(self.feature_favorited_indices)
        )
        self.feature_favorited_values_high_dimensional = (
            self.feature_favorited_values * self.data_batch_size
        )
        self.feature_favorited_row_lengths_high_dimensional = (
            self.feature_favorited_row_lengths * self.data_batch_size
        )

        # Create high dimensional features with shape(4, 4, 3)
        self.feature_friends_indices_high_dimensional = (
            create_hight_dimensional_indices(self.feature_friends_indices)
        )
        self.feature_friends_values_high_dimensional = (
            self.feature_friends_values * self.data_batch_size
        )
        self.feature_friends_row_lengths_high_dimensional = (
            self.feature_friends_row_lengths * self.data_batch_size
        )
    ...
    def _create_strategy_and_mid_level(
        self, optimizer_name
    ) -> Tuple[
    ...
    def _create_high_dimensional_sparse_dataset(
        self, strategy, include_weights=False, weight=0.5
    ):
        sparse_features = (
            sparse_tensor.SparseTensor(
    ...
    def _create_high_dimensional_ragged_dataset(
        self, strategy, include_weights=False, weight=0.5
    ):
        ragged_features = (
    ...
    def _create_dense_dataset(self, strategy, include_weights=False, weight=0.5):
        features = (
            ...
            constant_op.constant(
                self.feature_friends_values[: self.data_batch_size], dtype=dtypes.int32
            ),
        )
        if include_weights:
            weights = [
                array_ops.ones_like(t, dtype=dtypes.float32) * weight for t in features
            ]
    ...
    def _create_high_dimensional_dense_dataset(
        self, strategy, include_weights=False, weight=0.5
    ):
    ...
    def _check_results(
        ...
        if is_high_dimensional:
            activation_watched_gold = np.stack(
                [activation_watched_gold] * self.batch_size * num_replicas
            )

            activation_favorited_gold = np.stack(
                [activation_favorited_gold] * self.batch_size * num_replicas
            )

            activation_friends_gold = np.stack(
                [activation_friends_gold] * self.batch_size * num_replicas
            )
        else:
            if num_replicas == 1:
        ...
        if training:
            ...
            gradients_wrt_user = self._compute_gradients_wrt_embedding_table(
            ...
            gradients_wrt_video = self._compute_gradients_wrt_embedding_table(
                gradient_wrt_favorited_gold,
    ...
    def _check_embedding_and_slot_variables_for_adagrad_momentum(
        ...
        if optimizer.beta2 == 1.0:
            ...
            accumulator = (
                optimizer.beta2 * accumulator + (1 - optimizer.beta2) * gradients**2
            )
        accumulator_power = np.power(
            accumulator + optimizer.epsilon, -1.0 / optimizer.exponent
        )
        ...
        self.assertAllClose(
            self._get_variable(variable["accumulators"]).numpy(), accumulator, rtol=1e-3
        )
        self.assertAllClose(
    ...
    def _compute_loss(
        self, activation_watched, activation_favorited, activation_friends
    ):
