...
class TPUEmbeddingCorrectnessTest(
    ...
    def test_sequence_embeddings(self, sparse):
        feature_config = (
            ...
            tpu_embedding_v2_utils.FeatureConfig(
                table=self.table_video, name="favorited", max_sequence_length=2
            ),
            tpu_embedding_v2_utils.FeatureConfig(
                table=self.table_user, name="friends", max_sequence_length=3
            ),
        )
        ...
        def embedding_and_set_gradients(data):
            def tpu_fn():
                ...
                mid_level.apply_gradients(
                    nest.map_structure(array_ops.ones_like, activations)
                )
                return activations
        ...
        golden = tuple(
            [
                before - update * mask
                for before, update, mask in zip(before_update, per_row_update, masks)
            ]
        )
