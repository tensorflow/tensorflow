...
class TPUEmbedding(tracking.AutoTrackable):
    ...
    def _create_config_proto(
        ...
        for feature in nest.flatten(self._feature_config):
            num_features[feature.table] += (
                1 if feature.max_sequence_length == 0 else feature.max_sequence_length
            )
    ...
    def _compute_per_table_gradients(self, gradients) -> Dict[Text, List[core.Tensor]]:
    ...
    def dequeue(self, name: Optional[Text] = None):
        ...
        table_to_activation = {
            table: array_ops.reshape(
                activation, [self._batch_size, num_features[table], -1]
            )
        ...
        for feature in nest.flatten(self._feature_config):
            ...
            if feature.max_sequence_length == 0:
                per_feature_activations.append(activation[:, feature_index, :])
                table_to_position[feature.table] += 1
            else:
                per_feature_activations.append(
                    activation[
                        :,
                        feature_index : (feature_index + feature.max_sequence_length),
                        :,
                    ]
                )
        ...
        for inp, weight, (path, feature) in zip(
            ...
            if isinstance(inp, ops.Tensor):
                self._add_data_for_tensor(
                    inp, weight, indices, values, weights, int_zeros, float_zeros, path
                )
    ...
    def enqueue(
        ...
        if not self._verify_batch_size_on_enqueue:
            ...
            if batch_size is None and not self._built:
                raise RuntimeError(
                    "Unable to determine batch size from input features."
            ...
            if batch_size is not None:
                ...
                if self._batch_size != batch_size:
                    raise ValueError(
                        "Multiple calls to enqueue with different batch "
                        "sizes {} and {}.".format(self._batch_size, batch_size)
                    )
    ...
    def _get_batch_size(self, tensors, in_tpu_context: bool):
        ...
        for path, maybe_tensor in nest.flatten_with_joined_string_paths(tensors):
            ...
            if not in_tpu_context:
                ...
                for replica_id in range(self._strategy.num_replicas_in_sync):
                    tensor_list.append(
                        distribute_utils.select_replica(replica_id, maybe_tensor)
                    )
            else:
            ...
            for tensor in tensor_list:
                ...
                if shape[0] is not None:
                    if batch_size is None:
                        ...
                        raise ValueError(
                            "Found multiple batch sizes {} and {}. All inputs "
    ...
    for inp, weight, (path, feature) in zip(flat_inputs, flat_weights, flat_features):
        ...
        if isinstance(inp, ops.Tensor):
            ...
            if feature.max_sequence_length > 0:
                ...
                outputs.append(
                    _ragged_embedding_lookup_with_reduce(
