...
def _add_key_attr(op, name):
    op._set_attr(
        _NAME_KEY, attr_value_pb2.AttrValue(s=compat.as_bytes(name))
...
class TPUEmbedding(tracking.AutoTrackable):
    ...
    def __init__(
        ...
        feature_config: Union[
            tpu_embedding_v2_utils.FeatureConfig, Iterable
        ...
        for i, table in enumerate(self._table_config):
            ...
            if (table.optimizer is not None or self._using_tpu) and not isinstance(
                table.optimizer, tpu_embedding_v2_utils._Optimizer
            ):  # pylint: disable=protected-access
                raise ValueError(
                    "{} is an unsupported optimizer class. Please pass an "
                    "instance of one of the optimizer classes under "
                    "tf.tpu.experimental.embedding.".format(type(table.optimizer))
                )
    ...
    def build(
        self, per_replica_input_shapes=None, per_replica_batch_size=None
    ):  # pylint:disable=g-bare-generic
        ...
        if self._using_tpu:
            ...
            self._get_and_update_output_shapes_from_input(
                per_replica_input_shapes, per_replica_batch_size
            )
    ...
    def _maybe_build(
        self, output_shapes: Optional[Union[List[int], Iterable]] = None
    ):  # pylint:disable=g-bare-generic
        if not self._built:
    ...
    def _get_output_shapes_from_input_shapes(
        ...
        for input_shape in input_shapes:
            if input_shape.rank is None or input_shape.rank < 2:
                raise ValueError(
                    "Received input tensor of shape {}. Rank must be 2 and above".format(
                        input_shape
                    )
                )
    ...
    def embedding_tables(
        ...
        if self._using_tpu:
            ...
            raise RuntimeError(
                "Unable to retrieve embedding tables when using a TPU "
    ...
    def _create_config_proto(
        ...
        tensor_core_batch_size = self._get_tensor_core_batch_size(self._output_shapes)
        ...
        for i, feature in enumerate(nest.flatten(self._feature_config)):
            num_features[feature.table] += (
                self._get_reduce_prod(self._output_shapes[i]) // tensor_core_batch_size
            )
        ...
        for table in self._table_config:
            ...
            table_descriptor.vocabulary_size = max(
                table.vocabulary_size, self._strategy.extended.num_hosts
            )
            ...
            # rates.
            if callable(table.optimizer.learning_rate):
                parameters.learning_rate.dynamic.tag = learning_rate_index[
                    table.optimizer.learning_rate
                ]
            else:
            ...
            table.optimizer._set_optimization_parameters(
                parameters
            )  # pylint: disable=protected-access
    ...
    def apply_gradients(self, gradients, name: Optional[Text] = None):
        ...
        if not self._using_tpu:
            raise RuntimeError(
                "apply_gradients is not valid when TPUEmbedding "
                "object is not created under a TPUStrategy."
            )

        if not self._built:
            raise RuntimeError(
                "apply_gradients called on unbuilt TPUEmbedding "
        ...
        for (path, gradient), feature, output_shape in zip(
            ...
            if gradient is not None:
                if gradient.shape != full_output_shape:
                    raise ValueError(
                        "Found gradient of shape {} at path {}. Expected "
                        "shape {}.".format(gradient.shape, path, full_output_shape)
                    )
            else:
    ...
    def dequeue(self, name: Optional[Text] = None):
        ...
        if not self._using_tpu:
            raise RuntimeError(
                "dequeue is not valid when TPUEmbedding object is not "
                "created under a TPUStrategy."
            )

        if not self._built:
            raise RuntimeError(
                "dequeue called on unbuilt TPUEmbedding object. "
    ...
    def _create_variables_and_slots(
        ...
        def create_variables(table):
            ...
            parameters = variable_creator(
                table.name, table.initializer, trainable=not self._using_tpu
            )
            ...
            if table.optimizer is not None:
                slot_vars = table.optimizer._create_slots(
                    parameters, slot_creator
                )  # pylint: disable=protected-access
            else:
                slot_vars = {}
    ...
    def _load_variables(self):
        ...
        if (
            self._using_tpu
            and self._built
            and not (not context.executing_eagerly() and save_context.in_save_context())
        ):
            _load_variables_impl(
    ...
    def _gather_saveables_for_checkpoint(
        ...
        def factory(name=_HOOK_KEY):
            return TPUEmbeddingSaveable(
                name, self._load_variables, self._retrieve_variables
            )

        return {_HOOK_KEY: factory}

    # Some helper functions for the below enqueue function.
    def _add_data_for_tensor(
        self, tensor, weight, indices, values, weights, int_zeros, float_zeros, path
    ):
    ...
    def _add_data_for_sparse_tensor(
        self,
        tensor,
        weight,
        indices,
        ...
        if weight is not None:
            if not isinstance(weight, sparse_tensor.SparseTensor):
                raise ValueError(
                    "Weight for {} is type {} which does not match "
                    "type input which is SparseTensor.".format(path, type(weight))
                )
            weights.append(math_ops.cast(weight.values, dtypes.float32))
        else:
            weights.append(float_zeros)

    def _add_data_for_ragged_tensor(
        self,
        tensor,
        weight,
        row_lengths,
        ...
        if weight is not None:
            if not isinstance(weight, ragged_tensor.RaggedTensor):
                raise ValueError(
                    "Weight for {} is type {} which does not match "
                    "type input which is RaggedTensor.".format(path, type(weight))
                )
            weights.append(math_ops.cast(weight.values, dtypes.float32))
        else:
            weights.append(float_zeros)
    ...
    def _generate_enqueue_op(
        ...
        for inp, weight, (path, feature) in zip(
            ...
            if isinstance(inp, ops.Tensor):
                ...
                self._add_data_for_sparse_tensor(
                    inp,
                    weight,
                    indices_or_row_lengths,
                    values,
                ...
                self._add_data_for_ragged_tensor(
                    inp,
                    weight,
                    indices_or_row_lengths,
                    values,
                ...
                raise ValueError(
                    "Input {} is of unknown type {}. Please only pass "
        ...
        for path, input_tensor in nest.flatten_with_joined_string_paths(
            ...
            if not is_input:
                raise ValueError(
                    ...
                    "argument of strategy.run().".format(
                        path, input_tensor.op.name, input_tensor.op.type
                    )
                )
    ...
    def _raise_error_for_inputs_not_on_cpu(self, flat_inputs, flat_paths):
        ...
        for input_tensor, input_path in zip(flat_inputs, flat_paths):
            ...
            for t in input_tensors:
                if (
                    t.op.type == "Identity"
                    and t.op.inputs[0].op.type == "TPUReplicatedInput"
                ):
    ...
    def enqueue(
        ...
        if not self._using_tpu:
            raise RuntimeError(
                "enqueue is not valid when TPUEmbedding object is not "
                "created under a TPUStrategy."
            )
        ...
        if in_tpu_context:
            ...
            def generate_enqueue_ops():
                ...
                mode_override = array_ops.where_v2(
                    training,
            ...
            for replica_id in range(self._strategy.num_replicas_in_sync):
                replica_inputs = distribute_utils.select_replica(
                    replica_id, flat_inputs
                )
                replica_weights = distribute_utils.select_replica(
                    replica_id, flat_weights
                )
                ...
                device_ordinal = tf_device.DeviceSpec.from_string(
                    tpu_device
                ).device_index
    ...
    def _get_input_shapes(self, tensors, in_tpu_context: bool) -> List[TensorShape]:
    ...
    def _get_input_shape_for_tensor(self, tensor, feature, path) -> TensorShape:
        ...
        if len(shape) < 1:
            raise ValueError(
                "Only rank 1 and above dense tensor is supported,"
                " find rank {} sparse tensor for input {}".format(len(shape), path)
            )
        if shape[-1] != 1:
            return TensorShape(shape + [1])
        return TensorShape(shape)

    def _get_input_shape_for_sparse_tensor(self, tensor, feature, path) -> TensorShape:
        """Get the input shape for the sparse tensor."""
        shape = tensor.shape.as_list()
        # Only 2 and above rank sparse tensor is supported.
        if len(shape) < 2:
            raise ValueError(
                "Only rank 2 and above sparse tensor is supported,"
                " find rank {} sparse tensor for input {}".format(len(shape), path)
            )
        if not feature.output_shape and feature.max_sequence_length > 0:
    ...
    def _get_input_shape_for_ragged_tensor(self, tensor, feature, path) -> TensorShape:
        ...
        if len(shape) != 2:
            raise ValueError(
                "Only rank 2 ragged tensor is supported,"
                " find rank {} ragged tensor for input {}".format(len(shape), path)
            )
        if not feature.output_shape and feature.max_sequence_length > 0:
            # If the max_sequence_length is set and the output shape for FeatureConfig
    ...
    def _get_tensor_core_batch_size(self, output_shapes):
        ...
        for output_shape in output_shapes[1:]:
            tensor_core_batch_size = math.gcd(
                tensor_core_batch_size, self._get_reduce_prod(output_shape)
            )
    ...
    def _update_output_shapes(self, incoming_output_shapes: List[TensorShape]):
        ...
        for old_output_shape, incoming_output_shape in zip(
    ...
    def _check_output_shapes(self, incoming_output_shapes: List[TensorShape]):
        ...
        for (path, feature), old_output_shape, incoming_output_shape in zip(
            ...
            if old_output_shape and incoming_output_shape:
                ...
                if (
                    len(incoming_output_shape) == 1 or len(incoming_output_shape) == 2
                ) and len(old_output_shape) > len(incoming_output_shape):
                    continue
                if len(old_output_shape) != len(
                    incoming_output_shape
                ) or not self._is_tensor_shape_match(
    ...
    def _is_tensor_shape_match(
        self, shape_a: TensorShape, shape_b: TensorShape
    ) -> bool:
...
def _retrieve_variables_impl(
    ...
    for host_id, host in enumerate(hosts):
        with ops.device(host):
            for table in table_config:
                retrieved = (
                    table.optimizer._retrieve()(  # pylint: disable=protected-access
                        table_name=table.name,
                        num_shards=len(hosts),
                        shard_id=host_id,
                        config=config,
                ...
                for i, slot in enumerate(
                    ["parameters"] + table.optimizer._slot_names()
                ):  # pylint: disable=protected-access
...
class TPUEmbeddingSaveable(saveable_hook.SaveableHook):
    ...
    def __init__(
        self, name: Text, load: Callable[[], Any], retrieve: Callable[[], Any]
    ):
        self._load = load
...
def extract_variable_info(
    kwargs,
) -> Tuple[Text, Tuple[int, ...], dtypes.DType, Callable[[], Any]]:
    ...
    if isinstance(kwargs["initial_value"], functools.partial) and (
        "shape" in kwargs["initial_value"].keywords or kwargs["initial_value"].args
    ):
...
def make_sharded_variable_creator(
    ...
    def sharded_variable_creator(
        ...
        partitions = [full_rows_per_host + 1] * partial_partition + [
            full_rows_per_host
        ] * (num_hosts - partial_partition)
        variables = []
