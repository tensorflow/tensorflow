...
class TPUEmbedding(autotrackable.AutoTrackable):
        ...
        if self._using_tpu:
            ...
            for table in self._table_config:
                if (
                    ...
                    and table.optimizer.learning_rate
                    not in self._dynamic_learning_rates
                ):
    ...
    def _maybe_build(self, output_shapes: Optional[Union[List[int], Iterable]] = None):  # pylint:disable=g-bare-generic
        if not self._built:
    ...
    def _get_output_shapes_from_input_shapes(
        ...
        output_shapes = []
        for input_shape, feature in zip(
            input_shapes, nest.flatten(self._feature_config)
        ):
            if input_shape.rank is None or input_shape.rank < 1:
                raise ValueError(
                    "Received input tensor of shape {}. Rank must be 1 and above".format(
                        input_shape
                    )
                )
            ...
            def _split_fn(ts, idx):
                if ts is None:
                    ...
                    return array_ops.split(
                        ts, num_or_size_splits=self._num_cores_per_replica, axis=0
                    )[idx]
                elif isinstance(ts, sparse_tensor.SparseTensor):
                    return sparse_ops.sparse_split_v2(
                        sp_input=ts, num_split=self._num_cores_per_replica, axis=0
                    )[idx]
                else:
