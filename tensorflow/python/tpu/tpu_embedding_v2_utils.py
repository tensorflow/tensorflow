...
class Adagrad(_Optimizer):
    ...
    def _slot_initializers(self) -> List[init_ops_v2.Initializer]:
        return [
            init_ops_v2.Constant(self.initial_accumulator_value, support_partition=True)
        ]
...
class QuantizationConfig:
    ...
    def __init__(self, num_buckets: int, lower: float, upper: float):
        ...
        if num_buckets < 2:
            raise ValueError(
                f"num_buckets is {num_buckets}, must be at least 2 for "
    ...
    def __repr__(self):
        return (
            "QuantizationConfig(num_buckets={num_buckets!r}, lower={lower!r}, "
            "upper={upper!r})".format(
                num_buckets=self.num_buckets, lower=self.lower, upper=self.upper
            )
        )
...
class TableConfig:
    ...
    def _set_table_descriptor(
        self,
        table_descriptor: tpu_embedding_configuration_pb2.TPUEmbeddingConfiguration.TableDescriptor,
        num_hosts: int,
