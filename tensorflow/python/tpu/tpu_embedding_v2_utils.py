...
class CustomCombiner(_WithSlotVariables):
  ...
  ```python
  @tf.function
  def dot_combiner(valency, vectors):
    max_valency = vectors.shape[0]
    gains = tf.range(max_valency) < valency
    gains = tf.cast(gains, vectors.dtype)
    return tf.linalg.matvec(vectors, gains, transpose_a=True)
  ```
  ...
  embedding = tf.tpu.experimental.embedding.TPUEmbedding(
      ...
      batch_size=...)
  ```
  In this example, the combiner of the first table will be the `dot_combiner`.
