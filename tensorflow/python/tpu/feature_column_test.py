...
class EmbeddingColumnTest(test.TestCase):
    ...
    def test_get_dense_tensor(self):
        ...
        # Embedding variable.
        embedding_dimension = 2
        embedding_values = ((1.0, 2.0), (3.0, 5.0), (7.0, 11.0))  # id 0  # id 1  # id 2
        ...
        embedding_column = tpu_fc.embedding_column(
            categorical_column, dimension=embedding_dimension, initializer=_initializer
        )

        # Provide sparse input and get dense result.
        embedding_lookup = embedding_column._get_dense_tensor(
            fc._LazyBuilder({"aaa": sparse_input})
        )
...
class SharedEmbeddingColumnTest(test.TestCase):
    ...
    def test_defaults(self):
        ...
        embedding_column_b, embedding_column_a = tpu_fc.shared_embedding_columns(
            [categorical_column_b, categorical_column_a], dimension=embedding_dimension
        )
        ...
        self.assertEqual("aaa_bbb_shared_embedding", embedding_column_a._var_scope_name)
        self.assertEqual("aaa_bbb_shared_embedding", embedding_column_b._var_scope_name)
    ...
    def test_all_constructor_args(self):
        ...
        self.assertEqual(
            "var_scope_name", embedding_column_a.shared_embedding_collection_name
        )
        self.assertEqual(
            "var_scope_name", embedding_column_b.shared_embedding_collection_name
        )
        self.assertEqual("aaa_shared_embedding", embedding_column_a.name)
        self.assertEqual("bbb_shared_embedding", embedding_column_b.name)
    ...
    def test_get_dense_tensor(self):
        ...
        input_a = np.array(
            [[2, -1, -1], [0, 1, -1]]  # example 0, ids [2]
        )  # example 1, ids [0, 1]
        input_b = np.array(
            [[0, -1, -1], [-1, -1, -1]]  # example 0, ids [0]
        )  # example 1, ids []
        ...
        expected_lookups_a = (
            # example 0:
            (7.0, 11.0),  # ids [2], embedding = [7, 11]
            # example 1:
            (2.0, 3.5),  # ids [0, 1], embedding = mean([1, 2] + [3, 5]) = [2, 3.5]
        )
        expected_lookups_b = (
            # example 0:
            (1.0, 2.0),  # ids [0], embedding = [1, 2]
            # example 1:
            (0.0, 0.0),  # ids [], embedding = [0, 0]
        )
