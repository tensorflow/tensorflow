...
class TPUEmbeddingV3CheckpointTest(parameterized.TestCase, test.TestCase):
    ...
    def test_checkpoint_save_and_restore(self):
        feature_config_1 = tpu_embedding_v2_utils.FeatureConfig(
        ...
        feature_config_2 = tpu_embedding_v2_utils.FeatureConfig(
            table=tpu_embedding_v2_utils.TableConfig(
                vocabulary_size=self.vocabulary_size,
                dim=self.embedding_dim,
        ...
        with strategy.scope():
            model1 = tpu_embedding_v3.TPUEmbeddingV2(
                feature_config=feature_config_1, optimizer=tpu_embedding_v2_utils.SGD()
            )
            model1.build()
        ...
        expected_shard_shape = (
            self.vocabulary_size // strategy.num_replicas_in_sync,
            self.embedding_dim,
        )
        self.assertIsInstance(
            model1._variables["video"]["parameters"],
            tpu_embedding_v3.TPUEmbeddingShardedVariable,
        )
        self.assertLen(
            model1._variables["video"]["parameters"].values,
            strategy.num_replicas_in_sync,
        )
        self.assertEqual(
            model1._variables["video"]["parameters"].values[0].shape,
            expected_shard_shape,
        )
        self.assertAllEqual(
        ...
        with strategy.scope():
            model2 = tpu_embedding_v3.TPUEmbeddingV2(
                feature_config=feature_config_2, optimizer=tpu_embedding_v2_utils.SGD()
            )
