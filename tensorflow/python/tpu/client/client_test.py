...
class CloudTpuClientTest(test.TestCase):
    ...
    def testEnvironmentDiscoveryUrl(self):
        ...
        self.assertEqual(
            "https://{api}.internal/{apiVersion}", (client._environment_discovery_url())
        )
    ...
    def testEnvironmentVarToNetworkEndpointsSingleIp(self):
        self.assertEqual(
            ...
            list(client._environment_var_to_network_endpoints("1.2.3.4:1234")),
        )

    def testEnvironmentVarToNetworkEndpointsSingleGrpcAddress(self):
        self.assertEqual(
            [{"ipAddress": "1.2.3.4", "port": "2000"}],
            list(client._environment_var_to_network_endpoints("grpc://1.2.3.4:2000")),
        )
    ...
    def assertClientContains(self, c):
        ...
        self.assertEqual(
            [{"ipAddress": "10.1.2.3", "port": "8470"}], c.network_endpoints()
        )

    @mock.patch.object(
        client, "_request_compute_metadata", mock_request_compute_metadata
    )
    def testNetworkEndpointsNotReadyWithApi(self):
        ...
        self.assertRaisesRegex(
            RuntimeError, 'TPU .* is not yet ready; state: "None"', c.network_endpoints
        )
    ...
    def testInitializeIpAddress(self):
        ...
        self.assertIsNone(c._discovery_url)
        self.assertEqual(
            [{"ipAddress": "1.2.3.4", "port": "8470"}], c.network_endpoints()
        )
    ...
    def testHandlesByteStrings(self):
        self.assertEqual(
            client.Client(tpu="tpu_name", zone="zone", project="project")._full_name(),
            client.Client(
                tpu=b"tpu_name", zone=b"zone", project=b"project"
            )._full_name(),
        )
    ...
    def testWaitForHealthyRaisesError(self):
        ...
        # Doesn't throw RuntimeError as TPU becomes HEALTHY before timeout
        with self.assertRaisesRegex(
            RuntimeError, "Timed out waiting for TPU .* to become healthy"
        ):
