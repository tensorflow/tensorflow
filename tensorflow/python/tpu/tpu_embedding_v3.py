...
class TPUEmbeddingV2(tpu_embedding_base.TPUEmbeddingBase):
    ...
    def __init__(
        ...
        if not isinstance(
            ...
            raise RuntimeError(
                "TPUEmbeddingV2 should be created under TPUStrategy but found {}.".format(
                    self._strategy
                )
            )
    ...
    def _compute_sc_shard_idx_and_offset(
        self, table_name: str, shard_info: base.ShardInfo
    ) -> tuple[int, int]:
    ...
    def embedding_table_shards(
        ...
        table_shards = {
            name: [(device, var.read_from_device(device)) for device in ordered_devices]
    ...
    def variables(
        self,
    ) -> Dict[tpu_embedding_v2_utils.TableConfig, Dict[str, tf_variables.Variable]]:
    ...
    def _initialize_stacked_table_for_device(
        ...
        for table in stacked_tables:
            ...
            shard_info = self._get_shard_info_for_table(table, device_idx, num_devices)
    ...
    def _track_restore_info_for_cpu(self) -> None:
        ...
        logging.info("Adding sparse core layouts for %s tables", len(layouts.tables))
    ...
    def dequeue(
        ...
        # Take num_minibatches_per_physical_sparse_core from any table as
        # they are the same across tables.
        num_minibatches_per_physical_sparse_core = list(partitioned_tensors.values())[
            0
        ].num_minibatches_per_physical_sparse_core
    ...
    def _preprocess_features(
        ...
        table_to_csr_format_tensor = self._get_csr_wrapped_coo_from_sorted_coo_tensor(
            num_replicas_in_sync,
...
def extract_variable_info(
    kwargs: Any,
) -> Tuple[str, Tuple[int, ...], dtypes.DType, Callable[[], Any], Optional[int]]:
...
def is_checkpoint_initial_value(initial_value: Any) -> bool:
    ...
    return (
        ...
        or (
            ...
            and isinstance(initial_value.func, base.CheckpointInitialValueCallable)
        )
    )
