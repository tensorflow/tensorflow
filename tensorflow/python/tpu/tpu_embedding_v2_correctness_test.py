...
class TPUEmbeddingCorrectness(parameterized.TestCase, test.TestCase):
    ...
    def test_embedding(self, optimizer_name, training, sparse, is_high_dimensional):
    ...
    def _create_high_dimensional_dense_input_fn(
        self, strategy, include_weights=False, weight=0.5
    ):
        def input_fn(ctx):
            del ctx
    ...
    def _check_results(
        ...
        if is_high_dimensional:
            activation_watched_gold = np.stack(
                [activation_watched_gold] * self.data_batch_size
            )

            activation_favorited_gold = np.stack(
                [activation_favorited_gold] * self.data_batch_size
            )

            activation_friends_gold = np.stack(
                [activation_friends_gold] * self.data_batch_size
            )
...
def _compute_gradients_wrt_embedding_table(
    gradient_wrt_activation, embedding_table, feature_indices, feature_values, combiner
):
