...
class TensorTracer(object):
    ...
    def _create_or_get_tensor_values_cache(
        self, cache_name, graph=None, shape=None, dtype=dtypes.float32
    ):
    ...
    def _generate_flush_cache_op(self, num_replicas, on_tpu, tensor_trace_order):
        ...
        if on_tpu:
            ...
            flush_op = _flush_fun(
                cache_val, self._replica_id, training_util.get_or_create_global_step()
            )
        if self._use_temp_cache():
    ...
    def _flush_tensor_values_cache(
        self, tensor_fetches, op_fetches, on_tpu, tensor_trace_order
    ):
    ...
    def aggregate_global_cache(self, global_tt_summary_cache):
        ...
        for signature, idx in sorted(
            ...
            if signature not in agg_fn_map:
                raise RuntimeError(
                    "No aggregation function is defined for "
    ...
    def _trace_execution(self, graph, tensor_fetches, op_fetches=None, on_tpu=True):
        ...
        tensor_trace_order = self._determine_trace_and_create_report(graph, exec_op_set)
            ...
            if self._create_host_call():
                ...
                if not on_tpu:
                    ...
                    if _TT_SUMMARY_TAG in caches_to_write and "step" in caches_to_write:
                        ...
                        write_op = tpu.outside_compilation(
                            write_if_core_0,
                            step=step,
                        ...
                        raise ValueError(
                            "Outside compiled flush in only supported for "
                            "summary mode"
                        )
            else:
