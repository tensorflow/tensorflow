...
class SessionTest(test_util.TensorFlowTestCase):
    ...
    def testFeedIndexedSlices(self):
        with session.Session() as s:
            ...
            ind2 = indexed_slices.IndexedSlices(
    ...
    def testStringFetch(self):
        with session.Session():
            for shape in [(32, 4, 128), (37,), (2, 0, 6), (0, 0, 0)]:
                ...
                c_list = (
                    np.array(
                        [compat.as_bytes(str(i)) for i in range(size)], dtype=np.object_
                    ).reshape(shape)
                    if size > 0
                    else []
                )
    ...
    def testFetchByNameDifferentStringTypes(self):
        with session.Session() as sess:
            ...
            d = constant_op.constant(43.0, name="d")
    ...
    def testInvalidArgument(self):
        with self.assertRaisesRegex(TypeError, "Argument `target` must be a string"):
            session.Session(37)
        with self.assertRaisesRegex(
            TypeError, "Argument `config` must be a tf.ConfigProto"
        ):
            session.Session(config=37)
        with self.assertRaisesRegex(TypeError, "Argument `graph` must be a tf.Graph"):
