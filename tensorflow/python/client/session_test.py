...
class SessionTest(test_util.TensorFlowTestCase):
  ...
  def testFetchIndexedSlicesWithoutDenseShape(self):
    with session.Session() as s:
      ...
      # Single fetch, use as tuple
      ind_out = s.run(ind)
      values_out, indices_out = ind_out
      self.assertAllEqual(values_out, values)
      self.assertAllEqual(indices_out, indices)
      # Single fetch, use as IndexedSlicesWithoutDenseShapeValue
      ind_out = s.run(ind)
      self.assertAllEqual(ind_out.values, values)
      self.assertAllEqual(ind_out.indices, indices)
      # Tuple fetch, use as tuple
      values_out, indices_out = s.run(ind)
      self.assertAllEqual(values_out, values)
      self.assertAllEqual(indices_out, indices)
      # List fetch, use as tuple
      (values_out, indices_out), = s.run([ind])
      self.assertAllEqual(values_out, values)
      self.assertAllEqual(indices_out, indices)
      # List fetch, use as IndexedSlicesWithoutDenseShapeValue
      ind_out, = s.run([ind])
      self.assertAllEqual(ind_out.values, values)
      self.assertAllEqual(ind_out.indices, indices)

  def testFeedIndexedSlicesWithDenseShape(self):
    with session.Session() as s:
      values = np.array([1.0, 2.0]).astype(np.float32)
      indices = np.array([[3, 2, 0], [4, 5, 1]]).astype(np.int64)
      ind = ops.IndexedSlices(
      ...
      self.assertAllEqual(values_out, values)
      self.assertAllEqual(indices_out, indices)
      # Feed with IndexedSlicesWithoutDenseShapeValue
      values_out, indices_out = s.run(
          [ind_values, ind_indices],
          {ind: ops.IndexedSlicesWithoutDenseShapeValue(values, indices)})
      self.assertAllEqual(values_out, values)
      self.assertAllEqual(indices_out, indices)
      # Feed with IndexedSlicesWithoutDenseShapeValue, fetch IndexedSlicesWithoutDenseShapeValue
      ind2_out = s.run(ind2, {ind: ops.IndexedSlicesWithoutDenseShapeValue(values, indices)})
      self.assertAllEqual(ind2_out.values, values)
      self.assertAllEqual(ind2_out.indices, indices)
