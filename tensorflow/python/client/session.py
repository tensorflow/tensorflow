...
def register_session_run_conversion_functions(
    ...
    for conversion_function in _REGISTERED_EXPANSIONS:
        if issubclass(conversion_function[0], tensor_type):
            raise ValueError(
                f"{tensor_type} has already been registered so ignore " "it."
            )
...
class _FetchMapper(object):
    ...
    def for_fetch(fetch):
        ...
        if fetch is None:
            raise TypeError(
                f"Argument `fetch` = {fetch} has invalid type "
        ...
        raise TypeError(
            f"Argument `fetch` = {fetch} has invalid type " f'"{type(fetch).__name__}"'
        )
    ...
    def __init__(self, fetches, contraction_fn):
        ...
        for fetch in fetches:
            try:
                ...
                raise ValueError(
                    f"Argument `fetch` = {fetch} cannot be interpreted as "
                    f"a Tensor. ({str(e)})"
                )
...
class _DictFetchMapper(_FetchMapper):
    ...
    def __init__(self, fetches):
        ...
        if isinstance(fetches, collections.defaultdict):
            self._type_ctor = functools.partial(
                collections.defaultdict, fetches.default_factory
            )
        else:
...
class BaseSession(SessionInterface):
    ...
    def __init__(self, target="", graph=None, config=None):
        ...
        if graph is None:
            ...
            if not isinstance(graph, ops.Graph):
                raise TypeError(
                    "Argument `graph` must be a tf.Graph, but got "
        ...
        if target is not None:
            try:
                ...
                if isinstance(target, config_pb2.ConfigProto):
                    raise TypeError(
                        "Argument `target` must be a string, but got "
        ...
        if not isinstance(config, config_pb2.ConfigProto):
            raise TypeError(
                "Argument `config` must be a tf.ConfigProto, but got "
        ...
        def _feed_fn(feed):
            for tensor_type, _, _, feed_fn in _REGISTERED_EXPANSIONS:
                if isinstance(feed, tensor_type):
                    return feed_fn(feed)
            raise TypeError(
                f"Feed argument {feed} has invalid type " f'"{type(feed).__name__}"'
            )
        ...
        for feed in feeds:
            for subfeed in _feed_fn(feed):
                try:
                    ...
                    e.message = (
                        "Cannot interpret argument `feed` key as Tensor: "
                        f"{e.message}"
                    )
                    e.args = (e.message,)
                    raise e
    ...
    def _run(self, handle, fetches, feed_dict, options, run_metadata):
        ...
        def _feed_fn(feed, feed_val):
            ...
            raise TypeError(
                f"{feed} in argument `feed_dict` has invalid type "
        ...
        if feed_dict:
            ...
            for feed, feed_val in feed_dict.items():
                for subfeed, subfeed_val in _feed_fn(feed, feed_val):
                    ...
                    if (
                        not is_tensor_handle_feed
                        and not subfeed_t.get_shape().is_compatible_with(np_val.shape)
                    ):
                        raise ValueError(
    ...
    def make_callable(self, fetches, feed_list=None, accept_options=False):
        ...
        if feed_list is not None:
            if not isinstance(feed_list, (list, tuple)):
                raise TypeError(
                    "Argument `feed_list` must be a list or tuple. "
