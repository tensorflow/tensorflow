...
class _FetchMapper(object):
    ...
    def for_fetch(fetch):
        ...
        raise TypeError(
            f'Argument `fetch` = {fetch} has invalid type "{type(fetch).__name__}"'
        )
        ...
        def _feed_fn(feed):
            for tensor_type, _, _, feed_fn in _REGISTERED_EXPANSIONS:
                if isinstance(feed, tensor_type):
                    return feed_fn(feed)
            raise TypeError(
                f'Feed argument {feed} has invalid type "{type(feed).__name__}"'
            )
        ...
        for feed in feeds:
            for subfeed in _feed_fn(feed):
                try:
                    ...
                    e.message = (
                        f"Cannot interpret argument `feed` key as Tensor: {e.message}"
                    )
                    e.args = (e.message,)
                    raise e
