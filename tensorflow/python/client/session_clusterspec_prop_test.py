...
class SessionClusterSpecPropagationTest(test_util.TensorFlowTestCase):
        ...
        self.assertEqual(
            ...
            len(
                [
                    ...
                    if "/job:worker/replica:0/task:1/device:CPU:0" == dev_stats.device
                    and "Const" == node_stats.node_name
                ]
            ),
        )
    ...
    def testCanonicalDeviceNames(self):
        ...
        with (
            ops.Graph().as_default() as g,
            ops.device("/job:worker/task:1/device:CPU:0"),
        ):
            const = constant_op.constant(17)
        sess = session.Session(server1.target, config=config, graph=g)
        run_options = config_pb2.RunOptions(
    ...
    def testFullDeviceNames(self):
        ...
        with (
            ops.Graph().as_default() as g,
            ops.device("/job:renamed_worker/replica:0/task:1/device:CPU:0"),
        ):
            const = constant_op.constant(17)
        sess = session.Session(server1.target, config=config, graph=g)
        run_options = config_pb2.RunOptions(
    ...
    def testClusterSpecPropagationThreeServers(self):
        ...
        self.assertAllEqual(
            expected_ones, sess1.run(update_op, feed_dict={feed: expected_ones})
        )
        self.assertAllEqual(expected_ones, sess1.run(var))
        self.assertAllEqual(expected_zeros, sess2.run(var))
        self.assertAllEqual(
            expected_ones, sess2.run(update_op, feed_dict={feed: expected_ones})
        )
        self.assertAllEqual(
    ...
    def testClusterSpecPropagationThreeServersOneCluster(self):
        ...
        with session.Session(server1.target, config=config) as sess:
            output = sess.run(
                ...
                feed_dict={feed1: ones, feed2: ones, feed0: ones},
            )
