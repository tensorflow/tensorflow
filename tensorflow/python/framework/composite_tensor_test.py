...
class CT(composite_tensor.CompositeTensor):
    ...
    @property
    def _type_spec(self):
        component_specs = nest.map_structure(
            type_spec.type_spec_from_value, self.components
        )
...
class CompositeTensorTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.parameters(
        [
            {"structure": CT(0), "expected": [0], "paths": [("CT",)]},
            {"structure": CT("a"), "expected": ["a"], "paths": [("CT",)]},
            {
                "structure": CT(["a", "b", "c"]),
                "expected": ["a", "b", "c"],
    ...
    def testNestFlatten(self, structure, expected, paths, expand_composites=True):
        ...
        string_paths = [
            "/".join(str(p) for p in path) for path in paths
    ...
    @parameterized.parameters(
        [
            {"structure": CT(0), "sequence": [5], "expected": CT(5)},
            {
                "structure": CT(["a", "b", "c"]),
                "sequence": ["A", CT(["b"]), {"x": "y"}],
    ...
    def testNestPackSequenceAs(
        self, structure, sequence, expected, expand_composites=True
    ):
    ...
    @parameterized.parameters(
        [
            ...
            {"s1": CT([1, 2, 3]), "s2": [1, 2, 3], "check_types": False},
        ]
    )  # pyformat: disable
    ...
    @parameterized.parameters(
        [
            {"structure": CT(1, metadata=2), "expected": CT(11, metadata=2)},
            {
    ...
    @parameterized.parameters(
        [
            {"structure": CT("a"), "expected": CT("CT:a")},
            {"structure": CT(["a", "b"]), "expected": CT(["CT/0:a", "CT/1:b"])},
            {
                "structure": [[CT([1, 2, 3])], 100, {"y": CT([CT([4, 5]), 6])}],
                "expected": [
                    ...
                    {
                        "y": CT(
                            [CT(["2/y/CT/0/CT/0:4", "2/y/CT/0/CT/1:5"]), "2/y/CT/1:6"]
                        )
                    },
                ],
            },
    ...
    def testNestMapStructureWithPaths(
        self, structure, expected, expand_composites=True
    ):
        def func1(path, x):
            return "%s:%s" % (path, x)
    ...
    @parameterized.named_parameters(
        [
            (
                "IndexedSlicesNoDenseShape",
                lambda: indexed_slices.IndexedSlices(
                    constant_op.constant([1, 2, 3]), constant_op.constant([2, 8, 4])
                ),
            ),
            (
                "IndexedSlicesInt32DenseShape",
                lambda: indexed_slices.IndexedSlices(
            ...
            (
                "IndexedSlicesInt64DenseShape",
                lambda: indexed_slices.IndexedSlices(
                    constant_op.constant([[1, 2], [3, 4]]),
                    constant_op.constant([2, 8]),
            ...
            (
                "Nested structure",
                lambda: {
                    "a": indexed_slices.IndexedSlices(
                        constant_op.constant([1, 2, 3]), constant_op.constant([2, 8, 4])
                    ),
                    "b": [
    ...
    def testAssertSameStructureWithValueAndTypeSpec(self, value_func):
        ...
        spec = nest.map_structure(
            type_spec.type_spec_from_value, value, expand_composites=False
        )
