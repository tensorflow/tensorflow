...
class TensorAndShapeTest(test_util.TensorFlowTestCase):
    ...
    def testIterableGraph(self):
        ...
        t = op.outputs[0]
        with self.assertRaisesRegex(TypeError, "Iterating.*not allowed.*Graph mode"):
            next(iter(t))
        with self.assertRaisesRegex(
            ...
            with ag_ctx.ControlStatusCtx(ag_ctx.Status.ENABLED):
                next(iter(t))
        with self.assertRaisesRegex(TypeError, "Iterating.*AutoGraph.*not be visible"):
...
class OperationTest(test_util.TensorFlowTestCase):
    ...
    def testStr(self):
        ...
        op = ops.Operation.from_node_def(node_def, ops.Graph(), [], [dtypes.float32])
    ...
    def testSetFullType(self):
        ...
        def test_fn():
            ...
            self.assertEqual(
                ds.op.node_def.experimental_type.type_id, full_type_pb2.TFT_PRODUCT
            )
