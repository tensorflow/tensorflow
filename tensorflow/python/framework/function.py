...
class _DefinedFunction(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._arg_names = [
            argnames[i] if i < len(argnames) else ("arg%d" % i)
    ...
    def _create_definition_if_needed_impl(self):
        ...
        if not temp_graph._c_graph:  # pylint: disable=protected-access
            ...
            output_names = (
                [compat.as_bytes(x) for x in self._out_names] if self._out_names else []
            )
        ...
        self._stateful_ops = [
            (op.name, op.type) for op in temp_graph.get_operations() if op._is_stateful
        ]  # pylint: disable=protected-access
    ...
    def _set_c_attrs(self, attrs):
        ...
        for name, attr_value in attrs.items():
            ...
            c_api.TF_FunctionSetAttrValueProto(
                self._c_func.func, compat.as_str(name), serialized
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._shape_func is not None:
            ...
            if len(shapes) != len(op.outputs):
                raise ValueError(
                    f"shape_func {self._shape_func} produced "
...
class _FuncGraph(ops.Graph):
        ...
        **kwargs,
        ...
        with self._outer_graph.as_default():
            ...
            if (
                ...
                and self._capture_resource_var_by_value
            ):
    ...
    def _capture_tensor_as_extra_input(self, tensor, name=None):
        ...
        if isinstance(tensor, ops.EagerTensor):
            ...
            handle_data = c_api.GetHandleShapeAndType(
                tensor.graph._c_graph, tensor._as_tf_output()
            )

        if handle_data:
            c_api.SetHandleShapeAndType(
                ph.graph._c_graph, ph._as_tf_output(), compat.as_bytes(handle_data)
            )
        # pylint: enable=protected-access
    ...
    def _add_op_and_parents(self, op):
        ...
        if op._is_stateful and op not in self._allowlisted_stateful_ops:
            raise ValueError(
                f"Cannot capture a stateful node (name:{op.name}, "
                f"type:{op.type}) by value."
            )
        elif op.type in ("Placeholder", "PlaceholderV2"):
            raise ValueError(
                f"Cannot capture a placeholder (name:{op.name}, "
                f"type:{op.type}) by value."
            )
        # pylint: enable=protected-access
...
def func_graph_from_py_func(
    ...
    func_graph = _FuncGraph(
        name, capture_by_value, allowlisted_stateful_ops, capture_resource_var_by_value
    )
    ...
    with func_graph.as_default(), ops.device(device):
        ...
        outputs = [
            func_graph.capture(t) if t.graph is not func_graph else t for t in outputs
        ]
...
def _call(sig, *inputs, **kwargs):
    ...
    if len(inputs) != len(sig.input_arg):
        raise ValueError(
            f"Expected {len(sig.input_arg):d} arguments, got " f"{len(inputs):d}."
        )
...
def _from_definition(fdef, grad_func=None):
    ...
    result = _DefinedFunction(
        func, argnames, input_types, func_name, grad_func, python_grad_func, out_names
    )
    # pylint: disable=protected-access
...
def from_library(lib):
    ...
    for g in lib.gradient:
        if g.function_name not in funcs:
            raise ValueError(
                f"FunctionDefLibrary missing '{g.function_name}' " f"FunctionDef\n{lib}"
            )
        if g.gradient_func not in funcs:
            raise ValueError(
                f"FunctionDefLibrary missing '{g.gradient_func}' " f"FunctionDef\n{lib}"
            )
...
def _get_experimental_kwarg_as_attr(attr_name, value):
    ...
    if isinstance(value, bool):
        ...
        raise ValueError(
            f"Attribute {attr_name} must be bool, int, float, or "
...
def _type_list_to_str(types):
    if any(_ not in _DTYPE_TO_STR for _ in types):
        ...
        raise ValueError(
            f"Unsupported dtypes {unsupported_types} in "
