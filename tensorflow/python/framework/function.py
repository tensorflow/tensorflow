        ...
        if handle_data:
            with ph.graph._c_graph.get() as c_graph:
                c_api.SetHandleShapeAndType(
                    c_graph, ph._as_tf_output(), compat.as_bytes(handle_data)
                )
        # pylint: enable=protected-access
...
def _call(sig, *inputs, **kwargs):
    ...
    if len(inputs) != len(sig.input_arg):
        raise ValueError(
            f"Expected {len(sig.input_arg):d} arguments, got {len(inputs):d}."
        )
...
def from_library(lib):
    ...
    for g in lib.gradient:
        if g.function_name not in funcs:
            raise ValueError(
                f"FunctionDefLibrary missing '{g.function_name}' FunctionDef\n{lib}"
            )
        if g.gradient_func not in funcs:
            raise ValueError(
                f"FunctionDefLibrary missing '{g.gradient_func}' FunctionDef\n{lib}"
            )
