...
def convert_to_eager_tensor(value, ctx, dtype=None) -> ops._EagerTensorBase:
    ...
    if isinstance(value, ops.EagerTensor):
        if dtype is not None and value.dtype != dtype:
            raise TypeError(
                f"Expected tensor {value} with dtype {dtype!r}, but got "
...
def _constant_eager_impl(
    ...
    if verify_shape:
        raise TypeError(
            f"Expected Tensor {t} (converted from {value}) with shape "
...
def _tensor_shape_tensor_conversion_function(s, dtype=None, name=None, as_ref=False):
    ...
    if dtype is not None:
        if dtype not in (dtypes.int32, dtypes.int64):
            raise TypeError(
                f"Cannot convert TensorShape {s} to dtype {dtype}. "
                "Allowed dtypes are tf.int32 and tf.int64."
            )
        if dtype == dtypes.int32 and int64_value:
            raise ValueError(
                f"Cannot convert TensorShape {s} to dtype int32; "
...
def _dimension_tensor_conversion_function(d, dtype=None, name=None, as_ref=False):
    ...
    if dtype is not None:
        if dtype not in (dtypes.int32, dtypes.int64):
            raise TypeError(
                f"Cannot convert Dimension {d} to dtype {dtype}. "
                "Allowed dtypes are tf.int32 and tf.int64."
            )
    else:
        dtype = dtypes.int32
