...
def op_is_stateful(op):
    ...
    ret = (
        op._is_stateful
        and (
            (op.type not in ASYNC_STATEFUL_OPS)
            and (op.type not in LEGACY_RANDOM_OPS)
            and (op.type not in SKIPPED_ORDER_INSENSITIVE_STATEFUL_OPS)
        )
    ) or (op.type in _ALLOWLIST_STATELESS_OPS)
    return ret
...
class AutomaticControlDependencies(object):
    ...
    def __init__(
        self, record_initial_resource_uses=False, record_uses_of_resource_ids=None
    ):
    ...
    def run_independently(self, op):
        ...
        op._set_attr(
            "_independent_side_effects", attr_value_pb2.AttrValue(b=True)
        )  # pylint: disable=protected-access
    ...
    def __exit__(self, unused_type, unused_value, unused_traceback):
        ...
        if outer_graph is not None:
            self._graph._add_control_dependencies = (
                outer_graph._add_control_dependencies
            )
        else:
        ...
        for op in new_operations:
            ...
            if (op_def_registry.get(op.type) is None) or (
                op_is_stateful(op)
                and (
                    op.type not in utils.RESOURCE_READ_OPS
                    or any(output.consumers() for output in op.outputs)
                )
            ):
                ops_which_must_run.add(op)
            ...
            for inp, resource_type in _get_resource_inputs(op):
                ...
                do_record = (
                    self.record_initial_resource_uses
        ...
        if self.record_initial_resource_uses:
            ...
            for op in first_uses_by_output_ops:
                others = [other.name.encode() for other in first_uses_by_output_ops[op]]
