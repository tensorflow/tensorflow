...
class TwoCompositesSpec(type_spec.BatchableTypeSpec):
    ...
    def _batch(self, batch_size):
        return TwoCompositesSpec(
            self.x_spec._batch(batch_size), self.y_spec._batch(batch_size), self.color
        )

    def _unbatch(self):
        return TwoCompositesSpec(
            self.x_spec._unbatch(), self.y_spec._unbatch(), self.color
        )
...
class NestOfTensorsSpec(type_spec.TypeSpec):
    ...
    def __repr__(self):
        if (
            ...
            and all(isinstance(f, str) for f in self.spec._fields)
        ):
...
class TypeSpecTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ...
        (
            ...
            TwoTensorsSpecVariableSerialize(
                [5, None], dtypes.int32, [None], dtypes.bool
            ),
            TwoTensorsSpecVariableSerialize(
                [5, None], dtypes.int32, [None], dtypes.bool, "smaller_tuple"
            ),
        ),
        (
    ...
    def testRegistry(self):
        self.assertEqual(
            "tf.TwoCompositesSpec", type_spec_registry.get_name(TwoCompositesSpec)
        )
        self.assertEqual(
            "tf.TwoTensorsSpec", type_spec_registry.get_name(TwoTensorsSpec)
        )
        self.assertEqual(
            TwoCompositesSpec, type_spec_registry.lookup("tf.TwoCompositesSpec")
        )
...
class BatchableTypeSpecTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.parameters(
        [
            {
                "unbatched": TwoCompositesSpec(
                    tensor_spec.TensorSpec([]), tensor_spec.TensorSpec([8])
                ),
                "batch_size": 5,
                "batched": TwoCompositesSpec(
            ...
            },
            {
                ...
                "batch_size": None,
                "batched": TwoCompositesSpec(
                    tensor_spec.TensorSpec(None), tensor_spec.TensorSpec([None, 8])
                ),
            },
            {
                "unbatched": TwoCompositesSpec(
                    tensor_spec.TensorSpec([3, None]), tensor_spec.TensorSpec([8])
                ),
                "batch_size": 12,
                "batched": TwoCompositesSpec(
