...
RegisteredDispatchableTypesSnapshot
GetRegisteredDispatchableTypesSnapshot() {
...
bool IsRegisteredDispatchableType(
    PyObject* py_class,
    const std::vector<Safe_PyObjectPtr>& registered_types) {
  DCheckPyGilState();
...
PythonAPIDispatcher::PythonAPIDispatcher(
    const std::string& api_name, absl::Span<const char*> arg_names,
    absl::Span<PyObject*> defaults)
    : api_name_(api_name),
      targets_mu_(std::make_unique<absl::Mutex>()),
      canonicalizer_(arg_names, defaults) {}

void PythonAPIDispatcher::Register(
    PySignatureChecker signature_checker, PyObject* dispatch_target) {
  DCheckPyGilState();
...
Safe_PyObjectPtr PythonAPIDispatcher::Dispatch(PyObject* args,
  ...
  absl::Span<PyObject*> canonicalized_args_span(
      canonicalized_args_raw.data(), canonicalized_args_raw.size());
...
void PythonAPIDispatcher::Unregister(PyObject* func) {
  DCheckPyGilState();
  using DispatchTargetPair =
      std::pair<PySignatureChecker, Safe_PyObjectPtr>;
...
std::string PythonAPIDispatcher::DebugString() const {
  ...
  std::vector<std::pair<PySignatureChecker, Safe_PyObjectPtr>>
      targets_snapshot;
  {
    absl::MutexLock lock(targets_mu_.get());
    targets_snapshot.reserve(targets_.size());
...
PyInstanceChecker::PyInstanceChecker(
    const std::vector<PyObject*>& py_classes)
