...
class TypeSpecTraceType(trace.TraceType):
    ...
    def __eq__(self, other) -> bool:
        return isinstance(other, TypeSpecTraceType) and self._spec == other._spec
...
class TypeSpec(object, metaclass=abc.ABCMeta):
    ...
    def is_compatible_with(self, spec_or_value):
        ...
        return self.__is_compatible(
            self._serialize(), spec_or_value._serialize()
    ...
    def most_specific_compatible_type(self, other: "TypeSpec") -> "TypeSpec":
        ...
        merged = self.__most_specific_compatible_type_serialization(
            self._serialize(), other._serialize()
        )  # pylint: disable=protected-access
    ...
    def _from_compatible_tensor_list(self, tensor_list: typing.List["ops.Tensor"]):
    ...
    def __eq__(self, other) -> bool:
        # pylint: disable=protected-access
        return (
            type(other) is type(self) and self.__get_cmp_key() == other.__get_cmp_key()
        )
    ...
    def __check_tensor_list(self, tensor_list):
        ...
        if len(specs) != len(expected):
            raise ValueError(
                f"Cannot create a {self.value_type.__name__} from the "
    ...
    def __make_cmp_key(self, value):
        ...
        if isinstance(value, dict):
            return tuple(
                [
                    tuple([self.__make_cmp_key(key), self.__make_cmp_key(value[key])])
    ...
    def __is_compatible(a, b):
        ...
        if isinstance(a, (list, tuple)):
            return len(a) == len(b) and all(
                TypeSpec.__is_compatible(x, y) for (x, y) in zip(a, b)
            )
        if isinstance(a, dict):
            return (
                len(a) == len(b)
                and sorted(a.keys()) == sorted(b.keys())
                and all(TypeSpec.__is_compatible(a[k], b[k]) for k in a.keys())
            )
    ...
    def encode(self, type_spec, value, minimum_rank=0):
        if minimum_rank == 0:
            ...
            if not isinstance(type_spec, BatchableTypeSpec):
                raise ValueError(
                    f"{type_spec.__name__}.encode does not support " "minimum_rank>0."
                )
            return type_spec._to_batched_tensor_list(
                value
            )  # pylint: disable=protected-access
        else:
            raise ValueError(
                f"{type_spec.__name__}.encode does not support " "minimum_rank>1."
            )

    def decode(self, type_spec, encoded_value):
        return type_spec._from_tensor_list(
            encoded_value
        )  # pylint: disable=protected-access
...
class BatchableTypeSpec(TypeSpec, metaclass=abc.ABCMeta):
    ...
    ) -> typing.List["ops.Tensor"]:
        """Encodes `value` as a flat list of `ops.Tensor`."""
        component_tensor_lists = nest.map_structure(
            batchable_to_tensor_list, self._component_specs, self._to_components(value)
        )
    ...
    def _to_batched_tensor_list(
        ...
        get_spec_tensor_list = lambda spec, v: (  # pylint: disable=g-long-lambda
            ...
            if isinstance(spec, BatchableTypeSpec)
            else spec._to_tensor_list(v)
        )  # pylint: disable=protected-access
...
def get_batchable_flat_tensor_specs(spec, context_spec=None):
    ...
    if isinstance(spec, tensor_spec.TensorSpec):
        ...
        warnings.warn(
            f"Batchable type {context_spec} contains non-batchable "
...
def batchable_from_tensor_list(spec, tensor_list):
    ...
    if isinstance(spec, tensor_spec.TensorSpec):
        ...
        encoded_value = nest.map_structure_up_to(
            encoded_specs, batchable_from_tensor_list, encoded_specs, encoded_flats
        )
        return spec.__batch_encoder__.decode(spec, encoded_value)
    else:
        return spec._from_compatible_tensor_list(
            tensor_list
        )  # pylint: disable=protected-access
...
def type_spec_from_value(value) -> TypeSpec:
    ...
    raise TypeError(
        f"Could not build a TypeSpec for {value} of " f"unsupported type {type(value)}."
    )
        ...
        if isinstance(subspecs[0], BatchableTypeSpec):
            ...
            try:
                for subspec in subspecs[1:]:
                    merged_subspec = merged_subspec.most_specific_compatible_type(
                        subspec
                    )
                return merged_subspec._batch(
                    len(subspecs)
                )  # pylint: disable=protected-access
...
def register_type_spec_from_value_converter(
    ...
    def decorator_fn(cls):
        ...
        if name in _NAME_TO_TYPE_SPEC:
            raise ValueError(
                ...
                % (
                    name,
                    _NAME_TO_TYPE_SPEC[name].__module__,
...
def get_name(cls):
    ...
    if cls not in _TYPE_SPEC_TO_NAME:
        raise ValueError(
            "TypeSpec %s.%s has not been registered." % (cls.__module__, cls.__name__)
        )
