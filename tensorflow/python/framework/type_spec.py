...
class TypeSpec(
    ...
    def most_specific_common_supertype(
        ...
        def make_supertype_attribute(attribute_self, *attribute_others):
            ...
            if isinstance(attribute_self, trace.TraceType):
                ...
                if not all(
                    attribute_self == attribute_other
        ...
        try:
            ...
            serialized_supertype = nest.map_structure(
                make_supertype_attribute,
                self._serialize(),
                *(o._serialize() for o in others),
    ...
    def placeholder_value(self, placeholder_context):
        ...
        component_placeholders = nest.map_structure(
            lambda x: x.placeholder_value(placeholder_context), self._component_specs
        )
    ...
    def from_tensors(self, tensors):
        ...
        components = nest.map_structure(
            lambda spec: spec.from_tensors(tensors), self._component_specs
        )
    ...
    def flatten(self):
        ...
        flat = []
        nest.map_structure(
            lambda spec: flat.extend(spec.flatten()), self._component_specs
        )
        return flat
    ...
    def _from_compatible_tensor_list(
        self, tensor_list: List["core_types.Symbol"]
    ) -> Any:
    ...
    def _deserialize(cls, serialization):
        ...
        return cls(
            *serialization
        )  # pytype: disable=not-instantiable  # trace-all-classes
...
def type_spec_from_value(value) -> TypeSpec:
    ...
    raise TypeError(
        f"Could not build a TypeSpec for {value} of unsupported type {type(value)}."
    )
...
def _type_spec_from_value(value) -> TypeSpec:
    ...
    for entry in reversed(_TYPE_CONVERSION_FUNCTION_REGISTRY):
        ...
        if (
            (type(value) is type_object)  # pylint: disable=unidiomatic-typecheck
            or (allow_subclass and isinstance(value, type_object))
        ):
