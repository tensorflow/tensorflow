...
_DTYPES_INTERN_TABLE: dict[types_pb2.DataType, dtypes.DType] = dtypes._INTERN_TABLE
...
class _EagerTensorBase(tensor_lib.Tensor, internal.NativeObject, core_tf_types.Value):
    ...
    def __dlpack__(
        ...
        *,
        ...
        max_version=None,
        dl_device=None,
        copy=None,  # pylint: disable=redefined-outer-name
    ):
    ...
    def set_shape(self, shape) -> None:
        ...
        if len(shape_dims) != len(self_dims):
            raise ValueError(
                f"Tensor's shape {self.shape} is not compatible "
...
def pack_eager_tensors(tensors, ctx=None) -> EagerTensor:
    ...
    for i in range(len(tensors)):
        ...
        if not isinstance(t, EagerTensor):
            raise TypeError(
                f"All tensors being packed must be EagerTensor. "
...
def internal_convert_n_to_tensor(
    ...
    # TODO(b/268347915): Remove argument.
    ctx=None,
) -> list[Union[EagerTensor, SymbolicTensor]]:  # pylint: disable=unused-argument
...
def internal_convert_to_tensor_or_composite(
    ...
    if isinstance(value, composite_tensor.CompositeTensor):
        ...
        return convert_to_tensor(
            ...
            name=name,
            as_ref=as_ref,
            accepted_result_types=(tensor_lib.Tensor, composite_tensor.CompositeTensor),
        )
...
def internal_convert_n_to_tensor_or_composite(
    ...
    Union[EagerTensor, SymbolicTensor, composite_tensor.CompositeTensor, type(None)]
]:
...
_VALID_OP_NAME_REGEX: Pattern[str] = re.compile(r"^[A-Za-z0-9.][A-Za-z0-9_.\\/>-]*$")
_VALID_SCOPE_NAME_REGEX: Pattern[str] = re.compile(r"^[A-Za-z0-9_.\\/>-]*$")


@tf_export("__internal__.create_c_op", v1=[])
@traceback_utils.filter_traceback
def _create_c_op(
    graph, node_def, inputs, control_inputs, op_def=None, extract_traceback=True
) -> pywrap_tf_session.TF_Operation:
    ...
    with graph._c_graph.get() as c_graph:
        op_desc = pywrap_tf_session.TF_NewOperation(
            c_graph, compat.as_str(node_def.op), compat.as_str(node_def.name)
        )
    if node_def.device:
...
class Operation(pywrap_tf_session.PyOperation):
    ...
    def from_node_def(
        ...
        if not isinstance(g, Graph):
            raise TypeError(
                f"Argument g must be a Graph. Received an instance of type {type(g)}"
            )

        if not isinstance(node_def, node_def_pb2.NodeDef):
            raise TypeError(
                f"Argument node_def must be a NodeDef. "
        ...
        if inputs is None:
            ...
            raise TypeError(
                f"Argument inputs shall be a list of Tensors. "
                f"Received an instance of type {type(inputs)}"
            )
        for a in inputs:
            if not isinstance(a, tensor_lib.Tensor):
                raise TypeError(
                    f"Items of argument inputs shall be Tensor. "
        ...
        if control_inputs:
            for c in control_inputs:
                ...
                if isinstance(c, Operation):
                    ...
                    raise TypeError(
                        f"Control input must be an Operation, "
    ...
    def experimental_set_type(self, type_proto) -> None:
        ...
        """
        with self.graph._c_graph.get() as c_graph:  # pylint: disable=protected-access
            if type_proto.type_id not in (
                full_type_pb2.TFT_UNSET,
                full_type_pb2.TFT_PRODUCT,
            ):
                raise ValueError(
                    "error setting the type of ",
                    self.name,
...
class Graph(pywrap_tf_session.PyGraph):
    ...
    def _colocate_with_for_gradient(
        self, op, gradient_uid, ignore_existing=False
    ) -> Iterator[None]:
        ...
        *N.B.* The control dependencies context applies *only* to ops that
        ...
        for c in control_inputs:
            ...
            if c not in current:
                ...
                # pylint: disable=protected-access
                c._set_attr(
                    "_has_manual_control_dependencies",  # pytype: disable=attribute-error
    ...
    def _kernel_label_map(self, op_to_kernel_label_map) -> Iterator[None]:
        ...
        if not isinstance(op_to_kernel_label_map, dict):
            raise TypeError(
                "op_to_kernel_label_map must be a dictionary mapping strings to strings"
            )
    ...
    def _device_function_stack(
        self,
        device_function_stack: traceable_stack.TraceableStack,
    ) -> None:
        if self._stack_state_is_thread_local:
            # pylint: disable=protected-access
    ...
    def _colocation_stack(
        self,
        colocation_stack: traceable_stack.TraceableStack,
    ) -> None:
        if self._stack_state_is_thread_local:
            # pylint: disable=protected-access
    ...
    def _distribution_strategy_stack(
        self,
        _distribution_strategy_stack: list[Any],
    ) -> None:
        self._thread_local._distribution_strategy_stack = (  # pylint: disable=protected-access
...
def device(device_name_or_function) -> ContextManager[None]:
    ...
    if context.executing_eagerly():
        if callable(device_name_or_function):
            raise RuntimeError(
                "tf.device does not support functions when eager execution is enabled."
            )
...
class _DefaultGraphStack(stack.DefaultStack[Graph]):  # pylint: disable=protected-access
    ...
    def get_controller(self, default) -> Iterator[Graph]:
        ...
        try:
            with (
                super(_DefaultGraphStack, self).get_controller(default) as g,
                context.graph_mode(),
...
def _get_outer_context_and_inner_device_stack() -> tuple[
    Callable[[], ContextManager[Graph]], traceable_stack.TraceableStack
]:
...
def init_scope() -> Iterator[None]:
    ...
    if context.executing_eagerly():
        ...
        try:
            with (
                outer_context(),
                name_scope(scope, skip_on_eager=False),
