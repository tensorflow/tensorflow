...
def _node_def(from_node_def, export_scope, unbound_inputs, clear_devices=False):
    ...
    for i, v in enumerate(node_def.input):
        if export_scope and not node_def.input[i].lstrip("^").startswith(export_scope):
            ...
            # identifiable.
            node_def.input[i] = re.sub(
                r"([\^]|^)(.*)", r"\1" + _UNBOUND_INPUT_PREFIX + r"\2", compat.as_str(v)
            )
    ...
    for k, v in six.iteritems(from_node_def.attr):
        if k == "_class":
            new_s = [
                ...
                for s in v.list.s
                if not export_scope
                or compat.as_str(s).split("@")[1].startswith(export_scope)
            ]
...
def _find_extraneous_saver_nodes(graph_def, saver_def):
    ...
    extraneous_scopes = all_saver_scopes - set(
        [retain_scope_save, retain_scope_restore]
    )
...
def _should_include_node(node_or_node_name, export_scope, exclude_nodes):
    ...
    if exclude_nodes and (
        node_or_node_name in exclude_nodes or node_name in exclude_nodes
    ):
        return False
...
def create_meta_graph_def(
    ...
    if meta_info_def and not isinstance(
        ...
        raise TypeError(
            "meta_info_def must be of type MetaInfoDef, not %s", type(meta_info_def)
        )
    if graph_def and not isinstance(graph_def, graph_pb2.GraphDef):
        raise TypeError("graph_def must be of type GraphDef, not %s", type(graph_def))
    if saver_def and not isinstance(saver_def, saver_pb2.SaverDef):
        raise TypeError("saver_def must be of type SaverDef, not %s", type(saver_def))
    ...
    * Run inference from a saved graph and checkpoints.
    ...
    if unbound_inputs_col_name:
        for key, col_def in meta_graph_def.collection_def.items():
            if key == unbound_inputs_col_name:
                ...
                if field.value and (
                    not input_map
                    or sorted([compat.as_str(v) for v in field.value])
                    != sorted(input_map)
                ):
                    raise ValueError(
    ...
    with graph.as_default():
        ...
        if not tf_version:
            ...
            variables_have_trainable = distutils_version.LooseVersion(
                tf_version
        ...
        for key, col_def in sorted_collections:
            ...
            if from_proto and kind == "bytes_list":
                proto_type = ops.get_collection_proto_type(key)
                if (
                    key in ops.GraphKeys._VARIABLE_COLLECTIONS
                    ...
                    for value in col_def.bytes_list.value:
                        ...
                        graph.add_to_collection(
                            key,
                            from_proto(proto, import_scope=scope_to_prepend_to_names),
                        )
            else:
                field = getattr(col_def, kind)
        ...
        var_list = {}
        variables = graph.get_collection(
            ops.GraphKeys.GLOBAL_VARIABLES, scope=scope_to_prepend_to_names
        )
        for v in variables:
        ...
        if graph_def:
            ...
            for key in sorted(graph._nodes_by_id):
                if _should_include_node(
                    graph._nodes_by_id[key].name, export_scope, exclude_nodes
                ):
            ...
            graph._copy_functions_to_graph_def(
                graph_def, bytesize
            )  # pylint: disable=protected-access
...
def copy_scoped_meta_graph(from_scope, to_scope, from_graph=None, to_graph=None):
    ...
    if from_graph == to_graph and from_scope == to_scope:
        raise ValueError(
            "'from_scope' and 'to_scope' need to be different "
    ...
    var_list = import_scoped_meta_graph(
        orig_meta_graph, graph=to_graph, import_scope=to_scope
    )
    return var_list
