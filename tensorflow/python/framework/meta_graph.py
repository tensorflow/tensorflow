...
def _find_extraneous_saver_nodes(graph_def, saver_def):
    ...
    nodes = {
        node_def.name: (set(tensor.get_op_name(x) for x in node_def.input), node_def.op)
...
def create_meta_graph_def(
    ...
    if graph_def and not isinstance(graph_def, graph_pb2.GraphDef):
        raise TypeError(
            f"graph_def must be of type GraphDef. Received type: {type(graph_def)}."
        )
    if saver_def and not isinstance(saver_def, saver_pb2.SaverDef):
        raise TypeError(
            f"saver_def must be of type SaverDef. Received type: {type(saver_def)}."
        )
    ...
    * Run inference from a saved graph and checkpoints.
    ...
    with graph.as_default():
        ...
        if not tf_version:
            ...
            variables_have_trainable = packaging_version.parse(
                tf_version
