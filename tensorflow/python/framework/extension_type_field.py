...
class ExtensionTypeField(
    collections.namedtuple("ExtensionTypeField", ["name", "value_type", "default"])
):
...
def validate_field_value_type(
    ...
    if value_type in (int, float, str, bytes, bool, None, _NoneType, dtypes.DType):
        ...
        and _issubclass(value_type, composite_tensor.CompositeTensor)
    ):
        if in_mapping_key:
            raise TypeError(
                f"Mapping had a key {value_type.__name__!r} with type "
                f"{type(value_type).__name__!r}"
            )
    elif type_annotations.is_generic_tuple(
        value_type
...
def _convert_fields(fields, field_values, context):
    ...
    for field in fields:
        ...
        converted[field.name] = _convert_value(
            field_value, field.value_type, (field.name,), context
        )
...
def _convert_value(value, expected_type, path, context=_ConversionContext.VALUE):
    ...
    if expected_type is tensor.Tensor:
    ...
    elif isinstance(expected_type, type) and _issubclass(
        expected_type, composite_tensor.CompositeTensor
    ):
        ...
        try:
            ...
            raise TypeError(
                f"{''.join(path)}: expected 'tf.TensorShape', got "
        ...
        try:
            ...
            raise TypeError(
                f"{''.join(path)}: expected 'tf.DType', got {type(value).__name__!r}"
            ) from e
    elif expected_type in (int, float, bool, str, bytes, _NoneType):
        if not isinstance(value, expected_type):
            raise TypeError(
                f"{''.join(path)}: expected {expected_type.__name__!r}, "
        ...
        raise TypeError(
            f"{''.join(path)}: Unsupported type annotation {expected_type!r}"
        )
...
def _convert_tensor(value, path, context):
    ...
    if context == _ConversionContext.SPEC:
        if not (
            isinstance(value, type_spec.TypeSpec) and value.value_type is tensor.Tensor
        ):
            raise TypeError(
                f"{''.join(path)}: expected a TensorSpec, got {type(value).__name__!r}"
            )
        return value
    ...
    if not isinstance(value, tensor.Tensor):
        ...
        try:
            ...
            raise TypeError(
                f"{''.join(path)}: expected a Tensor, got {type(value).__name__!r}"
            ) from e
    return value
...
def _convert_composite_tensor(value, expected_type, path, context):
    ...
    if context == _ConversionContext.SPEC:
        if not (
            isinstance(value, type_spec.TypeSpec)
            and _issubclass(value.value_type, expected_type)
        ):
            raise TypeError(
                f"{''.join(path)}: expected a TypeSpec for "
...
def _convert_tuple(value, expected_type, path, context):
    ...
    if not isinstance(value, typing.Sequence):
        raise TypeError(
            f"{''.join(path)}: expected tuple, got {type(value).__name__!r}"
        )
...
def _convert_mapping(value, expected_type, path, context):
    ...
    if not isinstance(value, typing.Mapping):
        raise TypeError(
            f"{''.join(path)}: expected mapping, got {type(value).__name__!r}"
        )
...
def _convert_union(value, expected_type, path, context):
    ...
    for type_option in type_annotations.get_generic_type_args(expected_type):
        try:
            ...
            pass
    raise TypeError(
        f"{''.join(path)}: expected {expected_type!r}, got {type(value).__name__!r}"
    )
