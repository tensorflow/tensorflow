...
def _convert_value(value, expected_type, path, context=_ConversionContext.VALUE):
    ...
    if expected_type is ops.Tensor:
        ...
        try:
            ...
            raise TypeError(f'{"".join(path)}: expected tf.DType, got {value!r}') from e
    elif expected_type in (int, float, bool, str, bytes, _NoneType):
        if not isinstance(value, expected_type):
            raise TypeError(
                f'{"".join(path)}: expected ' f"{expected_type.__name__}, got {value!r}"
            )
        return value
        ...
        raise TypeError(
            f'{"".join(path)}: Unsupported type annotation ' f"{expected_type!r}"
        )
...
def _convert_tensor(value, path, context):
    ...
    if context == _ConversionContext.SPEC:
        if not (
            isinstance(value, type_spec.TypeSpec) and value.value_type is ops.Tensor
        ):
    ...
    if not isinstance(value, ops.Tensor):
        ...
        try:
            ...
            raise TypeError(
                f'{"".join(path)}: expected a Tensor, ' f"got {value!r}"
            ) from e
    return value
...
def _convert_composite_tensor(value, expected_type, path, context):
    ...
    if context == _ConversionContext.SPEC:
        if not (
            ...
            and issubclass(value.value_type, expected_type)
        ):
            raise TypeError(
    ...
    if not isinstance(value, expected_type):
        raise TypeError(
            f'{"".join(path)}: expected {expected_type.__name__}, ' f"got {value!r}"
        )
    return value
