...
def ExtractBitsFromBFloat16(x):
    return (
        numpy_compat.np_asarray(x, dtype=dtypes.bfloat16.as_numpy_dtype)
        .view(np.uint16)
        .item()
    )
...
def FastAppendBFloat16ArrayToTensorProto(tensor_proto, proto_values):
    fast_tensor_util.AppendBFloat16ArrayToTensorProto(
        ...
        numpy_compat.np_asarray(
            proto_values, dtype=dtypes.bfloat16.as_numpy_dtype
        ).view(np.uint16),
    )
...
def SlowAppendFloat8e5m2ArrayToTensorProto(tensor_proto, proto_values):
    tensor_proto.float8_val += (
        numpy_compat.np_asarray(proto_values, dtype=dtypes.float8_e5m2.as_numpy_dtype)
        .view(np.uint8)
        .tobytes()
    )
...
def SlowAppendFloat8e4m3fnArrayToTensorProto(tensor_proto, proto_values):
    tensor_proto.float8_val += (
        numpy_compat.np_asarray(proto_values, dtype=dtypes.float8_e4m3fn.as_numpy_dtype)
        .view(np.uint8)
        .tobytes()
    )
...
def SlowAppendFloat4e2m1fnArrayToTensorProto(tensor_proto, proto_values):
    tensor_proto.float8_val += (
        numpy_compat.np_asarray(proto_values, dtype=dtypes.float4_e2m1fn.as_numpy_dtype)
        .view(np.uint8)
        .tobytes()
    )
...
def SlowAppendInt4ArrayToTensorProto(tensor_proto, proto_values):
    ...
    x = numpy_compat.np_asarray(proto_values, dtype=dtypes.int4.as_numpy_dtype).astype(
        np.int8
    )
...
def SlowAppendUInt4ArrayToTensorProto(tensor_proto, proto_values):
    ...
    # implementation-defined, so we need to explicitly cast each
    # value to an int for packing.
    x = numpy_compat.np_asarray(proto_values, dtype=dtypes.uint4.as_numpy_dtype).astype(
        np.int8
    )
    tensor_proto.int_val.extend(x.tolist())
...
def SlowAppendInt2ArrayToTensorProto(tensor_proto, proto_values):
    ...
    # implementation-defined, so we need to explicitly cast each
    # value to an int for packing.
    x = numpy_compat.np_asarray(proto_values, dtype=dtypes.int2.as_numpy_dtype).astype(
        np.int8
    )
    tensor_proto.int_val.extend(x.tolist())
...
def SlowAppendUInt2ArrayToTensorProto(tensor_proto, proto_values):
    ...
    # implementation-defined, so we need to explicitly cast each
    # value to an int for packing.
    x = numpy_compat.np_asarray(proto_values, dtype=dtypes.uint2.as_numpy_dtype).astype(
        np.int8
    )
    tensor_proto.int_val.extend(x.tolist())
...
if _FAST_TENSOR_UTIL_AVAILABLE:
    _NP_TO_APPEND_FN = {
        ...
        dtypes.float4_e2m1fn.as_numpy_dtype: (FastAppendFloat4e2m1fnArrayToTensorProto),
    ...
    _NP_TO_APPEND_FN = {
        ...
        dtypes.float8_e4m3fn.as_numpy_dtype: (SlowAppendFloat8e4m3fnArrayToTensorProto),
        dtypes.float4_e2m1fn.as_numpy_dtype: (SlowAppendFloat4e2m1fnArrayToTensorProto),
...
def _AssertCompatible(values, dtype):
    ...
    try:
        ...
        if dtype is None:
            ...
            raise TypeError(
                f"Expected {dtype.name}, but got {mismatch} of type "
...
def make_tensor_proto(
    ...
    if isinstance(values, (np.ndarray, np.generic)):
        ...
        if shape is not None and np.prod(shape, dtype=np.int64) == 0:
            ...
            if list(nparray.shape) != _GetDenseDimensions(values) and not is_quantized:
                raise ValueError(
                    f"Expected values {values} to be a dense tensor with "
    ...
    if dtype is not None and (
        ...
        raise TypeError(
            f"`dtype` {dtype} is not compatible with {values} of dtype {nparray.dtype}."
        )
    ...
    if shape is None:
        ...
        if allow_broadcast:
            if nparray.shape == (1,) or nparray.shape == tuple():
                ...
                raise TypeError(
                    f"Expected Tensor's shape: {tuple(shape)}, but got {nparray.shape}."
                )
            ...
            if nparray.size > shape_size:
                raise ValueError(
                    "Too many elements provided. Takes at most "
    ...
    if numpy_dtype == dtypes.string and not isinstance(values, np.ndarray):
        ...
        try:
            ...
            raise TypeError(
                f"Failed to convert elements of {values} to Tensor. "
...
def MakeNdarray(tensor):
    ...
    if tensor_dtype == dtypes.float16 or tensor_dtype == dtypes.bfloat16:
        ...
        raise TypeError(
            f"Unsupported tensor type: {tensor.dtype}. See "
            "https://www.tensorflow.org/api_docs/python/tf/dtypes "
            "for supported TF dtypes."
        )
...
def ShapeEquals(tensor_proto, shape):
    ...
    if not isinstance(tensor_proto, tensor_pb2.TensorProto):
        raise TypeError(
            "`tensor_proto` must be a tensor_pb2.TensorProto object, "
