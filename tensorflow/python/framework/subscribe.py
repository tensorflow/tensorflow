...
def _recursive_apply(tensors, apply_fn):
    ...
    if isinstance(tensors, tensor_lib.Tensor):
        ...
        raise TypeError(
            f"_recursive_apply argument {tensors!r} has invalid type {tensors_type!r}"
        )
...
def _is_subscribed_identity(tensor):
    ...
    match = re.match(r"(?P<prefix_name>^.*?)/subscription/Identity[^/]+", tensor.name)
...
def _subscribe(tensor, side_effects, control_cache):
    ...
    if not tensor.dtype.is_numpy_compatible:
        logging.debug(
            ("Tensor {} has an un-supported {} type and cannot be subscribed.").format(
    ...
    assert len(matching_ops) <= 1, (
        "Op {} must only have one subscription op connected to it"
    ).format(tensor.op.name)
