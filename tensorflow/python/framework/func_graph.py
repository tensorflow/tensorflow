...
class FuncGraph(ops.Graph):
    ...
    def watch_variable(self, v):
        ...
        if (
            ...
            and v.handle in self._resource_tensor_inputs
        ):
            return
...
def _func_graph_as_default_inner_cm(
    ...
    if not context.executing_eagerly() and (
        ...
        or (
            func_graph._distribution_strategy_stack
...
def func_graph_from_py_func(
    ...
    with func_graph.as_default(), deps_control_manager as deps_ctx:
        ...
        check_func_mutation(
            func_args_before, func_kwargs_before, func_args, func_kwargs, original_func
        )
        current_scope.set_use_resource(default_use_resource)

        inputs = []
        for arg in composite_tensor_utils.flatten_with_variables(
...
def check_func_mutation(old_args, old_kwargs, new_args, new_kwargs, func):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"arguments. Modifying a copy is allowed."
        ) from e

    assert set(old_bound) == set(new_bound)
    modified_args = [
        arg_name
        for arg_name in new_bound
        if has_mutation(old_bound[arg_name], new_bound[arg_name])
    ]
    changes = ", ".join(modified_args)
    raise ValueError(
        f"{func_name}{signature} should not modify its Python "
