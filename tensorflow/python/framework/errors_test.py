...
class ErrorsTest(test.TestCase):
    ...
    def testUniqueClassForEachErrorCode(self):
        for error_code, exc_type in [
            ...
            self.assertTrue(
                isinstance(
                    errors_impl._make_specific_exception(None, None, None, error_code),
                    exc_type,
                )
            )
            ...
            self.assertEqual(
                error_code, errors_impl.error_code_from_exception_type(exc_type)
            )
    ...
    def testErrorPayloadsFromStatus(self):
        for code, expected_exception in [
            ...
            self.assertEqual(error.exception.experimental_payloads[b"key1"], b"value1")
            self.assertEqual(error.exception.experimental_payloads[b"key2"], b"value2")
