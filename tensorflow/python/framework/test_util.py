    ...
    def wrap_f(f):
        def decorator(self, *args, **kwargs):
            ...
            with context.eager_mode():
                ...
                assert (
                    not obj_count_by_type
                ), "The following objects were newly created: %s" % str(
                    obj_count_by_type
                )
                gc.enable()
    ...
    def decorator(cls):
        ...
        for name, value in cls.__dict__.copy().items():
            if (
                ...
                and not getattr(value, "_disable_eager_op_as_function", False)
            ):
                setattr(
                    cls,
                    name + "WithEagerOpAsFunctionEnabled",
...
class TensorFlowTestCase(googletest.TestCase):
    ...
    def assertShapeEqual(self, input_a, input_b, msg=None):
        ...
        shape_a = (
            ...
            if isinstance(input_a, ops.Tensor)
            else input_a.shape
        )
        shape_b = (
            input_b.get_shape().as_list()
            if isinstance(input_b, ops.Tensor)
            else input_b.shape
        )
