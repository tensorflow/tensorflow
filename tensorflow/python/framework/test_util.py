...
def _strip_checkpoint_v2_randomized(graph_def: graph_pb2.GraphDef) -> None:
    for node in graph_def.node:
        ...
        for attr_key in node.attr:
            attr_tensor_value = cast(tensor_pb2.TensorProto, node.attr[attr_key].tensor)
...
class TensorFlowTestCase(googletest.TestCase):
            ...
            try:
                ...
                if sparse_tensor.is_sparse(tensor):
                    ...
                    return indexed_slices.IndexedSlicesValue(
                        ...
                        dense_shape=None
                        if tensor.dense_shape is None
                        else tensor.dense_shape.numpy(),
                    )
                else:
    ...
    def evaluate(
        ...
        ragged_tensor_value.RaggedTensorValue, sparse_tensor.SparseTensorValue, None
    ]:
    ...
    def assertAllEqual(self, a, b, msg=None):
        ...
        if not np.all(same):
            ...
            if a.dtype.kind != b.dtype.kind and {a.dtype.kind, b.dtype.kind}.issubset(
                {"U", "S", "O"}
            ):
                a_list = []
                b_list = []
    ...
    def assertShapeEqual(self, input_a, input_b, msg=None):
        ...
        shape_a = (
            input_a.get_shape().as_list()
            if isinstance(input_a, tensor_lib.Tensor)
            else input_a.shape
        )
        shape_b = (
            input_b.get_shape().as_list()
            if isinstance(input_b, tensor_lib.Tensor)
            else input_b.shape
        )
...
class TestDelta:
    ...
    def Reset(self) -> None:
        self.last_value = _test_metrics_util.test_counter_value(self.name, self.label)
