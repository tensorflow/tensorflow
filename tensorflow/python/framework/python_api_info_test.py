...
class PythonAPIInfoTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def makeConverterFromParamSpecs(
        self, api_name, param_names, input_specs, attr_specs, defaults=()
    ):
        """Returns a PythonAPIInfo built from the given specs."""
        api_info = _pywrap_python_api_info.PythonAPIInfo(api_name)
        api_info.InitializeFromParamSpecs(
            input_specs, attr_specs, param_names, defaults
        )
        return api_info
    ...
    @parameterized.named_parameters(
        [
            ...
            (
                ...
                "DebugInfo for AllAttributeTypes:\n"
    ...
    def testInitializeFromParamSpecs(
        self, api_name, param_names, input_specs, attr_specs, debug_info
    ):
        api_info = self.makeConverterFromParamSpecs(
            api_name, param_names, input_specs, attr_specs
        )
        self.assertEqual(api_info.DebugInfo().strip(), debug_info.strip())
