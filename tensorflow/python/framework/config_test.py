...
class ConfigTest(test.TestCase, parameterized.TestCase):
    ...
    def testJit(self):
        ...
        a = constant_op.constant([2.0, 2.0])
        b = constant_op.constant([2.0, 2.0])
...
class DeviceTest(test.TestCase):
    ...
    def testGpuNone(self):
        ...
        with self.assertRaisesRegex(errors.InvalidArgumentError, "Could not satisfy"):
            with ops.device("/device:GPU:0"):
    ...
    def testGpuMultiple(self):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "Could not satisfy device specification"
        ):
    ...
    def testDeviceDetailsErrors(self):
        ...
        with self.assertRaisesRegex(ValueError, "must be a tf.config.PhysicalDevice"):
...
class TensorFloat32Test(test.TestCase):
    ...
    def _skip_if_tensor_float_32_unsupported(self):
        if not test_util.is_gpu_available(
            ...
            self.skipTest(
                "TensorFloat-32 requires an NVIDIA GPU with compute "
