...
class FunctionTest(test.TestCase):
    ...
    def testTanhSymGrad(self):
        ...
        cfg = config_pb2.ConfigProto(
            graph_options=config_pb2.GraphOptions(
                optimizer_options=config_pb2.OptimizerOptions(
                    opt_level=config_pb2.OptimizerOptions.L1, do_function_inlining=True
                )
            )
        )
    ...
    def testCustomGradient(self):
        ...
        def XentLoss(logits, labels):
            return math_ops.reduce_sum(labels * math_ops.log(nn_ops.softmax(logits)), 1)
        ...
        x = np.random.uniform(-10.0, 10.0, size=(4, 9)).astype(np.float32)
        prob = np.exp(x) / np.sum(np.exp(x), 1, keepdims=1)
        y = np.random.uniform(-10.0, 10.0, size=(4, 9)).astype(np.float32)
        for cfg in _OptimizerOptions():
    ...
    def testAssertOp(self):
        ...
        with g.as_default(), self.cached_session():
            ...
            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, "assertion failed.*-3"
            ):
                self.assertAllEqual(Foo(constant_op.constant(-3.0)), 6.0)
    ...
    def testAssertWrapper(self):
        ...
        with self.cached_session():
            ...
            with self.assertRaisesRegex(errors_impl.InvalidArgumentError, "assertion"):
                _ = MyFn(100.0).eval()
    ...
    def testWhileLoopCallsFunc(self):
        with self.session():
            ...
            self.assertAllClose(ans, 131072.0)
    ...
    def testControlFlowStrictness(self):
        ...
        with ops.device("CPU"):
            ...
            loop = control_flow_ops.while_loop(
                lambda y: pred, lambda y: AssertFail(y), [x]
            )
    ...
    def testUnusedFunction(self):
        ...
        with g.as_default():
            ...
            def Unused2():
                invoked = True
                return constant_op.constant(7.0)
    ...
    def testCapture(self):
        ...
        with g.as_default():
            ...
            y = Foo(constant_op.constant([[10.0]]))
    ...
    def testSignatureHash(self):
        ...
        with self.session(graph=g) as sess:
            ...
            self.assertAllEqual(v0, 20.0)
            self.assertAllEqual(v1, 20.0)
    ...
    def testTwoInputsSameOp(self):
        ...
        f = graph_to_function_def.graph_to_function_def(
            g, g.get_operations()[1:], [s, u, v], [result]  # skip the placeholder
        )
    ...
    def testSameFunctionDifferentGrads(self):
        ...
        self.assertAllEqual(v0, 2.0)
        self.assertAllEqual(v1, 101.0)
        self.assertAllEqual(v2, 50.0)
...
class FunctionsFromProtos(test.TestCase):
    ...
    def testCapturedInputs(self):
        ...
        self.assertEqual(
            new_func.declared_input_types, Foo.declared_input_types + (dtypes.int64,)
        )
    ...
    def testFromLibraryMissingFuncDef(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "FunctionDefLibrary missing 'G1_[0-9a-zA-Z]{8,11}' FunctionDef"
        ):
        ...
        with self.assertRaisesRegex(
            ValueError, "FunctionDefLibrary missing 'F1_[0-9a-zA-Z]{8,11}' FunctionDef"
        ):
            function.from_library(library)
    ...
    def testExperimentalAttrs(self):
        ...
        self.assertEqual(
            FunctionWithStrAttr.definition.attr["experimental_tag"].s, b"tag_value"
        )
        self.assertTrue("experimental_tag" in FunctionWithIntAttr.definition.attr)
        self.assertEqual(FunctionWithIntAttr.definition.attr["experimental_tag"].i, 123)
        ...
        self.assertEqual(
            FunctionWithBoolAttr.definition.attr["experimental_tag"].b, True
        )
    ...
    def testImplementsReferenceAttrs(self):
        ...
        self.assertEqual(
            FunctionWithStrAttr.definition.attr["_implements"].s, b"org.google.lstm"
        )
        self.assertIn("_reference", FunctionWithStrAttr.definition.attr)
        self.assertEqual(
            FunctionWithStrAttr.definition.attr["_reference"].s, b"arxiv.org"
        )
...
class FunctionOverloadTest(test.TestCase):
    ...
    def testBasic(self):
        @function.Defun()
        def Sinh(x):
            return 1 / 2.0 * (math_ops.exp(x) - math_ops.exp(-x))
    ...
    def testDocString(self):
        ...
        self.assertEqual(
            g.as_graph_def().library.function[0].signature.description,
            "Successor of x.",
        )
...
@test_util.run_all_without_tensor_float_32("Calls matmul in custom LSTM function")
class UnrollLSTMTest(test.TestCase):
    ...
    def LSTMCell(cls, x, mprev, cprev, weights):
        ...
        new_c = math_ops.sigmoid(f_g) * cprev + math_ops.sigmoid(i_g) * math_ops.tanh(
            i_i
        )
    ...
    def _BuildForward(self, weights, inp, mode="cell"):
        ...
        if mode == "loop10":
            ...
            @function.Defun(
                dtypes.float32, dtypes.float32, dtypes.float32, *([dtypes.float32] * 10)
            )
    ...
    def testUnrollLSTM(self):
        ...
        def RunForward(mode, cfg=None):
            ...
            finish = time.time()
            tf_logging.info(
                "time: %f txt size: %d gdef bin size: %d",
                finish - start,
                len(str(gdef)),
    ...
    def _testSimpleModel(self, use_forward_func, use_resource=False):
        ...
        def Grad(x, y0):
            if use_forward_func:
                ...
                y = _Model(x)
            loss = math_ops.reduce_mean(math_ops.reduce_sum(y0 * math_ops.log(y), 1), 0)
...
class DevicePlacementTest(test.TestCase):
    ...
    def _testNestedDeviceWithSameFunction(self, func_name):
        def MatmulWrap(a, b):
            @function.Defun(func_name=func_name, *[dtypes.int32] * 2)
            def Matmul(a, b):
                return math_ops.matmul(a, b)
