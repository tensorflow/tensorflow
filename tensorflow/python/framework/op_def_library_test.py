...
class OpDefLibraryTest(test_util.TensorFlowTestCase):
    ...
    def testSimpleFailures(self):
        with ops.Graph().as_default():
            ...
            self.assertIn(
                "Simple got unexpected keyword arguments: extra", str(cm.exception)
            )
            ...
            self.assertIn(
                "Simple got unexpected keyword arguments: extra1, extra2",
                str(cm.exception),
            )
    ...
    def testPolymorphic(self):
        ...
        """,
            ...
            self.assertEqual(
                str(cm.exception),
                "Should not specify value for inferred attr 'T' for Polymorphic.",
            )
    ...
    def testOutTypeList(self):
        ...
        """,
            ...
            self.assertEqual(
                str(cm.exception), "Expected list for attr T, obtained DType instead."
            )
...
class OpDefLibraryPybindTest(test_util.TensorFlowTestCase):
    def testPybind(self):
        ...
        attrs, inputs, input_types, output_structure = (
            op_def_library_pybind.process_inputs("AddV2", 1, {"x": x, "y": y})
        )
