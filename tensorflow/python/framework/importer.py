    ...
    * "foo:bar:baz" -> ValueError
    ...
    if len(components) == 2:
        ...
        try:
            ...
            raise ValueError(
                f"Cannot convert {tensor_name!r} to a tensor name. "
        ...
        raise ValueError(
            f"Cannot convert '{tensor_name}' to a tensor name. Tensor "
...
def _ProcessReturnElementsParam(return_elements):
    ...
    if not all(isinstance(x, compat.bytes_or_text_types) for x in return_elements):
        raise TypeError(
            "Argument `return_elements` must be a list of strings. "
    ...
    for op, coloc_op_list in colocation_pairs.items():
        ...
        for coloc_op_name in coloc_op_list:
            try:
                ...
                raise ValueError(
                    f"Specified colocation to an op: {coloc_op_name} that "
