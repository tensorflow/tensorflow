...
class VersionTest(test.TestCase):
  ...
  def testVersion(self):
    ...
    # Patched (allow arbitrary suffix after the patch number), Weekly-sync(241210):
    self.assertRegex(versions.__version__, r'^\d+\.\d+\.\d+.*$')
    # allow any additional suffix after the patch number,  Weekly-sync(241210):
    self.assertRegex(versions.VERSION, r'^\d+\.\d+\.\d+.*$')
