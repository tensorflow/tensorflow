...
class LSTMTest(test.TestCase):
    ...
    def testLSTMBlockCellErrorHandling(self):
        ...
        h_prev = constant_op.constant(0.592631638, shape=[28, 17], dtype=dtypes.float32)
        ...
        wco = constant_op.constant(0.592631638, shape=[28, 17], dtype=dtypes.float32)
    ...
    def testLSTMBlockCellGradErrorHandling(self):
        ...
        h_prev = constant_op.constant(0.504355371, shape=[1, 1], dtype=dtypes.float32)
        ...
        cs = constant_op.constant(0.504355371, shape=[1, 1, 1], dtype=dtypes.float32)
        ...
        ci = constant_op.constant(0.504355371, shape=[1, 1, 1], dtype=dtypes.float32)
        co = constant_op.constant(0.504355371, shape=[1, 1, 1], dtype=dtypes.float32)
        ...
        with self.assertRaisesRegex(
            (ValueError, errors_impl.InvalidArgumentError), "must be rank"
        ):
            self.evaluate(
...
class NestedLSTMTest(test.TestCase):
    ...
    def testNestedIOLSTMAllRNNContainers(self):
        ...
        with self.session(graph=ops.Graph()) as sess:
            ...
            inputs_c = (
                array_ops_stack.stack([input_[0] for input_ in inputs]),
