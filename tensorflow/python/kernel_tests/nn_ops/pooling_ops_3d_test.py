...
class PoolingTest(test.TestCase):
  ...
  def _ConstructAndTestGradientForConfig(self,
    ...
    jacob_a, jacob_n = self._getJacobians(
        pool_func, input_sizes, output_sizes, window, strides, padding,
        data_format, use_gpu, dtype=data_type.as_numpy_dtype)

    if data_type == dtypes.bfloat16:
      # Compare bf16 analytical gradients to fp32 numerical gradients.
      _, jacob_n =  self._getJacobians(
          pool_func, input_sizes, output_sizes, window, strides,
          padding, data_format, use_gpu, dtype=np.float32)
