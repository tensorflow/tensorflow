    ...
    for i in range(out_rows):
        for j in range(out_cols):
            for k in range(c):
                ...
                out[:, i, j, k * o : (k + 1) * o] = np.sum(
                    multiplied_slice, axis=(1, 2)
                )
    return out
...
def ConfigsToTest():
    ...
    def Config(
        input_size, filter_size, out_size, stride=1, padding="SAME", dilations=None
    ):
    ...
    return [
        ...
        Config([4, 147, 147, 2], [3, 3, 2, 8], [4, 49, 49, 16], 3, padding="VALID"),
        Config([3, 299, 299, 3], [3, 2, 3, 8], [3, 150, 150, 24], 2),
        Config([5, 183, 183, 1], [5, 5, 1, 2], [5, 92, 92, 2], 2),
        Config([5, 183, 183, 1], [5, 5, 1, 2], [5, 183, 183, 2], dilations=[2, 2]),
        Config(
            [5, 41, 35, 2],
            [4, 7, 2, 2],
            [5, 32, 23, 4],
...
def ConfigsToTestExplicit():
    ...
    def Config(
        input_size, filter_size, out_size, stride=1, padding=None, dilations=None
    ):
        return input_size, filter_size, out_size, stride, padding, dilations

    return [
        Config([4, 5, 5, 48], [1, 1, 48, 2], [4, 8, 12, 96], padding=[[1, 2], [3, 4]]),
        Config(
            [4, 1, 1, 3], [3, 3, 3, 2], [4, 29, 39, 6], padding=[[10, 20], [15, 25]]
        ),
        Config([4, 9, 27, 8], [3, 3, 8, 1], [4, 14, 31, 8], padding=[[3, 4], [4, 2]]),
        Config([4, 31, 31, 7], [3, 3, 7, 1], [4, 29, 29, 7], padding=[[0, 0], [0, 0]]),
        Config(
            [3, 299, 299, 3],
            [3, 2, 3, 8],
            [3, 150, 153, 24],
        ...
        Config(
            [5, 183, 183, 1], [5, 5, 1, 2], [5, 62, 60, 2], 3, padding=[[3, 2], [1, 0]]
        ),
        Config(
            [5, 29, 31, 1],
            [5, 4, 1, 2],
        ...
        Config([4, 5, 5, 48], [3, 3, 48, 1], [4, 5, 5, 48], padding=[[0, 2], [0, 2]]),
        Config([1, 8, 7, 2], [8, 7, 2, 1], [1, 8, 7, 2], padding=[[0, 7], [3, 3]]),
        Config([2, 4, 3, 2], [3, 2, 2, 1], [2, 4, 3, 2], padding=[[2, 0], [1, 0]]),
    ]
...
def CheckGradConfigsToTestExplicit():
    ...
    return [
        Config([2, 5, 8, 1], [4, 4, 1, 2], [2, 3, 10, 2], padding=[[0, 1], [2, 3]]),
        Config([4, 5, 5, 1], [2, 2, 1, 2], [4, 4, 5, 2], 2, padding=[[3, 1], [5, 0]]),
        Config([2, 4, 4, 2], [3, 1, 2, 2], [2, 7, 11, 4], padding=[[4, 1], [3, 4]]),
        Config([1, 15, 15, 2], [1, 3, 2, 1], [1, 18, 23, 2], padding=[[3, 0], [2, 8]]),
        Config(
            [2, 15, 16, 1], [3, 3, 1, 2], [2, 5, 8, 2], 3, padding=[[0, 0], [10, 0]]
        ),
        Config(
            [2, 5, 8, 1],
            [3, 4, 1, 2],
...
class DepthwiseConv2DTest(test.TestCase):
    ...
    def testDepthwiseConv2DExplicit(self):
        for index, (
            ...
            padding,
            dilations,
        ) in enumerate(ConfigsToTestExplicit()):
            tf_logging.info(
                "Testing DepthwiseConv2D, %dth config: %r * %r, stride: %d, padding: "
                "%s",
                index,
    ...
    def _VerifyHandValues(
        self, tensor_in_sizes, filter_in_sizes, stride, padding, expected, use_gpu
    ):
    ...
    def testDepthwiseConv2DInputGradCudnn(self):
        for index, (
            ...
            padding,
            dilations,
        ) in enumerate(CheckGradConfigsToTest()):
    ...
    def testDepthwiseConv2DInputGradExplicit(self):
        for index, (
            ...
            padding,
            dilations,
        ) in enumerate(CheckGradConfigsToTestExplicit()):
            tf_logging.info(
    ...
    def _CompareBackpropInput(
        self, input_sizes, filter_sizes, output_sizes, stride, padding, dtype
    ):
    ...
    def testDepthwiseConv2DInputGradCompare(self):
        for index, (
            ...
            self._CompareBackpropInput(
                input_size, filter_size, output_size, stride, padding, "float32"
            )
            # double datatype is currently not supported for convolution ops
            # on the ROCm platform
...
class DepthwiseConv2DDeterministicTest(test.TestCase):
    ...
    def _genParams(
        self, use_cudnn=False, data_format="NHWC", dtype=dtypes.float32, seed=123
    ):
    ...
    def _testForwardCase(
        self, use_cudnn=False, data_format="NHWC", dtype=dtypes.float32
    ):
        for seed in range(5):
    ...
    def _testBackwardCase(
        self, using_gpu=False, use_cudnn=False, data_format="NHWC", dtype=dtypes.float32
    ):
        ...
        with test_util.deterministic_ops():
            ...
            with ctx_mgr:
                ...
                for seed in (987, 988):
                    ...
                    input_gradients_a, filter_gradients_a = Gradients(
                        upstream_gradients
                    )
                    input_gradients_b, filter_gradients_b = Gradients(
                        upstream_gradients
                    )
