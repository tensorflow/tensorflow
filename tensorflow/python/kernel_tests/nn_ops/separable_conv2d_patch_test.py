...
class SeparableConv2DTest(test.TestCase, parameterized.TestCase):
  ...
  def testNegativeStride(self):
    ...
    depthwise_filter = constant_op.constant(np.random.rand(3, 3, 3, 1),
                                            dtype=dtypes.float32)
    pointwise_filter = constant_op.constant(np.random.rand(1, 1, 3, 2),
                                            dtype=dtypes.float32)
  ...
  def testNonIntegerStride(self):
    self.skipTest(("Non-integer strides are silently truncated in current "
                   "TensorFlow implementation; no TypeError raised."))
  ...
  def testInvalidStridesFormat(self):
    ...
    with self.assertRaisesRegex(
        (ValueError, errors.InvalidArgumentError),
        ("Current implementation does not yet support strides in the batch "
