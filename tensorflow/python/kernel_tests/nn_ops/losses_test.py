...
class AbsoluteDifferenceLossTest(test.TestCase):
    ...
    def testNonZeroLossWithScalarTensorWeight(self):
        ...
        loss = losses.absolute_difference(
            self._labels, self._predictions, constant_op.constant(weights)
        )
        with self.cached_session():
            self.assertAlmostEqual(5.5 * weights, self.evaluate(loss), 3)
    ...
    def testNonZeroLossWithOneDimBatchSpecificWeights(self):
        ...
        with self.cached_session():
            ...
            self.assertAlmostEqual(
                (1.2 + 3.4 + 5.6) * 10.0 / 3.0, self.evaluate(loss), 3
            )
...
class SparseSoftmaxCrossEntropyLossTest(test.TestCase):
    ...
    def testNonZeroLossWithPlaceholderForWeights(self):
        ...
        with self.cached_session() as sess:
            loss = losses.sparse_softmax_cross_entropy(labels, logits, weights)
            loss_val = sess.run(loss, feed_dict={weights: ((1.2,), (3.4,), (5.6,))})
    ...
    def testUnknownShapePlaceholderForLogitsLabelsButScalarWeights(self):
        ...
        with self.cached_session() as sess:
            ...
            loss_val = sess.run(
                loss,
                feed_dict={
                    logits: [[10.0, 0.0, 0.0], [0.0, 10.0, 0.0], [0.0, 0.0, 10.0]],
                    labels: [[2], [0], [1]],
                },
            )
...
class SigmoidCrossEntropyLossTest(test.TestCase):
    ...
    def testSigmoidFloat64(self):
        ...
        labels = constant_op.constant(
            ((1, 0, 1), (1, 1, 0), (0, 1, 1)), dtype=dtypes.int64
        )
        loss = losses.sigmoid_cross_entropy(labels, logits)
        self.assertEqual(logits.dtype, loss.dtype)
    ...
    def testSigmoidNoReduction(self):
        ...
        with self.cached_session():
            self.assertAllClose(
                ((0.0, 0.0, 0.0), (0.0, 100.0, 100.0), (100.0, 0.0, 100.0)),
                self.evaluate(loss),
                3,
            )
...
class LogLossTest(test.TestCase):
    def setUp(self):
        ...
        predictions = np.asarray([0.9, 0.2, 0.2, 0.8, 0.4, 0.6]).reshape((2, 3))
    ...
    def testNonZeroLoss(self):
        ...
        with self.cached_session():
            self.assertAlmostEqual(
                -np.sum(self._expected_losses) / 6.0, self.evaluate(loss), 3
            )
    ...
    def testNonZeroLossWithPythonScalarWeight(self):
        ...
        with self.cached_session():
            self.assertAlmostEqual(
                weights * -np.sum(self._expected_losses) / 6.0, self.evaluate(loss), 3
            )
    ...
    def testNonZeroLossWithScalarTensorWeightAndPlaceholder(self):
        ...
        weights = 2.3
        loss = losses.log_loss(
            self._labels, tf_predictions, constant_op.constant(weights)
        )
        with self.cached_session() as sess:
            loss = sess.run(loss, feed_dict={tf_predictions: self._np_predictions})
            self.assertAlmostEqual(
                weights * -np.sum(self._expected_losses) / 6.0, loss, 3
            )
    ...
    def testNonZeroLossWithOneDimBatchSpecificWeights(self):
        ...
        with self.cached_session():
            self.assertAlmostEqual(
                -np.sum(expected_losses) / 6.0, self.evaluate(loss), 3
            )

    def testNonZeroLossWithOneDimBatchSpecificWeightsSomeZero(self):
        weights = constant_op.constant((1.2, 0), shape=(2, 1))
        expected_losses = np.multiply(
            self._expected_losses, np.asarray([1.2, 1.2, 1.2, 0, 0, 0]).reshape((2, 3))
        )
        loss = losses.log_loss(self._labels, self._predictions, weights)
        with self.cached_session():
            self.assertAlmostEqual(
                -np.sum(expected_losses) / 3.0, self.evaluate(loss), 3
            )
    ...
    def testNonZeroLossWithMeasurementSpecificWeights(self):
        ...
        loss = losses.log_loss(
            self._labels, self._predictions, constant_op.constant(weights, shape=(2, 3))
        )
        with self.cached_session():
            self.assertAlmostEqual(
                -np.sum(expected_losses) / 5.0, self.evaluate(loss), 3
            )
    ...
    def testNonZeroLossWithMeasurementSpecificWeightsWithPlaceholder(self):
        ...
        loss = losses.log_loss(
            self._labels, tf_predictions, constant_op.constant(weights, shape=(2, 3))
        )
...
class HuberLossTest(test.TestCase):
    ...
    def testAllQuadratic(self):
        with self.cached_session():
            ...
            self.assertAllClose(loss, 0.5 * (0.25 + 0.16 + 1.0 + 0.25) / 4.0, atol=1e-5)
    ...
    def testAllLinear(self):
        with self.cached_session():
            ...
            loss = losses.huber_loss(labels, predictions)
            self.assertAllClose(loss, (1.5 + 2.4 + 1.0 + 1.5) / 4.0 - 0.5, atol=1e-5)

    @test_util.run_deprecated_v1
    def testMixedQuadraticLinear(self):
        with self.cached_session():
            predictions = constant_op.constant(
                [[1.5, -1.4, -1.0, 0.0], [1.5, -1.4, -1.0, 0.0]]
            )
            labels = constant_op.constant([[1.0, -1.0, 0.0, 0.5], [0.0, 1.0, 0.0, 1.5]])
            loss = losses.huber_loss(labels, predictions)
            quadratic = 0.5 * (0.25 + 0.16 + 1.0 + 0.25) / 4.0
            linear = (1.5 + 2.4 + 1.0 + 1.5) / 4.0 - 0.5
            expected_loss = (quadratic + linear) / 2.0
    ...
    def testScalar(self):
        with self.cached_session():
            self.assertEqual(
                0.0,
                losses.mean_squared_error(
                    predictions=constant_op.constant(0), labels=constant_op.constant(0)
                ).eval(),
            )
...
class MeanPairwiseSquaredErrorTest(test.TestCase):
    ...
    def _test_valid_weights(self, labels, predictions, expected_loss, weights=1.0):
        with self.cached_session():
    ...
    def testAllCorrectNoLossWeight(self):
        self._test_valid_weights(self._labels, self._labels, expected_loss=0.0)

    def testNonZeroLoss(self):
        self._test_valid_weights(
            self._labels, self._predictions, expected_loss=np.sum(self._expected_losses)
        )
    ...
    def testNonZeroLossWithScalarTensorWeight(self):
        ...
        with self.cached_session():
            self.assertAlmostEqual(
                weights * np.sum(self._expected_losses), self.evaluate(loss), 3
            )
    ...
    def _test_invalid_weights(self, labels, predictions, weights=1.0):
    ...
    def testInvalid3dWeighted2x0(self):
        ...
        self._test_invalid_weights(labels, predictions, weights=np.asarray((1.2, 3.4)))
    ...
    def testLossWithAllZeroBatchSpecificWeights(self):
        self._test_valid_weights(
            self._labels, self._predictions, expected_loss=0.0, weights=np.zeros((2, 1))
        )
    ...
    def testLossIsAssociativeAcrossBatchElements(self):
        with ops.Graph().as_default():
            ...
            loss0 = losses.mean_pairwise_squared_error(
                labels=labels0, predictions=predictions0
            )
            loss1 = losses.mean_pairwise_squared_error(
                labels=labels1, predictions=predictions1
            )
...
class CosineDistanceLossTest(test.TestCase):
    ...
    def testPartiallyCorrectFloatingPointValues(self):
        ...
        tf_labels = constant_op.constant(labels, shape=(3, 1, 3), dtype=dtypes.float32)
    ...
    def testMeasurementSpecificWeights(self):
        loss = losses.cosine_distance(
            ...
            labels=constant_op.constant(self._labels),
            dim=2,
            weights=constant_op.constant([1, 0, 0, 1, 1, 1], shape=(3, 2, 1)),
        )
        with self.cached_session():
            self.assertEqual(3.0 / 4.0, self.evaluate(loss))

    def testMeasurementSpecificWeightsWithPlaceholderWithShape(self):
        tf_predictions = array_ops.placeholder(dtypes.float32, shape=self._labels.shape)
        loss = losses.cosine_distance(
                ...
                with self.session(g):
                    for unweighted_loss in unweighted_losses:
                        if reduction == losses.Reduction.NONE:
                            self.assertAllClose(
                                self._raw_losses, self.evaluate(unweighted_loss)
                            )
                ...
                with self.session(g):
                    for unweighted_loss in unweighted_losses:
                        if reduction == losses.Reduction.NONE:
                            ...
                            self.assertAllClose(
                                np.sum(self._raw_losses),
                                unweighted_loss.eval(feed_dict),
                            )
                        else:
                            # reduction one of MEAN, SUM_OVER_NONZERO_WEIGHTS,
                            # SUM_BY_NONZERO_WEIGHTS or SUM_OVER_BATCH_SIZE.
        ...
        for reduction in losses.Reduction.all():
            with ops.Graph().as_default() as g:
                ...
                with self.session(g):
                    ...
                    if reduction == losses.Reduction.NONE:
                        self.assertAllClose(
                            weighted_losses, self.evaluate(weighted_loss)
                        )
                    elif reduction == losses.Reduction.SUM:
                        ...
                        if reduction == losses.Reduction.MEAN:
                            ...
                            or reduction == losses.Reduction.SUM_BY_NONZERO_WEIGHTS
                        ):
                            self.assertAllClose(
