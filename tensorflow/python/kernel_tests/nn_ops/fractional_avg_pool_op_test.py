...
class FractionalAvgTest(test.TestCase):
    ...
    def _AvgPoolAlongRows(self, input_matrix, row_seq, overlapping):
        ...
        for i in range(row_seq.shape[0] - 1):
            ...
            output_image = np.vstack(
                (output_image, np.mean(input_matrix[row_start:row_end, :], axis=0))
            )  # axis 0 is along row
    ...
    def _GetExpectedFractionalAvgPoolResult(
        self, input_tensor, row_seq, col_seq, overlapping
    ):
    ...
    def _ValidateFractionalAvgPoolResult(
        self, input_tensor, pooling_ratio, pseudo_random, overlapping
    ):
        ...
        with self.cached_session() as sess:
            p, r, c = nn_ops.fractional_avg_pool_v2(
                input_tensor, pooling_ratio, pseudo_random, overlapping, seed=self._SEED
            )
    ...
    def testLargePoolingRatio(self):
        ...
        for row_ratio in [math.sqrt(11), math.sqrt(37)]:
            for col_ratio in [math.sqrt(11), math.sqrt(27)]:
                ...
                rand_mat = self._PRNG.random_sample(tensor_shape) * 1000 - 500
                self._ValidateFractionalAvgPoolResult(
                    rand_mat, [1, row_ratio, col_ratio, 1], pseudo_random, overlapping
                )
    ...
    def testDivisiblePoolingRatio(self):
        ...
        rand_mat = self._PRNG.random_sample(tensor_shape) * 1000 - 500
        self._ValidateFractionalAvgPoolResult(
            rand_mat, [1, 2, 2, 1], pseudo_random, overlapping
        )
    ...
    def testDifferentInputTensorShape(self):
        ...
        with self.cached_session() as sess:
            ...
            overlapping = False
            p, r, c = nn_ops.fractional_avg_pool_v2(
                input_holder, pooling_ratio, pseudo_random, overlapping, seed=self._SEED
            )
            # First run.
    ...
    def testPoolingIllegalRatioForBatch(self):
        with self.cached_session() as _:
            with self.assertRaises(errors.UnimplementedError):
                result = nn_ops.gen_nn_ops.fractional_avg_pool(
                    np.zeros([3, 30, 50, 3]), [2, 3, 1.5, 1], True, True
                )
                self.evaluate(result)
                    ...
                    for num_channels in [1, 2]:
                        ...
                        with self.cached_session() as _:
                            ...
                            for dim_size in output_data.shape:
                                num_elements *= dim_size
                            output_backprop = (
                                self._PRNG.rand(num_elements) * 1000
                            ...
                            self.assertShapeEqual(
                                input_backprop, fap_input_backprop_tensor
                            )
                ...
                for num_cols in [5, 11]:
                    for num_channels in [1, 3]:
                        ...
                        with self.cached_session() as _:
                            ...
                            output_tensor, unused_a, unused_b = (
        ...
        with self.assertRaises((errors.InvalidArgumentError, ValueError)):
            with self.cached_session() as _:
                ...
                out_backprop = constant_op.constant(
                    [], shape=[0, 0, 0, 0], dtype=dtypes.float64
                )
