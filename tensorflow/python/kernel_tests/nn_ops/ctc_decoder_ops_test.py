...
class CTCGreedyDecoderTest(test.TestCase):
    ...
    def testCTCGreedyDecoder(self):
        ...
        decode_truth = [
            (
                ...
                np.array(
                    [0, 1, 1, 1, 0],  # batch 0, 2 values  # batch 1, 3 values
                    dtype=np.int64,
                ),
        ...
        self._testCTCDecoder(
            ctc_ops.ctc_greedy_decoder, inputs, seq_lens, log_prob_truth, decode_truth
        )
    ...
    def testCTCDecoderBeamSearch(self):
        ...
        log_prob_truth = np.array(
            [-5.811451, -6.63339], np.float32  # output beam 0  # output beam 1
        )[np.newaxis, :]
