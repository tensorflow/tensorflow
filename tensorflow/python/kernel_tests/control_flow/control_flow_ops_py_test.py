...
class ControlFlowTest(test.TestCase, parameterized.TestCase):
    ...
    def testSwitchCaseAccessBranch1TensorInBranch4Raises(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "Tensor %sbr1_identity:0 in branch 1 is accessed from branch 4." % prefix,
        ):
            f()
    ...
    def testNestedCond_Simple(self):
        with self.cached_session():
            ...
            y = tf_cond.cond(
                constant_op.constant(True),
                lambda: x,
                lambda: tf_cond.cond(x < 1.0, lambda: x, lambda: x),
            )
            ...
            def b(i, x):
                return [
                    ...
                    indexed_slices.IndexedSlices(
                        x.values * 2.0, x.indices, x.dense_shape
                    ),
                ]
        ...
        with self.cached_session(config=config) as sess:
            ...
            with ops.device("/cpu:0"):
                z = while_loop_tf.while_loop(
                    ...
                    lambda i, x: (
                        i + 1,
                        tf_cond.cond(pred, lambda: x * 2.0, lambda: 10.0),
                    ),
                    [0, x_init],
                )
    ...
    def testWhileWithRefsWithGradients_1(self):
        with self.cached_session() as sess:
            x = variable_v1.VariableV1(0.0)._ref()  # pylint: disable=protected-access
            i = constant_op.constant(0)
            c = lambda i, x: math_ops.less(i, 10)
    ...
    def testCaseSideEffects(self):
        with self.cached_session() as sess:
            ...
            r0 = control_flow_case.case(
                ((x < y, a), (x > y, b)), default=c, exclusive=True
            )
            r1 = control_flow_case.case(
                ((x > y, a), (x < y, b)), default=c, exclusive=True
            )
            r2 = control_flow_case.case(
                ((x > y, a), (x > y, b)), default=c, exclusive=True
            )
