...
class ResourceVariableOpsTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testGPUInteger(self, dtype):
        with context.eager_mode(), ops.device("gpu:0"):
            ...
            self.evaluate(
                v.scatter_add(
                    indexed_slices.IndexedSlices(
                        indices=[1], values=constant_op.constant([2], dtype=dtype)
                    )
                )
            )
            ...
            self.evaluate(
                v.scatter_update(
                    indexed_slices.IndexedSlices(
                        indices=[1], values=constant_op.constant([5], dtype=dtype)
                    )
                )
            )
            ...
            self.evaluate(
                v.scatter_max(
                    indexed_slices.IndexedSlices(
                        indices=[0, 1], values=constant_op.constant([2, 2], dtype=dtype)
                    )
                )
            )
    ...
    def testInitializeVariableUsingInitializedValue(self):
        ...
        var2 = resource_variable_ops.ResourceVariable(
            tf_cond.cond(
                variable_v1.is_variable_initialized(var1),
                var1.read_value,
        ...
        self.assertAllEqual(
            tf_cond.cond(
                variable_v1.is_variable_initialized(var2),
                var2.read_value,
    ...
    def testAssignVariableDtypeMismatchEager(self):
        with context.eager_mode():
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError,
                r"Trying to .* variable with wrong "
    ...
    def testReprUnavailable(self):
        with context.eager_mode():
            ...
            text = "%r" % v
            self.assertEqual(
                "<tf.Variable 'Variable:0' shape=() dtype=int32, numpy=<unavailable>>",
                text,
            )
    ...
    def testGradientCompositeVariable(self):
        composite_variable = CompositeVariable(
            resource_variable_ops.ResourceVariable([1.0, 2.0, 3.0])
        )
    ...
    def testVariableDefInitializedInstances(self):
        ...
        with ops.Graph().as_default(), self.cached_session():
            ...
            with self.assertRaises(ValueError):
                self.evaluate(
                    tf_cond.cond(
                        variable_v1.is_variable_initialized(v),
                        v.read_value,
    ...
    def testAssignDifferentShapes(self):
        with (
            ...
            variable_scope.variable_scope("foo", use_resource=True),
        ):
    ...
    def testUniqueIdPreservedThroughPackAndUnpack(self):
        ...
        reconstructed_v = nest.pack_sequence_as(
            v, nest.flatten(v, expand_composites=True), expand_composites=True
        )
