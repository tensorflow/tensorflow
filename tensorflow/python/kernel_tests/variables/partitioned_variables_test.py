...
class PartitionerCreatorsTest(test.TestCase):
    ...
    def _testVariableAxisSizePartitioner(
        self,
        name,
        axis,
        max_shard_bytes,
    ...
    def testVariableAxisSizePartitioner(self):
        with self.cached_session():
            ...
            partitioner_axis3_str = (
                partitioned_variables.variable_axis_size_partitioner(  # pylint: disable=line-too-long
    ...
    def _testMinMaxVariablePartitioner(
        self,
        max_partitions,
        axis,
        min_slice_size,
...
class PartitionedVariablesTestCase(test.TestCase):
    ...
    def testRandomInitUnevenPartitions(self):
        with self.cached_session():
            ...
            for i, vs in enumerate(var_lists):
                ...
                self.assertEqual(
                    [dtypes.float64] * len(vs), [v.dtype.base_dtype for v in vs]
                )
                if i < len(save_specs):
    ...
    def testIotaInitializer(self):
        ...
        self.assertAllClose(
            [[0.0, 1.0], [0.0, 10.0], [0.0, 100.0], [0.0, 1000.0]],
            _IotaInitializer([4, 2]),
        )
        with self.cached_session():
            vs = partitioned_variables.create_partitioned_variables(
                [13, 5], [3, 1], _IotaInitializer
            )
            self.evaluate(variables.global_variables_initializer())
    ...
    def testConcat(self):
        with self.cached_session() as session:
            var_x = variable_scope.get_variable(
                "x",
                initializer=constant_op.constant([1.0, 2.0]),
                partitioner=partitioned_variables.variable_axis_size_partitioner(4),
            )
