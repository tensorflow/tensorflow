...
def initialized_value(var):
    return tf_cond.cond(
        variable_v1.is_variable_initialized(var),
        var.read_value,
...
class VariablesTestCase(test.TestCase, parameterized.TestCase):
    ...
    def testIterableV2(self):
        ...
        for v in variables.Variable([0.0, 1.0]):
            values.append(v)
        self.assertAllClose([0.0, 1.0], values)
    ...
    def testAssignDifferentShapesAllowed(self):
        var = variables.Variable(
            np.zeros(shape=[1, 1]), shape=tensor_shape.TensorShape(None)
        )
        self.evaluate(variables.global_variables_initializer())
    ...
    def testZeroSizeVarSameAsConst(self):
        with self.cached_session():
            ...
            self.assertAllClose([[0.0, 0.0], [0.0, 0.0]], variable_output)
    ...
    def testCollections(self):
        with self.cached_session():
            ...
            self.assertEqual([var_x, var_y, var_z, var_t], variables.global_variables())
    ...
    def testConstraintArg(self):
        ...
        v = variables.Variable(lambda: constant_op.constant(1.0), constraint=constraint)
        self.assertEqual(v.constraint, constraint)

        constraint = 0
        with self.assertRaises(ValueError):
            v = variables.Variable(
                lambda: constant_op.constant(1.0), constraint=constraint
            )
    ...
    def testVariableDefInitializedInstances(self):
        with ops.Graph().as_default(), self.cached_session() as sess:
            v_def = variables.Variable(
                initial_value=constant_op.constant(3.0)
            ).to_proto()
    ...
    def testTrainableInProto(self):
        with ops.Graph().as_default():
            non_trainable_variable = variables.Variable(
                trainable=False, initial_value=constant_op.constant(10.0)
            )
            self.assertEqual(
                False,
                variables.Variable(
            ...
            trainable_variable = variables.Variable(
                trainable=True, initial_value=constant_op.constant(10.0)
            )
            self.assertEqual(
                True,
                variables.Variable(
    ...
    def testSynchronizationAndAggregationSaved(self):
        with ops.Graph().as_default():
            ...
            self.assertEqual(
                variables.VariableAggregation.ONLY_FIRST_REPLICA,
                original_variable.aggregation,
            )

            laundered = variables.Variable(variable_def=original_variable.to_proto())
            self.assertEqual(
                variables.VariableSynchronization.NONE, laundered.synchronization
            )
            self.assertEqual(
                variables.VariableAggregationV2.ONLY_FIRST_REPLICA,
    ...
    def testRepr(self):
        ...
        self.assertEqual(
            "<tf.Variable 'noop:0' shape=(5, 5) dtype=float32_ref>", repr(var)
        )
...
class IsInitializedTest(test.TestCase):
    ...
    def testTrainingWithZeroSizeVar(self):
        with ops.Graph().as_default(), self.cached_session() as sess:
            ...
            objective = math_ops.reduce_sum(b + math_ops.matmul(a, a, transpose_a=True))
            self.evaluate(variables.global_variables_initializer())
            do_opt = gradient_descent.GradientDescentOptimizer(0.1).minimize(objective)
