...
class PaddingFIFOQueueTest(test.TestCase):
    ...
    def testConstructorWithShapes(self):
        with ops.Graph().as_default():
            q = data_flow_ops.PaddingFIFOQueue(
                ...
                (dtypes_lib.int32, dtypes_lib.float32),
                shapes=(
                    tensor_shape.TensorShape([1, 1, 2, 3]),
    ...
    def testConstructPaddingFIFOQueueWithNoShape(self):
        with self.cached_session():
            with self.assertRaisesRegex(
                ...
                self.evaluate(
                    data_flow_ops.PaddingFIFOQueue(
                        10, dtypes_lib.float32, None
                    ).queue_ref
                )
    ...
    def testMultiEnqueueMany(self):
        with self.cached_session() as sess:
            q = data_flow_ops.PaddingFIFOQueue(
                10, (dtypes_lib.float32, dtypes_lib.int32), ((), (2,))
            )
    ...
    def testMultiDequeueManyWithPartiallyKnownShapesAndVariableSizeInput(self):
        with self.cached_session() as sess:
            q = data_flow_ops.PaddingFIFOQueue(
                10, (dtypes_lib.string, dtypes_lib.int32), shapes=((None,), (1, None))
            )
            str_elems = [
                ["a"],
                ["ab"],
    ...
    def testPartiallyKnownHighDimension(self):
        with self.cached_session():
            q = data_flow_ops.PaddingFIFOQueue(
                10, dtypes_lib.int32, ((4, None, 4, None),)
            )
            elems = np.array([[[[[x] * 4] * 4] * 4] * 4 for x in range(10)], np.int32)
            enqueue_op = q.enqueue_many((elems,))
            dequeued_t = q.dequeue_many(10)
    ...
    def testEnqueueWrongShape(self):
        q = data_flow_ops.PaddingFIFOQueue(
            10, (dtypes_lib.int32, dtypes_lib.int32), ((), (2,))
        )
    ...
    def testBatchSizeMismatch(self):
        q = data_flow_ops.PaddingFIFOQueue(
            10, (dtypes_lib.int32, dtypes_lib.int32, dtypes_lib.int32), ((), (), ())
        )
    ...
    def testEnqueueWrongPartiallyKnownShapeAtRuntime(self):
        with self.cached_session() as sess:
            ...
            # dimension must be 3.
            q = data_flow_ops.PaddingFIFOQueue(
                10, (dtypes_lib.int32, dtypes_lib.int32), ((2, 2), (None, 3))
            )
            ...
            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, r"Expected \[\?,3\], got \[3,4\]"
            ):
                sess.run(
                    [enqueue_op],
    ...
    def testDequeueFromClosedQueue(self):
        with self.cached_session():
            ...
            with self.assertRaisesRegex(
                errors_impl.OutOfRangeError, "is closed and has insufficient"
            ):
                self.evaluate(dequeued_t)
    ...
    def testClosedBlockingDequeueManyRestoresPartialBatch(self):
        with self.cached_session() as sess:
            q = data_flow_ops.PaddingFIFOQueue(
                4, (dtypes_lib.float32, dtypes_lib.float32), ((), ())
            )
            elems_a = [1.0, 2.0, 3.0]
    ...
    def testBigEnqueueMany(self):
        with self.cached_session() as sess:
            ...
            results.append(
                self.evaluate(deq)
            )  # Will only complete after the enqueue starts.
    ...
    def testUnknownRank(self):
        with self.assertRaisesRegex(ValueError, "must have a defined rank"):
            data_flow_ops.PaddingFIFOQueue(
                32, [dtypes_lib.float32], [tensor_shape.TensorShape(None)]
            )
...
class QueueFromListTest(test.TestCase):
    def testQueueFromListShapes(self):
        ...
        def _cmp(expected, *shapes):
            qs = [
                data_flow_ops.PaddingFIFOQueue(
                    10, [dtypes_lib.float32], [tensor_shape.TensorShape(s)]
                )
                for s in shapes
            ]
