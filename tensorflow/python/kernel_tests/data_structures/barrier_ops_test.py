...
class BarrierTest(test.TestCase):
    ...
    def testInsertManyEmptyTensor(self):
        with self.cached_session():
            error_message = (
                "Empty tensors are not supported, but received shape "
                r"\'\(0,\)\' at index 1"
            )
    ...
    def testTakeManySmallBatch(self):
        with self.cached_session() as sess:
            ...
            self.assertEqual(
                self.evaluate(size_i), [2]
            )  # assert that incomplete size = 2
            ...
            insert_1_2_op.run()
            self.assertEqual(
                self.evaluate(size_i), [1]
            )  # assert that incomplete size = 1
    ...
    def testParallelInsertManyTakeMany(self):
        with self.cached_session() as sess:
            ...
            insert_0_ops = [
                b.insert_many(0, keys_i(i), values_0 + i) for i in range(num_iterations)
            ]
            insert_1_ops = [
                b.insert_many(1, keys_i(i), values_1 + i) for i in range(num_iterations)
            ]
    ...
    def _testParallelPartialInsertManyTakeManyCloseHalfwayThrough(self, cancel):
        with self.cached_session() as sess:
            ...
            take_ops = [
                b.take_many(10, name="take_%d" % i) for i in range(num_iterations)
            ]
            ...
            def take(sess, i, taken):
                if cancel:
                    try:
                        indices_val, unused_keys_val, unused_val_0, unused_val_1 = (
                            sess.run(
                                [
                                    take_ops[i][0],
                                    take_ops[i][1],
