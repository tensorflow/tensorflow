...
class StaticHashTableTest(BaseLookupTableTest, parameterized.TestCase):
    ...
    def testDTypes(self, is_anonymous):
        ...
        with self.assertRaises(TypeError):
            self.getHashTable()(
                lookup_ops.KeyValueTensorInitializer(
                    ["a"], [1], [dtypes.string], dtypes.int64
                ),
                default_val,
                experimental_is_anonymous=is_anonymous,
            )
    ...
    def testImportedHashTable(self, is_anonymous):
        ...
        with g.as_default():
            t = lookup_ops.StaticHashTable(
                lookup_ops.KeyValueTensorInitializer(["a"], [1]), 2
            )
            init_op = t._init_op
    ...
    def test_multiple_tables(self, is_anonymous):
        with ops.name_scope("table_scope"):
            ...
            if not context.executing_eagerly():
                ...
                self.assertEqual(
                    "table_scope/hash_table", table1.resource_handle.op.name
                )
            ...
            if not context.executing_eagerly():
                ...
                self.assertEqual(
                    "table_scope/hash_table_1", table2.resource_handle.op.name
                )
...
class InitializeTableFromFileOpTest(BaseLookupTableTest):
    ...
    def testIdToStringTable(self, is_anonymous):
        ...
        with self.cached_session():
            ...
            self.assertAllEqual(
                [b"brain", b"salad", b"surgery", b"UNK"], self.evaluate(out)
            )
...
class StaticVocabularyTableTest(BaseLookupTableTest):
    ...
    def testStringStaticVocabularyTable(self, is_anonymous):
        ...
        table = self.getVocabularyTable()(
            lookup_ops.TextFileIdTableInitializer(vocab_file, vocab_size=vocab_size),
            oov_buckets,
            experimental_is_anonymous=is_anonymous,
        )
    ...
    def testStaticVocabularyTableWithMultipleInitializers(self, is_anonymous):
        ...
        table1 = self.getVocabularyTable()(
            init, oov_buckets, name="table1", experimental_is_anonymous=is_anonymous
        )

        table2 = self.getVocabularyTable()(
            init, oov_buckets, name="table2", experimental_is_anonymous=is_anonymous
        )
    ...
    def testSparseTensor(self, is_anonymous):
        ...
        sp_features = sparse_tensor.SparseTensor(
            ...
            constant_op.constant(
                ["brain", "salad", "brain", "surgery", "tarkus"], dtypes.string
            ),
...
class DenseHashTableOpTest(test.TestCase):
    ...
    def testSameEmptyAndDeletedKey(self, is_anonymous):
        with self.assertRaisesRegex(
            errors_impl.InvalidArgumentError, "Empty and deleted keys"
        ):
            table = lookup_ops.DenseHashTable(
                dtypes.int64,
                dtypes.int64,
    ...
    def testVectorValues(self, is_anonymous):
        ...
        values = constant_op.constant(
            [[0, 1, 2, 3], [3, 4, 5, 6], [6, 7, 8, 9]], dtypes.int64
        )
        ...
        result = self.evaluate(output)
        self.assertAllEqual(
            [[0, 1, 2, 3], [-1, -2, -3, -4], [2, 3, 4, 5], [-1, -2, -3, -4]], result
        )
    ...
    def testVectorSaveRestore(self, is_anonymous):
        ...
        with self.session(graph=ops.Graph()) as sess:
            ...
            keys = constant_op.constant(
                [[11, 12], [11, 14], [12, 13], [13, 14]], dtypes.int64
            )
            values = constant_op.constant(
                [[0, 1], [2, 3], [2, 4], [4, 5]], dtypes.int64
            )
            table = lookup_ops.DenseHashTable(
                dtypes.int64,
                dtypes.int64,
        ...
        with self.session(graph=ops.Graph()) as sess:
            ...
            table.insert(
                ...
                constant_op.constant([[21, 22], [23, 24]], dtypes.int64),
            ).run()
            self.assertAllEqual(2, table.size())
            self.assertAllEqual(64, len(table.export()[0].eval()))
            ...
            output = table.lookup(input_string)
            self.assertAllEqual(
                [[0, 1], [2, 3], [-1, -2], [4, 5], [-1, -2]], self.evaluate(output)
            )
    ...
    def testVectorScalarSaveRestore(self, is_anonymous):
        ...
        with self.session(graph=ops.Graph()) as sess:
            ...
            table.insert(
                constant_op.constant([[11, 12], [13, 15]], dtypes.int64),
                constant_op.constant([3, 4], dtypes.int64),
            ).run()
            self.assertAllEqual(2, table.size())
            self.assertAllEqual(64, len(table.export()[0].eval()))
    ...
    def testReprobe(self, is_anonymous):
        ...
        input_string = constant_op.constant(
            [10, 11, 12, 13, 14, 19, 20, 21, 22], dtypes.int64
        )
        output = table.lookup(input_string)
    ...
    def testErrors(self, is_anonymous):
        ...
        with self.assertRaisesRegex(
            errors_impl.InvalidArgumentError, "Expected key shape"
        ):
        ...
        with self.assertRaisesRegex(
            errors_impl.InvalidArgumentError, "Number of buckets must be"
        ):
        ...
        with self.assertRaisesRegex(
            errors_impl.InvalidArgumentError, "Empty and deleted keys cannot be equal"
        ):
...
class IndexTableFromFile(test.TestCase):
    ...
    def test_int32_index_table_from_file(self):
        ...
        table = lookup_ops.index_table_from_file(
            vocabulary_file=vocabulary_file, num_oov_buckets=1, key_dtype=dtypes.int32
        )
    ...
    def test_int64_index_table_from_file(self):
        ...
        table = lookup_ops.index_table_from_file(
            vocabulary_file=vocabulary_file, num_oov_buckets=1, key_dtype=dtypes.int64
        )
    ...
    def test_index_table_from_file_str_fails_with_zero_size_vocabulary(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            "`vocab_size` must be greater than 0, got 0 for "
    ...
    def test_index_table_from_file_with_vocab_size_too_large(self):
        ...
        with self.assertRaisesRegex(
            errors_impl.InvalidArgumentError, "Invalid vocab_size"
        ):
            table = lookup_ops.index_table_from_file(
...
class IndexTableFromTensor(test.TestCase):
    ...
    def test_index_table_from_tensor_with_default_value(self):
        default_value = -42
        table = lookup_ops.index_table_from_tensor(
            vocabulary_list=["brain", "salad", "surgery"], default_value=default_value
        )
        ids = table.lookup(constant_op.constant(["salad", "surgery", "tarkus"]))
...
class IndexToStringTableFromFileTest(test.TestCase):
    ...
    def test_index_to_string_table(self):
        ...
        for type_func in type_funcs:
            ...
            self.evaluate(lookup_ops.tables_initializer())
            self.assertAllEqual(
                (b"brain", b"salad", b"surgery", b"UNK"), self.evaluate(features)
            )
    ...
    def test_index_to_string_table_with_default_value(self):
        ...
        self.evaluate(lookup_ops.tables_initializer())
        self.assertAllEqual(
            (b"salad", b"surgery", default_value), self.evaluate(features)
        )
    ...
    def test_index_to_string_table_with_vocab_size_too_small(self):
        ...
        vocabulary_file = self._createVocabFile("f2i_vocab2.txt")
        table = lookup_ops.index_to_string_table_from_file(
            vocabulary_file=vocabulary_file, vocab_size=2, default_value=default_value
        )
        features = table.lookup(constant_op.constant([1, 2, 4], dtypes.int64))
        if not context.executing_eagerly():
            with self.assertRaises(errors_impl.OpError):
        ...
        self.evaluate(lookup_ops.tables_initializer())
        self.assertAllEqual(
            (b"salad", default_value, default_value), self.evaluate(features)
        )
...
class IdTableWithHashBucketsTest(test.TestCase):
    ...
    def testIdTableWithHashBucketsWithMultipleInitializers(self):
        ...
        table1 = lookup_ops.IdTableWithHashBuckets(
            vocab_table, oov_buckets, hasher_spec=lookup_ops.FastHashSpec, name="table1"
        )
