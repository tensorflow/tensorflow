...
class TensorArrayTest(test.TestCase):
    ...
    def testTensorArrayWriteRead(self):
        with self.session():
            ta = tensor_array_ops.TensorArray(
                dtype=dtypes.float32, tensor_array_name="foo", size=3, infer_shape=False
            )
    ...
    def _testTensorArrayWritePack(self, tf_dtype):
        with self.cached_session():
            ...
            c0 = self.evaluate(c0)
            self.assertAllEqual(convert([[[4.0, 5.0]], [[6.0, 7.0]], [[8.0, 9.0]]]), c0)
    ...
    def testTensorArrayWriteConcatInParallel(self):
        with self.session():
            ...
            def _concat_1():
                ...
                w1 = w0.write(
                    1, constant_op.constant([], shape=(0,), dtype=dtypes.int32)
                )
                return w1.concat()
            ...
            def _write(index, output):
                elements = cond.cond(math_ops.less(index, 3), _concat_1, _concat_2)
            ...
            init_state = (
                ...
                tensor_array_ops.TensorArray(
                    dtype=dtypes.int32, size=num_iterations, infer_shape=False
                ),
            )
            _, final_state = while_loop.while_loop(
    ...
    def testSkipEagerTensorArrayGradGrad(self):
        ...
        with self.test_session() as session:
            ...
            ta = tensor_array_ops.TensorArray(
                dtype=dtypes.float32, tensor_array_name="foo", size=1, infer_shape=False
            )
            w0 = ta.write(0, x)
            r0 = w0.read(0)
            y = r0 * r0
    ...
    def testTensorArrayReadWrongIndexOrDataTypeFails(self):
        with self.session():
            ...
            if (
                ...
                and not control_flow_util.ENABLE_CONTROL_FLOW_V2
            ):
    ...
    def testTensorArraySplitIncompatibleShapesFails(self):
        with self.session():
            ...
            error_msg = (
                "Unused values in tensor. Length of tensor: 3 Values used: 1"
                if control_flow_util.ENABLE_CONTROL_FLOW_V2 and not in_eager_mode
                else r"Expected sum of lengths to be equal to values.shape\[0\], "
    ...
    def testSkipEagerTensorArrayGradWithShapeKnownElementShape(self):
        with self.session() as sess:
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "Could not write to TensorArray"
            ):
            ...
            self.assertAllClose(
                fed_value, sess.run(read_value, feed_dict={value: fed_value})
            )
    ...
    def testSkipEagerTensorArrayGradWithShapeUnknownElementShape(self):
        with self.session() as sess:
            ta = tensor_array_ops.TensorArray(
                size=3, dtype=dtypes.float32, element_shape=None
            )  # Note that element_shape is unknown
            handle, flow = data_flow_ops.tensor_array_grad_with_shape(
                handle=ta.handle,
                flow_in=ta.flow,
                shape_to_prepend=tensor_shape.TensorShape([4, 5]),
    ...
    def _testTensorArrayGradientWriteReadType(self, dtype):
        with self.cached_session() as session:
            ...
            grad_r0_r0_2 = gradients_impl.gradients(
                ys=[r0, r0_2], xs=[value_0], grad_ys=[c([[2.0, 3.0]]), c([[1.0, -1.0]])]
            )
    ...
    def testSkipEagerTensorArrayGradientSplitConcat(self):
        with self.session() as session:
            ta = tensor_array_ops.TensorArray(
                dtype=dtypes.float32, tensor_array_name="foo", size=2, infer_shape=False
            )

            value = constant_op.constant([[1.0, -1.0], [10.0, -10.0], [100.0, -100.0]])
    ...
    def _testTensorArrayGradientDynamicUnpackRead(self):
        with self.cached_session() as session:
            ta = tensor_array_ops.TensorArray(
                dtype=dtypes.float32, tensor_array_name="foo", size=0, dynamic_size=True
            )
    ...
    def testWhileLoopWritePackGradients(self):
        self._testWhileLoopWritePackGradients(dynamic_size=False, dtype=dtypes.float32)
    ...
    def testSkipEagerWhileLoopDynamicWritePackGradients(self):
        self._testWhileLoopWritePackGradients(dynamic_size=True, dtype=dtypes.float32)
    ...
    def testGradSerialTwoLoops(self):
        with self.session():
            ...
            def loop(x):
                ...
                _, acc2 = while_loop.while_loop(
                    lambda i, acc: i < num_steps, fn, [i1, acc1]
                )
    ...
    def testShapeAfterWhileLoop(self):
        ...
        _, ta = while_loop.while_loop(
            lambda i, _: i < size,
            lambda i, ta: (i + 1, ta.write(i, [[0.0]])),
            [0, ta],
    ...
    def testSkipEagerGetGradSource_NoEnclosingScope(self):
        ...
        self.assertEqual("gradients_0", self._grad_source_for_name("gradients_0/foo"))
        self.assertEqual("gradients", self._grad_source_for_name("gradients/foo/bar"))
        self.assertEqual(
    ...
    def testSkipEagerGetGradSource_EnclosingScope(self):
        self.assertEqual("foo/gradients:0", self._grad_source_for_name("foo/gradients"))
        self.assertEqual(
        ...
        self.assertEqual(
            "foo/bar/gradients", self._grad_source_for_name("foo/bar/gradients/baz")
        )
        self.assertEqual(
            "foo/bar/gradients_0", self._grad_source_for_name("foo/bar/gradients_0/baz")
        )
                ...
                if not control_flow_util.ENABLE_CONTROL_FLOW_V2:
                    self.assertEqual(
                        tensor_shape.TensorShape(
                            ta1.handle.op.get_attr("element_shape")
                        ).ndims,
                        None,
                    )
    ...
    def testSkipEagerWriteUnknownShape(self):
        with self.session():
            ta = tensor_array_ops.TensorArray(
                dtype=dtypes.float32, tensor_array_name="foo", size=3, infer_shape=True
            )
    ...
    def testSkipEagerWriteButNotAllComponentsReadGrad(self):
        with self.cached_session() as session:
            ...
            ta = (
                tensor_array_ops.TensorArray(dtype=dtypes.float32, size=2)
                .write(0, x0)
                .write(1, x1)
            )
            r0 = ta.read(0)
    ...
    def testGatherShape(self):
        ...
        ta_gather_with_unknown_indices_shape = def_function.function(
            ta_gather,
            input_signature=[tensor_spec.TensorSpec(dtype=dtypes.int32, shape=[None])],
        )
        first_dim = None
    ...
    def _testTensorArrayEvalEmpty(self):
        with self.cached_session():
            ...
            v2_msg = (
                "Tried to stack elements of an empty list with "
    ...
    def testSkipEagerTensorArrayScatterPartialReadAndGradients(self):
        with self.session() as session:
            ...
            # Test combined gradients + aggregation of read(0)
            grad = gradients_impl.gradients(ys=[r0], xs=[value], grad_ys=[[2.0, 3.0]])[
                0
            ]
    ...
    def testScatterIntoExistingList(self):
        ...
        self.assertAllEqual(ta.stack(), [0.0, 0.0, 0.0, 1.0, 1.0])

        ta = ta.scatter(indices=[1], value=array_ops.ones([1]))
        self.assertAllEqual(ta.stack(), [0.0, 1.0, 0.0, 1.0, 1.0])

        ta = ta.scatter(indices=[0, 2], value=[5.0, 6.0])
        self.assertAllEqual(ta.stack(), [5.0, 1.0, 6.0, 1.0, 1.0])
    ...
    def testSkipEagerTensorArrayGetsDeviceFromFirstWrite(self):
        ...
        dev_stats = {d.device: d.node_stats for d in run_metadata.step_stats.dev_stats}
        for d in dev_stats:
            if "/task:1/" in d:
                self.assertTrue(
    ...
    def testSkipEagerTensorArrayGetsDeviceFromFirstWriteInWhileLoop(self):
        ...
        _, ta_out = while_loop.while_loop(lambda i, ta: i < 2, _body, loop_vars=[0, ta])
    ...
    def testSkipEagerTensorArrayDisabledColocateWithFirstWriteCall(self):
        ...
        self.assertTrue(run_metadata.HasField("step_stats"))
        dev_stats = {
            d.device: list(d.node_stats) for d in run_metadata.step_stats.dev_stats
        }
        for d in dev_stats:
    ...
    def testTensorArrayIdentity(self):
        with self.session():
            ...
            read0_v, read1_v, size0_v, size1_v = self.evaluate(
