...
class ScalarStrictTest(tf.test.TestCase):
  ...
  def check(self, op, args, error, correct=None):
    # The op should break with the most recent GraphDef version,
    # and work with GraphDef 3 iff we're within Google.
    if control_imports.USE_OSS:
      strict = [3, tf.GRAPH_DEF_VERSION]
      lenient = []
    else:
      strict = [tf.GRAPH_DEF_VERSION]
      lenient = [3]
