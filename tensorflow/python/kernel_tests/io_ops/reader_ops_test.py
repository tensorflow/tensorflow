...
class WholeFileReaderTest(test.TestCase):
    def setUp(self):
        ...
        self._filenames = [
            os.path.join(self.get_temp_dir(), "whole_file.%d.txt" % i) for i in range(3)
        ]
        self._content = [b"One\na\nb\n", b"Two\nC\nD", b"Three x, y, z"]
...
class FixedLengthRecordReaderTest(TFCompressionTestCase):
    ...
    def _CreateOverlappedRecordFiles(self, num_overlapped_records):
        ...
        for i in range(self._num_files):
            fn = os.path.join(
                self.get_temp_dir(), "fixed_length_overlapped_record.%d.txt" % i
            )
            filenames.append(fn)
            with open(fn, "wb") as f:
                f.write(b"H" * self._header_bytes)
                if num_overlapped_records > 0:
                    all_records_str = "".join(
                        [
                            ...
                            for i in range(
                                self._record_bytes
                                + self._hop_bytes * (num_overlapped_records - 1)
                            )
                        ]
                    )
