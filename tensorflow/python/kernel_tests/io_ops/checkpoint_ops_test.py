...
class GenerateVocabRemappingTest(test.TestCase):
    ...
    def setUp(self):
        self.new_vocab_file = os.path.join(self.get_temp_dir(), "keyword_shifted.txt")
        with open(self.new_vocab_file, "w") as f:
            f.write("\n".join(["MISSING", "knitting", "eminem"]) + "\n")
        self.old_vocab_file = os.path.join(self.get_temp_dir(), "keyword.txt")
...
class LoadAndRemapMatrixTest(test.TestCase):
    ...
    def setUp(self):
        ...
        self.matrix_value = np.reshape(
            range(0, self.old_num_rows * self.old_num_cols),
            (self.old_num_rows, self.old_num_cols),
        )
    ...
    def test_load_and_remap_no_missing(self):
        ...
        with self.cached_session():
            self.assertAllClose(
                self.matrix_value[row_remapping], self.evaluate(remapped_matrix)
            )
    ...
    def test_load_and_remap_with_init(self):
        ...
        with self.cached_session():
            self.assertAllClose(
                expected_remapped_matrix, self.evaluate(remapped_matrix)
            )
    ...
    def test_load_and_remap_invalid_dims(self):
        ...
        initializing_values = constant_op.constant(
            [], shape=[0, 1], dtype=dtypes.float32
        )
        with (
            self.cached_session(),
...
class LoadAndRemapMatrixWithMaxRowsTest(test.TestCase):
    ...
    def _test_loading_variable_with_max_rows(
        self, np_value, partitioner, max_rows_in_memory
    ):
        ...
        with self.cached_session() as sess:
            ...
            self.assertAllClose(
                np.vstack(
                    [
                        np.tile(42, [prefix_rows, num_cols]),
                        np_value[1:-1],
