...
class HistogramFixedWidthTest(test.TestCase):
    ...
    def test_single_bin(self):
        hist = histogram_ops.histogram_fixed_width(
            values=constant_op.constant([3e38, 100], dtype=dtypes.float32),
            value_range=constant_op.constant([-1e38, 3e38]),
            nbins=1,
        )
    ...
    def test_large_range(self):
        hist = histogram_ops.histogram_fixed_width(
            values=constant_op.constant([-(2**31), 2**31 - 1], dtype=dtypes.int32),
            value_range=constant_op.constant([-(2**31), 2**31 - 1], dtype=dtypes.int32),
            nbins=2,
        )
        self.assertAllEqual(hist, [1, 1])
