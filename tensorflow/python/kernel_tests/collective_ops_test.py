...
class CollectiveOpsV2(object):
    ...
    def all_reduce(t, group_size, group_key, instance_key, *args, **kwargs):
        ...
        return _collective_ops.all_reduce_v2(
    ...
    def all_gather(t, group_size, group_key, instance_key, *args, **kwargs):
        ...
        group_key = array_ops.identity(group_key)
        instance_key = array_ops.identity(instance_key)
        return _collective_ops.all_gather_v2(
            t, group_size, group_key, instance_key, *args, **kwargs
        )

    @staticmethod
    def broadcast_send(
        t, shape, dtype, group_size, group_key, instance_key, *args, **kwargs
    ):
        group_size = array_ops.identity(group_size)
        group_key = array_ops.identity(group_key)
        instance_key = array_ops.identity(instance_key)
        return _collective_ops.broadcast_send_v2(
            t, group_size, group_key, instance_key, *args, **kwargs
        )
...
device_combination = combinations.combine(
    device="CPU", communication="RING", required_gpus=0
) + combinations.combine(device="GPU", communication=["RING", "NCCL"], required_gpus=2)

collective_op_combinations = combinations.combine(
    collective_op=[
...
class CollectiveOpsTest(test.TestCase, parameterized.TestCase):
    ...
    def testReduce(self, collective_ops, device, communication):
        ...
        self.assertAllClose(run_all_reduce_1device(), [1.0], rtol=1e-5, atol=1e-5)
        for result in run_all_reduce_2devices():
            self.assertAllClose(result, [2.0], rtol=1e-5, atol=1e-5)
    ...
    def testGather(self, collective_ops, device, communication):
        ...
        self.assertAllClose(run_all_gather_1device(), [1.0], rtol=1e-5, atol=1e-5)
        for result in run_all_gather_2devices():
            self.assertAllClose(result, [1.0, 1.0], rtol=1e-5, atol=1e-5)
    ...
    def testBroadcast(self, collective_ops, device, communication):
        ...
        for result in run_broadcast_2devices():
            self.assertAllClose(result, [1.0, 2.0, 3.0], rtol=1e-5, atol=1e-5)
    ...
    def testInstanceKeyScopedUnderGroupKey(self, collective_ops, device, communication):
        ...
        def run_all_reduce_4devices_same_instance_key():
            ...
            with ops.device(dev0):
                collectives.append(
                    collective_ops.all_reduce(
                        constant_op.constant(1.0), group_size, group0_key, instance_key
                    )
                )
            with ops.device(dev1):
                collectives.append(
                    collective_ops.all_reduce(
                        constant_op.constant(2.0), group_size, group0_key, instance_key
                    )
                )
            with ops.device(dev2):
                collectives.append(
                    collective_ops.all_reduce(
                        constant_op.constant(3.0), group_size, group1_key, instance_key
                    )
                )
            with ops.device(dev3):
                collectives.append(
                    collective_ops.all_reduce(
                        constant_op.constant(4.0), group_size, group1_key, instance_key
                    )
                )
            return collectives

        results = run_all_reduce_4devices_same_instance_key()
        self.assertAllClose(results[0], 3.0, rtol=1e-5, atol=1e-5)
        self.assertAllClose(results[1], 3.0, rtol=1e-5, atol=1e-5)
        self.assertAllClose(results[2], 7.0, rtol=1e-5, atol=1e-5)
        self.assertAllClose(results[3], 7.0, rtol=1e-5, atol=1e-5)
    ...
    def testCollectiveInvalidKey(self, collective_ops, device, communication):
        ...
        with self.assertRaisesRegex(
            ...
            "instance 100 expected type 0 and data_type 1 but"
...
class XlaTest(test.TestCase, parameterized.TestCase):
    def testReduce(self):
        ...
        def all_reduce(device):
            ...
            def f():
                return _collective_ops.all_reduce_v2(
                    [1.0], group_size, group_key, instance_key
                )
...
class OpCancellationTest(test.TestCase, parameterized.TestCase):
            ...
            combinations.combine(
                collective_op=[
                    ...
                    combinations.NamedObject(
                        "all_reduce_v2", CollectiveOpsV2.all_reduce
                    ),
                    combinations.NamedObject("all_gather", CollectiveOpsV1.all_gather),
                    combinations.NamedObject(
                        "all_gather_v2", CollectiveOpsV2.all_gather
                    ),
                ],
                mode="eager",
            ),
    ...
    def testOpErrorAbortWithCollective(self, collective_op, device, communication):
        ...
        dataset = dataset_ops.Dataset.from_tensors([1.0]).apply(
    ...
    def testCancelDuringParamResolution(self, collective_op, device, communication):
        dev0 = "/device:%s:0" % device
        dev1 = "/device:%s:1" % device
        group_size = 2
        group_key = 100
...
class TimeoutTest(test.TestCase, parameterized.TestCase):
    ...
    def testTimeout(self, collective_op, device, communication):
        ...
        with self.assertRaisesRegex(
            errors.DeadlineExceededError, "Collective has timed out during execution"
        ):
            run(1, 2)
...
class CommunicationHintTest(test.TestCase, parameterized.TestCase):
    ...
    @combinations.generate(
        combinations.times(
            collective_op_combinations, combinations.combine(required_gpus=[0, 1])
        )
    )
...
class OrderingTest(test.TestCase, parameterized.TestCase):
    ...
    def testOrdering(self, collective_op, device, communication):
        ...
        with ops.device(dev0):
            token0 = resource_variable_ops.ResourceVariable(0.0)
        with ops.device(dev1):
            token1 = resource_variable_ops.ResourceVariable(0.0)
        ...
        for device in [dev0, dev1]:
            ...
            for op in graph.get_operations():
                if (
                    op.type.startswith("Collective")
                    and op.device.endswith(device)
                    and op.control_inputs
                    and op.control_inputs[0].type.startswith("Collective")
                ):
    ...
    def testAllReduceV3(self, device, communication):
        ...
        def run_all_reduce_2devices():
            ...
            with ops.device(dev0):
                ...
                collectives.append(
                    _collective_ops.all_reduce_v3(group_handle0, [1.0], reduction="Add")
                )
            with ops.device(dev1):
                ...
                collectives.append(
                    _collective_ops.all_reduce_v3(group_handle1, [2.0], reduction="Add")
                )
            return collectives

        for result in run_all_reduce_2devices():
            self.assertAllClose(result, [3.0], rtol=1e-5, atol=1e-5)
    ...
    def testAllToAllV3DifferentUserRankWithTensorInput(self, device, communication):
        ...
        def run_all_to_all_3devices():
            ...
            with ops.device(dev0):
                ...
                collectives.append(
                    _collective_ops.all_to_all_v3(
                        group_handle0, constant_op.constant([1.0, 2.0, 3.0])
                    )
                )
            with ops.device(dev1):
                group_handle1 = _collective_ops.initialize_communicator(
                ...
                collectives.append(
                    _collective_ops.all_to_all_v3(
                        group_handle1, constant_op.constant([4.0, 5.0, 6.0])
                    )
                )
            with ops.device(dev2):
                ...
                collectives.append(
                    _collective_ops.all_to_all_v3(
                        group_handle2, constant_op.constant([7.0, 8.0, 9.0])
                    )
                )
