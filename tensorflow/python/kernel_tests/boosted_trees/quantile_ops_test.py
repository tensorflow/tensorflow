...
from tensorflow.python.ops.gen_boosted_trees_ops import (
    boosted_trees_flush_quantile_summaries as flush_quantile_summaries,
)
from tensorflow.python.ops.gen_boosted_trees_ops import (
    boosted_trees_quantile_stream_resource_handle_op as resource_handle_op,
)
from tensorflow.python.ops.gen_boosted_trees_ops import (
...
class QuantileOpsTest(test_util.TensorFlowTestCase):
    def create_resource(self, name, eps, max_elements, num_streams=1):
        ...
        resources.register_resource(
            quantile_accumulator_handle, create_op, is_initialized_op
        )
    ...
    def testBasicQuantileBucketsSingleResource(self):
        with self.cached_session() as sess:
            quantile_accumulator_handle = self.create_resource(
                "floats", self.eps, self.max_elements, 2
            )
    ...
    def testBasicQuantileBucketsMultipleResources(self):
        with self.cached_session() as sess:
            quantile_accumulator_handle_0 = self.create_resource(
                "float_0", self.eps, self.max_elements
            )
            quantile_accumulator_handle_1 = self.create_resource(
                "float_1", self.eps, self.max_elements
            )
            resources.initialize_resources(resources.shared_resources()).run()
            summaries = boosted_trees_ops.make_quantile_summaries(
                [self._feature_0, self._feature_1],
            ...
            summary_op_0 = boosted_trees_ops.quantile_add_summaries(
                quantile_accumulator_handle_0, [summaries[0]]
            )
            summary_op_1 = boosted_trees_ops.quantile_add_summaries(
                quantile_accumulator_handle_1, [summaries[1]]
            )
