...
class ProdReductionTest(BaseReductionTest):
  ...
  def testXlaZeroWithOverflow(self):
    ...
    # 4. Signed Zeros with Overflow ([-1.0, 1e30, 1e30, 0.0] -> -0.0)
    for neg_inputs in [[-1.0, 1e30, 1e30, 0.0], [1e30, 1e30, -0.0]]:
      x_signed = constant_op.constant(neg_inputs, dtype=dtypes.float32)
    ...
    x_pos_zero = constant_op.constant(
        [-1.0, -1.0, 1e30, 1e30, 0.0], dtype=dtypes.float32
    )
    res_pos_zero = self.evaluate(reduce_prod(x_pos_zero))
    self.assertAllEqual(0.0, res_pos_zero)
    self.assertFalse(np.signbit(res_pos_zero), "Result is not positive zero")
    ...
    # 6. Data Types
    # Note: float16 accumulates in float32 in XLA ProdOp, so 9 elements of
    # 65504.0 (65504^9 ~= 2.4e43 > 3.4e38) are needed to overflow float32.
    dtypes_to_test = [
        (dtypes.float16, 65504.0),
        (dtypes.bfloat16, 1e30),
        (dtypes.float32, 1e30),
        (dtypes.float64, 1e300),
        (dtypes.complex64, 1e30),
        (dtypes.complex64, complex(1e30, 1e30)),
        (dtypes.complex128, 1e300),
        (dtypes.complex128, complex(1e300, 1e300)),
    ]
    for dtype, large_val in dtypes_to_test:
      x_dt = constant_op.constant([large_val] * 9 + [0.0], dtype=dtype)
