            ...
            for dtype, reductions in [
                ...
                for reduction in reductions:
                    x = array_ops.placeholder(
                        dtype=dtype, shape=None
                    )  # Some tensor w/ unknown shape.
                    y = reduction(x)
...
class BaseReductionTest(test.TestCase):
    ...
    def _compare(
        self, x, reduction_axes, keepdims, feed_dict=None, rtol=1e-6, atol=1e-6
    ):
    ...
    def _compareAll(self, x, reduction_axes, feed_dict=None, rtol=1e-6, atol=1e-6):
        ...
        self._compare(
            x, reduction_axes, keepdims=False, feed_dict=feed_dict, rtol=rtol, atol=atol
        )
        self._compare(
            x, reduction_axes, keepdims=True, feed_dict=feed_dict, rtol=rtol, atol=atol
        )
...
class SumReductionTest(BaseReductionTest):
    ...
    def testFloat16(self):
        ...
        self.assertAllClose(tf_out_mean, 1.0)

    @test_util.run_deprecated_v1
    def testBfloat16(self):
        for rank in range(1, _MAX_RANK + 1):
            np_arr = self._makeIncremental((2,) * rank, dtypes.bfloat16)
            self._compareAllAxes(np_arr, rtol=1e-3, atol=5.0)
            ...
            for size_y in [1, 3, 16, 33]:
                for size_z in [1, 3, 16, 33]:
                    ...
                    with self.session(graph=ops.Graph(), use_gpu=True) as sess:
                        ...
                        tf_out_sum_xz, tf_out_sum_y = self.evaluate(
                            [tf_sum_xz, tf_sum_y]
                        )
    ...
    def testPartialShapes(self):
        ...
        s_known_rank = math_ops.reduce_sum(c_known_rank, reduction_axes, keepdims=True)
        ...
        self.assertEqual(tensor_shape.unknown_shape(), s_unknown_indices.get_shape())
    ...
    def testWrongShapeForReductionIndices(self):
        ...
        c_unknown = array_ops.placeholder(dtypes.float32)
        with self.assertRaisesWithPredicateMatch(
            ValueError, ".*must be at most rank 1.*"
        ):
    ...
    def testInvalidRepeatedReductionIndices(self):
        ...
        with self.assertRaisesWithPredicateMatch(
            errors.InvalidArgumentError, ".*Axes contains duplicate dimension: 0.*"
        ):
...
class EuclideanNormReductionTest(BaseReductionTest, parameterized.TestCase):
    ...
    def _np_reduce(self, x, reduction_axes, keepdims):
        if isinstance(reduction_axes, list) or isinstance(reduction_axes, np.ndarray):
            reduction_axes = tuple(reduction_axes)
        np_fro = np.sqrt(np.sum(x * np.conj(x), axis=reduction_axes, keepdims=keepdims))
        if np.issubdtype(x.dtype, np.integer):
    ...
    def testSingleton(self):
        for dtype in [np.float32, np.float64]:
            np_arr = np.array([-1.0]).astype(dtype)
            self._compareAll(np_arr, None)
...
class ProdReductionTest(BaseReductionTest):
    ...
    def testGradientWithZeros(self):
        s = [2, 3, 4, 2]
        x = self._makeIncremental(s, dtypes.float32) / 20.0
        # No zeros in input
        self._compareGradientAxes(x, rtol=1e-3, atol=1e-3)
        # Zero at beginning
        x1 = x.copy()
    ...
    def testSpecialValues(self):
        for dtype in [np.float32, np.float64]:
            for size in range(1, 4):
                for arr in itertools.product(
                    [-np.inf, 1.0, np.nan, np.inf], repeat=size
                ):
...
class MaxReductionTest(test.TestCase):
    ...
    def testBfloat16Reduce3D(self):
        ...
        # dimensions
        np_arr = (
            np.arange(-31, -1).reshape([2, 3, 5]).astype(dtypes.bfloat16.as_numpy_dtype)
        )
        self._compareAll(np_arr, None)
        self._compareAll(np_arr, [])
        self._compareAll(np_arr, [0])
...
class AllReductionTest(test.TestCase):
    ...
    def testAxesType(self):
        for dtype in [dtypes.int64, dtypes.int32]:
            with self.session():
                v = math_ops.reduce_all(
                    [True, True], constant_op.constant(0, dtype=dtype)
                )
                tf_v = self.evaluate(v)
...
class CountNonzeroReductionTest(test.TestCase):
    def _compare(
        self, x, reduction_axes, keepdims, use_gpu=False, zero=0, feed_dict=None
    ):
    ...
    def testFloatReduce4D(self):
        ...
        # dimensions
        np_arr = (
            np.floor(np.arange(0.0, 210.0) / 100.0)
            .reshape([2, 3, 5, 7])
            .astype(np.float32)
        )
        self._compareAll(np_arr, None)
