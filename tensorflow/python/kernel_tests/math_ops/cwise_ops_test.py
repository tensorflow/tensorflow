...
class ComparisonOpTest(test.TestCase):
            ...
            for x in data:
                for y in data:
                    with self.subTest(t=t, x=x, y=y):
                        self.assertEqual(
                            self._compareScalar(math_ops.less, x, y, t), x < y
                        )
                        self.assertEqual(
    ...
    def testTensorCompareTensor(self):
        x = np.linspace(-15, 15, 6).reshape(
            1, 3, 2
        )  # pylint: disable=too-many-function-args
        y = np.linspace(20, -10, 6).reshape(
            1, 3, 2
        )  # pylint: disable=too-many-function-args
...
class LogicalOpTest(test.TestCase):
    ...
    def testTensor(self):
        x = (
            np.random.randint(0, 2, 6).astype(np.bool_).reshape(1, 3, 2)
        )  # pylint: disable=too-many-function-args
        y = (
            np.random.randint(0, 2, 6).astype(np.bool_).reshape(1, 3, 2)
        )  # pylint: disable=too-many-function-args
    ...
    def testShapeMismatch(self):
        ...
        y = (
            np.random.randint(0, 2, 6).astype(np.bool_).reshape(3, 2, 1)
        )  # pylint: disable=too-many-function-args
...
class SelectOpTest(test.TestCase):
    ...
    def testBasicBroadcast(self):
        ...
        c1 = (
            np.random.randint(0, 2, 2).astype(np.bool_).reshape(1, 1, 2)
        )  # pylint: disable=too-many-function-args
        c2 = (
            np.random.randint(0, 2, 3).astype(np.bool_).reshape(1, 3, 1)
        )  # pylint: disable=too-many-function-args
        c3 = (
            np.random.randint(0, 2, 1).astype(np.bool_).reshape(1, 1, 1)
        )  # pylint: disable=too-many-function-args
    ...
    def _testEmptyTensor(self, fn):
        c = (
            np.random.randint(0, 3, 0).astype(np.bool_).reshape(1, 3, 0)
        )  # pylint: disable=too-many-function-args
...
class MathOpsOverloadTest(test.TestCase):
    ...
    def _compareBinary(self, x, y, dtype, np_func, tf_func):
        ...
        if np_func != np.true_divide:
            ...
            np_ans = np_ans.astype(
                np.float32 if dtype == dtypes_lib.bfloat16 else dtype.as_numpy_dtype
            )
        rtol = 1e-2 if dtype in (dtypes_lib.bfloat16, dtypes_lib.float16) else 1e-6
        self.assertAllClose(
            np_ans, self._computeTensorAndLiteral(x, y, dtype, tf_func), rtol=rtol
        )
        self.assertAllClose(
            np_ans, self._computeLiteralAndTensor(x, y, dtype, tf_func), rtol=rtol
        )
...
class IsFiniteInfNanTest(test.TestCase):
    ...
    def _testDtype(self, dtype):
        if dtype != dtypes_lib.bfloat16.as_numpy_dtype:
            ...
            data = np.array(
                [0, -1, 1, 0.01, -0.01, -3.3895e38, 3.3895e38, -np.inf, np.inf, np.nan]
            ).astype(dtype)
...
class ComplexMakeRealImagTest(test.TestCase):
    ...
    def testAngle(self):
        ...
        cplx = np.append(cplx, [1.0, 1.0j, -1.0, -1.0j])
    ...
    def testConjString(self):
        ...
        with self.assertRaisesRegex(TypeError, r"Expected numeric or variant tensor"):
            math_ops.conj(x)
