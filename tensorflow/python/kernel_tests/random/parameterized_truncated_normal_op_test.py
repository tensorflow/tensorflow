...
class TruncatedNormalMoments:
    ...
    def __getitem__(self, moment):
        ...
        for k in range(len(self.memoized_moments), moment + 1):
            ...
            numerator = np.power(self.maxval, k - 1) * dist.pdf(self.maxval) - np.power(
                self.minval, k - 1
            ) * dist.pdf(self.minval)
            denominator = dist.cdf(self.maxval) - dist.cdf(self.minval)
            m = (
                (k - 1) * self.stddev**2 * m_k_minus_2
                + self.mean * m_k_minus_1
...
class ParameterizedTruncatedNormalTest(test.TestCase):
    ...
    def validateMoments(
        self, shape, mean, stddev, minval, maxval, use_stateless=False, seed=1618
    ):
        try:
            ...
            with self.cached_session():
                if use_stateless:
                    ...
                    new_seed = random_ops.random_uniform(
                        [2], seed=seed, minval=0, maxval=(2**31 - 1), dtype=np.int32
                    )
                    samples = stateless.stateless_parameterized_truncated_normal(
                        shape, new_seed, mean, stddev, minval, maxval
                    ).eval()
                else:
                    samples = random_ops.parameterized_truncated_normal(
                        shape, mean, stddev, minval, maxval
                    ).eval()
    ...
    def testRightTail(self):
        ...
        self.validateMoments([int(1e5)], 0.0, 1.0, 4.0, np.inf, use_stateless=True)

    @test_util.run_deprecated_v1
    def testLeftTail(self):
        self.validateMoments([int(1e5)], 0.0, 1.0, -np.inf, -4.0)
        self.validateMoments([int(1e5)], 0.0, 1.0, -np.inf, -4.0, use_stateless=True)
    ...
    def testTwoSidedLeftTailShifted(self):
        ...
        self.validateKolmogorovSmirnov(
            [int(1e5)], 6.0, 1.0, -1.0, 1.0, use_stateless=True
        )
    ...
    def testRightTailShifted(self):
        ...
        self.validateMoments([int(1e5)], -5.0, 1.0, 2.0, np.inf, use_stateless=True)
    ...
    def testTruncateOnLeft_entireTailOnRight(self):
        ...
        self.validateKolmogorovSmirnov(
            [int(1e5)], 10.0, 1.0, 4.0, np.inf, use_stateless=True
        )
    ...
    def testTruncateOnRight_entireTailOnLeft(self):
        ...
        self.validateKolmogorovSmirnov(
            [int(1e5)], -8.0, 1.0, -np.inf, -4.0, use_stateless=True
        )
    ...
    def testSmallStddev(self):
        ...
        self.validateKolmogorovSmirnov(
            [int(1e5)], 0.0, 0.1, 0.05, 0.10, use_stateless=True
        )

    @test_util.run_deprecated_v1
    def testSamplingWithSmallStdDevFarFromBound(self):
        sample_op = random_ops.parameterized_truncated_normal(
            shape=(int(1e5),), means=0.8, stddevs=0.05, minvals=-1.0, maxvals=1.0
        )
        new_seed = random_ops.random_uniform(
            [2], seed=1234, minval=0, maxval=(2**31 - 1), dtype=np.int32
        )
    ...
    def testShapeTypes(self):
        for shape_dtype in [np.int32, np.int64]:
            ...
            sample_op = random_ops.parameterized_truncated_normal(
                shape=shape, means=0.0, stddevs=0.1, minvals=-1.0, maxvals=1.0
            )
            new_seed = random_ops.random_uniform(
                [2], seed=1234, minval=0, maxval=(2**31 - 1), dtype=np.int32
            )
        ...
        with self.cached_session() as sess:
            ...
            try:
                ...
                scipy_maxval_grad = np.exp(
                    0.5 * (samples_np**2 - ((1.0 - 0.01) / 1.0) ** 2)
                    + np.log(sample_cdf)
                )
...
def parameterized_vs_naive(shape, num_iters, use_gpu=False):
    ...
    config = config_pb2.ConfigProto(
        graph_options=config_pb2.GraphOptions(optimizer_options=optimizer_options)
    )
...
class TruncatedNormalBenchmark(test.Benchmark):
    ...
    def _benchmarkParameterizedOpVsNaiveOp(self, use_gpu):
        ...
        print(
            (
                "Composition of new ParameterizedTruncatedNormalOp vs. "
                "naive TruncatedNormalOp [%d iters]"
            )
            % num_iters
        )
        ...
        for shape in [
            ...
            self.report_benchmark(
                name="parameterized_shape" + shape_str, iters=num_iters, wall_time=p_dt
            )
            self.report_benchmark(
    ...
    def _benchmarkRandnSampler(self, use_gpu):
        ...
        print(
            ("Randn Sampler vs uniform samplers [%d iters]\t%.4f\t%.4f")
