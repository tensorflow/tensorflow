...
class BaseSparseCrossOpTest(test.TestCase):
    def _sparse_tensor(self, data, batch_size=-1):
        ...
        value_type = (
            dtypes.string if not values or isinstance(values[0], str) else dtypes.int64
        )
...
class SparseCrossOpTest(test.TestCase):
        ...
        expected_out = self._sparse_tensor(
            [
                ...
                [
                    "333_X_batch2-FC2-F1",
                    "333_X_batch2-FC2-F2",
                    "5555_X_batch2-FC2-F1",
                    "5555_X_batch2-FC2-F2",
                ],
            ]
        )
        with self.cached_session():
    ...
    def test_invalid_sparse_tensors(self):
        ...
        with self.assertRaisesRegex(
            (errors.InvalidArgumentError, ValueError), "Input values should be a vector"
        ):
        ...
        with self.assertRaisesRegex(
            (errors.InvalidArgumentError, ValueError), "Input shapes should be a vector"
        ):
    ...
    def test_bad_tensor_shapes(self):
        ...
        with self.assertRaisesRegex(
            (errors.InvalidArgumentError, ValueError), "Expected D2 of index to be 2"
        ):
            st = sparse_tensor.SparseTensor([[0]], [0], [10])  # 1D SparseTensor
            self.evaluate(sparse_ops.sparse_cross([st]))

        with self.assertRaisesRegex(
            (errors.InvalidArgumentError, ValueError), "Dense inputs should be a matrix"
        ):
    ...
    def test_batch_size_mismatch(self):
        ...
        with self.assertRaisesRegex(
            (errors.InvalidArgumentError, ValueError), "Expected batch size"
        ):
...
class SparseCrossV2OpTest(BaseSparseCrossOpTest):
    ...
    def test_dense(self):
        ...
        dense_inp_1 = constant_op.constant(
            [["batch1-FC1-F1", "batch1-FC1-F2"], ["batch2-FC1-F1", "batch2-FC1-F2"]],
            dtypes.string,
        )
        dense_inp_2 = constant_op.constant(
            [["batch1-FC2-F1", "batch1-FC2-F2"], ["batch2-FC2-F1", "batch2-FC2-F2"]],
            dtypes.string,
        )
        inds, vals, shapes = gen_sparse_ops.sparse_cross_v2(
            indices=[],
    ...
    def test_dense_sep(self):
        ...
        expected_out = self._sparse_tensor(
            [
                [
                    "batch1-FC1-F1_batch1-FC2-F1",
                    "batch1-FC1-F1_batch1-FC2-F2",
                    "batch1-FC1-F2_batch1-FC2-F1",
                    "batch1-FC1-F2_batch1-FC2-F2",
                ],
                [
    ...
    def test_sparse_cross_dense(self):
        """Tests sparse and dense inputs."""
        sp_inp = self._sparse_tensor(
            [["batch1-FC1-F1"], ["batch2-FC1-F1", "batch2-FC1-F2"]]
        )
        dense_inp = constant_op.constant(
            [["batch1-FC2-F1", "batch1-FC2-F2"], ["batch2-FC2-F1", "batch2-FC2-F2"]],
            dtypes.string,
        )
        inds, vals, shapes = gen_sparse_ops.sparse_cross_v2(
    ...
    def test_permutation_3x3x3(self):
        ...
        inds, vals, shapes = gen_sparse_ops.sparse_cross_v2(
            ...
            shapes=[sp_inp_1.dense_shape, sp_inp_2.dense_shape, sp_inp_3.dense_shape],
            dense_inputs=[],
            sep="_X_",
        )
        expected_out = self._sparse_tensor(
    ...
    def testNonScalarInput(self):
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "Input separator should be a scalar."
        ):
            self.evaluate(
                sparse_ops.sparse_cross(
                    inputs=[],
                    name="a",
