...
class SparseAddTest(test.TestCase):
    ...
    def testAddSparseDense(self):
        ...
        for dtype in [np.float32, np.float64, np.int64, np.complex64]:
            for index_dtype in [np.int32, np.int64]:
                ...
                with test_util.force_cpu():
                    sparse, unused_nnz = _sparsify(
                        rand_vals_np, index_dtype=index_dtype
                    )
                    s = self.evaluate(
    ...
    def _testSparseDenseInvalidInputs(
        self, a_indices, a_values, a_shape, b, expected_error=""
    ):
        # Public API call to sparse-dense add.
        with self.assertRaisesRegex(
            (ValueError, errors_impl.InvalidArgumentError), expected_error
        ):
        ...
        for sparsity in [0.99, 0.5, 0.01]:
            for n in [1, 256, 50000]:
                for m in [100, 1000]:
                    ...
                    print(
                        "%.2f \t %d \t %d \t %.4f \t %.4f \t %.2f"
                        % (sparsity, n, m, s2d_dt, sa_dt, s2d_dt / sa_dt)
                    )
