...
class SparseTensorDenseMatMulGradientTest(test.TestCase):
    ...
    def _randomTensor(
        self, size, values_dtype, adjoint=False, sparse=False, indices_dtype=np.int64
    ):
        n, m = size
    ...
    def _testGradients(self, adjoint_a, adjoint_b, name, values_dtype, indices_dtype):
        ...
        with self.cached_session():
            ...
            sp_t_val_shape = [nnz]
            delta = 1 / 16.0 if values_dtype == np.float16 else 1e-3
            tolerance = delta / 2.0 if values_dtype == np.float16 else 1e-3
        ...
        for adjoint_a in [True, False]:
            for adjoint_b in [True, False]:
                name = "sparse_tensor_dense_matmul_%s_%s_%s_%s" % (
                    adjoint_a,
                    adjoint_b,
                    values_dtype.__name__,
                    indices_dtype.__name__,
                )
                self._testGradients(
                    adjoint_a, adjoint_b, name, values_dtype, indices_dtype
                )
