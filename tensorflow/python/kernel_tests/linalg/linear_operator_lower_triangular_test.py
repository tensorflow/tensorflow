...
class LinearOperatorLowerTriangularTest(
    ...
    def test_is_x_flags(self):
        ...
        operator = linalg.LinearOperatorLowerTriangular(
            tril, is_positive_definite=True, is_non_singular=True, is_self_adjoint=False
        )
    ...
    def test_triangular_diag_matmul(self):
        operator1 = linalg_lib.LinearOperatorLowerTriangular(
            [[1.0, 0.0, 0.0], [2.0, 1.0, 0.0], [2.0, 3.0, 3.0]]
        )
        operator2 = linalg_lib.LinearOperatorDiag([2.0, 2.0, 3.0])
        operator_matmul = operator1.matmul(operator2)
        self.assertTrue(
            isinstance(operator_matmul, linalg_lib.LinearOperatorLowerTriangular)
        )
        self.assertAllClose(
    ...
    def test_llt_composition_with_pd_l(self):
        l = linalg_lib.LinearOperatorLowerTriangular(
            [[1.0, 0.0], [0.5, 0.2]], is_non_singular=True, is_positive_definite=True
        )
    ...
    def test_llt_composition_with_non_pd_l(self):
        ...
        l = linalg_lib.LinearOperatorLowerTriangular(
            [[-1.0, 0.0, 0.0], [0.5, 0.2, 0.0], [0.1, 0.1, 1.0]], is_non_singular=True
        )
        llt = l @ l.H
        chol = llt.cholesky()
        ...
        self.assertAllClose(
            self.evaluate(llt.to_dense()), self.evaluate((chol @ chol.H).to_dense())
        )
    ...
    def test_llt_composition_with_non_pd_complex_l(self):
        ...
        l = linalg_lib.LinearOperatorLowerTriangular(
            [[-1.0 + i, 0.0, 0.0], [0.5, 0.2 - 2 * i, 0.0], [0.1, 0.1, 1.0]],
