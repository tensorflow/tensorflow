...
def _block_diag_dense(expected_shape, blocks):
    ...
    for block in blocks:
        ...
        rows.append(
            array_ops.concat([zeros_to_pad_before, block, zeros_to_pad_after], axis=-1)
        )
...
class SquareLinearOperatorBlockDiagTest(
    ...
    def operator_and_matrix(
        self, shape_info, dtype, use_placeholder, ensure_self_adjoint_and_pd=False
    ):
        shape = list(shape_info.shape)
        expected_blocks = (
            shape_info.__dict__["blocks"]
            if "blocks" in shape_info.__dict__
            else [shape]
        )
        matrices = [
    ...
    def test_block_diag_matmul_raises(self):
        ...
        operator2 = linalg.LinearOperatorFullMatrix(
            linear_operator_test_util.random_normal([15, 3], dtype=dtypes.float32)
        )
            ...
            matrices1.append(
                linalg.LinearOperatorFullMatrix(
                    linear_operator_test_util.random_tril_matrix(
                        [i, i], dtype=dtypes.float32, force_well_conditioned=True
                    )
                )
            )
