    ...
    def testBatchGradientUnknownSize(self):
        with self.cached_session():
            ...
            batch_identity = array_ops.tile(
                array_ops.expand_dims(array_ops.diag(array_ops.ones([matrix_size])), 0),
                [batch_size, 1, 1],
            )
...
def _GetMatrixBinaryFunctorGradientTest(
    functor_, dtype_, shape_, float32_tol_fudge=1.0, **kwargs_
):
    @test_util.run_in_graph_and_eager_modes(use_gpu=True)
    @test_util.run_without_tensor_float_32(
            ...
            for extra in [(), (2,), (3,)] + [(3, 2)] * (size < 10):
                for adjoint in False, True:
                    ...
                    for lower in True, False:
                        ...
                        name = "%s_%s_adj_%s_low_%s" % (
                            dtype.__name__,
                            "_".join(map(str, band_shape)),
                            str(adjoint),
                            lower,
                        )
                        _AddTest(
                            MatrixBinaryFunctorGradientTest,
                            "BandedTriangularSolveGradient",
                            name,
    ...
    for dtype in np.float32, np.float64:
        for size in 2, 5, 10:
            ...
            for extra in [(), (2,), (3,)] + [(3, 2)] * (size < 10):
                ...
                _AddTest(
                    MatrixUnaryFunctorGradientTest,
                    "MatrixDeterminantGradient",
                    name,
                    _GetMatrixUnaryFunctorGradientTest(
                        linalg_ops.matrix_determinant, dtype, shape
                    ),
                )
                _AddTest(
                    MatrixUnaryFunctorGradientTest,
                    ...
                    name,
                    _GetMatrixUnaryFunctorGradientTest(
                        lambda x: linalg_ops.log_matrix_determinant(x)[1], dtype, shape
                    ),
                )
                ...
                _AddTest(
                    MatrixUnaryFunctorGradientTest,
                    "MatrixSquareRootGradient",
                    name,
                    _GetMatrixUnaryFunctorGradientTest(
                        linalg_ops.matrix_square_root, dtype, shape
                    ),
                )
