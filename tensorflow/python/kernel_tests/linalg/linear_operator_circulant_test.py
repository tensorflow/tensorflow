...
class ExponentialPowerConvolutionKernelTest(parameterized.TestCase, test.TestCase):
    ...
    @parameterized.named_parameters(
        dict(testcase_name="1Deven_power1", grid_shape=[10], power=1.0),
        dict(testcase_name="2Deven_power1", grid_shape=[4, 6], power=1.0),
        dict(testcase_name="3Deven_power1", grid_shape=[4, 6, 8], power=1.0),
        dict(testcase_name="3Devenodd_power1", grid_shape=[4, 5, 7], power=1.0),
        dict(testcase_name="1Dodd_power2", grid_shape=[9], power=2.0),
        dict(testcase_name="2Deven_power2", grid_shape=[8, 4], power=2.0),
        dict(testcase_name="3Devenodd_power2", grid_shape=[4, 5, 3], power=2.0),
    )
    def test_makes_symmetric_and_real_circulant_with_ones_diag(self, grid_shape, power):
        d = len(grid_shape)
        length_scale = [0.2] * d
        kernel = exponential_power_convolution_kernel(
            grid_shape=grid_shape, length_scale=length_scale, power=power
        )
    ...
    def test_zero_inflation(self, grid_shape):
        ...
        self.assertAllClose(
            ...
            _matrix_with_zerod_diag(matrix_inflation_one_half),
            rtol=tol,
        )
        ...
        self.assertAllClose(
            ...
            _matrix_with_zerod_diag(matrix_inflation_one),
            rtol=tol,
        )
    ...
    def test_huge_scale_corresponds_to_ones_matrix(self, grid_shape):
        ...
        kernel = exponential_power_convolution_kernel(
            grid_shape=grid_shape, length_scale=[100.0] * d, power=2
        )
        matrix = self.evaluate(_operator_from_kernel(kernel, d).to_dense())
