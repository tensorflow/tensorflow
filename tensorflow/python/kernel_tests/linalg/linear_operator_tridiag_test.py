...
class _LinearOperatorTriDiagBase(object):
    def build_operator_and_matrix(
        ...
        matrix_diagonals = array_ops.stack([superdiag, diag, subdiag], axis=-2)
        ...
        if use_placeholder:
            if diagonals_format == "sequence":
                lin_op_diagonals = [
                    array_ops.placeholder_with_default(d, shape=None)
                    for d in lin_op_diagonals
                ]
            else:
...
class LinearOperatorTriDiagCompactTest(
    ...
    def test_tape_safe(self):
        diag = variables_module.Variable(
            [[3.0, 6.0, 2.0], [2.0, 4.0, 2.0], [5.0, 1.0, 2.0]]
        )
...
class LinearOperatorTriDiagSequenceTest(
    ...
    def test_tape_safe(self):
        diagonals = [
            variables_module.Variable([3.0, 6.0, 2.0]),
            variables_module.Variable([2.0, 4.0, 2.0]),
            variables_module.Variable([5.0, 1.0, 2.0]),
        ]
        operator = linalg_lib.LinearOperatorTridiag(
...
class LinearOperatorTriDiagMatrixTest(
    ...
    @test_util.disable_xla("Current implementation does not yet support pivoting")
    def test_tape_safe(self):
        matrix = variables_module.Variable(
            [[3.0, 2.0, 0.0], [1.0, 6.0, 4.0], [0.0, 2, 2]]
        )
