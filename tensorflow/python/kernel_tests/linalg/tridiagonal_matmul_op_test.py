...
class TridiagonalMulOpTest(test.TestCase):
    def _testAllFormats(
        ...
        diags_sequence_batch = [
            array_ops_stack.stack([x, 2 * x]) for x in diags_sequence
        ]
    ...
    def _testErrorWithShapesEager(
        self, exception_regex, superdiag_shape, maindiag_shape, subdiag_shape, rhs_shape
    ):
        with context.eager_mode():
            ...
            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, exception_regex
            ):
    ...
    def testInvalidShapesEagerGpu(self):
        ...
        self._testErrorWithShapesEager(
            "Input must have rank >= 2, but got ", [2], [2], [2], [2]
        )
        self._testErrorWithShapesEager(
