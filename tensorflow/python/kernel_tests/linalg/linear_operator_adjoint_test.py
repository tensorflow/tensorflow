...
class LinearOperatorAdjointTest(
    ...
    def test_matmul_adjoint_operator(self):
        ...
        self.assertAllClose(
            ...
            self.evaluate(full_matrix1.matmul(full_matrix2, adjoint=True).to_dense()),
        )
        ...
        self.assertAllClose(
            ...
            self.evaluate(
                full_matrix1.matmul(
                    full_matrix2, adjoint=True, adjoint_arg=True
                ).to_dense()
            ),
        )
    ...
    def test_solve_adjoint_operator(self):
        ...
        self.assertAllClose(
            ...
            self.evaluate(full_matrix1.solve(full_matrix2, adjoint=True).to_dense()),
        )

        self.assertAllClose(
            self.evaluate(linalg.triangular_solve(matrix1.T, matrix2.T, lower=False)),
            self.evaluate(
                full_matrix1.solve(
                    full_matrix2, adjoint=True, adjoint_arg=True
                ).to_dense()
    ...
    def test_solve_adjoint_complex_operator(self):
        ...
        self.assertAllClose(
            self.evaluate(
                linalg.triangular_solve(matrix1.conj().T, matrix2.conj().T, lower=False)
            ),
            self.evaluate(
                full_matrix1.solve(
                    full_matrix2, adjoint=True, adjoint_arg=True
...
class LinearOperatorAdjointNonSquareTest(
    ...
    def operator_and_matrix(self, build_info, dtype, use_placeholder):
        ...
        shape_before_adjoint[-1], shape_before_adjoint[-2] = (
            shape_before_adjoint[-2],
            shape_before_adjoint[-1],
        )
