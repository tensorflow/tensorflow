...
class LinearOperatorScaledIdentityTest(
    ...
    def operator_and_matrix(
        ...
        multiplier = linear_operator_test_util.random_sign_uniform(
            shape=batch_shape, minval=1.0, maxval=2.0, dtype=dtype
        )
    ...
    def test_assert_non_singular_does_not_raise_when_non_singular(self):
        with self.cached_session():
            operator = linalg_lib.LinearOperatorScaledIdentity(
                num_rows=2, multiplier=[1.0, 2.0, 3.0]
            )
            self.evaluate(operator.assert_non_singular())  # Should not fail
    ...
    def test_assert_non_singular_raises_when_singular(self):
        with self.cached_session():
            operator = linalg_lib.LinearOperatorScaledIdentity(
                num_rows=2, multiplier=[1.0, 2.0, 0.0]
            )
    ...
    def test_wrong_matrix_dimensions_raises_dynamic(self):
        ...
        with self.cached_session():
            with self.assertRaisesError("Dimensions.*not.compatible"):
                operator = linalg_lib.LinearOperatorScaledIdentity(
                    num_rows, multiplier=[1.0, 2], assert_proper_shapes=True
                )
                self.evaluate(operator.matmul(x))
    ...
    def test_is_x_flags(self):
        operator = linalg_lib.LinearOperatorScaledIdentity(
            num_rows=2, multiplier=1.0, is_positive_definite=False, is_non_singular=True
        )
    ...
    def test_identity_adjoint_type(self):
        operator = linalg_lib.LinearOperatorScaledIdentity(
            num_rows=2, multiplier=1.0, is_non_singular=True
        )
    ...
    def test_identity_matmul(self):
        ...
        self.assertIsInstance(
            operator1.matmul(operator1), linalg_lib.LinearOperatorIdentity
        )
        ...
        self.assertIsInstance(
            operator2.matmul(operator2), linalg_lib.LinearOperatorScaledIdentity
        )
    ...
    def test_identity_solve(self):
        ...
        operator2 = linalg_lib.LinearOperatorScaledIdentity(num_rows=2, multiplier=3.0)
        self.assertIsInstance(
            operator1.solve(operator1), linalg_lib.LinearOperatorIdentity
        )

        self.assertIsInstance(
            operator2.solve(operator2), linalg_lib.LinearOperatorScaledIdentity
        )
    ...
    def test_scaled_identity_cholesky_type(self):
        ...
        self.assertIsInstance(
            operator.cholesky(), linalg_lib.LinearOperatorScaledIdentity
        )
    ...
    def test_scaled_identity_inverse_type(self):
        ...
        self.assertIsInstance(
            operator.inverse(), linalg_lib.LinearOperatorScaledIdentity
        )
