...
from tensorflow.python.ops.linalg import (
    linear_operator_lower_triangular as lower_triangular,
)
...
class SquareLinearOperatorKroneckerTest(
        ...
        operator = kronecker.LinearOperatorKronecker(
            [
                linalg.LinearOperatorFullMatrix(
                    l, is_square=True, is_self_adjoint=True, is_positive_definite=True
                )
    ...
    def test_kronecker_cholesky_type(self):
        ...
        self.assertIsInstance(
            cholesky_factor.operators[0], lower_triangular.LinearOperatorLowerTriangular
        )
        self.assertIsInstance(
            cholesky_factor.operators[1], lower_triangular.LinearOperatorLowerTriangular
        )
    ...
    def test_tape_safe(self):
        ...
        operator = kronecker.LinearOperatorKronecker(
            [
                linalg.LinearOperatorFullMatrix(matrix_1, is_non_singular=True),
                linalg.LinearOperatorFullMatrix(matrix_2, is_non_singular=True),
            ],
            is_non_singular=True,
        )
    ...
    def test_composite_gradients(self):
        with backprop.GradientTape() as tape:
            op1 = linalg.LinearOperatorFullMatrix(
                [[1.0, 0.0], [0.0, 1.0]], is_non_singular=True
            )
            op2 = linalg.LinearOperatorFullMatrix(
                [[2.0, 0.0], [0.0, 2.0]], is_non_singular=True
            )
            tape.watch([op1, op2])
            operator = kronecker.LinearOperatorKronecker(
        ...
        self.assertAllClose(
            connected_grad.to_dense(), composite_grad.operators[0].to_dense()
        )
        self.assertAllClose(
            disconnected_component_grad,
