...
# ==============================================================================
"""Library to run CSR sparse matrix tests."""
...
class CSRSparseMatrixDenseMatMulGradTest(test.TestCase):
  ...
  def _testLargeBatchSparseMatrixMatMulGrad(
    ...
    if batched_inputs:
      a_shape = (3, 5, 11)
      b_shape = (3, 11, 13)
      def transpose(x): return np.transpose(x, (0, 2, 1))
    else:
      a_shape = (5, 11)
      b_shape = (11, 13)
      transpose = np.transpose

    def sparsify(m): return m * (m > 0)
    a_mats_val = sparsify(
...
def define_csr_sparse_matrix_dense_mat_mul_grad_tests(test):
  ...
  for dtype in dtypes_to_test:
    for (t_a, t_b, adj_a, adj_b, t_out,
      ...
      _add_test(
        test, "CSRSparseMatrixGradTest", name,
