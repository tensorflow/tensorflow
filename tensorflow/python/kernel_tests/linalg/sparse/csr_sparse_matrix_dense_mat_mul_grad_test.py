...
class CSRSparseMatrixDenseMatMulGradTest(test.TestCase):
    ...
    def _testLargeBatchSparseMatrixMatMulGrad(
        ...
        a_mats_val = sparsify(
            np.random.randn(*a_shape) + 1.0j * np.random.randn(*a_shape)
        ).astype(datatype)
        ...
        b_mats_val = (
            np.random.randn(*b_shape) + 1.0j * np.random.randn(*b_shape)
        ).astype(datatype)
...
for dtype in dtypes_to_test:
    for t_a, t_b, adj_a, adj_b, t_out, conj_out, batched in itertools.product(
        *(([False, True],) * 7)
    ):

        def create_mat_mul_test_fn(
            dtype_, t_a_, t_b_, adj_a_, adj_b_, t_out_, conj_out_, batched_
        ):
            # Skip invalid cases.
            ...
            def test_fn(self):
                self._testLargeBatchSparseMatrixMatMulGrad(
                    dtype_, t_a_, t_b_, adj_a_, adj_b_, t_out_, conj_out_, batched_
                )
        ...
        _add_test(
            ...
            name,
            create_mat_mul_test_fn(
                dtype, t_a, t_b, adj_a, adj_b, t_out, conj_out, batched
            ),
        )
