    ...
    def testAssertIntegerForm(self):
        ...
        with self.cached_session():
            ...
            with self.assertRaisesOpError("has non-integer components"):
                with ops.control_dependencies([du.assert_integer_form(y)]):
                    array_ops.identity(y).eval(feed_dict=feed_dict)

            with self.assertRaisesOpError("has non-integer components"):
                with ops.control_dependencies([du.assert_integer_form(z)]):
                    array_ops.identity(z).eval(feed_dict=feed_dict)

            with self.assertRaisesOpError("has non-integer components"):
                with ops.control_dependencies([du.assert_integer_form(w)]):
...
class MaybeGetStaticTest(test.TestCase):
    ...
    def testGetStaticInt(self):
        ...
        self.assertAllClose(
            np.array(2.0), du.maybe_get_static_value(x, dtype=np.float64)
        )
...
class GetLogitsAndProbsTest(test.TestCase):
    ...
    @test_util.run_in_graph_and_eager_modes
    def testLogits(self):
        p = np.array([0.01, 0.2, 0.5, 0.7, 0.99], dtype=np.float32)
        logits = _logit(p)

        new_logits, new_p = du.get_logits_and_probs(logits=logits, validate_args=True)

        self.assertAllClose(p, self.evaluate(new_p), rtol=1e-5, atol=0.0)
        self.assertAllClose(logits, self.evaluate(new_logits), rtol=1e-5, atol=0.0)
    ...
    def testProbabilityValidateArgs(self):
        p = [0.01, 0.2, 0.5, 0.7, 0.99]
        # Component less than 0.
        p2 = [-1, 0.2, 0.5, 0.3, 0.2]
        # Component greater than 1.
        p3 = [2, 0.2, 0.5, 0.3, 0.2]
    ...
    def testProbabilityValidateArgsMultidimensional(self):
        ...
        p2 = np.array([[-0.3, 0.4, 0.9], [0.1, 0.5, 0.4]], dtype=np.float32)
    ...
    def testTooSmall(self):
        with self.cached_session():
            with self.assertRaises(ValueError):
                ...
                checked_param = du.embed_check_categorical_event_shape(param)
...
class LogCombinationsTest(test.TestCase):
    ...
    def testLogCombinationsShape(self):
        ...
        n = np.array(n, dtype=np.float32)
        # Shape [2, 2, 4]
        counts = [[[1.0, 1, 0, 0], [2.0, 2, 1, 0]], [[4.0, 4, 1, 3], [10, 1, 1, 4]]]
        log_binom = du.log_combinations(n, counts)
...
class DynamicShapeTest(test.TestCase):
    ...
    def testSameDynamicShape(self):
        with self.cached_session():
            ...
            # Scalar
            self.assertTrue(du.same_dynamic_shape(scalar, scalar1).eval({scalar1: 2.0}))

            # Vector

            self.assertTrue(
                du.same_dynamic_shape(vector, vector1).eval({vector1: [2.0, 3.0, 4.0]})
            )
            self.assertTrue(
                du.same_dynamic_shape(vector1, vector2).eval(
                    {vector1: [2.0, 3.0, 4.0], vector2: [2.0, 3.5, 6.0]}
                )
            )

            # Multidimensional
            self.assertTrue(
                du.same_dynamic_shape(multidimensional, multidimensional1).eval(
                    {multidimensional1: [[2.0, 3.0], [3.0, 4.0]]}
                )
            )
            self.assertTrue(
                du.same_dynamic_shape(multidimensional1, multidimensional2).eval(
                    {
                        multidimensional1: [[2.0, 3.0], [3.0, 4.0]],
            ...
            # Scalar, X
            self.assertFalse(
                du.same_dynamic_shape(scalar, vector1).eval({vector1: [2.0, 3.0, 4.0]})
            )
            self.assertFalse(
                du.same_dynamic_shape(scalar1, vector1).eval(
                    {scalar1: 2.0, vector1: [2.0, 3.0, 4.0]}
                )
            )
            self.assertFalse(
            ...
            self.assertFalse(
                du.same_dynamic_shape(scalar1, multidimensional1).eval(
                    {scalar1: 2.0, multidimensional1: [[2.0, 3.0], [3.0, 4.0]]}
                )
            )

            # Vector, X
            self.assertFalse(
                du.same_dynamic_shape(vector, vector1).eval({vector1: [2.0, 3.0]})
            )
            self.assertFalse(
                du.same_dynamic_shape(vector1, vector2).eval(
                    {vector1: [2.0, 3.0, 4.0], vector2: [6.0]}
                )
            )
            self.assertFalse(
...
class RotateTransposeTest(test.TestCase):
    ...
    def testRollDynamic(self):
        with self.cached_session() as sess:
            ...
            for x_value in (
                np.ones(1, dtype=x.dtype.as_numpy_dtype()),
...
class PickVectorTest(test.TestCase):
    ...
    def testCorrectlyPicksVector(self):
        with self.cached_session():
            ...
            self.assertAllEqual(
                x, du.pick_vector(constant_op.constant(True), x, y)
            )  # No eval.
            self.assertAllEqual(
                y, du.pick_vector(constant_op.constant(False), x, y)
            )  # No eval.
...
class PreferStaticValueTest(test.TestCase):
    ...
    def testScalarTensor(self):
        ...
        value = du.prefer_static_value(x)
        self.assertIsInstance(value, np.ndarray)
        self.assertAllEqual(np.array(1.0), value)
    ...
    def testDynamicValueEndsUpBeingNonEmpty(self):
        ...
        with self.cached_session():
            self.assertAllEqual(
                np.zeros((2, 3)), value.eval(feed_dict={x: np.zeros((2, 3))})
            )
...
class FillTriangularTest(test.TestCase):
    ...
    def _fill_triangular(self, x, upper=False):
        ...
        x_tail = x[..., (m - (n * n - m)) :]
        y = np.concatenate(
            [x, x_tail[..., ::-1]] if upper else [x_tail, x[..., ::-1]], axis=-1
        )
        y = y.reshape(
            np.concatenate(
                [
    ...
    def _run_test(self, x_, use_deferred_shape=False, **kwargs):
        ...
        with self.cached_session() as sess:
            ...
            zeros_like_x_pl = x_pl * array_ops.stop_gradient(
                x_pl - 1.0
            ) - array_ops.stop_gradient(x_pl * (x_pl - 1.0))
            x = x_pl + zeros_like_x_pl
            actual = du.fill_triangular(x, **kwargs)
            grad_actual = gradients_impl.gradients(actual, x_pl)[0]
            [actual_, grad_actual_] = sess.run(
                [actual, grad_actual], feed_dict={x_pl: x_}
            )
    ...
    def testCorrectlyMakesBatchTriUpperUnknownShape(self):
        self._run_test(
            self._rng.randn(2, 2, int(3 * 4 / 2)), use_deferred_shape=True, upper=True
        )

    @test_util.run_deprecated_v1
    def testCorrectlyMakesBatch7x7TriUpperUnknownShape(self):
        self._run_test(
            self._rng.randn(2, 3, int(7 * 8 / 2)), use_deferred_shape=True, upper=True
        )
...
class FillTriangularInverseTest(FillTriangularTest):
    def _run_test(self, x_, use_deferred_shape=False, **kwargs):
        ...
        with self.cached_session() as sess:
            ...
            inverse_actual_ = sess.run(inverse_actual, feed_dict={x_pl: x_})
...
class ReduceWeightedLogSumExp(test.TestCase):
    ...
    def testNoWeights(self):
        logx_ = np.array([[0.0, -1, 1000.0], [0, 1, -1000.0], [-5, 0, 5]])
        with self.cached_session() as sess:
            ...
            [actual_, actual_sgn_, grad_actual_, expected_, grad_expected_] = sess.run(
        ...
        self.assertAllEqual([1.0, 1, 1], actual_sgn_)

    def testNegativeWeights(self):
        logx_ = np.array([[0.0, -1, 1000.0], [0, 1, -1000.0], [-5, 0, 5]])
        w_ = np.array([[1.0, 1, -1], [1, -2, 1], [1, 0, 1]])
    ...
    def testDocString(self):
        ...
        with self.cached_session():
            x = constant_op.constant([[0.0, 0, 0], [0, 0, 0]])

            w = constant_op.constant([[-1.0, 1, 1], [1, 1, 1]])
...
class SoftplusTest(test.TestCase):
    ...
    def _testSoftplus(self, np_features, use_gpu=False):
        ...
        with self.session(use_gpu=use_gpu) as sess:
            ...
            [tf_softplus, tf_softplus_inverse] = sess.run([softplus, softplus_inverse])
        ...
        self.assertAllCloseAccordingToType(
            np_features, tf_softplus_inverse, atol=0.0, rtol=rtol
        )
        self.assertAllEqual(np.ones_like(tf_softplus).astype(np.bool_), tf_softplus > 0)
...
class ArgumentsTest(test.TestCase):
    ...
    def testPositionalKeywordArgs(self):
        ...
        self.assertEqual(
            {"a": 1, "b": 2, "c": 3, "unicorn": None}, foo(a=1, b=2, c=3, unicorn=None)
        )
    ...
    def testNoVarargs(self):
        ...
        self.assertEqual(
            {"a": 1, "b": 2, "c": 3, "unicorn": None}, foo(1, 2, 3, unicorn=None)
        )
        self.assertEqual(
            {"a": 1, "b": 2, "c": 3, "unicorn": None},
