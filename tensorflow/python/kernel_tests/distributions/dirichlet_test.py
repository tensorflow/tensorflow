...
class DirichletTest(test.TestCase):
    ...
    def testCovarianceFromSampling(self):
        ...
        sample_cov = math_ops.reduce_mean(
            math_ops.matmul(x_centered[..., None], x_centered[..., None, :]), 0
        )
    ...
    def testDirichletDirichletKL(self):
        conc1 = np.array(
            [[1.0, 2.0, 3.0, 1.5, 2.5, 3.5], [1.5, 2.5, 3.5, 4.5, 5.5, 6.5]]
        )
        ...
        kl_expected = (
            ...
            + np.sum(
                ...
                * (
                    ...
                    - special.digamma(np.sum(conc1, -1, keepdims=True))
                ),
                -1,
            )
        )

        self.assertAllClose(kl_expected, kl_actual_val, atol=0.0, rtol=1e-6)
        self.assertAllClose(kl_sample_val, kl_actual_val, atol=0.0, rtol=1e-1)
