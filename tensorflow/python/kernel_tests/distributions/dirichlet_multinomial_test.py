...
class DirichletMultinomialTest(test.TestCase):
    ...
    def testNonStrictTurnsOffAllChecks(self):
        ...
        with self.cached_session():
            alpha = [[-1.0, 2]]  # alpha should be positive.
            counts = [[1.0, 0], [0.0, -1]]  # counts should be non-negative.
    ...
    def testSampleUnbiasedNonScalarBatch(self):
        with self.cached_session() as sess:
            dist = ds.DirichletMultinomial(
                total_count=5.0,
                concentration=1.0 + 2.0 * self._rng.rand(4, 3, 2).astype(np.float32),
            )
            n = int(3e3)
            ...
            sample_covariance = (
                math_ops.matmul(x_centered, x_centered, adjoint_b=True) / n
            )
            [
                sample_mean_,
                sample_covariance_,
            ...
            self.assertAllClose(actual_mean_, sample_mean_, atol=0.0, rtol=0.20)
            self.assertAllEqual([4, 3, 2, 2], sample_covariance.get_shape())
            self.assertAllClose(
                actual_covariance_, sample_covariance_, atol=0.0, rtol=0.20
            )
    ...
    def testSampleUnbiasedScalarBatch(self):
        with self.cached_session() as sess:
            dist = ds.DirichletMultinomial(
                total_count=5.0,
                concentration=1.0 + 2.0 * self._rng.rand(4).astype(np.float32),
            )
            n = int(5e3)
            x = dist.sample(n, seed=0)
            sample_mean = math_ops.reduce_mean(x, 0)
            ...
            sample_covariance = (
                math_ops.matmul(x_centered, x_centered, adjoint_a=True) / n
            )
            [
                sample_mean_,
                sample_covariance_,
