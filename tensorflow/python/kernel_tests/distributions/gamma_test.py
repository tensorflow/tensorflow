...
class GammaTest(test.TestCase):
    ...
    def testGammaSampleSmallAlpha(self):
        ...
        if not stats:
            return
        self.assertAllClose(
            sample_values.mean(), stats.gamma.mean(alpha_v, scale=1 / beta_v), atol=0.01
        )
        self.assertAllClose(
            sample_values.var(), stats.gamma.var(alpha_v, scale=1 / beta_v), atol=0.15
        )
    ...
    def testGammaPdfOfSampleMultiDims(self):
        ...
        self.assertAllClose(
            stats.gamma.mean(
                [[7.0, 11.0], [7.0, 11.0]], scale=1 / np.array([[5.0, 5.0], [6.0, 6.0]])
            ),
    ...
    def testGammaWithSoftplusConcentrationRate(self):
        ...
        self.assertAllEqual(
            self.evaluate(nn_ops.softplus(alpha_v)), self.evaluate(gamma.concentration)
        )
        self.assertAllEqual(
    ...
    def testGammaGammaKL(self):
        ...
        if not special:
            return
        kl_expected = (
            (alpha0 - alpha1) * special.digamma(alpha0)
            + special.gammaln(alpha1)
            - special.gammaln(alpha0)
            + alpha1 * np.log(beta0)
            - alpha1 * np.log(beta1)
            + alpha0 * (beta1 / beta0 - 1.0)
        )

        self.assertAllClose(kl_expected, kl_actual_, atol=0.0, rtol=1e-6)
        self.assertAllClose(kl_sample_, kl_actual_, atol=0.0, rtol=1e-1)
