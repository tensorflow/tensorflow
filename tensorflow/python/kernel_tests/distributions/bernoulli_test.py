...
class BernoulliTest(test.TestCase):
    ...
    def testPmfCorrectBroadcastDynamicShape(self):
        with self.cached_session():
            ...
            self.assertAllClose(
                dist.prob(event2).eval({p: [0.2, 0.3, 0.4]}), [[0.2, 0.7, 0.4]]
            )
    ...
    def testBroadcasting(self):
        with self.cached_session():
            ...
            self.assertAllClose(
                np.log([0.5, 0.5, 0.5]), dist.log_prob([1, 1, 1]).eval({p: 0.5})
            )
            self.assertAllClose(
                np.log([0.5, 0.5, 0.5]), dist.log_prob(1).eval({p: [0.5, 0.5, 0.5]})
            )
    ...
    def testSampleActsLikeSampleN(self):
        with self.cached_session() as sess:
            ...
            seed = 42
            self.assertAllEqual(
                self.evaluate(dist.sample(n, seed)), self.evaluate(dist.sample(n, seed))
            )
            n = array_ops.placeholder(dtypes.int32)
            sample1, sample2 = sess.run(
                [dist.sample(n, seed), dist.sample(n, seed)], feed_dict={n: 1000}
            )
    ...
    def testVarianceAndStd(self):
        ...
        self.assertAllClose(
            ...
            np.array([[var(0.2), var(0.7)], [var(0.5), var(0.4)]], dtype=np.float32),
        )
        self.assertAllClose(
    ...
    def testBernoulliBernoulliKL(self):
        ...
        kl_expected = a_p * np.log(a_p / b_p) + (1.0 - a_p) * np.log(
            (1.0 - a_p) / (1.0 - b_p)
        )
