...
class UnicodeEncodeOpTest(test.TestCase, parameterized.TestCase):
    ...
    def testStrictErrorsValid(self, encoding):
        ...
        test_value = np.array(
            [
                ...
                0xFDD0,  # noncharacter
                0xFFFE,
                0xFFFF,  # last two in BMP
                0x10FFFF,
            ],  # largest valid
            np.int32,
        )
        expected_value = ("Heo\0\b\U0001f600\ufdd0\ufffe\uffff\U0010ffff").encode(
            encoding
        )
        unicode_encode_op = ragged_string_ops.unicode_encode(
            test_value, encoding, "strict"
        )
    ...
    def testIgnoreErrors(self, encoding):
        ...
        test_value = np.array(
            [
                ...
                0x1F600,  # valid
                0xD83D,
                0xDE00,  # invalid, surrogate code points
                0xFDD0,  # noncharacter
                0xFFFE,
                0xFFFF,  # last two in BMP
    ...
    def testReplaceErrors(self, encoding):
        ...
        expected_value = "Heooo".encode(encoding)
        unicode_encode_op = ragged_string_ops.unicode_encode(
        ...
        test_value = np.array([ord("H"), ord("e"), 0x7FFFFFFF, -1, ord("o")], np.int32)
        expected_value = "He\U0000fffd\U0000fffdo".encode(encoding)
        ...
        expected_value = "\U0001ffff".encode(encoding)
        unicode_encode_op = ragged_string_ops.unicode_encode(
            test_value, encoding, "replace", 0x41
        )
        self.assertAllEqual(unicode_encode_op, expected_value)
    ...
    def testVector(self, encoding):
        ...
        expected_value = "Hello".encode(encoding)
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)

        test_value = np.array([ord("H"), ord("e"), 0xC3, 0xC3, 0x1F604], np.int32)
        expected_value = "He\xc3\xc3\U0001f604".encode(encoding)
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)

        # Single character string
        test_value = np.array([ord("H")], np.int32)
        expected_value = "H".encode(encoding)
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)

        test_value = np.array([0x1F604], np.int32)
        expected_value = "\U0001f604".encode(encoding)
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)
    ...
    def testMatrix(self, encoding):
        ...
        expected_value = [
            "H\U0001f604llo".encode(encoding),
            "W\U0001f604rld".encode(encoding),
        ]
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)
    ...
    def test3DimMatrix(self, encoding):
        ...
        expected_value = [
            ["Hello".encode(encoding), "World".encode(encoding)],
            ["fixed".encode(encoding), "words".encode(encoding)],
            ["Hyper".encode(encoding), "cube.".encode(encoding)],
        ]
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)
    ...
    def test3DimMatrixWithRagged2ndDim(self, encoding):
        ...
        expected_value = [
            ...
            [
                "Hyper".encode(encoding),
                "words".encode(encoding),
                "cube.".encode(encoding),
            ],
        ]
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)
    ...
    def test3DimMatrixWithRagged3rdDim(self, encoding):
        ...
        expected_value = [
            ["Hello".encode(encoding), "World.".encode(encoding)],
            ["Don't".encode(encoding), "w\xc3rry, be".encode(encoding)],
            ["\U0001f604".encode(encoding), "".encode(encoding)],
        ]
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)
    ...
    def test4DimRaggedMatrix(self, encoding):
        ...
        expected_value = [
            [["Hello".encode(encoding), "World".encode(encoding)]],
            [["".encode(encoding)], ["Hype".encode(encoding)]],
        ]
        unicode_encode_op = ragged_string_ops.unicode_encode(test_value, encoding)
        self.assertAllEqual(unicode_encode_op, expected_value)
