...
class SubstrOpTest(test.TestCase, parameterized.TestCase):
    ...
    def testScalarString(self, dtype, pos, unit):
        test_string = {
            "BYTE": b"Hello",
            "UTF8_CHAR": "He\xc3\xc3\U0001f604".encode("utf-8"),
        }[unit]
        expected_value = {
            "BYTE": b"ell",
            "UTF8_CHAR": "e\xc3\xc3".encode("utf-8"),
        }[unit]
    ...
    def testScalarString_EdgeCases(self, dtype, unit):
        ...
        test_string = {
            "BYTE": b"Hello",
            "UTF8_CHAR": "H\xc3ll\U0001f604".encode("utf-8"),
        }[unit]
        position = np.array(0, dtype)
        ...
        expected_string = {
            "BYTE": b"ello",
            "UTF8_CHAR": "\xc3ll\U0001f604".encode("utf-8"),
        }[unit]
    ...
    def testVectorStrings(self, dtype, pos, unit):
        test_string = {
            ...
            "UTF8_CHAR": [x.encode("utf-8") for x in ["H\xc3llo", "W\U0001f604rld"]],
        }[unit]
        expected_value = {
            "BYTE": [b"ell", b"orl"],
            "UTF8_CHAR": [x.encode("utf-8") for x in ["\xc3ll", "\U0001f604rl"]],
        }[unit]
        position = np.array(pos, dtype)
        length = np.array(3, dtype)
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
    ...
    def testMatrixStrings(self, dtype, unit):
        test_string = {
            ...
            "UTF8_CHAR": [
                ...
                [
                    x.encode("utf-8")
                    for x in ["He\xc3\xc3o", "W\U0001f604rld", "d\xfcd\xea"]
                ],
            ],
        }[unit]
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [
                    x.encode("utf-8")
                    for x in ["\U0001d227n", "\u053c\u025bv\u025b", "w\u0c1dlv"]
                ],
                [
                    x.encode("utf-8")
                    for x in ["e\xc3\xc3o", "\U0001f604rld", "\xfcd\xea"]
                ],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [x.encode("utf-8") for x in ["\U0001d229\U0001d227", "v\u025b", "lv"]],
                [x.encode("utf-8") for x in ["\xc3\xc3", "rl", "\xfcd"]],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
        with self.cached_session():
    ...
    def testElementWisePosLen(self, dtype, unit):
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [x.encode("utf-8") for x in ["\U0001d227n", "\u025bv", "lv\u025b"]],
                [x.encode("utf-8") for x in ["e\xc3\xc3o", "rld", "d\xfc"]],
                [
                    x.encode("utf-8")
                    for x in ["xt\xea\xean", "\U00010299ente", "h\x86een"]
                ],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
    ...
    def testBroadcast(self, dtype, unit):
        ...
        test_string = {
            ...
            "UTF8_CHAR": [
                ...
                [x.encode("utf-8") for x in ["nineteen", "twenty", "twentyone"]],
            ],
        }[unit]
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [x.encode("utf-8") for x in ["\U0001d227", "\u025bv", "lv\u025b"]],
                [x.encode("utf-8") for x in ["h", "t\xea", "tee"]],
                [x.encode("utf-8") for x in ["\xcd", "te", "h\x86e"]],
                [x.encode("utf-8") for x in ["i", "en", "nty"]],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
        with self.cached_session():
        ...
        expected_value = {
            ...
            "UTF8_CHAR": [
                [x.encode("utf-8") for x in ["h\xcdr", "\xean", "t"]],
                [x.encode("utf-8") for x in ["\xea", "ur", "\xcd\ua09ct"]],
                [x.encode("utf-8") for x in ["\xea\xea", "\xea\xea", "\ua09ct"]],
            ],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
        with self.cached_session():
        ...
        test_string = {
            "BYTE": b"thirteen",
            "UTF8_CHAR": "th\xcdrt\xea\xean".encode("utf-8"),
        }[unit]
        position = np.array([1, -4, 7], dtype)
        length = np.array([3, 2, 1], dtype)
        expected_value = {
            "BYTE": [b"hir", b"te", b"n"],
            "UTF8_CHAR": [x.encode("utf-8") for x in ["h\xcdr", "t\xea", "n"]],
        }[unit]
        substr_op = string_ops.substr(test_string, position, length, unit=unit)
        with self.cached_session():
            substr = self.evaluate(substr_op)
    ...
    def testOutOfRangeError_VectorScalar(self, dtype, pos, unit):
        ...
        test_string = {
            ...
            "UTF8_CHAR": [
                x.encode("utf-8") for x in ["g\xc3\xc3d", "b\xc3d", "g\xc3\xc3d"]
            ],
        }[unit]
        position = np.array(pos, dtype)
    ...
    def testOutOfRangeError_MatrixMatrix(self, dtype, unit):
        ...
        test_string = {
            ...
            "UTF8_CHAR": [
                [x.encode("utf-8") for x in ["g\xc3\xc3d", "g\xc3\xc3d", "g\xc3\xc3d"]],
                [x.encode("utf-8") for x in ["g\xc3\xc3d", "g\xc3\xc3d", "b\xc3d"]],
                [x.encode("utf-8") for x in ["g\xc3\xc3d", "g\xc3\xc3d", "g\xc3\xc3d"]],
            ],
        }[unit]
    ...
    def testSingleString(self, dtype, unit, rank):
        test_string = {
            ...
            "UTF8_CHAR": [
                "\U0001d229\U0001d227n\U0001d229\U0001d227n\U0001d229\U0001d227n".encode(
                    "utf-8"
                )
            ],
        }[unit]
