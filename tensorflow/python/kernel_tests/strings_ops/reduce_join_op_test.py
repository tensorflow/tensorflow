...
class ReduceJoinTest(UnicodeTestCase):
    def _testReduceJoin(
        self, input_array, truth, truth_shape, axis, keep_dims=False, separator=""
    ):
        ...
        with self.cached_session():
            output = string_ops.reduce_join(
                inputs=input_array, axis=axis, keep_dims=keep_dims, separator=separator
            )
    ...
    def testSeparator(self):
        ...
        self._testReduceJoin(
            input_array, truth_dim_zero, truth_shape_dim_zero, axis=0, separator="  "
        )
        self._testReduceJoin(
            input_array, truth_dim_one, truth_shape_dim_one, axis=1, separator="  "
        )
    ...
    def testKeepDims(self):
        ...
        self._testReduceJoin(
            input_array, truth_dim_zero, truth_shape_dim_zero, axis=0, keep_dims=True
        )
        self._testReduceJoin(
            input_array, truth_dim_one, truth_shape_dim_one, axis=1, keep_dims=True
        )

        expected_val = [["thisisatestpleasedonotpanic"]]
        expected_shape = [1, 1]
        self._testReduceJoin(
            constant_op.constant(input_array),
            expected_val,
            expected_shape,
            keep_dims=True,
            axis=None,
