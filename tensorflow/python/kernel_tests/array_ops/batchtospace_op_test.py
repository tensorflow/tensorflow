...
class BatchToSpaceNDErrorHandlingTest(test.TestCase):
    ...
    def testUnknownShape(self):
        ...
        # Dimensions are partially known.
        t = array_ops.batch_to_space_nd(
            array_ops.placeholder(dtypes.float32, shape=(3 * 2 * 3, None, None, 2)),
            [2, 3],
            array_ops.placeholder(dtypes.int32),
        )
        ...
        # Dimensions are partially known.
        t = array_ops.batch_to_space_nd(
            array_ops.placeholder(dtypes.float32, shape=(3 * 2 * 3, None, 2, 2)),
            [2, 3],
            [[1, 1], [0, 1]],
        )
        ...
        t = array_ops.batch_to_space_nd(
            array_ops.placeholder(dtypes.float32, shape=(3 * 2 * 3, 2, 1, 2)),
            [2, 3],
            [[1, 1], [0, 0]],
        )
...
class BatchToSpaceGradientTest(test.TestCase, PythonOpImpl):
    ...
    def _compare(self, b, h, w, d, block_size, crop_beg, crop_end):
        ...
        x = (
            ...
            .astype(np.float32)
            .reshape([b * block_size * block_size, h, w, d])
        )
