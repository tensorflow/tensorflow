...
class ShapeOpsTest(test.TestCase):
    ...
    def testExpandDimsErrors(self):
        with self.cached_session():
            self.assertRaises(
                ValueError, array_ops.expand_dims, np.zeros([2, 3, 5]), -5
            )
            self.assertRaises(
                ValueError, array_ops.expand_dims, [False, True, True], -5
            )
            self.assertRaises(ValueError, array_ops.expand_dims, np.zeros([2, 3, 5]), 4)
            self.assertRaises(ValueError, array_ops.expand_dims, [False, True, True], 4)
    ...
    def testSqueezeErrors(self):
        for use_gpu in [False, True]:
            with self.cached_session(use_gpu=use_gpu):
                self.assertRaises(
                    ValueError, array_ops.squeeze, np.zeros([1, 2, 1]), [-4]
                )
                self.assertRaises(
                    ValueError, array_ops.squeeze, np.zeros([1, 2, 1]), [0, -4]
                )
                self.assertRaises(
                    ValueError, array_ops.squeeze, np.zeros([1, 2, 1]), [3]
                )
                self.assertRaises(
                    ValueError, array_ops.squeeze, np.zeros([1, 2, 1]), [2, 3]
                )
...
class TileTest(test.TestCase, parameterized.TestCase):
    ...
    def _RunAndVerifyResult(self, rank, use_gpu):
        with self.cached_session(use_gpu=use_gpu):
            ...
            result = self.evaluate(tiled)
        self.assertTrue(
            (np.array(multiples) * np.array(inp.shape) == np.array(result.shape)).all()
        )
    ...
    def testGradientWithSparseGradWithRank1(self, multiples_dtype):
        ...
        outputs = array_ops.gather(
            array_ops.tile(inputs, multiples), [1, 5, 9, 3, 7, 2, 2, 2]
        )
        with self.cached_session():
    ...
    def testGradientWithSparseGradWithRank3(self):
        ...
        outputs = array_ops.gather(
            array_ops.tile(inputs, [3, 4, 2]), [1, 5, 9, 3, 7, 2, 2, 2]
        )
        with self.cached_session():
            error = gradient_checker.compute_gradient_error(
                inputs,
    ...
    def testShapeFunctionEdgeCases(self):
        ...
        with self.assertRaises(ValueError):
            tiled = array_ops.tile(inp, array_ops.placeholder(dtypes.int32, shape=[3]))
