...
def space_to_batch_direct(input_array, block_shape, paddings):
    ...
    padded = np.pad(
        ...
        pad_width=(
            [[0, 0]]
            + list(paddings)
            + [[0, 0]] * (input_array.ndim - 1 - num_block_dims)
        ),
        mode="constant",
    )
    ...
    permuted_reshaped_padded = np.transpose(
        reshaped_padded,
        (
            list(np.arange(num_block_dims) * 2 + 2)
            + [0]
            + list(np.arange(num_block_dims) * 2 + 1)
            + list(
                np.arange(input_array.ndim - num_block_dims - 1)
                + 1
                + num_block_dims * 2
            )
        ),
...
class SpaceToBatchTest(test.TestCase, parameterized.TestCase, PythonOpImpl):
    ...
    def _testPad(self, inputs, paddings, block_size, outputs, dtype=dtypes.float32):
...
class SpaceToBatchErrorHandlingCppTest(SpaceToBatchErrorHandlingTest, CppOpImpl):
    pass
...
class SpaceToBatchNDErrorHandlingTest(test.TestCase):
    ...
    def _testDynamicShape(self, input_shape, block_shape, paddings):
        ...
        t = array_ops.space_to_batch_nd(
            input_placeholder, block_shape_placeholder, paddings_placeholder
        )
    ...
    def testUnknown(self):
        ...
        _ = array_ops.space_to_batch_nd(
            array_ops.placeholder(dtypes.float32),
            array_ops.placeholder(dtypes.int32, shape=(2,)),
            array_ops.placeholder(dtypes.int32),
        )

        # Only number of input dimensions is known.
        t = array_ops.space_to_batch_nd(
            array_ops.placeholder(dtypes.float32, shape=(None, None, None, None)),
            array_ops.placeholder(dtypes.int32, shape=(2,)),
            array_ops.placeholder(dtypes.int32),
        )
        self.assertEqual(4, t.get_shape().ndims)
        ...
        t = array_ops.space_to_batch_nd(
            array_ops.placeholder(dtypes.float32, shape=(None, None, None, 2)),
            array_ops.placeholder(dtypes.int32, shape=(2,)),
            array_ops.placeholder(dtypes.int32),
        )
        ...
        # Dimensions are partially known.
        t = array_ops.space_to_batch_nd(
            array_ops.placeholder(dtypes.float32, shape=(3, None, None, 2)),
            [2, 3],
            array_ops.placeholder(dtypes.int32),
        )
        ...
        # Dimensions are partially known.
        t = array_ops.space_to_batch_nd(
            array_ops.placeholder(dtypes.float32, shape=(3, None, 2, 2)),
            [2, 3],
            [[1, 1], [0, 1]],
        )
        ...
        t = array_ops.space_to_batch_nd(
            array_ops.placeholder(dtypes.float32, shape=(3, 2, 3, 2)),
            [2, 3],
            [[1, 1], [0, 0]],
        )
    ...
    def testInvalidBlockShape(self):
        tf_in = constant_op.constant(-3.5e35, shape=[10, 20, 20], dtype=dtypes.float32)
        ...
        with self.assertRaisesRegex(
            (ValueError, errors.InvalidArgumentError), "block_shape must be positive"
        ):
            array_ops.space_to_batch_nd(tf_in, block_shape, paddings)

    @test_util.run_in_graph_and_eager_modes
    def testOutputSizeOutOfBounds(self):
        tf_in = constant_op.constant(-3.5e35, shape=[10, 19, 22], dtype=dtypes.float32)
...
class SpaceToBatchGradientTest(test.TestCase, PythonOpImpl):
    ...
    def _compare(self, b, h, w, d, block_size, pad_beg, pad_end):
        ...
        x = (
            np.random.normal(0, 1, b * h * w * d * block_size_sq)
            .astype(np.float32)
            .reshape([b, h * block_size, w * block_size, d])
        )
        paddings = np.array([[pad_beg, pad_end], [pad_beg, pad_end]], dtype=np.int32)
...
class SpaceToBatchNDGradientTest(test.TestCase):
    ...
    def _compare(self, input_shape, block_shape, paddings):
        x = (
            np.random.normal(0, 1, np.prod(input_shape))
            .astype(np.float32)
            .reshape(input_shape)
        )
...
class RequiredSpaceToBatchPaddingsTest(test.TestCase):
    def _checkProperties(
        self, input_shape, block_shape, base_paddings, paddings, crops
    ):
    ...
    def _test(self, input_shape, block_shape, base_paddings):
        ...
        self._checkProperties(
            input_shape, block_shape, base_paddings, paddings_const, crops_const
        )
        ...
        t_paddings, t_crops = array_ops.required_space_to_batch_paddings(
            input_shape_placeholder, block_shape_placeholder, base_paddings_placeholder
        )
        with self.cached_session():
