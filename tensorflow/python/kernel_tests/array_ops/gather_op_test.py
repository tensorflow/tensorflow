...
class GatherTest(test.TestCase, parameterized.TestCase):
                ...
                for itype in _INDEX_TYPES:
                    for axis in range(data.ndim):
                        with self.subTest(dtype=dtype, itype=itype, axis=axis):
                            ...
                            gather_val = self.evaluate(gather_t)
                            self.assertAllEqual(
                                np.take(params_np, 2, axis=axis), gather_val
                            )
                ...
                for itype in _INDEX_TYPES:
                    for axis in range(data.ndim):
                        with self.subTest(dtype=dtype, itype=itype, axis=axis):
                            ...
                            expected_shape = (
                                data.shape[:axis] + (4,) + data.shape[axis + 1 :]
                            )
                            self.assertEqual(expected_shape, gather_t.get_shape())
                    ...
                    for axis in range(len(shape)):
                        ...
                        with self.subTest(
                            ...
                            gather = array_ops.gather(
                                tf_params, tf_indices, axis=tf_axis
                            )
                            ...
                            params_grad, indices_grad, axis_grad = (
                                gradients_impl.gradients(
                                    gather,
                                    [tf_params, tf_indices, tf_axis],
                                    gather_grad,
                                )
                            )
    ...
    def testBadAxis(self):
        ...
        @def_function.function(
            autograph=False,
            jit_compile=False,
            input_signature=[tensor_spec.TensorSpec(shape=None, dtype=dtypes.int32)]
            * 3,
        )
        ...
        indices = 0
        functions = [
            ("array_ops.gather", array_ops.gather),
            ("gather", gather),
    ...
    @test_util.run_in_graph_and_eager_modes
    def testBatchDimsMatchesPythonBatching(
        self, params_shape, indices_shape, batch_dims, axis, output_shape
    ):
