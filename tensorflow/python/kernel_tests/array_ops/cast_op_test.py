    ...
    def testSaturateRealToComplex(self):
        ...
        for in_type in in_types:
            for out_type in out_types:
                ...
                x, y = self.evaluate([x, y])
                correct = np.maximum(
                    out_type.real_dtype.min, np.minimum(out_type.real_dtype.max, x)
                )
                self.assertAllEqual(correct, y)
        ...
        for in_type in in_types:
            for out_type in out_types:
                ...
                x_real = constant_op.constant(
                    [lo, lo + 1, lo // 2, hi // 2, hi - 1, hi], dtype=in_type.real_dtype
                )
                ...
                correct = np.maximum(
                    ...
                    np.minimum(out_type.real_dtype.max, np.real(x)),
                ) + 1j * np.maximum(
                    out_type.real_dtype.min,
