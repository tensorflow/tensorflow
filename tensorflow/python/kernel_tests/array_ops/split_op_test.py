...
_TEST_DTYPES = (dtypes.float32, dtypes.float64, dtypes.complex64, dtypes.complex128)
...
class SplitOpTest(test.TestCase):
    ...
    def testFailWithoutExplicitNum(self):
        ...
        with self.session() as sess:
            ...
            self.assertIn(
                "Cannot infer argument `num` from shape", str(context.exception)
            )
        ...
        for axis in [1, -1]:
            with self.cached_session():
                ...
                for i, output in enumerate(outputs):
                    self.assertEqual(
                        output.get_shape().as_list(), [None, size_splits[i]]
                    )
    ...
    def _RunAndVerify(self, dtype, large_num_splits=False):
        ...
        with test_util.device(use_gpu=True):
            result = self.evaluate(
                array_ops.split(value=inp, num_or_size_splits=num_split, axis=split_dim)
            )
        slices = [slice(0, x) for x in shape]
        offset = 0
    ...
    def testNonexistentDimTensor(self):
        ...
        with self.session() as sess:
            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, "must have exactly one element"
            ):
    ...
    def testBadSplitSizes(self):
        ...
        with self.assertRaisesRegex(
            (ValueError, errors_impl.InvalidArgumentError),
            "Determined shape must either match input" "|can't split axis",
        ):
            splits = [1, 2]
            self.evaluate(array_ops.split(x, splits, axis=0))
