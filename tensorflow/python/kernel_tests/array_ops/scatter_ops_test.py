...
class ScatterTest(test.TestCase):
    def _VariableRankTest(
        self, tf_scatter, vtype, itype, repeat_indices=False, updates_are_scalar=False
    ):
        np.random.seed(8)
                ...
                for extra_shape in (), (5,), (5, 9):
                    ...
                    if size > 1 and repeat_indices:
                        ...
                        for _ in range(size - size // 2):
                            ...
                            indices = np.append(
                                indices, indices[np.random.randint(size // 2)]
                            )
                        np.random.shuffle(indices)
    ...
    def _VariableRankTests(
        self, tf_scatter, repeat_indices=False, updates_are_scalar=False
    ):
        ...
        for vtype in vtypes:
            for itype in (np.int32, np.int64):
                self._VariableRankTest(
                    tf_scatter, vtype, itype, repeat_indices, updates_are_scalar
                )
    ...
    @test_util.run_v1_only(
        "ResrouceVariable has deterministic scatter " "implementation"
    )
    @test_util.run_cuda_only
    def testDeterminismExceptionThrowing(self):
        v = variables.RefVariable(np.array([1.0, 2.0, 3.0]))
