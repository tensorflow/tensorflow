...
@deprecated(
    None,
    "xla.experimental.compile is deprecated. Consider using "
    ...
    def AddOp(self, op: ops.Operation):
        ...
        if op.type in _DENYLISTED_OPS:
            logging.error(
                ...
                "fail if this op is used in the graph. ",
                op.type,
                op.name,
            )
...
def is_flat(outputs):
    ...
    if isinstance(outputs, collections_abc.Sequence):
        for o in outputs:
            if (
                isinstance(o, collections_abc.Sequence)
                or isinstance(o, collections_abc.Mapping)
                or hasattr(o.__class__, "__attrs_attrs__")
            ):
                return False
...
class _CapturedObject(object):
    ...
    def capture(self, o):
        if self._object:
            raise RuntimeError(
                "InternalError: _CapturedObject can capture only once. Please file bug."
            )
