...
class MemoryAlignmentTest(trt_test.TfTrtIntegrationTestBase):
    ...
    def GraphFn(self, inp):
        ...
        conv = nn.conv2d(
            input=inp, filter=e1, strides=[1, 1, 1, 1], padding="VALID", name="conv"
        )
        out = nn.conv2d(
            input=conv, filter=e2, strides=[1, 1, 1, 1], padding="VALID", name="conv_2"
        )
        return array_ops.squeeze(out, name="output_0")

    def GetParams(self):
        return self.BuildParams(
            self.GraphFn, dtypes.float32, [[2, 15, 15, 3]], [[2, 15, 15, 10]]
        )
    ...
    def ExpectedAbsoluteTolerance(self, run_params):
        ...
        return 1.0e-06 if run_params.precision_mode == "FP32" else 1.0e-02
