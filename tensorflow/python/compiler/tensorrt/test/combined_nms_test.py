...
class CombinedNmsTest(trt_test.TfTrtIntegrationTestBase):
    ...
    def ShouldRunTest(self, run_params):
        ...
        should_run = should_run and not trt_test.IsQuantizationMode(
            run_params.precision_mode
        )
        reason += " and precision != INT8"
        # Only run for TRT 7.1.3 and above.
        return (
            should_run and trt_utils.is_linked_tensorrt_version_greater_equal(7, 1, 3),
...
class CombinedNmsExecuteNativeSegmentTest(CombinedNmsTest):
    ...
    def ShouldRunTest(self, run_params):
        ...
        return (
            should_run and not run_params.dynamic_engine,
            reason + " and static engines",
        )
...
class CombinedNmsTestTopK(CombinedNmsTest):
    ...
    def GraphFn(
        self, pre_nms_boxes, pre_nms_scores, max_boxes_to_draw, max_detetion_points
    ):
        ...
        max_output_size_per_class_tensor = constant_op.constant(
            max_detetion_points, dtype=dtypes.int32, name="max_output_size_per_class"
        )
