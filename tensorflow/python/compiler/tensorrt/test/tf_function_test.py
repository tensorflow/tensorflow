...
from tensorflow.python.compiler.tensorrt.test.tf_trt_integration_test_base import (
    GraphState,
)
from tensorflow.python.compiler.tensorrt.test.tf_trt_integration_test_base import (
...
class TfFunctionTest(trt_test.TfTrtIntegrationTestBase):
    ...
    def ShouldRunTest(self, run_params):
        ...
        if not should_run:
            ...
            return (
                not IsQuantizationWithCalibration(run_params),
    ...
    def _ChainAllNodes(self, graph_def):
        return itertools.chain(
            graph_def.node,
            itertools.chain(*[func.node_def for func in graph_def.library.function]),
        )
                ...
                if not func_def.signature.name.startswith("TRTEngine"):
                    for key, value in self._test_conversion_params.items():
                        ...
                        if isinstance(value, str):
                            self.assertEqual(
                                func_def.attr[key].s, compat.as_bytes(value)
                            )
    ...
    def _conv_and_pool_0(self, inp):
        ...
        conv_filter = constant_op.constant(
            [[[[1.0, 0.5, 4.0], [1.0, 0.5, 1.0]]]], name="weights", dtype=dtype
        )
        conv = nn.conv2d(
            input=inp,
            filter=conv_filter,
    ...
    def GetParams(self):
        return self.BuildParams(
            self.GraphFn, dtypes.float32, [[10, 32, 32, 2]], [[10, 8, 8, 3]]
        )
    ...
    def _copy_test_attributes_to_func_def(self, func_def):
        self._copy_test_attr_to_func_def(
            func_def=func_def, param_name="_tftrt_convert_function", attr_value_type="b"
        )
        self._copy_test_attr_to_func_def(
            func_def=func_def, param_name="_tftrt_trt_logger_name", attr_value_type="s"
        )
        self._copy_test_attr_to_func_def(
            func_def=func_def, param_name="_tftrt_max_batch_size", attr_value_type="i"
        )
        self._copy_test_attr_to_func_def(
            func_def=func_def,
        ...
        self._copy_test_attr_to_func_def(
            func_def=func_def, param_name="_tftrt_precision_mode", attr_value_type="s"
        )
        self._copy_test_attr_to_func_def(
            func_def=func_def,
        ...
        self._copy_test_attr_to_func_def(
            func_def=func_def, param_name="_tftrt_use_calibration", attr_value_type="b"
        )
        self._copy_test_attr_to_func_def(
            func_def=func_def,
        ...
        self._copy_test_attr_to_func_def(
            func_def=func_def, param_name="_tftrt_profile_strategy", attr_value_type="s"
        )
        self._copy_test_attr_to_func_def(
            func_def=func_def,
    ...
    def _MakeSavedModelV1(self, run_params):
        ...
        old_saved_model_file = os.path.join(
            saved_model_dir, constants.SAVED_MODEL_FILENAME_PB
        )
    ...
    def _MakeSavedModelV2(self, run_params):
        ...
        for func_def in new_meta_graph_def.graph_def.library.function:
            logging.info(
                "_MakeSavedModelV2, func_def name: %s", func_def.signature.name
            )
    ...
    def _VerifyGraphDefV1(self, run_params, original_gdef, gdef_to_verify, graph_state):
        ...
        for node in all_nodes:
            if node.op == "TRTEngineOp":
                ...
                self.assertEqual(
                    run_params.dynamic_engine, is_dynamic_engine, node.name
                )
                self.assertEqual(
                    node.attr["use_calibration"].b,
                ...
                if (
                    IsQuantizationWithCalibration(run_params)
    ...
    def _ShouldConverterBuild(self, run_params):
        return (
            ...
            and run_params.dynamic_engine
        )

    def RunTest(self, run_params):
        self._test_conversion_params[
            "_tftrt_precision_mode"
        ] = run_params.precision_mode
        self._test_conversion_params[
            "_tftrt_use_calibration"
        ] = run_params.use_calibration
