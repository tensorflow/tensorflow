...
class TfTrtIntegrationTestBase(test_util.TensorFlowTestCase):
    ...
    def BuildParams(self, graph_fn, dtype, input_shapes, output_shapes):
        ...
        output_mask = [
            [False] + [True] * (len(shape) - 1) if shape else []
    ...
    def ExpectedAbsoluteTolerance(self, run_params):
        ...
        if run_params.precision_mode == "INT8":
            return 3e-1
        return 1.0e-05 if run_params.precision_mode == "FP32" else 2.0e-02
            ...
            def _BuildInputFn():
                for shapes in self._GetParamsCached().input_dims:
                    yield [
                        ...
                        for (x, spec) in zip(
                            shapes, self._GetParamsCached().input_specs
                        )
                    ]
    ...
    def _VerifyConnections(
        self, expected_engines, expected_input_map, original_gdef, converted_gdef
    ):
    ...
    def _VerifyGraphDefV2(self, run_params, original_gdef, gdef_to_verify, graph_state):
        ...
        for func in gdef_to_verify.library.function:
            if not re.search(
                r"TRTEngineOp_\d{3,}_\d{3,}_native_segment", func.signature.name
            ):
                ...
                for dim_list in self._GetParamsCached().input_dims:
                    ...
                    for spec, np_shape in zip(input_specs, dim_list):
                        ...
                        if not np.issubdtype(np_dtype, np.bool_):
                            ...
                            data = (scale * np.random.random_sample(np_shape)).astype(
                                np_dtype
                            )
                        else:
