...
def run_without_freezing():
  """Executes test with model freezing disabled.
  Returns:
...
def run_all_without_freezing():
  """Executes all tests in a class with model freezing disabled."""
...
class TwoEnginesOneVarModel(module.Module):
  ...
  The engines comprise ops that require constant inputs, so the tests check
  that the variable is appropriately passed as weights to their converters in
  this situation.
  """
  # TODO(lsugy): figure out why the two engines seem to have resource inputs.
...
class BiasAddTwoConstInputsModel(module.Module):
  ...
  If the model were frozen, this would be constant-folded. The tests check
  that the BiasAdd converter handles cases with two constant inputs.
  """
...
class BatchMatMulTwoConstInputsModel(module.Module):
  ...
  If the model were frozen, this would be constant-folded. The tests check
  that the BatchMatMul converter handles cases with two constant inputs.
  """
