...
def load_meta_graph(
    ...
    with (
        framework_ops.Graph().as_default() as graph,
        session.Session(graph=graph) as sess,
    ):
        ...
        with framework_ops.Graph().as_default():
            with session.Session(config=config_proto) as sess:
                ...
                try:
                    ...
                    raise RuntimeError(
                        "Failed to run model inference! Model information: {}".format(
                            str(self)
                        )
                    ) from exc
        return TestResult(
...
class ModelHandlerV2(_ModelHandlerBase):
    ...
    def run(
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to run model inference! Model information: {}".format(str(self))
            ) from exc
        return TestResult(
            model_config=self.model_config,
            enable_gpu=enable_gpu,
...
class _TrtModelHandlerBase(_ModelHandlerBase):
    ...
    def _check_contains_trt_engine(self, graph_def: graph_pb2.GraphDef):
        if "TRTEngineOp" not in [node.op for node in graph_def.node]:
            raise RuntimeError(
                "Failed to convert to TensorRT! Model Information: {}".format(str(self))
            )
