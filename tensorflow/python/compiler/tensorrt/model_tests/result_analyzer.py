...
def str_histogram(buffer, desc):
    ...
    for num, bin_start, bin_end in zip(hist, bin_edges, bin_edges[1:]):
        ...
        ret += (
            "({:<{max_start_bin_width}}, {:<{max_end_bin_width}}) | {:10} | {:}\n"
        ).format(
            bin_start,
            bin_end,
            num,
...
def check_column(
    df: DataFrame, row: int, name: str, fn: Callable[[float], bool]
) -> bool:
...
class ResultAnalyzer:
    ...
    def analysis(
        ...
        df_acc, acc_hist = analyze_test_numerics(
            test_results, self._use_cpu_numerics_baseline
        )
        df += df_acc
