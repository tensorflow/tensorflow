...
class SampleRunner(object):
  ...
  def _run_impl(
    ...
    for model_config in self._configs:
      ...
      with gfile.Open(
          os.path.join(test_dir, "accuracy_histograms.txt"), "w") as f:
...
def main(argv):
  ...
  tolerances = {
    trt.TrtPrecisionMode.FP32 : tol(perf=float(FLAGS.fp32_speedup_tolerance), acc=float(FLAGS.fp32_abs_tolerance)),
    trt.TrtPrecisionMode.FP16 : tol(perf=float(FLAGS.fp16_speedup_tolerance), acc=float(FLAGS.fp16_abs_tolerance)),
    trt.TrtPrecisionMode.INT8 : tol(perf=float(FLAGS.int8_speedup_tolerance), acc=float(FLAGS.int8_abs_tolerance)),
  }
