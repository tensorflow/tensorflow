...
flags.DEFINE_integer(
    "batch_size", 128, "The batch size used to run the testing model with."
)
...
flags.DEFINE_enum(
    "output_format", "CSV", ["CSV", "JSON"], "Output format of analysis results."
)
...
class SampleRunner(object):
    ...
    def __init__(
        ...
        self._model_handler_manager_cls = (
            ...
            if use_tf2
            else model_handler.ModelHandlerManagerV1
        )
    ...
    def _write_analysis_result(self, df: result_analyzer.DataFrame, path: str) -> None:
        if self._output_format == "CSV":
            ...
            raise NotImplementedError(
                "Unsupported output format: {}".format(self._output_format)
            )
    ...
    def _run_impl(
        ...
        for model_config in self._configs:
            ...
            with gfile.Open(
                os.path.join(test_dir, "default_tensorrt_params.txt"), "w"
            ) as f:
                f.write(repr(default_trt_converter_params))
            with gfile.Open(
                os.path.join(test_dir, "accuracy_histograms.txt"), "w"
            ) as f:
...
def main(argv):
    ...
    tolerances = {
        trt.TrtPrecisionMode.FP32: tol(
            perf=float(FLAGS.fp32_speedup_tolerance),
            acc=float(FLAGS.fp32_abs_tolerance),
        ),
        trt.TrtPrecisionMode.FP16: tol(
            perf=float(FLAGS.fp16_speedup_tolerance),
            acc=float(FLAGS.fp16_abs_tolerance),
        ),
        trt.TrtPrecisionMode.INT8: tol(
            perf=float(FLAGS.int8_speedup_tolerance),
    ...
    analyzer = result_analyzer.ResultAnalyzer(
        ...
        perf_checkers={
            precision: functools.partial(
                result_analyzer.check_column, name="speedup", fn=lambda x: x > tol.perf
            )
