...
def is_experimental_feature_activated(feature_name):
    ...
    return feature_name in os.environ.get(
        "TF_TRT_EXPERIMENTAL_FEATURES", default=""
    ).split(",")
...
def draw_graphdef_as_graphviz(graphdef, dot_output_filename):
    ...
    with open(dot_output_filename, "w") as f:
        ...
        for node in graphdef.node:
            ...
            print(
                f'    "{output_name}" [label=<{node_label}> fillcolor={color_idx}];',
                file=f,
            )
        ...
        for dtype, color_idx in dtype_index.items():
            print(
                f"    {dtype} [fillcolor={color_idx} label=<<b>{dtype}</b>>];", file=f
            )
