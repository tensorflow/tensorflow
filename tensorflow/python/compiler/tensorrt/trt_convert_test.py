...
from tensorflow.compiler.tf2tensorrt.utils.trt_engine_instance_pb2 import (
    TRTEngineInstance,
...
class TrtConvertTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def _ConvertGraphV1(
        ...
        converter = trt_convert.TrtGraphConverter(
            ...
            else self._GetGraphDefForV1(device),
    ...
    def _TestTrtGraphConverter(
        ...
        if output_saved_model_dir:
            saved_model_graph_def = saved_model_utils.get_meta_graph_def(
                output_saved_model_dir, tag_constants.SERVING
            ).graph_def
    ...
    def testTrtGraphConverter_OnlineConversion(self, device):
        ...
        config = self._GetConfigProto(
            rewriter_config=trt_convert.get_tensorrt_rewriter_config(
                conversion_params, is_dynamic_op=False, max_batch_size=1, is_v2=False
            )
        )
    ...
    def _GetAssetFile(self, output_saved_model_dir, trt_engine_name):
        asset_file = os.path.join(
            output_saved_model_dir, "assets/trt-serialized-engine." + trt_engine_name
        )
        return asset_file
    ...
    def _BuildGraphWithInputGeneratorTwoInputs(self, InputFunc, np_input=None):
        ...
        trt_engine_name = self._GetUniqueTRTEngineOp(
            converter._converted_graph_def
        ).name

        # Save the converted model without any TRT engine cache.
        output_saved_model_dir = self.mkdtemp()
        converter.save(output_saved_model_dir)
        unexpected_asset_file = self._GetAssetFile(
            output_saved_model_dir, trt_engine_name
        )
        ...
        output_saved_model_dir = self.mkdtemp()
        converter.save(output_saved_model_dir)
        expected_asset_file = self._GetAssetFile(
            output_saved_model_dir, trt_engine_name
        )
    ...
    def testTrtGraphConverter_DynamicConversion_v2(self):
        ...
        output_with_trt = converted_signature(
            inp1=ops.convert_to_tensor(np_input1), inp2=ops.convert_to_tensor(np_input2)
        )
        ...
        self.assertAllClose(
            expected_output, list(output_with_trt.values())[0], atol=1e-6, rtol=1e-6
        )
    ...
    def testTrtGraphConverter_ShapeOp_Int32InputOutput_v2(self):
        ...
        output_saved_model_dir, expected_output = self._BuildGraphWithInputGenerator(
            _InputFunc, np_input
        )
    ...
    def testTrtGraphConverter_AllowEngineNativeSegmentExecution(self):
        ...
        with self.assertRaisesRegex(
            errors.AbortedError, r"User disallowed engine native segment execution"
        ):
            try:
                converter.build(input_fn=_InputFn)
            finally:
    ...
    def testTrtGraphConverter_AllowBuildAtRuntime(
        ...
        def _CheckFn(node):
            self.assertEqual(
                node.attr["_allow_build_at_runtime"].b, allow_build_at_runtime
            )
    ...
    def testTrtGraphConverter_DevicePlacement(self, device_id):
        ...
        if len(gpus) < 2:
            self.skipTest(
                "Expected at least 2 GPUs but found {} GPUs".format(len(gpus))
            )
    ...
    def _TestVariableHelper(
        ...
        for lib_function in graph_def.library.function:
            if re.search(
                r"TRTEngineOp_\d+_\d+_native_segment", lib_function.signature.name
            ):
        ...
        self.assertEqual(output.shape, (4, 1, 1))
        self.assertAllClose(
            np.asarray([42.0, 42.0, 42.0, 42.0]).reshape([4, 1, 1]), output
        )
