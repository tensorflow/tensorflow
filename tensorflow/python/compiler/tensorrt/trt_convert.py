...
_pywrap_py_utils = LazyLoader(
    "_pywrap_py_utils", globals(), "tensorflow.compiler.tf2tensorrt._pywrap_py_utils"
)
...
if (
    _pywrap_py_utils.is_tensorrt_enabled()
...
def _check_conversion_params(conversion_params, is_v2=False):
    ...
    if (
        conversion_params.minimum_segment_size <= 0
        and conversion_params.minimum_segment_size != -1
    ):
        raise ValueError(
...
def _check_trt_version_compatibility():
    ...
    if loaded_version[0] != linked_version[
        0
    ] or not trt_utils.is_loaded_tensorrt_version_greater_equal(*linked_version):
        logging.error(
...
def _get_tensorrt_rewriter_config(
    conversion_params,
    is_dynamic_op=None,
    max_batch_size=None,
    is_v2=False,
    ...
    if (is_dynamic_op is None or is_dynamic_op) and max_batch_size is not None:
        raise ValueError(
            "max_batch_size has to be None for TF2 or when is_dynamic_op == True in TF1"
        )
    if (
        is_dynamic_op is not None
        and not is_dynamic_op
        and not isinstance(max_batch_size, int)
    ):
        raise ValueError(
    ...
    rewriter_config_with_trt.experimental_disable_folding_quantization_emulation = (
        trt_utils.is_linked_tensorrt_version_greater_equal(8, 0, 0)
        or trt_utils.is_loaded_tensorrt_version_greater_equal(8, 0, 0)
    )
...
def get_tensorrt_rewriter_config(
    ...
    return _get_tensorrt_rewriter_config(
        conversion_params,
        is_dynamic_op,
        max_batch_size,
        is_v2,
...
class TrtGraphConverter(object):
    ...
    def __init__(
        ...
        self._need_calibration = (
            (precision_mode == TrtPrecisionMode.INT8)
            or (precision_mode == TrtPrecisionMode.INT8.lower())
        ) and use_calibration
        ...
        if is_dynamic_op:
            ...
            if not isinstance(max_batch_size, int):
                raise ValueError(
                    "When is_dynamic_op==False max_batch_size should be an integer"
                )
    ...
    def _collections_to_keep(self, collection_keys):
        ...
        collections_to_remove = ops.GraphKeys._VARIABLE_COLLECTIONS + [
            ops.GraphKeys.TRAIN_OP,
    ...
    def calibrate(self, fetch_names, num_runs, feed_dict_fn=None, input_map_fn=None):
        ...
        with session.Session(
            graph=self._calibration_graph, config=calibrate_config
        ) as calibration_sess:
...
def _remove_native_segments(input_func):
    ...
    logging.info(
        f"Found and deleted native segments from {nodes_deleted} TRTEngineOp nodes."
    )
...
class TrtGraphConverterV2(object):
    ...
    @deprecation.deprecated_args(
        None, "Use individual converter parameters instead", "conversion_params"
    )
    def __init__(
        self,
        input_saved_model_dir=None,
        ...
        self._need_calibration = (
            ...
            or (conversion_params.precision_mode == TrtPrecisionMode.INT8.lower())
    ...
    def build(self, input_fn):
        ...
        if (
            ...
            and self._calibration_input_fn is None
        ):
            raise RuntimeError(
                "Need to provide the calibration_input_fn arg while calling convert()."
            )
    ...
    def save(
        self, output_saved_model_dir, save_gpu_specific_engines=True, options=None
    ):
    ...
    def summary(self, line_length=160, detailed=True, print_fn=None):
        ...
        if line_length < 160:
            raise ValueError(
                f"Invalid `line_length` value has been received: "
        ...
        columns = [
            ...
            ("TRTEngineOP Name", 0.20),  # 20%
            ("Device", 0.09),  # 29%
            ("# Nodes", 0.05),  # 34%
            ("# Inputs", 0.09),  # 43%
            ("# Outputs", 0.09),  # 52%
            ("Input DTypes", 0.12),  # 64%
            ("Output Dtypes", 0.12),  # 76%
            ("Input Shapes", 0.12),  # 88%
            ("Output Shapes", 0.12),  # 100%
        ]
        ...
        print_fn(
            f"[*] % of OPs Converted: {conversion_ratio:.2f}% "
