...
def _get_tensorrt_rewriter_config(
    ...
    if (is_dynamic_op is None or is_dynamic_op) and max_batch_size is not None:
        raise ValueError(
            "max_batch_size has to be None for TF2"
    ...
    if not disable_non_trt_optimizers:
        ...
        rewriter_config_with_trt.optimizers.extend(["constfold", "layout", "constfold"])
...
class TrtGraphConverterV2(object):
    ...
    def _rebuild_func(self, func):
        ...
        # serialization)
        rebuilt_func.graph.structured_input_signature = func.structured_input_signature
        return rebuilt_func
    ...
    def convert(self, calibration_input_fn=None):
        ...
        for f in self._converted_graph_def.library.function:
            while context.context().has_function(f.signature.name):
                logging.info(
                    "Removing original function %s from the context", f.signature.name
                )
