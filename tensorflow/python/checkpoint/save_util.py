...
) -> Tuple[List[_TrackableData], List[_TrackableData], Dict[str, List[_TrackableData]]]:
    ...
    for trackable in trackable_objects:
        ...
        for child in graph_view.list_children(trackable):
            children_proto.append(
                trackable_object_graph_pb2.TrackableObjectGraph.TrackableObject.ObjectReference(
                    node_id=node_ids[child.ref], local_name=child.name
                )
            )

        trackable_data.append(
            _TrackableData(
                trackable,
...
def _fill_object_graph_proto(
    ...
    for checkpoint_id, td in enumerate(trackable_data):
        ...
        object_graph_proto.nodes.add(
            slot_variables=td.slot_variable_proto, children=td.children_proto
        )
...
def _get_and_write_tensors_to_serialize(
    ...
    for td in tensor_trackables:
        ...
        if (
            not saveable_object_util.trackable_has_serialize_to_tensor(
                td.object_to_save
            )
            or legacy_name
        ):
...
def _get_tensors_from_legacy_saveable(
    ...
    named_saveable_objects, _ = save_util_v1.generate_saveable_objects(
...
def _get_tensors_from_trackable(
    ...
    for tensor_name, maybe_tensor in ret_tensor_dict.items():
        ...
        checkpoint_key = trackable_utils.checkpoint_key(
            trackable_data.object_name, local_name
        )
...
def _get_and_write_pystate_feed_additions(
    ...
    for td in pystate_trackables:
        trackable = td.object_to_save
        checkpoint_key = trackable_utils.checkpoint_key(
            td.object_name, python_state.PYTHON_STATE
        )
...
def serialize_graph_view(
    ...
    tensor_trackables, pystate_trackables, registered_trackables = _split_trackables(
        trackable_data
    )
    ...
    if cache is None:
        ...
        feed_additions = None
        serialized_tensors.update(
            _get_and_write_tensors_to_serialize(
                pystate_trackables,
                node_ids,
                call_with_mapped_captures,
                cache,
        ...
        new_serialized_tensors, feed_additions = _get_and_write_pystate_feed_additions(
            pystate_trackables, cache, object_graph_proto
        )
