...
class _CheckpointRestoreCoordinatorDeleter:
    ...
    def __del__(self):
        ...
        if unused_nodes_in_checkpoint or unrestored_attributes_in_object:
            ...
            log_fn(
                "Detecting that an object or model or tf.train.Checkpoint is being"
            ...
            for node_path, attr in unrestored_attributes_in_object:
                log_fn(
                    "An attribute in the restored object could not be found in the "
    ...
    def restore_saveables(
        ...
        if tensor_saveables or registered_savers:
            ...
            new_restore_ops = functional_saver.MultiDeviceSaver.from_saveables(
                flat_saveables, registered_savers
...
def _objects_with_attributes(full_list):
    ...
    return [
        o for o in full_list if saveable_object_util.saveable_objects_from_trackable(o)
    ]
...
class TrackableSaver:
    ...
    def _gather_serialized_tensors(self, object_graph_tensor=None):
        ...
        serialized_tensors, feed_additions, registered_savers, graph_proto = (
            save_util.serialize_graph_view(
                self._graph_view, self._object_map, cache=self._cache
            )
        )
    ...
    def _save_cached_when_graph_building(
        ...
        if (
            ...
            saver = functional_saver.MultiDeviceSaver(
                serialized_tensors, registered_savers
            )
    ...
    def save(self, file_prefix, checkpoint_number=None, session=None, options=None):
        ...
        if session:
            ...
            raise RuntimeError(
                f'Unable to save checkpoint to "{file_prefix}" '
    ...
    def restore(self, save_path, options=None):
        ...
        try:
            ...
            return NameBasedSaverStatus(
                restore_coordinator, object_graph_view=self._graph_view
            )
        ...
        restore_lib.CheckpointPosition(checkpoint=checkpoint, proto_id=0).restore(
            self._graph_view.root, reader
        )
...
class CheckpointV1(autotrackable.AutoTrackable):
    ...
    def _write(self, file_prefix, session=None, options=None):
        ...
        output = self._saver.save(
            file_prefix=file_prefix, session=session, options=options
        )
        end_time = time.time()
    ...
    def save(self, file_prefix, session=None, options=None):
        ...
        file_path = self.write(
            "%s-%d" % (file_prefix, checkpoint_number), session=session, options=options
        )
        checkpoint_management.update_checkpoint_state_internal(
...
class Checkpoint(autotrackable.AutoTrackable):
    ...
    def __init__(self, root=None, **kwargs):
        ...
        self._saver = TrackableSaver(
            graph_view_lib.ObjectGraphView(
                root if root else self, attached_dependencies=attached_dependencies
            )
        )
    ...
    def save(self, file_prefix, options=None):
        ...
        return self._write("%s-%d" % (file_prefix, checkpoint_number), options=options)
    ...
    def read(self, save_path, options=None):
        ...
        if (
            ...
            and self._checkpoint_options.experimental_enable_async_checkpoint
        ):
            if context.executing_eagerly():
    ...
    def restore(self, save_path, options=None):
        ...
        **Loading from SavedModel checkpoints**
        ...
        if (
            ...
            and (
                gfile.Exists(path_helpers.get_saved_model_pb_path(save_path))
                or gfile.Exists(path_helpers.get_saved_model_pbtxt_path(save_path))
            )
        ):
