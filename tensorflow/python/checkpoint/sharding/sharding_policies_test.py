...
class ShardingPoliciesTest(test.TestCase):
    def _get_shardable_tensors_by_task(self, root):
        serialized_tensors, _, _, _ = checkpoint.TrackableSaver(
        ...
        for obj, tensor_dict in serialized_tensors.items():
            ...
            for checkpoint_key, tensor_slice_dict in tensor_dict.items():
                ...
                for slice_spec, tensor_save_spec in tensor_slice_dict.items():
                    ...
                    device = (
                        device_lib.DeviceSpec.from_string(tensor_save_spec.device)
    ...
    def test_ShardByTaskPolicy(self):
        ...
        self.assertEqual(
            self.evaluate(shards[0]["v0/.ATTRIBUTES/VARIABLE_VALUE"][""]), v0.numpy()
        )
        self.assertEqual(
            self.evaluate(shards[1]["v1/.ATTRIBUTES/VARIABLE_VALUE"][""]), v1.numpy()
        )
        self.assertEqual(
            self.evaluate(shards[2]["v2/.ATTRIBUTES/VARIABLE_VALUE"][""]), v2.numpy()
        )
    ...
    def test_CheckpointOption_ShardByTaskPolicy(self):
        ...
        save_path = ckpt.save(
            ...
            options=checkpoint_options.CheckpointOptions(
                experimental_sharding_callback=(sharding_policies.ShardByTaskPolicy())
            ),
        )
    ...
    def test_MaxShardSizePolicy_1D(self):
        ...
        with ops.device("cpu:0"):
            v0 = resource_variable_ops.ResourceVariable(
                [0.0, 1.0, 2.0, 3.0], name="v0", dtype=dtypes.float32
            )
            v1 = resource_variable_ops.ResourceVariable(
                [[4], [5], [6], [7]], name="v1", dtype=dtypes.int32
            )
        self.evaluate(v0.initializer)
        self.evaluate(v1.initializer)
        root.v0 = v0
        ...
        self.assertAllEqual(self.evaluate(shards[0][v0_name][slice_spec]), [0.0, 1.0])

        slice_spec = V0SaveSliceInfo(var_offset=[2], var_shape=[2]).spec
        self.assertAllEqual(self.evaluate(shards[1][v0_name][slice_spec]), [2.0, 3.0])

        # V1
        slice_spec = V1SaveSliceInfo(var_offset=[0, 0], var_shape=[2, 1]).spec
        self.assertAllEqual(self.evaluate(shards[2][v1_name][slice_spec]), [[4], [5]])

        slice_spec = V1SaveSliceInfo(var_offset=[2, 0], var_shape=[2, 1]).spec
        self.assertAllEqual(self.evaluate(shards[3][v1_name][slice_spec]), [[6], [7]])
    ...
    def test_MaxShardSizePolicy_2D(self):
        ...
        with ops.device("cpu:0"):
            ...
            v1 = resource_variable_ops.ResourceVariable(
                [[[6.0], [7.0]], [[8.0], [9.0]], [[10.0], [11.0]]], name="v1"
            )
        self.evaluate(v0.initializer)
        self.evaluate(v1.initializer)
        root.v0 = v0
        ...
        self.assertAllEqual(self.evaluate(shards[0][v0_name][slice_spec]), [[0, 1]])

        slice_spec = V0SaveSliceInfo(var_offset=[1, 0], var_shape=[1, 2]).spec
        self.assertAllEqual(self.evaluate(shards[1][v0_name][slice_spec]), [[2, 3]])

        slice_spec = V0SaveSliceInfo(var_offset=[2, 0], var_shape=[1, 2]).spec
        self.assertAllEqual(self.evaluate(shards[2][v0_name][slice_spec]), [[4, 5]])
        ...
        self.assertAllEqual(
            self.evaluate(shards[2][v1_name][slice_spec]), [[[6.0]], [[8.0]], [[10.0]]]
        )
        ...
        slice_spec = V0SaveSliceInfo(var_offset=[2, 0], var_shape=[1, 2]).spec
        self.assertAllEqual(self.evaluate(shards[1][v0_name][slice_spec]), [[4, 5]])
    ...
    def test_MaxShardSizePolicy_Strings(self):
        ...
        with ops.device("cpu:0"):
            v0 = resource_variable_ops.ResourceVariable(
                v_strings, name="v0", dtype=dtypes.string
            )
        self.evaluate(v0.initializer)
        root.v0 = v0
    ...
    def test_MaxShardSizePolicy_LargeScalar(self):
        ...
        tensor_val = (
            self.evaluate(shards[1][v0_name][""])
    ...
    def test_MaxShardSizePolicy_PreSlicedTensor(self):
        ...
        with ops.device("cpu:0"):
            ...
            #            [5.0, 6.0, 7.0, 8.0, 9.0]]
            sliced_v0 = ResourceVariableWithSliceSpec(
