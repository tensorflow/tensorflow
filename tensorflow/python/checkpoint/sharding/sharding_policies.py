    ...
    class MaxShardSizePartitioner:
        ...
        def _get_next_partition(self) -> tuple[int, float]:
            ...
            for axis in range(1, rank):
                ...
                if (
                    parts_along_axis < min_parts
                    and part_size <= self._shard_size_remaining
                ):
        ...
        def _add_partition(self, part_axis: int, part_size: float):
            ...
            if slice_size_in_bytes > self.max_shard_size:
                logging.warning(
                    "Tensor %s's minimum slice %s has size %s bytes and "
                ...
                (
                    self._tensors_by_shard[-1].setdefault(self._checkpoint_key, {})[
                        slice_spec
                    ]
                ) = tensor_slice
            ...
            self._working_tensor_shape = tensor_shape.TensorShape(
                list(map(operator.sub, self._root_shape, relative_offset))
            )
        ...
        def get_shards(
            ...
            for shardable_tensor in shardable_tensors:
                ...
                if self._slice_spec:
                    ...
                    self._root_shape = tensor_shape.TensorShape(
                        save_slice_info.var_shape
                    )
                ...
                if self._checkpoint_key == base.OBJECT_GRAPH_PROTO_KEY:
                    ...
                    if not string_size_warning_printed:
                        logging.warning(
                            "The checkpoint sharding policy is being executed "
                    ...
                    with ops.device(self._device):
                        ...
                        if self._root_shape.rank is None or self._root_shape.rank == 0:
                            sizes = [
                                string_ops.string_length(self._root_tensor, unit="BYTE")
                            ]
                        else:
                            sizes = [
                                string_ops.string_length(elem, unit="BYTE")
                ...
                if self._root_shape.rank is None or self._root_shape.rank == 0:
                    if total_size > self.max_shard_size:
                        ...
                        if total_size > self._shard_size_remaining:
                            ...
                            self._shard_size_remaining = self.max_shard_size
                        (
                            self._tensors_by_shard[-1].setdefault(
                                self._checkpoint_key, {}
                            )[self._slice_spec]
                        ) = self._root_tensor
                ...
                while working_tensor_size > self._shard_size_remaining:
                    ...
                    if part_size == 0:
                        ...
                        working_tensor_size = (
                            int(math.prod(self._working_tensor_shape))
                            * self._dtype_size
                        )
                ...
                if self._working_tensor_shape.num_elements() > 0:
                    ...
                    remaining_tensor_slice_spec = variables.Variable.SaveSliceInfo(
                        ...
                        full_shape=self._full_shape,
                        var_offset=self._working_tensor_offset,
                        var_shape=self._working_tensor_shape,
                    ).spec.strip()
                    (
                        self._tensors_by_shard[-1].setdefault(self._checkpoint_key, {})[
                            remaining_tensor_slice_spec
                        ]
                    ) = working_tensor
