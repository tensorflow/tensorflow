...
class ShardingUtilTest(test.TestCase):
    ...
    def test_validate_shards_correct(self):
        ...
        self.assertEqual(
            shards[0]["v0/.ATTRIBUTES/VARIABLE_VALUE"][""].numpy(), v0.numpy()
        )
        self.assertEqual(
            shards[0]["v1/.ATTRIBUTES/VARIABLE_VALUE"][""].numpy(), v1.numpy()
        )
        self.assertEqual(
            shards[0]["v2/.ATTRIBUTES/VARIABLE_VALUE"][""].numpy(), v2.numpy()
        )
    ...
    def test_validate_shards_duplicate_tensor(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "multiple tensors with the same checkpoint key and slice spec were found",
        ):
            sharding_util.validate_shards(
                shards, shardable_tensors_flat, sharding_callback.description
            )
    ...
    def test_validate_shards_added_tensor(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "a tensor not originally in the object graph"
        ):
            sharding_util.validate_shards(
                shards, shardable_tensors_flat, sharding_callback.description
            )
    ...
    def test_validate_shards_shape_change(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "a tensor was found with an altered shape"
        ):
            sharding_util.validate_shards(
                shards, shardable_tensors_flat, sharding_callback.description
            )
    ...
    def test_validate_shards_dtype_change(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "a tensor was found with an altered dtype"
        ):
            sharding_util.validate_shards(
                shards, shardable_tensors_flat, sharding_callback.description
            )
    ...
    def test_validate_shards_task_change(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "a tensor was found with an altered task"
        ):
            sharding_util.validate_shards(
                shards, shardable_tensors_flat, sharding_callback.description
            )
    ...
    def test_validate_shards_different_tasks(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "tensors with different tasks were found"
        ):
            sharding_util.validate_shards(
                shards, shardable_tensors_flat, sharding_callback.description
            )
    ...
    def test_validate_shards_tensor_removal(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "tensors in the object graph were not found"
        ):
            sharding_util.validate_shards(
                shards, shardable_tensors_flat, sharding_callback.description
            )
