...
class ObjectGraphView(trackable_view.TrackableView):
    ...
    def list_children(self, obj, save_type=base.SaveType.CHECKPOINT, **kwargs):
        ...
        children = []
        for name, ref in (
            super(ObjectGraphView, self).children(obj, save_type, **kwargs).items()
        ):
    ...
    def frozen_saveable_objects(
        self, object_map=None, to_graph=None, call_with_mapped_captures=None
    ):
        """Creates SaveableObjects with the current object graph frozen."""
        return save_util_v1.frozen_saveables_and_savers(
            self, object_map, to_graph, call_with_mapped_captures
        )[0]
