...
class CheckpointPosition(object):
    ...
    def _create_serialize_to_tensor_saveable(self, saveable_factories):
        ...
        saveable_name = (
            _extract_saveable_name(self.object_proto.attributes[0].checkpoint_key)
            + suffix
        )
    ...
    def _create_saveables_by_attribute_name(self, saveable_factories):
        ...
        for serialized_tensor in self.object_proto.attributes:
            ...
            if any(
                serialized_tensor.name.startswith(name) for name in created_compat_names
            ):
            ...
            if saveables_cache is None:
                ...
                # building.
                saveable_list = saveables_cache.get(self.trackable, {}).get(
            ...
            if saveable is None:
                ...
                saveable = _get_saveable_from_factory(
                    saveable_factories, serialized_tensor, created_compat_names
                )
                if saveable is None:
    ...
    def restore_ops(self, reader=None):
        ...
        if self._has_registered_saver():
            raise ValueError(
                "Unable to run individual checkpoint restore for objects"
                " with registered savers."
            )
        (restore_ops, tensor_saveables, python_positions, _) = (
...
def restore_nodes(save_path, nodes_to_restore):
    ...
    for node_id, trackable in nodes_to_restore.items():
        ...
        if (
            node_id not in ckpt_view_descendants
            or ckpt_view._object_graph_proto.nodes[  # pylint: disable=protected-access
                node_id
            ]
            is None
        ):
    ...
    for node_id, current_trackable in nodes_to_restore.items():
        ...
        trackable_has_serialize_to_tensor = (
            saveable_object_util.trackable_has_serialize_to_tensor(current_trackable)
        )
        if not trackable_has_serialize_to_tensor:
            ...
            if len(node.attributes) != len(saveable_objects):
                raise ValueError(
                    "Size for saveable_objects for Trackable: "
        ...
        serialized_tensors[current_trackable] = (
            current_trackable._serialize_to_tensors()
        )  # pylint: disable=protected-access
        ...
        if len(node.attributes) != len(serialized_tensors[current_trackable]):
            raise ValueError(
                "Size for serialized_tensors for Trackable: "
        ...
        if not trackable_has_serialize_to_tensor:
            ...
            for attribute in node.attributes:
                ...
                serialized_tensors_renamed[current_trackable][checkpoint_key] = (
                    serialized_tensors[current_trackable][name]
                )
...
def _queue_children_for_restoration(checkpoint_position, visit_queue):
    ...
    for child in checkpoint_position.object_proto.children:
        ...
        local_object = trackable._lookup_dependency(
            child.local_name, trackable_children
        )
...
def _get_saveable_from_factory(
    saveable_factories, serialized_tensor, created_compat_names
):
        ...
        for factory_name, factory in saveable_factories.items():
            if expected_factory_name.startswith(factory_name):
                if matched_factory is not None:
                    ...
                    raise ValueError(
                        "Forward compatibility load error: Unable to load "
                ...
                factory_input_name = (
                    _extract_saveable_name(serialized_tensor.checkpoint_key)
                    + factory_name
                )
