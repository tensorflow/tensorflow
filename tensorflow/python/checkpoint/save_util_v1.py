...
def _add_attributes_to_object_graph(
    ...
    named_saveable_objects, feed_additions = generate_saveable_objects(
    ...
    for trackable, factory_data_list in checkpoint_factory_map.items():
        ...
        for factory_data in factory_data_list:
            ...
            if isinstance(saveables[0], saveable_object_util.TrackableSaveable) and (
                saveable_compat.force_checkpoint_conversion_enabled()
                or saveable_compat.get_saveable_name(object_to_save) is None
            ):
                for local_name, local_key in saveables[
                    0
                ].get_proto_names_and_checkpoint_keys():
...
def serialize_gathered_objects(
    graph_view, object_map=None, call_with_mapped_captures=None, saveables_cache=None
):
...
def frozen_saveables_and_savers(
    graph_view,
    object_map=None,
    to_graph=None,
    call_with_mapped_captures=None,
    ...
    with target_context():
        named_saveable_objects, graph_proto, _, registered_savers = (
            serialize_gathered_objects(
                graph_view, object_map, call_with_mapped_captures, saveables_cache
            )
        )
