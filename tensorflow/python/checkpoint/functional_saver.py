    ...
    for checkpoint_key, tensor_slices in shard.items():
        for slice_spec, tensor in tensor_slices.items():
            ...
            if tensor is not None:
                ...
                # wrapped properties.
                name = (
                    tensor._wrapped_name  # pylint: disable=protected-access
                    if hasattr(tensor, "_wrapped_name")
                    else checkpoint_key
                )
                spec = (
                    tensor._wrapped_slice_spec  # pylint: disable=protected-access
...
def _single_shard_restore(
    ...
    for shardable_tensor in shardable_tensors:
        ...
        (
            restored_tensor_dict.setdefault(shardable_tensor.checkpoint_key, {})[
...
def sharded_filename(
    filename_tensor: tensor_lib.Tensor, shard: int, num_shards: tensor_lib.Tensor
) -> tensor_lib.Tensor:
...
def registered_saver_filename(
    filename_tensor: tensor_lib.Tensor, saver_name: registration.RegisteredSaver
) -> tensor_lib.Tensor:
...
def _get_mapped_registered_restore_fn(
    ...
    if call_with_mapped_captures is None:
        ...
        def restore_fn_with_replaced_captures(
            merged_prefix: tensor_lib.Tensor,
        ) -> tensor_lib.Tensor:
...
class MultiDeviceSaver:
    ...
    def __init__(
        ...
        self._shardable_tensors_by_task: MutableMapping[
            device_lib.DeviceSpec, MutableSequence[sharding_util.ShardableTensor]
        ] = {}
    ...
    def _get_shards_by_task(
        self, sharding_callback: sharding_util.ShardingCallback
        ...
        def wrap_tensor(shardable_tensor):
            ...
            return dataclasses.replace(
                shardable_tensor, tensor=tensor_val, shape=tensor_shape
            )
        ...
        metrics.AddShardingCallbackDuration(
            callback_duration=max(1, callback_duration)
        )  # in microseconds
        ...
        def save_fn() -> ops.Operation:
                ...
                for shard in shards:
                    with ops.device(task):
                        shard_prefix = sharded_filename(
                            tmp_checkpoint_prefix, shard_idx, num_shards_tensor
                        )
                        shard_idx += 1
            ...
            with ops.control_dependencies(sharded_saves):
                ...
                merge_device_spec = (
                    ...
                    or saveable_object_util.set_cpu0(tensor_device_spec.to_string())
                )
    ...
    def restore(
                ...
                with ops.device(task):
                    ...
                    for ckpt_key, slice_and_tensor in restored_tensor_dict.items():
                        for slice_spec, tensor in slice_and_tensor.items():
                            restore_fn = self._keys_to_restore_fn[
                                (ckpt_key, slice_spec)
                            ]
                            ...
                            if slice_spec:
                                (
                                    restore_fn_inputs.setdefault(
                                        restore_fn, {}
                                    ).setdefault(ckpt_key, {})[slice_spec]
                                ) = tensor
                            else:
                            ...
                            if restore_fn_input_count[restore_fn] == 0:
                                ...
                                for ckpt_key, tensor in restore_fn_inputs[
                                    restore_fn
                                ].items():
                                    restored_tensors[
                                        trackable_utils.extract_local_name(ckpt_key)
                                    ] = tensor
        ...
        if context.executing_eagerly() and (
            self._num_unique_tasks > 1 or has_custom_device_saver
        ):
