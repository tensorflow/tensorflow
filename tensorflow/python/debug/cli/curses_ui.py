...
class ScrollBar(object):
    ...
    def __init__(self, min_x, min_y, max_x, max_y, scroll_position, output_num_rows):
        ...
        if self._max_x < self._min_x:
            raise ValueError(
                "Insufficient width for ScrollBar (%d)"
                % (self._max_x - self._min_x + 1)
            )
        if self._max_y < self._min_y + 3:
            raise ValueError(
                "Insufficient height for ScrollBar (%d)"
    ...
    def _block_y(self, screen_coord_sys=False):
        ...
        rel_block_y = (
            int(
                float(self._scroll_position)
                / (self._output_num_rows - 1)
                * (self._scroll_bar_height - 3)
            )
            + 1
        )
    ...
    def get_click_command(self, mouse_y):
        ...
        elif mouse_y > self._block_y(screen_coord_sys=True) and mouse_y < self._max_y:
            return _SCROLL_DOWN
        elif mouse_y < self._block_y(screen_coord_sys=True) and mouse_y > self._min_y:
            return _SCROLL_UP
        else:
            return None
...
class CursesUI(base_ui.BaseUI):
    ...
    _ARRAY_INDICES_COLOR_PAIR = cli_shared.COLOR_BLACK + "_on_" + cli_shared.COLOR_WHITE
    _ERROR_TOAST_COLOR_PAIR = cli_shared.COLOR_RED + "_on_" + cli_shared.COLOR_WHITE
    _INFO_TOAST_COLOR_PAIR = cli_shared.COLOR_BLUE + "_on_" + cli_shared.COLOR_WHITE
    _STATUS_BAR_COLOR_PAIR = cli_shared.COLOR_BLACK + "_on_" + cli_shared.COLOR_WHITE
    _UI_WAIT_COLOR_PAIR = cli_shared.COLOR_MAGENTA + "_on_" + cli_shared.COLOR_WHITE
    ...
    def _init_layout(self):
        ...
        self.rectangle = collections.namedtuple("rectangle", "top left bottom right")
    ...
    def _screen_color_init(self):
        ...
        for fg_color in self._FOREGROUND_COLORS:
            for bg_color in self._BACKGROUND_COLORS:
                ...
                curses.init_pair(
                    color_index,
                    self._FOREGROUND_COLORS[fg_color],
        ...
        try:
            ...
            self._color_pairs[cli_shared.COLOR_GRAY] = self._color_pairs[
                cli_shared.COLOR_GREEN
            ]
    ...
    def _screen_launch(self, enable_mouse_on_start):
        ...
        self.config.set_callback(
            "mouse_mode", lambda cfg: self._set_mouse_enabled(cfg.get("mouse_mode"))
        )
    ...
    def _screen_create_command_window(self):
        ...
        self._command_window = curses.newwin(
            self._command_textbox_height,
            self._max_x - len(self.CLI_PROMPT),
    ...
    def _screen_create_command_textbox(self, existing_command=None):
        ...
        # Command text box.
        self._command_textbox = textpad.Textbox(self._command_window, insert_mode=True)
    ...
    def _dispatch_command(self, command):
        ...
        if command in self.CLI_EXIT_COMMANDS:
            ...
            command == self._NAVIGATION_FORWARD_COMMAND
            or command == self._NAVIGATION_BACK_COMMAND
        ):
        ...
        if command.startswith(self.REGEX_SEARCH_PREFIX) and self._curr_unwrapped_output:
    ...
    def _on_textbox_keypress(self, x):
        ...
        if not isinstance(x, int):
            raise TypeError(
                "Key validator expected type int, received type %s" % type(x)
            )
    ...
    def _screen_draw_text_line(self, row, line, attr=curses.A_NORMAL, color=None):
        ...
        color_pair = (
            self._default_color_pair if color is None else self._color_pairs[color]
        )
    ...
    def _screen_display_output(self, output):
        ...
        (
            ...
            self._output_pad_width,
        ) = self._display_lines(self._curr_wrapped_output, self._output_num_rows)

        # The indices of lines with regex matches (if any) need to be mapped to
        # indices of wrapped lines.
        return [
            wrapped_line_indices[line] for line in self._unwrapped_regex_match_lines
        ]
    ...
    def _display_output(self, output, is_refresh=False, highlight_regex=None):
        ...
        if highlight_regex:
            ...
            if next_match_line >= 0:
                self._scroll_output(_SCROLL_TO_LINE_INDEX, line_index=next_match_line)
            else:
    ...
    def _display_nav_bar(self):
        ...
        self._screen_add_line_to_output_pad(
            self._nav_bar_pad,
            0,
            self._nav_bar.lines[0][: nav_bar_width - 1],
            color_segments=(
                self._nav_bar.font_attr_segs[0]
                if 0 in self._nav_bar.font_attr_segs
                else None
            ),
        )
    ...
    def _display_main_menu(self, output):
        ...
        if debugger_cli_common.MAIN_MENU_KEY in output.annotations:
            self._main_menu = output.annotations[
                debugger_cli_common.MAIN_MENU_KEY
            ...
            self._screen_add_line_to_output_pad(
                ...
                color_segments=(
                    wrapped_menu.font_attr_segs[0]
                    if 0 in wrapped_menu.font_attr_segs
                    else None
                ),
            )
        else:
    ...
    def _screen_add_line_to_output_pad(self, pad, row, txt, color_segments=None):
        ...
        for (curr_start, curr_end, curr_attrs), (next_start, _, _) in zip(
            ...
            for attr in curr_attrs:
                if self._mouse_enabled and isinstance(
                    attr, debugger_cli_common.MenuItem
                ):
    ...
    def _scroll_output(self, direction, line_index=None):
        ...
        if direction == _SCROLL_REFRESH:
            ...
            if not isinstance(line_index, int):
                raise TypeError(
                    "Invalid line_index type (%s) under mode %s"
    ...
    def _show_array_indices(self):
        ...
        if self._main_menu_pad:
            output_top += 1
        bottom_line_index = self._output_pad_screen_location.bottom - output_top - 1
    ...
    def _show_array_index_at_line(self, line_index):
        ...
        if (
            pointer in self._curr_wrapped_output.annotations
            and "i0" in self._curr_wrapped_output.annotations[pointer]
        ):
    ...
    def _toast(self, message, color=None, line_index=None):
        ...
        self._screen_scroll_output_pad(pad, 0, 0, line_index, 0, line_index, right_end)
