...
class RichTextLines:
    ...
    def extend(self, other):
        ...
        for line_index in other.font_attr_segs:
            self._font_attr_segs[orig_num_lines + line_index] = other.font_attr_segs[
                line_index
            ]
    ...
    def _extend_before(self, other):
        ...
        for line_index in self.font_attr_segs:
            new_font_attr_segs[other_num_lines + line_index] = self.font_attr_segs[
                line_index
            ]
...
class CommandHandlerRegistry:
    ...
    def dispatch_command(self, prefix, argv, screen_info=None):
        ...
        if not resolved_prefix:
            raise ValueError(
                'No handler is registered for command prefix "%s"' % prefix
            )
        ...
        try:
            ...
            lines = [
                "Syntax error for command: %s" % prefix,
    ...
    def _get_help_for_command_prefix(self, cmd_prefix):
        ...
        if resolved_prefix in self._prefix_to_aliases:
            lines.append(
                HELP_INDENT
                + "Aliases: "
                + ", ".join(self._prefix_to_aliases[resolved_prefix])
            )
...
class TabCompletionRegistry:
    ...
    def register_tab_comp_context(self, context_words, comp_items):
        ...
        if not isinstance(context_words, list):
            raise TypeError(
                "Incorrect type in context_list: Expected list, got %s"
                % type(context_words)
            )

        if not isinstance(comp_items, list):
            raise TypeError(
                "Incorrect type in comp_items: Expected list, got %s" % type(comp_items)
            )
    ...
    def deregister_context(self, context_words):
        ...
        for context_word in context_words:
            if context_word not in self._comp_dict:
                raise KeyError(
                    'Cannot deregister unregistered context word "%s"' % context_word
                )
...
class Menu:
    ...
    def format_as_single_line(
        ...
        if disabled_item_attrs is not None and not isinstance(
            disabled_item_attrs, list
        ):
