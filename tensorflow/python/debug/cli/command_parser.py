    ...
    for slice_string in slicing_string[1:-1].split(","):
        ...
        if len(indices) == 1:
            ...
            parsed.append(
                slice(
                    *[
                        int(index.strip()) if index.strip() else None
...
def parse_ranges(range_string):
    ...
    for item in ranges:
        if len(item) != 2:
            ...
            raise ValueError(
                "Incorrect type in the 1st element of range: %s" % type(item[0])
            )
        elif not isinstance(item[1], (int, float)):
            raise ValueError(
                "Incorrect type in the 2nd element of range: %s" % type(item[0])
            )
...
def parse_memory_interval(interval_str):
    ...
    return Interval(
        interval_start,
        str_interval.start_included,
        interval_end,
        str_interval.end_included,
    )
...
def parse_time_interval(interval_str):
    ...
    if interval_start > interval_end:
        raise ValueError(
            "Invalid interval %s. Start must be before end of interval." % interval_str
        )
    return Interval(
        interval_start,
        str_interval.start_included,
...
def parse_readable_size_str(size_str):
    ...
    if size_str.isdigit():
        ...
        raise ValueError(
            'Failed to parsed human-readable byte size str: "%s"' % size_str
        )
...
def get_print_tensor_argparser(description):
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Path of the numpy file to write the tensor data to, using numpy.save().",
    )
    return ap
