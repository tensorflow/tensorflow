...
class MockCursesUI(curses_ui.CursesUI):
    ...
    def _screen_getmouse(self):
        output = (
            0,
            self._mouse_xy_sequence[self._mouse_counter][0],
            self._mouse_xy_sequence[self._mouse_counter][1],
            0,
    ...
    def _screen_gather_textbox_str(self):
        return codes_to_string(
            self._command_sequence[self._command_counter][: self._command_key_counter]
        )
...
class CursesTest(test_util.TensorFlowTestCase):
    ...
    def _babble(self, args, screen_info=None):
        ...
        if parsed.menu:
            ...
            menu.append(debugger_cli_common.MenuItem("babble again", "babble"))
            menu.append(debugger_cli_common.MenuItem("ahoy", "ahoy", enabled=False))
    ...
    def testRunUIInvalidCommandPrefix(self):
        ...
        ui = MockCursesUI(
            40, 80, command_sequence=[string_to_codes("foo\n"), self._EXIT]
        )
        ui.run_ui()
        ...
        self.assertEqual(
            ['ERROR: Invalid command prefix "foo"'], ui.unwrapped_outputs[0].lines
        )
        # TODO(cais): Add explanation for the 35 extra lines.
        self.assertEqual(
            ['ERROR: Invalid command prefix "foo"'], ui.wrapped_outputs[0].lines[:1]
        )
    ...
    def testRunUIInvalidCommandSyntax(self):
        ...
        ui = MockCursesUI(
            40, 80, command_sequence=[string_to_codes("babble -z\n"), self._EXIT]
        )
    ...
    def testRunUIScrollTallOutputPageDownUp(self):
        ...
        ui = MockCursesUI(
            ...
            command_sequence=[
                ...
                [curses.KEY_NPAGE] * 2 + [curses.KEY_PPAGE] + self._EXIT,
            ],
        )
    ...
    def testCutOffTooManyOutputLines(self):
        ui = MockCursesUI(
            40, 80, command_sequence=[string_to_codes("babble -n 20\n"), self._EXIT]
        )
        ...
        self.assertEqual(
            ["bar"] * 10 + ["Output cut off at 10 lines!"],
    ...
    def testCompileHelpWithoutHelpIntro(self):
        ui = MockCursesUI(
            40, 80, command_sequence=[string_to_codes("help\n"), self._EXIT]
        )
    ...
    def testTabCompletionEmptyTriggerWithoutCommonPrefix(self):
        ui = MockCursesUI(
            ...
            80,
            command_sequence=[
                string_to_codes("\t"),  # Trigger tab completion.
                string_to_codes("\n"),
                self._EXIT,
            ],
        )
    ...
    def testTabCompletionNonemptyTriggerSingleCandidate(self):
        ui = MockCursesUI(
            ...
            80,
            command_sequence=[
                string_to_codes("b\t"),  # Trigger tab completion.
                string_to_codes("\n"),
                self._EXIT,
            ],
        )
    ...
    def testTabCompletionNoMatch(self):
        ui = MockCursesUI(
            ...
            80,
            command_sequence=[
                string_to_codes("c\t"),  # Trigger tab completion.
                string_to_codes("\n"),
                self._EXIT,
            ],
        )
        ...
        self.assertEqual(
            ['ERROR: Invalid command prefix "c"'], ui.unwrapped_outputs[0].lines
        )
        self.assertEqual(
            ['ERROR: Invalid command prefix "c"'], ui.wrapped_outputs[0].lines[:1]
        )
    ...
    def testTabCompletionTwice(self):
        ...
        self.assertEqual([["10", "120", "123"], ["120", "123"]], ui.candidates_lists)
    ...
    def testRegexSearch(self):
        ...
        for i in range(3):
            self.assertEqual(
                [(0, 1, "black_on_white"), (2, 3, "black_on_white")],
        ...
        for i in range(3):
            self.assertEqual(
                [(1, 2, "black_on_white")], ui.wrapped_outputs[2].font_attr_segs[i]
            )
    ...
    def testRegexSearchUnderLineWrapping(self):
        ...
        ui.register_command_handler("babble", self._babble, "babble some")
        ui.run_ui()

        self.assertEqual(4, len(ui.wrapped_outputs))
        for wrapped_output in ui.wrapped_outputs:
            self.assertEqual(
                ["foo-", "bar-", "baz-", "qux"] * 3, wrapped_output.lines[0:12]
            )
    ...
    def testDisplayTensorWithIndices(self):
        ...
        ui.register_command_handler(
            "print_ones", self._print_ones, "print an all-one matrix of specified size"
        )
        ui.run_ui()
        ...
        self.assertEqual({0: None, -1: [1, 0]}, ui.output_array_pointer_indices[0])
        self.assertIn(" Scroll (PgDn): 0.00% -[1,0] ", ui.scroll_messages[0])

        # Scrolled down one line.
        self.assertEqual({0: None, -1: [2, 0]}, ui.output_array_pointer_indices[1])
        self.assertIn(" Scroll (PgDn/PgUp): 16.67% -[2,0] ", ui.scroll_messages[1])

        # Scrolled down one line.
        self.assertEqual({0: [0, 0], -1: [3, 0]}, ui.output_array_pointer_indices[2])
        self.assertIn(" Scroll (PgDn/PgUp): 33.33% [0,0]-[3,0] ", ui.scroll_messages[2])

        # Scrolled down one line.
        self.assertEqual({0: [1, 0], -1: [4, 0]}, ui.output_array_pointer_indices[3])
        self.assertIn(" Scroll (PgDn/PgUp): 50.00% [1,0]-[4,0] ", ui.scroll_messages[3])

        # Scroll to the bottom.
        self.assertEqual({0: [4, 0], -1: None}, ui.output_array_pointer_indices[4])
        self.assertIn(" Scroll (PgUp): 100.00% [4,0]- ", ui.scroll_messages[4])

        # Attempt to scroll beyond the bottom should lead to no change.
        self.assertEqual({0: [4, 0], -1: None}, ui.output_array_pointer_indices[5])
        self.assertIn(" Scroll (PgUp): 100.00% [4,0]- ", ui.scroll_messages[5])

        # Scrolled up one line.
        self.assertEqual({0: [3, 0], -1: None}, ui.output_array_pointer_indices[6])
        self.assertIn(" Scroll (PgDn/PgUp): 83.33% [3,0]- ", ui.scroll_messages[6])

        # Scrolled up one line.
        self.assertEqual({0: [2, 0], -1: None}, ui.output_array_pointer_indices[7])
        self.assertIn(" Scroll (PgDn/PgUp): 66.67% [2,0]- ", ui.scroll_messages[7])

        # Scrolled up one line.
        self.assertEqual({0: [1, 0], -1: [4, 0]}, ui.output_array_pointer_indices[8])
        self.assertIn(" Scroll (PgDn/PgUp): 50.00% [1,0]-[4,0] ", ui.scroll_messages[8])

        # Scroll to the top.
        self.assertEqual({0: None, -1: [1, 0]}, ui.output_array_pointer_indices[9])
        self.assertIn(" Scroll (PgDn): 0.00% -[1,0] ", ui.scroll_messages[9])

        # Attempt to scroll pass the top limit should lead to no change.
        self.assertEqual({0: None, -1: [1, 0]}, ui.output_array_pointer_indices[10])
    ...
    def testScrollTensorByValidIndices(self):
        ...
        self.assertEqual({0: None, -1: [0, 0]}, ui.output_array_pointer_indices[0])
        self.assertEqual({0: [0, 0], -1: [2, 0]}, ui.output_array_pointer_indices[1])
        self.assertEqual({0: [1, 0], -1: [3, 0]}, ui.output_array_pointer_indices[2])
        self.assertEqual({0: [0, 0], -1: [2, 0]}, ui.output_array_pointer_indices[3])
    ...
    def testScrollTensorByInvalidIndices(self):
        ...
        self.assertEqual(
            "ERROR: invalid literal for int() with base 10: ''", ui.toasts[4]
        )
    ...
    def testIncompleteRedirectErrors(self):
        ui = MockCursesUI(
            40, 80, command_sequence=[string_to_codes("babble -n 2 >\n"), self._EXIT]
        )
...
class ScrollBarTest(test_util.TensorFlowTestCase):
    ...
    def testClickCommandsAreCorrectForMiddlePosition(self):
        ...
        self.assertEqual(curses_ui._SCROLL_UP_A_LINE, scroll_bar.get_click_command(0))
        self.assertEqual(curses_ui._SCROLL_UP, scroll_bar.get_click_command(1))
        self.assertEqual(curses_ui._SCROLL_UP, scroll_bar.get_click_command(2))
        self.assertIsNone(scroll_bar.get_click_command(3))
        self.assertEqual(curses_ui._SCROLL_DOWN, scroll_bar.get_click_command(5))
        self.assertEqual(curses_ui._SCROLL_DOWN, scroll_bar.get_click_command(6))
        self.assertEqual(curses_ui._SCROLL_DOWN_A_LINE, scroll_bar.get_click_command(7))
        self.assertIsNone(scroll_bar.get_click_command(8))
    ...
    def testClickCommandsAreCorrectForBottomPosition(self):
        ...
        self.assertEqual(curses_ui._SCROLL_UP_A_LINE, scroll_bar.get_click_command(0))
        for i in range(1, 6):
            self.assertEqual(curses_ui._SCROLL_UP, scroll_bar.get_click_command(i))
    ...
    def testClickCommandsAreCorrectForScrollBarNotAtZeroMinY(self):
        ...
        self.assertEqual(curses_ui._SCROLL_UP_A_LINE, scroll_bar.get_click_command(5))
        self.assertEqual(curses_ui._SCROLL_UP, scroll_bar.get_click_command(6))
        self.assertEqual(curses_ui._SCROLL_UP, scroll_bar.get_click_command(7))
        self.assertIsNone(scroll_bar.get_click_command(8))
        self.assertEqual(curses_ui._SCROLL_DOWN, scroll_bar.get_click_command(10))
        self.assertEqual(curses_ui._SCROLL_DOWN, scroll_bar.get_click_command(11))
        self.assertEqual(
            curses_ui._SCROLL_DOWN_A_LINE, scroll_bar.get_click_command(12)
        )
