...
def _matmul_op_name():
    if (
        ...
        and _get_graph_matmul_dtype() in _mkl_matmul_supported_types()
    ):
        return "_MklMatMul"
    else:
        return "MatMul"
...
def assert_column_header_command_shortcut(
    tst, command, reverse, node_name_regex, op_type_regex, tensor_filter_name
):
...
def assert_listed_tensors(
    ...
    if node_name_regex is not None:
        tst.assertEqual(
            'Node name regex filter: "%s"' % node_name_regex, next(line_iter)
        )
        line_counter += 1
...
def assert_node_attribute_lines(
    tst,
    out,
    node_name,
    op_type,
    ...
    tst.assertEqual("", next(line_iter))
    tst.assertEqual(
        "  %d input(s) + %d control input(s):"
        % (len(input_op_type_node_name_pairs), len(ctrl_input_op_type_node_name_pairs)),
        next(line_iter),
    )

    # Check inputs.
    tst.assertEqual(
        "    %d input(s):" % len(input_op_type_node_name_pairs), next(line_iter)
    )
    ...
    if ctrl_input_op_type_node_name_pairs:
        tst.assertEqual(
            "    %d control input(s):" % len(ctrl_input_op_type_node_name_pairs),
            next(line_iter),
        )
    ...
    tst.assertEqual(
        "  %d recipient(s) + %d control recipient(s):"
        % (
    ...
    tst.assertEqual(
        "    %d recipient(s):" % len(recipient_op_type_node_name_pairs), next(line_iter)
    )
    ...
    if show_stack_trace:
        ...
        if stack_trace_available:
            ...
            tst.assertEqual(
                "(Unavailable because no Python graph has been loaded)", next(line_iter)
            )
...
def check_main_menu(
    ...
    tst.assertEqual(
        list_tensors_enabled, menu.caption_to_item("list_tensors").is_enabled()
    )
...
def check_menu_item(
    tst, out, line_index, expected_begin, expected_end, expected_command
):
    ...
    for begin, end, attribute in attr_segs:
        ...
        menu_item = [
            ...
            if isinstance(attribute, debugger_cli_common.MenuItem)
        ]
        if menu_item:
...
class AnalyzerCLISimpleMulAddTest(test_util.TensorFlowTestCase):
    ...
    def testListTensorsWithInvalidSortByFieldGivesError(self):
        ...
        self.assertIn(
            "ValueError: Unsupported key to sort tensors by: foobar", out.lines
        )
    ...
    def testListTensorsFilterByNodeNameRegex(self):
        out = self._registry.dispatch_command(
            "list_tensors", ["--node_name_filter", ".*read.*"]
        )
        assert_listed_tensors(
            self,
            out,
    ...
    def testListTensorFilterByOpTypeRegex(self):
        out = self._registry.dispatch_command(
            "list_tensors", ["--op_type_filter", "Identity"]
        )
        assert_listed_tensors(
            self,
            out,
    ...
    def testListTensorNonexistentFilter(self):
        ...
        self.assertEqual(
            ['ERROR: There is no tensor filter named "foo_filter".'], out.lines
        )
        check_main_menu(self, out, list_tensors_enabled=False)
    ...
    def testNodeInfoNonexistentNodeName(self):
        ...
        self.assertEqual(
            ['ERROR: There is no node named "bar" in the partition graphs'], out.lines
        )
    ...
    def testPrintTensorAndWriteToNpyFile(self):
        ...
        out = self._registry.dispatch_command(
            "print_tensor", [tensor_name, "-w", npy_path], screen_info={"cols": 80}
        )
    ...
    def testPrintTensorHighlightingRanges(self):
        ...
        out = self._registry.dispatch_command(
            "print_tensor",
            [tensor_name, "--ranges", "[[-inf, -5.5], [5.5, inf]]"],
            screen_info={"cols": 80},
        )

        self.assertEqual(
            [
                'Tensor "%s:DebugIdentity": '
                % tensor_name
    ...
    def testPrintTensorWithSlicing(self):
        ...
        self.assertEqual(
            [
                'Tensor "%s:DebugIdentity[1, :]":' % tensor_name,
                "  dtype: float64",
                "  shape: (1,)",
                "",
                "array([-2.])",
    ...
    def testPrintTensorInvalidSlicingString(self):
        ...
        self.assertEqual(
            "Error occurred during handling of command: print_tensor "
            + tensor_name
            + "[1, foo()]:",
            out.lines[0],
        )
    ...
    def testPrintTensorMissingOutputSlotLeadsToOnlyDumpedTensorPrinted(self):
        ...
        self.assertEqual(
            [
                'Tensor "%s:0:DebugIdentity":' % node_name,
                "  dtype: float64",
                "  shape: (2, 1)",
                "",
                "array([[ 7.],",
    ...
    def testEvalExpression(self):
        ...
        out = self._registry.dispatch_command(
            "eval",
            ["np.matmul(`%s`, `%s`.T)" % (tensor_name, tensor_name)],
            screen_info={"cols": 80},
        )
    ...
    def testEvalExpressionAndWriteToNpyFile(self):
        ...
        out = self._registry.dispatch_command(
            "eval",
            ["np.matmul(`%s`, `%s`.T)" % (tensor_name, tensor_name), "-w", npy_path],
            screen_info={"cols": 80},
        )
    ...
    def testAddGetTensorFilterLambda(self):
        analyzer = analyzer_cli.DebugAnalyzer(
            self._debug_dump, _cli_config_from_temp_file()
        )
    ...
    def testAddTensorFilterNonStrName(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "Input argument filter_name is expected to be str, but is not"
        ):
    ...
    def testAddGetTensorFilterNonCallable(self):
        ...
        with self.assertRaisesRegex(
            TypeError,
            "Input argument filter_callable is expected to be callable, but is not.",
        ):
    ...
    def testPrintSourceForOpNamesWholeFileWorks(self):
        ...
        self.assertEqual(
            "pt simple_mul_add/u", out.font_attr_segs[index + 1][0][2].content
        )
        ...
        self.assertEqual(
            "pt simple_mul_add/u/read", out.font_attr_segs[index + 3][0][2].content
        )
        ...
        self.assertEqual(
            "pt simple_mul_add/v", out.font_attr_segs[index + 1][0][2].content
        )
        ...
        self.assertEqual(
            "pt simple_mul_add/matmul", out.font_attr_segs[index + 1][0][2].content
        )
        ...
        self.assertEqual(
            "pt simple_mul_add/add", out.font_attr_segs[index + 1][0][2].content
        )
    ...
    def testPrintSourceForTensorNamesWholeFileWorks(self):
        ...
        self.assertEqual(
            "pt simple_mul_add/u/read:0", out.font_attr_segs[index + 1][0][2].content
        )
        self.assertEqual(
            "pt simple_mul_add/u:0", out.font_attr_segs[index + 2][0][2].content
        )

    def testPrintSourceForOpNamesStartingAtSpecifiedLineWorks(self):
        self._debug_dump.set_python_graph(self._sess.graph)
        out = self._registry.dispatch_command(
            "print_source", [self._curr_file_path, "-b", "3"], screen_info={"cols": 80}
        )
    ...
    def testPrintSourceForOpNameSettingMaximumElementCountWorks(self):
        self._debug_dump.set_python_graph(self._sess.graph)
        out = self._registry.dispatch_command(
            "print_source", [self._curr_file_path, "-m", "1"], screen_info={"cols": 80}
        )
    ...
    def testListSourceWorks(self):
        ...
        non_tf_lib_files = [
            line.split(" ")[0]
            for line in out.lines[non_tf_lib_files_start:non_tf_lib_files_end]
        ]
        ...
        for i in range(non_tf_lib_files_end + 1, len(out.lines)):
            ...
            for attr_seg in out.font_attr_segs[i]:
                self.assertTrue(
                    cli_shared.COLOR_GRAY in attr_seg[2]
                    or attr_seg[2] == cli_shared.COLOR_GRAY
                )
    ...
    def testListSourceWithCompiledPythonSourceWorks(self):
        def fake_list_source_files_against_dump(
            dump, path_regex_allowlist=None, node_name_regex_allowlist=None
        ):
            del dump, path_regex_allowlist, node_name_regex_allowlist
            return [
                ("compiled_1.pyc", False, 10, 20, 30, 4),
        ...
        with test.mock.patch.object(
            ...
            self.assertEqual(
                (0, 14, [cli_shared.COLOR_WHITE]), out.font_attr_segs[4][0]
            )
            self.assertStartsWith(out.lines[5], "compiled_2.pyo")
            self.assertEqual(
                (0, 14, [cli_shared.COLOR_WHITE]), out.font_attr_segs[5][0]
            )
            ...
            self.assertEqual(
                "ps uncompiled.py -b 6", out.font_attr_segs[6][0][2][1].content
            )
        ...
        with session.Session(config=no_rewrite_session_config()) as sess:
            with ops.device("CPU:0"):
                ...
                self.assertAllEqual(
                    [0, 2, 2, 4],
                    sess.run(
                        idx_times_two, options=run_options, run_metadata=run_metadata
                    ),
                )
...
class AnalyzerCLIPrintLargeTensorTest(test_util.TensorFlowTestCase):
    ...
    def testPrintLargeTensorWithoutAllOption(self):
        ...
        out = self._registry.dispatch_command(
            "print_tensor", ["large_tensors/x:0[:, 0:7]"], screen_info={"cols": 80}
        )
    ...
    def testPrintLargeTensorWithAllOption(self):
        out = self._registry.dispatch_command(
            "print_tensor", ["large_tensors/x:0", "-a"], screen_info={"cols": 80}
        )
...
class AnalyzerCLIControlDepTest(test_util.TensorFlowTestCase):
    ...
    def testListInputsNonRecursiveNoControl(self):
        ...
        check_menu_item(
            ...
            len(out.lines[3]),
            "li -c -r control_deps/ctrl_dep_y",
        )
    ...
    def testListInputsNonRecursiveWithControls(self):
        ...
        self.assertEqual(
            [
                ...
                "|  |- ...",
                "|- (1) (Ctrl) [VariableV2] control_deps/x",
                "",
                "Legend:",
                "  (d): recursion depth = d.",
                "  (Ctrl): Control input.",
    ...
    def testListRecipientsRecursiveWithControlsWithDepthLimit(self):
        ...
        check_menu_item(
            ...
            len(out.lines[3]) - len("control_deps/ctrl_dep_y"),
            len(out.lines[3]),
            "lo -c -r control_deps/ctrl_dep_y",
        )
        check_menu_item(
            self,
            out,
            ...
            len(out.lines[5]),
            "lo -c -r control_deps/ctrl_dep_z",
        )

        # Verify the bold attribute of the node name.
        self.assertEqual(
            [(20, 20 + len("control_deps/x"), "bold")], out.font_attr_segs[0]
        )
...
class AnalyzerCLIWhileLoopTest(test_util.TensorFlowTestCase):
    ...
    def testMultipleDumpsPrintTensorNoNumber(self):
        ...
        for i in range(10):
            ...
            self.assertTrue(
                output.lines[i + 1].endswith(" ms] while/Identity:0:DebugIdentity")
            )
    ...
    def testMultipleDumpsPrintTensorWithNumber(self):
        for i in range(5):
            ...
            self.assertEqual(
                'Tensor "while/Identity:0:DebugIdentity (dump #%d)":' % i,
                output.lines[0],
            )
