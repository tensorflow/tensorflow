...
class ReconstructNonDebugGraphTest(test_util.TensorFlowTestCase):
    ...
    def _compareOriginalAndReconstructedGraphDefs(
        self, sess, fetches, feed_dict=None, expected_output=None
    ):
        run_options = config_pb2.RunOptions(output_partition_graphs=True)
        run_metadata = config_pb2.RunMetadata()
        output = sess.run(
            fetches, feed_dict=feed_dict, options=run_options, run_metadata=run_metadata
        )
        ...
        for i, non_debug_graph_def in enumerate(non_debug_graph_defs):
            ...
            reconstructed_again = debug_graphs.reconstruct_non_debug_graph_def(
    ...
    def testReconstructGraphWithGradients(self):
        with session.Session(config=self._no_rewrite_session_config()) as sess:
            ...
            train_op = gradient_descent.GradientDescentOptimizer(
                learning_rate=0.1
            ).minimize(toy_loss, name="train_op")
            self.evaluate(u.initializer)
            self.evaluate(v.initializer)
