...
class DumpingCallbackTest(
    ...
    def testNestedFunctionExecutionWithoutControlFlow(self, tensor_debug_mode):
        ...
        with debug_events_reader.DebugDataReader(self.dump_root) as reader:
            ...
            # 1st AddV2 op.
            self.assertEqual(
                reader.graph_by_id(graph_exec_traces[0].graph_ids[-1]).name, "log_sum"
            )
            self.assertEqual(
            ...
            # Log op.
            self.assertEqual(
                reader.graph_by_id(graph_exec_traces[1].graph_ids[-1]).name, "log_sum"
            )
            self.assertEqual(
            ...
            if tensor_debug_mode == "NO_TENSOR":
                ...
                self.assertAllClose(
                    full_tensor_values[2], np.log(5.0) + 1.0
                )  # 2nd AddV2 op.
                self.assertAllClose(
                    full_tensor_values[3], np.sin(np.log(5.0) + 1.0)
                )  # Sin op.
    ...
    def testFunctionExecutionWithControlFlow(self, tensor_debug_mode):
        ...
        with debug_events_reader.DebugDataReader(self.dump_root) as reader:
            ...
            graph_exec_traces = reader.graph_execution_traces()
            executed_op_types = [
                trace.op_type for trace in graph_exec_traces if trace.op_type != "Const"
            ]
            if tensor_debug_mode != "CURT_HEALTH":
        ...
        with debug_events_reader.DebugDataReader(self.dump_root) as reader:
            ...
            for op_digest in graph_op_digests:
                if op_digest.op_type == "Placeholder":
                    ...
                    self.assertEqual(
                        op_digest.input_names[0], placeholder_op_name + ":0"
                    )
                    ...
                    self.assertEqual(
                        op_digest.input_names[1], placeholder_op_name + ":0"
                    )
