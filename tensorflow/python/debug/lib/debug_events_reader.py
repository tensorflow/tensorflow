...
class DebugEventsReader:
    ...
    def _load_metadata_files(self):
        ...
        if not metadata_paths:
            raise ValueError(
                "Cannot find any tfdbg metadata file in directory: %s" % self._dump_root
            )
        wall_times = []
        run_ids = []
        tensorflow_versions = []
    ...
    def _generic_iterator(self, file_path):
        ...
        try:
            while True:
                ...
                try:
                    record, self._reader_offsets[file_path] = reader.read(
                        current_offset
                    )
    ...
    def graph_execution_traces_iterators(self):
        return [
            self._generic_iterator(path) for path in self._graph_execution_traces_paths
        ]
...
class DebugDataReader:
        ...
        for debug_event, offset in graphs_iter:
            if debug_event.graph_op_creation.ByteSize():
                ...
                for dst_slot, input_name in enumerate(op_creation_proto.input_names):
                    ...
                    debugged_graph.add_op_consumer(
                        src_op_name, int(src_slot), op_creation_proto.op_name, dst_slot
                    )
                ...
                if graph_proto.outer_context_id:
                    self._graph_by_id[graph_proto.outer_context_id].add_inner_graph_id(
                        graph.graph_id
                    )
    ...
    def outermost_graphs(self):
        ...
        return [
            graph for graph in self._graph_by_id.values() if not graph.outer_graph_id
        ]
    ...
    def device_name_map(self):
        ...
        return {
            device_id: self._device_by_id[device_id].device_name
    ...
    def graph_op_digests(self, op_type=None):
        ...
        if op_type is not None:
            return [
                digest for digest in self._graph_op_digests if digest.op_type == op_type
            ]
        else:
    ...
    def execution_to_tensor_values(self, execution):
        ...
        return [
            _parse_tensor_value(tensor_proto)
