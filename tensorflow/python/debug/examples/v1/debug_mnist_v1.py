...
def parse_args():
    ...
    parser.add_argument(
        "--max_steps", type=int, default=10, help="Number of steps to run trainer."
    )
    parser.add_argument(
        "--train_batch_size",
        type=int,
...
def main(_):
    ...
    ds_train = (
        ds_train.shuffle(1000, seed=RAND_SEED).repeat().batch(FLAGS.train_batch_size)
    )
    ...
    if FLAGS.debug:
        config_file_path = (
            tempfile.mktemp(".tfdbg_config") if FLAGS.use_random_config_path else None
        )
