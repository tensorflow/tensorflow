...
def scalar(name, tensor, collections=None, family=None):
  ...
  eager mode, this API will check if there is a TF2 summary writer set as
  default (see `tf.summary.SummaryWriter`). If so, this API will write to that
  writer by forwarding its arguments to the TF2 implementation of
  `tf.summary.scalar`, and the return value will be a tensor containing an empty
  string, to avoid duplicate summary writing. This forwarding is best-effort and
