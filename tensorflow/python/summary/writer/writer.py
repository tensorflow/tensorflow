...
class SummaryToEventTransformer(object):
    ...
    def add_graph(self, graph, global_step=None, graph_def=None):
        ...
        if graph is not None and graph_def is not None:
            raise ValueError(
                "Please pass only graph, or graph_def (deprecated), but not both."
            )
        ...
        if isinstance(graph, ops.Graph) or isinstance(graph_def, ops.Graph):
        ...
        elif isinstance(graph, graph_pb2.GraphDef) or isinstance(
            ...
            raise TypeError(
                "The passed graph must be an instance of `Graph` "
    ...
    def add_meta_graph(self, meta_graph_def, global_step=None):
        ...
        if not isinstance(meta_graph_def, meta_graph_pb2.MetaGraphDef):
            raise TypeError(
                "meta_graph_def must be type MetaGraphDef, saw type: %s"
                % type(meta_graph_def)
            )
...
class FileWriter(SummaryToEventTransformer):
    ...
    def _warn_if_event_writer_is_closed(self):
        if self._closed:
            warnings.warn(
                "Attempting to use a closed FileWriter. "
