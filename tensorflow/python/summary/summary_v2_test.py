...
class SummaryV2Test(test.TestCase):
    ...
    def test_scalar_summary_v2__w_writer(self):
        ...
        with test.mock.patch.object(
            summary_v2, "scalar", autospec=True
        ) as mock_scalar_v2:
    ...
    def test_image_summary_v2(self):
        ...
        with test.mock.patch.object(
            summary_v2, "image", autospec=True
        ) as mock_image_v2:
            with summary_ops_v2.create_summary_file_writer(
                self.get_temp_dir()
            ).as_default(step=2):
                ...
                with ops.name_scope_v2("outer"):
                    tensor = summary_lib.image(
                        "image", i, max_outputs=3, family="family"
                    )
        # Returns empty string.
        self.assertEqual(tensor.numpy(), b"")
        self.assertEqual(tensor.dtype, dtypes.string)
    ...
    def test_histogram_summary_v2(self):
        ...
        with test.mock.patch.object(
            summary_v2, "histogram", autospec=True
        ) as mock_histogram_v2:
            with summary_ops_v2.create_summary_file_writer(
                self.get_temp_dir()
            ).as_default(step=3):
    ...
    def test_audio_summary_v2(self):
        ...
        with test.mock.patch.object(
            summary_v2, "audio", autospec=True
        ) as mock_audio_v2:
            with summary_ops_v2.create_summary_file_writer(
                self.get_temp_dir()
            ).as_default(step=10):
