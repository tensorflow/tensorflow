...
class ScaffoldTest(test.TestCase):
        ...
        with ops.Graph().as_default():
            ...
            with self.cached_session() as sess:
                self.assertItemsEqual(
                    [b"my_var", b"my_local_var"], sess.run(scaffold.ready_op)
                )
                self.assertItemsEqual(
                    [b"my_var"], sess.run(scaffold.ready_for_local_init_op)
                )
    ...
    def test_caches_values(self):
        with ops.Graph().as_default():
            ...
            self.assertEqual(
                scaffold1.ready_for_local_init_op, scaffold2.ready_for_local_init_op
            )
    ...
    def test_saving_restoring_checkpoint(self):
        ...
        with ops.Graph().as_default():
            ...
            with monitored_session.MonitoredTrainingSession(
                is_chief=True, checkpoint_dir=logdir
            ) as session:
...
class MockExtended(object):
    def __init__(
        self, between_graph, should_init, should_checkpoint, should_save_summary
    ):
...
class MockStrategy(object):
    def __init__(
        ...
        self.extended = MockExtended(
            between_graph, should_init, should_checkpoint, should_save_summary
        )
    ...
    def test_summary_hook_enabled(self):
        ...
        with ops.Graph().as_default():
            ...
            with context, monitored_session.MonitoredTrainingSession(
                checkpoint_dir=logdir, save_summaries_steps=100, log_step_count_steps=10
            ) as session:
                for _ in range(101):
                    session.run(new_gstep)
...
class CoordinatedSessionTest(test.TestCase):
    ...
    def test_stop_threads_on_close_after_exception(self):
        with self.cached_session() as sess:
            ...
            coord = coordinator.Coordinator()
            threads = [
                threading.Thread(target=busy_wait_for_coord_stop, args=(coord,))
                for _ in range(3)
            ]
            for t in threads:
    ...
    def test_propagates_exception_trace(self):
        ...
        with self.cached_session() as sess:
            ...
            try:
                ...
                self.assertIn(
                    exc_source_basename,
                    ["session.py", "monitored_session.py"],
...
class FailTrainingAfterCoordinatorStopped(StopCoordinatorWithException):
    ...
    def after_run(self, run_context, run_values):
        ...
        try:
            ...
            raise errors_impl.CancelledError(
                None, None, "Session got garbage-collected."
            )
    ...
    def test_recovery_from_coordinator_exception(self):
        with self.cached_session() as test_session:
            ...
            session = monitored_session.MonitoredSession(
                session_creator, [StopCoordinatorWithException(calls_before_stopping=2)]
            )
    ...
    def testCallsHooksBeginEnd(self):
        with ops.Graph().as_default(), session_lib.Session() as sess:
            ...
            for hook in [mock_hook, mock_hook2]:
                ...
                self.assertEqual(
                    hook.last_run_context.original_args,
...
class RunOptionsMetadataHook(session_run_hook.SessionRunHook):
    ...
    def before_run(self, run_context):
        options = config_pb2.RunOptions(
            ...
            report_tensor_allocations_upon_oom=self._report_tensor_allocations_upon_oom,
        )
...
class MonitoredSessionTest(test.TestCase):
        ...
        with ops.Graph().as_default():
            ...
            with monitored_session.MonitoredSession(hooks=hooks) as session:
                ...
                save_path = saver_lib._get_saver_or_default().save(
                    session._coordinated_creator.tf_sess, os.path.join(logdir, "step-3")
                )
            ...
            with monitored_session.MonitoredSession(
                hooks=hooks, session_creator=session_creator
            ) as session:
    ...
    def test_recover_and_retry_on_aborted_error(self):
        ...
        logdir = _test_dir(
            self.get_temp_dir(), "test_recover_and_retry_on_aborted_error"
        )
        with ops.Graph().as_default():
            gstep = training_util.get_or_create_global_step()
            do_step = state_ops.assign_add(gstep, 1)
    ...
    def test_step_with_extra_arguments(self):
        with ops.Graph().as_default():
            ...
            with monitored_session.MonitoredSession() as session:
                with self.assertRaisesRegex(
                    ValueError, "`step_fn` may either have one `step_context` argument"
                ):
                    self.assertEqual(None, session.run_step_fn(step_fn))
    ...
    def test_step_fn_with_hooks(self):
        with ops.Graph().as_default():
            ...
            class Hook(session_run_hook.SessionRunHook):
                ...
                def after_run(self, run_context, run_values):
                    self._testing.assertNear(
                        0.3 + 0.5 + 0.7, run_context.session.run(var), 0.1
                    )
                    self._testing.assertNear(
                        0.3 + 0.5 + 0.7 + 1.1, run_context.session.run(stage_2), 0.1
                    )
    ...
    def test_step_fn_exception_from_before_run(self):
        ...
        with ops.Graph().as_default():
            ...
            with self.cached_session() as test_session:
                with monitored_session.MonitoredSession(
                    CountingSessionCreator(test_session), hooks=[Hook(self)]
                ) as session:
