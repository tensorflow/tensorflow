...
def _get_var_info(var, prev_tensor_name=None):
    ...
    elif isinstance(var, list) and all(checkpoint_utils._is_variable(v) for v in var):  # pylint: disable=protected-access
        current_var_name = _infer_var_name(var)
...
def _get_grouped_variables(vars_to_warm_start):
    ...
    if isinstance(vars_to_warm_start, str) or vars_to_warm_start is None:
        ...
        raise ValueError(
            "`vars_to_warm_start must be a `list` or `str`.  Given type is {}".format(
        ...
        if vocab_info:
            ...
            if vars_to_warm_start:
                ...
                if prev_tensor_name in vocabless_vars:
                    ...
                    # used tensor.
                    logging.debug(
                        "Requested prev_var_name {} initialize both {} and {}; "
                        "calling init_from_checkpoint.".format(
                            prev_tensor_name, vocabless_vars[prev_tensor_name], var
                        )
                    )
