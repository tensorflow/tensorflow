...
class SessionManagerTest(test.TestCase):
    ...
    def testInitWithNoneLocalInitOpError(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "If you pass a ready_for_local_init_op you must also pass a local_init_op ",
        ):
            session_manager.SessionManager(
    ...
    def testPrepareSessionWithPartialInitOp(self):
        with ops.Graph().as_default():
            ...
            with self.cached_session():
                ...
                self.assertEqual(False, variable_v1.is_variable_initialized(x).eval())
                self.assertEqual(
                    False, variable_v1.is_variable_initialized(v_res).eval()
                )
                self.assertEqual(
                    False, variable_v1.is_variable_initialized(w_res).eval()
                )
                self.assertEqual(
                    False, variable_v1.is_variable_initialized(x_res).eval()
                )
            sm2 = session_manager.SessionManager(
                local_init_op=[
                    w.initializer,
