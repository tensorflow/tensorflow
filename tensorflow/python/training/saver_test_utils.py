...
class CheckpointedOp:
    ...
    def __init__(self, name, table_ref=None):
        ...
        if not context.executing_eagerly():
            ...
            ops_lib.add_to_collection(
                ops_lib.GraphKeys.SAVEABLE_OBJECTS, self._saveable
            )
    ...
    def _export(self):
        return gen_lookup_ops.lookup_table_export_v2(
            self.table_ref, dtypes.string, dtypes.float32
        )
    ...
    class CustomSaveable(saver_module.BaseSaverBuilder.SaveableObject):
        ...
        def __init__(self, table, name):
            ...
            specs = [
                saver_module.BaseSaverBuilder.SaveSpec(tensors[0], "", name + "-keys"),
                saver_module.BaseSaverBuilder.SaveSpec(
                    tensors[1], "", name + "-values"
                ),
            ]
