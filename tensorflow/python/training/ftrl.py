...
class FtrlOptimizer(optimizer.Optimizer):
    ...
    def _create_slots(self, var_list):
        ...
        def _accum_initializer(shape, dtype=dtypes.float32, partition_info=None):
            del partition_info
            return (
                array_ops.ones(shape=shape, dtype=dtype)
    ...
    def _prepare(self):
        ...
        self._adjusted_l2_regularization_strength_tensor = ops.convert_to_tensor(
            ...
            + self._beta / (2.0 * math_ops.maximum(self._learning_rate, 1e-36)),
    ...
    def _apply_dense(self, grad, var):
        ...
        if self._l2_shrinkage_regularization_strength <= 0.0:
            return gen_training_ops.apply_ftrl(
                ...
                math_ops.cast(
                    self._l1_regularization_strength_tensor, var.dtype.base_dtype
                ),
                math_ops.cast(
                    self._adjusted_l2_regularization_strength_tensor,
            ...
            return gen_training_ops.apply_ftrl_v2(
                ...
                math_ops.cast(
                    self._l2_shrinkage_regularization_strength_tensor,
                    var.dtype.base_dtype,
                ),
                math_ops.cast(self._learning_rate_power_tensor, var.dtype.base_dtype),
                use_locking=self._use_locking,
    ...
    def _resource_apply_sparse(self, grad, var, indices):
        ...
        if self._l2_shrinkage_regularization_strength <= 0.0:
            return gen_training_ops.resource_sparse_apply_ftrl(
                ...
                math_ops.cast(
                    self._adjusted_l2_regularization_strength_tensor, grad.dtype
                ),
            ...
            return gen_training_ops.resource_sparse_apply_ftrl_v2(
                ...
                math_ops.cast(
                    self._l2_shrinkage_regularization_strength_tensor, grad.dtype
                ),
                math_ops.cast(self._learning_rate_power_tensor, grad.dtype),
                use_locking=self._use_locking,
            )
