...
class RMSPropOptimizerTest(test.TestCase):
    def _rmsprop_update_numpy(
        self, var, g, mg, rms, mom, lr, decay, momentum, epsilon, centered
    ):
    ...
    def testSparse(self):
        # TODO(yori): Use ParameterizedTest when available
        for dtype, learning_rate, decay, momentum, epsilon, centered, _ in _TESTPARAMS:
            with test_util.use_gpu():
                # Initialize variables for numpy implementation.
                var0_np = np.array([1.0, 2.0], dtype=dtype.as_numpy_dtype)
                ...
                for _ in range(1, 5):
                    ...
                    var0_np, mg0_np, rms0_np, mom0_np = (
                    ...
                    var1_np, mg1_np, rms1_np, mom1_np = (
                        self._sparse_rmsprop_update_numpy(
                            var1_np,
                            grads1_np_indices,
                            grads1_np,
            ...
            with test_util.use_gpu():
                ...
                self.assertAllCloseAccordingToType(
                    np.array(
                        [
                            3.0
                            - (0.01 * 2.0 / math.sqrt(0.90001 + 1.0))
