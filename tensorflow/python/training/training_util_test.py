...
class GlobalStepTest(test.TestCase):
    ...
    def test_invalid_dtype(self):
        with ops.Graph().as_default() as g:
            ...
            self.assertRaisesRegex(
                TypeError, "does not have integer type", training_util.get_global_step
            )
        self.assertRaisesRegex(
            TypeError, "does not have integer type", training_util.get_global_step, g
        )
    ...
    def test_invalid_shape(self):
        with ops.Graph().as_default() as g:
            ...
            self.assertRaisesRegex(
                TypeError, "not scalar", training_util.get_global_step
            )
        self.assertRaisesRegex(
            TypeError, "not scalar", training_util.get_global_step, g
        )
    ...
    def test_create_global_step(self):
        ...
        with ops.Graph().as_default() as g:
            ...
            self.assertRaisesRegex(
                ValueError, "already exists", training_util.create_global_step
            )
            self.assertRaisesRegex(
                ValueError, "already exists", training_util.create_global_step, g
            )
