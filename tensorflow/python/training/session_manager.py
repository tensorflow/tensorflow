    ...
    * Initializing variables on startup, restoring them from the most recent
    ...
    def _restore_checkpoint(
        ...
        if checkpoint_dir and checkpoint_filename_with_path:
            raise ValueError(
                "Can not provide both checkpoint_dir and checkpoint_filename_with_path."
            )
    ...
    def recover_session(
        ...
        if not local_init_success:
            logging.info(
                "Restoring model from %s did not make model ready for local init: %s",
                restoring_file,
                msg,
            )
            return sess, False
...
def _ready(op: ops.Operation, sess: session.Session, msg) -> Tuple[bool, Optional[str]]:
