...
class _MultiStepStopAfterNEvalsHook(session_run_hook.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
        ...
        if self._num_evals is None:
            ...
            steps = min(
                self._num_evals - evals_completed, self._steps_per_run_initial_value
            )
...
class _StopAfterNEvalsHook(session_run_hook.SessionRunHook):
    ...
    def __init__(self, num_evals, log_progress=True):
        ...
        self._log_frequency = (
            1 if (num_evals is None or num_evals < 20) else math.floor(num_evals / 10.0)
        )
...
def _evaluate_once(
    ...
    if eval_ops is not None:
        ...
        for h in hooks:
            if isinstance(h, (_StopAfterNEvalsHook, _MultiStepStopAfterNEvalsHook)):
                h._set_evals_completed_tensor(
                    eval_step_value
                )  # pylint: disable=protected-access

    logging.info(
        "Starting evaluation at " + time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())
    )
    start = time.time()
    ...
    logging.info(
        "Finished evaluation at " + time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())
    )
