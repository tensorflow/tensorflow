...
def error_translator(e):
    ...
    error_message = str(e)
    if (
        "not found in checkpoint" in error_message
        or ("Failed to find any " "matching files for") in error_message
    ):
        raise errors_impl.NotFoundError(None, None, error_message)
    elif (
        "Sliced checkpoints are not supported" in error_message
        or ("Data type " "not " "supported") in error_message
    ):
...
def has_tensor(self, tensor_str):
    return self._HasTensor(
        compat.as_bytes(tensor_str)
