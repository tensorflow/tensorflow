    ...
    def testOneThreadDynamicPad(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            string = array_ops.tile(
                ["string"],
                math_ops.cast(array_ops_stack.stack([counter]), dtypes.int32),
            )
            self.evaluate(variables.global_variables_initializer())
            variables.local_variables_initializer().run()
            batched = inp.batch(
    ...
    def testOneThreadEnqueueMany(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            sparse_counter = sparse_tensor.SparseTensor(
                ...
                values=array_ops_stack.stack([math_ops.cast(counter, dtypes.float32)]),
                dense_shape=[1],
            )
...
class BatchJoinTest(test_lib.TestCase):
    ...
    def testMismatchedDictKeys(self):
        with (
            ...
            self.assertRaisesRegex(ValueError, "must have the same keys"),
        ):
            inp.batch_join(
    ...
    def testTwoThreadsDynamicPad(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            batch_size = 5
            a = array_ops.tile(
                ["a"], math_ops.cast(array_ops_stack.stack([counter + 1]), dtypes.int32)
            )
            b = array_ops.tile(
                ["b"], math_ops.cast(array_ops_stack.stack([ninety_nine]), dtypes.int32)
            )
            batched = inp.batch_join(
