...
class State(resource.TrackableResource):
    ...
    def _initialize(self):
        gen_resource_variable_ops.assign_variable_op(
            self.resource_handle, self._initial_value
        )
    ...
    def read(self):
        return gen_resource_variable_ops.read_variable_op(
            self.resource_handle, dtypes.float32
        )
...
class SaveableCompatibilityEndToEndTest(test.TestCase):
    def test_checkpoint_comparison(self):
        ...
        status = checkpoint.Checkpoint(b=saveable_state, a=trackable_state).read(
            ckpt_path
        )
...
class HasSerializeToTensorTest(test.TestCase):
    def test_has_serialize_to_tensor(self):
        ...
        self.assertTrue(
            saveable_object_util.trackable_has_serialize_to_tensor(ReturnsTrue())
        )
        self.assertFalse(
            saveable_object_util.trackable_has_serialize_to_tensor(ReturnsFalse())
        )
