...
class OptimizerTest(tf.test.TestCase):
  ...
  def testBasicMaximize(self):
    for dtype in [tf.half, tf.float32, tf.float64]:
      with self.test_session():
        ...
        global_step = tf.Variable(tf.zeros([], tf.int64), name='global_step')
        sgd_op = tf.train.GradientDescentOptimizer(3.0)
        opt_op = sgd_op.maximize(cost, global_step, [var0, var1])
        ...
        opt_op.run()
        # Validate updated params
        self.assertAllClose([16., 17.], var0.eval())
        self.assertAllClose([12., 13.], var1.eval())
  ...
  def testPrecomputedGradientMaximize(self):
    for dtype in [tf.half, tf.float32, tf.float64]:
      with self.test_session():
        var0 = tf.Variable([1.0, 2.0], dtype=dtype)
        var1 = tf.Variable([3.0, 4.0], dtype=dtype)
