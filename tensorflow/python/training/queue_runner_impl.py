...
class QueueRunner:
    ...
    def _init_from_args(
        ...
        if queue_closed_exception_types is not None:
            if (
                not isinstance(queue_closed_exception_types, tuple)
                or not queue_closed_exception_types
                or not all(
                    issubclass(t, errors.OpError) for t in queue_closed_exception_types
                )
            ):
                raise TypeError(
    ...
    def _init_from_proto(self, queue_runner_def, import_scope=None):
        ...
        self._close_op = g.as_graph_element(
            ops.prepend_name_scope(queue_runner_def.close_op_name, import_scope)
        )
        self._cancel_op = g.as_graph_element(
            ops.prepend_name_scope(queue_runner_def.cancel_op_name, import_scope)
        )
        self._queue_closed_exception_types = tuple(
    ...
    def create_threads(self, sess, coord=None, daemon=False, start=False):
        ...
        for op in self._enqueue_ops:
            ...
            ret_threads.append(
                threading.Thread(target=self._run, args=(sess, op, coord), name=name)
            )
        if coord:
...
def start_queue_runners(
    ...
    if sess is None:
        ...
        if not sess:
            raise ValueError(
                "Cannot start queue runners: No default session is "
    ...
    if not isinstance(sess, session.SessionInterface):
        ...
        if sess.__class__.__name__ in ["MonitoredSession", "SingularMonitoredSession"]:
            return []
        raise TypeError(
            "sess must be a `tf.Session` object. Given class: {}".format(sess.__class__)
        )
    ...
    with sess.graph.as_default():
        ...
        for qr in ops.get_collection(collection):
            threads.extend(
                qr.create_threads(sess, coord=coord, daemon=daemon, start=start)
            )
    return threads
