...
class CheckpointingTests(test.TestCase):
    ...
    def testDeferredSlotRestoration(self):
        ...
        self.evaluate(
            state_ops.assign(optimizer.get_slot(name="m", var=root.var), 14.0)
        )
    ...
    def testMultipleGraphsNonSlotVariables(self):
        with context.graph_mode():
            ...
            with first_graph.as_default(), first_session.as_default():
                ...
                self.evaluate(
                    optimizer.get_slot(var=first_variable, name="m").assign([2.0])
                )
            ...
            with second_graph.as_default(), session_lib.Session(graph=second_graph):
                ...
                self.evaluate(
                    optimizer.get_slot(var=second_variable, name="m").assign([5.0])
                )
                beta1_power, _ = optimizer._get_beta_accumulators()
                ...
                self.evaluate(
                    optimizer.get_slot(var=second_variable, name="m").assign([8.0])
                )
                beta1_power, _ = optimizer._get_beta_accumulators()
                ...
                self.assertAllEqual(
                    [5.0],
                    self.evaluate(optimizer.get_slot(var=second_variable, name="m")),
                )
                beta1_power, _ = optimizer._get_beta_accumulators()
                self.assertAllEqual(6.0, self.evaluate(beta1_power))
            ...
            with first_graph.as_default(), first_session.as_default():
                ...
                self.assertAllEqual(
                    [2.0],
                    self.evaluate(optimizer.get_slot(var=first_variable, name="m")),
                )
                beta1_power, _ = optimizer._get_beta_accumulators()
...
class TemplateTests(test.TestCase):
    ...
    def test_trackable_save_restore(self):
        ...
        self.assertEqual("ManualScope", load_template._checkpoint_dependencies[2].name)
        status.assert_consumed().run_restore_ops()
