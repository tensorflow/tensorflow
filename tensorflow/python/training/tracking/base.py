...
class CheckpointPosition(object):
    ...
    def _queue_slot_variable_for_restoration(
        self, optimizer_object, variable, slot_variable_id, slot_name
    ):
        ...
        self.checkpoint.slot_restoration_tensor_saveables.update(new_tensor_saveables)
...
class Trackable(object):
    ...
    def _restore_from_checkpoint_position(self, checkpoint_position):
        ...
        while visit_queue:
            ...
            if registered_saver:
                object_name = current_position.object_proto.registered_saver.object_name
                registered_savers[registered_saver][object_name] = trackable
                trackable._self_update_uid = (
                    current_position.checkpoint.restore_uid
                )  # pylint: disable=protected-access
            else:
                (
                    new_restore_ops,
                    new_tensor_saveables,
                    new_python_saveables,
        ...
        restore_ops.extend(
            current_position.checkpoint.restore_saveables(
                tensor_saveables, python_saveables, registered_savers
            )
        )
        return restore_ops
