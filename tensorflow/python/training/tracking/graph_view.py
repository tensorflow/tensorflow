...
def _object_prefix_from_path(node_paths):
    return "/".join((_escape_local_name(trackable.name) for trackable in node_paths))
...
class ObjectGraphView(object):
    ...
    def _add_attributes_to_object_graph(
        ...
        checkpoint_factory_map, registered_savers = get_checkpoint_factories_and_keys(
    ...
    def _add_checkpoint_values_check(self, trackable_objects, object_graph_proto):
        ...
        for trackable, object_proto in zip(trackable_objects, object_graph_proto.nodes):
            if (
    ...
    def serialize_object_graph_with_registered_savers(self):
        ...
        return self._serialize_gathered_objects(trackable_objects, node_paths)
    ...
    def frozen_saveable_objects(
        ...
        return self.frozen_saveables_and_savers(
            object_map, to_graph, call_with_mapped_captures
        )[0]
    ...
    def frozen_saveables_and_savers(
        ...
        with target_context():
            (
                ...
                _,
                registered_savers,
            ) = self._serialize_gathered_objects(
                trackable_objects, node_paths, object_map, call_with_mapped_captures
            )
