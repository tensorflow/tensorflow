...
class BaseSaverBuilder(object):
    ...
    def restore_op(self, filename_tensor, saveable, preferred_shard):
        ...
        for spec in saveable.specs:
            tensors.append(
                io_ops.restore_v2(
                    filename_tensor, [spec.name], [spec.slice_spec], [spec.dtype]
                )[0]
            )
    ...
    def _AddRestoreOps(
        ...
        all_tensors = self.bulk_restore(
            filename_tensor, saveables, preferred_shard, restore_sequentially
        )
    ...
    def _GroupByDevices(self, saveables):
        ...
        for saveable in saveables:
            ...
            if len(canonical_device) != 1:
                raise ValueError(
                    "All tensors of a saveable object must be "
    ...
    def _build_internal(
        ...
        if not context.executing_eagerly() and (not build_save or not build_restore):
            raise ValueError(
                "save and restore operations need to be built together "
        ...
        with ops.name_scope(
            name, "save", [saveable.op for saveable in saveables]
        ) as name:
            ...
            if sharded:
                ...
                if build_restore:
                    restore_op = self._AddRestoreOps(
                        filename_tensor, saveables, restore_sequentially, reshape
                    )
    ...
    if savers:
        if len(savers) > 1:
            raise RuntimeError(
                ...
                "Please indicate which one to use by passing it to the constructor.".format(
                    collection_key
                )
            )
        return savers[0]
    ...
    * `keep_checkpoint_every_n_hours`: In addition to keeping the most recent
    ...
    def _check_saver_def(self):
        if not isinstance(self.saver_def, saver_pb2.SaverDef):
            raise ValueError(
                "saver_def must be a saver_pb2.SaverDef: %s" % self.saver_def
            )
        if not context.executing_eagerly():
    ...
    def to_proto(self, export_scope=None):
        ...
        if not (
            self.saver_def.filename_tensor_name.startswith(export_scope)
            and self.saver_def.save_tensor_name.startswith(export_scope)
    ...
    def restore(self, sess, save_path):
        ...
        if not checkpoint_management.checkpoint_exists_internal(checkpoint_prefix):
            raise ValueError(
                "The passed save_path is not a valid checkpoint: " + checkpoint_prefix
            )
...
def import_meta_graph(
    ...
    * Run inference from a saved graph and checkpoints.
    ...
    return _import_meta_graph_with_return_elements(
        meta_graph_or_file, clear_devices, import_scope, **kwargs
    )[0]
...
def _create_saver_from_imported_meta_graph(meta_graph_def, import_scope, imported_vars):
    ...
    if meta_graph_def.HasField("saver_def"):
        ...
        if variables._all_saveable_objects(
            scope=import_scope
        ):  # pylint: disable=protected-access
            ...
            logging.info(
                "Saver not created because there are no variables in the"
                " graph to restore"
            )
            return None
...
def export_meta_graph(
    ...
    **kwargs
    ...
    """
    # pylint: enable=line-too-long
    if context.executing_eagerly() and not (
        graph_def is not None and graph is not None
    ):
        raise RuntimeError(
            "Exporting/importing meta graphs is not supported when "
            "eager execution is enabled. No graph exists when eager "
...
def _wrap_restore_error_with_msg(err, extra_verbiage):
    err_msg = (
        "Restoring from checkpoint failed. This is most likely "
...
def saver_from_object_based_checkpoint(
    ...
    if names_to_keys is None:
        try:
            ...
            raise ValueError(
                "Checkpoint in %s not an object-based checkpoint." % checkpoint_path
            )
    if var_list is None:
