...
class WarmStartingUtilTest(test.TestCase):
  ...
  def testWarmStartEmbeddingColumnVarianceScalingInitializer(self):
    ...
    with ops.Graph().as_default() as g:
      with self.session(graph=g) as sess:
        ...
        vocab_info = ws_util.VocabInfo(
            ...
            backup_initializer=init_ops.variance_scaling_initializer(seed=0))
        ws_util.warm_start(
            self.get_temp_dir(),
            var_name_to_vocab_info={
        ...
        self._assert_cols_to_vars(
            cols_to_vars, {
                emb_vocab_column: [
                    ...
                    np.array([[0.5, 0.4], [-0.226891, 0.097242],
                              [0.310654, 0.24019]])
                ]
            }, sess)
