...
class Fingerprint:
    ...
    def from_proto(cls, proto: fingerprint_pb2.FingerprintDef) -> "Fingerprint":
        ...
        try:
            ...
            raise ValueError(
                f"Given proto could not be deserialized as fingerprint.{e}"
            ) from None

    def __eq__(self, other: Any) -> bool:
        if isinstance(other, Fingerprint) or isinstance(
            other, fingerprint_pb2.FingerprintDef
        ):
            try:
                return (
                    self.saved_model_checksum == other.saved_model_checksum
                    and self.graph_def_program_hash == other.graph_def_program_hash
                    and self.signature_def_hash == other.signature_def_hash
                    and self.saved_object_graph_hash == other.saved_object_graph_hash
                    and self.checkpoint_hash == other.checkpoint_hash
                )
    ...
    def __repr__(self) -> str:
        return (
            f"Fingerprint({self.saved_model_checksum}, "
    ...
    def singleprint(self) -> fingerprinting_pywrap.Singleprint:
        ...
        """
        try:
            return fingerprinting_pywrap.Singleprint(
                self.graph_def_program_hash,
