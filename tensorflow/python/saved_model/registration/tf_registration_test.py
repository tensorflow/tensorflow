...
SERIALIZABLE_ALLOWLIST = os.path.join(
    resource_loader.get_data_files_path(), "tf_serializable_allowlist.txt"
)
CHECKPOINT_SAVER_ALLOWLIST = os.path.join(
    resource_loader.get_data_files_path(), "tf_checkpoint_saver_allowlist.txt"
)
...
class AllowlistTest(test.TestCase):
    def _test_allowlist(self, allowlist_file, registrations):
        ...
        if missing_from_allowlist:
            msg = (
                "[NEEDS ATTENTION] Registered names found that were not added to "
        ...
        if missing_registered_names:
            msg += (
                "[NEEDS ATTENTION] Some names were found in the allowlist that "
            ...
            msg += (
                "[OK] All allowlisted names are registered in the Tensorflow library. ✓"
            )
    ...
    def test_checkpoint_savers(self):
        self._test_allowlist(
            CHECKPOINT_SAVER_ALLOWLIST, test_util.get_all_registered_checkpoint_savers()
        )

    def test_serializables(self):
        self._test_allowlist(
            SERIALIZABLE_ALLOWLIST, test_util.get_all_registered_serializables()
        )
