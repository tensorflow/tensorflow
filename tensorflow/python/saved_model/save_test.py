...
class SaveTest(test.TestCase, parameterized.TestCase):
            ...
            input_signature=[
                ...
                [
                    ...
                    tensor_spec.TensorSpec(shape=(), dtype=dtypes.float32),
                    {
                        ...
                        "dict_entry": {
                            "a": tensor_spec.TensorSpec(shape=(), dtype=dtypes.float32),
                            "b": tensor_spec.TensorSpec(shape=(), dtype=dtypes.float32),
                        },
                    },
                    tensor_spec.TensorSpec(shape=(), dtype=dtypes.float32),
                ],
    ...
    def test_signature_attribute_reserved(self):
        root = checkpoint.Checkpoint(signatures=variables.Variable(1.0))
        save_dir = os.path.join(self.get_temp_dir(), "saved_model")
    ...
    def test_concrete_function_with_set_shape(
        self,
    ):
...
class DependencyTest(test.TestCase):
    ...
    def test_validate_dependencies_error_cyclic(self):
        ...
        save_dir = os.path.join(self.get_temp_dir(), "saved_model")
        with self.assertRaisesRegex(
            ValueError, "dependency cycle in the saved Trackable"
        ):
...
class SavingOptionsTest(test.TestCase):
    ...
    def test_concrete_function_aliases(self):
        ...
        self.assertEqual(f.name.decode("utf-8"), list(function_aliases.keys())[0])
...
class AssetTests(test.TestCase):
    ...
    def test_sensible_function_building_exception(self):
        root = checkpoint.Checkpoint(v=variables.Variable(2.0))
        root.f = def_function.function(
            lambda x: 2.0 * root.v,
            input_signature=[tensor_spec.TensorSpec(None, dtypes.float32)],
        )
