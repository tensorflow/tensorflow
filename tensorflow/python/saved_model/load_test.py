...
def _test_load_base(path, tags=None, options=None, use_cpp_bindings=False):  # pylint: disable=unused-argument
...
def _test_load_internal(path, tags=None, options=None, use_cpp_bindings=False):
    ...
    return _test_load_base(
        path, tags=tags, options=options, use_cpp_bindings=use_cpp_bindings
    )
...
def _load_test_params():
    ...
    if not run_external:
        params.append(
            dict(testcase_name="ReloadOnceCpp", cycles=1, use_cpp_bindings=True)
        )
    return params
...
class LoadTest(test.TestCase, parameterized.TestCase):
    ...
    def test_custom_gradients(self, cycles, use_cpp_bindings):
        ...
        loaded = cycle(
            root,
            cycles,
            save_option=save_options.SaveOptions(experimental_custom_gradients=True),
            use_cpp_bindings=use_cpp_bindings,
        )
        with backprop.GradientTape() as tape2:
            with backprop.GradientTape() as tape:
...
class SingleCycleTests(test.TestCase, parameterized.TestCase):
    ...
    def test_function_aliases(self, use_cpp_bindings):
        ...
        loaded = test_load(
            save_dir,
            use_cpp_bindings=use_cpp_bindings,
            options=load_options.LoadOptions(experimental_load_function_aliases=True),
        )
    ...
    def test_function_aliases_with_concrete_functions(self, use_cpp_bindings):
        ...
        options = save_options.SaveOptions(
            function_aliases={
                ...
                "my_func": [
                    f.get_concrete_function(x=tensor_spec.TensorSpec([], dtypes.int32)),
                    f.get_concrete_function(
