...
from tensorflow.python.eager.polymorphic_function import (
    saved_model_utils as function_saved_model_utils,
)
...
class _WrapperFunction(function.ConcreteFunction):
    ...
    def _call_flat(self, args, captured_inputs):
        ...
        if (
            distribute_lib.get_replica_context() is not None
...
class Loader(object):
        ...
        for node_id, proto in self._iter_all_nodes():
            ...
            if proto.saveable_objects.keys() == {
                ...
                for name, saveable_object_proto in proto.saveable_objects.items():
                    save_fn_id = saveable_object_proto.save_function
                    restore_fn_id = saveable_object_proto.restore_function
                    saveable_fn_by_name[name] = (
                        self.get(save_fn_id),
                        self.get(restore_fn_id),
                    )

                node._self_saveable_object_factories = (  # pylint: disable=protected-access
                    saveable_object_util.recreate_saveable_objects(
                        saveable_fn_by_name, temp_session
                    )
                )
    ...
    def _load_nodes(self):
        ...
        for node_id, proto in self._iter_all_nodes():
            for slot_variable_proto in proto.slot_variables:
                slot_variable_node_id = slot_variable_proto.slot_variable_node_id
                slot_variable_node_ids[slot_variable_node_id] = (
                    node_id,
                    slot_variable_proto,
                )
    ...
    def _restore_checkpoint(self):
        ...
        if not context.executing_eagerly():
            ...
            for object_id, obj in dict(ckpt.object_by_proto_id).items():
                position = restore.CheckpointPosition(
                    checkpoint=ckpt, proto_id=object_id
                )
                ...
                if restore_ops:
                    ...
                    elif isinstance(obj, lookup_ops.LookupInterface) or isinstance(
                        obj, resource.CapturableResource
                    ):
    ...
    def _recreate_default(self, proto, node_id, deps):
        ...
        factory = {
            ...
            "bare_concrete_function": functools.partial(
                ...
                proto=proto.bare_concrete_function,
                dependencies=deps,
            ),
                ...
                try:
                    ...
                    try:
                        ...
                        raise ImportError(
                            ...
                            "contains an optimizer without the Keras module."
                        ) from e
            except ImportError:
                try:
        ...
        if self._save_options.experimental_load_function_aliases:
            if proto.concrete_functions and all(
                ...
                if set(f.name for f in aliased) == set(
                    f.name
                    for f in fn._list_all_concrete_functions()  # pylint: disable=protected-access
                ):
    ...
    def _recreate_variable(self, proto):
        ...
        with ops.get_default_graph()._variable_creator_scope(  # pylint: disable=protected-access
            ...
            load_with_device = (
                self._save_options.experimental_variable_policy._save_variable_devices()
...
def load_partial(export_dir, filters, tags=None, options=None):
    ...
    **Loading under different distribution strategies**
    ...
    if len(saved_model_proto.meta_graphs) == 1 and saved_model_proto.meta_graphs[
        ...
        if filters:
            raise ValueError(
                "SavedModels saved from Tensorflow 1.x) cannot be "
    ...
    try:
        ...
        metrics.SetReadFingerprint(
            fingerprint=fingerprinting_utils.to_proto(fingerprint).SerializeToString()
        )
    ...
    try:
        ...
        logging.info(
            "path_and_singleprint metric could not be logged. "
            "Saved model loading will continue."
        )
