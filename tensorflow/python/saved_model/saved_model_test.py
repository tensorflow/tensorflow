...
class SavedModelTest(SavedModelTestBase):
    def _validate_assets(
        ...
        self.assertEqual(
            expected_asset_file_contents, compat.as_text(actual_asset_contents)
        )
        self.assertEqual(expected_asset_file_name, asset_file_def[asset_id].filename)
        self.assertEqual(
            expected_asset_tensor_name, asset_file_def[asset_id].tensor_info.name
        )
    ...
    def testBadSavedModelFileFormat(self):
        ...
        path_to_pbtxt = os.path.join(export_dir, constants.SAVED_MODEL_FILENAME_PBTXT)
        with open(path_to_pbtxt, "w") as f:
            f.write("invalid content")
        with self.session(graph=ops.Graph()) as sess:
            with self.assertRaisesRegex(
                IOError, "Cannot parse file.*%s" % constants.SAVED_MODEL_FILENAME_PBTXT
            ):
                loader.load(sess, ["foo"], export_dir)
    ...
    def testSequence(self):
        ...
        with ops.Graph().as_default():
            ...
            with self.session(graph=ops.Graph()) as sess:
                ...
                self.assertRaises(
                    AssertionError, builder.add_meta_graph_and_variables, sess, ["baz"]
                )
    ...
    def testTags(self):
        ...
        with ops.Graph().as_default():
            ...
            with self.session(graph=ops.Graph()) as sess:
                loader.load(
                    sess, [tag_constants.SERVING, tag_constants.GPU], export_dir
                )
                self.assertEqual(
                    42,
                    self._eval(ops.get_collection(ops.GraphKeys.GLOBAL_VARIABLES)[0]),
            ...
            with self.session(graph=ops.Graph()) as sess:
                self.assertRaises(
                    RuntimeError, loader.load, sess, ["INVALID"], export_dir
                )
            ...
            with self.session(graph=ops.Graph()) as sess:
                self.assertRaises(
                    RuntimeError, loader.load, sess, ["foo", "baz"], export_dir
                )
    ...
    def testVariables(self):
        ...
        with ops.Graph().as_default():
            ...
            with self.session(graph=ops.Graph()) as sess:
                self.assertRaises(
                    errors.NotFoundError, loader.load, sess, ["baz"], export_dir
                )
    ...
    def testNoOverwrite(self):
        ...
        with ops.Graph().as_default():
            ...
            self.assertRaises(
                AssertionError, saved_model_builder._SavedModelBuilder, export_dir
            )
    ...
    def testSignatureDefValidationFails(self):
        ...
        self._validate_sig_def_keys(
            builder, valid_tensor_info, constants.INIT_OP_SIGNATURE_KEY
        )
        self._validate_sig_def_keys(
            builder, valid_tensor_info, constants.TRAIN_OP_SIGNATURE_KEY
        )
    ...
    def testMultipleAssets(self):
        ...
        with ops.Graph().as_default():
            with self.session(graph=ops.Graph()) as sess:
                ...
                asset_list = self._build_asset_collection(
                    "foo.txt", "content_foo", "asset_file_tensor"
                )
            ...
            with self.session(graph=ops.Graph()) as sess:
                ...
                asset_list = self._build_asset_collection(
                    "bar.txt", "content_bar", "asset_file_tensor"
                )
    ...
    def testDuplicateAssets(self):
        ...
        with ops.Graph().as_default():
            ...
            with self.session(graph=ops.Graph()) as sess:
                ...
                # content.
                asset_list = self._build_asset_collection(
                    "foo.txt", "content_bar", "asset_file_tensor"
                )
    ...
    def testCustomSaveable(self):
        ...
        with session.Session(
            graph=ops.Graph(), config=config_pb2.ConfigProto(device_count={"CPU": 2})
        ) as sess:
        ...
        def _validate_custom_saver(tag_name, saver_name):
            with ops.Graph().as_default() as graph:
                with self.session(graph=graph) as sess:
                    ...
                    self.assertEqual(saved_graph.saver_def.restore_op_name, saver_name)
        ...
        with ops.Graph().as_default():
            ...
            with self.session(graph=ops.Graph()) as sess:
                ...
                self.assertEqual(
                    ...
                    .get_tensor_by_name("scope_name/constant_tensor_name:0")
                    .eval(),
                )
    ...
    def testInconsistentConsumerDefaultAttrs(self):
      ...
      attr {
        ...
        sess = session.Session(graph=ops.Graph())
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "(?s)No OpKernel was registered.*DOUBLE"
        ):
            loader.load(sess, ["foo"], export_dir)
...
class SavedModelV1Test(SavedModelTestBase):
    def _validate_asset_collection(
        self,
        export_dir,
    ...
    def testStripDefaultAttrs(self):
        ...
        sess = session.Session(graph=ops.Graph())
        meta_graph_def = loader.load(sess, ["foo"], export_dir)
        complex_node = test_util.get_node_def_from_graph(
        ...
        node_def = test_util.get_node_def_from_graph(
            "complex", meta_graph_foo_def.graph_def
        )
        ...
        node_def = test_util.get_node_def_from_graph(
            "complex", meta_graph_bar_def.graph_def
        )
