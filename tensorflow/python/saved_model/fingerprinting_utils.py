...
from tensorflow.python.saved_model.pywrap_saved_model import (
    fingerprinting as fingerprinting_pywrap,
)
...
def write_fingerprint(export_dir: str) -> None:
    ...
    if flags.config().saved_model_fingerprinting.value():
        fingerprint_path = file_io.join(
            compat.as_str(export_dir), compat.as_str(constants.FINGERPRINT_FILENAME)
        )
        ...
        try:
            ...
            logging.info(
                "path_and_singleprint metric could not be set. "
