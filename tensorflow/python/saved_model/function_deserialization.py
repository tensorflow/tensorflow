    ...
    if function_spec_proto.is_method or (
        ...
        if not typeless_fullargspec.args:
            raise NotImplementedError(
                "Cannot deserialize a method function without a named 'self' argument."
            )
...
def set_preinitialized_function_spec(concrete_fn, spec):
    ...
    input_function_type, _ = function_type_lib.canonicalize_to_monomorphic(
        arg_specs,
        {function_type_lib.sanitize_arg_name(k): v for k, v in kwarg_specs.items()},
        spec.default_values,
        {},
        unconstrained_type,
    )
        ...
        for allow_conversion in [False, True]:
            for function_name in saved_function.concrete_functions:
                ...
                if any([inp is None for inp in function.captured_inputs]):
                    raise ValueError(
                        "Looks like you are trying to run a loaded "
...
def load_function_def_library(
    ...
    for fdef in _sort_function_defs(library, function_deps):
        ...
        if (
            saved_object_graph is not None
            and orig_name in saved_object_graph.concrete_functions
        ):
...
_FUNCTION_WRAPPER_NAME_REGEX = r"^%s(.*)_\d+$" % (function_lib._INFERENCE_PREFIX)  # pylint:disable=protected-access
