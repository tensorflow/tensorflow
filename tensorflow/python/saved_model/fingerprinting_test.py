...
class FingerprintingTest(test.TestCase):
    ...
    def test_model_saved_with_different_signature_options(self):
        ...
        # SignatureDef hashes.
        self.assertNotEqual(
            fingerprint_sig.signature_def_hash, fingerprint_no_sig.signature_def_hash
        )
        # Check that the model saved with the same concrete function has the same
        # regularized hashes.
        self.assertEqual(
            fingerprint_sig.graph_def_program_hash,
            fingerprint_input_sig.graph_def_program_hash,
        )
        self.assertEqual(
            fingerprint_sig.signature_def_hash, fingerprint_input_sig.signature_def_hash
        )
    ...
    def test_write_fingerprint(self):
        ...
        save.save_and_return_nodes(
            self._create_model_with_data(), save_dir, experimental_skip_checkpoint=True
        )  # checkpoint data won't be loaded
    ...
    def test_valid_singleprint(self):
        ...
        self.assertRegex(
            ...
            "/".join(
                [
                    "8947653168630125217",  # graph_def_program_hash
    ...
    def test_invalid_from_proto(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Given proto could not be deserialized as"
        ):
