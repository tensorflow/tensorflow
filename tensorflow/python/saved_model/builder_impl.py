...
class _SavedModelBuilder(object):
    ...
    def _save_and_write_assets(self, meta_graph_def, assets_list=None):
        ...
        copy_assets_to_destination_dir(
            asset_filename_map, self._export_dir, self._saved_asset_files
        )
...
class SavedModelBuilder(_SavedModelBuilder):
    ...
    def _add_train_op(self, train_op):
        ...
        if train_op is not None:
            if not isinstance(train_op, tensor.Tensor) and not isinstance(
    ...
    add_meta_graph.__doc__ = (_SavedModelBuilder.add_meta_graph.__doc__ or "").replace(
        "assets_list", "assets_collection"
    )
...
def copy_assets_to_destination_dir(
    asset_filename_map, destination_dir, saved_files=None
):
    ...
    for asset_basename, asset_source_filepath in asset_filename_map.items():
        ...
        # saved_files
        if (
            file_io.file_exists(asset_source_filepath)
            and asset_source_filepath != asset_destination_filepath
            and asset_destination_filepath not in saved_files
        ):
            file_io.copy(
