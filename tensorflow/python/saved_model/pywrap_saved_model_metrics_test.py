...
class MetricsTest(test.TestCase):
    ...
    def _get_serialized_fingerprint_def(self):
        return fingerprint_pb2.FingerprintDef(
            ...
            checkpoint_hash=5,
            version=versions_pb2.VersionDef(producer=6),
        ).SerializeToString()
    ...
    def test_checkpoint_add_write_duration(self):
        self.assertEqual(
            self._get_histogram_proto(
                metrics.GetCheckpointWriteDurations(api_label="foo")
            ).num,
            0,
        )
    ...
    def test_async_checkpoint_add_write_duration(self):
        self.assertEqual(
            self._get_histogram_proto(
                metrics.GetAsyncCheckpointWriteDurations(api_label="foo")
            ).num,
            0,
        )
    ...
    def test_checkpoint_add_read_duration(self):
        self.assertEqual(
            self._get_histogram_proto(
                metrics.GetCheckpointReadDurations(api_label="bar")
            ).num,
            0,
        )
    ...
    def test_SM_read_invalid_path_and_singleprint(self):
        ...
        self.assertRegex(
            str(excinfo.exception), "Invalid path_and_singleprint argument. Empty path."
        )
