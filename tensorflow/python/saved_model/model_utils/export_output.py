...
class ClassificationOutput(ExportOutput):
    ...
    def __init__(self, scores=None, classes=None):
        ...
        if scores is not None and not (
            isinstance(scores, tensor.Tensor) and scores.dtype.is_floating
        ):
            raise ValueError(
                "Classification scores must be a float32 Tensor; got {}".format(scores)
            )
        if classes is not None and not (
            isinstance(classes, tensor.Tensor)
            and dtypes.as_dtype(classes.dtype) == dtypes.string
        ):
            raise ValueError(
                "Classification classes must be a string Tensor; got {}".format(classes)
            )
...
class RegressionOutput(ExportOutput):
    ...
    def __init__(self, value):
        ...
        if not (isinstance(value, tensor.Tensor) and value.dtype.is_floating):
            raise ValueError(
                "Regression output value must be a float32 Tensor; got {}".format(value)
            )
        self._value = value
...
class _SupervisedOutput(ExportOutput):
    ...
    def _wrap_and_check_metrics(self, metrics):
        ...
        for key, value in metrics.items():
            ...
            if not (
                tensor_util.is_tf_type(metric_op)
