...
class ExportOutputTest(test.TestCase):
    ...
    def test_classify_requires_classes_or_scores(self):
        with self.assertRaisesRegex(
            ValueError, "Cannot create a ClassificationOutput with empty arguments"
        ):
    ...
    def test_build_standardized_signature_def_regression(self):
        with context.graph_mode():
            ...
            expected_signature_def.outputs[
                ...
                meta_graph_pb2.TensorInfo(
                    name="output-tensor-1:0", dtype=dtype_float, tensor_shape=shape
                )
            )
    ...
    def test_build_standardized_signature_def_classify_classes_only(self):
        ...
        with context.graph_mode():
            ...
            expected_signature_def.outputs[
                ...
                meta_graph_pb2.TensorInfo(
                    name="output-tensor-1:0", dtype=dtype_string, tensor_shape=shape
                )
            )
    ...
    def test_build_standardized_signature_def_classify_both(self):
        ...
        with context.graph_mode():
            ...
            expected_signature_def.outputs[
                ...
                meta_graph_pb2.TensorInfo(
                    name="output-tensor-scores:0", dtype=dtype_float, tensor_shape=shape
                )
            )
    ...
    def test_predict_outputs_valid(self):
        ...
        outputs = {
            ...
            "output1": constant_op.constant(["foo"]),
        }
    ...
    def test_predict_outputs_invalid(self):
        with self.assertRaisesRegex(
            ValueError, "Prediction output key must be a string"
        ):
...
class SupervisedOutputTest(test.TestCase):
    def test_supervised_outputs_valid(self):
        ...
        with context.graph_mode():
            ...
            predictions = {"output1": constant_op.constant(["foo"])}
            ...
            self.assertEqual(
                outputter.metrics["metrics/update_op"].name, "mean/update_op:0"
            )
            self.assertEqual(
    ...
    def test_supervised_outputs_tuples(self):
        ...
        with context.graph_mode():
            ...
            predictions = {("output1", "2"): constant_op.constant(["foo"])}
            mean, update_op = metrics_module.mean_tensor(constant_op.constant([0]))
            metrics = {
            ...
            self.assertEqual(
                set(outputter.predictions.keys()), set(["predictions/output1/2"])
            )
            self.assertEqual(
    ...
    def test_supervised_outputs_no_prepend(self):
        ...
        with context.graph_mode():
            ...
            predictions = {"predictions": constant_op.constant(["foo"])}
            mean, update_op = metrics_module.mean_tensor(constant_op.constant([0]))
            metrics = {
    ...
    def test_train_signature_def(self):
        with context.graph_mode():
            ...
            receiver = {
                "features": constant_op.constant(100, shape=(100, 2)),
    ...
    def test_metric_op_is_tensor(self):
        ...
        with context.graph_mode():
            ...
            self.assertTrue(
                outputter.metrics["metrics_1/update_op"].name.startswith(
                    "mean/update_op"
                )
            )
            ...
            self.assertEqual(
                outputter.metrics["metrics_2/value"], metrics["metrics_2"][0]
            )
            self.assertTrue(
                outputter.metrics["metrics_2/update_op"].name.startswith(
                    "metric_op_wrapper"
                )
            )
            self.assertIsInstance(
