...
from tensorflow.python.saved_model.pywrap_saved_model import merger as pywrap_merger
...
def parse_saved_model(export_dir):
  ...
  if file_io.file_exists(path_to_pb) or file_io.file_exists(path_to_cpb):
    path_to_prefix = file_io.join(
        compat.as_bytes(compat.path_to_str(export_dir)),
        compat.as_bytes(constants.SAVED_MODEL_FILENAME_PREFIX)).decode("utf-8")
    try:
    ...
    except pywrap_merger.MergerException as e:
      raise IOError(f"Cannot parse file {path_to_prefix}: {str(e)}.") from e
