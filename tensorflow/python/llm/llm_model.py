...
class TFLLMModel:
  ...
  def fit(
    ...
    try:
      ...
      logger.warning(
          "HuggingFace Trainer failed (%s). Falling back to manual loop.",
          exc
      )
      history = self._manual_train_loop(
          dataset, epochs, steps_per_epoch, verbose
      )
  ...
  def __repr__(self) -> str:
    lora_str = (
        f", lora=True(rank={self._lora_config.get('r', '?')})"
        if self.lora_enabled else ""
    )
...
def from_pretrained(
  ...
  Supported model families (prefix match):
      - ``meta-llama/Llama-3``, ``meta-llama/Llama-3.1``,
        ``meta-llama/Llama-3.2``
      - ``meta-llama/Llama-2``
  ...
  Args:
      ...
      token: HuggingFace API token for gated models (e.g. Llama 3).
          Can also be set via the ``HUGGINGFACE_TOKEN`` environment
          variable.
      lora_config: Optional default LoRA settings that will be passed to
          :meth:`TFLLMModel.compile` when called without explicit
          overrides.
  ...
  # ----------------------------------------------------------------
  meta_path = (
      os.path.join(model_name, "tf_llm_meta.json")
      if os.path.isdir(model_name) else None
  )
  ...
  tokenizer_source = (
      model_name if os.path.isdir(model_name) else base_model_name
  )
  ...
  logger.info(
      "Loading model weights for '%s' (dtype=%s, device_map=%s)…",
