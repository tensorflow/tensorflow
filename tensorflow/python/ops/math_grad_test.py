...
class SqrtGradTest(test.TestCase):
  ...
  def testFloat64PositiveSubnormalGradient(self):
    ...
    expected[is_positive_subnormal] = float(2**536) / np.sqrt(
        bits[is_positive_subnormal].astype(np.float64)
    )
    with np.errstate(divide="ignore", invalid="ignore"):
      expected[~is_positive_subnormal] = 0.5 / np.sqrt(
          values[~is_positive_subnormal]
      )
    expected *= upstream
  ...
  def testFloat64PositiveSubnormalHigherOrderGradients(self):
    ...
    with ops.device("/CPU:0"):
      ...
      third_derivative = third_order_tape.gradient(second_derivative, x)
  ...
  def testFloat64SubnormalHigherOrderGradientsWithSmallUpstream(self):
    with ops.device("/CPU:0"):
      x = constant_op.constant(np.nextafter(0.0, 1.0), dtype=dtypes.float64)
      ...
      with backprop.GradientTape() as outer_tape:
        ...
        with backprop.GradientTape() as inner_tape:
          inner_tape.watch(x)
          y = math_ops.sqrt(x)
        first_derivative = inner_tape.gradient(y, x, output_gradients=upstream)
