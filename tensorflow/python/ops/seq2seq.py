  ...
  with ops.op_scope(logits + targets + weights, name,
    ...
    for logit, target, weight in zip(logits, targets, weights):
      if softmax_loss_function is None:
        ...
        # rewrite this method.
        indices = target + num_decoder_symbols * math_ops.range(batch_size)
        with ops.device("/cpu:0"):  # Sparse-to-dense must be on CPU for now.
          dense = sparse_ops.sparse_to_dense(
              ...
              0.0)
        dense_target = array_ops.reshape(dense, [-1, num_decoder_symbols])
        crossent = nn_ops.softmax_cross_entropy_with_logits(logit, dense_target)
      else:
    ...
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        losses.append(sequence_loss(
            ...
            num_decoder_symbols, softmax_loss_function=softmax_loss_function))
