...
class TestFoldNonOverlapping(test.TestCase):
  ...
  def test_perfect_inverse_no_overlap_valid_basic(self):
    ...
    reconstructed = array_ops.fold(
        patches, output_size=(8, 8), sizes=4, strides=4, padding="VALID"
    )
    self.assertAllClose(
        reconstructed,
        x,
        ...
        for kernel_size in kernel_sizes:
          for channels in channel_sizes:
            ...
            patches = self._extract_patches(
                x, kernel=kernel_size, stride=kernel_size, padding="VALID"
            )
            reconstructed = array_ops.fold(
                patches,
...
class TestFoldOverlapping(test.TestCase):
  ...
  def test_fold_overlapping_patches_basic(self):
    ...
    reconstructed = array_ops.fold(
        patches, output_size=(4, 4), sizes=2, strides=1, padding="VALID"
    )
  ...
  def testFoldReductionMeanReconstruction(self):
    ...
    # * folded_img (with reduction="mean") =
    # * folded_img_summed / overlap count (ones_tensor)
    # [  1/1    4/2    3/1 ]  = [1,2,3]
    # [  8/2   20/4   12/2 ]  = [4,5,6]
    # [  7/1   16/2    9/1 ]  = [7,8,9]
...
class TestFoldInputValidation(test.TestCase):
  ...
  def test_invalid_dilation_raises(self):
    ...
    with self.assertRaisesRegex(ValueError, "dilation must be >= 1"):
      array_ops.fold(patches, output_size=(4, 4), sizes=2, strides=2, rates=-1)
  ...
  def test_invalid_reduction(self):
    patches = random_ops.random_normal([1, 3, 3, 4])
    with self.assertRaisesRegex(
        ValueError, "reduction must be 'sum' or 'mean'"
    ):
      array_ops.fold(
          patches,
          output_size=(4, 4),
...
class TestFoldSamePadding(test.TestCase):
  ...
  def test_perfect_inverse_no_overlap_same(self):
    ...
    reconstructed = array_ops.fold(
        patches, output_size=(5, 5), sizes=3, strides=3, padding="SAME"
    )
  ...
  def test_fold_same_padding_with_dilation(self):
    ...
    patches = self._extract_patches(
        x, kernel=kernel_size, stride=stride, padding="SAME", dilation=dilation
    )
  ...
  def test_fold_non_square_parameters(self):
    ...
    patches = array_ops.extract_image_patches_v2(
        x,
        sizes=[1, 3, 5, 1],
        strides=[1, 2, 3, 1],
        padding="SAME",
    ...
    ones = array_ops.ones_like(x)
    ones_patches = array_ops.extract_image_patches_v2(
        ones,
        sizes=[1, 3, 5, 1],
        strides=[1, 2, 3, 1],
        padding="SAME",
...
class TestFoldDeterminism(test.TestCase):
  ...
  def test_fold_gpu_deterministic(self):
    ...
    patches = self._extract_patches(x, kernel=15, stride=1, padding="VALID")
    outputs = []
    for _ in range(20):
      outputs.append(
          array_ops.fold(
  ...
  def test_fold_cpu_deterministic(self):
    ...
    with ops.device("/CPU:0"):
      ...
      patches = self._extract_patches(x, kernel=5, stride=1, padding="VALID")

      reference = array_ops.fold(
          patches, output_size=(32, 32), sizes=5, strides=1, padding="VALID"
      )
