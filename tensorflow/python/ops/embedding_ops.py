            ...
            if partition_strategy == "mod":
                ...
                if dim_0_size.value:
                    ...
                    num_total_ids = math_ops.reduce_sum(
                        math_ops.cast(
                            array_ops_stack.stack(dim_0_sizes), flat_ids.dtype
                        )
                    )
...
def embedding_lookup_sparse(
    ...
    if not ignore_weights:
        if not isinstance(sp_weights, sparse_tensor.SparseTensor):
            raise TypeError(
                f"sp_weights must be either None or SparseTensor,got {type(sp_weights)}"
            )
...
def safe_embedding_lookup_sparse(
    ...
    embedding_weights = [
        w
        if (resource_variable_ops.is_resource_variable(w) and dtype in (None, w.dtype))
