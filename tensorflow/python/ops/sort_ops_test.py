...
class SortTest(test.TestCase):
    ...
    def _test_sort(self, values, axis, direction):
        ...
        self.assertAllEqual(
            expected,
            sort_ops.sort(constant_op.constant(values), axis=axis, direction=direction),
        )
    ...
    def testUIntArray(self):
        ...
        edges = np.linspace(np.iinfo(dtype).min, np.iinfo(dtype).max, 10, dtype=dtype)
        self._test_sort(edges, 0, "ASCENDING")
        self._test_sort(edges, 0, "DESCENDING")
    ...
    def testNegativeOutOfBounds_staticShape(self):
        ...
        with self.assertRaisesRegex(
            (ValueError, errors.InvalidArgumentError), "slice index .* out of bounds"
        ):
    ...
    def testSort_staticallyKnownRank_constantTransposition(self):
        with ops.Graph().as_default():
            ...
            transposition = ops.get_default_graph().get_tensor_by_name(
                "sort/transposition:0"
            )
    ...
    def testArgsortStable(self):
        ...
        with self.cached_session():
            ...
            self.assertAllEqual(
                sort_ops.argsort(arr, direction="DESCENDING", stable=True), descending
            )
