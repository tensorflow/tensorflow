...
class VariableSpecTest(test.TestCase, parameterized.TestCase):
    def test_properties(self):
        ...
        self.assertAllEqual(
            spec._component_specs, [tensor_spec.TensorSpec([], dtypes.resource)]
        )
        ...
        self.assertAllEqual(
            spec2._component_specs, [tensor_spec.TensorSpec([], dtypes.resource)]
        )
    ...
    @parameterized.parameters(
        [
            dict(
                initial_value=[1, 2, 3], shape=[3], dtype=dtypes.int32, trainable=False
            ),
            dict(initial_value=[[1.0, 2.0, 3.0]], shape=[1, None]),
        ]
    )
    def testFromValue(
        self, initial_value=None, shape=None, dtype=dtypes.float32, trainable=True
    ):
        var = resource_variable_ops.ResourceVariable(
            initial_value=initial_value, shape=shape, dtype=dtype, trainable=trainable
        )
    ...
    @parameterized.parameters(
        [
            dict(initial_value=[1.0, 2.0, 3.0], shape=[3]),
            dict(initial_value=[1.0, 2.0, 3.0], shape=None),
            dict(initial_value=[[1, 2, 3]], shape=[1, None], dtype=dtypes.int32),
            dict(initial_value=[[1, 2, 3]], shape=[None, None], dtype=dtypes.int32),
        ]
    )
    def testToFromComponents(
        self, initial_value=None, shape=None, dtype=dtypes.float32, trainable=True
    ):
        var = resource_variable_ops.ResourceVariable(
            initial_value=initial_value, shape=shape, dtype=dtype, trainable=trainable
    ...
    def testFromComponentsSetHandleData(self):
        ...
        self.assertEqual(
            handle2_data.shape_and_type[0].shape,
            expected_handle_data.shape_and_type[0].shape,
        )
        self.assertEqual(
            handle2_data.shape_and_type[0].dtype,
    ...
    def testComponentSpecs(self):
        self.skipTest(
            "b/209081027: re-enable this test after ResourceVariable "
    ...
    def testRepr(self):
        ...
        spec_repr = repr(spec)
        expected_repr = (
            "VariableSpec(shape=(1, 3), dtype=tf.int32, trainable=False, alias_id=None)"
        )
