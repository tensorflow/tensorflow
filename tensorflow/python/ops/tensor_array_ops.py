...
class _GraphTensorArray:
    ...
    def __init__(
        ...
        if handle is not None and size is not None:
            raise ValueError(
                "Cannot provide both a `handle` and `size` arguments at the same time."
            )
...
class _EagerTensorArray:
    ...
    def __init__(
        ...
        if handle is not None:
            raise ValueError(
                "TensorArray handles are not supported when eager execution is enabled."
            )
        if size is None:
            raise ValueError(
...
class TensorArraySpec(type_spec.TypeSpec):
    ...
    def most_specific_common_supertype(self, others):
        ...
        return TensorArraySpec(
            common_shape, self._dtype, self._dynamic_size, infer_shape
        )
