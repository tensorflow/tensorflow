...
@deprecation.deprecated_endpoints(
    "uniform_unit_scaling_initializer", "initializers.uniform_unit_scaling"
)
...
class VarianceScaling(Initializer):
    ...
    def __init__(
        ...
        if distribution not in {
            ...
            raise ValueError(
                "Argument `distribution` should be one of ('normal', "
...
class ConvolutionDeltaOrthogonal(Initializer):
    ...
    def __call__(self, shape, dtype=None, partition_info=None):
        ...
        if shape[-2] > shape[-1]:
            raise ValueError(
                f"In_filters, specified by shape[-2]={shape[-2]} cannot "
        ...
        a = random_ops.random_normal(
            [shape[-1], shape[-1]], dtype=dtype, seed=self.seed
        )
        # Compute the qr factorization
        q, r = gen_linalg_ops.qr(a, full_matrices=False)
        # Make Q uniform
        ...
        if len(shape) == 3:
            weight = array_ops.scatter_nd(
                [[(shape[0] - 1) // 2]], array_ops.expand_dims(q, 0), shape
            )
        elif len(shape) == 4:
            weight = array_ops.scatter_nd(
                [[(shape[0] - 1) // 2, (shape[1] - 1) // 2]],
                array_ops.expand_dims(q, 0),
                shape,
            )
        else:
            weight = array_ops.scatter_nd(
                [[(shape[0] - 1) // 2, (shape[1] - 1) // 2, (shape[2] - 1) // 2]],
                array_ops.expand_dims(q, 0),
                shape,
            )
        return weight
...
class ConvolutionOrthogonal2D(ConvolutionOrthogonal):
    ...
    def __call__(self, shape, dtype=None, partition_info=None):
        ...
        if shape[0] != shape[1]:
            raise ValueError(
                f"Kernel sizes, specified by shape[0]={shape[0]} and "
    ...
    def _dict_to_tensor(self, x, k1, k2):
        ...
        return array_ops.stack(
            [array_ops.stack([x[i, j] for j in range(k2)]) for i in range(k1)]
        )
    ...
    def _matrix_conv(self, m1, m2):
        ...
        if n != (m2[0, 0]).shape.as_list()[0]:
            raise ValueError(
                "The entries in matrices m1 and m2 must have the same "
                ...
                for index1 in range(min(k, i + 1)):
                    for index2 in range(min(k, j + 1)):
                        if (i - index1) < l and (j - index2) < l:
                            result[i, j] += math_ops.matmul(
                                m1[index1, index2], m2[i - index1, j - index2]
                            )
        return result
    ...
    def _orthogonal_kernel(self, ksize, cin, cout):
        ...
        if cin > cout:
            raise ValueError(
                f"The number of input channels (cin={cin}) cannot exceed"
...
class ConvolutionOrthogonal3D(ConvolutionOrthogonal):
    ...
    def __call__(self, shape, dtype=None, partition_info=None):
        ...
        if shape[0] != shape[1] or shape[0] != shape[2]:
            raise ValueError(
                f"Kernel sizes, specified by shape[0]={shape[0]},  "
...
@deprecation.deprecated_endpoints(
    "glorot_uniform_initializer", "initializers.glorot_uniform"
)
...
@deprecation.deprecated_endpoints(
    "glorot_normal_initializer", "initializers.glorot_normal"
)
