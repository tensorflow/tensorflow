...
class _MultiMetaclass(type(_ResourceSummaryWriter), type(tracking.TrackableResource)):
    pass


class _TrackableResourceSummaryWriter(
    _ResourceSummaryWriter, tracking.TrackableResource, metaclass=_MultiMetaclass
):
...
def initialize(graph=None, session=None):  # pylint: disable=redefined-outer-name
    ...
    if session is None:
        ...
        if session is None:
            raise ValueError(
                "Argument `session must be passed if no default " "session exists"
            )
        ...
        with ops.init_scope():
            ...
            def create_fn():
                ...
                if context.executing_eagerly():
                    ...
                    shared_name = ops.name_from_scope_name(
                        scope
...
def write(tag, tensor, step=None, metadata=None, name=None):
    ...
    with ops.name_scope(name, "write_summary") as scope:
        ...
        def record():
            ...
            with ops.device("cpu:0"):
                summary_tensor = (
                    tensor() if callable(tensor) else array_ops.identity(tensor)
                )
        ...
        if not context.executing_eagerly():
            ops.add_to_collection(
                ops.GraphKeys._SUMMARY_COLLECTION, op
            )  # pylint: disable=protected-access
        return op
...
def image(name, tensor, bad_color=None, max_images=3, family=None, step=None):
    ...
    def function(tag, scope):
        bad_color_ = (
            constant_op.constant([255, 0, 0, 255], dtype=dtypes.uint8)
...
def graph(graph_data):
    ...
    with ops.device("cpu:0"):
        ...
        if isinstance(graph_data, (ops.Graph, graph_pb2.GraphDef)):
            ...
            raise ValueError(
                "Argument 'graph_data' is not tf.Graph or "
...
def import_event(tensor, name=None):
    ...
    return gen_summary_ops.import_event(
        _summary_state.writer._resource, tensor, name=name
    )  # pylint: disable=protected-access
...
def run_metadata_graphs(name, data, step=None):
    ...
    data = config_pb2.RunMetadata(
        function_graphs=data.function_graphs, partition_graphs=data.partition_graphs
    )

    with summary_scope(name, "graph_run_metadata_graph_summary", [data, step]) as (
        tag,
        _,
    ):
        with ops.device("cpu:0"):
...
def trace_export(name, step=None, profiler_outdir=None):
    ...
    with _current_trace_context_lock:
        if _current_trace_context is None:
            raise ValueError(
                "Must enable trace before export through " "tf.summary.trace_on."
            )
...
def trace_off():
    ...
    with _current_trace_context_lock:
        ...
        (
            graph,
            profiler,
        ) = _current_trace_context  # pylint: disable=redefined-outer-name, unpacking-non-sequence
        _current_trace_context = None
