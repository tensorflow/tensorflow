...
class ResourceCondTest(test_util.TensorFlowTestCase):
    ...
    def testBasic(self):
        ...
        output = cond.cond(training, TestFn, lambda: inputs)
...
class CustomGradientTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        [
            dict(testcase_name="Eager", decorator=lambda f: f),
            dict(testcase_name="Function", decorator=def_function.function),
        ]
    )
    def testCustomGradientRaggedTensor(self, decorator):
        with context.eager_mode():
            ...
            self.assertAllEqual(out, [[1.0, 4.0], [9.0]])
            self.assertIsInstance(result, ragged_tensor.RaggedTensor)
            self.assertAllEqual(result, [[3.0, 3.0], [3.0]])
    ...
    def testCustomGradientMultipleRaggedTensors(self, decorator):
        with context.eager_mode():
            ...
            rt1 = ragged_factory_ops.constant([[1.0, 2.0], [3.0]])
            rt2 = ragged_factory_ops.constant([[4.0], [5.0, 6.0]])
            with backprop.GradientTape() as tape:
                tape.watch((rt1, rt2))
    ...
    def testRVGradientsDynamicCond(self):
        with self.cached_session():
            ...
            output = cond.cond(conditional, lambda: alpha * 2, lambda: alpha * 3)
...
class VariablesGradientTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testRecomputeGradZeroSizeInput(self):
        ...
        f_graph = def_function.function(F, input_signature=[tensor.TensorSpec(None)])
