...
def _slice_ragged_row_dimension(rt_input, row_key):
    ...
    if row_key.step is None or row_key.step == 1:
        ...
        new_splits = array_ops.concat(
            [zero_pad[array_ops.size(new_starts) :], new_starts[:1], new_limits], axis=0
        )
        values_start = new_splits[0]
        values_limit = new_splits[-1]
        return ragged_tensor.RaggedTensor.from_row_splits(
            rt_input.values[values_start:values_limit],
            new_splits - values_start,
            validate=False,
        )
    ...
    # collect the necessary elements of `ragged.values(rt_input)`.
        ...
        return _build_ragged_tensor_from_value_ranges(
            new_starts, new_limits, 1, rt_input.values
        )
...
def _ragged_getitem_inner_dimensions(rt_input, key_list):
    ...
    if isinstance(column_key, slice):
        if (
            ...
            if not (
                isinstance(column_key.start, (ops.Tensor, int, type(None)))
                and isinstance(column_key.stop, (ops.Tensor, int, type(None)))
            ):
    ...
    oob_checks = [
        check_ops.assert_greater_equal(column_key, -row_length, message=oob_err_msg),
        check_ops.assert_less(column_key, row_length, message=oob_err_msg),
    ]
    with ops.control_dependencies(oob_checks):
        offset = _if_ge_zero(
            column_key, lambda: column_key, lambda: row_length + column_key
        )
...
def _build_ragged_tensor_from_value_ranges(starts, limits, step, values):
    ...
    value_indices = ragged_math_ops.range(
        starts, limits, step, row_splits_dtype=starts.dtype
    )
