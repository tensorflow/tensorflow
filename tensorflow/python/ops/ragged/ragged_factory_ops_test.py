...
class RaggedFactoryOpsTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testOpsWithDistributedDataset(self, string_factory, drop_remainder):
        ...
        self.assertAllEqual(
            [[5000, 3000, 5000, 3000][: expect_length[0]]],
            self.evaluate(result[0].values[0]),
        )
        self.assertAllEqual(
            [[9000, 9000, 9000, 9000][: expect_length[1]]],
            self.evaluate(result[0].values[1]),
        )
        self.assertAllEqual(
            [[0, 3000, 2000, 9000][: expect_length[2]]],
            self.evaluate(result[1].values[0]),
        )
        self.assertAllEqual(
            [[2000, 9000, 9000, 9000][: expect_length[3]]],
            self.evaluate(result[1].values[1]),
        )
        self.assertAllEqual(
            [[9000, 9000, 9000, 9000][: expect_length[4]]],
            self.evaluate(result[2].values[0]),
        )
        self.assertAllEqual(
            [[5000, 3000, 5000, 3000][: expect_length[5]]],
            self.evaluate(result[2].values[1]),
        )
        self.assertAllEqual(
            [[5000, 3000, 9000, 9000][: expect_length[6]]],
            self.evaluate(result[3].values[0]),
        )
        self.assertAllEqual(
            [[2000, 3000, 5000, 3000][: expect_length[7]]],
