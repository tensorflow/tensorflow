...
class RaggedShape(object):
    ...
    * `row_partitions`: A list of `RowPartition` objects, describing how
    ...
    def __init__(self, row_partitions, inner_shape, dtype=None, validate=False):
        ...
        for x in row_partitions:
            if not isinstance(x, RowPartition):
                raise TypeError(
                    "row_partitions contains " + str(x) + " which is not a RowPartition"
                )
        dtype = _find_dtype_iterable(row_partitions, dtype)
        dtype = _find_dtype(inner_shape, dtype)
        if (
            isinstance(inner_shape, np.ndarray)
            and inner_shape.dtype == np.int32
            and dtype is None
        ):
            dtype = dtypes.int32
        ...
        if self._row_partitions:
            for axis, row_partition in enumerate(self._row_partitions):
                if axis > 0:
                    ...
                    msg = (
                        "RowPartitions in RaggedShape do not align "
    ...
    def __repr__(self):
        ...
        return "<RaggedShape " "lengths=%s num_row_partitions=%r>" % (
            lengths,
    ...
    def is_uniform(self, axis):
        ...
        if axis < 0:
            ...
            return (
                axis == 0 or axis > len(self._row_partitions)
    ...
    def _alt_inner_shape(self, new_inner_rank):
        ...
        if new_inner_rank == 0:
            ...
            return array_ops.concat(
                [array_ops.stack(new_dims), self.inner_shape[1:]], axis=0
            )
...
def broadcast_dynamic_shape(shape_x: RaggedShape, shape_y: RaggedShape) -> RaggedShape:
...
def broadcast_dynamic_shape_extended(
    ...
    if a.rank is None or b.rank is None:
        ...
        [a_layer, b_layer, target] = _broadcast_dynamic_shape_one_layer(
            a.inner_shape, b.inner_shape
        )
        target_shape = RaggedShape._from_inner_shape(
            target
        )  # pylint: disable=protected-access
        return (
            target_shape,
            _Broadcaster(a, target_shape, [a_layer]),
            _Broadcaster(b, target_shape, [b_layer]),
        )

    if a.rank > b.rank:
        (c, bc, ac) = _broadcast_dynamic_shape_extended_helper(
            b, a
...
class _Broadcaster(object):
    ...
    def __repr__(self):
        return (
            "{src_shape:"
            + str(self._source_shape)
            + ", target_shape:"
            + str(self._target_shape)
            + " layer_broadcasters: "
    ...
    def with_dtype(self, dtype):
        ...
        return _Broadcaster(
            self._source_shape, self._target_shape, self._layer_broadcasters, dtype
        )
    ...
    def broadcast_flat_values(self, rt, inner_dimensions=True):
        ...
        if inner_rank > self._source_shape.rank:
            ...
            if self._source_shape.inner_rank != inner_rank:
                rt = array_ops.reshape(
                    rt, self._source_shape._alt_inner_shape(inner_rank)
                )  # pylint:disable=protected-access
    ...
    def broadcast(self, rt):
        ...
        return self.target_shape._add_row_partitions(
            flat_values
        )  # pylint:disable=protected-access
...
def _get_layer_broadcasters_from_rps(zero_broadcaster, source_rps, target_rps):
    ...
    tail_broadcasters = _get_layer_broadcasters_from_rps(
        next_broadcaster, source_rps[1:], target_rps[1:]
    )
...
def _get_broadcaster(source_shape, target_shape):
    ...
    if source_shape.rank is None or target_shape.rank is None:
        ...
        layer = _LayerBroadcaster.first_layer(
            source_shape.inner_shape[0], target_shape.inner_shape[0]
        )
    ...
    if len(source_rps) < len(target_rps):
        ...
        zeroth_broadcaster = neg_one_broadcaster.next_layer(
            neg_one_source_rp, neg_one_target_rp
        )
        target_rps_tail = target_rps[-len(source_rps) :] if len(source_rps) >= 1 else []

        layers = _get_layer_broadcasters_from_rps(
            zeroth_broadcaster, source_rps, target_rps_tail
        )
        return _Broadcaster(source_shape, target_shape, layers)
    else:
        assert len(target_rps) == len(source_rps)
        zeroth_broadcaster = _LayerBroadcaster.first_layer(
            source_rps[0].nrows(), target_rps[0].nrows()
        )
        layers = _get_layer_broadcasters_from_rps(
            zeroth_broadcaster, source_rps, target_rps
        )
...
def _get_identity_broadcaster(shape):
    ...
    layers = [
        _LayerBroadcaster.get_identity_broadcaster(shape._num_slices_in_dimension(i))
        for i in range(shape.rank)  # pylint: disable=protected-access
    ]
...
def _broadcast_dynamic_shape_one_layer(a, b):
    ...
    can_broadcast = math_ops.logical_or(
        can_broadcast_from_a, math_ops.logical_or(can_broadcast_from_b, nrows_equal)
    )
    ...
    results = control_flow_ops.cond(
        can_broadcast_from_a, true_fn=broadcast_from_a, false_fn=broadcast_not_from_a
    )

    results = [
        control_flow_ops.with_dependencies([check_can_broadcast], x) for x in results
    ]
    ...
    return [
        _LayerBroadcaster.from_gather_index(
            control_flow_ops.with_dependencies(
                [check_ops.assert_equal(can_broadcast, True)], x
            )
        )
        for x in result
    ]
...
def _broadcast_half(
    ac_0: _LayerBroadcaster, a_1: RowPartition
) -> Tuple[_LayerBroadcaster, RowPartition]:
...
def _broadcast_dynamic_shape_next_layer_half_ragged(
    ...
    [c_1_row_splits, ac_1_gather_index, bc_1_gather_index] = control_flow_ops.cond(
    ...
    broadcast_asserts = [
        check_ops.assert_equal(
            math_ops.logical_or(
                ...
                math_ops.equal(a_1.uniform_row_length(), b_1.uniform_row_length()),
            ),
            True,
        )
    ]
    ...
    if a_1.is_uniform():
        ...
        if b_1.is_uniform():
            [
                ...
                bc_1,
                ac_1,
            ] = _broadcast_dynamic_shape_next_layer_half_ragged(  # pylint: disable=arguments-out-of-order
                bc_0, ac_0, b_1, a_1
            )
            return (c_1, ac_1, bc_1)
        else:
            ...
            return (
                c_1a.with_dependencies(check_valid),
...
def _broadcast_dynamic_shape_from_rps(
    a_zero: _LayerBroadcaster,
    b_zero: _LayerBroadcaster,
    a_rps: Sequence[RowPartition],
    b_rps: Sequence[RowPartition],
) -> Tuple[
    Sequence[RowPartition], Sequence[_LayerBroadcaster], Sequence[_LayerBroadcaster]
]:
    ...
    if a_rps:
        (c_1, ac_1, bc_1) = _broadcast_dynamic_shape_next_layer(
            a_zero, b_zero, a_rps[0], b_rps[0]
        )
        (c_suffix, a_layers, b_layers) = _broadcast_dynamic_shape_from_rps(
            ac_1, bc_1, a_rps[1:], b_rps[1:]
        )
...
def _broadcast_dynamic_shape_extended_complete(
    ...
    b: RaggedShape,
    b_rps: Sequence[RowPartition],
    c_suffix: Sequence[RowPartition],
    ac: Sequence[_LayerBroadcaster],
    ...
    c = RaggedShape.from_row_partitions(
        c_prefix + tuple(c_suffix)
    )._with_num_row_partitions(c_num_row_partitions)
...
def _broadcast_dynamic_shape_extended_helper(
    ...
    if len(a_rps) < len(b_rps):
        ...
        (c_zero, ac_zero, bc_zero) = _broadcast_dynamic_shape_next_layer(
            neg_one_ac, neg_one_bc, neg_one_a_rp, neg_one_b_rp
        )
        b_rps_tail = b_rps[-len(a_rps) :] if len(a_rps) >= 1 else []

        (c_suffix, ac_layers, bc_layers) = _broadcast_dynamic_shape_from_rps(
            ac_zero, bc_zero, a_rps, b_rps_tail
        )
        ...
        assert len(a_rps) == len(b_rps)
        (ac_zero, bc_zero) = _broadcast_dynamic_shape_first_layer(
            a_rps[0].nrows(), b_rps[0].nrows()
        )

        (c_rps, a_layers, b_layers) = _broadcast_dynamic_shape_from_rps(
            ac_zero, bc_zero, a_rps, b_rps
        )
        return _broadcast_dynamic_shape_extended_complete(
            a=a,
            b=b,
...
def _first_layer_gather_index(nrows_source, nrows_target):
    ...
    do_broadcast = math_ops.equal(
        nrows_source, constant_op.constant(1, nrows_source.dtype)
    )
    nrows_equal = math_ops.equal(nrows_source, nrows_target)
    can_broadcast = check_ops.assert_equal(
        math_ops.logical_or(do_broadcast, nrows_equal), True, message="Cannot broadcast"
    )
...
def _next_layer_gather_index(bc, original_rp, broadcast_rp):
    ...
    do_broadcast = math_ops.equal(
        original_rp.uniform_row_length(), constant_op.constant(1, original_rp.dtype)
    )
    gather_index = control_flow_ops.cond(
...
) -> Tuple[Sequence[RowPartition], int]:
