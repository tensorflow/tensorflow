...
class RaggedMapInnerValuesOpTest(test_util.TensorFlowTestCase):
    ...
    def testOpWithSingleRaggedTensorArg(self):
        ...
        self.assertRaggedMapInnerValuesReturns(
            op=array_ops.zeros_like, args=(tensor,), expected=[[0, 0, 0], [], [0, 0]]
        )
    ...
    def testOpWithRaggedTensorListArg(self):
        ...
        self.assertRaggedMapInnerValuesReturns(
            op=math_ops.add_n, args=([x, y, x],), expected=[[12, 24, 36], [], [48, 60]]
        )
    ...
    def testOpWithRaggedRankGreaterThanOne(self):
        ...
        # ragged_rank=2
        x2 = ragged_factory_ops.constant(
            [[[3, 1, 4]], [], [[], [1, 5]], [[9, 2], [6, 5]]]
        )
        y2 = ragged_factory_ops.constant(
            [[[1, 2, 3]], [], [[], [4, 5]], [[6, 7], [8, 9]]]
        )
        self.assertRaggedMapInnerValuesReturns(
            op=math_ops.multiply,
        ...
        # ragged_rank=3
        x3 = ragged_factory_ops.constant(
            [[[[3, 1, 4]], []], [], [[[], [1, 5]]], [[[9, 2], [6, 5]]]]
        )
        y3 = ragged_factory_ops.constant(
            [[[[1, 2, 3]], []], [], [[[], [4, 5]]], [[[6, 7], [8, 9]]]]
        )
        self.assertRaggedMapInnerValuesReturns(
            op=math_ops.multiply,
    ...
    def testRaggedTensorSplitsValueMismatchError(self):
        ...
        y = ragged_factory_ops.constant([[1], [2, 3], [4, 5]])
        with self.assertRaisesRegex(
            (ValueError, errors.InvalidArgumentError), r"partitions have incompatible"
        ):
            ragged_functional_ops.map_flat_values(math_ops.add, x, y)
