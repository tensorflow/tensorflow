...
class RaggedSegmentStackOpTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.parameters(
        [
            ...
            dict(  # empty data, num_partitions>0
                data=[], partitions=[], num_partitions=3, expected=[[], [], []]
            ),
    ...
    def testRaggedSegmentStack(
        ...
        for seg_dtype in [dtypes.int32, dtypes.int64]:
            ...
            expected_tensor = ragged_factory_ops.constant(
                expected, row_splits_dtype=seg_dtype, ragged_rank=expected_ragged_rank
            )
            ...
            if (
                data_ragged_rank == 0
                and segment_ids_ragged_rank == 0
                and seg_dtype == dtypes.int32
            ):
                equiv = ragged_concat_ops.stack(
                    data_flow_ops.dynamic_partition(
                        data_tensor, segment_ids_tensor, num_partitions
                    )
                )
    ...
    def testUnknownRankError(self):
        ...
        with self.assertRaisesRegex(
            (ValueError, errors.InvalidArgumentError), "partitions must have known rank"
        ):
