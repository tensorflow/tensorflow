...
def _reshape(x, shape: RaggedShape):
    ...
    return RaggedTensor._from_nested_row_partitions(flat_values, shape.row_partitions)


def _to_row_partitions_from_lengths(
    lengths: Sequence[Union[int, Sequence[int]]]
) -> Sequence[RowPartition]:
    ...
    """
    (result, _) = ragged_shape._to_row_partitions_and_nvals_from_lengths(lengths)
    return result


def _to_ragged_tensor_from_lengths(
    values, lengths: Sequence[Union[int, Sequence[int]]]
) -> RaggedTensor:
...
def _num_elements_of_lengths_with_rows(
    rows, lengths: Sequence[Union[int, Sequence[int]]]
):
...
class RaggedShapeTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def assertLayerBroadcasterEq(
        self, x: _LayerBroadcaster, y: _LayerBroadcaster
    ) -> None:
    ...
    def assertBroadcasterEq(
        self, x: ragged_shape._Broadcaster, y: ragged_shape._Broadcaster
    ) -> None:
    ...
    @parameterized.parameters(
        [
            ...
            dict(
                ...
                row_partitions=lambda: [
                    RowPartition.from_row_lengths([3, 2]),
            ...
            dict(
                ...
                num_row_partitions=2,
                row_partitions=lambda: [
                    RowPartition.from_uniform_row_length(2, 4),
    ...
    def testFromLengths(
        self, lengths, row_partitions, inner_shape, num_row_partitions=None
    ):
        if callable(row_partitions):
            row_partitions = row_partitions()
        shape = RaggedShape.from_lengths(lengths, num_row_partitions=num_row_partitions)
        expected = RaggedShape(row_partitions, inner_shape)
        self.assertShapeEq(shape, expected)
    ...
    @parameterized.parameters(
        [
            dict(
                lengths=[2, (2, 1, 3)], num_row_partitions=1, msg="Shape not consistent"
            ),
            dict(
                lengths=[2, 3],
                num_row_partitions=2,
    ...
    def testGetBroadcaster2(self):
        ...
        dest_shape = RaggedShape(
            [RowPartition.from_row_splits([0, 2, 3])], inner_shape=[3]
        )
        actual = ragged_shape._get_broadcaster(origin_shape, dest_shape)
    ...
    @parameterized.parameters(
        [
            dict(lengths=[2, (1, 2), 4], new_dense_rank=2, expected_inner_shape=[3, 4]),
        ]
    )
    ...
    def testWithNumRowPartitionsRaises(
        self, lengths, num_row_partitions, error_type, error_regex
    ):
        original = RaggedShape.from_lengths(lengths)
        with self.assertRaisesRegex(error_type, error_regex):
    ...
    def testGetItemRaisesStatic(
        self, lengths, index, error_type, error_regex, num_row_partitions=None
    ):
        original = RaggedShape.from_lengths(lengths)
        if num_row_partitions is not None:
            original = original._with_num_row_partitions(num_row_partitions)
    ...
    def testBroadcastToAlt(self):
        ...
        expected = RaggedTensor.from_uniform_row_length(
            [3, 3, 4, 4, 5, 5], uniform_row_length=2
        )
    ...
    def testBroadcaster(
        ...
        expected = ragged_shape._Broadcaster(source, target, expected_gather_indices)
        self.assertBroadcasterEq(actual, expected)
    ...
    def testRaggedGradientSimple1(self):
        ...
        self.assertAllClose(
            ops.convert_to_tensor(g),
            [10.0, 10.0, 10.0, 10.0, 100.0, 100.0, 100, 1000.0],
        )
    ...
    def testRaggedGradientSimple2(self):
        ...
        def func(x):
            rt1 = RaggedTensor._from_row_partition(
                x, RowPartition.from_row_splits(row_splits=[0, 4, 7, 8], validate=False)
            )
            rt2 = rt1 * [[10], [100], [1000]]
            return rt2.flat_values
        ...
        def func(x):
            rt1 = RaggedTensor._from_row_partition(
                ...
                RowPartition.from_row_splits(
                    row_splits=[0, 4, 7, 8], dtype=dtypes.int32, validate=False
                ),
            )
            rt2 = rt1 * [[10], [100], [1000]]
            return rt2.flat_values
    ...
    def testRaggedMul(self):
        ...
        rt1 = RaggedTensor._from_row_partition(
            x,
            RowPartition.from_row_splits(
                row_splits=[0, 4, 7, 8], dtype=dtypes.int64, validate=False
            ),
        )
        rt2 = rt1 * [[10], [100], [1000]]
        self.assertAllClose(
            rt2.flat_values, [30.0, 10.0, 40.0, 10.0, 100.0, 0.0, 200.0, 1000.0]
        )
    ...
    def testIsUniformRaises(
        self, lengths, num_row_partitions, axis, error_type, error_regex
    ):
        shape = RaggedShape.from_lengths(lengths)._with_num_row_partitions(
            num_row_partitions
        )
    ...
    def testWithNumRowPartitions(
        self, lengths, num_row_partitions_a, num_row_partitions_b
    ):
        shape = RaggedShape.from_lengths(lengths)
    ...
    @parameterized.parameters(
        [
            ...
            dict(
                lengths=[2, (2, 3), 7, 4, 3], num_row_partitions=2, axis=4, expected=3
            ),
    ...
    def testBroadcastTensorTo(
        self, origin, expected_lengths, expected, expected_num_row_partitions=None
    ):
    ...
    def testBroadcastFlatValues(self):
        ...
        origin_shape = RaggedShape.from_lengths(
            origin_lengths
        )._with_num_row_partitions(3)
        dest_shape = RaggedShape.from_lengths(dest_lengths)._with_num_row_partitions(5)
    ...
    def testBroadcastRaggedTo(
        self, origin_lengths, origin_values, expected_lengths, expected_values
    ):
    ...
    def testRaggedShapeFromTensor2(self):
        ...
        expected_shape = RaggedShape.from_lengths(
            [3, (2, 1, 1), 2, 2]
        )._with_num_row_partitions(3)
    ...
    def testConstructorRaisesStatic(
        self,
        row_partitions,
        inner_shape,
        error_type,
    ...
    def testBroadcastDynamicShapeExtended(
        self,
        lengths_a,
        lengths_b,
        lengths_e,
        ...
        [actual, bc_a, bc_b] = ragged_shape.broadcast_dynamic_shape_extended(
            shape_a, shape_b
        )
        [
            actual_rev,
        ...
        rt_a = _reshape(_lowest_primes(_num_elements_of_lengths(lengths_a)), shape_a)
        ...
        rt_b = _reshape(_lowest_primes(_num_elements_of_lengths(lengths_b)), shape_b)
    ...
    @parameterized.parameters(
        [
            ...
            dict(original_lengths=[1, 4, 5], broadcast_lengths=[3, 4, 5]),
            # shape: [UNIFORM, UNIFORM, BROADCAST(UNIFORM)]
            dict(original_lengths=[3, 4, 1], broadcast_lengths=[3, 4, 5]),
            # shape: [UNIFORM, RAGGED, BROADCAST(UNIFORM)]
            dict(
                original_lengths=[3, (3, 2, 8), 1], broadcast_lengths=[3, (3, 2, 8), 5]
            ),
            # shape: [UNIFORM, RAGGED, RAGGED, UNIFORM, UNIFORM, BROADCAST(UNIFORM)]
            dict(
                original_lengths=[2, (2, 1), (3, 2, 8), 3, 4, 1],
            ...
            dict(original_lengths=[3, 1], broadcast_lengths=[3, (2, 0, 1)]),
            # shape: [UNIFORM, BROADCAST(UNIFORM), UNIFORM]
            dict(original_lengths=[3, 1, 5], broadcast_lengths=[3, (2, 0, 1), 5]),
            # shape: [UNIFORM, UNIFORM, BROADCAST(UNIFORM)]
            dict(
                original_lengths=[4, 3, 1],
                broadcast_lengths=[4, 3, (2, 0, 1, 3, 8, 2, 3, 4, 1, 8, 7, 0)],
            ),
            # shape: [UNIFORM, RAGGED, BROADCAST(UNIFORM)]
            dict(
                original_lengths=[2, (2, 1), 1],
                broadcast_lengths=[2, (2, 1), (2, 5, 3)],
            ),
            # shape: [UNIFORM, RAGGED, UNIFORM, UNIFORM, BROADCAST(UNIFORM), UNIFORM]
            dict(
                original_lengths=[2, (2, 1), 3, 2, 1, 8],
            ...
            dict(original_lengths=[1, (5,)], broadcast_lengths=[3, (5, 5, 5)]),
            # shape: [BROADCAST(UNIFORM), UNIFORM, RAGGED]
            dict(
                original_lengths=[1, 3, (3, 0, 2)],
                broadcast_lengths=[2, 3, (3, 0, 2, 3, 0, 2)],
            ),
            # shape: [BROADCAST(UNIFORM), RAGGED, RAGGED, UNIFORM, UNIFORM]
            dict(
                original_lengths=[1, (3,), (3, 5, 2), 9, 4, 5],
                broadcast_lengths=[3, (3, 3, 3), (3, 5, 2, 3, 5, 2, 3, 5, 2), 9, 4, 5],
            ),
            # shape: [BROADCAST(UNIFORM), UNIFORM, RAGGED, UNIFORM]
            dict(
                original_lengths=[1, 2, (2, 1), (3, 5, 2), 2],
            ...
            dict(
                ...
                broadcast_lengths=[
                    ...
                    (1, 0, 1, 1, 0, 1, 1, 0, 1, 1, 0, 1, 0, 2, 3, 3),  # axis=4
                    5,
                ],
            ),  # axis=5
            dict(
                original_lengths=[1, 1, 2, (2, 1)],
                broadcast_lengths=[2, 1, 2, (2, 1, 2, 1)],
            ),
            dict(
                original_lengths=[2, 1, 2, (2, 1, 2, 1)],
                broadcast_lengths=[2, (2, 1), 2, (2, 1, 2, 1, 2, 1)],
            ),
            dict(
                original_lengths=[2, (2, 1), 2, (2, 1, 2, 1, 2, 1)],
                broadcast_lengths=[2, (2, 1), 2, (2, 1, 2, 1, 2, 1)],
            ),
            dict(
                original_lengths=[2, (2, 1), 2, 1],
                broadcast_lengths=[2, (2, 1), 2, (2, 1, 2, 1, 2, 1)],
            ),
        ]
    )  # pyformat: disable
    ...
    def testAsRowPartitionsRaises(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "rank must be >= 1 for _as_row_partitions"
        ):
    ...
    @parameterized.parameters(
        [
            ...
            dict(x_dims=[3], y_dims=[3, (2, 3, 1), 1], expected_dims=[3, (2, 3, 1), 3]),
            dict(x_dims=[1], y_dims=[3, (2, 1, 3)], expected_dims=[3, (2, 1, 3)]),
            dict(x_dims=[1], y_dims=[3, (2, 1, 3), 8], expected_dims=[3, (2, 1, 3), 8]),
            dict(
                x_dims=[1],
                y_dims=[2, (2, 3), (5, 7, 2, 0, 9)],
                expected_dims=[2, (2, 3), (5, 7, 2, 0, 9)],
    ...
    def testBroadcastDynamicShapeFirstLayer(self):
        ...
        [a_layer, b_layer] = ragged_shape._broadcast_dynamic_shape_first_layer(a_0, b_0)
    ...
    @parameterized.parameters(
        [
            ...
            dict(
                ...
                expected=ragged_factory_ops.constant_value(
                    [[[], [1, 1]], [[2], [2, 2], []]], dtype=np.int32, ragged_rank=2
                ),
            ),
            dict(
                x=10,
            ...
            dict(
                x=ragged_factory_ops.constant_value([[[1], [2]], [[3]]], ragged_rank=1),
                lengths=[2, (2, 1), 2],
                expected=ragged_factory_ops.constant_value(
    ...
    @parameterized.parameters(
        [
            ...
            dict(
                doc=(
                    "x.shape=[2, (D1), 1]; y.shape=[1, (D2)]; "
                    "bcast.shape=[2, (D1), (D2)]"
                ),
                x=ragged_factory_ops.constant_value(
                    [[[1], [2], [3]], [[4]]], ragged_rank=1
                ),
            ...
            dict(
                doc=(
                    "x.shape=[2, (D1), 1]; y.shape=[1, 1, 4]; "
                    "bcast.shape=[2, (D1), 4]"
                ),
                x=ragged_factory_ops.constant_value(
                    [[[10], [20]], [[30]]], ragged_rank=1
                ),
                y=[[[1, 2, 3, 4]]],
                expected=ragged_factory_ops.constant_value(
    ...
    @parameterized.parameters(
        [
            ...
            dict(lengths=[2, (1, 2), 2], num_row_partitions=1, expected=[2, (1, 2), 2]),
        ]
    )
    def testStaticLengths(
        self, lengths, num_row_partitions, expected, expected_eager=None
    ):
    ...
    def testBroadcasterRepr(self):
        ...
        bc = ragged_shape._Broadcaster(source_shape, target_shape, layer_broadcasters)
        actual = str(bc)
    ...
    @parameterized.parameters(
        [
            dict(dtype_left=dtypes.int64, dtype_right=dtypes.int32),
            dict(dtype_left=dtypes.int32, dtype_right=dtypes.int64),
        ]
    )
    def testBroadcastWithDifferentDenseShapeDTypes(self, dtype_left, dtype_right):
        s_left = RaggedShape._from_inner_shape(constant_op.constant([4, 1], dtype_left))
        s_right = RaggedShape._from_inner_shape(
    ...
    def testBroadcastDynamicShapeNextLayerRaises(
        self, ac_0, bc_0, a_1, b_1, error_type, error_regex
    ):
        ac_0 = ac_0()
        bc_0 = bc_0()
        ...
        b_1 = b_1()
        with self.assertRaisesRegex(error_type, error_regex):
            ragged_shape._broadcast_dynamic_shape_next_layer(ac_0, bc_0, a_1, b_1)
...
class RaggedShapeErrorTest(parameterized.TestCase):
    ...
    def testBroadcastRaggedError(self, origin_lengths, origin_values, expected_lengths):
        ...
        # but it exists.
        with self.assertRaisesRegex(
            errors_impl.InvalidArgumentError, r"Cannot broadcast"
        ):
    ...
    def testConstructorRaisesDynamic(
        self, row_partitions, inner_shape, error_regex, validate=False, dtype=None
    ):
        with self.assertRaisesRegex(
            (errors_impl.InvalidArgumentError, ValueError), error_regex
        ):
            sess = session.Session()
            with sess.as_default():
                row_partitions = row_partitions()
