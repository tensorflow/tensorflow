...
class RaggedToStringTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        [
            ...
            dict(testcase_name="2d_empty_row", rt=[[]], ragged_rank=1, expected="[[]]"),
            dict(testcase_name="3d_empty_row", rt=[[]], ragged_rank=2, expected="[[]]"),
            dict(
    ...
    def testRaggedToStringUnknownRank(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "RaggedTensor to_string requires " "that rt.shape.rank is not None",
        ):
