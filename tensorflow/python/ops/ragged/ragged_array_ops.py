...
def split(value: ragged_tensor.Ragged, num_or_size_splits, axis=0, num=None, name=None):
    ...
    with ops.name_scope(name, "RaggedSplit"):
        ...
        check_ops.assert_integer_v2(
            num_or_size_splits,
            message=(
                "`num_or_size_splits` must be an `int` or 1-D list or "
                "`Tensor` of integers."
            ),
        )
        ...
        checks.append(
            check_ops.assert_non_negative_v2(
                num_or_size_splits, message="`num_or_size_splits` must be non-negative."
            )
        )
        checks.append(
...
def ragged_reshape(
    tensor: ragged_tensor.RaggedOrDense, shape: dynamic_ragged_shape.DenseOrRaggedShape
    ...
    if isinstance(shape, dynamic_ragged_shape.DynamicRaggedShape):
        ...
        return ragged_tensor.RaggedTensor._from_nested_row_partitions(  # pylint: disable=protected-access
            flat_values, shape.row_partitions, validate=False
        )
    else:
...
def bitcast(
    ...
    with ops.name_scope(name, "Bitcast", [input]):
        ...
        if input.dtype.size < type.size and input.flat_values.shape.rank < 2:
            raise ValueError(
                "`input.flat_values` is required to have rank >= 2 when "
