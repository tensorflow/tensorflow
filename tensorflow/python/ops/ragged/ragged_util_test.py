...
TENSOR_3D = [
    [[("%d%d%d" % (i, j, k)).encode("utf-8") for k in range(3)] for j in range(2)]
    for i in range(4)
]
...
TENSOR_4D = [
    [
        [
            [("%d%d%d%d" % (i, j, k, l)).encode("utf-8") for l in range(5)]
            for k in range(3)
        ]
        for j in range(2)
    ]
...
class RaggedUtilTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testError(
        self, descr, mode, data, repeats, axis, exception=ValueError, error=None
    ):
