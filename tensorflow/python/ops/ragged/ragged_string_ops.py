...
def reduce_join(
    inputs: ragged_tensor.Ragged, axis=None, keepdims=None, separator="", name=None
):
...
def string_format(
    template: str,
    inputs: typing.Union[
        ragged_tensor.Ragged, typing.List[ragged_tensor.RaggedOrDense]
    ],
    placeholder="{}",
    summarize=3,
    name=None,
    ...
    if len(inputs) != len(split_template) - 1:
        raise ValueError(
            "num placeholders in template and num inputs must match: {} vs {}".format(
                len(split_template) - 1, len(inputs)
            )
        )
...
def string_join(
    inputs: typing.List[ragged_tensor.RaggedOrDense], separator="", name=None
):
    ...
    with ops.name_scope(name, "RaggedStringJoin", inputs):
        return ragged_functional_ops.map_flat_values(
            string_ops.string_join, inputs, separator
        )
