...
class RaggedRowLengthsOp(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.parameters(
        [
            ...
            dict(
                rt_input=[["a"], ["b", "c", "d"], ["e"], [], ["f"]], axis=0, expected=5
            ),
            dict(rt_input=[["a", "b", "c", "d", "e", "f", "g"]], expected=[7]),
            dict(
                rt_input=[[], ["a", "b", "c", "d", "e", "f", "g"], []],
                expected=[0, 7, 0],
            ),
            dict(rt_input=[], ragged_rank=1, expected=[]),
            dict(rt_input=[], ragged_rank=1, axis=0, expected=0),
    ...
    def testRowLengths(
        self, rt_input, expected, axis=1, ragged_rank=None, expected_ragged_rank=None
    ):
