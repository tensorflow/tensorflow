...
class RaggedFillEmptyRowsTest(test_util.TensorFlowTestCase):
  ...
  def testFillInt(self):
    with test_util.use_gpu():
      ...
      ragged_input = ragged_factory_ops.constant([[], [1, 3, 5, 7], [],
                                         [2, 4, 6, 8], []], dtype=dtypes.int32)
      ragged_output, empty_row_indicator = ragged_array_ops.fill_empty_rows(
        ragged_input, default_value)

      self.assertAllEqual(ragged_output, [[-1], [1, 3, 5, 7], [-1],
                                                 [2, 4, 6, 8], [-1]])
      self.assertAllEqual(ragged_output.row_lengths(), [1, 4, 1, 4, 1])
      self.assertAllEqual(ragged_output.values,
                                [-1, 1, 3, 5, 7, -1, 2, 4, 6, 8, -1])
      self.assertAllEqual(empty_row_indicator, [True, False, True, False, True])

  @test_util.run_deprecated_v1
  def testFillFloat(self):
    with self.session():
      values = constant_op.constant([1., 3., 5., 7., 2., 4., 6., 8.],
                                    dtype=dtypes.float64)
      default_value = constant_op.constant(-1., dtype=dtypes.float64)
  ...
  def testFillString(self):
    with test_util.force_cpu():
      values = constant_op.constant(["a", "c", "e", "g", "b", "d", "f", "h"],
                                    dtype=dtypes.string)
      ...
      self.assertAllEqual(empty_row_indicator,
                          np.array([1, 0, 1, 0, 1]).astype(np.bool_))
