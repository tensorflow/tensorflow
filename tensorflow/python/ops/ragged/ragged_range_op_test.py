...
class RaggedRangeOpTest(test_util.TensorFlowTestCase):
    ...
    def testBasicRanges(self):
        ...
        self.assertAllEqual(
            ...
            [list(range(0, 4, 2)), list(range(3, 4, 3)), list(range(5, 15, 4))],
        )
    ...
    def testFloatRanges(self):
        ...
        actual = ragged_math_ops.range(
            [0.0, 3.0, 5.0], [3.9, 4.0, 15.0], [0.4, 1.5, 2.2]
        )
    ...
    def testNegativeDeltas(self):
        self.assertAllEqual(
            ...
            [list(range(0, 0, -1)), list(range(3, 0, -1)), list(range(5, 0, -1))],
        )

        self.assertAllEqual(
            ragged_math_ops.range([0, -3, 5], limits=0, deltas=[-1, 1, -2]),
            [list(range(0, 0, -1)), list(range(-3, 0, 1)), list(range(5, 0, -2))],
        )
    ...
    def testBroadcast(self):
        ...
        self.assertAllEqual(
            ...
            [list(range(0, 4, 3)), list(range(3, 4, 3)), list(range(5, 15, 3))],
        )
    ...
    def testShapeFnErrors(self):
        self.assertRaises(
            (ValueError, errors.InvalidArgumentError), ragged_math_ops.range, [[0]], 5
        )
        self.assertRaises(
            (ValueError, errors.InvalidArgumentError), ragged_math_ops.range, 0, [[5]]
        )
    ...
    def testKernelErrors(self):
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, r"Requires delta != 0"
        ):
            self.evaluate(ragged_math_ops.range(0, 0, 0))

        with self.assertRaisesRegex(
            errors.InvalidArgumentError, r"Requires \(\(limit - start\) / delta\) <="
        ):
