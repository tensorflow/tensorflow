    ...
    * `tf.RaggedTensor.from_nested_value_rowids`
    ...
    def __init__(self, values, row_partition, internal=False):
        ...
        if not isinstance(row_partition, RowPartition):
            raise TypeError(
                f"Argument `row_partition` must be a RowPartition. "
    ...
    def _from_row_partition(cls, values, row_partition, validate=True):
        ...
        if not isinstance(validate, bool):
            raise TypeError(
                f"Argument `validate` must have type bool. Received {validate}."
            )
    ...
    def from_value_rowids(
        ...
        with ops.name_scope(
            name, "RaggedFromValueRowIds", [values, value_rowids, nrows]
        ):
    ...
    def from_uniform_row_length(
        ...
        with ops.name_scope(
            name, "RaggedFromUniformRowLength", [values, uniform_row_length, nrows]
        ):
            values = _convert_to_ragged_tensor_values(values)
    ...
    def from_nested_value_rowids(
        ...
        if isinstance(nested_value_rowids, tensor_lib.Tensor):
            raise TypeError(
                f"Argument `nested_value_rowids` must be a list of "
        ...
        if nested_nrows is None:
            ...
            if isinstance(nested_nrows, tensor_lib.Tensor):
                raise TypeError(
                    f"Argument `nested_nrows` must be a list of "
    ...
    def from_nested_row_splits(
        ...
        if isinstance(nested_row_splits, tensor_lib.Tensor):
            raise TypeError(
                f"Argument `nested_row_splits` must be a list of "
                f"Tensors. Received {nested_row_splits}."
            )
        with ops.name_scope(
            name, "RaggedFromNestedRowSplits", [flat_values] + list(nested_row_splits)
        ):
            result = flat_values
    ...
    def from_nested_row_lengths(
        ...
        if isinstance(nested_row_lengths, tensor_lib.Tensor):
            raise TypeError(
                f"Argument `nested_row_lengths` must be a list of "
                f"Tensors. Received {nested_row_lengths}."
            )
        with ops.name_scope(
            name, "RaggedFromNestedRowlengths", [flat_values] + list(nested_row_lengths)
        ):
            result = flat_values
    ...
    @classmethod
    def _from_nested_row_partitions(
        cls, flat_values, nested_row_partitions, name=None, validate=True
    ):
        ...
        if isinstance(nested_row_partitions, RowPartition):
            raise TypeError(
                f"Argument `nested_row_partitions` must be a list of "
            ...
            if isinstance(axis, int):
                if axis == 0:
                    ...
                    result = math_ops.maximum(
                        math_ops.reduce_max(self.row_lengths()), 0
                    )
    ...
    def with_values(self, new_values):
        ...
        if (
            isinstance(new_values, RaggedTensor)
            and self._row_partition.dtype != new_values.row_splits.dtype
        ):
            if not ragged_config.auto_cast_partition_dtype():
                raise ValueError(
                    "self and new_values have mismatched row_splits "
    ...
    def with_row_splits_dtype(self, dtype):
        ...
        if dtype not in (dtypes.int32, dtypes.int64):
            raise ValueError(
                f"Argument `row_splits` dtype must be int32 or int64. Received {dtype}."
            )
    ...
    def merge_dims(self, outer_axis, inner_axis):
        ...
        outer_axis = array_ops.get_positive_axis(
            outer_axis, self.shape.rank, axis_name="outer_axis", ndims_name="rank(self)"
        )
        inner_axis = array_ops.get_positive_axis(
            inner_axis, self.shape.rank, axis_name="inner_axis", ndims_name="rank(self)"
        )
        if not outer_axis <= inner_axis:
            raise ValueError(
                f"Expected outer_axis ({outer_axis}) to be less than or "
    ...
    def from_tensor(
        ...
        if not isinstance(ragged_rank, int):
            raise TypeError(
                f"Argument `ragged_rank` must be an int. Received {ragged_rank}."
            )
        if ragged_rank <= 0:
            raise ValueError(
                f"Argument `ragged_rank` must be greater than 0. "
        ...
        with ops.name_scope(name, "RaggedFromTensor", [tensor, lengths, padding]):
            ...
            if tensor.shape.rank is not None and tensor.shape.rank < 2:
                raise ValueError(
                    f"The rank of a RaggedTensor must be greater than 1, "
            ...
            if (
                ...
                and len(lengths)
                and not isinstance(lengths[0], (int, float))
            ):
                if ragged_rank not in (1, len(lengths)):
                    ...
                    raise ValueError(
                        f"If Argument `lengths` is a tuple of row_lengths, "
            ...
            if padding is not None:
                ...
                length_for_nondefault_value = has_nondefault * array_ops.expand_dims(
            ...
            if lengths is not None:
                ...
                lengths = ragged_util.convert_to_int_tensor(
                    lengths, "lengths", row_splits_dtype
                )
    ...
    def to_tensor(self, default_value=None, name=None, shape=None):
        ...
        with ops.name_scope(name, "RaggedToTensor", [self, default_value, shape]):
            ...
            if (
                ...
                and any(isinstance(v, tensor_lib.Tensor) for v in shape)
                and all(isinstance(v, (int, tensor_lib.Tensor)) for v in shape)
            ):
    ...
    def from_sparse(cls, st_input, name=None, row_splits_dtype=dtypes.int64):
        ...
        if not sparse_tensor.is_sparse(st_input):
            raise TypeError(
                f"Argument `st_input` must be of type SparseTensor, but "
    ...
    def _from_variant(
        ...
        if (
            variant.shape.ndims is not None
            and input_ragged_rank is not None
            and output_ragged_rank != input_ragged_rank + variant.shape.ndims
        ):
            raise ValueError(
    ...
    def _shape_invariant_to_type_spec(self, shape):
        return RaggedTensorSpec(
            shape, self.dtype, self.ragged_rank, self.row_splits.dtype
        )
...
def match_row_splits_dtypes(*tensors, **kwargs):
    ...
    if has_int32 and has_int64:
        ...
        dtype = dtypes.int64
        tensors = tuple(
            t.with_row_splits_dtype(dtypes.int64) if isinstance(t, RaggedTensor) else t
            for t in tensors
        )
...
class RaggedTensorSpec(type_spec.BatchableTypeSpec, internal_types.RaggedTensorSpec):
    ...
    def __init__(
        ...
        if ragged_rank is None:
            if rank is None:
                raise ValueError(
                    "Must specify ragged_rank or a shape with a known rank."
                )
            ragged_rank = rank - 1
        ...
        if rank is not None:
            if ragged_rank >= rank:
                raise ValueError(
                    f"Argument `ragged_rank` ({ragged_rank}) must be less "
                    f"than rank ({rank})."
                )
    ...
    def _from_components(self, tensor_list):
        ...
        if self._shape.ndims is not None:
            if isinstance(result, RaggedTensor):
                ...
                if self.flat_values_spec is not None and hasattr(
    ...
    def _unbatch(self):
        ...
        return RaggedTensorSpec(
            self._shape[1:], self._dtype, self._ragged_rank - 1, self._row_splits_dtype
        )
    ...
    def from_value(cls, value):
        if isinstance(value, ragged_tensor_value.RaggedTensorValue) or isinstance(
            value.flat_values, tensor_lib.Tensor
        ):
            return cls(
                shape=value.shape,
...
# ===============================================================================
def convert_to_tensor_or_ragged_tensor(
    value, dtype=None, preferred_dtype=None, name=None
):
    ...
    * Otherwise, use `convert_to_tensor` to convert `value` to a `Tensor`.
    ...
    if isinstance(value, RaggedTensor):
        if dtype and not dtype.is_compatible_with(value.dtype):
            raise ValueError(
                f"Tensor conversion requested dtype {dtype.name} for "
...
def _assert_sparse_indices_are_ragged_right(indices):
    ...
    index_ok = array_ops.where(
        ...
        math_ops.equal(index_suffix[1:], 0),
...
def _ragged_tensor_to_sparse_gradient(
    op, unused_sparse_indices_grad, sparse_values_grad, unused_sparse_shape_grad
):
...
def _get_row_partition_type_tensor_pairs(rt_input):
    ...
    if partitions[0]._value_rowids is not None:  # pylint: disable=protected-access
        return [
            ("FIRST_DIM_SIZE", partitions[0].nrows()),
            ("VALUE_ROWIDS", partitions[0].value_rowids()),
        ] + tail
    else:
