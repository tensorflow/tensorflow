  ...
  *ragged-rank*.  A `RaggedTensor`'s ragged-rank is fixed at graph creation
  ...
  def __init__(self, values, row_partition, internal=False):
    ...
    if isinstance(values, RaggedTensor):
      assert row_partition.dtype == values.row_partition.dtype
  ...
  def _convert_values_and_partition(cls, values, row_partition, name):
    ...
    if isinstance(values, RaggedTensor):
      if values.row_partition.dtype != row_partition.dtype:
        if not ragged_config.auto_cast_partition_dtype():
          raise ValueError(
              "dtype mismatch: %s (%s) vs values.partition (%s)" %
              (name, row_partition.dtype, values.row_partition.dtype))
  ...
  def row_partition(self):
    """The row partition of the top level of the ragged tensor."""
  ...
  def row_lengths(self, axis=1, name=None):
    ...
    with ops.name_scope(name, "RaggedRowLengths", [self]):
      ...
      if axis == 0:
        ...
        shape = array_ops.shape(self.values, out_type=self.row_partition.dtype)
        return self.with_values(
            array_ops.ones(shape[:axis - 1], self.row_partition.dtype) *
            shape[axis - 1])
  ...
  def bounding_shape(self, axis=None, name=None, out_type=None):
    ...
    """
    if out_type is None:
      out_type = self.row_partition.dtype
    else:
  ...
  def with_values(self, new_values):
    ...
    if (isinstance(new_values, RaggedTensor) and
        self.row_partition.row_splits.dtype != new_values.row_splits.dtype):
      if not ragged_config.auto_cast_partition_dtype():
...
#=============================================================================
  ...
  def __repr__(self):
    if self._is_eager():
      ...
      return "tf.RaggedTensor(values=%s, row_splits=%s)" % (
          self._values, self.row_partition.row_splits)
  ...
  def _type_spec(self):
    return RaggedTensorSpec(
        ...
        row_splits_dtype=self.row_partition.row_splits.dtype)
...
def _get_nested_row_partitions(rt_input):
  ...
  if isinstance(rt_input.values, RaggedTensor):
    return [rt_input.row_partition] + _get_nested_row_partitions(
        rt_input.values)
  return [rt_input.row_partition]
...
def _get_optional_partition_dtype(values):
  ...
  if isinstance(values, RaggedTensor):
    return values.row_partition.dtype
  return None
