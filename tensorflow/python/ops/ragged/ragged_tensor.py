    ...
    * `tf.RaggedTensor.from_nested_value_rowids`
    ...
    def _from_row_partition(cls, values, row_partition, validate=True):
        ...
        if not isinstance(validate, bool):
            raise TypeError(
                f"Argument `validate` must have type bool. " f"Received {validate}."
            )
    ...
    def with_row_splits_dtype(self, dtype):
        ...
        if dtype not in (dtypes.int32, dtypes.int64):
            raise ValueError(
                f"Argument `row_splits` dtype must be int32 or int64. "
                f"Received {dtype}."
            )
    ...
    def from_tensor(
        ...
        if not isinstance(ragged_rank, int):
            raise TypeError(
                f"Argument `ragged_rank` must be an int. " f"Received {ragged_rank}."
            )
        if ragged_rank <= 0:
            raise ValueError(
    ...
    def to_tensor(self, default_value=None, name=None, shape=None):
        ...
        with ops.name_scope(name, "RaggedToTensor", [self, default_value, shape]):
            ...
            if (
                ...
                and any(isinstance(v, ops.Tensor) for v in shape)
                and all(isinstance(v, (int, ops.Tensor)) for v in shape)
            ):
...
class RaggedTensorSpec(type_spec.BatchableTypeSpec):
    ...
    def __init__(
        ...
        if not isinstance(self._ragged_rank, int):
            raise TypeError(
                f"Argument `ragged_rank` must be an int. " f"Recieved {ragged_rank}."
            )
    ...
    def _to_tensor_list(self, value):
        ...
        if ragged_rank != self._ragged_rank:
            raise ValueError(
                f"Ragged rank of value {ragged_rank} does not match "
    ...
    def _from_compatible_tensor_list(self, tensor_list):
        ...
        if self._ragged_rank < 0:
            raise ValueError(
                f"Argument `ragged_rank` must be non-negative. "
