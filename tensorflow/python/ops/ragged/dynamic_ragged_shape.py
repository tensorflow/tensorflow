...
class _DynamicRaggedShapeBatchEncoder(extension_type.ExtensionTypeBatchEncoder):
    ...
    def batch(
        self, spec: "DynamicRaggedShape.Spec", batch_size
        ...
        if spec.num_row_partitions:
            ...
            new_static_inner_shape = _batch_static_inner_shape(
                spec._static_inner_shape, batch_size
        ...
        if spec.num_row_partitions:
            ...
            return DynamicRaggedShape.Spec(
                ...
                static_inner_shape=_unbatch_static_inner_shape(
                    spec._static_inner_shape, scale
                ),  # pylint:disable=protected-access
                dtype=spec.dtype,
            )
    ...
    def encode(
        self, spec: "DynamicRaggedShape.Spec", value, minimum_rank=0
...
class DynamicRaggedShape(extension_type.BatchableExtensionType):
    ...
    * `row_partitions`: A list of `RowPartition` objects, describing how
    ...
    def __init__(
        ...
        if self._row_partitions:
            for axis, rp in enumerate(self._row_partitions):
                if axis > 0:
                    ...
                    msg = (
                        "RowPartitions in DynamicRaggedShape do not align "
        ...
        if checks:
            ...
            self._row_partitions = [
                rp._with_dependencies(checks)
                for rp in self._row_partitions  # pylint: disable=protected-access
            ]
    ...
    def __repr__(self):
        ...
        return "<DynamicRaggedShape lengths=%s num_row_partitions=%r>" % (
            lengths,
    ...
    def is_uniform(self, axis):
        ...
        if axis < 0:
            ...
            return (
                (axis == 0 or axis > len(self._row_partitions))  # pylint:disable=superfluous-parens
    ...
    def _alt_inner_shape(self, new_inner_rank):
        ...
        if new_inner_rank == 0:
            ...
            if self._static_inner_shape.is_fully_defined():
                return _alt_inner_shape_from_tensor_shape(
                    self._static_inner_shape, self.dtype, new_inner_rank
                )
    ...
    def _merge_dims(self, outer_axis: int, inner_axis: int) -> "DynamicRaggedShape":
        ...
        if inner_axis <= self.num_row_partitions:
            ...
            if rank is None:
                raise ValueError(
                    "Cannot merge_dims of the inner shape if the "
    ...
    def _merge_with(self, other: "DynamicRaggedShape") -> "DynamicRaggedShape":
        ...
        return DynamicRaggedShape(
            new_row_partitions, new_inner_shape, new_dtype, True, new_static_inner_shape
        )

    def _merge_with_spec(
        self, other: "DynamicRaggedShape.Spec"
    ) -> "DynamicRaggedShape":
    ...
    class Spec:
        ...
        def __init__(
            ...
            if inner_rank == 0:
                ...
                for i in range(len(row_partitions)):
                    ...
                    if i > 0:
                        ...
                        result = result.merge_with(
                            num_slices_in_dimension[-1] * previous_rp.uniform_row_length
                        )
                ...
                if row_partitions:
                    ...
                    result = (
                        num_slices_in_dimension[-1] * last_rp.uniform_row_length
                    ).merge_with(last_rp.nvals)
                ...
                row_partitions = [
                    ...
                    for i, rp in enumerate(row_partitions)
                ]
        ...
        def _from_tensor_shape(
            cls, shape: Any, num_row_partitions: int, dtype: dtypes.DType
        ) -> "DynamicRaggedShape.Spec":
            ...
            if shape.rank <= 1:
                ...
                if num_row_partitions:
                    raise ValueError(
                        "num_row_partitions should be zero "
        ...
        def _from_spec(
            ...
            if isinstance(spec, DynamicRaggedShape.Spec):
                ...
                return cls._from_tensor_shape(
                    spec.shape, spec.ragged_rank, spec.row_splits_dtype
                )
        ...
        def _merge_with(
            ...
            """
            max_num_row_partitions = max(
                self.num_row_partitions, other.num_row_partitions
            )
            a = self._with_num_row_partitions(max_num_row_partitions)
            b = other._with_num_row_partitions(max_num_row_partitions)

            new_rp = [
                a._merge_with(b) for (a, b) in zip(a._row_partitions, b._row_partitions)
            ]
        ...
        def _with_num_row_partitions(
            self, new_num_row_partitions: int
        ) -> "DynamicRaggedShape.Spec":
        ...
        def _truncate(self, new_rank: int) -> "DynamicRaggedShape.Spec":
            ...
            if new_rank == 1:
                ...
                return DynamicRaggedShape.Spec._from_tensor_shape(
                    [vector_size], 0, self.dtype
                )
...
def broadcast_dynamic_shape(
    shape_x: DynamicRaggedShape, shape_y: DynamicRaggedShape
) -> DynamicRaggedShape:
...
def ragged_binary_elementwise_op_impl(op, x, y):
    ...
    if (
        ...
        (shape_z, bcast_xz, bcast_yz) = broadcast_dynamic_shape_extended(
            shape_x, shape_y
        )
...
def _find_dtype_iterable(
    iterable: Iterable[Any], dtype: Optional[dtypes.DType]
...
def _broadcast_dynamic_shape_next_layer_half_ragged(
    ...
    [c_1_row_splits, ac_1_gather_index, bc_1_gather_index] = cond.cond(
...
def _safe_floor_div(
    dividend: tensor_shape.Dimension, divisor: tensor_shape.Dimension
...
def _merge_inner_shape(
    ...
    outer_axis: int,
    inner_axis: int,
) -> Tuple[tensor_lib.Tensor, tensor_shape.TensorShape]:
...
def _batch_rp_spec(
    rp_spec: RowPartitionSpec, batch_size: Optional[int]
) -> RowPartitionSpec:
...
def _batch_rp_spec_head(
    old_head: RowPartitionSpec, batch_size: Optional[int]
) -> RowPartitionSpec:
    """Creates a RowPartitionSpec representing the new dimension created."""
    nvals = (
        None
        if (old_head.nrows is None or batch_size is None)
        else batch_size * old_head.nrows
    )
    return RowPartitionSpec(
        nrows=batch_size,
        nvals=nvals,
...
def _batch_static_inner_shape(
    old_shape: tensor_shape.TensorShape, batch_size: Optional[int]
) -> tensor_shape.TensorShape:
...
def _batch_tensor_shape(
    old_shape: tensor_shape.TensorShape, batch_size: int
) -> tensor_shape.TensorShape:
...
def ones(
    shape: DynamicRaggedShape, dtype=dtypes.float32, name: Optional[str] = None
