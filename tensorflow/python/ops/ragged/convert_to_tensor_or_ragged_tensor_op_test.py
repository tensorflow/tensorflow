...
class RaggedConvertToTensorOrRaggedTensorTest(
    ...
    @parameterized.parameters(
        [
            ...
            dict(
                pylist=[np.array([1, 2]), np.array([3])], preferred_dtype=dtypes.string
            ),
            dict(
                pylist=np.array([[1, 2], [3]], dtype=object),
    ...
    @parameterized.parameters(
        [
            dict(
                ...
                message=(
                    "Tensor conversion requested dtype float32 for "
            ...
            dict(
                ...
                dtype=dtypes.string,
                message=(
                    "Tensor conversion requested dtype string for "
                    "RaggedTensor with dtype .*"
                ),
            ),
        ]
    )
    def testConvertRaggedTensorError(
        self, pylist, message, dtype=None, preferred_dtype=None
    ):
        rt = ragged_factory_ops.constant(pylist)
    ...
    @parameterized.parameters(
        [
            dict(
                value=ragged_factory_ops.constant_value([[1, 2], [3]], dtype=np.int32),
    ...
    def testConvertRaggedTensorValue(
        self, value, dtype=None, preferred_dtype=None, expected_dtype=None
    ):
    ...
    @parameterized.parameters(
        [
            dict(
                value=ragged_factory_ops.constant_value([["a", "b"], ["c"]], dtype=str),
                dtype=dtypes.int32,
                message=(
    ...
    def testConvertRaggedTensorValueError(
        self, value, message, dtype=None, preferred_dtype=None
    ):
        with self.assertRaisesRegex(ValueError, message):
    ...
    def testConvertNumpyArrayError(
        self, value, message, dtype=None, preferred_dtype=None
    ):
        with self.assertRaisesRegex(ValueError, message):
            ragged_tensor.convert_to_tensor_or_ragged_tensor(
