...
class RaggedReduceTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testStringReduceJoin(self, original, expected, separator=" ", axis=None):
        ...
        actual = ragged_string_ops.reduce_join(
            original_rt, axis=axis, separator=separator
        )
...
class RaggedSoftmaxTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testOrdinaryValues(self):
        ...
        y_expected = ragged_factory_ops.constant(
            [[0.5, 0.25, 0.25], [0.5, 0.5]], dtype=dtypes.float32
        )
...
def _cumsum_slow(rt, axis=0, exclusive=False, reverse=False, name=None):
    ...
    result = math_ops.cumsum(
        dense, axis=axis, exclusive=exclusive, reverse=reverse, name=name
    )
...
class RaggedCumsumTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    @parameterized.parameters(
        [
            dict(original=[[1, 2], [3, 4, 5]], expected=[[1, 2], [4, 6, 5]]),
            dict(
                original=[[1, 2], [3, 4, 5]], expected=[[4, 6], [3, 4, 5]], reverse=True
            ),
            dict(
                original=[[1, 2], [3, 4, 5]],
                expected=[[3, 4], [0, 0, 0]],
            ...
            dict(original=[[1, 2], [3, 4, 5]], expected=[[1, 3], [3, 7, 12]], axis=1),
            dict(
                original=[[1, 2], [3, 4, 5]],
                expected=[[3, 2], [12, 9, 5]],
                axis=1,
    ...
    @parameterized.parameters(
        [
            ...
            dict(
                expected=[[[1, 1], [5, 3]], [[9, 5], [13, 7], [17, 9]]],
                reverse=True,
                axis=2,
            ),
        ]
