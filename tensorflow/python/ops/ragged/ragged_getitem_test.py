...
def _make_tensor_slice_spec(slice_spec, use_constant=True):
    ...
    def make_piece_scalar(piece):
        if isinstance(piece, int):
            ...
            return slice(
                make_piece_scalar(piece.start),
                make_piece_scalar(piece.stop),
...
class RaggedGetItemTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    @parameterized.parameters(
        ...
        (
            SLICE_BUILDER[0.5],
            TypeError,
            re.escape(tensor_getitem_override._SLICE_TYPE_ERROR),
        ),
        (
    ...
    @parameterized.parameters(
        ...
        (
            SLICE_BUILDER[:, ::-1],
            [r[::-1] for r in EXAMPLE_RAGGED_TENSOR_3D],
            [2, 3, None],
        ),
    )  # pyformat: disable
