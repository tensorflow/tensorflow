...
class RowPartitionTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testPrecomputedSplits(self, rp_factory, expected_encodings):
        rp = rp_factory()
        self.assertEqual(
            rp._has_precomputed_row_splits(), "row_splits" in expected_encodings
        )
        self.assertEqual(
            rp._has_precomputed_row_lengths(), "row_lengths" in expected_encodings
        )
        self.assertEqual(
            rp._has_precomputed_value_rowids(), "value_rowids" in expected_encodings
        )
    ...
    def testMergePrecomputedEncodings(self, x, y, expected_encodings):
        ...
        for validate in (True, False):
            ...
            self.assertEqual(
                result._has_precomputed_row_splits(), "row_splits" in expected_encodings
            )
            self.assertEqual(
            ...
            self.assertEqual(
                result._has_precomputed_nrows(), "nrows" in expected_encodings
            )
            self.assertEqual(
    ...
    def testFromUniformRowLengthBugConvertToTensor(self):
        ...
        rp = RowPartition.from_uniform_row_length(
            row_length, nvals=nvals, nrows=nrows, dtype=dtypes.int64
        )
...
class RowPartitionSpecTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    @parameterized.parameters(
        [
            dict(
                original=RowPartitionSpec(),
                dtype=dtypes.int32,
                expected=RowPartitionSpec(dtype=dtypes.int32),
            ),
            dict(
                original=RowPartitionSpec(dtype=dtypes.int32),
                dtype=dtypes.int64,
                expected=RowPartitionSpec(),
            ),
            dict(
                original=RowPartitionSpec(nvals=20, nrows=4, uniform_row_length=5),
                dtype=dtypes.int32,
                expected=RowPartitionSpec(
                    nvals=20, nrows=4, uniform_row_length=5, dtype=dtypes.int32
                ),
            ),
        ]
    )
    ...
    @parameterized.parameters(
        [
            dict(
                a=RowPartitionSpec(nrows=3, nvals=10),
                b=RowPartitionSpec(uniform_row_length=5),
                error_type=ValueError,
                error_regex="Merging incompatible RowPartitionSpecs",
            ),
            dict(
                a=RowPartitionSpec(uniform_row_length=5, dtype=dtypes.int32),
...
def _assert_row_partition_equal(test_class, actual, expected):
    ...
    test_class.assertEqual(
        actual._has_precomputed_row_splits(), expected._has_precomputed_row_splits()
    )
    test_class.assertEqual(
        actual._has_precomputed_row_lengths(), expected._has_precomputed_row_lengths()
    )
    test_class.assertEqual(
        actual._has_precomputed_value_rowids(), expected._has_precomputed_value_rowids()
    )
    test_class.assertEqual(
        actual._has_precomputed_nrows(), expected._has_precomputed_nrows()
    )
    test_class.assertEqual(
