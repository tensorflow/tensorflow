...
class RowPartition(composite_tensor.CompositeTensor):
    ...
    def __init__(
        ...
        if internal is not _row_partition_factory_key:
            raise ValueError(
                "RowPartition constructor is private; please use one "
        ...
        for tensor in [row_lengths, value_rowids, nrows, uniform_row_length, nvals]:
            if tensor is not None:
                if not isinstance(tensor, tensor_lib.Tensor):
                    ...
                    raise ValueError(
                        f"Inconsistent dtype for encoding tensors: "
    ...
    def from_value_rowids(
        cls, value_rowids, nrows=None, validate=True, dtype=None, dtype_hint=None
    ):
    ...
    @classmethod
    def from_row_starts(
        cls, row_starts, nvals, validate=True, dtype=None, dtype_hint=None
    ):
        ...
        with ops.name_scope(None, "RowPartitionFromRowStarts", [row_starts]):
            ...
            return cls(
                row_splits=row_splits, nvals=nvals, internal=_row_partition_factory_key
            )
    ...
    def from_uniform_row_length(
        ...
        with ops.name_scope(
            ...
            [uniform_row_length, nvals, nrows] = _convert_all_to_tensors(
                [
            ...
            if (
                const_nvals is None
                and const_nrows is not None
                and const_uniform_row_length is not None
            ):
            ...
            if const_nrows is not None and const_row_length is not None:
                ...
                row_splits = (
                    math_ops.range(nrows + 1, dtype=uniform_row_length.dtype)
                    * uniform_row_length
                )
    ...
    def _convert_row_partition(cls, partition, name, dtype=None, dtype_hint=None):
        ...
        if (
            isinstance(partition, np.ndarray)
            and partition.dtype == np.int32
            and dtype is None
        ):
    ...
    def offsets_in_rows(self):
        ...
        return gen_ragged_math_ops.ragged_range(
            ...
            deltas=constant_op.constant(1, self.dtype),
        ).rt_dense_values
    ...
    def _static_check(self):
        ...
        if self._uniform_row_length is not None:
            if self._uniform_row_length.dtype != my_dtype:
                raise ValueError(
                    "_uniform_row_length.dtype="
                    + str(self._uniform_row_length.dtype)
                    + ", not "
                    + str(my_dtype)
                )
        ...
        if self._nrows is not None and self._nrows.dtype != my_dtype:
            raise ValueError(
                "_nrows.dtype=" + str(self._nrows.dtype) + ", not " + str(my_dtype)
            )
    ...
    def __repr__(self):
        if self._uniform_row_length is not None:
            return (
                f"tf.RowPartition(nrows={self._nrows}, "
    ...
    def _merge_with_spec(self, b):
        ...
        nrows = (
            constant_op.constant(b.nrows, self.dtype)
            if b.nrows is not None
            else self._nrows
        )
        nvals = (
            constant_op.constant(b.nvals, self.dtype)
            if b.nvals is not None
            else self._nvals
        )
        uniform_row_length = (
            constant_op.constant(b.uniform_row_length, self.dtype)
            if b.uniform_row_length is not None
            else self._uniform_row_length
        )
        return RowPartition(
            row_splits=self._row_splits,
            row_lengths=self._row_lengths,
    ...
    def _merge_precomputed_encodings(self, other, validate=True):
        ...
        if (
            self is other  # Fast path if row partitions are equal.
            or (
                ...
                and self._nvals is other._nvals
...
class RowPartitionSpec(type_spec.TypeSpec):
    ...
    def __deepcopy__(self, memo):
        ...
        uniform_row_length = (
            None
            if self._uniform_row_length is None
            else tensor_shape.dimension_value(self._uniform_row_length[0])
        )
...
def _merge_tensors(t1, t2, name, validate):
    ...
    if t1 is None:
        ...
        err_msg = (
            "RowPartition._merge_precomputed_encodings: partitions "
...
def _convert_all_to_tensors(values, dtype=None, dtype_hint=None):
    ...
    if convert_behavior:
        return [
            None
            if x is None
            else ops.convert_to_tensor(x, dtype=target_dtype, name=name)
