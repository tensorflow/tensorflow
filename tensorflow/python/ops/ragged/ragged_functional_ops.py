...
def map_flat_values(op, *args, **kwargs):
    ...
    inner_args = _replace_ragged_with_flat_values(
        args, partition_lists, flat_values_nrows
    )
    inner_kwargs = _replace_ragged_with_flat_values(
        kwargs, partition_lists, flat_values_nrows
    )
    ...
    if len(partition_dtypes) > 1:
        ...
        partition_lists = [
            [
                p.with_row_splits_dtype(dtypes.int64) for p in partition_list
    ...
    return ragged_tensor.RaggedTensor._from_nested_row_partitions(  # pylint: disable=protected-access
        op_output, _merge_partition_lists(partition_lists), validate=False
    )
