...
def _shape_invariant_to_type_spec(var, shape=None):
  ...
  Raises:
    ValueError: If `var` could not be converted to be a Tensor or a
      CompositeTensor.
  ...
  var = ops.convert_to_tensor_or_composite(var)
  if shape is None:
...
class WhileContext(ControlFlowContext):
  ...
  def BuildLoop(self, pred, body, loop_vars, shape_invariants,
    ...
    flat_loop_vars = nest.map_structure(
        _convert_tensorarray_to_flow, flat_orig_loop_vars)
    flat_loop_vars = nest.map_structure(
        ops.convert_to_tensor_or_composite, flat_loop_vars)
