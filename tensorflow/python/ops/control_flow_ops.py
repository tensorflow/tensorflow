...
def _Identity(tensor, name=None):
    ...
    if isinstance(tensor, tensor_lib.Tensor):
        ...
        raise TypeError(
            f"'tensor' must be a Tensor or CompositeTensor. Received: {type(tensor)}."
        )
...
def _convert_flow_to_tensorarray(tensor_or_tensor_array, tensor_or_flow):
    if isinstance(tensor_or_tensor_array, tensor_array_ops.TensorArray):
        return tensor_array_ops.build_ta_with_new_flow(
            tensor_or_tensor_array, tensor_or_flow
        )
    else:
    ...
    if isinstance(var, tensor_lib.Tensor):
        ...
        try:
            ...
            raise TypeError(
                ...
                f"{type(var._type_spec).__name__} instead of a TensorShape."
            ) from e  # pylint: disable=protected-access
...
def _AddNextAndBackEdge(m, v, enforce_shape_invariant=True):
    ...
    if isinstance(m, tensor_lib.Tensor):
        ...
        raise TypeError(
            f"'m' must be a Tensor or CompositeTensor. Received: {type(m)}."
        )
    return v
    ...
    def _InitializeValues(self, values):
        ...
        for x in values:
            if isinstance(x, tensor_lib.Tensor):
                ...
                raise TypeError(
                    f"'values' must be a list of Tensors. Received: {type(x)}."
                )

    def _BuildLoop(
        self, pred, body, flat_orig_loop_vars, flat_loop_vars, loop_vars_signature
    ):
        ...
        try:
            ...
            with ops.get_default_graph()._mutation_lock():  # pylint: disable=protected-access
                original_body_result, exit_vars = self._BuildLoop(
                    pred, body, flat_orig_loop_vars, flat_loop_vars, loop_vars_signature
                )
        finally:
            self.Exit()
    ...
    def _FixControlInputsAndContext(self, enters):
        ...
        for e in enters:
            if isinstance(e, tensor_lib.Tensor):
                ...
                raise TypeError(
                    f"'enters' must be a list of Tensors. Received: {type(e)}."
                )
            for x in xs:
...
def tuple(tensors, name=None, control_inputs=None):  # pylint: disable=redefined-builtin
    ...
    with ops.name_scope(name, "tuple", tensors) as name:
        ...
        if not gating_ops:
            raise ValueError(
                f"'tensors' must have at least one Tensor. Received: {tensors}."
            )
