...
class ReduceTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testReduceVar(self):
        ...
        x = ragged_factory_ops.constant(
            [[5.0, 1.0, 4.0, 1.0], [], [5.0, 9.0, 2.0], [5.0], []]
        )
...
class EqualityTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testEqualityNoDowncast(self, is_equals, float_literal):
        if (
            tf2.enabled()
            and isinstance(float_literal, np.float32)
            or not tf2.enabled()
            and isinstance(float_literal, float)
        ):
...
class ComparisonOps(parameterized.TestCase, test_util.TensorFlowTestCase):
    ...
    def test_math_maximum(self):
        ...
        self.assertAllEqual(
            gen_math_ops.maximum(np.int_(1), constant_op.constant(1.5, dtypes.float32)),
            np.array(1.5, np.float32),
        )
        self.assertAllEqual(
    ...
    def test_math_minimum(self):
        ...
        self.assertAllEqual(
            gen_math_ops.minimum(np.int_(1), constant_op.constant(1.1, dtypes.float32)),
            1,
        )
        self.assertAllEqual(
...
class VariableInplaceOpsTest(parameterized.TestCase, test_util.TensorFlowTestCase):
    @parameterized.parameters(
        itertools.product(
            allowed_var_op_input_combinations, ("assign", "assign_add", "assign_sub")
        )
    )
    ...
    @parameterized.parameters(
        itertools.product(
            disallowed_var_op_input_combinations, ("assign", "assign_add", "assign_sub")
        )
    )
