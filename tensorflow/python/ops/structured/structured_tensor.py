...
class StructuredTensor(extension_type.BatchableExtensionType):
    ...
    * A ***structure*** maps each field in the schema to a tensor value (which
    ...
    def _old_init(cls, fields, shape, nrows, row_partitions, internal=False):
        ...
        assert row_partitions is None or isinstance(row_partitions, tuple), (
            row_partitions
        )
        ...
        if len(self.shape) > 0:  # pylint: disable=g-explicit-length-test
            if not result:  # special-case for StructuredTensors w/ no fields.
                return _empty_dict_pylist_from_row_partitions(
                    self.row_partitions, self.nrows()
                )
    ...
    def _from_pydict(cls, pyval, typespec, path_so_far):
        ...
        if typespec is None:
            ...
            if not (
                isinstance(typespec, StructuredTensor.Spec)
    ...
    def _from_pylist_of_dict(cls, pyval, keys, rank, typespec, path_so_far):
        ...
        if typespec is None:
            ...
            if (
                (not isinstance(typespec, StructuredTensor.Spec))  # pylint: disable=superfluous-parens
                or (set(fields) - set(field_specs))
            ):
                raise ValueError(
                    "Value at %r does not match typespec: %r vs %r"
                    % (path_so_far, pyval, typespec)
    ...
    def _from_pyscalar(cls, pyval, typespec, path_so_far):
        ...
        if typespec is None:
            ...
            if not (
                isinstance(typespec, tensor.TensorSpec) and typespec.shape.rank == 0
            ):
                raise ValueError(
                    "Value at %r does not match typespec: %r vs %r"
                    % (path_so_far, typespec, pyval)
...
def _find_shape_dtype(
    fields: Mapping[str, _FieldValue],
    nrows: Optional[tensor.Tensor],
    row_partitions: Optional[Sequence[RowPartition]],
) -> dtypes.DType:
...
def _dynamic_ragged_shape_from_tensor(
    ...
    raise TypeError(
        f"Expected shape tf.shape({field}) to return a Tensor or a "
...
def _shape_from_fields(
    fields, rank: int, dtype: dtypes.DType
) -> Optional[dynamic_ragged_shape.DynamicRaggedShape]:
...
def _fields_with_dtype(
    fields: Mapping[str, _FieldValue], dtype: dtypes.DType
) -> Mapping[str, _FieldValue]:
...
def _dynamic_ragged_shape_init(fields, shape, nrows, row_partitions):
    ...
    assert isinstance(shape, tensor_shape.TensorShape), shape
    assert (
        nrows is None or isinstance(nrows, tensor.Tensor) or isinstance(nrows, int)
    ), nrows
    ...
    if rank == 1:
        ...
        if result is None:
            raise ValueError(
                "Must specify `nrows`, a fully specified `shape`,"
    ...
    if result is None:
        raise ValueError(
            "Must specify row_partitions, a fully specified shape, "
            + "or have fields if rank > 1"
        )
    return result
...
def StructuredTensorSpec(shape, field_specs):  # pylint:disable=invalid-name
    ...
    for k, v in field_specs.items():
        ...
        if untruncated_rank is not None and untruncated_rank < rank:
            raise ValueError(
                f"Rank of field {k} is {untruncated_rank}, but must be at least {rank}."
            )
