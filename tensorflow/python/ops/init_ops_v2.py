...
class Zeros(Initializer):
    ...
    def __call__(self, shape, dtype=dtypes.float32, **kwargs):
        ...
        if not dtype.is_numpy_compatible or dtype == dtypes.string:
            raise ValueError(
                "Argument `dtype` expected to be numeric or boolean. "
                f"Received {dtype}."
            )
...
class VarianceScaling(Initializer):
    ...
    def __init__(
        ...
        if scale <= 0.0:
            raise ValueError(
                "Argument `scale` must be a positive float. Received: " f"{scale}"
            )
...
def _assert_float_dtype(dtype):
    ...
    if not dtype.is_floating:
        raise ValueError(
            "Argument `dtype` is expected to be floating point. " f"Received: {dtype}."
        )
    return dtype
