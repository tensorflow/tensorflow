...
class Zeros(Initializer):
    ...
    def __call__(self, shape, dtype=dtypes.float32, **kwargs):
        ...
        if not dtype.is_numpy_compatible or dtype == dtypes.string:
            raise ValueError(
                f"Argument `dtype` expected to be numeric or boolean. Received {dtype}."
            )
...
class VarianceScaling(Initializer):
    ...
    def __init__(
        ...
        if scale <= 0.0:
            raise ValueError(
                f"Argument `scale` must be a positive float. Received: {scale}"
            )
        if mode not in {"fan_in", "fan_out", "fan_avg"}:
            raise ValueError(
                "Argument `mode` should be one of ('fan_in', 'fan_out', "
        ...
        if distribution not in {"uniform", "truncated_normal", "untruncated_normal"}:
            raise ValueError(
                "Argument `distribution` should be one of ('uniform', "
...
def _assert_float_dtype(dtype):
    ...
    if not dtype.is_floating:
        raise ValueError(
            f"Argument `dtype` is expected to be floating point. Received: {dtype}."
        )
    return dtype
