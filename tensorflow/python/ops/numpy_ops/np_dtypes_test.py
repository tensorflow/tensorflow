...
class DTypeTest(test.TestCase, parameterized.TestCase):
    ...
    def testAllowF64False(self, prefer_f32):
        ...
        self.assertEqual(
            dtypes.float32, np_dtypes._result_type(np.zeros([], np.float64), 1.1)
        )
    ...
    def testAllowF64TruePreferF32False(self):
        ...
        self.assertEqual(dtypes.complex128, np_dtypes._result_type(1.0j))
    ...
    def testAllowF64TruePreferF32True(self):
        ...
        self.assertEqual(
            dtypes.float64, np_dtypes._result_type(np.zeros([], np.float64), 1.1)
        )
        self.assertEqual(dtypes.complex64, np_dtypes._result_type(1.1j))
        self.assertEqual(
            dtypes.complex128, np_dtypes._result_type(np.zeros([], np.complex128), 1.1j)
        )
        self.assertEqual(
            dtypes.complex64, np_dtypes._result_type(np.zeros([], np.float32), 1.1j)
        )
