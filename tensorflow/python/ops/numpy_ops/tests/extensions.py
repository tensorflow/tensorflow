    ...
    def _tf_f(*args, **kwargs):
        ...
        if xla_forced_compile:
            ...
            if output_is_empty[0]:
                ...
                isinstance(np_out, list) and len(np_out) == 1 and not output_is_list[0]
            ):
                np_out = np_out[0]
        else:
...
def _compose_output_rep(
    lhs_rep, rhs_rep, lhs_contraction, rhs_contraction, lhs_batch, rhs_batch
):
...
def tf_dot_general(lhs, rhs, dimension_numbers):
    ...
    if (
        ...
        and lhs_batch == (0,)
        and rhs_batch == (0,)
        and lhs_contraction == (2,)
        and rhs_contraction == (1,)
    ):
    ...
    output_rep = _compose_output_rep(
        lhs_rep, rhs_rep, lhs_contraction, rhs_contraction, lhs_batch, rhs_batch
    )
...
def _conv_general_param_type_converter(window_strides, lhs_dilation, rhs_dilation, dim):
    ...
    return (
        _as_list_of_size(window_strides, dim),
...
def tf_conv_general_dilated(
    ...
    if lhs_spec != out_spec:
        raise ValueError(
            "Current implementation requires the `data_format` of the "
            "inputs and outputs to be the same."
        )
    if len(lhs_spec) >= 6:
        raise ValueError(
            "Current implmentation does not support 4 or higher"
            "dimensional convolution, but got: ",
            len(lhs_spec) - 2,
        )
    dim = len(lhs_spec) - 2
    if lhs_dilation and rhs_dilation:
        if lhs_dilation == (1,) * dim and rhs_dilation == (1,) * dim:
            ...
            raise ValueError(
                "Current implementation does not support that "
    ...
    if padding not in ["SAME", "VALID"]:
        raise ValueError(
            "Current implementation requires the padding parameter"
            "to be either 'VALID' or 'SAME', but got: ",
            padding,
        )
    if batch_group_count != 1 or feature_group_count != 1:
        raise NotImplementedError(
            "batch_group_count and feature_group_count "
            "other than 1 is currently not supported, but"
            " got feature_group_count: {}, batch_group_count"
            ": {}".format(feature_group_count, batch_group_count)
        )
    if precision is not None:
        raise NotImplementedError(
            "precision other than `None` is currently not "
    ...
    if rhs_dilation or (lhs_dilation is None and rhs_dilation is None):
        output = _tf_nn_APIs[dim][0](
            lhs, rhs, strides, padding, data_format, rhs_dilation
        )
    else:
...
def conv(inp, fltr, window_strides, padding, dimension_numbers, filter_dilation=None):
    ...
    for ds_el in flat_ds:
        if not isinstance(ds_el, (tensor_lib.Tensor, dataset_ops.DatasetV2)):
            ...
            raise ValueError(
                ...
                "structure of) tf.Tensors or tf.data.Datasets. Got: %s" % types
            )
...
def _psum(tensor, axis_name=None):
    ...
    if axis_name != _pmap_config.axis_name():
        raise ValueError(
            "axis_name (%s) is not equal to that of the surrounding "
...
def psum(tensors, axis_name=None):
    return nest.map_structure(functools.partial(_psum, axis_name=axis_name), tensors)
...
def pmap(f, axis_name=None, devices=None):
    ...
    def wrapper(*args):
        ...
        if _pmap_config.devices() is not None:
            raise ValueError(
                "Found a surrounding pmap. Nested pmap is not supported yet."
            )
        ...
        for i in range(len(flattened_results[0])):
            tensors = []
            for j, device in enumerate(devices):
                assert isinstance(flattened_results[j][i], tensor_lib.Tensor), (
