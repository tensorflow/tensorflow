...
flags.DEFINE_integer(
    "num_generated_cases", 10, help="Number of generated cases to test"
)
...
def _dtype(x):
    ...
    return (
        ...
        or onp.dtype(python_scalar_dtypes.get(type(x), None))
        or numpy_compat.np_asarray(x).dtype
    )
...
def if_device_under_test(device_type: Union[str, Sequence[str]], if_true, if_false):
    """Chooses `if_true` of `if_false` based on device_under_test."""
    if device_under_test() in (
        [device_type] if isinstance(device_type, str) else device_type
    ):
        return if_true
    else:
        return if_false
...
def skip_if_unsupported_type(dtype):
    if dtype not in supported_dtypes():
        raise unittest.SkipTest(f"Type {dtype} not supported on {device_under_test()}")
...
def format_test_name_suffix(opname, shapes, dt):
    arg_descriptions = (
        format_shape_dtype_string(shape, dtype) for shape, dtype in zip(shapes, dt)
    )
...
def _rand_dtype(rand, shape, dtype, scale=1.0, post=lambda x: x):
    ...
    if onp.issubdtype(dtype, onp.complexfloating):
        ...
        vals = r()
    return _cast_to_shape(numpy_compat.np_asarray(post(vals), dtype), shape, dtype)
...
def rand_not_small(offset=10.0):
  ...
  """
    ...
    def rand(shape, dtype):
        ...
        if onp.issubdtype(dtype, onp.complexfloating):
            ...
            out = rand(shape, base_dtype) + onp.array(1j, dtype) * rand(
                shape, base_dtype
            )
        ...
        return _cast_to_shape(numpy_compat.np_asarray(vals, dtype=dtype), shape, dtype)
...
def rand_unique_int():
    ...
    def fn(shape, dtype):
        return randchoice(
            onp.arange(onp.prod(shape), dtype=dtype), size=shape, replace=False
        )
...
class TestCase(parameterized.TestCase):
    ...
    def _CheckAgainstNumpy(
        self, numpy_reference_op, lax_op, args_maker, check_dtypes=True, tol=None
    ):
        args = args_maker()
        lax_ans = lax_op(*args)
        numpy_ans = numpy_reference_op(*args)
        self.assertAllClose(
            numpy_ans, lax_ans, check_dtypes=check_dtypes, atol=tol, rtol=tol
        )

    def _CompileAndCheck(
        self,
        fun,
        args_maker,
        check_dtypes=True,
        ...
        def check_compile(**kwargs):
            ...
            if not skip_retracing_test:
                ...
                self.assertAllClose(
                    new_python_ans, compiled_ans, check_dtypes, atol, rtol
                )
    ...
    def check_grads(self, f, args, atol=None, rtol=None, delta=None):
        ...
        self.assertAllClose(
            theoretical, numerical, check_dtypes=False, atol=atol, rtol=rtol
        )
