...
class IndexingTest(jtu.TestCase):
    ...
    def testFloatIndexingError(self):
        ...
        # Test tnp
        with self.assertRaisesRegex(IndexError, error_regex):
            tnp.zeros(2)[0.0]  # pylint: disable=expression-not-assigned
        with self.assertRaisesRegex(IndexError, error_regex):
            tnp.zeros((2, 2))[(0, 0.0)]  # pylint: disable=expression-not-assigned
        # Test with jit
        with self.assertRaisesRegex(IndexError, error_regex):
...
class IndexedUpdateTest(jtu.TestCase):
    ...
    def testStaticIndexing(
        ...
        check_xla = (
            not has_non_trivial_stride(indexer)  # b/123559667
            and not (
