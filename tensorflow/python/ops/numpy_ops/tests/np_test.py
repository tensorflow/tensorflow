...
JAX_ONE_TO_ONE_OP_RECORDS = [
    ...
    op_record(
        "nextafter",
        2,
        [f for f in float_dtypes if f not in (tnp.bfloat16, onp.float16)],
        all_shapes,
        jtu.rand_default,
        ["rev"],
        inexact=True,
...
JAX_COMPOUND_OP_RECORDS = [
    ...
    op_record(
        "sinc",
        1,
        [t for t in number_dtypes if t != tnp.bfloat16],
        all_shapes,
        jtu.rand_default,
        ["rev"],
...
JAX_BITWISE_OP_RECORDS = [
    op_record("bitwise_and", 2, int_dtypes, all_shapes, jtu.rand_default, []),
    op_record("bitwise_not", 1, int_dtypes, all_shapes, jtu.rand_default, []),
    op_record("bitwise_or", 2, int_dtypes, all_shapes, jtu.rand_default, []),
    op_record("bitwise_xor", 2, int_dtypes, all_shapes, jtu.rand_default, []),
]
...
if numpy_version >= (1, 15):
    JAX_COMPOUND_OP_RECORDS += [
        op_record(
            "isclose",
            2,
            [t for t in all_dtypes if t != tnp.bfloat16],
            all_shapes,
            jtu.rand_small_positive,
            [],
        ),
...
class LaxBackedNumpyTests(jtu.TestCase):
    ...
    def _GetArgsMaker(self, rng, shapes, dtypes, onp_arrays=True):
        def f():
            out = [
                rng(shape, dtype or tnp.float64) for shape, dtype in zip(shapes, dtypes)
            ]
    ...
    @unittest.skipIf(
        onp.__version__ >= onp.lib.NumpyVersion("2.0.0"),
    ...
    def testDot(self, lhs_shape, lhs_dtype, rhs_shape, rhs_dtype, rng_factory):
        ...
        self._CheckAgainstNumpy(
            onp_dot, tnp.dot, args_maker, check_dtypes=True, tol=tol
        )
    ...
    def testVDot(self, lhs_shape, lhs_dtype, rhs_shape, rhs_dtype, rng_factory):
        ...
        self._CheckAgainstNumpy(
            onp.vdot, tnp.vdot, args_maker, check_dtypes=True, tol=tol
        )
        # XLA lacks int32/int64 MatMul kernels (b/168657656).
        check_xla = not set((lhs_dtype, rhs_dtype)).intersection((onp.int32, onp.int64))
        self._CompileAndCheck(
    ...
    def testOperatorRound(self):
        self.assertAllClose(
            round(onp.float32(7.532), 1), round(tnp.float32(7.5), 1), check_dtypes=True
        )
        self.assertAllClose(
            round(onp.float32(1.234), 2),
            round(tnp.float32(1.234), 2),
            check_dtypes=True,
        )
        self.assertAllClose(
            round(onp.float32(1.234)), round(tnp.float32(1.234)), check_dtypes=False
        )
        self.assertAllClose(
    ...
    def testAppend(self, axis, base_shape, arg_dtypes, rng_factory):
        ...
        def onp_fun(arr, values):
            ...
            values = (
                values.astype(onp.float32) if tnp.bfloat16 == values.dtype else values
            )
        ...
        jtu.cases_from_list(
            ...
            for i, (arg, dtype) in enumerate(
                [
                    ...
                    (3.0, tnp.float64),
                    ([1, 2, 3], tnp.int_),
                    ([1.0, 2.0, 3.0], tnp.float64),
                    ([[1, 2], [3, 4], [5, 6]], tnp.int_),
                    ([[1, 2.0], [3, 4], [5, 6]], tnp.float64),
                    ([[1.0, 2j], [3.0, 4.0], [5.0, 6.0]], tnp.complex128),
                    (
                        [
                            [3, onp.array(2, dtype=tnp.float64), 1],
                            onp.arange(3.0, dtype=tnp.float64),
                        ],
                        tnp.float64,
    ...
    def testOnpMean(self):
        ...
        x = tnp.add(tnp.eye(3, dtype=tnp.float64), 0.0)
        ans = onp.mean(x)
    ...
    def testNanToNum(self, rng_factory, shape, dtype):
        ...
        check_dtypes = shape is not jtu.PYTHON_SCALAR_SHAPE
        self._CheckAgainstNumpy(
            onp_fun, tnp.nan_to_num, args_maker, check_dtypes=check_dtypes
        )
    ...
    def testQuantile(
        ...
        def onp_fun(*args):
            args = [
                x if tnp.result_type(x) != tnp.bfloat16 else onp.asarray(x, onp.float32)
                for x in args
            ]
    ...
    def testSelect(self, rng_factory, shapes, dtypes):
        ...
        def onp_fun(condlist, choicelist, default):
            choicelist = [
                x if tnp.bfloat16 != tnp.result_type(x) else x.astype(onp.float32)
                for x in choicelist
            ]
    ...
    def testArange(self):
        ...
        self.assertAllClose(
            tnp.arange(77), onp.arange(77, dtype=tnp.int_), check_dtypes=True
        )
        self.assertAllClose(
            tnp.arange(2, 13), onp.arange(2, 13, dtype=tnp.int_), check_dtypes=True
        )
        self.assertAllClose(
            tnp.arange(4, 21, 9),
            onp.arange(4, 21, 9, dtype=tnp.int_),
            check_dtypes=True,
        )
        self.assertAllClose(
            tnp.arange(53, 5, -3),
        ...
        self.assertAllClose(
            tnp.arange(77, dtype=float), onp.arange(77, dtype=float), check_dtypes=True
        )
        self.assertAllClose(
    ...
    def testMathSpecialFloatValues(self, op, dtype):
        ...
        for x in (
            ...
            onp.inf,
            tnp.finfo(dtype).max,
            onp.sqrt(tnp.finfo(dtype).max),
            onp.sqrt(tnp.finfo(dtype).max) * 2.0,
        ):
            if (
    ...
    def testVar(self, shape, dtype, out_dtype, axis, ddof, keepdims, rng_factory):
        ...
        def onp_fun(x):
            out = onp.var(
                x.astype(tnp.promote_types(onp.float32, dtype)),
                axis=axis,
                ddof=ddof,
                keepdims=keepdims,
            )
    ...
    @named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": (
                    "_start_shape={}_stop_shape={}_num={}_endpoint={}_base={}_dtype={}"
                ).format(
                    start_shape,
                    stop_shape,
                    num,
    ...
    def testLogspace(
        ...
        if (
            ...
            raise unittest.SkipTest(
                "GPUx32 truncated exponentiation doesn't exactly match other platforms."
            )
        rng = rng_factory()
        # relax default tolerances slightly
        tol = {
    ...
    def testReluGradientConstants(self):
        ...
        def body(i, xy):
            x, y = xy
            y = y + jax.grad(lambda z: tnp.sum(tnp.maximum(z, 0.0)))(x)  # pylint: disable=undefined-variable
            return x, y
    ...
    def testBroadcastToIntIssue1548(self):
        self.assertAllClose(
            tnp.broadcast_to(1, (3, 2)), onp.ones((3, 2)), check_dtypes=False
        )
    ...
    def testPrecision(self):
        ...
        jtu.assert_dot_precision(
            HIGHEST, partial(tnp.dot, precision=HIGHEST), ones_1d, ones_1d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(tnp.dot, precision=HIGHEST), ones_3d, ones_3d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(tnp.matmul, precision=HIGHEST), ones_2d, ones_2d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(tnp.vdot, precision=HIGHEST), ones_1d, ones_1d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(tnp.tensordot, axes=2, precision=HIGHEST), ones_2d, ones_2d
        )
        jtu.assert_dot_precision(
            HIGHEST,
        ...
        jtu.assert_dot_precision(
            HIGHEST, partial(tnp.einsum, "i,i", precision=HIGHEST), ones_1d, ones_1d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(tnp.einsum, "ij,ij", precision=HIGHEST), ones_2d, ones_2d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(tnp.inner, precision=HIGHEST), ones_1d, ones_1d
        )
...
GRAD_SPECIAL_VALUE_TEST_RECORDS = [
    GradSpecialValuesTestSpec(tnp.arcsinh, [0.0, 1000.0], 2),
    GradSpecialValuesTestSpec(tnp.arccosh, [1000.0], 2),
    GradSpecialValuesTestSpec(tnp.arctanh, [0.0], 2),
