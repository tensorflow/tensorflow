...
def diagonal(a, offset=0, axis1=0, axis2=1):  # pylint: disable=missing-docstring
    ...
    def _zeros():  # pylint: disable=missing-docstring
        return (
            array_ops.zeros(array_ops.concat([a_shape[:-1], [0]], 0), dtype=a.dtype),
            0,
        )
...
def _reduce(
    ...
    if isinstance(axis, ops.Tensor) and axis.dtype not in (dtypes.int32, dtypes.int64):
...
def pad(array, pad_width, mode, **kwargs):  # pylint: disable=redefined-outer-name
    ...
    return array_ops.pad(
        tensor=array, paddings=pad_width, mode=mode, constant_values=constant_values
    )
...
def flip(m, axis=None):  # pylint: disable=missing-docstring
    ...
    axis = np_utils._canonicalize_axis(
...
def _slice_helper(tensor, slice_spec, update_method=None, updates=None):
    ...
    with ops.name_scope(
        ...
        # do a gather instead.
        rank = np_utils._maybe_static(
            array_ops.rank(tensor)
        )  # pylint: disable=protected-access
