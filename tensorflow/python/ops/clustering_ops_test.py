...
class NearestCentersTest(test.TestCase):
    def setUp(self):
        self._points = np.array(
            [[100.0, 0.0], [101.0, 2.0], [99.0, 2.0], [1.0, 1.0]]
        ).astype(np.float32)

        self._centers = np.array(
            [[100.0, 0.0], [99.0, 1.0], [50.0, 50.0], [0.0, 0.0], [1.0, 1.0]]
        ).astype(np.float32)
    ...
    def testNearest2(self):
        with self.cached_session():
            ...
            self.assertAllClose(
                distances, [[0.0, 2.0], [5.0, 5.0], [1.0, 5.0], [0.0, 2.0]]
            )
...
class NearestCentersLargeTest(test.TestCase):
    def setUp(self):
        ...
        nearest_neighbors = [
            sorted(
                [
                    (squared_distance(point, self._centers[j]), j)
    ...
    def testNearest1(self):
        with self.cached_session():
            ...
            self.assertAllClose(
                indices, self._expected_nearest_neighbor_indices[:, [0]]
            )
            self.assertAllClose(
                distances, self._expected_nearest_neighbor_squared_distances[:, [0]]
            )
    ...
    def testNearest5(self):
        with self.cached_session():
            ...
            self.assertAllClose(
                indices, self._expected_nearest_neighbor_indices[:, 0:5]
            )
            self.assertAllClose(
                distances, self._expected_nearest_neighbor_squared_distances[:, 0:5]
            )
