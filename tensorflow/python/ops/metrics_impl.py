...
def _maybe_expand_labels(labels, predictions):
    ...
    with ops.name_scope(None, "expand_labels", (labels, predictions)) as scope:
        ...
        if isinstance(labels, sparse_tensor.SparseTensor):
            return cond.cond(
                math_ops.equal(
                    array_ops.rank(predictions), array_ops.size(labels.dense_shape) + 1
                ),
...
def auc(
    ...
    if context.executing_eagerly():
        raise RuntimeError(
            "tf.metrics.auc is not supported when eager execution is enabled."
        )
...
def mean_iou(
    ...
    if context.executing_eagerly():
        raise RuntimeError(
            "tf.metrics.mean_iou is not supported when eager execution is enabled."
        )
    ...
    with variable_scope.variable_scope(
        ...
        def compute_mean_iou(_, total_cm):
            ...
            num_valid_entries = math_ops.reduce_sum(
                math_ops.cast(math_ops.not_equal(denominator, 0), dtype=dtypes.float32)
            )
...
def mean_tensor(
    ...
    if context.executing_eagerly():
        raise RuntimeError(
            "tf.metrics.mean_tensor is not supported when eager execution is enabled."
        )
...
def precision(
    ...
    if context.executing_eagerly():
        raise RuntimeError(
            "tf.metrics.precision is not supported when eager execution is enabled."
        )
    ...
    with variable_scope.variable_scope(
        ...
        p = _aggregate_across_replicas(
            metrics_collections, once_across_replicas, true_p, false_p
        )
    ...
    with variable_scope.variable_scope(name, "recall", (predictions, labels, weights)):
        ...
        def compute_recall(true_p, false_n, name):
            return array_ops.where(
                ...
                math_ops.divide(true_p, true_p + false_n),
                0,
                name,
            )
...
def _select_class_id(ids, selected_id):
    ...
    filled_selected_id = array_ops.fill(
        filled_selected_id_shape, math_ops.cast(selected_id, dtypes.int64)
    )
...
def _streaming_sparse_true_positive_at_k(
    ...
    with ops.name_scope(
        name,
        _at_k_name("true_positive", k, class_id=class_id),
...
def _streaming_sparse_false_negative_at_k(
    ...
    with ops.name_scope(
        name,
        _at_k_name("false_negative", k, class_id=class_id),
        (predictions_idx, labels, weights),
    ) as scope:
...
def recall_at_k(
    ...
    if context.executing_eagerly():
        raise RuntimeError(
            "tf.metrics.recall_at_k is not supported when eager execution is enabled."
        )

    with ops.name_scope(
        name, _at_k_name("recall", k, class_id=class_id), (predictions, labels, weights)
    ) as scope:
...
def sensitivity_at_specificity(
    ...
    if specificity < 0 or specificity > 1:
        raise ValueError(
            "`specificity` must be in the range [0, 1]. Currently, "
...
def _expand_and_tile(tensor, multiple, dim=0, name=None):
    ...
    if multiple < 1:
        raise ValueError(
            f"Invalid argument multiple={multiple} for "
    ...
    with ops.name_scope(name, "expand_and_tile", (tensor, multiple, dim)) as scope:
        ...
        if isinstance(tensor, sparse_tensor.SparseTensor):
            ...
            expanded_shape = array_ops.concat(
                (
                    array_ops.slice(tensor.dense_shape, [0], expand_dims),
                    [1],
...
def _num_relevant(labels, k):
    ...
    with ops.name_scope(None, "num_relevant", (labels,)) as scope:
        ...
        num_labels = math_ops.reduce_sum(
            array_ops.where_v2(
                math_ops.greater_equal(labels, 0),
...
def _sparse_average_precision_at_top_k(labels, predictions_idx):
    ...
    with ops.name_scope(None, "average_precision", (predictions_idx, labels)) as scope:
        ...
        if k is None:
            raise ValueError(
                "The last dimension of predictions_idx must be set. "
...
def _streaming_sparse_false_positive_at_k(
    ...
    with ops.name_scope(
        name,
        _at_k_name("false_positive", k, class_id=class_id),
        (predictions_idx, labels, weights),
    ) as scope:
...
def precision_at_top_k(
    ...
    with ops.name_scope(
        name,
        _at_k_name("precision", k, class_id=class_id),
        (predictions_idx, labels, weights),
    ) as scope:
        labels = _maybe_expand_labels(labels, predictions_idx)
        top_k_idx = math_ops.cast(predictions_idx, dtypes.int64)
...
def specificity_at_sensitivity(
    ...
    if sensitivity < 0 or sensitivity > 1:
        raise ValueError(
            "`sensitivity` must be in the range [0, 1]. Currently, "
