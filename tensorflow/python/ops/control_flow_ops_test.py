    ...
    def testIndexedSlicesWithShapeGradientInWhileLoop(self):
        for dtype in [dtypes.float32, dtypes.float64]:
            with self.cached_session() as sess:
                ...
                _, outputs = while_loop.while_loop(
    ...
    def testCond_gradient(self):
        ...
        with backprop.GradientTape(persistent=True) as tape:
            ...
            cond_true = tf_cond.cond(
                array_ops.constant(True), lambda: true_in**2.0, lambda: false_in**2.0
            )
            cond_false = tf_cond.cond(
                array_ops.constant(False), lambda: true_in**2.0, lambda: false_in**2.0
            )
...
class DataTypesTest(test_util.TensorFlowTestCase):
    ...
    def _testShape(self, fn_true, fn_false, expected_shape, strict=False):
        ...
        output_cond = tf_cond.cond(condition, fn_true, fn_false, strict=strict)
        self.assertEqual(
            _raw_nested_shape(_get_nested_shape(output_cond)),
            _raw_nested_shape(expected_shape),
        )

        output_case = control_flow_case.case(
    ...
    def test_singleton_strict(self):
        ...
        with self.assertRaises(ValueError):
            tf_cond.cond(constant_op.constant(True), fn_tensor, fn_list, strict=True)

        with self.assertRaises(TypeError):
            tf_cond.cond(constant_op.constant(True), fn_list, fn_tuple, strict=True)

        with self.assertRaises(ValueError):
            control_flow_case.case(
...
class IndexedCaseTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testParallelExecution(self):
        ...
        with ops.Graph().as_default() as g:
            ...
            _, result = while_loop.while_loop(cond, body, [0, 0.0])
