...
def _build_cond(
    ...
    with ops.control_dependencies(
        list(true_graph.function_captures.control)
        + list(false_graph.function_captures.control)
    ):
        true_stateful_ops = [
            op for op in true_graph.get_operations() if _is_op_stateful(op)
        ]
        false_stateful_ops = [
            op for op in false_graph.get_operations() if _is_op_stateful(op)
        ]
        ...
        def _make_op(inputs):
            if_op, tensors = util.get_op_and_outputs(
                op_fn(
                    pred,
                    inputs,
    ...
    structured_output_specs = _get_compatible_structured_output_specs(
        true_graph, false_graph
    )
...
def get_func_graphs(op):
    ...
    if op.type in ["If", "StatelessIf"]:
        ...
        return [
            _get_func_graph_for_branch(branch_fn, "_branch_graph_{}".format(i))
        ...
        for branch_idx, branch_out in enumerate(branch_outs):
            if isinstance(branch_out, indexed_slices.IndexedSlices):
                ...
                with branch_graphs[branch_idx].as_default():
                    branch_outputs[branch_idx][output_idx] = (
...
def _check_same_outputs(op_type, graphs):
    ...
    for b in range(1, len(graphs)):
        ...
        if len(graphs[0].outputs) != len(graphs[b].outputs):
            raise ValueError(
                "Lengths of branch outputs of {op_type} must match.\n"
...
def indexed_case(
    branch_index, branch_fns, name="indexed_case", lower_using_switch_merge=None
):
    ...
    with ops.name_scope(name) as scope:
        ...
        return _build_case(
            branch_index,
            branch_graphs,
            [g.external_captures for g in branch_graphs],
            name=scope,
...
def _build_case(
    branch_index, branch_graphs, branch_inputs, name=None, lower_using_switch_merge=None
):
    ...
    with ops.control_dependencies(
        ...
        def _make_op(inputs):
            case_op, tensors = util.get_op_and_outputs(
                op_fn(
                    branch_index,
                    inputs,
