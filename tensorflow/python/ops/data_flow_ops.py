...
class PaddingRandomShuffleQueue(QueueBase):
  """A RandomShuffleQueue that supports batching variable-sized tensors by padding.
  ...
  @@__init__
  """
  def __init__(self, capacity, min_after_dequeue, dtypes, shapes,
               names=None, seed=None, shared_name=None,
               name="padding_random_shuffle_queue"):
               """Create a queue that dequeues elements in a random order.
    ...
    Args:
      ...
      dtypes:  A list of `DType` objects. The length of `dtypes` must equal
        the number of tensors in each queue element.
      shapes: A list of fully-defined `TensorShape` objects
        with the same length as `dtypes`.Any dimension in the `TensorShape` containing value `None` is dynamic and allows values to be enqueued with
    ...
    if seed1 is None and seed2 is None:
      ...
      string = (str(seed1) + shared_name).encode("utf-8")
      seed2 = int(hashlib.md5(string).hexdigest()[:8], 16) & 0x7FFFFFFF
    queue_ref = gen_data_flow_ops._padding_random_shuffle_queue(
        component_types=dtypes, shapes=shapes, capacity=capacity,
        min_after_dequeue=min_after_dequeue, seed=seed1, seed2=seed2,
        shared_name=shared_name, name=name)

    super(PaddingRandomShuffleQueue, self).__init__(dtypes, shapes, names, queue_ref)
