...
def _ndtr(x):
    ...
    half_sqrt_2 = constant_op.constant(
        0.5 * np.sqrt(2.0), dtype=x.dtype, name="half_sqrt_2"
    )
    w = x * half_sqrt_2
    z = math_ops.abs(w)
    y = array_ops.where_v2(
        math_ops.less(z, half_sqrt_2),
        1.0 + math_ops.erf(w),
        array_ops.where_v2(
            math_ops.greater(w, 0.0), 2.0 - math_ops.erfc(z), math_ops.erfc(z)
        ),
    )
    return 0.5 * y
...
def _ndtri(p):
    ...
    second_term_small_p = (
        _create_polynomial(1.0 / z, p2) / _create_polynomial(1.0 / z, q2) / z
    )
    second_term_otherwise = (
        _create_polynomial(1.0 / z, p1) / _create_polynomial(1.0 / z, q1) / z
    )
    x_for_small_p = first_term - second_term_small_p
    x_otherwise = first_term - second_term_otherwise

    x = array_ops.where_v2(
        sanitized_mcp > np.exp(-2.0),
        x_for_big_p,
    ...
    x_nan_replaced = array_ops.where_v2(
        p <= 0.0, -infinity, array_ops.where_v2(p >= 1.0, infinity, x)
    )
...
def log_ndtr(x, series_order=3, name="log_ndtr"):
    ...
    with ops.name_scope(name, values=[x]):
        ...
        return array_ops.where_v2(
            ...
            -_ndtr(
                -x
            ),  # log(1-x) ~= -x, x << 1  # pylint: disable=invalid-unary-operand-type
            array_ops.where_v2(
