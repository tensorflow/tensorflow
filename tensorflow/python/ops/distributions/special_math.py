...
def _ndtr(x):
  ...
  w = x * half_sqrt_2
  z = math_ops.abs(w)
  y = array_ops.where_v2(math_ops.less(z, half_sqrt_2),
                         1. + math_ops.erf(w),
                         array_ops.where_v2(math_ops.greater(w, 0.),
                                            2. - math_ops.erfc(z),
                                            math_ops.erfc(z)))
  return 0.5 * y
...
def _ndtri(p):
  ...
  x = array_ops.where_v2(sanitized_mcp > np.exp(-2.),
                         x_for_big_p,
                         array_ops.where_v2(z >= 8.0, x_for_small_p,
                                            x_otherwise))
  ...
  with ops.name_scope(name, values=[x]):
    ...
    return array_ops.where_v2(
        ...
        array_ops.where_v2(math_ops.greater(x, lower_segment),
                           math_ops.log(_ndtr(math_ops.maximum(x,
                                                               lower_segment))),
