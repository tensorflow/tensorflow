...
class TransformedDistribution(distribution_lib.Distribution):
    ...
    def _log_prob(self, y):
        ...
        return math_ops.reduce_logsumexp(array_ops_stack.stack(lp_on_fibers), axis=0)
    ...
    def _log_survival_function(self, y):
        if self._is_maybe_event_override:
            raise NotImplementedError(
                "log_survival_function is not implemented when overriding event_shape"
            )
        if not self.bijector._is_injective:  # pylint: disable=protected-access
            raise NotImplementedError(
    ...
    def _survival_function(self, y):
        ...
        if not self.bijector._is_injective:  # pylint: disable=protected-access
            raise NotImplementedError(
                "survival_function is not implemented when bijector is not injective."
            )
        x = self.bijector.inverse(y)
