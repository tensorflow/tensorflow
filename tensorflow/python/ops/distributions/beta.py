    ...
    * `Gamma` is the [gamma function](
    ...
    def _sample_n(self, n, seed=None):
        expanded_concentration1 = (
            array_ops.ones_like(self.total_concentration, dtype=self.dtype)
            * self.concentration1
        )
    ...
    def _log_unnormalized_prob(self, x):
        ...
        return math_ops.xlogy(self.concentration1 - 1.0, x) + (
            self.concentration0 - 1.0
        ) * math_ops.log1p(-x)  # pylint: disable=invalid-unary-operand-type

    def _log_normalization(self):
        return (
            math_ops.lgamma(self.concentration1)
    ...
    def _entropy(self):
        return (
            ...
            - (self.concentration1 - 1.0) * math_ops.digamma(self.concentration1)
            - (self.concentration0 - 1.0) * math_ops.digamma(self.concentration0)
            + (
                (self.total_concentration - 2.0)
    ...
    def _variance(self):
        return self._mean() * (1.0 - self._mean()) / (1.0 + self.total_concentration)
    ...
    def _mode(self):
        ...
        if self.allow_nan_stats:
            ...
            is_defined = math_ops.logical_and(
                self.concentration1 > 1.0, self.concentration0 > 1.0
            )
    ...
    def __init__(
        ...
        with ops.name_scope(name, values=[concentration1, concentration0]) as name:
            super(BetaWithSoftplusConcentration, self).__init__(
                concentration1=nn.softplus(
                    concentration1, name="softplus_concentration1"
                ),
                concentration0=nn.softplus(
                    concentration0, name="softplus_concentration0"
                ),
                validate_args=validate_args,
                allow_nan_stats=allow_nan_stats,
                name=name,
