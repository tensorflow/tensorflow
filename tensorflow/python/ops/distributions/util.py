...
def embed_check_categorical_event_shape(
    ...
    with ops.name_scope(name, values=[categorical_param]):
        ...
        if max_event_size == 0:
            raise TypeError(
                "Unable to validate size of unrecognized dtype ({}).".format(
                    x_dtype.name
                )
            )
        try:
            ...
            raise ValueError(
                "A categorical-distribution parameter must have at least 1 dimension."
            )
        if tensor_shape.dimension_value(x_shape_static[-1]) is not None:
            event_size = x_shape_static.dims[-1].value
            if event_size < 2:
                raise ValueError(
                    "A categorical-distribution parameter must have at least 2 events."
                )
...
def embed_check_integer_casting_closed(
    ...
    with ops.name_scope(name, values=[x]):
        ...
        if not _is_integer_like_by_dtype(target_dtype) and not target_dtype.is_floating:
            raise TypeError(
                "target_dtype ({}) must be floating- or integer-type.".format(
                    target_dtype.name
                )
            )
