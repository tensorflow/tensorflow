...
class FuncRegistry:
    ...
    def __call__(self, token, device, args):
        ...
        if func is None:
            raise ValueError(
                f"Could not find callback with key={token} in the registry."
            )
...
def _internal_py_func(
    ...
    if not callable(func):
        raise ValueError(
            f"Expected func to be callable. Received func={func} of type {type(func)}."
        )
    ...
    handle_composite_tensors = use_eager_py_func and (
        any(isinstance(v, composite_tensor.CompositeTensor) for v in inp)
        or any(isinstance(t, type_spec.TypeSpec) for t in Tout)
    )
...
def _wrap_for_composites(func, inp, Tout):
    ...
    def wrapped_func(*flat_inp):
        ...
        for elt, expected_type in zip(out, out_structure):
            if isinstance(expected_type, type_spec.TypeSpec) and not isinstance(
                expected_type, tensor_spec.TensorSpec
            ):
