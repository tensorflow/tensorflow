...
def while_loop(
    ...
    if shape_invariants is not None:
        loop_vars_signature = nest.map_structure(
            control_flow_ops._shape_invariant_to_type_spec, loop_vars, shape_invariants
        )
    else:
        loop_vars_signature = nest.map_structure(
    ...
    with ops.name_scope(name) as scope:
        ...
        def wrapped_body(loop_counter, maximum_iterations_arg, *args):
            ...
            try:
                ...
                nest.assert_same_structure(
                    outputs, orig_loop_vars, check_types=False, expand_composites=True
                )
            except ValueError:
                ...
                nest.assert_same_structure(
                    vars1, vars2, check_types=False, expand_composites=True
                )
        ...
        deferred_external_captures = nest.flatten(
            [c() for c in body_graph.deferred_external_captures], expand_composites=True
        )
        loop_vars = (
        ...
        with ops.control_dependencies(
            list(cond_graph.function_captures.control)
            + list(body_graph.function_captures.control)
        ):
...
def _get_intermediates(func_graph):
    ...
    for op in func_graph.get_operations():
        ...
        for o in op.outputs:
            if (
                o is not func_graph.inputs[0]  # Loop counter.
                and o.dtype != dtypes.resource  # Do not accumulate resource tensors.
                and _get_accumulator(o) is None  # Has existing accumulator.
    ...
    def _capture_helper(self, tensor, name):
        ...
        if tensor.graph is not self._forward_graph:
            ...
            if not already_captured:
                ...
                self.internal_capture_to_output[ops.tensor_id(captured_tensor)] = (
                    captured_tensor
                )
        ...
        self._indirect_captures[ops.tensor_id(tensor)] = captured_tensor
        self.internal_capture_to_output[ops.tensor_id(captured_accumulator)] = (
            new_tensor_list
        )
        return captured_tensor
...
def _check_shapes_compat(
    flat_output_tensors, flat_shape_invariants, flat_input_tensors
):
    for t, shape, input_t in zip(
        flat_output_tensors, flat_shape_invariants, flat_input_tensors
    ):
...
def _duplicate_body_captures_in_cond(cond_graph, body_graph_captures):
    ...
    for k, v in zip(keys, tuples):
        cond_graph._function_captures.add_or_replace(
            key=k, external=v[0], internal=v[1], is_by_ref=False
        )
...
def _pack_sequence_as(loop_vars_signature, flat_orig_loop_vars, loop_vars):
    ...
    flattened_loop_vars = [
        ...
        for z in zip(
            nest.flatten(loop_vars, expand_composites=True), flat_orig_loop_vars
        )
    ]
    return nest.pack_sequence_as(
        loop_vars_signature, flattened_loop_vars, expand_composites=True
    )
