...
def _assert_dilated_depthwise_fits(input, filter, rate, padding, data_format):
  ...
  if isinstance(padding, str):
    ...
    spatial_pads = [list(p) for p in padding]
    spatial_pads = spatial_pads[2:] if channels_first else spatial_pads[1:-1]
    pads = [spatial_pads[0][0] + spatial_pads[0][1],
  ...
  check = check_ops.assert_greater_equal(
      available,
      dilated,
      message=("Dilated depthwise convolution: the input (plus padding) is "
  ...
  with ops.name_scope(name, "depthwise", [input, filter]) as name:
    ...
    if (any(_ > 1 for _ in rate) and
      ...
      if in_shape is None or None in in_shape:
        input = _assert_dilated_depthwise_fits(input, filter, rate, padding,
                                               data_format)
