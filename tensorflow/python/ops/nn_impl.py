...
def log_poisson_loss(targets, log_input, compute_full_loss=False, name=None):
  ...
  with ops.name_scope(name, "log_poisson_loss", [log_input, targets]) as name:
    ...
    if compute_full_loss:
      ...
      result += array_ops.where_v2(cond, zeros, stirling_approx)
    return result
...
def sigmoid_cross_entropy_with_logits(  # pylint: disable=invalid-name
  ...
  with ops.name_scope(name, "logistic_loss", [logits, labels]) as name:
    ...
    cond = (logits >= zeros)
    relu_logits = array_ops.where_v2(cond, logits, zeros)
    neg_abs_logits = array_ops.where_v2(cond, -logits, logits)
    return math_ops.add(
