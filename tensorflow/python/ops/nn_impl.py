...
def swish(features, beta=1.0):
  ...
  def multiply_with_sigmoid(multiplier, logits, sigmoid_logits):
    ...
    # sigmoid(logits) may underflow even when the scaled result is
    # representable. In that range sigmoid(logits) and exp(logits) differ by
    # less than the dtype's precision, so evaluate the product in the log
    # domain.
