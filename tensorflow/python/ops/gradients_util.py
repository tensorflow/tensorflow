...
def _DefaultGradYs(
    ...
    if len(grad_ys) != len(ys):
        raise ValueError(
            f"Length mismatch. Passed {len(grad_ys)} grad_ys for {len(ys)} ys"
        )
...
def _MaybeCaptured(t):
    ...
    if (
        ...
        and _IsFunction(t.op.graph)
        and t.op.type == "Placeholder"
    ):
...
def _GradientsHelper(
    ...
    if any(isinstance(x, composite_tensor.CompositeTensor) for x in xs) or any(
        isinstance(y, composite_tensor.CompositeTensor) for y in ys
    ):
        ...
        flat_grad_ys = (
            None
            if grad_ys is None
            else composite_tensor_gradient.get_flat_tensors_for_gradients(grad_ys)
        )
        flat_grads = _GradientsHelper(
            flat_ys,
            flat_xs,
            flat_grad_ys,
            name,
    ...
    with ops.name_scope(
        name, "gradients", list(ys) + list(xs) + list(stop_gradients) + list(grad_ys)
    ) as grad_scope:
        ...
        while queue:
            ...
            with _maybe_colocate_with(op, gradient_uid, colocate_gradients_with_ops):
                ...
                out_grads = _AggregatedGrads(
                    grads, op, gradient_uid, loop_state, aggregation_method
                )
                if loop_state:
                ...
                if (grad_fn or is_func_call) and has_out_grads:
                    ...
                    for i, out_grad in enumerate(out_grads):
                        if (
                            (
                                not isinstance(out_grad, tensor_lib.Tensor)
                                and not out_grad
                            )
                            and (
                    ...
                    with ops.name_scope(op.name + "_grad"):
                        ...
                        with src_graph._original_op(op):
                            ...
                            if (
                                gate_gradients
                                and len([x for x in in_grads if x is not None]) > 1
                            ):
                                with ops.device(None):
                                    with ops._colocate_with_for_gradient(  # pylint: disable=protected-access
                                        None, gradient_uid, ignore_existing=True
                                    ):
                ...
                for i, (t_in, in_grad) in enumerate(zip(_Inputs(op, xs_set), in_grads)):
                    if in_grad is not None:
                        if (
                            isinstance(in_grad, tensor_lib.Tensor)
            ...
            _UpdatePendingAndEnqueueReady(
                grads, op, queue, pending_count, loop_state, xs_set
            )
...
def _LogOpGradients(op: ops.Operation, out_grads, in_grads):
    ...
    logging.vlog(
        1, "  in  --> %s", ", ".join(x.name for x in out_grads if _FilterGrad(x))
    )
    logging.vlog(
        1, "  out --> %s", ", ".join(x.name for x in in_grads if _FilterGrad(x))
    )
...
def _MultiDeviceAddN(tensor_list, gradient_uid):
    ...
    for dev in sorted(tensors_on_device, key=DeviceKey):
        ...
        with ops._colocate_with_for_gradient(  # pylint: disable=protected-access
            tensors[0].op, gradient_uid, ignore_existing=True
        ):
    ...
    for i, out_grad in enumerate(out_grads):
        ...
        if out_grad:
            if len(out_grad) < 2:
                ...
                logging.vlog(
                    2,
                    "  _AggregatedGrads %d x %s using %s",
                    len(out_grad),
                    tensor_shape,
                    used,
                )
