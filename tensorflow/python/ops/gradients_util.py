...
def _DefaultGradYs(
    ...
    if len(grad_ys) != len(ys):
        raise ValueError(
            f"Length mismatch. Passed {len(grad_ys)} grad_ys for " f"{len(ys)} ys"
        )
...
def _IsFunction(graph):
    return isinstance(graph, FuncGraph) or isinstance(
        graph, framework_function._FuncGraph
    )  # pylint: disable=protected-access
...
def _Captures(func_graph):
    if isinstance(func_graph, FuncGraph):
        ...
        assert isinstance(
            func_graph, framework_function._FuncGraph
        )  # pylint: disable=protected-access
        return func_graph.captures
...
def _GradientsHelper(
    ...
    while _IsFunction(curr_graph):
        ...
        if isinstance(curr_graph, FuncGraph):
            ...
            assert isinstance(
                curr_graph, framework_function._FuncGraph
            )  # pylint: disable=protected-access
    ...
    with ops.name_scope(
        ...
        xs = [
            x.handle if resource_variable_ops.is_resource_variable(x) else x for x in xs
        ]
