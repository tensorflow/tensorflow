...
def _map_coordinate(coord, length, fill_mode):
  ...
  if fill_mode == "WRAP":
    ...
    below = array_ops.where_v2(
        folded_low < -length_f, folded_low + sz2, -folded_low - 1.0
    )
    folded_high = coord - sz2 * _trunc_div(coord, safe_sz2)
    above = array_ops.where_v2(
        folded_high >= length_f, sz2 - folded_high - 1.0, folded_high
    )
  else:
    raise ValueError("Unknown fill_mode %r" % (fill_mode,))

  mapped = array_ops.where_v2(
      coord < 0.0, below, array_ops.where_v2(coord > last, above, coord)
  )
...
def _image_projective_transform_grad_impl(
    images, transforms, grad, interpolation, fill_mode
):
  ...
  *resampling* op on `grad` through the inverted transform, which is only
  ...
  Args:
    ...
    transforms: the forward op's `transforms` input, rank 1 `[8]` or rank 2 `[N,
      8]` with `N` equal to 1 (broadcast) or to the batch size.
  ...
  transform_row = array_ops.where_v2(
      ...
      array_ops.zeros([batch], dtype=dtypes.int32),
      math_ops.range(batch),
  )
  ...
  batch_idx = array_ops.broadcast_to(
      array_ops.reshape(math_ops.range(batch), [-1, 1, 1]), array_ops.shape(x)
  )  # int32, [batch, out_H, out_W]
