...
class LinearOperator(
    ...
    * If `is_X == None` (the default), callers should have no expectation either
    ...
    def inverse(self, name: str = "inverse") -> "LinearOperator":
        ...
        if self.is_square is False:  # pylint: disable=g-bool-id-comparison
            raise ValueError(
                "Cannot take the Inverse: This operator represents a non square matrix."
            )
        if self.is_non_singular is False:  # pylint: disable=g-bool-id-comparison
            raise ValueError(
                "Cannot take the Inverse: This operator represents a singular matrix."
            )
    ...
    def from_operator(cls, operator):
        ...
        for k, v in list(kwargs.items()):
            ...
            if all(is_tensor):
                ...
                raise NotImplementedError(
                    f"Field {k} contains a mix of `Tensor` and  non-`Tensor` values."
                )
