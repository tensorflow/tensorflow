...
def _DenseToCSRSparseMatrixGrad(op: ops.Operation, grad):
    ...
    grad_values = sparse_csr_matrix_ops.csr_sparse_matrix_to_dense(
...
def _PruneSparseTensor(unpruned, pruned_pattern):
    ...
    """
    pruned_indices = sparse_ops.sparse_reshape(pruned_pattern, shape=(-1,)).indices[
        ..., 0
    ]
    ...
    return (
        array_ops.gather_nd(unpruned.indices, keep_indices),
        array_ops.gather_nd(unpruned.values, keep_indices),
        pruned_pattern.dense_shape,
    )
...
def _PruneCSRMatrix(unpruned, pruned_pattern):
    ...
    coo_unpruned = sparse_tensor.SparseTensor(
        *sparse_csr_matrix_ops.csr_sparse_matrix_to_sparse_tensor(unpruned, type=dtype)
    )
...
def _SparseMatrixAddGrad(op: ops.Operation, grad):
    ...
    return (
        sparse_csr_matrix_ops.sparse_matrix_mul(_PruneCSRMatrix(grad, a_csr), alpha),
        sparse_csr_matrix_ops.sparse_matrix_mul(_PruneCSRMatrix(grad, b_csr), beta),
        None,
        None,
    )


def _PrunedDenseMatrixMultiplication(
    a,
    b,
    indices,
    transpose_a=False,
