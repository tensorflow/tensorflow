...
class BaseLinearOperatorIdentity(linear_operator.LinearOperator):
    ...
    def _check_num_rows_possibly_add_asserts(self):
        ...
        if not self._num_rows.dtype.is_integer:
            raise TypeError(
                "Argument num_rows must be integer type.  Found: %s" % self._num_rows
            )
        ...
        if num_rows_static.ndim != 0:
            raise ValueError(
                "Argument num_rows must be a 0-D Tensor.  Found: %s" % num_rows_static
            )

        if num_rows_static < 0:
            raise ValueError(
                "Argument num_rows must be non-negative.  Found: %s" % num_rows_static
            )
    ...
    def _ones_diag(self):
        ...
        if self.shape.is_fully_defined():
            ...
            d_shape = array_ops.concat(
                [self.batch_shape_tensor(), [self._min_matrix_dim_tensor()]], axis=0
            )
    ...
    * If `is_X == None` (the default), callers should have no expectation either
    ...
    def _trace(self):
        ...
        if self._min_matrix_dim() is not None:
            ...
            return (
                math_ops.cast(self._min_matrix_dim_tensor(), self.dtype) * batch_of_ones
            )
    ...
    * If `is_X == None` (the default), callers should have no expectation either
    ...
    def _trace(self):
        ...
        if self._min_matrix_dim() is not None:
            ...
            return (
                self.multiplier
                * math_ops.cast(self._min_matrix_dim_tensor(), self.dtype)
                * batch_of_ones
            )
    ...
    def _cond(self):
        ...
        return array_ops.where_v2(
            math_ops.equal(self._multiplier, 0.0),
            math_ops.cast(np.nan, dtype=self.dtype),
            math_ops.cast(1.0, dtype=self.dtype),
        )
