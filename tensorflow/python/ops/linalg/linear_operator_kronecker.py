...
def _prefer_static_concat_shape(first_shape, second_shape_int_list):
    ...
    if isinstance(first_shape, tensor_shape.TensorShape) and all(
        s is not None for s in second_shape_int_list_static
    ):
...
class LinearOperatorKronecker(linear_operator.LinearOperator):
    ...
    * If `is_X == None` (the default), callers should have no expectation either
    ...
    def _solve_matmul_internal(
        self, x, solve_matmul_fn, adjoint=False, adjoint_arg=False
    ):
    ...
    def _matmul(self, x, adjoint=False, adjoint_arg=False):
        ...
        return self._solve_matmul_internal(
            x=x, solve_matmul_fn=matmul_fn, adjoint=adjoint, adjoint_arg=adjoint_arg
        )
    ...
    def _solve(self, rhs, adjoint=False, adjoint_arg=False):
        ...
        return self._solve_matmul_internal(
            x=rhs, solve_matmul_fn=solve_fn, adjoint=adjoint, adjoint_arg=adjoint_arg
        )
