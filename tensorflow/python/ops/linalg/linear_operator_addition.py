...
def _static_check_for_same_dimensions(operators):
    ...
    if len(set(value for name, value in domain_dimensions)) > 1:
        raise ValueError(
            f"All `operators` must have the same `domain_dimension`. "
    ...
    if len(set(value for name, value in range_dimensions)) > 1:
        raise ValueError(
            f"All operators must have the same `range_dimension`. "
...
class _Hints:
    ...
    def __init__(
        self, is_non_singular=None, is_positive_definite=None, is_self_adjoint=None
    ):
...
def _type(operator):
    ...
    if isinstance(operator, linear_operator_diag.LinearOperatorDiag):
        return _DIAG
    if isinstance(
        operator, linear_operator_lower_triangular.LinearOperatorLowerTriangular
    ):
        return _TRIL
    ...
    raise TypeError(
        f"Expected operator to be one of [LinearOperatorDiag, "
