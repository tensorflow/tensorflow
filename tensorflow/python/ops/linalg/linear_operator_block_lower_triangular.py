    ...
    * If `is_X == None` (the default), callers should have no expectation either
            ...
            for j in range(i):
                ...
                if (
                    ...
                    if op.domain_dimension != above_op.domain_dimension:
                        raise ValueError(
                            f"Argument `operators[{i}][{j}].domain_dimension` "
                ...
                if (
                    ...
                    if op.range_dimension != right_op.range_dimension:
                        raise ValueError(
                            f"Argument `operators[{i}][{j}].range_dimension` "
    ...
    def _validate_square(self, is_square):
        if is_square is False:
            raise ValueError(
                f"`LinearOperatorBlockLowerTriangular` must be square. "
    ...
    def _linop_inverse(self) -> "LinearOperatorBlockLowerTriangular":
        ...
        upper_left_inverse = LinearOperatorBlockLowerTriangular(
            self.operators[:-1]
        ).inverse()
