    ...
    * If `is_X == None` (the default), callers should have no expectation either
    ...
    def __init__(
        ...
        with ops.name_scope(name, values=[perm]):
            ...
            if is_non_singular is False:  # pylint:disable=g-bool-id-comparison
                raise ValueError(
                    f"A Permutation operator is always non-singular. "
            ...
            if is_square is False:  # pylint:disable=g-bool-id-comparison
                raise ValueError(
                    f"A Permutation operator is always square. "
    ...
    def _check_perm(self, perm):
        ...
        if perm.shape.ndims is not None and perm.shape.ndims < 1:
            raise ValueError(
                f"Argument `perm` must have at least 1 dimension. " f"Received: {perm}."
            )
        if not perm.dtype.is_integer:
            raise TypeError(
                f"Argument `perm` must be integer dtype. " f"Received: {perm}."
            )
