...
def exponential_power_convolution_kernel(
    ...
    for i, x in enumerate(array_ops.meshgrid(*ts, indexing="ij")):
        ...
        midpoint = ts[i][
            math_ops.cast(math_ops.floor(one / 2.0 * grid_shape[i]), dtypes.int32)
        ]
        log_vals.append(
            -((math_ops.abs((x - midpoint) / length_scale[i])) ** power) / divisor
        )
    kernel = math_ops.exp(
...
class _BaseLinearOperatorCirculant(linear_operator.LinearOperator):
    ...
    def _linop_matmul(
        ...
        if not isinstance(
            right_operator, _BaseLinearOperatorCirculant
    ...
    def _cond(self):
        ...
        return math_ops.reduce_max(abs_singular_values, axis=-1) / math_ops.reduce_min(
            abs_singular_values, axis=-1
        )
...
class LinearOperatorCirculant(_BaseLinearOperatorCirculant):
    ...
    * Values returned by all methods, such as `matmul` or `determinant` will be
    ...
    def _linop_matmul(
        ...
        if not isinstance(right_operator, LinearOperatorCirculant) or not isinstance(
            left_operator, type(right_operator)
        ):
            return super()._linop_matmul(left_operator, right_operator)
...
class LinearOperatorCirculant2D(_BaseLinearOperatorCirculant):
    ...
    * Values returned by all methods, such as `matmul` or `determinant` will be
    ...
    def _linop_matmul(
        ...
        if not isinstance(right_operator, LinearOperatorCirculant2D) or not isinstance(
            left_operator, type(right_operator)
        ):
            return super()._linop_matmul(left_operator, right_operator)
...
class LinearOperatorCirculant3D(_BaseLinearOperatorCirculant):
    ...
    * If `is_X == None` (the default), callers should have no expectation either
    ...
    def _linop_matmul(
        ...
        if not isinstance(right_operator, LinearOperatorCirculant3D) or not isinstance(
            left_operator, type(right_operator)
        ):
            return super()._linop_matmul(left_operator, right_operator)
