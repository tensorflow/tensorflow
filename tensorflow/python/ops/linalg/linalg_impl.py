...
def logdet(matrix, name=None):
    ...
    with ops.name_scope(name, "logdet", [matrix]):
        ...
        return 2.0 * math_ops.reduce_sum(
            math_ops.log(math_ops.real(array_ops.matrix_diag_part(chol))), axis=[-1]
        )
...
def matrix_exponential(input, name=None):  # pylint: disable=redefined-builtin
    ...
    with ops.name_scope(name, "matrix_exponential", [input]):
        ...
        l1_norm = math_ops.reduce_max(
            math_ops.reduce_sum(
                math_ops.abs(matrix), axis=array_ops.size(array_ops.shape(matrix)) - 2
            ),
            axis=-1,
        ...
        def _nest_where(vals, cases):
            ...
            if len(vals) == 1:
                ...
                return array_ops.where_v2(
                    math_ops.less(l1_norm, const(vals[0])),
                    cases[0],
        ...
        if matrix.dtype in [dtypes.float16, dtypes.float32, dtypes.complex64]:
            ...
            squarings = math_ops.maximum(
                math_ops.floor(
                    math_ops.log(l1_norm / maxnorm) / math_ops.log(const(2.0))
                ),
                0,
            )
        ...
        result = control_flow_ops.cond(
            is_finite,
            lambda: linalg_ops.matrix_solve(-u + v, u + v),
...
def tridiagonal_solve(
    ...
    **Note**: with large batch sizes, the computation on the GPU may be slow, if
    ...
    if diagonals_format == "sequence":
        ...
        if not subdiag.shape[:-1].is_compatible_with(
            maindiag.shape[:-1]
        ) or not superdiag.shape[:-1].is_compatible_with(maindiag.shape[:-1]):
            raise ValueError(
    ...
    if diagonals_format == "matrix":
        ...
        if m1 and m2 and m1 != m2:
            raise ValueError(
                "Expected last two dimensions of diagonals to be same, got {} and {}".format(
                    m1, m2
                )
            )
        m = m1 or m2
        diagonals = array_ops.matrix_diag_part(
            diagonals, k=(-1, 1), padding_value=0.0, align="LEFT_RIGHT"
        )
        return _tridiagonal_solve_compact_format(
            diagonals,
            rhs,
...
def _tridiagonal_solve_compact_format(
    diagonals,
    rhs,
    transpose_rhs,
    conjugate_rhs,
    ...
    if diags_rank:
        ...
        if rhs_rank and not diagonals.shape[:-2].is_compatible_with(
            ...
            raise ValueError(
                "Batch shapes {} and {} are incompatible".format(
    ...
    if rhs_rank and diags_rank and rhs_rank == diags_rank - 1:
        ...
        return array_ops.squeeze(
            linalg_ops.tridiagonal_solve(
                diagonals, rhs, partial_pivoting, perturb_singular, name
            ),
            -1,
        )
    ...
    check_num_lhs_matches_num_rhs()
    return linalg_ops.tridiagonal_solve(
        diagonals, rhs, partial_pivoting, perturb_singular, name
    )
    ...
    with ops.name_scope(name or "lu_solve"):
        ...
        if rhs.shape.rank == 2 and perm.shape.rank == 1:
            ...
            broadcast_batch_shape = array_ops.broadcast_dynamic_shape(
                rhs_shape[:-2], array_ops.shape(perm)[:-1]
            )
        ...
        lower = set_diag(
            ...
            array_ops.ones(array_ops.shape(lower_upper)[:-1], dtype=lower_upper.dtype),
        )
...
def lu_reconstruct(lower_upper, perm, validate_args=False, name=None):
    ...
    with ops.name_scope(name or "lu_reconstruct"):
        ...
        if (
            lower_upper.shape is None
            or lower_upper.shape.rank is None
            ...
            x = array_ops.gather_nd(x, array_ops.stack([batch_indices, perm], axis=-1))
    ...
    with ops.name_scope(name or "eigh_tridiagonal"):
        ...
        def _compute_eigenvalues(alpha, beta):
            ...
            def _sturm(alpha, beta_sq, pivmin, alpha0_perturbation, x):
                ...
                with ops.name_scope("sturm"):
                    ...
                    def sturm_step(i, q, count):
                        ...
                        q = array_ops.where(
                            q <= pivmin, math_ops.minimum(q, -pivmin), q
                        )
                        return q, count
            ...
            with ops.name_scope("compute_eigenvalues"):
                ...
                alpha0_perturbation = array_ops.broadcast_to(
                    alpha0_perturbation, target_shape
                )
                ...
                _, lower, upper = control_flow_ops.while_loop(
                    continue_binary_search, binary_search_step, [0, lower, upper]
                )
        ...
        def _compute_eigenvectors(alpha, beta, eigvals):
            ...
            with ops.name_scope("compute_eigenvectors"):
                ...
                ortho_interval_end = (
                    array_ops.squeeze(array_ops.where_v2(ortho_interval_end), axis=-1)
                    + 1
                )
                ...
                _, v, nrm_v, _ = control_flow_ops.while_loop(
                    continue_iteration, inverse_iteration_step, [0, v0, nrm_v, zero_nrm]
                )
                return transpose(v)
