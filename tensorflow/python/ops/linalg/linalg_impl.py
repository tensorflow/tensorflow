...
from tensorflow.python.ops import control_flow_ops  # pylint: disable=unused-import
...
def logdet(matrix, name=None):
  ...
  with ops.name_scope(name, 'logdet', [matrix]):
    ...
    # For complex matrices, numerical noise in LU decomposition may cause
    # real(sign) to deviate slightly from 1.0 for positive definite matrices.
    # We use a small tolerance to account for this.
    # We also return the real part of log_abs_det to keep the output real,
    # matching numpy behavior.
    return array_ops.where_v2(
        math_ops.greater(math_ops.real(sign), 0.99),
        math_ops.real(log_abs_det),
        math_ops.cast(float('nan'), math_ops.real(log_abs_det).dtype),
    )
