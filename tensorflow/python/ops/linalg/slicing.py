...
def _broadcast_parameter_with_batch_shape(
    ...
    return array_ops.broadcast_to(
        param, array_ops.broadcast_dynamic_shape(base_shape, array_ops.shape(param))
    )
    ...
    for slc in slices:
        if slc is Ellipsis:  # Switch over to negative indexing.
            ...
            num_remaining_non_newaxis_slices = sum(
                s is not array_ops.newaxis for s in slices[slices.index(Ellipsis) + 1 :]
            )
...
def _slice_single_param(param, param_ndims_to_matrix_ndims, slices, batch_shape):
    ...
    if (
        tensor_util.constant_value(array_ops.size(batch_shape)) != 0
        and tensor_util.constant_value(array_ops.size(param_batch_shape)) == 0
    ):
        return param
...
def batch_slice(linop, params_overrides, slices):
    ...
    if len(slices) == 1 and slices[0] is Ellipsis:
        ...
        ) in linop._experimental_parameter_ndims_to_matrix_ndims.items():  # pylint:disable=protected-access,line-too-long
