...
def for_loop(loop_fn, loop_fn_dtypes, iters, parallel_iterations=None):
    ...
    output = [
        None if is_none else ta.stack() for ta, is_none in zip(ta_list, is_none_list)
    ]
    ...
    if not output:
        ...
        try:
            ...
            out_shapes = [
                [0] + ops.convert_to_tensor(x).shape for x in nest.flatten(loop_fn_out)
            ]
            output = [
...
def pfor(
    loop_fn, iters, fallback_to_while_loop=True, parallel_iterations=None, warn=False
):
...
def _should_expand_composite(value):
    return (
        isinstance(value, composite_tensor.CompositeTensor)
...
def _composite_to_tensors(value, is_batched=False):
    ...
    if _should_expand_composite(value):
        ...
        if not isinstance(spec, type_spec.BatchableTypeSpec):
            raise ValueError(
                f"CompositeTensor instance {value} returned from "
...
# pylint: disable=protected-access
def _composite_from_tensors(stacked_tensors, preconverted_value, batch_size):
...
def _pfor_impl(
    ...
    with ops.name_scope("loop_body"):
        ...
        loop_fn_output_tensors = nest.map_structure(
            _composite_to_tensors, loop_fn_outputs
        )
    ...
    for loop_fn_output in nest.flatten(loop_fn_output_tensors):
        if loop_fn_output is not None and not isinstance(
            loop_fn_output, (ops.Operation, tensor.Tensor, sparse_tensor.SparseTensor)
        ):
    ...
    loop_fn_output_tensors = nest.pack_sequence_as(
        loop_fn_output_tensors, tmp_loop_fn_outputs
    )
    ...
    if parallel_iterations is None:
        ...
        with ops.name_scope("pfor_tiled"):
            loop_fn_dtypes = [
                ops.convert_to_tensor(x).dtype for x in flattened_output_tensors
            ]
            ...
            if iters_value is None or iters_value % parallel_iterations:
                output_tensors = cond.cond(
                    ...
                    lambda: [
                        ...
                        for x, y in zip(remaining_output_tensors, tiled_output_tensors)
                    ],
                )
            else:
    ...
    return nest.map_structure_up_to(
        ...
        nest.pack_sequence_as(loop_fn_output_tensors, flattened_output_tensors),
        loop_fn_outputs,
    )
...
def _gather_from_tensor_or_composite(x, i):
    ...
    if _should_expand_composite(x):
        spec = x._type_spec
        gathered_tensors = [
            _broadcasting_gather(t, i) for t in spec._to_batched_tensor_list(x)
        ]
...
def vectorized_map(fn, elems, fallback_to_while_loop=True, warn=True):
    ...
    elems = nest.map_structure(ops.convert_to_tensor, elems, expand_composites=True)
    ...
    flat_elems = nest.flatten(
        nest.map_structure(
            functools.partial(_composite_to_tensors, is_batched=True), elems
        )
    )
    ...
    return pfor(
        loop_fn, batch_size, fallback_to_while_loop=fallback_to_while_loop, warn=warn
    )
