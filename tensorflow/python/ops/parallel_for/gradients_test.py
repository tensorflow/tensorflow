...
def lstm_model_fn(batch_size, state_size, steps, inputs_size=None):
    ...
    inputs = [random_ops.random_normal([batch_size, inputs_size]) for _ in range(steps)]
...
def dynamic_lstm_model_fn(batch_size, state_size, max_steps):
    ...
    sequence_length = constant_op.constant(
        np.random.randint(0, size=[batch_size], high=max_steps + 1), dtype=dtypes.int32
    )

    cell = rnn_cell.BasicLSTMCell(state_size)
    initial_state = cell.zero_state(batch_size, dtypes.float32)
    return inputs, rnn.dynamic_rnn(
        cell, inputs, sequence_length=sequence_length, initial_state=initial_state
    )


def create_fc_batch_jacobian(batch_size, activation_size, num_layers):
    inp, output = fully_connected_model_fn(batch_size, activation_size, num_layers)
...
def create_lstm_batch_jacobian(batch_size, state_size, steps, inputs_size=None):
    inp, output = lstm_model_fn(batch_size, state_size, steps, inputs_size=inputs_size)
    pfor_jacobian = gradients.batch_jacobian(output, inp, use_pfor=True)
    while_jacobian = gradients.batch_jacobian(output, inp, use_pfor=False)
    return pfor_jacobian, while_jacobian
...
def create_dynamic_lstm_batch_jacobian(batch_size, state_size, max_steps):
    inp, (_, final_state) = dynamic_lstm_model_fn(batch_size, state_size, max_steps)
...
def create_fc_per_eg_grad(batch_size, activation_size, num_layers):
    inp = random_ops.random_normal([batch_size, activation_size])
    layers = [
        tf_layers.Dense(activation_size, activation=nn.relu) for _ in range(num_layers)
    ]
...
def create_lstm_per_eg_grad(batch_size, state_size, steps, inputs_size=None):
    ...
    def loop_fn(i):
        loop_inputs = [array_ops.expand_dims(array_ops.gather(x, i), 0) for x in inputs]
...
def create_mnist_autobatch(batch_size, data_format, training):
    ...
    pfor_outputs = control_flow_ops.pfor(loop_fn, batch_size)
    while_outputs = control_flow_ops.for_loop(loop_fn, dtypes.float32, batch_size)
...
def create_mnist_per_eg_grad(batch_size, data_format, training):
    images = random_ops.random_uniform([batch_size, 28, 28])
    sparse_labels = np.random.randint(low=0, high=10, size=[batch_size]).astype(
        np.int32
    )
    labels = np.zeros((batch_size, 10)).astype(np.float32)
    labels[np.arange(batch_size), sparse_labels] = 1.0
    model = Mnist(data_format)
    ...
    while_outputs = control_flow_ops.for_loop(
        loop_fn, [dtypes.float32] * len(variables.trainable_variables()), batch_size
    )
    return pfor_outputs, while_outputs
...
def create_mnist_per_eg_jacobian(batch_size, data_format, training):
    ...
    while_outputs = control_flow_ops.for_loop(
        ...
        [dtypes.float32] * len(variables.trainable_variables()),
        batch_size,
    )
    return pfor_outputs, while_outputs
...
def create_fc_per_eg_jacobians(batch_size, activation_size, num_layers):
    model = FullyConnectedModel(activation_size=activation_size, num_layers=num_layers)
    inp = random_ops.random_normal([batch_size, activation_size])
    output = model(inp)
...
class GradientsTest(test.TestCase):
    ...
    def test_jacobian_while_loop_shape(self):
        # Shape x: [3, 4]
        x = random_ops.random_uniform([3, 4])
        _, y = tf_control_flow_ops.while_loop(
            lambda i, a: i > 5.0,
            lambda i, a: (i + 1, a + i),
            (constant_op.constant(0.0), x),
        )
        # Shape y: [2, 3]
        y = y[:2, :3]
        jacobian = gradients.jacobian(y, x)
    ...
    def test_jacobian_unknown_shape(self):
        with self.cached_session() as sess:
            ...
            ans, pfor_value, while_value = sess.run(
                [answer, jacobian_pfor, jacobian_while], feed_dict={x: [[1, 2], [3, 4]]}
            )
    ...
    def test_jacobian_parallel_iterations(self):
        ...
        self.assertAllClose(
            gradients.jacobian(y, x, parallel_iterations=2),
    ...
    def test_batch_jacobian_fixed_shape(self):
        ...
        two_x = 2 * x
        answer = array_ops.stack([array_ops.diag(two_x[0]), array_ops.diag(two_x[1])])
    ...
    def test_batch_jacobian_unknown_shape(self):
        with self.cached_session() as sess:
            ...
            answer = array_ops.stack(
                [array_ops.diag(two_x[0]), array_ops.diag(two_x[1])]
            )
            ans, pfor_value, while_value = sess.run(
    ...
    def test_batch_jacobian_parallel_iterations(self):
        ...
        self.assertAllClose(
            gradients.batch_jacobian(y, x, parallel_iterations=2),
    ...
    def test_mnist_per_eg_grad(self):
        ...
        data_format = "channels_first" if test.is_gpu_available() else "channels_last"
    ...
    def test_fc_jacobian(self):
        (
            jacobians,
            per_eg_jacobians_pfor,
            per_eg_jacobians_while,
        ) = create_fc_per_eg_jacobians(batch_size=8, activation_size=4, num_layers=2)
        self.run_and_assert_equal(
            jacobians, per_eg_jacobians_pfor, rtol=2e-3, atol=1e-3
        )
        self.run_and_assert_equal(
            jacobians, per_eg_jacobians_while, rtol=2e-3, atol=1e-3
        )
...
class GradientsBenchmarks(test.Benchmark):
    ...
    def benchmark_mnist_autobatch(self):
        with ops.Graph().as_default():
            data_format = (
                "channels_first" if test.is_gpu_available() else "channels_last"
            )
    ...
    def benchmark_fc_per_eg_jacobian(self):
        with ops.Graph().as_default():
            (
                ...
                per_eg_jacobians_pfor,
                per_eg_jacobians_while,
            ) = create_fc_per_eg_jacobians(
                batch_size=128, activation_size=32, num_layers=3
            )
            self._run(jacobians, 30, name="fc_jacobians_pfor")
            self._run(per_eg_jacobians_pfor, 100, name="fc_per_eg_jacobians_pfor")
            self._run(per_eg_jacobians_while, 10, name="fc_per_eg_jacobians_while")
