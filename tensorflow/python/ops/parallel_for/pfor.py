...
flags.DEFINE_bool(
    "op_conversion_fallback_to_while_loop", True, "DEPRECATED: Flag is ignored."
)
...
def _parse_variant_shapes_and_types(t):
    ...
    if shapes_and_types[0].type.type_id == full_type_pb2.TFT_ARRAY:
        ...
        if shapes_and_types[0].type.type_id == full_type_pb2.TFT_UNSET:
            ...
            raise ValueError(
                "Attempted to stack a variant-dtype tensor with no type set ({!r})".format(
                    t
                )
            )
...
class WhileOp(object):
    ...
    def _init_pfor(self, parent_pfor, indices, cond_stacked, inputs, inputs_stacked):
        ...
        for i in range(num_outputs):
            ...
            pfor._add_conversion(
                merge.outputs[1], wrap(constant_op.constant(-1.0), False)
            )
            ...
            for i, inp in enumerate(inputs):
                ...
                if i < num_outputs:
                    ...
                    if enter.op in self._pfor_ops:
                        body_output_stacked = self._maybe_stacked(
                            maybe_stacked_cache, body_output
                        )
                    else:
        ...
        for i, out_ta in enumerate(output_tas):
            ...
            new_output_tas.append(
                control_flow_ops.cond(
                    not_all_done, lambda: out_ta, lambda: out_ta.write(0, inp)
                )
            )
        # pylint: enable=cell-var-from-loop
        return not_all_done, indices, inputs, new_output_tas

    def _process_cond_stacked(
        self, conditions, indices, inputs, inputs_stacked, output_tas
    ):
        num_outputs = len(self._outputs)
    ...
    def _process_body(
        ...
        body_pfor = self._init_pfor(
            pfor_input.pfor, new_indices, cond_stacked, new_inputs, inputs_stacked
        )
        new_outputs = []
    ...
    def __call__(self, pfor_input):
        ...
        def body(not_all_done, indices, *args):
            ...
            with ops.name_scope("while_body"):
                ...
                new_outputs = self._process_body(
                    pfor_input,
                    inputs_stacked,
                    new_indices,
                    cond_stacked,
...
WrappedTensor = collections.namedtuple(
    "WrappedTensor", ["t", "is_stacked", "is_sparse_stacked"]
)
...
def wrap(tensor, is_stacked=True, is_sparse_stacked=False):
    ...
    assert not is_sparse_stacked or is_stacked, (
        "If the wrapped tensor is "
...
def _fallback_converter(pfor_input, warn=True):
    ...
    def while_body(i, *ta_list):
        ...
        op_outputs = _create_op(
            pfor_input.op_type, inputs, output_dtypes, attrs=pfor_input.op.node_def.attr
        ).outputs
    ...
    ta_list = control_flow_ops.while_loop(
        lambda i, *ta: i < iters,
        while_body,
        [0] + [tensor_array_ops.TensorArray(dtype, iters) for dtype in output_dtypes],
    )[1:]
...
class PForConfig(object):
    ...
    def reduce(self, fn, *args):
        ...
        for arg in args:
            ...
            batched_shape = tensor_shape.TensorShape([self._maybe_iters]).concatenate(
                arg.shape
            )
            tensor_specs.append(
        ...
        with ops.control_dependencies(args):
            for output in concrete_function.outputs:
                if not isinstance(output, ops.Tensor):
                    raise ValueError(
                        f"Got a non-Tensor output {output} while running " "reduce."
                    )
    ...
    def _convert_helper(self, op_or_tensor):
        ...
        while stack:
            ...
            if y in self._conversion_map:
                assert isinstance(
                    self._conversion_map[y], (WrappedTensor, ops.Operation)
                )
                stack.popleft()
                continue
            ...
            with ops.control_dependencies(control_dependencies), ops.name_scope(
                y_op.name + "/pfor/"
            ), ops.get_default_graph()._original_op(y_op):
                ...
                # None of the inputs and control inputs were converted.
                    ...
                    or (
                        not is_stateful
                        and not some_input_converted
                        and not some_control_input_converted
                    )
                ) and y.graph == ops.get_default_graph():
                    if y is y_op:
                    ...
                    if converter is None:
                        has_variant_outputs = any(
                            x.dtype == dtypes.variant for x in y_op.outputs
                        )
                        has_vectorized_variant_inputs = any(
                            _is_variant_with_internal_stacking(x) for x in y_op.inputs
                        )
                        if (
                            self._fallback_to_while_loop
                            and not has_variant_outputs
                            and not has_vectorized_variant_inputs
                        ):
                            converter = _fallback_converter
                        else:
                            message = (
                                f"No pfor vectorization defined for {y_op.type}\n"
                                f"{y_op}\n inputs: {converted_inputs}."
                            )
                            if not self._fallback_to_while_loop:
                                message += (
                                    "Consider enabling the fallback_to_while_loop "
                    ...
                    try:
                        try:
                            ...
                            has_vectorized_variant_inputs = any(
                                _is_variant_with_internal_stacking(x)
                                for x in y_op.inputs
                            )
                            if (
                                self._fallback_to_while_loop
                                and not has_vectorized_variant_inputs
                                ...
                                six.reraise(
                                    ValueError, ValueError(str(e)), sys.exc_info()[2]
                                )
                    ...
                    assert isinstance(
                        new_outputs, (list, tuple, ops.Operation)
                    ), new_outputs
                ...
                if y is y_op:
                    ...
                    for old_output, new_output in zip(y_op.outputs, new_outputs):
                        assert isinstance(new_output, WrappedTensor), (
                            new_output,
                            y,
                            y_op,
                        )
                        assert old_output.dtype == new_output.t.dtype, (
                            new_output,
                            y,
                            y_op,
                        )
                        ...
                        if not new_output.is_sparse_stacked:
                            if new_output.is_stacked:
                                loop_len = tensor_util.constant_value(
                                    self.loop_len_vector
                                )
                                if loop_len is None:
...
def _convert_flatten_batch_shape_input(pfor_input, op_type, flatten_dims, shape_dim):
    del op_type
...
def _convert_depthwise_conv2d_native_backprop_input(pfor_input):
    ...
    input_sizes_mutipliers = [
        constant_op.constant([1] * c_dim, dtype=dtypes.int32),
        [stack_size],
    ]
    if c_dim < 3:
        input_sizes_mutipliers += [
    ...
    result = _create_op(
        ...
        [input_sizes, kernel, out_backprop],
        [x.dtype for x in pfor_input.outputs],
        attrs=pfor_input.op.node_def.attr,
    ).outputs[0]
...
def _convert_fill(pfor_input):
    ...
    new_shape = array_ops.concat(
        [[-1], array_ops.ones([array_ops.size(dims)], dtype=dtypes.int32)], axis=0
    )
...
def _convert_broadcast_to(pfor_input):
    ...
    return wrap(
        array_ops.broadcast_to(array_ops.reshape(t, t_expanded_shape), new_shape), True
    )
...
def _convert_matrix_band_part(pfor_input):
    ...
    return wrap(
        array_ops.matrix_band_part(t, num_lower=num_lower, num_upper=num_upper), True
    )
...
def _convert_slice(pfor_input):
    ...
    if not begin_stacked:
        ...
        original_unstacked_shape = _stack(
            array_ops.shape(t)[1:], pfor_input.pfor.loop_len_vector
        ).t
        broadcast_size = _stack(size, pfor_input.pfor.loop_len_vector).t
        result_shape = array_ops.where(
            math_ops.less(broadcast_size, 0),
            original_unstacked_shape - begin + broadcast_size + 1,
            broadcast_size,
        )
        result_shape = math_ops.cast(
            math_ops.reduce_max(result_shape, axis=0), dtypes.int64
        )
        ...
        result_flat = array_ops.gather_nd(
            params=t, indices=result_coordinates, batch_dims=1
        )
...
def _convert_pack(pfor_input):
    ...
    if axis >= 0:
        axis += 1
    return wrap(array_ops.stack([x.t for x in pfor_input.inputs], axis=axis), True)
        ...
        if batch_dims != 0:
            ...
            order = array_ops.concat(
                [
                    ...
                    [0],
                    math_ops.range(batch_dims_pos + 1, array_ops.rank(indices)),
                ],
                axis=0,
            )
...
def _convert_concatv2(pfor_input):
    ...
    return wrap(
        array_ops.concat([x.t for x in pfor_input.inputs[: n - 1]], axis=axis), True
    )
...
def _convert_strided_slice_grad(pfor_input):
    ...
    shape = array_ops.concat(
        [math_ops.cast(pfor_input.pfor.loop_len_vector, shape.dtype), shape], axis=0
    )
    begin = array_ops.concat([[0], begin], axis=0)
    end = array_ops.concat([[0], end], axis=0)
    strides = array_ops.concat([[1], strides], axis=0)
...
def _convert_roll(pfor_input):
    ...
    if not shift_stacked:
        ...
        axis_segment_ids = (
            math_ops.range(loop_len, dtype=dtypes.int64)[:, None] * num_unstacked_axes
            + axis[None, :]
        )
        axis_offsets = array_ops.reshape(
            math_ops.unsorted_segment_sum(
                data=shift,
                segment_ids=axis_segment_ids,
        ...
        result_coordinates = (
            math_ops.range(num_unstacked_elements, dtype=dtypes.int64)[None, :, None]
            // cumsize[None, None, :]
            - axis_offsets[:, None, :]
...
def _convert_matmul(pfor_input):
    ...
    if a_stacked and b_stacked:
        ...
        if a.shape.is_fully_defined():
            ...
            x, y, z = [
                array_ops.reshape(i, []) for i in array_ops.split(array_ops.shape(a), 3)
            ]
        ...
        if b.shape.is_fully_defined():
            ...
            x, y, z = [
                array_ops.reshape(i, []) for i in array_ops.split(array_ops.shape(b), 3)
            ]
...
def _convert_clip_by_value(pfor_input):
    ...
    return wrap(gen_math_ops.clip_by_value(t, clip_value_min, clip_value_max), True)
...
def _convert_unsortedsegmentsum(pfor_input, _, op_func):
    ...
    segment_offset = array_ops.reshape(
        segment_offset, array_ops.concat([[n], ones], axis=0)
    )
...
@RegisterPForWithArgs(
    "SparseSegmentMeanWithNumSegments", math_ops.sparse_segment_mean_v2
)
@RegisterPForWithArgs(
    "SparseSegmentSqrtNWithNumSegments", math_ops.sparse_segment_sqrt_n_v2
)
...
def _convert_equal(pfor_input):
    ...
    return wrap(
        gen_math_ops.equal(x, y, incompatible_shape_error=incompatible_shape_error),
        True,
    )
...
def _convert_not_equal(pfor_input):
    ...
    incompatible_shape_error = pfor_input.get_attr("incompatible_shape_error")
    return wrap(
        gen_math_ops.not_equal(x, y, incompatible_shape_error=incompatible_shape_error),
        True,
    )
...
def _convert_shape(pfor_input):
    out_type = pfor_input.get_attr("out_type")
    return wrap(
        array_ops.shape(pfor_input.stacked_input(0), out_type=out_type)[1:], False
    )
...
def _convert_shape_n(pfor_input):
    ...
    shapes = [
        ...
        if stacked
        else array_ops.shape(x, out_type=out_type)
        for x, stacked, _ in pfor_input.inputs
    ]
...
def _convert_size(pfor_input):
    ...
    return wrap(
        array_ops.size(pfor_input.stacked_input(0), out_type=out_type) // n, False
    )
...
def _convert_select(pfor_input):
    ...
    cond, t, e = smart_cond.smart_cond(
        cond_rank > 1,
        lambda: _inputs_with_flattening(pfor_input, [0, 1, 2]),
        lambda: [cond, t, e],
    )
    outputs = _create_op(
        pfor_input.op_type,
    ...
    n = pfor_input.pfor.loop_len_vector
    out = smart_cond.smart_cond(
        cond_rank > 1, lambda: _unflatten_first_dim(outputs[0], n), lambda: outputs[0]
    )
...
def _transpose_dim_to_front(x, dim):
    ...
    return array_ops.transpose(
        ...
        perm=array_ops.concat(
            [[dim], math_ops.range(0, dim), math_ops.range(dim + 1, rank)], axis=0
        ),
    )
...
def _convert_random(pfor_input, op_type, *args, **kw_args):
    ...
    # inputs[0] is "shape"
    inputs[0] = array_ops.concat([pfor_input.pfor.loop_len_vector, inputs[0]], axis=0)
    logging.warning(
...
def _convert_multinomial(pfor_input):
    ...
    if logits_stacked:
        ...
        samples = gen_random_ops.multinomial(
            logits, num_samples * n, seed=seed, seed2=seed2, output_dtype=output_dtype
        )
...
def _convert_einsum(pfor_input, op_type):
    ...
    inputs, input_stacked, _ = zip(
        *[pfor_input.input(i) for i in range(pfor_input.num_inputs)]
    )
...
def _convert_tensor_array_write_v3(pfor_input):
    ...
    if value_stacked and pfor_input.pfor.all_indices_partitioned:
        ...
        raise ValueError(
            "Writing non loop invariant values to TensorArray from "
...
def _convert_tensor_array_scatter_v3(pfor_input):
    ...
    if is_inside:
        ...
        # stacked or not.
        if not flow_stacked and not value_stacked:
            flow_out = data_flow_ops.tensor_array_scatter_v3(
                handle, indices, value, flow
            )
            return wrap(flow_out, False)
        if not value_stacked:
...
def _convert_tensor_array_grad_v3(pfor_input):
    ...
    grad_handle, flow_out = data_flow_ops.tensor_array_grad_with_shape(
        handle=handle, flow_in=flow, shape_to_prepend=shape_to_prepend, source=source
    )
...
def _convert_optional_from_value(pfor_input):
    pfor_input.stack_inputs()
    return wrap(
        gen_dataset_ops.optional_from_value([x.t for x in pfor_input.inputs]), True
    )
...
def _convert_optional_get_value(pfor_input):
    ...
    results = gen_dataset_ops.optional_get_value(handle, output_types, output_shapes)
...
def _convert_tensor_list_reserve(pfor_input):
    ...
    element_shape = _stack_tensor_list_shape(
        element_shape, pfor_input.pfor.loop_len_vector
    )
...
def _stack_tensor_list(handle, dtype, loop_len_vector, element_shape=None):
    ...
    return control_flow_ops.while_loop(
        lambda i, _: i < length, _body_fn, [0, new_handle]
    )[1]
        ...
        if index_stacked:
            ...
            def _map_fn(i):
                item_i = list_ops.tensor_list_get_item(
                    handle, index[i], element_dtype=element_dtype
                )
...
def _convert_tensor_array_set_item(pfor_input):
    ...
    if not handle_stacked:
        ...
        if index is pfor_input.pfor.all_indices:
            ...
            handle = _stack_tensor_list(
                handle, item.dtype, pfor_input.pfor.loop_len_vector
            )
    else:
        handle = _untile_variant(handle)
    ...
    if index_stacked:
        ...
        raise ValueError(
            "Vectorizing writes to a TensorList with loop "
...
def _convert_tensor_list_push_back(pfor_input):
    ...
    if handle_stacked:
        ...
        handle = _stack_tensor_list(
            handle, tensor.dtype, pfor_input.pfor.loop_len_vector
        )
...
def _convert_tensor_list_concat_v2(pfor_input):
    ...
    new_handle = control_flow_ops.while_loop(
        lambda i, _: i < length, _transpose_elem, [0, new_handle]
    )[1]
...
def _convert_tensor_list_stack(pfor_input):
    ...
    output = list_ops.tensor_list_stack(
        handle, element_dtype, element_shape=input_shape, num_elements=num_elements
    )
    output = _transpose_first_two_dims(output)
    return wrap(output, True)
...
def _convert_tensor_list_scatter(pfor_input):
    ...
    if indices_stacked:
        ...
        unique_indices_loop_idx = array_ops.reshape(
            array_ops.tile(loop_idx[None, :], [scatters_per_op, 1]), [-1]
        )
        scatter_indices = array_ops.stack(
            [unique_indices.idx, unique_indices_loop_idx], axis=1
        )
...
def _convert_decode_csv(pfor_input):
    ...
    return [
        wrap(t, True)
        for t in parsing_ops.decode_csv(
            lines,
            record_defaults,
...
def _convert_parse_example_v2(pfor_input):
    ...
    if serialized.shape.ndims not in (None, 1):
        raise ValueError(
            "ParseExampleV2 can only be converted if `serialized` "
...
def _convert_function_call(func, converter, inputs):
    ...
    def f(*args):
        ...
        return tuple([converter._convert_helper(x).t for x in func._func_graph_outputs])
...
def _convert_partitioned_call(pfor_input):
    ...
    assert isinstance(
        func.graph, func_graph.FuncGraph
    ...
    if cond_stacked:
        ...
        if pfor_input.pfor.all_indices_partitioned:
            else_indices, then_indices = data_flow_ops.dynamic_partition(
                math_ops.range(pfor_input.pfor.loop_len_vector[0]), cond_int, 2
            )
        else:
        ...
        # Convert "then" branch.
        then_outputs = _outputs_for_branch(
            then_branch.name, true_indices, pfor_input, then_inputs
        )

        # Convert "else" branch.
        else_outputs = _outputs_for_branch(
            else_branch.name, false_indices, pfor_input, else_inputs
        )
        ...
        for then_output, else_output in zip(then_outputs, else_outputs):
            out = data_flow_ops.dynamic_stitch(
                [then_indices, else_indices], [then_output, else_output]
            )
...
class WhileV2(object):
    ...
    def __init__(self, pfor_input):
        ...
        self._cond_func = pfor_input.op.graph._get_function(
            compat.as_bytes(cond_func_name)
        )
        body_func_name = pfor_input.get_attr("body").name
        self._body_func = pfor_input.op.graph._get_function(
            compat.as_bytes(body_func_name)
        )
        if self._cond_func is None or self._body_func is None:
            raise ValueError(
                "Error extracting cond and body functions for op "
        ...
        for i, (inp, out) in enumerate(
            zip(self._body_func.graph.inputs, self._body_func.graph.outputs)
        ):
            if id(inp) == id(out):
                ...
                if variant_type_id in _INTERNAL_STACKING_TYPE_IDS:
                    ...
                    def _init_loop_body(index, output_ta):
                        output_ta = output_ta.write(
                            index,
                            list_ops.tensor_list_reserve(
                                element_shape, loop_len, dtype
                            ),
                        )
                    ...
                    _, output_ta = control_flow_ops.while_loop(
                        lambda index, _: index < length, _init_loop_body, [0, output_ta]
                    )
                else:
                    output_ta = tensor_array_ops.TensorArray(
                        inp.t.dtype, size=loop_len, dynamic_size=False, infer_shape=True
                    )
        ...
        for i, (inp, stacked) in enumerate(zip(inputs, inputs_stacked)):
            ...
            if not pass_through and _variant_type_id(inp) == full_type_pb2.TFT_ARRAY:
                ...
                def _split_vectorized_ta_element(index, new_inp, new_out_ta):
                    elem = list_ops.tensor_list_get_item(
                        inp, index, shape_and_type.dtype, element_shape
                    )
                    if stacked:
                        done_elem, new_elem = data_flow_ops.dynamic_partition(
                            elem, conditions_int, 2
                        )
                        new_inp = list_ops.tensor_list_set_item(
                            new_inp, index, new_elem
                        )
                    else:
    ...
    def _process_body(
        self, inputs_stacked, new_indices, cond_stacked, new_inputs, not_all_done
    ):
        """Convert the body function."""
        ...
        def true_fn():
            """Converts the body function for all but last iteration."""
            wrapped_inputs = [
                wrap(inp, stacked) for inp, stacked in zip(new_inputs, inputs_stacked)
            ]
            ...
            while True:
                ...
                body_pfor = PFor(
                    ...
                    all_indices_partitioned=(
                        self._pfor.all_indices_partitioned or cond_stacked
                    ),
                    pfor_config=self._pfor.pfor_config,
                )
                stacking_mismatch = False
                outputs = _convert_function_call(
                    self._body_func, body_pfor, wrapped_inputs
                )
                        ...
                        if inp.t.dtype == dtypes.variant:
                            stacked = wrap(
                                _tile_variant_with_length(
                                    stacked.t, [array_ops.size(new_indices)]
                                )
                            )
                        ...
                        with ops.control_dependencies(
                            [
                                control_flow_ops.Assert(
                                    False,
                                    ["pfor ERROR: this branch should never execute"],
                                )
                            ]
                        ):
        ...
        # body function.
        return (
            control_flow_ops.cond(not_all_done, true_fn, lambda: list(new_inputs)),
            mismatching_stacked_indices,
        )
    ...
    def __call__(self):
        ...
        def body(not_all_done, indices, *args):
            ...
            with ops.name_scope("while_cond"):
                ...
                wrapped_inputs = [
                    wrap(inp, stacked) for inp, stacked in zip(inputs, inputs_stacked)
                ]
                conditions, cond_stacked, _ = _convert_function_call(
                    self._cond_func, cond_pfor, wrapped_inputs
                )[0]
                cond_is_stacked[0] = cond_stacked
        ...
        def while_fn():
            ...
            ta_shape_invariants = [
                tensor_shape.TensorShape([]) for _ in self._pfor_input.outputs
            ]
            shape_invariants = (
                        ...
                        if i in self._body_pass_through_indices:
                            ...
                            if _variant_type_id(inp) == full_type_pb2.TFT_ARRAY:
                                ...
                                output_list = list_ops.tensor_list_reserve(
                                    tensor_shape.TensorShape(shape_and_type.shape),
                                    length,
                                    shape_and_type.dtype,
                                )
