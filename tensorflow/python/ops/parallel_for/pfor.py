...
class WhileOp:
    ...
    def _process_cond_unstacked(self, conditions, indices, inputs, output_tas):
        ...
        for i, out_ta in enumerate(output_tas):
            ...
            new_output_tas.append(
                tf_cond.cond(not_all_done, lambda: out_ta, lambda: out_ta.write(0, inp))
            )
    ...
    def __call__(self, pfor_input: "_PforInput"):
        ...
        def body(not_all_done, indices, *args):
            ...
            if not cond_stacked:
                (not_all_done, new_indices, new_inputs, new_output_tas) = (
    ...
    def unstacked_input(self, index):
        ...
        if is_stacked:
            ...
            if op_def is None:
                ...
                input_name = '"%s"' % op_def.input_arg[index].name
            raise ConversionNotImplementedError(
                f"Input {input_name} of op '{op_type}' expected to be loop invariant."
            )
        return t
        ...
        while stack:
            ...
            with (
                ...
                # None of the inputs and control inputs were converted.
                    ...
                    try:
                        try:
                            ...
                            if (
                                ...
                                raise ValueError(str(e)).with_traceback(
                                    sys.exc_info()[2]
                                )
                    ...
                    assert isinstance(new_outputs, (list, tuple, ops.Operation)), (
                        new_outputs
                    )
...
def _convert_tensor_list_concat_v2(pfor_input: _PforInput):
    ...
    new_handle = while_loop.while_loop(
...
def _convert_decode_csv(pfor_input: _PforInput):
    ...
    return [
        wrap(t, True)
        for t in gen_parsing_ops.decode_csv(
            lines,
            record_defaults,
    ...
    if is_stacked:
        ...
        for b in branches:
            ...
            def new_function(func=b.name):
                return _outputs_for_branch(
                    func, None, pfor_input, pfor_input.inputs[1:]
                )
...
class WhileV2:
        ...
        with ops.name_scope("while_init"):
            for inp in self._pfor_input.inputs:
                ...
                if variant_type_id in _INTERNAL_STACKING_TYPE_IDS:
                    ...
                    _, output_ta = while_loop.while_loop(
    ...
    def _process_body(
        ...
        # body function.
        return tf_cond.cond(
            not_all_done, true_fn, lambda: list(new_inputs)
        ), mismatching_stacked_indices
