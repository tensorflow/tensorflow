    ...
    **WARNING**: Any Tensors or Operations created outside of `true_fn` and
    ...
    with ops.name_scope(name, "cond", [pred]):
        ...
        for x, y in zip(res_t_flat, res_f_flat):
            assert isinstance(x, tensor_lib.Tensor) and isinstance(y, tensor_lib.Tensor)
...
def _eager_cond_implementation(pred, true_fn, false_fn, strict, name):
    ...
    if pred_constant_value is None:
        ...
        if not isinstance(true_fn, core.PolymorphicFunction) or not isinstance(
            false_fn, core.PolymorphicFunction
        ):
            raise TypeError(
