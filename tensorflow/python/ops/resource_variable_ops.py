...
class BaseResourceVariable(variables.Variable, core.Tensor):
    ...
    def _serialize_to_tensors(self):
        ...
        return {
            trackable.VARIABLE_VALUE_KEY: tensor_callable.Callable(
    ...
    def __init__(
        ...
        if variable_def:
            ...
            self._init_from_proto(
                variable_def, import_scope=import_scope, validate_shape=validate_shape
            )
...
class VariableSpec(tensor_module.DenseSpec):
    ...
    def is_compatible_with(self, spec_or_value):
        ...
        * they share the same alias_id if `spec_or_value` is a `VariableSpec`.
        ...
        if not isinstance(spec_or_value, (type(self), self.value_type)):
            return False
        compatible = (
            self.shape.is_compatible_with(spec_or_value.shape)
            and self.dtype == spec_or_value.dtype
            and self.trainable == spec_or_value.trainable
        )
    ...
    def _from_components(self, components):
        if not isinstance(components, (list, tuple)):
            raise TypeError(
                f"Components of a ResourceVariable must be a list or "
                f"tuple, got f{components} instead."
            )
        if len(components) != 1:
            raise ValueError(
                f"Components of a ResourceVariable must only contain "
                f"its resource handle, got f{components} instead."
            )
        handle = components[0]
        if (
            not isinstance(handle, tensor_module.Tensor)
            or handle.dtype != dtypes.resource
        ):
            raise ValueError(
                f"The handle of a ResourceVariable must be a resource "
                f"tensor, got {handle} instead."
            )
        return ResourceVariable(
            trainable=self.trainable, shape=self.shape, dtype=self.dtype, handle=handle
        )
    ...
    def is_subtype_of(self, other):
        ...
        if self.alias_id is None or other.alias_id is None:
            raise NotImplementedError(
                f"VariableSpec.is_subtype_of doesn't support "
    ...
    def most_specific_common_supertype(self, others):
        ...
        if self.alias_id is None or any(other.alias_id is None for other in others):
            raise NotImplementedError(
                f"VariableSpec.most_specific_common_supertype "
    ...
    def placeholder_value(self, placeholder_context):
        ...
        placeholder.op._set_attr(  # pylint: disable=protected-access
            "_user_specified_name", attr_value_pb2.AttrValue(s=compat.as_bytes(name))
        )
        return variable
    ...
    def __repr__(self):
        return (
            f"{type(self).__name__}(shape={self.shape}, dtype={self.dtype!r}, "
...
def write_object_proto_for_resource_variable(
    resource_variable, proto, options, enforce_naming=True
):
    """Writes additional information of the variable into the SavedObject proto.
