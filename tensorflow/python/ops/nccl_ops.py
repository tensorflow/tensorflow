...
def _all_sum_grad(op, grad):
    ...
    if op.get_attr("reduction") != b"sum":
        raise LookupError(
            "No gradient defined for NcclAllReduce except for " 'reduction="sum".'
        )
...
def _check_device(tensor, expected=None):
    if not device.canonical_name(tensor.device):
        raise ValueError(
            f"Device assignment for tensor={tensor} required for nccl " "collective ops"
        )
    if expected and expected != tensor.device:
        raise ValueError(
            f"Expected device {expected}, got {tensor.device} for " f"tensor={tensor}."
        )
