...
class Conv2DTest(test.TestCase):
    ...
    def _VerifyValues(
        ...
        output_min = value[1]
        output_max = value[2]
        float_output = self._QuantizedOutputToFloat(
            quantized_output, output_min, output_max
        )
    ...
    def _QuantizedOutputToFloat(self, quantized, quantized_min, quantized_max):
        ...
        result = np.array(
            [
                (quantized_min + ((float(x) - lowest_quantized) * range_scale))
    ...
    def _testBadInputSize(
        ...
        with self.assertRaisesRegex(
            (ValueError, errors.InvalidArgumentError), error_regex
        ):
            self.evaluate(
