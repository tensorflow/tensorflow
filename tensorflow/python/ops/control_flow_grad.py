    ...
    if isinstance(op_ctxt, WhileContext):
        ...
        if merge_grad is not None:
            ...
            if grad[1] is not None:
                # pylint: disable=protected-access
                control_flow_ops._AddNextAndBackEdge(
                    merge_grad, grad[1], enforce_shape_invariant=False
                )
                # pylint: enable=protected-access
            return None, None
        ...
        if zero_grad is None:
            ...
            if op.inputs[0].dtype == dtypes.resource:
                return merge([grad[op_ctxt.branch]] * 2, name="cond_resource_grad")[
                    0
                ], None
            return None, None
...
def _EnterGrad(op, grad):
    ...
    if op.get_attr("is_constant"):
        ...
        if isinstance(grad, tensor.Tensor):
            ...
            raise TypeError(
                f"Type {type(grad)} not supported,must be Tensor or Indexed Slices"
            )
    else:
        result = exit(grad)
