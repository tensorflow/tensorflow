...
class PruningMode:
    ...
    def from_str(cls, mode):
        if mode in cls._map:
            ...
            raise ValueError(
                "pruning_mode mode must be one of: {}. Found: {}".format(
                    ", ".join(sorted(cls._map)), mode
                )
            )
...
class QuantileAccumulatorSaveable(saver.BaseSaverBuilder.SaveableObject):
    ...
    def __init__(self, resource_handle, create_op, num_streams, name):
        ...
        bucket_boundaries = get_bucket_boundaries(
            self.resource_handle, self._num_streams
        )
        slice_spec = ""
        specs = []
        ...
        super(QuantileAccumulatorSaveable, self).__init__(
            self.resource_handle, specs, name
        )
...
class QuantileAccumulator:
    ...
    def _serialize_to_tensors(self):
        raise NotImplementedError(
            "When the need arises, TF2 compatibility can be "
