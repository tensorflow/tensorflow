...
def _check_dilated_convolution_shapes(input_tensor, filters, dilations, padding,
                                      num_batch_dims, channels_first,
                                      num_spatial_dims):
  ...
  if input_tensor.shape.rank is None or filters.shape.rank is None:
    return
  if (input_tensor.shape.rank < num_spatial_dims + num_batch_dims + 1 or
      filters.shape.rank < num_spatial_dims):
  ...
  for i in range(num_spatial_dims):
    ...
    if available < dilated:
      raise ValueError(
          ...
          f"available, so the convolution has no valid window.")


def _assert_dilated_convolution_fits(input_tensor, filters, dilations, padding,
                                     num_batch_dims, channels_first,
                                     num_spatial_dims):
  ...
  dilated = (f_spatial - 1) * ops.convert_to_tensor(
      dilations, dtype=f_spatial.dtype) + 1
  check = check_ops.assert_greater_equal(
      available,
      dilated,
      message=("Dilated convolution: the input (plus padding) is smaller than "
...
def convolution_internal(
  ...
  with ops.name_scope(name, default_name, [input, filters]) as name:
    ...
    if not is_dilated_conv or has_tpu_context:
      ...
      _check_dilated_convolution_shapes(input, filters, dilations, padding,
                                        num_batch_dims, channels_first,
                                        num_spatial_dims)
      ...
      if not (isinstance(padding, str) and padding == "SAME"):
        in_shape = (input.shape.as_list()
                    if input.shape.rank is not None else None)
        if in_shape is None or None in in_shape:
          input = _assert_dilated_convolution_fits(
              input, filters, dilations, padding, num_batch_dims,
              channels_first, num_spatial_dims)
