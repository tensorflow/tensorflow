...
def _get_sequence(value, n, channel_index, name):
    ...
    if len_value == 1:
        ...
        raise ValueError(
            f"{name} should be of length 1, {n} or {n + 2}. "
    ...
    def __init__(
        ...
        if conv_dims == 1:
            ...
            if data_format is None or data_format == "NDHWC":
                ...
                raise ValueError(
                    "`data_format` must be 'NDHWC' or 'NCDHW'. "
                    f"Received: data_format={data_format}"
                )
            self.strides = strides
            self.data_format = data_format
...
def dilation2d_v2(
    ...
    if data_format != "NHWC":
        raise ValueError(
            "`data_format` values other  than 'NHWC' are not "
...
class _WithSpaceToBatch:
    ...
    def __init__(
        ...
        if padding == "SAME":
            ...
            raise ValueError(
                "`padding` must be one of 'SAME' or 'VALID'. "
...
def _get_strides_and_dilation_rate(num_spatial_dims, strides, dilation_rate):
    ...
    if dilation_rate is None:
        ...
        raise ValueError(
            f"`len(dilation_rate)` should be {num_spatial_dims}. "
    ...
    if strides is None:
        ...
        raise ValueError(
            f"`len(strides)` should be {num_spatial_dims}. "
            f"Received: strides={strides} of length {len(strides)}"
        )
    strides = np.array(strides, dtype=np.int32)
    if np.any(strides < 1):
        raise ValueError(
            f"all values of `strides` must be positive. Received: strides={strides}"
        )
...
def pool(
    ...
    with ops.name_scope(name, "%s_pool" % (pooling_type.lower()), [input]) as scope:
        ...
        if num_spatial_dims < 1 or num_spatial_dims > 3:
            raise ValueError(
                "`len(window_shape)` must be 1, 2, or 3. Received: "
...
def convert_padding(padding, expected_length=4):
    ...
    if padding == "EXPLICIT":
        raise ValueError(
            "'EXPLICIT' is not a valid value for `padding`. To use "
    ...
    if isinstance(padding, (list, tuple)):
        for i, dim_paddings in enumerate(padding):
            if not isinstance(dim_paddings, (list, tuple)):
                raise ValueError(
                    "When `padding` is a list, each element of `padding` "
...
def conv1d_transpose(
    ...
    with ops.name_scope(
        ...
        output_shape = (
            list(output_shape)
            if not isinstance(output_shape, tensor_lib.Tensor)
            else output_shape
        )
...
def atrous_conv2d_transpose(value, filters, output_shape, rate, padding, name=None):
    ...
    with ops.name_scope(
        ...
        if not output_shape_.get_shape().is_compatible_with(
            ...
            raise ValueError(
                "`output_shape` must have shape (4,). "
    ...
    with ops.name_scope(name, "BiasAdd", [value, bias]) as name:
        if data_format is not None:
            if data_format.startswith("NC"):
                ...
                raise ValueError(
                    "`data_format` must be of the form `N...C` or "
...
def _ensure_xent_args(name, labels, logits):
    if labels is None or logits is None:
        raise ValueError(
            f"Both `labels` and `logits` must be provided for {name}"
...
def _sparse_softmax_cross_entropy_with_rank_2_logits(logits, labels, name):
    if config.is_op_determinism_enabled():
        ...
        cost = array_ops.where(
            math_ops.logical_or(
                math_ops.less(labels, 0), math_ops.greater_equal(labels, class_count)
            ),
            cost_all_nans,
            cost,
        )
    ...
    **Note that to avoid confusion, it is required to pass only named arguments to
    ...
    with ops.name_scope(name, "SparseSoftmaxCrossEntropyWithLogits", [labels, logits]):
        ...
        if logits.get_shape().ndims is not None and logits.get_shape().ndims == 0:
            raise ValueError(f"`logits` cannot be a scalar. Received logits={logits}`")
...
@dispatch.add_dispatch_support
def avg_pool(value, ksize, strides, padding, data_format="NHWC", name=None, input=None):  # pylint: disable=redefined-builtin
...
def max_pool_v2(input, ksize, strides, padding, data_format=None, name=None):
    ...
    if isinstance(padding, (list, tuple)) and data_format == "NCHW_VECT_C":
        raise ValueError(
            "`data_format='NCHW_VECT_C'` is not supported with "
    ...
    if isinstance(padding, (list, tuple)) and n == 3:
        raise ValueError(
            "Explicit padding is not supported with an input "
...
@dispatch.add_dispatch_support
def max_pool(value, ksize, strides, padding, data_format="NHWC", name=None, input=None):  # pylint: disable=redefined-builtin
...
def fractional_max_pool(
    ...
    if config.is_op_determinism_enabled() and (
        not seed or not seed2 or not deterministic
    ):
        raise ValueError(
