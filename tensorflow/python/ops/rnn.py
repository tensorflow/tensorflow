...
def _best_effort_input_batch_size(flat_input):
    ...
    for input_ in flat_input:
        ...
        if shape.rank < 2:
            raise ValueError(
                "Input tensor should have rank >= 2. Received input="
...
def dynamic_rnn(
    ...
    with vs.variable_scope(scope or "rnn") as varscope:
        ...
        if initial_state is not None:
            ...
            if not dtype:
                raise ValueError(
                    "If no initial_state is provided, argument `dtype` "
                    "must be specified"
                )
...
def _dynamic_rnn_loop(
    ...
    if not in_graph_mode:
        final_outputs = nest.map_structure_up_to(
            cell.output_size, lambda x: array_ops_stack.stack(x, axis=0), final_outputs
        )
...
def raw_rnn(cell, loop_fn, parallel_iterations=None, swap_memory=False, scope=None):
    ...
    **NOTE: This method is still in testing, and the API may change.**
    ...
    if not callable(loop_fn):
        raise TypeError(f"Argument `loop_fn` must be a callable. Received: {loop_fn}.")
    ...
    with vs.variable_scope(scope or "rnn") as varscope:
        ...
        (
            ...
            emit_structure,
            init_loop_state,
        ) = loop_fn(time, None, None, None)  # time, cell_output, cell_state, loop_state
...
def static_state_saving_rnn(
    ...
    if state_is_tuple != state_name_tuple:
        raise ValueError(
            "Argument `state_name` should be the same type as "
    ...
    if state_is_tuple:
        ...
        if len(state_name_flat) != len(state_size_flat):
            raise ValueError(
                "Number of elements in argument `state_name` and "
