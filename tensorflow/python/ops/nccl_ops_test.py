...
class NcclTestCase(test.TestCase):
    def _Test(
        ...
        numpy_fn,
        device_sets=(
            ["/device:GPU:1", "/device:GPU:2", "/device:GPU:0"],
        ...
        for dtype in [np.float16, np.float32, np.int32, np.int64, np.float64]:
            ...
            with self.test_session():
                for devices in device_sets:
                    shape = (3, 4)
                    random = (np.random.random_sample(shape) - 0.5) * 1024
                    tensors = []
                    for _ in devices:
...
class SingleReduceTest(NcclTestCase):
    ...
    def testSumGrad(self):
        self._TestGradient(partial(_NcclReduce, nccl_ops.reduce_sum), lambda x, y: x)
...
class BroadcastTest(NcclTestCase):
    ...
    def testBroadcastSingleDevice(self):
        ...
        self._Test(
            _NcclBroadcast, lambda x, y: x, (["/device:GPU:0", "/device:GPU:0"],)
        )
    ...
    def testBroadcastToCpuError(self):
        try:
            ...
            self._Test(
                _NcclBroadcast, lambda x, y: x, (["/device:GPU:0", "/device:CPU:0"],)
            )
