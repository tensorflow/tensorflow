...
class DefFunctionTest(xla_test.XLATestCase):
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/203146273): MLIR bridge does not" "support CollectiveReduceV2"
    )
    ...
    @test_util.disable_mlir_bridge(
        "TODO(b/199737685): MLIR bridge does not" "support tf.unique via jit_compile"
    )
    def testUniqueErrMsg(self):
        ...
        with ops.device("device:{}:0".format(self.device)):
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "Fail to proof the equality"
            ):
    ...
    @test_util.disable_mlir_bridge(
        "MLIR does not support resource update for"
