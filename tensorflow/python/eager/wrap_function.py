...
class VariableHolder(object):
    ...
    def variable_creator_scope(self, next_creator, **kwargs):
        ...
        with ops.name_scope(
            kwargs.get("name", None), "Variable", skip_on_eager=False
        ) as name:
...
def _get_element_from_tensor_info(tensor_info, graph):
    ...
    if encoding == "name":
        ...
        return sparse_tensor.SparseTensor(
            ...
            graph.get_tensor_by_name(tensor_info.coo_sparse.dense_shape_tensor_name),
        )
...
def _lift_single_variable(old_variable, graph, variable_holder):
    ...
    new_variable._initializer_op = (
        old_variable._initializer_op
...
class WrappedFunction(function.ConcreteFunction):
    ...
    def prune(self, feeds, fetches, name=None, input_signature=None):
        ...
        pruned_fn._num_positional_args = len(
            flat_feeds
        )  # pylint: disable=protected-access
        # TODO(kathywu): Enable keyword arguments if an input signature is specified
        pruned_fn._arg_keywords = [
            tensor.op.name for tensor in flat_feeds
        ]  # pylint: disable=protected-access
        return pruned_fn
...
class WrappedGraph(object):
    ...
    def _wrap_function(self, fn, args=None, kwargs=None, signature=None, name=None):
        ...
        fn_outputs = nest.pack_sequence_as(
            self.graph.structured_outputs, flat_fn_outputs
        )
