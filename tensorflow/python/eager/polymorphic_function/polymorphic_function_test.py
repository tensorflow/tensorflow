...
class FunctionTest(test.TestCase, parameterized.TestCase):
    ...
    def testImplementsAttributeBasic(self):
        v = polymorphic_function.function(experimental_implements="func")(
            lambda x, y: x + y
        )
        with context.graph_mode(), self.cached_session():
            ...
            for f in fdefs:
                ...
                if "forward" in name or "backward" in name:
                    ...
                    present += 1
                    self.assertEqual(
                        f.attr[attributes_lib.IMPLEMENTS].s, "func".encode("ascii"), f
                    )
    ...
    def testImplementsAttributeAsNameAttrList(self):
        ...
        v = polymorphic_function.function(experimental_implements=implements_attr)(
            lambda x, y: x + y
        )
        with context.graph_mode(), self.cached_session():
            a = array_ops.placeholder(dtypes.float32, ())
    ...
    def testInputShapeFunctionRelaxationWithStructuredTensors(self):
        ...
        check_trace(  # relax & retrace
            ...
            structured_tensor.StructuredTensor.Spec._from_fields_and_rank(
                fields={"a": tensor_lib.TensorSpec((None,), dtypes.int32)}, rank=0
            ),
        )
    ...
    def testInputShapeFunctionRelaxationWithDatasetIterators(self):
        ...
        check_trace(  # shape=[1, 2]: retrace
            ...
            iterator_ops.IteratorSpec(tensor_lib.TensorSpec([1, 2], dtypes.float32)),
        )
        ...
        check_trace(  # shape=[2, 2]: relax to [None, 2] and retrace
            ...
            iterator_ops.IteratorSpec(tensor_lib.TensorSpec([None, 2], dtypes.float32)),
        )
    ...
    def testShareRendezvous(self):
        ...
        def fn(n):
            functional_ops.While(
                [n], cond_fn.get_concrete_function(), send_body.get_concrete_function()
            )
            return functional_ops.While(
                [n], cond_fn.get_concrete_function(), recv_body.get_concrete_function()
            )
    ...
    def testVariableSpecWithInputSignature(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "input_signature doesn't support VariableSpec"
        ):
    ...
    def testStructuredSignatureAndMultipleVariables(self):
        self.skipTest(
            "b/209081027: Enable this test after Variable becomes a "
    ...
    def testShapeInferencePropagateConstNestedStack(self):
        ...
        @polymorphic_function.function(
            input_signature=[tensor_lib.TensorSpec(shape=(3, 6), dtype=dtypes.int32)]
        )
        def g(x):
            y = f(x, s=5)
    ...
    def test_functools_partial(self):
        self.assertAllClose(
            3.0,
            polymorphic_function.function(functools.partial(lambda x, y: x + y, 1.0))(
                constant_op.constant(2.0)
            ),
        )
    ...
    def test_concrete_function_from_signature(self):
        ...
        self.assertEqual(
            signature_args, (tensor_lib.TensorSpec(None, dtypes.float32, name="x"),)
        )
    ...
    def test_serialization_signature_cache(self):
        ...
        self.assertEqual(
            ...
            set(
                (
                    (
                        tensor_lib.TensorSpec([1, 2], dtypes.float32, name="x"),
    ...
    def testErrorMessageWhenGraphTensorIsPassedToEager(self):
        ...
        with self.assertRaisesRegex(
            TypeError, re.compile("polymorphic_function_test.*out of scope", re.DOTALL)
        ):
            failing_function()
...
class MultiDeviceTest(test.TestCase, parameterized.TestCase):
    ...
    def testRaiseReplaceCaptureWithDeferredTypeSpecMismatch(self):
        ...
        def fn():
            deferred_tensor = ops.get_default_graph().capture_call_time_value(
                lambda: value, tensor_lib.TensorSpec(shape=(1,), dtype=dtypes.float32)
            )
    ...
    @parameterized.parameters(
        (1, int, 2, int, 1), (1, constant_op.constant, 2, constant_op.constant, 1)
    )
    def testRetraceLogicWithSideInputs(
        self, val_before, type_before, val_after, type_after, expected_len
    ):
        @polymorphic_function.function
        def f():
            func = lambda: x
    ...
    def testDataClassWithSubTraceType(self):
        ...
        mt4 = MaskedTensor(
            mask=False,
            value=constant_op.constant([[1], [2]], shape=[2, 1], dtype=dtypes.int32),
        )
