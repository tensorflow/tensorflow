...
class AtomicFunction(core.AtomicFunction):
    ...
    def graph_call_attrs(self) -> Dict[str, Any]:
        ...
        attrs = {
            ...
            "tout": [o.dtype.as_datatype_enum for o in self.function_type.flat_outputs],
...
def make_call_op_in_graph(
    ...
    op = partitioned_call_op(  # pytype: disable=wrong-arg-types  # always-use-property-annotation
        ...
        tout=[o.dtype.as_datatype_enum for o in atomic.function_type.flat_outputs],
...
def from_func_graph(
    ...
    if attrs and attributes_lib.IMPLEMENTS in attrs:
        ...
        assert not any((has_resource_vars, captured_inputs)), (
    ...
    if graph_output_names is not None and all(
        ...
        output_names = [
            compat.as_bytes(graph_output_names[ops.tensor_id(t)]) for t in graph.outputs
        ]
    ...
    call_options = CallOptions(
        collective_manager_ids_used=getattr(graph, "collective_manager_ids_used", []),
