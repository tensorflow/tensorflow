...
class _TapeGradientFunctions(object):
    ...
    def _build_functions_for_outputs(self, outputs, inference_args, input_tangents):
        ...
        (wrapped_backwards_graph, forward_wrapper) = (
        ...
        (wrapped_forward_function, wrapped_backward_function) = (
...
class ConcreteFunction(core.ConcreteFunction, trackable.Trackable):
    ...
    def _function_spec(self):
        ...
        return function_type_utils.FunctionSpec(
            self.function_type,
            {p.default for p in self.function_type.parameters.values() if p.optional},
            False,
            name=self.name,
        )
    ...
    def _call_flat(self, tensor_inputs, captured_inputs):
        ...
        if not executing_eagerly:
            for i, tensor_input in enumerate(tensor_inputs):
                ...
                if (
                    tensor_input.dtype == dtypes.resource
                    or tensor_input.dtype == dtypes.variant
                ):
                    continue
    ...
    def _trackable_children(self, save_type="checkpoint", **kwargs):
        ...
        for n, (capture, _) in enumerate(self.graph.captures):
            if capture.dtype not in (
                dtypes.variant,
                dtypes.resource,
            ) and not resource_variable_ops.is_resource_variable(capture):
