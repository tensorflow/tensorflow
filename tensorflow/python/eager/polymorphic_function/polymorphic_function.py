...
_tf_function_counter = monitoring.Counter(
    ...
    "Counter for the number of tf.functions created when Eager execution is enabled.",
                ...
                with ops.init_scope():
                    ...
                    if initial_value is not None:
                        with (
                            ...
                            ops.colocate_with(self._handle),
                        ):
...
JIT_COMPILE_FUNCTIONS = os.getenv(
    "TF_FUNCTION_JIT_COMPILE_DEFAULT", "false"
).lower() in ("true", "1")
...
class Function(core.PolymorphicFunction, trackable.Trackable):
    ...
    def _generate_scoped_tracing_options(self, scope, scope_type):
        ...
        return self._generate_tracing_options(
            tf_decorator.make_decorator(self._python_function, wrapped_fn), scope_type
        )
        ...
        def fn_with_cond(inner_args, inner_kwds):
            ...
            for v, _ in initializers:
                condition = math_ops.logical_and(
                    condition, resource_variable_ops.var_is_initialized_op(v.handle)
                )
    ...
    def _trackable_children(self, save_type="checkpoint", **kwargs):
        ...
        return {
            ...
            for n, fn in enumerate(
                self._list_all_concrete_functions_for_serialization()
            )
        }
    ...
    def __get__(self, instance, owner):
        ...
        if (
            isinstance(instance, composite_tensor.CompositeTensor)
            and instance._type_spec is not None
        ):  # pylint: disable=protected-access
...
def class_method_to_instance_method(original_function, instance):
    ...
    bound_method = types_lib.MethodType(
        ...
        tf_method_target.TfMethodTarget(
            weak_instance, original_function.python_function
        ),
    )
