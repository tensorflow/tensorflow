...
def _extract_tensors_and_variables(tensor):
    ...
    for obj in nest.flatten(tensor):
        if _pywrap_utils.IsTensor(obj) or _pywrap_utils.IsVariable(obj):
            ...
            raise ValueError(
                f"Passed in object {obj} of type {type(obj).__name__!r}"
...
class GradientTape:
    ...
    def gradient(
        ...
        if target is None:
            raise TypeError(
                "Argument `target` should be a list or nested structure"
        ...
        if output_gradients is not None:
            ...
            output_gradients = composite_tensor_gradient.get_flat_tensors_for_gradients(
                output_gradients
            )
            output_gradients = [
