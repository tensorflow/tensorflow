...
class FunctionTest(test.TestCase, parameterized.TestCase):
    ...
    def testInputSignatureWithKeywordOnlyArgs(self):
        ...
        defined = function.defun(functools.partial(f, c=4), input_signature=signature)
        self.assertEqual(defined(1, 2).numpy(), 11)

        defined = function.defun(functools.partial(f, d=5), input_signature=signature)
        self.assertEqual(defined(1, 2).numpy(), 11)
    ...
    def _total_function_cache_def_func(self, defined):
        return (
            defined._list_all_concrete_functions()
        )  # pylint: disable=protected-access
        ...
        def test_fn():
            if array_ops.constant(False):
                ...
                return script_ops.eager_py_func(
                    func=lambda: array_ops.constant([2.0]), inp=(), Tout=dtypes.int32
                )
...
class MultiDeviceTest(test.TestCase, parameterized.TestCase):
    ...
    def testRaiseReplaceCaptureWithDeferredTypeSpecMismatch(self):
        ...
        def fn():
            deferred_tensor = ops.get_default_graph().capture_call_time_value(
                lambda: value, tensor_spec.TensorSpec(shape=(1,), dtype=dtypes.float32)
            )
        ...
        with self.assertRaisesRegex(
            ValueError, "Attempting to substitute closure with spec*"
        ):
            concrete_fn.replace_capture_with_deferred_capture(
    ...
    def testGraphReplaceCaptureAndSetExternalCapture(self):
        ...
        concrete_fn.set_external_captures(
            [closure, concrete_fn._captured_inputs[1], concrete_fn._captured_inputs[2]]
        )
        self.assertAllEqual(concrete_fn(), [5.0])
