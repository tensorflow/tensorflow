...
class CacheKeyGenerationTest(test.TestCase, parameterized.TestCase):
    ...
    def testVariableAliasing(self):
        ...
        all_unique = function_trace_type.get_arg_spec((v1, v2, v3), False, True, True)
        ...
        all_unique_again = function_trace_type.get_arg_spec(
            (v3, v4, v5), False, True, True
        )
        all_same_again = function_trace_type.get_arg_spec(
            (v4, v4, v4), False, True, True
        )
    ...
    def testTensorEquality(self):
        ...
        tensor_a = array_ops.zeros([11, 3, 5], dtype=dtypes.int32).__tf_tracing_type__(
            context
        )
        tensor_b = array_ops.zeros([11, 4, 5], dtype=dtypes.int32).__tf_tracing_type__(
            context
        )
        tensor_c = array_ops.zeros(
            [11, 3, 5], dtype=dtypes.float32
        ).__tf_tracing_type__(context)
        tensor_d = array_ops.ones([11, 3, 5], dtype=dtypes.int32).__tf_tracing_type__(
            context
        )
    ...
    def testTensorAndSpecEquality(self):
        context = function_trace_type.SignatureContext()
        tensor = array_ops.zeros([11, 3, 5], dtype=dtypes.int32).__tf_tracing_type__(
            context
        )
        ...
        spec_with_name = tensor_spec.TensorSpec(
            [11, 3, 5], dtype=dtypes.int32, name="name"
        ).__tf_tracing_type__(context)
    ...
    def testTensorShapeUnknown(self):
        context = function_trace_type.SignatureContext()
        spec_1 = tensor_spec.TensorSpec(None, dtype=dtypes.int32).__tf_tracing_type__(
            context
        )
        spec_2 = tensor_spec.TensorSpec(None, dtype=dtypes.int32).__tf_tracing_type__(
            context
        )
    ...
    def testAttrsCacheKeyGeneration(self):
        ...
        expected = function_trace_type.AttrsType(
            TestAttrsClass,
            (function_trace_type.GenericType(1), function_trace_type.GenericType(2)),
        )
...
class CacheKeyGenerationBenchmark(test.Benchmark):
    def benchmarkTensor(self):
        ...
        def encode_tensors(tensors):
            function_trace_type.get_arg_spec(
                tensors, False, False, function.USE_FULL_TRACE_TYPE
            )
    ...
    def benchmarkTensorSpec(self):
        ...
        def encode_tensor_specs(tensor_specs):
            function_trace_type.get_arg_spec(
                tensor_specs, False, False, function.USE_FULL_TRACE_TYPE
            )
    ...
    def benchmarkVariable(self):
        ...
        def encode_variables(var_list):
            function_trace_type.get_arg_spec(
                var_list, False, False, function.USE_FULL_TRACE_TYPE
            )
    ...
    def benchmarkKerasModel(self):
        ...
        def encode_model(model):
            function_trace_type.get_arg_spec(
                model, False, False, function.USE_FULL_TRACE_TYPE
            )
    ...
    def benchmarkCacheKeyLookup(self):
        ...
        self.report_benchmark(
            ...
            wall_time=t,
            metrics=[
                {"name": "cache_key_lookup_avg_ms", "value": t / iterations * 1000}
            ],
        )
    ...
    def benchmarkNestedStruct(self):
        ...
        def encode_struct(struct):
            function_trace_type.get_arg_spec(
                struct, False, False, function.USE_FULL_TRACE_TYPE
            )
    ...
    def benchmarkFunctionInvocation(self):
        struct = (
            variables.Variable(1.0),
            array_ops.zeros([5, 13]),
            {"tensor": array_ops.zeros([5, 20]), "variable": variables.Variable(1.0)},
        )
...
class TraceTypeEncodingTest(test.TestCase):
    ...
    def testOrderedCollectionTypeSupertype(self):
        ...
        self.assertIsNone(collection_a.most_specific_common_supertype([collection_b]))
        self.assertEqual(
    ...
    def testDictTypeSubtype(self):
        ...
        dict_a = dict_type(
            {"a": MockSubtypeOf2(1), "b": MockSubtypeOf2(1), "c": MockSubtypeOf2(1)}
        )
        dict_b = dict_type(
            {"a": MockSubtypeOf2(2), "b": MockSubtypeOf2(2), "c": MockSubtypeOf2(2)}
        )
