...
class TFETensorTest(test_util.TensorFlowTestCase):
    ...
    def testBadConstructorArgs(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "Expecting a DataType value for dtype. Got"
        ):
    ...
    def testConstantDtype(self):
        self.assertEqual(constant_op.constant(1, dtype=np.int64).dtype, dtypes.int64)
    ...
    def testBool(self):
        ...
        self.assertFalse(bool(_create_tensor([0.0])))
        self.assertTrue(bool(_create_tensor([1])))
        self.assertTrue(bool(_create_tensor([1.0])))
    ...
    def testIterateOverScalarTensorRaises(self):
        t = _create_tensor(1)
        with self.assertRaisesRegex(TypeError, "Cannot iterate over a scalar tensor"):
            iter(t)
    ...
    def testConvertToTensorNumpyZeroDim(self):
        for np_type, dtype in [
            (np.int32, dtypes.int32),
            (np.half, dtypes.half),
            (np.float32, dtypes.float32),
        ]:
            x = ops.convert_to_tensor(
                [np.array(65, dtype=np_type), np.array(16, dtype=np_type)]
            )
    ...
    def testConvertToTensorNumpyScalar(self):
        x = ops.convert_to_tensor(
            [np.array(321, dtype=np.int64).item(), np.array(16, dtype=np.int64).item()]
        )
        self.assertAllEqual(x, [321, 16])

    def testEagerTensorError(self):
        with self.assertRaisesRegex(
            TypeError, "Cannot convert .* to EagerTensor of dtype .*"
        ):
            _ = ops.convert_to_tensor(1.0, dtype=dtypes.int32)
    ...
    def test_numpyFailsForResource(self):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "Cannot convert .+ resource"
        ):
            v._handle._numpy()
    ...
    def test_numpyFailsForVariant(self):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "Cannot convert .+ variant"
        ):
            variant_t._numpy()
    ...
    def testMemoryviewScalar(self):
        ...
        self.assertAllEqual(np.array(memoryview(t)), np.array(42.0, dtype=np.float32))
    ...
    def testMemoryviewCopyToCPU(self):
        with ops.device("/device:GPU:0"):
            t = constant_op.constant([0.0])
        self.assertAllEqual(np.array(memoryview(t)), np.array([0.0], dtype=np.float32))

    @test_util.disable_tfrt(
        "b/169877776: ResourceVariable is not initialized " "properly in TFRT"
    )
    ...
    def testEagerTensorFormatForResource(self):
        ...
        self.assertEqual(
            f"{t!s}", "tf.Tensor(<Resource Tensor>, shape=(), dtype=resource)"
        )
        self.assertEqual(
            repr(t), "<tf.Tensor: shape=(), dtype=resource, value=<Resource Tensor>>"
        )
        self.assertEqual(
            f"{t!r}", "<tf.Tensor: shape=(), dtype=resource, value=<Resource Tensor>>"
        )
    ...
    def testEagerTensorFormatForVariant(self):
        ...
        self.assertEqual(f"{t!s}", "tf.Tensor(<TensorList>, shape=(), dtype=variant)")
        self.assertEqual(
...
class TFETensorUtilTest(test_util.TensorFlowTestCase):
    ...
    def testTensorListContainsNonTensors(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"Expected a list of EagerTensors but element 1 has type \"str\""
        ):
            pywrap_tfe.TFE_Py_TensorShapeSlice([t1, "abc"], 0)

        with self.assertRaisesRegex(
            TypeError, r"Expected a list of EagerTensors but element 0 has type \"int\""
        ):
    ...
    def testTensorListNotList(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"tensors argument must be a list or a tuple. Got.*EagerTensor"
        ):
    ...
    def testUnicode(self):
        self.assertEqual(constant_op.constant("asdf").numpy(), b"asdf")

    def testFloatTensor(self):
        self.assertEqual(
            dtypes.float64, _create_tensor(np.float64()).dtype
        )  # pylint: disable=no-value-for-parameter
        self.assertEqual(
            dtypes.float32, _create_tensor(np.float32()).dtype
        )  # pylint: disable=no-value-for-parameter
        self.assertEqual(
            dtypes.float16, _create_tensor(np.float16()).dtype
        )  # pylint: disable=no-value-for-parameter
