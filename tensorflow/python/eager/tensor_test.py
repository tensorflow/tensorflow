...
class TFETensorTest(test_util.TensorFlowTestCase):
    ...
    def testEagerTensorFormatForResource(self):
        ...
        self.assertRegex(
            str(t), make_regex(f"tf.Tensor({handle_str}, shape=(), dtype=resource)")
        )
        self.assertRegex(
            f"{t!s}", make_regex(f"tf.Tensor({handle_str}, shape=(), dtype=resource)")
        )
        self.assertRegex(
            repr(t),
            make_regex(f"<tf.Tensor: shape=(), dtype=resource, value={handle_str}>"),
        )
        self.assertRegex(
            f"{t!r}",
            make_regex(f"<tf.Tensor: shape=(), dtype=resource, value={handle_str}>"),
    ...
    def testNumpyTooManyDimensions(self):
        ...
        t = constant_op.constant(1.0, shape=[1] * (max_dims + 1))
        with self.assertRaisesRegex(
