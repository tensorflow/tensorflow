...
def _DTypeFromTensor(tensor):
    ...
    if dtype.base_dtype == dtypes.variant:
        ...
        if (
            ...
            if first_type != types_pb2.DT_INVALID and all(
                shape_and_type.dtype == first_type
...
def IsTrainable(tensor_or_dtype):
    ...
    if flags.config().enable_quantized_dtypes_training.value():
        trainable_dtypes.extend(
            [dtypes.qint8, dtypes.qint16, dtypes.qint32, dtypes.quint8, dtypes.quint16]
        )
