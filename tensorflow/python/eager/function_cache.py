...
ExecutionContext = collections.namedtuple(
    "ExecutionContext",
    [
        "parent_graph",
        "device_functions",
...
class FunctionCacheKey(trace.TraceType):
    ...
    def __init__(
        self, function_signature: trace.TraceType, call_context: ExecutionContext
    ):
    ...
    def most_specific_common_subtype(
        ...
        if not all(
            isinstance(other, FunctionCacheKey)
            and self.call_context == other.call_context
            for other in others
        ):
            return None
...
def make_cache_key(
    args, include_tensor_ranks_only: bool = False
) -> Tuple[FunctionCacheKey, function_trace_type.WeakrefDeletionObserver]:
    """Computes the cache key given the function arguments."""
    signature_context = function_trace_type.SignatureContext(include_tensor_ranks_only)
    function_signature = function_trace_type.make_function_signature(
        args, signature_context, _ENCODE_VARIABLES_BY_RESOURCE_ID, USE_FULL_TRACE_TYPE
    )
    return (
        FunctionCacheKey(function_signature, _make_execution_context()),
