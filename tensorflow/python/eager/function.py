...
ag_ctx = lazy_loader.LazyLoader(
    "ag_ctx", globals(), "tensorflow.python.autograph.core.ag_ctx"
)
...
def _parse_func_attrs(attributes):
    ...
    for key, value in attributes.items():
        ...
        # bool type check has to happen before int since bool is a subclass of int.
            ...
            raise ValueError(
                f"Attribute {key} must be bool, int, float, string, or "
    ...
    def __exit__(self, typ, exc, tb):
        ...
        for func_tag in func_tags:
            ...
            if func_tag.name == compat.as_str(self._func.name):
                ...
                next_func = g._get_function(
                    func_tag.name
                )  # pylint: disable=protected-access
                if next_func is not None and isinstance(
                    next_func, _EagerDefinedFunction
                ):
                    g = next_func.graph
        if g:
            exc._message = error_interpolation.interpolate(
                message, g
            )  # pylint: disable=protected-access
        return False
...
class _EagerDefinedFunction(object):
    ...
    def __init__(self, name, graph, inputs, outputs, attrs):
        ...
        fn = pywrap_tf_session.TF_GraphToFunction_wrapper(
            ...
            output_names,
            [
                o._c_op for o in graph.control_outputs
            ],  # pylint: disable=protected-access
        ...
        self.graph = graph
        self._stateful_ops = tuple(
            op for op in operations if op._is_stateful
        )  # pylint: disable=protected-access
...
def _create_forward_backward_with_graph(attrs, forward_graph, backwards_graph):
    ...
    backward_function = ConcreteFunction(backwards_graph, attrs=backward_function_attr)
...
class _DelayedRewriteGradientFunctions(object):
    ...
    def __init__(self, func_graph, attrs, func_graph_deleter):
        ...
        self._inference_function = _EagerDefinedFunction(
            _inference_name(self._func_graph.name),
            self._func_graph,
    ...
    def _construct_forward_backward(self, num_doutputs):
        ...
        trainable_outputs = [
            output
            for output in self._func_graph.outputs[:num_doutputs]
        ...
        def _backprop_function(*grad_ys):
            with ops.device(None):
                return (
                    gradients_util._GradientsHelper(  # pylint: disable=protected-access
                        trainable_outputs,
                        self._func_graph.inputs,
                        grad_ys=grad_ys,
        ...
        with self._func_graph.as_default():
            ...
            captures_from_forward = [
                c
                for c in backwards_graph_captures
    ...
    def forward(self, inference_args=None, input_tangents=None):
        ...
        if input_tangents:
            ...
            raise errors.InternalError(
                "unexpectedly got forwardprop information in "
    ...
    def record(self, flat_outputs, inference_args, input_tangents):
        ...
        tape.record_operation(
            self._inference_function.signature.name,
            to_record,
...
class _TapeGradientFunctions(object):
    ...
    def _build_functions_for_outputs(self, outputs, inference_args, input_tangents):
        ...
        with backwards_graph.as_default():
            ...
            captures_from_forward = [
                c
                for c in backwards_graph.external_captures
                if not isinstance(c, ops.EagerTensor) and c.graph is self._func_graph
            ]
            existing_outputs = object_identity.ObjectIdentitySet(
                self._func_graph.outputs
        ...
        return (
            wrapped_forward_function,
            forward_wrapper.graph,
            wrapped_backward_function,
            forward_wrapper.output_indices,
            len(forward_wrapper.output_tangents),
        )

    def _wrap_forward_function_with_jvps(
        self, forward_function, backward_function, inference_args, input_tangents
    ):
        ...
        with forward_wrapper_graph.as_default():
            ...
            with forwardprop_util.push_forwardprop_state():
                ...
                for input_index, real_input in enumerate(self._func_graph.inputs):
                    ...
                    input_placeholder = array_ops.placeholder(
                        dtype=real_input.dtype, shape=real_input.shape
                    )
                ...
                for tape_indices in self._forwardprop_input_indices:
                    for input_index, jvp_index in tape_indices:
                        ...
                        jvp_placeholder = graph_placeholder(
                            gradient_dtype, gradient_shape
                        )
                        ...
                        tensor_shape.TensorShape(
                            external_jvp.shape
                        ).assert_is_compatible_with(jvp_placeholder.shape)
                        tape.record_operation(
                            "captured_value",
                            [jvp_placeholder],
                            [external_jvp],
                ...
                gradient_function = (
                    self._delayed_rewrite_functions._rewrite_forward_and_call_backward
                )  # pylint: disable=protected-access
                with ops.get_default_graph()._override_gradient_function(  # pylint: disable=protected-access
                    ...
                    forward_outputs = forward_function.call(
                        context.context(), forward_inputs
                    )
            ...
            output_indices, output_tangents = pywrap_tfe.TFE_Py_PackJVPs(
                forward_outputs
            )
            output_tangents = [
                forward_wrapper_graph.capture(t) for t in output_tangents
            ]
        return _ForwardWrapper(
            graph=forward_wrapper_graph,
        ...
        with wrapped_backwards_graph.as_default():
            ...
            for output in recorded_outputs:
                if backprop_util.IsTrainable(output):
                    ...
                    doutput_placeholder = graph_placeholder(
                        doutput.dtype, doutput.shape
                    )
                ...
                while num_processed_output_tangents != len(output_tangents):
                    for output in output_tangents[num_processed_output_tangents:]:
                        (
                            gradient_shape,
                            gradient_dtype,
                        ) = default_gradient.shape_and_dtype(output)
                    ...
                    captures_from_forward = [
                        c
                        for c in wrapped_backwards_graph.external_captures
                        if (
                            not isinstance(c, ops.EagerTensor)
                    ...
                    output_indices, output_tangents = forwardprop_util.pack_tangents(
                        forward_wrapper.outputs
                    )
                    output_tangents = [
                        forward_wrapper.graph.capture(t) for t in output_tangents
                    ]
    ...
    def _wrap_backward_function(self, forward_graph, backward, outputs):
        ...
        if any(
            ...
            incorrect_mapping = [
                t
                for t in remapped_captures
                if (not isinstance(t, ops.EagerTensor) and t.graph is not forward_graph)
            ]
            raise errors.InternalError(
        ...
        if self._num_forwardprop_outputs and not self._need_gradients_for_jvps:
            relevant_outputs = (
                ...
                + outputs[self._num_inference_outputs + self._num_forwardprop_outputs :]
            )
        else:
...
class _HigherOrderTapeGradientFunctions(_TapeGradientFunctions):
    ...
    def _forward_and_backward_functions(self, inference_args, input_tangents):
        ...
        while (
            len(outputs) < len(self._func_graph.outputs)
            ...
            # reason to continue.
            and any(
                backprop_util.IsTrainable(output)
            ...
            if iteration_count >= 20 and iteration_count % 5 == 0:
                ...
                logging.warning(
                    (
                        ...
                        self._func_graph.name,
                        iteration_count,
                        num_new_trainable_outputs,
                        new_op_with_trainable_output,
                    )
                )
        ...
        if len(self._func_graph.outputs) > len(outputs) and any(
            backprop_util.IsTrainable(output)
            for output in self._func_graph.outputs[len(outputs) :]
        ):
            raise errors.InternalError(
...
class ConcreteFunction(core.ConcreteFunction, trackable.Trackable):
    ...
    def __init__(
        self, func_graph, attrs=None, shared_func_graph=True, function_spec=None
    ):
        ...
        if attrs and IMPLEMENTS_ATTRIBUTE_NAME in attrs:
            ...
            has_resource_vars = any(inp.dtype == dtypes.resource for inp in self.inputs)
    ...
    def _call_impl(self, args, kwargs, cancellation_manager=None):
        ...
        with trace.Trace(self._func_graph.name, tf_function_call="concrete"):
            ...
            if self._function_spec is not None:
                try:
                    return self._call_with_structured_signature(
                        args, kwargs, cancellation_manager
                    )
                except TypeError as structured_err:
                    try:
                        return self._call_with_flat_signature(
                            args, kwargs, cancellation_manager
                        )
                    except TypeError:
    ...
    def _call_with_flat_signature(self, args, kwargs, cancellation_manager):
        ...
        for keyword in self._arg_keywords[len(args) :]:
            try:
                ...
                specified_keywords = list(self._arg_keywords[: len(args)]) + list(
                    kwargs.keys()
                )
                ...
                raise TypeError(
                    f"{self._flat_signature_summary()} missing required "
        ...
        if kwargs:
            ...
            for unused_key in kwargs:
                if unused_key in positional_arg_keywords:
                    raise TypeError(
                        f"{self._flat_signature_summary()} got two values "
                        f"for '{unused_key}'."
                    )
            raise TypeError(
                f"{self._flat_signature_summary()} got unexpected "
        ...
        for i, arg in enumerate(args):
            if not isinstance(
                ...
                raise TypeError(
                    f"{self._flat_signature_summary()}: expected argument "
    ...
    def _structured_signature_check_missing_args(self, args, kwargs):
        ...
        if missing_arguments:
            raise TypeError(
                f"{self._structured_signature_summary()} missing "
                "required arguments: "
    ...
    def _structured_signature_check_unexpected_args(self, args, kwargs):
        ...
        if len(kwargs) > len(kwarg_specs):
            ...
            raise TypeError(
                f"{self._structured_signature_summary()} got unexpected "
    ...
    def _call_flat(self, args, captured_inputs, cancellation_manager=None):
        ...
        if tape.could_possibly_record() or hasattr(default_graph, "watch_variable"):
        ...
        for i, arg in enumerate(args):
            if isinstance(arg, resource_variable_ops.BaseResourceVariable):
                ...
                raise ValueError(
                    f"{i:d}-th input {arg} must be a Tensor, got "
        ...
        if (
            possible_gradient_type == gradients_util.POSSIBLE_GRADIENT_TYPES_NONE
        ...
        forward_backward = self._select_forward_and_backward_functions(
            args, possible_gradient_type, executing_eagerly
        )
    ...
    def replace_capture_with_deferred_capture(
        self, tensor, closure, spec, placeholder=None, default_value=None
    ):
        ...
        if not (
            spec.is_compatible_with(tensor) or spec.is_compatible_with(placeholder)
        ):
            raise ValueError(
    ...
    def _select_forward_and_backward_functions(
        ...
        need_gradients_for_jvps = tape.should_record_backprop(input_tangents.tangents)
        ...
        if possible_gradient_type == gradients_util.POSSIBLE_GRADIENT_TYPES_FIRST_ORDER:
            if input_tangents.indices or executing_eagerly:
                ...
                return _ForwardBackwardCall(
                    self._delayed_rewrite_functions,
                    args,
    ...
    def _build_call_outputs(self, result):
        ...
        ret = nest.pack_sequence_as(
            self._func_graph.structured_outputs, outputs_list, expand_composites=True
        )
        return ret
    ...
    def pretty_printed_signature(self, verbose=True):
        ...
        if kwarg_specs:
            for kwarg in sorted(kwarg_specs):
                ...
                if _contains_type_spec(spec):
                    arg_details.append(
                        "    {}: {}".format(kwarg, pretty_print_spec(spec))
                    )
...
class FunctionSpec(object):
    ...
    def from_function_and_signature(
        ...
        if input_signature is not None and set(fullargspec.kwonlyargs) - set(
            fullargspec.kwonlydefaults or ()
        ):
            ...
            if fullargspec.defaults or fullargspec.kwonlydefaults:
                ...
                if fullargspec.defaults:
                    ...
                    new_args_with_defaults = list(
                        itertools.compress(
                            args_with_defaults, non_keyword_defaults_mask
                        )
                    )
                    # Keep all args.
                    ...
                    new_defaults = [
                        default
                        for _, default in new_args_with_defaults
    ...
    def _convert_variables_to_tensors(self, args, kwargs):
        ...
        kwargs = {kw: self._to_tensor_or_tensor_spec(x) for kw, x in kwargs.items()}
    ...
    def canonicalize_function_inputs(self, *args, **kwargs):
        ...
        if self._input_signature is not None:
            if arglen > len(self._input_signature):
                raise TypeError(
                    f"{self.signature_summary()} specifies "
            ...
            for arg in six.iterkeys(kwargs):
                ...
                if index is None:
                    raise TypeError(
                        f"{self.signature_summary()} got unexpected keyword "
                        f"argument `{arg}`."
                    )
        ...
        if not kwargs:
            ...
            if self._arg_indices_to_default_values:
                try:
                    ...
                    raise TypeError(
                        f"{self.signature_summary()} missing required "
            ...
            for arg, value in six.iteritems(kwargs):
                ...
                if index is not None:
                    if index < arglen:
                        raise TypeError(
                            f"{self.signature_summary()} got two values for "
                            f"{arg!r}."
                        )
            ...
            if missing_arg_indices:
                ...
                if len(missing_args) == 1:
                    raise TypeError(
                        f"{self.signature_summary()} missing 1 required "
            ...
            if kwargs and self._input_signature is not None:
                raise TypeError(
                    "Keyword arguments are not supported when "
...
def _convert_numpy_inputs(inputs):
    ...
    for index, value in enumerate(flat_inputs):
        if isinstance(value, (ops.Tensor, resource_variable_ops.BaseResourceVariable)):
            filtered_flat_inputs.append(value)
        elif hasattr(value, "__array__") and not (
            hasattr(value, "_should_act_as_resource_variable")
            or isinstance(value, (np.str_, type, composite_tensor.CompositeTensor))
        ):
            # This case is equivalent to _is_ndarray(value) == True
            a = value.__array__()
            if not isinstance(a, np.ndarray):
                raise TypeError(
                    f"The output of __array__ must be an np.ndarray, "
    ...
    if need_packing:
        return (
            nest.pack_sequence_as(
                structure=inputs, flat_sequence=flat_inputs, expand_composites=True
            ),
            flat_inputs,
            filtered_flat_inputs,
        )
...
def _convert_inputs_to_signature(inputs, input_signature, flat_input_signature):
    ...
    try:
        flatten_inputs = nest.flatten_up_to(
            ...
            expand_composites=True,
            check_types=False,
        )  # lists are convert to tuples for `tf.data`.
    except ValueError:
        raise ValueError(
            "Structure of Python function inputs does not match "
            "input_signature:\n"
            f"{format_error_message(inputs, input_signature)}."
        )

    need_packing = False
    for index, (value, spec) in enumerate(zip(flatten_inputs, flat_input_signature)):
        if isinstance(spec, tensor_spec.TensorSpec) and not _pywrap_utils.IsTensor(
            value
        ):
            try:
    ...
    if any(
        not spec.is_compatible_with(other)
...
class Function(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        with self._lock:
            (graph_function, filtered_flat_args) = self._maybe_define_function(
                args, kwargs
            )
    ...
    def _get_concrete_function_garbage_collected(self, *args, **kwargs):
        ...
        with self._lock:
            ...
            for arg in graph_function.graph.inputs:
                ...
                graph_function._arg_keywords.append(
                    proposal
                )  # pylint: disable=protected-access
            # Anything can be a positional argument, in the same order as .inputs
            graph_function._num_positional_args = (
                num_positional  # pylint: disable=protected-access
            )
            return graph_function
    ...
    def get_concrete_function(self, *args, **kwargs):
        ...
        Args:
          ...
          **kwargs: keyword inputs to specialize on. Concrete values (e.g. 1)
             or `tf.Tensor` or `tf.TensorSpec`.
        """
        graph_function = self._get_concrete_function_garbage_collected(*args, **kwargs)
        graph_function._garbage_collector.release()  # pylint: disable=protected-access
        return graph_function
    ...
    def _define_function_with_shape_relaxation(
        self, args, kwargs, flat_args, filtered_flat_args
    ):
        ...
        if relaxed_arg_function is not None and all(
            _is_type_subset(x, y) for (x, y) in zip(relaxed_arg_specs, arg_specs)
        ):
        ...
        if relaxed_arg_specs is None:
            ...
            if len(arg_specs) != len(relaxed_arg_specs):
                raise RuntimeError(
                    "Expected arg_specs len to match relaxed_arg_specs "
        ...
        self._function_cache.arg_relaxed_specs[rank_only_cache_key] = relaxed_arg_specs
        relaxed_arg_shapes = [
        ...
        return (
            ...
            [
                t
                for t in nest.flatten((args, kwargs), expand_composites=True)
                if isinstance(
                    t, (ops.Tensor, resource_variable_ops.BaseResourceVariable)
                )
            ],
        )
    ...
    def _maybe_define_function(self, args, kwargs):
        ...
        graph_function = self._function_cache.lookup(cache_key, USE_FUNCTION_SUBTYPING)
            ...
            with trace.Trace("tf.function-graph_building"):
                ...
                with ag_ctx.ControlStatusCtx(
                    ...
                    if (
                        ...
                        and self._function_cache.has_call_context(
                            cache_key.call_context
                        )
                    ):
                    ...
                    self._function_cache.add(
                        cache_key, cache_key_deletion_observer, graph_function
                    )
...
def register(func, *args, **kwargs):
    ...
    if not isinstance(func, Function):
        raise ValueError(
            "Only defun function is allowed to be registered. "
...
def validate_signature(signature):
    ...
    if any(
        not isinstance(arg, tensor_spec.DenseSpec)
