...
class MicroBenchmarks(benchmarks_test_base.MicroBenchmarksBase):
    ...
    def _benchmark_defun_matmul_relaxed_shape(self, m, num_iters, execution_mode=None):
    ...
    def _benchmark_matmul_forward_backward_2_by_2_CPU_async(self, run_eager=False):
        def_function.run_functions_eagerly(run_eager)
        with context.device(CPU):
            m = self._m_2_by_2.cpu()
            self._benchmark_defun_matmul_forward_backward(
    ...
    @test_util.disable_tfrt("b/169371527: Support inserting transfer op in lowering.")
    ...
    def _benchmark_tf_reduce_logsumexp(
        self, device=CPU, execution_mode=None, defunc=False, xla_compile=False
    ):
    ...
    def benchmark_tf_reduce_logsumexp_GPU_defun_compile(self):
        self._benchmark_tf_reduce_logsumexp(device=GPU, defunc=True, xla_compile=True)
    ...
    def _benchmark_tf_dropout_2_by_2(
        self, rate=0.5, is_rate_tensor=True, noise_shape=None, device=CPU
    ):
        if is_rate_tensor:
    ...
    def benchmark_tf_dropout_scalar_rate_2_by_2_GPU_rate_0(self):
        self._benchmark_tf_dropout_2_by_2(rate=0.0, is_rate_tensor=False, device=GPU)
    ...
    def benchmark_function_trace(self):
        ...
        self._run(lambda: (def_function.function(func)(x) for x in range(1000)), 30000)
    ...
    def _benchmark_tf_range_return(
        self, limit=100000, dtype=dtypes.int32, device=CPU, num_iters=100000
    ):
        def func(lim):
