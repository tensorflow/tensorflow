...
class SingleWorkerTest(test.TestCase, parameterized.TestCase):
    ...
    def testMultiDeviceFunctionRemoteOutput(self):
        ...
        self.assertEqual(
            rets[0].backing_device, "/job:localhost/replica:0/task:0/device:CPU:0"
        )
        self.assertEqual(
            rets[1].backing_device, "/job:worker/replica:0/task:0/device:CPU:0"
        )
    ...
    def testTwoExecutors(self):
        ...
        with context.executor_scope(
            executor.new_executor(enable_async=False, enable_streaming_enqueue=False)
        ):
            with ops.device("job:worker/replica:0/task:0/device:CPU:0"):
    ...
    def testRemoteCall(self):
        ...
        def func(x):
            return functional_ops.remote_call(
                args=[x], Tout=[dtypes.int32], f=remote_fn, target="/job:worker/task:0"
            )
...
class MultiWorkersTest(test.TestCase, parameterized.TestCase):
    ...
    def testMultiDeviceFunctionAmbiguousDevice(self):
        ...
        self.assertIn(
            "the output node must match exactly one device", cm.exception.message
        )
    ...
    def testMultiDeviceFunctionExecutionOrderingWithPackedInput(self):
        ...
        with ops.device("/job:worker/replica:0/task:1/device:CPU:0"):
            self.assertAllEqual(
                func(packed_input).numpy(), array_ops.ones(shape).numpy() * 2
            )
    ...
    def testMultiDeviceFunctionWithPackedVariable(self):
        ...
        self.assertEqual(
            packed_var.device, "/job:localhost/replica:0/task:0/device:COMPOSITE:0"
        )
        self.assertEqual(
            packed_var.backing_device,
    ...
    def testMultiDeviceFunctionRemoteOutput(self):
        ...
        with ops.device("/job:worker/replica:0/task:0/cpu:0"):
            rets = remote_output(constant_op.constant([1]))
        self.assertEqual(
            rets[0].backing_device, "/job:worker/replica:0/task:0/device:CPU:0"
        )
        self.assertEqual(
            rets[1].backing_device, "/job:worker/replica:0/task:1/device:CPU:0"
        )
        self.assertAllEqual(rets[0].numpy(), [2])
        self.assertAllEqual(rets[1].numpy(), 2)
...
class MultiJobsTest(test.TestCase, parameterized.TestCase):
    ...
    def testMultipleDeviceFoundCheck(self):
        ...
        with ops.device("/job:my_ps/task:1"):
            output = self.evaluate(func())
            self.assertEqual(
                compat.as_bytes("/job:my_ps/replica:0/task:1/device:CPU:0"), output[0]
            )
    ...
    def testConnectToClusterWithoutLocalGpu(self):
        ...
        remote.connect_to_cluster(
            self._cluster_resolver, make_master_device_default=False
        )
