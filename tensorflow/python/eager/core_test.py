...
class TFETest(test_util.TensorFlowTestCase):
  ...
  def testExecutionModeIsStoredThreadLocal(self):
    ...
    count = [0]
    num_threads = 10
    # Exceptions raised inside a thread (including failed assertions) are
    # not propagated by Thread.join() -- they are only printed to stderr,
    # so this test would silently report success even if the underlying
    # thread-local behavior were broken. Capture them here and re-raise on
    # the main thread once every worker has finished.
    errors_by_thread = [None] * num_threads

    def execution_mode_test(cond, count, num_threads, ctx, mode, index):
      try:
        with cond:
    ...
    for i in range(num_threads):
      t = threading.Thread(
          ...
          args=(cv, count, num_threads, ctx,
                context.SYNC if i % 2 == 0 else context.ASYNC, i))
      t.start()
      threads.append(t)
