...
def _api_gen_rule_impl(ctx):
    ...
    flags = ["--apidir=" + ctx.outputs.outs[0].dirname] + ctx.attr.flags
    flags = [ctx.expand_location(flag) for flag in flags]
    flags = [ctx.expand_make_variables("tf_api_version", flag, {}) for flag in flags]
...
api_gen_rule = rule(
    ...
    attrs = {
        ...
        "api_gen_binary_target": attr.label(executable = True, cfg = "target", mandatory = True),
