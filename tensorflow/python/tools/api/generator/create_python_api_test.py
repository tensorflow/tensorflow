...
class CreatePythonApiTest(test.TestCase):
    ...
    def testFunctionImportIsAdded(self):
        ...
        if create_python_api._LAZY_LOADING:
            ...
            expected_import = (
                "from tensorflow.python.test_module import test_op as test_op1"
            )
        self.assertTrue(
        ...
        self.assertFalse(
            "compat.v1" in imports, msg="compat.v1 in %s" % str(imports.keys())
        )
    ...
    def testClassImportIsAdded(self):
        ...
        if create_python_api._LAZY_LOADING:
            expected_import = (
                "'NewTestClass': ('tensorflow.python.test_module', 'TestClass')"
            )
        else:
    ...
    def testConstantIsAdded(self):
        ...
        if create_python_api._LAZY_LOADING:
            expected = (
                "'_TEST_CONSTANT': ('tensorflow.python.test_module', '_TEST_CONSTANT')"
            )
        else:
            expected = "from tensorflow.python.test_module import _TEST_CONSTANT"
        self.assertTrue(
            expected in str(imports), msg="%s not in %s" % (expected, str(imports))
        )
    ...
    def testCompatModuleIsAdded(self):
        ...
        self.assertTrue(
            "compat.v1" in imports, msg="compat.v1 not in %s" % str(imports.keys())
        )
        self.assertTrue(
            "compat.v1.test" in imports,
    ...
    def testProxyAPIFileIsGenerated(self):
        ...
        create_python_api.create_proxy_api_files(
            [proxy_file], proxy_module_root, save_dir
        )
