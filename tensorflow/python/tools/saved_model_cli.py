...
_SMCLI_TAG_SET = flags.DEFINE_string(
    name="tag_set",
    default=None,
    help="Comma-separated set of tags that identify variant graphs in the SavedModel.",
)
...
_SMCLI_WORKER = flags.DEFINE_string(
    name="worker",
    default=None,
    help="If specified, runs the session on the given worker (bns or gRPC path).",
)
...
def _show_defined_functions(saved_model_dir, meta_graphs):
    ...
    children = list(
        save._AugmentedGraphView(trackable_object).list_children(trackable_object)  # pylint: disable=protected-access
    )
...
def run():
    ...
    tensor_key_feed_dict = load_inputs_from_input_arg_string(
        _SMCLI_INPUTS.value, _SMCLI_INPUT_EXPRS.value, _SMCLI_INPUT_EXAMPLES.value
    )
...
def aot_compile_cpu():
    ...
    saved_model_aot_compile.aot_compile_cpu_meta_graph_def(
        ...
        multithreading=(_SMCLI_MULTITHREADING.value.lower() not in ("f", "false", "0")),
    )
