...
class DistributedValuesTest(test.TestCase, parameterized.TestCase):
    @combinations.generate(
        combinations.combine(
            distribution=(
                strategy_combinations.all_strategies_minus_default
    ...
    def testMakeDistributedValueFromTensor(self, distribution):
        ...
        distributed_values = distribution.experimental_distribute_values_from_function(
            value_fn
        )
        self.assertAllEqual(
            ds_test_util.gather(distribution, distributed_values),
            constant_op.constant(1.0, shape=(distribution.num_replicas_in_sync)),
        )
    ...
    def testMakeDistributedValueSingleNumpyArrayConstant(self, distribution):
        ...
        self.assertAllEqual(
            ...
            [[1.0, 2.0, 3.0]] * distribution.num_replicas_in_sync,
        )
    ...
    def testMakeDistributedValueTupleConstant(self, distribution):
        ...
        expected = tuple(
            [v for i in range(distribution.num_replicas_in_sync)] for v in tuple_value
        )
    ...
    def testMakeDistributedValueAndRun(self, distribution):
        ...
        def run():
            ...
            outputs = ds_test_util.gather(
                distribution, distribution.run(computation, args=(distributed_values,))
            )
            return outputs
    ...
    def testMakeDistributedValueExplicitDevicePlacement(self, distribution, op_type):
        if not tf2.enabled():
            self.skipTest("Only V2 is supported.")
...
class PerWorkerResourceTest(test.TestCase, parameterized.TestCase):
    ...
    def testMapFnTracing(self, dataset_fn_as_tf_function):
        ...
        def dataset_fn():
            dataset = (
                dataset_ops.DatasetV2.from_tensors([0, 1, 2])
                .repeat()
...
def _make_replica_local(method, strategy=None):
    ...
    for d, n, init in zip(devices, ["v", "v/replica"], [1.0, 2.0]):
        with ops.device(d):
            v.append(
                variable_scope.get_variable(name=n, initializer=init, use_resource=True)
            )
...
class DistributedVariableTest(test.TestCase, parameterized.TestCase):
    ...
    def testProperties(self):
        ...
        v, replica_local = _make_replica_local(variable_scope.VariableAggregation.SUM)
    ...
    def testCanPassToDefFun(self, distribution):
        ...
        self.assertEqual(2.0, self.evaluate(add1(v)))
    ...
    def testValueInCrossReplicaContext(self, distribution):
        ...
        self.assertEqual(
            self.evaluate(replica_local.value()), self.evaluate(value_list[0].value())
        )
    ...
    def testSaveAndRestoreReplicaLocalSumOneGraph(self, distribution):
        with self.cached_session() as sess:
            ...
            self._assign_replica_local(v, [3.0, 4.0])
            ...
            with distribution.scope():
                ...
                self._assign_replica_local(v, [5.0, 6.0])
    ...
    def _save_replica_local_sum(self, distribution):
        ...
        with self.session(graph=ops.Graph()) as sess:
            ...
            # Overwrite the initial values.
            self._assign_replica_local(v, [1.5, 2.0])
    ...
    def _save_normal(self):
        ...
        with self.session(graph=ops.Graph()) as sess:
            var = variable_scope.get_variable(
                name="v", initializer=1.0, use_resource=True
            )
    ...
    def _restore_normal(self, save_path):
        ...
        with self.session(graph=ops.Graph()) as sess:
            var = variable_scope.get_variable(
                name="v", initializer=7.0, use_resource=True
            )
    ...
    def _restore_replica_local_mean(self, save_path, distribution):
        ...
        with self.session(graph=ops.Graph()) as sess:
            ...
            # Overwrite the initial values.
            self._assign_replica_local(v, [7.0, 8.0])
