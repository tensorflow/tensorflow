...
class TFFunctionTest(test.TestCase, parameterized.TestCase):
    ...
    def testDefaultDeviceInsideFunctionWithScope(
        ...
        try:
            ...
            worker = None
        expected_device = (
            device_util.canonicalize("cpu:0", worker) if run_functions_eagerly else ""
        )
    ...
    def testReadVariableInsideFunction(self, distribution, run_functions_eagerly):
        if (
            not run_functions_eagerly
            and config.list_physical_devices("TPU")
            and FLAGS.tpu_use_tfrt
        ):
