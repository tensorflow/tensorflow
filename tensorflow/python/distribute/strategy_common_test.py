...
class ReplicaCtxAllReduceTest(test.TestCase, parameterized.TestCase):
    def testDense(self, strategy, tf_function):
        if (
            strategy_test_lib.is_tpu_strategy(strategy)
    ...
    def testSyncOnReadVariableInput(self, strategy, tf_function):
        if (
            not strategy_test_lib.is_mirrored_strategy(strategy)
            and not strategy_test_lib.is_multi_worker_mirrored_strategy(strategy)
        ...
        if (
            ...
            self.skipTest("Skip TPUStrategy + eager combination.")
        if (
            strategy_test_lib.is_multi_worker_mirrored_strategy(strategy)
