...
class DistributedDatasetV1(input_lib.DistributedDataset):
    ...
    def _make_one_shot_iterator(self):
        ...
        if not context.executing_eagerly():
            raise ValueError(
                "Cannot create a one shot iterator. Please use "
    ...
    def _make_initializable_iterator(
        ...
        if context.executing_eagerly():
            raise ValueError(
                "Cannot create initializable iterator in Eager mode. "
    ...
    def _get_iterator(self):
        ...
        cardinality = input_lib._cardinality(
            self._cloned_datasets[0]
    ...
    def __iter__(self):
        if (
            ...
            or ops.get_default_graph().building_function
        ):
            return self._get_iterator()

        raise RuntimeError(
            "__iter__() is only supported inside of tf.function "
...
class DistributedDatasetsFromFunctionV1(input_lib.DistributedDatasetsFromFunction):
    ...
    def _get_iterator(self):
        iterators = _create_iterators_per_worker(
            self._datasets, self._input_workers, self._options
        )
        cardinality = input_lib._cardinality(
            self._datasets[0]
        )  # pylint: disable=protected-access
    ...
    def __init__(self, input_fn, input_workers, input_contexts, strategy):
        ...
        if input_workers.num_workers != len(input_contexts):
            raise ValueError(
                ...
                "input_contexts (%d)" % (input_workers.num_workers, len(input_contexts))
            )
...
class _SingleWorkerDatasetIterator(
    input_lib._SingleWorkerDatasetIteratorBase
):  # pylint: disable=protected-access
        ...
        with ops.device(self._worker):
            if self._options is not None:
                self._iterator = multi_device_iterator_ops.MultiDeviceIterator(
                    ...
                    prefetch_buffer_size=self._options.experimental_per_replica_buffer_size,
                )
            else:
                self._iterator = multi_device_iterator_ops.MultiDeviceIterator(
                    self._dataset,
