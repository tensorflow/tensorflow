...
class DistributedIteratorTest(test.TestCase, parameterized.TestCase):
    ...
    def testTypeSpec(self, input_type, distribution, enable_get_next_as_optional):
        if not tf2.enabled():
            self.skipTest(
                "DistributedIterator has CompositeTensor support in TF 2 only."
            )
    ...
    def testTypeSpecRoundTrip(
        self, input_type, distribution, enable_get_next_as_optional
    ):
        if not tf2.enabled():
            self.skipTest(
    ...
    def testDoesNotTriggerFunctionTracing(
        self, input_type, distribution, enable_get_next_as_optional, drop_remainder
    ):
        trace_count = [0]
...
class InputTypeSpecTest(test.TestCase, parameterized.TestCase):
    ...
    def testInputSignatureForPerReplicaValues(
        ...
        ds = dataset_ops.DatasetV2.from_tensor_slices(
            np.ones([9, 12]).astype(np.float32)
        ).batch(4, drop_remainder=drop_remainder)
    ...
    def testMostSpecificCompatibleType(
        self, input_type, distribution, enable_get_next_as_optional
    ):
        if not tf2.enabled():
            self.skipTest(
    ...
    def testFromFunctionInputSignatureForPerReplicaValuesWithOptions(
        ...
        if experimental_place_dataset_on_device and experimental_fetch_to_device:
            self.skipTest(
                "Setting experimental_place_dataset_on_device and "
        ...
        def dataset_fn(input_context):
            ...
            return (
                readers.TextLineDatasetV2(dataset)
                .map(string_ops.string_to_number)
                .batch(input_context.get_per_replica_batch_size(4))
            )
...
class DistributedDatasetTypeSpecTest(test.TestCase, parameterized.TestCase):
    ...
    def testTypeSpecReturnedFromTFFunction(
        ...
        self.skipTest(
            "Failures observed in Ubuntu presubmit: No unary variant  "
    ...
    def testTypeSpecComponents(
        ...
        self.assertAllEqual(
            dist_dataset._cloned_datasets, re_dist_dataset._cloned_datasets
        )
...
class RaggedTensorDistributedIteratorTest(test.TestCase, parameterized.TestCase):
    ...
    def testTypeSpecRoundTrip(self, distribution, enable_get_next_as_optional):
        ...
        if isinstance(
            distribution, (tpu_strategy.TPUStrategyV2, tpu_strategy.TPUStrategy)
        ):
