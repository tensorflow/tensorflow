...
class BaseFaultToleranceTest(object):  # pylint: disable=missing-docstring
    ...
    def _create_model_and_run_indefinitely(self):
        ...
        while (
            self.cluster_coord._cluster.closure_queue._inflight_closure_count
            < self.num_workers
        ):
            time.sleep(0.1)
        return model
    ...
    def testPSPreemptionErrorType(self):
        ...
        with ops.device("/job:ps/replica:0/task:0"):
            v = variables.Variable(
                initial_value=random_ops.random_uniform((2, 10)), dtype=dtypes.float32
            )
        ...
        try:
            ...
            if isinstance(e, errors.AbortedError):
                self.assertTrue(
                    "RecvTensor expects a different device incarnation" in str(e)
                    or "Unable to find a context_id" in str(e)
                )
            self._ensure_threads_closed()
    ...
    def testWorkerExecutionAfterPsFailureRaisesExpectedError(self):
        ...
        for i in range(self.num_workers):
            ...
            raise AssertionError(
                "Executing a function after PS fails, should " "result in a PS failure."
            )
