...
class CoordinatedClosureQueueTest(test.TestCase):
    ...
    def testPutRaiseError(self):
        ...
        with self.assertRaisesRegex(
            ...
            "The corresponding function is cancelled. Please reschedule the function.",
        ):
    ...
    def _test_cancel_closure_when_error(self, call_wait):
        ...
        self.assertEqual(
            closure2.output_remote_value._status, remote_value.RemoteValueStatus.READY
        )
...
class ShardedVariableTest(TestCaseWithErrorReportingThread):
    ...
    def testEmbeddingLookup(self):
        ...
        with self.strategy.scope():
            sv = variables.Variable(
                initial_value=np.arange(10).reshape((5, 2)) + 1, dtype=dtypes.float32
            )
        ...
        for func in [lookup, sparse_lookup, safe_sparse_lookup]:
            ...
            closure = coordinator_lib.Closure(
                func, self.coordinator._cluster.closure_queue._cancellation_mgr
            )  # pylint: disable=protected-access
            ...
            self.assertEqual(
                num_rv_ops,
                0,
                f"Function {func.__name__} graph has some "
