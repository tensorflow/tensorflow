...
@tf_export(
    "distribute.experimental.coordinator.RemoteValue",
    "distribute.coordinator.RemoteValue",
    v1=[],
)
        ...
        with self._has_fetched_to_local_lock:
            if not self._has_fetched_to_local:
                ...
                if self._values is not None:
                    ...
                    self._fetched_tensors = nest.map_structure(
                        copy_tensor, self._values
                    )
...
@tf_export(
    "distribute.experimental.coordinator.PerWorkerValues",
    "distribute.coordinator.PerWorkerValue",
    v1=[],
)
class PerWorkerValues(composite_tensor.CompositeTensor):
    ...
    def _type_spec(self):
        return PerWorkerValuesTypeSpec(
            self._values[0]._type_spec, type(self)  # pylint: disable=protected-access
        )
...
class PerWorkerValuesTypeSpec(type_spec_lib.TypeSpec):
    ...
    def most_specific_compatible_type(self, other):
        raise NotImplementedError("most_specific_compatible_type is not implemented")
...
class PerWorkerDatasetFromDatasetFunction(object):
    ...
    def __iter__(self):
        ...
        for iterator_remote_value in per_worker_iterator._values:
            iterator_remote_value._type_spec = input_lib.get_iterator_spec_from_dataset(
...
class PerWorkerDistributedIterator(PerWorkerValues):
    ...
    def get_next(self, name=None):
        ...
        raise NotImplementedError(
            "Iterating over an `AsyncDistributedIterator` "
