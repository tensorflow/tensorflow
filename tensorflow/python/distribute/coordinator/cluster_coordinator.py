...
class ClosureAbortedError(Exception):
    ...
    def __init__(self, original_exception):
        ...
        if isinstance(original_exception, (ClosureInputError, ClosureAbortedError)):
            ...
            self.original_exception = original_exception
        message = (
            "Other function has an execution error, as a result, the "
...
class Closure(object):
    ...
    def execute_on(self, worker):
        ...
        e = _get_error_from_remote_values(
            replica_args
        ) or _get_error_from_remote_values(replica_kwargs)
        if e:
                ...
                with coordinator_context.with_dispatch_context(worker):
                    with metric_utils.monitored_timer("closure_execution"):
                        output_values = self._function(
                            *nest.map_structure(
                                coordinator_context.maybe_get_remote_value, replica_args
                            ),
                            **nest.map_structure(
                                coordinator_context.maybe_get_remote_value,
                                replica_kwargs,
                            ),
                        )
        self.maybe_call_with_output_remote_value(lambda r: r._set_values(output_values))  # pylint: disable=protected-access
...
class _CoordinatedClosureQueue(object):
    ...
    def _cancel_all_closures(self):
        ...
        logging.info(
            "Canceling all closures: canceling remaining closures on the queue"
        )
        while True:
            try:
    ...
    def get(self, timeout=None, tag=None):
        ...
        with self._queue_lock:
            while (
                ...
                and (tag is None or self._tagged_queue[tag].empty())
            ):
...
class CoordinationServicePreemptionHandler(object):
    ...
    def __init__(self, server_def, cluster):
        ...
        self._task_state_poller_thread = utils.RepeatedTimer(
            interval=_POLL_FREQ_IN_SEC, function=self._get_task_states
        )
                ...
                if self._cluster.closure_queue._cancellation_mgr.is_cancelled:  # pylint: disable=protected-access
                    ...
                    # CancelledError.
                        ...
                        raise errors.CancelledError(
                            None,
                            None,
                            "The corresponding function was cancelled while "
                            "attempting to recover from worker failure.",
                        )
                # Else, preemption
                self._handle_failure_and_recovery(
                    e,
                    on_failure_fn,
    ...
    def _handle_failure_and_recovery(
        self,
        e,
        on_failure_fn,
        on_transient_failure_fn,
    ...
    def _get_task_states(self):
        ...
        try:
            ...
            if (
                isinstance(e, errors.InternalError)
            ...
            with self._cluster_update_lock:
                try:
                    ...
                    try:
                        ...
                        logging.info(
                            "Error that occurred while updating server def is not "
...
class Worker(object):
    ...
    def _on_closure_failure(self, closure, e):
        logging.info(
            "[Worker %d] Putting back a closure after it failed.", self.worker_index
        )
    ...
    def _on_resource_closure_failure(self, e):
        ...
        """
        logging.info(
            "[Worker %d] Clearing tagged queue after resource closure failure.",
            self.worker_index,
        )
        with self._resource_tracking_lock:
            self._is_dead_with_error = e
    ...
    def _process_resource_closure(self, closure):
        ...
        try:
            ...
            # Avoid logging the derived cancellation error
            logging.info(
                "[Worker %d] got an exception when processing resource closure",
                self.worker_index,
            )
            if not isinstance(e, errors.CancelledError):
                logging.error(
                    " /job:worker/task:%d encountered the following error when "
                    "processing resource closure: %r:%s",
                    self.worker_index,
                    e,
                    e,
                )
    ...
    def create_resource(self, function, args=None, kwargs=None):
        ...
        closure = ResourceClosure(
            function, self._cluster.resource_cancellation_mgr, args=args, kwargs=kwargs
        )
...
class Cluster(object):
    ...
    def __init__(self, strategy):
        ...
        self._transient_timeouts_threshold = int(
            os.environ.get(
                "TF_COORDINATOR_IGNORE_TRANSIENT_TIMEOUTS", self._num_workers // 10
            )
        )
        ...
        if os.getenv("TF_PSS_ENABLE_COORDINATION_SERVICE"):
            self.failure_handler = CoordinationServicePreemptionHandler(
                context.get_server_def(),
                self,
            )
        else:
            self.failure_handler = WorkerPreemptionHandler(
...
@tf_export(
    "distribute.experimental.coordinator.ClusterCoordinator",
    "distribute.coordinator.ClusterCoordinator",
    v1=[],
)
...
def _is_ps_failure(error):
    ...
    if isinstance(error, errors.InvalidArgumentError):
        if "unknown device" in str(
            error
        ).lower() or "Unable to find the relevant tensor remote_handle" in str(error):
            return True
...
def _is_worker_failure(error):
    ...
    if (
        _handle_graph_execution_error_as_worker_failure()
        and isinstance(error, errors.UnknownError)
        and "Graph execution error" in str(error)
    ):
    ...
    if isinstance(error, errors.InvalidArgumentError):
        if (
            ...
            or "Primary device is not remote" in str(error)
