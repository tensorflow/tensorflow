...
class MultiProcessRunner(object):
  ...
  def _start_subprocess_and_reading_thread(self,
    ...
    p = _Process(
        ...
        daemon=self._daemon)
    logging.info(f"[{task_type}-{task_id}] 
                    Starting subprocess (PID will be assigned)")
    start_time = time.time()
    p.start()
    startup_time = time.time() - start_time
    logging.info(f"[{task_type}-{task_id}] 
                    Subprocess started in {startup_time:.2f}s, PID: {p.pid}")
...
class MultiProcessPoolRunner(object):
  ...
  def run(self, fn, args=None, kwargs=None):
    ...
    for (task_type, task_id), conn in self._conn.items():
      ...
      if remaining_time <= 0:
        raise RuntimeError(f'Total timeout of {total_timeout}s 
                              exceeded while waiting for processes')
      
      logging.info('Waiting for the result from %s-%d 
                    (timeout: %.1fs)', task_type, task_id, remaining_time)
      ...
      if recv_thread.is_alive():
        # Timeout occurred
        logging.info(f"[{task_type}-{task_id}] Timeout waiting for 
                        process result after {min(remaining_time, 120)}s")
        ...
        with self._runner._process_lock if self._runner else threading.Lock():
          if self._runner and (task_type, task_id) in self._runner._processes:
            ...
            logging.info(f"[{task_type}-{task_id}] Process state: 
                            exitcode={process.exitcode}, pid={process.pid}")
        
        raise RuntimeError(f'Timeout waiting for {task_type}-{task_id} 
                              after {min(remaining_time, 120)}s. '
                              f'This often indicates a subprocess is stuck 
                              in initialization or pipe reading. '
                              f'Try increasing Docker container resources 
