...
class MultiProcessRunner(object):
  ...
  def _start_subprocess_and_reading_thread(self,
    ...
    p = _Process(
        ...
        daemon=self._daemon)
    logging.info('[%s-%d] Starting subprocess (PID will be assigned)',
                  task_type, task_id)
    start_time = time.time()
    p.start()
    startup_time = time.time() - start_time
    logging.info('[%s-%d] Subprocess started in %.2fs, PID: %d',
                  task_type, task_id, startup_time, p.pid)
...
class MultiProcessPoolRunner(object):
  ...
  def run(self, fn, args=None, kwargs=None):
    ...
    for (task_type, task_id), conn in self._conn.items():
      ...
      if remaining_time <= 0:
        raise RuntimeError(f'Total timeout of {total_timeout} s exceeded '
      ...
      if recv_thread.is_alive():
        # Timeout occurred
        logging.info('[%s-%d] Timeout waiting for '
                     'process result after %d s',
                     task_type, task_id, min(remaining_time, 120))
        ...
        with self._runner._process_lock if self._runner else threading.Lock():
          if self._runner and (task_type, task_id) in self._runner._processes:
            ...
            logging.info('[%s-%d] Process state: exitcode=%d, pid=%d',
                         task_type, task_id, process.exitcode, process.pid)
