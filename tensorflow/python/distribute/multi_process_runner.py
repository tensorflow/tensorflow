...
class MultiProcessRunner(object):
  ...
  def _start_subprocess_and_reading_thread(self,
    ...
    p = _Process(
        ...
        daemon=self._daemon)
    print(f"[{task_type}-{task_id}] Starting subprocess (PID will be assigned)")
    start_time = time.time()
    p.start()
    startup_time = time.time() - start_time
    print(f"[{task_type}-{task_id}] Subprocess started in {startup_time:.2f}s, PID: {p.pid}")
...
class MultiProcessPoolRunner(object):
  ...
  def run(self, fn, args=None, kwargs=None):
    ...
    for (task_type, task_id), conn in self._conn.items():
      ...
      recv_thread.start()
      recv_thread.join(timeout=min(remaining_time, 120))  # Max 2 minutes per process
      
      if recv_thread.is_alive():
        # Timeout occurred
        print(f"[{task_type}-{task_id}] Timeout waiting for process result after {min(remaining_time, 120)}s")
        ...
        with self._runner._process_lock if self._runner else threading.Lock():
          if self._runner and (task_type, task_id) in self._runner._processes:
            ...
            print(f"[{task_type}-{task_id}] Process state: exitcode={process.exitcode}, pid={process.pid}")
