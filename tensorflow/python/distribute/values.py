...
class PerReplica(
    DistributedValues, composite_tensor.CompositeTensor, ds_types.PerReplica
):
...
def _per_replica_to_tensor(var, dtype=None, name=None, as_ref=False):
    ...
    if distribute_lib.in_cross_replica_context() or not distribute_lib.has_strategy():
        raise ValueError(
            "It looks like you are using a PerReplica object while "
...
class DistributedVariableTraceType(trace.TraceType):
    ...
    def __init__(self, distributed_variable):
        ...
        self.components = (
            tuple(distributed_variable.shape.as_list()),
...
class DistributedVariable(DistributedDelegate, variables_lib.Variable, core.Tensor):
    ...
    def __init__(self, strategy, values, aggregation, var_policy=None):
        ...
        for v in values:
            ...
            if isinstance(v, composite_tensor.CompositeTensor) and hasattr(v, "handle"):
    ...
    def _export_to_saved_model_graph(
        self, object_map=None, tensor_map=None, options=None, **kwargs
    ):
        ...
        resource_list = self._primary._export_to_saved_model_graph(  # pylint:disable=protected-access
            object_map=object_map, tensor_map=tensor_map, options=options, **kwargs
        )
    ...
    def _copy_trackable_to_cpu(self, object_map):
        ...
        if self not in object_map:
            ...
            op_device = (
                pydev.DeviceSpec.from_string(self.device)
    ...
    def _dense_var_to_tensor(self, dtype=None, name=None, as_ref=False):
        ...
        with distribute_lib.enter_or_assert_strategy(self._distribute_strategy):
            ...
            if (
                ...
                and distribute_lib.in_variable_sync_on_read_context()
            ):
                if self._aggregation == vs.VariableAggregation.ONLY_FIRST_REPLICA:
                    return ops.convert_to_tensor(
...
def _tensor_conversion_mirrored_val(value, dtype=None, name=None, as_ref=False):
    return ops.convert_to_tensor(value._get(), dtype=dtype, name=name, as_ref=as_ref)  # pylint: disable=protected-access
...
class PerWorkerResource:
    ...
    def __init__(self, strategy, host_to_resources):
        distribute_lib.distribution_strategy_input_api_counter.get_cell(
            "PerWorkerResource", "TPUDistributedLookupTable"
        ).increase_by(1)
