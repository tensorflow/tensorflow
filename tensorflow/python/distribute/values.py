...
def _on_write_update_replica(var, update_fn, value, **kwargs):
    ...
    if var.aggregation == vs.VariableAggregation.NONE:
        return update_fn(
            var._get_on_device_or_primary(), value, **kwargs
        )  # pylint: disable=protected-access

    if (
        not ds_context.get_strategy().extended._use_merge_call()
    ):  # pylint: disable=protected-access
        ...
        if (
            var.aggregation == vs.VariableAggregation.MEAN
            and (not var.dtype.is_floating)
            and tensor_util.is_tf_type(value)
        ):
            raise ValueError(
        ...
        return ds_context.get_replica_context()._update(  # pylint: disable=protected-access
            var, update_fn, args=(aggregated_value,), kwargs=kwargs, group=True
        )
        ...
        def merge_fn(strategy, value, **kwargs):
            ...
            if (
                var.aggregation == vs.VariableAggregation.MEAN
                and (not var.dtype.is_floating)
                and isinstance(value, PerReplica)
            ):
                raise ValueError(
                    "Cannot update non-float variables with "
                    "tf.VariableAggregation.MEAN aggregation in replica context. "
            ...
            return var._update_cross_replica(
                update_fn, v, **kwargs
            )  # pylint: disable=protected-access
...
def apply_aggregation_replica_context(value, aggregation, destinations):
    ...
    if aggregation == vs.VariableAggregation.ONLY_FIRST_REPLICA:
        ...
        aggregated_value = ds_context.get_strategy().extended._replica_ctx_all_reduce(  # pylint: disable=protected-access
            reduce_op, value
        )
...
class DistributedValues(object):
    ...
    def __str__(self):
        debug_str = ",\n".join("  %d: %s" % (i, v) for i, v in enumerate(self._values))
        return "%s:{\n%s\n}" % (self.__class__.__name__, debug_str)

    def __repr__(self):
        debug_repr = ",\n".join("  %d: %r" % (i, v) for i, v in enumerate(self._values))
...
class DistributedVarOp(object):
    ...
    def __eq__(self, o):
        ...
        return (
            ...
            and self.traceback == o.traceback
...
class DistributedVariable(DistributedDelegate, variables_lib.Variable, core.Tensor):
    ...
    def __init__(self, strategy, values, aggregation, var_policy=None):
        if (
            ...
            and not values[0].dtype.is_floating
        ):
            raise ValueError(
        ...
        for v in values:
            v._distributed_container = weakref.ref(
                self
            )  # pylint: disable=protected-access
    ...
    def initializer(self):
        ...
        if self._initializer_op:
            ...
            init_op = control_flow_ops.group(tuple(v.initializer for v in self._values))
        return init_op
    ...
    def numpy(self):
        if context.executing_eagerly():
            ...
            raise NotImplementedError(
                "DistributedVariable.numpy() is only available "
    ...
    def _as_graph_element(self):
        ...
        if self._policy:
            return self._policy._as_graph_element(
                self
            )  # pylint: disable=protected-access
    ...
    def _get_cross_replica(self):
        if values_util.is_saving_non_distributed():
            return self._primary
        if self._policy:
            return self._policy._get_cross_replica(
                self
            )  # pylint: disable=protected-access
    ...
    def _update_replica(self, update_fn, value, **kwargs):
        ...
        """
        if self._policy:
            return self._policy._update_replica(
                self, update_fn, value, **kwargs
            )  # pylint: disable=protected-access
        raise NotImplementedError(
    ...
    def _map_resources(self, save_options):
        ...
        obj_map, resource_map = self._primary._map_resources(
            save_options
        )  # pylint:disable=protected-access
        for v in [v for v in self._values if v != self._primary]:

            if (
                save_options.experimental_variable_policy._expand_distributed_variables()  # pylint:disable=protected-access
            ):
                v_obj_map, v_resource_map = v._map_resources(
                    save_options
                )  # pylint:disable=protected-access
    ...
    def _write_object_proto(self, proto, options):
        ...
        if self._policy:
            if self._policy._is_mirrored():  # pylint: disable=protected-access
                self._policy._write_object_proto(
                    self, proto, options
                )  # pylint: disable=protected-access
...
class _MirroredSaveable(saveable_object.SaveableObject):
    ...
    def __init__(self, mirrored_variable, primary_variable, name):
        ...
        tensor, spec = values_util.get_on_write_saveable(
            self._mirrored_variable, primary_variable, name
        )
...
class MirroredVariable(DistributedVariable, Mirrored):
    ...
    def scatter_min(self, *args, **kwargs):
        ...
        if (
            self._aggregation != vs.VariableAggregation.ONLY_FIRST_REPLICA
            and self._aggregation != vs.VariableAggregation.NONE
        ):
            raise NotImplementedError(
    ...
    def _as_graph_element(self):
        return (
            self._get_on_device_or_primary()._as_graph_element()
        )  # pylint: disable=protected-access
...
class _SyncOnReadSaveable(saveable_object.SaveableObject):
    ...
    def restore(self, restored_tensors, restored_shapes):
        ...
        return values_util.get_on_read_restore_ops(
            self._sync_on_read_variable, tensor, self._sync_on_read_variable.aggregation
        )
...
class SyncOnReadVariable(DistributedVariable):
    ...
    def assign_sub(self, value, use_locking=False, name=None, read_value=True):
        ...
        with ds_context.enter_or_assert_strategy(self._distribute_strategy):
            if (
                ...
                and not values_util.in_replica_update_context()
            ):
                values_util.mark_as_unsaveable()
                ...
                return super(SyncOnReadVariable, self).assign_sub(
    ...
    def assign_add(self, value, use_locking=False, name=None, read_value=True):
        ...
        with ds_context.enter_or_assert_strategy(self._distribute_strategy):
            if (
                ...
                return super(SyncOnReadVariable, self).assign_add(
                    value, use_locking, name, read_value
                )
    ...
    def assign(self, value, use_locking=False, name=None, read_value=True):
        ...
        with ds_context.enter_or_assert_strategy(self._distribute_strategy):
            if (
                ...
                return super(SyncOnReadVariable, self).assign(
                    value, use_locking, name, read_value
                )
    ...
    def _dense_var_to_tensor(self, dtype=None, name=None, as_ref=False):
        ...
        with ds_context.enter_or_assert_strategy(self._distribute_strategy):
            replica_context = ds_context.get_replica_context()
            if (
                replica_context is not None
                and ds_context.in_variable_sync_on_read_context()
            ):
                if self._aggregation == vs.VariableAggregation.ONLY_FIRST_REPLICA:
                    return ops.convert_to_tensor(
                ...
                if self._aggregation == vs.VariableAggregation.SUM:
                    values_util.mark_as_unsaveable()
                # pylint: disable=protected-access
                reduced = replica_context.strategy.extended._replica_ctx_all_reduce(
                    reduce_util.ReduceOp.from_variable_aggregation(self._aggregation),
...
# DistributedVariable
def _tensor_conversion_distributed_var(var, dtype=None, name=None, as_ref=False):
...
ops.register_tensor_conversion_function(MirroredVariable, _tensor_conversion_mirrored)


# Mirrored Values
def _tensor_conversion_mirrored_val(value, dtype=None, name=None, as_ref=False):
    return ops.convert_to_tensor(
        value._get(), dtype=dtype, name=name, as_ref=as_ref
    )  # pylint: disable=protected-access


ops.register_tensor_conversion_function(Mirrored, _tensor_conversion_mirrored_val)
...
class OnReadPolicy(VariablePolicy):
    ...
    def _scatter_not_implemented(self, method):
        raise NotImplementedError(
            f"ON_READ variables doesn't support `{method}` " "in cross replica context"
        )
...
class OnWritePolicy(VariablePolicy):
    ...
    def _as_graph_element(self, var):
        return (
            var._get_on_device_or_primary()._as_graph_element()
        )  # pylint: disable=protected-access
...
class PerWorkerResource:
    ...
    def local_resource(self):
        ...
        return self._host_to_resources.get(
            host_device, self._host_to_resources[next(iter(self._host_to_resources))]
        )
