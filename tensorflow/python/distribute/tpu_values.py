...
_scatter_error_msg = (
    "{op_name} is only supported for distributed "
...
class TPUVariableMixin(object):
    ...
    def handle(self):
        ...
        if tpu_context is None or context.executing_eagerly():
            ...
            return tpu_context.get_replicated_var_handle(
                self._common_name, self._handle_id, val, self._is_mirrored(), is_packed
            )
    ...
    def op(self):
        ...
        return values.DistributedVarOp(
            self._primary.op.name,
...
class TPUMirroredVariable(TPUVariableMixin, values.MirroredVariable):
    ...
    def device(self):
        if (
            self._is_replicated_or_sharded_to_logical_cores()
    ...
    def assign(self, value, use_locking=False, name=None, read_value=True):
        ...
        if (
            tpu_util.enclosing_tpu_context()
            and self.aggregation == variable_scope.VariableAggregation.NONE
        ):
            return tpu_util.make_raw_assign_fn(
...
class TPUOnWritePolicy(values.OnWritePolicy):
    ...
    def assign_sub(self, var, value, use_locking=False, name=None, read_value=True):
        if (
            tpu_util.enclosing_tpu_context()
            and var.aggregation == variable_scope.VariableAggregation.NONE
        ):
            return tpu_util.make_raw_assign_fn(
                gen_resource_variable_ops.assign_sub_variable_op
            )(
