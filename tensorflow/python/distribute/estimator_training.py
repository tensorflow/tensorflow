...
def _get_global_id(cluster_spec, task_type, task_id, chief_task_type):
    ...
    # `task_type_ordered_list`.
    raise RuntimeError(
        "Internal Error: `task_type` ({}) is not in "
...
def init_run_config(config, tf_config):
    ...
    # pylint: disable=protected-access
    if (
        config._experimental_distribute
        and config._experimental_distribute.train_distribute
    ):
        if config._train_distribute:
            raise ValueError(
                "Either `train_distribute` or"
    ...
    if (
        config._experimental_distribute
        and config._experimental_distribute.eval_distribute
    ):
        if config._eval_distribute:
            raise ValueError(
                "Either `eval_distribute` or"
    ...
    if (
        config._train_distribute
        and config._experimental_distribute
        and config._experimental_distribute.remote_cluster
    ):
        if cluster_spec:
            raise ValueError(
                'Cannot set both "cluster_spec" of TF_CONFIG and '
        ...
        logging.info(
            "RunConfig initialized for Distribute Coordinator with "
    ...
    if (
        not cluster_spec
        or "master" in cluster_spec.jobs
        or not config._train_distribute
    ):
...
def should_run_distribute_coordinator(config):
    ...
    # pylint: disable=protected-access
    if (
        not hasattr(config, "_distribute_coordinator_mode")
        or config._distribute_coordinator_mode is None
    ):
        logging.info("Not using Distribute Coordinator.")
        return False
    if not isinstance(
        config._distribute_coordinator_mode, six.string_types
    ) or config._distribute_coordinator_mode not in [
        dc.CoordinatorMode.STANDALONE_CLIENT,
        dc.CoordinatorMode.INDEPENDENT_WORKER,
    ]:
        logging.warning(
            "Unexpected distribute_coordinator_mode: %r",
            config._distribute_coordinator_mode,
        )
        return False
    if not config.cluster_spec:
        logging.warning(
            "Running `train_and_evaluate` locally, ignoring "
...
def train_and_evaluate(estimator, train_spec, eval_spec, executor_cls):
    ...
    def _worker_fn(strategy):
        ...
        if (
            run_config._distribute_coordinator_mode
            == dc.CoordinatorMode.INDEPENDENT_WORKER  # pylint: disable=protected-access
            or context.is_chief
        ):
        ...
        local_estimator._config._distribute_coordinator_mode = (
            None  # pylint: disable=protected-access
        )
        local_estimator.train(
            input_fn=train_spec.input_fn, max_steps=train_spec.max_steps, hooks=hooks
        )
...
def estimator_train(estimator, train_distributed_fn, hooks):
    ...
    if "evaluator" in cluster_spec.jobs:
        raise ValueError(
            "'evaluator' job is not supported if you don't use " "`train_and_evaluate`"
        )

    if (
        estimator._config._distribute_coordinator_mode
        != dc.CoordinatorMode.STANDALONE_CLIENT  # pylint: disable=protected-access
    ):
        raise ValueError(
...
def estimator_evaluate(estimator, evaluate_distributed_fn, hooks):
    ...
    if estimator._config._eval_distribute.extended.experimental_between_graph:
        ...
        # return values from `_worker_fn`s.
        raise ValueError(
            "`Estimator.evaluate` API is not supported for %s with "
            "`STANDALONE_CLIENT` mode."
