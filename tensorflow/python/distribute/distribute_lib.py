        ...
        if self._resource_creator_scope:
            try:
                if isinstance(self._resource_creator_scope, list):
                    ...
                    nest.map_structure(
                        lambda scope: scope.__exit__(
                            exception_type,
                            exception_value,  # pylint:disable=g-long-lambda
                            traceback,
                        ),
...
class ValueContext(object):
    ...
    def __str__(self):
        return (
            "tf.distribute.ValueContext(replica id {},  total replicas in sync: {})"
    ...
    * _How to update a distributed variable_
    ...
    def _scope(self, strategy):
        ...
        def creator_with_resource_vars(next_creator, **kwargs):
            ...
            with monitoring.MonitoredTimer(
                distributed_variable_creation_time_counter.get_cell(
                    strategy.__class__.__name__, if_graph_building
                )
            ):
                ...
                if isinstance(
                    kwargs["initial_value"], trackable.CheckpointInitialValue
                ):
    ...
    def reduce_to(self, reduce_op, value, destinations, options=None):
        ...
        * `tf.distribute.Strategy.reduce`: a more convenient method to reduce
        ...
        """
        with (
            monitoring.MonitoredTimer(
                distributed_api_time_counter.get_cell(
                    self.__class__.__name__, "Reduce_to_eagerly"
                )
            )
    ...
    def batch_reduce_to(self, reduce_op, value_destination_pairs, options=None):
        ...
        * `tf.distribute.Strategy.reduce`: a more convenient method to reduce
        ...
        with (
            monitoring.MonitoredTimer(
                distributed_api_time_counter.get_cell(
                    self.__class__.__name__, "Batch_reduce_to_eagerly"
                )
            )
            if not ops.inside_function()
            else contextlib.nullcontext()
...
class ReplicaContextBase(object):
    ...
    def _merge_call(self, merge_fn, args, kwargs):
        ...
        _push_per_thread_mode(  # thread-local, so not needed with multiple threads
            _CrossReplicaThreadMode(self._strategy)
        )  # pylint: disable=protected-access
        try:
...
class _DefaultDistributionExtended(StrategyExtendedV1):
    ...
    def parameter_devices(self):
        raise RuntimeError(
            "parameter_devices() method unsupported by default tf.distribute.Strategy."
        )
