...
def _load_tf_config(port):
    ...
    assert all(
        [x in os.environ for x in [_SM_CURRENT_HOST, _SM_HOSTS]]
    ), "Not a SageMaker Environment"
    hosts = (
        sorted(json.loads(os.environ[_SM_HOSTS])) if os.environ[_SM_HOSTS] != "" else []
    )
    ...
    tf_config = {
        _CLUSTER_KEY: {_WORKER_KEY: hosts},
...
class SageMakerClusterResolver(ClusterResolver):
    ...
    def __init__(
        self, port=2223, task_type=None, task_id=None, rpc_layer=None, environment=None
    ):
