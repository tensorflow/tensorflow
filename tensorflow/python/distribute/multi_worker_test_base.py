...
class MultiProcessCluster(object):
    ...
    def kill_task(self, task_type, task_id):
        ...
        assert self._mpr
        if (
            not self._start_events[task_type][task_id].is_set()
            or self._finish_events[task_type][task_id].is_set()
        ):
    ...
    def start_task(self, task_type, task_id):
        ...
        if (
            not self._start_events[task_type][task_id].is_set()
            or not self._finish_events[task_type][task_id].is_set()
        ):
            raise ValueError(
...
def create_multi_process_cluster(
    num_workers,
    num_ps,
    has_chief=False,
    has_eval=False,
    ...
    logging.info(
        "Now creating a MultiProcessCluster with "
        f"num_workers={num_workers}, num_ps={num_ps}."
    )
    cluster_spec = create_cluster_spec(
        has_chief=has_chief, num_workers=num_workers, num_ps=num_ps, has_eval=has_eval
    )
...
class MultiWorkerTestBase(test.TestCase):
    ...
    @classmethod
    def setUpClass(
        cls, num_workers=2, num_ps=1
    ):  # pylint: disable=g-missing-super-call
        """Create a local cluster with 2 workers."""
        cls._cluster_spec = create_in_process_cluster(
    ...
    def _run_client(
        self, client_fn, task_type, task_id, num_gpus, eager_mode, *args, **kwargs
    ):
    ...
    def _run_between_graph_clients(
        self, client_fn, cluster_spec, num_gpus, *args, **kwargs
    ):
...
class IndependentWorkerTestBase(test.TestCase):
    ...
    def _run_task_in_thread(
        self, task_fn, cluster_spec, task_type, task_id, *args, **kwargs
    ):
    ...
    def run_multiple_tasks_in_threads(self, task_fn, cluster_spec, *args, **kwargs):
        ...
        for task_type in cluster_spec.keys():
            ...
            for task_id in range(len(cluster_spec[task_type])):
                t = self._run_task_in_thread(
                    task_fn, cluster_spec, task_type, task_id, *args, **kwargs
                )
...
class MultiWorkerMultiProcessTest(test.TestCase):
    ...
    def _run_task_in_process(self, cmd_args, cluster_spec, task_type, task_id):
        ...
        env["TF_CONFIG"] = json.dumps(
            {"cluster": cluster_spec, "task": {"type": task_type, "index": task_id}}
        )
    ...
    @deprecation.deprecated(
        None,
        "`run_multiple_tasks_in_processes` is deprecated; any new test "
    ...
    def run_multiple_tasks_in_processes(self, cmd_args, cluster_spec):
        ...
        for task_type in cluster_spec.keys():
            processes[task_type] = []
            for task_id in range(len(cluster_spec[task_type])):
                p = self._run_task_in_process(
                    cmd_args, cluster_spec, task_type, task_id
                )
    ...
    @deprecation.deprecated(
        None,
        "`join_independent_workers` is deprecated; any new test "
        "requiring multiple processes should use `multi_process_runner` for "
        "better support of log printing, streaming, and more functionality.",
    )
    ...
    @deprecation.deprecated(
        None,
        "`stream_stderr` is deprecated; any new test "
        "requiring multiple processes should use `multi_process_runner` for "
        "better support of log printing, streaming, and more functionality.",
    )
