...
class CollectiveReplicaLauncher(object):
    ...
    def _all_gather(
        self, input_tensor: core.TensorLike, options: Optional[collective_util.Options]
    ) -> core.Tensor:
    ...
    def all_reduce_indexed_slices(
        ...
        with ops.device(self._device):
            ...
            def all_gather_with_padding(
                input_tensor: core.TensorLike,
                options: Optional[collective_util.Options],
            ) -> core.Tensor:
    ...
    with ops.device(device):
        if isinstance(value, indexed_slices.IndexedSlices):
            ...
            if value.dense_shape is not None:
                ...
                copied_shape = None
            result = indexed_slices.IndexedSlices(
                copied_values, copied_indices, copied_shape
            )
        else:
