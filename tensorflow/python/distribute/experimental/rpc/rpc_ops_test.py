...
class RpcOpsTest(test.TestCase):
    ...
    def test_generated_rpc_ops(self):
        ...
        self.assertAllEqual(
            rpc_ops.gen_rpc_ops.rpc_get_value(future_resource, Tout=[dtypes.int32]), [6]
        )
    ...
    def test_rpc_ops_non_blocking_convenience_methods(self):
        ...
        self.assertEqual(
            client.multiply.__doc__, "RPC Call for multiply method to server " + address
        )
    ...
    def test_call_register_ordering(self):
        ...
        with self.assertRaises(errors.DeadlineExceededError):
            rpc_ops.GrpcClient(
                address, name="client1", list_registered_methods=True, timeout_in_ms=1
            )
        ...
        result_or = client.call(
            "assign_add", [variables.Variable(2, dtype=dtypes.int64)]
        )
        self.assertAllEqual(result_or.is_ok(), True)
    ...
    def test_client_timeout(self):
        ...
        def ensure_server_is_ready(client):
            ...
            while not server_ready:
                result_or = client.call(
                    "add", [constant_op.constant(20), constant_op.constant(30)]
                )
                if result_or.is_ok():
                    server_ready = True
                else:
        ...
        result_or = client.call(
            "add", [constant_op.constant(20), constant_op.constant(30)], timeout_in_ms=1
        )
        ...
        result_or = client.call(
            "add",
            [constant_op.constant(20), constant_op.constant(30)],
            timeout_in_ms=5000,
        )
        self.assertAllEqual(result_or.is_ok(), True)
        error_code, _ = result_or.get_error()
    ...
    def test_async_call_op_wrapper(self):
        ...
        for _ in range(10):
            futures.append(
                client.call("assign_add", [variables.Variable(2, dtype=dtypes.int64)])
            )
    ...
    def test_register_method_twice(self):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "assign is already registered."
        ):
