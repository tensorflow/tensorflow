...
def get_output_specs_from_function(func: tf_function.ConcreteFunction):
    output_specs = nest.map_structure(
        type_spec.type_spec_from_value, func.structured_outputs
    )
...
class Server(object):
    ...
    def register(
        ...
        raise NotImplementedError(
            "Please use create_server method to create aconcrete subclass of Server."
        )
...
class GrpcClient(Client):
    ...
    def __init__(
        ...
        for method in methods.numpy():
            ...
            self._add_method(
                m.method, output_specs, input_specs, self._client_handle, doc_string
            )
...
class StatusOrResult(object):
    ...
    def is_ok(self):
        ...
        return math_ops.equal(
            self._error_code, constant_op.constant(0, dtype=dtypes.int64)
        )
    ...
    def get_value(self):
        ...
        self._check_status()
        if self._output_specs is None or isinstance(
            self._output_specs, none_tensor.NoneTensorSpec
        ):
            flat_output_dtypes = []
            return_none = True
        else:
