...
class StrategyBaseTest(test_util.DTensorBaseTest):
    ...
    def test_nested_structure_output(self):
        strategy = mirrored_strategy.MirroredStrategy(mesh=self.mesh)
        array_value = np.array([3.0, 2.0, 1.0])
        ...
        self.assertAllClose(result["b"].values[0], constant_op.constant([8.0, 10.0]))
    ...
    def test_gather_dtensor_value(self):
        ...
        def value_fn(value_context):
            ...
            return array_ops.reshape(
                math_ops.range(start=start, limit=start + 6), shape=(1, 2, 3)
            )
    ...
    def test_experimental_local_results_with_inputs(self):
        ...
        self.assertDictEqual(
            local_result[0],
            {"a": constant_op.constant([6.0]), "b": constant_op.constant([8.0, 10.0])},
        )
        self.assertDictEqual(
            local_result[1],
            {"a": constant_op.constant([4.0]), "b": constant_op.constant([6.0, 8.0])},
        )
...
class StrategyDatasetTest(test_util.DTensorBaseTest):
    ...
    def test_create_partial_batched_dataset(self):
        ...
        dataset = dataset_ops.Dataset.from_tensors((self.images, self.labels)).repeat(
            30
        )  # There is a last partial batch
        ...
        self.assertEqual(
            ...
            (
                tensor_spec.TensorSpec(
                    shape=(8, 8, 8, 3), dtype=dtypes.float32, name=None
                ),
                tensor_spec.TensorSpec(shape=(8, 1), dtype=dtypes.float32, name=None),
            ),
        )
    ...
    def test_distribute_values_from_function(self):
        ...
        self.assertDTensorEqual(
            constant_op.constant([3.0, 2.0], dtype=dtypes.float64),
    ...
    def test_distribute_values_from_function_with_nested_structure(self):
        ...
        self.assertAllClose(unpacked_a[0], [3.0])
        self.assertAllClose(unpacked_a[1], [2.0])
        self.assertDTensorEqual(
            constant_op.constant([4.0, 5.0, 3.0, 4.0], dtype=dtypes.float64),
            layout.Layout.batch_sharded(self.mesh, batch_dim="batch", rank=1),
