...
from tensorflow.python.distribute.experimental import (
    multi_worker_mirrored_strategy as mwms,
)
...
class MultiWorkerMirroredStrategyTest(tf_test.TestCase, parameterized.TestCase):
    ...
    def test_strategy_creation_with_default_cluster_resolver(self):
        ...
        self.assertLen(
            mesh.global_device_ids(), self.num_client * self.num_local_devices
        )
        self.assertLen(mesh.local_device_ids(), self.num_local_devices)
        self.assertIsInstance(
            strategy._cluster_resolver,
    ...
    def test_invalid_init_arguments(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Mesh and cluster_resolver can not be provided at the same time"
        ):
            mwms.MultiWorkerMirroredStrategy(
                mesh=mesh, cluster_resolver=cluster_resolver
            )
    ...
    def test_num_replica_in_sync(self):
        strategy = mwms.MultiWorkerMirroredStrategy()
        self.assertEqual(
            strategy.num_replicas_in_sync, self.num_client * self.num_local_devices
        )
    ...
    def test_nested_structure_output(self):
        ...
        for i in range(self.num_local_devices):
            self.assertAllClose(
                ...
                constant_op.constant(
                    [
                        (strategy.num_replicas_in_sync + 1.0) * 2.0,
    ...
    def test_gather_dtensor_value(self):
        ...
        self.assertAllClose(
            result,
            array_ops.reshape(math_ops.range(start=start, limit=end), shape=(1, -1)),
        )
...
class StrategyDatasetTest(tf_test.TestCase, parameterized.TestCase):
    ...
    def test_distribute_dataset_from_fn(self):
        ...
        def dataset_fn(option):
            del option
            return (
                dataset_ops.Dataset.from_tensors((self.images, self.labels))
                .repeat()
        ...
        self.assertEqual(
            distributed_dataset.element_spec,
            (
                tensor_spec.TensorSpec(
                    shape=(global_batch_size, 8, 8, 3), dtype=dtypes.float32, name=None
                ),
                tensor_spec.TensorSpec(
                    shape=(global_batch_size, 1), dtype=dtypes.float32, name=None
                ),
            ),
        )
    ...
    def test_distribute_values_from_function(self):
        ...
        self.assertEqual(
            d_api.fetch_layout(distributed_values),
            layout.Layout.batch_sharded(strategy.mesh, batch_dim="batch", rank=1),
        )
