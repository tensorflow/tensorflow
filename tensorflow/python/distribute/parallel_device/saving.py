...
if issubclass(_variable_type, _wrapt_type):
    ...
    class VariableProxyMetaClass(
        _wrapt_type, _variable_type
...
class ParallelVariable(
    ...
    def _gather_saveables_for_checkpoint(self):
        ...
        for index, handle in enumerate(self._self_parallel_device.unpack(self.handle)):
            if index == 0:
            ...
            component_saveables[attribute] = functools.partial(
...
def _variable_creator(next_creator, parallel_device, **kwargs):
    ...
    if (
        variable.device == parallel_device._name
    ):  # Friend access; pylint: disable=protected-access
