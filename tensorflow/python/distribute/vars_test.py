...
class OnWriteVariableSyncScatterTests(test.TestCase, parameterized.TestCase):
    ...
    def testScatterMin(self, distribution):
        ...
        with self.assertRaisesRegex(NotImplementedError, "scatter_min.*"):
            self.evaluate(
                test_util.gather(
                    distribution, distribution.run(scatter_min, args=(v1,))
                )
            )

        per_replica_results = self.evaluate(
            test_util.gather(distribution, distribution.run(scatter_min, args=(v2,)))
        )
    ...
    def testScatterMax(self, distribution):
        ...
        with self.assertRaisesRegex(NotImplementedError, "scatter_max.*"):
            self.evaluate(
                test_util.gather(
                    distribution, distribution.run(scatter_max, args=(v1,))
                )
            )

        per_replica_results = self.evaluate(
            test_util.gather(distribution, distribution.run(scatter_max, args=(v2,)))
        )
    ...
    def testScatterUpdate(self, distribution):
        ...
        with self.assertRaisesRegex(NotImplementedError, "scatter_update.*"):
            self.evaluate(
                test_util.gather(
                    distribution, distribution.run(scatter_update, args=(v1,))
                )
            )

        per_replica_results = self.evaluate(
            test_util.gather(distribution, distribution.run(scatter_update, args=(v2,)))
        )
