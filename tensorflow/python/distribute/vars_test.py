...
class OnWriteVariableSync(test.TestCase, parameterized.TestCase):
    ...
    def testAssign(self, distribution, experimental_run_tf_function):
        ...
        for update, aggregation, cross_replica in options:
            ...
            with distribution.scope():
                v = variable_scope.variable(0.0, aggregation=aggregation)
    ...
    def testAssignOnWriteVar(self, distribution, experimental_run_tf_function):
        ...
        with distribution.scope():
            v_to_assign = variable_scope.variable(
                2.0, aggregation=variables_lib.VariableAggregation.MEAN
            )
    ...
    def testAssignPerReplicaVal(self, distribution, experimental_run_tf_function):
        ...
        if strategy_test_lib.is_tpu_strategy(distribution):
            self.skipTest(
                "Assigning PerReplica values is not supported. See"
        ...
        updates = [
            ("assign", per_replica_value),
            ("assign_add", per_replica_value),
    ...
    def testValueInReplicaContext(self, distribution):
        with distribution.scope():
            v = variables_lib.Variable(
                1.0, aggregation=variables_lib.VariableAggregation.MEAN
            )
            self.evaluate(variables_lib.global_variables_initializer())
            ...
            results = self.evaluate(
                distribution.experimental_local_results(distribution.run(f))
            )
    ...
    def testReadValueInReplicaContext(self, distribution, experimental_run_tf_function):
        ...
        for aggregation in aggregations:
            ...
            results = self.evaluate(
                distribution.experimental_local_results(distribution.run(read_var_fn))
            )
    ...
    def testReadValueInCrossReplicaContext(
        ...
        for aggregation in aggregations:
            with distribution.scope():
                v = variable_scope.variable(2.0, aggregation=aggregation)
            self.evaluate(variables_lib.global_variables_initializer())
            ...
            for component in v._values:
                self.assertEqual(
                    self.evaluate(component.read_value()), self.evaluate(results)
                )
            ...
            with ops.init_scope():
                if obj.w is None:
                    obj.w = variables_lib.Variable(
                        0.0, aggregation=variables_lib.VariableAggregation.MEAN
                    )
...
class OnWriteVariableSyncScatterTests(test.TestCase, parameterized.TestCase):
    ...
    def testScatterSub(self, distribution):
        with distribution.scope():
            v = variables_lib.Variable(
                [0.0, 0.0, 0.0], aggregation=variables_lib.VariableAggregation.MEAN
            )
        ...
        per_replica_results = self.evaluate(
            distribution.experimental_local_results(distribution.run(scatter_sub))
        )
    ...
    def testScatterAdd(self, distribution):
        ...
        per_replica_results = self.evaluate(
            distribution.experimental_local_results(distribution.run(scatter_add))
        )
    ...
    def testScatterDiv(self, distribution):
        ...
        per_replica_results = self.evaluate(
            distribution.experimental_local_results(distribution.run(scatter_div))
        )
    ...
    def testScatterMul(self, distribution):
        with distribution.scope():
            v = variables_lib.Variable(
                [2.0, 1.0, 1.0], aggregation=variables_lib.VariableAggregation.MEAN
            )
        self.evaluate(v.initializer)
        ...
        per_replica_results = self.evaluate(
            distribution.experimental_local_results(distribution.run(scatter_mul))
        )
    ...
    def testScatterOpsWithNoneAggregation(self, distribution):
        def assert_close(v, op, delta, expect):
            ...
            per_replica_results = self.evaluate(
                distribution.experimental_local_results(distribution.run(scatter_xxx))
            )
            self.assertAllClose([expect, expect], per_replica_results)

        with distribution.scope():
            v = variables_lib.Variable(
                [4.0], aggregation=variables_lib.VariableAggregation.NONE
            )
        self.evaluate(variables_lib.global_variables_initializer())
...
class OnReadVariableSyncTest(test.TestCase, parameterized.TestCase):
    ...
    def testAssignOnReadVar(self, distribution, experimental_run_tf_function):
        ...
        for update, aggregation, cross_replica in options:
            ...
            if cross_replica:
                for component in v._values:
                    self.assertAllEqual(
                        expected_cross_replica.get(aggregation),
                        self.evaluate(component.read_value()),
                    )
            else:
                for component in v._values:
                    self.assertAllEqual(
                        expected_replica.get(aggregation),
                        self.evaluate(component.read_value()),
                    )
    ...
    def testAssignPerReplicaVal(self, distribution, experimental_run_tf_function):
        ...
        self.skipTest(
            "We don't support assiging PerReplica values in cross "
    ...
    def testReadValueInCrossReplicaContext(
        ...
        for aggregation in aggregations:
            ...
            self.assertEqual(
                expected, self.evaluate(array_ops.identity(v)), aggregation
            )

    # TODO(b/145574622): Re-enable this test once ReduceOp argument is
    # respected on GPUs.
    @combinations.generate(strategy_and_run_tf_function_combinations())
    def disable_testAllReduce(self, distribution, experimental_run_tf_function):
        with distribution.scope():
            v = variable_scope.variable(
                2.0,
                synchronization=variables_lib.VariableSynchronization.ON_WRITE,
    ...
    @combinations.generate(strategy_and_run_tf_function_combinations())
    def testAssignPerReplicaBeforeRead(
        self, distribution, experimental_run_tf_function
    ):
        aggregations = [
            variables_lib.VariableAggregation.SUM,
        ...
        for agg in aggregation:
            ...
            with strategy.scope():
                ...
                def assign_fn():
                    ...
                    if (
                        cluster_resolver and cluster_resolver.task_type == "worker"
                    ) or math_ops.equal(
                        replica_ctx.replica_id_in_sync_group, constant_op.constant(1)
                    ):
                        v_on_read.assign(3.0)  # pylint:disable=cell-var-from-loop
                    else:
                        v_on_read.assign(4.0)  # pylint:disable=cell-var-from-loop
...
class SyncOnReadScatterReplicaTest(test.TestCase, parameterized.TestCase):
        ...
        delta = values.PerReplica(
            [
                indexed_slices.IndexedSlices(
                    values=[[0.0], [1.0]], indices=[0, 1], dense_shape=(3,)
                ),
                indexed_slices.IndexedSlices(
                    values=[[1.0], [2.0]], indices=[1, 2], dense_shape=(3,)
                ),
            ]
        )
    ...
    def testScatterDiv(self, distribution, aggregation):
        ...
        delta = values.PerReplica(
            [
                indexed_slices.IndexedSlices(
                    values=[[2.0], [2.0]], indices=[0, 1], dense_shape=(3,)
                ),
                indexed_slices.IndexedSlices(
                    values=[[3.0], [3.0]], indices=[1, 2], dense_shape=(3,)
                ),
            ]
        )
    ...
    def testScatterMul(self, distribution, aggregation):
        ...
        delta = values.PerReplica(
            [
                indexed_slices.IndexedSlices(
                    values=[[2.0], [3.0]], indices=[0, 1], dense_shape=(3,)
                ),
                indexed_slices.IndexedSlices(
                    values=[[4.0], [5.0]], indices=[1, 2], dense_shape=(3,)
                ),
            ]
        )
    ...
    def testScatterMin(self, distribution, aggregation):
        ...
        delta = values.PerReplica(
            [
                indexed_slices.IndexedSlices(
                    values=[[1.0], [8.0]], indices=[0, 1], dense_shape=(3,)
                ),
                indexed_slices.IndexedSlices(
                    values=[[9.0], [2.0]], indices=[1, 2], dense_shape=(3,)
                ),
            ]
        )
    ...
    def testScatterUpdate(self, distribution, aggregation):
        ...
        delta = values.PerReplica(
            [
                indexed_slices.IndexedSlices(
                    values=[[1.0], [2.0]], indices=[0, 1], dense_shape=(3,)
                ),
                indexed_slices.IndexedSlices(
                    values=[[3.0], [4.0]], indices=[1, 2], dense_shape=(3,)
                ),
            ]
        )
