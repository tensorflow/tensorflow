...
def is_mirrored_strategy(strategy: distribute_lib.Strategy) -> bool:
    return isinstance(
        strategy, (mirrored_lib.MirroredStrategy, mirrored_lib.MirroredStrategyV1)
    )
...
def is_multi_worker_mirrored_strategy(strategy: distribute_lib.Strategy) -> bool:
    return isinstance(
        strategy,
        (mwms_lib.CollectiveAllReduceStrategy, mwms_lib.CollectiveAllReduceStrategyV1),
    )
...
class DistributionTestBase(test.TestCase):
    ...
    def _test_replica_id(self, d):
        with d.scope():
            ...
            self.assertAllEqual(
                expected_devices, [True] * len(d.extended.worker_devices)
            )
    ...
    def _input_fn_to_test_input_context(
        self,
        ...
        def _input_fn(input_context):
            ...
            self.assertEqual(
                expected_num_replicas_in_sync, input_context.num_replicas_in_sync
            )
            self.assertEqual(
                expected_num_input_pipelines, input_context.num_input_pipelines
            )
            if expected_input_pipeline_id is not None:
                self.assertEqual(
                    expected_input_pipeline_id, input_context.input_pipeline_id
                )
            else:
    ...
    def _test_numpy_dataset(self, strategy, session=None, run_in_function=False):
        ...
        with strategy.scope(), cached_session as sess:
            ...
            def run_and_concatenate(strategy, i):
                ...
                x, y = self.evaluate(
                    (
                        strategy.experimental_local_results(x),
...
class OneDeviceDistributionTestBase(test.TestCase):
    ...
    def _test_all_reduce_sum(self, strategy):
        self._test_collective_comms(
            strategy, _all_sum, inputs=(4.0, [42.0, 43.0]), expected=(4.0, [42.0, 43.0])
        )

    def _test_all_reduce_sum_gradients(self, strategy):
        self._test_collective_comms_gradients(
            strategy, _all_sum, inputs=[4.0], expected_grads=[4.0]
        )
    ...
    def _test_all_reduce_mean(self, strategy):
        self._test_collective_comms(
            strategy,
            _all_mean,
            inputs=(2.0, [21.0, 22.0]),
            expected=(2.0, [21.0, 22.0]),
        )

    def _test_all_reduce_mean_gradients(self, strategy):
        self._test_collective_comms_gradients(
            strategy, _all_mean, inputs=[5.0], expected_grads=[5.0]
        )
...
class TwoDeviceDistributionTestBase(test.TestCase):
    ...
    def _test_run(self, strategy, run_in_function=False):
        out1 = strategy.run(
            _maybe_run_in_function(
                lambda: distribute_lib.get_replica_context().replica_id_in_sync_group
                + 1,
                run_in_function,
            )
        )
        ...
        out2 = strategy.run(
            _maybe_run_in_function(lambda x: {"a": x * 2, "b": x * x}, run_in_function),
            args=(out1,),
        )
        out2_vals = self.evaluate(nest.map_structure(strategy.unwrap, out2))
        ...
        out3 = strategy.run(
            _maybe_run_in_function(lambda b, a: a + 2 * b + 2, run_in_function),
            kwargs=out2,
        )
    ...
    def _test_all_reduce_sum(self, strategy, run_in_function=False):
        self._test_collective_comms(
            strategy,
            _all_sum,
            inputs=([1.0, 3.0], [[39.0, 2.0], [3.0, 41.0]]),
            expected=(4.0, [42.0, 43.0]),
    ...
    def _test_collective_comms(
        self, strategy, comm_fn, inputs, expected, run_in_function=False
    ):
        inputs = strategy.make_input_fn_iterator(
    ...
    def _test_collective_comms_gradients(
        self, strategy, comm_fn, inputs, expected_grads, run_in_function=False
    ):
        if context.executing_eagerly() and not run_in_function:
            self.skipTest(
                "`tf.gradients` is not supported with eager execution "
