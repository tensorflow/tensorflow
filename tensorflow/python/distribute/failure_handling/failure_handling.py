...
class TerminationConfig(object):
    ...
    * For Other platform:
    ...
    def __init__(
        self, termination_watcher_fn=None, exit_fn=None, grace_period=None, save_fn=None
    ):
...
class GcpGpuTerminationConfig(TerminationConfig):
    ...
    def __init__(  # pylint: disable=super-init-not-called
        ...
        self.grace_period = (
            grace_period
            if grace_period or grace_period == 0
            else failure_handling_util.GRACE_PERIOD_GCE
        )
        self.save_fn = save_fn
...
def _complete_config_for_environment(platform_device, termination_config):
    ...
    if platform_device is failure_handling_util.PlatformDevice.GCE_GPU:
        return GcpGpuTerminationConfig(
        ...
        return GcpCpuTerminationConfig(
            termination_config.termination_watcher_fn,
            termination_config.exit_fn,
            termination_config.grace_period,
            termination_config.save_fn,
        ...
        return BorgTPUTerminationConfig(
            termination_config.termination_watcher_fn,
            termination_config.exit_fn,
            termination_config.grace_period,
            termination_config.save_fn,
...
class PreemptionCheckpointHandler(object):
    ...
    **A note on the platform:**
    ...
    def __init__(
        ...
        if (
            isinstance(checkpoint_or_checkpoint_manager, checkpoint_lib.Checkpoint)
            and not checkpoint_dir
        ):
            raise errors.InvalidArgumentError(
                "When a checkpoint is passed, a checkpoint_dir must be passed as well."
            )
    ...
    def _initialize_for_mirrored_and_multi_worker_mirrored(self):
        ...
        if not hasattr(
            self._write_checkpoint_manager._checkpoint,  # pylint: disable=protected-access
            _ITERATION_VARIABLE,
        ):
            setattr(
                self._write_checkpoint_manager._checkpoint,
                _ITERATION_VARIABLE,  # pylint: disable=protected-access
                self._checkpointed_runs,
            )

        if not hasattr(
            self._read_checkpoint_manager._checkpoint,  # pylint: disable=protected-access
            _ITERATION_VARIABLE,
        ):
            setattr(
                self._read_checkpoint_manager._checkpoint,
                _ITERATION_VARIABLE,  # pylint: disable=protected-access
                self._checkpointed_runs,
            )
        ...
        distribute_lib.distribution_strategy_input_api_counter.get_cell(
            self._platform_device.name, "PreemptionCheckpointHandler"
        ).increase_by(1)
        ...
        if isinstance(
            ...
            if self._is_chief:
                ...
                self._write_checkpoint_manager = (
                    checkpoint_management.CheckpointManager(
                        self._checkpoint_or_checkpoint_manager,
                        _non_chief_checkpoint_dir(
                            self._checkpoint_dir, self._cluster_resolver.task_id
                        ),
                        max_to_keep=1,
                    )
                )
    ...
    def _poll_termination_signal(self):
        ...
        while True:
            if (
                self._poll_termination_signal_thread_should_stop.is_set()
    ...
    def _maybe_set_received_own_sigterm(self):
        ...
        if self._local_mode:
            logging.info(
                "Member %s has received termination notice.", self._id_in_cluster
            )
        ...
        try:
            context.context().set_config_key_value(
                _PREEMPTION_WORKER_KEY, self._id_in_cluster
            )
            logging.info(
                "Member %s has received termination notice.", self._id_in_cluster
            )
    ...
    def _stop_cluster_wise_termination_watcher_thread(self):
        ...
        if getattr(self, "_cluster_wise_termination_watcher_thread", None):
            try:
                ...
                # process.
                logging.info(
                    "Ignoring error when shutting down "
                    "_stop_cluster_wise_termination_watcher_thread: " + str(e)
                )

            try:
                context.context().set_config_key_value(
                    _FINAL_RUN_COUNT_KEY, _STOP_WATCHING_CLUSTER_VALUE
                )
            except (errors.AlreadyExistsError, errors.UnavailableError):
                pass
    ...
    def total_run_calls(self):
        ...
        if self._platform_device == failure_handling_util.PlatformDevice.INTERNAL_TPU:
            raise NotImplementedError(
                "Please create variables saved in checkpoint "
        ...
        if self._platform_device == failure_handling_util.PlatformDevice.INTERNAL_TPU:
            ...
            try:
                ...
                if not self._local_mode:
                    logging.info("Propagating error to cluster: %r: %s", e, e)
                    try:
                        context.context().report_error_to_cluster(
    ...
    def _save_checkpoint(self, *args, **kwargs):
        ...
        distribute_lib.distribution_strategy_input_api_counter.get_cell(
            self._platform_device.name, "PreemptionCheckpointHandler Saving Checkpoint"
        ).increase_by(1)
        ...
        logging.info(
            "Checkpoint finished at path %s", self._write_checkpoint_manager.directory
        )
    ...
    def _check_preemption_and_maybe_checkpoint(self, *args, **kwargs):
        ...
        if self._received_checkpoint_step.is_set():
            ...
            if self._local_mode:
                ...
                context.context().set_config_key_value(
                    run_count_config_key, step_to_save_at
                )
    ...
    def _setup_countdown_if_has_grace_period_and_not_already_counting_down(self):
        ...
        if self._grace_period > 0 and not self._final_checkpoint_countdown:
            ...
            self._target_time_for_termination = (
                self._received_own_sigterm_time
                + self._grace_period
    ...
    def _watch_step_to_save_key(self):
        ...
        if step_value != _STOP_WATCHING_CLUSTER_VALUE:
            ...
            ack_key = (
                f"{_ACKNOWLEDGE_KEY}_{_INITIAL_RUN_COUNT_KEY}_{self._id_in_cluster}"
            )
            context.context().set_config_key_value(ack_key, "1")
            logging.info(
                "PreemptionCheckpointHandler: %s set, "
                "preemption awareness acknowledged",
                ack_key,
            )
