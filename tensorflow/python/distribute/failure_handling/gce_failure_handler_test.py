...
class GceFailureHandlingTest(test.TestCase, parameterized.TestCase):
    ...
    def worker_fn(
        ...
        with (
            mock.patch.object(
                ...
                "termination_watcher_function_gce",
                mock_termination_watcher_function_gce,
            ),
            mock.patch.object(
                failure_handling_util,
                "detect_platform",
            ...
            if test_util.has_thread(
                _PEER_WATCHER_THREAD_PREFIX, running_threads
            ) and test_util.has_thread(_LOCAL_WATCHER_THREAD_PREFIX, running_threads):
                try:
                    ...
                    running_threads = test_util.get_running_threads()
                    self.assertFalse(
                        test_util.has_thread(
                            _LOCAL_WATCHER_THREAD_PREFIX, running_threads
                        )
                    )
                    self.assertFalse(
                        test_util.has_thread(
    ...
    def test_multiple_workers_preempted_consecutively(
        self, grace_period, input_arg, strategy_option, api_wrapping_train
    ):
        checkpoint_dir = os.path.join(self.get_temp_dir(), "fh_ckpt/")
        if _is_oss():
            rpc_layer = "grpc"
    ...
    def test_grace_period_continue_training(
