...
def _make_checkpoint_manager(checkpoint, checkpoint_dir, cluster_resolver):
    if (
        not cluster_resolver
        or not cluster_resolver.cluster_spec().as_dict()
        or (
...
class PreemptionCheckpointTest(test.TestCase, parameterized.TestCase):
    ...
    def worker_fn(
        ...
        with mock.patch.object(failure_handling_util, "on_gcp", lambda: False):
            with strategy.scope():
                ...
                preemption_handler = failure_handling.PreemptionCheckpointHandler(
                    strategy.cluster_resolver,
                    checkpoint_or_manager,
                    checkpoint_dir,
                    termination_config,
                )
            ...
            def distributed_train_step(current_epoch, current_step):
                ...
                def train_step():
                    if cluster_spec and (
                        distribute_lib.get_distribution_strategy().cluster_resolver.task_id
            ...
            for epoch in range(
                preemption_handler.total_run_calls // STEPS_PER_EPOCH, EPOCHS_TO_RUN
            ):
                for step in range(
    ...
    def test_preemption_checkpointing(
        self, input_arg, strategy_option, api_wrapping_train
    ):
        has_chief = False
        ...
        def exit_fn_checking_metric():
            self.assertGreater(
                tracking_util._preemption_checkpoint_saved_time_usecs.get_cell().value(),
                0,
