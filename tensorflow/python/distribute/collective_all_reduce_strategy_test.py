...
def create_test_objects(
    cluster_spec=None, task_type=None, task_id=None, num_gpus=None, num_tpus=None
):
    if num_gpus is None:
...
class CollectiveAllReduceStrategyTestBase(multi_worker_test_base.MultiWorkerTestBase):
    ...
    def _get_test_object(
        self, task_type, task_id, num_gpus=0, num_tpus=0, use_devices_arg=False
    ):
    ...
    def _test_minimize_loss_graph(self, task_type, task_id, num_gpus):
        ...
        with (
            ...
            if (
                distribution.extended._local_device_type == "GPU"
                and context.num_gpus() < distribution.extended._num_devices_per_worker
            ):
                return True
...
class SingleWorkerCollectiveAllReduceStrategy(
    ...
    def testStrategyInitializationError(self):
        with self.assertRaisesRegex(
            ValueError, "cluster_resolver and devices cannot be set at the same time"
        ):
    ...
    @combinations.generate(
        combinations.combine(
            mode=["eager"], required_tpus=[0, 1, 2], use_devices_arg=[True, False]
        )
    )
...
def _replica_id_f32():
    return math_ops.cast(
        distribute_lib.get_replica_context().replica_id_in_sync_group, dtypes.float32
    )
