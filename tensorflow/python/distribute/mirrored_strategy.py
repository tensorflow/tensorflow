...
def _is_device_list_single_worker(devices):
    ...
    if any_local and not all_local:
        raise ValueError(
            "Local device should have only 'localhost' in the job "
    ...
    if num_workers == 1 and not all_local:
        if any(d.task is None for d in specs):
            raise ValueError(
                "Remote device string must have task specified."
                "E.g. 'task:0' in "
            ...
            for device_in_task in devices_in_task:
                ...
                for d in device_in_task:
                    ...
                    if d_spec.device_type == "GPU" and d_spec.device_index >= num_gpus:
                        raise ValueError(
                            "GPU `device_index` on a worker should be "
...
def all_devices():
    ...
    if tfconfig.cluster_spec().as_dict():
        devices = _cluster_spec_to_device_list(
            tfconfig.cluster_spec(), context.num_gpus()
        )
    return devices if devices else all_local_devices()


@tf_export(
    "distribute.MirroredStrategy", v1=[]
...
class MirroredExtended(distribute_lib.StrategyExtendedV1):
    ...
    def __init__(self, container_strategy, devices=None, cross_device_ops=None):
        ...
        if context.executing_eagerly():
            if devices and not _is_device_list_single_worker(devices):
                raise RuntimeError(
                    "In-graph multi-worker training with "
                ...
                if TFConfigClusterResolver().cluster_spec().as_dict():
                    ...
                    # path is supported.
                    logging.info(
                        "Initializing local devices since in-graph multi-worker "
        ...
        assert devices, (
            "Got an empty `devices` list and unable to recognize " "any local devices."
        )
    ...
    def _initialize_single_worker(self, devices):
        ...
        self._inferred_cross_device_ops = (
            None
            if self._cross_device_ops
            else (cross_device_ops_lib.select_cross_device_ops(devices))
        )
    ...
    def _initialize_multi_worker(self, devices):
        ...
        if len(workers) > 1:
            ...
            if (
                ...
                or self._cross_device_ops._num_between_graph_workers > 1
    ...
    def _input_workers_with_options(self, options=None):
        ...
        if (
            options.experimental_replication_mode
            == distribute_lib.InputReplicationMode.PER_REPLICA
        ):
            if options.experimental_place_dataset_on_device:
                self._input_workers_devices = tuple(
                    (device_util.canonicalize(d, d), (d,)) for d in self._devices
                )
            else:
                self._input_workers_devices = tuple(
                    (device_util.canonicalize("/device:CPU:0", d), (d,))
    ...
    def _make_input_fn_iterator(
        ...
        return input_lib_v1.InputFunctionIterator(
            input_fn, self._input_workers, input_contexts, self._container_strategy()
        )
    ...
    def _experimental_distribute_values_from_function(self, value_fn):
        ...
        for replica_id in range(self._num_replicas_in_sync):
            per_replica_values.append(
                value_fn(
                    distribute_lib.ValueContext(replica_id, self._num_replicas_in_sync)
                )
            )
    ...
    def _experimental_run_steps_on_iterator(
        ...
        for (
            ...
            ctx._last_step_outputs_reduce_ops.items()
        ):  # pylint: disable=protected-access
    ...
    def _gather_to_implementation(self, value, destinations, axis, options):
        if not isinstance(value, values.DistributedValues):
            ...
            return value
        return self._get_cross_device_ops(
            value
        )._gather(  # pylint: disable=protected-access
            value,
    ...
    def _reduce_to(self, reduce_op, value, destinations, options):
        if (
            distribute_utils.is_mirrored(value)
        ...
        def get_values(value):
            ...
            if (
                self._use_merge_call()
                and self._collective_ops_in_use
                and (
                    (
                        not cross_device_ops_lib._devices_match(value, destinations)
                        or any(  # pylint: disable=protected-access
                            "cpu" in d.lower()
    ...
    def _batch_reduce_to(self, reduce_op, value_destination_pairs, options):
        ...
        for value, _ in value_destination_pairs:
            if cross_device_ops is None:
                ...
                raise ValueError(
                    "Inputs to batch_reduce_to must be either all on "
    ...
    def _update(self, var, fn, args, kwargs, group):
        ...
        for i, v in enumerate(var.values):
            name = "update_%d" % i
            with ops.device(v.device), distribute_lib.UpdateContext(i), ops.name_scope(
                name
            ):
    ...
    def _replica_ctx_all_reduce(self, reduce_op, value, options=None):
        ...
        return self._get_cross_device_ops(
            value
        )._all_reduce(  # pylint: disable=protected-access
            reduce_op,
            value,
