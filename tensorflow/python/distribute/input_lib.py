...
class DistributedIteratorBase(
    collections_abc.Iterator, distribute_types.DistributedIteratorInterface
):
    ...
    def get_next_as_optional(self):
        ...
        if (
            self._cardinality == 0
            and not self._strategy.extended._in_multi_worker_mode()
        ):  # pylint: disable=protected-access
    ...
    def _reorder_replicas(self, replicas):
        assert len(self._replica_order) == len(replicas), (
...
class DistributedDatasetAndIteratorSpec(type_spec.TypeSpec):
    ...
    def sanity_check_type(self, other):
        ...
        if self._input_workers.serialize() != other._input_workers.serialize():
            raise ValueError(
                "_input_workers is not compatible with both %s and %s" % (self, other)
            )
...
class _IterableInput(
    collections_abc.Iterable, distribute_types.DistributedDatasetInterface
):
...
class DistributedDataset(_IterableInput, composite_tensor.CompositeTensor):
    ...
    def build(self, dataset_to_replace=None):
        ...
        if context.executing_eagerly():
            ...
            distribute_duration_ms = (
                time.time_ns() - distribute_start_time_ns
            ) // 1_000_000
            _distributed_dataset_initialization_time_milliseconds.get_cell(
                self._strategy.__class__.__name__, str(self._input_workers.num_workers)
            ).add(distribute_duration_ms)
    ...
    def auto_shard(self, num_shards, shard_ix):
        assert len(self._cloned_datasets) == len(self._input_workers.worker_devices), (
        ...
        def rebatch_fn(dataset, worker_index):
            try:
                ...
                def apply_rebatch():
                    ...
                    return dataset.rebatch(batch_sizes).prefetch(
...
class DistributedDatasetsFromFunction(_IterableInput, composite_tensor.CompositeTensor):
    ...
    def auto_shard(self, num_shards, shard_ix):
        assert len(self._datasets) == len(self._input_workers.worker_devices), (
                    ...
                    if produce_dummy:
                        ...
                        value_list.append(
                            tf_cond.cond(
                                ...
                                lambda: _dummy_tensor_fn(
                                    optional_list[i][j].element_spec
                                ),
                                strict=True,
                            )
                        )
...
def _create_iterators_per_worker(
    worker_datasets, input_workers, options=None, canonicalize_devices=False
):
...
def _always_wrap(strategy):
    ...
    return (
        ...
        or len(  # pylint: disable=protected-access
