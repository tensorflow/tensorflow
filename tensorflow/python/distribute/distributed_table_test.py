...
class DistributedTableTest(test.TestCase, parameterized.TestCase):
    ...
    def createStaticHashTable(
        self, init_source=None, vals=None, default_value=None, initializer=None
    ):
        if not initializer:
    ...
    def makeDatasetFromTensorWithoutUsingResource(self, input_context, tensor):
        ...
        dataset = (
            dataset_ops.DatasetV2.from_tensors(tensor)
            .repeat()
    ...
    @combinations.generate(source_combination)
    def testAccessingResourceHandleInDatasetFnWithMapFnDefinedInside(self, source):
    ...
    def testInModelAndCapture(self, source):
        ...
        deferred_captures = (
            model.use_table.get_concrete_function().graph.deferred_external_captures
        )
        ...
        func_captures = (
            distributed_model.use_table.get_concrete_function().graph.external_captures
        )
        ...
        deferred_captures = (
            distributed_model.use_table.get_concrete_function().graph.deferred_external_captures
        )
    ...
    def testDistributeTableSaveAndServe(self, load, source):
        ...
        loaded_func_captures_without_strategy = (
            loaded_without_strategy.use_table.get_concrete_function().graph.external_captures
        )
        loaded_func_deferred_captures_without_strategy = (
            loaded_without_strategy.use_table.get_concrete_function().graph.deferred_external_captures
        )
    ...
    def testDistributeTableSaveAndLoadUnderStrategy(self, load, source):
        ...
        loaded_func_deferred_captures = (
            loaded.use_table.get_concrete_function().graph.deferred_external_captures
        )
        ...
        self.assertLen(
            [
                t
                for t in loaded.use_table.get_concrete_function().captured_inputs
