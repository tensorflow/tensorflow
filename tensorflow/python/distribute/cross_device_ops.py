...
def check_destinations(destinations):
    ...
    if isinstance(
        destinations, (resource_variable_ops.BaseResourceVariable, ops.Tensor)
    ):
...
def validate_destinations(destinations):
    ...
    if not isinstance(
        ...
        raise ValueError(
            "destinations must be one of a `DistributedValues` object,"
...
def reduce_non_distributed_value(
    reduce_op, value, destinations, num_replicas_in_graph, canonicalize_devices=True
):
    """Reduce a non-DistributedValue `value` to `destinations`."""
    if isinstance(value, value_lib.DistributedValues):
        raise ValueError(
            "You are passing a `DistributedValues` to "
...
def _make_tensor_into_per_replica(input_tensor):
    ...
    if isinstance(input_tensor, (tuple, list)):
        raise ValueError(
            "Cannot convert `input_tensor` to a `PerReplica` object, "
...
def _devices_match(left, right, canonicalize_devices=True):
    return left is right or set(get_devices_from(left, canonicalize_devices)) == set(
...
def _all_devices_match(value_destination_pairs, canonicalize_devices=True):
    if not all(
        _devices_match(v, d, canonicalize_devices) for v, d in value_destination_pairs
    ):
        return False
    if not all(
...
def simple_broadcast(
    value, destinations, always_mirrored=False, canonicalize_devices=True
):
...
class CrossDeviceOps(object):
    ...
    * `tf.distribute.HierarchicalCopyAllReduce`
    ...
    def reduce(self, reduce_op, per_replica_value, destinations, options=None):
        ...
        if (
            self._num_between_graph_workers == 1
            and len(per_replica_value.values) == 1
            and _devices_match(
        ...
        if options is None:
            options = collective_util.Options()
        return self.reduce_implementation(
    ...
    def _gather(self, per_replica_value, destinations, axis, options=None):
        ...
        if isinstance(per_replica_value, indexed_slices.IndexedSlices):
            raise NotImplementedError(
                "gather/all_gather does not support " "IndexedSlices"
            )
        if options is None:
            options = collective_util.Options()
        ...
        return self._gather_implementation(
    ...
    def batch_reduce(self, reduce_op, value_destination_pairs, options=None):
        ...
        if (
            self._num_between_graph_workers == 1
            and _all_devices_match(value_destination_pairs, self._canonicalize_devices)
            and len(value_destination_pairs[0][0].values) == 1
        ):
            return [
        ...
        if options is None:
            options = collective_util.Options()
        return self.batch_reduce_implementation(
    ...
    def reduce_implementation(
        self, reduce_op, per_replica_value, destinations, options
    ):
...
class ReductionToOneDevice(CrossDeviceOps):
    ...
    def reduce_implementation(
        ...
        logging.log_first_n(
            logging.INFO,
            "Reduce to %s then broadcast to %r." % (reduce_to_device, devices),
            10,
        )
        reduced = _simple_reduce(
            per_replica_value, reduce_to_device, self.accumulation_fn, reduce_op
        )
    ...
    def _gather_implementation(self, per_replica_value, destinations, axis, options):
        ...
        logging.log_first_n(
            logging.INFO,
            "Gather to %s then broadcast to %r." % (reduce_to_device, devices),
            10,
        )
    ...
    def batch_reduce_implementation(self, reduce_op, value_destination_pairs, options):
        return [
            self.reduce_implementation(reduce_op, t, destinations=v, options=options)
...
def _ungroup_and_make_mirrored(
    grouped_reduced, destinations, reduce_op, num_between_graph_workers=1
):
    ...
    def unpack(self, summed_device_grad_packs):
        ...
        for (
            ...
            with ops.colocate_with(summed_device_grad_packs[0][0]):
                ...
                summed_device_grads = [
                    (g, v)
                    for g, (_, v) in zip(grads_with_shapes, device_grads_and_vars)
                ]
...
class AllReduceCrossDeviceOps(CrossDeviceOps):
    ...
    def reduce_implementation(
        ...
        if _devices_match(per_replica_value, destinations) and not any(
            "cpu" in d.lower() for d in get_devices_from(destinations)
        ):
            return self._batch_all_reduce(reduce_op, [per_replica_value])[0]
        else:
            return self._simple_cross_replica_ops.reduce(
    ...
    def batch_reduce_implementation(self, reduce_op, value_destination_pairs, options):
        if _all_devices_match(value_destination_pairs):
            return self._batch_all_reduce(
                reduce_op, [v[0] for v in value_destination_pairs]
            )
        else:
            return [
    ...
    def _do_batch_all_reduce(self, reduce_op, dense_values):
        ...
        if self._all_reduce_alg == "nccl":
            ...
            # order.
            reduced = cross_device_utils.aggregate_gradients_using_hierarchical_copy(
    ...
    def _gather_implementation(self, per_replica_value, destinations, axis, options):
        logging.warning(
            "gather/all_gather with NCCL or HierarchicalCopy is not "
            "supported. Falling back to gather on one device and "
            "then broadcast. We're working on a more efficient "
            "implementation."
        )
        return ReductionToOneDevice()._gather(
            per_replica_value,
            destinations,
            axis,  # pylint: disable=protected-access
            options,
        )
...
class HierarchicalCopyAllReduce(AllReduceCrossDeviceOps):
    ...
    def __init__(self, num_packs=1):
        ...
        super(HierarchicalCopyAllReduce, self).__init__(
            all_reduce_alg="hierarchical_copy", num_packs=num_packs
        )
...
class CollectiveAllReduce(CrossDeviceOps):
    ...
    def _all_reduce(self, reduce_op, value, replica_id, options):
        ...
        if dense_values:
            ...
            packs = cross_device_utils.group_by_size(
                dense_values, options.bytes_per_pack
            )
        ...
        if sparse_values:
            if not context.executing_eagerly() and replica_id == 0:
                logging.info(
                    ...
                    "%d, group_size = %d, implementation = %s",
                    len(sparse_values),
                    len(self._launchers),
                    self._group_size,
                    options.implementation,
            ...
            def thread_fn(device_id):
                with context.eager_mode():
                    return self._all_reduce(
                        reduce_op, values_by_device[device_id], device_id, options
                    )
    ...
    def reduce_implementation(
        ...
        all_reduced = self._all_reduce_per_replica_values(
            reduce_op, [per_replica_value], options
        )[0]
        devices = get_devices_from(destinations, self._canonicalize_devices)
            ...
            for d in devices:
                with ops.device(d):
                    for v in all_reduced.values:
                        ...
                        index.append(
                            array_ops.identity(all_reduced._primary)
                        )  # pylint: disable=protected-access
        return distribute_utils.regroup(index, wrap_class=value_lib.Mirrored)

    def batch_reduce_implementation(self, reduce_op, value_destination_pairs, options):
        values_util.mark_as_unsaveable()
        all_devices_match = _all_devices_match(
            value_destination_pairs, self._canonicalize_devices
        )
                ...
                with ops.device(d):
                    for v in all_gathered.values:
                        if v.device == d:
                            ...
                            index.append(
                                array_ops.identity(all_gathered._primary)
                            )  # pylint: disable=protected-access
        return distribute_utils.regroup(index, wrap_class=value_lib.Mirrored)
    ...
    def _batch_all_gather(self, per_replica_values, axis, options):
        ...
        logging.log_first_n(
            logging.INFO,
            "Collective batch_all_gather: %d all-gathers, "
