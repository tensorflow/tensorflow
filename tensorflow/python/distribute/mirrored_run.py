...
class _MirroredReplicaThread(threading.Thread):
    ...
    def __init__(
        ...
        self.replica_id_in_sync_group = dist.extended._get_replica_id_in_sync_group(
            replica_id
    ...
    def run(self):
        ...
        try:
            ...
            if (
                self.caching_scope_entered is not None
                and self.caching_scope_exited is not None
            ):
                distribute_utils.caching_scope_local.new_cache_scope_count = (
                    self.caching_scope_entered
                )
                distribute_utils.caching_scope_local.cache_scope_exited_count = (
                    self.caching_scope_exited
                )
            # TODO(josh11b): Use current logical device instead of 0 here.
            with (
                self.coord.stop_on_exception(),
                _enter_graph(self._init_graph, self._init_in_eager),
                _enter_graph(self.graph, self.in_eager, self._variable_creator_stack),
                context.device_policy(self.context_device_policy),
                _MirroredReplicaContext(
                    self.distribution, self.replica_id_in_sync_group
                ),
