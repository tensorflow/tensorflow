...
def single_loss_example(
    optimizer_fn, distribution, use_bias=False, iterations_per_step=1
):
...
def batchnorm_example(
    ...
    def dataset_fn():
        ...
        return dataset_ops.Dataset.from_tensor_slices(
            [
                [[float(x * 8 + y + z * 100) for y in range(8)] for x in range(16)]
        ...
        def loss_fn():
            ...
            with ops.control_dependencies(
                ...
                loss = math_ops.reduce_mean(
                    math_ops.reduce_sum(layer(y)) - constant_op.constant(1.0)
                )
