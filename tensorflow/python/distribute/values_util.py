...
def write_object_proto(var, proto, options):
    ...
    """
    if (
        options.experimental_variable_policy._expand_distributed_variables()  # pylint: disable=protected-access
    ):
...
def get_on_write_restore_ops(var, tensor):
    ...
    if packed_var is not None:
        return control_flow_ops.group(
            tuple(assign_on_device(d, packed_var, tensor) for d in packed_var.devices)
        )
    return control_flow_ops.group(
        tuple(assign_on_device(v.device, v, tensor) for v in var.values)
    )
...
def assign_on_each_device(var, assign_func, value, read_value):
    ...
    if var._packed_variable is not None:  # pylint: disable=protected-access
        update = control_flow_ops.group(
            tuple(assign_func(d, var._packed_variable, value) for d in var._devices)
        )  # pylint: disable=protected-access
    else:
        update = control_flow_ops.group(
            tuple(assign_func(v.device, v, value) for v in var._values)
        )  # pylint: disable=protected-access
    if not read_value:
        return update
        ...
        if ds_context.in_cross_replica_context():
            ...
            if var.aggregation == vs.VariableAggregation.SUM:
                ...
                tensor = math_ops.cast(
                    tensor / strategy.num_replicas_in_sync, var.dtype
                )
...
def scatter_sub(var, sparse_delta, use_locking=False, name=None):
    ...
    return var._update(  # pylint: disable=protected-access
        update_fn=scatter_sub_fn, value=sparse_delta, use_locking=use_locking, name=name
    )
...
def scatter_add(var, sparse_delta, use_locking=False, name=None):
    ...
    return var._update(  # pylint: disable=protected-access
        update_fn=scatter_add_fn, value=sparse_delta, use_locking=use_locking, name=name
    )
...
def scatter_mul(var, sparse_delta, use_locking=False, name=None):
    ...
    return var._update(  # pylint: disable=protected-access
        update_fn=scatter_mul_fn, value=sparse_delta, use_locking=use_locking, name=name
    )
...
def scatter_div(var, sparse_delta, use_locking=False, name=None):
    ...
    return var._update(  # pylint: disable=protected-access
        update_fn=scatter_div_fn, value=sparse_delta, use_locking=use_locking, name=name
    )
...
def scatter_min(var, sparse_delta, use_locking=False, name=None):
    ...
    return var._update(  # pylint: disable=protected-access
        update_fn=scatter_min_fn, value=sparse_delta, use_locking=use_locking, name=name
    )
...
def scatter_max(var, sparse_delta, use_locking=False, name=None):
    ...
    return var._update(  # pylint: disable=protected-access
        update_fn=scatter_max_fn, value=sparse_delta, use_locking=use_locking, name=name
    )
...
def apply_aggregation(strategy, value, aggregation, destinations):
    if aggregation == vs.VariableAggregation.ONLY_FIRST_REPLICA:
        return strategy.extended.broadcast_to(
            strategy.experimental_local_results(value)[0], destinations=destinations
        )
...
scatter_error_msg = (
    "{op_name} is only supported for mirrored "
