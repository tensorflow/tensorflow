...
class AggregatingVariable(resource_variable_ops.BaseResourceVariable, core.Tensor):
    ...
    def _export_to_saved_model_graph(self, object_map, tensor_map, options, **kwargs):
        ...
        resource_list = self._v._export_to_saved_model_graph(
            object_map,
            tensor_map,  # pylint:disable=protected-access
            options,
            **kwargs,
        )
    ...
    def _copy_trackable_to_cpu(self, object_map):
        ...
        if self not in object_map:
            ...
            object_map[self] = AggregatingVariable(
                self._distribute_strategy, object_map[self._v], self._aggregation
            )
...
class PerWorkerVariable(resource_variable_ops.BaseResourceVariable):
    ...
    def _maybe_create_per_worker_vars(self):
        ...
        if not self._per_worker_vars:
            self._per_worker_vars = self._coordinator._create_per_worker_variables(
                self._var_creator
...
class DistributedTable(lookup_ops.StaticHashTable):
    ...
    def __init__(self, strategy, wrapped_creator):
        distribute_lib.distribution_strategy_input_api_counter.get_cell(
            self.__class__.__name__, "PSSDistributedLookupTable"
        ).increase_by(1)
        ...
        with self._distributed_table_creation_lock:
            if not self._distributed_table:
                ...
                self._distributed_table = (
                    self._coordinator._create_per_worker_resources(create_copy)
                )  # pylint: disable=protected-access
    ...
    def resource_handle(self):
        if context.executing_eagerly() or save_context.in_save_context():
            ...
            return ops.get_default_graph().capture_call_time_value(
                closure, spec, default_value=self._coordinator_instance.resource_handle
            )
...
class RestoredDistributedTable(DistributedTable):
        ...
        def closure():
            ...
            if dispatch_context:
                ...
                if local_resource_restore_context:
                    remote_value = (
                        local_resource_restore_context.instance.resource_handle
                    )
    ...
    def __setattr__(self, name, value):
        if name in TRACKABLE_RESOURCE_METHODS:
            ...
            if all(
                method in self._restored_function
                        ...
                        with with_local_resource_restore_context(new_table):
                            ...
                            if not context.executing_eagerly():
                                ops.add_to_collection(
                                    ops.GraphKeys.TABLE_INITIALIZERS, init_op
                                )
