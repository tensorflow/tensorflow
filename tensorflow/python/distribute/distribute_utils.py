...
def regroup(values, wrap_class=values_lib.PerReplica, always_wrap=False):
    ...
    if isinstance(v0, tuple):
        for v in values[1:]:
            ...
            assert len(v) == len(v0), (
                "Values to regroup had different lengths: "
    ...
    if (
        not isinstance(v0, resource_variable_ops._UnreadVariable)  # pylint: disable=protected-access
        and value_container(v0) is not v0
    ):
...
def value_container(val):
    ...
    if not isinstance(val, values_lib.DistributedVariable):
        if hasattr(val, "_distributed_container"):
            ...
            isinstance(val, composite_tensor.CompositeTensor)
            and hasattr(val, "handle")
            and hasattr(val.handle, "_distributed_container")
        ):
