...
bool Initialize() {
  ...
  if (!RegisterCustomFloatCast<float8_e4m3b11, bfloat16>()) {
    return false;
  }
  if (!RegisterCustomFloatCast<bfloat16, float8_e4m3b11>()) {
    return false;
  }
  // Safe casts from fp8 to bf16 and f16:
  if (PyArray_RegisterCanCast(&TypeDescriptor<float8_e4m3b11>::npy_descr,
                              TypeDescriptor<bfloat16>::Dtype(),
                              NPY_NOSCALAR) < 0) {
    return false;
  }
  if (PyArray_RegisterCanCast(&TypeDescriptor<float8_e4m3b11>::npy_descr,
                              NPY_HALF, NPY_NOSCALAR) < 0) {
    return false;
  }
  return true;
}
