...
def dispatch_for_api(api, *signatures):
    ...
    * `@dispatch_for_api(tf.add, {'x': Union[MaskedTensor, Tensor], 'y':
    ...
    signature_checkers = [
        _make_signature_checker(api_signature, signature) for signature in signatures
    ]
    ...
    def decorator(dispatch_target):
        ...
        if not callable(dispatch_target):
            raise TypeError(
                f"Expected dispatch_target to be callable; got {dispatch_target!r}"
            )
...
def type_based_dispatch_signatures_for(cls):
    ...
    def contains_cls(x):
        ...
        if isinstance(x, dict):
            ...
            return True
        elif type_annotations.is_generic_list(x) or type_annotations.is_generic_union(
            x
        ):
...
def _add_name_scope_wrapper(func, api_signature):
    ...
    wrapped_func.__signature__ = func_signature.replace(
        parameters=(
            list(func_signature.parameters.values())
...
def unregister_dispatch_for(dispatch_target):
    ...
    elementwise_keys_to_delete = [
        key
        for (key, handler) in _ELEMENTWISE_API_HANDLERS.items()
    ...
    if not found:
        raise ValueError(
            f"Function {dispatch_target} was not registered using "
...
def add_type_based_api_dispatcher(target):
    ...
    setattr(
        ...
        _api_dispatcher.PythonAPIDispatcher(
            unwrapped.__name__, target_argspec.args, target_argspec.defaults
        ),
    )
...
def _check_signature(api_signature, func):
    ...
    func_argspec = tf_inspect.getargspec(func)
    if (
        func_argspec.varargs is not None
        and func_argspec.keywords is not None
    ...
    if ok:
        for param_1, param_2 in zip(
            api_signature.parameters.values(), func_signature.parameters.values()
        ):
...
def _make_signature_checker(api_signature, signature):
    ...
    for param_name, param_type in signature.items():
        ...
        if param is None:
            raise ValueError(
                f"signature includes annotation for unknown parameter {param_name!r}."
            )
        if param.kind not in (
            tf_inspect.Parameter.POSITIONAL_ONLY,
    ...
    if type_annotations.is_generic_union(annotation):
        ...
        if len(simple_types) > 1:
            ...
            options = [_is_instance_checker_cache[simple_types]] + [
                make_type_checker(t) for t in type_args if not isinstance(t, type)
            ]
...
def _signature_from_annotations(func):
    ...
    if not signature:
        raise ValueError(
            "The dispatch_for_api decorator must be called with at "
...
def dispatch_for_unary_elementwise_apis(x_type):
    ...
    def decorator(handler):
        if (x_type,) in _ELEMENTWISE_API_HANDLERS:
            raise ValueError(
                "A unary elementwise dispatch handler "
...
def dispatch_for_binary_elementwise_apis(x_type, y_type):
    ...
    def decorator(handler):
        if (x_type, y_type) in _ELEMENTWISE_API_HANDLERS:
            raise ValueError(
                "A binary elementwise dispatch handler "
...
def dispatch_for_binary_elementwise_assert_apis(x_type, y_type):
    ...
    def decorator(handler):
        ...
        if api_handler_key in _ELEMENTWISE_API_HANDLERS:
            raise ValueError(
                "A binary elementwise assert dispatch handler "
...
def _add_dispatch_for_unary_elementwise_api(api, x_type, elementwise_api_handler):
    ...
    need_to_bind_api_args = (
        len(api_signature.parameters) > 2 or "name" not in api_signature.parameters
    )
...
def _add_dispatch_for_binary_elementwise_api(
    api, x_type, y_type, elementwise_api_handler
):
    ...
    need_to_bind_api_args = (
        len(api_signature.parameters) > 3 or "name" not in api_signature.parameters
    )
...
def update_docstrings_with_api_lists():
    ...
    _update_docstring_with_api_list(
        dispatch_for_binary_elementwise_assert_apis, _BINARY_ELEMENTWISE_ASSERT_APIS
    )
...
def add_dispatch_support(target=None, iterable_parameters=None):
    ...
    if not (
        iterable_parameters is None
        or (
            isinstance(iterable_parameters, (list, tuple))
            and all(isinstance(p, str) for p in iterable_parameters)
        )
    ):
