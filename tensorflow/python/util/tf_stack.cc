 ...
 public:
  ...
  void UpdateTo(const py::tuple& source_map) {
    ...
    for (const auto& item : source_map) {
      ...
      updated.emplace(SourceLoc{k_filename, k_lineno},
...
class PyBindGraphDebugInfoBuilder {
 public:
  absl::Status AppendGraphDebugInfo(std::string fn_name, py::bytes debug_info) {
    // Convert Python-owned data before acquiring the native mutex.
...
class StackTraceWrapper : public AbstractStackTrace {
 public:
  StackTraceWrapper(const std::shared_ptr<StackTrace>& captured,
                    ...
                    const std::shared_ptr<PyBindFileSet::State>& filter,
                    int stacklevel)
      : captured_(captured),
 ...
 private:
  ...
  std::vector<StackFrame> ToUncachedFrames(const SourceMap& source_map,
                                           const StringSet& filter) const {
...
PYBIND11_MODULE(_tf_stack, m, pybind11::mod_gil_not_used()) {
  ...
  m.def(
      ...
      [](const PyBindSourceMap& source_map, const PyBindFileSet& file_set,
        ...
        return StackTraceWrapper::ExtractStack(source_map.GetState(),
                                               file_set.GetState(), stacklevel);
      },
