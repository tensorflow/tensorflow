...
//
// We store the retrieved stack trace with the *uninstantiated* graph object,
// and we "fake" the Python list interface: on demand, we convert the stack
// trace to C++ datastructures, which we then expose through Python.
//
// Finally, inside the FunctionLibraryDefinition we store the mapping from
// function names to corresponding stack trace maps.
...
class StackTraceWrapper : public AbstractStackTrace {
 public:
  StackTraceWrapper(StackTrace&& captured, StackTraceMap&& stack_trace_map,
                    StackTraceFilter&& stack_trace_filter)
      : captured_(std::move(captured)),
        stack_trace_mapper_(std::move(stack_trace_map)),
        stack_trace_filter_(std::move(stack_trace_filter)) {}
  ...
  static StackTraceWrapper ExtractStack(const py::object& limit,
    ...
    for (const std::pair<py::handle, py::handle>& p : source_map) {
    ...
    for (const py::handle& h : filtered_filenames) {
      f.emplace(py::cast<py::str>(h));
    }

    return StackTraceWrapper{StackTrace::Capture(casted_limit), std::move(m),
                             std::move(f)};
  }
 ...
 private:
  ...
  StackTraceMap stack_trace_mapper_;
...
PYBIND11_MODULE(_tf_stack, m) {
  ...
  m.def(
      ...
      [](const py::object& limit, const py::list& mappers,
         const py::list& filters, TF_Graph* graph,
         const char* node_name) -> const StackTraceWrapper& {
        mutex_lock l(graph->mu);
        auto it = graph->graph.GetMutableStackTraces().emplace(
            node_name,
            absl::make_unique<StackTraceWrapper>(
                StackTraceWrapper::ExtractStack(limit, mappers, filters)));
        DCHECK(it.second) << "Node names in a graph should be unique";
        return *(static_cast<StackTraceWrapper*>(it.first->second.get()));
      },
