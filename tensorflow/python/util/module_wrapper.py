...
compat_v1_usage_gauge = monitoring.BoolGauge(
    "/tensorflow/api/compat/v1", "compat.v1 usage"
)
...
class TFModuleWrapper(FastModuleType):
    ...
    def _tfmw_import_module(self, name):
        ...
        if (
            self._tfmw_is_compat_v1
            and name != "app"
            and not TFModuleWrapper.compat_v1_usage_recorded
        ):
    ...
    def _getattribute(self, name):
        ...
        if (
            name.startswith("__")
            or name.startswith("_tfmw_")
            or name.startswith("_fastdict_")
        ):
            func__fastdict_insert(name, attr)
            return attr
