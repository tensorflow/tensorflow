...
def _packed_nest_with_indices(structure, flat, index, is_nested_fn, sequence_fn=None):
...
def flatten_up_to(shallow_tree, input_tree, check_types=True, expand_composites=False):
    ...
    return [v for _, v in _yield_flat_up_to(shallow_tree, input_tree, is_nested_fn)]
...
def map_structure_with_tuple_paths_up_to(shallow_tree, func, *inputs, **kwargs):
    ...
    *inputs.
    ...
    flat_path_gen = (
        path for path, _ in _yield_flat_up_to(shallow_tree, inputs[0], is_nested_fn)
    )
