...
def _numeric_column(
    ...
    if not (dtype.is_integer or dtype.is_floating):
        raise ValueError(
            "dtype must be convertible to float. dtype: {}, key: {}".format(dtype, key)
        )
...
def _bucketized_column(source_column, boundaries):
    ...
    if len(source_column.shape) > 1:
        raise ValueError(
            "source_column must be one-dimensional column. Given: {}".format(
                source_column
            )
        )
...
def _categorical_column_with_hash_bucket(key, hash_bucket_size, dtype=dtypes.string):
    ...
    if hash_bucket_size < 1:
        raise ValueError(
            "hash_bucket_size must be at least 1. hash_bucket_size: {}, key: {}".format(
...
def _crossed_column(keys, hash_bucket_size, hash_key=None):
    ...
    * SparseTensor referred by second key:
    ...
    if not hash_bucket_size or hash_bucket_size < 1:
        raise ValueError(
            "hash_bucket_size must be > 1. hash_bucket_size: {}".format(
                hash_bucket_size
            )
        )
...
class _LazyBuilder(object):
    ...
    def get(self, key):
        ...
        if not isinstance(key, _FeatureColumn):
            raise TypeError(
                '"key" must be either a "str" or "_FeatureColumn". Provided: {}'.format(
                    key
                )
            )
...
def _check_shape(shape, key):
    ...
    for dimension in shape:
        if not isinstance(dimension, six.integer_types):
            raise TypeError(
                "shape dimensions must be integer. shape: {}, key: {}".format(
                    shape, key
                )
            )
        if dimension < 1:
            raise ValueError(
                "shape dimensions must be greater than 0. shape: {}, key: {}".format(
                    shape, key
                )
            )
    return shape
