...
def make_parse_example_spec_v2(feature_columns):
    ...
    for column in feature_columns:
        if not isinstance(column, fc_types.FeatureColumn):
            raise ValueError(
                "All feature_columns must be FeatureColumn instances. Given: {}".format(
                    column
                )
            )
...
def crossed_column(keys, hash_bucket_size, hash_key=None):
    ...
    * SparseTensor referred by second key:
    ...
    for key in keys:
        ...
        if isinstance(key, (HashedCategoricalColumn, fc_old._HashedCategoricalColumn)):  # pylint: disable=protected-access
            raise ValueError(
...
class FeatureTransformationCache(object):
    ...
    def get(self, key, state_manager, training=None):
        ...
        if not isinstance(key, fc_types.FeatureColumn):
            raise TypeError(
                '"key" must be either a "str" or "FeatureColumn". Provided: {}'.format(
                    key
                )
            )
