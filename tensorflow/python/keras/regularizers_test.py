...
class KerasRegularizersTest(keras_parameterized.TestCase,
  ...
  def test_activity_regularization(self, regularizer):
    ...
    self.assertEqual(len(model.losses),
                     0 if context.executing_eagerly() else 1)
    model.fit(x_train, y_train, batch_size=10,
              epochs=1, verbose=0)

  def test_zero_regularization(self):
    inputs = keras.backend.ones(shape=(10, 10))
    layer = keras.layers.Dense(3, kernel_regularizer=regularizers.l2(0))
    layer(inputs)
  ...
  def test_regularization_shared_layer(self, regularizer):
    ...
    model.compile(loss='categorical_crossentropy',
                  optimizer='sgd',
                  run_eagerly=testing_utils.should_run_eagerly())
    self.assertEqual(len(model.losses), 4 if context.executing_eagerly() else 6)
  ...
  def test_regularization_shared_layer_in_different_models(self, regularizer):
    ...
    model.compile(loss='categorical_crossentropy',
                  optimizer='sgd',
                  run_eagerly=testing_utils.should_run_eagerly())
    self.assertEqual(len(model.losses), 7 if context.executing_eagerly() else 9)
