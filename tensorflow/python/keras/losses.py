    ...
    return math_ops.reduce_all([
        math_ops.equal(
            math_ops.reduce_variance(
                math_ops.cast(row_lens, K.floatx())),
  ...
  def _wrapper(inputs):
    ...
    if isinstance(y_pred, ragged_tensor.RaggedTensor):
      return control_flow_ops.cond(
          ...
          lambda: loss_fn(*_convert_to_dense(inputs)),
          lambda: loss_fn(*inputs))
...
@dispatch.dispatch_for_types(
    categorical_crossentropy, ragged_tensor.RaggedTensor)
def _ragged_tensor_categorical_crossentropy(y_true,
  ...
  """
  fn = functools.partial(categorical_crossentropy, from_logits=from_logits,
                         label_smoothing=label_smoothing)
