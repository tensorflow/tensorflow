...
class MergeLayersTest(keras_parameterized.TestCase):
  ...
  def test_merge_add(self):
    ...
    self.assertEqual(add_layer.compute_mask([i1, i2, i3],
                                            [None, None, None]), None)
    self.assertTrue(np.all(K.eval(
        add_layer.compute_mask([i1, i2],
  ...
  def test_merge_subtract(self):
    ...
    self.assertTrue(np.all(K.eval(
        subtract_layer.compute_mask([i1, i2],
                                    [K.variable(x1), K.variable(x2)]))))
  ...
  def test_merge_concatenate(self):
    ...
    self.assertTrue(np.all(K.eval(
        concat_layer.compute_mask([i1, i2],
                                  [K.variable(x1), K.variable(x2)]))))
