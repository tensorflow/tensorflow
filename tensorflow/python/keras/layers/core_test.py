...
class DropoutLayersTest(keras_parameterized.TestCase):
    ...
    def test_spatial_dropout_1d(self):
        testing_utils.layer_test(
            keras.layers.SpatialDropout1D, kwargs={"rate": 0.5}, input_shape=(2, 3, 4)
        )
...
class TestStatefulLambda(keras_parameterized.TestCase):
    ...
    def test_lambda_with_variable_in_model(self):
        v = variables.Variable(1.0, trainable=True)
        ...
        self.assertAllClose(keras.backend.get_value(model.trainable_weights[0]), 2.0)

    @keras_parameterized.run_all_keras_modes
    @keras_parameterized.run_with_all_model_types
    def test_creation_inside_lambda(self):
        def lambda_fn(x):
            scale = variables.Variable(1.0, trainable=True, name="scale")
            shift = variables.Variable(1.0, trainable=True, name="shift")
...
class CoreLayersTest(keras_parameterized.TestCase):
    def test_masking(self):
        testing_utils.layer_test(keras.layers.Masking, kwargs={}, input_shape=(3, 2, 3))
    ...
    def test_activation(self):
        ...
        testing_utils.layer_test(
            keras.layers.Activation, kwargs={"activation": "relu"}, input_shape=(3, 2)
        )
    ...
    def test_reshape(self):
        testing_utils.layer_test(
            keras.layers.Reshape, kwargs={"target_shape": (8, 1)}, input_shape=(3, 2, 4)
        )
    ...
    def test_permute_errors_on_invalid_starting_dims_index(self):
        with self.assertRaisesRegex(ValueError, r"Invalid permutation .*dims.*"):
            testing_utils.layer_test(
                keras.layers.Permute, kwargs={"dims": (0, 1, 2)}, input_shape=(3, 2, 4)
            )

    def test_permute_errors_on_invalid_set_of_dims_indices(self):
        with self.assertRaisesRegex(ValueError, r"Invalid permutation .*dims.*"):
            testing_utils.layer_test(
                keras.layers.Permute, kwargs={"dims": (1, 4, 2)}, input_shape=(3, 2, 4)
            )

    def test_flatten(self):
        testing_utils.layer_test(keras.layers.Flatten, kwargs={}, input_shape=(3, 2, 4))
        ...
        target_outputs = np.reshape(np.transpose(inputs, (0, 2, 3, 1)), (-1, 5 * 5 * 3))
