...
class ConvRNN(RNN):
  ...
  Input shape:
    (3 + `rank`)D tensor with shape:
    `(samples, timesteps, channels, img_dimensions...)`
    if data_format='channels_first' or shape:
    `(samples, timesteps, img_dimensions..., channels)`
    if data_format='channels_last'.

  Output shape:
    - If `return_state`: a list of tensors. The first tensor is
      the output. The remaining tensors are the last states,
      each (2 + `rank`)D tensor with shape:
      `(samples, filters, new_img_dimensions...)`
      if data_format='channels_first'
      or shape:
      `(samples, new_img_dimensions..., filters)`
      if data_format='channels_last'.
      img_dimension values might have changed due to padding.
    - If `return_sequences`: (3 + `rank`)D tensor with shape:
      `(samples, timesteps, filters, new_img_dimensions...)`
      if data_format='channels_first'
      or shape:
      `(samples, timesteps, new_img_dimensions..., filters)`
      if data_format='channels_last'.
    - Else, (2 + `rank`)D tensor with shape:
      `(samples, filters, new_img_dimensions...)`
      if data_format='channels_first'
      or shape:
      `(samples, new_img_dimensions..., filters)`
      ...
      - Specify a fixed batch size for your model, by passing
          ...
          - If functional model with 1 or more Input layers:
            ...
            *including the batch size*.
            ...
            e.g. `(32, 10, 100, 100, 32)`. for rank 2 convolution
            Note that the image dimensions should be specified
            too.
  ...
  def __init__(self,
    ...
    super(ConvRNN, self).__init__(cell,
                                  return_sequences,
                                  return_state,
                                  go_backwards,
                                  stateful,
  ...
  def compute_output_shape(self, input_shape):
    ...
    norm_img_dims = tuple([
        conv_utils.conv_output_length(img_dims[idx],
    ...
    if self.return_state:
      ...
      if cell.data_format == 'channels_first':
        output_shape += [(input_shape[0], cell.filters) +
                          norm_img_dims for _ in range(2)]
      elif cell.data_format == 'channels_last':
        output_shape += [(input_shape[0],) + norm_img_dims +
                         (cell.filters,) for _ in range(2)]
    return output_shape
  ...
  def build(self, input_shape):
    ...
    if self.state_spec is not None:
      ...
      if self.cell.data_format == 'channels_first':
        self.state_spec = [InputSpec(shape=(None, dim) + img_dims)
                           for dim in state_size]
      elif self.cell.data_format == 'channels_last':
        self.state_spec = [InputSpec(shape=(None,) + img_dims + (dim,))
                           for dim in state_size]
    if self.stateful:
      self.reset_states()
    self.built = True
...
class ConvLSTMCell(DropoutRNNCellMixin, Layer):
  ...
  def call(self, inputs, states, training=None):
    ...
    (kernel_i, kernel_f,
     kernel_c, kernel_o) = array_ops.split(self.kernel, 4, axis=self.rank+1)
    (recurrent_kernel_i,
     recurrent_kernel_f,
     recurrent_kernel_c,
     recurrent_kernel_o) = array_ops.split(self.recurrent_kernel, 4,
                                           axis=self.rank+1)
...
class ConvLSTM(ConvRNN):
  ...
  def __init__(self,
    ...
    super(ConvLSTM, self).__init__(rank,
                                   cell,
  ...
  def call(self, inputs, mask=None, training=None, initial_state=None):
    return super(ConvLSTM, self).call(inputs,
                                      mask=mask,
                                      training=training,
...
class ConvLSTM1D(ConvLSTM):
  ...
  Output shape:
    ...
    - If `return_sequences`: 4D tensor with shape:
      `(samples, timesteps, filters, new_rows)`
      if data_format='channels_first'
      or shape:
...
class ConvLSTM3D(ConvLSTM):
  ...
  Input shape:
    - If data_format='channels_first'
        ...
        `(samples, time, channels, rows, cols, depth)`
    - If data_format='channels_last'
        5D tensor with shape:
        `(samples, time, rows, cols, depth, channels)`

  Output shape:
    - If `return_state`: a list of tensors. The first tensor is
      the output. The remaining tensors are the last states,
      each 5D tensor with shape:
      `(samples, filters, new_rows, new_cols, new_depth)`
      if data_format='channels_first'
      or shape:
      `(samples, new_rows, new_cols, new_depth, filters)`
      if data_format='channels_last'.
      `rows`, `cols`, and `depth` values might have changed due to padding.
    - If `return_sequences`: 6D tensor with shape:
      `(samples, timesteps, filters, new_rows, new_cols, new_depth)`
      if data_format='channels_first'
      or shape:
      `(samples, timesteps, new_rows, new_cols, new_depth, filters)`
      if data_format='channels_last'.
