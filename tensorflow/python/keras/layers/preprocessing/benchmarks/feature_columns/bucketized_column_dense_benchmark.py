...
from tensorflow.python.keras.layers.preprocessing.benchmarks.feature_columns import feature_column_benchmark as fc_bm
...
def embedding_varlen(batch_size, max_length):
  ...
  data = fc_bm.create_data(
      max_length, batch_size * NUM_REPEATS, 100000, dtype=int)
  ...
  sm = fc_bm.TestStateManager()
  fc.create_state(sm)

  # Wrap the FC implementation in a tf.function for a fair comparison
  @tf_function()
  def fc_fn(tensors):
    fc.transform_feature(fcv2.FeatureTransformationCache(tensors), sm)

  # Benchmark runs
  keras_data = {"data": data.to_tensor(default_value="")}
  k_avg_time = fc_bm.run_keras(keras_data, model, batch_size, NUM_REPEATS)

  fc_data = {"data": data.to_tensor(default_value="")}
