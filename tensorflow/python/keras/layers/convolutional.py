...
class SeparableConv2DTranspose(SeparableConv):
  ...
  Separable convolutions consist of first performing
  a pointwise transposed convolution (which acts to mix channels)
  followed by a depthwise transposed spatial convolution
  (which acts on each output channel separately).
  ...
  Returns:
    A tensor of rank 4 representing
    `activation(separable_conv2d_transpose(inputs, kernel) + bias)`.
  """
  ...
  def get_config(self):
    config = super(SeparableConv2DTranspose, self).get_config()
    config['output_padding'] = self.output_padding
    return config
...
SeparableConvolution2DTranspose = SeparableConv2DTranspose
