...
@combinations.generate(combinations.combine(mode=["graph", "eager"]))
class LocallyConnectedImplementationModeTest(test.TestCase, parameterized.TestCase):
    ...
    def test_locallyconnected_implementation(self, width, data_format):
        with self.cached_session():
            ...
            self.assertAllCloseAccordingToType(out_2, out_1, rtol=1e-5, atol=1e-5)
            self.assertAllCloseAccordingToType(out_2, out_3, rtol=1e-5, atol=1e-5)
            self.assertAllCloseAccordingToType(out_1, out_3, rtol=1e-5, atol=1e-5)
...
def get_inputs(data_format, filters, height, num_samples, width):
    ...
    inputs = np.random.normal(0, 1, (num_samples,) + input_shape).astype(np.float32)
    return inputs


def xent(y_true, y_pred):
    y_true = keras.backend.cast(keras.backend.reshape(y_true, (-1,)), dtypes.int32)

    return nn.sparse_softmax_cross_entropy_with_logits(labels=y_true, logits=y_pred)


def get_model(
    implementation, filters, kernel_size, strides, layers, num_classes, data_format
):
...
def copy_lc_weights_2_to_1(lc_layer_2_from, lc_layer_1_to):
    ...
    lc_2_kernel_flat = array_ops.boolean_mask(lc_2_kernel_masked, lc_2_kernel_mask)
...
def copy_model_weights(model_from, model_to):
    for l in range(len(model_from.layers)):
        ...
        if isinstance(
            ...
            layer_to, (keras.layers.LocallyConnected2D, keras.layers.LocallyConnected1D)
        ):
