...
class RNNCell(base_layer.Layer):
    ...
    def zero_state(self, batch_size, dtype):
        ...
        if is_eager and _hasattr(self, "_last_zero_state"):
            (last_state_size, last_batch_size, last_dtype, last_output) = getattr(
                self, "_last_zero_state"
            )
            if (
...
class LSTMStateTuple(_LSTMStateTuple):
    ...
    def dtype(self):
        ...
        if c.dtype != h.dtype:
            raise TypeError(
                "Inconsistent internal state: %s vs %s" % (str(c.dtype), str(h.dtype))
            )
        return c.dtype
...
class BasicLSTMCell(LayerRNNCell):
    ...
    def build(self, inputs_shape):
        ...
        self._kernel = self.add_variable(
            _WEIGHTS_VARIABLE_NAME, shape=[input_depth + h_depth, 4 * self._num_units]
        )
        self._bias = self.add_variable(
            _BIAS_VARIABLE_NAME,
...
class LSTMCell(LayerRNNCell):
    ...
    def build(self, inputs_shape):
        ...
        if self.dtype is None:
            ...
            initializer = init_ops.zeros_initializer(dtype=self.dtype)
        self._bias = self.add_variable(
            _BIAS_VARIABLE_NAME, shape=[4 * self._num_units], initializer=initializer
        )
    ...
    def call(self, inputs, state):
        ...
        if self._use_peepholes:
            ...
            ) * c_prev + sigmoid(i + self._w_i_diag * c_prev) * self._activation(j)
        else:
            c = sigmoid(f + self._forget_bias) * c_prev + sigmoid(i) * self._activation(
                j
            )
