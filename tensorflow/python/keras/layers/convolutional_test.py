    ...
    def test_conv1d_recreate_conv_unknown_dims(self):
        with self.cached_session():
            ...
            @def_function.function(
                input_signature=[tensor_spec.TensorSpec([1, None, 1])]
            )
            def fn(inpt):
                return layer(inpt)
...
class Conv2DTest(keras_parameterized.TestCase):
    ...
    def test_conv2d(
        ...
        if not requires_gpu or test.is_gpu_available(cuda_only=True):
            ...
            self._run_test_extra_batch_dim(kwargs, expected_output_shape, spatial_shape)
...
class Conv3DTest(keras_parameterized.TestCase):
    ...
    def test_conv3d(self, kwargs, expected_output_shape=None, requires_gpu=False):
        ...
        if not requires_gpu or test.is_gpu_available(cuda_only=True):
            ...
            self._run_test_extra_batch_dim(kwargs, expected_output_shape, test_training)
...
class Conv1DTransposeTest(keras_parameterized.TestCase):
    ...
    def test_conv1d_transpose(self, kwargs, expected_output_shape=None):
        ...
        ) or test.is_gpu_available(cuda_only=True):
            self._run_test(kwargs, expected_output_shape)
...
class ZeroPaddingTest(keras_parameterized.TestCase):
    ...
    def test_zero_padding_2d(self, data_format):
        ...
        # correctness test
        with self.cached_session():
            layer = keras.layers.ZeroPadding2D(padding=(2, 2), data_format=data_format)
            ...
            if data_format == "channels_last":
                ...
                for bottom_offset in [-1, -2]:
                    np.testing.assert_allclose(np_output[:, bottom_offset, :, :], 0.0)
        ...
        with self.cached_session():
            ...
            if data_format == "channels_last":
                for offset in [0, 1, -1, -2]:
                    ...
                    np.testing.assert_allclose(np_output[:, :, offset, :, :], 0.0)
                    np.testing.assert_allclose(np_output[:, :, :, offset, :], 0.0)
                np.testing.assert_allclose(np_output[:, 2:-2, 2:-2, 2:-2, :], 1.0)
            elif data_format == "channels_first":
                for offset in [0, 1, -1, -2]:
                    np.testing.assert_allclose(np_output[:, :, offset, :, :], 0.0)
                    np.testing.assert_allclose(np_output[:, :, :, offset, :], 0.0)
            ...
            if data_format == "channels_last":
                ...
                for offset in [-1, -2]:
                    np.testing.assert_allclose(np_output[:, :, :, offset, :], 0.0)
                np.testing.assert_allclose(np_output[:, 1:-2, 3:-4, 0:-2, :], 1.0)
            elif data_format == "channels_first":
                for offset in [0]:
                    np.testing.assert_allclose(np_output[:, :, offset, :, :], 0.0)
                for offset in [-1, -2]:
