...
class BaseDenseAttention(Layer):
  ...
  def call(self, inputs, mask=None, training=None):
    self._validate_call_args(inputs=inputs, mask=mask, training=training)
    q = inputs[0]
    v = inputs[1]
  ...
  def compute_mask(self, inputs, mask=None, training=None):
    self._validate_call_args(inputs=inputs, mask=mask, training=training)
    if mask:
      q_mask = mask[0]
      if q_mask is None:
  ...
  def _validate_call_args(self, inputs, mask, training):
    ...
    if self.dropout_rate > 0.0 and training is None:
      raise ValueError(
