...
def _exact_gaussian(stddev):
    return functools.partial(kernelized_utils.exact_gaussian_kernel, stddev=stddev)


def _exact_laplacian(stddev):
    return functools.partial(kernelized_utils.exact_laplacian_kernel, stddev=stddev)
...
class RandomFourierFeaturesTest(test.TestCase, parameterized.TestCase):
    ...
    def test_unsupported_kernel_type(self):
        with self.assertRaisesRegex(
            ...
            _ = kernel_layers.RandomFourierFeatures(3, "unsupported_kernel", stddev=2.0)
    ...
    def test_invalid_input_shape(self):
        ...
        rff_layer = kernel_layers.RandomFourierFeatures(output_dim=10, scale=3.0)
        with self.assertRaisesRegex(
            ValueError, r"The rank of the input tensor should be 2. Got 3 instead."
        ):
            _ = rff_layer(inputs)
    ...
    def test_call_on_placeholder(self, initializer):
        with ops.Graph().as_default():
            ...
            with self.assertRaisesRegex(
                ValueError,
                r"The last dimension of the inputs to "
    ...
    def test_compute_output_shape(self, output_dim, initializer, scale):
        ...
        self.assertEqual(
            [None, output_dim], rff_layer.compute_output_shape((None, 3)).as_list()
        )
        self.assertEqual(
            [None, output_dim],
    ...
    def test_different_params_similar_approximation(self, initializer, scale):
        ...
        rff_layer1 = kernel_layers.RandomFourierFeatures(
            output_dim=3000, kernel_initializer=initializer, scale=scale, name="rff1"
        )
        rff_layer2 = kernel_layers.RandomFourierFeatures(
            output_dim=2000, kernel_initializer=initializer, scale=scale, name="rff2"
        )
        # Two distinct inputs.
