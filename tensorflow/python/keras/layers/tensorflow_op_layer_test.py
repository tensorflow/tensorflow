...
class AutoLambdaTest(keras_parameterized.TestCase):
    @parameterized.named_parameters(
        ...
        ("shape_op_slice_and_range_known_dim", _shape_op_slice_and_range_known_dim),
        ("int32_manipulation_too_big_for_shape", _int32_manipulation_too_big_for_shape),
        (
    ...
    def test_getitem_slice_with_step_only(self):
        ...
        batch_size = 7
        step = 3
        x = array_ops.stack([math_ops.range(8) for _ in range(batch_size)])
        ...
        if ops.executing_eagerly_outside_functions():
            self.assertIn("tf.__operators__.getitem", (x.name for x in model.layers))
    ...
    def test_getitem_complex_slicing(self):
        ...
        model = keras.Model(
            inputs=[inp, first_dim, slice_start, slice_stop, slice_stride], outputs=out
        )
        model.compile(
            adam.Adam(0.001), "mse", run_eagerly=testing_utils.should_run_eagerly()
        )
        ...
        expected = array_ops.stack(
            [
                array_ops.stack([math_ops.range(8)[start:stop:step] for _ in range(4)])
                for _ in range(batch_size)
            ]
        )
    ...
    def test_numerical_correctness_simple(self):
        x = ops.convert_to_tensor_v2_with_dispatch([[-1.0, 0.0, -2.0, 1.0]])
        inputs = keras.Input(shape=(4,))
    ...
    def test_gradient_tape_in_function(self):
        ...
        self.assertAllEqual(
            f(constant_op.constant(10.0, shape=(1, 1))),
    ...
    def test_json_serialization(self):
        ...
        outputs = math_ops.cast(inputs, "float32") / 4.0
