...
class StackedRNNCells(Layer):
    ...
    def __init__(self, cells, **kwargs):
        for cell in cells:
            if "call" not in dir(cell):
                raise ValueError(
                    "All cells must have a `call` method. received cells:", cells
                )
            if "state_size" not in dir(cell):
                raise ValueError(
                    "All cells must have a `state_size` attribute. received cells:",
                    cells,
                )
        self.cells = cells
    ...
    def state_size(self):
        return tuple(
            c.state_size
            for c in (self.cells[::-1] if self.reverse_state_order else self.cells)
        )
    ...
    def call(self, inputs, states, constants=None, training=None, **kwargs):
        ...
        state_size = (
            self.state_size[::-1] if self.reverse_state_order else self.state_size
        )
        ...
        return inputs, nest.pack_sequence_as(
            state_size, nest.flatten(new_nested_states)
        )
    ...
    def build(self, input_shape):
        ...
        for cell in self.cells:
            ...
            input_shape = tuple(
                [input_shape[0]] + tensor_shape.TensorShape(output_dim).as_list()
            )
        self.built = True
...
class RNN(Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "call" not in dir(cell):
            raise ValueError(
                "`cell` should have a `call` method. The RNN was passed:", cell
            )
        if "state_size" not in dir(cell):
            raise ValueError(
                "The RNN cell should have "
    ...
    def compute_output_shape(self, input_shape):
        ...
        if getattr(self.cell, "output_size", None) is not None:
            ...
            output_shape = nest.flatten(
                nest.map_structure(_get_output_shape, self.cell.output_size)
            )
    ...
    def _validate_state_spec(cell_state_sizes, init_state_specs):
        ...
        for cell_state_spec, cell_state_size in zip(
            ...
            ).is_compatible_with(tensor_shape.TensorShape(cell_state_size)):
                raise validation_error
    ...
    def __call__(self, inputs, initial_state=None, constants=None, **kwargs):
        ...
        for tensor in flat_additional_inputs:
            if backend.is_keras_tensor(tensor) != is_keras_tensor:
                raise ValueError(
                    "The initial state or constants of an RNN"
        ...
        if is_keras_tensor:
            ...
            if self.built:
                ...
                full_input_spec = (
                    generic_utils.to_list(nest.map_structure(lambda _: None, inputs))
                    + additional_specs
                )
    ...
    def _process_inputs(self, inputs, initial_state, constants):
        ...
        if self.stateful:
            if initial_state is not None:
                ...
                non_zero_count = math_ops.add_n(
                    [math_ops.count_nonzero_v2(s) for s in nest.flatten(self.states)]
                )
                # Set strict = True to keep the original structure of the state.
                initial_state = cond.cond(
                    non_zero_count > 0,
    ...
    def _validate_args_if_ragged(self, is_ragged_input, mask):
        ...
        if mask is not None:
            raise ValueError(
                "The mask that was passed in was "
                + str(mask)
    ...
    def reset_states(self, states=None):
        ...
        if nest.flatten(self.states)[0] is None:
            if getattr(self.cell, "get_initial_state", None):
                flat_init_state_values = nest.flatten(
            ...
            self.states = nest.pack_sequence_as(
            ...
            for state, size in zip(
                nest.flatten(self.states), nest.flatten(self.cell.state_size)
            ):
                backend.set_value(
                    state,
...
class DropoutRNNCellMixin(object):
    ...
    def _create_dropout_mask(self, inputs, training, count=1):
        return _generate_dropout_mask(
            array_ops.ones_like(inputs), self.dropout, training=training, count=count
        )
