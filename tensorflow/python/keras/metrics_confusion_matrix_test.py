...
class PrecisionTest(test.TestCase, parameterized.TestCase):
    def test_config(self):
        ...
        self.assertEqual(
            [v.name for v in p_obj.variables], ["true_positives:0", "false_positives:0"]
        )
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertArrayNear(initial_precision, self.evaluate(p_obj.result()), 1e-3)
    ...
    def test_weighted_with_threshold(self):
        ...
        y_pred = constant_op.constant(
            [[1, 0], [0.6, 0]], shape=(2, 2), dtype=dtypes.float32
        )
        weights = constant_op.constant(
            [[4, 0], [3, 1]], shape=(2, 2), dtype=dtypes.float32
        )
        self.evaluate(variables.variables_initializer(p_obj.variables))
    ...
    def test_weighted_top_k(self):
        ...
        self.evaluate(
            p_obj(
                y_true1, y_pred1, sample_weight=constant_op.constant([[1, 4, 2, 3, 5]])
            )
        )
...
class RecallTest(test.TestCase, parameterized.TestCase):
    def test_config(self):
        ...
        self.assertEqual(
            [v.name for v in r_obj.variables], ["true_positives:0", "false_negatives:0"]
        )
    ...
    def test_weighted_with_threshold(self):
        ...
        weights = constant_op.constant(
            [[1, 4], [3, 2]], shape=(2, 2), dtype=dtypes.float32
        )
        self.evaluate(variables.variables_initializer(r_obj.variables))
    ...
    def test_multiple_updates(self):
        ...
        weighted_positives = ((0 + 3.0) + (4.0 + 0.0)) + ((0 + 3.0) + (4.0 + 0.0))
        expected_recall = weighted_tp / weighted_positives
        self.assertArrayNear([expected_recall, 0], self.evaluate(r_obj.result()), 1e-3)
...
class SensitivityAtSpecificityTest(test.TestCase, parameterized.TestCase):
    def test_config(self):
        s_obj = metrics.SensitivityAtSpecificity(
            0.4, num_thresholds=100, class_id=12, name="sensitivity_at_specificity_1"
        )
    ...
    def test_value_is_idempotent(self):
        ...
        y_pred = random_ops.random_uniform(
            (10, 3), maxval=1, dtype=dtypes.float32, seed=1
        )
        y_true = random_ops.random_uniform(
            (10, 3), maxval=2, dtype=dtypes.int64, seed=1
        )
...
class SpecificityAtSensitivityTest(test.TestCase, parameterized.TestCase):
    def test_config(self):
        s_obj = metrics.SpecificityAtSensitivity(
            0.4, num_thresholds=100, class_id=12, name="specificity_at_sensitivity_1"
        )
...
class RecallAtPrecisionTest(test.TestCase, parameterized.TestCase):
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertAlmostEqual(initial_recall, self.evaluate(s_obj.result()), 1e-3)
...
class AUCTest(test.TestCase, parameterized.TestCase):
    ...
    def test_config(self):
        self.setup()
        auc_obj = metrics.AUC(
            num_thresholds=100, curve="PR", summation_method="majoring", name="auc_1"
        )
    ...
    def test_weighted_pr_majoring(self):
        self.setup()
        auc_obj = metrics.AUC(
            num_thresholds=self.num_thresholds, curve="PR", summation_method="majoring"
        )
        self.evaluate(variables.variables_initializer(auc_obj.variables))
        result = auc_obj(self.y_true, self.y_pred, sample_weight=self.sample_weight)
    ...
    def test_weighted_pr_minoring(self):
        self.setup()
        auc_obj = metrics.AUC(
            num_thresholds=self.num_thresholds, curve="PR", summation_method="minoring"
        )
        self.evaluate(variables.variables_initializer(auc_obj.variables))
        result = auc_obj(self.y_true, self.y_pred, sample_weight=self.sample_weight)
...
class MultiAUCTest(test.TestCase, parameterized.TestCase):
    def setup(self):
        ...
        self.y_pred = constant_op.constant(
            np.array([[0, 0.5, 0.3, 0.9], [0.1, 0.2, 0.3, 0.4]]).T, dtype=dtypes.float32
        )
        ...
        self.y_true_bad = constant_op.constant(np.array([[0, 0, 1, 1], [1, 1, 0, 0]]).T)
        self.sample_weight = [1, 2, 3, 4]
    ...
    def test_unweighted_from_logits(self):
        with self.test_session():
            self.setup()
            auc_obj = metrics.AUC(
                num_thresholds=self.num_thresholds, multi_label=True, from_logits=True
            )
            self.evaluate(variables.variables_initializer(auc_obj.variables))
    ...
    def test_pr_interpolation(self):
        with self.test_session():
            ...
            self.assertAllClose(self.evaluate(good_result), (0.939 + 1.0) / 2.0, 1e-1)
