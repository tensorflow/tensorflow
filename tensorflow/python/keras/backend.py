...
def _get_session():
  ...
  if default_session is not None:
    ...
    # We use an init_scope since want to use the outermost graph even if we are
    # in the middle of defining a function.
    with ops.init_scope():
      ...
      if (getattr(_SESSION, 'session', None) is None or
          _SESSION.session.graph is not ops.get_default_graph()):
