...
def cast_to_floatx(x):
    ...
    """
    if isinstance(
        x, (tensor_lib.Tensor, variables_module.Variable, sparse_tensor.SparseTensor)
    ):
        ...
        with ops.init_scope():
            if context.executing_eagerly():
                if previous_eager_value is not None:
                    _GRAPH_LEARNING_PHASES[_DUMMY_EAGER_GRAPH.key] = (
...
def _current_graph(op_input_list, graph=None):
    ...
    for op_input in op_input_list:
        ...
        if isinstance(
            op_input,
            (ops.Operation, tensor_lib.Tensor, composite_tensor.CompositeTensor),
        ) and (
            (not isinstance(op_input, tensor_lib.Tensor))
            or type(op_input) == tensor_lib.Tensor
...
def placeholder(
    ...
    if ops.executing_eagerly_outside_functions():
        if sparse:
            ...
            spec = tensor_lib.TensorSpec(shape=shape, dtype=dtype, name=name)
...
def batch_dot(x, y, axes=None):
    ...
    if x_ndim > 3:
        ...
        x_squashed_shape = array_ops_stack.stack([x_shape[0], -1, x_shape[-1]])
    ...
    if y_ndim > 3:
        ...
        y_squashed_shape = array_ops_stack.stack([y_shape[0], y_shape[1], -1])
    ...
    def __call__(self, inputs):
        ...
        for tensor, value in zip(self.inputs, inputs):
            ...
            if tensor_util.is_tf_type(value):
                ...
                array_vals.append(
                    numpy_compat.np_asarray(value, dtype=tensor_type.as_numpy_dtype)
                )
...
def rnn(
    ...
    if unroll:
        ...
        if (
            ...
            and control_flow_util.GraphOrParentsInXlaContext(ops.get_default_graph())
        ):
