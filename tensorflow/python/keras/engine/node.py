...
class Node:
  ...
  """
    ...
    def map_arguments(self, tensor_dict):
      ...
      Args:
      tensor_dict: A dictionary mapping Keras tensors (or their IDs) to their computed values.
    ...
    def serialize(self, make_node_key, node_conversion_map):
        ...
        def serialize_first_arg_tensor(t):
            if is_keras_tensor(t):
                ...
                node_key = make_node_key(kh.layer.name, node_index)
                new_node_index = node_conversion_map.get(node_key, 0)
                data_instance = [kh.layer.name, new_node_index, kh.tensor_index, kwargs]
            elif isinstance(t, np.ndarray):
                data_instance = t.tolist()
            elif isinstance(t, tensor_lib.Tensor):
                data_instance = backend.get_value(t).tolist()
            ...
            return tf_utils.ListWrapper(data_instance)
