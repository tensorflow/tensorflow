...
def model_iteration(
    ...
    **kwargs
    ...
    input_iterator = None
    is_dataset = isinstance(inputs, (dataset_ops.DatasetV1, dataset_ops.DatasetV2))
    ...
    if isinstance(val_inputs, (dataset_ops.DatasetV1, dataset_ops.DatasetV2)):
        ...
        val_samples_or_steps = (
            val_inputs and nest.flatten(val_inputs)[0].shape[0] or None
        )
...
def _prepare_feed_values(model, inputs, targets, sample_weights, mode):
    ...
    if isinstance(
        inputs, (dataset_ops.DatasetV1, dataset_ops.DatasetV2, iterator_ops.Iterator)
    ):
...
class ArrayLikeTrainingLoop(training_utils_v1.TrainingLoop):
    ...
    def predict(
        self, model, x, batch_size=None, verbose=0, steps=None, callbacks=None, **kwargs
    ):
        batch_size = model._validate_or_infer_batch_size(batch_size, steps, x)
