    ...
    def _to_placeholder(self):
        ...
        if self._inferred_value is not None:
            ...
            inferred_value = array_ops.shape(
                array_ops.placeholder(shape=self._inferred_value, dtype=dtypes.int32)
            )
    ...
    def set_shape(self, shape):
        ...
        if not self.shape.is_compatible_with(shape):
            raise ValueError(
                ...
                "compatible with supplied shape %s" % (self.shape, shape)
            )
        else:
...
RaggedKerasTensor._overload_operator(
    ragged_tensor.RaggedTensor, "__getitem__"
)  # pylint: disable=protected-access

# Overload math ops
RaggedKerasTensor._overload_operator(
    ragged_tensor.RaggedTensor, "__add__"
)  # pylint: disable=protected-access
RaggedKerasTensor._overload_operator(
    ragged_tensor.RaggedTensor, "__radd__"
)  # pylint: disable=protected-access
RaggedKerasTensor._overload_operator(
    ragged_tensor.RaggedTensor, "__mul__"
)  # pylint: disable=protected-access
RaggedKerasTensor._overload_operator(
