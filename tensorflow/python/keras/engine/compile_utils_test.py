...
class LossesContainerTest(keras_parameterized.TestCase):
    ...
    def test_loss_dict_with_nones(self):
        loss_container = compile_utils.LossesContainer({"out1": None, "out2": "mae"})
    ...
    def test_nested_structure(self):
        loss_container = compile_utils.LossesContainer(
            {"b": ["mse", None], "a": "mae"}, loss_weights={"b": [0.5, 0], "a": 1}
        )

        y_t = {
            "b": [array_ops.ones((10, 1)), array_ops.zeros((10, 1))],
            "a": array_ops.zeros((10, 1)),
        }
        y_p = {
            "b": [array_ops.zeros((10, 1)), array_ops.zeros((10, 1))],
    ...
    def test_mismatched_dtypes(self):
        ...
        y_p = constant_op.constant([4, 8, 12, 8], shape=(2, 2), dtype=dtypes.float32)
    ...
    def test_float_dtypes(self):
        y_t = constant_op.constant([1, 9, 2, -5], shape=(2, 2), dtype=dtypes.float32)
        y_p = constant_op.constant([4, 8, 12, 8], shape=(2, 2), dtype=dtypes.float64)
    ...
    def test_loss_masking(self):
        ...
        y_p._keras_mask = constant_op.constant([[1, 0], [1, 0]], dtype=dtypes.float32)
    ...
    def test_loss_sample_weight(self):
        ...
        y_p = constant_op.constant([[[1], [1]], [[0], [0]]], dtype=dtypes.float32)
        y_t = constant_op.constant([[[1], [1]], [[1], [1]]], dtype=dtypes.float32)
        sw = constant_op.constant([[0.2, 0.3], [0.5, 0]], dtype=dtypes.float32)
    ...
    def test_ragged_tensor_output(self):
        ...
        v_t = constant_op.constant([[3.0, 4.0], [1.0, 2.0], [3.0, 5.0]])
        v_p = constant_op.constant([[3.1, 4.0], [1.0, 2.0], [3.0, 5.0]])
...
class MetricsContainerTest(keras_parameterized.TestCase):
    ...
    def test_metric_partial_dict_with_nones(self):
        metric_container = compile_utils.MetricsContainer({"out1": None, "out2": "mae"})
    ...
    def test_nested_structure(self):
        ...
        y_t = {
            "b": [2 * array_ops.ones((10, 1)), array_ops.zeros((10, 1))],
            "a": array_ops.zeros((10, 1)),
        }
        y_p = {
            "b": [array_ops.zeros((10, 1)), array_ops.zeros((10, 1))],
    ...
    def test_crossentropy(self):
        ...
        metric_container.update_state(y_t, y_p)
        self.assertEqual(
            metric_container.metrics[0]._fn, metrics_mod.sparse_categorical_crossentropy
        )
        ...
        metric_container.update_state(y_t, y_p)
        self.assertEqual(
            metric_container.metrics[0]._fn, metrics_mod.categorical_crossentropy
        )
    ...
    def test_accuracy(self):
        ...
        y_t, y_p = array_ops.ones((10, 1)), array_ops.ones((10, 1))
        metric_container.update_state(y_t, y_p)
        self.assertEqual(metric_container.metrics[0]._fn, metrics_mod.binary_accuracy)
        ...
        metric_container.update_state(y_t, y_p)
        self.assertEqual(
            metric_container.metrics[0]._fn, metrics_mod.sparse_categorical_accuracy
        )
    ...
    def test_metrics_masking(self):
        ...
        y_p = constant_op.constant([[[1], [1]], [[0], [0]]], dtype=dtypes.float32)
        y_t = constant_op.constant([[[1], [1]], [[1], [1]]], dtype=dtypes.float32)
        y_p._keras_mask = constant_op.constant([[1, 1], [0, 0]], dtype=dtypes.float32)
    ...
    def test_metrics_sample_weight(self):
        ...
        weighted_mae_metric = metrics_container.metrics[1]
        self.assertEqual(weighted_mae_metric.name, "mse")
        self.assertAlmostEqual(weighted_mae_metric.result().numpy(), 0.5)  # .5 / 1
    ...
    def test_metrics_masking_sample_weight(self):
        ...
        y_p = constant_op.constant([[[1], [1]], [[0], [1]]], dtype=dtypes.float32)
        y_t = constant_op.constant([[[1], [1]], [[1], [1]]], dtype=dtypes.float32)
        sw = constant_op.constant([[0.3, 0.2], [0.2, 0.3]], dtype=dtypes.float32)
        y_p._keras_mask = constant_op.constant([[1, 0], [1, 0]], dtype=dtypes.float32)
