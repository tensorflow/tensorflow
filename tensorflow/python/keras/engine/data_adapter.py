...
class DataHandler(object):
    ...
    def _log_data_exhausted_warning(self):
        logging.warning("Your input ran out of data; hence clipping steps_per_epoch "
                        "to maximum possible value (in this case {} steps). Not that this "
                        "means entire dataset will be "
                        "re-used for this operation for every epoch "
                        "Make sure that your dataset or generator can generate at "
                        "least `steps_per_epoch * epochs` batches. You may need to "
                        "use the repeat() function when building "
                        "your dataset.".format(len(self._dataset)))
    ...
    def _infer_steps(self, steps, dataset):
        ...
        if is_infinite and steps is None:
            raise ValueError(
