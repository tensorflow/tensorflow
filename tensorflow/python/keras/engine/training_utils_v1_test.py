...
class DatasetUtilsTest(test.TestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        ...
        ("FromTensorSlices", lambda: dataset_ops.Dataset.from_tensor_slices([0, 0, 0])),
        (
            "Interleave",
        ...
        (
            "Interleave_Shuffle",
            lambda: dataset_ops.Dataset.range(5).interleave(
                lambda _: dataset_ops.Dataset.from_tensors(0).shuffle(1), cycle_length=1
            ),
            True,
        ),
...
class StandardizeWeightsTest(keras_parameterized.TestCase):
    ...
    def test_class_weights(self):
        ...
        weights = training_utils_v1.standardize_weights(y, class_weight=class_weights)
    ...
    def test_dataset_with_class_weight(self):
        ...
        model.fit(
            dataset, epochs=1, steps_per_epoch=2, verbose=1, class_weight=class_weight
        )
...
class AggregationTest(keras_parameterized.TestCase):
    def setUp(self):
        ...
        self._old_threshold = training_utils_v1.SliceAggregator._BINARY_SIZE_THRESHOLD
        self._old_timeout = training_utils_v1.SliceAggregator._MAX_COPY_SECONDS
        training_utils_v1._COPY_POOL = MonitoredPool(training_utils_v1._COPY_THREADS)

    def tearDown(self):
        super(AggregationTest, self).tearDown()
        training_utils_v1._COPY_POOL = self._old_pool
        training_utils_v1.SliceAggregator._BINARY_SIZE_THRESHOLD = self._old_threshold
    ...
    def _run_without_steps(self):
        aggregator = training_utils_v1.OutputsAggregator(use_steps=False, num_samples=6)
