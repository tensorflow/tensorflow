...
def _eager_metrics_fn(model, outputs, targets, sample_weights=None, masks=None):
    ...
    metric_results.extend(
        [m.result() for m in model.metrics if m not in model._compile_metric_functions]
    )
...
def _model_loss(
    ...
    with backend.name_scope("loss"):
        ...
        if not loss_fns and not custom_losses:
            if training:
                raise ValueError(
                    "The model cannot be trained because it has no loss to optimize."
                )
            else:
                raise ValueError(
                    "The model cannot be evaluated because it has no loss to compute."
                )
...
def _process_single_batch(
    ...
    """
    with (
        backend.eager_learning_phase_scope(1 if training else 0),
