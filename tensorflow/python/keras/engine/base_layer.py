...
metrics_mod = generic_utils.LazyLoader(
    "metrics_mod", globals(), "tensorflow.python.keras.metrics"
)
...
# to tf.cast?
_AUTOCAST_TYPES = (ops.Tensor, sparse_tensor.SparseTensor, ragged_tensor.RaggedTensor)
    ...
    * `get_config(self)`: Returns a dictionary containing the configuration used
    ...
    def add_weight(
        ...
        **kwargs
        ...
        if (
            autocast
            and self._dtype_policy.compute_dtype != self._dtype_policy.variable_dtype
    ...
    def _keras_tensor_symbolic_call(self, inputs, input_masks, args, kwargs):
        if self.dynamic:
            ...
            input_signature = nest.map_structure(
                lambda x: tensor_spec.TensorSpec(shape=x.shape, dtype=x.dtype), inputs
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        if any(
            isinstance(x, (np_arrays.ndarray, np.ndarray, float, int))
            for x in input_list
        ):
        ...
        with call_context.enter(
            layer=self, inputs=inputs, build_graph=not eager, training=training_mode
        ):
    ...
    def __delattr__(self, name):
        ...
        if existing_value not in reference_counts:
            super(tracking.AutoTrackable, self).__delattr__(
                name
    ...
    def __setattr__(self, name, value):
        ...
        if getattr(self, "_auto_track_sub_layers", True) and (
            isinstance(value, module.Module) or base_layer_utils.has_weights(value)
        ):
            self._maybe_create_attribute("_self_tracked_trackables", [])
    ...
    def _flatten_modules(self, recursive=True, include_self=True):
        ...
        if trackables:
            ...
            while deque:
                ...
                if isinstance(trackable_obj, module.Module) and not isinstance(
    ...
    def _set_save_spec(self, inputs):
        ...
        self._saved_model_inputs_spec = nest.map_structure(
            tf_utils.get_tensor_spec, inputs
        )
    ...
    def _list_extra_dependencies_for_serialization(self, serialization_cache):
        return (
            self._trackable_saved_model_saver.list_extra_dependencies_for_serialization(
                serialization_cache
            )
        )
