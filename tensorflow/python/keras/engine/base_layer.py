    ...
    * `get_config(self)`: Returns a dictionary containing the configuration used
    ...
    def compute_output_signature(self, input_signature):
        ...
        return nest.map_structure(
            lambda s: tensor_lib.TensorSpec(dtype=dtype, shape=s), output_shape
        )
    ...
    def _keras_tensor_symbolic_call(self, inputs, input_masks, args, kwargs):
        if self.dynamic:
            ...
            input_signature = nest.map_structure(
                lambda x: tensor_lib.TensorSpec(shape=x.shape, dtype=x.dtype), inputs
            )
    ...
    def input_spec(self, value):
        for v in nest.flatten(value):
            if v is not None and not isinstance(v, InputSpec):
                raise TypeError(
                    "Layer input_spec must be an instance of InputSpec. Got: {}".format(
                        v
                    )
                )
    ...
    def input_shape(self):
        ...
        if not self._inbound_nodes:
            raise AttributeError(
                "The layer has never been called and thus has no defined input shape."
            )
