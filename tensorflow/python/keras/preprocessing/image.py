...
def smart_resize(x, size, interpolation='bilinear'):
    ...
    if img.shape.rank == 4:
        crop_box_start = array_ops.stack(
            [0, crop_box_hstart, crop_box_wstart, 0])
...
@keras_export('keras.preprocessing.image.load_base64_2_image')
def load_base64_2_image(base64_string, grayscale=False, color_mode='rgb',
    ...
    ```
    image = tf.keras.preprocessing.image.load_base64_2_image(base64_string)
    input_arr = tf.keras.preprocessing.image.img_to_array(image)
    input_arr = np.array([input_arr])  # Convert single image to a batch.
    predictions = model.predict(input_arr)
    ...
    b64image_type = base64_string.split(':')
    b64image_type = b64image_type[1].split(";")
    b64image_type = b64image_type[0]
    ...
    try:
        ...
        if target_size:
            pil_image = pil_image.resize(
                size=target_size, resample=interpolation)

        pil_image = pil_image.convert('RGB')

    except ValueError:
        raise ValueError("Can't decode base64string to PIL format.")
