  ...
  def testSparseBasicWithLearningRateDecay(self):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      ...
      if context.executing_eagerly():
        ...
        self.evaluate(update_op)
      # Validate updated params
      self.assertAllCloseAccordingToType([[1.0 - 3.0 * 0.1 - 2.0 * 0.1], [2.0]],
                                         self.evaluate(var0))
      self.assertAllCloseAccordingToType(
...
class MomentumOptimizerTest(test.TestCase, parameterized.TestCase):
  ...
  def testNesterovMomentum(self):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      ...
      self.evaluate(variables.global_variables_initializer())
      for i in range(1, 5):
        # already updated once in eager mode
        if i != 1 and context.executing_eagerly():
          mom_op.minimize(loss, [var0, var1])
        else:
  ...
  def testSparseNesterovMomentum(self):
    for dtype in [dtypes.float32, dtypes.float64]:
      ...
      for t in range(1, 5):
        y = ops.IndexedSlices(
            constant_op.constant(grads[t - 1], dtype=dtype),
            constant_op.constant([0, 1]), constant_op.constant([2]))
        grads_and_vars.append([(y, var0),
                               (constant_op.constant([3.0, 3.0],
                                                     dtype=dtype), var1)])
      if not context.executing_eagerly():
        opt_update = []
        for t in range(1, 5):
          opt_update.append(mom_op.apply_gradients(grads_and_vars[t - 1]))
      self.evaluate(variables.global_variables_initializer())
      for t in range(1, 5):
        if context.executing_eagerly():
          mom_op.apply_gradients(grads_and_vars[t - 1])
        else:
          self.evaluate(opt_update[t - 1])
        var0_np, accum0_np = self._update_nesterov_momentum_numpy(
            var0_np, accum0_np, var0_np * 10, 2.0, 0.9)
        var1_np, accum1_np = self._update_nesterov_momentum_numpy(
            var1_np, accum1_np, 3, 2.0, 0.9)
