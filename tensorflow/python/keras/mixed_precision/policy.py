...
class Policy(object):
    ...
    def _parse_name(self, name):
        ...
        if name.endswith("_float32_vars"):
            error_msg = (
                "Policies ending in '_float32_vars' have been removed from TensorFlow."
            )
...
def _policy_equivalent_to_dtype(policy):
    ...
    return (
        type(policy) == Policy  # pylint: disable=unidiomatic-typecheck
        and list(policy.get_config().keys()) == ["name"]
