...
class PolicyTest(test.TestCase, parameterized.TestCase):
    ...
    def test_repr(self):
        # Test Policy repr
        for policy in ("float32", "int8", "mixed_float16", "mixed_bfloat16", "_infer"):
            self.assertEqual(repr(mp_policy.Policy(policy)), '<Policy "%s">' % policy)
    ...
    def test_policy_errors(self):
        ...
        with self.assertRaisesRegex(TypeError, "'name' must be a string, but got: 5"):
            mp_policy.Policy(5)
    ...
    def test_loss_scale_warning(self):
        with test.mock.patch.object(tf_logging, "warning") as mock_warn:
            mp_policy.PolicyV1("float32", loss_scale=2.0)
            self.assertEqual(
        ...
        for policy_name in "float16", "mixed_float16":
            ...
            mp_policy.PolicyV1(policy_name, loss_scale=2.0)
            with test.mock.patch.object(tf_logging, "warning") as mock_warn:
                mp_policy.PolicyV1(policy_name, loss_scale=2.0)
    ...
    def test_serialization(self):
        ...
        for policy in (
            ...
            mp_policy.PolicyV1("mixed_float16", loss_scale=2.0),
            mp_policy.PolicyV1("mixed_float16", loss_scale=None),
            mp_policy.PolicyV1("mixed_bfloat16", loss_scale=2.0),
