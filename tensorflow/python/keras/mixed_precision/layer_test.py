...
def create_central_storage_strategy():
    ...
    compute_devices = (
        ["cpu:0", "gpu:0"] if (tf_config.list_logical_devices("GPU")) else ["cpu:0"]
    )
...
class LayerTest(keras_parameterized.TestCase):
    ...
    def test_config(self, strategy_fn):
        ...
        with strategy_fn().scope():
            for layer, dtype in (
                ...
                (mp_test_util.MultiplyLayer(dtype=policy.Policy("float64")), "float64"),
            ):
    ...
    def test_config_policy_v1(self, strategy_fn):
        ...
        with strategy_fn().scope():
            ...
            layer = mp_test_util.MultiplyLayer(
                dtype=policy.PolicyV1("float64", loss_scale=2.0)
            )
            config = layer.get_config()
            self.assertEqual(
                config["dtype"],
            ...
            layer = mp_test_util.MultiplyLayer(
                dtype=policy.PolicyV1("_infer", loss_scale=2.0)
            )
            config = layer.get_config()
            self.assertEqual(
                config["dtype"],
