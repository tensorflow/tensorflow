...
class LayerCallCollection(object):
  ...
  def add_function(self, python_function, name):
    ...
    self._functions[name] = fn = LayerCall(
        self, python_function, name, input_signature=self.fn_input_signature,
        autograph=False)
...
def infer_inputs_from_restored_call_function(fn):
  ...
  """
  spec = nest.flatten(fn.concrete_functions[0].structured_input_signature)[0]
  for concrete in fn.concrete_functions[1:]:
    spec2 = nest.flatten(concrete.structured_input_signature)[0]
    spec = tensor_spec.TensorSpec(
        defun.common_shape(spec.shape, spec2.shape), spec.dtype, spec.name)
  return spec
