...
def wrap_layer_objects(layer, serialization_cache):
    ...
    wrapped_layer_losses = [keras_loss_cache[fn] for fn in layer._callable_losses[:]]  # pylint: disable=protected-access

    layer_metrics = data_structures.wrap_or_unwrap({m.name: m for m in layer._metrics})  # pylint: disable=protected-access
    return dict(
...
class LayerCallCollection(object):
    ...
    def _generate_input_signature(self, layer):
        ...
        elif layer.input_spec is not None and layer._use_input_spec_as_call_signature:  # pylint: disable=protected-access
