...
class TestBasicConvUtilsTest(test.TestCase):
    def test_convert_data_format(self):
        self.assertEqual("NCDHW", conv_utils.convert_data_format("channels_first", 5))
        self.assertEqual("NCHW", conv_utils.convert_data_format("channels_first", 4))
        ...
        self.assertEqual("NDHWC", conv_utils.convert_data_format("channels_last", 5))

        with self.assertRaises(ValueError):
            conv_utils.convert_data_format("invalid", 2)

    def test_normalize_tuple(self):
        self.assertEqual((2, 2, 2), conv_utils.normalize_tuple(2, n=3, name="strides"))
        self.assertEqual(
            (2, 1, 2), conv_utils.normalize_tuple((2, 1, 2), n=3, name="strides")
        )
    ...
    def test_deconv_output_length(self):
        ...
        self.assertEqual(5, conv_utils.deconv_output_length(4, 2, "valid", stride=1))
        self.assertEqual(8, conv_utils.deconv_output_length(4, 2, "valid", stride=2))
        ...
        self.assertEqual(
            5, conv_utils.deconv_output_length(4, 2, "same", output_padding=2, stride=1)
        )
        self.assertEqual(
            7, conv_utils.deconv_output_length(4, 2, "same", output_padding=1, stride=2)
        )
        self.assertEqual(
            7,
            conv_utils.deconv_output_length(4, 2, "valid", output_padding=2, stride=1),
        )
        self.assertEqual(
            9,
            conv_utils.deconv_output_length(4, 2, "valid", output_padding=1, stride=2),
        )
        self.assertEqual(
            5, conv_utils.deconv_output_length(4, 2, "full", output_padding=2, stride=1)
        )
        self.assertEqual(
            7, conv_utils.deconv_output_length(4, 2, "full", output_padding=1, stride=2)
        )
        self.assertEqual(
            5,
...
class TestConvUtils(test.TestCase, parameterized.TestCase):
    def test_conv_kernel_mask_fc(self, *input_shape):
        ...
        self.assertAllEqual(
            mask,
            conv_utils.conv_kernel_mask(input_shape, kernel_shape, strides, padding),
        )
