...
class KernelizedUtilsTest(test.TestCase, parameterized.TestCase):
    ...
    def test_matrices_varying_similarity(self, exact_kernel_fn, expected_values):
        ...
        x = constant_op.constant([1.0, 2.0, -2.0, 0.9, 3.3, -1.0], shape=[3, 2])
        y = constant_op.constant([1.1, 2.1, -2.0, 0.9], shape=[2, 2])
        exact_kernel = exact_kernel_fn(x, y)
    ...
    def test_completely_dissimilar_vectors(self, exact_kernel_fn, expected_values):
