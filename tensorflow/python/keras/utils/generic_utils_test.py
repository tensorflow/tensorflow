...
class HasArgTest(test.TestCase):
    def test_has_arg(self):
        ...
        self.assertTrue(keras.utils.generic_utils.has_arg(f_x, "x", accept_all=False))
        self.assertFalse(keras.utils.generic_utils.has_arg(f_x, "y", accept_all=False))
        self.assertTrue(
...
class SerializeKerasObjectTest(test.TestCase):
    def test_serialize_none(self):
        ...
        deserialized = keras.utils.generic_utils.deserialize_keras_object(serialized)
    ...
    def test_serialize_custom_class_with_custom_name(self):
        ...
        self.assertEqual(serialized_name, class_name)
        fn_class_name = keras.utils.generic_utils.get_registered_name(OtherTestClass)
    ...
    def test_serialize_custom_class_without_get_config_fails(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Cannot register a class that does " "not have a get_config.*"
        ):
    ...
    def test_serialize_type_object_initializer(self):
        ...
        config = keras.layers.serialize(layer)
        self.assertEqual(config["config"]["bias_initializer"]["class_name"], "Zeros")
        self.assertEqual(config["config"]["kernel_initializer"]["class_name"], "Ones")
    ...
    def test_deserialize_unknown_object(self):
        ...
        with self.assertRaisesRegexp(ValueError, "passed to the `custom_objects` arg"):
...
class SliceArraysTest(test.TestCase):
    def test_slice_arrays(self):
        input_a = list([1, 2, 3])
        self.assertEqual(
            keras.utils.generic_utils.slice_arrays(input_a, start=0), [None, None, None]
        )
        self.assertEqual(
            keras.utils.generic_utils.slice_arrays(input_a, stop=3), [None, None, None]
        )
        self.assertEqual(
...
class SharedObjectScopeTest(test.TestCase):
    def test_shared_object_saving_scope_single_object_doesnt_export_id(self):
        with generic_utils.SharedObjectSavingScope() as scope:
            ...
            self.assertNotIn(generic_utils.SHARED_OBJECT_KEY, single_object_config)
