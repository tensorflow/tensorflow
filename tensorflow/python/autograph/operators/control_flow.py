...
def _verify_tf_condition(cond, tag):
    ...
    if cond.shape is None or cond.shape.ndims is None:
        ...
        if np.prod(known_dims) > 1:
            raise ValueError(
                "condition of {} expected to be `tf.bool` scalar, got {}; {}".format(
                    tag, cond, extra_hint
                )
            )
        else:
        ...
        try:
            ...
            try:
                ...
                nest.assert_same_structure(
                    body_var_tensors, orelse_var_tensors, expand_composites=True
                )
            except (ValueError, TypeError) as e:
                raise TypeError(
...
def _placeholder_value(like, shape_invariant, original=None):
    ...
    if like is None:
        ...
        if shape_invariant is None:
            zipped = nest.map_structure(
                lambda v: _placeholder_value(v, None), nest.flatten(like)
            )
        else:
            zipped = nest.map_structure(
                _placeholder_value, nest.flatten(like), nest.flatten(shape_invariant)
            )
    ...
    raise TypeError(
        ...
        " Supported types include Tensor, int, float, bool, list, tuple or dict.".format(
            type(like).__name__, like
        )
    )


def _try_handling_undefineds(
    body, get_state, set_state, init_vars, nulls, shape_invariants, symbol_names
):
    ...
    try:
        ...
        inits_and_invariants = tuple(
            ...
            for v, n, iv, i in zip(init_vars, nulls, first_iter_vars, shape_invariants)
        )
...
def _tf_while_stmt(test, body, get_state, set_state, symbol_names, opts):
    ...
    if any(nulls):
        ...
        (require_one_iteration, init_vars, extra_shape_invariants) = (
            _try_handling_undefineds(
                body,
                get_state,
                set_state,
                init_vars,
    ...
    if require_one_iteration:
        ...
        merged_shape_invariants = tuple(
            (nv, merged_shape_invariants[id(nv)])
            for nv in init_vars
    ...
    if require_one_iteration:
        ...
        if "shape_invariants" in while_loop_opts:
            while_loop_opts["shape_invariants"] = (None,) + while_loop_opts[
                "shape_invariants"
            ]
    else:
        aug_init_vars = init_vars

    final_loop_vars = while_loop.while_loop(
