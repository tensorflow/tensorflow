...
def _partial_shaped_bools():
    rand_vect = math_ops.range(
        random_ops.random_uniform(shape=(), minval=2, maxval=3, dtype=dtypes.int32)
    )
...
class WhileLoopTest(testing.AutoGraphTestCase):
    ...
    def test_tensor_failing_to_determine_placeholder(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            re.compile("must be defined.*tried to define.*unsupported type", re.DOTALL),
        ):
            control_flow.while_stmt(
    ...
    def test_python_for_large_unroll_warning(self):
        ...
        with test.mock.patch.object(
            ...
            with ops.Graph().as_default():
                ...
                self.assertTrue(
                    re.match(r".* Large unrolled loop.*Add.*", out_capturer.getvalue())
                )
