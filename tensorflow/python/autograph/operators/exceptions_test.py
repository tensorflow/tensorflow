...
class ExceptionsTest(test.TestCase):
    ...
    def test_assert_tf_triggered(self):
        with self.cached_session() as sess:
            ...
            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, "test message"
            ):
                self.evaluate(t)
    ...
    def test_assert_tf_multiple_printed_values(self):
        ...
        with self.cached_session() as sess:
            t = exceptions.assert_stmt(constant_op.constant(False), lambda: two_tensors)

            with self.assertRaisesRegex(
                errors_impl.InvalidArgumentError, "test message.*another message"
            ):
                self.evaluate(t)
