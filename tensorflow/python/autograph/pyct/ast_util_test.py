...
class AstUtilTest(test.TestCase):
  ...
  def test_find_matching_lambda_definitions(self):
    node = parser.parse_str(
        textwrap.dedent("""
      f = lambda x: 1
    """))
  ...
  def test_find_matching_lambda_definitions_multiple_matches(self):
    node = parser.parse_str(
        textwrap.dedent("""
  ...
  def test_find_matching_lambda_definitions_uses_arg_names(self):
    node = parser.parse_str(
        textwrap.dedent("""
