...
class ApiTest(test.TestCase):
    ...
    def test_inspect_source_unsupported(self):
        ...
        with (
            patch.dict(os.environ, {"AUTOGRAPH_STRICT_CONVERSION": "0"}),
            patch.object(inspect, "findsource", side_effect=OSError()),
            self.assertLogs(level="WARNING") as logs,
        ):
            with (
                patch.object(ag_ctx, "INSPECT_SOURCE_SUPPORTED", False),
                self.assertRaisesRegex(
                    tf_errors.OperatorNotAllowedInGraphError,
            ...
            with (
                patch.object(ag_ctx, "INSPECT_SOURCE_SUPPORTED", True),
                self.assertRaisesRegex(
                    tf_errors.OperatorNotAllowedInGraphError,
