...
class CheckpointInputPipelineHook(session_run_hook.SessionRunHook):
  ...
  This hook saves the state of the iterators in the `Graph` so that
  the input pipeline can continue from where it left off when training
  is resumed.
  This could potentially avoid overfitting in certain pipelines where the
  number of training steps per eval are small compared to the dataset
  size or if the training pipeline is preempted.
  ...
  eval. You can do that without adding the iterator to the SAVEABLE_OBJECTS
