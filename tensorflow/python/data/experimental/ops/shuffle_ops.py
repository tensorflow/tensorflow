...
def _process_file_infos(file_infos):
    ...
    for file_info in file_infos:
        ...
        if "skip" in file_info:
            if file_info["skip"] < -1:
                raise ValueError(
                    "`skip` should be greater than `-1` but got {}".format(
                        file_info["skip"]
                    )
                )
            if file_info["skip"] == -1:
        ...
        if "take" in file_info:
            if file_info["take"] < -1:
                raise ValueError(
                    "`take` should be greater than `-1` but got {}".format(
                        file_info["take"]
                    )
                )
...
def _adjust_index(index, thresholds, offsets):
    ...
    t_index = (
        array_ops.shape(
            array_ops.boolean_mask(thresholds, math_ops.less_equal(thresholds, index))
        )[0]
        - 1
    )
...
def index_shuffle(
    ...
    def sequential_index_shuffle(seeds):
        ...
        def read_element(dataset, index):
            ...
            if "thresholds" in result and "offsets" in result:
                shuffled_index = _adjust_index(
                    shuffled_index, result["thresholds"], result["offsets"]
                )
            # 3) Perform the read.
    ...
    rng_ds = dataset_ops.Dataset.random(
        seed=seed, rerandomize_each_iteration=reshuffle_each_iteration
    )
