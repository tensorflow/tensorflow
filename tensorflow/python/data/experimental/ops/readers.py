...
def _infer_column_names(filenames, field_delim, use_quote_delim, file_io_fn):
    ...
    with file_io_fn(filenames[0]) as f:
        try:
            ...
            raise ValueError(
                "Failed when reading the header line of "
...
def _get_sorted_col_indices(select_columns, column_names):
    ...
    if len(results) != len(select_columns):
        ...
        duplicate_columns = set(
            [a for a, b in zip(sorted_names[:-1], sorted_names[1:]) if a == b]
        )
        raise ValueError(
            "The `select_columns` argument contains duplicate "
...
def make_csv_dataset_v2(
    ...
    if column_names is None:
        if not header:
            raise ValueError(
                "Expected `column_names` or `header` arguments. Neither is provided."
            )
    ...
    if label_name is not None and label_name not in column_names:
        raise ValueError(
            "`label_name` provided must be one of the columns: "
