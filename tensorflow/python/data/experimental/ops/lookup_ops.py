...
def _check_table_initializer_element_spec(element_spec):
    ...
    base_error = (
        "Datasets used to initialize lookup tables must "
...
class DatasetInitializer(lookup_ops.TableInitializerBase):
    ...
    def initialize(self, table):
        ...
        init_op = ged_ops.initialize_table_from_dataset(
            table.resource_handle, self.dataset._variant_tensor
...
def table_from_dataset(
    ...
    if default_value is None:
        ...
        if not (elem_spec[1].dtype.is_integer or elem_spec[1].dtype.is_floating):
            raise ValueError(
                "`default_value` must be specified when creating a "
                "table from a dataset that produces values of type "
                f"{elem_spec[1].dtype}."
            )
    if num_oov_buckets < 0:
        raise ValueError(
            "`num_oov_buckets` must be greater than or equal to 0, "
            f"got {num_oov_buckets}."
        )
    if (
        not isinstance(vocab_size, tensor.Tensor)
    ...
    if (not key_dtype.is_integer) and (dtypes.string != key_dtype.base_dtype):
        raise TypeError(
            f"`key_dtype` must be either an integer or string type, but got {key_dtype}"
        )
...
def index_table_from_dataset(
    ...
    Raises:
      ValueError: If
        ...
        * The `key_dtype` is not integer or string
    """
    return table_from_dataset(
        dataset.enumerate().map(lambda v, k: (k, v)),
        num_oov_buckets,
        vocab_size,
        default_value,
        hasher_spec,
