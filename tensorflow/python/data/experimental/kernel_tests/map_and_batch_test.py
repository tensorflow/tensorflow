...
class MapAndBatchTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(
        combinations.times(
            ...
            + combinations.combine(num_parallel_calls=None, num_parallel_batches=10),
        )
    )
    def testMapAndBatch(self, num_parallel_calls, num_parallel_batches):
        ...
        for i in range(num_batches):
            ...
            for component, result_component in zip(components, result):
                for j in range(14):
                    self.assertAllEqual(
                        component[(i * 14 + j) % 7] ** 2, result_component[j]
                    )
        ...
        for i in range(num_batches - 1):
            ...
            for component, result_component in zip(components, result):
                for j in range(8):
                    self.assertAllEqual(
                        component[(i * 8 + j) % 7] ** 2, result_component[j]
                    )
        ...
        for component, result_component in zip(components, result):
            for j in range((14 * 7) % 8):
                self.assertAllEqual(
                    component[((num_batches - 1) * 8 + j) % 7] ** 2, result_component[j]
                )
        with self.assertRaises(errors.OutOfRangeError):
            self.evaluate(get_next())
    ...
    def testMapAndBatchParallelGetNextDropRemainder(self):
        dataset = dataset_ops.Dataset.range(49999).apply(
            batching.map_and_batch(lambda x: x, batch_size=100, drop_remainder=True)
        )
    ...
    def testMapAndBatchImplicitDispose(self):
        ...
        dataset = (
            dataset_ops.Dataset.from_tensor_slices(components)
            .repeat(1000)
            .apply(batching.map_and_batch(_map_fn, batch_size=100))
        )
    ...
    def testMapAndBatchMapError(self, threshold):
        ...
        for i in range(threshold // 10):
            self.assertAllEqual(
                [i * 10 + j for j in range(10)], self.evaluate(get_next())
            )
    ...
    @combinations.generate(
        combinations.times(
            ...
            + combinations.combine(
                element=42.0, dtype=[dtypes.float16, dtypes.float32, dtypes.float64]
            )
    ...
    def testMapAndBatchTypes(self, element, dtype):
        ...
        dataset = (
            dataset_ops.Dataset.from_generator(gen, dtype)
            .repeat(100)
            .apply(batching.map_and_batch(lambda x: x, batch_size=10))
        )
    ...
    def testShortCircuitIdentity(self):
        map_fn = lambda x: x
        dataset = (
            self.structuredDataset(None)
            .repeat()
            .apply(batching.map_and_batch(map_fn, batch_size=10))
        )
        get_next = self.getNext(dataset)
    ...
    def testShortCircuitSwap(self):
        ...
        dataset = (
            self.structuredDataset((None, None))
            .repeat()
            .apply(batching.map_and_batch(map_fn, batch_size=10))
        )
        get_next = self.getNext(dataset)
    ...
    def testShortCircuitCapturedInput(self):
        ...
        dataset = (
            self.structuredDataset(None)
            .repeat()
            .apply(batching.map_and_batch(lambda x: captured_t, batch_size=10))
        )
    ...
    def testMapAndBatchControlFlow(self):
        ...
        for i in range(10):
            if i < 5:
                self.assertAllEqual(
                    [i * 10 + j + 1 for j in range(10)], self.evaluate(get_next())
                )
            else:
                self.assertAllEqual(
    ...
    def testCheckpointLargeBatches(self):
        ...
        dataset = dataset_ops.Dataset.from_tensors(
            array_ops.ones((64, 1024, 1024), dtype=dtypes.float32)
        ).repeat()
...
class MapAndBatchCheckpointTest(
    ...
    def testNumParallelBatches(self, verify_fn, drop_remainder):
        ...
        def build_ds(range_start, drop_remainder):
            ...
            return (
                dataset_ops.Dataset.range(range_start, range_start + range_size)
                .shard(num_shards=num_shards, index=0)
                .repeat(num_repeats)
                .apply(
                    batching.map_and_batch(
                        map_func=_map_fn,
        ...
        verify_fn(
            self, lambda: build_ds(10, drop_remainder=drop_remainder), num_outputs
        )
