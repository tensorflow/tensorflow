...
class DenseToSparseBatchTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testBasic(self):
        ...
        dataset = (
            ...
            .map(lambda x: array_ops.fill([x], x))
            .apply(batching.dense_to_sparse_batch(4, [12]))
        )
    ...
    def testWithUnknownShape(self):
        ...
        dataset = (
            dataset_ops.Dataset.from_tensor_slices(components)
            .map(lambda x: array_ops.fill([x, x], x))
            .apply(batching.dense_to_sparse_batch(4, [5, None]))
        )
        ...
        for start in range(0, len(components), 4):
            ...
            self.assertAllEqual(
                [
                    c
                    for c in components[start : start + 4]
                    for _ in range(c)
                    for _ in range(c)
                ],
                results.values,
    ...
    def testShapeErrors(self):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "incompatible with the row shape"
        ):
            self.evaluate(get_next())
