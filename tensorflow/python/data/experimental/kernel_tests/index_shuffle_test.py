...
class IndexShuffleTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testProcessFileInfos(self):
        file_infos = []
        file_infos.append(
            {"path": "take_50", "num_elements": 100, "skip": 25, "take": 50}
        )
        file_infos.append(
            {
                "path": "skip_all",
        ...
        file_infos.append(
            {"path": "take_10", "num_elements": 100, "skip": 90, "take": 20}
        )
        result = shuffle_ops._process_file_infos(file_infos)
        self.assertEqual(
            result["files"], ["take_50", "skip_all", "take_all", "take_10"]
        )
        ...
        for i, expected in enumerate(expected):
            self.assertEqual(
                self.evaluate(
                    shuffle_ops._adjust_index(
                        [inputs[i]], result["thresholds"], result["offsets"]
                    )
                ),
                expected,
            )
...
class IndexShuffleCheckpointTest(
    ...
    def _build_dataset(
        ...
        def reader_factory(files):
            return dataset_ops.Dataset.range(
                num_elements_per_file * array_ops.shape(files, out_type=dtypes.int64)[0]
            )
