...
class AutoShardDatasetTest(
    tf_record_test_base.TFRecordTestBase, parameterized.TestCase
):
    def setUp(self):
    ...
    def assertDatasetProducesWithShuffle(
        self, dataset, expected, batch, num_examples, shuffle
    ):
        if shuffle:
            actual = []
            next_fn = self.getNext(dataset)
    ...
    def testValidPipelineWithRangeDataset(self, shuffle):
        ...
        dataset = dataset.map(
            lambda n: string_ops.string_join(  # pylint:disable=g-long-lambda
                [
                    self.get_temp_dir(),
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(
                params=[(1, 0, 10, 10), (2, 1, 20, 5), (10, 1, 1, 10)]
            ),
        )
    )
    ...
    def testReplicateAndShardProduceDisjointData(self, shuffle, sharding_policy):
        ...
        ds1 = distribute._RemoteDataset(
            graph_def, "/device:CPU:0", dataset.element_spec
        )
        ds2 = distribute._RemoteDataset(
            graph_def, "/device:CPU:0", dataset.element_spec
        )
    ...
    def testMakeBatchedFeaturesDataset(self):
        ...
        for file_index in range(files):
            filename = os.path.join(
                self.get_temp_dir(), "tf_record.%d.txt" % file_index
            )
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(auto_shard_policy=list(options_lib.AutoShardPolicy)),
        )
    )
...
class AutoShardWithRebatchDatasetTest(
    ...
    def testFileShardingWithRebatch(self):
        ...
        expected = [
            [self._record(1, 0), self._record(1, 1)],
            [self._record(1, 2)],
    ...
    def testUseLegacyRebatchWithDataSharding(self, sharding_policy, with_prefetch):
        ...
        # of the dataset.
        worker_a_dataset = distribute._RebatchDataset(dataset, batch_sizes=[2, 1, 1])
        if with_prefetch:
        ...
        worker_b_dataset = distribute._RebatchDataset(dataset, batch_sizes=[1, 1, 2])
        if with_prefetch:
        ...
        worker_c_dataset = distribute._RebatchDataset(dataset, batch_sizes=[1, 2, 1])
        if with_prefetch:
...
class AutoShardDatasetCheckpointTest(
