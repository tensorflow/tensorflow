...
class MapDefunTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testNoIntraOpLimit(self):
        ...
        r = map_defun.map_defun(
            simple_fn, [elems], [dtypes.int32], [(2,)], max_intra_op_parallelism=0
        )[0]
        expected = elems * 2 + 3
    ...
    def testMapDefunMultipleOutputs(self):
        ...
        elems = constant_op.constant(nums, dtype=dtypes.int32, name="data")
        r = map_defun.map_defun(
            fn, [elems], [dtypes.int32, dtypes.float64], [(2,), (2,)]
        )
    ...
    def testMapDefunRaisesErrorOnRuntimeShapeMismatch(self):
        ...
        result = map_defun.map_defun(
            fn, [elems1, elems2], [dtypes.int32, dtypes.int32], [(), ()]
        )
    ...
    def testMapDefunCancelledCorrectly(self):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, r"indices = 10 is not in \[0, 5\)"
        ):
    ...
    def testMapDefunWithDifferentOutputShapeEachRun(self):
        @function.defun(input_signature=[tensor_spec.TensorSpec(None, dtypes.int32)])
        def simple_fn(x):
            return x * 2 + 3
    ...
    def testMapDefunWithVariantTensor(self):
        @function.defun(input_signature=[tensor_spec.TensorSpec([], dtypes.variant)])
        def fn(x):
            return x
        ...
        map_defun_op = map_defun.map_defun(fn, [serialized], [dtypes.variant], [None])[
            0
        ]
