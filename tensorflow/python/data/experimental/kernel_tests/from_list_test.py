...
class FromListTest(test_base.DatasetTestBase, parameterized.TestCase):
            ...
            combinations.combine(
                map_fn=[
                    ...
                    combinations.NamedObject(
                        "ordered_dicts", lambda x: collections.OrderedDict([("x", x)])
                    ),
                ]
            ),
        )
    ...
    def testNonRectangularInputs(self):
        ...
        dataset = from_list.from_list(elements)
        self.assertEqual(
            tensor_shape.Dimension(1), dataset_ops.get_legacy_output_shapes(dataset)[0]
        )
        self.assertDatasetProduces(dataset, expected_output=elements)
    ...
    def testDictInputs(self):
        elements = [
            {"foo": [1, 2, 3], "bar": [[4.0], [5.0], [6.0]]},
            {"foo": [4, 5, 6], "bar": [[7.0], [8.0], [9.0]]},
        ]
        dataset = from_list.from_list(elements)
        self.assertEqual(
...
class FromListCheckpointTest(
    ...
    def test(self, verify_fn, symbolic_checkpoint):
        ...
        verify_fn(
            self, lambda: self._build_list_dataset(elements, options), num_outputs=3
        )
...
class FromListGlobalShuffleTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(
