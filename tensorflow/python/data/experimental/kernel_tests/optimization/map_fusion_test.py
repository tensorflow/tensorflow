...
class MapFusionTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(
        combinations.times(
            ...
            _test_combinations(),
            combinations.combine(num_parallel_calls=[None, 2, dataset_ops.AUTOTUNE]),
    ...
    def testMixedMapAndParallelMap(self):
        ...
        dataset = dataset.map(lambda x: 2 * x, num_parallel_calls=dataset_ops.AUTOTUNE)
    ...
    def testStatefulness(self):
        def f(x):
            return control_flow_ops.with_dependencies([check_ops.assert_negative(x)], x)
        ...
        options.experimental_optimization.map_fusion = True
        dataset = dataset.with_options(options)
        with self.assertRaisesRegex(errors.InvalidArgumentError, "assertion failed"):
