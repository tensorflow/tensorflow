...
class MakeDeterministicTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(
                use_function=[False, True], use_legacy_map_and_batch=[False, True]
            ),
        )
    )
    ...
    def test_text_line_dataset(self, use_function, use_control_flow):
        ...
        with test_util.deterministic_ops():
            ...
            self.assertDatasetProduces(
                dataset, expected_output=["1", "4", "7", "2", "5", "8", "3", "6", "9"]
            )
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(
                local_determinism=[None, True, False], global_determinism=[True, False]
            ),
        )
    )
