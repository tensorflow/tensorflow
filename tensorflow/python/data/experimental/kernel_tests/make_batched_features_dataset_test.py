...
class MakeBatchedFeaturesDatasetTest(
    tf_record_test_base.FeaturesTestBase, parameterized.TestCase
):
    ...
    def testRead(self, batch_size, num_epochs):
        ...
        self.outputs = self.getNext(
            self.make_batch_feature(
                filenames=self._filenames, num_epochs=num_epochs, batch_size=batch_size
            )
        )
    ...
    def testParallelReadersAndParsers(
        self, batch_size, num_epochs, reader_num_threads, parser_num_threads
    ):
        self.outputs = self.getNext(
            self.make_batch_feature(
                filenames=self._filenames,
