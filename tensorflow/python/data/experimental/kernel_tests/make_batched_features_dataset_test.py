...
class MakeBatchedFeaturesDatasetTest(
  ...
  def testReadWithEquivalentDataset(self):
    ...
    for file_batch, _, _, _, record_batch, _ in self._next_expected_batch(
      ...
      actual_batch = self.evaluate(get_next())
  ...
  def testReadWithFusedShuffleRepeatDataset(self):
    ...
    for batch_size in [1, 2]:
      ...
      next1 = self.getNext(self.make_batch_feature(
          filenames=self.test_filenames[0],
          num_epochs=num_epochs,
          batch_size=batch_size,
          shuffle=True,
          shuffle_seed=5))
      next2 = self.getNext(self.make_batch_feature(
          filenames=self.test_filenames[0],
          num_epochs=num_epochs,
          batch_size=batch_size,
          shuffle=True,
      ...
      for _ in range(total_records // batch_size):
        batch1 = self._run_actual_batch(next1)
        batch2 = self._run_actual_batch(next2)
  ...
  def testDropFinalBatch(self):
    for batch_size in [1, 2]:
      for num_epochs in [1, 10]:
        # Basic test: read from file 0.
        get_next = self.getNext(self.make_batch_feature(
            filenames=self.test_filenames[0],
            label_key="label",
            num_epochs=num_epochs,
            batch_size=batch_size,
            drop_final_batch=True))
        outputs = self.evaluate(get_next())
