...
class DatasetInitializerTest(test.TestCase):
    ...
    def test_from_multiple_files(self):
        vocabulary_file1 = self._createVocabFile("test1.txt", ("one", "two", "three"))
        vocabulary_file2 = self._createVocabFile("test2.txt", ("four", "five", "six"))
    ...
    def test_index_table_from_dataset(self):
        ds = dataset_ops.Dataset.range(100).map(lambda x: string_ops.as_string(x * 2))
