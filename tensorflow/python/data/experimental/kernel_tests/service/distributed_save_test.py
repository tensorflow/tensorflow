...
class DistributedSaveTest(data_service_test_base.TestBase, parameterized.TestCase):
    ...
    def testCompression(self, compression):
        ...
        dataset = dataset_ops.Dataset.load(snapshot_dir.full_path)
        self.assertDatasetProduces(dataset, list(range(10)), assert_items_equal=True)
    ...
    def testWriteMultipleDatasets(self, num_workers):
        ...
        self.assertDatasetProduces(dataset1, list(range(100)), assert_items_equal=True)
        self.assertDatasetProduces(
    ...
    def testLoadWithCustomReaderFunc(self, num_workers, snapshot_max_chunk_size_bytes):
    ...
    def testDistributedLoad(self, num_workers, repeated_load, sharding_policy):
        ...
        dataset = dataset.apply(
            data_service_ops.distribute(
                processing_mode=sharding_policy, service=cluster.dispatcher_address()
            )
        )
    ...
    def testBadCardinality(self):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "Saving an infinite dataset is not allowed"
        ):
            self.evaluate(
                distributed_save_op.distributed_save(
                    dataset, snapshot_dir.full_path, cluster.dispatcher_address()
    ...
    def testBadElementSpec(self):
        ...
        with self.assertRaisesRegex(
            ...
            _ = dataset_ops.Dataset.load(
                snapshot_dir.full_path, element_spec="bad_element_spec"
            )
    ...
    def testBadCompression(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "User specified compression ZLIB, but the actual compression is SNAPPY.",
        ):
