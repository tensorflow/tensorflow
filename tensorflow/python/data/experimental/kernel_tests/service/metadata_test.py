...
def _cardinality_test_combinations():
    ...
    def _infinite_dataset_with_hint_shard():
        return (
            ...
            .shard(distribute.SHARD_HINT, distribute.SHARD_HINT)
            .repeat()
        )

    def _empty_dataset_with_hint_shard():
        return dataset_ops.Dataset.range(0).shard(
            distribute.SHARD_HINT, distribute.SHARD_HINT
        )
    ...
    v2_only_cases = [
        ...
        (
            "DynamicShardingInfinite",
            lambda: dataset_ops.Dataset.range(5).repeat(),
            data_service_ops.ShardingPolicy.DYNAMIC,
            dataset_ops.INFINITE,
        ),
        (
            "DataShardingInfinite",
            lambda: dataset_ops.Dataset.range(10).repeat(),
        ...
        (
            "HintShardingZero",
            _empty_dataset_with_hint_shard,
            data_service_ops.ShardingPolicy.HINT,
            dataset_ops.UNKNOWN,
        ),
    ]
    v1_and_v2_cases = [
        (
            "Finite",
            lambda: dataset_ops.Dataset.range(10),
            data_service_ops.ShardingPolicy.OFF,
            dataset_ops.UNKNOWN,
        ),
        (
        ...
        (
            "HintShardingUnknown",
            _infinite_dataset_with_hint_shard,
            data_service_ops.ShardingPolicy.HINT,
            dataset_ops.UNKNOWN,
        ),
    ]
...
class DataServiceMetadataTest(data_service_test_base.TestBase, parameterized.TestCase):
    ...
    @combinations.generate(_cardinality_test_combinations())
    def testFromDatasetIdCardinality(
        self, dataset_fn, sharding_policy, expected_result
    ):
        cluster = data_service_test_base.TestCluster(num_workers=2)
        dataset = dataset_fn()
