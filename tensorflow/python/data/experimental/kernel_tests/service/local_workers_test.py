...
class LocalWorkersTest(data_service_test_base.TestBase, parameterized.TestCase):
    ...
    def testMultipleConsumers(self):
        ...
        self.assertCountEqual(
            results,
            (num_local_workers + num_remote_workers) * list(range(num_elements)),
        )
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(num_local_workers=[0, 3], num_remote_workers=[1, 3]),
        )
    )
    ...
    def testNoLocalWorker(self):
        ...
        with self.assertRaisesRegex(
            ...
            "Local reads require local tf.data workers, but no local worker is found.",
        ):
...
class LocalTaskGarbageCollectTest(
    ...
    def testReadFromDeletedTask(self, num_remote_workers):
        ...
        with self.assertRaisesRegex(
            errors.FailedPreconditionError, "which has been deleted."
        ):
            get_next = self.getNext(dataset)
            while True:
