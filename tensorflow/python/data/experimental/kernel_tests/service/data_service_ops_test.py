...
class DataServiceOpsTest(data_service_test_base.TestBase, parameterized.TestCase):
    ...
    def testDontGcUsedJob(self):
        ...
        it3 = (
            iter(  # this iterator keeps the task alive. pylint: disable=unused-variable
    ...
    def testFromDatasetId(self):
        ...
        from_dataset_id_ds = self.from_dataset_id(
            "parallel_epochs", cluster, dataset_id, ds.element_spec
        )
    ...
    def testFromDatasetIdWrongElementSpec(self):
        ...
        from_dataset_id_ds = self.from_dataset_id(
            "parallel_epochs", cluster, dataset_id, wrong_spec
        )

        if data_service_test_base.TRANSFER_PROTOCOL.value:
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "Data type mismatch at component 0"
            ):
    ...
    def testFromDatasetIdNotRegistered(self):
        ...
        with self.assertRaisesRegex(errors.NotFoundError, "Dataset id 0 not found"):
            from_dataset_id_ds = self.from_dataset_id(
                "parallel_epochs", cluster, dataset_id, element_spec
            )
            self.evaluate(self.getNext(from_dataset_id_ds)())
