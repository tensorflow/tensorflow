...
from tensorflow.python.data.experimental.kernel_tests.service.multi_process_cluster import (
    MultiProcessCluster,
)
...
def _make_service_cluster(num_workers, local_shard_index, worker_addresses=None):
    if worker_addresses is None:
        worker_addresses = ["localhost" for _ in range(num_workers)]

    cluster = MultiProcessCluster(
        num_local_workers=0, num_remote_workers=0, worker_addresses=worker_addresses
    )
...
class AutoShardTest(
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(
                sharding_policy=[ShardingPolicy.DATA, ShardingPolicy.FILE_OR_DATA]
            ),
        )
    )
    ...
    def testRangeDataset_FileShard(self):
        ...
        with self.assertRaisesRegex(
            errors.NotFoundError, "Found an unshardable source dataset"
        ):
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(
                sharding_policy=[ShardingPolicy.FILE_OR_DATA, ShardingPolicy.FILE]
            ),
        )
    )
    ...
    def testTFRecordDataset_FewerFilesThanWorkers(self, sharding_policy):
        ...
        with self.assertRaisesRegex(
            errors.InvalidArgumentError, "not enough for the required 5 shards/workers."
        ):
            self.getDatasetOutput(dataset)
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(
                sharding_policy=[ShardingPolicy.FILE_OR_DATA, ShardingPolicy.DATA]
            ),
        )
    )
    ...
    def testInvalidPorts(self):
        with self.assertRaisesRegex(
            RuntimeError, "The worker's address is not configured"
        ):
    ...
    def testEmptyWorkerList(self):
        ...
        with self.assertRaisesRegex(
            errors.NotFoundError, "Worker .* is not in the workers list."
        ):
            self.getDatasetOutput(dataset)
    ...
    def testMoreWorkersThanConfigured(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "other workers are already running at the configured host"
        ):
            _ = _make_service_cluster(
