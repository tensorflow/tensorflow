...
class CoordinatedReadTest(data_service_test_base.TestBase, parameterized.TestCase):
    ...
    def testConsumerRestart(self):
        ...
        with self.assertRaisesRegex(
            errors.FailedPreconditionError, "current round has already reached"
        ):
    ...
    def testBucketizing(self):
        ...
        ds = ds.apply(
            grouping.bucket_by_sequence_length(
                lambda x: x, bucket_boundaries, bucket_batch_sizes, drop_remainder=True
            )
        )
        ds = ds.apply(
        ...
        def get_bucket(elem):
            bucket_ind = 0
            while (
                bucket_ind < len(bucket_boundaries)
