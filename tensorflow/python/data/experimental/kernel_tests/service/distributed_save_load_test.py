...
class DistributedSaveLoadTest(data_service_test_base.TestBase, parameterized.TestCase):
    """Tests for distributed save/load with the new load algorithm."""
    ...
    def test_save_load(
        ...
        cluster = data_service_test_base.TestCluster(
            num_workers=num_workers, snapshot_max_chunk_size_bytes=max_chunk_size_bytes
        )
    ...
    def test_deterministic_load_order(
        ...
        output_per_repetition = [
            output[i : i + num_elements] for i in range(0, len(output), num_elements)
        ]
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(num_elements=[20], num_repetitions=[10]),
        )
    )
    def test_shuffle_chunks(self, num_elements: int, num_repetitions: int):
        cluster = data_service_test_base.TestCluster(
        ...
        for i in range(1, num_repetitions):
            self.assertNotEqual(output_per_repetition[i], output_per_repetition[i - 1])
    ...
    def test_distributed_load(
        ...
        dataset = dataset.apply(
            data_service_ops.distribute(sharding_policy, cluster.dispatcher_address())
        )
    ...
    def test_save_before_sample(self, num_workers: int):
        ...
        dataset = dataset_ops.Dataset.sample_from_datasets(
            loaded_datasets, weights=[1.0] * num_datasets, stop_on_empty_dataset=False
        )
        self.assertDatasetProduces(
            dataset, list(range(num_elements)) * num_datasets, assert_items_equal=True
        )
    ...
    def test_dataset_spec_file_is_optional(self, num_elements: int):
        ...
        # the same output.
        os.remove(
            os.path.join(snapshot_dir.full_path, dataset_ops.DATASET_SPEC_FILENAME)
        )
        dataset = dataset_ops.Dataset.load(snapshot_dir.full_path, wait=True)
        self.assertDatasetProduces(
            dataset, list(range(num_elements)), assert_items_equal=True
...
class SaveLoadCheckpointTest(
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(
                num_workers=[1, 3], num_elements=[0, 10], num_repetitions=[1, 5]
            ),
        )
    )
