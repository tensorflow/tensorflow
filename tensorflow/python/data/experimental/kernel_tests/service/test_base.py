...
class TestWorker:
    ...
    def restart(self, use_same_port=True):
        ...
        self._server = _make_worker(
            ...
            self._shutdown_quiet_period_ms,
            port,
        )
        self._server.start()
        self._port = int(self._server._address.split(":")[1])
    ...
    def restart_dispatcher(self):
        ...
        self.dispatcher = server_lib.DispatchServer(
            server_lib.DispatcherConfig(
                ...
                worker_max_concurrent_snapshots=self._worker_max_concurrent_snapshots,
            )
        )
...
class TestBase(test_base.DatasetTestBase):
    ...
    def make_distributed_dataset(
        self, dataset, cluster, processing_mode="parallel_epochs", **kwargs
    ):
        ...
        return dataset.apply(
            data_service_ops._distribute(
                processing_mode, cluster.dispatcher_address(), **kwargs
            )
        )
    ...
    def make_coordinated_read_dataset(
        ...
        ds = ds.interleave(
            lambda x: x, cycle_length=num_consumers, num_parallel_calls=num_consumers
        )
        return ds
...
class TempDir:
    ...
    def __init__(self):
        ...
        self._path = os.path.join(tempfile.mkdtemp(dir=temp_dir), "tf_data_snapshot")
