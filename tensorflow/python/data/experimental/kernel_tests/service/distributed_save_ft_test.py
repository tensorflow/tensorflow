...
def get_stream_assignment(cluster, worker_idx, path, block=True, active_only=False):
    while True:
        for progress in cluster.workers[worker_idx].snapshot_task_progresses():
            if progress.snapshot_task_base_path.decode() == path and not (
                active_only and progress.completed
            ):
...
def wait_for_snapshot(paths, f=lambda: None):
    if isinstance(paths, str):
        paths = [paths]
    while not all(
        [snapshot_is_done(path) or snapshot_has_error(path) for path in paths]
    ):
        f()
        time.sleep(0.1)
...
class SnapshotFtTest(data_service_test_base.TestBase, parameterized.TestCase):
    ...
    def testSnapshotRecoveryFailsWithBadSourceName(self, bad_source_dir_name):
        ...
        os.makedirs(
            os.path.join(self._splits_dir(snapshot_dir.full_path), bad_source_dir_name)
        )
        with self.assertRaisesRegex(RuntimeError, "Can't parse"):
            cluster.restart_dispatcher()
    ...
    def testSnapshotRecoveryFailsWithOutOfBoundsSourceName(self):
        ...
        os.makedirs(os.path.join(self._splits_dir(snapshot_dir.full_path), "source_1"))
    ...
    def testSnapshotRecoveryFailsWithBadSplitNames(self, bad_split_filename):
        ...
        write_file(
            os.path.join(self._source_dir(snapshot_dir.full_path), bad_split_filename)
        )
        with self.assertRaisesRegex(
    ...
    def testSnapshotRecoveryFailsWithOutOfOrderSplitName(self):
        ...
        write_file(os.path.join(self._source_dir(snapshot_dir.full_path), "split_1_0"))
        with self.assertRaisesRegex(
    ...
    def testSnapshotRecoveryFailsWithMissingGlobalIndexInSplitNames(self):
        ...
        write_file(os.path.join(self._source_dir(snapshot_dir.full_path), "split_0_1"))
    ...
    def testSnapshotRecoveryFailsWithDuplicateGlobalIndexInSplitName(self):
        ...
        write_file(
            os.path.join(
                self._source_dir(snapshot_dir.full_path, stream_idx=1, worker=1),
                "split_0_1",
            )
        )
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(worker_max_concurrent_snapshots=[1, 2]),
        )
    )
    def testWorkersDontExceedMaxStreamAssignments(
        ...
        def get_assignments_and_update_max_assignments():
            ...
            for worker_idx, worker_assignments in assignments.items():
                max_assignments[worker_idx] = max(
                    max_assignments[worker_idx], len(worker_assignments)
                )
            return assignments
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(
                num_elements=[1, 2, 1000], num_workers=[1, 3], num_repetitions=[1, 10]
            ),
        )
    )
    ...
    def testMultipleDatasetRecoversAndCompletes(self, num_workers):
        ...
        self.assertDatasetProduces(dataset, list(range(1000)), assert_items_equal=True)
