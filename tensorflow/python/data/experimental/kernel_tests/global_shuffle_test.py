...
class GlobalShuffleTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testRange(
        ...
        output_per_iteration = [
            output[i : i + dataset_range] for i in range(0, len(output), dataset_range)
        ]
    ...
    def testNegativeRange(self, dataset_range: int, seed: Optional[int]):
        ...
        self.assertCountEqual(
            output_per_iteration[0], list(range(dataset_range, -dataset_range, -1))
        )
        self.assertCountEqual(
            output_per_iteration[1], list(range(dataset_range, -dataset_range, -1))
        )
        self.assertCountEqual(
            output_per_iteration[2], list(range(dataset_range, -dataset_range, -1))
        )
        if dataset_range > 1:
            self.assertNotEqual(output_per_iteration[0], output_per_iteration[1])
            self.assertNotEqual(output_per_iteration[0], output_per_iteration[2])
...
class GlobalShuffleCheckpointTest(
