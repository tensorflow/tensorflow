...
class AutotuneBenchmark(benchmark_base.DatasetBenchmarkBase):
    ...
    def _run_benchmark(
        self, dataset, autotune, benchmark_iters, benchmark_label, benchmark_id
    ):
        ...
        wall_time = self.run_and_report_benchmark(
            ...
            iters=1,
            extras={
                "model_name": "autotune.benchmark.%s.%d"
                % (benchmark_label, benchmark_id),
                "parameters": "%s" % autotune,
            },
    ...
    def _benchmark_batch(self, autotune, benchmark_id):
        ...
        dataset = dataset_ops.Dataset.from_tensors(
            (np.random.rand(1, 4 * k), np.random.rand(4 * k, 1))
        ).repeat()
