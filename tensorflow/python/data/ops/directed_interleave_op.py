        ...
        for i, data_input in enumerate(self._data_inputs[1:]):
            ...
            try:
                spec = nest.map_structure(
                    common_supertype, spec, data_input.element_spec
                )
            except (TypeError, ValueError) as e:
                raise TypeError(
                    f"Invalid `datasets`. `datasets` must have compatible "
                    f"element specs.\n Dataset 0 "
                    f"element_spec={data_inputs[0].element_spec}.\n"
                    f"Dataset {i + 1} "
                    f"element_spec={data_input.element_spec}."
                ) from e
        self._element_spec = spec

        # pylint: disable=protected-access
        variant_tensor = ged_ops.directed_interleave_dataset(
