...
class IteratorType(trace.TraceType):
    ...
    def __eq__(self, other) -> bool:
        return isinstance(other, IteratorType) and self._components == other._components


def use_anonymous_iterator_v3():
    return (
        forward_compat.forward_compatible(2021, 12, 14)
        or context.run_eager_op_as_function_enabled()
    )
...
class OwnedIterator(IteratorBase):
    ...
    def __tf_tracing_type__(self, tracing_context):
        return IteratorType(
            self._type_spec, tracing_context.get_local_id(self._iterator_resource._id)
        )  # pylint:disable=protected-access
