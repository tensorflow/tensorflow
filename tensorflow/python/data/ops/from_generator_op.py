...
def _from_generator(
    generator, output_types, output_shapes, args, output_signature, name
):
    ...
    if output_signature is not None:
        if output_types is not None:
            raise TypeError(
                "The `output_types` argument can not be used together "
                "with the `output_signature` argument."
            )
        if output_shapes is not None:
            raise TypeError(
                "The `output_shapes` argument can not be used together "
                "with the `output_signature` argument."
            )
        for spec in nest.flatten(output_signature):
            if not isinstance(spec, type_spec.TypeSpec):
                raise TypeError(
                    f"`output_signature` must contain objects that are "
        ...
        if output_types is None:
            raise TypeError(
                "To specify the output signature you need to provide "
    ...
    if output_signature is None:
        ...
        output_signature = nest.map_structure_up_to(
            output_types, tensor_spec.TensorSpec, output_shapes, output_types
        )
    if all(
        isinstance(x, tensor_spec.TensorSpec) for x in nest.flatten(output_signature)
    ):
...
class _GeneratorDataset(dataset_ops.DatasetSource):
    ...
    def __init__(
        ...
        self._init_func = structured_function.StructuredFunctionWrapper(
            init_func, self._transformation_name(), input_structure=self._init_structure
        )
