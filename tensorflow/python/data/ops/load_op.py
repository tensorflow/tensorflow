  ...
  while True:
    try:
      dataset = dataset_ops.Dataset.load(
          ...
          wait=False)
      if isinstance(dataset, _LoadDataset):
        raise ValueError(
            "Reading an ongoing snapshot with `wait=True` is only supported "
            "for distributed tf.data snapshot. To fix this, set `wait` to "
            "`False`, or use `distributed_save` to write the snapshot.")
