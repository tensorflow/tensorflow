...
parsing_ops = lazy_loader.LazyLoader(
    "parsing_ops", globals(), "tensorflow.python.ops.parsing_ops"
)
...
def _validate_and_encode(name):
    if not name.isidentifier():
        raise ValueError(
            "Invalid `name`. The argument `name` needs to be a valid "
    ...
    **Component**: The leaf in the nested structure of an element.
            ...
            if node.name in asset_tracker:
                ...
                with context.eager_mode(), ops.device("CPU"):
                    node_value = parsing_ops.parse_tensor(
                        tensor_proto.SerializeToString(), dtypes.string
                    ).numpy()
    ...
    def __iter__(self):
        ...
        if context.executing_eagerly() or ops.inside_function():
            ...
            raise RuntimeError(
                "`tf.data.Dataset` only supports Python-style "
    ...
    def __len__(self):
        ...
        if not context.executing_eagerly():
            raise TypeError(
                "`tf.data.Dataset` only supports `len` in eager mode. "
    ...
    def __debug_string__(self):
        ...
        while to_process:
            ...
            to_process.extend(
                [(ds, depth + 1) for ds in dataset._inputs()]
            )  # pylint: disable=protected-access
        return "\n".join(lines)
    ...
    def as_numpy_iterator(self):
        ...
        if not context.executing_eagerly():
            raise RuntimeError(
                "`tf.data.Dataset.as_numpy_iterator()` is only "
    ...
    def enumerate(self, start=0, name=None):
        ...
        return Dataset.zip(
            (Dataset.range(start, max_value, name=name), self), name=name
        )
    ...
    def padded_batch(
        ...
        * If the dimension is unknown, the component will be padded out to the
        ...
        return PaddedBatchDataset(
            self, batch_size, padded_shapes, padding_values, drop_remainder, name=name
        )
...
name=None))
    ...
    def get_single_element(self, name=None):
        ...
        return structure.from_compatible_tensor_list(
            self.element_spec,
            gen_dataset_ops.dataset_to_single_element(self._variant_tensor, **kwargs),
        )  # pylint: disable=protected-access
    ...
    def group_by_window(
        self, key_func, reduce_func, window_size=None, window_size_func=None, name=None
    ):
        ...
        if (
            ...
            raise ValueError(
                "Either the `window_size` argument or the "
    ...
    def snapshot(
        self, path, compression="AUTO", reader_func=None, shard_func=None, name=None
    ):
    ...
    def rejection_resample(
        ...
        if initial_dist is not None:
            ...
            initial_dist_ds = DatasetV2.from_tensors(initial_dist_t, name=name).repeat(
                name=name
            )
            acceptance_dist_ds = DatasetV2.from_tensors(
                acceptance_dist, name=name
            ).repeat(name=name)
            prob_of_original_ds = DatasetV2.from_tensors(
                prob_of_original, name=name
            ).repeat(name=name)
        else:
    ...
    def choose_from_datasets(datasets, choice_dataset, stop_on_empty_dataset=True):
        ...
        if not isinstance(choice_dataset, DatasetV2):
            raise TypeError(
                f"Invalid `choice_dataset`. `choice_dataset` should be a "
        ...
        if not structure.are_compatible(
            ...
            raise TypeError(
                f"Invalid `choice_dataset`. Elements of `choice_dataset` "
...
class DatasetV1(DatasetV2):
    ...
    def _make_initializable_iterator(
        ...
        if context.executing_eagerly():
            raise RuntimeError(
                "`make_initializable_iterator()` is not supported in "
    ...
    def batch(
        ...
        return DatasetV1Adapter(
            super(DatasetV1, self).batch(
                batch_size, drop_remainder, num_parallel_calls, deterministic, name=name
            )
        )
    ...
    def padded_batch(
        ...
        return DatasetV1Adapter(
            super(DatasetV1, self).padded_batch(
                batch_size, padded_shapes, padding_values, drop_remainder, name=name
            )
        )
    ...
    def window(self, size, shift=None, stride=1, drop_remainder=False, name=None):
        return DatasetV1Adapter(
            super(DatasetV1, self).window(
                size, shift, stride, drop_remainder, name=name
            )
        )
...
def get_structure(dataset_or_iterator):
    ...
    try:
        ...
        raise TypeError(
            f"Invalid `dataset_or_iterator`. `dataset_or_iterator` "
...
class TensorSliceDataset(DatasetSource):
    ...
    def __init__(self, element, is_files=False, name=None):
        ...
        kwargs = {"output_shapes": structure.get_flat_tensor_shapes(self._structure)}
        if compat.forward_compatible(2021, 9, 20):
            kwargs["is_files"] = is_files
        if name or compat.forward_compatible(2021, 9, 30):
            kwargs["metadata"] = self._metadata.SerializeToString()
        variant_tensor = gen_dataset_ops.tensor_slice_dataset(self._tensors, **kwargs)
        ...
        for ds in nest.flatten(datasets):
            if not isinstance(ds, DatasetV2):
                if isinstance(ds, list):
                    raise TypeError(
                        "Invalid `datasets`. `datasets` is expected to be a "
                    ...
                    raise TypeError(
                        f"Invalid `datasets`. `datasets` is expected to be a "
...
class RangeDataset(DatasetSource):
    ...
    def _parse_args(self, *args, **kwargs):
        ...
        if len(args) == 1:
            ...
            raise ValueError(
                f"Invalid `args`. The lenght of `args` should be "
    ...
    def __init__(self, input_dataset, options, name=None):
        ...
        with ops.colocate_with(input_dataset._variant_tensor):
            variant_tensor = gen_dataset_ops.options_dataset(
                input_dataset._variant_tensor, options_pb.SerializeToString(), **kwargs
            )
...
def normalize_to_dense(dataset):
    ...
    if structured_function._should_unpack(
        dataset.element_spec
    ):  # pylint: disable=protected-access
    ...
    def __init__(self, input_dataset, name=None):
        ...
        for s in flat_shapes:
            try:
                ...
                raise ValueError(
                    f"`unbatch()` is only supported for datasets of "
...
class _GroupByWindowDataset(UnaryDataset):
    ...
    def __init__(
        self, input_dataset, key_func, reduce_func, window_size_func, name=None
    ):
    ...
    def _make_window_size_func(self, window_size_func):
        ...
        if not self._window_size_func.output_structure.is_compatible_with(
            ...
            raise ValueError(
                f"Invalid `window_size_func`. `window_size_func` must "
    ...
    def _make_key_func(self, key_func, input_dataset):
        ...
        if not self._key_func.output_structure.is_compatible_with(
            ...
            raise ValueError(
                f"Invalid `key_func`. `key_func` must return a single "
    ...
    def _make_reduce_func(self, reduce_func, input_dataset):
        ...
        if not isinstance(self._reduce_func.output_structure, DatasetSpec):
            raise TypeError(
                f"Invalid `reduce_func`. `reduce_func` must return a "
...
def _filter_ds(
    dataset, acceptance_dist_ds, initial_dist_ds, class_func, seed, name=None
):
    ...
    acceptance_dist_ds = DatasetV2.zip(
        (acceptance_dist_ds, initial_dist_ds), name=name
    ).map(maybe_warn_on_large_rejection, name=name)
    ...
    current_probabilities_and_class_and_data_ds = DatasetV2.zip(
        (acceptance_dist_ds, dataset), name=name
    ).map(_gather_and_copy, name=name)
...
def _estimate_initial_dist_ds(
    ...
    initial_dist_ds = (
        class_values_ds.batch(dist_estimation_batch_size, name=name)
        .scan(initial_examples_per_class_seen, update_estimate_and_tile, name=name)
        .unbatch(name=name)
    )
...
class _SnapshotDataset(UnaryUnchangedStructureDataset):
    ...
    def __init__(
        ...
        if (
            ...
            raise TypeError(
                f"Invalid `shard_func`. `shard_func` must return "
                f"`tf.int64` scalar tensor but its return type is "
    ...
    def __init__(
        ...
        while need_to_rerun:
            ...
            if not (
                ...
                raise TypeError(
                    f"Invalid `scan_func`. `scan_func` should return a "
            ...
            for new_state_class, old_state_class in zip(
                ...
                if not issubclass(new_state_class, old_state_class):
                    raise TypeError(
                        f"Invalid `scan_func`. The element classes for the "
            ...
            for new_state_type, old_state_type in zip(
                ...
                if new_state_type != old_state_type:
                    raise TypeError(
                        f"Invalid `scan_func`. The element types for the "
                        f"new state must match the initial state. Expected "
...
class _DirectedInterleaveDataset(DatasetV2):
    ...
    def __init__(self, selector_input, data_inputs, stop_on_empty_dataset=False):
        ...
        for i, data_input in enumerate(data_inputs[1:]):
            if (
                get_legacy_output_types(data_input) != first_output_types
                or get_legacy_output_classes(data_input) != first_output_classes
            ):
                raise TypeError(
                    f"Invalid `datasets`. `datasets` must have the same "
                    f"type and class.\n Dataset 0 "
