...
prefetch_op = lazy_loader.LazyLoader(
    "prefetch_op", globals(), "tensorflow.python.data.ops.prefetch_op"
)
# Loaded lazily due to a circular dependency (dataset_ops ->
# shuffle_op -> dataset_ops).
shuffle_op = lazy_loader.LazyLoader(
    "shuffle_op", globals(), "tensorflow.python.data.ops.shuffle_op"
)
...
class DatasetV2(
    ...
    **Component**: The leaf in the nested structure of an element.
            ...
            if node.name in asset_tracker:
                ...
                asset_tracker[node.name] = [
                    self._track_trackable(
                        asset.Asset(n), name=node.name + "_" + str(i), overwrite=True
                    )
    ...
    def _trackable_children(
        self, save_type=tracking_base.SaveType.CHECKPOINT, **kwargs
    ):
    ...
    def as_numpy_iterator(self):
        ...
        if not context.executing_eagerly():
            raise RuntimeError(
                "`tf.data.Dataset.as_numpy_iterator()` is only supported in eager mode."
            )
        for component_spec in nest.flatten(self.element_spec):
            if not isinstance(
                ...
                (
                    ...
                    sparse_tensor_lib.SparseTensorSpec,
    ...
    def from_generator(
        ...
        return from_generator_op._from_generator(
    ...
    def batch(
        ...
        return batch_op._batch(
            self, batch_size, drop_remainder, num_parallel_calls, deterministic, name
        )
        # pylint: enable=g-import-not-at-top,protected-access,redefined-outer-name
    ...
    def padded_batch(
        ...
        * If the dimension is unknown, the component will be padded out to the
        ...
        return padded_batch_op._padded_batch(
            self, batch_size, padded_shapes, padding_values, drop_remainder, name
        )
        # pylint: enable=g-import-not-at-top,protected-access
    ...
    def ragged_batch(
        ...
        *  If an input element is a `tf.RaggedTensor` or any other type, then it is
        ...
        return ragged_batch_op._ragged_batch(
            self, batch_size, drop_remainder, row_splits_dtype, name
        )
        # pylint: enable=g-import-not-at-top,protected-access
...
name=None))
    ...
    def interleave(
        ...
        return interleave_op._interleave(
            self,
            map_func,
            cycle_length,
            block_length,
...
class DatasetV1(DatasetV2, data_types.DatasetV1):
    ...
    def _make_initializable_iterator(self, shared_name=None) -> iterator_ops.Iterator:  # pylint: disable=missing-docstring
        if context.executing_eagerly():
            raise RuntimeError(
...
class DatasetSpec(type_spec.BatchableTypeSpec):
    ...
    def most_specific_common_supertype(self, others):
        ...
        for i, self_component in enumerate(self_components):
            ...
            if self_component is not None and common_components[i] is None:
                return None
        common_element_spec = tf_nest.pack_sequence_as(
            self._element_spec, common_components
        )

        common_dataset_shape = self._dataset_shape.most_specific_common_supertype(
            [other._dataset_shape for other in others]
        )  # pylint: disable=protected-access
    ...
    def __eq__(self, other):
        return (
            isinstance(other, DatasetSpec)
            and self._element_spec == other._element_spec
            and self._dataset_shape == other._dataset_shape
        )
...
batch_op = lazy_loader.LazyLoader(
    "batch_op", globals(), "tensorflow.python.data.ops.batch_op"
)
...
repeat_op = lazy_loader.LazyLoader(
    "repeat_op", globals(), "tensorflow.python.data.ops.repeat_op"
)
...
def apply_rewrite(dataset, rewrite):
    ...
    return _VariantDataset(
        gen_dataset_ops.rewrite_dataset(
            dataset._variant_tensor, rewrite, **dataset._flat_structure
        ),
        dataset.element_spec,
    )
