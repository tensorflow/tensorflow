...
class DatasetV2(tracking_base.Trackable, composite_tensor.CompositeTensor):
  ...
  **Component**: The leaf in the nested structure of an element.
  ...
  def from_generator(generator,
    ...
    given `output_types` and (optional) `output_shapes` arguments or with the
    given `output_spec` argument whichiver was specified.
    ...
    * Using `output_spec` argument. In this case the output will be assumed to
    ...
    Use `output_types` argument in simpler cases to benefit from its shorter
    form:
    ...
    Use `output_spec` for more complicated cases, e.g. to specify an output
    containing `tf.RaggedTensor`, `tf.SparseTensor` or other objects different
    from `tf.Tensor`:
    ...
    Args:
      ...
      output_types: (Optional.) A nested structure of `tf.DType` objects
        ...
        Ignored if `output_spec` is specified.
      output_shapes: (Optional.) A nested structure of `tf.TensorShape` objects
        corresponding to each component of an element yielded by `generator`.
        Ignored if `output_spec` is specified.
      args: (Optional.) A tuple of `tf.Tensor` objects that will be evaluated
        and passed to `generator` as NumPy-array arguments.
      output_spec: (Optional.) A nested structure of `tf.TypeSpec` objects
        corresponding to each component of an element yielded by `generator`.
    ...
    flat_output_types = structure.get_flat_tensor_types(output_spec)
    ...
    def generator_next_fn(iterator_id_t):
      ...
      def generator_py_func(iterator_id):
        ...
        try:
          output_dtypes = nest.map_structure(lambda t: t.dtype, output_spec)
...
class _GeneratorDataset(DatasetSource):
  ...
  def __init__(self, init_args, init_func, next_func, finalize_func,
    ...
    Args:
      ...
      output_spec: A nested structure of `tf.TypeSpec` objects describing the
      output of `next_func`.
    """
