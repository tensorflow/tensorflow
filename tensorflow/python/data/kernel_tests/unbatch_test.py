...
class UnbatchCheckpointTest(
    ...
    def build_dataset(
        self, multiplier=15.0, tensor_slice_len=2, batch_size=2, options=None
    ):
        components = (
...
class UnbatchRandomAccessTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(test_base.default_test_combinations())
    def test(self):
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), combinations.combine(index=[-1, 100])
        )
    )
