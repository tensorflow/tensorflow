...
class BatchRandomAccessTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testBasic(self, count, batch_size, drop_remainder):
        ...
        for i in range(num_full_batches):
            ...
            self.assertAllEqual(
                expected_batch, self.evaluate(random_access.at(dataset, i))
            )
    ...
    def testRandomAccessBatchWithShuffle(self):
        ...
        for i in range(4):
            self.assertAllEqual(
                expected_output[i], self.evaluate(random_access.at(batch_dataset, i))
            )
        ...
        self.assertAllEqual(
            expected_output[3][0], self.evaluate(random_access.at(shuffle_dataset, 6))
        )
...
class BatchGlobalShuffleTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(dataset_range=[100], batch_size=[2, 7]),
        )
    )
    ...
    def testReshuffleRepeatEpochs(
        self, dataset_range: int, batch_size: int, reshuffle: bool, seed: Optional[int]
    ):
        dataset = dataset_ops.Dataset.range(dataset_range)
        dataset = dataset.batch(batch_size, drop_remainder=True)
        dataset = dataset.prefetch(buffer_size=dataset_ops.AUTOTUNE)
    ...
    def testNoDropRemainder(self, dataset_range: int, batch_size: int):
        dataset = dataset_ops.Dataset.range(dataset_range)
...
class BatchGlobalShuffleCheckpointTest(
    checkpoint_test_base.CheckpointTestBase, parameterized.TestCase
):
    @combinations.generate(
        combinations.times(
            ...
            checkpoint_test_base.default_test_combinations(),
            combinations.combine(
                dataset_range=[10], batch_size=[2, 3], symbolic_checkpoint=[True, False]
            ),
        )
    )
    def testBatch(
