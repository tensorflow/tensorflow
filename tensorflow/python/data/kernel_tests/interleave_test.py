...
class InterleaveTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testPythonImplementation(
        self, input_values, cycle_length, block_length, expected_elements
    ):
        input_lists = _repeat(input_values, 2)

        for expected, produced in zip(
            expected_elements, _interleave(input_lists, cycle_length, block_length)
        ):
    ...
    def testInterleaveDataset(
        self, input_values, cycle_length, block_length, num_parallel_calls
    ):
        count = 2
        dataset = (
        ...
        expected_output = [
            element
            for element in _interleave(
                _repeat(input_values, count),
                cycle_length,
                block_length,
                num_parallel_calls,
            )
    ...
    def testInterleaveDatasetError(
        self, input_values, cycle_length, block_length, num_parallel_calls
    ):
        dataset = (
            dataset_ops.Dataset.from_tensor_slices(input_values)
    ...
    def testInterleaveSparse(self):
        ...
        dataset = (
            ...
            .map(_map_fn)
            .interleave(_interleave_fn, cycle_length=1)
        )
        get_next = self.getNext(dataset)
        for i in range(10):
            for j in range(2):
    ...
    def testSloppyInterleaveDataset(
        self, input_values, cycle_length, block_length, num_parallel_calls
    ):
        count = 2
        dataset = (
...
class InterleaveDatasetCheckpointTest(
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(
                cycle_length=2, block_length=[1, 3], num_parallel_calls=[None, 1, 2]
            ),
        )
    )
