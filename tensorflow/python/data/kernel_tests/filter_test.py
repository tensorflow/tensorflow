...
class FilterTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testFilterDataset(self, apply_filter):
        ...
        def do_test(count, modulus):  # pylint: disable=missing-docstring
            ...
            dataset = apply_filter(
                dataset, lambda x, _y, _z: math_ops.equal(math_ops.mod(x, modulus), 0)
            )
    ...
    def testParallelFilters(self, apply_filter):
        ...
        self.assertEqual(
            [0 for _ in range(10)],
            self.evaluate([next_element() for next_element in next_elements]),
        )
...
class FilterCheckpointTest(
    ...
    def test(self, verify_fn, symbolic_checkpoint):
        ...
        verify_fn(
            self, lambda: self._build_filter_range_dataset(div, options), num_outputs
        )
...
class FilterGlobalShuffleTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(test_base.default_test_combinations())
