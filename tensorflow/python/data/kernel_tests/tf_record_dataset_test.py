...
class TFRecordDatasetTest(tf_record_test_base.TFRecordTestBase, parameterized.TestCase):
    def _dataset_factory(
        self, filenames, compression_type="", num_epochs=1, batch_size=None
    ):

        repeat_dataset = readers.TFRecordDataset(filenames, compression_type).repeat(
            num_epochs
        )
        if batch_size:
...
class TFRecordDatasetCheckpointTest(
    ...
    def make_dataset(
        self, num_epochs, batch_size=1, compression_type=None, buffer_size=None
    ):
        ...
        return (
            readers.TFRecordDataset(
                filenames, compression_type, buffer_size=buffer_size
            )
            .repeat(num_epochs)
            .batch(batch_size)
        )
    ...
    def testCompressionTypes(self, verify_fn, compression_type):
        ...
        verify_fn(
            self,
            lambda: self.make_dataset(num_epochs, compression_type=compression_type),
            num_outputs,
        )
