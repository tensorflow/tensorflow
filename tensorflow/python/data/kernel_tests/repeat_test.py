...
class RepeatDatasetCheckpointTest(
    ...
    def _build_repeat_dataset(
        self, num_elements, num_epochs, num_outputs=None, options=None
    ):
...
class RepeatRandomAccessTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(elements=[0, 5, 10], count=[0, 3, 8]),
        )
    )
...
class RepeatGlobalShuffleTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(
