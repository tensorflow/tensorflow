...
class ShuffleTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testTensorInput(self, dataset_range, buffer_size, seed, use_tensor_input):
        ...
        seed = (
            constant_op.constant(seed, dtype=dtypes.int64)
            if seed and use_tensor_input
            else seed
        )
    ...
    def testDontRerandomizeOnReplicate(self, reshuffle):
        ...
        dataset_2 = dataset_1.shuffle(num_elements, reshuffle_each_iteration=reshuffle)
...
class ShuffleRandomAccessTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(test_base.default_test_combinations())
    def testInvalidIndex(self):
        dataset = dataset_ops.Dataset.from_tensor_slices([1, 2, 3]).shuffle(
            buffer_size=100
        )
        with self.assertRaises(errors.OutOfRangeError):
    ...
    @combinations.generate(test_base.default_test_combinations())
    def testEmptyDataset(self):
        dataset = dataset_ops.Dataset.from_tensor_slices([]).shuffle(buffer_size=100)
        with self.assertRaises(errors.OutOfRangeError):
