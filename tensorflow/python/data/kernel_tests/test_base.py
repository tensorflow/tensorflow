...
class DatasetTestBase(test.TestCase):
    ...
    def assertDatasetsEqual(self, dataset1, dataset2):
        ...
        self.assertTrue(
            structure.are_compatible(
                dataset_ops.get_structure(dataset1), dataset_ops.get_structure(dataset2)
            )
        )
    ...
    def assertDatasetsRaiseSameError(
        self, dataset1, dataset2, exception_class, replacements=None
    ):
    ...
    def verifyRandomAccessInfiniteCardinality(self, dataset, expected):
        ...
        for i in indices:
            self.assertAllEqual(
                expected[i], self.evaluate(random_access.at(dataset, i))
            )
    ...
    def graphRoundTrip(self, dataset, allow_stateful=False):
        ...
        graph = gen_dataset_ops.dataset_to_graph(
            dataset._variant_tensor, allow_stateful=allow_stateful
        )  # pylint: disable=protected-access
        return dataset_ops.from_variant(
            gen_experimental_dataset_ops.dataset_from_graph(graph), dataset.element_spec
        )
    ...
    def configureDevicesForMultiDeviceTest(self, num_devices):
        ...
        config.set_logical_device_configuration(
            cpus[0], [context.LogicalDeviceConfiguration() for _ in range(num_devices)]
        )
