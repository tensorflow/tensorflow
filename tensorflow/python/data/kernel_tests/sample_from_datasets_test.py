...
class SampleFromDatasetsTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _weights_type_combinations()
        )
    )
    ...
    def testSampleFromDatasetsStoppingOnEmptyDataset(self, weights_type):
        ...
        samples_list = self.getIteratorOutput(
            self.getNext(sample_dataset, requires_initialization=True)
        )
    ...
    def testErrors(self):
        with self.assertRaisesRegex(ValueError, r"should have the same length"):
            dataset_ops.Dataset.sample_from_datasets(
                [dataset_ops.Dataset.range(10), dataset_ops.Dataset.range(20)],
...
class SampleFromDatasetsCheckpointTest(
    checkpoint_test_base.CheckpointTestBase, parameterized.TestCase
):
    def _build_dataset(self, probs, num_samples, options=None):
        datasets = [
            dataset_ops.Dataset.from_tensors(i).repeat(None) for i in range(len(probs))
        ]
    ...
    def test(self, verify_fn, symbolic_checkpoint):
        ...
        verify_fn(
            self, lambda: self._build_dataset([0.5, 0.5], 100, options), num_outputs=100
        )
