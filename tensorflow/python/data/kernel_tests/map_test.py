...
def _test_combinations_with_mode(mode):
    return _test_combinations_with_mode_v1(mode) + _test_combinations_with_mode_v2(mode)


def _test_combinations():
    return _test_combinations_with_mode("eager") + _test_combinations_with_mode("graph")
...
class MapTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def _parallel_map_dataset_factory(
        self,
        components,
        apply_map,
        count,
    ...
    def testParallelMapDataset(
        self, apply_map, num_parallel_calls, buffer_size, use_unbounded_threadpool
    ):
        """Test an dataset that maps a TF function across its input elements."""
        ...
        # Test single-threaded access to the iterator.
        get_next = self.getNext(
            self._parallel_map_dataset_factory(
                components,
                apply_map,
                14,
                num_parallel_calls,
    ...
    def testParallelMapDatasetMultiThreaded(
        self, apply_map, num_parallel_calls, buffer_size
    ):
        # Test multi-threaded access to the same iterator.
        components = (
            np.arange(7),
        ...
        get_next = self.getNext(
            self._parallel_map_dataset_factory(
                components, apply_map, 18, num_parallel_calls, buffer_size
            )
        )
        results = []
        with self.cached_session() as sess:
    ...
    def testImplicitDisposeParallelMapDataset(self, apply_map):
        ...
        dataset = self._parallel_map_dataset_factory(
            components, apply_map, 1000, 100, 100
        )
    ...
    def testCaptureHashTable(self, apply_map):
        ...
        dataset = apply_map(
            input_sentences, lambda x: string_ops.string_split([x]).values
        )
    ...
    def testCaptureSameResourceMultipleTimes(self, apply_map):
        ...
        for i in range(100):
            self.assertCountEqual(
                [elements[i * 2], elements[i * 2 + 1]], self.evaluate(get_next())
            )
        with self.assertRaises(errors.OutOfRangeError):
            self.evaluate(get_next())
    ...
    def testMapDataclassWithInputAndOutput(self, apply_map):
        ...
        dataset = apply_map(dataset, lambda x, y: MaskedTensor(mask=True, value=x + y))
        dataset = apply_map(dataset, lambda m: NestedMaskedTensor(mask=False, value=m))
        self.assertDatasetProduces(
            dataset,
            expected_output=[
                NestedMaskedTensor(mask=False, value=MaskedTensor(mask=True, value=7))
            ],
        )
    ...
    def testUseStepContainerInMap(self, apply_map):
        ...
        dataset = apply_map(
            dataset, lambda elems: map_fn.map_fn(lambda x: x * x, elems)
        )
    ...
    def testCaseAndCondInWhileInMap(self, apply_map):
        ...
        self.assertAllEqual(
            [
                (x // 2 if x % 2 else x * 2) if (num == 2 or num == 3) else x * 2
                for x in row
            ],
            self.evaluate(get_next()),
        )
    ...
    @combinations.generate(
        combinations.times(
            _test_combinations(), combinations.combine(buffer_size=[1, 2, 3, 4])
        )
    )
    ...
    def testTensorArray(self, apply_map):
        def _tensor_array(i):
            ...
            return tensor_array_ops.TensorArray(
                dtypes.int32, element_shape=(), size=i
        ...
        with ops.Graph().as_default() as g:
            ...
            def _check_warning(caught_warnings, expected_result):
                ...
                for warning in caught_warnings:
                    if (
                        "Explicitly set the seed in the function if this is not the "
            ...
            with warnings.catch_warnings(record=True) as w:
                _ = (
                    dataset_ops.Dataset.range(10)
                    .batch(2)
                    .map(lambda x: random_ops.random_shuffle(x, seed=37))
                )
            _check_warning(w, False)
    ...
    def testReturnValueError(self, apply_map):
        ...
        with self.assertRaisesRegex(
            TypeError,
            r"Unsupported return value from function passed to "
            r"Dataset.map\(\)",
        ):
    ...
    def testNoInterOpParallelism(self, apply_map, num_parallel_calls):
        ...
        dataset = apply_map(dataset, _map_fn)
        dataset._variant_tensor.op._set_attr(
            "use_inter_op_parallelism", attr_value_pb2.AttrValue(b=False)
        )
        get_next = self.getNext(dataset)
    ...
    @combinations.generate(
        combinations.times(
            _test_combinations(), combinations.combine(num_parallel_calls=[None, 12])
        )
    )
    ...
    @combinations.generate(
        combinations.combine(
            tf_api_version=2, mode=["eager", "graph"], num_parallel_calls=[None, 12]
        )
    )
    ...
    def testDeterminismConfiguration(
        self, apply_map, local_determinism, global_determinism
    ):
    ...
    def testCheckpointLargeBuffer(self):
        if (
            pywrap_sanitizers.is_asan_enabled()
            or pywrap_sanitizers.is_tsan_enabled()
            or pywrap_sanitizers.is_msan_enabled()
        ):
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(use_unbounded_threadpool=[True, False]),
        )
    )
    ...
    def testCaptureConstantInMapFn(self, verify_fn, num_parallel_calls):
        ...
        def _build_ds():
            ...
            return (
                dataset_ops.Dataset.from_tensors(0)
                .repeat(10)
                .map(lambda x: x + constant_var, num_parallel_calls=num_parallel_calls)
            )
...
class MapRandomAccessTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(
        combinations.times(
            test_base.v2_only_combinations(), combinations.combine(index=[-1, 4, 5])
        )
    )
    def testInvalidIndex(self, index):
        dataset = dataset_ops.Dataset.from_tensor_slices([-1, 0, 1, 2]).map(
            lambda x: x * 2
        )
        with self.assertRaises(errors.OutOfRangeError):
    ...
    @combinations.generate(
        combinations.times(
            test_base.v2_only_combinations(), combinations.combine(index=[-1, 0])
        )
    )
    ...
    def testBasic(self):
        dataset = dataset_ops.Dataset.from_tensor_slices([0, 1, 2, 3, 4, 5]).map(
            lambda x: x * 3
        )
        for i in range(5):
    ...
    def testMultipleCombinations(self, elements, num_parallel_calls):
        ...
        for i in range(elements):
            self.assertEqual(self.evaluate(random_access.at(dataset, index=i)), i // 2)
...
class MapGlobalShuffleCheckpointTest(
    checkpoint_test_base.CheckpointTestBase, parameterized.TestCase
):
    @combinations.generate(
        combinations.times(
