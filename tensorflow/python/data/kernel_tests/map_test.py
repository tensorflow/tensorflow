...
class MapTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def _parallel_map_dataset_factory(
        self, components, apply_map, count, num_parallel_calls, buffer_size
    ):
        def _map_fn(x, y, z):
            return math_ops.square(x), math_ops.square(y), math_ops.square(z)
    ...
    def testParallelMapDataset(self, apply_map, num_parallel_calls, buffer_size):
        ...
        get_next = self.getNext(
            self._parallel_map_dataset_factory(
                components, apply_map, 14, num_parallel_calls, buffer_size
            )
        )
        for _ in range(14):
            for i in range(7):
    ...
    def testReturnValueError(self, apply_map):
        ...
        with self.assertRaisesRegex(
            TypeError,
            r"Unsupported return value from function passed to " r"Dataset.map\(\)",
        ):
