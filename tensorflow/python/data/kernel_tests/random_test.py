...
class RandomTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testRerandomizeEachIterationEpochsIgnored(self, rerandomize):
        with warnings.catch_warnings(record=True) as w:
            dataset = dataset_ops.Dataset.random(
                seed=42, rerandomize_each_iteration=rerandomize, name="random"
            ).take(10)
        ...
        if rerandomize:
            if not tf2.enabled() and rerandomize:
                found_warning = False
                for warning in w:
                    if "In TF 1, the `rerandomize_each_iteration=True` option" in str(
                        warning
                    ):
                        found_warning = True
                        break
    ...
    def testRerandomizeEachIterationEpochs(self, rerandomize):
        ...
        if rerandomize:
            self.assertEqual(
                first_epoch == second_epoch, not rerandomize or rerandomize is None
            )
        else:
            self.assertEqual(first_epoch, second_epoch)
    ...
    def testName(self):
        ...
        self.assertDatasetProduces(
            dataset, expected_output=[42], requires_initialization=True
        )


class RandomCheckpointTest(
    checkpoint_test_base.CheckpointTestBase, parameterized.TestCase
):
    def _build_random_dataset(
        self, num_elements=10, seed=None, rerandomize_each_iteration=None
    ):
        dataset = dataset_ops.Dataset.random(
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(rerandomize_each_iteration=[True, False]),
        )
    )
