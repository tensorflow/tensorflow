...
class SnapshotTest(tf_record_test_base.TFRecordTestBase, parameterized.TestCase):
    ...
    def assertSnapshotDirectoryContains(
        ...
        for i in range(num_fingerprints):
            ...
            self.assertEqual(
                fingerprint_dir_list[num_runs_per_fingerprint], "snapshot.metadata"
            )
        ...
        dataset = dataset.snapshot(
            ...
            reader_func=(
                lambda ds: ds.interleave(  # pylint:disable=g-long-lambda
                    lambda x: x, cycle_length=4, num_parallel_calls=4
                )
            ),
        )
        self.assertDatasetProduces(dataset, expected)
    ...
    def testWriteSnapshotDatasetWithTuples(self):
        ...
        expected = list(
            zip(range(0, 1000), range(1000, 2000), range(2000, 3000), range(3000, 4000))
        )
        self.assertDatasetProduces(dataset, expected)
        self.assertSnapshotDirectoryContains(
            self._snapshot_dir,
...
class LegacySnapshotTest(tf_record_test_base.TFRecordTestBase, parameterized.TestCase):
    ...
    def testSpecifySnapshotNameWriteAndRead(self):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(tmpdir, "custom-my_custom_snapshot", "custom"))
        )
    ...
    def testReadShuffledSnapshotAfterWrite(self):
        ...
        dataset2 = core_readers._TFRecordDataset(filenames)
        dataset2 = dataset2.apply(
            snapshot.legacy_snapshot(tmpdir, shard_size_bytes=100, shuffle_on_read=True)
        )
    ...
    def testReadShuffledSnapshotWithSeedAfterWrite(self):
        ...
        dataset2 = dataset2.apply(
            snapshot.legacy_snapshot(
                tmpdir, shard_size_bytes=10, shuffle_on_read=True, shuffle_seed=123456
            )
        )
        next2 = self.getNext(dataset2)
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.times(
                combinations.combine(
                    compression=[snapshot.COMPRESSION_NONE, snapshot.COMPRESSION_GZIP]
                ),
...
class SnapshotCheckpointTest(
    ...
    def testCheckpointBeforeOneEpochWithReading(self):
        ...
        self.assertSequenceEqual(
            outputs, list(range(50)) + list(range(50, 100)) + list(range(100))
        )
    ...
    def testCheckpointAfterOneEpoch(self):
        ...
        outputs.extend(t)
        self.assertSequenceEqual(
            outputs, list(range(100)) + list(range(10)) + list(range(10, 100))
        )
...
class LegacySnapshotCheckpointTest(
    ...
    def testSnapshotBeforeEpochEnd(self, pending_snapshot_expiry_seconds):
        ...
        self.assertSequenceEqual(outputs, range(100))
        outputs.extend(
            self.gen_outputs(ds_fn, [], 900, ckpt_saved=True, verify_exhausted=False)
        )
    ...
    def testCheckpointAfterOneEpoch(self, pending_snapshot_expiry_seconds):
        ds_fn = self._build_snapshot_dataset(
            repeat=True, pending_snapshot_expiry_seconds=pending_snapshot_expiry_seconds
        )
    ...
    def testCheckpointAfterOneEpochThenRunFewSteps(
        ...
        # 100.
        outputs = self.gen_outputs(
            ds_fn, [1100], 1200, verify_exhausted=False, save_checkpoint_at_end=False
        )
        self.assertSequenceEqual(
            outputs, list(range(1000)) + list(range(100)) + list(range(100))
        )
