...
class DatasetTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    @combinations.generate(
        combinations.times(
            ...
            combinations.combine(init_source=["textfile", "keyvaluetensor", "dataset"]),
        )
    )
    ...
    def testAsFunctionFromReader(self):
        with ops.device("CPU"):
            file_path = os.path.join(
                self.get_temp_dir(), "{}.tfrecord.gz".format("tf_record_asset")
            )
            with tf_record.TFRecordWriter(file_path, "GZIP") as f:
                for v in ["a", "aa", "aaa"]:
                    f.write(str(v))
            original_dataset = readers.TFRecordDataset(
                [file_path], compression_type="GZIP"
            )
            fn = original_dataset._trace_variant_creation()
            variant = fn()
    ...
    def _testVariadicInputs(self, dataset_fn, input_datasets):
        self.assertEqual(
            nest.flatten(input_datasets), dataset_fn(input_datasets)._inputs()
        )
    ...
    def testZipTupleInputs(self):
        input_datasets = (dataset_ops.Dataset.range(0), dataset_ops.Dataset.range(1))
        self._testVariadicInputs(dataset_ops.Dataset.zip, input_datasets)
    ...
    def _testDatasetSpec(self, tf_value, expected_element_structure):
        ...
        self.assertEqual(
            [dtypes.variant], structure.get_flat_tensor_types(dataset_structure)
        )
        self.assertEqual(
    ...
    def testDatasetDatasetSpec(self):
        self._testDatasetSpec(
            dataset_ops.Dataset.from_tensor_slices(constant_op.constant([1, 2, 3])),
    ...
    def testDatasetAsFunctionArgument(self):
        ...
        with ops.device("CPU"):
            ...
            self.assertIs(
                first_concrete, _uses_dataset.get_concrete_function(second_dataset)
            )
    ...
    def testLegacyStructureAPI(self):
        ...
        self.assertEqual(
            ([3], ([2], [2]), [3]), dataset_ops.get_legacy_output_shapes(dataset)
        )
        ...
        self.assertEqual(
            (([3], [3]), ([2], [2])), dataset_ops.get_legacy_output_shapes(dataset)
        )

        dataset = dataset.flat_map(
            lambda x, y: dataset_ops.Dataset.from_tensors(((x[0], x[1]), (y[0], y[1])))
        )
        self.assertEqual(
            ((dtypes.int64, dtypes.int64), (dtypes.float64, dtypes.float64)),
            dataset_ops.get_legacy_output_types(dataset),
        ...
        components_for_slices = (
            np.array([1, 2, 3], dtype=np.int64),
            (np.array([4.0, 5.0, 6.0]), np.array([7.0, 8.0, 9.0])),
    ...
    def testNoneComponentInFunction(self):
        ...
        dataset = dataset_ops.Dataset.range(10, output_type=dtypes.int32).map(
            lambda x: (x, None)
        )
...
class DebugDatasetTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testDebugModeGenerator(self):
        ...
        ds = dataset_ops.Dataset.from_generator(
            gen, output_signature=tensor_spec.TensorSpec(shape=(), dtype=dtypes.int64)
        )
        self.assertDatasetProduces(ds, list(range(10)))
    ...
    def testDebugModeSequentialExecution(self):
        ds = dataset_ops.Dataset.range(10)
        ds = ds.apply(testing.assert_next(["Interleave", "Map", "Batch", "FiniteTake"]))
        ds = ds.interleave(
            dataset_ops.Dataset.from_tensors, cycle_length=10, num_parallel_calls=10
        )
