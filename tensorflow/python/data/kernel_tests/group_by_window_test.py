...
class GroupByWindowTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testSingleBucket(self):
        ...
        input_dataset = dataset_ops.Dataset.from_tensor_slices(math_ops.range(32)).map(
            _map_fn
        )
    ...
    def testEvenOddBuckets(self):
        ...
        input_dataset = dataset_ops.Dataset.from_tensor_slices(math_ops.range(64)).map(
            _map_fn
        )
        ...
        for i in range(0, 32):
            ...
            expected_vec3_str = np.vstack(
                3 * [np.arange(1, 32 * 2 + 1, 2).astype(bytes)]
            ).T
    ...
    def testConsumeWindowDatasetMoreThanOnce(self):
        ...
        dataset = dataset.group_by_window(
            key_func=lambda x: math_ops.cast(array_ops.shape(x)[0] // 10, dtypes.int64),
        ...
        with self.assertRaises(errors.OutOfRangeError):
            while True:
                ...
                self.assertAllEqual(
                    tight_result, multiple_of_10_result[:, : tight_result.shape[1]]
                )
...
class GroupByWindowCheckpointTest(
...
class GroupByWindowErrorMessageTest(test_base.DatasetTestBase, parameterized.TestCase):
    @combinations.generate(test_base.default_test_combinations())
    def testReduceFuncError(self):
        ...
        self.assertIn(
            ...
            msg,
            "{} should show up in the error message".format(my_reduce_func.__name__),
        )
    ...
    def testPropagateUserDefinedFunctionErrorMessage(self):
        ...
        with self.assertRaisesRegex(
            ...
            msg=(
                "The name of user-defined-function should show up in the error message"
            ),
        ):
