...
def _test_is_compatible_with_structure_combinations():
    cases = [
        ...
        (
            "Variable",
            lambda: variables.Variable(100.0),
        ...
        (
            ...
            lambda: [
                {"a": constant_op.constant(15.0), "b": constant_op.constant([4, 5, 6])}
            ],
            lambda: [
                {
                ...
                {"a": constant_op.constant(15), "b": constant_op.constant([4, 5, 6])},
                {
...
def _test_structure_from_value_equality_combinations():
    ...
    def reduce_fn(x, y):
        ...
        return x + combinations.combine(
            value1_fn=combinations.NamedObject("value1_fn.{}".format(name), value1_fn),
            value2_fn=combinations.NamedObject("value2_fn.{}".format(name), value2_fn),
...
def _test_hash_combinations():
    ...
    def reduce_fn(x, y):
        ...
        return x + combinations.combine(
            value1_fn=combinations.NamedObject("value1_fn.{}".format(name), value1_fn),
            value2_fn=combinations.NamedObject("value2_fn.{}".format(name), value2_fn),
            value3_fn=combinations.NamedObject("value3_fn.{}".format(name), value3_fn),
        )
...
def _test_convert_legacy_structure_combinations():
    cases = [
        ...
        (
            ...
            {"a": tensor.Tensor, "b": (sparse_tensor.SparseTensor, tensor.Tensor)},
            {
...
def _test_batch_combinations():
    cases = [
        ...
        (
            ...
            128,
            {
                "a": tensor.TensorSpec([128], dtypes.float32),
                "b": (
                    sparse_tensor.SparseTensorSpec([128, 2, 2], dtypes.int32),
...
class StructureTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_flat_structure_combinations()
        )
    )
    def testFlatStructure(
        self, value_fn, expected_structure_fn, expected_types_fn, expected_shapes_fn
    ):
        value = value_fn()
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_hash_combinations()
        )
    )
    ...
    def testRoundTripConversion(self, value_fn):
        ...
        for b, a in zip(flat_before, flat_after):
            if isinstance(b, sparse_tensor.SparseTensorValue):
                ...
                b, (ragged_tensor.RaggedTensor, ragged_tensor_value.RaggedTensorValue)
            ):
    ...
    def preserveStaticShape(self):
        ...
        self.assertEqual(
            rt_after.row_splits.shape.as_list(), rt.row_splits.shape.as_list()
        )
        ...
        self.assertEqual(
            st_after.dense_shape.shape.as_list(), st.dense_shape.shape.as_list()
        )
    ...
    def testIncompatibleStructure(self):
        ...
        flat_sparse_tensor = structure.to_tensor_list(
            s_sparse_tensor, value_sparse_tensor
        )
    ...
    def testConvertLegacyStructure(
        self, output_types, output_shapes, output_classes, expected_structure
    ):
    ...
    def testConvertLegacyStructureFail(self):
        with self.assertRaisesRegex(
            TypeError,
            "Could not build a structure for output class "
    ...
    def testNestedNestedStructure(self):
        s = (
            tensor.TensorSpec([], dtypes.int64),
            (
                tensor.TensorSpec([], dtypes.float32),
                tensor.TensorSpec([], dtypes.string),
            ),
        ...
        (actual_int64_t, (actual_float32_t, actual_string_t)) = (
            structure.from_tensor_list(s, tensor_list)
        )
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_batch_combinations()
        )
    )
    def testBatch(self, element_structure, batch_size, expected_batched_structure):
        batched_structure = nest.map_structure(
            lambda component_spec: component_spec._batch(batch_size), element_structure
        )
        self.assertEqual(batched_structure, expected_batched_structure)

    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_unbatch_combinations()
        )
    )
