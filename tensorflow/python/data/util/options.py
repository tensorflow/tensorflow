...
class OptionsBase(object):
    ...
    def __eq__(self, other):
        ...
        for name in set(self._options) | set(
            other._options
    ...
    def __setattr__(self, name, value):
        if not self._mutable:
            raise ValueError(
                "Mutating `tf.data.Options()` returned by "
        ...
        if hasattr(self, name):
            ...
            raise AttributeError(
                "Cannot set the property {} on {}.".format(name, type(self).__name__)
            )
    ...
    for options in options_list:
        ...
        for name in options._options:  # pylint: disable=protected-access
            ...
            if that == default:
                ...
                logging.warning(
                    "Changing the value of option %s from %r to %r.", name, this, that
                )
                setattr(result, name, that)
    return result
