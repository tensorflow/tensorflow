...
def _test_random_seed_combinations():
    cases = [
        ...
        ("TestCase_1", lambda: (None, 1), lambda: (random_seed.DEFAULT_GRAPH_SEED, 1)),
        ...
        (
            ...
            lambda: (
                2**31 - 1,
                constant_op.constant(0, dtype=dtypes.int64, name="zero"),
            ),
        ...
        (
            ...
            lambda: (
                0,
                constant_op.constant(2**31 - 1, dtype=dtypes.int64, name="intmax"),
            ),
    ...
    def reduce_fn(x, y):
        ...
        return x + combinations.combine(
            ...
            output_fn=combinations.NamedObject("output_fn.{}".format(name), output_fn),
        )
...
class RandomSeedTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(), _test_random_seed_combinations()
        )
    )
