                ...
                if spec.__class__.__name__ == "DatasetSpec":
                    ...
                    normalized_components.append(
                        ops.convert_to_tensor(
                            t, name=f"component_{i}", dtype=spec.dtype
                        )
                    )
...
def _from_tensor_list_helper(decode_fn, element_spec, tensor_list):
    ...
    if sum(flat_spec_lengths) != len(tensor_list):
        raise ValueError(
            "Expected {} tensors but got {}.".format(
...
def type_spec_from_value(element, use_fallback=True):
    ...
    if isinstance(element, tuple):
        if (
            ...
            and all(isinstance(f, str) for f in element._fields)
        ):
    ...
    raise TypeError(
        "Could not build a `TypeSpec` for {} with type {}".format(
