...
class RemapperTest(test.TestCase, parameterized.TestCase):
    ...
    def test_matmul_biasadd_activation_fusion(self, mode):
        ...
        def gelu_exact_erfc(x):
            return (
                0.5 * x * math_ops.erfc(-x * math_ops.cast(0.7071067811865476, x.dtype))
            )
        ...
        if (
            ...
            and not _pywrap_utils.IsDataTypeSupportedByOneDNNOnThisCPU(dtypes.float16)
        ):
            use_fp16 = False
        if mode == "mkl":
        ...
        for precision, act_fn, act_name in config:
            for transpose in (False, True):
                ...
                if (
                    transpose
                    and (device == "/device:CPU:0")
                    and act_name in (b"GeluApproximate", b"GeluExact")
                ):
                ...
                graph = self._VerifyValues(
                    out, precision != dtypes.float32, fused_op, epilog_ops
                )
