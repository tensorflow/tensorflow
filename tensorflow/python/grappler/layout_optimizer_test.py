...
def _is_transpose(node):
    return (
        node.endswith("TransposeNHWCToNCHW-LayoutOptimizer")
        or node.endswith("TransposeNCHWToNHWC-LayoutOptimizer")
        or node.endswith("TransposeNDHWCToNCDHW-LayoutOptimizer")
        or node.endswith("TransposeNCDHWToNDHWC-LayoutOptimizer")
    )
...
@test_util.for_all_test_methods(
    test_util.no_xla_auto_jit, "Test does not apply in XLA setting"
)
class LayoutOptimizerTest(test.TestCase):
    ...
    def testTile(self):
        if test.is_gpu_available(cuda_only=True):
            ...
            with session.Session(config=_get_config()) as sess:
                metadata = config_pb2.RunMetadata()
                output_val = sess.run(
                    output, run_metadata=metadata, feed_dict={multiple: multiple_val}
                )
    ...
    def testReverseWithNonConstDims(self):
        if test.is_gpu_available(cuda_only=True):
            ...
            with session.Session(config=_get_config()) as sess:
                metadata = config_pb2.RunMetadata()
                output_val = sess.run(
                    output, run_metadata=metadata, feed_dict={dims: dims_val}
                )
    ...
    def testPadWithNonConstPaddings(self):
        if test.is_gpu_available(cuda_only=True):
            ...
            with session.Session(config=_get_config()) as sess:
                metadata = config_pb2.RunMetadata()
                output_val = sess.run(
                    output, run_metadata=metadata, feed_dict={paddings: paddings_val}
                )
    ...
    def testMaxPoolV2(self):
        if test.is_gpu_available(cuda_only=True):
            ...
            with session.Session(config=_get_config()) as sess:
                metadata = config_pb2.RunMetadata()
                output_val = sess.run(
                    output, run_metadata=metadata, feed_dict={strides: strides_val}
                )
    ...
    def testMaxPoolGradV2(self):
        if test.is_gpu_available(cuda_only=True):
            ...
            ksize = constant_op.constant([1, 2, 3, 1], shape=[4])
            strides = array_ops.placeholder(dtype="int32", shape=[4])
            max_pool_grad = gen_nn_ops.max_pool_grad_v2(
                conv, conv, conv, ksize, strides, "VALID"
            )
    ...
    def testStridedSliceWithNonConstAxis(self):
        if test.is_gpu_available(cuda_only=True):
            ...
            with session.Session(config=_get_config()) as sess:
                metadata = config_pb2.RunMetadata()
                output_val = sess.run(
                    output, run_metadata=metadata, feed_dict={end: end_val}
                )
    ...
    def testStridedSliceGradWithNonConstAxis(self):
        if test.is_gpu_available(cuda_only=True):
            ...
            s_grad = array_ops.strided_slice_grad(
                shape, [0, 0, 0, 0], end, [1, 2, 3, 1], s
            )
    ...
    def testCheckpointCompatibility(self):
        ...
        for var_expected, var_layout_optimized in zip(
            vars_expected, vars_layout_optimized
        ):
