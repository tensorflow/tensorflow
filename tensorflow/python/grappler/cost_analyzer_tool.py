...
from tensorflow.contrib.fused_conv.ops import (
    gen_fused_conv2d_bias_activation_op,
...
def main(_):
    ...
    report = cost_analyzer.GenerateCostReport(
        metagraph, FLAGS.per_node_report, FLAGS.verbose
    )
    print(report)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--memory_report", action="store_true", help="Generate memory usage report."
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
