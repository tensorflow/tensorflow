...
Status InitialGradients(
  ...
  for (int i = 0, iter_limit = target_tensor_ids.size(); i < iter_limit; ++i) {
...
Status GradientTape<Gradient, BackwardFunction, TapeTensor>::ComputeGradient(
  ...
  while (!op_stack.empty()) {
    ...
    for (int i = 0, iter_limit = trace.input_tensor_id.size(); i < iter_limit; i++) {
    ...
    for (int i = 0, iter_limit = trace.output_tensor_info.size(); i < iter_limit; ++i) {
    ...
    for (int i = 0, iter_limit = in_gradients.size(); i < iter_limit; ++i) {
...
ForwardAccumulator<Gradient, BackwardFunction, TapeTensor>::ForwardpropFromTape(
  ...
  */
  ...
  for (int grad_index = 0, iter_limit = grad.size(); grad_index < iter_limit; ++grad_index) {
