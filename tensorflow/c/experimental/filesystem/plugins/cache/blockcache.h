...
#ifndef TENSORFLOW_C_EXPERIMENTAL_FILESYSTEM_PLUGINS_CACHE_BLOCKCACHE_H_
#define TENSORFLOW_C_EXPERIMENTAL_FILESYSTEM_PLUGINS_CACHE_BLOCKCACHE_H_
...
/// BlockCacheStatsInterface allows for instrumentation of the block cache.
///
/// BlockCacheStatsInterface and its subclasses must be safe to use from
/// multiple threads concurrently.
///
/// WARNING! This is an experimental interface that may change or go away at any
/// time.
class BlockCacheStatsInterface {
 public:
  ...
  /// re-initialized).
  virtual void Configure(const BlockCache* block_cache) = 0;
  ...
  virtual ~BlockCacheStatsInterface() = default;
};
...
class BlockCache {
 public:
  ...
  /// cache is constructed. Status should be OK as long as the
  ...
  void SetStats(BlockCacheStatsInterface* stats) {
    if (stats == nullptr) {
      std::cerr
 ...
 protected:
  BlockCacheStatsInterface* cache_stats_ = nullptr;  // Not owned.
};

}  // namespace tf_cache

#endif  // TENSORFLOW_C_EXPERIMENTAL_FILESYSTEM_PLUGINS_CACHE_BLOCKCACHE_H_
