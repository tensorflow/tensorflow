...
using tf_cache::RamBlockCache;
namespace {

Status ReadCache(RamBlockCache* cache, const string& filename, size_t offset,
...
TEST(RamBlockCacheTest, IsCacheEnabled) {
  ...
  RamBlockCache cache1(0, 0, 0, fetcher);
  RamBlockCache cache2(16, 0, 0, fetcher);
  RamBlockCache cache3(0, 32, 0, fetcher);
  RamBlockCache cache4(16, 32, 0, fetcher);
...
TEST(RamBlockCacheTest, ValidateAndUpdateFileSignature) {
  int calls = 0;
  ...
  RamBlockCache cache(16, 32, 0, fetcher);
  std::vector<char> out;
...
TEST(RamBlockCacheTest, PassThrough) {
  ...
  RamBlockCache cache1(1, 0, 0, fetcher);
  RamBlockCache cache2(0, 1, 0, fetcher);
  RamBlockCache cache3(0, 0, 0, fetcher);
  RamBlockCache cache4(1000, 1000, 0, fetcher);
  std::vector<char> out;
...
TEST(RamBlockCacheTest, BlockAlignment) {
  ...
  for (size_t block_size = 2; block_size <= 4; block_size++) {
    ...
    RamBlockCache cache(block_size, block_size, 0, fetcher);
...
TEST(RamBlockCacheTest, CacheHits) {
  ...
  RamBlockCache cache(block_size, block_count * block_size, 0, fetcher);
  std::vector<char> out;
...
TEST(RamBlockCacheTest, OutOfRange) {
...
TEST(RamBlockCacheTest, Inconsistent) {
  ...
  RamBlockCache cache(block_size, 2 * block_size, 0, fetcher);
  std::vector<char> out;
...
TEST(RamBlockCacheTest, MaxStaleness) {
  int calls = 0;
  auto fetcher = [&calls](const string& filename, size_t offset, size_t n,
                          char* buffer, size_t* bytes_transferred,
                          TF_Status* status) {
  ...
  // expected.
  RamBlockCache cache1(8, 16, 2 /* max staleness */, fetcher,
  ...
  calls = 0;
  env->SetNowSeconds(0);
  RamBlockCache cache2(8, 16, 0 /* max staleness */, fetcher,
                       [&env]() { return env->NowSeconds(); });
  // Execute the first read to load the block.
...
TEST(RamBlockCacheTest, RemoveFile) {
  int calls = 0;
  auto fetcher = [&calls](const string& filename, size_t offset, size_t n,
                          char* buffer, size_t* bytes_transferred,
                          TF_Status* status) {
  ...
  const size_t n = 3;
  RamBlockCache cache(8, 32, 0, fetcher);
  std::vector<char> out;
...
TEST(RamBlockCacheTest, Prune) {
  ...
  RamBlockCache cache(8, 32, 1 /* max staleness */, fetcher,
                      [&env]() { return env->NowSeconds(); });
...
TEST(RamBlockCacheTest, ParallelReads) {
  ...
  RamBlockCache cache(block_size, 2 * callers * block_size, 0, fetcher);
...
TEST(RamBlockCacheTest, CoalesceConcurrentReads) {
