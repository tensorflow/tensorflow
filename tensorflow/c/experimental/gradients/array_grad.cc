...
class PadGradientFunction : public GradientFunction {
 public:
  ...
  Status Compute(Context* ctx, const IncomingGradients& grad_inputs,
    ...
    TF_RETURN_IF_ERROR(
        Rank(ctx->ctx, {x}, absl::MakeSpan(temp_outputs), name.c_str()));
    AbstractTensorHandle* Rank_x = temp_outputs[0];

    // TODO(amturati): Getting error here in graph mode when trying to create a
    // scalar TensorHandle for zero and one, needed for Concatenate function
    // below.
    ...
    AbstractTensorHandlePtr begin =
        GetTensorHandleUtilInt(ctx->ctx, begin_vals, begin_dims, 1);
    ...
    name = "Reshape_fc";
    AbstractTensorHandlePtr minus_one =
        GetScalarTensorHandleUtilInt(ctx->ctx, -1);
    ...
    name = "Shape_x";
    TF_RETURN_IF_ERROR(
        Shape(ctx->ctx, {x}, absl::MakeSpan(temp_outputs), name.c_str()));
