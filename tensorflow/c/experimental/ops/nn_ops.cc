...
// Computes Relu given input features
Status Relu(AbstractContext* ctx,
                absl::Span<AbstractTensorHandle* const> inputs,
                absl::Span<AbstractTensorHandle*> outputs, const char* name) {
  AbstractOperationPtr relu_op(ctx->CreateOperation());
  TF_RETURN_IF_ERROR(
      relu_op->Reset("Relu", /*raw_device_name=*/nullptr));

  if (isa<tracing::TracingOperation>(relu_op.get())) {
    TF_RETURN_IF_ERROR(dyn_cast<tracing::TracingOperation>(relu_op.get())
                           ->SetOpName(name));
  }

  TF_RETURN_IF_ERROR(relu_op->AddInput(inputs[0]));  // Relu input
