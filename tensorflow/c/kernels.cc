...
string_view TF_OpKernelConstruction_GetName(TF_OpKernelConstruction* ctx) { 
  auto* cc_ctx = reinterpret_cast<tensorflow::OpKernelConstruction*>(ctx); 
  string_view string_view_of_name = {.data = cc_ctx->def().name().data(), 
                                     .len = cc_ctx->def().name().length()}; 
