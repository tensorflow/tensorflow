...
ForwardTypeInferenceFn ContainerMap(
  ...
  return [t, input_idx,
    ...
    if (in_cont_t.type_id() != t) {
      return Status(
          error::INVALID_ARGUMENT,
          absl::StrCat("For ForwardTypeInferenceFn ContainerMap, expected a ",
                       t, "container, not ", in_cont_t.type_id(),
                       ".\nInput type: ", in_cont_t.DebugString()));
    }
    if (in_cont_t.args_size() != 1) {
      return Status(
          error::INVALID_ARGUMENT,
          absl::StrCat("For ForwardTypeInferenceFn ContainerMap, container ",
                       "type is expected to have 1 arg, not ",
                       in_cont_t.args_size(), ". ", in_cont_t.DebugString()));
    }
    const FullTypeDef& in_el_t = in_cont_t.args(0);
    if (in_el_t.type_id() != TFT_PRODUCT) {
      return Status(error::INVALID_ARGUMENT,
                    absl::StrCat("expected PRODUCT element type for qinput ",
...
FullTypeDef MapTensor(const FullTypeDef& t) {
  ...
  // If the input type havs a shape in the future, this function needs to be
  // changed so that the output shape is computed based on the input shape and
  // the effect of the particular map operation, e.g the effect of an op that
  // changes the batch size (and this function would require more information to
  // do this computation).
  return t;
}
