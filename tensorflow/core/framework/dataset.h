...
class IteratorStateReader {
 public:
  ...
  virtual absl::Status ReadScalar(StringPiece key, int64_t* val) const = 0;
  virtual absl::Status ReadScalar(StringPiece name, StringPiece key,
                                  int64_t* val) const = 0;

  // Reads a string for the given key.
  virtual absl::Status ReadScalar(StringPiece key, tstring* val) const = 0;
  virtual absl::Status ReadScalar(StringPiece name, StringPiece key,
                                  tstring* val) const = 0;

  // Reads a tensor for the given key.
  // TODO(jsimsa): Remove non-FLR overrides once all callers are updated.
  virtual absl::Status ReadTensor(StringPiece key, Tensor* val) const = 0;
  virtual absl::Status ReadTensor(FunctionLibraryRuntime* flr, StringPiece key,
                                  Tensor* val) const = 0;
  virtual absl::Status ReadTensor(StringPiece name, StringPiece key,
                                  Tensor* val) const = 0;
  virtual absl::Status ReadTensor(FunctionLibraryRuntime* flr, StringPiece name,
...
class IteratorStateWriter {
 public:
  ...
  virtual absl::Status WriteScalar(StringPiece key, const int64_t val) = 0;
  virtual absl::Status WriteScalar(StringPiece name, StringPiece key,
                                   const int64_t val) = 0;

  // Writes a string for the given key.
  virtual absl::Status WriteScalar(StringPiece key, const tstring& val) = 0;
  virtual absl::Status WriteScalar(StringPiece name, StringPiece key,
                                   const tstring& val) = 0;

  // Writes a tensor for the given key.
  virtual absl::Status WriteTensor(StringPiece key, const Tensor& val) = 0;
  virtual absl::Status WriteTensor(StringPiece name, StringPiece key,
                                   const Tensor& val) = 0;
