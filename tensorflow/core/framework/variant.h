...
class Variant {
 public:
  ...
  static constexpr bool CanInlineType() {
    return ((sizeof(Value<VT>) <= InlineValue::kMaxValueSize) &&
            (alignof(Value<VT>) <= alignof(ValueInterface)));
  }
 ...
 private:
  ...
  struct Value : ValueInterface {
    ...
    // its alignment <= alignof(ValueInterface).  This check is performed by
    // CanInlineType<VT>().
    //
  ...
  struct InlineValue {
    ...
    alignas(alignof(ValueInterface)) ValueDataArray value_data;
