...
void StartWorkers(int steps, int delay_recv_sec = 0, int delay_send_sec = 0) {
  ...
  auto worker_thread_fn = [&](int worker_id) {
    ...
    TFE_Context* ctx = TFE_NewContext(context_opts, status);
...
TEST(CAPI, MultiClientSingleStepDistributedFunction) { StartWorkers(1); }

TEST(CAPI, MultiClientMultiStepDistributedFunction) { StartWorkers(3); }

TEST(CAPI, MultiClientMultiStepDistributedFunctionWithRecvDelay) {
  StartWorkers(5, 0, 2);
}

TEST(CAPI, MultiClientMultiStepDistributedFunctionWithSendDelay) {
  StartWorkers(5, 2, 0);
}
