...
void UpdateStepId(const tensorflow::protobuf::Map<
  ...
  for (const auto& step_count_pair : step_count) {
    max_step_count = std::max(max_step_count, step_count_pair.second);
  }
  // Filter out noisy and incomplete original step ids.
  for (const auto& step_count_pair : step_count) {
    if (static_cast<double>(step_count_pair.second) / max_step_count >
        kStepFilterRatio) {
      step_map[step_count_pair.first] = kUnknownStep;
    }
  }
...
void ProcessActiveAllocations(int64 peak_bytes_profile_step_id,
  ...
  // frontend.
  std::vector<IndexMetaPair> active_allocs;
  for (const auto& [address, index_meta] : active_alloc_map) {
    active_allocs.push_back(index_meta);
  }
...
void ProcessMemoryProfileProto(MemoryProfile* memory_profile) {
  ...
  for (const auto& id_profile_pair :
       memory_profile->memory_profile_per_allocator()) {
    if (!id_profile_pair.second.memory_profile_snapshots().empty()) {
      memory_profile->add_memory_ids(id_profile_pair.first);
    }
  }
  std::sort(memory_profile->mutable_memory_ids()->begin(),
            memory_profile->mutable_memory_ids()->end());

  for (auto& [memory_id, allocator_memory_profile] :
    ...
    std::sort(
        allocator_memory_profile.mutable_memory_profile_snapshots()->begin(),
        allocator_memory_profile.mutable_memory_profile_snapshots()->end(),
