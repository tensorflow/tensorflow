...
void UpdateOpMetricsDbFromHloModuleMap(OpMetricsDb& op_metrics_db,
  ...
  DeviceOpMetricsDbBuilder device_op_metrics_db_builder(&op_metrics_db);
  for (const OpMetrics& op_metrics : op_metrics_db.metrics_db()) {
    device_op_metrics_db_builder.EnterOpMetadataFromHloModuleMap(
        op_metrics.hlo_module_id(), op_metrics.name(), hlo_module_map);
  }
}
