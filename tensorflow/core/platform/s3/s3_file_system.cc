...
Status S3FileSystem::Stat(const string& fname, FileStatistics* stats/*, TransactionToken* token */) {
...
Status S3FileSystem::GetFileSize(const string& fname, uint64* file_size/*, TransactionToken* token */) {
  FileStatistics stats;
  TF_RETURN_IF_ERROR(this->Stat(fname, &stats));
...
Status S3FileSystem::RenameFile(const string& src, const string& target/*, TransactionToken* token */) {
