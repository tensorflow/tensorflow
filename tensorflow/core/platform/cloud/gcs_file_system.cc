...
Status GcsFileSystem::Stat(const string& fname, FileStatistics* stat/*, TransactionToken* token */) {
  if (!stat) {
...
Status GcsFileSystem::CreateDir(const string& dirname/*, TransactionToken* token */) {
  string bucket, object;
...
Status GcsFileSystem::DeleteDir(const string& dirname/*, TransactionToken* token */) {
  std::vector<string> children;
...
Status GcsFileSystem::GetFileSize(const string& fname, uint64* file_size/*, TransactionToken* token */) {
  if (!file_size) {
...
Status GcsFileSystem::RenameFile(const string& src, const string& target/*, TransactionToken* token */) {
...
Status GcsFileSystem::IsDirectory(const string& fname/*, TransactionToken* token */) {
  string bucket, object;
  TF_RETURN_IF_ERROR(ParseGcsPath(fname, true, &bucket, &object));
  if (object.empty()) {
    bool is_bucket;
