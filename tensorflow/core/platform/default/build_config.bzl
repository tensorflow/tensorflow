...
def tf_proto_library_cc(name, srcs = [], has_services = None,
  ...
  cc_proto_library(
      name = name + "_cc",
      srcs = srcs,
      deps = tf_deps(protodeps, "_cc") + ["@protobuf_git//:cc_wkt_protos"],
      cc_libs = cc_libs + ["@protobuf_git//:protobuf"],
...
def tf_proto_library_py(name, srcs=[], protodeps=[], deps=[], visibility=[],
  ...
  py_proto_library(
      ...
      deps = deps + tf_deps(protodeps, "_py") + ["@protobuf_git//:protobuf_python"],
      protoc = "@protobuf_git//:protoc",
