...
Status HadoopFileSystem::NewRandomAccessFile(
    const string& fname, std::unique_ptr<RandomAccessFile>* result/*, TransactionToken* token */) {
  hdfsFS fs = nullptr;
...
Status HadoopFileSystem::NewWritableFile(
    const string& fname, std::unique_ptr<WritableFile>* result/*, TransactionToken* token */) {
  hdfsFS fs = nullptr;
  TF_RETURN_IF_ERROR(Connect(fname, &fs));
...
Status HadoopFileSystem::FileExists(const string& fname/*, TransactionToken* token */) {
  hdfsFS fs = nullptr;
  TF_RETURN_IF_ERROR(Connect(fname, &fs));
...
Status HadoopFileSystem::DeleteFile(const string& fname/*, TransactionToken* token */) {
  hdfsFS fs = nullptr;
  TF_RETURN_IF_ERROR(Connect(fname, &fs));
...
Status HadoopFileSystem::GetFileSize(const string& fname, uint64* size/*, TransactionToken* token */) {
  hdfsFS fs = nullptr;
  TF_RETURN_IF_ERROR(Connect(fname, &fs));
...
Status HadoopFileSystem::RenameFile(const string& src, const string& target/*, TransactionToken* token */) {
  hdfsFS fs = nullptr;
...
Status HadoopFileSystem::Stat(const string& fname, FileStatistics* stats/*, TransactionToken* token */) {
  hdfsFS fs = nullptr;
  TF_RETURN_IF_ERROR(Connect(fname, &fs));
