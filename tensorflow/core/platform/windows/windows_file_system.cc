...
Status WindowsFileSystem::FileExists(const string& fname/*, TransactionToken* token */) {
  constexpr int kOk = 0;
...
Status WindowsFileSystem::DeleteFile(const string& fname/*, TransactionToken* token */) {
  Status result;
...
Status WindowsFileSystem::DeleteDir(const string& name/*, TransactionToken* token */) {
  Status result;
  std::wstring ws_name = Utf8ToWideChar(name);
...
Status WindowsFileSystem::GetFileSize(const string& fname, uint64* size/*, TransactionToken* token */) {
  string translated_fname = TranslateName(fname);
...
Status WindowsFileSystem::IsDirectory(const string& fname/*, TransactionToken* token */) {
...
Status WindowsFileSystem::RenameFile(const string& src, const string& target/*, TransactionToken* token */) {
  Status result;
...
Status WindowsFileSystem::Stat(const string& fname, FileStatistics* stat/*, TransactionToken* token */) {
  Status result;
  struct _stat sbuf;
  std::wstring ws_translated_fname = Utf8ToWideChar(TranslateName(fname));
