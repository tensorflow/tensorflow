...
Status MemmappedFileSystem::FileExists(const string& fname/*, TransactionToken* token */) {
  if (!mapped_memory_) {
...
Status MemmappedFileSystem::GetFileSize(const string& filename, uint64* size/*, TransactionToken* token */) {
  if (!mapped_memory_) {
    return errors::FailedPrecondition("MemmappedEnv is not initialized");
  }
  const auto dir_element = directory_.find(filename);
...
Status MemmappedFileSystem::Stat(const string& fname, FileStatistics* stat/*, TransactionToken* token */) {
  uint64 size;
...
Status MemmappedFileSystem::DeleteFile(const string& filename/*, TransactionToken* token */) {
  return errors::Unimplemented("memmapped format doesn't support DeleteFile");
}

Status MemmappedFileSystem::CreateDir(const string& dirname/*, TransactionToken* token */) {
  return errors::Unimplemented("memmapped format doesn't support CreateDir");
}

Status MemmappedFileSystem::DeleteDir(const string& dirname/*, TransactionToken* token */) {
