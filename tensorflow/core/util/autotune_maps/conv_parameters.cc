...
StatusOr<string> DeviceIdToIdentifier(int device_id) {
...
ConvParameters::ConvParameters(
  ...
  *proto_.mutable_padding() = {padding.begin(), padding.end()};
  ...
  StatusOr<string> device_identifier = DeviceIdToIdentifier(device_id);
  if (device_identifier.ok()) {
    proto_.set_device_identifier(device_identifier.ValueOrDie());
  } else {
    proto_.set_device_identifier("Unknown Graphics Device");
  }
