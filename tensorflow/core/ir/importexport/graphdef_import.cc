...
absl::StatusOr<unsigned int> GraphDefImporter::ArgNumType(
  ...
  if (!arg_def.type_list_attr().empty()) {
    if (auto v =
            dyn_cast_or_null<ArrayAttr>(attrs.get(arg_def.type_list_attr()))) {
      for (Attribute attr : v) {
        if (auto dtype = dyn_cast<TypeAttr>(attr)) {
  ...
  if (!arg_def.number_attr().empty()) {
    if (auto v =
            dyn_cast_or_null<IntegerAttr>(attrs.get(arg_def.number_attr()))) {
  ...
  if (arg_def.type() != DataType::DT_INVALID) {
    ...
    if (auto v = dyn_cast_or_null<TypeAttr>(attrs.get(arg_def.type_attr()))) {
      dtype = v.getValue();
    } else {
