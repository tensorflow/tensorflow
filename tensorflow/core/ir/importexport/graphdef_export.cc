...
absl::StatusOr<std::optional<GradientDef>> GraphDefExporter::ExportFunction(
  ...
  for (int i = 0, e = func.getNumArguments(); i < e; i += 2) {
    auto attrs = cast<DictionaryAttr>(func.getArgAttrs().value()[i]);
    ...
    TF_RETURN_IF_ERROR(ConvertToDataType(
        cast<TensorType>(func.getArgument(i).getType()).getElementType(),
        &dtype));
    arg.set_type(dtype);
  ...
  for (auto it :
    ...
    DataType dtype;
    TF_RETURN_IF_ERROR(ConvertToDataType(
        cast<TensorType>(std::get<0>(it)).getElementType(), &dtype));
    arg.set_type(dtype);
    // Map the result.
  ...
  for (auto it :
    ...
    std::string name = cast<StringAttr>(attrs.begin()->getValue()).str();
...
static void ExtractExperimentalDebugInfoFromLocation(
  ...
  if (auto fused = dyn_cast<mlir::FusedLoc>(inst_loc)) {
    for (Location loc : fused.getLocations())
      if (auto name_loc = dyn_cast<mlir::NameLoc>(loc)) add_name_loc(name_loc);
    return;
  }
  if (auto name_loc = dyn_cast<mlir::NameLoc>(inst_loc)) add_name_loc(name_loc);
}
...
static absl::StatusOr<std::string> GetValueName(
  ...
  if (auto arg = dyn_cast<BlockArgument>(value)) {
    ...
    auto attrs = cast<DictionaryAttr>(
...
static absl::StatusOr<unsigned int> GetOutputSegmentSize(
  ...
  if (!arg.type_list_attr().empty()) {
    if (auto v = dyn_cast<ArrayAttr>(op->getAttr(arg.type_list_attr())))
      return v.size();
    return InvalidArgument("Type attr not found: ", arg.type_list_attr());
  }
  if (arg.number_attr().empty()) return 1;
  if (auto v = dyn_cast<IntegerAttr>(op->getAttr(arg.number_attr())))
