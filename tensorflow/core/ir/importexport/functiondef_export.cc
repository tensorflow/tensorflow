...
static absl::StatusOr<std::string> GetValueName(Value operand,
  ...
  OpResult op_result = dyn_cast<OpResult>(operand);
  if (!op_result) {
    BlockArgument block_operand = dyn_cast<BlockArgument>(operand);
...
absl::StatusOr<FunctionDef> ConvertGenericFunctionToFunctionDef(
  ...
  if (auto attrs = func_op->getAttrOfType<DictionaryAttr>("tfg.func_attrs")) {
    for (NamedAttribute attr : attrs) {
      ...
      DictionaryAttr dict_attr = dyn_cast<DictionaryAttr>(attr.getValue());
  ...
  if (auto control_outputs =
    ...
    for (Attribute attr : control_outputs) {
      StringAttr output = dyn_cast<StringAttr>(attr);
      if (!output)
        return InvalidArgument(
  ...
  for (int arg_num : llvm::seq<int>(0, func_op.getNumArguments())) {
    ...
    DictionaryAttr arg_attrs = cast<DictionaryAttr>(args_attr[arg_num]);
  ...
  for (const auto &indexed_result : llvm::enumerate(return_op->getOperands())) {
    ...
    if (res_num >= results_attr.size())
      return InvalidArgument("Can't export function ", func_op.getName().str(),
                             ...
                             res_num);
    auto res_attrs = cast<DictionaryAttr>(results_attr[res_num]);
