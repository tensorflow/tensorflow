...
tfg.graph #tf_type.version<producer = 42, min_consumer = 33> {
  ...
  // CHECK-NEXT: }
  // CHECK-SAME: {Tcond = i1, Tout = [f32], output_shapes = [#tf_type.shape<>]}
  // CHECK-SAME: : (tensor<{{.*}}i1>) -> (tensor<{{.*}}>, tensor<{{.*}}>)
...
tfg.graph #tf_type.version<producer = 42, min_consumer = 33> {
  ...
  // CHECK-NEXT: },  {
  // CHECK-NEXT:   yield(%[[C]])
  // CHECK-NEXT: } {Tout = [f32], branch_attrs = [{}, {}, {}], output_shapes = [#tf_type.shape<>]}
...
tfg.graph #tf_type.version<producer = 42, min_consumer = 33> {
  ...
  // CHECK-NEXT: }
  // CHECK-SAME: {T = [f32, i32], output_shapes = [#tf_type.shape<>], parallel_iterations = 10 : i64}
  ...
  {T = [f32, i32], output_shapes = [#tf_type.shape<>], parallel_iterations = 10 : i64}
...
tfg.graph #tf_type.version<producer = 42, min_consumer = 33> {
  ...
  // CHECK-NEXT: } {T = [f32], output_shapes = [#tf_type.shape<>]} : tensor<{{.*}}>
  ...
  } {T = [f32], output_shapes = [#tf_type.shape<>]} : tensor<*xf32>
}
