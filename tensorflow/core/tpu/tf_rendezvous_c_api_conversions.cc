...
#include "tensorflow/core/tpu/tf_rendezvous_c_api_defn.h"

using TF_StatusCallback = std::function<void(const TF_Status*)>;

namespace tensorflow {

TF_AllocatorAttributes ToC(const tsl::AllocatorAttributes& attributes) {
  TF_AllocatorAttributes c_attributes;
  c_attributes.value = attributes.value;
  c_attributes.scope_id = attributes.scope_id;
  return c_attributes;
}

tsl::AllocatorAttributes FromC(const TF_AllocatorAttributes& c_attributes) {
...
void Destroy(tsl::AllocatorAttributes* attributes) {}
