...
int ComputeBatchSizePerTensorCore(
  ...
  int batch_size = input_feature_batch_sizes[0];
  for (const int input_feature_batch_size : input_feature_batch_sizes) {
    batch_size = std::gcd(batch_size, input_feature_batch_size);
  }
  return batch_size;
}
