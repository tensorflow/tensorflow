...
Status ExtractExecuteNodeInfo(const Node* compile_node, const Graph& graph,
  ...
  TF_RETURN_IF_ERROR(
      GetNodeAttribute(compile_node->attrs(), "metadata", &metadata_string));
  ...
  TF_RETURN_IF_ERROR(GetNodeAttribute((*execute_node_info)[0].execute_node->attrs(),
                                      "Targs", &arg_types));
...
Status GetOrCreateBeforeEachIterationNode(Graph* graph, Node* loop_switch_node,
  ...
  DataType dtype;
  TF_RETURN_IF_ERROR(GetNodeAttribute(loop_switch_node->def(), "T", &dtype));
