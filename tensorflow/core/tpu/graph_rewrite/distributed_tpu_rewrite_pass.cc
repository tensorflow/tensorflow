  ...
  for (Node* node : graph->op_nodes()) {
    if (node->type_string() != "_TPUReplicate") {
      ...
      if (cluster_attr == nullptr) {
        ...
        int replica_id;
        if (TryGetNodeAttribute(node->def(), kXlaReplicaIdAttrName, &replica_id)) {
...
Status FillPaddingMap(
  ...
  TF_RETURN_IF_ERROR(
      GetNodeAttribute(replicate_node.attrs(), "padding_map", &padding_map_strs));
...
Status GetStepMarkerLocation(const Node& replicate_node,
  ...
  TF_RETURN_IF_ERROR(GetNodeAttribute(replicate_node.attrs(), "step_marker_location",
...
xla::StatusOr<ShardedInputInfo> CreateOrGetSplitNodesForInputSharding(
  ...
  while (!split_nodes_for_dimension.empty()) {
    ...
    int num_splits;
    TF_RETURN_IF_ERROR(
        GetNodeAttribute(split_node->def(), "num_split", &num_splits));
...
Status SetPaddingNodesDevices(Graph* graph) {
  for (Node* n : graph->op_nodes()) {
    ...
    if (n->type_string() == "Pad" &&
        GetNodeAttribute(n->attrs(), kPostDeviceRewriteAttr, &tpu_padding_attr)
            .ok()) {
      Node* unpadded_input;
      ...
      if (!requested_device.empty() || !assigned_device.empty()) {
        ...
        for (Node* out : unpadded_input->out_nodes()) {
          if (GetNodeAttribute(out->attrs(), kPostDeviceRewriteAttr,
                               &tpu_padding_attr)
                  .ok()) {
        ...
        [&](const Edge* call_edge) {
          ...
          for (auto& associated_function : associated_functions) {
            ...
            for (Node* body_node : g->nodes()) {
              ...
              int index;
              TF_RETURN_IF_ERROR(
                  GetNodeAttribute(body_node->attrs(), "index", &index));
...
bool UseSpmdForXlaPartitioning(const Node* replicate_node) {
  bool spmd_attr;
  if (!replicate_node ||
      !TryGetNodeAttribute(replicate_node->attrs(), "use_spmd_for_xla_partitioning",
                           &spmd_attr)) {
    spmd_attr = false;
  }
  return spmd_attr;
...
Status DistributedTPURewritePass::FindVariableInputs(
  ...
  for (int i = variables_limits.first; i < variables_limits.second; ++i) {
    ...
    if (node->type_string() == "VarHandleOp") {
      DataType dtype;
      TF_RETURN_IF_ERROR(GetNodeAttribute(node->attrs(), "dtype", &dtype));
      variables->push_back(VariableInput{input_edges[i]->src(),
                                         input_edges[i]->src_output(), dtype});
    } else if (node->type_string() == "_Arg") {
      std::vector<DataType> dtypes;
      TF_RETURN_IF_ERROR(GetNodeAttribute(node->attrs(), "_handle_dtypes", &dtypes));
      if (dtypes.empty()) {
        return errors::Internal(
...
/* static */ Status DistributedTPURewritePass::CopyOutsideCompilationEdges(
  ...
  for (Node* node : outside_compilation_nodes) {
    ...
    for (const Edge* edge : in_edges) {
      ...
      if (GetNodeAttribute(src->def(), kXlaIsLiftedArgAttrName, &is_lifted_arg)
              .ok() &&
          GetNodeAttribute(src->def(), kOutsideCompilationAttr,
      ...
      if (GetNodeAttribute(src->def(), kXlaIsPlaceholderForArg,
                           &is_placeholder_for_arg)
              .ok() &&
          GetNodeAttribute(src->def(), kXlaOutsideCompilationInputsAttrName,
...
DistributedTPURewritePass::LowerOutsideCompilationFunctionalNodes(
  ...
  do {
    ...
    auto lower_functional_node = [&flib_def, &g](Node* n) -> Status {
      ...
      int replica_id;
      TF_RETURN_IF_ERROR(
          GetNodeAttribute(n->def(), kXlaReplicaIdAttrName, &replica_id));

      string outside_compilation_attr;
      TF_RETURN_IF_ERROR(GetNodeAttribute(n->def(), kOutsideCompilationAttr,
  ...
  for (Node* n : g->op_nodes()) {
    ...
    string replicate;
    TF_RETURN_IF_ERROR(GetNodeAttribute(n->def(), kTPUReplicateAttr, &replicate));
...
/* static */ Status DistributedTPURewritePass::ParseHostComputeCores(
  ...
  TF_RETURN_IF_ERROR(GetNodeAttribute(replicate_node.attrs(), "host_compute_core",
                                      &hc_core_string));
  TF_RETURN_IF_ERROR(
...
/* static */ Status DistributedTPURewritePass::GetDeviceTopology(
  ...
  TF_RETURN_IF_ERROR(
      GetNodeAttribute(replicate_node.attrs(), "num_replicas", num_replicas));
  if (*num_replicas < 1) {
  ...
  string topology;
  TF_RETURN_IF_ERROR(
      GetNodeAttribute(replicate_node.attrs(), "topology", &topology));
  ...
  TF_RETURN_IF_ERROR(GetNodeAttribute(replicate_node.attrs(), "device_assignment",
                                      &device_assignment));
...
/* static */ Status DistributedTPURewritePass::GetIOTypes(
  ...
  TF_RETURN_IF_ERROR(
      GetNodeAttribute(replicate_node.attrs(), "Tinputs", &input_types));
  TF_RETURN_IF_ERROR(GetNodeAttribute(replicate_node.attrs(), "Tbroadcast_inputs",
                                      &broadcast_input_types));
  TF_RETURN_IF_ERROR(GetNodeAttribute(replicate_node.attrs(),
  ...
  int num_variables;
  TF_RETURN_IF_ERROR(
      GetNodeAttribute(replicate_node.attrs(), "NumVariables", &num_variables));
  ...
  TF_RETURN_IF_ERROR(
      GetNodeAttribute(replicate_node.attrs(), "computation", function));
...
/* static */ Status DistributedTPURewritePass::UpdateHeadTailOutsideCompilation(
  ...
  for (Node* node : head_tail_outside_compilation_nodes) {
    int replica_id;
    TF_RETURN_IF_ERROR(
        GetNodeAttribute(node->def(), kXlaReplicaIdAttrName, &replica_id));
...
/* static */ Status DistributedTPURewritePass::RewriteTPUReplicateNode(
  ...
  string replicate;
  TF_RETURN_IF_ERROR(
      GetNodeAttribute(replicate_node->def(), kTPUReplicateAttr, &replicate));
...
Status DistributedTPURewritePass::Run(
  ...
  for (Node* n : graph->op_nodes()) {
    ...
    if (n->type_string() == "IdentityN" &&
        GetNodeAttribute(n->def(), kXlaOutsideCompilationInputsAttrName,
