...
class HostComputeOp : public XlaOpKernel {
 public:
  explicit HostComputeOp(OpKernelConstruction* ctx) : XlaOpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("cost_estimate_ns", &cost_estimate_));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("key", &key_));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("tpu_core", &tpu_core_));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("Tinputs", &input_dtypes_));
    OP_REQUIRES(ctx, ctx->num_inputs() == input_dtypes_.size(),
                errors::InvalidArgument("Tinputs size=", input_dtypes_.size(),
                                        ...
                                        " inputs."));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("Toutputs", &output_dtypes_));
    OP_REQUIRES(ctx, ctx->num_outputs() == output_dtypes_.size(),
                errors::InvalidArgument("Toutputs size=", output_dtypes_.size(),
                                        ...
                                        " outputs."));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("ancestors", &ancestors_));
    NameAttrList shape_inference_graph;
    OP_REQUIRES_OK(
        ctx, ctx->GetAttribute("shape_inference_graph", &shape_inference_graph));
    if (shape_inference_graph.name().empty()) {
      OP_REQUIRES_OK(ctx, ctx->GetAttribute("shapes", &static_output_shapes_));
    ...
    OP_REQUIRES_OK(
        ctx, ctx->GetAttribute(kXlaTokenInputNodesAttrName, &token_input_nodes_));
    ...
    OP_REQUIRES_OK(ctx, ctx->GetAttribute(kXlaOriginalOutsideCompilationNodeName,
                                          &original_node_name_));
  }
...
class SendToHostOp : public XlaOpKernel {
 public:
  explicit SendToHostOp(OpKernelConstruction* ctx) : XlaOpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("Tinput", &input_dtype_));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("key", &key_));
    OP_REQUIRES_OK(
        ctx, ctx->GetAttribute(kXlaTokenInputNodesAttrName, &token_input_nodes_));
    OP_REQUIRES(ctx, !token_input_nodes_.empty(),
...
class RecvFromHostOp : public XlaOpKernel {
 public:
  explicit RecvFromHostOp(OpKernelConstruction* ctx) : XlaOpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("Toutput", &output_dtype_));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("shape", &output_shape_));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("key", &key_));
    OP_REQUIRES_OK(
        ctx, ctx->GetAttribute(kXlaTokenInputNodesAttrName, &token_input_nodes_));
    OP_REQUIRES(ctx, !token_input_nodes_.empty(),
