...
Status CompileOpMetadataFromContext(OpKernelConstruction* ctx,
  ...
  TF_RETURN_IF_ERROR(ctx->GetAttribute("num_computations", &num_computations));

  std::string metadata_string;
  TF_RETURN_IF_ERROR(ctx->GetAttribute("metadata", &metadata_string));
  ...
  if (function_name != nullptr) {
    TF_RETURN_IF_ERROR(ctx->GetAttribute("function", function_name));
  }

  if (mlir_module != nullptr) {
    TF_RETURN_IF_ERROR(ctx->GetAttribute("mlir_module", mlir_module));
  }
