...
class AllToAllOp : public XlaOpKernel {
 public:
  explicit AllToAllOp(OpKernelConstruction* ctx) : XlaOpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("split_dimension", &split_dimension_));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("concat_dimension", &concat_dimension_));
    OP_REQUIRES_OK(ctx, ctx->GetAttribute("split_count", &split_count_));
  }
