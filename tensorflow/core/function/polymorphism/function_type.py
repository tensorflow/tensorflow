...
PROTO_TO_PY_ENUM = {
    function_type_pb2.Parameter.Kind.POSITIONAL_ONLY: inspect.Parameter.POSITIONAL_ONLY,
    function_type_pb2.Parameter.Kind.POSITIONAL_OR_KEYWORD: inspect.Parameter.POSITIONAL_OR_KEYWORD,
    function_type_pb2.Parameter.Kind.VAR_POSITIONAL: inspect.Parameter.VAR_POSITIONAL,
    function_type_pb2.Parameter.Kind.KEYWORD_ONLY: inspect.Parameter.KEYWORD_ONLY,
    function_type_pb2.Parameter.Kind.VAR_KEYWORD: inspect.Parameter.VAR_KEYWORD,
}
...
class Parameter(inspect.Parameter):
    ...
    def __init__(
        ...
        if optional and kind not in [
            ...
            raise ValueError(
                ...
                + "KEYWORD_ONLY, POSITIONAL_OR_KEYWORD}. Got: "
                + str(kind)
            )
        ...
        super().__init__(
            ...
            annotation=type_constraint if type_constraint is not None else self.empty,
        )

    @classmethod
    def from_proto(cls, proto: Any) -> "Parameter":
        """Generate a Parameter from the proto representation."""
        deserialized_type_constraint = (
            serialization.deserialize(proto.type_constraint)
            if proto.HasField("type_constraint")
            else None
        )
        return Parameter(
            proto.name,
            PROTO_TO_PY_ENUM[proto.kind],
            proto.is_optional,
            deserialized_type_constraint,
        )
    ...
    def to_proto(self) -> function_type_pb2.Parameter:
        ...
        serialized_type_constraint = (
            serialization.serialize(self.type_constraint)
    ...
    def most_specific_common_supertype(
        self, others: Sequence["Parameter"]
    ) -> Optional["Parameter"]:
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (self.name, self.kind, self.optional, self.type_constraint) == (
            other.name,
            other.kind,
            other.optional,
    ...
    def __reduce__(self):
        return (
            self.__class__,
            (self.name, self.kind, self.optional, self.type_constraint),
        )
...
class FunctionType(core.FunctionType):
    ...
    def output(self) -> Optional[trace.TraceType]:
        ...
        return (
            self.return_annotation if self.return_annotation is not self.empty else None
        )
    ...
    def from_proto(cls, proto: Any) -> "FunctionType":
        ...
        return FunctionType(
            [Parameter.from_proto(p) for p in proto.parameters],
            collections.OrderedDict(
                [
                    (c.name, serialization.deserialize(c.type_constraint))
    ...
    def is_supertype_of(self, other: "FunctionType") -> bool:
        ...
        return all(
            capture_type.is_subtype_of(other.captures[name])
            for name, capture_type in self.captures.items()
        )

    def most_specific_common_subtype(
        self, others: Sequence["FunctionType"]
    ...
    def placeholder_arguments(
        ...
        for parameter in self.parameters.values():
            if parameter.kind in {Parameter.VAR_POSITIONAL, Parameter.VAR_KEYWORD}:
                raise ValueError(
                    "Can not generate placeholder values for "
                    "variable length function type."
                )

            if not parameter.type_constraint:
                raise ValueError(
                    "Can not generate placeholder value for "
...
def _make_validated_mono_param(name, value, kind, type_context, poly_type) -> Parameter:
    ...
    if poly_type and not mono_type.is_subtype_of(poly_type):
        raise TypeError(
            f"Parameter `{name}` was expected to be of type "
...
def canonicalize_to_monomorphic(
    args: Tuple[Any, ...],
    kwargs: Dict[Any, Any],
    default_values: Dict[Any, Any],
    capture_types: collections.OrderedDict,
    polymorphic_type: FunctionType,
    ...
    has_var_positional = any(
        p.kind is Parameter.VAR_POSITIONAL for p in polymorphic_type.parameters.values()
    )
    ...
    for name, arg in poly_bound_arguments.arguments.items():
        ...
        if (
            has_var_positional
            and poly_parameter.kind is Parameter.POSITIONAL_OR_KEYWORD
        ):
...
def add_type_constraints(
    function_type: FunctionType, input_signature: Any, default_values: Dict[str, Any]
) -> FunctionType:
    ...
    for param in function_type.parameters.values():
        ...
        sanitized_kind = (
            ...
            if has_var_pos and param.kind is param.POSITIONAL_OR_KEYWORD
            else param.kind
        )
        ...
        if param.name == "self":
            ...
            if constraints:
                parameters.append(
                    Parameter(
                        param.name, sanitized_kind, param.optional, constraints.pop(0)
                    )
                )
                ...
                parameters.append(
                    Parameter(
                        param.name, sanitized_kind, param.optional, type_constraint
                    )
                )
            else:
                raise TypeError(
