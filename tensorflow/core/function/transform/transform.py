...
def transform_function(
    ...
    nested_mlir_transforms: Optional[dict[str, Optional[Union[str, list[str]]]]] = None,
    ...
    if nested_fn_transforms or nested_mlir_transforms:
        ...
        # `nested_mlir_transforms`.
        for atomic_name in nested_mlir_transforms.keys() | nested_fn_transforms.keys():
...
def transform_atomic_function(
    ...
    transform_fn: Union[_FunctionDefTransformerType, list[_FunctionDefTransformerType]],
    ...
    atomic = function_lib.from_func_graph(
        fndef.signature.name, func_graph, fndef.attr, overwrite=True
    )
    # pylint: enable=protected-access
