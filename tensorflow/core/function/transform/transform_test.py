...
class TransformTest(test.TestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        dict(
            testcase_name="transform", transform_fn=add_to_multiply, mlir_pipeline=None
        ),
        dict(
            testcase_name="mlir_pipeline", transform_fn=None, mlir_pipeline="test-pass"
        ),
        dict(
    ...
    def test_concrete_function_with(self, transform_fn, mlir_pipeline):
        ...
        self.assertEqual(f_g(constant_op.constant(2.0), constant_op.constant(3.0)), 5.0)
    ...
    def test_function_spec(self):
        ...
        self.assertSequenceAlmostEqual(
            f.get_concrete_function(*args).pretty_printed_signature().split("\n")[1:],
    ...
    @parameterized.named_parameters(
        dict(testcase_name="fn", transform_fn=add_to_multiply),
        dict(testcase_name="mlir", mlir_pipeline="test-pass"),
        dict(
    ...
    @test_util.run_v2_only
    def test_nested_python_function_transform_with(
        self, transform_fn=None, mlir_pipeline=None
    ):
        @def_function.function
        def f(x, y):
            def inner_add():
        ...
        # Transform `f`.
        updated_f = transform.transform_function(
            f, inputs=inputs, transform_fn=transform_fn, mlir_pipeline=mlir_pipeline
        )
    ...
    @test_util.run_v2_only
    def test_nested_transform_with(
        self, transform_fn=None, mlir_pipeline=None, nested_fn=False, nested_mlir=False
    ):
        @def_function.function
        def f(x, y, z):
            @def_function.function
    ...
    @test_util.run_v2_only
    def test_nested_transform_in_tf_function_with(
        self, transform_fn=None, mlir_pipeline=None, nested_fn=False, nested_mlir=False
    ):
        @def_function.function
        def g(x, y, z):
    ...
    def test_save_transform_for_all_signatures(self):
        ...
        self.assertEqual(
            m.f(
                ...
                constant_op.constant(False, dtypes.bool),
            ),
            3,
        )
        ...
        self.assertEqual(
            m.f(
                constant_op.constant(1, dtypes.int32),
                constant_op.constant(2, dtypes.int32),
                constant_op.constant(True, dtypes.bool),
            ),
            5,
        )
