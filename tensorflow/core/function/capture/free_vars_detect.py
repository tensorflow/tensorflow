...
def _detect_function_free_vars(fn):
    ...
    assert isinstance(fn, types.FunctionType) or isinstance(fn, types.MethodType), (
...
def generate_free_var_logging(fn, fn_threshold=5, var_threshold=10):
    ...
    if not (
        ...
        or isinstance(fn, types.MethodType)
        or isinstance(fn, functools.partial)
        or isinstance(fn, functools.partialmethod)
    ):
        return None
    fn = _handle_wrap_partial_func(fn)
    ...
    fn_threshold -= 1
    try:
        tf_fn_line = one_line_logging(
            tf_fn_name, fn_vars_map[tf_fn_name], var_threshold
        )
    except Exception:  # pylint: disable=broad-except
