...
void GraphDebugInfoBuilder::AccumulateStackTrace(
  ...
  if (trace_to_index_.contains(p)) {
    trace_index = trace_to_index_[p];
  } else {
...
void GraphDebugInfoBuilder::AppendToStackTraceProto(
    ...
    GraphDebugInfo::StackTrace& stack_trace_proto) {
  int frame_index = 0;
  if (frame_to_index_.contains(stack_frame)) {
    frame_index = frame_to_index_[stack_frame];
  } else {
    ...
    if (file_name_to_index_.contains(stack_frame.file_name)) {
      frame.set_file_index(file_name_to_index_[stack_frame.file_name]);
    } else {
