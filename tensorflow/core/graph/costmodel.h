...
class CostModel {
 public:
  ...
  // of "node".
  void RecordMaxMemSize(const Node* node, int output_slot, Bytes bytes);

  // Returns the maximum size in bytes of the tensor generated by "output_slot"
  // of "node".
  Bytes MaxMemSize(const Node* node, int output_slot) const;

  // Returns the size in bytes of temporary memory consumed by "node".
  Bytes TempMemSize(const Node* node) const;
