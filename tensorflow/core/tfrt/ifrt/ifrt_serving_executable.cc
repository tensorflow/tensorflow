...
absl::Status LiftInlinedMeshes(mlir::ModuleOp module) {
  ...
  // that we lift inlined meshes. We don't need to invoke SdyRoundTrip export
  // here as MLIR to HLO will perform that.
...
IfrtServingExecutable::CreateExecutableSynchronously(
  ...
  if (use_shardy_partitioner) {
    TF_RETURN_IF_ERROR(LiftInlinedMeshes(mlir_hlo_module.get()));
  }
