...
tsl::DeviceReservation IfrtServingCoreSelector::Enqueue(int device_id,
                                                        int64_t program_id) {
  device_selector_->Enqueue(device_id, absl::StrCat(program_id));
  return tsl::DeviceReservation(device_id, device_selector_);
}
