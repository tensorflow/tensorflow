...
TEST(PjRtUtilTest, ResetPjRtClientInTFGlobalResourceManager) {
  xla::CpuClientOptions options;
  options.asynchronous = true;
  options.cpu_device_count = 1;
  ...
  TF_ASSERT_OK(ResetPjRtClientInTFGlobalResourceManager(DEVICE_CPU));
  ...
  TF_ASSERT_OK(
      SetPjRtGpuClientCreationInfoInTFGlobalResourceManager(std::move(info)));
  TF_ASSERT_OK(ResetPjRtClientInTFGlobalResourceManager(DEVICE_GPU));
  TF_ASSERT_OK_AND_ASSIGN(PjRtGpuClientCreationInfo * retrieved_info,
                          GetPjRtGpuClientCreationInfo());
