...
Type ConsolidateAttributesPassImpl::refineTypeWithOutputShapes(
  ...
  if (auto output_shapes =
          dyn_cast_or_null<ArrayAttr>(attrs.get(output_shapes_id_))) {
    if (output_shapes.size() == 1 && IsArrayOfShapes(output_shapes)) {
      ...
      return GetReifiedType(type, cast<ShapeAttr>(output_shapes[0]));
    }
  }
  return type;
}
...
Type ConsolidateAttributesPassImpl::refineTypeWithHandleData(
  ...
  // well-formed.
  for (Type type : cast<ArrayAttr>(handle_data).getAsValueRange<TypeAttr>())
    subtypes.push_back(cast<TensorType>(type));
  auto resource =
...
ArrayAttr ConsolidateAttributesPassImpl::reifyAndDropFunctionArgumentAttributes(
  ...
  ArrayAttr input_shapes =
      dyn_cast_or_null<ArrayAttr>(func->getAttr(input_shapes_id_));
  ...
  for (auto i : llvm::seq<unsigned>(0, num_args)) {
    ...
    if (input_shapes)
      arg_type = GetReifiedType(arg_type, cast<ShapeAttr>(input_shapes[i]));
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ArrayAttr output_shapes =
        dyn_cast_or_null<ArrayAttr>(op->getAttr(output_shapes_id_));
...
DictionaryAttr PrepareAttributesForExportPassImpl::prepareAttributesFor(
  ...
  if (attrs.erase(regenerate_output_shapes_id_)) {
    auto shape = ShapeAttr::get(&getContext(),
                                isa<RankedTensorType>(type)
                                    ? cast<RankedTensorType>(type).getShape()
                                    : std::optional<ArrayRef<int64_t>>());
    attrs.set(output_shapes_id_, ArrayAttr::get(&getContext(), {shape}));
  }
  auto element_type = cast<TensorType>(type).getElementType();
  if (auto resource = dyn_cast<ResourceType>(element_type)) {
  ...
  if (isa<tf_type::TensorFlowRefType>(element_type))
...
static ArrayAttr GetElementTypesAttr(PatternRewriter &rewriter,
  ...
  for (Value value : values) {
    types.push_back(
        TypeAttr::get(cast<TensorType>(value.getType()).getElementType()));
  }
...
struct MaterializeIfAttrs : public MaterializeAttrsPattern<IfLikeOp> {
  ...
  LogicalResult matchAndRewrite(IfLikeOp op,
    ...
    attrs.set(op.getTcondAttrName(),
              TypeAttr::get(
                  cast<TensorType>(op.getCond().getType()).getElementType()));
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    for (Value result : results) {
      if (auto ranked = dyn_cast<RankedTensorType>(result.getType())) {
