...
NamedAttrList BasePattern::BuildAttributes(RegionAttr preserved,
  ...
  const auto build_attrs = [&](ArrayAttr attr, auto &it,
    ...
    NamedAttrList attrs(attr ? cast<DictionaryAttr>(attr[it.index()])
                             : DictionaryAttr());
...
LogicalResult ConvertCaseLikeOp<CaseLikeRegionOp, CaseLikeOp>::matchAndRewrite(
  ...
  for (const auto &it : llvm::enumerate(op.getBranches())) {
    ...
    RegionAttr preserved = op.getRegionAttrs()
                               ? cast<RegionAttr>(op.getRegionAttrsAttr()[idx])
                               : nullptr;
    DictionaryAttr attrs = branch_func_attrs
                               ? cast<DictionaryAttr>(branch_func_attrs[idx])
                               : nullptr;
