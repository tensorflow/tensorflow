...
static FailureOr<TFOp> CreateConstantTensorOp(
  ...
  if (isa<VariantType>(type)) return failure();
  ...
  state.attributes.set(
      "dtype",
      TypeAttr::get(cast<ShapedType>(tensor_value.getType()).getElementType()));
...
static TFOp GetControlAnchorForSwitchResult(
  ...
  identity_op.setName(Twine(switch_op.name(), "/ControlDependencyCtrl_") +
                      Twine(cast<OpResult>(value).getResultNumber()));
  return identity_op;
}
...
static Value GetControlDependency(OpBuilder &builder, Value value) {
  if (isa<ControlType>(value.getType())) return value;
...
static void AddControlOperand(Operation *op, Value control,
  ...
  assert(isa<ControlType>(control.getType()));
...
static FailureOr<TFOp> ReplaceOpWithConstant(OpBuilder &builder, Operation *op,
  ...
  auto res = cast<ShapedType>((*op->result_type_begin()));
...
static FailureOr<TFOp> ReplaceOpWithBroadcastTo(OpBuilder &builder, TFOp op,
                                                int idx_to_replace) {
  ShapedType tensor_type = cast<ShapedType>((*op->result_type_begin()));
...
bool OpPropertyHelper::IsFoldableUncached(TFOp op) {
  ...
  for (Value operand : operands) {
    ...
    if (operand_op && dialect_->IsConstant(operand_op)) {
      ...
      if (!dtype || isa<tf_type::StringType>(dtype.getValue())) return false;
  ...
  for (Value operand : operands) {
    auto shape = dyn_cast<ShapedType>(operand.getType());
  ...
  for (Value res : op->getResults().drop_back()) {
    auto shape = dyn_cast<ShapedType>(res.getType());
    if (!shape || !shape.hasStaticShape()) continue;
    auto element_type = shape.getElementType();
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    for (TypedAttr r : result)
      if (r && isa<VariantType>(r.getType())) return failure();
...
class MaterializeShapeOp : public FolderPatternBase<MaterializeShapeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto input_shape = cast<ShapedType>(input.getType());
...
class MaterializeSizeOp : public FolderPatternBase<MaterializeSizeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ShapedType result_type = cast<ShapedType>((*op->result_type_begin()));
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    for (const auto &it : llvm::enumerate(TFOp(op).getNonControlOperands())) {
      ...
      auto operand_shape = cast<ShapedType>(operand.getType());
...
class MaterializeReductionIndices
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto indices_shape = cast<ShapedType>(indices->getResult(0).getType());
    ...
    auto input_shape = cast<ShapedType>(op->getOperand(0).getType());
    // Unexpected graph, don't try to change it.
    if (!input_shape.hasRank() || input_shape.getRank() < 1) return failure();

    auto output_shape = cast<ShapedType>(op->getResult(0).getType());
    ...
    if (!full_reduction) {
      ...
      for (Operation *user : op->getResult(0).getUsers()) {
        ...
        auto shape = cast<ShapedType>(user->getResult(0).getType());
...
class MaterializeFillNode : public FolderPatternBase<MaterializeFillNode> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto output_type = cast<ShapedType>(op->getResult(0).getType());
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (output_element_type.isIntOrIndex()) {
      ...
      const_attr = SplatElementsAttr::get(
          output_type,
          APFloat(cast<FloatType>(output_element_type).getFloatSemantics(),
                  value));
    }
...
class RemoveShuffleOp : public FolderPatternBase<RemoveShuffleOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ShapedType x_shape = cast<ShapedType>(op->getOperand(0).getType());
...
class RemoveRandomShuffleOp : public FolderPatternBase<RemoveRandomShuffleOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
                                PatternRewriter &rewriter) const override {
    auto shape = cast<ShapedType>(op->getOperand(0).getType());
...
class RemoveReverse : public FolderPatternBase<RemoveReverse> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
                                PatternRewriter &rewriter) const override {
    ShapedType tensor_type = cast<ShapedType>(op->getOperand(0).getType());
...
class SimplifySliceOp : public FolderPatternBase<SimplifySliceOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ShapedType input_type = cast<ShapedType>(op->getOperand(0).getType());
...
class SimplifySqueezeOp : public FolderPatternBase<SimplifySqueezeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
                                PatternRewriter &rewriter) const override {
    auto shape_type = cast<ShapedType>(op->getOperand(0).getType());
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (ShapedType values_type =
            dyn_cast<ShapedType>(non_control_operands[0].getType());
        !values_type || isa<VariantType>(values_type.getElementType()))
      return failure();

    // It's unsafe to add a control dependency on the feed node, because it
    // might have been never executed otherwiwise.
    if (isa<BlockArgument>(non_control_operands[0])) return failure();
...
class SimplifyReductionOp : public FolderPatternBase<SimplifyReductionOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto input_type = cast<ShapedType>(op->getOperand(0).getType());
    auto op_type = cast<ShapedType>((*op->result_type_begin()));
 ...
 private:
  Operation *ReplaceReductionWithReshape(OpBuilder &builder, Operation *op,
    ...
    const int new_num_dimensions =
        cast<ShapedType>((*op->result_type_begin())).getRank();
...
class SimplifyArithmeticOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ShapedType op_type = cast<ShapedType>((*op->result_type_begin()));
    ShapedType x_type = cast<ShapedType>((*x->result_type_begin()));
    ShapedType y_type = cast<ShapedType>((*y->result_type_begin()));
    ...
    if (y_matches_output_shape && is_any_div && x_is_one) {
      ...
      if (isa<FloatType>(type_attr.getValue()) ||
          isa<ComplexType>(type_attr.getValue())) {
...
class ReduceDivToReciprocalMul
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (dialect_->IsDiv(op) && !(isa<FloatType>(type_attr.getValue()) ||
                                 isa<ComplexType>(type_attr.getValue()))) {
      return failure();
    }
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (!dialect_->IsConstant(y_node)) {
      ...
      auto c_shape = cast<ShapedType>(const_op->getResult(0).getType());
      auto x_shape = cast<ShapedType>(x_value.getType());
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    for (OpOperand &operand : op->getOpOperands()) {
      Value v = operand.get();
      if (isa<ControlType>(v.getType())) break;
      ...
      int res_index = cast<OpResult>(v).getResultNumber();
...
class MulConvPushDown : public ConstantPatternBase<MulConvPushDown, FolderTrait,
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ShapedType mul_shape = cast<ShapedType>((*op->result_type_begin()));
    ShapedType conv_shape = cast<ShapedType>((*conv_node->result_type_begin()));
    // TODO(chiahungduan): Symbolic shape equivalence is acceptable.
    ...
    auto filter_shape = cast<ShapedType>(conv_node->getOperand(1).getType());

    Operation *const_node =
        left_child_is_constant ? mul_left_child : mul_right_child;
    auto const_node_shape =
        cast<ShapedType>((*const_node->result_type_begin()));
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto hasRank = [&](Value value) {
      return cast<ShapedType>(value.getType()).hasRank();
    };
    ...
    const int left_leaf_rank =
        cast<ShapedType>(add_child->getOperand(0).getType()).getRank();
    const int right_leaf_rank =
        cast<ShapedType>(add_child->getOperand(1).getType()).getRank();
    ...
    auto vector_type =
        cast<ShapedType>(add_child->getOperand(vector_idx).getType());
    Type vector_d_type = vector_type.getElementType();

    auto const_type = cast<ShapedType>(const_child->getResultTypes()[0]);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ShapedType matrix_type =
        cast<ShapedType>(add_child->getOperand(matrix_idx).getType());
    ...
    ShapedType const_type =
        cast<ShapedType>(const_child->getResult(0).getType());
    const int const_rank = const_type.getRank();
    Type const_d_type = const_type.getElementType();
...
class SimplifySelectOpBase : public FolderPatternBase<ConcreteType> {
 protected:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto condition_type = cast<ShapedType>(op->getOperand(0).getType());
    auto t_type = cast<ShapedType>(op->getOperand(1).getType());
    auto e_type = cast<ShapedType>(op->getOperand(2).getType());
