...
module  {
  tfg.graph #tf_type.version<producer = 1010, min_consumer = 0> {
    ...
    %Conv2D, %ctl_1 = Conv2D(%Placeholder, %Const) name("conv") {T = f32, data_format = "NHWC", dilations = [1, 1, 1, 1], explicit_paddings = [], padding = "VALID", strides = [1, 1, 1, 1], use_cudnn_on_gpu = true} : (tensor<4x10x10x3xf32>, tensor<2x2x3x5xf32>) -> (tensor<*xf32>)
    ...
    %Mul, %ctl_4 = Mul(%Const_2, %Conv2D) name("mul") {T = f32} : (tensor<f32>, tensor<*xf32>) -> (tensor<*xf32>)
  }
}
