...
std::optional<std::string> GetColocateStringName(const Node& node) {
  std::string device = "";
  std::string colocation_prefix = "loc:@";
  std::string colocation_name;
  for (auto node : node.out_nodes()) {
    if (!IsValidIdentityNode(*node)) return std::nullopt;
    if (device == "") {
      device = node->requested_device();
      colocation_name = absl::StrCat(colocation_prefix, node->name());
    } else if (device != node->requested_device()) {
      return std::nullopt;
    }
  }
...
Status ColocatePredecessorTreesPass::Run(
  ...
  for (Node* node : graph->nodes()) {
    ...
    if (AreAllInNodesQualifiedConst(*node)) {
