...
//   called physical GPU id as below. See
//   http://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#env-vars
//   for more details.
// - physical GPU id (also called *visible* GPU id in
...
//   hardware, so the mapping between TF GPU id and physical GPU id is not a 1:1
...
// the following mapping between physical GPU id and physical GPU id:
//
//        physical GPU id ->  physical GPU id
//                 0  ->  1
//                 1  ->  2
//                 2  ->  3
//
...
// the following mappting between TF GPU id and physical GPU id:
//
//                  TF GPU id  ->  physical GPU ID
//      0 (i.e. /device:GPU:0) ->  2
//      1 (i.e. /device:GPU:1) ->  0
//
// Note that physical GPU id 1 is filtered out by GPUOptions::visible_device_list,
...
//
// We create strong-typed integer classes for both TF GPU id and physical GPU id to
...
// TF GPU id (or physical GPU id) we should use TfGpuId (or PhysicalGpuId) instead of a
// raw integer.
TF_LIB_GTL_DEFINE_INT_TYPE(TfGpuId, int32);
TF_LIB_GTL_DEFINE_INT_TYPE(PhysicalGpuId, int32);
