...
class BaseGPUDeviceFactory : public DeviceFactory {
 public:
  ...
  struct InterconnectMap {
    ...
    std::set<std::pair<PhysicalGpuId, PhysicalGpuId>> directed_links;
  };
 ...
 private:
  ...
  // Returns into 'ids' the list of valid physical GPU ids, in the order that
  ...
  // ids.
  Status GetValidDeviceIds(const std::vector<PhysicalGpuId>& visible_gpu_order,
                           std::vector<PhysicalGpuId>* ids);

  // visible_gpu_initialized_[physical_gpu_id] is true if visible GPU physical_gpu_id
