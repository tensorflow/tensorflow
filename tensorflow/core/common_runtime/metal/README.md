# Metal PluggableDevice plugin

A Metal GPU backend for Apple silicon, built from this tree as a loadable
PluggableDevice. It changes nothing about how TensorFlow itself is built: it
adds one target that produces one shared object.
...
TensorFlow loads it the way it loads any PluggableDevice, by finding it in
`site-packages/tensorflow-plugins` or through
`TF_PLUGGABLE_DEVICE_LIBRARY_PATH`, and `/device:GPU:0` appears.
...
a released TensorFlow declares but does not export, which is a separate bug
filed as #126374. None is unaccounted for.
...
`dlopen`'ed: `metal_plugin_init.cc` exports `SE_InitPlugin` and
`TF_InitKernel`, which is what TensorFlow looks up. That reuses the whole of
...
| `metal_plugin_init.cc` | `SE_InitPlugin` and `TF_InitKernel`, the two symbols TensorFlow looks up |
