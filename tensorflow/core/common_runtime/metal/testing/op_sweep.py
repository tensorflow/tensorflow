"""Runs every op the Metal backend registers and compares it to the CPU kernel.

Run it against a TensorFlow built with --config=metal:

    python -m tensorflow.core.common_runtime.metal.testing.op_sweep

or against the backend built as an out-of-tree plugin:

    python .../op_sweep.py --plugin path/to/libmetal_plugin.dylib
...
from tensorflow.core.common_runtime.metal.testing import op_recipes as recipes
...
def check_cudnn_rnn(name, kwargs, outputs):
  ...
  if "CanonicalToParams" in name:
    expected = kwargs.pop("_expected_size")
    packed = int(first.shape[0])
    return packed == expected, f"packs {packed} floats"
...
def main():
  ...
  parser.add_argument("--ops",
                      default=os.path.join(HERE, "registered_ops.txt"))
  # An in-tree build has the backend linked in already; a plugin build has to
  # be told where the shared object is.
