...
bool SendSingletonTensorsAsProtos() {
  static bool send_singleton_tensors_as_protos = []() {
    bool result;
    TF_CHECK_OK(tsl::ReadBoolFromEnvVar("TF_SEND_SINGLETON_TENSORS_AS_PROTOS",
                                        false, &result));
    return result;
  }();
  return send_singleton_tensors_as_protos;
}
...
Status EagerRemoteExecute(EagerOperation* op, TensorHandle** retvals,
  ...
  tensorflow::Device* op_device = std::get<Device*>(op->Device());
  {
    ...
    for (int i = 0, end = inputs->size(); i < end; i++) {
      ...
      if (((input->DataType() != DT_RESOURCE) &&
           (input->DataType() != DT_VARIANT)) &&
          (num_elements == 1) && SendSingletonTensorsAsProtos()) {
        ...
        LOG(INFO) << "input_tensor_proto: "
                  << input_tensor_proto->DebugString();
        ...
        LOG(INFO) << "non proto-able input_handle: # elements " << num_elements
                  << " type " << input->TypeString() << " dtype "
                  << input->DataType();
        auto* input_handle =
