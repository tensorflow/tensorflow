...
struct DilationBackpropInput<GPUDevice, T> {
  void operator()(const GPUDevice& d, typename TTypes<T, 4>::ConstTensor input,
    ...
    TF_CHECK_OK(GpuLaunchKernel(
        ...
        d.stream(), config->virtual_thread_count, in_backprop.data()));

    // Accumulate.
    total_count = batch * output_rows * output_cols * depth;
    config = GetGpuLaunchConfig64(total_count, d,
                                  DilationBackpropInputKernel<T>, 0, 0);
    CHECK_OK(config.status());  // Crash OK
    TF_CHECK_OK(GpuLaunchKernel(
...
struct DilationBackpropFilter<GPUDevice, T> {
  void operator()(const GPUDevice& d, typename TTypes<T, 4>::ConstTensor input,
    ...
    TF_CHECK_OK(GpuLaunchKernel(
        SetZero<T>, config->block_count, config->thread_per_block, 0,
        d.stream(), config->virtual_thread_count, filter_backprop.data()));

    // Accumulate.
    total_count = batch * output_rows * output_cols * depth;
    config = GetGpuLaunchConfig64(total_count, d,
                                  DilationBackpropFilterKernel<T>, 0, 0);
    CHECK_OK(config.status());  // Crash OK
    TF_CHECK_OK(GpuLaunchKernel(
