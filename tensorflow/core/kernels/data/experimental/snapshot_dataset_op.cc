...
SnapshotDatasetV2Op::SnapshotDatasetV2Op(OpKernelConstruction* ctx)
  ...
  if (ctx->HasAttr(kReaderPrefix)) {
    OP_REQUIRES_OK(ctx, ctx->GetAttribute(kReaderPrefix, &reader_prefix_));
  }

  if (ctx->HasAttr(kWriterPrefix)) {
    OP_REQUIRES_OK(ctx, ctx->GetAttribute(kWriterPrefix, &writer_prefix_));
  }
  OP_REQUIRES_OK(ctx, ctx->GetAttribute(kHashValid, &hash_valid_));
  int64 hash;
  OP_REQUIRES_OK(ctx, ctx->GetAttribute(kHash, &hash));
