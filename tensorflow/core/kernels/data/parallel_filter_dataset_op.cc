  ...
  class Iterator : public DatasetIterator<Dataset> {
   ...
   private:
    struct InvocationResult {
      ...
      std::list<std::shared_ptr<InvocationResult>>::iterator iter;
    ...
    void CallFunction(const std::shared_ptr<IteratorContext>& ctx,
      ...
      auto done = [this, ctx, result](Status status) {
        if (result->predicate_values.size() != 1 ||
          ...
          this->invocation_results_.push_back(result);
        } else {
          auto matched = result->predicate_values[0].scalar<bool>()();
          if (matched) {
            mutex_lock l(*this->mu_);
            // Entry passes filter
...
void ParallelFilterDatasetOp::MakeDataset(OpKernelContext* ctx,
  ...
  *output =
      new Dataset(ctx, input, num_parallel_calls, std::move(captured_func));
}
