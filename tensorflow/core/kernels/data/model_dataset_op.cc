...
class ModelDatasetOp::Dataset : public DatasetBase {
 public:
  ...
  std::unique_ptr<IteratorBase> MakeIteratorInternal(
    ...
    return std::unique_ptr<IteratorBase>(new Iterator(
        {this, strings::StrCat(prefix, "::Model")}, model_factory_));
  }
 ...
 private:
  class Iterator : public DatasetIterator<Dataset> {
   public:
    explicit Iterator(const Params& params, ModelFactory model_factory)
        : DatasetIterator<Dataset>(params), model_(model_factory()) {}
  ...
  const ModelFactory model_factory_;
};
...
void ModelDatasetOp::MakeDataset(OpKernelContext* ctx, DatasetBase* input,
                                 DatasetBase** output) {
  *output = new ModelDatasetOp::Dataset(ctx, input, CreateModelFactory());
}

ModelDatasetOp::ModelFactory ModelDatasetOp::CreateModelFactory() const {
  return []() { return std::make_shared<model::Model>(); };
}
