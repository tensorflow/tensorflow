...
//
//   GlobalBatchStats().model("M").batch_size(4).tpu_cost.Register(cost);
//
// To get the mean cost later, do:
//
//   std::optional<absl::Duration> cost =
//       GlobalBatchStats().model("M").batch_size(4).tpu_cost.mean();
//
...
class BatchStats {
 public:
  // Returns a reference to ModelBatchStats for the provided model name.
  //
  // Upon invocation with a not-yet-seen model_name, creates an empty
  // ModelBatchStats for this model.
  //
  // The returned reference persist for as long as 'this' is alive.
  ModelBatchStats& model(absl::string_view model_name) {
    mutex_lock l(mu_);
    return model_batch_stats_by_model_name_[model_name];
  }
 ...
 private:
  ...
  // map and don't track their lifetime. We are using the node hash map so that
  // elements, once created, are fixed in memory.
  absl::node_hash_map<std::string, ModelBatchStats>
      model_batch_stats_by_model_name_ TF_GUARDED_BY(mu_);
};
