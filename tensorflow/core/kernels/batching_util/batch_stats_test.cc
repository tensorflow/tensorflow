...
TEST(BatchStatsTest, BasicOperation) {
  BatchStats stats;
  stats.model("a").batch_size(1).tpu_cost().Register(absl::Hours(5));
  ASSERT_EQ(stats.model("a").batch_size(1).tpu_cost().mean(), absl::Hours(5));
}

TEST(BatchStatsTest, ModelBatchStatsAreUniqueForEachModelName) {
  BatchStats stats;
  ASSERT_NE(&stats.model("a"), &stats.model("b"));
}
