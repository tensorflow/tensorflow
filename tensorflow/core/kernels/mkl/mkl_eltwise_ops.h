...
// See docs in ../../ops/nn_ops.cc and ../../ops/math_ops.cc
#ifdef INTEL_MKL
...
class MklEltwiseOpBase : public OpKernel {
  ...
  void Compute(OpKernelContext* context) override {
    try {
      ...
      if (src_tensor.shape().num_elements() == 0) {
        ...
        tf_shape_dst = MklGetInput(context, KSrcIndex).shape();
      ...
      if (is_src_reordered) {
        ...
        AllocateOutputSetMklShape(context, KDstIndex, dnn_shape_dst);
      }
 ...
 protected:
  ...
  const int KDstIndex = 0;  // index of dst output tensor
};
 ...
 public:
  ...
  void Compute(OpKernelContext* context) {
    try {
      ...
      KDiffDstIndex = GetDiffDstIndex();
      KSrcIndex = GetSrcIndex();
      KDiffSrcIndex = GetDiffSrcIndex();
      ...
      if (src_tensor.shape().num_elements() == 0) {
        ...
        tf_shape_diff_src = MklGetInput(context, KDiffSrcIndex).shape();
      ...
      OP_REQUIRES_OK(context, context->forward_input_or_allocate_output(
                                  {KDiffDstIndex}, KDiffSrcIndex,
                                  tf_shape_diff_src, &diff_src_tensor));
      AllocateOutputSetMklShape(context, KDiffSrcIndex, dnn_shape_diff_src);
