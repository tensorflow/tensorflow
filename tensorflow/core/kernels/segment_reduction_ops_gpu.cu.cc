...
// SortedSegmentMeanCustomKernel adds input data just as
// SortedSegmentReductionCustomKernel does except that the value to be
// accumulated is divided every time the accumulation is executed with an
// auxiliary array, that is, divided by the frequency of occurrence of the
// current id, so as to achieve the purpose of average calculation.
// The reason for using additional memory space to store auxiliary arrays
// instead of traversing forward or backward every time, is to avoid in the
// worst case when all the elements of 'segment_ids' are same, the entire kernel
// degenerates into serial execution.
template <typename T, typename Index, int OuterDimTileSize>
__global__ void SortedSegmentMeanCustomKernel(
    const Index input_outer_dim_size, const Index inner_dim_size,
    const Index output_outer_dim_size, const Index* __restrict__ segment_ids,
    const T* __restrict__ input, T* __restrict__ output,
    const Index* __restrict__ segment_ids_freq,
  ...
  for (int stripe_index : GpuGridRangeX(total_stripe_count)) {
    ...
    for (Index j = 0; j < actual_stripe_height; j++) {
      ...
      if (current_output_segment_id > last_output_segment_id) {
        const Index output_index =
            last_output_segment_id * inner_dim_size + segment_offset;
        cur_segment_num = segment_ids_freq[input_outer_dim_index_base + j - 1];
        // Decide whether to write result to global memory using atomic
        // operations.
        if (last_output_segment_id == first_segment_id) {
    ...
    const Index output_index =
        last_output_segment_id * inner_dim_size + segment_offset;
    cur_segment_num = segment_ids_freq[last_input_outer_dim_index];
...
// SetSegmentFreq counts the frequency of occurrence of each element in
// 'segment_ids', and stores the result in each corresponding position
// of 'segment_freq'.
template <typename Index, int OuterDimTileSize>
__global__ void SetSegmentFreq(const int work_count, const int element_size,
                               const Index* __restrict__ segment_ids,
                               Index* __restrict__ segment_freq) {
  // Check that the grid is one dimensional and index doesn't overflow.
  assert(blockDim.y == 1 && blockDim.z == 1);
  assert(blockDim.x * gridDim.x / blockDim.x == gridDim.x);

  for (int work_index : CudaGridRangeX(work_count)) {

    const Index element_index_base = Index(OuterDimTileSize) * work_index;
    const Index actual_length = min(Index(OuterDimTileSize),
                                    element_size - element_index_base);

    Index first_segment_id = segment_ids[element_index_base];
    Index last_segment_id = segment_ids[element_index_base + actual_length - 1];

    Index cur_segment_id_freq = 0;
    for (Index j = 0; j < actual_length; j++) {
      Index current_element_index = element_index_base + j;
      Index current_segment_id = segment_ids[current_element_index];

      if (current_segment_id > last_segment_id) {
        if (last_segment_id == first_segment_id) {
          Index previous_freq = 0;
          for (Index k = 1; element_index_base - k >= 0; k++) {
            if (segment_ids[element_index_base] !=
                  segment_ids[element_index_base - k])
              break;
            previous_freq++;
          }
          cur_segment_id_freq += previous_freq;
          for (Index k = element_index_base; k < current_element_index; k++) {
            segment_freq[k] = cur_segment_id_freq;
          }
        } else {
          for (Index k = current_element_index - cur_segment_id_freq;
               k < current_element_index;
               k++) {
            segment_freq[k] = cur_segment_id_freq;
          }
        }
        cur_segment_id_freq = 0;
      }
      cur_segment_id_freq++;
      last_segment_id = current_segment_id;
    }

    Index current_element_index = element_index_base + actual_length;
    Index after_freq = 0;
    for (Index k = current_element_index; k < element_size; k++) {
      if (segment_ids[current_element_index - 1] != segment_ids[k])
        break;
      after_freq++;
    }
    cur_segment_id_freq += after_freq;
    for (Index k = current_element_index - cur_segment_id_freq + after_freq;
         k < current_element_index;
         k++) {
      segment_freq[k] = cur_segment_id_freq;
    }
...
void SegmentMeanFunctor<T, Index>::operator()(
  ...
  const auto segment_ids_data = segment_ids.data();
  Tensor segment_ids_freq_tensor;
  const Tensor& segment_ids_tensor = ctx->input(1);
  ctx->allocate_temp(segment_ids_tensor.dtype(), segment_ids_shape,
                     &segment_ids_freq_tensor);
  const auto segment_ids_freq = segment_ids_freq_tensor.flat<Index>().data();

  config = GetGpuLaunchConfig(input_outer_dim_num_stripe, d);
  TF_CHECK_OK(GpuLaunchKernel(
      SetSegmentFreq<Index, OuterDimTileSize>,
      config.block_count, config.thread_per_block, 0, d.stream(),
      input_outer_dim_num_stripe, input_outer_dim_size,
      segment_ids_data, segment_ids_freq);
  ...
  TF_CHECK_OK(GpuLaunchKernel(
      ...
      config.block_count, config.thread_per_block, 0, d.stream(),
      input_outer_dim_size, input_inner_dim_size, output_rows,
      segment_ids.data(), data, output.data(), segment_ids_freq,
      total_stripe_count));
}
