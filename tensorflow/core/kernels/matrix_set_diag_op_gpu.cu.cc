...
struct MatrixSetDiag<GPUDevice, Scalar> {
  static void Compute(OpKernelContext* context, const GPUDevice& device,
    ...
    if (input.data() == output.data()) {
      ...
      CudaLaunchKernel(MatrixSetDiagKernel<Scalar>, config.block_count,
      ...
      CudaLaunchKernel(MatrixCopyInputAndSetDiagKernel<Scalar>,
                       config.block_count, config.thread_per_block, 0,
                       device.stream(), config.virtual_thread_count, m, n,
                       minsize, input.data(), diag.data(), output.data());
    }
  }
};
