...
class MklConvBwdInputPrimitiveFactory : public MklPrimitiveFactory<T> {
 ...
 public:
  static MklConvBwdInputPrimitive<T>* Get(
    ...
    if (conv_bwd_input == nullptr) {
      conv_bwd_input = new MklConvBwdInputPrimitive<T>(
          convBwdInputDims);
 ...
 private:
  ...
  MklPrimitive* GetConvBwdInput(
      const MklConvBwdInputParams& convBwdInputDims) {
    string key = CreateKey(convBwdInputDims);
    return this->GetOp(key);
  }

  void SetConvBwdInput(
...
class MklConvCustomBackpropInputOp
 ...
 public:
  ...
  void Compute(OpKernelContext* context) {
    try {
      ...
      conv_bwd_input = MklConvBwdInputPrimitiveFactory<T>::Get(
          convBwdInputDims);
      ...
      if (diff_dst_md.data.format !=
          conv_bwd_input->GetDiffDstMemoryFormat()) {
 ...
 private:
  ...
  TensorShape MakeInputTfShape(OpKernelContext* context,
    ...
    // Conv[2D|3D]BackpropInputV2 supports both DT_INT32 and DT_INT64 output_shape
    // MakeShape is able to handle both DT_INT32 and DT_INT64 for input_tensor.
    CHECK_EQ(
        this->MakeShape(input_tensor, &input_tf_shape)
            .ok(),
        true);
