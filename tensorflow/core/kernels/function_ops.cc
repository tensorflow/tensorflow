...
TF_INCLUDE_IF_WITH_EXTRA_TYPES(
    false, REGISTER_KERNEL_BUILDER(Name(kArgOp)
                                       .Device(DEVICE_GPU)
                                       .HostMemory("output")
                                       .TypeConstraint<int32>("T"),
                                   ArgOp));
...
TF_INCLUDE_IF_WITH_EXTRA_TYPES(
    true,
    REGISTER_KERNEL_BUILDER(
        Name(kArgOp).Device(DEVICE_GPU).TypeConstraint<ResourceHandle>("T"),
        ArgOp));
;
TF_INCLUDE_IF_WITH_EXTRA_TYPES(
    true,
...
TF_INCLUDE_IF_WITH_EXTRA_TYPES(
    false, REGISTER_KERNEL_BUILDER(Name(kRetOp)
                                       .Device(DEVICE_GPU)
                                       .HostMemory("input")
                                       .TypeConstraint<int32>("T"),
                                   RetvalOp));
...
TF_INCLUDE_IF_WITH_EXTRA_TYPES(
    false, REGISTER_KERNEL_BUILDER(Name("_ListToArray")
                                       .Device(DEVICE_GPU)
                                       .HostMemory("input")
                                       .HostMemory("output")
                                       .TypeConstraint<int32>("T"),
                                   PassOn));
TF_INCLUDE_IF_WITH_EXTRA_TYPES(
    false, REGISTER_KERNEL_BUILDER(Name("_ArrayToList")
                                       .Device(DEVICE_GPU)
                                       .HostMemory("input")
                                       .HostMemory("output")
                                       .TypeConstraint<int32>("T"),
