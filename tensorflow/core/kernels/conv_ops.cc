    ...
    if (cudnn_use_autotune &&
      ...
      OP_REQUIRES(ctx,
                  ...
                  errors::Unknown(
                      ...
                      "above. Or make sure the CuDNN version >= 8.1."));
      ...
      for (auto& profile_plan: exec_plans) {
        ...
        if (cudnn_launch_status.ok() && profile_result.is_valid()) {
          ...
          *result.mutable_run_time() = proto_utils::ToDurationProto(
          ...
          // Therefore, we add an dummy result even if the profiling fails.
          results.emplace_back();
          auto& result = results.back();
    ...
    if (cudnn_use_autotune &&
      ...
      OP_REQUIRES(
          ...
          stream->parent()->GetMIOpenConvolveAlgorithms(
              se::dnn::ConvolutionKind::FORWARD, se::dnn::ToDataType<T>::value,
              stream, input_desc, input_ptr, filter_desc, filter_ptr,
