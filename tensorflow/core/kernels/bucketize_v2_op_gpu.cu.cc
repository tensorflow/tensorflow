...
template <typename T>
struct BucketizeV2Functor<GPUDevice, T> {
    ...
    if (shared_mem_size < d.sharedMemPerBlock() &&
      ...
      BucketizeCustomKernel<T, true>
          <<<config.block_count, config.thread_per_block, shared_mem_size,
             d.stream()>>>(input.size(), input.data(), boundaries.size(),
                           boundaries.data(), output.data());
    } else {
      BucketizeCustomKernel<T, false>
          <<<config.block_count, config.thread_per_block, 0, d.stream()>>>(
              input.size(), input.data(), boundaries.size(),
              boundaries.data(), output.data());
    }
    return Status::OK();
  }
...
#define REGISTER_GPU_SPEC(type) \
  template struct functor::BucketizeV2Functor<GPUDevice, type>;
