...
template <typename Device, typename Tinput, typename Tfilter, typename Tbias,
          typename Toutput, typename Ttemp_output, typename Tpadding, bool biasEnabled,
          bool padEnabled>
class MklConvOp : public OpKernel {
 public:
  ~MklConvOp() {}
...
#define REGISTER_MKL_CPU_2D(T)                                              \
  REGISTER_KERNEL_BUILDER(Name("_MklConv2D")                             \
                              ...
                              .TypeConstraint<T>("T")                    \
                              .Label(mkl_op_registry::kMklOpLabel),      \
                          MklConvOp<CPUDevice, float, float, float, float, float, int32, false, false>); \
  REGISTER_KERNEL_BUILDER(Name("_MklConv2DWithBias")                     \
                              .Device(DEVICE_CPU)                        \
                              .TypeConstraint<T>("T")                    \
                              .Label(mkl_op_registry::kMklOpLabel),      \
                          MklConvOp<CPUDevice, float, float, float, float, float, int32, true, false>);  \
  ...
  REGISTER_KERNEL_BUILDER(Name("_MklPadWithConv2D")                      \
                              ...
                              .Label(mkl_op_registry::kMklOpLabel),      \
                          MklConvOp<CPUDevice, float, float, float, float, float, int32, false, true>);  \
  REGISTER_KERNEL_BUILDER(Name("_MklPadWithConv2D")                      \
                              .Device(DEVICE_CPU)                        \
                              .TypeConstraint<T>("T")                    \
                              .TypeConstraint<int64>("Tpaddings")        \
                              .Label(mkl_op_registry::kMklOpLabel),      \
                          MklConvOp<CPUDevice, float, float, float, float, float, int64, false, true>);  \
