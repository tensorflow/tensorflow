...
#define REGISTER_MKL_CPU(T)                                         \
  REGISTER_KERNEL_BUILDER(                                          \
      Name("_MklConv2D")                                            \
          ...
          .TypeConstraint<T>("T")                                   \
          .Label(mkl_op_registry::kMklOpLabel),                     \
      MklConvOp<CPUDevice, T, false, mkldnn::algorithm_undef>);     \
  REGISTER_KERNEL_BUILDER(                                          \
      Name("_MklConv2DWithBias")                                    \
          .Device(DEVICE_CPU)                                       \
          .TypeConstraint<T>("T")                                   \
          .Label(mkl_op_registry::kMklOpLabel),                     \
      MklConvOp<CPUDevice, T, true, mkldnn::algorithm_undef>);      \
  REGISTER_KERNEL_BUILDER(                                          \
      Name("_MklConv2DWithRelu")                                    \
          .Device(DEVICE_CPU)                                       \
          .TypeConstraint<T>("T")                                   \
          .Label(mkl_op_registry::kMklOpLabel),                     \
      MklConvOp<CPUDevice, T, false, mkldnn::eltwise_relu>);        \
  REGISTER_KERNEL_BUILDER(                                          \
      Name("_MklConv2DWithBiasAndRelu")                             \
          .Device(DEVICE_CPU)                                       \
          .TypeConstraint<T>("T")                                   \
          .Label(mkl_op_registry::kMklOpLabel),                     \
      MklConvOp<CPUDevice, T, true, mkldnn::eltwise_relu>);         \
  REGISTER_KERNEL_BUILDER(                                          \
      ...
      MklDummyOp<CPUDevice, T>);                                    \
  REGISTER_KERNEL_BUILDER(                                          \
      Name("__MklDummyConv2DWithRelu")                              \
          .Device(DEVICE_CPU)                                       \
          .TypeConstraint<T>("T")                                   \
          .Label(mkl_op_registry::kMklOpLabel),                     \
      MklDummyOp<CPUDevice, T>);                                    \
  REGISTER_KERNEL_BUILDER(                                          \
      Name("__MklDummyConv2DWithBiasAndRelu")                       \
          .Device(DEVICE_CPU)                                       \
          .TypeConstraint<T>("T")                                   \
          .Label(mkl_op_registry::kMklOpLabel),                     \
      MklDummyOp<CPUDevice, T>);           
