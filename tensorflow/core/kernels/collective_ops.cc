...
class CollectiveReduceV2OpKernel : public CollectiveOpKernel {
 public:
  explicit CollectiveReduceV2OpKernel(OpKernelConstruction* c)
    ...
    OP_REQUIRES_OK(c, c->GetAttribute("final_op", &final_op_name));
    OP_REQUIRES_OK(
        c, c->GetAttribute("communication_hint",
                           &col_params_->instance.impl_details.communication_hint));
    OP_REQUIRES_OK(
        c, c->GetAttribute("timeout_seconds",
                           &col_params_->instance.impl_details.timeout_seconds));
...
class CollectiveGatherV2OpKernel : public CollectiveOpKernel {
 public:
  explicit CollectiveGatherV2OpKernel(OpKernelConstruction* c)
    ...
    OP_REQUIRES_OK(c, c->GetAttribute("T", &data_type_));
    OP_REQUIRES_OK(c, c->GetAttribute("communication_hint", &communication_hint_));
    OP_REQUIRES_OK(c, c->GetAttribute("timeout_seconds", &timeout_seconds_));
