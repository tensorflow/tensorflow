...
__global__ void SparseIndicesToRaggedRowSplitsKernel(
    GpuLaunchConfig config, const int num_nonzero,
    const bool validate_ragged_right, const IndexType* indices_flat_2d,
    const IndexType* dense_shape, IndexType* row_splits,
    int32_t* invalid_flag) {
  auto num_rows = dense_shape[0];
  auto num_cols = dense_shape[1];

  GPU_1D_KERNEL_LOOP(kernel_index, config.virtual_thread_count) {
    int kernel_count = config.virtual_thread_count;
    ...
    // In case of very unusual dimensions, return early if there is no work to
    // do.
    ...
    for (; n < end_index; ++n) {
      ...
      // Fill in the row_splits with the current index; the loop will fill in
      // any needed entries for empty rows between the previous and current
      // index.
...
struct SparseIndicesToRaggedRowSplitsFunctor<GPUDevice, IndexType> {
  Status operator()(OpKernelContext* context, int num_nonzero,
    ...
    return GpuLaunchKernel(SparseIndicesToRaggedRowSplitsKernel<IndexType>,
                           ...
                           d.stream(), config, num_nonzero,
                           validate_ragged_right, indices_flat_2d, dense_shape,
                           row_splits, invalid_flag);
  }
};
