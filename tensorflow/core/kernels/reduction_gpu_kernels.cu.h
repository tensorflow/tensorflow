...
struct SqrtOfReal<hipFloatComplex> {
  __host__ __device__ hipFloatComplex
  operator()(const hipFloatComplex& a) const {
...
struct SqrtOfReal<hipDoubleComplex> {
  __host__ __device__ hipDoubleComplex
  operator()(const hipDoubleComplex& a) const {
...
template <>
struct inner_float<double> {
  typedef double IT;
};

template <>
struct inner_float<std::complex<double>> {
  typedef double IT;
};

#if TENSORFLOW_USE_ROCM
template <>
struct inner_float<hipDoubleComplex> {
  typedef double IT;
};
#endif

// divisor_type was previously same as T, but it's wasteful since the
// constructor argument is always integer (and it introduces troubles with
// complex division)
...
__global__ __launch_bounds__(1024) void RowReduceKernel(
  ...
  sum = WarpReduce(temp_storage).Reduce(sum, op, min(num_cols, WARPSIZE));
...
__global__ __launch_bounds__(1024) void ColumnReduceMax16ColumnsKernel(
  ...
  __shared__ __align__(alignof(value_type)) char
      partial_sums_raw[WARPSIZE * (WARPSIZE + 1) * sizeof(value_type)];
  value_type* partial_sums = reinterpret_cast<value_type*>(partial_sums_raw);
#elif TENSORFLOW_USE_ROCM
  __shared__ storage_type<value_type> partial_sums[WARPSIZE * (WARPSIZE + 1)];
#endif
  ...
  if (lane < num_cols) partial_sums[lane * (WARPSIZE + 1) + threadIdx.y] = sum;
...
void LaunchRowReduction(OpKernelContext* ctx, OUT_T out, IN_T in, int num_rows,
  ...
  if (num_cols < 1024) {
    ...
    TF_CHECK_OK(
        GpuLaunchKernel(RowReduceKernel<IN_T, OUT_T, Op, TF_RED_WARPSIZE>,
                        num_blocks, threads_per_block, 0, cu_stream, in, out,
...
void LaunchColumnReduction_LTE16Cols(OpKernelContext* ctx, OUT_T out, IN_T in,
  ...
  dim3 block_dim(
      WARPSIZE,
      std::min(Eigen::divup(extent_x, rows_per_warp), (1024 / WARPSIZE)), 1);
  dim3 grid_dim(1,
  ...
  if (grid_dim.y == 1) {
    TF_CHECK_OK(GpuLaunchKernel(
        ColumnReduceMax16ColumnsKernel<IN_T, OUT_T, Op, WARPSIZE>, grid_dim,
    ...
    OP_REQUIRES_OK(ctx,
                   ctx->allocate_temp(DT_INT8,
                                      ...
                                      &temp_storage));
    TF_CHECK_OK(GpuLaunchKernel(
        ColumnReduceMax16ColumnsKernel<IN_T, T*, Op, WARPSIZE>, grid_dim,
        block_dim, 0, cu_stream, in, (T*)temp_storage.flat<int8_t>().data(),
        extent_x, extent_y, op, init));

    dim3 new_grid_dim((grid_dim.y * extent_y + (WARPSIZE - 1)) / WARPSIZE, 1,
                      1);
...
void LaunchColumnReduction_LTE4096Cols(OpKernelContext* ctx, OUT_T out, IN_T in,
  ...
  constexpr int WARPSIZE = TF_RED_WARPSIZE;
#endif
  dim3 block_dim(WARPSIZE, std::min(extent_x, (1024 / WARPSIZE)), 1);
  dim3 grid_dim((extent_y + (WARPSIZE - 1)) / WARPSIZE, 1, 1);

  if (grid_dim.x < 16)
    grid_dim.y = std::min((extent_x + (WARPSIZE - 1)) / WARPSIZE, WARPSIZE);
  ...
  if (grid_dim.y == 1) {
    TF_CHECK_OK(GpuLaunchKernel(ColumnReduceKernel<IN_T, OUT_T, Op, WARPSIZE>,
    ...
    TF_CHECK_OK(GpuLaunchKernel(ColumnReduceKernel<IN_T, T*, Op, WARPSIZE>,
...
struct IdentityValue<hipFloatComplex, Prod<hipFloatComplex>> {
  hipFloatComplex operator()() { return hipFloatComplex(1.0, 0.0); }
};

template <>
struct IdentityValue<hipDoubleComplex, Prod<hipDoubleComplex>> {
  hipDoubleComplex operator()() { return hipDoubleComplex(1.0, 0.0); }
};
#endif
...
struct ReduceFunctor<GPUDevice, Eigen::internal::SumReducer<T>> {
  ...
  static void FillIdentity(const GPUDevice& d, OUT_T out,
                           const Eigen::internal::SumReducer<T>& reducer) {
    FillIdentityEigenImplWithCast<T>(d, To32Bit(out),
                                     Eigen::internal::SumReducer<TM>());
  }
};
