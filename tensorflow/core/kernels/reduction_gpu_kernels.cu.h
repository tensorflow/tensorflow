...
void LaunchColumnReduction_LTE16Cols(OpKernelContext* ctx, OUT_T out, IN_T in,
  ...
  if (grid_dim.y == 1) {
    GPU_LAUNCH_KERNEL((ColumnReduceMax16ColumnsKernel<IN_T, OUT_T, Op>), grid_dim, block_dim, 0,
    ...
    OP_REQUIRES_OK(ctx,
                   ctx->allocate_temp(DT_INT8,
                                      ...
                                      &temp_storage));
    GPU_LAUNCH_KERNEL((ColumnReduceMax16ColumnsKernel<IN_T, T*, Op>),
    ...
    GPU_LAUNCH_KERNEL((CleanupSegments<T*, OUT_T, Op>), new_grid_dim,
...
void LaunchColumnReduction_LTE4096Cols(OpKernelContext* ctx, OUT_T out, IN_T in,
  ...
  if (grid_dim.y == 1) {
    GPU_LAUNCH_KERNEL((ColumnReduceKernel<IN_T, OUT_T, Op>), grid_dim,
...
void LaunchColumnReduction(OpKernelContext* ctx, OUT_T out, IN_T in,
  ...
  if (extent_y <= 16) {
    ...
    GPU_LAUNCH_KERNEL((ColumnReduceSimpleKernel<IN_T, OUT_T, Op>),
