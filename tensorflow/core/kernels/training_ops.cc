...
void Compute(OpKernelContext* ctx) override {
 const bool sparse = false;
 auto locks = MaybeLockVariableInputMutexesInOrder<Device, T>(
        ctx, use_exclusive_lock_, sparse, {0, 4});
 Tensor var;
 OP_REQUIRES_OK(ctx, GetInputTensorFromVariable<Device, T>(ctx, 0, use_exclusive_lock_, false, &var));
 OP_REQUIRES(
	 ctx, var.IsInitialized(),
	 errors::FailedPrecondition(
		 "Attempting to use uninitialized variables: ", requested_input(0)));
 ...
 Tensor shadow;
 OP_REQUIRES_OK(ctx, GetInputTensorFromVariable<Device, T>(ctx, 4, use_exclusive_lock_, false, &shadow));
 OP_REQUIRES(
...
#define REGISTER_KERNELS(D, T)                                       \
REGISTER_KERNEL_BUILDER(                                           \
    Name("ApplyDelayCompensatedGradientDescent").Device(DEVICE_##D).TypeConstraint<T>("T"), \
    ApplyDelayCompensatedGradientDescentOp<D##Device, T>);                                  \
REGISTER_KERNEL_BUILDER(Name("ResourceApplyDelayCompensatedGradientDescent")                \
                            .HostMemory("var")                     \
                            .HostMemory("shadow")                       \
                            .Device(DEVICE_##D)                    \
                            .TypeConstraint<T>("T"),               \
