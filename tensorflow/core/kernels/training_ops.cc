...
template <typename T>
struct ApplyRadam<CPUDevice, T> : ApplyRadamNonCuda<CPUDevice, T> {};
template <typename T>
struct ApplyRadam<SYCLDevice, T> : ApplyRadamNonCuda<SYCLDevice, T> {};


template <typename Device, typename T>
struct ApplyNadamNonCuda {
  void operator()(const Device& d, typename TTypes<T>::Flat var,
                  typename TTypes<T>::Flat m, typename TTypes<T>::Flat v,
                  typename TTypes<T>::ConstScalar beta1_power,
                  typename TTypes<T>::ConstScalar beta2_power,
...
template <typename T>
struct ApplyNadam<CPUDevice, T> : ApplyNadamNonCuda<CPUDevice, T> {};
template <typename T>
struct ApplyNadam<SYCLDevice, T> : ApplyNadamNonCuda<SYCLDevice, T> {};
...
#define DECLARE_GPU_SPEC(T)                                   \
  template <>                                                 \
  void ApplyRadam<GPUDevice, T>::operator()(                  \
      const GPUDevice& d, typename TTypes<T>::Flat var,       \
      typename TTypes<T>::Flat m, typename TTypes<T>::Flat v, \
      typename TTypes<T>::ConstScalar beta1_power,            \
      typename TTypes<T>::ConstScalar beta2_power,            \
      ...
      typename TTypes<T>::ConstScalar gamma,                  \
      typename TTypes<T>::ConstFlat grad);                    \
  extern template struct ApplyRadam<GPUDevice, T>;
DECLARE_GPU_SPEC(Eigen::half);
DECLARE_GPU_SPEC(float);
DECLARE_GPU_SPEC(double);
#undef DECLARE_GPU_SPEC
...
class ApplyNadamOp : public OpKernel {
 public:
  explicit ApplyNadamOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
  }
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const Device& device = ctx->template eigen_device<Device>();
    functor::ApplyNadam<Device, T>()(device, var.flat<T>(), m.flat<T>(),
                                    v.flat<T>(), beta1_power.scalar<T>(),
                                    beta2_power.scalar<T>(), lr.scalar<T>(),
                                    beta1.scalar<T>(), beta2.scalar<T>(),
                                    epsilon.scalar<T>(), grad.flat<T>());
...
#define REGISTER_KERNELS(D, T)                                             \
  REGISTER_KERNEL_BUILDER(                                                 \
      Name("ApplyNadam").Device(DEVICE_##D).TypeConstraint<T>("T"),         \
      ApplyNadamOp<D##Device, T>);                                          \
  REGISTER_KERNEL_BUILDER(                                                 \
      Name("ResourceApplyNadam").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      ApplyNadamOp<D##Device, T>);
#define REGISTER_CPU_KERNELS(T) REGISTER_KERNELS(CPU, T);
...
#define DECLARE_GPU_SPEC(T)                                   \
  template <>                                                 \
  void ApplyNadam<GPUDevice, T>::operator()(                  \
      const GPUDevice& d, typename TTypes<T>::Flat var,       \
      typename TTypes<T>::Flat m, typename TTypes<T>::Flat v, \
      typename TTypes<T>::ConstScalar beta1_power,            \
      typename TTypes<T>::ConstScalar beta2_power,            \
      ...
      typename TTypes<T>::ConstScalar epsilon,                \
      typename TTypes<T>::ConstFlat grad);                    \
  extern template struct ApplyNadam<GPUDevice, T>;
DECLARE_GPU_SPEC(Eigen::half);
DECLARE_GPU_SPEC(float);
DECLARE_GPU_SPEC(double);
#undef DECLARE_GPU_SPEC
