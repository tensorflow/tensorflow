...
class PaddingRandomShuffleQueue : public RandomShuffleQueue {
 public:
  PaddingRandomShuffleQueue(int32 capacity, int32 min_after_dequeue, int64 seed,
...
protected:
 ...
 private:
  ~PaddingRandomShuffleQueue() override {}

  static Status GetElementComponent(const PaddingRandomShuffleQueue::Tuple& tuple,
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(PaddingRandomShuffleQueue);
...
PaddingRandomShuffleQueue::PaddingRandomShuffleQueue(int32 capacity, int32 min_after_dequeue, int64 seed,
                     int64 seed2, const DataTypeVector& component_dtypes,
                     const std::vector<PartialTensorShape>& partial_shapes,
                     const string& name): RandomShuffleQueue(capacity, min_after_dequeue,seed,
                     seed2,component_dtypes,
                     ConvertShapesPartialDimensionsToZero(partial_shapes), name),
      partial_shapes_(partial_shapes) {}

Status PaddingRandomShuffleQueue::Initialize() {
  Status s = RandomShuffleQueue::Initialize();
  if (!s.ok()) return s;
...
/* static */
Status PaddingRandomShuffleQueue::GetElementComponent(
  const PaddingRandomShuffleQueue::Tuple& tuple,
  int component, OpKernelContext* ctx,
  PersistentTensor* out_tensor){
...
void PaddingRandomShuffleQueue::TryDequeueMany(int num_elements, OpKernelContext* ctx,
                                        bool allow_small_batch,
                                        CallbackWithTuple callback) 
{
    if (num_elements == 0) {
      ...
      for (int i = 0; i < num_components(); ++i) {
        ...
        // See similar comment in random_shuffle_queue_ops.cc
        Tensor element;
                  ...
                  if (allow_small_batch && queues_[0].size() > 0) {
                    ...
                    if (attempt->context->status().ok()) {
                      attempt->context->SetStatus(errors::OutOfRange(
                          "PaddingRandomShuffleQueue '", name_, "' is closed and has ",
...
Status PaddingRandomShuffleQueue::ValidateTuple(const Tuple& tuple) {
...
Status PaddingRandomShuffleQueue::ValidateManyTuple(const Tuple& tuple) {
  TF_RETURN_IF_ERROR(ValidateTupleCommon(tuple));
...
Status PaddingRandomShuffleQueue::CompatibleNodeDefShapes(const NodeDef& node_def) const {
...
Status PaddingRandomShuffleQueue::MatchesNodeDef(const NodeDef& node_def) {
  TF_RETURN_IF_ERROR(MatchesNodeDefOp(node_def, "PaddingRandomShuffleQueue"));
...
static Status ValidateElementToLargerSlice(const Tensor& element, Tensor* parent) {
...
Status HandleElementToLargerSlice(const Tensor& element, Tensor* parent,int index) {
...
Status PaddingRandomShuffleQueue::CopyElementToLargerSlice(const Tensor& element,
...
// Static method
Status PaddingRandomShuffleQueue::SetElementZero(Tensor* element) {
#define HANDLE_TYPE(T)                                \
...
std::vector<TensorShape> PaddingRandomShuffleQueue::ConvertShapesPartialDimensionsToZero(
    const gtl::ArraySlice<PartialTensorShape>& partial_shapes) {
...
// Defines a PaddingRandomShuffleQueueOp, which produces a Queue (specifically, one
// backed by PaddingRandomShuffleQueue) that persists across different graph
// executions, and sessions. Running this op produces a single-element
// tensor of handles to Queues in the corresponding device.
class PaddingRandomShuffleQueueOp : public QueueOp {
 public:
  explicit PaddingRandomShuffleQueueOp(OpKernelConstruction* context): QueueOp(context) {
 ...
 protected:
  CreatorCallback GetCreator() const override {
    return [this](QueueInterface** ret) {
      auto* q = new PaddingRandomShuffleQueue(capacity_, min_after_dequeue_, seed_,
 ...
 private:
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(PaddingRandomShuffleQueueOp);
};

REGISTER_KERNEL_BUILDER(Name("PaddingRandomShuffleQueue").Device(DEVICE_CPU),
