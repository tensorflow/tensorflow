...
bool RunAvePoolBackwardNHWC(const T* const top_diff, const int num,
  ...
  AvePoolBackwardNHWC<T><<<config.block_count, config.thread_per_block, 0, d.stream()>>>(
      config.virtual_thread_count, top_diff, num, height, width, channels, pooled_height,
