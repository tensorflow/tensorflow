...
CuboidConvolution(const Input& input, const Kernel& kernel,
  ...
  switch (padding_type) {
    case PADDING_VALID:
      out_depth = numext::ceil((inputPlanes - kernelDepth + 1.f) / stride_planes_f);
      out_height = numext::ceil((inputRows - kernelRows + 1.f) / stride_rows_f);
      out_width = numext::ceil((inputCols - kernelCols + 1.f) / stride_cols_f);
      break;
    case PADDING_SAME:
      out_depth = numext::ceil(inputPlanes / stride_planes_f);
      out_height = numext::ceil(inputRows / stride_rows_f);
      out_width = numext::ceil(inputCols / stride_cols_f);
      break;
    default:
