...
struct LogSumExp {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T packetOp(const T& a,
    ...
    // Short-circuit when ma is -inf or +inf (avoids inf-inf = NaN).
    auto ma_is_inf = por(pcmp_lt(ma, pset1(Eigen::NumTraits<T>::lowest())),
                         pcmp_lt(pset1(Eigen::NumTraits<T>::highest()), ma));
    return pselect(ma_is_inf, ma, logsumexp);
  }
};
