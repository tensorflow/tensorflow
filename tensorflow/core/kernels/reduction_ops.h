...
struct Identity<functor::MeanReducer<hipFloatComplex>> {
  static hipFloatComplex identity(
      const functor::MeanReducer<hipFloatComplex>&) {
    return hipFloatComplex(Eigen::NumTraits<float>::quiet_NaN(),
                           Eigen::NumTraits<float>::quiet_NaN());
  }
};
template <>
struct Identity<functor::MeanReducer<hipDoubleComplex>> {
  static hipDoubleComplex identity(
      const functor::MeanReducer<hipDoubleComplex>&) {
    return hipDoubleComplex(Eigen::NumTraits<double>::quiet_NaN(),
...
void FillIdentityEigenImplWithCast(const Device& d, OUT_T out,
                                   const Reducer& reducer) {
  auto id = Identity<Reducer>::identity(reducer);
  T cast_id;
  static_assert(
      sizeof(id) == sizeof(cast_id),
