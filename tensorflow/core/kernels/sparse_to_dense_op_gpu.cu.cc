...
__global__ void SparseToDenseKernel(const Index* __restrict__ indices,
  ...
  GPU_1D_KERNEL_LOOP(thread_idx, nnz) {
    ...
    if (output_idx >= 0 && output_idx < dense_size) {
...
template <typename T, typename Index>
__global__ void SetDefaultValue(const T default_value, const int64_t dense_size,
...
template <typename T, typename Index>
absl::Status LaunchComputeKernels(OpKernelContext* c, const int64_t dense_size,
...
void LaunchSparseToDense<T, Index>::operator()(
  ...
  const int64_t dense_size = dense->NumElements();
  const int64_t num_values = values.NumElements();
