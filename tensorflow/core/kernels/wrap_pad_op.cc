...
class WrapPadOp : public OpKernel {
 ...
 private:
  ...
  void Operate(OpKernelContext* context,
    ...
    functor::WrapPad<Device, T, Tpadding, Dims> functor;
...
#define DECLARE_GPU_SPEC(T, Dims)                                           \
  template <>                                                               \
  void WrapPad<GPUDevice, T, int32, Dims>::operator()(                          \
      const GPUDevice& d, typename TTypes<T, Dims>::Tensor output,          \
      typename TTypes<T, Dims>::ConstTensor input,                          \
      Eigen::array<Eigen::IndexPair<int32>, Dims> paddings, T pad_value);   \
  extern template struct WrapPad<GPUDevice, T, int32, Dims>;                    \
  template <>                                                               \
  void WrapPad<GPUDevice, T, int64_t, Dims>::operator()(                        \
      const GPUDevice& d, typename TTypes<T, Dims>::Tensor output,          \
      typename TTypes<T, Dims>::ConstTensor input,                          \
      Eigen::array<Eigen::IndexPair<int64_t>, Dims> paddings, T pad_value); \
  extern template struct WrapPad<GPUDevice, T, int64_t, Dims>;
...
REGISTER_KERNEL_BUILDER(Name("WrapPad")
                            ...
                            .HostMemory("paddings")
                            .HostMemory("output"),
                        WrapPadOp<CPUDevice, int32, int32>);
REGISTER_KERNEL_BUILDER(Name("WrapPad")
                            .Device(DEVICE_GPU)
                            .TypeConstraint<int32>("T")
                            .TypeConstraint<int64_t>("Tpaddings")
                            ...
                            .HostMemory("paddings")
                            .HostMemory("output"),
                        WrapPadOp<CPUDevice, int32, int64>);
