...
#include "third_party/eigen3/unsupported/Eigen/CXX11/Tensor"
   ...
   public:
    explicit ResizeBilinearOp(OpKernelConstruction* context)
      ...
      OP_REQUIRES_OK(context,
                     context->GetAttr("align_corners", &align_corners_));
      OP_REQUIRES_OK(context, context->GetAttr("half_pixel_centers",
                                               &half_pixel_centers_));
    }
    ...
    void Compute(OpKernelContext* context) override {
      ...
      // !!! It may bring troubles here. Because in the original code, we use
      // internal::SubtleMustCopy to copy the shape.
      st.ValidateAndCreateOutput(context, input.shape(),
                                 context->input(1).shape());

      if (!context->status().ok()) return;

      // Return if the output is empty.
      if (st.GetOutput()->NumElements() == 0) return;

      typename TTypes<T, 4>::ConstTensor image_data(input.tensor<T, 4>());
      TTypes<float, 4>::Tensor output_data = st.GetOutput()->tensor<float, 4>();

      const ImageResizerStateData& data = st.GetData();
      functor::ResizeBilinear<Device, T>()(
          context->eigen_device<Device>(), image_data, data.height_scale,
          data.width_scale, half_pixel_centers_, output_data);
    }
  ...
  inline void compute_interpolation_weights(
      const Scaler scaler, const int64 out_size, const int64 in_size,
      const float scale, CachedInterpolation* interpolation) {
  ...
  inline __m128 compute_lerp_v(const __m128 top_left, const __m128 top_right,
    ...
    const __m128 top = _mm_add_ps(
        top_left, _mm_mul_ps(_mm_sub_ps(top_right, top_left), x_lerp));
  ...
  void ResizeLine3ChannelsVector(const T* const ys_input_lower_ptr,
    ...
    for (x = 0; x < out_width - 1; ++x) {
      ...
      const __m128 bottom_left_v =
          load_3xfloat_v(ys_input_upper_ptr + xs_lower);
      const __m128 bottom_right_v =
          load_3xfloat_v(ys_input_upper_ptr + xs_upper);
  ...
  struct ResizeBilinear<CPUDevice, T> {
    void operator()(const CPUDevice& d,
      ...
      // Scale x interpolation weights to avoid a multiplication during
      // iteration.
  ...
  class ResizeBilinearOpGrad : public OpKernel {
   public:
    ...
    void Compute(OpKernelContext* context) override {
      ...
      const ImageResizerStateData& st_data = st.GetData();
      ...
      if (!std::is_same<T, Eigen::half>::value &&
        ...
        typename TTypes<T, 4>::Tensor output_grad(
            st.GetOutput()->tensor<T, 4>());
        functor::ResizeBilinearGrad<Device, T>()(
            context->eigen_device<Device>(), input_grad, st_data.height_scale,
            st_data.width_scale, half_pixel_centers_, output_grad);
      } else {
        // Accumulate output to float instead of half/bfloat16 tensor, since
        // float accumulation is more numerically stable and GPU half
        // implementation is slow.
        // TODO(b/165759037): Create optimized and numerically stable half and
        // bfloat16 implementation
        Tensor output_grad;
        OP_REQUIRES_OK(context,
                       context->allocate_temp(DT_FLOAT, st.GetOutput()->shape(),
                                              &output_grad));
        functor::ResizeBilinearGrad<Device, float>()(
            context->eigen_device<Device>(), input_grad, st_data.height_scale,
            st_data.width_scale, half_pixel_centers_,
  ...
  struct ResizeBilinearGrad<CPUDevice, T> {
    ...
    void ResizeGradCore(const Scaler& scaler,
      ...
      // contributed to each output pixel and propagate the gradient at the
      // output pixel location to each of those four input pixel locations in
      // the same proportions that they originally contributed to the output
      // pixel. Here is the forward-propagation pseudo-code, for reference:
