...
#include "tensorflow/core/kernels/text/darts_clone_trie_wrapper.h"
...
TEST(DartsCloneTrieTest, CreateCursorPointToRootAndTryTraverseOneStep) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto trie_array, BuildDartsCloneTrie(vocab_tokens));
...
TEST(DartsCloneTrieBuildError, KeysValuesSizeDifferent) {
  ...
  // Create the trie instance.
  EXPECT_EQ(BuildDartsCloneTrie(keys, values).status().code(),
...
TEST(DartsCloneTrieBuildError, DuplicatedKeys) {
  ...
  // Create the trie instance.
  EXPECT_EQ(BuildDartsCloneTrie(vocab_tokens).status().code(),
            absl::StatusCode::kInvalidArgument);
}
...
TEST(DartsCloneTrieBuildError, NegativeValues) {
  ...
  // Create the trie instance.
  EXPECT_EQ(BuildDartsCloneTrie(vocab_tokens, vocab_values).status().code(),
            absl::StatusCode::kInvalidArgument);
}
