...
#include "tensorflow/core/kernels/text/sentencepiece/optimized_decoder.h"
#include "tensorflow/core/kernels/text/sentencepiece/sentencepiece_detokenizer.h"
...
class TFSentencepieceDetokenizerOp : public tensorflow::OpKernel {
 public:
  ...
  void Compute(tensorflow::OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(
        ctx, input_splits_flat.size() >= 1,
        errors::InvalidArgument(
            "input_splits must have at least 1 element; got 0 elements"));
    ...
    for (int i = 0; i < num_of_sentences; i++) {
      ...
      const auto res =
          sentencepiece::DecodeString(codes_for_split, model_tensor.data());
