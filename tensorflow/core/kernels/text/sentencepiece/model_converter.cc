...
#include "tensorflow/core/kernels/text/sentencepiece/decoder_config_generated.h"
...
#include "tensorflow/core/kernels/text/sentencepiece/sentencepiece_constants.h"
...
absl::StatusOr<std::string> ConvertSentencepieceModelToFlatBufferForDecoder(
    const std::string& model_config_str, int encoding_offset) {
  ::sentencepiece::ModelProto model_config;
  if (!model_config.ParseFromString(model_config_str)) {
    return absl::InvalidArgumentError(
