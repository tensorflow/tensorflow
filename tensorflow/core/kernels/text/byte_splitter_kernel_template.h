...
#ifndef TENSORFLOW_CORE_KERNELS_TEXT_BYTE_SPLITTER_KERNEL_TEMPLATE_H_
#define TENSORFLOW_CORE_KERNELS_TEXT_BYTE_SPLITTER_KERNEL_TEMPLATE_H_
...
template <tflite::shim::Runtime Rt>
absl::Status ByteSplitterWithOffsetsOp<Rt>::Invoke(InvokeContext* context) {
  // Inputs
...
absl::Status ByteSplitByOffsetsOp<Rt>::ShapeInference(
  ...
  if (!input_starts_shape.Compatible(rank_1_shape)) {
    return absl::FailedPreconditionError(
        absl::StrCat("Input start offsets shape must be rank 1: ",
  ...
  if (!input_ends_shape.Compatible(rank_1_shape)) {
    return absl::FailedPreconditionError(
        absl::StrCat("Input end offsets shape must be rank 1: ",
  ...
  if (!input_row_splits_shape.Compatible(rank_1_shape)) {
    return absl::FailedPreconditionError(
        absl::StrCat("Input row splits shape must be rank 1: ",
                     input_row_splits_shape.ToString()));
  }

  SH_RETURN_IF_ERROR(c->SetOutputShape(kOutputValues, input_starts_shape));
  SH_RETURN_IF_ERROR(
      c->SetOutputShape(kOutputRowSplits, input_row_splits_shape));

  return absl::OkStatus();
}

template <tflite::shim::Runtime Rt>
absl::Status ByteSplitByOffsetsOp<Rt>::Invoke(InvokeContext* context) {
  // Inputs
...
#endif  // TENSORFLOW_CORE_KERNELS_TEXT_BYTE_SPLITTER_KERNEL_TEMPLATE_H_
