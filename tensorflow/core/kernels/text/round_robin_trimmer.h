...
#ifndef TENSORFLOW_CORE_KERNELS_TEXT_ROUND_ROBIN_TRIMMER_H_
#define TENSORFLOW_CORE_KERNELS_TEXT_ROUND_ROBIN_TRIMMER_H_
...
class RoundRobinTrimmer : Trimmer<T>, BatchTrimmer<T, Tsplits> {
 ...
 public:
  ...
  std::vector<Mask> GenerateMasks(const std::vector<Values_>& values) const;
 ...
 protected:
  ...
  std::pair<std::vector<Values_>, std::vector<RowSplits_>> TrimInternal(
      ValuesIterator flat_values_begin, ValuesIterator flat_values_end,
  ...
  ProcessBatch(values.begin(), values.end(),
               [&masks](std::vector<Row>* value_row_sizes) {
                 for (int i = 0; i < masks.size(); ++i) {
                   ...
                   mask.insert(mask.end(), values_row.size - values_row.used,
                               false);
                 }
               });
  return masks;
...
RoundRobinTrimmer<T, Tsplits>::TrimBatch(
    const std::vector<Values_>& flat_values,
    const std::vector<RowSplits_>& row_splits) const {
  return TrimInternal(flat_values.begin(), flat_values.end(),
...
RoundRobinTrimmer<T, Tsplits>::TrimInternal(
    ValuesIterator flat_values_begin, ValuesIterator flat_values_end,
    RowSplitsIterator splits_begin, RowSplitsIterator splits_end) const {
  ...
  ProcessSplitsByBatch(
      splits_begin, splits_end,
      [&trimmed, flat_values_begin,
       splits_begin](std::vector<Row>* values_row) {
...
#endif  // TENSORFLOW_CORE_KERNELS_TEXT_ROUND_ROBIN_TRIMMER_H_
