...
absl::Status ValidateConstraintTensor(const Tensor& tensor,
  ...
  if (tensor.shape().dim_size(0) != expected_size) {
    return errors::InvalidArgument(tensorflow::strings::StrCat(
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(
        ...
        errors::InvalidArgument("The score tensor must be of rank 2 or 3."));
    ...
    OP_REQUIRES(context, lengths_tensor.NumElements() == batch_size,
                errors::InvalidArgument(tensorflow::strings::StrCat(
    ...
    if (has_transition_weights) {
      ...
      if (!use_log_space_) {
        for (int i = 0; i < transition_weights_tensor.NumElements(); ++i) {
          OP_REQUIRES(
              context, transition_weights_tensor.flat<float>()(i) >= 0,
              errors::InvalidArgument("The transition weights tensor must not "
...
REGISTER_KERNEL_BUILDER(Name("ConstrainedSequence")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<int32>("Tin")
                            .TypeConstraint<int32>("Tsplits"),
                        ConstrainedSequence<int32, int32>);

REGISTER_KERNEL_BUILDER(Name("ConstrainedSequence")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<int32>("Tin")
                            .TypeConstraint<int64>("Tsplits"),
                        ConstrainedSequence<int32, int64>);

REGISTER_KERNEL_BUILDER(Name("ConstrainedSequence")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<int64>("Tin")
                            .TypeConstraint<int32>("Tsplits"),
                        ConstrainedSequence<int64, int32>);

REGISTER_KERNEL_BUILDER(Name("ConstrainedSequence")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<int64>("Tin")
                            .TypeConstraint<int64>("Tsplits"),
                        ConstrainedSequence<int64, int64>);
