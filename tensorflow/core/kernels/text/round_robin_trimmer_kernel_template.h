...
#ifndef TENSORFLOW_CORE_KERNELS_TEXT_ROUND_ROBIN_TRIMMER_KERNEL_TEMPLATE_H_
#define TENSORFLOW_CORE_KERNELS_TEXT_ROUND_ROBIN_TRIMMER_KERNEL_TEMPLATE_H_
...
class RoundRobinTrimOp
 ...
 private:
  enum Inputs { kMaxSeqLength = 0, kInputValues, kInputRowSplits };
  enum Outputs { kOutputValues = 0, kOutputRowSplits };
...
absl::Status RoundRobinTrimOp<Rt, T, Tsplits>::ShapeInference(
  ...
  for (int i = 0; i < num_segments; ++i) {
    SH_ASSIGN_OR_RETURN(
        ...
        c->GetInputShape((kInputValues - 1) * num_segments + i + 1));
    ...
    SH_ASSIGN_OR_RETURN(
        ...
        c->GetInputShape((kInputRowSplits - 1) * num_segments + i + 1));
    if (!row_splits_shape.Compatible(rank_1_shape)) {
      return absl::FailedPreconditionError(
          absl::StrCat("Shape must be rank 1: ", row_splits_shape.ToString()));
    }

    SH_RETURN_IF_ERROR(c->SetOutputShape(kOutputRowSplits * num_segments + i,
                                         row_splits_shape));
    SH_RETURN_IF_ERROR(
        c->SetOutputShape(kOutputValues * num_segments + i, rank_1_shape));
  }
...
absl::Status RoundRobinTrimOp<Rt, T, Tsplits>::Invoke(InvokeContext* context) {
  ...
  auto [trimmed_vals, trimmed_splits] =
      trimmer.TrimBatch(list_of_values, list_of_splits);
  ...
  for (int i = 0; i < number_of_segments_; ++i) {
    ...
    SH_RETURN_IF_ERROR(this->template FillOutputTensor<Tsplits, Tsplits>(
        trimmed_splits[i], (kOutputRowSplits * number_of_segments_) + i,
        context));
  }
...
std::vector<std::string> RoundRobinGenerateMasksOp<Rt, T, Tsplits>::Inputs() {
  // TODO(rneale): use templated value
  return {"max_sequence_length: int32", "input_values: N * T",
          "input_row_splits: N * Tsplits"};
}
...
absl::Status RoundRobinGenerateMasksOp<Rt, T, Tsplits>::ShapeInference(
  ...
  for (int i = 0; i < num_segments; ++i) {
    ...
    SH_RETURN_IF_ERROR(
        c->SetOutputShape(kOutputMasks * num_segments + i, values_shape));
  }
...
absl::Status RoundRobinGenerateMasksOp<Rt, T, Tsplits>::Invoke(
  ...
  for (int i = 0; i < number_of_segments_; ++i) {
    // Allocate output & fill output tensors.
    SH_RETURN_IF_ERROR(this->template FillOutputTensor<bool, bool>(
        masks[i], (kOutputMasks * number_of_segments_) + i, context));
  }
...
#endif  // TENSORFLOW_CORE_KERNELS_TEXT_ROUND_ROBIN_TRIMMER_KERNEL_TEMPLATE_H_
