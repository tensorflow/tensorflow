...
Status ToStatus(const LookupStatus& status) {
  ...
  return absl::InvalidArgumentError(status.error_msg);
}
...
LookupStatus LookupTableVocab::Contains(const absl::string_view key,
  ...
  if (!status.ok()) {
    ...
    // includes `message().` One will need to wait for the release
    // following 2.12.0. The code can be updated to just be the else branch
    // after such release exists.
