...
#include "tensorflow/core/kernels/text/round_robin_trimmer.h"
...
TEST_P(RoundRobinTrimmerTest, Trim_int64) {
  ...
  for (int i = 0; i < input_splits.size(); ++i)
    input_splits_64[i].insert(input_splits_64[i].end(), input_splits[i].begin(),
                              input_splits[i].end());
  std::vector<std::vector<int64_t>> input_vals_64(input_vals.size());
  for (int i = 0; i < input_vals.size(); ++i)
    input_vals_64[i].insert(input_vals_64[i].end(), input_vals[i].begin(),
                            input_vals[i].end());
...
INSTANTIATE_TEST_SUITE_P(RoundRobinTrimmerTestSuite, RoundRobinTrimmerTest,
