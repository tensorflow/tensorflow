...
#ifndef TENSORFLOW_CORE_KERNELS_TEXT_WHITESPACE_TOKENIZER_H_
#define TENSORFLOW_CORE_KERNELS_TEXT_WHITESPACE_TOKENIZER_H_
...
class WhitespaceTokenizerConfig {
 public:
  ...
  inline bool IsWhitespace(const UChar32 codepoint) const {
    return codepoint != U_SENTINEL && codepoint < max_codepoint_ &&
...
class WhitespaceTokenizer {
 public:
  ...
  void Tokenize(const absl::string_view input, std::vector<std::string>* tokens,
                std::vector<int>* start_offsets, std::vector<int>* end_offsets);
...
#endif  // TENSORFLOW_CORE_KERNELS_TEXT_WHITESPACE_TOKENIZER_H_
