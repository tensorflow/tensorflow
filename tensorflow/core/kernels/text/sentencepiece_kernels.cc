...
#include "third_party/sentencepiece/src/sentencepiece.pb.h"
#include "third_party/sentencepiece/src/sentencepiece_model.pb.h"
        ...
        [ctx, this](SentencepieceResource** resource)
            ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
              ...
              if (TF_PREDICT_FALSE(model_proto_attr.empty())) {
                return Status(absl::InvalidArgumentError(
              ...
              // TODO(rneale): Determine a better computation of what the memory
          ...
          [ctx, sp, &input_values_flat, &input_splits_flat, &output_flat](
            ...
            for (int i = start; i < limit; ++i) {
              if (i + 1 >= input_splits_flat.size()) {
                ctx->CtxFailure(
                    absl::OutOfRangeError(absl::StrCat("Invalid splits; ", i)));
                return;
              }
              if (input_splits_flat(i) > input_values_flat.size()) {
                ctx->CtxFailure(absl::OutOfRangeError(
                    absl::StrCat("Splits and values do not match; split ",
