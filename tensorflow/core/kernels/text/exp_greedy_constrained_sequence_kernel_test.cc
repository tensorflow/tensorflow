...
#include "tensorflow/core/kernels/text/text_kernels_test_util.h"
...
class ExpGreedyConstrainedSequenceTest : public tensorflow::OpsTestBase {
 public:
...
TEST_F(ExpGreedyConstrainedSequenceTest,
...
TEST_F(ExpGreedyConstrainedSequenceTest, int64inint32out) {
  // Prepare graph.
  SetUpOpWithDefaults();
...
TEST_F(ExpGreedyConstrainedSequenceTest, TwoDimensionalSequenceLengths) {
  // Prepare graph.
  SetUpOpWithDefaults();
...
TEST_F(ExpGreedyConstrainedSequenceTest,
  ...
  // All scores should be multiplied by the last row in the weight tensor, so
  // the 'real' scores are:
  // 1: {0.1 6.0 3.5 4.0}   (max is 3). OUT->3 is OK.
  // 2: {0.1, 4.5, 5.5, 5.0}   (max is 2). OUT->2 is OK.
  // 3: {10.0, 12.0, 1.5, 4.0} (max is 1). OUT->1 is not OK, so go with 0.
  // STEP 2:
  // 1: In state '3', so use row 3 in the weight tensor.
  // Weights are {5, 5, 10, 5}; 3->2 is OK and 2->OUT is OK; use 2.
  // 2: In state '2', so use row 2 in the weight tensor.
  // Weights are {5, 7.5, .5, 6.0}; 2->3 is not OK and 2->1 is not OK, so 0.
  // 3: In state 0, so use row 0 in the weight tensor.
  // Weights are {0.5, 5.5, 0.5, 5}; 0->1 is OK but 1->OUT is not, so 3.

  std::vector<int32> expected_transitions({3, 2, 2, 0, 0, 3});
...
TEST_F(ExpGreedyConstrainedSequenceTest,
  ...
  // the 'real' scores are:
  // 1: {0.1 6.0 3.5 4.0}   (max is 3). OUT->3 is OK.
  // 2: {0.1, 4.5, 5.5, 5.0}   (max is 2). OUT->2 and 2->OUT are OK.
  // 3: {10.0, 12.0, 1.5, 4.0} (max is 1). OUT->1 is not OK, so go with 0.
  // STEP 2:
  // 1: In state '3', so use row 3 in the weight tensor.
  // Weights are {5, 5, 10, 5}; 3->2 is OK and 2->OUT is OK; use 2.
  // 2: End of sequence; no change.
  // 3: In state 0, so use row 0 in the weight tensor.
  // Weights are {0.5, 5.5, 0.5, 5}; 0->1 is OK but 1->OUT is not, so 3.
...
TEST_F(ExpGreedyConstrainedSequenceTest, ErrorsIfGivenInsufficientScores) {
  // Prepare graph.
  SetUpOpWithDefaults();
