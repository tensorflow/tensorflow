...
#include "tensorflow/core/kernels/text/edit_changes.pb.h"
...
class CaseFoldUTF8Op : public tensorflow::OpKernel {
 public:
  ...
  void Compute(tensorflow::OpKernelContext* context) override {
    ...
    // TODO(gbillock): support forwarding
    ...
    for (int64 i = 0; i < input_vec.size(); ++i) {
      ...
      OP_REQUIRES(context, icu_error.isSuccess(),
                  absl::InternalError("Could not normalize input string: " +
                                      input_vec(i)));
  ...
  void Compute(tensorflow::OpKernelContext* context) override {
    ...
    for (int64 i = 0; i < input_vec.size(); ++i) {
      ...
      OP_REQUIRES(
          ...
          absl::InternalError(absl::StrCat(
              icu_error.errorName(), ": Could not normalize input string: ",
