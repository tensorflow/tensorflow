...
class UnicodeScriptTokenizeWithOffsetsOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    auto allocate_and_fill_output_tensor =
        [&](absl::string_view name, const auto& values) -> absl::Status {
      using dtype = typename std::decay_t<decltype(values)>::value_type;
      Tensor* output_tensor = nullptr;
      TF_RETURN_IF_ERROR(context->allocate_output(
          name, TensorShape({static_cast<int64>(values.size())}),
          &output_tensor));
      auto output_data = output_tensor->flat<dtype>().data();
      if (!values.empty()) {
        memcpy(output_data, values.data(), values.size() * sizeof(dtype));
      }
      return absl::OkStatus();
    };

    OP_REQUIRES_OK(context, allocate_and_fill_output_tensor("output_values",
                                                            output_values));
    OP_REQUIRES_OK(context,
                   allocate_and_fill_output_tensor("output_values_inner_splits",
                                                   output_values_inner_splits));
    OP_REQUIRES_OK(context, allocate_and_fill_output_tensor(
                                "output_offset_starts", output_offset_starts));
    OP_REQUIRES_OK(context, allocate_and_fill_output_tensor(
                                "output_offset_limits", output_offset_limits));
  }
