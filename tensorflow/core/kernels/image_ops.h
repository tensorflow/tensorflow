...
struct MapCoordinate<Device, Mode::CONSTANT> {
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE float operator()(const float out_coord,
                                                         const DenseIndex len) {
    if (out_coord < 0 || out_coord > len - 1) {
      return -1;
    }
    return out_coord;
  }
...
class ProjectiveGenerator {
 ...
 public:
  ...
  operator()(const array<DenseIndex, 4>& coords) const {
    ...
    const int64 output_x = coords[1];
    const int64 output_y = coords[2];
    ...
    const float y = map_functor(input_y, input_.dimension(2));

    const DenseIndex batch = coords[0];
    const DenseIndex channels = coords[3];
    switch (interpolation_) {
      case NEAREST:
        return nearest_interpolation(batch, x, y, channels, fill_value);
      case BILINEAR:
        return bilinear_interpolation(batch, x, y, channels, fill_value);
    }
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE T read_with_fill_value(
      const DenseIndex batch, const DenseIndex x, const DenseIndex y,
    ...
    // the input.
    return (0 <= x && x < input_.dimension(1) && 0 <= y &&
            y < input_.dimension(2))
               ? input_(array<DenseIndex, 4>{batch, x, y, channel})
               : fill_value;
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE T
  bilinear_interpolation(const DenseIndex batch, const float x, const float y,
                         const DenseIndex channel, const T fill_value) const {
    ...
    const float f11 = static_cast<float>(read_with_fill_value(
        batch, DenseIndex(x_floor), DenseIndex(y_floor), channel, fill_value));
    const float f21 = static_cast<float>(read_with_fill_value(
        batch, DenseIndex(x_ceil), DenseIndex(y_floor), channel, fill_value));
    const float value_yfloor = (x_ceil - x) * f11 + (x - x_floor) * f21;

    // f(x, y_ceil) = (x_ceil - x) / (x_ceil - x_floor) * f(x_floor, y_ceil)
    //              + (x - x_floor) / (x_ceil - x_floor) * f(x_ceil, y_ceil)
    const float f12 = static_cast<float>(read_with_fill_value(
        batch, DenseIndex(x_floor), DenseIndex(y_ceil), channel, fill_value));
    const float f22 = static_cast<float>(read_with_fill_value(
        batch, DenseIndex(x_ceil), DenseIndex(y_ceil), channel, fill_value));
    const float value_yceil = (x_ceil - x) * f12 + (x - x_floor) * f22;

    // f(x, y) = (y_ceil - y) / (y_ceil - y_floor) * f(x, y_floor)
    //         + (y - y_floor) / (y_ceil - y_floor) * f(x, y_ceil)
    T value = T((y_ceil - y) * value_yfloor + (y - y_floor) * value_yceil);
    return value;
  }
};
