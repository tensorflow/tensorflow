...
SpatialConvolutionBackwardInput(const Kernel& kernel, const OutputBackward& output_backward, typename internal::traits<OutputBackward>::Index inputRows, typename internal::traits<OutputBackward>::Index inputCols, const DenseIndex stride = 1, const DenseIndex in_stride = 1) {
  ...
  const TensorIndex forward_pad_top = numext::maxi<Index>(0, ((outputRows - 1) * stride + kernelRowsEff - inputRows) / 2);
  const TensorIndex forward_pad_left = numext::maxi<Index>(0, ((outputCols - 1) * stride + kernelColsEff - inputCols) / 2);
  ...
  const TensorIndex padding_bottom = inputRows - (outputRows - 1) * stride - 2 - padding_top + kernelRowsEff;
  const TensorIndex padding_right = inputCols - (outputCols - 1) * stride - 2 - padding_left + kernelColsEff;
