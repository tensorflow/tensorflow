...
bool MaxPoolForwardNoMask_NCHW_VECT_C::operator()(
  ...
  TF_CHECK_OK(GpuLaunchKernel(MaxPoolForwardNoMaskKernel_NCHW_VECT_C,
...
bool MaxPoolForwardWithOptionalArgmax<T>::operator()(
  ...
  if (output_size == 0) return true;
  if (propagate_nans) {
    TF_CHECK_OK(GpuLaunchKernel(MaxPoolForwardNHWC<true, T>,
        dim3((output_size + kThreadsPerBlock - 1) / kThreadsPerBlock),
        dim3(kThreadsPerBlock), 0, d.stream(),
    ...
    TF_CHECK_OK(GpuLaunchKernel(MaxPoolForwardNHWC<false, T>,
        dim3((output_size + kThreadsPerBlock - 1) / kThreadsPerBlock),
        dim3(kThreadsPerBlock), 0, d.stream(),
        output_size, bottom_data, height, width, channels, pooled_height,
        pooled_width, kernel_h, kernel_w, stride_h, stride_w, pad_t, pad_l,
...
bool MaxPoolBackwardNoMask<T>::operator()(
  ...
  TF_CHECK_OK(GpuLaunchKernel(MaxPoolBackwardNoMaskNHWC<T>,
...
bool MaxPoolBackwardWithArgmax<T>::operator()(
  ...
  TF_CHECK_OK(GpuLaunchKernel(MaxPoolBackward<T>,
      dim3((output_size + kThreadsPerBlock - 1) / kThreadsPerBlock),
      dim3(kThreadsPerBlock), 0, d.stream(),
...
>>>>>>> upstream/master
  ...
  if (data_format == FORMAT_NHWC) {
    TF_CHECK_OK(GpuLaunchKernel(MaxPoolGradBackwardNoMaskNHWC<T>,
      ...
      TF_CHECK_OK(GpuLaunchKernel(MaxPoolGradBackwardNoMaskNCHW<T>, dim3(config.block_count),
