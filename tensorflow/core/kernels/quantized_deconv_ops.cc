...
class ReferenceConv2DTransposeFunctor {
 public:
  void operator()(OpKernelContext* context, const T1* input_data,
                  ...
                  int filter_height, int filter_width, int filter_depth,
          ...
          for (int w = 0; w < input_width; w++) {
            ...
            for (int kx = 0; kx < filter_height; kx++) {
              for (int ky = 0; ky < filter_width; ky++) {
                ...
                // initialize
                int32 total = output_data[output_index];
                for (int f = 0; f < input_depth; f++) {
                  ...
                  const T2 filter_source_value =
                      filter_data[(kx * filter_width * output_depth *
                                   input_depth) +
                ...
                const int32 clamped_output =
                    std::max(top_clamped_output, lowest);
...
class QuantizedConv2DTransposeOp : public OpKernel {
 public:
  explicit QuantizedConv2DTransposeOp(OpKernelConstruction* context)
    ...
    OP_REQUIRES(
        context, strides_[1] == strides_[2],
        errors::InvalidArgument("Current implementation only supports equal "
                                "strides in the row and column dimensions"));
    // OP_REQUIRES(context, (strides_[1] == 1) & (strides_[2] == 1),
    //            errors::InvalidArgument("Current implementation doesn't
    //            support stride larger than 1"));
    OP_REQUIRES(
        context, (strides_[0] == 1) | (strides_[3] == 1),
        errors::InvalidArgument("Current implementation doesn't support stride "
                                "in batch or depth dimension"));
    OP_REQUIRES_OK(context, context->GetAttr("padding", &padding_));
    OP_REQUIRES(
        context, padding_ == VALID,
        errors::InvalidArgument(
    ...
    // Currently QuantizedConv2DTransposeOp doesn't support this parameter,
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(context, in_depth == filter.dim_size(3),
                errors::InvalidArgument("input(", in_depth, ") and filter(",
                                        filter.dim_size(3),
    ...
    const int expected_input_rows =
        ceil((float)(out_rows - filter_rows + 1) / stride);
    const int expected_input_cols =
        ceil((float)(out_cols - filter_cols + 1) / stride);
    ...
    // Create output_shape (TensorShape instance) from the input Tensor
    // output_sizes
    TensorShape output_shape;
    OP_REQUIRES_OK(context, TensorShapeUtils::MakeShape(
                                output_sizes.vec<int32>(), &output_shape));
    ...
    conv_trans_functor(context, input.flat<T1>().data(), batch, input_rows,
                       input_cols, in_depth, offset_input,
                       filter.flat<T2>().data(), filter_rows, filter_cols,
                       out_depth, offset_filter, stride, padding_,
