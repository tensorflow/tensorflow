...
Status FindBestConvolveAlgorithm(
           ...
           const LogFunc& log, se::dnn::AlgorithmConfig* algorithm_config) {
  ...
  if (CudnnUseFrontend()) {
    ...
    for (const auto& plan : plans) {
      configs.push_back(
          se::dnn::AlgorithmConfig(se::dnn::AlgorithmDesc{plan->getTag(),
                                                          plan->get_raw_desc()},
  ...
  for (const auto& profile_config : configs) {
    ...
    Status cudnn_launch_status = launch(profile_config, allocator_used,
