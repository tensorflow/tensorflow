...
struct CropAndResize<CPUDevice, T> {
  bool operator()(const OpKernelContext* context,
    ...
    std::function<void(int32 b_in, int b, int y, const float x1, const float x2,
                       const float in_y, const int image_width,
                       const float width_scale)>
        interpolation_fn = [&](int32 b_in, int b, int y, const float x1,
                               const float x2, const float in_y,
                               const int image_width, const float width_scale) {
...
struct CropAndResizeBackpropImage<CPUDevice, T> {
  bool operator()(const OpKernelContext* context,
    ...
    std::function<void(int32 b_in, int b, int x, int y, const float in_x,
                       const float in_y)>
        interpolation_fn = [&](int32 b_in, int b, int x, int y,
    ...
    if (method_name == "nearest") {
      interpolation_fn = [&](int32 b_in, int b, int x, int y, const float in_x,
                             const float in_y) {
        const int closest_x_index = roundf(in_x);
        const int closest_y_index = roundf(in_y);
