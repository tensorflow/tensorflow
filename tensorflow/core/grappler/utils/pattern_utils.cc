...
bool SubGraphMatcher<MatchingDirection::kFollowInputs>::DoesOpTypePatternMatch(
  ...
  //  - If the node has control inputs and the node will NOT remain after the fusion.
  //  - If the node has control outputs and the node will be removed after the fusion.
  if ((node_view->NumControllingFanins() > 0 && pattern.node_status != NodeStatus::kRemain)
      || (node_view->NumControlledFanouts() > 0 && pattern.node_status == NodeStatus::kRemove))
    return false;
