...
bool IsCpuCompatibleDataFormat(const NodeDef* conv_node) {
...
bool IsCpuCompatible(const RemapperContext& ctx, const Pattern& matched) {
  ...
  if (IsConv2D(node)) {
    ...
    return (IsMKLEnabled() && IsCpuCompatibleConv3D(&node));
  } else {
    return false;
  }
}
