...
inline bool VerifySingleConstant(utils::MutableNodeView* node_view,
                                 const float value) {
  ...
  if (node_def != nullptr && node_def->op() == "Const" &&
    ...
    if (const_tensor.NumElements() == 1) {
      ...
      if (std::abs(const_value - value) > 1e-2) {
        return false;
      } else {
        return true;
      }
...
inline bool VerifyConstants(RemapperContext* ctx,
  ...
  for (auto it = values_map->begin(); it != values_map->end(); ++it) {
    ...
    if (!VerifySingleConstant(node_view, it->second)) return false;
  }
  return true;
}
...
bool FindHardSwish(RemapperContext& ctx, int node_index,
  ...
  if (found_match) {
    ...
    std::map<string, float> values_map = {{"three", 3.0},
                                          {"one_sixth", 0.16666}};
    for (auto it = values_map.begin(); it != values_map.end(); ++it) {
      auto* const_or_cast_view =
          ctx.graph_view.GetNode(matched_nodes_map->at(it->first));
      if (const_or_cast_view->node()->op() == "Const") {
        if (!VerifySingleConstant(const_or_cast_view, it->second)) return false;
      } else {
        // There is Cast op after Const
        auto* const_node_view =
            const_or_cast_view->GetRegularFanin(0).node_view();
        if (const_node_view->node()->op() != "Const") return false;
        if (!VerifySingleConstant(const_node_view, it->second)) return false;
      }
    }
  }
