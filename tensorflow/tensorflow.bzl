...
def tf_custom_op_library(
    ...
    if use_pywrap_rules():
        deps = [clean_dep("//tensorflow:tensorflow_common_framework")] + deps
    else:
        deps = list(deps)
...
def py_test(
        ...
        env = {},
        extra_pywrap_deps = [clean_dep("//tensorflow:_pywrap_tensorflow")],
        **kwargs):
...
def pybind_extension(
    ...
    if use_pywrap_rules():
        _pybind_extension(
            name = name,
            common_lib_packages = common_lib_packages + ["tensorflow"],
            **kwargs
        )
