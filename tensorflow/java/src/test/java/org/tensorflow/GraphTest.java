...
public class GraphTest {
  ...
  public void buildWhileLoopSingleInput() {
    try(Graph g = new Graph();
      ...
      Graph.WhileSubgraphBuilder condGraphBuilder = new Graph.WhileSubgraphBuilder() {
        @Override
        public void buildSubgraph(Graph condGraph,
                                  Output<?>[] condInputs,
                                  Output<?>[] condOutputs) {
          Output<Integer> sixteen = TestUtil.constant(condGraph, "sixteen", 16);
          // condInputs[0] < 16
          Output<?> condOutput = condGraph.opBuilder("Less", "cond")
                  .addInput(condInputs[0]).addInput(sixteen).build().output(0);
      ...
      // could write this using lambda after Java 8
      Graph.WhileSubgraphBuilder bodyGraphBuilder = new Graph.WhileSubgraphBuilder() {
        @Override
        public void buildSubgraph(Graph bodyGraph,
                                  Output<?>[] bodyInputs,
                                  Output<?>[] bodyOutputs) {
    ...
    try(Graph g = new Graph();
      ...
      Graph.WhileSubgraphBuilder condGraphBuilder = new Graph.WhileSubgraphBuilder() {
        ...
        public void buildSubgraph(Graph condGraph,
          ...
          Output<Integer> sixteen = TestUtil.constant(condGraph, "sixteen", 16);
          Output<?> condOutput = condGraph.opBuilder("Less", "cond")
                .addInput(condInputs[0]).addInput(sixteen).build().output(0); // condInputs[0] < 16
