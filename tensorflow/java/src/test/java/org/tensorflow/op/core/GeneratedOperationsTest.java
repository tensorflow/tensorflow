...
public final class GeneratedOperationsTest {
   ...
   *
   * Creates an add node with a control dependency to an assign node. In other words,
   * the assign node is a control input to the add node.
   * When the add node is run, the assign node is expected to have run beforehand
   * due to the control dependency.
   */
  @Test
  public void testControlDependencies() {
    try (Graph g = new Graph();
      ...
      Operand<Integer> x = ops.withControlDependencies(controls).math().add(variable, ops.constant(0));
