...
class OptimizableVariable {
 public:
  ...
  virtual Output UpdateOp(const Scope& scope, const Optimizer& optimizer,
                          const Output& grad) = 0;
...
class RefVariableProcessor : public OptimizableVariable {
 public:
  ...
  Output UpdateOp(const Scope& scope, const Optimizer& optimizer,
...
void Optimizer::ComputeGradients(const Scope& scope,
  ...
  TF_CHECK_OK(AddSymbolicGradients(scope_gradient, loss, var_list, grad_loss,
                                   &grad_outputs));
...
std::vector<Output> Optimizer::ApplyGradients(
    const Scope& scope, const GradAndVar& grads_and_vars) {
