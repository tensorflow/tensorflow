...
string GetConstructorDecl(const OpInfo& op_info,
  ...
  const string prefix = absl::StrCat(op_name_prefix, op_info.op_name, "(");
  string c_decl;
...
void GetOutput(const OpInfo& op_info, string* out) {
  ...
  string return_on_error = absl::StrCat("if (!", scope_str, ".ok()) return;");
  ...
  for (int i = 0; i < op_info.graph_op_def.output_arg_size(); ++i) {
    const string arg_range = absl::StrCat(
