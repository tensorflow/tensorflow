...
_CORNER_CASES = {
    ...
    "estimator.NanLossDuringTrainingError": {"message": {}},
    "train.LooperThread": {"isAlive": {}, "join": {}, "native_id": {}},
}
...
if sys.version_info.major == 3:
    ...
    _NORMALIZE_ISINSTANCE = {
        "<class "
        "'tensorflow.lite.python.op_hint.OpHint.OpHintArgumentTracker'>": "<class "  # pylint: disable=line-too-long
        "'tensorflow.lite.python.op_hint.OpHintArgumentTracker'>",
        "<class "
        "'tensorflow.python.training.monitored_session._MonitoredSession.StepContext'>": "<class "  # pylint: disable=line-too-long
        "'tensorflow.python.training.monitored_session.StepContext'>",
        "<class "
        "'tensorflow.python.ops.variables.Variable.SaveSliceInfo'>": "<class "
    ...
    _NORMALIZE_ISINSTANCE = {
        "<class 'pybind11_object'>": "<class 'pybind11_builtins.pybind11_object'>",
    }
    ...
    def __call__(self, path, parent, children):
        ...
        def _AddMember(member_name, member_obj, proto):
            ...
            if _SkipMember(parent, member_name) or isinstance(
                ...
                return
            if member_name == "__init__" or not six.ensure_str(member_name).startswith(
                "_"
            ):
