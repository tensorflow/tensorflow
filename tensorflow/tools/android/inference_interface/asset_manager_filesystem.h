...
class AssetManagerFileSystem : public FileSystem {
 public:
  ...
  Status GetFileSize(const string& f, uint64* s/*, TransactionToken* token = nullptr*/) override;
  // Currently just returns size.
  Status Stat(const string& fname, FileStatistics* stat/*, TransactionToken* token = nullptr*/) override;
  Status GetChildren(const string& dir, std::vector<string>* r/*, TransactionToken* token = nullptr*/) override;
  ...
  Status NewAppendableFile(const string& fname,
                           std::unique_ptr<WritableFile>* result/*, TransactionToken* token = nullptr*/) override;
  Status DeleteFile(const string& f/*, TransactionToken* token = nullptr*/) override;
  ...
  Status RenameFile(const string& s, const string& t/*, TransactionToken* token = nullptr*/) override;
