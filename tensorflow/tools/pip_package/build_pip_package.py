...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--headers", help="header files for the wheel", action="append")
    parser.add_argument("--srcs", help="source files for the wheel", action="append")
    parser.add_argument("--dests", help="", action="append", default=[])
    parser.add_argument("--xla_aot", help="xla aot compiled sources", action="append")
...
def prepare_aot(aot: list[str], srcs_dir: str) -> None:
    ...
    shutil.move(
        os.path.join(srcs_dir, "tensorflow/tools/pip_package/xla_build/CMakeLists.txt"),
...
def prepare_wheel_srcs(
    ...
    shutil.move(
        os.path.join(srcs_dir, "tensorflow/tools/pip_package/THIRD_PARTY_NOTICES.txt"),
...
def rename_libtensorflow(srcs_dir: str, version: str):
    ...
    if is_macos():
        shutil.move(
            ...
            os.path.join(srcs_dir, "libtensorflow_cc.{}.dylib".format(major_version)),
        )
        shutil.move(
            os.path.join(srcs_dir, "libtensorflow_framework.{}.dylib".format(version)),
            os.path.join(
