...
def main():
          ...
          if line_str and not line_str.startswith(";"):
            ...
            if len(parts) >= 4:
              ...
              whitelist = {
                  ...
                  "FunctionDef",
                  "GraphDef",
                  "DataType_internal_data_",
                  "GetTableStacks",
                  "RetrieveCollectedErrors",
                  "DataServiceDispatcherClient",
                  "DebugEventsWriter",
                  "MakeSavedModelPathAndSingleprint",
                  "GetSnapshotSplit",
                  "ExperimentalWriteBytecode",
                  "ImportGraphDef",
                  "NewProfiler",
              }
              if any(
                ...
                if not any(
                    w in (cls_parts + [full_sym_check, fn_check] + full_sym_check.split("::"))
                    for w in whitelist
                ):
                  continue
  ...
  # Build a flat set of all harvested COFF symbols
  all_harvested_coff_symbols = set()
  for rcs_set in coff_symbols_map.values():
    all_harvested_coff_symbols.update(rcs_set)

  # Unconditionally preserve previous symbols if they are still defined in COFF
  preserve_count = 0
  common_previous = previous_symbols.intersection(all_harvested_coff_symbols)
  for sym in common_previous:
    if sym not in regenerated_symbols:
      regenerated_symbols.add(sym)
      mangled_to_unmangled[sym] = sym
      preserve_count += 1
  print(
      f"Preserved {preserve_count} symbols from previous DEF file that are"
      " still active."
  )
  ...
  to_remove_final = set()
  for sym in regenerated_symbols:
    if sym in previous_symbols:
      # Never prune already known required previous symbols
      continue
    # Split by mangled separation chars (@ and ?) to check if any component matches the excluded names
    parts = sym.replace("?", "@").split("@")
    cleaned_parts = []
    for p in parts:
      if not p:
        continue
      if p.startswith("?0") or p.startswith("?1"):
        cleaned_parts.append(p[2:])
      else:
    ...
    if own_namespaces.intersection(cleaned_parts):
      continue
    if stage2_exclude.intersection(cleaned_parts):
      to_remove_final.add(sym)
  regenerated_symbols -= to_remove_final
  print(
      f"Final safety pruning removed {len(to_remove_final)} unwanted COFF"
      " symbols matching inferred stage2_exclude."
  )
