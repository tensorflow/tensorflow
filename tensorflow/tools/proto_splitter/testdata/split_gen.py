...
_CHILD_NODES_FIELD_TAG = test_message_pb2.StringNode.DESCRIPTOR.fields_by_name[
    "child_nodes"
].number
...
class StringNodeSplitter(split.ComposableSplitter):
    ...
    def __init__(
        self,
        proto: test_message_pb2.StringNode,
...
def main(argv: Sequence[str]) -> None:
    ...
    def make_string_tree(
        ...
        for i in range(level + 1):
            make_string_tree(
                string_tree.child_nodes.add(),
                level + 1,
                label + str(level + 1) + str(i),
            )
        return string_tree
    ...
    file_io.write_string_to_file(
        os.path.join(SPLITTER_TESTDATA_PATH.value, "split-tree.pbtxt"), str(string_tree)
    )
