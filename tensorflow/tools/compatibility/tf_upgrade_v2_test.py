...
"""
    ...
    def testExtractGlimpse(self):
        text = (
            'tf.image.extract_glimpse(x, size, off, False, False, False, name="foo")\n'
        )
        _, unused_report, unused_errors, new_text = self._upgrade(text)
        self.assertEqual(
            new_text,
    ...
    def test_substr(self):
        ...
        _, unused_report, errors, new_text = self._upgrade(text)
        self.assertEqual(
            "tf.strings.substr(input, pos, len, name=name, unit=unit)\n", new_text
        )
        self.assertEqual(errors, [])
    ...
    def testBatchToSpace(self):
        ...
        expected_text = (
            "tf.batch_to_space(input, crops=crops, block_shape=block_size, name=name)"
        )
        _, unused_report, unused_errors, new_text = self._upgrade(text)
        self.assertEqual(new_text, expected_text)
    ...
    def testSoftMaxCrossEntropyWithLogits(self):
        ...
        expected_text = (
            "tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(foo(bar)))"
        )
        _, unused_report, unused_errors, new_text = self._upgrade(text)
    ...
    def testSparseMatmul(self):
        ...
        expected_text = (
            "tf.linalg.matmul(a, b, c, d, a_is_sparse=e, b_is_sparse=f, name=g)\n"
        )
        _, unused_report, unused_errors, new_text = self._upgrade(text)
        self.assertEqual(new_text, expected_text)
    ...
    def testNnInTopK(self):
        ...
        expected_text = (
            "tf.nn.in_top_k(predictions=predictions, targets=targets, k=k, name=name)"
        )
        _, unused_report, unused_errors, new_text = self._upgrade(text)
        self.assertEqual(new_text, expected_text)
    ...
    def testSpaceToDepth(self):
        ...
        expected_text = (
            "tf.nn.space_to_depth(input, block_size, name=name, "
            "data_format=data_format)"
        )
        _, unused_report, unused_errors, new_text = self._upgrade(text)
        self.assertEqual(new_text, expected_text)
...
def _log_prob(self, x):
  return tf.debugging.assert_all_finite(
    ...
    def test_sample_distorted_bounding_box(self):
        ...
        expected = (
            "tf.image.sample_distorted_bounding_box(a, b, c, "
    ...
    def test_contrib_summary_generic(self):
        ...
        expected = (
            "tf.compat.v2.summary.write(tag='foo', data=myval, metadata=meta, step=42)"
        )
    ...
    def test_contrib_summary_image(self):
        ...
        expected = (
            "tf.compat.v2.summary.image(name='foo', data=myval, max_outputs=3, step=42)"
        )
        _, _, errors, new_text = self._upgrade(text)
        self.assertEqual(expected, new_text)
    ...
    def test_api_spec_reset_between_files(
        self, upgrade_compat_v1_import, text_a, expected_text_a, text_b, expected_text_b
    ):
