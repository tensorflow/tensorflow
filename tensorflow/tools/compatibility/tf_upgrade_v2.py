...
class CompatV1ImportReplacer(ast.NodeVisitor):
    ...
    def visit_Import(self, node):  # pylint: disable=invalid-name
        ...
        for import_alias in node.names:
            ...
            if (
                import_alias.name == "tensorflow.compat.v1"
                and import_alias.asname == "tf"
            ):
...
class TFAPIChangeSpec(ast_edits.NoUpdateSpec):
    ...
    def preprocess(self, root_node, after_compat_v1_upgrade=False):
        ...
        if (
            compat_v1_import in detections
            and self.upgrade_compat_v1_import
            and not after_compat_v1_upgrade
        ):
    ...
    def clear_preprocessing(self):
        self.__init__(
            import_rename=self.import_rename,
...
def _iterator_transformer(parent, node, full_name, name, logs):
    ...
    # conversion.
    if full_name and (
        full_name.startswith("tf.compat.v1.data") or full_name.startswith("tf.data")
    ):
        return
        ...
        if kw.arg == "skip_empty":
            if _is_ast_false(kw.value):
                ...
                return _rename_to_compat_v1(
                    ...
                    full_name,
                    logs,
                    "tf.string_split's replacement no longer "
