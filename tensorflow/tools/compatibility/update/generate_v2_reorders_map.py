...
def collect_function_arg_names(
    function_names, return_all_args_function_names, function_renames
):
    ...
    for name_v1, attr_v1 in function_name_v1_to_attr.items():
        ...
        if name_v1 in function_renames:
            ...
            if name_v2.startswith("tf.compat.v1."):
                raise ValueError(
                    f"Symbol `{name_v1}` is renamed to `{name_v2}`, "
        ...
        if args_v1 == args_v2:
            raise ValueError(
                f"Symbol `{name_v1}` has no changes in arguments, "
                "no need to add keyword argument names, "
                "remove from `reordered_function_names`"
            )
...
def update_reorders_v2(output_file_path):
    ...
    all_reorders = collect_function_arg_names(
        reordered_function_names, need_kwargs_function_names, function_renames
    )

    # List of reorder lines to write to output file in the form:
    #   'tf.function_name': ['arg1', 'arg2', ...]
    rename_lines = [
        get_reorder_line(name, arg_names) for name, arg_names in all_reorders.items()
    ]
