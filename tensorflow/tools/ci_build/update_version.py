...
def check_existence(filename):
    ...
    if not os.path.exists(filename):
        raise RuntimeError(
            "%s not found. Are you under the TensorFlow source root"
            " directory?" % filename
        )
...
def get_current_semver_version():
    ...
    return Version(old_major, old_minor, old_patch_num, old_extension, version_type)


def update_version_h(old_version, new_version):
    """Update tensorflow/core/public/version.h."""
    replace_string_in_line(
        "#define TF_MAJOR_VERSION %s" % old_version.major,
        "#define TF_MAJOR_VERSION %s" % new_version.major,
        VERSION_H,
    )
    replace_string_in_line(
        "#define TF_MINOR_VERSION %s" % old_version.minor,
        "#define TF_MINOR_VERSION %s" % new_version.minor,
        VERSION_H,
    )
    replace_string_in_line(
        "#define TF_PATCH_VERSION %s" % old_version.patch,
...
def update_readme(old_version, new_version):
    ...
    replace_string_in_line(
        r"%s\.%s\.([[:alnum:]]+)-" % (old_version.major, old_version.minor),
        "%s-" % pep_440_str,
        README_MD,
    )
...
def update_tensorflow_bzl(old_version, new_version):
    ...
    old_mmp = "%s.%s.%s" % (old_version.major, old_version.minor, old_version.patch)
    new_mmp = "%s.%s.%s" % (new_version.major, new_version.minor, new_version.patch)
    replace_string_in_line(
        'VERSION = "%s"' % old_mmp, 'VERSION = "%s"' % new_mmp, TENSORFLOW_BZL
    )
...
def check_for_lingering_string(lingering_string):
    ...
    if linger_strs:
        print(
            "WARNING: Below are potentially instances of lingering old version "
        ...
        print(
            'No lingering old version strings "%s" found in source directory'
...
def main():
    ...
    # Arg information
    parser.add_argument(
        "--version", help="<new_major_ver>.<new_minor_ver>.<new_patch_ver>", default=""
    )
    parser.add_argument(
        "--nightly", help="disable the service provisioning step", action="store_true"
    )
