...
syslibs_configure_path = os.path.join(
    tf_source_path, "third_party", "systemlibs", "syslibs_configure.bzl"
)
...
if not os.path.isdir(tf_source_path):
    raise ValueError(
        "The path to the TensorFlow source must be passed as the first argument"
    )

if not os.path.isfile(syslibs_configure_path):
    raise ValueError(
        "Could not find syslibs_configure.bzl at %s" % syslibs_configure_path
    )
...
missing_build_files = [
    file
    for file in system_build_files_from_workspace
...
if syslibs != syslibs_from_workspace:
    ...
    if additional_syslibs:
        ...
        print(
            "Libs missing in workspace (or superfluous in syslibs_configure): " + libs
        )
