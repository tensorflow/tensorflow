...
flags.DEFINE_string(
    "hub_username", None, "Dockerhub username, only used with --upload_to_hub"
)
...
flags.DEFINE_integer(
    "hub_timeout", 3600, "Abort Hub upload if it takes longer than this."
)
...
flags.DEFINE_boolean("build_images", False, "Do not build images", short_name="b")
...
class TfDockerTagValidator(cerberus.Validator):
    ...
    def _validate_ispartial(self, ispartial, field, value):
        ...
        if ispartial and value not in self.partials:
            self._error(
                field, "{} is not present in the partials directory.".format(value)
            )
...
def gather_tag_args(slices, cli_input_args, required_args):
    ...
    for arg in required_args:
        if arg not in args:
            eprint(
                (
                    "> Error: {} is not a valid slice_set, and also isn't an arg "
        ...
        for tag_spec in release["tag_specs"]:
            ...
            for slices in slice_combos:
                ...
                tag_name = build_name_from_slices(
                    tag_spec, slices, tag_args, release["is_dockerfiles"]
                )
                ...
                dockerfile_contents = merge_partials(
                    spec["header"], used_partials, all_partials
                )
    ...
    for path, _, files in os.walk(partial_path):
        for name in files:
            ...
            if ".partial.Dockerfile" not in fullpath:
                eprint(
                    (
                        "> Probably not a problem: skipping {}, which is not a "
                        "partial."
                    ).format(fullpath)
                )
                continue
...
def main(argv):
    ...
    if not v.validate(tag_spec):
        eprint(
            "> Error: {} is an invalid spec! The errors are:".format(FLAGS.spec_file)
        )
    ...
    if FLAGS.upload_to_hub:
        ...
        if not FLAGS.hub_repository:
            eprint("> Error: please set --hub_repository when uploading to Dockerhub.")
            exit(1)
    ...
    for tag, tag_defs in all_tags.items():
        for tag_def in tag_defs:
            ...
            if FLAGS.exclude_tags_matching and re.match(
                FLAGS.exclude_tags_matching, tag
            ):
                eprint(
                    '>> Excluded due to match against "{}".'.format(
                        FLAGS.exclude_tags_matching
                    )
                )
                continue

            if FLAGS.only_tags_matching and not re.match(FLAGS.only_tags_matching, tag):
                eprint(
                    '>> Excluded due to failure to match against "{}".'.format(
            ...
            if (
                is_x86
                and any(arch in tag for arch in ["ppc64le"])
                or not is_x86
                and proc_arch not in tag
            ):
                continue
                ...
                try:
                    ...
                    if FLAGS.run_tests_path:
                        ...
                        for test in tag_def["tests"]:
                            ...
                            if code != 0:
                                eprint(
                                    '>> {} failed tests with status: "{}"'.format(
                                        repo_tag, code
                                    )
                                )
                    ...
                    eprint(
                        '>> {} failed to build with message: "{}"'.format(
                            repo_tag, e.msg
                        )
                    )
    ...
    eprint(
        "> Writing built{} tags to standard out.".format(
