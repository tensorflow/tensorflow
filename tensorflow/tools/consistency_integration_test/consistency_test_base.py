...
class ConsistencyTestBase(tf.test.TestCase):
    ...
    def _deep_equal(self, left, right):
        ...
        if isinstance(left, tf.SparseTensor) and isinstance(right, tf.SparseTensor):
            return (
                self._deep_equal(left.indices, right.indices)
    ...
    def _generic_test(self, f_raw, examples, input_signature=None, skip_modes=None):
        ...
        for arg, out, failure, bugs in examples:
            del out
            self.recordProperty(
                'Input "{}"'.format(arg), {"not-working": failure, "bugs": bugs}
            )
