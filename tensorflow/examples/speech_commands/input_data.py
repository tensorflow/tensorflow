...
def load_wav_file(filename):
    ...
    with tf.compat.v1.Session(graph=tf.Graph()) as sess:
        ...
        return sess.run(
            wav_decoder, feed_dict={wav_filename_placeholder: filename}
        ).audio.flatten()
...
class AudioProcessor(object):
    ...
    def prepare_background_data(self):
        ...
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            ...
            for wav_path in gfile.Glob(search_path):
                wav_data = sess.run(
                    wav_decoder, feed_dict={wav_filename_placeholder: wav_path}
                ).audio.flatten()
    ...
    def prepare_processing_graph(self, model_settings, summaries_dir):
        ...
        with tf.compat.v1.get_default_graph().name_scope("data"):
            ...
            background_mul = tf.multiply(
                self.background_data_placeholder_, self.background_volume_placeholder_
            )
            ...
            if model_settings["preprocess"] == "average":
                ...
                tf.compat.v1.summary.image(
                    "shrunk_spectrogram", self.output_, max_outputs=1
                )
            elif model_settings["preprocess"] == "mfcc":
    ...
    def get_unprocessed_data(self, how_many, model_settings, mode):
        ...
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            ...
            scaled_foreground = tf.multiply(
                wav_decoder.audio, foreground_volume_placeholder
            )
