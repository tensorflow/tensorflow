...
def create_model(
    ...
    if model_architecture == "single_fc":
        ...
        return create_tiny_embedding_conv_model(
            fingerprint_input, model_settings, is_training
        )
    else:
        raise Exception(
...
def create_conv_model(fingerprint_input, model_settings, is_training):
    ...
    max_pool = tf.nn.max_pool2d(
        input=first_dropout, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME"
    )
    ...
    second_conv = (
        tf.nn.conv2d(
            input=max_pool, filters=second_weights, strides=[1, 1, 1, 1], padding="SAME"
        )
        + second_bias
    )
...
def create_low_latency_svdf_model(
    ...
    if input_shape[-1].value is None:
        raise ValueError(
            "The last dimension of the input to `SVDF` should be defined. Found `None`."
        )
    if input_shape[-1].value % input_frequency_size != 0:
        raise ValueError(
            "The last dimension of the input to `SVDF` = {0} must be "
...
def create_tiny_conv_model(fingerprint_input, model_settings, is_training):
    ...
    first_dropout_element_count = int(
        first_dropout_output_width * first_dropout_output_height * first_filter_count
    )
    flattened_first_dropout = tf.reshape(
        first_dropout, [-1, first_dropout_element_count]
    )
    label_count = model_settings["label_count"]
    final_fc_weights = tf.compat.v1.get_variable(
        name="final_fc_weights",
...
def create_tiny_embedding_conv_model(fingerprint_input, model_settings, is_training):
    """Builds a convolutional model aimed at microcontrollers.
    ...
    second_dropout_element_count = int(
        second_dropout_output_width * second_dropout_output_height * second_filter_count
    )
    flattened_second_dropout = tf.reshape(
        second_dropout, [-1, second_dropout_element_count]
    )
    label_count = model_settings["label_count"]
    final_fc_weights = tf.compat.v1.get_variable(
        name="final_fc_weights",
    ...
    final_fc = tf.matmul(flattened_second_dropout, final_fc_weights) + final_fc_bias
    if is_training:
        return final_fc, dropout_rate
    else:
        return final_fc
