...
func ExampleSessionOptions_configProto() {
	// ConfigProto is a tensorflow.ConfigProto protocol buffer message.
	m := corepb.ConfigProto{
		// See ConfigProto documentation for a complete list of options. The options
		// set in this example are only intended to demonstrate use.
	...
	// SessionOptions accepts the binary serialized ConfigProto message.
	_ = SessionOptions{Config: b}
}
...
func TestSessionConfig(t *testing.T) {
	...
	if output[0].Value().(int32) != 14 {
		t.Fatalf("Got %v, want 14", output[0].Value())
	}
}

func TestSessionConfig_RawMessage(t *testing.T) {
	// The []byte form of Config here was generated using a toy
