...
// SimpleITensor implementation moved to Proxy ITensor class file.
// See tensorflow/compiler/tf2tensorrt/utils/trt_tensor_proxy.h
      ...
      for (int h = 0; h < shape.d[2]; ++h) {
        for (int w = 0; w < shape.d[3]; ++w) {
          odata[n * ostrides.d[0] + c * ostrides.d[1] + h * ostrides.d[2] +
                w * ostrides.d[3]] = idata[n * istrides.d[0] + c * istrides.d[1] +
...
void Converter::PropagateQuantizationRanges() {
  ...
  while (information_added) {
    ...
    for (auto it = quantization_infer_proxy_.begin();
      ...
      // We can remove edges when the output range is known
      if (quantization_ranges_proxy_.find(it->second) != quantization_ranges_proxy_.end()) {
...
Status ConvertShape(OpConverterParams* params) {
  ...
  nvinfer1::IShapeLayer* shape_layer =
      params->converter->network()->addShape(*inputs.at(0).tensor()->trt_tensor());
...
Status ConvertDynamicReshape(OpConverterParams* params) {
  ...
  if (params->validation_only) return Status::OK();
  nvinfer1::IShuffleLayer* layer =
      params->converter->network()->addShuffle(*input_tensor.tensor()->trt_tensor());
...
Status Converter::DynamicReshape(ITensorProxyPtr input,
  ...
  *output = nullptr;
  ...
  ITensorProxyPtr shape = network()->addShape(*input->trt_tensor())->getOutput(0);
  ...
  nvinfer1::IShuffleLayer* shuffle = network()->addShuffle(*input->trt_tensor());
...
Status ConvertPool3D(OpConverterParams* params) {
  ...
  nvinfer1::IPoolingLayer* layer =
      params->converter->network()->addPoolingNd(*tensor->trt_tensor(), type, ksize);
  TFTRT_RETURN_ERROR_IF_NULLPTR(layer, node_def.name());
  ITensorProxyPtr output_tensor = layer->getOutput(0);
...
Status ConvertQuantize(OpConverterParams* params) {
  ...
  params->converter->ProvideQuantizationRange(&input0, min_range,
                                              max_range);
...
StatusOr<ITensorProxyPtr> ConvertMatMulImpl(OpConverterParams* params,
  ...
  nvinfer1::IMatrixMultiplyLayer* layer =
      params->converter->network()->addMatrixMultiply(*tensor_a->trt_tensor(), op_a,
                                                      *tensor_b->trt_tensor(), op_b);
...
struct EinsumDescriptor {
  ...
  Status SetDynamicSize(OpConverterParams* params,
    ...
    for (int i = 0; i < operand.GetTrtDims().nbDims; i++) {
      ...
      auto* layer = params->converter->network()->addSlice(*shape->trt_tensor(), {1, {idx}},
                                                           {1, {1}}, {1, {1}});
...
Status GetDimsProdDynamic(OpConverterParams* params,
  ...
  *out = *begin;
  ...
  while (begin != end) {
    nvinfer1::IElementWiseLayer* layer =
        params->converter->network()->addElementWise(
            *(*out)->trt_tensor(), *(*begin)->trt_tensor(), nvinfer1::ElementWiseOperation::kPROD);
    TFTRT_RETURN_ERROR_IF_NULLPTR(layer, params->node_def.name());
...
CalcDepthSpaceDynamicShape(OpConverterParams* params, int block_size,
  ...
  ITensorProxyPtr num_channels =
      params->converter->network()
          ->addSlice(*shape->trt_tensor(), {1, {channels_axis}}, {1, {1}}, {1, {1}})
          ->getOutput(0);
  ITensorProxyPtr h =
      params->converter->network()
  ...
  if (node_def.op() == "DepthToSpace") {
    ...
    first_shuffle_tensors[3] =
        params->converter->network()
            ->addElementWise(*num_channels->trt_tensor(), *r_squared->trt_tensor(),
    ...
    second_shuffle_tensors[2] =
        params->converter->network()
            ->addElementWise(*h->trt_tensor(), *r->trt_tensor(), nvinfer1::ElementWiseOperation::kPROD)
            ->getOutput(0);
    second_shuffle_tensors[3] =
        params->converter->network()
            ->addElementWise(*w->trt_tensor(), *r->trt_tensor(), nvinfer1::ElementWiseOperation::kPROD)
            ->getOutput(0);
    ...
    first_shuffle_tensors[2] =
        params->converter->network()
            ->addElementWise(*h->trt_tensor(), *r->trt_tensor(), nvinfer1::ElementWiseOperation::kDIV)
            ->getOutput(0);
    first_shuffle_tensors[3] = r;
    first_shuffle_tensors[4] =
        params->converter->network()
            ->addElementWise(*w->trt_tensor(), *r->trt_tensor(), nvinfer1::ElementWiseOperation::kDIV)
            ->getOutput(0);
...
Status ConvertDepthSpaceShuffle(OpConverterParams* params) {
  ...
  params->converter->MarkQuantizationRangesAsInferrable(
      &input_tensor, &first_shuffle_tensor);
  params->converter->MarkQuantizationRangesAsInferrable(
      &first_shuffle_tensor, &second_shuffle_tensor);
...
Status ConvertCombinedNMS(OpConverterParams* params) {
  ...
  nvinfer1::IPluginV2Layer* layer = params->converter->network()->addPluginV2(
      &trt_plugin_inputs[0], static_cast<int>(trt_plugin_inputs.size()), *plugin);
  TFTRT_RETURN_ERROR_IF_NULLPTR(layer, node_def.name());
