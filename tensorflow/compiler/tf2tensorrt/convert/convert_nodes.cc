...
// Sets the ILayer name in the form of <tf_related_part>:<trt_operation_name>.
void SetLayerNameHelper(nvinfer1::ILayer* layer, absl::string_view tf_name) {
  const char* trt_name = LayerTypeToString(layer->getType());
  layer->setName(absl::StrCat(tf_name, ":", trt_name).c_str());
}
...
void SetLayerName(nvinfer1::ILayer* layer, const NodeDef& node_def,
  ...
  if (sub_op_suffix.empty()) {
    SetLayerNameHelper(layer, node_def.name());
  } else {
    SetLayerNameHelper(layer,
...
void SetLayerName(nvinfer1::ILayer* layer, absl::string_view main_op_name,
  ...
  SetLayerNameHelper(layer, absl::StrCat(main_op_name, "-", layer_name_suffix));
}
