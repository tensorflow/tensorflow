...
Status ConvertSlice(OpConverterParams* params) {
  ...
  auto begin_weights = inputs.at(1).weights();
  auto size_weights = inputs.at(2).weights();

  // Check that "begin" is not negative.
  {
    auto begin_view = begin_weights.GetSpan<int32>();
    if (std::any_of(begin_view.begin(), begin_view.end(),
                    [](const int32 val) { return val < 0; })) {
      return errors::InvalidArgument("\"begin\" in Slice is out of range, at ",
                                     node_def.name());
    }
  }
  ...
  {
    auto size_view = size_weights.GetSpan<int32>();
    if (std::any_of(size_view.begin(), size_view.end(),
                    [](const int32 val) { return val < -1; })) {
      return errors::InvalidArgument("\"size\" in Slice is out of range, at ",
                                     node_def.name());
    }
  }
  ...
  absl::InlinedVector<int64, 4> input_dims(dims.d, dims.d + dims.nbDims);
  if (params->use_implicit_batch) {
  ...
  if (params->use_implicit_batch) {
    ...
    const bool begin_is_modified = begin_v[0] != 0;
  ...
  // Setup begin_mask, end_mask, end_tensor, strides_tensor, and end_tensor.
  ...
  for (int i = 0; i < input_shape.dims(); i++) {
    ...
    if (size_vec(i) == -1) {
      ...
      if (end_vec(i) > input_dims[i] && input_dims[i] > 0) {
        return errors::InvalidArgument("\"begin\" + \"size\" for dimension ", i,
                                       " in Slice is out of range, at ",
                                       node_def.name());
      }
    }
  }
  ...
  // Calcualte the begin, stride, and end parameters to be passed to
  // ConvertStridedSliceHelper.
  // auto size = size_weights.GetSpan<int32>();
  // absl::InlinedVector<int64, 4> stride_v(
  //     begin.size(), 1);  // The Slice operation always has a stride of 1.
  // absl::InlinedVector<int64, 4> begin_v;
  // absl::InlinedVector<int64, 4> end_v;
  // for (int i = 0; i < input_dims.size(); i++) {
  //   if (size[i] < 0) {
  //     if (size[i] == -1) {
  //       // This case is handled at runtime, see ConvertStridedSliceHelper.
  //       end_v[i] = 0;
  //     } else {
  //       return errors::InvalidArgument("Invalid size value at ",
  //                                      node_def.name());
  //     }
  //   } else {
  //     if (begin[i] < 0 || begin[i] > input_dims[i]) {
  //       return errors::InvalidArgument(
  //           "\"begin\" for dimension ", std::to_string(i), " in ",
