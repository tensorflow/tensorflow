...
class TRTEngineResourceOpsTest
 ...
 protected:
  ...
  ITensorProxyPtr NetworkWith2Inputs(nvinfer1::INetworkDefinition* network,
    ...
    auto slice_layer = network->addSlice(*input->trt_tensor(), start, stride, stride);
    ...
    auto layer = network->addElementWise(*sliced_input->trt_tensor(), *sliced_input->trt_tensor(),
