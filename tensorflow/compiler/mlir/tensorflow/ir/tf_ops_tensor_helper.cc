...
RankedTensorType GetRankedTensorTypeForOperand(Value operand) {
  ...
  if (matchPattern(operand, m_Constant(&attr))) {
    return dyn_cast<RankedTensorType>(attr.getType());
  }
  return dyn_cast<RankedTensorType>(operand.getType());
}
...
Type DeduceEqualCmpOpType(Builder *builder, Location loc, Value x, Value y,
  ...
  auto ranked_type = dyn_cast<RankedTensorType>(result_type);
...
Type InferReductionOpType(Value input, Value reduction_indices,
  ...
  auto ranked_ty = dyn_cast<RankedTensorType>(input_ty);
