...
OpFoldResult AddNOp::fold(FoldAdaptor adaptor) {
  ...
  auto IsKnownZero = [](Attribute attr) {
    ...
    if (isa<FloatType>(element_ty))
      return splat.getSplatValue<llvm::APFloat>().isZero();
    if (isa<IntegerType>(element_ty))
  ...
  auto result_ty = dyn_cast<ShapedType>(getType());
  if (!result_ty || !result_ty.hasStaticShape()) return {};

  if (non_zero_index == -1) {
    return SplatElementsAttr::get(
        result_ty,
        cast<DenseElementsAttr>(*operands.begin()).getSplatValue<Attribute>());
  }
...
LogicalResult BatchToSpaceOp::verify() {
  ...
  auto crops_type = cast<TensorType>(op.getCrops().getType());
  ...
  auto output_type = cast<TensorType>(op.getOutput().getType());
...
LogicalResult BatchToSpaceNDOp::verify() {
  ...
  auto block_shape_ty = cast<ShapedType>(op.getBlockShape().getType());
  auto crops_ty = cast<ShapedType>(op.getCrops().getType());
...
LogicalResult BiasAddOp::verify() {
  ...
  RankedTensorType bias_ty = dyn_cast<RankedTensorType>(op.getBias().getType());
...
LogicalResult BroadcastGradientArgsOp::verify() {
  ...
  // shape.
  auto r0_ty = dyn_cast<RankedTensorType>(op.getR0().getType());
  auto r1_ty = dyn_cast<RankedTensorType>(op.getR1().getType());
...
LogicalResult BroadcastGradientArgsOp::fold(
  ...
  auto build_out_dense_element = [](SmallVectorImpl<int64_t>& shape,
                                    Type input_type) {
    Type element_type = cast<mlir::TensorType>(input_type).getElementType();
...
LogicalResult FoldConstantCaseOp::matchAndRewrite(
  ...
  auto func = cast<SymbolRefAttr>(op.getBranches()[index]);
...
static LogicalResult VerifyCaseOrIfOpBranchFunctions(
  ...
  for (const auto& branch : llvm::enumerate(branches)) {
    auto branch_func = symbol_table.lookupNearestSymbolFrom<func::FuncOp>(
        op, cast<SymbolRefAttr>(branch.value()));
    if (!branch_func)
      return op->emitOpError()
...
HoistCwiseBinaryOutOfConcat::GetHoistParams(
  ...
  auto ranked = dyn_cast<RankedTensorType>(op.getType());
...
static LogicalResult Verify(OpT op) {
  ...
  if (matchPattern(op.getAxis(), m_Constant(&axis_attr))) {
    auto input_ty = dyn_cast<RankedTensorType>(op.getX().getType());
    if (input_ty) {
...
LogicalResult ConcatOffsetOp::verify() {
  ...
  auto ranked_dim = dyn_cast<RankedTensorType>(op.getConcatDim().getType());
  ...
  for (const auto& shape_offset_idx :
    ...
    auto ranked_shape = dyn_cast<RankedTensorType>(shape.getType());
...
LogicalResult ConcatOffsetOp::fold(FoldAdaptor adaptor,
  ...
  auto concat_dim_attr = dyn_cast_or_null<DenseIntElementsAttr>(operands[0]);
  ...
  for (Attribute shape : llvm::drop_begin(operands, 1))
    if (auto shape_attr = dyn_cast_or_null<DenseIntElementsAttr>(shape))
...
void ConstOp::build(OpBuilder& builder, OperationState& result,
                    Attribute value) {
  ShapedType type;
  if (auto elem_attr = dyn_cast<ElementsAttr>(value)) {
    ...
    auto typed_attr = cast<TypedAttr>(value);
...
void ConstOp::build(OpBuilder& builder, OperationState& result, Type type,
  ...
  if (isa<TensorType>(type) && isa<ElementsAttr>(value)) {
...
static LogicalResult VerifyConvOpAttributes(
  ...
  auto is_not_positive = [](Attribute val) {
    return cast<IntegerAttr>(val).getValue().getSExtValue() <= 0;
  };
...
static LogicalResult Verify(OpT op) {
  ...
  if (padding == tensorflow::Padding::EXPLICIT) {
    ...
    ArrayAttr explicit_pad =
        dyn_cast_or_null<::mlir::ArrayAttr>(op->getAttr("explicit_paddings"));
    if (!explicit_pad) {
  ...
  if (auto ty = dyn_cast<RankedTensorType>(op.getInput().getType())) {
    absl::string_view data_format(op.getDataFormat().data(),
                                  op.getDataFormat().size());
    tensorflow::TensorFormat format;
  ...
  if (auto ty = dyn_cast<RankedTensorType>(op.getFilter().getType())) {
...
static LogicalResult inferConvReturnTypeComponents(
  ...
  const TensorType input_ty = cast<TensorType>(input.getType());
  const TensorType filter_ty = cast<TensorType>(filter.getType());
...
LogicalResult Conv2DOp::inferReturnTypeComponents(
  ...
  ArrayRef<Attribute> explicit_padding;
  ArrayAttr explicit_pad =
      dyn_cast_or_null<::mlir::ArrayAttr>(op.getExplicitPaddings());
  if (!explicit_pad) {
...
StringRef Conv2DOp::GetOptimalLayout(const RuntimeDevices& devices) {
  ...
  auto input_ty = dyn_cast<TensorType>(getInput().getType());
  ...
  auto filter_ty = dyn_cast<RankedTensorType>(getFilter().getType());
...
StringRef Conv2DBackpropInputOp::GetOptimalLayout(
  ...
  auto filter_ty = dyn_cast<TensorType>(getFilter().getType());
...
class DivNoNanOrMulNoNanConstantY : public OpRewritePattern<OpT> {
  ...
  LogicalResult matchAndRewrite(OpT op,
    ...
    if (auto yDefOp = dyn_cast_or_null<TF::ConstOp>(y.getDefiningOp())) {
      ...
      bool yHasComplexElements = isa<ComplexType>(typeOfElementsInY);
...
LogicalResult DynamicStitchOp::verify() {
  ...
  if (RankedTensorType out_ty = dyn_cast<RankedTensorType>(op.getType())) {
    if (out_ty.getRank() == 0) {
  ...
  for (auto it : llvm::zip(op.getIndices(), op.getData())) {
    ...
    RankedTensorType index_ty = dyn_cast<RankedTensorType>(index.getType());
  ...
  if (all_indices_const) {
    ...
    if (inferred_item_shape) {
      ...
      auto out_ty = cast<TensorType>(op.getType());
...
OpFoldResult EmptyOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto float_type = dyn_cast<FloatType>(etype)) {
  ...
  if (auto int_type = dyn_cast<IntegerType>(etype)) {
...
OpFoldResult EnsureShapeOp::fold(FoldAdaptor) {
  ShapedType type = dyn_cast<ShapedType>(getInput().getType());
...
static LogicalResult flipComatibleShapeError(Ty op, PatternRewriter& rewriter) {
  ...
  // always fail.
  auto x_ty = dyn_cast<RankedTensorType>(op.getX().getType());
  auto y_ty = dyn_cast<RankedTensorType>(op.getY().getType());
  if (ty.getRank() == 0 &&
...
Type InferExpandDimsOpType(Value input, Value dim) {
  Type element_ty = cast<TensorType>(input.getType()).getElementType();
...
LogicalResult FakeQuantWithMinMaxVarsPerChannelOp::verify() {
  ...
  auto inputs_type = dyn_cast<RankedTensorType>(inputs.getType());
...
static ShapedType InferFillOpType(Value dims, Value value) {
  Type etype = cast<ShapedType>(value.getType()).getElementType();
  ...
  if (auto shape_op = dims.getDefiningOp<ShapeOp>()) {
    if (auto t = dyn_cast<ShapedType>(shape_op.getInput().getType())) {
      return t;
    }
  }
...
OpFoldResult FillOp::fold(FoldAdaptor adaptor) {
  ...
  auto value = dyn_cast_or_null<ElementsAttr>(operands[1]);
  if (!value) return {};

  if (type.hasStaticShape())
    return DenseElementsAttr::get(type, value.getValues<Attribute>()[0]);

  auto dims = dyn_cast_or_null<DenseIntElementsAttr>(operands[0]);
  if (!dims) return {};
...
StringRef FusedBatchNormGradV3Op::GetOptimalLayout(
  ...
  // For f16 data type on devices with Tensor Cores support NHWC data format
  // is up to ~2x faster.
  auto x_ty = cast<TensorType>(getX().getType());
...
static StringRef GetOptimalLayout(const RuntimeDevices& devices, Op* op) {
  ...
  // For f16 data type on devices with Tensor Cores support NHWC data format
  // is up to ~2x faster.
  auto x_ty = cast<TensorType>(op->getX().getType());
  const bool is_f16 = x_ty.getElementType().isF16();
  if (is_f16 && CanUseTensorCores(devices)) return "NHWC";
...
LogicalResult GatherV2Op::verify() {
  ...
  if (auto ty = dyn_cast<RankedTensorType>(op.getIndices().getType())) {
  ...
  if (matchPattern(op.getAxis(), m_Constant(&axis_attr))) {
    int64_t axis = (*axis_attr.begin()).getSExtValue();
    if (auto ty = dyn_cast<RankedTensorType>(op.getParams().getType())) {
      int64_t rank = ty.getRank();
      if (axis >= rank || axis < -rank)
...
LogicalResult InvertPermutationOp::verify() {
  ...
  auto x_type = cast<TensorType>(op.getX().getType());
...
OpFoldResult LeakyReluOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto arg = dyn_cast_or_null<FloatAttr>(operands[0])) {
    return calculate(arg);
  } else if (auto arg = dyn_cast_or_null<SplatElementsAttr>(operands[0])) {
    if (auto elementAttr = dyn_cast<FloatAttr>(arg.getSplatValue<Attribute>()))
...
OpFoldResult LogicalAndOp::fold(FoldAdaptor adaptor) {
  ...
  for (const auto& operand : operands) {
    auto splat_attr = dyn_cast_or_null<SplatElementsAttr>(operand);
...
LogicalResult MeanOp::FoldOperandsPermutation(ArrayRef<int64_t> permutation) {
  ...
  auto reductions_value = dyn_cast<DenseElementsAttr>(reduction_op.getValue());
