  ...
  for (func::FuncOp func : outlined_module.getOps<func::FuncOp>()) {
    func.walk([&](Operation *op) {
      for (NamedAttribute attr : op->getAttrs()) {
        if (auto symbol_ref = dyn_cast<FlatSymbolRefAttr>(attr.getValue())) {
          MoveFuncOp(symbol_ref, symbol_table, outlined_symbol_table);
          continue;
        }
        if (auto array_attr = dyn_cast<ArrayAttr>(attr.getValue())) {
          for (const Attribute &attribute : array_attr) {
            auto symbol_ref = dyn_cast<FlatSymbolRefAttr>(attribute);
