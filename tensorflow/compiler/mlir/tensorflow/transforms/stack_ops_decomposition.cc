...
int64_t FindAliasedInput(func::FuncOp func, int64_t return_index) {
  ...
  auto maybe_arg = dyn_cast<BlockArgument>(return_val);
...
LogicalResult HandlePartitionedCallOp(
  ...
  if (!info.signature_change) {
    ...
    for (auto& entry : callee_map) {
      info.stack_var_arg_to_size_arg[cast<BlockArgument>(entry.getFirst())
                                         .getArgNumber()] =
          cast<BlockArgument>(entry.getSecond()).getArgNumber();
    }
    if (lowered_callee != callee) {
...
LogicalResult HandleStackV2Op(
  ...
  auto var_type = RankedTensorType::get(
      {}, TF::ResourceType::get(
              ArrayRef<TensorType>{cast<TensorType>(buffer.getType())},
              stack.getContext()));
