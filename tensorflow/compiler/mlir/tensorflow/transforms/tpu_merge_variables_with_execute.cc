...
struct TPUMergeVariablesWithExecutePass
    : public TF::PerFunctionAggregateAnalysisConsumerPass<
          TPUMergeVariablesWithExecutePass, TF::ResourceAliasAnalysis> {
  ...
  void runOnFunction(
      FuncOp func,
      const TF::ResourceAliasAnalysis::Info& resource_analysis_info);
};
...
void TPUMergeVariablesWithExecutePass::runOnFunction(
  ...
  for (auto execute_launch : execute_launches) {
    ...
    if (failed(MergeForOneTPUExecute(execute_launch, resource_analysis_info,
                                     /*check_device=*/!parent_is_replicate,
                                     /*check_same_region=*/parent_is_replicate,
                                     &builder))) {
      signalPassFailure();
      return;
    }
