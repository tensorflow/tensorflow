...
TF::SumOp createSumOp(Value value, Location loc,
  ...
  auto value_type = cast<RankedTensorType>(value.getType());
...
LogicalResult rewriteToReduceSumAndTranspose(TF::EinsumOp op,
  ...
  for (int64_t i = 0; i < dnums.lhs_out.size(); ++i) {
    if (std::get<0>(dnums.lhs_out[i]) >
        cast<RankedTensorType>(lhs.getType()).getRank() - 1) {
      continue;
    }
...
LogicalResult reshapeForBatchMatmul(const Location& loc,
                                    ...
                                    PatternRewriter* rewriter) {
  RankedTensorType lhs_type = cast<RankedTensorType>(lhs->getType());
  RankedTensorType rhs_type = cast<RankedTensorType>(rhs->getType());
...
LogicalResult matchAndRewriteUnaryEinsumOp(TF::EinsumOp op,
  ...
  RankedTensorType lhs =
      dyn_cast_or_null<RankedTensorType>(op.getOperand(0).getType());
  if (!lhs) {
    return failure();
  }
...
LogicalResult ConvertTFEinsumOp::matchAndRewrite(
  ...
  RankedTensorType rhs =
      dyn_cast_or_null<RankedTensorType>(op.getOperand(1).getType());
  if (!lhs || !rhs) {
    return failure();
  }
