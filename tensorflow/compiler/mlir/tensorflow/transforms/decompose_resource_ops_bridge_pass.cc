...
#include "tensorflow/compiler/mlir/tensorflow/transforms/decompose_resource_ops_bridge.h"
...
#define GEN_PASS_DEF_DECOMPOSERESOURCEOPSBRIDGEPASS
#include "tensorflow/compiler/mlir/tensorflow/transforms/tf_device_passes.h.inc"

struct DecomposeResourceOpsBridgePass
    : public impl::DecomposeResourceOpsBridgePassBase<
          DecomposeResourceOpsBridgePass> {
  void runOnOperation() override {
    // Add lowering patterns to the list.
    RewritePatternSet patterns(&getContext());
    TF::PopulateDecomposeResourceOpsBridgePatterns(&getContext(), &patterns);
...
#define GEN_PASS_DEF_DECOMPOSERESOURCEOPSBRIDGEINCLUSTERPASS
#include "tensorflow/compiler/mlir/tensorflow/transforms/tf_device_passes.h.inc"

struct DecomposeResourceOpsBridgeInClusterPass
    : public impl::DecomposeResourceOpsBridgeInClusterPassBase<
          DecomposeResourceOpsBridgeInClusterPass> {
  void runOnOperation() override {
    // Add lowering patterns to the list.
    RewritePatternSet patterns(&getContext());
    TF::PopulateDecomposeResourceOpsBridgePatterns(&getContext(), &patterns);
...
CreateDecomposeResourceOpsBridgePass() {
  return std::make_unique<DecomposeResourceOpsBridgePass>();
}

std::unique_ptr<OperationPass<ModuleOp>>
CreateDecomposeResourceOpsBridgeInClusterPass() {
  return std::make_unique<DecomposeResourceOpsBridgeInClusterPass>();
}
