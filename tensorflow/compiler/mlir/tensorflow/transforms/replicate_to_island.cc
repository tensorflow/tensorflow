...
llvm::SmallVector<tf_executor::IslandOp, 8> ExpandReplicateIntoReplicas(
  ...
  llvm::SmallVector<ValuePtr, 8> replica_inputs(island_op.controlInputs());
...
void CreateIslandsFromReplicate(const Dialect* tf_dialect,
  ...
  llvm::SmallVector<ValuePtr, 8> replicas_outputs(replicate_op.getNumResults(),
                                                  nullptr);
  ...
  // island has no uses.
  llvm::SmallVector<ValuePtr, 8> island_operands;
