/* Copyright 2025 The TensorFlow Authors. All Rights Reserved.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
==============================================================================*/

#include <memory>
#include <utility>
#include <vector>

#include "llvm/ADT/SmallVector.h"
#include "llvm/Support/Casting.h"
#include "mlir/Dialect/Func/IR/FuncOps.h"  // from @llvm-project
#include "mlir/IR/Block.h"  // from @llvm-project
#include "mlir/IR/BuiltinOps.h"  // from @llvm-project
#include "mlir/IR/Operation.h"  // from @llvm-project
#include "mlir/IR/Value.h"  // from @llvm-project
#include "mlir/Pass/Pass.h"  // from @llvm-project
#include "tensorflow/compiler/mlir/tensorflow/analysis/side_effect_analysis.h"
#include "tensorflow/compiler/mlir/tensorflow/ir/tf_device.h"
#include "tensorflow/compiler/mlir/tensorflow/ir/tf_ops.h"
#include "tensorflow/compiler/mlir/tensorflow/transforms/passes.h"
#include "tensorflow/compiler/mlir/tensorflow/utils/topological_sort.h"

namespace mlir {
namespace TF {

namespace {

#define GEN_PASS_DEF_ORDERFORPROGRAMKEYPASS
#include "tensorflow/compiler/mlir/tensorflow/transforms/tf_passes.h.inc"

struct OrderForProgramKeyPass
    : public impl::OrderForProgramKeyPassBase<OrderForProgramKeyPass> {
  void runOnOperation() override;
};

int DialectOrdering(Operation* predecessor, Operation* op) {
  // Emit TPUCompileMlirOps first.
  if (auto launch = llvm::dyn_cast<tf_device::LaunchOp>(op)) {
    for (Operation* _ : launch.getOps<TF::_TPUCompileMlirOp>()) {
      return 1;
    }
  } else if (llvm::isa<TF::_TPUCompileMlirOp>(op)) {
    return 1;
  }
  return 0;
}

void OrderForProgramKeyPass::runOnOperation() {
  ModuleOp module = getOperation();
  for (func::FuncOp func : module.getOps<func::FuncOp>()) {
    std::vector<std::pair<Operation*, Operation*>> side_effect_data;
    const detail::SideEffectAnalysisInfo* info =
        &getAnalysis<mlir::TF::SideEffectAnalysis>().GetAnalysisForFunc(func);
    auto extra_dependencies =
        [&](Operation* op,
            bool incoming) -> llvm::SmallVector<Operation*, 4> const& {
      return incoming ? info->DirectControlPredecessors(op)
                      : info->DirectControlSuccessors(op);
    };
    func->walk([&](Operation* function) {
      for (Region& region : function->getRegions()) {
        for (Block& block : region.getBlocks()) {
          if (block.empty()) continue;
          auto ops = SortBlockTopologically(block, DialectOrdering,
                                            extra_dependencies);
          // Replace the block with the reordered block.
          for (Operation* op : ops) {
            op->remove();
            block.push_back(op);
          }
        }
      }
    });
  }
}
}  // namespace

std::unique_ptr<OperationPass<ModuleOp>> CreateOrderForProgramKeyPass() {
  return std::make_unique<OrderForProgramKeyPass>();
}

}  // namespace TF
}  // namespace mlir
