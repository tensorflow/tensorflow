...
//
static ValuePtr LowerCondition(Location loc, ValuePtr value,
                               OpBuilder* builder) {
...
static Operation* CallFn(Location loc,
                         const std::function<ValuePtr(int)>& get_arg, FuncOp fn,
                         OpBuilder* builder) {
...
static llvm::SmallVector<ValuePtr, 4> PrepareValsForJump(
    Location loc, const std::function<ValuePtr(int)>& get_val, Block* block,
    OpBuilder* builder) {
  llvm::SmallVector<ValuePtr, 4> result;
  int num_vals = block->getNumArguments();
  result.reserve(num_vals);
  for (int i = 0; i < num_vals; ++i) {
    ValuePtr val = get_val(i);
...
// Requires the function to provide values for each of the block arguments and
// they should be pair-wise compatible for tensor cast.
static void JumpToBlock(Location loc,
                        const std::function<ValuePtr(int)>& get_arg,
...
static void ReplaceOpResultWithBlockArgs(Location loc, Operation* op,
  ...
  for (unsigned i = 0, e = op->getNumResults(); i != e; ++i) {
    ValuePtr arg = block->getArgument(i);
...
static LogicalResult LowerIfOp(IfOp op) {
  ...
  ValuePtr cond_i1 = LowerCondition(loc, op.cond(), &builder);
  ...
  for (ValuePtr value : op_inst->getResults())
  ...
  builder.create<CondBranchOp>(loc, cond_i1, then_block,
                               llvm::ArrayRef<ValuePtr>(), else_block,
...
static LogicalResult LowerWhileOp(WhileOp op) {
  ...
  ValuePtr condition =
      LowerCondition(loc, cond_call_op->getResult(0), &builder);
  auto br_operands =
