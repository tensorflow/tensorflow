...
static Value GetDimensionSize(OpBuilder *builder, Location loc, Value input,
  ...
  if (auto ranked_ty = dyn_cast_or_null<RankedTensorType>(input.getType())) {
...
static DenseElementsAttr DequantizeHalfRange(OpBuilder *builder, Value input) {
  auto input_type = dyn_cast_or_null<ShapedType>(input.getType());
...
class LowerAddNOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (isa<VariantType>(getElementTypeOrSelf(addn_op.getType())))
      return failure();
...
class LowerDynamicStitchOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    RankedTensorType out_ty = dyn_cast<RankedTensorType>(op.getType());
    ...
    for (auto it : llvm::zip(op.getIndices(), op.getData())) {
      ...
      RankedTensorType data_ty = dyn_cast<RankedTensorType>(data.getType());
    ...
    for (auto it : llvm::zip(indices, op.getData())) {
      ...
      auto num_items =
          cast<RankedTensorType>(reshaped_data.getType()).getShape()[0];
...
class LowerInvertPermutationOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    Location loc = op.getLoc();
    auto x_type = dyn_cast<RankedTensorType>(op.getX().getType());
...
class LowerLgammaOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    Location loc = op.getLoc();
    Value input = op.getX();
    TensorType original_tensor_type = cast<TensorType>(op.getX().getType());
    ...
    FloatType float_type = cast<FloatType>(tensor_type.getElementType());
...
class LowerSpaceToBatchNDOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    Location loc = op.getLoc();
    auto input_type = cast<TensorType>(op.getInput().getType());
    auto element_type = input_type.getElementType();
    if (!input_type.hasStaticShape()) {
      return failure();
    }
    ArrayRef<int64_t> input_shape = input_type.getShape();
    auto block_shape_type = cast<TensorType>(op.getBlockShape().getType());
    if (!block_shape_type.hasStaticShape()) {
      return failure();
    }
    auto paddings_type = cast<ShapedType>(op.getPaddings().getType());
...
class LowerSparseMatMulOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    if (!cast<TensorType>(op.getProduct().getType()).getElementType().isF32()) {
      return failure();
    }
    MLIRContext *context = rewriter.getContext();
    llvm::SmallVector<Value, 2> operands{op.getA(), op.getB()};
    for (Value &operand : operands) {
      TensorType tensor_type = cast<TensorType>(operand.getType());
...
class LowerResizeNearestNeighbor : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    auto out_size_ty = cast<ShapedType>(out_size.getType());
...
struct LowerRollOp : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto input_ty = dyn_cast<RankedTensorType>(tf_roll_op.getInput().getType());
    ...
    auto shift_ranked_attr_type = dyn_cast<RankedTensorType>(shift.getType());
    ...
    auto axis_ranked_attr_type = dyn_cast<RankedTensorType>(axis.getType());
