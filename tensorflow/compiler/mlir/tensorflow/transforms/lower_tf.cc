...
class ConvertBatchToSpaceND : public OpRewritePattern<TF::BatchToSpaceNDOp> {
 public:
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(TF::BatchToSpaceNDOp op,
                                PatternRewriter &rewriter) const override {
    auto input = op.input();
    auto input_ty = input.getType().cast<ShapedType>();
    auto element_ty = input_ty.getElementType();
...
void PopulateLoweringTFPatterns(MLIRContext *context,
  ...
  patterns->insert<LowerAddNOp, ConvertBatchToSpaceND,
                   ConvertFakeQuantWithMinMaxVarsOp, LowerDynamicStitchOp,
