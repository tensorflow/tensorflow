...
LogicalResult ConvertTFBatchMatMulOp<BatchMatMulOpType>::matchAndRewrite(
  ...
  if (!isa<RankedTensorType>(input_lhs.getType())) {
    // LHS must be a ranked tensor type
    return failure();
  }
  if (!isa<RankedTensorType>(input_rhs.getType())) {
    // RHS must be a ranked tensor type
    return failure();
  }

  auto lhs_type = cast<RankedTensorType>(input_lhs.getType());
  auto rhs_type = cast<RankedTensorType>(input_rhs.getType());
