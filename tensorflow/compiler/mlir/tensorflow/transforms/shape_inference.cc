...
Type TypeMeet(Type lhs, Type rhs) {
  ...
  auto rhs_shape_type = dyn_cast<ShapedType>(rhs);
  if (!rhs_shape_type) return lhs;
  auto lhs_shape_type = cast<ShapedType>(lhs);
  ...
  auto rhs_element_type_with_subtype =
      dyn_cast<TF::TensorFlowTypeWithSubtype>(rhs_shape_type.getElementType());
  ...
  if (rhs_element_type_with_subtype &&
    ...
    auto lhs_element_type_with_subtype =
        dyn_cast<TF::TensorFlowTypeWithSubtype>(lhs_element_type);
    TensorType subtype;
    if (!lhs_element_type_with_subtype) {
      ...
      auto refined_subtype = cast<TensorType>(
...
RankedTensorType DropFirstDimension(Type type) {
  RankedTensorType ranked_type = dyn_cast<RankedTensorType>(type);
  ...
  while (!worklist.empty()) {
    ...
    for (auto& use : tensorlist.getUses()) {
      if (auto push = llvm::dyn_cast<TensorListPushBackOp>(use.getOwner())) {
        auto element_type =
            dyn_cast<RankedTensorType>(push.getTensor().getType());
      ...
      if (auto set_item = llvm::dyn_cast<TensorListSetItemOp>(use.getOwner())) {
        auto element_type =
            dyn_cast<RankedTensorType>(set_item.getItem().getType());
...
Type GetType(Attribute shape_attr, Attribute type_attr) {
  auto shape = cast<tf_type::ShapeAttr>(shape_attr);
  auto type = cast<TypeAttr>(type_attr);
  if (shape.hasRank())
...
bool CanBeRefined(Type type) {
  auto shape_type = dyn_cast<ShapedType>(type);
  ...
  auto type_with_subtype =
      dyn_cast<TF::TensorFlowTypeWithSubtype>(shape_type.getElementType());
...
Type GetNewArgType(Type old_arg_type, ArrayRef<int64_t> shape,
  ...
  if (auto input_ty = dyn_cast<RankedTensorType>(old_arg_type)) {
...
struct ValuePort {
  ...
  explicit ValuePort(Value v) {
    OpResult opr = dyn_cast<OpResult>(v);
    if (opr) {
      producer = opr.getOwner();
      port = {opr.getResultNumber()};
    } else {
      producer = cast<BlockArgument>(v);
      port = {0};
    }
  }
...
ValuePort ComputeInputComponentFor(PackOp op, ArrayRef<unsigned int> port) {
  auto type = cast<TensorType>(op.getType());
...
ValuePort ComputeInputComponentFor(ConcatV2Op op, ArrayRef<unsigned int> port) {
  ...
  for (Value val : op.getValues()) {
    auto val_ty = cast<TensorType>(val.getType());
...
Attribute ComputeOutputComponent(const ValuePort& value_port,
  ...
  if (auto shape_op = dyn_cast<TF::ShapeOp>(op)) {
    ...
    auto operand_ty =
        dyn_cast<RankedTensorType>(shape_op.getOperand().getType());
...
bool ShapeInference::InferShapeForCast(Operation* op) {
  ...
  if (!new_type) {
    ...
    auto ranked_operand_type = dyn_cast<RankedTensorType>(operand_type);
    if (!ranked_operand_type) return false;
    auto ranked_res_type = dyn_cast<RankedTensorType>(result.getType());
    if (ranked_res_type &&
        ranked_operand_type.getShape() == ranked_res_type.getShape())
      return false;

    auto shaped_res_type = dyn_cast<ShapedType>(result_type);
...
bool ShapeInference::InferShapeForXlaCallModule(XlaCallModuleOp op) {
  ...
  for (auto output_type : main_output_types) {
    ...
    auto output_type_ranked = dyn_cast<RankedTensorType>(output_type);
...
bool ShapeInference::InferShapeForRestore(Operation* op) {
  ...
  while (!worklist.empty()) {
    ...
    auto subtypes =
        cast<TF::ResourceType>(getElementTypeOrSelf(assign_op.getResource()))
            .getSubtypes();
    if (subtypes.empty()) {
      continue;
    }
    auto subtype = dyn_cast<ShapedType>(subtypes.front());
    if (subtype == nullptr) {
      continue;
    }
...
bool ShapeInference::InferShapeForTensorListPopBackOp(TensorListPopBackOp op) {
  ...
  auto src_list_handle_t =
      dyn_cast_or_null<TensorType>(op.getOperand(0).getType());
  ...
  auto variant_element_t =
      dyn_cast_or_null<VariantType>(dst_list_handle_t.getElementType());
...
bool ShapeInference::InferShapeForVarHandleOp(VarHandleOp op) {
  ...
  for (auto& use : make_early_inc_range(resource.getUses())) {
    ...
    TensorType resource_subtype = cast<TensorType>(value.getType());
...
bool ShapeInference::InferShapeForXlaReduceWindowOp(XlaReduceWindowOp op) {
  ...
  if (input_ty.hasStaticShape() &&
    ...
    auto output_shape = InferWindowOutputShape(
        ...
        cast<ShapedType>(op.getInitValue().getType()).getElementType());
...
bool ShapeInference::InferShapeForXlaSelectAndScatterOp(
  ...
  auto operand_shape = cast<ShapedType>(op.getOperand().getType());
  auto source_shape = cast<ShapedType>(op.getSource().getType());
...
LogicalResult PrecheckForXlaConvV2Op(XlaConvV2Op op) {
  ...
  auto input_args_have_static_shape = [&]() -> bool {
    return cast<TensorType>(input_tensor.getType()).hasStaticShape() &&
           cast<TensorType>(kernel_tensor.getType()).hasStaticShape() &&
           cast<TensorType>(window_strides.getType()).hasStaticShape() &&
           cast<TensorType>(padding.getType()).hasStaticShape() &&
           cast<TensorType>(lhs_dilation.getType()).hasStaticShape() &&
           cast<TensorType>(rhs_dilation.getType()).hasStaticShape() &&
           cast<TensorType>(feature_group_count.getType()).hasStaticShape();
  };
  ...
  auto input_tensor_shape =
      cast<RankedTensorType>(input_tensor.getType()).getShape();
  auto kernel_tensor_shape =
      cast<RankedTensorType>(kernel_tensor.getType()).getShape();
...
bool ShapeInference::InferShapeForXlaConvV2Op(XlaConvV2Op op) {
  ...
  if (matchPattern(window_strides, m_Constant(&window_strides_attr)) &&
    ...
    auto input_tensor_shape = cast<RankedTensorType>(input_tensor.getType());
    ...
    auto kernel_tensor_shape = cast<RankedTensorType>(kernel_tensor.getType());
  ...
  for (unsigned int i = 0, e = dims.size(); i != e; ++i) {
    ...
    while (!worklist.empty()) {
      ...
      if (worklist.empty()) {
        ...
        if (auto dea = dyn_cast<DenseIntElementsAttr>(ret)) {
...
bool ShapeInference::RefineTypeForPassThroughOperands(Operation* op,
  ...
  for (auto entry : llvm::zip(operands, results)) {
    Type operand_type = std::get<0>(entry).getType();
    Value result = std::get<1>(entry);
    TensorType result_type = cast<TensorType>(result.getType());
...
Type GetElementTypeFromOperand(TensorType operand_type,
  ...
  auto operand_handle_type =
      dyn_cast<TensorFlowTypeWithSubtype>(operand_type.getElementType());
  if (!operand_handle_type) return result_type.getElementType();
  auto result_handle_type =
      cast<TensorFlowTypeWithSubtype>(result_type.getElementType());
...
bool ShapeInference::InferShapeForWhile(WhileOpTy op,
  ...
  for (auto entry :
    ...
    Value result = std::get<1>(entry);
    TensorType body_result_type = cast<TensorType>(std::get<2>(entry));
    auto result_type = cast<TensorType>(result.getType());
    ...
    if (CanWhileTypeBeRefinedWith(result_type, body_result_type)) {
      ...
      TensorType operand_type = cast<TensorType>(std::get<0>(entry));
...
bool ShapeInference::InferShapeForSingleOperation(Operation* op,
  ...
  auto result_element_type_fn = [&](int index) {
    return cast<TensorType>(op->getResult(index).getType()).getElementType();
  };
  ...
  for (auto result : llvm::zip(op->getResults(), inferred_return_shapes)) {
    ...
    inferred_type =
        cast<TensorType>(TypeMeet(op_result.getType(), inferred_type));
...
llvm::SmallVector<Type, 4> GetWhileCompatibleTypes(
  ...
  for (auto entry :
    ...
    auto operand_type = cast<TensorType>(std::get<0>(entry));
    auto result_type = cast<TensorType>(std::get<1>(entry));
    if (operand_type == result_type) {
      types.push_back(operand_type);
    } else if (RankedAndSameRank(operand_type, result_type)) {
      auto potential_refined_type =
          GetCompatibleRankedTensorType(cast<RankedTensorType>(operand_type),
                                        cast<RankedTensorType>(result_type));
      types.push_back(potential_refined_type);
    } else {
      auto region_argument_type = cast<TensorType>(std::get<2>(entry));
      Type element_type = GetElementTypeFromOperand(
          cast<TensorType>(operand_type), region_argument_type);
...
LogicalResult ShapeInference::TryToFold(Operation* op) {
  ...
  for (auto result : zip(op->getResults(), fold_results)) {
    ...
    if (ElementsAttr eattr = dyn_cast_or_null<ElementsAttr>(attr)) {
...
FailureOr<bool> InferShapeForFunction(func::FuncOp func,
  ...
  for (size_t i = 0; i < func_type.getNumInputs(); ++i) {
    ...
    Type element_type;
    if (auto input_ty = dyn_cast<RankedTensorType>(func_type.getInput(i))) {
      if (input_ty.getRank() != shape.size()) {
        return failure();
      }
      element_type = input_ty.getElementType();
    } else {
      auto unranked_input_ty = dyn_cast<TensorType>(func_type.getInput(i));
