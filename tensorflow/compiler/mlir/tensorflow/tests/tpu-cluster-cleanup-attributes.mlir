// RUN: tf-opt %s -split-input-file -verify-diagnostics -tf-tpu-cleanup-cluster-attributes | FileCheck %s
...
func @cleanup_attributes_failure(%arg0: tensor<i1>, %arg1: tensor<f32>, %arg2: tensor<f32>) ->  tensor<f32> {
  %1 = "tf_device.cluster"() ({
    // expected-error @+1 {{'tf_device.launch' op is expected to have both _replication_info and _xla_compile_device_type attributes}}
    %2 = "tf_device.launch"() ({
      %3 = "tf.Add"(%arg1, %arg2) : (tensor<f32>, tensor<f32>) -> tensor<f32>
      tf_device.return %3 : tensor<f32>
    }) {_replication_info = "x", device = "y"} : () -> tensor<f32>
    tf_device.return %2 : tensor<f32>
  }) {cluster_attr = "cluster_attr", _xla_compile_device_type = "TPU", _replication_info = "x", device = "y"} : () -> tensor<f32>
