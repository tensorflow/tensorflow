...
// CHECK: %outputs_2, %control_3 = tf_executor.island wraps "tf.Const"() <{value = dense<0.000000e+00> : tensor<f32>}> : () -> tensor<f32>
// CHECK: %outputs_4, %control_5 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> : () -> tensor<2xi64>
// CHECK: %outputs_6, %control_7 = tf_executor.island wraps "tf.Fill"(%outputs_4, %outputs_2) : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
// CHECK: %outputs_8, %control_9 = tf_executor.island wraps "tf.Const"() <{value = dense<0.000000e+00> : tensor<f32>}> : () -> tensor<f32>
// CHECK: %outputs_10, %control_11 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> : () -> tensor<2xi64>
// CHECK: %outputs_12, %control_13 = tf_executor.island wraps "tf.Fill"(%outputs_10, %outputs_8) : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
// CHECK: %outputs_14, %control_15 = tf_executor.island wraps "tf.Const"() <{value = dense<0.000000e+00> : tensor<f32>}> : () -> tensor<f32>
// CHECK: %outputs_16, %control_17 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> : () -> tensor<2xi64>
// CHECK: %outputs_18, %control_19 = tf_executor.island wraps "tf.Fill"(%outputs_16, %outputs_14) : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
// CHECK: %outputs_20, %control_21 = tf_executor.island wraps "tf.Const"() <{value = dense<0.000000e+00> : tensor<f32>}> : () -> tensor<f32>
// CHECK: %outputs_22, %control_23 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> : () -> tensor<2xi64>
// CHECK: %outputs_24, %control_25 = tf_executor.island wraps "tf.Fill"(%outputs_22, %outputs_20) : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
// CHECK: %outputs_26, %control_27 = tf_executor.island wraps "tf.Const"() <{value = dense<0.000000e+00> : tensor<f32>}> : () -> tensor<f32>
// CHECK: %outputs_28, %control_29 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> : () -> tensor<2xi64>
// CHECK: %outputs_30, %control_31 = tf_executor.island wraps "tf.Fill"(%outputs_28, %outputs_26) : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
// CHECK: %outputs_32, %control_33 = tf_executor.island wraps "tf.Const"() <{value = dense<0.000000e+00> : tensor<f32>}> : () -> tensor<f32>
// CHECK: %outputs_34, %control_35 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> : () -> tensor<2xi64>
// CHECK: %outputs_36, %control_37 = tf_executor.island wraps "tf.Fill"(%outputs_34, %outputs_32) : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
// CHECK: %outputs_38:2, %control_39 = tf_executor.island wraps "tf._TPUCompileMlir"()
// CHECK: %control_42 = tf_executor.island wraps "tf.TPUCompileSucceededAssert"(%outputs_40) {device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<!tf_type.string>) -> ()
// CHECK: %outputs_43, %control_44 = tf_executor.island wraps "tf.Identity"(%outputs) {_parallel_execution_ids = "r0:0", device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK: %outputs_45, %control_46 = tf_executor.island wraps "tf.Identity"(%outputs_0) {_parallel_execution_ids = "r0:0", device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK: %outputs_47:2, %control_48 = tf_executor.island wraps "tf.TPUExecute"(%outputs_43, %outputs_45, %outputs_38#1) {_parallel_execution_ids = "r0:0", device = "/job:tpu_host_worker/replica:0/task:0/device:TPU:0"} : (tensor<25x5xf32>, tensor<25x5xf32>, tensor<3x!tf_type.string>) -> (tensor<128xf32>, tensor<128xf32>)
// CHECK: %outputs_49, %control_50 = tf_executor.island wraps "tf.Identity"(%outputs_6) {_parallel_execution_ids = "r0:1", device = "/job:tpu_host_worker/replica:0/task:1/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK: %outputs_51, %control_52 = tf_executor.island wraps "tf.Identity"(%outputs_24) {_parallel_execution_ids = "r0:1", device = "/job:tpu_host_worker/replica:0/task:1/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK: %outputs_53:2, %control_54 = tf_executor.island wraps "tf.TPUExecute"(%outputs_49, %outputs_51, %outputs_38#1) {_parallel_execution_ids = "r0:1", device = "/job:tpu_host_worker/replica:0/task:1/device:TPU:0"} : (tensor<25x5xf32>, tensor<25x5xf32>, tensor<3x!tf_type.string>) -> (tensor<128xf32>, tensor<128xf32>)
// CHECK: %outputs_55, %control_56 = tf_executor.island wraps "tf.Identity"(%outputs_12) {_parallel_execution_ids = "r0:2", device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK: %outputs_57, %control_58 = tf_executor.island wraps "tf.Identity"(%outputs_30) {_parallel_execution_ids = "r0:2", device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK: %outputs_59:2, %control_60 = tf_executor.island wraps "tf.TPUExecute"(%outputs_55, %outputs_57, %outputs_38#1) {_parallel_execution_ids = "r0:2", device = "/job:tpu_host_worker/replica:0/task:0/device:TPU:1"} : (tensor<25x5xf32>, tensor<25x5xf32>, tensor<3x!tf_type.string>) -> (tensor<128xf32>, tensor<128xf32>)
// CHECK: %outputs_61, %control_62 = tf_executor.island wraps "tf.Identity"(%outputs_18) {_parallel_execution_ids = "r0:3", device = "/job:tpu_host_worker/replica:0/task:1/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK: %outputs_63, %control_64 = tf_executor.island wraps "tf.Identity"(%outputs_36) {_parallel_execution_ids = "r0:3", device = "/job:tpu_host_worker/replica:0/task:1/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK: %outputs_65:2, %control_66 = tf_executor.island wraps "tf.TPUExecute"(%outputs_61, %outputs_63, %outputs_38#1) {_parallel_execution_ids = "r0:3", device = "/job:tpu_host_worker/replica:0/task:1/device:TPU:1"} : (tensor<25x5xf32>, tensor<25x5xf32>, tensor<3x!tf_type.string>) -> (tensor<128xf32>, tensor<128xf32>)
// CHECK: %outputs_67, %control_68 = tf_executor.island wraps "tf.Identity"(%outputs_53#0) {device = "/job:tpu_host_worker/replica:0/task:1/device:TPU:0"} : (tensor<128xf32>) -> tensor<128xf32>
// CHECK: %outputs_69, %control_70 = tf_executor.island wraps "tf.Identity"(%outputs_65#1) {device = "/job:tpu_host_worker/replica:0/task:1/device:TPU:1"} : (tensor<128xf32>) -> tensor<128xf32>
// CHECK: tf_executor.fetch %outputs_67, %outputs_69, %control, %control_1, %control_48, %control_54, %control_60, %control_66 : tensor<128xf32>, tensor<128xf32>, !tf_executor.control, !tf_executor.control, !tf_executor.control, !tf_executor.control, !tf_executor.control, !tf_executor.control
