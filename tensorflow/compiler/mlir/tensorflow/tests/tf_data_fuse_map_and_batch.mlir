...
func @fuse_map_and_batch() -> tensor<!tf_type.variant> attributes {tf.entry_function = {control_outputs = "", inputs = "", outputs = "BatchDatasetV2"}} {
  ...
  %3 = "tf.TensorSliceDataset"(%2) {device = "", output_shapes = [#tf_type.shape<>]} : (tensor<3xi32>) -> tensor<*x!tf_type.variant>
  ...
  %4 = "tf.MapDataset"(%3) {device = "",
           ...
           use_inter_op_parallelism = true} : (tensor<*x!tf_type.variant>) -> tensor<!tf_type.variant>
  %5 = "tf.BatchDatasetV2"(%4, %0, %1) {device = "", output_shapes = [#tf_type.shape<>], output_types = [i32], parallel_copy = false} : (tensor<!tf_type.variant>, tensor<i64>, tensor<i1>) -> tensor<!tf_type.variant>
