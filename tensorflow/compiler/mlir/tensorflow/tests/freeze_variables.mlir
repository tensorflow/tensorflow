...
module {
  ...
  func @main() -> tensor<0xf32> {
    %handle = "tf.VarHandleOp"() {container="", shared_name="var1", device = "/job:worker/replica:0/task:1/device:CPU:0"} : () -> tensor<!tf.resource<tensor<0xf32>>>
    %val = "tf.ReadVariableOp"(%handle) : (tensor<!tf.resource<tensor<0xf32>>>) -> tensor<0xf32>
...
module {
  func @f() {
    ...
    %handle = "tf.VarHandleOp"() {container="", shared_name="var1", device = "/job:worker/replica:0/task:1/device:CPU:0"} : () -> tensor<!tf.resource<tensor<0xf32>>>
    "tf.AssignVariableOp"(%handle, %cst) : (tensor<!tf.resource<tensor<0xf32>>>, tensor<0xf32>) -> ()
...
module {
  func @f() {
    ...
    "tf.AssignVariableOp"(%handle, %0) : (tensor<!tf.resource<tensor<0xf32>>>, tensor<0xf32>) -> ()
    // CHECK: "tf.VarHandleOp"
    return
  }
}
...
module {
  func @f() -> tensor<0xf32> {
    // CHECK: "tf.VarHandleOp"
    %handle = "tf.VarHandleOp"() {container="", shared_name="var1", device = "/job:worker/replica:0/task:1/device:CPU:0"} : () -> tensor<!tf.resource<tensor<0xf32>>>
    %val = "tf.PartitionedCall"(%handle) {config = "", config_proto = "", executor_type = "", f = @f_callee} : (tensor<!tf.resource<tensor<0xf32>>>) -> (tensor<0xf32>)
    return %val : tensor<0xf32>
  }

  // CHECK: func private @f_callee(%arg0: tensor<*x!tf.resource>) -> tensor<0xf32>
  func private @f_callee(%arg0: tensor<*x!tf.resource>) -> tensor<0xf32> {
    %val = "tf.PartitionedCall"(%arg0) {config = "", config_proto = "", executor_type = "", f = @f_callee_callee} : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
    return %val : tensor<0xf32>
  }

  // CHECK: func private @f_callee_callee(%arg0: tensor<*x!tf.resource>) -> tensor<0xf32>
  func private @f_callee_callee(%arg0: tensor<*x!tf.resource>) -> tensor<0xf32> {
    %c0 = "tf.Const"() { value = dense<1.0> : tensor<0xf32> } : () -> tensor<0xf32>
    "tf.AssignVariableOp"(%arg0, %c0) : (tensor<*x!tf.resource>, tensor<0xf32>) -> ()
...
module {
  ...
  func private @f_callee_callee(%arg0: tensor<*x!tf.resource>) -> tensor<0xf32> {
    %val = "tf.ReadVariableOp"(%arg0) : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
    return %val : tensor<0xf32>
  }
}
...
module {
  ...
  func private @f_callee_callee(%arg0: tensor<*x!tf.resource>) -> tensor<0xf32> {
    %val = "tf.ReadVariableOp"(%arg0) : (tensor<*x!tf.resource>) -> tensor<0xf32>
    %0 = "tf.AddV2"(%val, %val) : (tensor<0xf32>, tensor<0xf32>) -> tensor<0xf32>
    "tf.AssignVariableOp"(%arg0, %0) : (tensor<*x!tf.resource>, tensor<0xf32>) -> ()
...
module {
  ...
  func private @testIfThen(%arg0:tensor<*xf32>, %arg1:tensor<*x!tf.resource>) -> tensor<*xf32> {
    %val = "tf.ReadVariableOp"(%arg1) : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
    %0 = "tf.AddV2"(%val, %arg0) : (tensor<0xf32>, tensor<*xf32>) -> tensor<*xf32>
    return %0 : tensor<*xf32>
  }

  // CHECK: func private @testIfElse(%arg0: tensor<*xf32>) -> tensor<*xf32>
  func private @testIfElse(%arg0:tensor<*xf32>, %arg1:tensor<*x!tf.resource>) -> tensor<*xf32> {
    %val = "tf.ReadVariableOp"(%arg1) : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
  ...
  func @f(%arg0: tensor<i1>, %arg1: tensor<2xf32>) -> tensor<2xf32> {
    ...
    } : (tensor<i1>, tensor<2xf32>, tensor<!tf.resource<tensor<0xf32>>>) -> tensor<2xf32>
...
module {
  // CHECK: @testIfThen(%arg0: tensor<*xf32>, %arg1: tensor<*x!tf.resource>)
  func private @testIfThen(%arg0:tensor<*xf32>, %arg1:tensor<*x!tf.resource>) -> tensor<*xf32> {
    %val = "tf.ReadVariableOp"(%arg1) : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
    %0 = "tf.AddV2"(%val, %arg0) : (tensor<0xf32>, tensor<*xf32>) -> tensor<*xf32>
    "tf.AssignVariableOp"(%arg1, %0) : (tensor<*x!tf.resource>, tensor<*xf32>) -> ()
    return %0 : tensor<*xf32>
  }

  // CHECK: @testIfElse(%arg0: tensor<*xf32>, %arg1: tensor<*x!tf.resource>)
  func private @testIfElse(%arg0:tensor<*xf32>, %arg1:tensor<*x!tf.resource>) -> tensor<*xf32> {
    %val = "tf.ReadVariableOp"(%arg1) : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
    %0 = "tf.Mul"(%val, %arg0) : (tensor<0xf32>, tensor<*xf32>) -> tensor<*xf32>
    return %0 : tensor<*xf32>
...
module {
  ...
  func private @cond(%arg0:tensor<0xf32>, %arg1:tensor<*x!tf.resource>) -> tensor<i1> {
    %val = "tf.ReadVariableOp"(%arg1) : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
  ...
  func private @body(%arg0:tensor<0xf32>, %arg1:tensor<*x!tf.resource>) -> (tensor<0xf32>, tensor<*x!tf.resource>) {
    %val = "tf.ReadVariableOp"(%arg1) : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
    %0 = "tf.Mul"(%val, %arg0) : (tensor<0xf32>, tensor<0xf32>) -> tensor<0xf32>
    return %0, %arg1 : tensor<0xf32>, tensor<*x!tf.resource>
  }
  ...
  func @f(%arg0: tensor<0xf32>) -> tensor<0xf32> {
    // CHECK-NOT: "tf.VarHandleOp"
    %handle = "tf.VarHandleOp"() {container="", shared_name="var1", device = "/job:worker/replica:0/task:1/device:CPU:0"} : () -> tensor<!tf.resource<tensor<0xf32>>>
    %0, %1 = "tf.While"(%arg0, %handle) {cond = @cond, body = @body, is_stateless = false} : (tensor<0xf32>, tensor<!tf.resource<tensor<0xf32>>>) -> (tensor<0xf32>, tensor<!tf.resource<tensor<*xf32>>>)
    return %0 : tensor<0xf32>
  }
}
...
module {
  ...
  // CHECK: func private @f_callee_callee() -> tensor<0xf32>
  func private @f_callee_callee(%arg0: tensor<*x!tf.resource>) -> tensor<0xf32> {
    %0 = "tf.PartitionedCall"(%arg0) {config = "", config_proto = "", executor_type = "", f = @f_callee_callee} : (tensor<*x!tf.resource>) -> (tensor<0xf32>)
    return %0 : tensor<0xf32>
  }
}
...
module {
  func @f(%arg0: tensor<i1>) -> tensor<0xf32> {
    ...
    %handle = "tf.VarHandleOp"() {container="", shared_name="var1", device = "/job:worker/replica:0/task:1/device:CPU:0"} : () -> tensor<!tf.resource<tensor<0xf32>>>
    %0 = "tf.IfRegion"(%arg0) ( {
      %1 = "tf.ReadVariableOp"(%handle) : (tensor<!tf.resource<tensor<0xf32>>>) -> (tensor<0xf32>)
  ...
  func @f(%arg0: tensor<i32>) -> tensor<0xf32> {
    ...
    %1:2 = "tf.WhileRegion"(%arg0, %0) ( {
      ...
      ^bb0(%barg0: tensor<i32>, %barg1: tensor<0xf32>):
        %2 = "tf.ReadVariableOp"(%handle) : (tensor<!tf.resource<tensor<0xf32>>>) -> (tensor<0xf32>)
...
module {
  func @f(%arg0: tensor<i32>) -> tensor<0xf32> {
    ...
    %1:3 = "tf.WhileRegion"(%arg0, %0, %handle) ( {
      ...
      ^bb0(%carg0: tensor<i32>, %carg1: tensor<0xf32>, %carg2: tensor<!tf.resource<tensor<0xf32>>>):
         %limit = constant dense<5> : tensor<i32>
         %cond = "tf.NotEqual"(%carg0, %limit) : (tensor<i32>, tensor<i32>) -> tensor<i1>
         "tf.Yield"(%cond) : (tensor<i1>) -> ()
    },  {
      // CHECK: ^bb0(%arg1: tensor<i32>, %arg2: tensor<0xf32>)
      ^bb0(%barg0: tensor<i32>, %barg1: tensor<0xf32>, %barg2: tensor<!tf.resource<tensor<0xf32>>>):
        %2 = "tf.ReadVariableOp"(%barg2) : (tensor<!tf.resource<tensor<0xf32>>>) -> (tensor<0xf32>)
        "tf.Yield"(%barg0, %2, %barg2) : (tensor<i32>, tensor<0xf32>, tensor<!tf.resource<tensor<0xf32>>>) -> ()
    }) {is_stateless = true} : (tensor<i32>, tensor<0xf32>, tensor<!tf.resource<tensor<0xf32>>>) -> (tensor<i32>, tensor<0xf32>, tensor<!tf.resource<tensor<0xf32>>>)
    return %1#1 : tensor<0xf32>
  }
}
...
module {
  ...
  func @f2() -> tensor<0xf32> {
    %handle = "tf.VarHandleOp"() {container="", shared_name="var1", device = "/job:worker/replica:0/task:1/device:CPU:0"} : () -> tensor<!tf.resource<tensor<0xf32>>>
    %0 = "tf.ReadVariableOp"(%handle) : (tensor<!tf.resource<tensor<0xf32>>>) -> (tensor<0xf32>)
    // CHECK: "tf.VarHandleOp"
    return %0 : tensor<0xf32>
  }
}
