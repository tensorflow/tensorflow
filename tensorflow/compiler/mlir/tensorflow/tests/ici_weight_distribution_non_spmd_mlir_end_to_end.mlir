...
// CHECK %outputs_4, %control_5 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> {_ici_weight_distribution_mlir_bridge_marker = true} : () -> tensor<2xi64>
// CHECK %outputs_6, %control_7 = tf_executor.island wraps "tf.Fill"(%outputs_4, %outputs_2) {_ici_weight_distribution_mlir_bridge_marker = true} : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
...
// CHECK %outputs_10, %control_11 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> {_ici_weight_distribution_mlir_bridge_marker = true} : () -> tensor<2xi64>
// CHECK %outputs_12, %control_13 = tf_executor.island wraps "tf.Fill"(%outputs_10, %outputs_8) {_ici_weight_distribution_mlir_bridge_marker = true} : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
// CHECK %outputs_14, %control_15 = tf_executor.island wraps "tf.Const"() <{value = dense<0.000000e+00> : tensor<f32>}> {_ici_weight_distribution_mlir_bridge_marker = true} : () -> tensor<f32>
// CHECK %outputs_16, %control_17 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> {_ici_weight_distribution_mlir_bridge_marker = true} : () -> tensor<2xi64>
// CHECK %outputs_18, %control_19 = tf_executor.island wraps "tf.Fill"(%outputs_16, %outputs_14) {_ici_weight_distribution_mlir_bridge_marker = true} : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
// CHECK %outputs_20, %control_21 = tf_executor.island wraps "tf.Const"() <{value = dense<0.000000e+00> : tensor<f32>}> {_ici_weight_distribution_mlir_bridge_marker = true} : () -> tensor<f32>
// CHECK %outputs_22, %control_23 = tf_executor.island wraps "tf.Const"() <{value = dense<[25, 5]> : tensor<2xi64>}> {_ici_weight_distribution_mlir_bridge_marker = true} : () -> tensor<2xi64>
// CHECK %outputs_24, %control_25 = tf_executor.island wraps "tf.Fill"(%outputs_22, %outputs_20) {_ici_weight_distribution_mlir_bridge_marker = true} : (tensor<2xi64>, tensor<f32>) -> tensor<25x5xf32>
...
// CHECK %outputs_31, %control_32 = tf_executor.island wraps "tf.Identity"(%outputs) {_ici_weight_distribution_mlir_bridge_marker = true, _parallel_execution_ids = "r0:0", device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK %outputs_33, %control_34 = tf_executor.island wraps "tf.Identity"(%outputs_0) {_ici_weight_distribution_mlir_bridge_marker = true, _parallel_execution_ids = "r0:0", device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
...
// CHECK %outputs_37, %control_38 = tf_executor.island wraps "tf.Identity"(%outputs_6) {_ici_weight_distribution_mlir_bridge_marker = true, _parallel_execution_ids = "r0:1", device = "/job:tpu_host_worker/replica:0/task:1/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK %outputs_39, %control_40 = tf_executor.island wraps "tf.Identity"(%outputs_18) {_ici_weight_distribution_mlir_bridge_marker = true, _parallel_execution_ids = "r0:1", device = "/job:tpu_host_worker/replica:0/task:1/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
...
// CHECK %outputs_43, %control_44 = tf_executor.island wraps "tf.Identity"(%outputs_12) {_ici_weight_distribution_mlir_bridge_marker = true, _parallel_execution_ids = "r0:2", device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK %outputs_45, %control_46 = tf_executor.island wraps "tf.Identity"(%outputs_24) {_ici_weight_distribution_mlir_bridge_marker = true, _parallel_execution_ids = "r0:2", device = "/job:tpu_host_worker/replica:0/task:0/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
...
// CHECK %outputs_49, %control_50 = tf_executor.island wraps "tf.Identity"(%outputs_6) {_ici_weight_distribution_mlir_bridge_marker = true, _parallel_execution_ids = "r0:3", device = "/job:tpu_host_worker/replica:0/task:1/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
// CHECK %outputs_51, %control_52 = tf_executor.island wraps "tf.Identity"(%outputs_18) {_ici_weight_distribution_mlir_bridge_marker = true, _parallel_execution_ids = "r0:3", device = "/job:tpu_host_worker/replica:0/task:1/device:CPU:0"} : (tensor<25x5xf32>) -> tensor<25x5xf32>
