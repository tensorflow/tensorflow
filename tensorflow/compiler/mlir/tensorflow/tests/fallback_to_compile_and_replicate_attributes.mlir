// RUN: tf-opt %s -split-input-file -verify-diagnostics -tf-fallback-to-legacy-compile-and-replicate-attributes | FileCheck %s

// CHECK-LABEL: func @fallback_to_legacy_attribute
func @fallback_to_legacy_attribute(%arg0: tensor<*xf32>, %arg1: tensor<f32>, %arg2: tensor<f32>) -> tensor<*xf32> attributes {tf._disable_call_shape_inference = true, tf.signature.is_stateful} {
...
func @fallback_to_legacy_attributes_failurre(%arg0: tensor<*xf32>, %arg1: tensor<f32>, %arg2: tensor<f32>) -> tensor<*xf32> attributes {tf._disable_call_shape_inference = true, tf.signature.is_stateful} {
  %0 = tf_executor.graph {
    %outputs, %control = tf_executor.island wraps "tf.GuaranteeConst"(%arg1) {T = f32, device = ""} : (tensor<f32>) -> tensor<f32>
    %outputs_0, %control_1 = tf_executor.island wraps "tf.GuaranteeConst"(%arg2) {T = f32, device = ""} : (tensor<f32>) -> tensor<f32>
    %control_2 = tf_executor.island wraps "tf.NoOp"() {_pivot_for_cluster = "cluster", device = ""} : () -> ()
    ...
    %outputs_11, %control_12 = tf_executor.island wraps "tf.Mul"(%outputs, %outputs_9) {_replication_info = "cluster", _xla_compile_device_type = "TPU", device = ""} : (tensor<f32>, tensor<*xf32>) -> tensor<*xf32>
    %outputs_13, %control_14 = tf_executor.island wraps "tf.AddV2"(%outputs_11, %outputs_0) {_replication_info = "cluster", _xla_compile_device_type = "TPU", device = ""} : (tensor<*xf32>, tensor<f32>) -> tensor<*xf32>
    // expected-error @+1 {{'tf.Identity' op is expected to have both _replication_info and _xla_compile_device_type attributes.}}
