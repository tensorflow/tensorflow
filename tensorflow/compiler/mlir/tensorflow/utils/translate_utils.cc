...
mlir::LogicalResult ExtractTfVersions(mlir::ModuleOp module,
  ...
  auto producer =
      dyn_cast_or_null<mlir::IntegerAttr>(version_attr.get("producer"));
  if (!producer) return mlir::failure();
  versions->set_producer(producer.getInt());

  auto min_consumer =
      dyn_cast_or_null<mlir::IntegerAttr>(version_attr.get("min_consumer"));
  if (min_consumer) versions->set_min_consumer(min_consumer.getInt());

  auto bad_consumers =
      dyn_cast_or_null<mlir::ArrayAttr>(version_attr.get("bad_consumers"));
  if (!bad_consumers) return mlir::success();

  for (auto bad_consumer : bad_consumers) {
    auto bad_consumer_int_attr =
        dyn_cast_or_null<mlir::IntegerAttr>(bad_consumer);
...
::tsl::StatusOr<int64_t> GetTfGraphProducerVersion(mlir::ModuleOp module) {
  ...
  auto producer = dyn_cast<mlir::IntegerAttr>(versions.get("producer"));
  if (!producer) {
    return errors::Internal(
