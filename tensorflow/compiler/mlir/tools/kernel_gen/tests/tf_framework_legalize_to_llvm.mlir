// RUN: kernel-gen-opt %s -test-tf-framework-legalize-to-llvm -split-input-file | FileCheck %s

// CHECK: llvm.func @_mlir_ciface_tf_alloc_raw
// CHECK-SAME:  (!llvm<"i8*">, !llvm.i64) -> !llvm<"i8*">

// CHECK-LABEL: llvm.func @alloc_raw(
// CHECK-SAME:    [[TF_CTX:%.*]]: !llvm<"i8*">,
...
// CHECK: [[NULL:%.*]] = llvm.mlir.null : !llvm<"float*">
...
// CHECK-SAME:            !llvm<"float*"> to !llvm.i64
...
// CHECK-SAME:                  !llvm<"i8*"> to !llvm<"float*">
...
// CHECK: llvm.func @_mlir_ciface_tf_dealloc_raw(!llvm<"i8*">)
...
// CHECK-SAME:                   !llvm<"float*"> to !llvm<"i8*">

// Deallocate.
// CHECK: llvm.call @_mlir_ciface_tf_dealloc_raw(
// CHECK-SAME: [[TF_CTX]], [[VOID_PTR]]) : (!llvm<"i8*">, !llvm<"i8*">) -> ()
