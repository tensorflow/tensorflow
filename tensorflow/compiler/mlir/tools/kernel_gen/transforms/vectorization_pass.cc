...
class SplitSCFForOp : public OpRewritePattern<scf::ForOp> {
 public:
  using OpRewritePattern<scf::ForOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(scf::ForOp scf_for,
    ...
    auto IsOpOfInterest = [&](AffineMinOp min_op, Value iv) {
      if (min_op.getDimOperands().size() + min_op.getSymbolOperands().size() !=
          2)
        return false;
      bool min_by_step = false;
      for (auto i : min_op.getAffineMap().getResults()) {
        if (i == rewriter.getAffineConstantExpr(step_bound_value.getInt())) {
          min_by_step = true;
          continue;
        }
        if (i == rewriter.getAffineSymbolExpr(0) -
                     rewriter.getAffineDimExpr(0) &&
            min_op.getDimOperands().front() == iv &&
            min_op.getSymbolOperands().front() == scf_for.upperBound())
          continue;
        if (i == rewriter.getAffineDimExpr(0) - rewriter.getAffineDimExpr(1) &&
    ...
    scf_for->walk([&](AffineMinOp min_op) {
      if (IsOpOfInterest(min_op, scf_for.getInductionVar()))
    ...
    rewriter.setInsertionPoint(scf_for);
    Value split_point = rewriter.create<SubIOp>(
        scf_for.getLoc(), scf_for.upperBound(),
        rewriter.create<UnsignedRemIOp>(
            scf_for.getLoc(),
            rewriter.create<SubIOp>(scf_for.getLoc(), scf_for.upperBound(),
                                    scf_for.lowerBound()),
            scf_for.step()));

    // New primary loop with relevant min ops replaced with their constant value
    BlockAndValueMapping mapper;
    auto new_loop = llvm::cast<scf::ForOp>(rewriter.clone(*scf_for, mapper));
    new_loop.setUpperBound(split_point);

    new_loop->walk([&](AffineMinOp min_op) {
      if (IsOpOfInterest(min_op, new_loop.getInductionVar()))
        rewriter.replaceOp(min_op, scf_for.step());
    });
    ...
    auto tail_loop =
        llvm::cast<scf::ForOp>(rewriter.clone(*scf_for, tail_mapper));
    tail_loop.setLowerBound(split_point);
    rewriter.replaceOp(scf_for, tail_loop.results());
...
struct VectorizationPass : public VectorizationPassBase<VectorizationPass> {
  ...
  void runOnFunction() override {
    ...
    tilingOptions = tilingOptions.setTileSizes(llvm::makeArrayRef<int64_t>(4));
    auto alignment = 16;
...
struct VectorizationCleanupPass
  ...
  void runOnFunction() override {
    ...
    patterns.insert<SplitSCFForOp>(&getContext());
