...
struct TileTransposePass : public TileTransposeBase<TileTransposePass> {
  void runOnOperation() override {
    auto get_tile_size = [&](mlir::OpBuilder b, Operation *op) {
      ...
      auto num_loops = generic_op.getNumLoops();
      ...
      auto indexing_maps = generic_op.getIndexingMaps();
      int last_dim = num_loops - 1;
      int vec_factor0 = 8, vec_factor1 = 8;
      int vec_dim0 = indexing_maps[0].getDimPosition(last_dim);
      int vec_dim1 = indexing_maps[1].getDimPosition(last_dim);
