...
func @main(
    %arg0: tensor<!tf.resource<tensor<i32>>> {tf_saved_model.bound_input = @y})
  ...
  "tf.AssignVariableOp"(%arg0, %0) {device = "/device:CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>, tensor<i32>) -> ()
  %1 = "tf.ReadVariableOp"(%arg0) {device = "/device:CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
...
func private @callee(
  %arg0: tensor<!tf.resource<tensor<i32>>>) -> tensor<!tf.resource<tensor<i32>>>
{
    return %arg0: tensor<!tf.resource<tensor<i32>>>
}
...
func @main(
  ...
  %0 = "tf.Const"() {value = dense<1> : tensor<i32>} : () -> tensor<i32>
  // expected-error @+1 {{'tf.AssignVariableOp' op unsupported users of resource variables}}
  "tf.AssignVariableOp"(%res, %0) {device = "/device:CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>, tensor<i32>) -> ()
  %1 = "tf.ReadVariableOp"(%arg0) {device = "/device:CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
