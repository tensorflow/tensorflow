...
// RUN: tf-tfrt-opt -pass-pipeline='builtin.module(tf-to-tfrt{enable-native-ops=false target-tpurt=true tpu-use-core-selector=false})' %s | FileCheck %s --dump-input=fail --dump-input-filter=all
