...
func @gather(%indices: tensor<?xi32>,
             %resource: tensor<*x!tf.resource>) -> tensor<*xi32> {
  %0 = "tf.ResourceGather"(%resource, %indices) {batch_dims = 0 : i64, device = "/device:CPU:0", validate_indices = true}: (tensor<*x!tf.resource>, tensor<?xi32>) -> (tensor<*xi32>)
