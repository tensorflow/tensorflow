...
module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 567 : i32}} {
  ...
  func @__inference_pruned_35() -> tensor<!tf_type.variant> attributes {tf.entry_function = {control_outputs = "", inputs = "", outputs = "flatmapdataset__4_RetVal"}} {
    ...
    %3 = "tf.RangeDataset"(%0, %1, %2) {device = "/device:CPU:0", output_shapes = [#tf_type.shape<>], output_types = [i64]} : (tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<!tf_type.variant>
    ...
    %4 = "tf.FlatMapDataset"(%3) {Targuments = [], device = "/device:CPU:0", f = @__inference_Dataset_flat_map_lambda_190, output_shapes = [#tf_type.shape<>], output_types = [i64]} : (tensor<!tf_type.variant>) -> tensor<!tf_type.variant>
  ...
  func private @__inference_Dataset_flat_map_lambda_190(%arg0: tensor<i64> {tf._user_specified_name = "args_0"}) -> tensor<!tf_type.variant> attributes {tf._tf_data_function = true, tf.signature.is_stateful} {
    ...
    %3 = "tf.RangeDataset"(%0, %2, %1) {device = "/device:CPU:0", output_shapes = [#tf_type.shape<>], output_types = [i64]} : (tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<!tf_type.variant>
    ...
    %4 = "tf.MapDataset"(%3) {device = "/device:CPU:0", f = @__inference_Dataset_map_lambda_160, f._tf_data_function = true, output_shapes = [#tf_type.shape<>], output_types = [i64], preserve_cardinality = true, use_inter_op_parallelism = true} : (tensor<!tf_type.variant>) -> tensor<!tf_type.variant>
