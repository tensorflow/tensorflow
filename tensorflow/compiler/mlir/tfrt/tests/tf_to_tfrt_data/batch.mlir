...
// CHECK-LABEL: func @main() -> !tfrt_data.dataset
  func @main() -> tensor<*x!tf.variant> {
    ...
    %range = "tf.RangeDataset"(%start, %stop, %step) {output_shapes = [#tf.shape<>], output_types = [i64]} : (tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<*x!tf.variant>
    %batch_size = "tf.Const"() {value = dense<10> : tensor<i64>} : () -> tensor<i64>
    %drop_remainder = "tf.Const"() {value = dense<false> : tensor<i1>} : () -> tensor<i1>
    %batch = "tf.BatchDatasetV2"(%range, %batch_size, %drop_remainder) {output_shapes = [#tf.shape<>], output_types = [i64], parallel_copy = false} : (tensor<*x!tf.variant>, tensor<i64>, tensor<i1>) -> tensor<*x!tf.variant>
    return %batch : tensor<*x!tf.variant>
  }
}
