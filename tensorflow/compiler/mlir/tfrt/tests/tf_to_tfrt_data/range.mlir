...
// CHECK-LABEL: func @main() -> !tfrt_data.dataset
  func @main() -> tensor<*x!tf.variant> {
    ...
    %4 = "tf.RangeDataset"(%1, %3, %2) {device = "", output_shapes = [#tf.shape<>], output_types = [i64]} : (tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<*x!tf.variant>
    return %4 : tensor<*x!tf.variant>
  }
}
