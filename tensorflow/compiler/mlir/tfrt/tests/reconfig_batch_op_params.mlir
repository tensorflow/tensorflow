// RUN: tf-tfrt-opt -split-input-file -tfrt-reconfig-batch-op="tfrt-min-num-batch-threads=4 tfrt-min-max-enqueued-batches=4 tfrt-num-batch-threads=3 tfrt-max-batch-size=3 tfrt-batch-timeout-micros=3 tfrt-allowed-batch-sizes=3,4 tfrt-max-enqueued-batches=0" %s | FileCheck %s --dump-input=always
...
// [1, 2].
// The max_enqueued_batches is updated to 0 from the original attribute of 2,
...
func.func @main(%arg0: tensor<1x3xf32>) -> tensor<*xf32> {
  ...
  // CHECK-SAME: max_enqueued_batches = 0 : i64
