module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 398 : i32}, tf_saved_model.semantics} {
  ...
  func @serving_default(
      ...
      %arg1: tensor<!tf.resource<tensor<3x1xi32>>> {tf_saved_model.bound_input = @y},
      %arg2: tensor<!tf.string> {tf_saved_model.bound_input = @z}
  ...
  {
    %0 = "tf.ReadVariableOp"(%arg1) {device = ""} : (tensor<!tf.resource<tensor<3x1xi32>>>) -> tensor<3x1xi32>
  ...
  func @predict(
    ) -> (tensor<0x!tf.string> {tf_saved_model.index_path = ["r"]})
      attributes {
        tf.entry_function = {control_outputs = "", inputs = "input:0", outputs = "result:0"},
        tf_saved_model.exported_names = ["predict"]
    }
  {
    %0 = "tf.Const"() {dtype = !tf.string, value = dense<[]> : tensor<0x!tf.string>} : () -> tensor<0x!tf.string>
    return %0 : tensor<0x!tf.string>
  }
}
