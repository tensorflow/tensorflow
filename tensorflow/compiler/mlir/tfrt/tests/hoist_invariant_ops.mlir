...
// CHECK: [[handle:%.*]] = "tf.VarHandleOp"() {container = "", shared_name = "x"} : () -> tensor<!tf.resource<tensor<i32>>>
// CHECK: [[x:%.*]] = "tf.ReadVariableOp"([[handle]]) {device = "/CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
...
func @hoist_varhandleop(%arg: tensor<i32> {tf_saved_model.index_path = ["input"]}) -> (tensor<i32> {tf_saved_model.index_path = ["r"]})
  ...
  // CHECK: return [[r]]
  %handle = "tf.VarHandleOp"() {container = "", shared_name = "x"} : () -> tensor<!tf.resource<tensor<i32>>>
  %x = "tf.ReadVariableOp"(%handle) {device = "/CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
...
func @hoist_hash_table(%arg: tensor<?x!tf.string> {tf_saved_model.index_path = ["input"]}, %default: tensor<i64> {tf_saved_model.index_path = ["default"]}) -> (tensor<i64> {tf_saved_model.index_path = ["r"]}, tensor<*xi64> {tf_saved_model.index_path = ["r1"]})
  ...
  %0 = "tf.HashTableV2"() {container = "", device = "", key_dtype = !tf.string, shared_name = "x", use_node_name_sharing = false, value_dtype = i64} : () -> tensor<!tf.resource>
  %1 = "tf.LookupTableSizeV2"(%0) {device = ""} : (tensor<!tf.resource>) -> tensor<i64>
  %2 = "tf.LookupTableFindV2"(%0, %arg, %default) {device = "/CPU:0"} : (tensor<!tf.resource>, tensor<?x!tf.string>, tensor<i64>) -> tensor<*xi64>
  return %1, %2 : tensor<i64>, tensor<*xi64>
}
...
// CHECK: [[handle:%.*]] = "tf.VarHandleOp"() {container = "", shared_name = "x"} : () -> tensor<!tf.resource<tensor<i32>>>
// CHECK: "tf._TfrtSetResource"([[handle]]) {device = "/job:localhost/replica:0/task:0/device:CPU:0", index = [[handle_idx:.*]] : i64} : (tensor<!tf.resource<tensor<i32>>>) -> ()
...
func @hoist_var_read_write() -> (tensor<i32> {tf_saved_model.index_path = ["x"]}, tensor<i32> {tf_saved_model.index_path = ["r"]})
  ...
  // CHECK-NOT: tf.VarHandleOp
  // CHECK: [[v:%.*]]:2 = "tf._TfrtGetResource"() {device = "/job:localhost/replica:0/task:0/device:CPU:0", indices = [0, 1]} : () -> ({{.*}})
  // CHECK: [[x:%.*]] = "tf.ReadVariableOp"([[v]]#[[handle_idx]]) {device = "/CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
  // CHECK-NEXT: "tf.AssignVariable"([[v]]#[[handle_idx]], [[v]]#[[const_idx]]) {device = "/CPU:0"} : (tensor<!tf.resource<tensor<i32>>>, tensor<i32>) -> ()
  // CHECK-NEXT: [[r:%.*]] = "tf.ReadVariableOp"([[v]]#[[handle_idx]]) {device = "/CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
  ...
  %handle = "tf.VarHandleOp"() {container = "", shared_name = "x"} : () -> tensor<!tf.resource<tensor<i32>>>
  %x = "tf.ReadVariableOp"(%handle) {device = "/CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
  "tf.AssignVariable"(%handle, %const) {device = "/CPU:0"} : (tensor<!tf.resource<tensor<i32>>>, tensor<i32>) -> ()
  %r = "tf.ReadVariableOp"(%handle) {device = "/CPU:0", dtype = i32} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
  return %x, %r : tensor<i32>, tensor<i32>
}
...
func private @some_func(
    %arg: tensor<!tf.resource<tensor<i32>>>) -> tensor<i32> {
  %0 = "tf.ReadVariableOp"(%arg) {device = "cpu"} : (tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
  return %0 : tensor<i32>
}
...
func @not_hoist_stateful_call(%arg: tensor<i32> {tf_saved_model.index_path = ["input"]}) -> (tensor<i32> {tf_saved_model.index_path = ["r"]})
  ...
  %x = "tf.StatefulPartitionedCall"(%handle) {device = "/CPU:0", config = "", config_proto = "", executor_type = "", f = @some_func} : (tensor<!tf.resource<tensor<i32>>>) -> (tensor<i32>)
  %r = "tf.AddV2"(%arg, %x) {device = "/CPU:0"} : (tensor<i32>, tensor<i32>) -> tensor<i32>
  return %r : tensor<i32>
}
...
func @not_hoist_if(%arg: tensor<i32> {tf_saved_model.index_path = ["input"]}) -> (tensor<i32> {tf_saved_model.index_path = ["r"]})
  ...
  // CHECK: tf.If
  %x = "tf.If"(%cond, %handle) {then_branch = @some_func, else_branch = @some_func, is_stateless = false} : (tensor<i1>, tensor<!tf.resource<tensor<i32>>>) -> tensor<i32>
  %r = "tf.AddV2"(%arg, %x) {device = "/CPU:0"} : (tensor<i32>, tensor<i32>) -> tensor<i32>
  return %r : tensor<i32>
}
