...
struct AllowSinkHelper {
  explicit AllowSinkHelper(mlir::Operation* original_defining_op,
                           mlir::Operation* user, int arg_index) {
    ...
    if (llvm::isa<mlir::TF::VarHandleOp>(original_defining_op) &&
...
void FindSinkTarget(
  ...
  for (mlir::OpOperand &use : value.getUses()) {
    ...
    if (helper.allow_sink_to) {
      ...
      // we don't need to sink it. that directly returned
