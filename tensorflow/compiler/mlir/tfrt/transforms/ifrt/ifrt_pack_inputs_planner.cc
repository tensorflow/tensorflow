...
inline constexpr int64_t kPackGroupLayoutFussy = -2;
 ...
 public:
  ...
  void ProcessCall(
    ...
    for (int i = 0; i < num_args; ++i) {
      ...
      if (callee && static_cast<unsigned>(i) < callee.getNumArguments() &&
          FeedsLayoutFussyOp(callee.getArgument(i))) {
        LOG(INFO) << "IFRT Pack-Inputs: skipping operand " << i
                  << " (feeds layout-fussy consumer)";
        group_ids[i] = kPackGroupLayoutFussy;
        continue;
      }
      int64_t bytes_per_elt = ElementByteSize(
          llvm::cast<mlir::RankedTensorType>(call.getArgs()[i].getType())
              .getElementType());
      VLOG(1) << "IFRT Pack-Inputs: packing operand " << i << " (" << bytes
              << " bytes) into group " << bytes_per_elt;
      group_ids[i] = bytes_per_elt;
    }
