...
mlir::Attribute ConvertAttribute(mlir::Attribute attr, mlir::Builder& builder) {
  ...
  if (isa<mlir::IntegerAttr, mlir::FloatAttr, mlir::BoolAttr, mlir::StringAttr,
  ...
  if (auto type_attr = dyn_cast<mlir::TypeAttr>(attr)) {
    if (auto shape_type = dyn_cast<mlir::TensorType>(type_attr.getValue())) {
...
bool IsSupportedTfrtNumericDType(mlir::Type type) {
  ...
  if (auto complex_type = dyn_cast<mlir::ComplexType>(type)) {
