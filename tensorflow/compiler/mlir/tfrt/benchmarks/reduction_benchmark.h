...
// MLIR benchmarks
#define BM_TFMlir(NAME, TYPE, INPUT_RANK, SPEC)               \
  static void BM_mlir__##INPUT_RANK##D_##NAME##_##TYPE(       \
      ::testing::benchmark::State& state) {                   \
    RunReductionMlirBenchmark<TYPE, INPUT_RANK>(state, SPEC); \
  }                                                           \
  BENCHMARK(BM_mlir__##INPUT_RANK##D_##NAME##_##TYPE)

#define ARGS_1D \
  Args({3})->Args({8})->Args({80})->Args({800})->Args({8000})->Args({8131})

#define BM_TFMlir1(NAME, TYPE, SPEC) BM_TFMlir(NAME, TYPE, 1, SPEC)->ARGS_1D
...
#define ARGS_2D          \
  Args({2, 80})          \
      ...
      ->Args({800, 600}) \
      ->Args({802, 602})
#define BM_TFMlir2(NAME, TYPE, SPEC) BM_TFMlir(NAME, TYPE, 2, SPEC)->ARGS_2D

// Eigen benchmarks
#define BM_Eigen(NAME, TYPE, INPUT_RANK, OUTPUT_RANK, SPEC)                 \
  static void BM_eigen_##INPUT_RANK##D_##NAME##_##TYPE(                     \
      ::testing::benchmark::State& state) {                                 \
    RunReductionEigenBenchmark<TYPE, INPUT_RANK, OUTPUT_RANK>(state, SPEC); \
  }                                                                         \
  BENCHMARK(BM_eigen_##INPUT_RANK##D_##NAME##_##TYPE)

#define BM_Eigen1(NAME, TYPE) \
  BM_Eigen(NAME, TYPE, 1, 0, EigenSpec({0}))->ARGS_1D

#define BM_Eigen2(NAME, TYPE, OUTPUT_RANK, SPEC) \
  BM_Eigen(NAME, TYPE, 2, OUTPUT_RANK, SPEC)->ARGS_2D
