...
std::string MeanRow(bool dynamic_row, bool dynamic_col, int32_t rows,
                    int32_t cols) {
  return GetReductionIR("tf.Mean", {rows, cols}, {dynamic_row, dynamic_col},
                        {1}, "f32");
}
...
#define BM_SUITE(NAME, DYNAMIC_ROW, DYNAMIC_COL, ROWS, COLS)             \
  BM(CpurtV(NAME, MeanRow(DYNAMIC_ROW, DYNAMIC_COL, ROWS, COLS), "main", \
            Inputs(ROWS, COLS)));                                        \
  BM(Eigen(NAME, EigenMeanRow(), Inputs(ROWS, COLS)));                   \
  BM(Tfrt(NAME, MeanRow(DYNAMIC_ROW, DYNAMIC_COL, ROWS, COLS), "main",   \
          Inputs(ROWS, COLS)))

#define BM_DYNAMIC_ALL(ROWS, COLS)                                            \
  BM_SUITE(MeanRowDynamicAll_##ROWS##_##COLS, kDynamicDim, kDynamicDim, ROWS, \
           COLS)
BM_DYNAMIC_ALL(2, 80);
BM_DYNAMIC_ALL(8, 6);
BM_DYNAMIC_ALL(80, 1);
...
#define BM_STATIC_ROW(ROWS, COLS) \
  BM_SUITE(MeanRowStaticRow##ROWS##_##COLS, kStaticDim, kDynamicDim, ROWS, COLS)
BM_STATIC_ROW(2, 80);
BM_STATIC_ROW(8, 6);
BM_STATIC_ROW(80, 1);
BM_STATIC_ROW(80, 60);
...
#define BM_STATIC_COL(ROWS, COLS)                                           \
  BM_SUITE(MeanRowStaticCol_##ROWS##_##COLS, kDynamicDim, kStaticDim, ROWS, \
           COLS)
BM_STATIC_COL(2, 80);
BM_STATIC_COL(8, 6);
BM_STATIC_COL(80, 1);
...
#define BM_STATIC_ALL(ROWS, COLS) \
  BM_SUITE(MeanRowStaticAll_##ROWS##_##COLS, kStaticDim, kStaticDim, ROWS, COLS)
BM_STATIC_ALL(2, 80);
BM_STATIC_ALL(8, 6);
BM_STATIC_ALL(80, 1);
BM_STATIC_ALL(80, 60);
