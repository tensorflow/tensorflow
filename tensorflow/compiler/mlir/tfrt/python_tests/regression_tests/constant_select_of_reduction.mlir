func.func @test(%V__0: tensor<?x?xi64> { python_test_attrs.static_type = tensor<1x2xi64> }, %V__1: tensor<?x?xi32> { python_test_attrs.static_type = tensor<1x2xi32> }) -> tensor<?x?xi32> {
  %0 = "tf.Equal"(%V__0, %V__0) { device = "/job:localhost/replica:0/task:0/device:CPU:0" } : (tensor<?x?xi64>, tensor<?x?xi64>) -> tensor<?x?xi1>
