...
flags.DEFINE_boolean("vectorize", None, "Whether vectorization should be enabled")
            ...
            for arg_attr in arg_attrs:
                ...
                if _SHAPE_VALUE_ATTRIBUTE_NAME in attr_dict:
                    ...
                    static_type = ir.TypeAttr(
                        attr_dict[_STATIC_TYPE_ATTRIBUTE_NAME]
                    ).value
                    ...
                    arg = np.random.uniform(
                        -10000.0, 10000.0, size=shaped_type.shape
                    ).astype(np_element_type)
                    args.append(arg)
