builtin.func @test(%V__0 : tensor<?x?xi64> { python_test_attrs.static_type = tensor<1x36xi64> }) -> tensor<?x?xi64> {
  ...
  %1 = "tf.Const"() { value = dense<[65, 1]> : tensor<2xi32>, device = "/job:localhost/replica:0/task:0/device:CPU:0" } : () -> tensor<2xi32>
