...
class RewriteTFRCallOp : public OpRewritePattern<CallOp> {
 ...
 private:
  ...
  Value CastToNonDerivedType(PatternRewriter& rewriter, Location loc,
    ...
    auto tensor_type = dyn_cast<TFRTensorType>(input_tfr_type);
    ...
    Type original_input_type =
        cast<TypeAttr>(cast_op.getInputElementType()).getValue();
  ...
  void CastValuesToSameType(PatternRewriter& rewriter, Location loc,
    ...
    Type target_input_type = cast<TypeAttr>(input_types[0]).getValue();
    auto result_type = UnrankedTensorType::get(target_input_type);
    for (auto i = 1; i < input_types.size(); ++i) {
      Type current_input_type = cast<TypeAttr>(input_types[i]).getValue();
...
LogicalResult RewriteTFRCallOp::AddDerivedAttrs(
  ...
  if (auto tensor_type = dyn_cast<TFRTensorType>(input_tfr_type)) {
    auto attr_names = tensor_type.getAttrKeys();
  ...
  if (auto list_type = dyn_cast<TFRTensorListType>(input_tfr_type)) {
...
LogicalResult RewriteTFRCallOp::DeriveOutputTypes(
  ...
  for (auto res : llvm::enumerate(signature.getResults())) {
    if (auto tensor_type = dyn_cast<TFRTensorType>(res.value())) {
      ...
      if (auto attr = attrs.lookup(attr_key.getValue())) {
        output_types->push_back(
            UnrankedTensorType::get(cast<TypeAttr>(attr).getValue()));
    ...
    if (auto list_type = dyn_cast<TFRTensorListType>(res.value())) {
      ...
      if (attr_keys.size() == 2) {
        ...
        int list_size =
            cast<IntegerAttr>(attrs.lookup(attr_keys[0].getValue())).getInt();
        Type list_type =
            cast<TypeAttr>(attrs.lookup(attr_keys[1].getValue())).getValue();
...
LogicalResult RewriteTFRCallOp::CreateAndReplaceOp(
  ...
  for (auto res : llvm::enumerate(call_op.getResultTypes())) {
    ...
    if (dyn_cast<TFRTensorType>(res_type)) {
    ...
    } else if (auto list_type = dyn_cast<TFRTensorListType>(res.value())) {
