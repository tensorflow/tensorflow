...
bool HasEqualElementSize(Value val1, Value val2, ArrayRef<int> val1_indices,
  ...
  ShapedType val1_shape = cast<ShapedType>(val1.getType());
  ShapedType val2_shape = cast<ShapedType>(val2.getType());
...
Value ReshapeTo1DTensor(OpBuilder& builder, Location loc, Value value) {
  auto shape = cast<ShapedType>(value.getType());
  if (shape.getRank() != 1) {
...
Value MakeOneDimValueBroadcastable(OpBuilder& builder, Location loc,
  ...
  ShapedType value_shape = dyn_cast_or_null<ShapedType>(value.getType());
...
bool CanBeSymmetricallyQuantized(Value weight) {
  ...
  auto qtype = cast<TensorType>(dq_op.getArg().getType()).getElementType();
...
Value MultiplyFakeQuantValue(OpBuilder& builder, Location loc, Value value,
  ...
  auto element_type = cast<TensorType>(q_op.getType()).getElementType();
