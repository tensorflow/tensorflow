...
void PrepareXlaConvParams(OpBuilder &builder, Location loc, ArrayAttr strides,
  ...
  window_strides =
      Create1DConstValue<int32_t>(builder, loc, stride_values);
  lhs_dilation =
      Create1DConstValue<int32_t>(builder, loc, lhs_dilation_values);
  rhs_dilation =
      Create1DConstValue<int32_t>(builder, loc, rhs_dilation_values);
...
Value CreateZeroPointPartialOffset(OpBuilder &builder, Location loc,
  ...
  auto zp =
      CreateScalarConstValue<int32_t>(builder, loc, other_tensor_zp);
...
Value MergeZeroPointOffset(OpBuilder &builder, Location loc, Value weight,
  ...
  if (!weight_non_output_dynamic_indices.empty()) {
    ...
    auto slice_stride =
        CreateConstValue<int64_t>(builder, loc, {1}, {1});
    for (int64_t weight_idx : weight_non_output_dynamic_indices) {
      auto start =
          CreateConstValue<int64_t>(builder, loc, {1}, {weight_idx});
      auto end =
