...
void FindQuantizationUnitsRecursively(Location loc,
  ...
  ArrayRef<Location> locations = cast<FusedLoc>(loc).getLocations();
  if (IsImportLocPattern(cast<FusedLoc>(loc))) {
    ...
    if (isa<NameLoc>(locations.back())) {
      StringRef name_loc_id =
          cast<NameLoc>(locations.back()).getName().strref();
      set_node_and_func_name(new_unit, name_loc_id);
    } else {
      Location callee = cast<CallSiteLoc>(locations.back()).getCallee();
      StringRef name_loc_id = cast<NameLoc>(callee).getName().strref();
      set_node_and_func_name(new_unit, name_loc_id);
    }
