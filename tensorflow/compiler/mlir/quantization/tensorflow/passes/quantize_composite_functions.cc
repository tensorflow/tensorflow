...
LogicalResult CreateQuantizationParams(QuantizedType elem_type, Location loc,
  ...
  if (auto qtype = dyn_cast<UniformQuantizedType>(elem_type)) {
    return CreateUniformQuantizedTypeParams(qtype, loc, rewriter, scale,
                                            zero_point);
  } else if (auto qtype =
                 dyn_cast<quant::UniformQuantizedPerAxisType>(elem_type)) {
...
ShapedType ConvertIntToQint(ShapedType input_type, MLIRContext* ctx) {
  ...
  } else if (QuantizedType qtype = dyn_cast<QuantizedType>(ele_type)) {
...
class ReplaceQuantizePattern
 ...
 private:
  ...
  LogicalResult matchAndRewrite(quantfork::QuantizeCastOp q_op,
    ...
    auto output_type = cast<TensorType>(q_op.getType());
    auto elem_type = dyn_cast<QuantizedType>(output_type.getElementType());
    ...
    if (target_opset_ == OpSet::UNIFORM_QUANTIZED) {
      ShapedType new_output_type = ConvertIntToQint(
          cast<ShapedType>(output_type), rewriter.getContext());
      if (!new_output_type) {
        q_op->emitError(
...
class ReplaceDequantizePattern
 ...
 private:
  ...
  LogicalResult matchAndRewrite(quantfork::DequantizeCastOp dq_op,
                                PatternRewriter& rewriter) const override {
    auto input_type = cast<TensorType>(dq_op.getArg().getType());
    auto elem_type = dyn_cast<QuantizedType>(input_type.getElementType());
    ...
    if (target_opset_ == OpSet::UNIFORM_QUANTIZED) {
      ...
      output_type = cast<TensorType>(new_output_type);
    }
...
bool IsQuantizedCallforDynamicRange(TF::PartitionedCallOp call_op) {
  ...
  for (Value output : call_op.getOutput()) {
    if (auto type = dyn_cast<TensorType>(output.getType())) {
...
bool IsQuantizedCallforStaticRange(TF::PartitionedCallOp call_op) {
  ...
  for (Value input : call_op.getArgs()) {
    if (auto type = dyn_cast<TensorType>(input.getType())) {
      if (isa<QuantizedType>(type.getElementType())) {
...
bool ContainsFloatResultType(ArrayRef<Type> result_types) {
  for (auto current_type : result_types) {
    if (dyn_cast<TensorType>(current_type).getElementType().isF32())
      return true;
  }
  return false;
}
...
class QuantizeFunctionPattern
 ...
 private:
  ...
  LogicalResult matchAndRewrite(TF::PartitionedCallOp call_op,
                                PatternRewriter& rewriter) const override {
    const auto f_attr = dyn_cast<FlatSymbolRefAttr>(call_op.getFAttr());
    ...
    for (Value arg : call_op.getArgs()) {
      if (const auto arg_type = dyn_cast<TensorType>(arg.getType())) {
        QuantizedType qtype =
            dyn_cast<QuantizedType>(arg_type.getElementType());
        if (!qtype) continue;
        if (!isa<UniformQuantizedType, quant::UniformQuantizedPerAxisType>(
                qtype)) {
          return failure();
        }
        Value scale, zero_point;
    ...
    for (Value result : call_op->getResults()) {
      if (auto result_type = dyn_cast<TensorType>(result.getType())) {
        QuantizedType qtype =
            dyn_cast<QuantizedType>(result_type.getElementType());
        if (!qtype) continue;
        if (!isa<UniformQuantizedType, quant::UniformQuantizedPerAxisType>(
                qtype)) {
          return failure();
    ...
    for (Value arg : call_op.getArgs()) {
      TensorType arg_type = dyn_cast<TensorType>(arg.getType());
      if (!arg_type) {
        args.push_back(arg);
        continue;
      }
      QuantizedType qtype = dyn_cast<QuantizedType>(arg_type.getElementType());
      if (!qtype) {
        args.push_back(arg);
        continue;
      }
      ...
      if (target_opset_ == OpSet::UNIFORM_QUANTIZED) {
        ShapedType new_arg_type =
            ConvertIntToQint(cast<ShapedType>(arg_type), rewriter.getContext());
        if (!new_arg_type) {
          call_op->emitError(
              "Failed to convert the type to the corresponding qtype.");
          return failure();
        ...
        scast_op = rewriter.create<quantfork::StorageCastOp>(
            arg.getLoc(), cast<TensorType>(new_arg_type), arg);
      } else {
        scast_op = rewriter.create<quantfork::StorageCastOp>(
    ...
    for (Value result : call_op->getResults()) {
      ...
      if (target_opset_ == OpSet::UNIFORM_QUANTIZED) {
        ShapedType new_result_type = ConvertIntToQint(
            cast<ShapedType>(result_type), rewriter.getContext());
...
class QuantizeConstPattern
 ...
 private:
  LogicalResult matchAndRewrite(quantfork::QuantizeCastOp q_op,
    ...
    ShapedType tensor_qtype = cast<ShapedType>(q_op.getResult().getType());
    ...
    Type storage_type =
        cast<QuantizedType>(tensor_qtype.getElementType()).getStorageType();
    ...
    if (target_opset_ == OpSet::UNIFORM_QUANTIZED) {
      ...
      if (!mlir::tfg::ConvertToTensorProto(
               cast<ElementsAttr>(tensor_proto_attr), &tensor_proto)
               .ok()) {
        return failure();
      }

      const int bit_width =
          dyn_cast<QuantizedType>(tensor_qtype.getElementType())
...
class RestoreWeightShapePattern
 ...
 private:
  LogicalResult addReshapeOpToDepthwiseWeight(TF::PartitionedCallOp op,
    ...
    auto weight_type = dyn_cast<ShapedType>(weight_op->getResult(0).getType());
    auto input_type = dyn_cast<ShapedType>(op.getOperand(0).getType());
...
class QuantizationSummary {
 ...
 private:
  ...
  FailureOr<StringRef> GetRepresentativeName(StringRef func_name) {
    ...
    return cast<StringAttr>(quantized_ops.front()).getValue();
  }
