...
bool QuantizationUnitLoc::classof(Attribute attr) {
  ...
  if (!isa<NameLoc>(callsite_loc.getCaller())) return false;
  StringRef caller_name =
      cast<NameLoc>(callsite_loc.getCaller()).getName().strref();
...
FindQuantizationUnitFromLoc(Location loc) {
  if (isa<QuantizationUnitLoc>(loc)) {
    Location caller = cast<CallSiteLoc>(loc).getCaller();
    StringRef caller_name = cast<NameLoc>(caller).getName().strref();
    ...
    for (Location child_loc : cast<FusedLoc>(loc).getLocations()) {
    ...
    return FindQuantizationUnitFromLoc(cast<CallSiteLoc>(loc).getCallee());
  }
