...
func.func @same_scale_op_before_matmul(%arg0: tensor<*xf32>) -> tensor<*xf32> {
  ...
  %q_weight = "quantization.qcast"(%weight) : (tensor<144x12xf32>) -> tensor<144x12x!quant.uniform<i8:f32, 0.074855112561992565:-1>>
  %dq_weight = "quantization.dcast"(%q_weight) : (tensor<144x12x!quant.uniform<i8:f32, 0.074855112561992565:-1>>) -> tensor<144x12xf32>
  %9 = "tf.MatMul"(%7, %dq_weight) {transpose_a = false, transpose_b = false} : (tensor<*xf32>, tensor<144x12xf32>) -> tensor<*xf32>
  %10 = "quantization.qcast"(%9) {volatile} : (tensor<*xf32>) -> tensor<*x!quant.uniform<i8:f32, 4.000000e-03:-12>>
  %11 = "quantization.dcast"(%10) : (tensor<*x!quant.uniform<i8:f32, 4.000000e-03:-12>>) -> tensor<*xf32>
...
// CHECK: %[[scast:.*]] = "quantization.scast"(%[[reshape_i8]]
...
func.func private @avgpool_after_conv(%input: tensor<1x3x4x3xf32> {tf._user_specified_name = "input_tensor"}) -> tensor<*xf32> attributes {tf._construction_context = "kEagerRuntime", tf._input_shapes = [#tf_type.shape<1x3x4x3>]} {
  ...
  %q_pool = "quantization.qcast"(%avg_pool) {volatile} : (tensor<*xf32>) -> tensor<*x!quant.uniform<i8:f32, 0.023529411764705882:-128>>
  %dq_pool = "quantization.dcast"(%q_pool) : (tensor<*x!quant.uniform<i8:f32, 0.023529411764705882:-128>>) -> tensor<*xf32>
  %reshape = "tf.Reshape"(%dq_pool, %cst) {device = ""} : (tensor<*xf32>, tensor<2xi32>) -> tensor<*xf32>
  %q_reshape = "quantization.qcast"(%reshape) {volatile} : (tensor<*xf32>) -> tensor<*x!quant.uniform<i8:f32, 0.023529411764705882:-128>>
  %dq_reshape = "quantization.dcast"(%q_reshape) : (tensor<*x!quant.uniform<i8:f32, 0.023529411764705882:-128>>) -> tensor<*xf32>
...
// CHECK: %[[scast:.*]] = "quantization.scast"(%[[conv]]
...
// CHECK: %[[sc2:.*]] = "quantization.scast"(%[[reshape]])
