...
from tensorflow.compiler.mlir.quantization.stablehlo import (
    quantization_config_pb2 as stablehlo_quant_config_pb2,
)
from tensorflow.compiler.mlir.quantization.tensorflow.calibrator import (
    custom_aggregator_op_wrapper,
)
...
_CalibrationMethod = stablehlo_quant_config_pb2.CalibrationOptions.CalibrationMethod
...
class CustomAggregatorTest(test.TestCase):
    ...
    def testHistogramCalibration(self):
        with self.session():
            input_tensor = array_ops.constant([1.0, 1.0, 3.0, 4.0, 6.0], dtypes.float32)
