...
std::optional<Value> AddUniformQuantizeOps(PatternRewriter& rewriter,
  ...
  ShapedType shaped_quantized_type = cast<ShapedType>(quantized_type);
  DenseElementsAttr tensor_proto_attr =
      dyn_cast<DenseElementsAttr>(Quantize(attr, shaped_quantized_type));
  if (!tensor_proto_attr) {
    return nullptr;
  }

  Type storage_type =
      cast<QuantizedType>(shaped_quantized_type.getElementType())
          .getStorageType();
...
Operation* LogicsForUniformDequanization(PatternRewriter& rewriter,
  ...
  // TODO - b/278949920: Enable Per-Channel Quantization for XLA Opset
  auto qtype = dyn_cast<UniformQuantizedType>(quant_type);
