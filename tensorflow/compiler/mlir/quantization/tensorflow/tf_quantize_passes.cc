...
void AddQuantizeQatPasses(
  ...
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::tf_quant::CreateTFConvertFakeQuantToQdqPass());
  ...
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::tf_quant::CreateTFConvertTfXlaOpToTfOpPass());
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::tf_quant::CreateTFPrepareLiftingPass(
          quantization_options.op_set()));

  pm.addPass(mlir::tf_quant::CreateTFLiftQuantizableSpotsAsFunctionsPass(
      quantization_options));
  pm.addPass(mlir::tf_quant::CreateTFInsertQuantizedFunctionsPass(
      quantization_options.quantization_method().preset_method(),
      quantization_options.op_set()));
  // TODO: b/260677670 - Pass quantization options as pass's inputs where
  // applicable
  pm.addPass(mlir::tf_quant::CreateTFQuantizeCompositeFunctionsPass(
      quantization_options.quantization_method().preset_method(),
      quantization_options.op_set(),
  ...
  if (quantization_options.op_set() != OpSet::TF) {
    ...
    if (quantization_options.op_set() == OpSet::XLA) {
      pm.addNestedPass<mlir::func::FuncOp>(
          mlir::tf_quant::CreateTFReplaceCastHacksWithTFXLAOpsPass());
    }
    pm.addNestedPass<mlir::func::FuncOp>(mlir::createCSEPass());
  }
  pm.addNestedPass<mlir::func::FuncOp>(mlir::tf_quant::CreateTFOptimizePass());
}
...
void AddQuantizePtqDynamicRangePasses(
  ...
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::tf_quant::CreateTFPrepareLiftingPass(
          quantization_options.op_set()));
  pm.addPass(mlir::tf_quant::CreateTFLiftQuantizableSpotsAsFunctionsDRQPass(
      quantization_options.quantization_method().preset_method(),
      quantization_options.op_set(),
      quantization_options.min_num_elements_for_weights()));
  pm.addPass(mlir::tf_quant::CreateTFInsertQuantizedFunctionsPass(
...
void AddQuantizePtqPreCalibrationPasses(
  ...
  if (quantization_options.has_debugger_config()) {
    pm.addPass(mlir::tf_quant::CreateTFAddDumpTensorOpPass(
        quantization_options.debugger_config().debugger_type(),
        quantization_options.debugger_config().log_dir_path()));
  }
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::tf_quant::CreateTFInsertCustomAggregationOpsPass(
...
void AddQuantizePtqPostCalibrationPasses(
  ...
  pm.addPass(mlir::TF::CreateTFShapeInferencePass());
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::tf_quant::CreateTFConvertCustomAggregationOpToQuantStatsPass());
  pm.addPass(mlir::tf_quant::CreateTFInsertQuantizedFunctionsPass(
      quantization_options.quantization_method().preset_method(),
      quantization_options.op_set()));
  pm.addPass(mlir::tf_quant::CreateTFQuantizeCompositeFunctionsPass(
...
void AddQuantizeWeightOnlyPasses(
  ...
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::tf_quant::CreateTFPrepareLiftingPass(
          quantization_options.op_set()));
  pm.addPass(mlir::tf_quant::CreateTFQuantizeWeightsPass(quantization_options));
  pm.addPass(mlir::tf_quant::CreateTFPropagateQuantizeTypePass());
  pm.addPass(mlir::createSymbolDCEPass());
  pm.addPass(mlir::createInlinerPass());
  pm.addPass(mlir::TF::CreateTFShapeInferencePass());
  pm.addNestedPass<mlir::func::FuncOp>(mlir::createCanonicalizerPass());
