...
from tensorflow.compiler.mlir.quantization.tensorflow.python.integration_test import (
...
class MultipleSignatureModel(module.Module):
    ...
    @def_function.function(
        input_signature=[tensor_spec.TensorSpec(shape=[1, 4], dtype=dtypes.float32)]
    )
...
class StaticRangeQuantizationTest(quantize_model_test_base.QuantizedModelTest):
    ...
    def test_qat_conv_model(
        ...
        input_data = np.random.uniform(low=-0.1, high=0.2, size=(1, 3, 4, 3)).astype(
            "f4"
        )
    ...
    def test_qat_vocab_table_lookup_model(self):
        ...
        with session.Session(graph=ops.Graph()) as sess:
            ...
            self.assertTrue(
                self._contains_quantized_function_call(output_meta_graph_def.graph_def)
            )
            self.assertCountEqual(
    ...
    @parameterized.named_parameters(
        ...
        {
            "testcase_name": ("dilation_with_bias_and_bn_and_relu6_to_xla_per_channel"),
            "activation_fn": nn_ops.relu6,
            "has_bias": True,
            "has_batch_norm": True,
            "target_opset": quant_opts_pb2.XLA,
        ...
        {
            "testcase_name": ("with_bias_and_bn_and_relu6_to_xla_dynamic_per_channel"),
            "activation_fn": nn_ops.relu6,
            "has_bias": True,
            "has_batch_norm": True,
            "target_opset": quant_opts_pb2.XLA,
        ...
        {
            "testcase_name": ("dilation_with_bias_and_bn_and_relu6_to_uq_per_channel"),
            "activation_fn": nn_ops.relu6,
            "has_bias": True,
            "has_batch_norm": True,
            "target_opset": quant_opts_pb2.UNIFORM_QUANTIZED,
    ...
    @test_util.run_v2_only
    def test_gather_model(self, input_type, use_variable, expect_quantized_gather):
    ...
    def test_ptq_multiple_signatures_invalid_dataset_raises_value_error(self):
        ...
        invalid_dataset: repr_dataset.RepresentativeDataset = [
            {"matmul_input": random_ops.random_uniform(shape=(1, 4))} for _ in range(8)
        ]
    ...
    def test_conv3d_ptq_model(
        ...
        input_data = np.random.uniform(low=-0.1, high=0.2, size=(1, 3, 4, 3, 3)).astype(
            "f4"
        )
...
class DynamicRangeQuantizationTest(quantize_model_test_base.QuantizedModelTest):
    ...
    def test_conv_model(
        ...
        if target_opset == quant_opts_pb2.UNIFORM_QUANTIZED:
            self.assertTrue(
                self._contains_op(output_graphdef, "UniformQuantizedConvolutionHybrid")
            )
            self.assertFalse(self._contains_op(output_graphdef, "Conv2D"))
            if enable_per_channel_quantization:
                self.assertTrue(
...
class DebuggerTest(quantize_model_test_base.QuantizedModelTest):
    ...
    def test_conv2d_ptq_model_per_layer_verify(
        ...
        sample_inputs = [
            {"input_tensor": np.random.uniform(low=0, high=1, size=sample_input_size)},
            {"input_tensor": np.random.uniform(low=0, high=1, size=sample_input_size)},
        ]
