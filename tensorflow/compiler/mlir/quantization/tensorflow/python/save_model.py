...
def _validate_signatures(signature_def_map: _SignatureDefMap,
  ...
  we try to fix that with `_restore_output_tensor_names`. However, if that
  doesn't work the signatures can't be used with the converted graph.
...
def save_model_v1(graph_def: graph_pb2.GraphDef,
  ...
  with session.Session(graph=ops.Graph()) as sess:
    ...
    _validate_signatures(signature_def_map, ops.get_default_graph())
