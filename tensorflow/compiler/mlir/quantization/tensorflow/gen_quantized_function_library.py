...
_NAMESPACE = flags.DEFINE_string(
    "namespace", "mlir::quant", "namespace in the generated file"
)
...
def _substitute_quantized_function_name_template(module: str) -> str:
    ...
    compiled_regex = re.compile(r"GenerateQuantizedFunctionName(\([\w\s\'\"\[\],]+\))")
    while True:
        func_match = re.search(compiled_regex, module)
        if func_match is None:
            break
    ...
    for src_file in src_files:
        with open(src_file, "r") as f:
            ...
            module_match = re.search(
                r"(^module\s\{)(.*)(^\})", content, re.MULTILINE | re.DOTALL
            )
