...
from tensorflow.compiler.mlir.quantization.stablehlo.python.integration_test import (
    quantize_model_test_base,
)
from tensorflow.compiler.mlir.quantization.tensorflow.python import (
    representative_dataset as repr_dataset,
)
...
class StaticRangeQuantizationTest(quantize_model_test_base.QuantizedModelTest):
    ...
    def test_matmul_ptq_model(
        ...
        input_data = ops.convert_to_tensor(
            rng.uniform(low=0.0, high=1.0, size=static_input_shape).astype(np.float32)
        )
