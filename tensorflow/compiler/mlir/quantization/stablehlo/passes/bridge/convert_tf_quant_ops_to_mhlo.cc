...
FailureOr<TensorType> GetUniformQuantizedType(
  ...
  if (!isa<TF::Qint8Type, TF::Qint32Type>(original_element_type)) {
  ...
  return cast<TensorType>(original_type).clone(elem_ty);
}
...
FailureOr<ElementsAttr> ConvertPaddingAttr(
  ...
  ShapedType lhs_shape = cast<ShapedType>(op.getLhs().getType());
  ShapedType rhs_shape = cast<ShapedType>(op.getRhs().getType());
  ...
  if (conv_padding.strref().equals("EXPLICIT")) {
    ...
    for (int i = 0; i < dnums.input_spatial_dimensions_size(); ++i) {
      const int64_t stride =
          cast<IntegerAttr>(op.getWindowStridesAttr()[i]).getInt();
      const int64_t lhs_size_dilated =
          ::tensorflow::UniformQuantizedConvolutionParams::DilatedSize(
              ...
              cast<IntegerAttr>(op.getLhsDilationAttr()[i]).getInt());
      const int64_t rhs_size_dilated =
          ::tensorflow::UniformQuantizedConvolutionParams::DilatedSize(
              rhs_shape.getDimSize(dnums.kernel_spatial_dimensions(i)),
              cast<IntegerAttr>(op.getRhsDilationAttr()[i]).getInt());
...
FailureOr<SmallVector<NamedAttribute>> ConvertToMhloConvolutionOpAttrs(
  ...
  for (auto attr : op->getAttrs()) {
    if (attr.getName() == op.getFeatureGroupCountAttrName() ||
      ...
      attr.setValue(ConvertToDenseElementsAttr(cast<ArrayAttr>(attr.getValue()),
                                               rewriter));
      converted_attrs.push_back(attr);
    }
  }
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::UniformQuantizeOp op,
    ...
    rewriter.replaceOpWithNewOp<mhlo::BitcastConvertOp>(
        op,
        output_type->clone(
            dyn_cast<quant::QuantizedType>(output_type->getElementType())
                .getStorageType()),
        result);
...
class ConvertUniformQuantizedAddOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    auto lhs_type = cast<ShapedType>(lhs.getType());
    if (!lhs_type.hasRank()) {
      return rewriter.notifyMatchFailure(
