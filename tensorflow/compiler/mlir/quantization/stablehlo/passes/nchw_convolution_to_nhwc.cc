 ...
 public:
  ...
  void rewrite(mlir::stablehlo::ConvolutionOp op,
    ...
    const TensorType new_input_tensor_type = GetTransposedTensorType(
        cast<TensorType>(input.getType()), kNchwToNhwcPermutation);
    ...
    const TensorType new_filter_tensor_type = GetTransposedTensorType(
        cast<TensorType>(filter.getType()), kOihwToHwioPermutation);
    ...
    auto output_tensor_type = cast<TensorType>(op->getResult(0).getType());
