...
TEST(GetIntTypeFromTFQintTest, ChecksIntTypesFromTFQint) {
  ...
  EXPECT_EQ(dyn_cast<IntegerType>(type).getWidth(), 8);
  EXPECT_FALSE(dyn_cast<IntegerType>(type).isSigned());
  EXPECT_FALSE(dyn_cast<IntegerType>(type).isUnsigned());

  type = GetIntTypeFromTFQint(TF::Qint16Type::get(context.get()));
  EXPECT_TRUE(llvm::isa<IntegerType>(type));
  EXPECT_EQ(dyn_cast<IntegerType>(type).getWidth(), 16);
  EXPECT_FALSE(dyn_cast<IntegerType>(type).isSigned());
  EXPECT_FALSE(dyn_cast<IntegerType>(type).isUnsigned());

  type = GetIntTypeFromTFQint(TF::Qint32Type::get(context.get()));
  EXPECT_TRUE(llvm::isa<IntegerType>(type));
  EXPECT_EQ(dyn_cast<IntegerType>(type).getWidth(), 32);
  EXPECT_FALSE(dyn_cast<IntegerType>(type).isSigned());
  EXPECT_FALSE(dyn_cast<IntegerType>(type).isUnsigned());
  ...
  EXPECT_TRUE(llvm::isa<IntegerType>(type));
  EXPECT_EQ(dyn_cast<IntegerType>(type).getWidth(), 8);
  EXPECT_TRUE(dyn_cast<IntegerType>(type).isUnsigned());
