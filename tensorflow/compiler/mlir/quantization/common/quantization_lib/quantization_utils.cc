...
QuantizedType ResetMinMaxFromNumBits(const QuantizedType type,
  ...
  if (auto q_type = mlir::dyn_cast<UniformQuantizedType>(type)) {
...
quant::UniformQuantizedPerAxisType ResetAxisAndBroadcast(
    ...
    const int quant_dim) {
  const auto shaped = mlir::dyn_cast<RankedTensorType>(target);
  if (!shaped) return {};
...
Type GetQuantizedType(Builder builder, const Type input_type,
  ...
  if (min.size() == 1 && max.size() == 1 && quant_dim == -1) {
    ...
    auto shape = mlir::dyn_cast<ShapedType>(input_type);
...
TypeAttr RescaleQuantizedType(const Type input, const Attribute factor) {
  const auto factor_values =
      mlir::dyn_cast_or_null<DenseFPElementsAttr>(factor);
  if (!factor_values) return {};
  const auto ele_type = quant::QuantizedType::getQuantizedElementType(input);
  if (!ele_type) return {};
  if (auto qtype =
          mlir::dyn_cast<quant::UniformQuantizedPerAxisType>(ele_type)) {
...
TypeAttr GetQuantizedTypeAttr(const Builder builder, const Type input_type,
  ...
  const auto mins = mlir::dyn_cast<DenseFPElementsAttr>(min);
  const auto maxs = mlir::dyn_cast<DenseFPElementsAttr>(max);
  if (mins && maxs) {
    ...
    const auto fmin = mlir::dyn_cast<FloatAttr>(min);
    const auto fmax = mlir::dyn_cast<FloatAttr>(max);
    if (fmin && fmax) {
...
TypeAttr CastQuantizedTypeAttrFromExpressedType(const Builder builder,
  ...
  const auto source_type =
      mlir::dyn_cast_or_null<ShapedType>(source.getValue());
  if (!source_type) return {};
  const auto src_ele_type = source_type.getElementType();
  auto qtype = mlir::dyn_cast<quant::QuantizedType>(src_ele_type);

  // Reset the quantization dimensions if it is per-axis.
  if (const auto per_axis =
          mlir::dyn_cast_or_null<quant::UniformQuantizedPerAxisType>(qtype)) {
...
Type GetUniformQuantizedTypeForWeight(
  ...
  const auto fp = mlir::dyn_cast<DenseFPElementsAttr>(attr);
  if (!fp) return {};
  ...
  const auto type =
      GetQuantizedType(builder, attr.getType(), mins[0], maxs[0],
                       ...
                       legacy_float_scale, use_fake_quant_num_bits);
  if (const auto ele_type = mlir::dyn_cast_or_null<TensorType>(type))
...
Type GetUniformQuantizedPerAxisTypeForWeight(
    ...
    const bool legacy_float_scale, const bool use_fake_quant_num_bits) {
  const Builder builder(attr.getContext());
  const auto shape = mlir::cast<ShapedType>(attr.getType()).getShape();
  ...
  if (auto ele_type = mlir::dyn_cast_or_null<TensorType>(type))
    return ele_type.getElementType();
...
quant::QuantizedType GetUniformQuantizedTypeForBias(
  ...
  for (const auto op_type : op_types) {
    ...
    if (const auto type =
            mlir::dyn_cast<quant::UniformQuantizedPerAxisType>(op_type)) {
    ...
    } else if (!mlir::isa<quant::UniformQuantizedType>(op_type)) {
      return {};
    }
  }
    ...
    if (const auto type =
      ...
      for (const auto& index_scale : llvm::enumerate(type.getScales())) {
        scales[index_scale.index()] *= index_scale.value();
                   ...
                   mlir::dyn_cast<quant::UniformQuantizedType>(op_type)) {
...
ElementsAttr QuantizeLegacy(const Attribute real_value,
  ...
  if (!mlir::isa<DenseFPElementsAttr>(real_value) ||
      !quant::QuantizedType::getQuantizedElementType(tensor_type)) {
    return {};
  }
  const auto real_values_attr = mlir::cast<DenseFPElementsAttr>(real_value);
  ...
  const ShapedType new_dense_type = mlir::dyn_cast_or_null<ShapedType>(
      q_type.castExpressedToStorageType(real_values_attr.getType()));
  const int width =
      mlir::dyn_cast<mlir::IntegerType>(q_type.getStorageType()).getWidth();
  ...
  if (width == 8 && q_type.getStorageTypeMax() == 127 &&
    ...
    if (auto uniform_type = mlir::dyn_cast<UniformQuantizedType>(q_type)) {
      float min, max, scale;
      ...
      if (std::abs(scale - uniform_type.getScale()) > 1e-3) {
        return Quantize(real_value, tensor_type);
                   ...
                   mlir::dyn_cast<quant::UniformQuantizedPerAxisType>(q_type)) {
    ...
    if (const auto uniform_type =
            mlir::dyn_cast<UniformQuantizedType>(q_type)) {
...
ElementsAttr Quantize(const Attribute real_value, const Type tensor_type) {
  if (const auto q_type =
          quant::QuantizedType::getQuantizedElementType(tensor_type)) {
    Type converted_type;
    return mlir::dyn_cast_or_null<ElementsAttr>(
...
static bool PreferResultScale(Operation* op) {
  ...
  for (auto operand : op->getOperands()) {
    if (auto operand_type = mlir::dyn_cast<ShapedType>(operand.getType())) {
      if (mlir::isa<FloatType>(operand_type.getElementType())) {
  ...
  for (int i = 1; i < collected_quant_params.size(); ++i) {
    ...
    if (!same_scale_op.RequiredSameQuantizedAxes()) {
      const auto expected_per_axis_qtype =
          mlir::dyn_cast<quant::UniformQuantizedPerAxisType>(expected_params);
      const auto compared_per_axis_qtype =
          mlir::dyn_cast<quant::UniformQuantizedPerAxisType>(compared_params);
...
quant::UniformQuantizedType GetFixedOutputRange(
    ...
    int64_t storage_max) {
  const auto result_type = mlir::cast<ShapedType>(tensor_type);
  if (!mlir::isa<FloatType>(result_type.getElementType())) return {};
...
Type ConvertSignedQuantizedToUnsigned(const Type signed_tensor_type,
  ...
  QType new_qtype;
  if (auto uqtype = mlir::dyn_cast<quant::UniformQuantizedType>(qtype)) {
