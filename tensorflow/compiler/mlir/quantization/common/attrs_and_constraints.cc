...
Type CloneTypeWithNewElementType(Type old_type, Type element_type) {
  if (!isa<ShapedType>(old_type)) return {};

  return cast<ShapedType>(old_type).clone(element_type);
}
...
absl::StatusOr<bool> IsDotGeneralFullyConnected(DotGeneralOp dot_general_op) {
  ...
  const int64_t input_rank =
      dyn_cast<ShapedType>(dot_general_op.getOperand(0).getType()).getRank();
  const int64_t filter_rank =
      dyn_cast<ShapedType>(dot_general_op.getOperand(1).getType()).getRank();
