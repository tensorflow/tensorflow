...
module {
    ...
    // CHECK-DAG: %[[shift49:.*]] = "tosa.const"() <{values = dense<49> : tensor<1xi8>}> : () -> tensor<1xi8>
    // CHECK-DAG: %[[shift11:.*]] = "tosa.const"() <{values = dense<11> : tensor<1xi8>}> : () -> tensor<1xi8>
    ...
    // CHECK: %[[VAL_2:.*]] = tosa.rescale %[[VAL_1]], %[[mult1073741824]], %[[shift11]] {input_unsigned = false, input_zp = 2 : i32, output_unsigned = false, output_zp = 0 : i32, per_channel = false, rounding_mode = "DOUBLE_ROUND", scale32 = true} : (tensor<2x3x!quant.uniform<i8:f32, 1.000000e-01:2>>, tensor<1xi32>, tensor<1xi8>) -> tensor<2x3xi32>
    // CHECK: %[[VAL_3:.*]] = tosa.rescale %[[VAL_4:.*]], %[[mult1073741824]], %[[shift11]] {input_unsigned = false, input_zp = 2 : i32, output_unsigned = false, output_zp = 0 : i32, per_channel = false, rounding_mode = "DOUBLE_ROUND", scale32 = true} : (tensor<2x3x!quant.uniform<i8:f32, 1.000000e-01:2>>, tensor<1xi32>, tensor<1xi8>) -> tensor<2x3xi32>
    // CHECK: %[[VAL_5:.*]] = tosa.add %[[VAL_2]], %[[VAL_3]] : (tensor<2x3xi32>, tensor<2x3xi32>) -> tensor<2x3xi32>
    // CHECK: %[[VAL_6:.*]] = tosa.rescale %[[VAL_5]], %[[mult1073741824]], %[[shift49]] {input_unsigned = false, input_zp = 0 : i32, output_unsigned = false, output_zp = 2 : i32, per_channel = false, rounding_mode = "DOUBLE_ROUND", scale32 = true} : (tensor<2x3xi32>, tensor<1xi32>, tensor<1xi8>) -> tensor<2x3x!quant.uniform<i8:f32, 1.000000e-01:2>>
