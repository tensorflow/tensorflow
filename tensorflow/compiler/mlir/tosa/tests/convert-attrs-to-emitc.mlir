// RUN: tf-tosa-opt --tfl-to-tosa-pipeline=convert-attrs-to-emitc %s | FileCheck %s
...
module attributes { } {
  func.func @main(%arg0: tensor<1xf32> {tf_saved_model.index_path = ["another_feature"]}, %arg1: tensor<1xf32> {tf_saved_model.index_path = ["some_feature"]}) -> (tensor<1xf32> {tf_saved_model.index_path = ["output_0"]}) attributes { } {
    %0 = tfl.add %arg1, %arg0 {fused_activation_function = "NONE"} : tensor<1xf32>
...
// CHECK-SAME:  %arg0: tensor<1xf32> {emitc.hint_name = "another_feature"}, 
// CHECK-SAME:  %arg1: tensor<1xf32> {emitc.hint_name = "some_feature"}) 
