...
static FailureOr<std::pair<Type, Value>> getTosaBias(
  ...
  if (output_is_qtype) {
    ...
    int bias_bits = input_bits == 16 ? 48 : 32;
  ...
  if (!output_is_qtype) {
    // No further processing required now that we've ensured that the bias
    // exists.
    return std::make_pair(bias_ety, bias);
  }

  if (dyn_cast<tosa::SliceOp>(bias.getDefiningOp())) {
    // This indicates that the bias was already handled by
    // lowerGroupedConvolution().
    return std::make_pair(bias_ety, bias);
  }

  auto const_op = dyn_cast<tosa::ConstOp>(bias.getDefiningOp());
  if (!const_op) {
    return rewriter.notifyMatchFailure(op, "bias not a ConstOp or SliceOp");
  }
