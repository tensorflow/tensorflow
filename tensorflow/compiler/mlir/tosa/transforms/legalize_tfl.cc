...
static LogicalResult LegalizeQuantizedLeakyRelu(Operation* op,
  ...
  if (tfl_leaky_relu_op == nullptr)
    return rewriter.notifyMatchFailure(op, "op is not leaky ReLU");
...
LogicalResult ConvertTFLLeakyReluOp::matchAndRewrite(
  ...
  if (input_is_qtype != output_is_qtype) {
    return rewriter.notifyMatchFailure(
        op,
        "ConvertTFLLeakyReluOp: input/output tensor should be all quantized or "
        "all floating-point.");
  }
  ...
  Value input = tfl_leakyrelu_op.input();

  if (output_is_qtype) {
    return LegalizeQuantizedLeakyRelu(op, rewriter, tfl_leakyrelu_op.getInput(),
                                      alpha, output_type);
  }

  return LegalizeFloatingPointLeakyRelu(
