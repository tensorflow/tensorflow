...
LogicalResult ConvertTFLCustomOp::matchAndRewrite(
  ...
  rewriter.replaceOpWithNewOp<tosa::CustomOp>(
      op, op->getResultTypes(), tfl_custom_op.getCustomCode(), rewriter.getStringAttr("TFL"),
      tfl_custom_op.getCustomOption().cast<mlir::TFL::ConstBytesAttr>().getValue().str(), op->getOperands());
