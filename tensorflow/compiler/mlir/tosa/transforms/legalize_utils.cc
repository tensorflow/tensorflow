...
Value buildRescaleFromInt32(PatternRewriter& rewriter, Operation* op,
  ...
  auto input_type = input_val.getType().dyn_cast<mlir::RankedTensorType>();
  assert(input_type);
  assert(input_type.getElementType().isSignedInteger(32));
...
Value buildRescaleOpConvOutput(PatternRewriter& rewriter, Operation* op,
  ...
  auto weight_qtype =
      weight_type.getElementType().dyn_cast<mlir::quant::QuantizedType>();
  ...
  if (auto weight_per_tensor_qtype =
    ...
    ArrayAttr multiplier_array = rewriter.getI32ArrayAttr({multiplier_arr});
    ArrayAttr shift_array = rewriter.getI32ArrayAttr({shift_arr});
...
Value getTosa1DConstTensorTable(PatternRewriter& rewriter, Operation* op,
  ...
  auto const_attr = DenseElementsAttr::get(
      const_type, llvm::makeArrayRef<int16_t>(table_vec));
...
template Value get1DConstTensor<tosa::ConstOp, float>(
    PatternRewriter&, Operation*, SmallVector<float, 8> arr);
