...
// Given
//  'actual_element_type': Return element-type of ConvOp.
//  'expected_element_type': The return element-type of ConvOP inferred using
//     the input arguments and the convolution attributes.
// The 'actual_element_type' of the ConvOp is valid if
//  1. 'actual_element_type' == 'expected_element_type'
//  2. 'expected_element_type' is a float-type.
//  3. Bitwidth of 'actual_element_type' >= Bitwidth of 'expected_element_type'.
static LogicalResult verifyConvOpCompatibleElementType(
    ConvOp op, Type expected_element_type, Type actual_element_type) {
  if ((actual_element_type == expected_element_type) ||
      expected_element_type.isa<FloatType>())
    return success();

  if (actual_element_type.getIntOrFloatBitWidth() <
      expected_element_type.getIntOrFloatBitWidth())
    return op.emitOpError()
           << "expects the width of the actual return element-type "
           << actual_element_type
           << " to be >= to that of expected element-type "
...
// and returns it.
static Type higherPrecisionElementType(const Type& a, const Type& b) {
  // Returns a tuple where the elements are lexicographically ordered in terms
  // of importance.
  auto type_properties = [](Type type) {
    auto component_type = type.isa<ComplexType>()
                              ? type.cast<ComplexType>().getElementType()
                              : type;
    ...
    return std::make_tuple(
        ...
        component_type.isa<FloatType>()
            ? APFloat::semanticsMaxExponent(
                  component_type.cast<FloatType>().getFloatSemantics())
            : 0,
        ...
        component_type.getIntOrFloatBitWidth(),
        // Prefer signed integer types over unsigned integer types.
        component_type.isSignedInteger());
  };
  ...
  assert(a == b &&
         "higherPrecisionElementType:: The non-floating point types a and b "
         "must be equal");
  return a;
}

// Convert a 1D dense int64 attribute to a list of values.
static SmallVector<int64_t> convertDenseIntAttr(
...
// Convert a (N, 2) dense int64 attribute to a list of tuples.
static SmallVector<std::pair<int64_t, int64_t>> convertNx2Attribute(
    llvm::Optional<mlir::DenseIntElementsAttr> optional_attr) {
...
static int64_t dilatedBound(int64_t bound, int64_t dilation) {
...
static FailureOr<SmallVector<WindowDimension>>
verifyWindowAttributesAndInferWindowDimensions(
  ...
  if (failed(verifySize(window_strides.size(), "window strides")))
    return failure();
  if (failed(verifySize(lhs_dilation.size(), "lhs dilation factors")))
    return failure();
  if (failed(verifySize(rhs_dilation.size(), "rhs dilation factors")))
    return failure();
  // Note that padding is a Nx2-D array. The following will check properties on
  // N. 'parseWindowAttributes' checks each of the N vector has a pair of
  // elements.
  ...
  for (size_t i = 0; i < window_dimensions.size(); i++) {
    ...
    if (!isDynamicDimSize(dim.size) && dim.size <= 0)
      return (mlir::emitError(loc)
                  << "expects window to have positive dimension for " << i
...
static LogicalResult isSpatialDimensionsValid(ConvOp op) {
...
static LogicalResult verifyConvolutionAttributes(ConvOp op) {
  // P1.
...
// Infer the spatial dimensions of the output.
//  Foreach d,
//    output-spatial-shape[d] =
//            stridedBound(padding_low + dilatedBound(input[d]) + padding_high,
//                         dilatedBound(kernel[d]))
//      where (padding_low, padding_high) is the padding pair for d.
static SmallVector<int64_t> inferConvOpOutputSpatialShape(
  ...
  for (int64_t i = 0; i < window.size(); ++i) {
    if (isDynamicDimSize(input_spatial_dim_vals[i]) ||
        isDynamicDimSize(window[i].size)) {
      output_dimensions[i] = ShapedType::kDynamicSize;
    } else {
      const auto& dim = window[i];

      const int64_t dilated_base =
          dilatedBound(input_spatial_dim_vals[i], dim.base_dilation);
...
// Infer the return-type of ConvOp.
// Precondition: Input args to ConvOp 'op' are RankedTypes.
static RankedTensorType inferConvOpReturnType(
    ConvOp op, const ArrayRef<WindowDimension> window) {
  ...
  auto window_output_shape = inferConvOpOutputSpatialShape(op, window);
  ...
  auto element_type = higherPrecisionElementType(lhs_type.getElementType(),
...
LogicalResult ConvOp::verify() {
  ...
  auto window_or_err = verifyWindowAttributesAndInferWindowDimensions(
      window_dimensions, convertDenseIntAttr(window_strides()),
      convertNx2Attribute(padding()), convertDenseIntAttr(lhs_dilation()),
      convertDenseIntAttr(rhs_dilation()), getLoc());
  if (failed(window_or_err)) return failure();

  // P4.
  auto actual_return_type = getResult().getType().dyn_cast<ShapedType>();
  if (!actual_return_type)
    return emitOpError()
           << "expects convolution return-type to be shaped-type.";
  ...
  auto expected_return_type = inferConvOpReturnType(*this, *window_or_err);
  if (failed(verifyCompatibleShape(expected_return_type,
                                   actual_return_ranked_type)))
    return emitOpError() << "expects convolution return-type to be "
                         << expected_return_type << ", but got "
                         << actual_return_ranked_type << ".";

  return verifyConvOpCompatibleElementType(
      *this, expected_return_type.getElementType(),
      actual_return_ranked_type.getElementType());
}
