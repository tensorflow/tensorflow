...
inline Value MhloAlwaysPropagateNaN(Value v, ValueRange args, Location loc,
  ...
  if (auto float_type = element_type.dyn_cast<FloatType>()) {
    ...
    Value nan = b->create<arith::ConstantOp>(
        loc, b->getFloatAttr(args[0].getType(), nan_apfloat));
...
inline Value MapMhloOpToStdScalarOp<mhlo::NotOp>(
  ...
  if (auto integer_type = element_type.dyn_cast<IntegerType>()) {
    // lmhlo.not(x) -> x ^ -1
    Value all_ones = b->create<arith::ConstantOp>(
        loc,
        b->getIntegerAttr(args[0].getType(),
...
inline Value MapMhloOpToStdScalarOp<mhlo::SignOp>(Location loc,
  ...
  if (auto integer_type = element_type.dyn_cast<IntegerType>()) {
    ...
    Value zero =
        b->create<arith::ConstantOp>(loc, b->getZeroAttr(args[0].getType()));
    Value bitwidth_minus_one = b->create<arith::ConstantOp>(
        loc, b->getIntegerAttr(
                 args[0].getType(),
                 APInt(integer_type.getWidth(), integer_type.getWidth() - 1)));
    Value one = b->create<arith::ConstantOp>(
        loc, b->getIntegerAttr(args[0].getType(),
                               APInt(integer_type.getWidth(), 1)));
