// RUN: mlir-hlo-opt -disc-ral-to-llvm %s -o - | FileCheck %s

// check global definitions
// CHECK-DAG: llvm.mlir.global internal constant @ral_send_output___cpu___pvoid_i64_m2df32___void("ral_send_output___cpu___pvoid_i64_m2df32___void\00")
...
// CHECK-DAG: llvm.mlir.global internal constant @ral_recv_input___cpu___pvoid_i64___m2df32("ral_recv_input___cpu___pvoid_i64___m2df32\00")
...
module @main attributes {gpu.container_module}  {
  ...
  // CHECK: llvm.func @main(%[[CTX:.*]]: !llvm.ptr<i8>) {
  func @main(%arg0: !disc_ral.context) {
    // CHECK-DAG: %[[i64_0:.*]] = llvm.mlir.constant(0 : index) : i64
    // CHECK-DAG: %[[i64_1:.*]] = llvm.mlir.constant(1 : index) : i64
    ...
    // CHECK: %[[OIDX:.*]] = llvm.mlir.constant(9 : i32) : i32
    // CHECK: %[[OARGS:.*]] = llvm.alloca %[[OIDX]] x !llvm.ptr<i8> : (i32) -> !llvm.ptr<ptr<i8>>
    ...
    // CHECK: %[[T24:.*]] = llvm.mlir.addressof @ral_send_output___cpu___pvoid_i64_m2df32___void : !llvm.ptr<array<48 x i8>>
    // CHECK: %[[T25:.*]] = llvm.getelementptr %[[T24]]{{.*}} : (!llvm.ptr<array<48 x i8>>, i64, i64) -> !llvm.ptr<i8>
    // CHECK: llvm.call @disc_ral_call(%[[CTX]], %[[T25]], %[[OARGS]]) : (!llvm.ptr<i8>, !llvm.ptr<i8>, !llvm.ptr<ptr<i8>>) -> ()

    "disc_ral.dispatch"(%arg0, %c0, %0) {backend_config = "cpu", call_target_name = "ral_send_output", has_side_effect = false} : (!disc_ral.context, index, memref<?x?xf32>) -> ()
    return
  }
}
