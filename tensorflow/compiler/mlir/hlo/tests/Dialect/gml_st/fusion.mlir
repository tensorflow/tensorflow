...
func.func @dynamic_broadcast_in_dim(%arg : tensor<?x?xf32>, 
    %shape : tensor<3xindex>) -> tensor<?x?x?xf32> {
  ...
  // CHECK-DAG: %[[RESULT_SPACE:.*]] = gml_st.space [123, 456, 789] : !gml_st.tile<?x?x?>
  // CHECK-DAG: %[[RESULT_TILE:.*]] = gml_st.tile %[[RESULT_SPACE]] [0, 1, 2] [3, 4, 5] [1, 1, 1] : !gml_st.tile<?x?x?> to !gml_st.tile<?x?x?>
  ...
  // CHECK-DAG: %[[OPERAND_TILE:.*]] = gml_st.operand_tile_for_dynamic_broadcast_in_dim %[[RESULT_TILE]], %[[OPERAND_SPACE]], %[[RESULT_SPACE]], [0, 2] : !gml_st.tile<?x?x?>, !gml_st.tile<?x?>, !gml_st.tile<?x?x?> -> !gml_st.tile<?x?>
  // CHECK-DAG: %[[SUB_ARG:.*]] = gml_st.materialize %[[ARG]] at %[[OPERAND_TILE]] : tensor<?x?xf32> at !gml_st.tile<?x?>
  // CHECK-DAG: %[[SUB_INIT:.*]] = gml_st.materialize %[[INIT]] at %[[RESULT_TILE]] : tensor<?x?x?xf32> at !gml_st.tile<?x?x?>
  // CHECK-DAG: %[[SUB_RESULT:.*]] = gml_st.dynamic_broadcast_in_dim %[[SUB_INIT]], %[[SUB_ARG]], [0, 2] : tensor<?x?x?xf32>, tensor<?x?xf32> -> tensor<?x?x?xf32>
  ...
  // Materialze a tile.
  %space = gml_st.space [123, 456, 789] : !gml_st.tile<?x?x?>
  %tile = gml_st.tile %space [0, 1, 2] [3, 4, 5] [1, 1, 1] 
      : !gml_st.tile<?x?x?> to !gml_st.tile<?x?x?>
  %bcast_sub = gml_st.materialize %bcast at %tile
      : tensor<?x?x?xf32> at !gml_st.tile<?x?x?>

  func.return %bcast_sub : tensor<?x?x?xf32>
} 
