...
func @condBranchNestedRegion(%arg0: i1, %arg1: memref<2xf32>,
                             %arg2: memref<2xf32>, %index: memref<i32>) {
  %0 = memref.alloc() : memref<2xf32>
  %1 = memref.alloc() : memref<2xf32>
  cond_br %arg0, ^bb1, ^bb2
^bb1:
  "lmhlo.case"(%index) ( {
    ...
    ^bb0:
      "lmhlo.copy"(%arg2, %0) : (memref<2xf32>, memref<2xf32>) -> ()
      "lmhlo.terminator"() : () -> ()
    }
  ) : (memref<i32>) -> ()
  br ^bb3
...
// CHECK-SAME: %[[ARG0:.*]]: {{.*}}, %[[ARG1:.*]]: {{.*}}, %[[ARG2:.*]]: {{.*}}, %[[ARG4:.*]]: {{.*}}
// CHECK-NEXT: %[[ALLOC0:.*]] = memref.alloc()
// CHECK-NEXT: cond_br %[[ARG0]], ^[[BB1:.*]], ^[[BB2:.*]]
//      CHECK: ^[[BB1]]:
// CHECK-NEXT: "lmhlo.case"(%[[ARG4]])
//      CHECK: "lmhlo.negate"(%[[ARG1]], %[[ALLOC0]]
//      CHECK: "lmhlo.copy"(%[[ARG2]], %[[ALLOC0]]
//      CHECK: br ^[[BB3:.*]]
//      CHECK: ^[[BB2]]:
// CHECK-NEXT: "lmhlo.negate"(%[[ARG1]], %[[ALLOC0]]
//      CHECK: br ^[[BB3:.*]]
