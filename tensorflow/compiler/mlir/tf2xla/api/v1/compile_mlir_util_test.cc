...
absl::StatusOr<std::unique_ptr<Graph>> BuildDummyOpGraphWithOutputShapes() {
...
absl::Status BuildHloFromGraph(Graph& graph, bool use_output_shapes) {
  ...
  return BuildHloFromGraph(
      graph, builder, mlir_ctx, xla_params, returns, use_output_shapes,
      mlir::SpanToArrayRef<XlaCompiler::Argument>({}), control_rets, DEVICE_TPU,
      function_library_definition, debug_info, {});
}

TEST(CompileMlirUtil, UsesCorrectOriginalShapeWithoutOutputShapes) {
  TF_ASSERT_OK_AND_ASSIGN(auto graph, BuildDummyOpGraphWithOutputShapes());
