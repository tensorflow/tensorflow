...
Status GetOutputInfo(
  ...
  for (const auto& type_and_idx : llvm::enumerate(func_type.getResults())) {
    ...
    auto result_ty = cast<mlir::RankedTensorType>(type_and_idx.value());
    ...
    auto tensor_type = dyn_cast<mlir::RankedTensorType>(type_and_idx.value());
...
static absl::StatusOr<std::vector<int>> RewriteWithArgs(
  ...
  for (int idx = 0; idx < args.size(); idx++) {
    ...
    if (xla_arg.kind == XlaArgument::kResource) {
      ...
      auto tensor_type = cast<mlir::TensorType>(mlir_arg.getType());
