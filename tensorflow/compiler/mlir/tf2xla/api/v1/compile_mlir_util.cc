...
// Rewrites the given module with specified args. If inline_const_args is true,
// each of the constant args gets inlined in the "main' function and the
// corresponding argument is removed from the signature. For resource args,
// their subtypes are populated. Returns the original indices for the other
// arguments on success.
static absl::StatusOr<std::vector<int>> RewriteWithArgs(
    mlir::ModuleOp module_op, llvm::ArrayRef<XlaArgument> args,
    bool inline_const_args = true) {
  ...
  for (int idx = 0; idx < args.size(); idx++) {
    ...
    if (xla_arg.kind != XlaArgument::kConstant || !inline_const_args) {
...
// Prepares the module for compilation.  If inline_const_args is true, the
// constant args are inlined in the "main" function and the corresponding
// argument is removed from the signature; The original indices of all
// non-inlined arguments are returned in remaining_params.
Status CompileGraphSetup(
    mlir::ModuleOp module_op, llvm::ArrayRef<XlaArgument> args,
    std::vector<int>* remaining_params,
    llvm::SmallVector<TensorOrResourceShape, 4>& arg_shapes,
    bool inline_const_args = true) {
  TF_ASSIGN_OR_RETURN(*remaining_params,
                      RewriteWithArgs(module_op, args, inline_const_args));
...
Status BuildHloFromModule(mlir::ModuleOp module_op, xla::XlaBuilder& builder,
  ...
  std::vector<int> remaining_params;
  llvm::SmallVector<TensorOrResourceShape, 4> arg_shapes;
  TF_RETURN_IF_ERROR(CompileGraphSetup(module_op, args, &remaining_params,
                                       arg_shapes,
                                       /*inline_const_args=*/false));
  TF_RETURN_IF_ERROR(CompileMlirSetup(module_op, arg_shapes));

  mlir::PassManager tf2xla(module_op.getContext());
  CreateMlirLoweringPassesPipeline(tf2xla);

  MaybeDumpMlirModuleAndPasses(tf2xla, module_op, "", "legalize_hlo_before");
  ...
  mlir::Block& block =
      module_op.lookupSymbol<mlir::func::FuncOp>("main").front();
  TF_RETURN_IF_ERROR(
      mlir::BuildHloFromMlirHlo(block, builder, xla_params, returns));
