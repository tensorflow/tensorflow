...
#ifndef TENSORFLOW_COMPILER_MLIR_TF2XLA_API_V2_GRAPH_TO_TF_EXECUTOR_UTIL_H_
#define TENSORFLOW_COMPILER_MLIR_TF2XLA_API_V2_GRAPH_TO_TF_EXECUTOR_UTIL_H_
...
//  - Uses dynamic padding (based on whether any TPUReplicateMetadata op has a
//    nonempty padding attribute)
//  - Graph is invalid or otherwise encounters error during traversal
// Record stats per feature above if `record_stats` equals true.
...
// Device is used to determine which device we are compiling for
enum class DeviceTypeEnum {
  kCPU,
  kTPU,
  kGPU,
};
// Context is used to determine which stack (or part of the stack) we are using
// to compile
...
#endif  // TENSORFLOW_COMPILER_MLIR_TF2XLA_API_V2_GRAPH_TO_TF_EXECUTOR_UTIL_H_
