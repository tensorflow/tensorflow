...
Value CreateZeroInput(Location loc, OpBuilder& builder, Attribute zero_attr,
                      DenseIntElementsAttr shape_attr, std::string host) {
  Value zero = builder.create<ConstOp>(loc, zero_attr);
  zero.getDefiningOp()->setAttr("device", builder.getStringAttr(host));
  Value shape = builder.create<ConstOp>(loc, shape_attr);
  shape.getDefiningOp()->setAttr("device", builder.getStringAttr(host));
  Value fill = builder.create<FillOp>(loc, shape, zero);
  fill.getDefiningOp()->setAttr("device", builder.getStringAttr(host));
  return fill;
}
...
LogicalResult MoveBroadcastToCluster(OpBuilder& builder,
  ...
  for (uint32_t replica = 1; replica < num_replicas; ++replica) {
    ...
    if (host_to_fill.find(host) == host_to_fill.end()) {
      host_to_fill[host] = CreateZeroInput(val_bcast.getLoc(), builder,
                                           zero_attr, shape_attr, host);
    }
