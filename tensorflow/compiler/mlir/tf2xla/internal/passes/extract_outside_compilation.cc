  ...
  for (Operation* op : cluster_ops) {
    op->walk([&](Operation* walked_op) {
      ...
      for (Value v : walked_op->getOperands()) {
        ...
        auto block_arg = cast<BlockArgument>(v);
...
LogicalResult GetShardShapedType(Operation* context_op,
                                 ...
                                 Type& shard_type) {
  RankedTensorType ranked_type = dyn_cast<RankedTensorType>(full_type);
  if (!ranked_type)
...
LogicalResult CreateHostComputeMap(
  ...
  for (auto [full_type, out] :
    ...
    RankedTensorType full_type_ranked = dyn_cast<RankedTensorType>(full_type);
    if (!full_type_ranked)
      return original_op->emitOpError()
