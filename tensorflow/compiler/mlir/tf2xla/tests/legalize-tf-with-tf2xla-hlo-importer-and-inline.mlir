// RUN: tf-opt --pass-pipeline="builtin.module(xla-legalize-tf{allow-partial-conversion=false device-type=XLA_CPU_JIT legalize-chlo=true prefer-tf2xla=true use-tf2xla-fallback=true use-tf2xla-hlo-importer=true}, inline{default-pipeline=canonicalize max-iterations=4 })" %s -verify-diagnostics | FileCheck %s
// Test that after running tf2xla legalizations with the HLO module importer,
// the generated functions have MHLO.
