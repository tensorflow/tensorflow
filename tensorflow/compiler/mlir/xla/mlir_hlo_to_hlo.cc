...
static void ExtractShardingsFromFunction(
  ...
  for (int i = 0, iter_limit = function.getNumArguments(); i < iter_limit; ++i)
    if (auto sharding =
  ...
  ret_shardings->resize(function.getNumResults(),
                        absl::optional<xla::OpSharding>());
  for (int i = 0, iter_limit = function.getNumResults(); i < iter_limit; ++i)
    if (auto sharding =
...
LogicalResult ExportXlaOp(PadOp op, OpLoweringContext ctx) {
  ...
  for (xla::int64 i = 0, iter_limit = edge_padding_low.size(); i < iter_limit; ++i) {
