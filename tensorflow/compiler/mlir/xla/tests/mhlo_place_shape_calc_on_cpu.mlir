// RUN: tf-opt --mhlo-placer %s | FileCheck %s

// CHECK: module
module {
  func @main(%arg0: tensor<?x?x?xf32>, %arg1: tensor<?x6x2xi64>) -> tensor<?x6x?xf32> attributes {tf.entry_function = {input_placements = "cpu", inputs = "input0", output_placements = "cpu", outputs = "output0"}} {
    // CHECK: memref.dim %arg0, %c2 : tensor<?x?x?xf32> 
    // CHECK: index_cast %0 : index to i64 
    // CHECK: tensor.from_elements %c1_i64, %c1_i64_0, %1 {mhlo_device_type = "cpu"} : tensor<3xi64>
    ...
    // CHECK: "mhlo.d2h"(%4) {mhlo_device_type = "cpu"} : (tensor<?x6x?xf32>) -> tensor<?x6x?xf32>
    // CHECK: return
