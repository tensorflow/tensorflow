...
class ConvertMatrixDiagPartV3Op
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::MatrixDiagPartV3Op op,
    ...
    int rows = input_type.getDimSize(num_dims - 2);  // rows
    int cols = input_type.getDimSize(num_dims - 1);  // cols
    ...
    int64_t max_diag_len =
        std::min(rows + std::min(k[1], 0l), cols + std::min(-k[0], 0l));
    ...
    Value min_d_zero = rewriter.create<MinOp>(loc, d, b_zero);
    Value max_d_zero = rewriter.create<MaxOp>(loc, d, b_zero);
    ...
    Value diag_len_d = rewriter.create<MinOp>(
        loc, rewriter.create<AddOp>(loc, b_rows, min_d_zero),
        rewriter.create<SubOp>(loc, b_cols, max_d_zero));
    ...
    // x = max(d, 0) - offset
    // y = max(-d, 0) - offset
    Value x = rewriter.create<SubOp>(loc, max_d_zero, offset);
