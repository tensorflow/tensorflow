...
/// The std.constant op generates tensors, which could be supplied to other
/// xla_hlo ops. This conversion pattern replaces such std.constant operations
/// by xla_lhlo.const ops.
class StdToLhloConstantOpConverter : public BaseOpConversion<mlir::ConstantOp> {
 public:
  using BaseOpConversion<mlir::ConstantOp>::BaseOpConversion;
  LogicalResult matchAndRewrite(
      mlir::ConstantOp constOp, ArrayRef<Value> operands,
      ConversionPatternRewriter& rewriter) const final {
    ...
    // Tensors generated by constant ops are always statically shaped.
    CHECK(resultType.hasStaticShape());

    Value memref_arg =
        InsertAlloc(op->getLoc(), result, this->bufferAssignment, &rewriter);
    rewriter.create<xla_lhlo::ConstOp>(
        op->getLoc(), constOp.getValue().cast<ElementsAttr>(), memref_arg);
    rewriter.replaceOp(op, memref_arg);
    return success();
  }
};
...
struct HloLegalizeToLhlo
  ...
  void runOnOperation() override {
    ...
    // All standard dialect ops other than constant ops that produce statically
    // shaped tensors are legal; the latter will be converted to lhlo const ops.
    target.addLegalDialect<StandardOpsDialect>();
    target.addDynamicallyLegalOp<ConstantOp>([](ConstantOp op) {
      auto resultType = op.value().getType().dyn_cast<RankedTensorType>();
