...
absl::StatusOr<std::string> PywrapSavedModelToStablehlo(
  ...
  if (!module.ok()) {
    return absl::AbortedError("Failed to convert SavedModel to StableHLO.");
  }

  auto bytecode = ModuleToBytecode(module.value().get());
  if (!bytecode.ok()) {
    return absl::AbortedError("Failed to serialize MLIR module to bytecode.");
  }
...
absl::StatusOr<std::string> PywrapTfModuleToStablehlo(
  ...
  if (!tf_module) {
    return absl::AbortedError("Failed to parse MLIR module.");
  }

  auto mlir_file_path = ExportModule(*tf_module);
  if (!mlir_file_path.ok()) {
    return absl::AbortedError("Failed to write MLIR module to file.");
  }

  auto module =
      TfToStablehlo(*mlir_file_path, &context,
                    /*exported_model_signatures=*/exported_model_signatures,
                    /*tag_names=*/tag_names, input_arg_shapes_str,
