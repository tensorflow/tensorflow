...
StatusOr<QuantizedType> GetQuantizedType(const TensorT& tensor, Builder builder,
  ...
  if (!storage_type) {
    ...
    if (!isa<mlir::IntegerType>(raw_elem_type)) {
      return absl::InvalidArgumentError(
          "Quantized tensors must be stored as integers");
    }
    storage_type = cast<mlir::IntegerType>(raw_elem_type);
  }
...
mlir::ElementsAttr GetSplat(RankedTensorType type, int unique_index,
  ...
  if (isa<mlir::FloatType>(element_ty))
    return DenseElementsAttr::get(
        type, builder.getFloatAttr(element_ty, unique_index));

  if (auto qtype = dyn_cast<QuantizedType>(element_ty)) {
...
StatusOr<mlir::ElementsAttr> ConvertIntBuffer(
  ...
  unsigned bit_width;
  if (auto itype = dyn_cast<mlir::IntegerType>(elem_type)) {
    bit_width = itype.getWidth();
  } else if (auto qtype = dyn_cast<mlir::quant::QuantizedType>(elem_type)) {
