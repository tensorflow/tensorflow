...
TEST_F(LstmUtilsTest, ConvertLSTMCellSimple) {
  ...
  transpose_op++;
  EXPECT_EQ(cast<RankedTensorType>(transpose_op->getOperand(0).getType())
                .getDimSize(0),
            3);
  EXPECT_EQ(cast<RankedTensorType>(transpose_op->getOperand(0).getType())
                .getDimSize(1),
            12);
  EXPECT_EQ(cast<RankedTensorType>(transpose_op->getResult(0).getType())
                .getDimSize(0),
            12);
  EXPECT_EQ(cast<RankedTensorType>(transpose_op->getResult(0).getType())
                .getDimSize(1),
  ...
  EXPECT_FALSE(isa<NoneType>(it->getOperand(1).getType()));
  // input layer norm is None
  EXPECT_TRUE(isa<NoneType>(it->getOperand(20).getType()));
  // proj_bias is F32
  EXPECT_TRUE(cast<RankedTensorType>(it->getOperand(17).getType())
                  .getElementType()
                  .isF32());
  ...
  EXPECT_TRUE(cast<ElementsAttr>(mlir::cast<mlir::arith::ConstantOp>(
  ...
  EXPECT_EQ(cast<RankedTensorType>(output_types[0]).getShape().size(),
            output_shape.size());
  for (int i = 0; i < output_shape.size(); i++) {
    EXPECT_EQ(cast<RankedTensorType>(output_types[0]).getDimSize(i),
              output_shape[i]);
  }
}
...
TEST_F(LstmUtilsTest, ConvertLSTMCellSimpleToFusedLSTMCoupleInputForget) {
  ...
  EXPECT_TRUE(isa<NoneType>(it->getOperand(1).getType()));
}
...
TEST_F(LstmUtilsTest, ConvertLayerNormLSTMCellSimpleToFusedLSTM) {
  ...
  // input layer norm
  EXPECT_FALSE(isa<NoneType>(it->getOperand(20).getType()));
  EXPECT_EQ(
      cast<RankedTensorType>(it->getOperand(20).getType()).getShape().size(),
      1);
  EXPECT_EQ(cast<RankedTensorType>(it->getOperand(20).getType()).getDimSize(0),
            3);
