...
Value CreateStridedSliceOp(mlir::Location loc, ArrayRef<int64_t> output_shape,
  ...
  auto output_type = RankedTensorType::get(
      output_shape, cast<RankedTensorType>(input.getType()).getElementType());
...
void ConvertLSTMCellSimpleToFusedLSTM::UpdateFuncSignature() {
  ...
  auto output_type = tensorflow::GetTypeFromTFTensorShape(
      output_shape, cast<RankedTensorType>(input_.getType()).getElementType());
...
LogicalResult ConvertLSTMCellSimpleToFusedLSTM::RewriteFunc() {
  ...
  auto func_result_type = tensorflow::GetTypeFromTFTensorShape(
      func_output_shape,
      cast<RankedTensorType>(input_.getType()).getElementType());
...
LogicalResult ConvertLSTMCellSimpleToFusedLSTM::Initialize() {
  ...
  weight_type_ = cast<RankedTensorType>(weight_.getType());
  ...
  projection_type_ = cast<RankedTensorType>(projection_.getType());
...
LogicalResult ConvertLayerNormalizedLSTMCellSimpleToFusedLSTM::Initialize() {
  ...
  layer_norm_scale_type_ = cast<RankedTensorType>(layer_norm_scale_.getType());
...
LogicalResult ConvertKerasLSTMLayer(mlir::func::FuncOp func_op,
  ...
  RankedTensorType weight_type =
      cast<RankedTensorType>(weight_kernel.getType());
  ...
  RankedTensorType recurrent_kernel_type =
      cast<RankedTensorType>(recurrent_kernel.getType());
  ...
  Value recurrent_to_input_weights =
      indy ? cast<Value>(CreateFlattenOP(recurrent_weights_array->getResult(0),
                                         func_op.getLoc(), builder)
                             .getResult())
           : recurrent_weights_array->getResult(0);
  Value recurrent_to_forget_weights =
      indy ? cast<Value>(CreateFlattenOP(recurrent_weights_array->getResult(1),
                                         func_op.getLoc(), builder)
                             .getResult())
           : recurrent_weights_array->getResult(1);
  Value recurrent_to_cell_weights =
      indy ? cast<Value>(CreateFlattenOP(recurrent_weights_array->getResult(2),
                                         func_op.getLoc(), builder)
                             .getResult())
           : recurrent_weights_array->getResult(2);
  Value recurrent_to_output_weights =
      indy ? cast<Value>(CreateFlattenOP(recurrent_weights_array->getResult(3),
                                         func_op.getLoc(), builder)
                             .getResult())
  ...
  auto result_type = mlir::RankedTensorType::get(
      output_shape,
      cast<RankedTensorType>(final_inputs.getType()).getElementType());
  ...
  for (int i = 2; i < 5; ++i) {
    auto result_type = dyn_cast<RankedTensorType>(func_op.getResultTypes()[i]);
