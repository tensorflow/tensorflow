...
inline bool OpHasSameStaticShapes(Operation* op) {
  ...
  for (Value value : values) {
    auto shaped_type = dyn_cast<ShapedType>(value.getType());
...
inline bool IsAllOnesConstant(Attribute value) {
  auto values = cast<DenseElementsAttr>(value).getValues<int32_t>();
...
inline bool HasNonNegativeValues(Attribute value) {
  auto values = cast<DenseElementsAttr>(value).getValues<APInt>();
  return !std::any_of(
...
inline TypedAttr GetOffSet(Attribute begin, Attribute end) {
  auto begin_values = cast<DenseElementsAttr>(begin).getValues<int32_t>();
  auto end_values = cast<DenseElementsAttr>(end).getValues<int32_t>();
...
inline DenseElementsAttr GetShape(Value output_val, bool truncate = false) {
  auto output_shape = dyn_cast<ShapedType>(output_val.getType()).getShape();
