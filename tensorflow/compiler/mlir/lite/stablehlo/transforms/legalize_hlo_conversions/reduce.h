...
class ConvertReduceOpToArgMinMax : public OpConversionPattern<mhlo::ReduceOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    const bool is_float = isa<FloatType>(operand_init.getElementType());
    ...
    if (!isa<ShapedType>(operand.getType())) return failure();
    auto operand_type = cast<ShapedType>(operand.getType());
