...
DenseIntElementsAttr SliceStartFromNegPadLows(mhlo::PadOp op) {
  auto vals = UnrollIntSplat<int64_t>(op.getEdgePaddingLow());
...
DenseIntElementsAttr SliceEndFromNegPadHighs(mhlo::PadOp op) {
  auto vals = UnrollIntSplat<int64_t>(op.getEdgePaddingHigh());
...
DenseIntElementsAttr ReplaceNegsWithZero(DenseElementsAttr data) {
  auto vals = UnrollIntSplat<int64_t>(data);
  auto res =
...
bool AnyNegativePads(mhlo::PadOp op) {
  ...
  auto lows_data = UnrollIntSplat<int64_t>(op.getEdgePaddingLow());
  auto highs_data = UnrollIntSplat<int64_t>(op.getEdgePaddingHigh());
