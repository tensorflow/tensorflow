...
namespace mlir {
namespace odml {
// Converts stablehlo.dot_general to tfl.BatchMatMul. Reshape and Transpose ops
// will be inserted to convert to well-formed matrix multiply; i.e.,
// stablehlo.dot_general
// -> tfl.batch_matmul(stablehlo.transpose(stablehlo.reshape(operand)), ...).
// Note:
...
// stablehlo.dot_general has no such restriction.
// 2) Inserted stablehlo.reshape/transpose will be legalized to
// tf.reshape/transpose in LegalizeHloToTf (then from tf to tfl later). 3) If
// the operands are dynamic shaped tensors, stablehlo.DynamicReshapeOp is
...
// (ConvertDotOp).
class LowerDotGeneralOp : public OpConversionPattern<stablehlo::DotGeneralOp> {
 public:
