...
struct ReplaceCustomCallWithComposite final
  ...
  LogicalResult matchAndRewrite(mlir::stablehlo::CustomCallOp op,
    ...
    auto backendConfig =
        dyn_cast<DictionaryAttr>(op->getAttr("composite.backend_config"));
    if (!backendConfig)
      return op->emitError(
          "custom_call has no 'composite.backend_config' attribute or the "
          "attribute is not a dictionary");

    auto name = dyn_cast<StringAttr>(backendConfig.get("name"));
    if (!name)
      return op->emitError(
          "backend_config has no 'name' key or the name value is not a string");

    auto attrs = dyn_cast<DictionaryAttr>(backendConfig.get("attributes"));
    if (!attrs)
      return op->emitError(
    ...
    auto decomposition = cast<FlatSymbolRefAttr>(calledComputations[0]);
