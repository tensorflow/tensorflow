...
class CanoncalizeConvolutionOp
    : public OpConversionPattern<mhlo::ConvolutionOp> {
 public:
  explicit CanoncalizeConvolutionOp(MLIRContext* context)
      : OpConversionPattern(context, /*benefit=*/100) {}
  ...
  LogicalResult matchAndRewrite(
    ...
    if (conv_op.getWindowStrides().has_value() &&
        ...
        conv_op.getLhsDilation().has_value() &&
        conv_op.getRhsDilation().has_value()) {
      return failure();
    }
    auto dnums = conv_op.getDimensionNumbers();
    int32_t input_spatial_dims =
        static_cast<int32_t>(dnums.getInputSpatialDimensions().size());
    DenseIntElementsAttr window_strides_attr;
    window_strides_attr = (conv_op.getWindowStrides().has_value())
                              ? conv_op.getWindowStrides().value()
                              : rewriter.getI64TensorAttr(std::vector<int64_t>(
                                    input_spatial_dims, 1));

    // If there is no value for the following attributes we set the default
    // values according to the MHLO_ConvolutionOp spec
    DenseIntElementsAttr padding_attr;
    padding_attr = (conv_op.getPadding().has_value())
                       ? conv_op.getPadding().value()
                       : DenseIntElementsAttr::get(
                             RankedTensorType::get({input_spatial_dims, 2},
                                                   rewriter.getI64Type()),
                             SmallVector<int64_t>(input_spatial_dims * 2, 0));

    DenseIntElementsAttr lhs_dilation_attr;
    lhs_dilation_attr = (conv_op.getLhsDilation().has_value())
                            ? conv_op.getLhsDilation().value()
                            : rewriter.getI64TensorAttr(
                                  std::vector<int64_t>(input_spatial_dims, 1));
    DenseIntElementsAttr rhs_dilation_attr;
    rhs_dilation_attr = (conv_op.getRhsDilation().has_value())
                            ? conv_op.getRhsDilation().value()
                            : rewriter.getI64TensorAttr(
                                  std::vector<int64_t>(input_spatial_dims, 1));

    auto new_conv_op = rewriter.create<mhlo::ConvolutionOp>(
        conv_op.getLoc(), conv_op.getType(), conv_op.getLhs(), conv_op.getRhs(),
        window_strides_attr, padding_attr, lhs_dilation_attr, rhs_dilation_attr,
    ...
    rewriter.replaceOp(conv_op, new_conv_op);
    return success();
  }
};
...
void PopulateLegalizeHloToTfPatterns(RewritePatternSet* patterns,
                                     MLIRContext* context) {
  patterns->add<
      CanoncalizeConvolutionOp, ConvertAvgPoolOp, Convert2DConvOp,
      Convert1DConvOp, ConvertNonTrivialConvOp, ConvertDynamicSliceOp,
