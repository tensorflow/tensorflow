...
using tflite::impl::FlatBufferModelBase;

std::unique_ptr<FlatBufferModelBase<FlatBufferModelAbslError>> ReadTestModel() {
  auto model_path = tsl::io::JoinPath(
      *g_test_model_dir, ::mlir::lite::internal::kConvModelWith0Plus10Weights);
  return FlatBufferModelBase<FlatBufferModelAbslError>::BuildFromFile(
      model_path.c_str());
}

std::unique_ptr<FlatBufferModelBase<FlatBufferModelAbslError>>
ReadSharedWeightsTestModel() {
  auto model_path = tsl::io::JoinPath(
...
class QuantizeWeightsTest : public testing::Test {
 protected:
  ...
  std::unique_ptr<FlatBufferModelBase<FlatBufferModelAbslError>> input_model_;
  const Model* model_;
