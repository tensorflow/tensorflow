...
module attributes {tf_saved_model.semantics} {
  ...
  func @serving_default(%arg0: tensor<1x10xf32> {tf_saved_model.index_path = ["x"]}) ->
    ...
    %handle_0 = "tf.VarHandleOp"() {container="c", shared_name="a"} : () -> tensor<!tf.resource<tensor<1x10xf32>>>
    %handle_1 = "tf.VarHandleOp"() {container="c", shared_name="b"} : () -> tensor<!tf.resource<tensor<1x10xi64>>>
    %0 = "tf.ReadVariableOp"(%handle_0) : (tensor<!tf.resource<tensor<1x10xf32>>>) -> tensor<1x10xf32>
    %1 = tfl.add %0, %arg0 {fused_activation_function = "NONE"} : tensor<1x10xf32>
    "tf.AssignVariableOp"(%handle_0, %1) : (tensor<!tf.resource<tensor<1x10xf32>>>, tensor<1x10xf32>) -> ()
    %2 = "tf.ReadVariableOp"(%handle_0) {device = ""} : (tensor<!tf.resource<tensor<1x10xf32>>>) -> tensor<1x10xf32>
    %3 = "tf.ReadVariableOp"(%handle_1) {device = ""} : (tensor<!tf.resource<tensor<1x10xi64>>>) -> tensor<1x10xi64>
    return %2, %3 : tensor<1x10xf32>, tensor<1x10xi64>
  }

  // CHECK: %[[RESOURCE:.*]] = "tfl.var_handle_op"() {container = "c", shared_name = "a"} : () -> tensor<!tf.resource<tensor<1x10xf32>>>
  // CHECK: %[[RESOURCE_1:.*]] = "tfl.var_handle_op"() {container = "c", shared_name = "b"} : () -> tensor<!tf.resource<tensor<1x10xi64>>>
  // CHECK: %[[VAR_VAL:.*]] = "tfl.read_variable"(%[[RESOURCE]]) : (tensor<!tf.resource<tensor<1x10xf32>>>) -> tensor<1x10xf32>
  // CHECK: %[[ADD:.*]] = tfl.add %[[VAR_VAL]], %arg0 {fused_activation_function = "NONE"} : tensor<1x10xf32>
  // CHECK: "tfl.assign_variable"(%[[RESOURCE]], %[[ADD]]) : (tensor<!tf.resource<tensor<1x10xf32>>>, tensor<1x10xf32>) -> ()
  // CHECK: %[[RESULT:.*]] = "tfl.read_variable"(%[[RESOURCE]]) : (tensor<!tf.resource<tensor<1x10xf32>>>) -> tensor<1x10xf32>
  // CHECK: %[[VAR:.*]] = "tfl.read_variable"(%[[RESOURCE_1]]) : (tensor<!tf.resource<tensor<1x10xi64>>>) -> tensor<1x10xi64>
...
module attributes {tf_saved_model.semantics} {
  ...
  attributes {tf_saved_model.exported_names = ["__tf_saved_model_session_initializer"]} {
    ...
    "tf.AssignVariableOp"(%handle_0, %cst_1) : (tensor<!tf.resource<tensor<1x10xf32>>>, tensor<1x10xf32>) -> ()
    return
    // CHECK: %[[CST:.*]] = constant dense<1.000000e+00> : tensor<1x10xf32>
    // CHECK: %[[RESOURCE:.*]] = "tfl.var_handle_op"() {container = "c", shared_name = "a"} : () -> tensor<!tf.resource<tensor<1x10xf32>>>
    // CHECK: "tfl.assign_variable"(%[[RESOURCE]], %[[CST]]) : (tensor<!tf.resource<tensor<1x10xf32>>>, tensor<1x10xf32>) -> ()
  }
...
module attributes {tf_saved_model.semantics} {
  func @serving_default() ->
    ...
    %handle_0 = "tf.VarHandleOp"() {container="c", shared_name="a"} : () -> tensor<!tf.resource<tensor<1x10xui64>>>
    %0 = "tf.ReadVariableOp"(%handle_0) {device = ""} : (tensor<!tf.resource<tensor<1x10xui64>>>) -> tensor<1x10xui64>
...
module attributes {tf_saved_model.semantics, tfl._legalize_tfl_variables = true} {
  func @serving_default() ->
    ...
    "tf.AssignVariableOp"(%handle_0, %cst) : (tensor<!tf.resource<tensor<1x10xui64>>>, tensor<1x10xui64>) -> ()
    return
  }
