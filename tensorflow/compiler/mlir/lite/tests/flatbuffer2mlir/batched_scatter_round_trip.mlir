...
module {
  ...
  func.func public @main(%arg0: tensor<3x2x4x7x9xi32>, %arg1: tensor<4x3x5x2xi32>, %arg2: tensor<4x3x5x8xi32>) -> tensor<3x2x4x7x9xi32> {
    // CHECK-ROUNDTRIP:       %0 = "tfl.pseudo_const"() <{value = dense<[4, 3, 5, 1]> : tensor<4xi64>}> : () -> tensor<4xi64>
    // CHECK-ROUNDTRIP:       %1 = "tfl.pseudo_const"() <{value = dense<1> : tensor<i32>}> : () -> tensor<i32>
    // CHECK-ROUNDTRIP:       %2 = "tfl.fill"(%0, %1) : (tensor<4xi64>, tensor<i32>) -> tensor<4x3x5x1xi32>
    // CHECK-ROUNDTRIP:       %3 = "tfl.pseudo_const"() <{value = dense<{{\[\[\[\[}}0]], {{\[\[}}1]], {{\[\[}}2]]]]> : tensor<1x3x1x1xi32>}> : () -> tensor<1x3x1x1xi32>
    // CHECK-ROUNDTRIP:       %[[iota_1:.*]] = tfl.mul(%2, %3) <{fused_activation_function = "NONE"}> : (tensor<4x3x5x1xi32>, tensor<1x3x1x1xi32>) -> tensor<4x3x5x1xi32>
    // CHECK-ROUNDTRIP:       %5 = "tfl.pseudo_const"() <{value = dense<{{\[\[\[\[}}0]]], {{\[\[\[}}1]]], {{\[\[\[}}2]]], {{\[\[\[}}3]]]]> : tensor<4x1x1x1xi32>}> : () -> tensor<4x1x1x1xi32>
    // CHECK-ROUNDTRIP:       %[[iota_2:.*]] = tfl.mul(%2, %5) <{fused_activation_function = "NONE"}> : (tensor<4x3x5x1xi32>, tensor<4x1x1x1xi32>) -> tensor<4x3x5x1xi32>
    ...
    // CHECK-ROUNDTRIP:       %[[scatter:.*]] = "stablehlo.scatter"(%arg0, %7, %arg2) <{
