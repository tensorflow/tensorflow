...
Value CreateInt32ConstOrCast(Value val, Location loc,
  ...
  if (cast<ShapedType>(val.getType()).hasStaticShape()) {
...
Value GetShape(Value input, Location loc, PatternRewriter& rewriter) {
  auto shaped_type = cast<ShapedType>(input.getType());
...
LogicalResult ConvertTFMatMulOp::matchAndRewrite(
  ...
  auto transpose = [&](Value input) -> std::pair<LogicalResult, Value> {
    RankedTensorType type = dyn_cast_or_null<RankedTensorType>(input.getType());
...
bool ConvertTFMatrixDiagV2orV3(Operation* op, PatternRewriter* rewriter) {
  ...
  mlir::Type element_type =
      cast<ShapedType>(padding_value.getType()).getElementType();
...
struct LegalizeUnidirectionalSequenceLstm : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation* op,
    ...
    for (auto index_attr : tflite_indices_attr.getValue()) {
      IntegerAttr index = cast<IntegerAttr>(index_attr);
...
class ApplyExplicitBroadcasting : public OpRewritePattern<SourceOp> {
 public:
  ...
  LogicalResult rewriteOpWithDynamicInput(Operation* op,
    ...
    if (!cast<ShapedType>(lhs.getType()).hasRank() ||
        !cast<ShapedType>(rhs.getType()).hasRank()) {
      return failure();
    }
    if (!OpTrait::util::getBroadcastedShape(
            cast<ShapedType>(lhs.getType()).getShape(),
  ...
  LogicalResult matchAndRewrite(SourceOp src_op,
    ...
    if (!cast<ShapedType>(lhs.getType()).hasStaticShape() ||
        !cast<ShapedType>(rhs.getType()).hasStaticShape()) {
      return rewriteOpWithDynamicInput(op, rewriter);
    }

    auto lhs_shape = cast<ShapedType>(lhs.getType()).getShape();
    auto rhs_shape = cast<ShapedType>(rhs.getType()).getShape();
...
class ApplyExplicitBroadcasting<TF::SelectV2Op>
 ...
 public:
  ...
  LogicalResult rewriteOpWithDynamicInput(Operation* op,
    ...
    if (!cast<ShapedType>(lhs.getType()).hasRank() ||
        !cast<ShapedType>(rhs.getType()).hasRank() ||
        !cast<ShapedType>(cond.getType()).hasRank()) {
      return failure();
    }
    ...
    SmallVector<int64_t, 4> symbolic_broadcast_shape;
    if (!OpTrait::util::getBroadcastedShape(
            cast<ShapedType>(cond.getType()).getShape(),
  ...
  LogicalResult matchAndRewrite(TF::SelectV2Op src_op,
    ...
    if (!cast<ShapedType>(lhs.getType()).hasStaticShape() ||
        !cast<ShapedType>(rhs.getType()).hasStaticShape() ||
        !cast<ShapedType>(cond.getType()).hasStaticShape()) {
      return rewriteOpWithDynamicInput(op, rewriter);
    }

    auto lhs_shape = cast<ShapedType>(lhs.getType()).getShape();
    auto rhs_shape = cast<ShapedType>(rhs.getType()).getShape();
    auto cond_shape = cast<ShapedType>(cond.getType()).getShape();
