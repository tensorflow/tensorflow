...
class CheckRangeAndConvertI8ToI4 : public OpRewritePattern<arith::ConstantOp> {
 public:
  ...
  LogicalResult matchAndRewrite(arith::ConstantOp op,
    ...
    auto const_type = dyn_cast<ShapedType>(op.getType());
    if (!const_type || !const_type.getElementType().isSignlessInteger(8)) {
      return failure();
    }

    auto attr = cast<ElementsAttr>(op.getValue());
    ...
    auto newAttr = DenseElementsAttr::getFromRawBuffer(
        shaped_type, cast<DenseElementsAttr>(op.getValue()).getRawData());
...
class SanitizeGatherOpOutputToI4 : public OpRewritePattern<TFL::GatherOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TFL::GatherOp op,
                                PatternRewriter &rewriter) const override {
    auto const_type = dyn_cast<ShapedType>(op.getOperand(0).getType());
    auto result_type = dyn_cast<ShapedType>(op.getResult().getType());
    ...
    auto new_gather_op = rewriter.create<TFL::GatherOp>(
        op.getLoc(),
        /*result=*/
        cast<TensorType>(op.getResult().getType()).clone(builder.getI4Type()),
