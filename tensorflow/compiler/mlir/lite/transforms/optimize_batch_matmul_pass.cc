...
struct ConvertBatchMatMulOp2FullyConnectedOp
  ...
  LogicalResult matchAndRewrite(TFL::BatchMatMulOp bmm_op,
    ...
    // Check if RHS is a quantized int4 or int8 type. TFL doesn't support
    // BatchMatMul with int4 quantized types, so we should not convert it to FC.
    bool is_int4_or_int8_quantized_rank_2_value = false;
    if (auto rhs_type = mlir::dyn_cast<quant::QuantizedType>(
      ...
      if ((rhs_type.getStorageTypeIntegralWidth() == 4 ||
           rhs_type.getStorageTypeIntegralWidth() == 8) &&
          rhs_type.isSigned() && rhs_type_rank == 2) {
        is_int4_or_int8_quantized_rank_2_value = true;
      }
    }
    ...
    if (!is_rank_2_constant && !is_int4_or_int8_quantized_rank_2_value) {
