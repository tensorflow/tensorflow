...
ElementsAttr FlattenTo1D(Attribute a) {
  auto elements = cast<DenseElementsAttr>(a);
...
bool L2NormalizeReduceAxis(Value sq_op, DenseElementsAttr axis) {
  ...
  if (cast<ShapedType>(sq_op.getType()).getRank() - 1 ==
          *axis.getValues<int>().begin() ||
      *axis.getValues<int>().begin() == -1) {
    return true;
  }
  if (cast<ShapedType>(sq_op.getType()).getRank() != axis.getNumElements()) {
    return false;
  }
  auto shape = cast<ShapedType>(sq_op.getType());
...
bool BroadcastDimsProductEqual(Value input, Value output,
  ...
  ArrayRef<int64_t> output_shape =
      cast<ShapedType>(output.getType()).getShape();
...
bool OperandsBroadcastToOutputType(Type a, Type b, Type expected_output) {
  Type output_element_type = cast<ShapedType>(expected_output).getElementType();
...
bool IsTailOfShape(Type type1, Type type2) {
  auto tail_type = dyn_cast<ShapedType>(type1);
  auto full_type = dyn_cast<ShapedType>(type2);
...
bool IsLastDimEqualToNumElements(Type type1, Type type2) {
  return (cast<ShapedType>(type1).getRank() >= 1 &&
          cast<ShapedType>(type1).getDimSize(cast<ShapedType>(type1).getRank() -
                                             1) ==
              cast<ShapedType>(type2).getNumElements());
}
...
bool CanFuseConvOrDepthwiseConv(Value filter, Attribute val,
                                bool is_depthwise) {
  const auto elements = dyn_cast<DenseElementsAttr>(val);
  if (!elements) {
    return false;
  }
  const auto elements_shape = elements.getType().getShape();
  const auto filter_shape = cast<ShapedType>(filter.getType()).getShape();
...
bool CanFuseConvOrDepthwiseConv(Attribute filter, Attribute val,
                                bool is_depthwise) {
  if (const auto elements = dyn_cast<DenseElementsAttr>(val)) {
    if (const auto filter_elements = dyn_cast<DenseElementsAttr>(filter)) {
...
bool CanOptimizeIdentityGatherNdOrScatterNdOp(Value params,
                                              ...
                                              Type output_type) {
  auto params_type = dyn_cast<RankedTensorType>(params.getType());
  auto indices_type = dyn_cast<RankedTensorType>(indices.getType());
...
bool CanOptimizeIdentitySliceOp(Value input, Attribute begin, Attribute size) {
  ...
  auto begin_attr = dyn_cast<DenseIntElementsAttr>(begin);
  auto size_attr = dyn_cast<DenseIntElementsAttr>(size);
...
ElementsAttr ExpandTo4DForConvImpl(Attribute a, bool is_depthwise) {
  auto elements = dyn_cast<DenseElementsAttr>(a);
...
static bool ShapeMatchesReduceWithKeepAxes(Value input,
  ...
  DenseIntElementsAttr axes_attr = dyn_cast_or_null<DenseIntElementsAttr>(axes);
  DenseIntElementsAttr shape_attr =
      dyn_cast_or_null<DenseIntElementsAttr>(shape);
...
bool IsConstantValueOf(mlir::TypedAttr value, T raw_value) {
  auto element_type = cast<ShapedType>(value.getType()).getElementType();

  if (isa<FloatType>(element_type)) {
    return FloatValueEquals(value, raw_value);
  } else if (isa<IntegerType>(element_type)) {
    auto int_attr = dyn_cast_or_null<DenseIntElementsAttr>(value);
...
bool IsF32Value(Value value) {
  return cast<ShapedType>(value.getType()).getElementType().isF32();
}
...
TypedAttr GetNumElementsOrOne(Type type) {
  auto shaped_type = cast<ShapedType>(type);
...
bool IsLastDimensionEqualOne(Value val) {
  const auto val_type = cast<ShapedType>(val.getType());
...
bool IsOneHotIndexAttribute(Attribute attr) {
  const auto dense_attr = dyn_cast_or_null<DenseIntElementsAttr>(attr);
  if (!dense_attr) {
    return false;
  }
...
Type GetEmbeddingLookupShape(Value lookup, Value value) {
  auto lookup_type = cast<ShapedType>(lookup.getType());
  if (!lookup_type.hasStaticShape()) {
    return nullptr;
  }
  auto value_type = cast<ShapedType>(value.getType());
...
TypedAttr ConvertSingleElementAttrToFloatAttr(Attribute attr) {
  const auto dense_fp_attr = dyn_cast_or_null<DenseFPElementsAttr>(attr);
  if (dense_fp_attr) {
  ...
  const auto dense_int_attr = dyn_cast<DenseIntElementsAttr>(attr);
...
struct SqueezeReshapesAroundBroadcastOp
  ...
  LogicalResult matchAndRewrite(TFL::BroadcastToOp tfl_broadcast_to_op,
    ...
    // dimensions.
    if (cast<ShapedType>(tfl_broadcast_to_op.getShape().getType())
            .getNumElements() < 6) {
    ...
    auto broadcast_output_shapetype =
        cast<ShapedType>(tfl_broadcast_to_op.getOutput().getType());
    ...
    llvm::SmallVector<int32_t, 6> new_reshape_shape_i32{
        cast<RankedTensorType>(inner_reshape_op.getOutput().getType())
            .getShape()
...
struct ConvertTFLBroadcastToMulOp
  ...
  LogicalResult matchAndRewrite(TFL::BroadcastToOp tfl_broadcast_to_op,
                                PatternRewriter &rewriter) const override {
    auto input_type =
        cast<ShapedType>(tfl_broadcast_to_op.getInput().getType());
    auto output_type =
        cast<ShapedType>(tfl_broadcast_to_op.getOutput().getType());
    auto shape_type =
        cast<ShapedType>(tfl_broadcast_to_op.getShape().getType());
    ...
    // I16.
    if (!(isa<BFloat16Type, Float32Type>(element_type) ||
...
struct FuseAddAndStridedSlice : public OpRewritePattern<TFL::StridedSliceOp> {
  ...
  LogicalResult matchAndRewrite(TFL::StridedSliceOp strided_slice_op,
    ...
    mlir::TensorType constant_val_type =
        cast<TensorType>(constant_val.getType());
    ...
    mlir::RankedTensorType end_type =
        dyn_cast<RankedTensorType>(strided_slice_op.getEnd().getType());
    ...
    DenseElementsAttr new_added_value =
        added_value.reshape(RankedTensorType::get(
            {num_dims},
            cast<ShapedType>(added_value.getType()).getElementType()));
...
struct FuseFullyConnectedAndAdd : public OpRewritePattern<TFL::AddOp> {
  ...
  LogicalResult matchAndRewrite(TFL::AddOp add_op,
    ...
    auto constant_val_type = cast<TensorType>(constant_val.getType());
    ...
    const bool is_none_bias = isa<NoneType>(bias.getType());
...
struct FuseAddAndFullyConnected
  ...
  LogicalResult matchAndRewrite(TFL::FullyConnectedOp fc_op,
    ...
    {
      auto addend_shape = cast<ShapedType>(add_op.getRhs().getType());
    ...
    if (!old_bias || isa<NoneType>(old_bias.getType())) {
...
struct FuseMulAndFullyConnected
  ...
  LogicalResult matchAndRewrite(TFL::FullyConnectedOp fc_op,
    ...
    {
      auto multiplier_shape = cast<ShapedType>(mul_op.getRhs().getType());
...
struct FuseFullyConnectedAndMul : public OpRewritePattern<TFL::MulOp> {
  ...
  LogicalResult matchAndRewrite(TFL::MulOp mul_op,
    ...
    auto filter_type = cast<ShapedType>(filter.getType());
...
struct FuseAffinOpAndMulWithQDQs : public OpRewritePattern<TFL::MulOp> {
  ...
  LogicalResult matchAndRewrite(TFL::MulOp mul_op,
    ...
    auto gamma_shape = cast<ShapedType>(gamma.getType());
...
struct FuseBinaryOpToFollowingAffineOp : public OpRewritePattern<AffineOpType> {
  ...
  LogicalResult matchAndRewrite(AffineOpType fc_op,
    ...
    if (llvm::isa<AddOp, SubOp>(binary_op)) {
      ...
      if (isa<NoneType>(bias.getType())) {  // none bias, a list of zeros
...
struct ScalarizeSplatConstantForBroadcastableOps
  ...
  LogicalResult matchAndRewrite(BinaryOpType binary_op,
    ...
    auto result_type = cast<ShapedType>(binary_op.getResult().getType());
    mlir::Value non_splat_operand =
        binary_op.getOperand(1 - kSplatOperandIndex);
    auto non_splat_operand_type = cast<ShapedType>(non_splat_operand.getType());
 ...
 private:
  ...
  bool IsScalarizableSplatConstant(mlir::Value value,
    ...
    if (isa<quant::UniformQuantizedPerAxisType>(element_type)) {
      return false;
    }
    if (IsScalar(value)) {
      return false;
  ...
  bool CanFuseAffineOp(Operation *affine_op, Operation *binary_op) const {
    ...
    auto is_fusable_type = [](Type t) {
      Type element_type = t;
      if (auto shaped_type = dyn_cast<ShapedType>(t)) {
...
struct ConvertTrivialTransposeOpToReshapeOp
  ...
  LogicalResult matchAndRewrite(TFL::TransposeOp transpose_op,
                                PatternRewriter &rewriter) const override {
    auto input_type = cast<ShapedType>(transpose_op.getInput().getType());
    auto output_type = cast<ShapedType>(transpose_op.getOutput().getType());
...
struct RemoveReshapeBeforeFullyConnected
  ...
  LogicalResult matchAndRewrite(TFL::FullyConnectedOp fully_connected_op,
    ...
    auto input_ty = dyn_cast<ShapedType>(input.getType());
    auto output_ty =
        dyn_cast<ShapedType>(fully_connected_op.getOutput()[0].getType());
    ...
    auto reshape_input_ty = dyn_cast<ShapedType>(reshape_input.getType());
...
struct RemoveReshapeAfterFullyConnected
  ...
  LogicalResult matchAndRewrite(TFL::ReshapeOp reshape_op,
    ...
    auto input_shape =
        cast<ShapedType>(fully_connected_op.getInput().getType());
    auto output_shape = cast<ShapedType>(fully_connected_op.getType(0));
    auto reshape_shape = cast<ShapedType>(reshape_op.getType());
...
struct FuseUnpackAndConcatToReshape
  ...
  LogicalResult matchAndRewrite(TFL::ConcatenationOp concat_op,
    ...
    auto output_type = cast<ShapedType>(concat_op.getType());
...
struct OptimizeTopK : public OpRewritePattern<TFL::TopKV2Op> {
  ...
  std::optional<int32_t> ComputeSliceK(Value value) const {
    ...
    auto input_ty = dyn_cast_or_null<ShapedType>(value.getType());
    auto result_ty = dyn_cast<ShapedType>(slice_op.getType());
  ...
  LogicalResult matchAndRewrite(TFL::TopKV2Op op,
    ...
    auto values_ty = dyn_cast<ShapedType>(values.getType());
    auto indices_ty = dyn_cast<ShapedType>(indices.getType());
...
struct FuseLogSoftmax : public OpRewritePattern<TFL::SubOp> {
  ...
  LogicalResult matchAndRewrite(TFL::SubOp sub_op,
    ...
    if (!sum_op || !sum_op.getKeepDims() ||
        !isSupportedAxis(
            sum_op.getAxes(),
            cast<ShapedType>(sum_op.getOperand(0).getType()).getRank())) {
      return failure();
    }
    if (!sum_op->hasOneUse()) {
      return failure();
    ...
    if (!reduce_max_op || !reduce_max_op->hasOneUse() ||
        ...
        !isSupportedAxis(reduce_max_op.getAxes(),
                         cast<ShapedType>(reduce_max_op.getOperand(0).getType())
                             .getRank())) {
      return failure();
    }
...
struct UndoBroadcastFullyConnectedBiasAddWithQDQs
  ...
  LogicalResult match(TFL::AddOp add_op) const override {
    ...
    auto bias_rank = cast<ShapedType>(bias_type).getRank();
  ...
  void rewrite(TFL::AddOp add_op, PatternRewriter &rewriter) const override {
    ...
    auto new_q_op_type =
        RankedTensorType::Builder(
            cast<RankedTensorType>(q_op.getResult().getType()))
            .setShape(cast<ShapedType>(new_bias_type).getShape());
    ...
    auto new_dq_op_type =
        RankedTensorType::Builder(
            cast<RankedTensorType>(dq_op.getResult().getType()))
            .setShape(cast<ShapedType>(new_bias_type).getShape());
