...
static Value CreateTFCastOpI32(OpBuilder *builder, Location loc, Value x,
                               BoolAttr truncate) {
  auto x_type = dyn_cast_or_null<ShapedType>(x.getType());
...
class ConvertTFConvOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto filter_type = dyn_cast<RankedTensorType>(filter.getType());
...
class ConvertTFConv2D : public ConvertTFConvOp<ConvertTFConv2D, TF::Conv2DOp> {
 ...
 private:
  ...
  Value legalizeFilter(PatternRewriter &rewriter, Location loc,
    ...
    auto filter_type = cast<RankedTensorType>(filter.getType());
    auto result_shape =
...
class ConvertTFDepthwiseConv2dNative
 ...
 public:
  ...
  TFL::DepthwiseConv2DOp createTFLOp(ConvertTFConvOpMatchState *state,
    ...
    auto multiplier = cast<RankedTensorType>(filter.getType()).getDimSize(3);
...
struct ConvertTFStridedSlice : public RewritePattern {
  ...
  LogicalResult RewriteEllipsisMask(Operation *op,
    ...
    auto begin_ranked_attr_type = dyn_cast<RankedTensorType>(begin.getType());
    ...
    auto end_ranked_attr_type = dyn_cast<RankedTensorType>(end.getType());
    ...
    auto stride_ranked_attr_type = dyn_cast<RankedTensorType>(stride.getType());
    ...
    RankedTensorType begin_type = cast<RankedTensorType>(begin.getType());
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto ranked_input_type =
        dyn_cast<RankedTensorType>(strided_slice_op.getInput().getType());
    ...
    auto begin_attr_type = dyn_cast<RankedTensorType>(begin_attr.getType());
    auto end_attr_type = dyn_cast<RankedTensorType>(end_attr.getType());
    auto strides_attr_type = dyn_cast<RankedTensorType>(strides_attr.getType());
...
struct FusedBatchNormV3Pat : public ::mlir::RewritePattern {
  ...
  ::mlir::LogicalResult matchAndRewrite(
...

    {
      ...
      if (!(((isa<::mlir::FloatAttr>(epsilon))) &&
            ((cast<::mlir::FloatAttr>(epsilon).getType().isF32())))) {
    ...
    int64_t last_dim = ShapedType::kDynamic;
    {
      auto is_last_dim_compatible = [](const Value &v, int64_t &last_dim) {
        auto v_type = dyn_cast_or_null<RankedTensorType>(v.getType());
    ...
    if (is_training.getValue()) {
      auto input_type = dyn_cast_or_null<RankedTensorType>(
...
struct ConvertRfftToRfft2d : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto fft_len_type = dyn_cast_or_null<ShapedType>(fft_len.getType());
    if (!fft_len_type) return failure();

    auto output_type =
        dyn_cast_or_null<RankedTensorType>(rfft_op.getResult().getType());
