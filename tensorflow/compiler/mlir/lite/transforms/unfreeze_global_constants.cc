...
class UnfreezeMutableGlobalTensorsPass
    : public PassWrapper<UnfreezeMutableGlobalTensorsPass,
                         OperationPass<ModuleOp>> {
 public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(UnfreezeMutableGlobalTensorsPass)

  explicit UnfreezeMutableGlobalTensorsPass() = default;

  UnfreezeMutableGlobalTensorsPass(
      const UnfreezeMutableGlobalTensorsPass& other)
      : UnfreezeMutableGlobalTensorsPass{} {}

  StringRef getArgument() const override {
    return "unfreeze-global-constant-tensors";
  }
...
using GlobalTensorUsesMap =
    std::map<func::FuncOp, std::vector<std::pair<GlobalTensorOp, size_t>>>;
...
func::FuncOp CreateInitializerFunc(ModuleOp module_op) {
  ...
  if (init_func) {
    ...
    init_func =
        builder.create<func::FuncOp>(loc, /*sym_name=*/"NoOp", func_type);
...
void UnfreezeMutableGlobalTensorsPass::runOnOperation() {
  ...
  if (!target_global_tensor_ops.empty()) {
    ...
    GlobalTensorUsesMap global_tensor_uses =
        CreateGlobalTensorUsesMap(module_op);
    for (auto& it : global_tensor_uses) {
      auto func = it.first;
      llvm::BitVector args_to_erase(func.getNumArguments());

      for (auto& [global_tensor, arg_idx] : it.second) {
    ...
    absl::c_for_each(target_global_tensor_ops,
                     [](auto global_tensor_op) { global_tensor_op.erase(); });
  }
}
