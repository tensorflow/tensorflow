...
LogicalResult CreateTflFusableOpCustomOptions(
  ...
  for (auto attr : attrs) {
    if (auto float_attr = dyn_cast_or_null<FloatAttr>(attr.second)) {
      fbb.Float(attr.first.data(), float_attr.getValue().convertToFloat());
    } else if (auto int_attr = dyn_cast_or_null<IntegerAttr>(attr.second)) {
      fbb.Int(attr.first.data(), int_attr.getInt());
    } else if (auto bool_attr = dyn_cast_or_null<BoolAttr>(attr.second)) {
  ...
  if (api_name.starts_with(kTFTextAPIPrefix) && enable_fuse_tftext) {
    ...
    for (auto attr_item : dict_attr) {
      ...
      if (attr_item.getName() == kTFLFusableOp &&
          dyn_cast<BoolAttr>(attr_item.getValue()).getValue()) {
        tfl_fusable_op = true;
      } else {
