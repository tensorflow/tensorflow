...
class FuseQDQ : public OpRewritePattern<mlir::TFL::DequantizeOp> {
  ...
  LogicalResult matchAndRewrite(mlir::TFL::DequantizeOp op,
    ...
    for (mlir::Operation* quantizing_op : quantizing_ops) {
      // TODO (majiddadashi) make sure the case of DQ -> Q -> Q is handled.
      ...
      for (const auto& enumerated_result :
        ...
        if (op_quant_type == OpQuantizationType::kSRQ) {
          ...
          if (num_observed_qs == 0) {
            int num_uses = std::distance(result.use_begin(), result.use_end());
            if (num_uses == 0) {
              op->getParentOp()->dumpPretty();
            }
            assert(num_uses == 1 &&
                   "if there are no observed scales, there must be only be one "
...
void FuseQDQPass::runOnOperation() {
  ...
  patterns.add<FuseQDQ, RemoveUnusedFQ, QuantizeConstPattern, FuseDqQToRequant,
               FuseQQToRequant, RemoveNoOpQ>(ctx);

  // TODO(majiddadashi): Bring over the fold trivial requant pattern from
  // post_quantize.
