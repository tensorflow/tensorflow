...
struct PropagatedQuantType {
  ...
  bool operator<(const PropagatedQuantType& other) const {
    return std::forward_as_tuple(!is_propagated, is_from_result) >
           std::forward_as_tuple(!other.is_propagated, other.is_from_result);
  }
};
...
std::optional<quant::QuantizedType> IsProducedByDQ(mlir::Value value) {
...
std::optional<quant::QuantizedType> IsDefiningQ(mlir::Value value) {
  if (!value.hasOneUse()) {
    return std::nullopt;
  }
