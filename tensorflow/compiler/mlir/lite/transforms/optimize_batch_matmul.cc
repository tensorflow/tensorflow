...
struct ConvertBatchMatMulOpToReduceSum
  ...
  LogicalResult matchAndRewrite(TFL::BatchMatMulOp bmm_op,
    ...
    llvm::ArrayRef<int64_t> lhs_shape =
        cast<RankedTensorType>(bmm_op.getX().getType()).getShape();
    ...
    llvm::ArrayRef<int64_t> rhs_shape =
        cast<RankedTensorType>(bmm_op.getY().getType()).getShape();
 ...
 private:
  bool SplatValueEquals(SplatElementsAttr float_or_int, double rhs) const {
    if (isa<DenseFPElementsAttr>(float_or_int)) {
      return cast<DenseFPElementsAttr>(float_or_int)
          .getSplatValue<APFloat>()
          .isExactlyValue(rhs);
    } else if (cast<DenseIntElementsAttr>(float_or_int)) {
