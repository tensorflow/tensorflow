...
LogicalResult ConvertTFDilatedConvOp<Conv2dOpTy>::matchAndRewrite(
  ...
  if (llvm::isa<TF::ExpandDimsOp>(producer_op)) {
    ...
    if (auto const_op =
      ...
      expand_axis = (*cast<DenseElementsAttr>(const_op.getValue())
                          .getValues<APInt>()
                          .begin())
                        .getSExtValue();
    ...
    int64_t squeeze_axis = cast<IntegerAttr>(squeeze_dims[0]).getInt();
    if (squeeze_axis < 0) {
  ...
  if (expand_op) {
    if (dyn_cast<RankedTensorType>(stb_op.getInput().getType()) == nullptr) {
      return rewriter.notifyMatchFailure(
  ...
  if (expand_op) {
    ...
    auto input_shape = cast<ShapedType>(stb_op.getInput().getType()).getShape();
    ...
    auto bts_output_shape =
        cast<ShapedType>(bts_op.getOutput().getType()).getShape();
