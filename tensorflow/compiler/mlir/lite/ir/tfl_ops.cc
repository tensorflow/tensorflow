...
void RunBinaryCalculation(llvm::ArrayRef<ElementT> lhs_data,
  ...
  for (int64_t result_index = 0; result_index < num_elements; ++result_index) {
    ...
    // the index in the flat representation of the original tensor
    // to use.
    const int64_t lhs_index = GetElementIndex(lhs_shape, current_index);
    const int64_t rhs_index = GetElementIndex(rhs_shape, current_index);

    if constexpr (std::is_same_v<ElementT, float>) {
      result_data[result_index] =
          calculate(APFloat(rhs_data[rhs_index]), APFloat(lhs_data[lhs_index]))
              .convertToFloat();
    } else {
      result_data[result_index] =
          calculate(APInt(bit_width, rhs_data[rhs_index], is_signed),
                    APInt(bit_width, lhs_data[lhs_index], is_signed))
              .getSExtValue();
    }
...
Attribute ConstFoldBinaryOpDenseResources(ShapedType result_type,
                                          DenseResourceElementsAttr lhs,
                                          DenseResourceElementsAttr rhs,
  ...
  auto lhs_data_handle = lhs.getRawHandle();
  auto rhs_data_handle = rhs.getRawHandle();
  AsmResourceBlob* lhs_blob = lhs_data_handle.getBlob();
  AsmResourceBlob* rhs_blob = rhs_data_handle.getBlob();
  if (!lhs_blob || !rhs_blob) return {};

  llvm::StringRef lhs_blob_name = lhs_data_handle.getKey();
  llvm::StringRef rhs_blob_name = rhs_data_handle.getKey();

  type = type.clone(result_type.getElementType());

  // Flags to check if we can rewrite the LHS or RHS.
  bool can_rewrite_lhs =
      (lhs.getType().getShape() == result_type.getShape()) && lhs_has_one_use;
  bool can_rewrite_rhs =
      (rhs.getType().getShape() == result_type.getShape()) && rhs_has_one_use;

  ArrayRef<ElementT> lhs_data = lhs_blob->getDataAs<ElementT>();
  ArrayRef<ElementT> rhs_data = rhs_blob->getDataAs<ElementT>();
  if (can_rewrite_lhs) {
    RunBinaryCalculation(
        lhs_data, rhs_data,
        mlir::MutableArrayRef<ElementT>(const_cast<ElementT*>(lhs_data.data()),
                                        lhs_data.size()),
        lhs.getType(), rhs.getType(), type, calculate);

    return DenseResourceElementsAttr::get(
        result_type, lhs_blob_name.str() + "_binary_fold_result",
        std::move(*lhs_blob));
    ...
    return DenseResourceElementsAttr::get(
        result_type, rhs_blob_name.str() + "_binary_fold_result",
        std::move(*rhs_blob));
  } else {
      ...
      switch (bit_width) {
        case 32:
          return ConstFoldBinaryOpDenseResources<
              ...
              type,
              mlir::dyn_cast_or_null<DenseResourceElementsAttr>(lhs_elements),
              mlir::dyn_cast_or_null<DenseResourceElementsAttr>(rhs_elements),
              operands[0].hasOneUse(), operands[1].hasOneUse(),
              float_calculate);
        default:
          return {};
      }
      ...
      switch (bit_width) {
        case 32:
          return ConstFoldBinaryOpDenseResources<
              ...
              mlir::dyn_cast_or_null<DenseResourceElementsAttr>(lhs_elements),
              mlir::dyn_cast_or_null<DenseResourceElementsAttr>(rhs_elements),
              operands[0].hasOneUse(), operands[1].hasOneUse(), int_calculate);
        default:
          return {};
      }
    }
...
OpFoldResult ReshapeOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto dense_elements =
    ...
    if (AsmResourceBlob* blob =
      ...
      return DenseResourceElementsAttr::get(result_type, key, std::move(*blob));
    }
  }
...
OpFoldResult TransposeOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto dense_elements =
    ...
    // permutation.
    llvm::OwningArrayRef<uint64_t> current_input_index(input_type.getRank());

    // Allocate raw data and retrieve address of the first char in its raw
    // buffer.
    llvm::OwningArrayRef<char> raw_output_arr(dense_elements.getRawData());
    ...
    const bool valid_output_buffer = DenseElementsAttr::isValidRawBuffer(
        input_type, raw_output_arr, detected_splat);
    ...
    if (AsmResourceBlob* blob =
      ...
      auto raw_output_blob =
          mlir::HeapAsmResourceBlob::allocateAndCopyInferAlign(
              llvm::ArrayRef<char>(blob->getData().data(),
                                   blob->getData().size()));
      ...
      auto result_type =
          RankedTensorType::get(output_shape, input_tensor.getElementType());
      dense_resource_elements.getRawHandle().getResource()->setBlob({});
