...
void buildComparisonBinOp(Builder *builder, OperationState &result,
...
LogicalResult Verify(ConcatenationOp op) {
  ...
  for (ValuePtr operand : op.values())
...
OpFoldResult ConcatenationOp::fold(ArrayRef<Attribute> operands) {
  ...
  SmallVector<ValuePtr, 4> non_empty_values;
  for (ValuePtr value : this->values()) {
...
static void BuildGatherOp(Builder *builder, OperationState &result,
                          ValuePtr params, ValuePtr indices, IntegerAttr axis) {
...
static LogicalResult Verify(PackOp op) {
  ...
  ValuePtr operand0 = op.getOperand(0);
...
struct RemoveRedundantUnpackPack : public RewritePattern {
  ...
  PatternMatchResult matchAndRewrite(Operation *op,
    ...
    for (auto input_output :
      ...
      ValuePtr pack_input = std::get<0>(input_output);
      ValuePtr unpack_output = std::get<1>(input_output);
...
static void BuildTopKOp(Builder *builder, OperationState &result,
                        ValuePtr input, ValuePtr k) {
...
static llvm::Optional<int64_t> ExtractConstantIntFromTensor(ValuePtr value) {
  ElementsAttr attr;
...
static LogicalResult VerifySplitOpOutputTypes(
  ...
  for (int64_t i = 0; i < num_splits; ++i) {
    ...
    ValuePtr output = op->getResult(i);
...
static void BuildSelectV2Op(Builder *builder, OperationState &result,
                            ValuePtr cond, ValuePtr x, ValuePtr y) {
  auto operand_type =
