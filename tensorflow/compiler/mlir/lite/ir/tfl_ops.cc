...
// Mutex to guard the computations of the permutation.
// This is needed because the computation of the permutation is not thread
// safe.
...
Attribute ConstFoldBinaryOpDenseResources(ShapedType result_type,
  ...
  if (can_rewrite_lhs && lhs_is_resource) {
  ...
  } else if (can_rewrite_rhs && rhs_is_resource) {
