...
#include "tensorflow/compiler/mlir/lite/python/model_utils_core/mlir_adapter.h"
...
class MuMlirObject : public MuContainer {
 public:
  explicit MuMlirObject(T&& data) : MuContainer(std::forward<T>(data)) {}
...
class MuMlirAttribute : public MuMlirObject<mlir::Attribute> {
 public:
  explicit MuMlirAttribute(mlir::Attribute attr)
      : MuMlirObject(std::move(attr)) {
...
class MuMlirType : public MuMlirObject<mlir::Type> {
 public:
  explicit MuMlirType(mlir::Type type) : MuMlirObject(std::move(type)) {
...
#undef DEFINE_PY_MLIR_OBJECT
  ...
  mu.def("module_python_to_mlir", [](py::object py_module_op) {
    assert_ir_context_set();
    ...
    build_op = [&](py::handle py_op,
      ...
      if (py::hasattr(py_op, "attributes") &&
        ...
        for (auto [py_name, py_attr] : py_attributes) {
          ...
          attributes.push_back(mlir::NamedAttribute(name, *wrapped_attr.get()));
        }
      }
    ...
    absl::flat_hash_map<PyObject*, mlir::Value> mapping;
    mlir::Operation* op = build_op(py_module_op, mapping);
    mlir::ModuleOp module_op = mlir::cast<mlir::ModuleOp>(op);
    return MuMlirOperation(std::move(module_op));
  });

  mu.def("module_mlir_to_python", [](MlirTransformRegistry& mlir_transforms,
                                     py::object region_cls,
                                     py::object block_cls,
                                     MuMlirOperation& module_op_) {
    assert_ir_context_set();
    mlir::Operation* module_op = *module_op_.get();
    ...
    llvm::DenseMap<mlir::Value, py::object> mapping;
    return build_op(*module_op, mapping);
  });
}
