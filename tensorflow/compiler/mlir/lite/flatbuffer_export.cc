...
static StatusOr<tflite::TensorType> GetTFLiteType(Type type,
  ...
  } else if (isa<mlir::TF::Quint8Type>(type)) {
    return tflite::TensorType_UINT8;
...
static bool IsTFResourceOp(Operation* op) {
  for (const auto& operand : op->getOperands()) {
    ...
    if (isa<mlir::TF::ResourceType>(elementType)) {
      return true;
    }
  }
...
static std::string GetOpDescriptionForDebug(Operation* inst) {
  ...
  if (!inst->getAttrDictionary().empty()) {
    ...
    for (auto& named_attr : inst->getAttrDictionary()) {
      ...
      if (auto element_attr = dyn_cast<ElementsAttr>(named_attr.getValue())) {
...
static bool HasValidTFLiteType(Value value, T& error_handler) {
  ...
  if (isa<NoneType>(value.getType())) return true;
  ...
  for (auto fn : module.getOps<FuncOp>()) {
    ...
    for (auto arg : bb.getArguments()) {
      if (!HasValidTFLiteType(arg, fn)) {
        ...
        if (isa<mlir::TF::VariantType>(elementType)) {
          return fn.emitError(
    ...
    for (auto& inst : bb) {
      ...
      for (auto result : inst.getResults()) {
        if (isa<mlir::TFL::ControlType>(result.getType())) continue;
...
std::optional<BufferOffset<tflite::Buffer>> Translator::BuildBuffer(
  ...
  if (auto cst = dyn_cast<mlir::arith::ConstantOp>(inst)) {
    ...
    // TFLite module.
    attr = cast<ElementsAttr>(cst.getValue());
    ...
    mlir::VhloToStablehloTypeConverter vhlo_type_converter;
    auto tensor_v1_attr = cast<mlir::vhlo::TensorV1Attr>(cst.getValue());
...
Translator::BuildTFVariantType(mlir::Type element_type) {
  ...
  auto variant_type = dyn_cast<mlir::TF::VariantType>(element_type);
  if (!variant_type) {
...
std::optional<BufferOffset<tflite::Tensor>> Translator::BuildTensorFromType(
  ...
  if (auto qtype = dyn_cast<mlir::quant::UniformQuantizedType>(element_type)) {
    ...
    q_params = tflite::CreateQuantizationParameters(
        ...
        builder_.CreateVector<int64_t>(zero_points));
                 ...
                 dyn_cast<mlir::quant::CalibratedQuantizedType>(element_type)) {
...
std::optional<BufferOffset<tflite::Tensor>> Translator::BuildTensor(
  ...
  if (type.hasStaticShape()) {
    ...
    // its attribute type.
    auto tensor_attr = cast<mlir::TypedAttr>(inst->getAttr("value"));
    llvm::ArrayRef<int64_t> shape_ref =
        cast<TensorType>(tensor_attr.getType()).getShape();
    if (mlir::failed(check_shape(shape_ref))) return std::nullopt;
  ...
  if (auto qtype = dyn_cast<mlir::quant::UniformQuantizedType>(element_type)) {
    ...
    q_params = tflite::CreateQuantizationParameters(
        ...
        builder_, /*min=*/0, /*max=*/0, builder_.CreateVector<float>(scales),
        builder_.CreateVector<int64_t>(zero_points));
  } else if (auto qtype = dyn_cast<mlir::quant::UniformQuantizedPerAxisType>(
                 element_type)) {
...
BufferOffset<tflite::Operator> Translator::BuildCustomOperator(
  ...
  const std::string attrs =
      cast<mlir::TFL::ConstBytesAttr>(op.getCustomOption()).getValue().str();
...
Translator::BuildStablehloPrecisionConfig(::mlir::ArrayAttr precisionConfig) {
  ...
  for (auto it = precisionConfig.begin(); it != precisionConfig.end(); it++) {
    precision_config_vec.push_back(static_cast<uint32_t>(
        (cast<mlir::stablehlo::PrecisionAttr>(*it)).getValue()));
  }
...
Translator::BuildVhloPrecisionConfigV1(
  ...
  for (auto it = values.begin(); it != values.end(); it++) {
    precision_config_vec.push_back(static_cast<uint32_t>(
        (cast<mlir::vhlo::PrecisionV1Attr>(*it)).getValue()));
  }
  return builder_.CreateVector(precision_config_vec);
}
...
std::optional<BufferOffset<tflite::Operator>> Translator::BuildVhloGatherV1Op(
  ...
  auto offset_dims = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(gather_op.getOffsetDims()),
      vhlo_type_converter));
  auto collapsed_slice_dims = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(gather_op.getCollapsedSliceDims()),
      vhlo_type_converter));
  auto start_index_map = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(gather_op.getStartIndexMap()),
      vhlo_type_converter));
  auto slice_sizes = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(gather_op.getSliceSizes()),
      vhlo_type_converter));

  auto gather_option = tflite::CreateStablehloGatherOptions(
      builder_, offset_dims, collapsed_slice_dims, start_index_map,
      cast<mlir::vhlo::IntegerV1Attr>(gather_op.getIndexVectorDim())
          .getValue()
          .getSExtValue(),
      slice_sizes,
      cast<mlir::vhlo::BooleanV1Attr>(gather_op.getIndicesAreSorted())
          .getValue());
...
std::optional<BufferOffset<tflite::Operator>> Translator::BuildVhloScatterV1Op(
  ...
  int64_t index_vector_dim =
      cast<mlir::vhlo::IntegerV1Attr>(scatter_op.getIndexVectorDim())
          .getValue()
          .getSExtValue();
  bool unique_indices =
      cast<mlir::vhlo::BooleanV1Attr>(scatter_op.getUniqueIndices()).getValue();
  bool indices_are_sorted =
      cast<mlir::vhlo::BooleanV1Attr>(scatter_op.getIndicesAreSorted())
          .getValue();

  auto update_window_dims = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(scatter_op.getUpdateWindowDims()),
      vhlo_type_converter));
  auto inserted_window_dims = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(scatter_op.getInsertedWindowDims()),
      vhlo_type_converter));
  auto scatter_dims_to_operand_dims = builder_.CreateVector(
      mlir::GetVector<int64_t>(cast<mlir::vhlo::TensorV1Attr>(
...
Translator::BuildVhloReduceWindowV1Op(
  ...
  auto window_dimensions = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(reduce_window_op.getWindowDimensions()),
      vhlo_type_converter));
  auto window_strides = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(reduce_window_op.getWindowStrides()),
      vhlo_type_converter));
  auto base_dilations = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(reduce_window_op.getBaseDilations()),
      vhlo_type_converter));
  auto window_dilations = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(reduce_window_op.getWindowDilations()),
      vhlo_type_converter));
  auto padding = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(reduce_window_op.getPadding()),
      vhlo_type_converter));
  auto& body = reduce_window_op.getBody();
  int32_t subgraph_index = UnnamedRegionToSubgraph(
      &body, tflite::BuiltinOperator_STABLEHLO_REDUCE_WINDOW);
...
Translator::BuildVhloRngBitGeneratorV1Op(
  ...
  uint32_t opcode_index = GetOpcodeIndex(
      op_name, tflite::BuiltinOperator_STABLEHLO_RNG_BIT_GENERATOR);
  tflite::RngAlgorithm algorithm = tflite::RngAlgorithm_DEFAULT;
  switch (cast<mlir::vhlo::RngAlgorithmV1Attr>(rng_op.getRngAlgorithm())
              .getValue()) {
...
std::optional<BufferOffset<tflite::Operator>> Translator::BuildVhloPadV1Op(
  ...
  auto edge_padding_low = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(pad_op.getEdgePaddingLow()),
      vhlo_type_converter));
  auto edge_padding_high = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(pad_op.getEdgePaddingHigh()),
      vhlo_type_converter));
  auto interior_padding = builder_.CreateVector(mlir::GetVector<int64_t>(
      cast<mlir::vhlo::TensorV1Attr>(pad_op.getInteriorPadding()),
      vhlo_type_converter));
    ...
    if (convert_stablehlo_) {
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::IotaOpV1>(inst)) {
        ...
        auto iota_option = tflite::CreateStablehloIotaOptions(
            builder_,
            cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getIotaDimension())
                .getValue()
                .getSExtValue());
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::DynamicSliceOpV1>(inst)) {
        ...
        auto slice_sizes = builder_.CreateVector(mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getSliceSizes()),
            vhlo_type_converter));
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::CompareOpV1>(inst)) {
        ...
        if (compare_type_attr)
          compare_type = static_cast<tflite::StablehloComparisonType>(
              cast<mlir::vhlo::ComparisonTypeV1Attr>(compare_type_attr)
                  .getValue());
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::ConcatenateOpV1>(inst)) {
        ...
        auto concat_option = tflite::CreateStablehloConcatenateOptions(
            builder_, cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getDimension())
                          .getValue()
                          .getSExtValue());
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::SliceOpV1>(inst)) {
        ...
        auto start_indices = builder_.CreateVector((mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getStartIndicesAttr()),
            vhlo_type_converter)));
        auto limit_indices = builder_.CreateVector(mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getLimitIndicesAttr()),
            vhlo_type_converter));
        auto strides = builder_.CreateVector(mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getStridesAttr()),
            vhlo_type_converter));
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::ConvolutionOpV1>(inst)) {
        ...
        auto window_strides = builder_.CreateVector(mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getWindowStrides()),
            vhlo_type_converter));
        auto padding = builder_.CreateVector(mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getPadding()),
            vhlo_type_converter));
        auto lhs_dialation = builder_.CreateVector(mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getLhsDilation()),
            vhlo_type_converter));
        auto rhs_dialation = builder_.CreateVector(mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getRhsDilation()),
            vhlo_type_converter));
        auto window_reversal = builder_.CreateVector(mlir::GetVector<bool>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getWindowReversal()),
            vhlo_type_converter));
        auto input_batch_dimension =
            cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getInputBatchDimension())
                .getValue()
                .getSExtValue();
        auto input_feature_dimension =
            cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getInputFeatureDimension())
                .getValue()
                .getSExtValue();
        ...
        auto output_batch_dimension =
            cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getOutputBatchDimension())
                .getValue()
                .getSExtValue();
        auto output_feature_dimension =
            cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getOutputFeatureDimension())
                .getValue()
                .getSExtValue();
        ...
        if (vhlo_op.getPrecisionConfig()) {
          precision_config = BuildVhloPrecisionConfigV1(
              dyn_cast<mlir::vhlo::ArrayV1Attr>(vhlo_op.getPrecisionConfig()));
        }
        ...
        auto convolution_option = tflite::CreateStablehloConvolutionOptions(
            ...
            cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getFeatureGroupCount())
                .getValue()
                .getSExtValue(),
            cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getBatchGroupCount())
                .getValue()
                .getSExtValue(),
            precision_config);
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::CustomCallOpV1>(inst)) {
        ...
        auto op_api_version = cast<mlir::vhlo::CustomCallApiVersionV1Attr>(
        ...
        auto call_target_name = builder_.CreateString(
            cast<mlir::vhlo::StringV1Attr>(vhlo_op.getCallTargetName())
                .getValue()
                .str());
        auto backend_config = builder_.CreateString(
            cast<mlir::vhlo::StringV1Attr>(vhlo_op.getBackendConfig())
                .getValue()
                .str());
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::ReduceOpV1>(inst)) {
        ...
        auto dimension = builder_.CreateVector(mlir::GetVector<int64_t>(
            cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getDimensions()),
            vhlo_type_converter));
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::DotGeneralOpV1>(inst)) {
        ...
        if (vhlo_op.getPrecisionConfig()) {
          precision_config = BuildVhloPrecisionConfigV1(
              cast<mlir::vhlo::ArrayV1Attr>(vhlo_op.getPrecisionConfig()));
        }
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::SortOpV1>(inst)) {
        ...
        auto sort_option = tflite::CreateStablehloSortOptions(
            builder_,
            cast<mlir::vhlo::IntegerV1Attr>(vhlo_op.getDimension())
                .getValue()
                .getSExtValue(),
            cast<mlir::vhlo::BooleanV1Attr>(vhlo_op.getIsStable()).getValue(),
      ...
      if (auto vhlo_op = llvm::dyn_cast<mlir::vhlo::TransposeOpV1>(inst)) {
        ...
        auto transpose_option = tflite::CreateStablehloTransposeOptions(
            ...
            builder_.CreateVector(mlir::GetVector<int64_t>(
                cast<mlir::vhlo::TensorV1Attr>(vhlo_op.getPermutation()),
                vhlo_type_converter)));
...
void Translator::InitializeNamesFromAttribute(FuncOp fn, bool* has_input_attr) {
  ...
  if (auto str = dyn_cast_or_null<mlir::StringAttr>(dict_attr.get("inputs"))) {
  ...
  if (auto str = dyn_cast_or_null<mlir::StringAttr>(dict_attr.get("outputs"))) {
...
Translator::GetQuantizationForQuantStatsOpOutput(
  ...
  auto layer_stats = cast<mlir::DenseFPElementsAttr>(stats_op.getLayerStats());
  ...
  mlir::DenseFPElementsAttr min_max_attr =
      axis_stats.has_value()
          ? cast<mlir::DenseFPElementsAttr>(axis_stats.value())
          : layer_stats;
  ...
  for (const auto& item : llvm::enumerate(bb)) {
    ...
    if (llvm::isa<mlir::TFL::LSTMOp, mlir::TFL::UnidirectionalSequenceLSTMOp>(
      ...
      for (const std::string& intermediate : intermediate_names) {
        ...
        if (auto attr = dyn_cast_or_null<mlir::TypeAttr>(intermediate_attr)) {
...
Translator::CreateMetadataVector() {
  ...
  if (dict_attr) {
    for (const auto& named_attr : dict_attr) {
      ...
      if (auto content = dyn_cast<StringAttr>(attr)) {
...
llvm::SmallVector<llvm::StringRef, 2> GetStringsFromAttrWithSeparator(
  ...
  if (auto str = dyn_cast_or_null<mlir::StringAttr>(attr.get(attr_key))) {
  ...
  for (const auto& arg_attr : dict_attrs) {
    ...
    for (const auto attr : attrs) {
      if (attr.getName() == attr_name) {
        auto array_attr = dyn_cast_or_null<mlir::ArrayAttr>(attr.getValue());
        if (!array_attr || array_attr.empty()) continue;
        auto string_attr = dyn_cast_or_null<mlir::StringAttr>(array_attr[0]);
...
std::vector<SignatureDefData> BuildSignaturedef(
  ...
  if (auto name_attr = dyn_cast_or_null<StringAttr>(exported_name[0]))
...
BufferOffset<tflite::SparsityParameters> Translator::BuildSparsityParameters(
  ...
  for (int i = 0; i < dim_size; i++) {
    const auto dim_metadata =
        dyn_cast<mlir::TFL::DimensionMetadataAttr>(s_attr.getDimMetadata()[i]);
