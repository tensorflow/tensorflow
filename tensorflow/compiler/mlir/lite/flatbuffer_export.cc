...
static bool IsUnsupportedLocation(const mlir::Location& loc) {
  return !mlir::isa<mlir::NameLoc, mlir::FileLineColLoc, mlir::CallSiteLoc,
                    mlir::FusedLoc>(loc);
}
...
uint32_t CreateLocation(
  ...
  if (mlir::isa<mlir::UnknownLoc>(mlir_location) ||
      IsUnsupportedLocation(mlir_location)) {
    location_map.insert(
...
std::vector<uint32_t> CreateOpLocation(
  ...
  if (auto fused_loc = mlir::dyn_cast<mlir::FusedLoc>(loc_to_serialize)) {
    if (auto str_attr =
        ...
        str_attr && str_attr.getValue() == "tflite.importer_wrapper") {
