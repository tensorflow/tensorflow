...
class Translator {
 ...
 private:
  ...
  // copy. As it is, this data will be copied atleast once to the flatbuffer. We
  // need to find a way to avoid this copy.
  absl::flat_hash_map<int, absl::string_view> buffer_data_map_;
  bool buffer_data_exported_ = false;
  // Tensors that need to be deleted after the flatbuffer is built. We're
  // currently using this to hold the tensors that are created from
  // DenseResourceElementsAttr or DenseElementsAttr and hold constant data.
...
std::optional<BufferOffset<tflite::Buffer>> Translator::BuildBuffer(
  ...
  if (tflite_element_type == tflite::TensorType_INT4) {
    ...
    if (use_buffer_offset_) {
      buffer_data_map_[index] = absl::string_view(
          reinterpret_cast<char*>(packed_buffer.data()), packed_buffer.size());
