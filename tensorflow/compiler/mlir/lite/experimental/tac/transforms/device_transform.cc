...
struct FoldQuantizedI32ToFloat : public OpRewritePattern<TFL::DequantizeOp> {
  ...
  LogicalResult matchAndRewrite(TFL::DequantizeOp dequant_op,
    ...
    auto output_type =
        dyn_cast_or_null<ShapedType>(dequant_op.getOutput().getType());
    if (!output_type || !output_type.getElementType().isF32()) return failure();

    auto input_type = dyn_cast<ShapedType>(input_dequant.getType());
    // TODO(renjieliu): support UniformQuantizedPerAxisType.
    auto q_type = dyn_cast_or_null<quant::UniformQuantizedType>(
    ...
    auto dequant_values =
        cast<DenseIntOrFPElementsAttr>(input_values)
