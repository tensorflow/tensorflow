...
void TacFilterPass::runOnOperation() {
  ...
  TacFilters* active_filters = tac_filters_;
      ...
      tac_filters {
        op_filter {
          op_name_pattern: "^test_op"
                                        ...
                                        &test_tac_filters);
    active_filters = &test_tac_filters;
  }
  ...
  std::sort(active_filters->mutable_tac_filters()->pointer_begin(),
            active_filters->mutable_tac_filters()->pointer_end(),
  ...
  for (const auto& tac_filter :
       llvm::enumerate(active_filters->tac_filters())) {
