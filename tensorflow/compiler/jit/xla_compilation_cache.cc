...
bool XlaCompilationCache::Signature::operator==(const Signature& other) const {
  ...
  for (int i = 0, iter_limit = arg_values.size(); i < iter_limit; ++i) {
...
Status XlaCompilationCache::BuildExecutable(
  ...
  for (int i = 0, iter_limit = result.xla_input_shapes.size(); i < iter_limit; ++i) {
...
static xla::StatusOr<std::unique_ptr<Graph>> CreateGraph(
  ...
  for (int64 i = 0, iter_limit = args.size(); i < iter_limit; ++i) {
    Node* node;
  ...
  for (int64 i = 0, iter_limit = result_types.size(); i < iter_limit; ++i) {
    Node* node;
...
Status XlaCompilationCache::CompileSingleOp(
  ...
  *name.mutable_attr() = def.attr();
  ...
  auto compile_op = [&](XlaCompiler* compiler,
    ...
    for (int i = 0, iter_limit = result_dtypes.size(); i < iter_limit; ++i) {
