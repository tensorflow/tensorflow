...
Status PreprocessDataEdgesBetweenOutsideCompilations(
  ...
  for (int i = 0, iter_limit = edges.size(); i < iter_limit; i++) {
    ...
    *new_def.mutable_input(dst_input) = placeholder_node->name();
    ...
    for (int j = i + 1, iter_limit = edges.size(); j < iter_limit; j++) {
...
Status PostprocessDataEdgesBetweenOutsideCompilations(
  ...
  for (auto n : placeholder_nodes) {
    ...
    for (int i = 0, iter_limit = data_edges.size(); i < iter_limit; i++) {
      ...
      *new_def.mutable_input(dst_input) =
      ...
      // `replace_node`.
      for (int j = i + 1, iter_limit = data_edges.size(); j < iter_limit; j++) {
