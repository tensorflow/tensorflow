...
absl::StatusOr<std::string> GetThunkSpecificInitializer(
    const xla::cpu::CompilationResultProto& proto,
  ...
  std::string initialize_constant_allocations = absl::StrCat(
      R"(
...
for (size_t i = 0; i < num_buffers() ; ++i) {
  if (!buffer_infos()[i].is_constant()) { continue; }

  const auto& [buffer_size, buffer_data] =
      embedded_constant_buffers[embedded_constant_buffers_idx++];

  CHECK(buffer_size == buffer_infos()[i].size());
  std::memcpy(buffer_table()[i], buffer_data, buffer_infos()[i].size());
}

CHECK(embedded_constant_buffers_idx == embedded_constant_buffers.size());
)");

  return initialize_constant_allocations;
}
...
#include "xla/types.h")";
  ...
  if (HasThunkKind(aot_thunks->proto().thunk_sequence(),
    ...
    absl::StrAppend(&runtime_specific_includes,
                    "\n#include "
  ...
  if (!rng_thunks.empty()) {
    absl::StrAppend(&runtime_specific_includes,
                    "\n#include "
                    "\"tensorflow/compiler/xla/backends/cpu/"
                    "runtime/rng_state_lib.h\"");
  }

  TF_ASSIGN_OR_RETURN(const std::string thunk_specific_initializer,
                      GetThunkSpecificInitializer(aot_thunks->proto(),
  ...
  if (codegen_opts.use_xla_nanort_runtime) {
    ...
    absl::StrAppend(&runtime_specific_includes,
                    ...
                    "xla_compiled_cpu_function_thunks.h\"");
    ...
    absl::string_view thunk_specific_run_impl_format = R"(
    bool Run() override {
      {{THUNK_SPECIFIC_RUN_IMPL}}
      return true;
  })";
    ...
    thunk_specific_run_impl =
        absl::StrReplaceAll(thunk_specific_run_impl_format,
                            {{"{{THUNK_SPECIFIC_RUN_IMPL}}", run_impl}});
    ...
    if (!rng_thunks.empty()) {
      std::string rng_states_format = R"(
  std::array<xla::cpu::RngState, {{NUM_RNG_STATES}}> rng_states_{
    {{RNG_STATES_VALUES}}
    };
  )";
      std::vector<std::string> rng_states_values;
      rng_states_values.reserve(rng_thunks.size());
      for (const auto& rng_thunk : rng_thunks) {
        rng_states_values.push_back(absl::StrCat(
            "xla::cpu::RngState(",
            rng_thunk.rng_get_and_update_state_thunk().delta(), ")"));
      }

      rng_states = absl::StrReplaceAll(
          rng_states_format,
          {{"{{NUM_RNG_STATES}}", absl::StrCat(rng_thunks.size())},
           {"{{RNG_STATES_VALUES}}", absl::StrJoin(rng_states_values, ",")}});
    }
  }
  ...
  std::vector<std::pair<std::string, std::string>> rewrites_thunks = {
      ...
      {"{{THUNK_SPECIFIC_INITIALIZER}}", thunk_specific_initializer},
      {"{{MAYBE_RUN_IMPL}}", thunk_specific_run_impl},
      {"{{EXECUTABLE_PROTO_GETTER}}", executable_proto_getter},
      {"{{NANORT_SPECIFIC_STATIC_DATA_SETTERS}}",
       nanort_specific_static_data_setters},
      {"{{IS_THUNK_MODE}}", "true"},
      {"{{COMPUTATION_CLASS_BASE}}", computation_class_base},
      {"{{MAYBE_RNG_STATES}}", rng_states},
