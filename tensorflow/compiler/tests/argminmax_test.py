    ...
    for dtype in self.float_types:
      ...
      for output_type in minmax_types:
        ...
        self._assertOpOutputMatchesExpected(
            ...
            axis=0,
            output_type=output_type,
            op_input=np.array([5,6,7,-1,4,5], dtype=dtype),
            expected=output_type(2))
        # NAN tests (ignore nan in tensor)
        self._assertOpOutputMatchesExpected(
            math_ops.argmax,
            axis=0,
            output_type=output_type,
    ...
    for dtype in self.complex_types:
      ...
      for output_type in minmax_types:
        ...
        self._assertOpOutputMatchesExpected(
            ...
            axis=0,
            output_type=output_type,
            op_input=np.array([1+2j, 10+3j, 27-2j, 27+0j, 3+30j, 4+5j], dtype=dtype),
            expected=output_type(3))
        # Axis is 0
        self._assertOpOutputMatchesExpected(
            math_ops.argmax,
            axis=0,
            output_type=output_type,
            op_input=np.array([[4+5j, 1-6j, 7+3j], [3-7j, 2+1j, 7+8j]], dtype=dtype),
        ...
        self._assertOpOutputMatchesExpected(
            ...
            axis=0,
            output_type=output_type,
            op_input=np.array([complex(2, np.nan),complex(1, np.nan)], dtype=dtype),
            expected=output_type(0))
        self._assertOpOutputMatchesExpected(
            math_ops.argmax,
            axis=0,
            output_type=output_type,
            op_input=np.array([complex(2, np.nan),complex(np.nan, 1)], dtype=dtype),
            expected=output_type(0))
        self._assertOpOutputMatchesExpected(
            math_ops.argmax,
            axis=0,
            output_type=output_type,
            op_input=np.array([complex(np.nan, np.nan),complex(np.nan, 1)], dtype=dtype),
            expected=output_type(0))
        self._assertOpOutputMatchesExpected(
            math_ops.argmax,
            axis=0,
            output_type=output_type,
            op_input=np.array([complex(np.nan, 1), complex(1, 1)], dtype=dtype),
            expected=output_type(1))
        # Inf test
        self._assertOpOutputMatchesExpected(
            math_ops.argmax,
            axis=0,
            output_type=output_type,
            op_input=np.array([np.complex(np.inf, np.inf),np.complex(0, 1)], dtype=dtype),
            expected=output_type(0))
        self._assertOpOutputMatchesExpected(
            math_ops.argmax,
            axis=0,
            output_type=output_type,
            op_input=np.array([np.complex(0, np.inf),np.complex(1, 1)], dtype=dtype),
            expected=output_type(1))
        self._assertOpOutputMatchesExpected(
            math_ops.argmax,
            axis=0,
            output_type=output_type,
            op_input=np.array([np.complex(np.inf, 3),np.complex(np.inf, 1)], dtype=dtype),
            expected=output_type(0))
        ...
        self._assertOpOutputMatchesExpected(
            ...
            axis=0,
            output_type=output_type,
            op_input=np.array([3+32j, 10+3j, 27-2j, 27+0j, 3+30j, 4+5j], dtype=dtype),
            expected=output_type(4))
        # Axis is 0
        self._assertOpOutputMatchesExpected(
            math_ops.argmin,
        ...
        self._assertOpOutputMatchesExpected(
            ...
            axis=0,
            output_type=output_type,
            op_input=np.array([np.complex(2, np.nan),np.complex(1, np.nan)], dtype=dtype),
            expected=output_type(0))
        self._assertOpOutputMatchesExpected(
            math_ops.argmin,
            axis=0,
            output_type=output_type,
            op_input=np.array([np.complex(2, np.nan),np.complex(np.nan, 1)], dtype=dtype),
            expected=output_type(0))
        self._assertOpOutputMatchesExpected(
            math_ops.argmin,
            axis=0,
            output_type=output_type,
            op_input=np.array([np.complex(np.nan, np.nan),np.complex(np.nan, 1)], dtype=dtype),
            expected=output_type(0))
        self._assertOpOutputMatchesExpected(
            math_ops.argmin,
            axis=0,
            output_type=output_type,
            op_input=np.array([np.complex(np.nan, 1), np.complex(1, 1)], dtype=dtype),
            expected=output_type(1))
        # Inf test
        self._assertOpOutputMatchesExpected(
            math_ops.argmin,
        ...
        self._assertOpOutputMatchesExpected(
            ...
            axis=0,
            output_type=output_type,
            op_input=np.array([np.complex(3, np.inf),np.complex(0, 1)], dtype=dtype),
            expected=output_type(1))
        self._assertOpOutputMatchesExpected(
            math_ops.argmin,
            axis=0,
