...
class GatherNdTest(xla_test.XLATestCase):
    ...
    def testIndexScalar(self):
        params = np.array(
            [[-8, -1, -2, -3, -7, -5], [8, 1, 2, 3, 7, 5]], dtype=np.float32
        ).T
    ...
    def testEmptyIndicesLastRankMeansCopyEntireTensor(self):
        ...
        gather_nd_val = self._runGather(params, indices)
        self.assertAllEqual(
            np.vstack((params[np.newaxis, :], params[np.newaxis, :])), gather_nd_val
        )
    ...
    def testHigherRankParamsAndIndicesLargerThanIndexSlices(self):
        ...
        gather_nd_val = self._runGather(params, indices)
        self.assertAllEqual(
            params[[3, 2, 1, 4, 4, 0]].reshape(2, 3, 2, 2), gather_nd_val
        )
    ...
    def testHigherRankParams(self):
        ...
        indices = np.vstack(
            [np.random.randint(0, s, size=2000, dtype=np.int32) for s in shape]
        ).T
        gather_nd_val = self._runGather(params, indices)
