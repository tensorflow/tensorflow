...
load("//tensorflow:strict.default.bzl", "py_strict_binary", "py_strict_test")
...
def tf_xla_combined_py_test(name = "", package = None, test_files = [], **kwargs):
    ...
    # build the combined test generator
    generate_name = name + "_generate_combined_xla_test"
    py_strict_binary(
        name = generate_name,
        testonly = 1,
        main = "xla_combined_test_generator.py",
        srcs = ["//tensorflow/compiler/tests:xla_combined_test_generator"],
    ...
    native.genrule(
        ...
        outs = [test_file],
        cmd = "mkdir -p $(@D) &&" +
              "$(locations %s) " % generate_name +
              "--test_files=" + ",".join(["$(locations :%s)" % f for f in test_files]) + " " +
              "--package=" + package + " " +
              "> $@",
        tools = [generate_name],
