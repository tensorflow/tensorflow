...
class StatelessRandomOpsTest(xla_test.XLATestCase, parameterized.TestCase):
    ...
    def testLargeNormal(self):
        ...
        with self.session() as sess, self.test_scope():
            ...
            key, counter, alg = (
                gen_stateless_random_ops_v2.stateless_random_get_key_counter_alg(seed_t)
            )
            ...
            for s0, v0 in values:
                for s1, v1 in values:
                    if s0 == s1:
                        ...
                        if not (
                            dtype in (dtypes.bfloat16, dtypes.float16) and shape == ()
    ...
    def testDistributionOfStatelessRandomUniform(self, alg, dtype, seed):
        ...
        if (
            device_type in ("CPU", "GPU")
            and alg in (philox, auto_select)
            and (dtype, seed) in bad_combos1
            or device_type == "TPU"
            and (
                alg == philox
                and (dtype, seed) in bad_combos1
                or alg == auto_select
                and (dtype, seed) in bad_combos2
            )
        ):
            self.skipTest(
    ...
    def _testParameterizedTruncatedNormal(
        self, means, stddevs, minvals, maxvals, variance_rtol=None
    ):
    ...
    def testParameterizedTruncatedNormalRightTail(self):
        ...
        self._testParameterizedTruncatedNormal(0.0, 1.0, 4.0, 20.0, variance_rtol=2e-2)

    def testParameterizedTruncatedNormalLeftTail(self):
        self.skipTest("b/276957102")
        self._testParameterizedTruncatedNormal(
            0.0, 1.0, -20.0, -4.0, variance_rtol=5e-2
        )

    def testParameterizedTruncatedNormalLeftTailTwoSidedBounds(self):
        self._testParameterizedTruncatedNormal(0.0, 1.0, -6.0, -3.0, variance_rtol=5e-2)

    def testParameterizedTruncatedNormalSmallStddev(self):
        self._testParameterizedTruncatedNormal(0.0, 0.1, 0.05, 0.10)
