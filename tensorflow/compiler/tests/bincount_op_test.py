  ...
  def testNegativeInputRuntimeIgnored(self):
    ...
    with self.session():
      with self.test_scope():
        ...
        def f(arr):
          return gen_math_ops.dense_bincount(input=arr, size=5, weights=[])
  ...
  def testNegativeInputRuntime2d(self):
    ...
    with self.session():
      with self.test_scope():
        ...
        def f(arr):
          return gen_math_ops.dense_bincount(input=arr, size=4, weights=[])

        # Row 0: [0, -1, 2] -> counts: [1, 0, 1, 0]
        # Row 1: [1, -3, 3] -> counts: [0, 1, 0, 1]
        result = self.evaluate(
            f(array_ops.constant([[0, -1, 2], [1, -3, 3]], dtype=dtypes.int32))
        )
