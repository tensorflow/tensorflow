...
class PoolingTest(xla_test.XLATestCase):
    def _VerifyOneTest(
        self, pool_func, input_sizes, ksize, strides, padding, data_format, expected
    ):
    ...
    def _VerifyValues(self, pool_func, input_sizes, ksize, strides, padding, expected):
        ...
        for data_format in GetTestConfigs():
            self._VerifyOneTest(
                pool_func, input_sizes, ksize, strides, padding, data_format, expected
            )
...
class PoolGradTest(xla_test.XLATestCase):
        ...
        with self.session() as sess:
            ...
            with ops.device(self.CPU_DEVICE):
                ...
                expected_input_gradient_vals = sess.run(
                    ...
                    {inputs: x, output_gradients: output_gradient_vals},
                )
                ...
                if pool_grad_grad_func is not None:
                    ...
                    expected_grad_gradients_vals = sess.run(
                        expected_grad_gradients,
                        {inputs: x, output_grad_gradients: output_grad_grad_vals},
                    )
            ...
            actual_input_gradients_vals = sess.run(
    ...
    def testAvgPool(self):
        ...
        # MaxPoolGrad.
        def AvgPoolGrad(
            inputs, outputs, output_gradients, ksize, strides, padding, data_format
        ):
