...
class StatefulRandomOpsTest(xla_test.XLATestCase, parameterized.TestCase):
    ...
    @parameterized.parameters(
        list(itertools.product(ALGS, INTS + FLOATS, (12, 13, 123, 4321)))
    )
    def testDistributionOfUniform(self, alg, dtype, seed):
        ...
        if (
            (alg, dtype, seed)
            in [(three_fry, dtypes.int64, 123), (three_fry, dtypes.uint64, 123)]
            or is_tpu
            and (alg, dtype, seed)
            in [(auto_select, dtypes.int64, 123), (auto_select, dtypes.uint64, 123)]
            or is_megacore
            and (alg, dtype, seed)
            in [
                (auto_select, dtypes.int32, 123),
