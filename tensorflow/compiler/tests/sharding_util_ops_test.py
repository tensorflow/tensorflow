...
class XlaSplitNDOpTest(xla_test.XLATestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        ("Tensor", create_tensor_split_graph), ("Resource", create_resource_split_graph)
    )
...
def create_resource_concat_graph(
    ...
    variable = resource_variable_ops.ResourceVariable(
        initial_value=np.zeros(variable_shape, dtype=input_dtype), dtype=input_dtype
    )
    sess.run(variables.variables_initializer([variable]))
    const_input_ops = [constant_op.constant(i, dtype=input_dtype) for i in input_values]
    concat = gen_tpu_ops.assign_variable_xla_concat_nd(
        variable.handle, const_input_ops, num_concats, paddings
    )
...
class XlaConcatNDOpTest(xla_test.XLATestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        ("Tensor", create_tensor_concat_graph),
    ...
    def testDifferentShapedInputs(self, graph_fn):
        for dtype in self.numeric_types:
            with self.session() as sess, self.device_scope():
                concat = graph_fn(
                    sess, input_values=[[0], [1, 2]], input_dtype=dtype, num_concats=[2]
                )
    ...
    def testRanked(self, graph_fn, rank):
        ...
        for dtype in self.numeric_types:
            with self.session() as sess, self.device_scope():
                ...
                result = sess.run(concat)
            expected_output = (
                np.arange(0, num_inputs).reshape(num_concats).astype(dtype)
            )
            self.assertAllClose(result, expected_output)
...
def create_tensor_roundtrip_graph(
    ...
    split = gen_tpu_ops.xla_split_nd(
        const_input_op, np.prod(num_partitions), num_partitions, paddings=paddings
    )
...
def create_resource_roundtrip_graph(
    ...
    concat = gen_tpu_ops.assign_variable_xla_concat_nd(
        variable.handle, split, num_partitions, paddings
    )
    with control_dependencies([concat]):
