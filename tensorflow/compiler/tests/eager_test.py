...
class EagerTest(xla_test.XLATestCase):
    ...
    def testShapeN(self):
        with self.test_scope():
            ...
            # Shapes of tensors created by op running on device
            shapes = array_ops.shape_n(
                [array_ops.ones([]), array_ops.ones([3]), array_ops.ones([2, 2])]
            )
            self.assertAllEqual([[], [3], [2, 2]], [x.numpy().tolist() for x in shapes])
    ...
    def testSize(self):
        with self.test_scope():
            ...
            self.assertEqual(
                4,
                array_ops.size(constant_op.constant([[1.0, 2.0], [3.0, 4.0]])).numpy(),
            )
    ...
    def testRank(self):
        with self.test_scope():
            ...
            self.assertEqual(
                2,
                array_ops.rank(constant_op.constant([[1.0, 2.0], [3.0, 4.0]])).numpy(),
            )
    ...
    def testAdamSparse(self):
        ...
        self.assertAllClose(
            [[2.0, 2.0], [1.9, 1.9], [2.0, 2.0]], embedding_matrix.numpy()
        )
...
class EagerFunctionTest(xla_test.XLATestCase):
    ...
    def testReturnResourceHandle(self):
        with self.test_scope():
            ...
            self.assertAllEqual(
                v.numpy(),
                resource_variable_ops.read_variable_op(handle, dtypes.float32).numpy(),
            )
