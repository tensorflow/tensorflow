...
class BucketizationOpTest(xla_test.XLATestCase):
    def testInt(self):
        with self.session() as sess:
            ...
            self.assertAllEqual(
                expected_out, sess.run(op, {p: [-5, 0, 2, 3, 5, 8, 10, 11, 12]})
            )
    ...
    def testFloat(self):
        with self.session() as sess:
            ...
            with self.test_scope():
                op = math_ops._bucketize(p, boundaries=[0.0, 3.0, 8.0, 11.0])
            expected_out = [0, 1, 1, 2, 2, 3, 3, 4, 4]
            self.assertAllEqual(
                expected_out,
                sess.run(op, {p: [-5.0, 0.0, 2.0, 3.0, 5.0, 8.0, 10.0, 11.0, 12.0]}),
            )
    ...
    def test2DInput(self):
        with self.session() as sess:
            ...
            self.assertAllEqual(
                expected_out, sess.run(op, {p: [[-5, 0, 2, 3, 5], [8, 10, 11, 12, 0]]})
            )
