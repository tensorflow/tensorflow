...
class CumsumTest(xla_test.XLATestCase):
    ...
    def testMixedPrecision(self):
        with self.session(), self.test_scope():
            y = math_ops.cumsum(
                constant_op.constant([1.0, 2.0, 3.0, 4.0], dtypes.bfloat16),
                -1,
                exclusive=True,
            ).eval()
...
class CumulativeLogSumExpTest(xla_test.XLATestCase):
    ...
    def _compare(self, x, axis, exclusive, reverse):
        ...
        np_out = handle_options(
            np.logaddexp.accumulate, neginf_like, x, axis, exclusive, reverse
        )
        with self.session(), self.test_scope():
            p = array_ops.placeholder(x.dtype)
            tf_out = math_ops.cumulative_logsumexp(p, axis, exclusive, reverse).eval(
                feed_dict={p: x}
            )
