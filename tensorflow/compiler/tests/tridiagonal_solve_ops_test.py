...
class TridiagonalSolveOpsTest(xla_test.XLATestCase):
    ...
    def testTridiagonalSolverSolves1Rhs(self):
        ...
        diagonals_np = np.random.normal(size=(batch_size, 3, num_dims)).astype(
            np.float32
        )
        ...
        with self.session() as sess, self.test_scope():
            ...
            x_np = sess.run(
                ...
                feed_dict={diags: diagonals_np, rhs: rhs_np},
            )[:, :, 0]
    ...
    def testTridiagonalSolverSolvesKRhs(self):
        ...
        rhs_np = np.random.normal(size=(batch_size, num_dims, num_rhs)).astype(
            np.float32
        )
    ...
    def _testWithDiagonalLists(
        self, diags, rhs, expected, diags_format="compact", transpose_rhs=False
    ):
        with self.session() as sess, self.test_scope():
            self.assertAllClose(
                sess.run(
                    linalg_impl.tridiagonal_solve(
                        [_tfconst(x) for x in diags],
                        _tfconst(rhs),
                        diags_format,
                        transpose_rhs,
                        conjugate_rhs=False,
    ...
    def testWithTwoBatchingDimensions(self):
        self._test(
            ...
            rhs=np.array(
                [
                    [_sample_rhs, 2 * _sample_rhs, 3 * _sample_rhs],
    ...
    def testMatrixFormatWithBatching(self):
        self._test(
            diags=[
                [[1, 2, 0, 0], [1, 3, 1, 0], [0, -1, 2, 4], [0, 0, 1, 2]],
                [[-1, -2, 0, 0], [-1, -3, -1, 0], [0, 1, -2, -4], [0, 0, -1, -2]],
            ],
            rhs=[[1, 2, 3, 4], [1, 2, 3, 4]],
            expected=[[-9, 5, -4, 4], [9, -5, 4, -4]],
    ...
    def _makeDataForGradientWithBatching(self):
        ...
        grad_diags = np.array([[-5, 0, 4, 0], [9, 0, -4, -16], [0, 0, 5, 16]]).astype(
            np.float32
        )
