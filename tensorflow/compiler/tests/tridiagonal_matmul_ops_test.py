...
class TridiagonalMatMulOpsTest(xla_test.XLATestCase):
    ...
    def _testAllFormats(self, superdiag, maindiag, subdiag, rhs, dtype=dtypes.float64):
        ...
        with self.test_scope():
            ...
            results_batch = [
                self._jit_tridiagonal_matmul(
                    diags_sequence_batch_op, rhs_batch_op, diagonals_format="sequence"
                ),
                self._jit_tridiagonal_matmul(
    ...
    def testNDimensional(self):
        with self.test_scope():
            ...
            expected = self._tridiagonal_matmul(
                (superdiag, maindiag, subdiag), rhs, diagonals_format="sequence"
            )

            real = self._jit_tridiagonal_matmul(
                (superdiag, maindiag, subdiag), rhs, diagonals_format="sequence"
            )
