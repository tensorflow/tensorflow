...
class ListOpsTest(parameterized.TestCase, xla_test.XLATestCase):
    ...
    def testPushPop(self):
        with self.session() as sess, self.test_scope():
            l = list_ops.empty_tensor_list(
                element_shape=(7, 15), element_dtype=dtypes.float32, max_num_elements=10
            )
    ...
    def testPushPopSeparateLists(self):
        with self.session() as sess, self.test_scope():
            l = list_ops.empty_tensor_list(
                element_shape=[], element_dtype=dtypes.float32, max_num_elements=20
            )
    ...
    def testEmptyTensorListNoMax(self):
        with self.session() as sess, self.test_scope():
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError, "Set the max number of elements"
            ):
    ...
    def testEmptyTensorListMax(self):
        with self.session() as sess, self.test_scope():
            l = list_ops.empty_tensor_list(
                element_shape=(10, 15), element_dtype=dtypes.float32, max_num_elements=2
            )
            l = list_ops.tensor_list_push_back(
    ...
    def testGetSetReservedNonScalar(self):
        with self.session() as sess, self.test_scope():
            l = list_ops.tensor_list_reserve(
                element_dtype=dtypes.float32, element_shape=(7, 15), num_elements=2
            )
    ...
    def testStack(self):
        with self.session(), self.test_scope():
            l = list_ops.empty_tensor_list(
                element_dtype=dtypes.float32, element_shape=[], max_num_elements=2
            )
            l = list_ops.tensor_list_push_back(l, constant_op.constant(1.0))
            e = list_ops.tensor_list_get_item(l, 0, element_dtype=dtypes.float32)
            self.assertAllEqual(e, 1.0)
...
if __name__ == "__main__":
    os.environ["TF_XLA_FLAGS"] = "--tf_xla_min_cluster_size=2 " + os.environ.get(
        "TF_XLA_FLAGS", ""
    )
    test.main()
