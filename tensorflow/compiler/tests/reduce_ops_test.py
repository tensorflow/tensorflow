...
@parameterized.named_parameters(
    ("32_bit_index", dtypes.int32), ("64_bit_index", dtypes.int64)
)
class ReduceOpsTest(xla_test.XLATestCase, parameterized.TestCase):
    ...
    def testReduceSumF32(self, index_dtype):
        self._testReduction(
            math_ops.reduce_sum, np.sum, np.float32, self.REAL_DATA, index_dtype
        )

    def testReduceSumC64(self, index_dtype):
        self._testReduction(
            math_ops.reduce_sum, np.sum, np.complex64, self.COMPLEX_DATA, index_dtype
        )

    def testReduceProdF32(self, index_dtype):
        self._testReduction(
            math_ops.reduce_prod, np.prod, np.float32, self.REAL_DATA, index_dtype
        )

    def testReduceProdC64(self, index_dtype):
        self._testReduction(
            math_ops.reduce_prod, np.prod, np.complex64, self.COMPLEX_DATA, index_dtype
        )
        ...
        def reference_min(dtype, inp, axis):
            ...
            if inp.shape[axis] == 0:
                if np.issubdtype(dtype, np.floating):
                    return np.full(
                        inp.shape[0:axis] + inp.shape[axis + 1 :], float("inf")
                    )
                return np.full(
                    inp.shape[0:axis] + inp.shape[axis + 1 :], np.iinfo(dtype).max
                )
            return np.amin(inp, axis)

        for dtype in set(self.all_types).intersection([np.float32, np.int32, np.int64]):
            self._testReduction(
                math_ops.reduce_min,
                functools.partial(reference_min, dtype),
                dtype,
                self.REAL_DATA,
                index_dtype,
            )
    ...
    def testReduceMax(self, index_dtype):
        def reference_max(dtype, inp, axis):
            ...
            if inp.shape[axis] == 0:
                ...
                return np.full(
                    inp.shape[0:axis] + inp.shape[axis + 1 :], np.iinfo(dtype).min
                )
        ...
        for dtype in set(self.all_types).intersection([np.float32, np.int32, np.int64]):
            self._testReduction(
                math_ops.reduce_max,
                functools.partial(reference_max, dtype),
                dtype,
                self.REAL_DATA,
                index_dtype,
            )
    ...
    def testReduceMeanF16(self, index_dtype):
        if np.float16 in self.all_types:
            self._testReduction(
                math_ops.reduce_mean, np.mean, np.float16, self.ONES, index_dtype
            )
    ...
    def testReduceAll(self, index_dtype):
        self._testReduction(
            math_ops.reduce_all, np.all, np.bool_, self.BOOL_DATA, index_dtype
        )

    def testReduceAny(self, index_dtype):
        self._testReduction(
            math_ops.reduce_any, np.any, np.bool_, self.BOOL_DATA, index_dtype
        )
...
class ReduceOpPrecisionTest(xla_test.XLATestCase):
    def _testReduceSum(self, expected_result, dtype, test_inputs, rtol=1e-3, atol=1e-4):
        ...
        for test_input in test_inputs:
            with self.session() as sess:
                with self.test_scope():
                    ...
                    out = math_ops.reduce_sum(a, index)
                result = sess.run(
                    out, {a: np.array(test_input, dtype=dtype), index: [0]}
                )
    ...
    def testReduceSumBF16(self):
        ...
        self._testReduceSum(
            dtypes.bfloat16.as_numpy_dtype(value),
