...
class TensorArrayTest(xla_test.XLATestCase):
    ...
    def _testTensorArraySplitRead(self, tf_dtype):
        with self.session() as session, self.test_scope():
            ...
            def fn():
                ...
                lengths = constant_op.constant([1, 1, 1])
                w0 = ta.split(
                    convert([[1.0, 101.0], [2.0, 121.0], [3.0, 127.0]]), lengths=lengths
                )
                r0 = w0.read(0)
                r1 = w0.read(1)
                r2 = w0.read(2)
    ...
    def testTensorArraySplitIncompatibleShapesFails(self):
        with self.session(), self.test_scope():
            ...
            with self.assertRaisesOpError(r"lengths must be equal: 1 vs. 2"):
                xla.compile(fn)[0].eval()
            ...
            with self.assertRaisesOpError(r"value must have rank >= 1"):
                xla.compile(fn)[0].eval()
    ...
    def _testTensorArrayWriteGradientAddMultipleAdds(self, dtype):
        with self.session(), self.test_scope():
            ...
            with self.assertRaisesOpError(r"Mismatched TensorArray sizes"):
                xla.compile(fn)[0].eval()
    ...
    def _testTensorArrayGradientWriteReadType(self, dtype):
        with self.session() as session, self.test_scope():
            ...
            [grad_just_r0_vals, grad_r0_r0_2_vals, grad_just_r1_vals, grad_vals] = (
                self.evaluate(xla.compile(fn))
            )
        ...
        with self.session() as session, self.test_scope():
            ...
            def fn():
                ...
                return gradients_impl.gradients(
                    ...
                    xs=[value],
                    grad_ys=[
                        [[2.0, -2.0], [20.0, -20.0], [200.0, -200.0], [2000.0, -2000.0]]
                    ],
                )
    ...
    def _testTensorArrayEvalEmpty(self):
        with self.session(), self.test_scope():
            ...
            with self.assertRaisesWithPredicateMatch(
                ...
                "Uninitialized TensorArray passed to "
    ...
    def testTensorArrayScatterReadAndGradients(self):
        with self.session() as session, self.test_scope():
            ...
            read_vals, grad_vals = session.run(
                xla.compile(fn), feed_dict={id0: 1, id1: 8}
            )
