...
class WhileTest(xla_test.XLATestCase):
    ...
    def testCountingLoopHandrolled(self):
        ...
        with self.session() as sess:
            ...
            with self.test_scope():
                loop_outputs = xla.while_loop(
                    [init_index, init_sum], loop_cond, loop_body
                )
...
def is_compile_on_demand():
    return (
        "TF_XLA_FLAGS" in os.environ
        and "tf_xla_compile_on_demand" in os.environ["TF_XLA_FLAGS"]
    )
