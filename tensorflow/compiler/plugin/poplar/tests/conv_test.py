...
class IpuXlaConvTest(test_util.TensorFlowTestCase):
    ...
    for fmt in self.data_formats:
      with ops.device("/device:IPU:0"):
        pa = array_ops.placeholder(
            np.float32, self._ip_shp([1, 1, 5, 1], fmt), name="a")
  ...
  def testConv3x3_Pad1x1(self):
    for fmt in self.data_formats:
      with ops.device("/device:IPU:0"):
        ...
        output = nn_ops.convolution(
            pa, pb, padding="SAME", data_format=fmt, name='cnv2')
  ...
  def testConv1x1_WithBias(self):
    for fmt in self.data_formats:
      with ops.device("/device:IPU:0"):
        inp = array_ops.placeholder(
            np.float32, self._ip_shp([1, 1, 1, 4], fmt), name="inp")
  ...
  def testDepthwiseConvBackpropInput(self):
    with ops.device("/device:IPU:0"):
      ...
      c = nn.depthwise_conv2d_native_backprop_input(
          pa, filt, outb, strides=[1, 1, 1, 1], padding="SAME")
