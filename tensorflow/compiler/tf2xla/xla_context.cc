...
StatusOr<int64_t> XlaContext::RecordCollectiveReduceV2OpInfo(int group_key,
                                                             int group_size) {
  if (!collective_reduce_info_) {
    collective_reduce_info_ = {group_key, group_size, 0};
  ...
  return (static_cast<int64_t>(group_key) << 32) |
         collective_reduce_info_->next_id++;
}
