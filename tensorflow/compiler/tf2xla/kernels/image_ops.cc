...
xla::XlaOp MapCoordReflect(xla::XlaBuilder* b, xla::XlaOp coord, int64_t len) {
  ...
  auto branch_b =
      xla::Sub(xla::Neg(coord_shifted_neg), xla::ConstantR0<float>(b, 1.0f));
  ...
  auto pos_result = xla::Select(ge_len,
  ...
  auto result =
      xla::Select(is_neg, neg_result, xla::Select(is_ge, pos_result, coord));
...
xla::XlaOp ApplyFillMode(xla::XlaBuilder* b, xla::XlaOp coord, int64_t len,
...
class ImageProjectiveTransformV2Op : public XlaOpKernel {
 public:
  explicit ImageProjectiveTransformV2Op(OpKernelConstruction* ctx)
    ...
    OP_REQUIRES(ctx,
                interpolation_ == "NEAREST" || interpolation_ == "BILINEAR",
                absl::InvalidArgumentError(
 ...
 protected:
  void CompileWithFillValue(XlaOpKernelContext* ctx, int fill_value_input) {
    // ---- Validate inputs ----
    ...
    if (fill_value_input >= 0) {
      OP_REQUIRES(ctx,
                  TensorShapeUtils::IsScalar(ctx->InputShape(fill_value_input)),
    ...
    OP_REQUIRES(
        ctx, out_size.size() == 2,
        absl::InvalidArgumentError(absl::StrCat(
            "output_shape must have 2 elements, got ", out_size.size())));
    ...
    xla::XlaOp out_y_2d =
        xla::BroadcastInDim(out_y, {out_height, out_width}, {0});
    xla::XlaOp out_x_2d =
        xla::BroadcastInDim(out_x, {out_height, out_width}, {1});
    ...
    xla::XlaOp ox =
        xla::BroadcastInDim(out_x_2d, {batch, out_height, out_width}, {1, 2});
    xla::XlaOp oy =
        xla::BroadcastInDim(out_y_2d, {batch, out_height, out_width}, {1, 2});

    // ---- Compute projective transform ----
    // projection = g*out_x + h*out_y + 1
    xla::XlaOp one = xla::ConstantR0<float>(b, 1.0f);
    xla::XlaOp projection =
        xla::Add(xla::Add(xla::Mul(g, ox), xla::Mul(h, oy)), one);

    // Guard against division by zero: where projection == 0, set to 1 and
    // mark those pixels as out-of-bounds later.
    xla::XlaOp proj_zero = xla::Eq(projection, xla::ConstantR0<float>(b, 0.0f));
    xla::XlaOp safe_proj =
        xla::Select(proj_zero, xla::Broadcast(one, {}), projection);

    // src_x = (a*out_x + b*out_y + c) / projection
    // src_y = (d*out_x + e*out_y + f) / projection
    xla::XlaOp src_x = xla::Div(
        xla::Add(xla::Add(xla::Mul(a, ox), xla::Mul(bb, oy)), c), safe_proj);
    xla::XlaOp src_y = xla::Div(
        xla::Add(xla::Add(xla::Mul(d, ox), xla::Mul(e, oy)), f), safe_proj);
    ...
    auto max_x = xla::ConstantR0<float>(b, static_cast<float>(in_width - 1));
    auto max_y = xla::ConstantR0<float>(b, static_cast<float>(in_height - 1));
    ...
    if (interpolation_ == "NEAREST") {
      ...
      xla::XlaOp batch_indices =
          xla::BroadcastInDim(batch_iota, {batch, out_height, out_width}, {0});
      xla::XlaOp batch_expanded =
          xla::Reshape(batch_indices, {batch, out_height, out_width, 1});
      xla::XlaOp iy_expanded =
          xla::Reshape(iy, {batch, out_height, out_width, 1});
      xla::XlaOp ix_expanded =
          xla::Reshape(ix, {batch, out_height, out_width, 1});
      xla::XlaOp indices =
          xla::ConcatInDim(b, {batch_expanded, iy_expanded, ix_expanded}, 3);
      ...
      result = xla::Gather(images, indices, dim_numbers, {1, 1, 1, channels},
      ...
      auto clamp_and_int = [&](xla::XlaOp v, float max_val) -> xla::XlaOp {
        return xla::ConvertElementType(
            xla::Clamp(fzero, v, xla::ConstantR0<float>(b, max_val)), xla::S32);
      };

      xla::XlaOp ix0 = clamp_and_int(floor_x, static_cast<float>(in_width - 1));
      xla::XlaOp ix1 = clamp_and_int(ceil_x, static_cast<float>(in_width - 1));
      xla::XlaOp iy0 =
          clamp_and_int(floor_y, static_cast<float>(in_height - 1));
      xla::XlaOp iy1 = clamp_and_int(ceil_y, static_cast<float>(in_height - 1));
      ...
      auto gather_at = [&](xla::XlaOp gy, xla::XlaOp gx) -> xla::XlaOp {
        ...
        xla::XlaOp batch_e =
            xla::Reshape(batch_indices, {batch, out_height, out_width, 1});
        xla::XlaOp gy_e = xla::Reshape(gy, {batch, out_height, out_width, 1});
        xla::XlaOp gx_e = xla::Reshape(gx, {batch, out_height, out_width, 1});
        ...
        return xla::Gather(images_f32, idx, dn, {1, 1, 1, channels},
                           /*indices_are_sorted=*/false);
      };
      ...
      if (fill_mode_ == "CONSTANT") {
        ...
        auto in_bounds = [&](xla::XlaOp y, xla::XlaOp x) -> xla::XlaOp {
          return xla::And(xla::And(xla::Ge(x, fzero), xla::Le(x, max_x)),
                          xla::And(xla::Ge(y, fzero), xla::Le(y, max_y)));
        };
        top_left =
            select_fill_for_invalid(top_left, in_bounds(floor_y, floor_x));
        top_right =
            select_fill_for_invalid(top_right, in_bounds(floor_y, ceil_x));
        bottom_left =
            select_fill_for_invalid(bottom_left, in_bounds(ceil_y, floor_x));
        bottom_right =
            select_fill_for_invalid(bottom_right, in_bounds(ceil_y, ceil_x));
      }
      ...
      xla::XlaOp one_minus_wx = xla::Sub(xla::ConstantR0<float>(b, 1.0f), wx_b);
      xla::XlaOp one_minus_wy = xla::Sub(xla::ConstantR0<float>(b, 1.0f), wy_b);

      // Bilinear interpolation.
      xla::XlaOp top =
          xla::Add(xla::Mul(top_left, one_minus_wx), xla::Mul(top_right, wx_b));
      xla::XlaOp bottom = xla::Add(xla::Mul(bottom_left, one_minus_wx),
                                   xla::Mul(bottom_right, wx_b));
      result = xla::Add(xla::Mul(top, one_minus_wy), xla::Mul(bottom, wy_b));
    }
    ...
    if (interpolation_ == "NEAREST" && original_type != xla::F32) {
      fill_for_result = xla::ConvertElementType(fill_for_result, original_type);
    }
    xla::XlaOp fill_bcast = xla::Broadcast(
        fill_for_result, {batch, out_height, out_width, channels});
...
REGISTER_XLA_OP(
    Name("ImageProjectiveTransformV2").CompileTimeConstantInput("output_shape"),
...
REGISTER_XLA_OP(
    Name("ImageProjectiveTransformV3").CompileTimeConstantInput("output_shape"),
