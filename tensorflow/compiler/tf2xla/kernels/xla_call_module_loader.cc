...
bool IsTokenType(mlir::Type type) {
  return isa<mlir::stablehlo::TokenType>(type) ||
         isa<mlir::mhlo::TokenType>(type);
}
...
absl::Status XlaCallModuleLoader::SetPlatformIndex(
  ...
  mlir::RankedTensorType arg_ranked_type =
      dyn_cast<mlir::RankedTensorType>(platform_index_arg.getType());
...
absl::Status XlaCallModuleLoader::RefineDynamicShapes(
  ...
  for (int i = 0, end = nr_inputs; i < end; ++i) {
    ...
    if (IsTokenType(arg_type)) {
      ...
      mlir::TensorType arg_type =
          dyn_cast<mlir::TensorType>(main_body.getArgument(i).getType());
      if (arg_type == nullptr) {
        return absl::InvalidArgumentError(absl::StrCat(
      ...
      if (auto ranked_arg_type = dyn_cast<mlir::RankedTensorType>(arg_type)) {
  ...
  for (auto i = 0; i < main_body.getNumArguments(); ++i) {
    ...
    auto ranked_arg_type = dyn_cast<mlir::RankedTensorType>(arg_type);
    if (!ranked_arg_type || !ranked_arg_type.hasStaticShape()) {
      auto type = cast<mlir::RankedTensorType>(static_array_input_types[i]);
      auto custom_call =
  ...
  for (auto call : llvm::make_early_inc_range(
    ...
    if (cast<mlir::StringAttr>(call->getAttr("call_target_name")).strref() ==
