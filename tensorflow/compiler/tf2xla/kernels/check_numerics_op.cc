...
class CheckNumericsOp : public XlaOpKernel {
 public:
  explicit CheckNumericsOp(OpKernelConstruction* ctx)
      : XlaOpKernel(ctx),
  ...
  void Compile(XlaOpKernelContext* ctx) override {
    ...
    absl::string_view device_type = device_type_string_;
    if (ctx->compiler() != nullptr &&
        !ctx->compiler()->options().device_type.type_string().empty()) {
      device_type = ctx->compiler()->options().device_type.type_string();
    }
    bool is_gpu = (device_type == "GPU" || device_type == "XLA_GPU" ||
