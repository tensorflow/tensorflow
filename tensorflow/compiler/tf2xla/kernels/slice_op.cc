      ...
      if (all_sizes_are_constant && !constant_size_is_minus_one) {
        ...
        for (int i = 0; i < input_dims; i++) {
          ...
          if (constant_size->AllValid()) {
            ...
            auto constant_bound = ctx->value_inference().AnalyzeConstant(
                dynamic_size, xla::ValueInferenceMode::kUpperBound);
            OP_REQUIRES_OK(ctx, constant_bound.status());
            if (constant_bound->AllValid()) {
              sliced = xla::SliceInDim(
                  sliced, 0, constant_bound->Get<int32>({}).value(), 1, i);
            }
