...
std::vector<DataType> ShuffleInputDataTypeAttribute(
  ...
  for (int i = 0, iter_limit = in_types.size(); i < iter_limit; i++) {
...
Status InputTypesNeedsRearrange(const std::vector<DataType>& in_types,
  ...
  *need_rewrite = false;
  for (int i = first_resource_index + 1, iter_limit = in_types.size(); i < iter_limit; i++) {
...
Status CalculateRetvalRearrange(
  ...
  for (int i = 0, iter_limit = ret_nodes.size(); i < iter_limit; i++) {
    Node* n = ret_nodes[i];
    DataType t;
...
Status MaybeRewriteWhileNode(
  ...
  *node_rewritten = true;
  ...
  for (auto const& attr_name : std::vector<string>{"cond", "body"}) {
    ...
    if (attr_name == "body") {
      for (int i = 0, iter_limit = fbody->ret_nodes.size(); i < iter_limit; i++) {
