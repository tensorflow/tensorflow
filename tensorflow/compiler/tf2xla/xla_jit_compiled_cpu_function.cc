...
XlaJitCompiledCpuFunction::Compile(
  ...
  jit->pjrt_client_ = std::move(client);
...
XlaJitCompiledCpuFunction::eigen_intra_op_thread_pool_device() const {
  auto* common_client =
      absl::down_cast<xla::CommonPjRtClient*>(pjrt_client_.get());
  auto* raw_client =
      absl::down_cast<xla::PjRtCpuRawClient*>(common_client->raw_client());
  return raw_client->eigen_intraop_device();
}
