...
Status RewriteAndPruneGraph(
  ...
  for (const Node* n : graph->op_nodes()) {
    if (n->type_string() == FunctionLibraryDefinition::kArgOp) {
      string feed_id;
      TF_RETURN_IF_ERROR(GetNodeAttribute(n->attrs(), kFeedIdAttr, &feed_id));
      ...
      string fetch_id;
      TF_RETURN_IF_ERROR(GetNodeAttribute(n->attrs(), kFetchIdAttr, &fetch_id));
...
Status CreateXlaArgs(const Graph& graph,
  ...
  for (const Node* node : arg_nodes) {
    ...
    TF_RETURN_IF_ERROR(GetNodeAttribute(node->attrs(), "T", &arg.type));
    TensorShape shape;
    TF_RETURN_IF_ERROR(GetNodeAttribute(node->attrs(), kShapeAttr, &shape));
    arg.shape = shape;
    TF_RETURN_IF_ERROR(
        GetNodeAttribute(node->attrs(), kDebugNameAttr, &arg.name));
