...
class Sharding(object):
    ...
    def split(cls, tensor, split_dimension, num_devices, input_shape=None):
        ...
        if shape[split_dimension] is not None and shape[split_dimension] < num_devices:
            raise ValueError(
                "Split dimension was smaller than the required number "
    ...
    def apply_to_tensor(
        ...
        tensor.op._set_attr(
            "_XlaSharding", attr_value_pb2.AttrValue(s=proto.SerializeToString())
        )
        return tensor
...
def split(
    ...
    """
    return Sharding.split(
        tensor, split_dimension, num_devices, input_shape
    ).apply_to_tensor(
        tensor,
        assign_tuple_sharding=assign_tuple_sharding,
        use_sharding_op=use_sharding_op,
...
def partial_tile(tensor, tile_assignment, use_sharding_op=False, unspecified_dims=None):
    ...
    return Sharding.partial_tile(tile_assignment).apply_to_tensor(
        tensor, use_sharding_op=use_sharding_op, unspecified_dims=unspecified_dims or []
    )
...
def auto_to_manual_spmd_partition(
    tensor, manual_sharding, single_dim=-1, unspecified_dims=None
):
...
def manual_to_auto_spmd_partition(
    tensor, manual_sharding, full_shape, single_dim=-1, unspecified_dims=None
):
...
def mesh_split(
    ...
    Raises:
      ValueError: The number of tensor split dimensions is larger than device mesh
        rank.
    """
    sharding = mesh_split_sharding(
        device_mesh, tensor_split_dims_mapping, manual_mesh_dims
    )
