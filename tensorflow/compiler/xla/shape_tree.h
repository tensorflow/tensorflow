...
class ShapeTree {
 public:
  ...
  struct IndexTableEntry {
    ...
    ssize_t children_start_id = -1;
  };

  using Nodes = absl::InlinedVector<Node, 1>;
  using IndexTable = absl::InlinedVector<IndexTableEntry, 1>;
  ...
  ShapeTree<U> Map(absl::FunctionRef<U(const T&)> func) {
    ...
    absl::c_transform(nodes_, std::back_inserter(result_nodes), func);

    ShapeTree<U> result(shape_, std::move(result_nodes), index_table_);
  ...
  void CopySubtreeFrom(const ShapeTree<T>& other, const ShapeIndex& src_index,
    ...
    std::transform(first, last, find(dst_index), [&](const Node& node) -> Node {
      DCHECK(ShapeIndexView(node.first).StartsWith(src_index));
      // For each shape index, replace the prefix `src_index` with `dst_index`.
      int64_t offset = static_cast<int64_t>(src_index.size());
      ShapeIndexView without_prefix{node.first, offset};
      ShapeIndex index;
      index.reserve(dst_index.size() + without_prefix.size());
      index.insert(index.end(), dst_index.begin(), dst_index.end());
      index.insert(index.end(), without_prefix.begin(), without_prefix.end());
      return {std::move(index), node.second};
    });
  }
  ...
  StatusOr<ShapeTree<T>> SubShapeTree(const ShapeIndex& index) const {
    ...
    for (auto it = find(index), end = it + count; it != end; ++it) {
      ...
      int64_t offset = static_cast<int64_t>(index.size());
      ShapeIndexView new_index{it->first, offset};
      sub_tree_nodes.push_back(Node{new_index, it->second});
    }
 ...
 private:
  ...
  ShapeTree(const Shape* shape, Nodes nodes, IndexTable index_table = {})
      : nodes_(std::move(nodes)),
        index_table_(std::move(index_table)),
        shape_(shape) {
    DCHECK_EQ(nodes_.size(), ShapeUtil::SubshapeCount(*shape));
    if (index_table_.empty()) {
      index_table_.reserve(nodes_.size());
      ComputeIndexTable(*shape);
    }
    DCHECK_EQ(nodes_.size(), index_table_.size());
  }
  ...
  void ComputeIndexTable(const Shape& shape) {
    // TODO(cjfj): Index table cache?.
    index_table_.resize(1);
    size_t next_node_id = 0;
    ComputeIndexTableEntry(index_table_[0], shape, next_node_id);
  }

  void ComputeIndexTableEntry(IndexTableEntry& entry, const Shape& shape,
                              size_t& next_node_id) {
    ...
    entry.children_start_id = index_table_.size();
    // Add entry for children first, before recursing, so they are consecutive.
    index_table_.resize(index_table_.size() + shape.tuple_shapes_size());
    for (size_t i = 0; i < shape.tuple_shapes_size(); ++i) {
      ComputeIndexTableEntry(index_table_[entry.children_start_id + i],
                             shape.tuple_shapes(i), next_node_id);
    }
  }

  // Returns the index table entry for the given shape index.
  const IndexTableEntry& Lookup(ShapeIndexView index) const {
    const IndexTableEntry* result = &index_table_.front();
    for (int64_t i : index) {
      CHECK_GE(result->children_start_id, 0);
      result = &index_table_[result->children_start_id + i];
    }
    return *result;
  }
...
class ShapeTree<T>::LeafIterator
 ...
 private:
  bool IsLeaf() const {
    // The empty tuple is considered to have no leaves.
