...
void PoolingWorkspaceCache::insert(
  ...
  if (it != cache.end()) {
    ...
    desc = &it->second;
    CHECK_EQ(hipStreamSynchronize(hip_stream), hipSuccess) << "Failed to sync hipStream";
...
void PoolingWorkspaceCache::trim(hipStream_t hip_stream) {
  ...
  while (true) {
    ...
    if (must_sync) CHECK_EQ(hipStreamSynchronize(hip_stream), hipSuccess) << "Failed to sync hipStream";
    must_sync = true;
