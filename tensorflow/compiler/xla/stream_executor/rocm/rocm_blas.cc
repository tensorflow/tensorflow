...
port::Status ROCMBlas::DoBlasGemmBatchedInternal(
  ...
  if constexpr (std::is_same<T, Eigen::bfloat16>::value){
    ok = DoBlasInternal(rocblas_func, stream, /* pointer_mode_host = */ true,
                        ...
                        n, k, GpuComplex(alpha_ptr), GpuMemory(a), rocblas_datatype_bf16_r, 
                        lda, batch_stride_a, GpuMemory(b), rocblas_datatype_bf16_r, ldb, 
                        batch_stride_b, GpuComplex(beta_ptr), GpuMemoryMutable(&c), 
                        rocblas_datatype_bf16_r, ldc, batch_stride_c, 
                        GpuMemoryMutable(&c), rocblas_datatype_bf16_r, 
                        ldc, batch_stride_c, batch_count, rocblas_datatype_f32_r, 
                        rocblas_gemm_algo_standard, 0, 0);    
  }else{
    ok = DoBlasInternal(rocblas_func, stream, /* pointer_mode_host = */ true,
                        ROCMBlasTranspose(transa), ROCMBlasTranspose(transb), m,
...
bool ROCMBlas::DoBlasGemmBatched(
    ...
    const absl::Span<DeviceMemory<Eigen::bfloat16> *const> &b_array, int ldb, float beta, 
    const absl::Span<DeviceMemory<Eigen::bfloat16> *const> &c_array, int ldc, 
    int batch_count, ScratchAllocator *scratch_allocator) {  
  blas_log("DoBlasGemmBatched");
  ...
  port::Status status = DoBlasGemmBatchedInternal(
    ...
    alpha_bf16, a_array, lda, b_array, ldb, beta_bf16, c_array, ldc, batch_count, 
    scratch_allocator);
  if (!status.ok()){
    LOG(ERROR) << status;
  }
