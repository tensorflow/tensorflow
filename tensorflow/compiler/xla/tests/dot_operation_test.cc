...
// enabled, because we want to move the transpose from the non-constant
// side to the constant side and then check if constant folding indeed
// happens and the transpose is folded.
class DotOperationTestNoConstantFolding : public DotOperationTest {
 public:
  DotOperationTestNoConstantFolding() {
    mutable_debug_options()->clear_xla_disable_hlo_passes();
  }
};

XLA_TEST_F(DotOperationTestNoConstantFolding,
           ReorderContractingDimsConstLHS_RL) {
...
XLA_TEST_F(DotOperationTestNoConstantFolding,
           ReorderContractingDimsConstLHS_RR) {
  Array3D<float> input_arr(2, 2, 3);
  Array2D<float> const_arr(2, 6);
  input_arr.FillRandom(1.f, 1.f);
  const_arr.FillRandom(1.f, 1.f);
...
XLA_TEST_F(DotOperationTestNoConstantFolding,
           ReorderContractingDimsConstRHS_LR) {
  Array3D<float> input_arr(2, 3, 2);
  Array2D<float> const_arr(2, 6);
  input_arr.FillRandom(1.f, 1.f);
  const_arr.FillRandom(1.f, 1.f);
...
XLA_TEST_F(DotOperationTestNoConstantFolding,
           ReorderContractingDimsConstRHS_MM) {
...
XLA_TEST_F(DotOperationTestNoConstantFolding,
           ReorderContractingDims_Multipass) {
