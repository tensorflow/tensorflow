...
StatusOr<std::unique_ptr<Literal>> ClientLibraryTestBase::ExecuteAndTransfer(
  ...
  // Build the computation, as a convenience.
  TF_ASSIGN_OR_RETURN(auto computation, builder->Build());
  return client_->ExecuteAndTransfer(
      computation, arguments, shape_with_output_layout, &compilation_options_);
}
...
string ClientLibraryTestBase::ExecuteToString(
  ...
  auto result = client_->ExecuteAndTransfer(
      computation, arguments, /*shape_with_output_layout=*/nullptr,
      &compilation_options_);
  if (!result.ok()) {
