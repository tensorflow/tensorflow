...
TEST_F(DynamismInferenceTest, VariadicReduce) {
  ...
  EXPECT_FALSE(
      ComputeDynamismLiteral(result, &b).ValueOrDie().Get<bool>({0, 0}));
  EXPECT_FALSE(
      ComputeDynamismLiteral(result, &b).ValueOrDie().Get<bool>({0, 1}));
  EXPECT_TRUE(
      ComputeDynamismLiteral(result, &b).ValueOrDie().Get<bool>({1, 0}));
  EXPECT_TRUE(
      ComputeDynamismLiteral(result, &b).ValueOrDie().Get<bool>({1, 1}));
}
