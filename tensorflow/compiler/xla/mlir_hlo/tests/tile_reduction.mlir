...
func.func @tile_reduction(%arg0 : tensor<1x?xf32>) -> tensor<1xf32> {
  ...
  // CHECK:      gml_st.parallel (%[[LANE:.*]]) = (%c0) to (%c32) step (%c1) distribution ("warp")
  // CHECK:        %[[INITVAL:.*]] = gml_st.materialize %[[PARTIAL]]
  // CHECK:        %[[PARTVAL:.*]] = gml_st.for (%[[COL:.*]]) = (%[[LANE]]) to (%[[RDIM]]) step (%c32)
  ...
  // CHECK-SAME:         ins(%[[INVAL]] : tensor<1x1xf32>) outs(%[[OUTVAL]] : tensor<1xf32>)
  // CHECK:          gml_st.set_yield %[[ACCVAL]] into %[[OUTVAL]]
  // CHECK:        gml_st.set_yield %[[PARTVAL]] into %[[PARTIAL]]
  // CHECK:      %[[EXPAND:.*]] = tensor.expand_shape %[[PARTIAL]]
  // CHECK:      %[[RESULT2:.*]] = linalg.generic
  // CHECK-SAME:     ins(%[[EXPAND]] : tensor<1x32xf32>) outs(%[[RESULT1]] : tensor<1xf32>)
