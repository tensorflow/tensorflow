...
/* static */ bool ShapeUtil::FillNewShape(PrimitiveType element_type,
                                          ...
                                          Shape* shape) {
  // 1. Sidha API se size lein taaki lookup table ki dependency khatam ho
  int64_t dense_shape_size = ByteSizeOfPrimitiveType(element_type);
  
  // Opaque, Tuple, ya Token types ke liye size 0 hota hai, jo array shape ke liye galat hai
  ...
  const int64_t ndims = dimensions.size();
  
  // Layout initialize 
  auto* minor_to_major = shape->mutable_layout()->mutable_minor_to_major();
  minor_to_major->reserve(ndims);
  ...
  for (int64_t i = 0; i < ndims; ++i) {
    ...
    // 2. Overflow check 
    int64_t new_size = MultiplyWithoutOverflow(dense_shape_size, d);
    if (new_size < 0 && d != 0) { 
      return false;
    }
    dense_shape_size = new_size;

    shape->add_dimensions(d);
    minor_to_major->insert(minor_to_major->begin(), i);
  }
...
/* static */ int64_t ShapeUtil::GetDimension(const Shape& shape,
  ...
  auto dim_or = GetDimensionNumber(shape, dimension_number);
  TF_CHECK_OK(dim_or.status());
  return shape.dimensions(dim_or.value());
}

/* static */ StatusOr<int64_t> ShapeUtil::GetDimensionNumber(const Shape& shape,
                                                   int64_t dimension_number) {
  if (dimension_number < 0) {
    dimension_number += shape.rank();
  }
  if (dimension_number < 0 || dimension_number >= shape.rank()) {
    return InvalidArgument("Dimension number %d out of range for shape %s",dimension_number, ShapeUtil::HumanString(shape));
  }
