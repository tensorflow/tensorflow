...
def generate_tiling_configs(
    ...
    num_warps: typing.List[int]) -> typing.Iterator[MatmulTiling]:
...
def _matmul_kernel(
    ...
    force_num_warps: tl.constexpr,
    force_num_stages: tl.constexpr,
):
...
def benchmark_matmul_tiling(
    ...
    repetitions: int,
    debug=False) -> typing.Optional[MatmulTiming]:
  ...
  reduction_grid = lambda META: (
      triton.cdiv(dims.M, 32), triton.cdiv(dims.N, 32), 1,),
  def run_matmul():
    ...
    if tiling.SPLIT_K != 1:
      ...
      _reduce_kernel[(triton.cdiv(dims.M * dims.N, 1024),)](scratchpad, c,
                     row_size=int(dims.M),
  ...
  for dim in ['M', 'N', 'K']:
    ...
    if next_pow2(dim_size) < tile_size:
      if debug:
        LOG.error("Tile %s larger than the dimension %s (%s)",
                  tile_size, dim, dim_size)
      return None
  ...
  with torch.cuda.stream(shared_stream):
    try:
      percentiles = triton.testing.do_bench(
          run_matmul, warmup=0, rep=repetitions, percentiles=(0.001, 0.1, 0.5, 0.9)
      )
...
def benchmark_matmul(
    dims: MatmulSize, pbar: tqdm.std.tqdm,
    shared_stream: torch.cuda.Stream,
  ...
  Args:
    ...
    pbar: a progress bar
    s: stream to execute benchmarks on
  ...
  with torch.cuda.stream(s):
    if dims.quantized_lhs:
      a = torch.randint(
          0, 128,
          (dims.M, dims.K), device='cuda', dtype=torch.int8)
    else:
      a = torch.randn(dims.M, dims.K, device='cuda', dtype=torch.bfloat16)
  ...
  for tiling in tilings:
    ...
    timing = benchmark_matmul_tiling(dims, tiling, s, shared_stream, a, b, c,
                                     scratchpad,
