      ...
      [&](const Shape& subshape, const ShapeIndex& shape_index) {
        if (subshape.IsArray()) {
          if (subshape.element_type() ==
            ...
            for (int64 i = 0, iter_limit = src.size(); i < iter_limit; ++i) {
...
/* static */ Literal LiteralUtil::CreateR1U8(absl::string_view value) {
  ...
  for (int i = 0, iter_limit = value.size(); i < iter_limit; ++i) {
...
/* static */ Literal LiteralUtil::ReshapeSlice(
  ...
  for (int64 i = 0, iter_limit = new_dimensions.size(); i < iter_limit; ++i) {
...
/* static */ Literal LiteralUtil::MakeTupleOwned(
  ...
  Literal literal(ShapeUtil::MakeTupleShape(element_shapes));
  for (int64 i = 0, iter_limit = elements.size(); i < iter_limit; ++i) {
    TF_CHECK_OK(
