...
StatusOr<Literal> TransferManager::TransferArrayFromDevice(
  ...
  Shape on_device_shape = HostShapeToDeviceShape(shape);
  Literal literal(on_device_shape);
  ...
  TF_RETURN_IF_ERROR(TransferLiteralFromDevice(stream, shaped_buffer, &literal,
                                               transfer_metadata));
  return (shape.layout() == on_device_shape.layout()) ? std::move(literal)
                                                      : literal.Relayout(shape);
}
