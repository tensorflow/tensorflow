...
int mkstemps_mode(char* pattern, int suffix_len, int mode) {
  static const char letters[] =
      "abcdefghijklmnopqrstuvwxyz"
      "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
      "0123456789";
  static constexpr int num_letters = 62;
  static constexpr int MAX_TEMP_FILE = 16384;
  ...
  ticks.QuadPart *= 1000;
  uint64_t sec = ticks.QuadPart / freq.QuadPart;
  ticks.QuadPart *= 1000;
  uint64_t usec = ticks.QuadPart / freq.QuadPart;

  uint64_t value = ((size_t)(usec << 16)) ^ sec ^ GetCurrentProcessId();
  char* tmpl = &pattern[len - 6 - suffix_len];
  int fd;

  // Replace pattern's XXXXXX characters with randomness.
  // Try MAX_TEMP_FILE different filenames.
  for (int count = 0; count < MAX_TEMP_FILE; ++count) {
    uint64_t v = value;
    // Fill in the random bits.
    tmpl[0] = letters[v % num_letters];
    v /= num_letters;
    tmpl[1] = letters[v % num_letters];
    v /= num_letters;
    tmpl[2] = letters[v % num_letters];
    v /= num_letters;
    tmpl[3] = letters[v % num_letters];
    v /= num_letters;
    tmpl[4] = letters[v % num_letters];
    v /= num_letters;
    tmpl[5] = letters[v % num_letters];
    v /= num_letters;

    fd = _open(pattern, O_CREAT | O_EXCL | O_RDWR, mode);
    if (fd >= 0) return fd;
...
int mkstemps(char* pattern, int suffix_len) {
  return mkstemps_mode(pattern, suffix_len, 0600);
}
