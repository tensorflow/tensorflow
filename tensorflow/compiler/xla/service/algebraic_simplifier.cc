...
AlgebraicSimplifierVisitor::OptimizeDotOfReorderContractingDims(
  ...
  for (int64 i = 1;
       i <= reshape->operand(0)->shape().rank() - reshape->shape().rank();
       ++i) {
  ...
  // a new dot with lhs contrating dims as:
  // new_lhs_contracting_dims[i] = transpose_dims[old_lhs_contracting_dims[i]]
  auto old_lhs_contracting_dims = lhs_contracting_dims;
  lhs_contracting_dims.Clear();
  for (auto dim : old_lhs_contracting_dims) {
    lhs_contracting_dims.Add(transpose_dims[dim]);
  }
  lhs = lhs->mutable_operand(0);
