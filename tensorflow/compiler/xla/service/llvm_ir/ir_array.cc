...
IrArray::Index IrArray::Index::SourceIndexOfReshape(
  ...
  if (std::get<0>(trivial_reshape)) {
    ...
    for (int64 i = 0, j = 0, k = 0, l = 0, 
         iter_limit = source_multidim_index.size(); i < iter_limit; ++i) {
...
IrArray::Index IrArray::Index::SourceIndexOfSlice(
  ...
  for (int i = 0, iter_limit = multidim_.size(); i < iter_limit; ++i) {
...
llvm::Value* IrArray::EmitArrayElementAddress(const IrArray::Index& index,
  ...
  for (int64 i = 0, iter_limit = LayoutUtil::MinorToMajor(shape_).size(); i < iter_limit; ++i) {
