...
bool AllowedInstructionForRowOptimization(const HloInstruction* instr) {
  bool ret = instr->IsElementwise() || instr->opcode() == HloOpcode::kParameter ||
      ...
      (instr->opcode() == HloOpcode::kBroadcast &&
       ...
       instr->dimensions().back() == (instr->shape().rank() - 1));
  if (!ret) {
    VLOG(3) << "Row vectorization not enabled due to this op: " << instr->ToString();
  }
  return ret;
}
