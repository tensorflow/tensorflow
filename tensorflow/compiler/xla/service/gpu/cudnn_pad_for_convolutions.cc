...
static StatusOr<bool> ResolvePadedShapesForTensorCore(
...
// maps multiple of 4
static StatusOr<bool> ResolvePadedShapesForIntegerConvolution(
    HloCustomCallInstruction* conv, std::vector<Shape>* new_input_shapes_ptr,
    Shape* new_result_shape_ptr) {
  TF_ASSIGN_OR_RETURN(auto kind, GetCudnnConvKind(conv));
...
StatusOr<bool> CudnnPadForConvolutions::Run(HloModule* module) {
  ...
  for (HloComputation* comp : module->MakeNonfusionComputations()) {
    for (HloCustomCallInstruction* conv : GetRelevantConvs(comp)) {
      TF_ASSIGN_OR_RETURN(
          bool local_changed,
          ResolveAndPad(conv, ResolvePadedShapesForIntegerConvolution));
    ...
    for (HloCustomCallInstruction* conv : GetRelevantConvs(comp)) {
      if (is_volta_or_later_) {
        TF_ASSIGN_OR_RETURN(
            bool local_changed,
            ResolveAndPad(conv, ResolvePadedShapesForTensorCore));
        changed |= local_changed;
      }
    }
  }
