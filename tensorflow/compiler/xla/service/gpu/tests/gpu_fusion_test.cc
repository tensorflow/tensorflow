...
TEST_F(GpuFusionTest, FusedBiggerThenThresholdButDoNotChangeTheFusionl) {
  ...
  HloInstruction* input = b.AddInstruction(
      HloInstruction::CreateParameter(0, input_shape, "p"));

  std::vector<HloInstruction*> slice_params;
  for (int64 i = 0; i < kNumParams; ++i) {
    slice_params.push_back(b.AddInstruction(
        HloInstruction::CreateSlice(slice_shape, input, {0,0}, {10,2}, {1,1})));
  }
  auto root = b.AddInstruction(HloInstruction::CreateConcatenate(
