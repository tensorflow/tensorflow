...
TEST_P(ParameterizedFp8GemmRewriteTest,
  ...
  const char* hlo_text = R"(
    ...
    ENTRY test {
      ...
      b = f32[4,15,31] parameter(2)
      x_f32 = f32[4,15,15] convert(x)
      y_f32 = f32[15,31] convert(y)
      x_scale = f32[] parameter(3)
      y_scale = f32[] parameter(4)
      x_scale_bcast = f32[4,15,15] broadcast(x_scale), dimensions={}
      y_scale_bcast = f32[15,31] broadcast(y_scale), dimensions={}
      x_unscaled = f32[4,15,15] multiply(x_f32, x_scale_bcast)
      x_unscaled_bitcast = f32[60,15] bitcast(x_unscaled)
      y_unscaled = f32[15,31] multiply(y_f32, y_scale_bcast)
      dot_a = f32[60,31] dot(x_unscaled_bitcast, y_unscaled), lhs_contracting_dims={1}, rhs_contracting_dims={0}
      dot_a_bitcast = f32[4,15,31]{2,1,0} bitcast(dot_a)
      ROOT out = f32[4,15,31] add(dot_a_bitcast, b)
          }
)";
  ...
  RunAndFilecheckHloRewrite(hlo_text,
                            GemmRewriter(se::CudaComputeCapability{
                                se::CudaComputeCapability::HOPPER, 0}),
                            R"(
; CHECK-LABEL: ENTRY %test (x: f8e4m3fn[4,15,15], y: f8e4m3fn[15,31], b: f32[4,15,31], x_scale: f32[], y_scale: f32[]) -> f32[4,15,31] {
...
; CHECK:           }"
; CHECK:         ROOT [[OUT:%[^ ]+]] = f32[4,16,32]{2,1,0} bitcast([[GEMM]])
; CHECK:         %slice = f32[60,31]{1,0} slice(%cublas-gemm.2), slice={[0:60], [0:31]}
      )");
}
