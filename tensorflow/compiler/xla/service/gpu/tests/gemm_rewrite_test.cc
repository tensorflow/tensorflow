...
TEST_F(CublasLtF8GemmRewriteTest, ScaledABUnscaledDF8NN) {
  if (!GetCudaComputeCapability().IsAtLeast(
          se::CudaComputeCapability::HOPPER)) {
    GTEST_SKIP()
        << "cuBLASLt FP8 kernels require Hopper or newer architecture.";
...
TEST_F(CublasLtF8GemmRewriteTest, ScaledABUnscaledDF8TN) {
  if (!GetCudaComputeCapability().IsAtLeast(
          se::CudaComputeCapability::HOPPER)) {
    GTEST_SKIP()
        << "cuBLASLt FP8 kernels require Hopper or newer architecture.";
  ...
  const char* hlo_text = R"(
    ...
    ENTRY test {
      x = f8e4m3fn[128,64]{1,0} parameter(0)
      x_f32 = f32[128,64]{1,0} convert(x)
      x_scale = f32[] parameter(2)
      x_scale_bcast = f32[128,64] broadcast(x_scale), dimensions={}
      x_unscaled = f32[128,64] multiply(x_f32, x_scale_bcast)
      y = f8e4m3fn[128,32]{1,0} parameter(1)
      y_f32 = f32[128,32]{1,0} convert(y)
      y_scale = f32[] parameter(3)
      y_scale_bcast = f32[128,32] broadcast(y_scale), dimensions={}
      y_unscaled = f32[128,32] multiply(y_f32, y_scale_bcast)
      ROOT out = f32[64,32] dot(x_unscaled, y_unscaled), lhs_contracting_dims={0}, rhs_contracting_dims={0}
    }
...
; CHECK-LABEL: ENTRY %test (x: f8e4m3fn[128,64], y: f8e4m3fn[128,32], x_scale: f32[], y_scale: f32[]) -> f32[64,32] {
; CHECK-NEXT:    [[P0:%[^ ]+]] = f8e4m3fn[128,64]{1,0} parameter(0)
; CHECK-NEXT:    [[P_TRANSPOSE4:%[^ ]+]] = f8e4m3fn[64,128]{1,0} transpose([[P0]]), dimensions={1,0}
; CHECK-NEXT:    [[P1:%[^ ]+]] = f8e4m3fn[128,32]{1,0} parameter(1)
; CHECK-NEXT:    [[P_TRANSPOSE5:%[^ ]+]] = f8e4m3fn[32,128]{1,0} transpose([[P1]]), dimensions={1,0}
; CHECK-NEXT:    [[C2:%[^ ]+]] = f32[] constant(0)
; CHECK-NEXT:    [[BCAST1:%[^ ]+]] = f32[64,32]{1,0} broadcast([[C2]]), dimensions={}
; CHECK-NEXT:    [[P2:%[^ ]+]] = f32[] parameter(2)
; CHECK-NEXT:    [[P3:%[^ ]+]] = f32[] parameter(3)
; CHECK-NEXT:    [[C3:%[^ ]+]] = f32[] constant(1)
; CHECK-NEXT:    ROOT [[OUT:%[^ ]+]] = f32[64,32]{1,0} custom-call([[P_TRANSPOSE4]], [[P_TRANSPOSE5]], [[BCAST1]], [[P2]], [[P3]], /*index=5*/[[C3]], [[C3]]),
; CHECK:           custom_call_target="__cublas$lt$matmul$f8",
; CHECK:           backend_config="{
; CHECK-DAG:         \"alpha_real\":1
; CHECK-DAG:         \"alpha_imag\":0
...
TEST_F(CublasLtF8GemmRewriteTest, ScaledABUnscaledDF8NT) {
  if (!GetCudaComputeCapability().IsAtLeast(
          se::CudaComputeCapability::HOPPER)) {
    GTEST_SKIP()
        << "cuBLASLt FP8 kernels require Hopper or newer architecture.";
  ...
  const char* hlo_text = R"(
    ...
    ENTRY test {
      ...
      x_scale = f32[] parameter(2)
      x_scale_bcast = f32[64,128] broadcast(x_scale), dimensions={}
      x_unscaled = f32[64,128] multiply(x_f32, x_scale_bcast)
      y = f8e4m3fn[32,128]{1,0} parameter(1)
      y_f32 = f32[32,128]{1,0} convert(y)
      y_scale = f32[] parameter(3)
      y_scale_bcast = f32[32,128] broadcast(y_scale), dimensions={}
      y_unscaled = f32[32,128] multiply(y_f32, y_scale_bcast)
      ROOT out = f32[64,32] dot(x_unscaled, y_unscaled), lhs_contracting_dims={1}, rhs_contracting_dims={1}
    }
...
; CHECK-LABEL: ENTRY %test (x: f8e4m3fn[64,128], y: f8e4m3fn[32,128], x_scale: f32[], y_scale: f32[]) -> f32[64,32] {
; CHECK-NEXT:    [[P0:%[^ ]+]] = f8e4m3fn[64,128]{1,0} parameter(0)
; CHECK-NEXT:    [[P1:%[^ ]+]] = f8e4m3fn[32,128]{1,0} parameter(1)
; CHECK-NEXT:    [[C2:%[^ ]+]] = f32[] constant(0)
; CHECK-NEXT:    [[BCAST1:%[^ ]+]] = f32[64,32]{1,0} broadcast([[C2]]), dimensions={}
; CHECK-NEXT:    [[P2:%[^ ]+]] = f32[] parameter(2)
; CHECK-NEXT:    [[P3:%[^ ]+]] = f32[] parameter(3)
; CHECK-NEXT:    [[C3:%[^ ]+]] = f32[] constant(1)
; CHECK-NEXT:    ROOT [[OUT:%[^ ]+]] = f32[64,32]{1,0} custom-call([[P0]], [[P1]], [[BCAST1]], [[P2]], [[P3]], /*index=5*/[[C3]], [[C3]]),
; CHECK:           custom_call_target="__cublas$lt$matmul$f8",
; CHECK:           backend_config="{
; CHECK-DAG:         \"alpha_real\":1
; CHECK-DAG:         \"alpha_imag\":0
...
TEST_F(CublasLtF8GemmRewriteTest, ScaledABUnscaledDF8RowMajorNT) {
  if (!GetCudaComputeCapability().IsAtLeast(
          se::CudaComputeCapability::HOPPER)) {
    GTEST_SKIP()
        << "cuBLASLt FP8 kernels require Hopper or newer architecture.";
  ...
  const char* hlo_text = R"(
    ...
    ENTRY test {
      x = f8e4m3fn[64,16]{0,1} parameter(0)
      x_f32 = f32[64,16]{0,1} convert(x)
      x_scale = f32[] parameter(2)
      x_scale_bcast = f32[64,16] broadcast(x_scale), dimensions={}
      x_unscaled = f32[64,16] multiply(x_f32, x_scale_bcast)
...
; CHECK-LABEL: ENTRY %test (x: f8e4m3fn[64,16], y: f8e4m3fn[32,16], x_scale: f32[], y_scale: f32[]) -> f32[64,32] {
; CHECK-NEXT:    [[P0:%[^ ]+]] = f8e4m3fn[64,16]{0,1} parameter(0)
; CHECK-NEXT:    [[BTCT2:%[^ ]+]] = f8e4m3fn[16,64]{1,0} bitcast([[P0]])
; CHECK-NEXT:    [[P1:%[^ ]+]] = f8e4m3fn[32,16]{0,1} parameter(1)
; CHECK-NEXT:    [[BTCT3:%[^ ]+]] = f8e4m3fn[16,32]{1,0} bitcast([[P1]])
; CHECK-NEXT:    [[C2:%[^ ]+]] = f32[] constant(0)
; CHECK-NEXT:    [[BCAST1:%[^ ]+]] = f32[64,32]{1,0} broadcast([[C2]]), dimensions={}
; CHECK-NEXT:    [[P2:%[^ ]+]] = f32[] parameter(2)
; CHECK-NEXT:    [[P3:%[^ ]+]] = f32[] parameter(3)
; CHECK-NEXT:    [[C3:%[^ ]+]] = f32[] constant(1)
; CHECK-NEXT:    ROOT [[OUT:%[^ ]+]] = f32[64,32]{1,0} custom-call([[BTCT2]], [[BTCT3]], [[BCAST1]], [[P2]], [[P3]], /*index=5*/[[C3]], [[C3]]),
; CHECK:           custom_call_target="__cublas$lt$matmul$f8",
; CHECK:           backend_config="{
; CHECK-DAG:         \"alpha_real\":1
; CHECK-DAG:         \"alpha_imag\":0
