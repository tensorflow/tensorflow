...
void VlogIfFailureToMatch(
    HloInstruction* instr, const LogPattern& log_pattern,
    absl::string_view desc, const FilterPattern& filter_pattern,
...
StatusOr<bool> FuseRelu(HloComputation* comp) {
  ...
  for (HloInstruction* instr : comp->MakeInstructionPostOrder()) {
    ...
    if (!Match(instr, m::MaximumAnyOrder(
                          ...
                          m::GetTupleElement(&gte, m::Op(&conv).WithPredicate(
                                                       IsConvCustomCall))))) {
      continue;
    }
