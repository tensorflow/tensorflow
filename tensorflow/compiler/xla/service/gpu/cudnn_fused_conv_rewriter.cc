...
// Describes a matched pattern:
//   convert(clamp(broadcast(-128), (get_tuple_element(custom_call(x,w, ...)), broadcast(127));
// where the custom_call calls cudnn convolution (either pure convolution or
// fused convlution) with integer inputs.
...
absl::optional<ConvWithConvert> FindConvWithConvert(HloInstruction* instr) {
  ...
  if (Match(instr, pattern)) {
    if (primitive_util::IsIntegralType(
      ...
      HloCustomCallInstruction* conv = CastOrNull<HloCustomCallInstruction>(conv_instr);
...
Status TryRewriteToCudnnForwardConvert(
    ConvWithConvert match) {
  auto conv = match.conv;
  auto gte = match.gte;
  ...
  // Remove clamp/convert and so on and just keep get_tuple_element(custom_call(x,w, ...))
