...
template <typename T>
Status RunCudnnConvImpl(CudnnConvParams params, CudnnConvDescriptors descs,
  ...
  switch (kind) {
    case CudnnConvKind::kForward:
      ...
      stream->ThenConvolveWithAlgorithm(descs.input, input_buf, descs.filter,
                                        filter_buf, descs.conv, descs.output,
                                        &output_buf, scratch_allocator,
    ...
    case CudnnConvKind::kBackwardInput:
      ...
      stream->ThenConvolveBackwardDataWithAlgorithm(
          descs.filter, filter_buf, descs.output, output_buf, descs.conv,
          descs.input, &input_buf, scratch_allocator, algorithm,
          profile_result);
      break;
    case CudnnConvKind::kBackwardFilter:
      ...
      stream->ThenConvolveBackwardFilterWithAlgorithm(
          descs.input, input_buf, descs.output, output_buf, descs.conv,
          descs.filter, &filter_buf, scratch_allocator, algorithm,
          profile_result);
      break;
    case CudnnConvKind::kForwardActivation: {
      ...
      bias_desc.set_count(1)
          ...
          .set_layout(descs.output.layout());
      ...
      stream->ThenFusedConvolveWithAlgorithm(
          descs.input, input_buf, params.conv_result_scale, descs.filter,
          filter_buf, descs.conv, side_input, params.fusion->side_input_scale,
          bias_desc, DeviceMemory<T>(params.fusion->bias_buf),
          params.fusion->mode, descs.output, &output_buf, scratch_allocator,
          algorithm, profile_result);
      break;
    }
  }
...
// Returns the cudnn convolution descriptors generated from the given
// convolution parameters
StatusOr<CudnnConvDescriptors> GetCudnnConvDescriptors(
    const HloCustomCallInstruction* conv,

    const CudnnConvParams& params) {
  const int num_dimensions = params.window->dimensions_size();
  ...
  const bool dims_reversed = params.window->dimensions()[0].window_reversal();

  CHECK_EQ(num_dimensions, params.dnums->input_spatial_dimensions_size());
  CHECK_EQ(num_dimensions, params.dnums->kernel_spatial_dimensions_size());
  CHECK_EQ(num_dimensions, params.dnums->output_spatial_dimensions_size());
  for (const WindowDimension& dim : params.window->dimensions()) {
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      XlaConvLayoutsToStreamExecutorLayouts(
          *params.dnums, params.input_shape->layout(),
          params.filter_shape->layout(), params.output_shape->layout()));

  BatchDescriptor input_descriptor(effective_num_dimensions);
  input_descriptor.set_layout(input_dl)
      .set_feature_map_count(params.input_shape->dimensions(
          params.dnums->input_feature_dimension()))
      .set_count(params.input_shape->dimensions(
          params.dnums->input_batch_dimension()));
  for (int dim = 0; dim < num_dimensions; ++dim) {
    ...
    input_descriptor.set_spatial_dim(
        ...
        params.input_shape->dimensions(
            params.dnums->input_spatial_dimensions(dim)));
  }

  FilterDescriptor filter_descriptor(effective_num_dimensions);
  filter_descriptor.set_layout(filter_dl)
      .set_input_feature_map_count(params.filter_shape->dimensions(
          params.dnums->kernel_input_feature_dimension()))
      .set_output_feature_map_count(params.filter_shape->dimensions(
          params.dnums->kernel_output_feature_dimension()));
  for (int dim = 0; dim < num_dimensions; ++dim) {
    filter_descriptor.set_spatial_dim(
        static_cast<DimIndex>(effective_num_dimensions - dim - 1),
        params.filter_shape->dimensions(
            params.dnums->kernel_spatial_dimensions(dim)));
  }

  ConvolutionDescriptor convolution_descriptor(effective_num_dimensions);
  convolution_descriptor.set_group_count(params.feature_group_count);
  convolution_descriptor.set_convolution_not_crosscorr(dims_reversed);
  for (int dim = 0; dim < num_dimensions; ++dim) {
    convolution_descriptor
        .set_zero_padding(
            static_cast<DimIndex>(effective_num_dimensions - dim - 1),
            params.window->dimensions(dim).padding_low())
        .set_filter_stride(
            static_cast<DimIndex>(effective_num_dimensions - dim - 1),
            params.window->dimensions(dim).stride());
  }

  BatchDescriptor output_descriptor(effective_num_dimensions);
  output_descriptor.set_layout(output_dl)
      .set_feature_map_count(params.output_shape->dimensions(
          params.dnums->output_feature_dimension()))
      .set_count(params.output_shape->dimensions(
          params.dnums->output_batch_dimension()));
  for (int dim = 0; dim < num_dimensions; ++dim) {
    output_descriptor.set_spatial_dim(
        static_cast<DimIndex>(effective_num_dimensions - dim - 1),
        params.output_shape->dimensions(
            params.dnums->output_spatial_dimensions(dim)));
  }
  ...
  descriptors.input = input_descriptor;
  descriptors.filter = filter_descriptor;
  descriptors.conv = convolution_descriptor;
  descriptors.output = output_descriptor;

  PrimitiveType output_primitive_type =
      conv->shape().tuple_shapes(0).element_type();
  switch (output_primitive_type) {
    case F16:
      descriptors.element_type = se::dnn::ToDataType<Eigen::half>::value;
      break;
    case F32:
      descriptors.element_type = se::dnn::ToDataType<float>::value;
      break;
    case F64:
      descriptors.element_type = se::dnn::ToDataType<double>::value;
      break;
    default:
      LOG(FATAL)
          << "failure to determine dnn::DataType from convolution datatype";
      break;
  }
...
StatusOr<CudnnConvDescriptors> GetCudnnConvDescriptors(
  ...
  TF_ASSIGN_OR_RETURN(CudnnConvDescriptors descs,
                      GetCudnnConvDescriptors(conv, params));
...
Status RunCudnnConv(const HloCustomCallInstruction* conv,
  ...
  switch (output_primitive_type) {
    case F16:
      return RunCudnnConvImpl<Eigen::half>(params, descs, scratch_allocator,
                                           stream, options.profile_result);
    case F32:
      return RunCudnnConvImpl<float>(params, descs, scratch_allocator, stream,
                                     options.profile_result);
    case F64:
      return RunCudnnConvImpl<double>(params, descs, scratch_allocator, stream,
                                      options.profile_result);
    default:
