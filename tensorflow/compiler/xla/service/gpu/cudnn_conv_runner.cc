...
Status RunCudnnConvImpl(const CudnnConvParams& params,
  ...
  Status run_status = RunCudnnConvInternalImpl<ElementType, BiasType, OutputType>(
  ...
  if (!stream->ok()) {
    if(algorithm.algorithm_no_scratch().has_value()) {
    return InternalError(
    ...
    return InternalError(
        "Unable to launch convolution with type %s and algorithm %d",
        CudnnConvKindToString(params.kind), algorithm.algorithm()->algo_id());
    }
  }
  return Status::OK();
...
Status RunCudnnConv(const HloCustomCallInstruction* conv,
  ...
  PrimitiveType input_primitive_type =
      conv->operand(0)->shape().element_type();
  switch (input_primitive_type) {
    case F16:
      return RunCudnnConvImpl<Eigen::half, Eigen::half, Eigen::half>(params, scratch_allocator, stream,
                                           options);
    case F32:
      return RunCudnnConvImpl<float, float, float>(params, scratch_allocator, stream,
                                     options);
    case F64:
