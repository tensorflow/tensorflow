...
std::vector<uint8> EmitModuleToHsaco(Module* module,
                                     llvm::TargetMachine* target_machine,
                                     int amdgpu_version) {
  ...
  std::string inject_isabin_path =
      tensorflow::io::JoinPath(".", isabin_filename);
...
tensorflow::Status LinkROCDLIfNecessary(llvm::Module* module,
                                        int amdgpu_version,
  ...
  for (auto& rocdl_bitcode : rocdl_bitcode_vector) {
    string rocdl_path = tensorflow::io::JoinPath(rocdl_dir_path, rocdl_bitcode);
...
StatusOr<std::vector<uint8>> CompileModuleToHsaco(
  ...
  return std::move(
      EmitModuleToHsaco(module, target_machine.get(), amdgpu_version));
}
...
void GPUBackendInit(const HloModuleConfig& hlo_module_config) {
  ...
  // its specific initialization functions instead of the catch-all InitializeAll*.
...
StatusOr<std::vector<uint8>> CompileToHsaco(
  ...
  std::vector<uint8> hsaco;
  {
    tensorflow::profiler::TraceMe activity(
        [&] {
          return absl::StrCat("Compiling IR",
