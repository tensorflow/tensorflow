...
class GpuExecutable : public Executable {
 public:
  // binary (i.e. the compiled kernel) may be empty, in which case we leave
  // compilation up to the GPU driver.
  GpuExecutable(const string& asm_text, const std::vector<uint8>& binary,
  ...
  // Returns the compiled kernel text for the computation.
  tensorflow::StringPiece asm_text() const { return asm_text_; }

  // Returns the compiled kernel binary stored in this GpuExecutable.  May be
 ...
 private:
  ...
  // The compilex kernel text for the computation.
  const string asm_text_;
  ...
  se::DeviceVersion device_hardware_version_;
