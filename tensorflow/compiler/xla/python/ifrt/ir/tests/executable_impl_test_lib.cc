...
module {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      ...
      client_->GetDefaultCompiler()->Compile(
          xla::ifrt::XlaProgram(*mhlo_module),
...
module {
  ...
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<LoadedExecutable> loaded_exec,
                          client_->GetDefaultCompiler()->Compile(
                              IfrtIRProgram(*mlir_module), std::move(options)));
