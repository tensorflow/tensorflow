...
// A scoped log sink for absl::logging. For testing only.
class TestingScopedLogSink
    : public absl::LogSink,
      public std::enable_shared_from_this<TestingScopedLogSink> {
 public:
  TestingScopedLogSink() : active_(false) {}

  ~TestingScopedLogSink() override {
    if (active_) {
      absl::RemoveLogSink(this);
    }
  }

  // Enables the log sink as entering the context.
  std::shared_ptr<TestingScopedLogSink> Enter() {
    if (!active_) {
      active_ = true;
  ...
  // Enables the log sink as exiting the context.
  bool Exit(const py::object& type, const py::object& value,
    ...
    // Do not shallow any exception.
    return false;
  }

  // Returns logs that have been captured so far.
 ...
 private:
  // Implements absl::LogSink::Send to capture log messages.
  void Send(const absl::LogEntry& entry) override {
    absl::MutexLock lock(&mu_);
    logs_.push_back(std::string(entry.text_message()));
  }

  bool active_;
  mutable absl::Mutex mu_;
  std::vector<std::string> logs_ ABSL_GUARDED_BY(mu_);
};
...
void BuildTransferGuardSubmodule(py::module& m) {
  ...
  py::class_<TestingScopedLogSink, std::shared_ptr<TestingScopedLogSink>>
      scoped_log_sink(tglib, "_TestingScopedLogSink");
  scoped_log_sink.def(py::init<>());
  scoped_log_sink.def("__enter__", &TestingScopedLogSink::Enter);
  scoped_log_sink.def("__exit__", &TestingScopedLogSink::Exit);
  scoped_log_sink.def("logs", &TestingScopedLogSink::logs);
}
