...
Status PrintTopLabels(const std::vector<Tensor>& outputs,
	const string& labels_file_name, string& highest) {
...
INIT_API int init_main_interface()
{
	...
	int argc = 1;
	char** argv = new char*;
	tensorflow::port::InitMain("Test Main", &argc, &argv);
	if (argc > 1) {
...
const char* main_interface(const float* source_data) {
	// These are the command-line flags the program can understand.
	// They define where the graph and input data is located, and what kind of
	// input the model expects. If you train your own model, or use something
	// other than inception_v3, then you'll need to update these.
	string image = "beach.jpg";
	string graph =
		"mobilenet.pb";
	string labels =
		"mobilenet.txt";
	...
	for (int y = 0; y < 224; ++y) {
		const float* source_row = source_data + (y * 224 * 3);
		for (int x = 0; x < 224; ++x) {
	...
	Status print_status = PrintTopLabels(outputs, labels, output_labels);
	if (!print_status.ok()) {
		LOG(ERROR) << "Running print failed: " << print_status;
		return {};
	}
	char * cstr = new char[output_labels.length() + 1];
	std::strcpy(cstr, output_labels.c_str());
	//const char* test = output_labels.c_str();
	//std::cout << test << std::endl;
	//std::cout << cstr << std::endl;
	return cstr;
}

void main_interface2(const float* source_data, char** cstr, int* a) {
	// These are the command-line flags the program can understand.
	// They define where the graph and input data is located, and what kind of
	// input the model expects. If you train your own model, or use something
	// other than inception_v3, then you'll need to update these.
	...
	*cstr = new char[output_labels.length() + 1];
	std::strcpy(*cstr, output_labels.c_str());
	//*cstr = "Hallo";
	*a = 10;
	//const char* test = output_labels.c_str();
