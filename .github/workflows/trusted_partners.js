...
const filter_action = async ({github, context, domain}) => {
  ...
  if (lowercased_title.includes("onednn"))
  ...
  if (lowercased_title.includes('nvidia') &&
      domain.includes('nvidia.com')) {
    if (lowercased_title.includes('jax')) {
      assignees.push('hawkinsp', 'yashk2810', 'skye');
    }
    if (lowercased_title.includes('xla') ||
        lowercased_title.includes('gpu')) {
  ...
  if (lowercased_title.includes('linaro') &&
