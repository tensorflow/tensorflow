...
if [[ ${VERSION} == "python3.14" ]]; then
  # Build python 3.14.0a6 from source now so that we can create an archive for
  # hermetic python build. This is a temporary solution until the astral python
  # standalone build repo release supports for 3.14.0b1.
