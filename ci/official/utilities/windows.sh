...
# Some applications on Windows don't understand Linux-like paths, so
# `cygpath -m` is used to retrieve the original Windows path from a given MSYS
# path.
function msys_linux_to_win_path () {
  cygpath -m "$1"
}

function msys_linux_to_win_docker_path () {
  msys_linux_to_win_path "$1" | replace_drive_letter_with_c
}
