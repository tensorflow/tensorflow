...
BAZEL_COMMAND_RE = re.compile(r"(^| )(?P<command>bazel (.*? )?(?P<type>test|build) .+)")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--xml-out-path",
        required=False,
        help="Path to which to output the JUnit-based XML with ResultStore links.",
    )
    parser.add_argument(
        "--print",
        action="store_true",
    ...
    for url, lines in result_store_links.items():
        ...
        while k > start_line:
            ...
            if build_failed or tests_failed:
                ...
                lines["status"] = (
                    InvokeStatus.build_failed
                    if build_failed
                    else InvokeStatus.tests_failed
                )
                if verbose:
                    print(
                        f"Found failed invocation: {url.rsplit('/')[-1]}\n"
                        f"Log fragment:\n"
                        f"```\n{log_fragment}\n```\n"
                        f"{'=' * 140}"
                    )
        ...
        # invocation.
        bazel_comm_min_line_i = (
            previous_end_line if previous_end_line is not None else 0
        )
...
def create_xml_file(
    result_store_dict: ResultDictType, output_path: str, verbose: bool = False
):
    ...
    for url, invocation_results in result_store_dict.items():
        ...
        if status in (InvokeStatus.tests_failed, InvokeStatus.build_failed):
            ...
            failure = ElemTree.SubElement(
                testcase, elem_name, message=f"Bazel invocation {invocation_id} failed."
            )
            ...
            ElemTree.SubElement(
                properties, "property", name="description", value=success_msg
            )
            if command:
                ElemTree.SubElement(
                    properties, "property", name="bazel_command", value=command
                )

        suite_specific = {
            "tests": str(len(result_store_dict)),
...
def print_invocation_results(result_store_dict: ResultDictType):
    ...
    if not result_store_dict:
        ...
        print(
            f"Found {len(result_store_dict)} ResultStore link(s) for "
            f"Bazel invocations.\n"
    ...
    for url, invocation_results in result_store_dict.items():
        ...
        if command:
            ...
            line_str += (
                "Couldn't parse the bazel command, check inside the build log instead"
            )
        line_str += f"\n{url}\n"
        print(line_str)
        i += 1
