...
tfrun bazel run $TFCI_BAZEL_COMMON_ARGS --config=cuda_wheel //tensorflow/tools/pip_package:rename_wheel $TFCI_BUILD_PIP_PACKAGE_ARGS -- --output-path=$TFCI_OUTPUT_DIR
tfrun bazel run $TFCI_BAZEL_COMMON_ARGS --config=cuda_wheel //tensorflow/tools/pip_package:rename_wheel $TFCI_BUILD_PIP_PACKAGE_ARGS -- --output-path=dist
